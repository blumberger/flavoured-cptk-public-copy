# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_methods.F"
!-----------------------------------------------------------------------------!
...
# 91 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_methods.F" 2
...
! code
    ...
    ELSE
       ...
       CALL cp_dbcsr_allocate_matrix_set(p_env%p1,n_spins)
       DO spin=1, n_spins
          ALLOCATE(p_env%p1(spin)%matrix)
          CALL cp_dbcsr_init(p_env%p1(spin)%matrix)
          ...
          CALL cp_dbcsr_set(p_env%p1(spin)%matrix,0.0_dp)
       END DO
    END IF
...
! *****************************************************************************
  SUBROUTINE p_env_check_i_alloc(p_env, qs_env)
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",335)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",336)
...
! *****************************************************************************
  SUBROUTINE p_env_psi0_changed(p_env, qs_env, psi0, Hrho_psi0d)
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",412)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",413)
    ...
    ELSE
       ...
       DO spin=1,n_spins
          ...
          IF (lfomo>nmo) THEN
             CALL cp_dbcsr_set(rho_ao(spin)%matrix,0.0_dp)
             ...
             CALL cp_dbcsr_scale(rho_ao(spin)%matrix,alpha_scalar=maxocc)
          ELSE
             CALL cp__b("qs_p_env_methods.F",512,"symmetrized onesided smearing to do")
          END IF
       END DO
...
! code
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",619)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",620)
    NULLIFY(dft_control)
    CALL get_qs_env(qs_env, dft_control=dft_control)
...
! code
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",677)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",678)
    IF(.NOT.(0<spin))CALL cp__a("qs_p_env_methods.F",679)
    IF(.NOT.(spin<=dft_control%nspins))CALL cp__a("qs_p_env_methods.F",680)

    CALL mpools_get(qs_env%mpools, maxao_maxmo_fm_pool=maxao_maxmo_fm_pool)
    CALL fm_pool_create_fm(maxao_maxmo_fm_pool, tmp)
...
! *****************************************************************************
  SUBROUTINE p_op_l2(p_env, qs_env, p1, res, alpha, beta)
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",760)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",761)
    ...
    DO ispin=1, SIZE(p1)
       ...
       IF(.not.ASSOCIATED(rho1_ao(ispin)%matrix,p1(ispin)%matrix)) THEN
          CALL cp_dbcsr_copy(rho1_ao(ispin)%matrix,p1(ispin)%matrix)
       ENDIF
    ENDDO
...
! code
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",834)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",835)
    ...
    IF(.NOT.(SIZE(v)>=n_spins))CALL cp__a("qs_p_env_methods.F",842)
    ! alloc tmp storage
    ...
    DO spin=1, n_spins
       ...
       IF(.NOT.(v_rows>=p_env%n_ao(spin)))CALL cp__a("qs_p_env_methods.F",866)
       cols = v_cols
       IF (PRESENT(n_cols)) THEN
          IF(.NOT.(n_cols(spin)<=cols))CALL cp__a("qs_p_env_methods.F",869)
          cols=n_cols(spin)
       END IF
       IF(.NOT.(cols<=max_cols))CALL cp__a("qs_p_env_methods.F",872)
...
! code
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_methods.F",936)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_methods.F",937)
    ...
    maxmo_maxmo_fmstruct => fm_pool_get_el_struct(maxmo_maxmo_fm_pool)
    CALL cp_fm_struct_get(maxmo_maxmo_fmstruct, nrow_global=nmo2,ncol_global=maxnmo)
    IF(.NOT.(SIZE(v)>=n_spins))CALL cp__a("qs_p_env_methods.F",944)
    ! alloc tmp storage
    IF (PRESENT(n_cols)) THEN
       max_cols=MAXVAL(n_cols(1:n_spins))
    ELSE
    ...
    DO spin=1, n_spins
       ...
       CALL cp_fm_get_info(v(spin)%matrix, &
            nrow_global=v_rows, ncol_global=v_cols)
       IF(.NOT.(v_rows>=p_env%n_ao(spin)))CALL cp__a("qs_p_env_methods.F",968)
       cols = v_cols
       IF (PRESENT(n_cols)) THEN
          IF(.NOT.(n_cols(spin)<=cols))CALL cp__a("qs_p_env_methods.F",971)
          cols=n_cols(spin)
       END IF
       IF(.NOT.(cols<=max_cols))CALL cp__a("qs_p_env_methods.F",974)
