# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_messages.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_messages.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_messages.F" 2
...
!> \retval master return value, logical
...
! *****************************************************************************
  FUNCTION check_if_group_master(para_env) RESULT(master)
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_messages.F",96)
...
! *****************************************************************************
  SUBROUTINE tmc_message(msg_type, send_recv, dest, para_env, tmc_params, &
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_messages.F",143)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",144)
    ...
    IF(act_send_recv.EQV.send_msg) THEN
      ...
      CASE(TMC_STAT_ANALYSIS_REQUEST)
        IF(.NOT.(PRESENT(list_elem)))CALL cp__a("tmc/tmc_messages.F",199)
        CALL create_analysis_request_message(list_elem, m_send, tmc_params)
      CASE DEFAULT
        CALL cp__b("tmc/tmc_messages.F",202,"try to send unknown message type "//cp_to_string(msg_type))
      END SELECT
      !set message info
    ...
    IF((act_send_recv.EQV.send_msg).AND.(dest.NE.bcast_group)) THEN
      ...
      IF(m_send%info(4).GT.0) THEN
        CALL cp__b("tmc/tmc_messages.F",223,"")
    ...
    IF(dest.EQ.bcast_group) THEN
      IF(para_env%num_pe.GT.1) THEN
        ...
        IF(m_send%info(4).GT.0) THEN
          ...
          CALL cp__b("tmc/tmc_messages.F",253,"")
    ...
    IF((act_send_recv.EQV.recv_msg).AND.dest.NE.bcast_group) THEN
      ...
      IF(m_send%info(4).GT.0) THEN
        ...
        CALL cp__b("tmc/tmc_messages.F",322,"")
    ...
    IF(act_send_recv.EQV.recv_msg) THEN
      ...
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_messages.F",380),&
...
! *****************************************************************************
  ...
  SUBROUTINE create_status_message(m_send)
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",406)

    ! nothing to do, send just the message tag

    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",410)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",411)
...
! *****************************************************************************
  SUBROUTINE create_worker_init_message(tmc_params, m_send)
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",515)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",516)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",517)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",518)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_char)))CALL cp__a("tmc/tmc_messages.F",519)
    IF(.NOT.(ASSOCIATED(tmc_params%cell)))CALL cp__a("tmc/tmc_messages.F",520)
    ...
    IF(.NOT.(counter.EQ.SIZE(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",534)
    ...
    counter = counter + 1+INT(m_send%task_real(counter))
    m_send%task_real(counter) = REAL(message_end_flag, KIND=dp) !message end
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",546)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",547)
...
! *****************************************************************************
  SUBROUTINE read_worker_init_message(tmc_params, m_send)
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",566)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",567)
    IF(.NOT.(m_send%info(3).GE.4))CALL cp__a("tmc/tmc_messages.F",568)

    IF(.NOT.ASSOCIATED(tmc_params%cell)) ALLOCATE(tmc_params%cell)
    counter = 1
    !int array 
    flag = INT(m_send%task_int(1)) .EQ.SIZE(tmc_params%cell%perd)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_messages.F",574)
    ...
    counter = counter +3
    IF(.NOT.(counter.EQ.m_send%info(2)))CALL cp__a("tmc/tmc_messages.F",581)
    IF(.NOT.(m_send%task_int(counter).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",582)

    !float array with cell vectors
    counter = 1
    flag = INT(m_send%task_real(counter)).EQ.SIZE(tmc_params%cell%hmat)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_messages.F",587)
    tmc_params%cell%hmat = &
      RESHAPE(m_send%task_real(counter+1:counter+&
        SIZE(tmc_params%cell%hmat)),(/3,3/) )
    counter = counter + 1+INT(m_send%task_real(counter))

    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",593)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",594)
...
! *****************************************************************************
  SUBROUTINE create_start_conf_message(msg_type, elem, result_count, &
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",622)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",623)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",624)
    IF(.NOT.(ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_messages.F",625)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",626)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",627)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_char)))CALL cp__a("tmc/tmc_messages.F",628)

    counter = 1
    msg_size_int = 1+SIZE(tmc_params%cell%perd) +1+1 +1 +1+SIZE(elem%mol) +1
    IF(msg_type.EQ.TMC_STAT_INIT_ANALYSIS) THEN
      IF(.NOT.(PRESENT(result_count)))CALL cp__a("tmc/tmc_messages.F",633)
      IF(.NOT.(ASSOCIATED(result_count)))CALL cp__a("tmc/tmc_messages.F",634)
    ...
    END IF
    m_send%task_int(counter) = message_end_flag
    IF(.NOT.(counter.EQ.SIZE(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",656)
    ...
    counter = counter + 1+INT(m_send%task_real(counter))
    m_send%task_real(counter) = REAL(message_end_flag, KIND=dp) !message end
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",677)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",678)
...
! *****************************************************************************
  SUBROUTINE read_start_conf_message(msg_type, elem, result_count, m_send, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",706)
    IF(.NOT.(.NOT.ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_messages.F",707)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",708)
    IF(.NOT.(.NOT.ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",709)
    IF(.NOT.(m_send%info(3).GE.4))CALL cp__a("tmc/tmc_messages.F",710)
    ...
    !int array 
    flag = INT(m_send%task_int(1)) .EQ.SIZE(tmc_params%cell%perd)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_messages.F",718)
    counter = 1+ m_send%task_int(1)+1
    tmc_params%cell%perd = m_send%task_int(2:counter-1)
    tmc_params%cell%symmetry_id = m_send%task_int(counter+1)
    tmc_params%cell%orthorhombic = .FALSE.
    ...
    counter = counter +1+m_send%task_int(counter)
    IF(msg_type.EQ.TMC_STAT_INIT_ANALYSIS) THEN
      IF(.NOT.(PRESENT(result_count)))CALL cp__a("tmc/tmc_messages.F",728)
      IF(.NOT.(.NOT.ASSOCIATED(result_count)))CALL cp__a("tmc/tmc_messages.F",729)
      ALLOCATE(result_count(m_send%task_int(counter)))
      result_count(:) = m_send%task_int(counter+1:counter+m_send%task_int(counter))
      counter = counter +1+m_send%task_int(counter)
    END IF
    IF(.NOT.(counter.EQ.m_send%info(2)))CALL cp__a("tmc/tmc_messages.F",734)
    IF(.NOT.(m_send%task_int(counter).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",735)
    ...
    flag = INT(m_send%task_real(counter)).EQ.SIZE(tmc_params%cell%hmat)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_messages.F",742)
    tmc_params%cell%hmat = &
      RESHAPE(m_send%task_real(counter+1:counter+&
        SIZE(tmc_params%cell%hmat)),(/3,3/) )
    counter = counter + 1+INT(m_send%task_real(counter))
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",755)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",756)
...
! *****************************************************************************
  SUBROUTINE create_energy_request_message(elem, m_send, &
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",783)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",784)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",785)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",786)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",787)
    ...
    counter = counter + 1+m_send%task_int(counter)
    m_send%task_int(counter) = message_end_flag
    IF(.NOT.(SIZE(m_send%task_int).EQ.msg_size_int))CALL cp__a("tmc/tmc_messages.F",801)
    IF(.NOT.(m_send%task_int(msg_size_int).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",802)
    ...
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",818)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",819)
...
! *****************************************************************************
  SUBROUTINE read_energy_request_message(elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",839)
    IF(.NOT.(m_send%info(3).GT.0))CALL cp__a("tmc/tmc_messages.F",840)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",841)
    IF(.NOT.(.NOT.ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",842)
    ...
    IF(.NOT.(m_send%info(2).GT.0))CALL cp__a("tmc/tmc_messages.F",850)
    counter = 1
    elem%sub_tree_nr = m_send%task_int(counter+1)
    counter = counter + 1+m_send%task_int(counter)
    elem%nr = m_send%task_int(counter+1)
    counter = counter + 1+m_send%task_int(counter)
    IF(.NOT.(m_send%task_int(counter).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",856)
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",868)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",869)
...
! *****************************************************************************
  SUBROUTINE create_energy_result_message(elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",890)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",891)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",892)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",893)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",894)
    ...
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",935)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",936)
...
! *****************************************************************************
  SUBROUTINE read_energy_result_message(elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",956)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",957)
    IF(.NOT.(m_send%info(3).GT.0))CALL cp__a("tmc/tmc_messages.F",958)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",959)
    ...
    IF(DEBUG.GT.0)THEN 
       ...
       IF(elem%sub_tree_nr .NE. m_send%task_int(counter+1) .OR. &
          ...
          CALL cp__b("tmc/tmc_messages.F",970,"read_energy_result: got energy result from unexpected tree element.")
       END IF
    ELSE
      IF(.NOT.(m_send%info(2).EQ.0))CALL cp__a("tmc/tmc_messages.F",973)
    END IF
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",988)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",989)
...
! *****************************************************************************
  SUBROUTINE create_approx_energy_result_message(elem, m_send, &
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1011)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",1012)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",1013)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1014)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1015)
    ...
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",1035)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1036)
...
! *****************************************************************************
  SUBROUTINE read_approx_energy_result(elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1056)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1057)
    IF(.NOT.(m_send%info(2).EQ.0.AND.m_send%info(3).GT.0))CALL cp__a("tmc/tmc_messages.F",1058)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1059)
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",1069)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1070)
...
! *****************************************************************************
  SUBROUTINE create_NMC_request_massage(msg_type, elem, m_send, &
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1097)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1098)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",1099)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",1100)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1101)
    ...
    IF(.NOT.(SIZE(m_send%task_int).EQ.msg_size_int))CALL cp__a("tmc/tmc_messages.F",1161)
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",1162)
    IF(.NOT.(m_send%task_int(msg_size_int).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1163)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1164)
...
! *****************************************************************************
  SUBROUTINE read_NMC_request_massage(msg_type, elem, m_send, &
    ...
    IF(.NOT.(.NOT.ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1188)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1189)
    IF(.NOT.(m_send%info(2).GT.5.AND.m_send%info(3).GT.8))CALL cp__a("tmc/tmc_messages.F",1190)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1191)
    ...
    counter = counter +2
    IF(.NOT.(counter.EQ.m_send%info(2)))CALL cp__a("tmc/tmc_messages.F",1215)
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",1240)
    IF(.NOT.(m_send%task_int(m_send%info(2)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1241)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1242)
...
! *****************************************************************************
  SUBROUTINE create_NMC_result_massage(msg_type, elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1268)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",1269)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",1270)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1271)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1272)
    ...
    IF(.NOT.(SIZE(m_send%task_int).EQ.msg_size_int))CALL cp__a("tmc/tmc_messages.F",1372)
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",1373)
    IF(.NOT.(m_send%task_int(msg_size_int).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1374)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1375)
...
! *****************************************************************************
  SUBROUTINE read_NMC_result_massage(msg_type, elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1402)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1403)
    IF(.NOT.(m_send%info(2).GT.0.AND.m_send%info(3).GT.0))CALL cp__a("tmc/tmc_messages.F",1404)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1405)
    ...
    IF(DEBUG.GT.0) THEN
       IF((m_send%task_int(counter+1) .NE. elem%sub_tree_nr) .AND. (m_send%task_int(counter+3).NE.elem%nr)) THEN
          CALL cp__b("tmc/tmc_messages.F",1411,"read_NMC_result_massage: got result of wrong element")
       ENDIF
       counter = counter + 2+2
    END IF
    ! the molecule information
    ...
    IF(.NOT.ANY(tmc_params%sub_box_size.LE.0.1_dp)) THEN
       ...
       counter = counter +1 +m_send%task_int(counter)   
    END IF
    IF(.NOT.(counter.EQ.m_send%info(2)))CALL cp__a("tmc/tmc_messages.F",1447)
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",1486)
    IF(.NOT.(m_send%task_int(m_send%info(2)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1487)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1488)
...
! *****************************************************************************
  SUBROUTINE create_analysis_request_message(list_elem, m_send, &
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1516)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_int)))CALL cp__a("tmc/tmc_messages.F",1517)
    IF(.NOT.(.NOT.ALLOCATED(m_send%task_real)))CALL cp__a("tmc/tmc_messages.F",1518)
    IF(.NOT.(ASSOCIATED(list_elem)))CALL cp__a("tmc/tmc_messages.F",1519)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1520)
    ...
    counter = counter + 1+m_send%task_int(counter)
    m_send%task_int(counter) = message_end_flag
    IF(.NOT.(SIZE(m_send%task_int).EQ.msg_size_int))CALL cp__a("tmc/tmc_messages.F",1534)
    IF(.NOT.(m_send%task_int(msg_size_int).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1535)
    ...
    IF(.NOT.(SIZE(m_send%task_real).EQ.msg_size_real))CALL cp__a("tmc/tmc_messages.F",1551)
    IF(.NOT.(INT(m_send%task_real(msg_size_real)).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1552)
...
! *****************************************************************************
  SUBROUTINE read_analysis_request_message(elem, m_send, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1573)
    IF(.NOT.(m_send%info(3).GT.0))CALL cp__a("tmc/tmc_messages.F",1574)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1575)
    IF(.NOT.(.NOT.ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1576)
    ...
    END IF
    ! read the integer values
    IF(.NOT.(m_send%info(2).GT.0))CALL cp__a("tmc/tmc_messages.F",1584)
    counter = 1
    elem%sub_tree_nr = m_send%task_int(counter+1)
    counter = counter + 1+m_send%task_int(counter)
    elem%nr = m_send%task_int(counter+1)
    counter = counter + 1+m_send%task_int(counter)
    IF(.NOT.(m_send%task_int(counter).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1590)
    ...
    IF(.NOT.(counter.EQ.m_send%info(3)))CALL cp__a("tmc/tmc_messages.F",1602)
    IF(.NOT.(INT(m_send%task_real(m_send%info(3))).EQ.message_end_flag))CALL cp__a("tmc/tmc_messages.F",1603)
...
! *****************************************************************************
  SUBROUTINE read_scf_step_ener(elem, m_send)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_messages.F",1622)
    IF(.NOT.(ASSOCIATED(m_send)))CALL cp__a("tmc/tmc_messages.F",1623)
...
! *****************************************************************************
  SUBROUTINE communicate_atom_types(atoms, source, para_env)
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_messages.F",1655)
    IF(.NOT.(source.GE.0))CALL cp__a("tmc/tmc_messages.F",1656)
    IF(.NOT.(source.LT.para_env%num_pe))CALL cp__a("tmc/tmc_messages.F",1657)
...
!    INTEGER, DIMENSION(MPI_STATUS_SIZE)      :: status_single
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_messages.F",1701)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_messages.F",1702)
    ...
    IF(PRESENT(worker_info)) THEN
      IF(.NOT.(ASSOCIATED(worker_info)))CALL cp__a("tmc/tmc_messages.F",1710)
    ...
    IF(para_env%mepos.EQ.MASTER_COMM_ID)THEN
      wait_for_receipts: DO
        ...
        CASE(TMC_CANCELING_RECEIPT)
          IF(PRESENT(worker_info)) THEN
            ...
            CALL tmc_message(msg_type=stat, send_recv=send_msg, dest=dest_rank,&
                             para_env=para_env, tmc_params=tmc_params)
          ELSE
            CALL cp__b("tmc/tmc_messages.F",1760,"group master should not receive cancel receipt")
          END IF
        ...
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_messages.F",1768),&
    ...
    ELSE
      CALL cp__b("tmc/tmc_messages.F",1775,"only (group) master should stop other participants")
    END IF
