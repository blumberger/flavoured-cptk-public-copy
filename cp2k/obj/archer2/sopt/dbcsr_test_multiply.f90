# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_multiply.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_multiply.F"
!-----------------------------------------------------------------------------!
...
# 61 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_multiply.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_test_multiplies(test_name, mp_group, mp_env, npdims, io_unit, &
    ...
    INTEGER                                  :: a_c, a_r, a_tr, b_c, b_r, &
                                                ...
                                                isymm, itype, mynode, &
                                                numnodes, TYPE
    INTEGER, DIMENSION(:), POINTER           :: my_sizes_k, my_sizes_m, &
          ...
          DO b_tr = 1,SIZE(trans)
             ...
             ELSE
                CALL dbcsr_assert(.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
                     routineN, " something wrong here... ", 260)
             END IF
...
! *****************************************************************************
  SUBROUTINE test_multiply (test_name, mp_group, mp_env, npdims, io_unit,&
    ...
    INTEGER                                  :: c_a, c_b, c_c, handle, r_a, &
                                                r_b, r_c
...
!   ---------------------------------------------------------------------------
    ...
    IF (ALL(limits==0)) THEN
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, " limits shouldnt be 0 ",494)
    ELSE
...
! *****************************************************************************
  SUBROUTINE dbcsr_check_multiply (test_name, matrix_c, dense_c_dbcsr, dense_a, dense_b, dense_c,&
    ...
    INTEGER :: a_col, a_m, a_n, a_row, b_col, b_m, b_n, b_row, c_col, &
      c_col_size, c_row, c_row_size, handle, i, istat, j, k, lda, ldb, ldc, &
      lwork, m, mynode, n, numnodes
    ...
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",615)
    CALL dbcsr_data_get_sizes (dense_c, ldc, i, valid)
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",618)
    CALL dbcsr_data_get_sizes (dense_a, lda, i, valid)
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",621)
    CALL dbcsr_data_get_sizes (dense_b, ldb, i, valid)
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",624)
    !
    !
    ...
    CASE (dbcsr_type_real_8_2d)
       ...
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocation problem",664)
       eps          = dlamch('eps')
    ...
    CASE (dbcsr_type_real_4_2d)
       ...
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocation problem",688)
    ...
    CASE (dbcsr_type_complex_8_2d)
       ALLOCATE(work(lwork), STAT=istat)
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocation problem",746)
       eps          = dlamch('eps')
    ...
    CASE (dbcsr_type_complex_4_2d)
       ALLOCATE(work_sp(lwork), STAT=istat)
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocation problem",770)
       eps          = REAL(slamch('eps'),real_8)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect or 1-D data type", 793)
    END SELECT
