# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_env_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_env_types.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_env_types.F" 2
...
! *****************************************************************************
  SUBROUTINE fist_nonbond_env_set(fist_nonbond_env, potparm14, potparm, &
       ...
       shift_cutoff, r_last_update, r_last_update_pbc, rshell_last_update_pbc, &
       rcore_last_update_pbc, cell_last_update, num_update, last_update, &
       counter, natom_types, long_range_correction, eam_data, quip_data, charges)
  ...
  SUBROUTINE init_fist_nonbond_env(fist_nonbond_env, atomic_kind_set, &
    ...
    IF (do_nonbonded) THEN
       ...
       IF (use_potparm14) THEN
          ...
          IF(.NOT.(check))CALL cp__a("fist_nonbond_env_types.F",384)
       END IF
       IF (use_potparm) THEN
          check = (SIZE(potparm%pot,1)==natom_types)
          IF(.NOT.(check))CALL cp__a("fist_nonbond_env_types.F",388)
       END IF
...
!> \author Fawzi Mohamed
! *****************************************************************************
  SUBROUTINE fist_nonbond_env_retain(fist_nonbond_env)
    TYPE(fist_nonbond_env_type), POINTER     :: fist_nonbond_env

    CHARACTER(len=*), PARAMETER :: routineN = 'fist_nonbond_env_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(fist_nonbond_env)))CALL cp__a("fist_nonbond_env_types.F",465)
    IF(.NOT.(fist_nonbond_env%ref_count>0))CALL cp__a("fist_nonbond_env_types.F",466)
...
! *****************************************************************************
  SUBROUTINE fist_nonbond_env_release(fist_nonbond_env)
    ...
    IF (ASSOCIATED(fist_nonbond_env)) THEN
       IF(.NOT.(fist_nonbond_env%ref_count>0))CALL cp__a("fist_nonbond_env_types.F",484)
       fist_nonbond_env%ref_count=fist_nonbond_env%ref_count-1
