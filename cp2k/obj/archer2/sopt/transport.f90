# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/transport.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/transport.F"
!-----------------------------------------------------------------------------!
...
# 61 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/transport.F" 2
...
! *****************************************************************************
  SUBROUTINE transport_env_create(qs_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(transport_env)))CALL cp__a("transport.F",111)

    ALLOCATE(transport_env)
    CALL transport_init_read_input(input,transport_env)
    CALL transport_init_contact_params(qs_env, transport_env)
...
! read from input
    ...
    IF (contact_explicit) THEN
       ...
       DO i = 1, n_contacts
          ...
          CALL section_vals_val_get(contact_section,"N_ATOMS",i_rep_section=i,&
    ...
    ELSE
       ...
       ELSE
          CALL cp__b("transport.F",200,"No contact region is defined.")
       END IF
    END IF
...
! *****************************************************************************
   SUBROUTINE external_scf_method(transport_env, matrix_s, matrix_ks, matrix_p, &
    ...
    IF(.NOT.C_ASSOCIATED(transport_env%ext_c_method_ptr))&
       CALL cp_abort(cp__l("transport.F",324),&
...
! csr to csr_interop
    ...
    IF (csr_mat%nze_total>HUGE(csr_interop_mat%nze_total)) THEN
        CALL cp__b("transport.F",431,"overflow in nze")
    ENDIF
...
!> \author Mohammad Hossein Bani-Hashemian
! *****************************************************************************
  SUBROUTINE transport_init_contact_params(qs_env, transport_env)
    TYPE(qs_environment_type), POINTER       :: qs_env
    TYPE(transport_env_type), INTENT(INOUT)  :: transport_env

    CHARACTER(len=*), PARAMETER :: &
      routineN = 'transport_init_contact_params', &
      routineP = moduleN//':'//routineN
  ...
  END SUBROUTINE transport_init_contact_params
