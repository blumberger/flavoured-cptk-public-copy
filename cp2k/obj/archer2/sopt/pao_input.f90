# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_input.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_input.F" 2
...
! *****************************************************************************
  SUBROUTINE parse_pao_section(pao, input)
    ...
    IF(pao%penalty_dist<0.0_dp) CALL cp__b("pao_input.F",83,"PAO: PENALTY_DISTANCE < 0")
    ...
    IF(pao%penalty_strength<0.0_dp) CALL cp__b("pao_input.F",87,"PAO: PENALTY_STRENGTH < 0")
    ...
    IF(pao%cg_init_steps<1) CALL cp__b("pao_input.F",91,"PAO: CG_INIT_STEPS < 1")
    ...
    IF(pao%num_grad_eps<0.0_dp) CALL cp__b("pao_input.F",104,"PAO: NUM_GRADIENT_EPS < 0")
  ...
  FUNCTION id2str(id) RESULT(s)
    ...
    SELECT CASE(id)
      ...
      CASE(pao_exp_param)
        s = "EXP"
      CASE DEFAULT
        CALL cp__b("pao_input.F",131,"PAO: unkown id")
    END SELECT
    s = ADJUSTR(s)
  END FUNCTION id2str
...
! *****************************************************************************
  SUBROUTINE create_pao_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("pao_input.F",152)
