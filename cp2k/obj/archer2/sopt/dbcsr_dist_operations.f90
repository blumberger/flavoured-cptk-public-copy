# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_dist_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_dist_operations.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_dist_operations
  ...
  USE dbcsr_dist_methods,              ONLY: &
       ...
       dbcsr_distribution_num_local_row_clusters, &
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/../../base/base_uses.f90" 1
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_dist_operations.F" 2
...
!>                            block resides
! *****************************************************************************
  PURE SUBROUTINE get_stored_coordinates_type(matrix, row, column,processor)
...
!>                            block resides
! *****************************************************************************
  SUBROUTINE get_stored_coordinates_obj(matrix, row, column, processor)
...
!   ---------------------------------------------------------------------------
    ...
    IF (debug_mod) THEN
       CALL dbcsr_assert (dbcsr_is_initialized(matrix),&
            ...
            "matrix%m not associated",260)
    ENDIF
    IF (PRESENT (processor)) THEN
       CALL dbcsr_get_stored_coordinates (matrix%m, row, column, processor)
    ELSE
       CALL dbcsr_get_stored_coordinates (matrix%m, row, column)
    ENDIF
...
! *****************************************************************************
  SUBROUTINE dbcsr_transpose_dims(dist_tr, dist_normal)
    ...
    INTEGER                                  :: i, j, nclusters, ncols_tr, &
                                                npcols_tr, nprows_tr, &
                                                nrows_tr, vgcd, vlcm
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: img_data, reorder
    INTEGER, DIMENSION(:), POINTER           :: col_dist_data_tr, &
                                                col_img_data_tr, &
                                                row_dist_data_tr, &
                                                row_img_data_tr
...
!   ---------------------------------------------------------------------------
    ...
    IF (dbcsr_distribution_has_col_clusters(dist_normal)) THEN
       nclusters = npcols_tr*dbcsr_distribution_num_local_col_clusters (dist_normal)
       ALLOCATE(img_data(nrows_tr))
       img_data(:) = MOD(dbcsr_distribution_col_clusters (dist_normal)-1,&
            dbcsr_distribution_num_local_col_clusters (dist_normal))
       CALL rebin_imaged_distribution_clusters (row_dist_data_tr, row_img_data_tr,&
            dbcsr_distribution_col_dist (dist_normal),&
            img_data,&
            nprows_tr,&
            nclusters/gcd(nclusters,npcols_tr),&
            nclusters)
       ! Sort clusters for convenience
       ALLOCATE(reorder(SIZE(row_img_data_tr)))
       CALL sort(row_img_data_tr,SIZE(row_img_data_tr),reorder)
       ! Realign blocks distribution
       DO i = 1, SIZE(reorder)
          j = reorder(i)
          IF (i.GE.j) CYCLE
          CALL swap(reorder(i),reorder(j))
          CALL swap(row_dist_data_tr(i),row_dist_data_tr(j))
       ENDDO
       DEALLOCATE(img_data,reorder)
    ELSE
    ...
    IF (dbcsr_distribution_has_row_clusters(dist_normal)) THEN
       nclusters = nprows_tr*dbcsr_distribution_num_local_row_clusters (dist_normal)
       ALLOCATE(img_data(ncols_tr))
       img_data(:) = MOD(dbcsr_distribution_row_clusters (dist_normal)-1,&
            dbcsr_distribution_num_local_row_clusters (dist_normal))
       CALL rebin_imaged_distribution_clusters (col_dist_data_tr, col_img_data_tr,&
            dbcsr_distribution_row_dist (dist_normal),&
            img_data,&
            npcols_tr,&
            nclusters/gcd(nclusters,nprows_tr),&
            nclusters)
       ! Sort clusters for convenience
       ALLOCATE(reorder(SIZE(col_img_data_tr)))
       CALL sort(col_img_data_tr,SIZE(col_img_data_tr),reorder)
       ! Realign blocks distribution
       DO i = 1, SIZE(reorder)
          j = reorder(i)
          IF (i.GE.j) CYCLE
          CALL swap(reorder(i),reorder(j))
          CALL swap(col_dist_data_tr(i),col_dist_data_tr(j))
       ENDDO
       DEALLOCATE(img_data,reorder)
    ELSE
    ...
    CALL dbcsr_distribution_new(dist_tr, mp_env, &
         ...
         row_dist_cluster=row_img_data_tr, col_dist_cluster=col_img_data_tr, &
         reuse_arrays=.TRUE.)
...
!   ---------------------------------------------------------------------------
    ...
    IF(MOD(nbins*nimages,multiplicity).NE.0)&
       CALL cp__w("dbcsr/dist/dbcsr_dist_operations.F",652,"mulitplicity is not divisor of new process grid coordinate")
...
!> \param nclusters ...
! *****************************************************************************
  SUBROUTINE rebin_imaged_distribution_clusters (new_bins, new_images,&
       source_bins, source_images, nbins, multiplicity, nclusters)
    INTEGER, DIMENSION(:), INTENT(OUT)       :: new_bins, new_images
    INTEGER, DIMENSION(:), INTENT(IN)        :: source_bins, source_images
    INTEGER, INTENT(IN)                      :: nbins, multiplicity, nclusters

    CHARACTER(len=*), PARAMETER :: &
      routineN = 'rebin_imaged_distribution_clusters', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: dim, i, nimages, virtual_bin

!   ---------------------------------------------------------------------------

    dim = SIZE (new_bins)
    nimages = nclusters/nbins
    CALL dbcsr_assert (dim.EQ.SIZE(source_bins),&
         dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Wrong bins array dimension",851)
    CALL dbcsr_assert (dim.EQ.SIZE(new_images).AND.dim.EQ.SIZE(source_images),&
         dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Wrong images array dimensions",854)
    DO i = 1, dim
       virtual_bin = source_bins(i)*multiplicity + source_images(i)
       new_bins(i) = virtual_bin / nimages
       new_images(i) = 1 + virtual_bin
       CALL dbcsr_assert (new_bins(i).lt.nbins, dbcsr_fatal_level, dbcsr_internal_error,&
            routineN, "Wrong bin calculation",860)
       CALL dbcsr_assert (new_images(i).le.nclusters, dbcsr_fatal_level, dbcsr_internal_error,&
            routineN, "Wrong image calculation",862)
    ENDDO
  END SUBROUTINE rebin_imaged_distribution_clusters
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_distribution_has_threads (product_dist), &
         dbcsr_fatal_level, dbcsr_internal_error, routineN,&
         "Product distribution must have threads.", 1609)
    ...
    ELSE
       IF (careful_mod) THEN
          CALL dbcsr_assert (ASSOCIATED (dist%d%other_l_rows),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Local rows mapping does not exist.", 1846)
       ENDIF
    ENDIF
    ...
    ELSE
       IF (careful_mod) THEN
          CALL dbcsr_assert (ASSOCIATED (dist%d%other_l_cols),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Local columns mapping does not exist.", 1899)
       ENDIF
    ENDIF
    ...
    ELSE
       IF (careful_mod) THEN
          CALL dbcsr_assert (array_exists (dist%d%global_col_map),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Column map does not exist.", 1973)
       ENDIF
    ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_get_local_rows (dist, matrix%m%local_rows,&
         matrix%m%index(dbcsr_slot_home_prow))
    ...
    CALL dbcsr_get_global_row_map (dist, matrix%m%global_rows)
    ...
    CALL dbcsr_get_local_cols (dist, matrix%m%local_cols,&
         matrix%m%index(dbcsr_slot_home_pcol))
    ...
    CALL dbcsr_get_global_col_map (dist, matrix%m%global_cols)
    ...
    ELSE
       IF (careful_mod) THEN
          CALL dbcsr_assert (ASSOCIATED (imgdist%i%other_vl_rows),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Local rows mapping does not exist.", 2075)
       ENDIF
    ENDIF
    ...
    ELSE
       IF (careful_mod) THEN
          CALL dbcsr_assert (ASSOCIATED (imgdist%i%other_vl_cols),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Local cols mapping does not exist.", 2130)
       ENDIF
    ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    ELSE
       matrix%m%local_rows = imgdist%i%main%d%local_rows
    ENDIF
    CALL array_hold (matrix%m%local_rows)
    matrix%m%has_local_rows = .TRUE.
    matrix%m%nblkrows_local = array_size (matrix%m%local_rows)
    matrix%m%index(dbcsr_slot_nblkrows_local) = array_size (matrix%m%local_rows)
    CALL dbcsr_get_global_vrow_map (imgdist, matrix%m%global_rows)
    CALL array_hold (matrix%m%global_rows)
    matrix%m%has_global_rows = .TRUE.
    ! Columns
    IF (.NOT.PRESENT(do_rows) .OR. .NOT.my_do_rows) THEN
       CALL dbcsr_get_local_vcols (imgdist, matrix%m%local_cols,&
            matrix%m%index(dbcsr_slot_home_vpcol))
    ELSE
       matrix%m%local_cols = imgdist%i%main%d%local_cols
    ENDIF
    CALL array_hold (matrix%m%local_cols)
    matrix%m%has_local_cols = .TRUE.
    matrix%m%nblkcols_local = array_size (matrix%m%local_cols)
    matrix%m%index(dbcsr_slot_nblkcols_local) = array_size (matrix%m%local_cols)
    CALL dbcsr_get_global_vcol_map (imgdist, matrix%m%global_cols)
    CALL array_hold (matrix%m%global_cols)
    matrix%m%has_global_cols = .TRUE.
