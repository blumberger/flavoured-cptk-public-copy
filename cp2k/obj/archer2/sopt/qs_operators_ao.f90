# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_operators_ao.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_operators_ao.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_operators_ao.F" 2
...
! *****************************************************************************
  SUBROUTINE build_lin_mom_matrix(qs_env,matrix)
    ...
    INTEGER :: handle, i, iatom, icol, ikind, inode, irow, iset, jatom, &
      jkind, jset, last_jatom, ldai, maxco, maxlgto, maxsgf, natom, ncoa, &
      ncob, neighbor_list_id, nkind, nseta, nsetb, sgfa, sgfb
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (new_atom_b) THEN
          ...
          DO i=1,3
             ...
             IF(.NOT.(ASSOCIATED(INTEGRAL(i)%block)))CALL cp__a("qs_operators_ao.F",209)
          ENDDO
       END IF
      ...
      ! basis jkind
       ...
       IF (new_atom_b) THEN
          ...
          DO i=1,3
             ...
             CALL cp_dbcsr_get_block_p(matrix=matrix(i)%matrix,&
                  row=irow,col=icol,BLOCK=INTEGRAL(i)%block,found=found)
             IF(.NOT.(ASSOCIATED(INTEGRAL(i)%block)))CALL cp__a("qs_operators_ao.F",582)
          ENDDO
       END IF
...
! *****************************************************************************
  ...
  SUBROUTINE p_xyz_ao(op,qs_env,minimum_image)
    ...
    INTEGER :: handle, i, iatom, icol, ikind, inode, irow, iset, jatom, &
      jkind, jset, last_jatom, ldab, ldsa, ldsb, ldwork, maxl, ncoa, ncob, &
      nkind, nseta, nsetb, sgfa, sgfb
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, lb_max, &
                                                lb_min, npgfa, npgfb, nsgfa, &
                                                nsgfb
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgfa, first_sgfb
    LOGICAL                                  :: found, my_minimum_image, &
                                                new_atom_b
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (new_atom_b) THEN
         ...
         DO i = 1,3
            ...
            IF(.NOT.(ASSOCIATED(op_dip(i)%block)))CALL cp__a("qs_operators_ao.F",993)
         END DO
       END IF  ! new_atom_b
       rab2 = rab(1)*rab(1) + rab(2)*rab(2) + rab(3)*rab(3)
       dab = SQRT(rab2)
...
! *****************************************************************************
  ...
  SUBROUTINE rRc_xyz_ao(op,qs_env,rc,order,minimum_image,soft)
    ...
    INTEGER :: handle, i, iatom, icol, ikind, imom, inode, irow, iset, jatom, &
      jkind, jset, last_jatom, ldab, ldsa, ldsb, ldwork, M_dim, maxl, ncoa, &
    ...
    IF(.NOT.(M_dim<=SIZE(op,1)))CALL cp__a("qs_operators_ao.F",1185)
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (new_atom_b) THEN
         ...
         DO imom = 1, M_dim
            ...
            IF(.NOT.(ASSOCIATED(op_dip(imom)%block)))CALL cp__a("qs_operators_ao.F",1268)
         END DO   ! imom
       END IF  ! new_atom_b
...
! *****************************************************************************
  SUBROUTINE rRc_xyz_der_ao(op,op_der,qs_env,rc,order,minimum_image,soft)
    ...
    INTEGER :: handle, i, iatom, icol, idir, ikind, imom, inode, ipgf, irow, &
      iset, j, jatom, jkind, jpgf, jset, last_jatom, lda_min, ldab, ldb_min, &
      ldsa, ldsb, ldwork, M_dim, maxl, na, nb, ncoa, ncob, nda, ndb, nkind, &
      nseta, nsetb, sgfa, sgfb
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, lb_max, &
                                                lb_min, npgfa, npgfb, nsgfa, &
                                                nsgfb
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgfa, first_sgfb
    LOGICAL                                  :: my_minimum_image, my_soft, &
                                                new_atom_b, op_der_found, &
                                                op_found
    ...
    IF(.NOT.(ASSOCIATED(op)))CALL cp__a("qs_operators_ao.F",1434)
    IF(.NOT.(ASSOCIATED(op_der)))CALL cp__a("qs_operators_ao.F",1435)
    ...
    IF(.NOT.cp_dbcsr_get_matrix_type(op_der(1,1)%matrix).EQ.dbcsr_type_no_symmetry) THEN
       CALL cp__b("qs_operators_ao.F",1438,"")
    END IF
    ...
    M_dim = ncoset(order)-1
    IF(.NOT.(M_dim<=SIZE(op,1)))CALL cp__a("qs_operators_ao.F",1473)
       ...
       IF (new_atom_b) THEN
         ...
         DO imom = 1, M_dim
            ...
            CALL cp_dbcsr_get_block_p (matrix=op(imom)%matrix,&
                                    ...
                                    found=op_found)
IF(.NOT.(op_found.and.ASSOCIATED(op_dip(imom)%block)))CALL cp__a("qs_operators_ao.F",1564)
            DO idir = 1,3
              ...
              CALL cp_dbcsr_get_block_p (matrix=op_der(imom,idir)%matrix,&
                                      ...
                                      found=op_der_found)
IF(.NOT.(op_der_found.and.ASSOCIATED(op_dip_der(imom,idir)%block)))CALL cp__a("qs_operators_ao.F",1571)
           END DO  ! idir
         END DO   ! imom
       END IF  ! new_atom_b
