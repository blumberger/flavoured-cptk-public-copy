# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xbeef.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xbeef.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xbeef.F" 2
...
! *****************************************************************************
  SUBROUTINE xbeef_lda_eval(rho_set,deriv_set,grad_deriv,xbeef_params)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_xbeef.F",160)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_xbeef.F",161)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_xbeef.F",162)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_xbeef.F",163)
    ...
    END IF
    IF (grad_deriv>1.OR.grad_deriv<-1) THEN
       CALL cp__b("xc/xc_xbeef.F",188,"derivatives greater than 1 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE xbeef_lsd_eval(rho_set,deriv_set,grad_deriv,xbeef_params)
    ...
    CALL section_vals_val_get(xbeef_params,"scale_x",r_val=sx)
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_xbeef.F",349)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_xbeef.F",350)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_xbeef.F",351)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_xbeef.F",352)
   ...
   IF (grad_deriv>=0) THEN
    ...
    END IF
    IF (grad_deriv>1.OR.grad_deriv<-1) THEN
       CALL cp__b("xc/xc_xbeef.F",388,"derivatives greater than 1 not implemented")
    END IF
