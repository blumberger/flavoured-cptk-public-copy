# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_thomas_fermi.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_thomas_fermi.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_thomas_fermi.F" 2
...
! *****************************************************************************
  SUBROUTINE thomas_fermi_lda_eval(rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_thomas_fermi.F",136)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_thomas_fermi.F",137)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_thomas_fermi.F",138)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_thomas_fermi.F",139)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_thomas_fermi.F",174,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE thomas_fermi_lsd_eval(rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_thomas_fermi.F",210)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_thomas_fermi.F",211)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_thomas_fermi.F",212)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_thomas_fermi.F",213)
    ...
    DO ispin=1,2
       ...
       END IF
       IF ( order>3.OR.order<-3) THEN
          CALL cp__b("xc/xc_thomas_fermi.F",258,"derivatives bigger than 3 not implemented")
       END IF
    END DO
    CALL timestop(handle)
