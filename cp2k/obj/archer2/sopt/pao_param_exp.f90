# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_exp.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_exp.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_exp.F" 2
...
! *****************************************************************************
  SUBROUTINE pao_param_init_exp(pao, qs_env, reuse_matrix_X)
    ...
    IF(dft_control%nspins /= 1) CALL cp__b("pao_param_exp.F",68,"open shell not yet implemented")
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(ASSOCIATED(block_H) .AND. ASSOCIATED(block_N)))CALL cp__a("pao_param_exp.F",99)
...
! *****************************************************************************
  SUBROUTINE pao_calc_U_exp(pao, iatom, block_X, block_U, block_M, block_G)
    ...
    IF(.NOT.(ASSOCIATED(block_Y)))CALL cp__a("pao_param_exp.F",157)
    N=SIZE(block_Y, 1)
    M=SIZE(block_Y, 2)

    CALL cp_dbcsr_get_block_p(matrix=pao%matrix_U0, row=iatom, col=iatom, block=block_U0, found=found)
    IF(.NOT.(ASSOCIATED(block_U0)))CALL cp__a("pao_param_exp.F",162)
    ...
    IF(PRESENT(block_G))THEN
       IF(.NOT.(PRESENT(block_M)))CALL cp__a("pao_param_exp.F",189)
...
! *****************************************************************************
   SUBROUTINE diag_antisym(matrix, evecs, evals)
    ...
    IF(MAXVAL(ABS(matrix+TRANSPOSE(matrix)))>1e-14_dp) CALL cp__b("pao_param_exp.F",280,"Expected anti-symmetric matrix")
    N = SIZE(matrix, 1)
...
! *****************************************************************************
   SUBROUTINE zheevd_wrapper(matrix,eigenvectors,eigenvalues)
    ...
    IF(SIZE(matrix, 1) /= SIZE(matrix, 2)) CALL cp__b("pao_param_exp.F",315,"expected square matrix")
    IF(MAXVAL(ABS(matrix-CONJG(TRANSPOSE(matrix))))>1e-14_dp) CALL cp__b("pao_param_exp.F",316,"Expect hermitian matrix")
    ...
    IF (info /= 0) CALL cp__b("pao_param_exp.F",346,"diagonalization failed")
