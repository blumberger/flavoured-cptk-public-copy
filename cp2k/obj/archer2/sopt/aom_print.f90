# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aom_print.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aom_print.F"
!-----------------------------------------------------------------------------!
...
# 65 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aom_print.F" 2
...
!----PC------
  ...
  END SUBROUTINE adiab_print_pvecs
   ...
   SUBROUTINE adiab_print_site_energies(adiab_section, energies, nadiab, deltatMD, times)
    TYPE(section_vals_type), POINTER         :: adiab_section
    REAL(KIND=dp), DIMENSION(:), INTENT(IN)  :: energies
    INTEGER, INTENT(IN)                      :: nadiab
    REAL(KIND=dp), INTENT(IN)                :: times
    ...
    unit_nr=cp_print_key_unit_nr(logger,adiab_section,"PRINT%SITE_ENERGIES",&
               extension=".xyz", middle_name="site",log_filename=.FALSE.)
    unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")

    IF (unit_nr>0) THEN
       CALL section_vals_val_get(adiab_section,"PRINT%SITE_ENERGIES%UNIT",&
            c_val=unit_str)
       unit_conv = cp_unit_from_cp2k(1.0_dp,TRIM(unit_str))
       unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")
       time = times*deltatMD*unit_conv_fs

       WRITE ( unit_nr, "( A,A,A,I8,A,I8)" ) "Site Energies (",TRIM(unit_str),");  nadiab =", nadiab
       WRITE ( unit_nr, '(A,I0,A,F20.3,A,F0.10)' )   " i = ",INT(times),", time = ",time, " (fs)"
    ...
    CALL cp_print_key_finished_output(unit_nr,logger,adiab_section,"PRINT%SITE_ENERGIES")
   END SUBROUTINE adiab_print_site_energies

   SUBROUTINE adiab_print_off_diagonals(adiab_section, coupling, nadiab, deltatMD, times)
    TYPE(section_vals_type), POINTER         :: adiab_section
    REAL(KIND=dp), DIMENSION(:), INTENT(IN)  :: coupling
    INTEGER, INTENT(IN)                      :: nadiab
    REAL(KIND=dp), INTENT(IN)                :: times
    REAL(KIND=dp), INTENT(IN)                :: deltatMD
    ...
    unit_nr=cp_print_key_unit_nr(logger,adiab_section,"PRINT%OFF_DIAGONALS",&
               extension=".xyz", middle_name="couplings",log_filename=.FALSE.)
    unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")

    IF (unit_nr>0) THEN
       CALL section_vals_val_get(adiab_section,"PRINT%OFF_DIAGONALS%UNIT",&
            c_val=unit_str)
       unit_conv = cp_unit_from_cp2k(1.0_dp,TRIM(unit_str))
       unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")
       time = times*deltatMD*unit_conv_fs

       WRITE ( unit_nr, "( A,A,A,I8,A,I8)" ) "Couplings (",TRIM(unit_str),");  ncouplings =", nadiab
       WRITE ( unit_nr, '(A,I0,A,F20.3,A,F0.10)' )   " i = ",INT(times),", time = ",time, " (fs)"

       DO iadiab=1, nadiab
         my_format="(I4,"
         WRITE(unit_nr,TRIM(my_format)//"1X,F20.10)") iadiab, coupling(iadiab) 
       ENDDO

    END IF

    CALL cp_print_key_finished_output(unit_nr,logger,adiab_section,"PRINT%OFF_DIAGONALS")
   END SUBROUTINE adiab_print_off_diagonals
   ...
   SUBROUTINE adiab_print_adiab_pop(adiab_section, populations, nadiab, norbitals,itimes, deltatMD, times)
    TYPE(section_vals_type), POINTER         :: adiab_section
    REAL(KIND=dp), DIMENSION(:), INTENT(IN)  :: populations
    INTEGER, INTENT(IN)                      :: nadiab, norbitals, itimes
    REAL(KIND=dp),INTENT(IN)                 :: deltatMD, times
    ...
    CALL cp_print_key_finished_output(unit_nr,logger,adiab_section,"PRINT%ADIAB_POPULATIONS")
   END SUBROUTINE adiab_print_adiab_pop
   ...
   SUBROUTINE adiab_print_pseudo_hamiltonian(adiab_section, hamiltonian, nadiab, &
              norbitals, itimes, deltatMD)
    TYPE(section_vals_type), POINTER         :: adiab_section
    REAL(KIND=dp), DIMENSION(:,:)            :: hamiltonian
    ...
    unit_nr=cp_print_key_unit_nr(logger,adiab_section,"PRINT%PSEUDO_HAMILTONIAN",&
               extension=".xyz",middle_name="pseudo-hamilt", log_filename=.FALSE.,&
               is_new_file=new_file)

    IF (norbitals .ne. 1) then 
    CALL cp__b("aom_print.F",572,"This pseudo-H printing is not ready to cope with multiple orbitals per site")
    END IF

    IF (unit_nr>0) THEN

       CALL section_vals_val_get(adiab_section,"PRINT%PSEUDO_HAMILTONIAN%UNIT",&
            c_val=unit_str)
       unit_conv = cp_unit_from_cp2k(1.0_dp,TRIM(unit_str))
       unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")
       time = unit_conv_fs*itimes*deltatMD
       ...
       ! here in practice nadiab =nabiab as norbitals=1
       nadiab = nadiab*norbitals

       DO iadiab=1, nadiab
          DO jadiab=iadiab,nadiab
            my_format="(I4,1X,I4,"
            write(my_format2,"(I8)") nadiab*norbitals
            if (hamiltonian(iadiab,jadiab) .ne. 0.0D0) then 
               WRITE(unit_nr,TRIM(my_format)//"1X,"//TRIM(my_format2)//"F20.10)") iadiab, jadiab, &
                           unit_conv*hamiltonian(iadiab,jadiab) 
            end if
          ENDDO
       ENDDO

    END IF
    CALL cp_print_key_finished_output(unit_nr,logger,adiab_section,"PRINT%PSEUDO_HAMILTONIAN")
   END SUBROUTINE adiab_print_pseudo_hamiltonian
  ...
  SUBROUTINE adiab_print_nace_active(adiab_section, active_state, nace, nadiab, &
             norbitals, itimes, deltatMD, times)
   TYPE(section_vals_type), POINTER         :: adiab_section
   REAL(KIND=dp), DIMENSION(:,:)            :: nace
   INTEGER, INTENT(IN)                      :: nadiab, norbitals
   ...
   CHARACTER(len=*), PARAMETER :: routineN = 'adiab_print_nace_active', &
     routineP = moduleN//':'//routineN
   ...
   unit_nr=cp_print_key_unit_nr(logger,adiab_section,"PRINT%NACE",&
              extension=".xyz",middle_name="nace-active", log_filename=.FALSE.,&
              is_new_file=new_file)
   ...
   IF (unit_nr>0) THEN
      ...
      WRITE ( unit_nr, "( A,A,A,I8,A,I8)" ) "NACE_ACTIVE (",TRIM(unit_str),");  nadiab =", nadiab, "; norbitals =",&
                                  norbitals
      WRITE ( unit_nr, '(A,I0,A,F20.3,A,F0.10)' )   " i = ",itimes,", time = ",time, " (fs)"
      ...
      DO iorbitals=1,norbitals
        my_format="(I4,1X,I4,"
        write(my_format2,"(I8)") nadiab*norbitals
        WRITE(unit_nr,TRIM(my_format)//"1X,"//TRIM(my_format2)//"F20.10)") active_state, iorbitals, &
                          unit_conv*nace( (active_state-1)*norbitals + iorbitals,:)
      ENDDO

   END IF
   CALL cp_print_key_finished_output(unit_nr,logger,adiab_section,"PRINT%NACE")

  END SUBROUTINE adiab_print_nace_active
   ...
   SUBROUTINE print_new_state_indices(sh_env, sh_print_env)

! subroutine for printing out the indices of adiabats of the previous
! timestep, to which the indicies of the adiabats in the current
! timestep correspond -- FI 18/03/2024

        TYPE(sh_print_env_type), INTENT(IN) :: sh_print_env
        TYPE(sh_env_type), INTENT(IN) :: sh_env
        INTEGER :: unit_number
        REAL(KIND=dp) :: times, real_time, convert_to_fs, delta_t_MD
        TYPE(section_vals_type), POINTER :: adiab_section
        TYPE(cp_logger_type), POINTER :: logger
        ...
        logger => cp_get_default_logger()
        times = sh_print_env%times
        delta_t_MD = sh_print_env%deltatMD 
        adiab_section => sh_print_env%adiab_section

        unit_number=cp_print_key_unit_nr(logger,adiab_section,"PRINT%NEW_STATE_INDICES",&
               extension=".log", middle_name="state-tracker",log_filename=.FALSE.)

        if (unit_number > 0) then

          convert_to_fs = cp_unit_from_cp2k(1.0_dp,"fs")
          real_time = times*delta_t_MD*convert_to_fs

          write( unit_number, '(A,I0,A,F20.3,A,F0.10)' )   "   i = ",INT(times),", time = ",real_time, " (fs)"
          write(unit_number,*) sh_env%adiab_nace_env%map_new2old

        end if


    CALL cp_print_key_finished_output(unit_number,logger,adiab_section,"PRINT%NEW_STATE_INDICES")

   END SUBROUTINE print_new_state_indices
   ...
   SUBROUTINE adiab_print_sh_info(sh_print_env, keyword)
    ...
    IF (unit_nr>0) THEN
       ...
       CASE("BOMD")
! this part is just TO PRINT FINAL STATE IN CASE OF BOMD 
               !ALLOCATE(pop_init(nadiab))
         unit_conv_fs = cp_unit_from_cp2k(1.0_dp,"fs")
         time = times*deltatMD*unit_conv_fs 
         WRITE ( unit_nr, '(A,I0,A,F20.3,A,F0.10)' )   "   i = ",INT(times),", time = ",time, " (fs)"
         ...
         !WRITE( unit_nr, "(A,I8)" ) "First Adiabatic State = ", first_adiabat
         WRITE( unit_nr, "(A,I8)" ) "Final state   = ", first_adiabat
         WRITE( unit_nr, "(A)") "  "
         !DEALLOCATE(pop_init)
       ...
       CASE("TULLY_TEST")
           ...
           if (print_all) then
               ...
               IF (attempt_hop) THEN
                 WRITE( unit_nr, "(A,I8,A,I8)" ) "   ATTEMPT between old_state = ", old_state, & 
                                                                    " and new_state = ", new_state
               ELSE
       ...
       CASE("NACV_RESCALING")
           ...
           if (print_all) then
               ...
               WRITE( unit_nr, "(A,F20.10)" ) "   Energy old state (Ha) = ", adiab_energies_t(old_state)
               WRITE( unit_nr, "(A,F20.10)" ) "   Energy new state (Ha) = ", adiab_energies_t(new_state)
               WRITE( unit_nr, "(A,F20.10)" ) "   Energy diff (Ha) = ", adiab_energies_t(new_state) - adiab_energies_t(old_state)
               WRITE( unit_nr, "(A,F20.10)" ) "   A-term   = ", A
               WRITE( unit_nr, "(A,F20.10)" ) "   B-term   = ", B
               WRITE( unit_nr, "(A,F20.10)" ) "   NACV kinetic energy (Ha)  = ", B**2/(4*A)
