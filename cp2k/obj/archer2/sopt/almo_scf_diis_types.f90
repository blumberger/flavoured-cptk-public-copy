# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_diis_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_diis_types.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_diis_types.F" 2
...
! *****************************************************************************
  SUBROUTINE almo_scf_diis_init_dbcsr(diis_env,sample_err,sample_var,error_type,&
    ...
    IF( max_length.le.0 ) THEN
       CALL cp__b("almo_scf_diis_types.F",110,"DIIS: max_length is less than zero")
    END IF
    ...
    DO im=1,diis_env%max_buffer_length
       CALL cp_dbcsr_init(diis_env%m_err(im))
       CALL cp_dbcsr_create(diis_env%m_err(im),&
               template=sample_err)
       CALL cp_dbcsr_init(diis_env%m_var(im))
...
! *****************************************************************************
  SUBROUTINE almo_scf_diis_init_domain(diis_env,sample_err,error_type,&
    ...
    TYPE(domain_submatrix_type), &
      DIMENSION(:), INTENT(IN)               :: sample_err
    INTEGER, INTENT(IN)                      :: error_type, max_length
    ...
    IF( max_length.le.0 ) THEN
       CALL cp__b("almo_scf_diis_types.F",177,"DIIS: max_length is less than zero")
    END IF
  ...
  SUBROUTINE almo_scf_diis_push(diis_env,var,err,d_var,d_err)
    ...
    IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
       IF ( .NOT.(PRESENT(var).AND.PRESENT(err)) ) THEN
          CALL cp__b("almo_scf_diis_types.F",245,"provide DBCSR matrices")
       ENDIF
    ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
       IF ( .NOT.(PRESENT(d_var).AND.PRESENT(d_err)) ) THEN
          CALL cp__b("almo_scf_diis_types.F",249,"provide domain submatrices")
       ENDIF
    ELSE
       CALL cp__b("almo_scf_diis_types.F",252,"illegal DIIS ENV type")
    ENDIF
    ...
    IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
       CALL cp_dbcsr_copy(diis_env%m_var(in_point),var) 
       CALL cp_dbcsr_copy(diis_env%m_err(in_point),err) 
    ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
  ...
  SUBROUTINE almo_scf_diis_extrapolate(diis_env,extr_var,d_extr_var)
    ...
    IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
       IF ( .NOT.PRESENT(extr_var) ) THEN
          CALL cp__b("almo_scf_diis_types.F",377,"provide DBCSR matrix")
       ENDIF
    ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
       IF ( .NOT.PRESENT(d_extr_var) ) THEN
          CALL cp__b("almo_scf_diis_types.F",381,"provide domain submatrices")
       ENDIF
    ELSE
       CALL cp__b("almo_scf_diis_types.F",384,"illegal DIIS ENV type")
    ENDIF
    ...
    DO idomain=1, ndomains
       ...
       IF (diis_env%m_b(idomain)%domain.gt.0) THEN
          ...
          CALL DSYEV('V','L',diis_env%buffer_length+1,m_b_copy,&
                  diis_env%buffer_length+1,eigenvalues,WORK,LWORK,INFO)
          IF( INFO.NE.0 ) THEN
             CALL cp__b("almo_scf_diis_types.F",412,"DSYEV failed")
          END IF
          DEALLOCATE(WORK)
          ...
          checksum=0.0_dp
          IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
             CALL cp_dbcsr_set(extr_var,0.0_dp)
...
!> \param d_A ...
!> \param d_B ...
!> \retval almo_scf_diis_error_overlap ...
!> \par History
  ...
  FUNCTION almo_scf_diis_error_overlap(diis_env,A,B,d_A,d_B)
    ...
    IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
       IF ( .NOT.(PRESENT(A).AND.PRESENT(B)) ) THEN
          CALL cp__b("almo_scf_diis_types.F",503,"provide DBCSR matrices")
       ENDIF
    ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
       IF ( .NOT.(PRESENT(d_A).AND.PRESENT(d_B)) ) THEN
          CALL cp__b("almo_scf_diis_types.F",507,"provide domain submatrices")
       ENDIF
    ELSE
       CALL cp__b("almo_scf_diis_types.F",510,"illegal DIIS ENV type")
    ENDIF

    SELECT CASE(diis_env%error_type)
    CASE (diis_error_orthogonal)
       IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
          CALL cp_dbcsr_trace(A, B, trace, 'T', 'N')
       ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
          IF(.NOT.(SIZE(d_A%mdata,1).eq.SIZE(d_B%mdata,1)))CALL cp__a("almo_scf_diis_types.F",518)
          IF(.NOT.(SIZE(d_A%mdata,2).eq.SIZE(d_B%mdata,2)))CALL cp__a("almo_scf_diis_types.F",519)
          IF(.NOT.(d_A%domain.eq.d_B%domain))CALL cp__a("almo_scf_diis_types.F",520)
          IF(.NOT.(d_A%domain.gt.0))CALL cp__a("almo_scf_diis_types.F",521)
          IF(.NOT.(d_B%domain.gt.0))CALL cp__a("almo_scf_diis_types.F",522)
          trace=SUM(d_A%mdata(:,:)*d_B%mdata(:,:))
       ENDIF
    CASE DEFAULT
       CALL cp__b("almo_scf_diis_types.F",526,"Vector type is unknown")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE almo_scf_diis_release(diis_env)
    ...
    DO im=1,diis_env%max_buffer_length
       IF (diis_env%diis_env_type.eq.diis_env_dbcsr) THEN
          CALL cp_dbcsr_release(diis_env%m_err(im))
          CALL cp_dbcsr_release(diis_env%m_var(im))
       ELSE IF (diis_env%diis_env_type.eq.diis_env_domain) THEN
