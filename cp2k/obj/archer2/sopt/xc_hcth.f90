# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_hcth.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_hcth.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_hcth.F" 2
...
! *****************************************************************************
  SUBROUTINE hcth_lda_info(iparset,reference,shortform, needs, max_deriv)
    ...
    CASE(407)
       ...
       END IF
    CASE default
       CALL cp__b("xc/xc_hcth.F",88,"Invalid HCTH parameter set requested ("//cp_to_string(iparset)//")")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE hcth_lda_eval(iparset,rho_set,deriv_set,grad_deriv)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_hcth.F",128)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_hcth.F",129)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_hcth.F",130)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_hcth.F",131)
    ...
    IF (grad_deriv>1.OR.grad_deriv<-1) THEN
       CALL cp__b("xc/xc_hcth.F",148,"derivatives bigger than 1 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE hcth_lda_calc(iparset,rho,norm_drho,e_0,e_rho,e_ndrho,&
    ...
    CASE DEFAULT
       CALL cp__b("xc/xc_hcth.F",282,"Invalid HCTH parameter set requested ("//cp_to_string(iparset)//")")
    END SELECT
