# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/colvar_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/colvar_types.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/colvar_types.F" 2
...
! *****************************************************************************
  SUBROUTINE colvar_create(colvar,colvar_id)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(colvar)))CALL cp__a("subsys/colvar_types.F",385)
    ALLOCATE(colvar)
    ...
    CASE(hydronium_colvar_id)
       ALLOCATE(colvar%hydronium_param)
    ...
    CASE(no_colvar_id)
       ! Do nothing
    CASE DEFAULT
       CALL cp__b("subsys/colvar_types.F",471,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE colvar_setup(colvar)
    ...
    IF(.NOT.(ASSOCIATED(colvar)))CALL cp__a("subsys/colvar_types.F",491)
    ...
    CASE(coord_colvar_id)
       ...
       IF(colvar%coord_param%n_atoms_to_b/=0) THEN
         ...
         ENDDO
       END IF
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",543)
    CASE(population_colvar_id)
       ...
       DO ii=1,colvar%population_param%n_atoms_to
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",569)
    CASE(gyration_colvar_id)
      ...
      ! Create a List of points...
       ...
       DO ii=1,colvar%gyration_param%n_atoms
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",586)
    CASE(angle_colvar_id)
       np = 3
       ! Number of real atoms involved in the colvar
       colvar%n_atom_s= 0
       ...
       DO ii=1, 3
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",603)
    CASE(torsion_colvar_id)
       np = 4
       ! Number of real atoms involved in the colvar
       colvar%n_atom_s= 0
       ...
       DO ii=1, 4
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",620)
    CASE(plane_distance_colvar_id)
       np = 4
       ! Number of real atoms involved in the colvar
       colvar%n_atom_s= 0
       ...
       list(4) = i
       idum = idum + 1
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",642)
    CASE(plane_plane_angle_colvar_id)
       np = 0
       ...
       IF(np==0)&
          CALL cp_abort(cp__l("subsys/colvar_types.F",649),&
       ...
       IF (colvar%plane_plane_angle_param%plane2%type_of_def==plane_def_atoms) THEN
          ...
          ENDDO
       END IF
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",686)
    CASE(dfunct_colvar_id)
       np = 4
       ! Number of real atoms involved in the colvar
       colvar%n_atom_s= 0
       ...
       DO ii=1, 4
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",703)
    CASE(rotation_colvar_id)
       np = 4
       ! Number of real atoms involved in the colvar
       colvar%n_atom_s= 0
    ...
    CASE(qparm_colvar_id)
       ...
       DO ii=1,colvar%qparm_param%n_atoms_to
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",751)
    CASE(hydronium_colvar_id)
       ...
       DO ii=1,colvar%hydronium_param%n_hydrogens
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",777)
    CASE(rmsd_colvar_id)
    ...
    CASE(u_colvar_id)
       ...
       IF(.NOT.(stat==0))CALL cp__a("subsys/colvar_types.F",845)
    ...
    CASE(Wc_colvar_id)
       ...
       DO ii=1, 3
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",863)
    CASE(HBP_colvar_id)
       ...
       DO j=1,colvar%HBP%nPoints
         ...
         ENDDO
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",884)
    CASE(ring_puckering_colvar_id)
       ...
       DO ii=1,colvar%ring_puckering_param%nring
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",901)
    CASE(mindist_colvar_id)
       ...
       DO ii=1,colvar%mindist_param%n_coord_to
          ...
          list(idum) = i
       ENDDO
       IF(.NOT.(idum==np))CALL cp__a("subsys/colvar_types.F",937)
    CASE(combine_colvar_id)
       colvar%n_atom_s=0
    ...
    DO i = 1, SIZE(list)
       ...
       END IF
    END DO
    IF(.NOT.(iend==colvar%n_atom_s))CALL cp__a("subsys/colvar_types.F",977)
    DEALLOCATE(list)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE colvar_release(colvar)
    ...
    IF(.NOT.(ASSOCIATED(colvar)))CALL cp__a("subsys/colvar_types.F",1016)
    IF (ASSOCIATED(colvar%dsdr)) THEN
       DEALLOCATE(colvar%dsdr)
    END IF
    IF (ASSOCIATED(colvar%i_atom)) THEN
    ...
    CASE (hydronium_colvar_id)
       DEALLOCATE(colvar%hydronium_param%i_oxygens)
       DEALLOCATE(colvar%hydronium_param%i_hydrogens)
       DEALLOCATE(colvar%hydronium_param)
    CASE (reaction_path_colvar_id, distance_from_path_colvar_id)
       IF(colvar%reaction_path_param%dist_rmsd .OR. colvar%reaction_path_param%rmsd) THEN
    ...
    CASE(no_colvar_id)
       ! Do nothing
    CASE default
       CALL cp__b("subsys/colvar_types.F",1161,"")
    END SELECT
    DEALLOCATE(colvar)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE colvar_clone(colvar_out, colvar_in, i_atom_offset)
    ...
    my_offset = 0
    IF(.NOT.(ASSOCIATED(colvar_in)))CALL cp__a("subsys/colvar_types.F",1185)
    IF(.NOT.(.NOT.ASSOCIATED(colvar_out)))CALL cp__a("subsys/colvar_types.F",1186)
    ...
    CASE(qparm_colvar_id)
     ...
     CASE(hydronium_colvar_id)
       ...
       ALLOCATE(colvar_out%hydronium_param%i_oxygens(ndim))
       ndim = SIZE(colvar_in%hydronium_param%i_hydrogens)
       ALLOCATE(colvar_out%hydronium_param%i_hydrogens(ndim))
...
! *****************************************************************************
  SUBROUTINE colvar_clone_points(colvar_out, colvar_in, offset)
    ...
    IF (colvar_in%use_points) THEN
       IF(.NOT.(ASSOCIATED(colvar_in%points)))CALL cp__a("subsys/colvar_types.F",1529)
