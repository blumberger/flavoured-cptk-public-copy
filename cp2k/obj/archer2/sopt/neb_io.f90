# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_io.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_io.F"
!-----------------------------------------------------------------------------!
...
# 70 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_io.F" 2
...
! *****************************************************************************
  SUBROUTINE read_neb_section(neb_env, neb_section)
    ...
    IF(.NOT.(ASSOCIATED(neb_env)))CALL cp__a("motion/neb_io.F",99)
    neb_env%istep = 0
    ...
    IF (neb_env%pot_type/=pot_neb_full) THEN
       ...
       IF(.NOT.neb_env%use_colvar)&
          CALL cp_abort(cp__l("motion/neb_io.F",118),&
       ...
       CASE(pot_neb_fe)
          ...
          IF(.NOT.explicit)&
             CALL cp_abort(cp__l("motion/neb_io.F",128),&
       ...
       CASE(pot_neb_me)
          ...
          CALL section_vals_get(wrk_section,explicit=explicit)
          IF(.NOT.explicit)&
             CALL cp_abort(cp__l("motion/neb_io.F",135),&
    ...
    ELSE
       IF(neb_env%use_colvar)&
          CALL cp_abort(cp__l("motion/neb_io.F",141),&
...
! *****************************************************************************
  SUBROUTINE handle_band_file_names(rep_env, irep, n_rep, istep)
    ...
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_io.F",482)
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE neb_rep_env_map_info(rep_env, neb_env)
    ...
    CALL f_env_rm_defaults(f_env=f_env,ierr=ierr,handle=handle2)
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_io.F",578)
