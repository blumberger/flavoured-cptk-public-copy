# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F" 2
...
SUBROUTINE cp_fm_struct_create(fmstruct,para_env,context, nrow_global,&
  ...
  IF (.NOT.PRESENT(template_fmstruct)) THEN
     IF(.NOT.(PRESENT(context)))CALL cp__a("fm/cp_fm_struct.F",161)
     IF(.NOT.(PRESENT(nrow_global)))CALL cp__a("fm/cp_fm_struct.F",162)
     IF(.NOT.(PRESENT(ncol_global)))CALL cp__a("fm/cp_fm_struct.F",163)
...
# 258 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F"
  ! block = full matrix
  ...
  IF (PRESENT(local_leading_dimension)) THEN
     IF(MAX(1,fmstruct%nrow_locals(fmstruct%context%mepos(1)))>local_leading_dimension)&
        CALL cp_abort(cp__l("fm/cp_fm_struct.F",269),"local_leading_dimension too small ("//&
...
# 293 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F"
  END IF
...
SUBROUTINE cp_fm_struct_retain(fmstruct)
  ...
  IF(.NOT.(ASSOCIATED(fmstruct)))CALL cp__a("fm/cp_fm_struct.F",309)
  IF(.NOT.(fmstruct%ref_count>0))CALL cp__a("fm/cp_fm_struct.F",310)
...
SUBROUTINE cp_fm_struct_release(fmstruct)
  ...
  IF (ASSOCIATED(fmstruct)) THEN
     IF(.NOT.(fmstruct%ref_count>0))CALL cp__a("fm/cp_fm_struct.F",328)
     fmstruct%ref_count=fmstruct%ref_count-1
...
FUNCTION cp_fm_struct_equivalent(fmstruct1,fmstruct2) RESULT(res)
  ...
  IF(.NOT.(ASSOCIATED(fmstruct1)))CALL cp__a("fm/cp_fm_struct.F",370)
  IF(.NOT.(ASSOCIATED(fmstruct2)))CALL cp__a("fm/cp_fm_struct.F",371)
  IF(.NOT.(fmstruct1%ref_count>0))CALL cp__a("fm/cp_fm_struct.F",372)
  IF(.NOT.(fmstruct2%ref_count>0))CALL cp__a("fm/cp_fm_struct.F",373)
...
SUBROUTINE cp_fm_struct_get(fmstruct, para_env, context,&
  ...
  IF(.NOT.(ASSOCIATED(fmstruct)))CALL cp__a("fm/cp_fm_struct.F",436)
  IF(.NOT.(fmstruct%ref_count>0))CALL cp__a("fm/cp_fm_struct.F",437)
 ...
 IF (PRESENT(row_indices)) THEN
    ...
    IF (.NOT.ASSOCIATED(row_indices)) THEN
       ...
       ALLOCATE(fmstruct%row_indices &
            (MAX(fmstruct%nrow_locals(myprow),1)))
       row_indices => fmstruct%row_indices
# 476 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_struct.F"
 ...
 IF (PRESENT(col_indices)) THEN
    ...
    IF (.NOT.ASSOCIATED(col_indices)) THEN
       ALLOCATE(fmstruct%col_indices &
            (MAX(fmstruct%ncol_locals(mypcol),1)))
       col_indices => fmstruct%col_indices
