# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/cp2k_shell.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/cp2k_shell.F"
!-----------------------------------------------------------------------------!
...
PROGRAM cp2k_shell
  ...
  USE f77_interface,                   ONLY: &
       calc_energy_force, calc_force, create_force_env, destroy_force_env, &
       finalize_cp2k, get_cell, get_energy, get_force, get_natom, get_pos, &
       get_stress_tensor, init_cp2k, set_cell, set_pos
  ...
  USE kinds,                           ONLY: default_path_length,&
                                             ...
                                             dp
  USE machine,                         ONLY: &
       default_output_unit, m_chdir, m_flush, m_getarg, m_getcwd, m_getlog, &
       m_getpid, m_hostnm, m_iargc
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/../base/base_uses.f90" 1
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/cp2k_shell.F" 2
  ...
  CHARACTER(LEN=default_string_length)     :: cmd,units,line
  ...
  IF(m_iargc()>0) THEN
     ...
     IF(m_iargc()>1 .OR. out_filename(:16) /= "--redirect-comm=") &
       CALL cp__b("start/cp2k_shell.F",136,"Usage: cp2k_shell [--redirect-comm=filename]")
  ...
  IF (ierr/=0) CALL cp__b("start/cp2k_shell.F",147,'init_cp2k failure')
  ...
  DO
     ...
     CASE('HELP')
        IF (para_env%source==para_env%mepos) THEN
            DO i=1,SIZE(helpMsg)
                ...
                IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",204,'writing help')
            END DO
            CALL m_flush(sout)
        END IF
     ...
     CASE('VERSION')
       ! Queried by ASE. Increase version after bug-fixing or behavior changes.
        IF (para_env%source==para_env%mepos) THEN
            ...
            IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",217,'writing version')
        END IF
     CASE('WRITE_FILE')
        IF (para_env%mepos==para_env%source) THEN
           READ (*,'(a)',iostat=iostat) out_filename
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",222,'WRITE_FILE bad filename')
           READ (*,*,iostat=iostat) n_lines
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",224,'WRITE_FILE bad number of lines')
           ...
           DO i=1, n_lines
              ...
              IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",231,'WRITE_FILE read error')
              WRITE(file_unit,'(a)',iostat=iostat) TRIM(line)
              IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",233,'WRITE_FILE write error')
           END DO
           READ(*,'(a)',iostat=iostat) line
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",236,'WRITE_FILE read error')
           IF (TRIM(line) /= "*END") CALL cp__b("start/cp2k_shell.F",237,'WRITE_FILE bad end delimiter')
           CALL close_file(unit_number=file_unit)
        ENDIF
     CASE('LAST_ENV_ID')
        IF (para_env%source==para_env%mepos) THEN
           WRITE (sout,'(i10)',iostat=iostat) last_env_id
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",243,cmd//' failed writing last_env_id')
           CALL m_flush(sout)
        END IF
     CASE('BG_LOAD','BGLOAD')
        shift=shift2+1
     ...
     CASE('LOAD')
        ...
        IF (para_env%source==para_env%mepos) THEN
           WRITE (sout,'(i10)',iostat=iostat) last_env_id
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",296,cmd//' failed writing last_env_id')
           CALL m_flush(sout)
        END IF
        IF (last_env_id<=0) failure=.TRUE.
     CASE ('DESTROY')
     ...
     CASE('SETPOS','SET_POS')
        ...
        IF (para_env%mepos==para_env%source) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",328,'setpos read n_atom')
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",340,'setpos read coord')
           pos(:) = pos(:)/pos_fact
           READ(*,'(a)',iostat=iostat) cmdStr
           CALL compress(cmdStr,full=.TRUE.)
           CALL uppercase(cmdStr)
     ...
     CASE('SETCELL','SET_CELL')
        ...
        IF (para_env%mepos==para_env%source) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",366,'setcell read')
           cell(:,:) = cell(:,:)/pos_fact
        END IF
     ...
     CASE('GETCELL','GET_CELL')
        ...
        IF (para_env%mepos==para_env%source) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",380,cmd//' write get_cell')
           CALL m_flush(sout)
        END IF
     CASE('GETSTRESS','GET_STRESS')
        ...
        IF (para_env%mepos==para_env%source) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",391,cmd//' write get_stress')
           CALL m_flush(sout)
        END IF
     CASE('GETPOS','GET_POS')
        env_id=parse_env_id(str=cmdStr,startI=shift2+1,default_val=last_env_id)
        ...
        IF (stat/=0) CALL cp__b("start/cp2k_shell.F",402,cmd//' failed alloc')
        CALL get_pos(env_id, pos=pos, n_el=3*n_atom, ierr=ierr)
        CALL my_assert(ierr==0,'get_pos error',failure)
        pos(:) = pos(:)*pos_fact
        IF (.not.failure.and.para_env%mepos==para_env%source) THEN
           WRITE (sout,'(i10)',iostat=iostat) 3*n_atom
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",408,cmd//' write ncoord')
           WRITE (sout,'(3ES22.13)',iostat=iostat) pos
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",410,cmd//' write coord')
           WRITE (sout,'(a)',iostat=iostat) "* END"
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",412,cmd//' write coord')
           CALL m_flush(sout)
        END IF
        DEALLOCATE(pos)
     CASE('GETE','GET_E')
        ...
        IF (para_env%mepos==para_env%source) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",424,cmd//' write e_pot')
           CALL m_flush(sout)
        END IF
     CASE('EVALE','EVAL_E')
        env_id=parse_env_id(str=cmdStr,startI=shift2+1,default_val=last_env_id)
     ...
     CASE('CALCE','CALC_E')
        ...
        IF (para_env%mepos==para_env%source) THEN
           WRITE (sout,'(ES22.13)',iostat=iostat) e_pot
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",442,cmd//' write e_pot')
           CALL m_flush(sout)
        END IF
     CASE('EVALEF','EVAL_EF')
        env_id=parse_env_id(str=cmdStr,startI=shift2+1,default_val=last_env_id)
     ...
     CASE('GETF','GET_F')
        ...
        IF (failure) GOTO 10
        ALLOCATE(pos(3*n_atom),stat=stat)
        IF (stat/=0) CALL cp__b("start/cp2k_shell.F",458,cmd//' failed alloc')
        ...
        IF (.not.failure.and.para_env%mepos==para_env%source) THEN
           WRITE (sout,'(i10)',iostat=iostat) 3*n_atom
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",464,cmd//' write nforce')
           WRITE (sout,'(3ES22.13)',iostat=iostat) pos
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",466,cmd//' write force')
           WRITE (sout,'("* END")',iostat=iostat)
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",468,cmd//' write coord')
           CALL m_flush(sout)
        END IF
        DEALLOCATE(pos)
     CASE('CALCEF','CALC_EF')
        ...
        IF (failure) GOTO 10
        ALLOCATE(pos(3*n_atom),stat=stat)
        IF (stat/=0) CALL cp__b("start/cp2k_shell.F",487,cmd//' failed alloc')
        CALL get_force(env_id, frc=pos, n_el=3*n_atom, ierr=ierr)
        CALL my_assert(ierr==0,'get_force error',failure)
        pos(:) = pos(:)*(e_fact/pos_fact)
        IF (.not.failure.and.para_env%mepos==para_env%source) THEN
           WRITE (sout,'(ES22.13)',iostat=iostat) e_pot
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",493,cmd//' write e_pot')
           WRITE (sout,'(i10)',iostat=iostat) 3*n_atom
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",495,cmd//' write nforce')
           WRITE (sout,'(3ES22.13)',iostat=iostat) pos
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",497,cmd//' write force')
           WRITE (sout,'("* END")',iostat=iostat)
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",499,cmd//' write coord')
           CALL m_flush(sout)
        END IF
        DEALLOCATE(pos)
     ...
     CASE('UNITS')
        IF (para_env%source==para_env%mepos) THEN
           ...
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",514,cmd//' writing units')
           CALL m_flush(sout)
        END IF
     CASE('HARSH')
        harsh=.TRUE.
     ...
     END SELECT
10   IF (failure) THEN
        IF (harsh) CALL cp__b("start/cp2k_shell.F",547,'failure')
        IF (para_env%mepos==para_env%source) THEN
           WRITE(sout,'("* ERROR, type HELP for help")',iostat=iostat)
           IF (iostat/=0) CALL cp__b("start/cp2k_shell.F",550,'writing error')
           CALL m_flush(sout)
        END IF
        failure=.FALSE.
     END IF
  ...
  IF (ierr/=0) CALL cp__b("start/cp2k_shell.F",560,'finalize_cp2k failure')
...
! *****************************************************************************
  SUBROUTINE my_assert(tst,msg,failure)
    ...
    IF (.not.tst) THEN
       IF (harsh) CALL cp__b("start/cp2k_shell.F",576,msg)
       IF (para_env%mepos==para_env%source) THEN
...
! *****************************************************************************
  FUNCTION get_eof_stat() RESULT(res)
    ...
    IF (.NOT.did_init) THEN
       ...
       eof_stat=stat_n
       IF (eof_stat >= 0) THEN
          CALL cp__b("start/cp2k_shell.F",694,"ERROR,cp_files:get_eof_stat failed identification of EOF IOSTAT")
          eof_stat=-1
       END IF
