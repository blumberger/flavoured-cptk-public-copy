# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner.F"
!-----------------------------------------------------------------------------!
...
# 48 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner.F" 2
...
SUBROUTINE make_preconditioner(preconditioner_env, precon_type, solver_type, matrix_h, matrix_s, &
    ...
    IF(my_mixed_precision) THEN
       ...
       CASE(ot_precond_full_kinetic,ot_precond_full_single_inverse)
          !supported
       CASE DEFAULT
          CALL cp__b("preconditioner.F",134,"This precond with mixed precision is not supported yet")
       END SELECT
    ENDIF
    ...
    CASE DEFAULT
          CALL cp__b("preconditioner.F",163,"The preconditioner is unknown ...")
    END SELECT
    ...
    ELSE
       IF (needs_homo) THEN
          CALL cp__b("preconditioner.F",187,"Not yet implemented")
       ENDIF
    ENDIF
  ...
  SUBROUTINE restart_preconditioner(qs_env,preconditioner,prec_type,nspins)
    ...
    IF (ASSOCIATED(preconditioner)) THEN
       ...
       CASE(ot_precond_none,ot_precond_full_kinetic,ot_precond_s_inverse,ot_precond_full_single_inverse) ! these are 'independent'
         ! do nothing
       CASE DEFAULT
           CALL cp__b("preconditioner.F",259,"")
       END SELECT
    END IF
  ...
  SUBROUTINE prepare_preconditioner(qs_env,mos,matrix_ks,matrix_s,&
    ...
    IF(dft_control%qs_control%semi_empirical .OR. dft_control%qs_control%dftb) THEN
      IF(prec_type==ot_precond_full_kinetic) THEN
          ...
          CALL cp__b("preconditioner.F",347,TRIM(msg))
      END IF
      matrix_t => matrix_s(1)%matrix
    ELSE
      IF(.NOT.(.NOT. my_has_unit_metric))CALL cp__a("preconditioner.F",351)
