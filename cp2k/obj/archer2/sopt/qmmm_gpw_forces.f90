# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_forces.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_forces.F"
!-----------------------------------------------------------------------------!
...
# 79 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_forces.F" 2
...
! *****************************************************************************
  SUBROUTINE qmmm_forces(qs_env,qmmm_env,mm_particles,calc_force,mm_cell)
    ...
    INTEGER                                  :: handle, iatom, image_IndMM, &
                                                Imm, IndMM, ispin, iw
    ...
    IF (dft_control%qs_control%semi_empirical) THEN
       ...
       CASE(do_qmmm_pcharge)
          CALL cp__b("qmmm_gpw_forces.F",176,"Point Charge QM/MM electrostatic coupling not yet implemented for SE.")
       CASE(do_qmmm_gauss,do_qmmm_swave)
          CALL cp__b("qmmm_gpw_forces.F",178,"GAUSS or SWAVE QM/MM electrostatic coupling not yet implemented for SE.")
       CASE(do_qmmm_none)
       ...
       CASE DEFAULT
          ...
          CALL cp__b("qmmm_gpw_forces.F",184,"")
       END SELECT
    ELSEIF (dft_control%qs_control%dftb) THEN
       ...
       CASE(do_qmmm_pcharge)
          CALL deriv_dftb_qmmm_matrix_pc(qs_env,qmmm_env,mm_particles,mm_cell,para_env,&
               need_f,Forces,Forces_added_charges)
       CASE(do_qmmm_gauss,do_qmmm_swave)
          CALL cp__b("qmmm_gpw_forces.F",199,"GAUSS or SWAVE QM/MM electrostatic coupling not yet implemented for DFTB.")
       CASE DEFAULT
          IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')"Unknown Coupling..."
          CALL cp__b("qmmm_gpw_forces.F",202,"")
       END SELECT
       IF (need_f) THEN
    ...
    ELSE
       ...
       IF (need_f) THEN
          ...
          SELECT CASE(qmmm_env%qmmm_coupl_type)
          CASE(do_qmmm_coulomb)
             CALL cp__b("qmmm_gpw_forces.F",250,"Coulomb QM/MM electrostatic coupling not implemented for GPW/GAPW.")
          CASE(do_qmmm_pcharge)
             CALL cp__b("qmmm_gpw_forces.F",252,"Point Charge QM/MM electrostatic coupling not yet implemented for GPW/GAPW.")
          CASE(do_qmmm_gauss,do_qmmm_swave)
             IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')&
          ...
          CASE DEFAULT
             IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')"- Unknown Coupling..."
 CALL cp__b("qmmm_gpw_forces.F",277,"")
          END SELECT
       END IF
    END IF
...
! Statements
    ...
    CALL timeset(routineN,handle)
    NULLIFY(grids,tmp)
    IF(.NOT.(ASSOCIATED(mm_particles)))CALL cp__a("qmmm_gpw_forces.F",441)
    IF(.NOT.(ASSOCIATED(qmmm_env%mm_atom_chrg)))CALL cp__a("qmmm_gpw_forces.F",442)
    IF(.NOT.(ASSOCIATED(qmmm_env%mm_atom_index)))CALL cp__a("qmmm_gpw_forces.F",443)
    IF(.NOT.(ASSOCIATED(Forces)))CALL cp__a("qmmm_gpw_forces.F",444)
    !Statements
    ...
    ELSE IF (lb(1)==glb(1)) THEN
       ALLOCATE(tmp(rho%pw_grid%bounds_local(1,2):rho%pw_grid%bounds_local(2,2),&
            ...
            stat=stat)
       IF(.NOT.(stat==0))CALL cp__a("qmmm_gpw_forces.F",490)
       tmp=rho%cr3d(lb(1),:,:)
    ...
    ELSE IF (ub(1)==gub(1)) THEN
       ALLOCATE(tmp(rho%pw_grid%bounds_local(1,2):rho%pw_grid%bounds_local(2,2),&
            rho%pw_grid%bounds_local(1,3):rho%pw_grid%bounds_local(2,3)),&
            stat=stat)
       IF(.NOT.(stat==0))CALL cp__a("qmmm_gpw_forces.F",499)
    ...
    CASE(spline3_nopbc_interp, spline3_pbc_interp)
       ...
       END DO
    CASE DEFAULT
       CALL cp__b("qmmm_gpw_forces.F",534,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE qmmm_force_with_gaussian_low(grids, mm_particles, mm_charges, &
    ...
    REAL(KIND=dp)                            :: alpha, dvol, height, &
                                                sph_chrg_factor, W
...
! *****************************************************************************
  SUBROUTINE qmmm_forces_with_gaussian_LG  (pgfs,cgrid,num_mm_atoms,mm_charges,mm_atom_index,&
    ...
    REAL(KIND=dp) :: a1, a2, a3, abc_X(4,4), abc_X_Y(4), b1, b2, b3, c1, c2, &
      c3, d1, d2, d3, dr1, dr1c, dr1i, dr2, dr2c, dr2i, dr3, dr3c, dr3i, &
      dvol, e1, e2, e3, f1, f2, f3, fac, ft1, ft2, ft3, g1, g2, g3, h1, h2, &
      h3, p1, p2, p3, q1, q2, q3, qt, r1, r2, r3, rt1, rt2, rt3, rv1, rv2, &
      rv3, s1, s1d, s1o, s2, s2d, s2o, s3, s3d, s3o, s4, s4d, s4o, &
      sph_chrg_factor, t1, t1d, t1o, t2, t2d, t2o, t3, t3d, t3o, t4, t4d, &
      t4o, u1, u2, u3, v1, v1d, v1o, v2, v2d, v2o, v3, v3d, v3o, v4, v4d, &
      v4o, xd1, xd2, xd3, xs1, xs2, xs3
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :)                        :: LForces
...
! *****************************************************************************
  SUBROUTINE qmmm_forces_with_gaussian_LR  (pgfs,cgrid,num_mm_atoms,mm_charges,mm_atom_index,&
    ...
    INTEGER                                  :: handle, i, Imm, IndMM, &
                                                ...
                                                my_i, my_j, my_k, myind, n1, &
                                                n2, n3
    INTEGER, DIMENSION(2, 3)                 :: bo, gbo
    REAL(KIND=dp) :: dr1, dr2, dr3, dvol, dx, fac, ft1, ft2, ft3, qt, r, r2, &
      rd1, rd2, rd3, rt1, rt2, rt3, rv1, rv2, rv3, rx, rx2, sph_chrg_factor, &
      Term, xs1, xs2, xs3
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :)                        :: LForces
...
! *****************************************************************************
  SUBROUTINE qmmm_debug_forces(rho,qs_env,qmmm_env,Analytical_Forces,&
    ...
    Atoms: DO I = 1, num_mm_atoms
       ...
       Coords: DO J = 1, 3
          ...
          Diff: DO K = 1, 2
             ...
             SELECT CASE(qmmm_env%qmmm_coupl_type)
             CASE(do_qmmm_coulomb)
                CALL cp__b("qmmm_gpw_forces.F",1457,"Coulomb QM/MM electrostatic coupling not implemented for GPW/GAPW.")
             CASE(do_qmmm_pcharge)
                CALL cp__b("qmmm_gpw_forces.F",1459,"Point Charge  QM/MM electrostatic coupling not implemented for GPW/GAPW.")
             CASE(do_qmmm_gauss,do_qmmm_swave)
             ...
             CASE DEFAULT
                ...
                CALL cp__b("qmmm_gpw_forces.F",1477,"")
             END SELECT
    ...
    SELECT CASE(qmmm_env%qmmm_coupl_type)
    CASE(do_qmmm_coulomb)
       CALL cp__b("qmmm_gpw_forces.F",1492,"Coulomb QM/MM electrostatic coupling not implemented for GPW/GAPW.")
    CASE(do_qmmm_pcharge)
       CALL cp__b("qmmm_gpw_forces.F",1494,"Point Charge QM/MM electrostatic coupling not implemented for GPW/GAPW.")
    CASE(do_qmmm_gauss,do_qmmm_swave)
       ...
       DO I =  1, num_mm_atoms
          ...
          IF(.NOT.(ABS(Err(1))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1509)
          IF(.NOT.(ABS(Err(2))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1510)
          IF(.NOT.(ABS(Err(3))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1511)
       END DO
    CASE(do_qmmm_none)
       IF (iw>0) WRITE(iw,'(T3,A)')"No QM/MM Derivatives to debug. Just Mechanical Coupling!"
    CASE DEFAULT
       IF (iw>0) WRITE(iw,'(T3,A)')"Unknown Coupling..."
       CALL cp__b("qmmm_gpw_forces.F",1517,"")
    END SELECT
    CALL cp_print_key_finished_output(iw,logger,print_section,"PROGRAM_RUN_INFO")
...
! *****************************************************************************
  SUBROUTINE debug_integrate_gf_rspace_NoPBC(ilevel, zetp, rp, W, pwgrid, cube_info,&
    ...
    IF(.NOT.(ABS(Err(1))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1625)
    IF(.NOT.(ABS(Err(2))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1626)
    IF(.NOT.(ABS(Err(3))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1627)
...
! *****************************************************************************
  SUBROUTINE debug_qmmm_forces_with_gauss_LG(pgfs,aug_pools, rho, mm_charges, mm_atom_index,&
    ...
    DO I =  1, num_mm_atoms
       ...
       IF(.NOT.(ABS(Err(1))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1756)
       IF(.NOT.(ABS(Err(2))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1757)
       IF(.NOT.(ABS(Err(3))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1758)
    END DO
    ...
    DO I =  1, num_mm_atoms
       ...
       IF (iw>0)&
            WRITE(iw,100)IndMM,debug_force(1,I),Num_Forces(1,I),Err(1),&
                               debug_force(2,I),Num_Forces(2,I),Err(2),&
                               debug_force(3,I),Num_Forces(3,I),Err(3)
       IF(.NOT.(ABS(Err(1))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1879)
       IF(.NOT.(ABS(Err(2))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1880)
       IF(.NOT.(ABS(Err(3))<=MaxErr))CALL cp__a("qmmm_gpw_forces.F",1881)
    END DO
