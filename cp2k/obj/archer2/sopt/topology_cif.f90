# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_cif.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_cif.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_cif.F" 2
...
! *****************************************************************************
  SUBROUTINE read_coordinate_cif (topology,para_env,subsys_section)
    ...
    CALL parser_search_string(parser,"_cell_length_a",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",157,"The field (_cell_length_a) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_cell_length_b",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",165,"The field (_cell_length_b) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_cell_length_c",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",173,"The field (_cell_length_c) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_cell_angle_alpha",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",181,"The field (_cell_angle_alpha) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_cell_angle_beta",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",189,"The field (_cell_angle_beta) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_cell_angle_gamma",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",197,"The field (_cell_angle_gamma) was not found in CIF file! ")
    ...
    CALL parser_search_string(parser,"_atom_site_label",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_cif.F",213,"The field _atom_site_label was not found in CIF file! ")
    ifield     = 0
    ii         = 1
    ifield(ii) = 0
    ...
    DO WHILE (INDEX(parser%input_line,"_atom_site_")/=0)
       ii          = ii + 1
       IF(.NOT.(ii<=20))CALL cp__a("topology_cif.F",220)
    ...
    DO ii = 1, natom
       ...
       DO jj = ii+1,natom
          ...
          IF(.NOT.(check))CALL cp__a("topology_cif.F",290)
       END DO
    END DO
    ...
    END IF
    IF(.NOT.found)&
       CALL cp_warn(cp__l("topology_cif.F",310), "The fields (_symmetry_equiv_pos_as_xyz) or "//&
    ...
    IF (found) THEN
       ...
       DO WHILE ((INDEX(parser%input_line,"loop_")==0).AND.(parser%input_line(1:1)/="_"))
          ...
          END IF
          IF(.NOT.(iln1/=0))CALL cp__a("topology_cif.F",330)
          IF(.NOT.(iln2/=iln1))CALL cp__a("topology_cif.F",331)
          IF(.NOT.(iln3/=iln2))CALL cp__a("topology_cif.F",332)
          CALL initf(3)
