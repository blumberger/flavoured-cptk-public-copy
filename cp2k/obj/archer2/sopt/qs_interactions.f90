# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_interactions.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_interactions.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_interactions.F" 2
...
!> \version 1.0
! *****************************************************************************
  SUBROUTINE init_interaction_radii(qs_control,atomic_kind_set,qs_kind_set)
    ...
    INTEGER :: handle, i, iexp_ppl, ikind, ip, iprj_ppnl, j, l, lppl, lppnl, &
      lprj, lprj_ppnl, maxl, maxnset, nexp_lpot, nexp_lsd, nexp_ppl, nkind
    INTEGER, DIMENSION(:), POINTER           :: nct_lpot, nct_lsd, nprj, &
                                                nprj_ppnl
    ...
    NULLIFY (all_potential, gth_potential, orb_basis_set, aux_basis_set, soft_basis, &
             lri_basis, paw_proj_set)
    ...
    CALL get_qs_kind_set(qs_kind_set, maxnset=maxnset)
    ...
    DO ikind=1,nkind
      ...
      CALL get_qs_kind(qs_kind_set(ikind),basis_set=lri_basis,basis_type="LRI")
      CALL get_qs_kind(qs_kind_set(ikind),soft_basis_set=soft_basis)
...
! *****************************************************************************
  SUBROUTINE write_pgf_orb_radii(basis,atomic_kind_set,qs_kind_set,subsys_section)
    ...
    ELSE IF ( bas(1:3) == "LRI" ) THEN
      bas="LOCAL RI"
    ELSE
      CALL cp__b("qs_interactions.F",617,"Undefined basis set type")
    END IF
    ...
    IF (output_unit>0) THEN
      ...
      DO ikind=1,nkind
        ...
        ELSE IF ( bas(1:3) == "LOC" ) THEN
          ...
          END IF
        ELSE
          CALL cp__b("qs_interactions.F",653,"Undefined basis set type")
        END IF
    ...
    IF (output_unit>0) THEN
      ...
      DO ikind=1,nkind
        ...
        ELSE IF ( bas(1:3) == "LOC" ) THEN
          ...
          END IF
        ELSE
          CALL cp__b("qs_interactions.F",698,"Undefined basis set type")
        END IF
        IF(ASSOCIATED(aux_basis_set).OR.ASSOCIATED(orb_basis_set)) THEN
    ...
    IF (output_unit>0) THEN
      ...
      DO ikind=1,nkind
        ...
        ELSE IF ( bas(1:3) == "LOC" ) THEN
          ...
          END IF
        ELSE
          CALL cp__b("qs_interactions.F",745,"Undefined basis type")
        END IF
...
! *****************************************************************************
  SUBROUTINE write_ppl_radii(atomic_kind_set,qs_kind_set,subsys_section)
    ...
    logger => cp_get_default_logger()
    output_unit = cp_print_key_unit_nr(logger,subsys_section,&
         "PRINT%RADII/GTH_PPL_RADII",extension=".Log")
    CALL section_vals_val_get(subsys_section,"PRINT%RADII%UNIT",c_val=unit_str)
    conv = cp_unit_from_cp2k(1.0_dp,TRIM(unit_str))
    IF (output_unit>0) THEN
      nkind = SIZE(atomic_kind_set)
...
! *****************************************************************************
  SUBROUTINE write_ppnl_radii(atomic_kind_set,qs_kind_set,subsys_section)
    ...
    logger => cp_get_default_logger()
    output_unit = cp_print_key_unit_nr(logger,subsys_section,&
         "PRINT%RADII/GTH_PPNL_RADII",extension=".Log")
    CALL section_vals_val_get(subsys_section,"PRINT%RADII%UNIT",c_val=unit_str)
    conv = cp_unit_from_cp2k(1.0_dp,TRIM(unit_str))
    IF (output_unit>0) THEN
      nkind = SIZE(atomic_kind_set)
