# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_analysis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_analysis.F"
!-----------------------------------------------------------------------------!
...
MODULE tmc_analysis
  ...
  USE tmc_analysis_types,              ONLY: &
       ana_type_default, ana_type_ice, ana_type_sym_xyz, atom_pairs_type, &
       dipole_moment_type, pair_correl_type, search_pair_in_list, &
       tmc_ana_density_create, tmc_ana_density_file_name, &
       tmc_ana_dipole_analysis_create, tmc_ana_dipole_moment_create, &
       tmc_ana_displacement_create, tmc_ana_env_create, &
       tmc_ana_pair_correl_create, tmc_ana_pair_correl_file_name, &
       tmc_analysis_env
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_analysis.F" 2
...
! *****************************************************************************
  SUBROUTINE tmc_read_ana_input(tmc_ana_section, tmc_ana)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'tmc_read_ana_input', &
    ...
    IF(.NOT.(ASSOCIATED(tmc_ana_section)))CALL cp__a("tmc/tmc_analysis.F",104)
    IF(.NOT.(.NOT.ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_analysis.F",105)
    ...
    IF(explicit) THEN
      ...
      IF(explicit_key)THEN
        ...
        IF(SIZE(i_arr_tmp(:)).EQ.3)THEN
          IF(ANY(i_arr_tmp(:).LE.0))&
             CALL cp_abort(cp__l("tmc/tmc_analysis.F",127), "The amount of intervals in each "//&
                  "direction has to be greater than 0.")
          nr_bins(:) = i_arr_tmp(:)
        ELSE IF(SIZE(i_arr_tmp(:)).EQ.1) THEN
          IF(ANY(i_arr_tmp(:).LE.0))&
             CALL cp__b("tmc/tmc_analysis.F",132,"The amount of intervals has to be greater than 0.")
          nr_bins(:) = i_arr_tmp(1)
        ELSE IF(SIZE(i_arr_tmp(:)).EQ.0) THEN
          nr_bins(:) = 1
        ELSE
          CALL cp__b("tmc/tmc_analysis.F",137,"unknown amount of dimentions for the binning.")
        END IF
      ...
      IF(explicit_key)THEN
        ...
        ELSE 
          CALL cp_abort(cp__l("tmc/tmc_analysis.F",162),&
      ...
      IF(explicit_key)THEN
        ...
        CASE DEFAULT
          CALL cp__w("tmc/tmc_analysis.F",187,'unknown analysis type "'//TRIM(c_tmp)//'" specified. Set to default.')
          tmc_ana%dip_ana%ana_type = ana_type_default
        END SELECT
      END IF
...
! *****************************************************************************
  SUBROUTINE analysis_init(ana_env, nr_dim)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'analysis_init', &
      routineP = moduleN//':'//routineN

    CHARACTER(LEN=default_path_length)       :: tmp_cell_file, tmp_dip_file, &
                                                tmp_pos_file

    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",218)
    IF(.NOT.(nr_dim>0))CALL cp__a("tmc/tmc_analysis.F",219)
...
! *****************************************************************************
  SUBROUTINE analysis_restart_print(ana_env)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'analysis_restart_print', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",289)
    IF(.NOT.(ASSOCIATED(ana_env%last_elem)))CALL cp__a("tmc/tmc_analysis.F",290)
    ...
    IF(l_tmp)THEN
      WRITE(file_ptr)ana_env%pair_correl%conf_counter, &
               ...
               ana_env%pair_correl%step_lenght, &
...
! *****************************************************************************
  SUBROUTINE analysis_restart_read(ana_env, elem)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'analysis_restart_read', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",390)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",391)
    IF(.NOT.ana_env%restart) RETURN
    ...
    IF(l_tmp) THEN
      ...
      CALL open_file(file_name=file_name_tmp, file_status="OLD", file_form="UNFORMATTED", &
                   file_action="READ", unit_number=file_ptr) 
      READ(file_ptr) temp
      IF(.NOT.(ana_env%temperature.EQ.temp))CALL cp__a("tmc/tmc_analysis.F",408)
      ...
      ! then the variables for each calculation type
      READ(file_ptr) l_tmp
      IF(.NOT.(ASSOCIATED(ana_env%density_3d).EQV.l_tmp))CALL cp__a("tmc/tmc_analysis.F",415)
      IF(l_tmp)THEN
      ...
      READ(file_ptr) l_tmp
      IF(.NOT.(ASSOCIATED(ana_env%pair_correl).EQV.l_tmp))CALL cp__a("tmc/tmc_analysis.F",428)
      IF(l_tmp)THEN
      ...
      READ(file_ptr) l_tmp
      IF(.NOT.(ASSOCIATED(ana_env%dip_mom).EQV.l_tmp))CALL cp__a("tmc/tmc_analysis.F",438)
      IF(l_tmp)THEN
      ...
      READ(file_ptr) l_tmp
      IF(.NOT.(ASSOCIATED(ana_env%dip_ana).EQV.l_tmp))CALL cp__a("tmc/tmc_analysis.F",446)
      IF(l_tmp)THEN
      ...
      READ(file_ptr) l_tmp
      IF(.NOT.(ASSOCIATED(ana_env%displace).EQV.l_tmp))CALL cp__a("tmc/tmc_analysis.F",458)
      IF(l_tmp)THEN
...
! *****************************************************************************
  SUBROUTINE do_tmc_analysis(elem, ana_env)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",491)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",492)
...
! *****************************************************************************
  SUBROUTINE finalize_tmc_analysis(ana_env)
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",611)
...
! *****************************************************************************
  SUBROUTINE analyze_file_configurations(start_id, end_id, dir_ind, &
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",665)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_analysis.F",666)
...
! *****************************************************************************
  SUBROUTINE calc_density_3d(elem, weight, atoms, ana_env)
    ...
    CHARACTER(LEN=default_path_length)       :: file_name, file_name_tmp
    INTEGER                                  :: atom, bin_x, bin_y, bin_z, &
                                                file_ptr, handle
    LOGICAL                                  :: flag
    REAL(KIND=dp)                            :: mass_total, r_tmp, vol_cell, &
                                                vol_sub_box
    REAL(KIND=dp), DIMENSION(3)              :: atom_pos, cell_size, &
                                                interval_size
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",761)
    IF(.NOT.(ASSOCIATED(elem%pos)))CALL cp__a("tmc/tmc_analysis.F",762)
    IF(.NOT.(weight.GT.0))CALL cp__a("tmc/tmc_analysis.F",763)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_analysis.F",764)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",765)
    IF(.NOT.(ASSOCIATED(ana_env%cell)))CALL cp__a("tmc/tmc_analysis.F",766)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d)))CALL cp__a("tmc/tmc_analysis.F",767)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d%sum_density)))CALL cp__a("tmc/tmc_analysis.F",768)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d%sum_dens2)))CALL cp__a("tmc/tmc_analysis.F",769)
    ...
    DO atom=1, SIZE(elem%pos), ana_env%dim_per_elem
      ...
      IF(.NOT.(bin_x.GT.0.AND.bin_y.GT.0.AND.bin_z.GT.0))CALL cp__a("tmc/tmc_analysis.F",824)
      IF(.NOT.(bin_x.LE.SIZE(ana_env%density_3d%sum_density(:,1,1))))CALL cp__a("tmc/tmc_analysis.F",825)
      IF(.NOT.(bin_y.LE.SIZE(ana_env%density_3d%sum_density(1,:,1))))CALL cp__a("tmc/tmc_analysis.F",826)
      IF(.NOT.(bin_z.LE.SIZE(ana_env%density_3d%sum_density(1,1,:))))CALL cp__a("tmc/tmc_analysis.F",827)
    ...
    IF(.NOT.(ABS(r_tmp).LT.1E-5))CALL cp__a("tmc/tmc_analysis.F",843)
...
! *****************************************************************************
  SUBROUTINE print_density_3d(ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: fmt_my = '(T2,A,"| ",A,T41,A40)', &
      plabel = "TMC_ANA", routineN = 'print_density_3d', &
      routineP = moduleN//':'//routineN

    CHARACTER(LEN=default_path_length)       :: file_name, file_name_vari
    INTEGER                                  :: bin_x, bin_y, bin_z, &
                                                file_ptr_dens, file_ptr_vari, &
                                                handle, i, j, k
    REAL(KIND=dp), DIMENSION(3)              :: cell_size, interval_size

    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",922)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d)))CALL cp__a("tmc/tmc_analysis.F",923)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d%sum_density)))CALL cp__a("tmc/tmc_analysis.F",924)
    IF(.NOT.(ASSOCIATED(ana_env%density_3d%sum_dens2)))CALL cp__a("tmc/tmc_analysis.F",925)
...
! *****************************************************************************
  SUBROUTINE ana_pair_correl_init(ana_pair_correl, atoms, cell)
    ...
    INTEGER                                  :: counter, f_n, handle, list, &
                                                list_ind, s_n
    REAL(KIND=dp), DIMENSION(3)              :: cell_size
    TYPE(atom_pairs_type), DIMENSION(:), &
      POINTER                                :: pairs_tmp

    IF(.NOT.(ASSOCIATED(ana_pair_correl)))CALL cp__a("tmc/tmc_analysis.F",1037)
    IF(.NOT.(.NOT.ASSOCIATED(ana_pair_correl%g_r)))CALL cp__a("tmc/tmc_analysis.F",1038)
    IF(.NOT.(.NOT.ASSOCIATED(ana_pair_correl%pairs)))CALL cp__a("tmc/tmc_analysis.F",1039)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_analysis.F",1040)
    IF(.NOT.(SIZE(atoms).GT.1))CALL cp__a("tmc/tmc_analysis.F",1041)
    IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("tmc/tmc_analysis.F",1042)
    ...
    END IF
    ana_pair_correl%step_lenght = MAXVAL(cell_size(:))/2.0_dp/&
...
! *****************************************************************************
  SUBROUTINE calc_paircorrelation(elem, weight, atoms, ana_env)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",1112)
    IF(.NOT.(ASSOCIATED(elem%pos)))CALL cp__a("tmc/tmc_analysis.F",1113)
    IF(.NOT.(ALL(elem%box_scale(:).GT.0.0_dp)))CALL cp__a("tmc/tmc_analysis.F",1114)
    IF(.NOT.(weight.GT.0))CALL cp__a("tmc/tmc_analysis.F",1115)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_analysis.F",1116)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1117)
    IF(.NOT.(ASSOCIATED(ana_env%cell)))CALL cp__a("tmc/tmc_analysis.F",1118)
    IF(.NOT.(ASSOCIATED(ana_env%pair_correl)))CALL cp__a("tmc/tmc_analysis.F",1119)
    IF(.NOT.(ASSOCIATED(ana_env%pair_correl%g_r)))CALL cp__a("tmc/tmc_analysis.F",1120)
    IF(.NOT.(ASSOCIATED(ana_env%pair_correl%pairs)))CALL cp__a("tmc/tmc_analysis.F",1121)
    ...
    first_elem_loop:DO i=1, SIZE(elem%pos), ana_env%dim_per_elem
      second_elem_loop:DO j=i+3, SIZE(elem%pos), ana_env%dim_per_elem
        ...
        ind = CEILING(dist/ana_env%pair_correl%step_lenght)
        IF(ind.LE.ana_env%pair_correl%nr_bins)THEN
          ...
          IF(.NOT.(pair_ind.GT.0))CALL cp__a("tmc/tmc_analysis.F",1138)
...
! *****************************************************************************
  SUBROUTINE print_paircorrelation(ana_env)
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1169)
    IF(.NOT.(ASSOCIATED(ana_env%pair_correl)))CALL cp__a("tmc/tmc_analysis.F",1170)
    ...
    DO pair=1, SIZE(ana_env%pair_correl%pairs)
      ...
      WRITE(file_ptr,*)"# using a bin size of ",&
        ana_env%pair_correl%step_lenght*au2a,&
        "[A] (for Vol changes: refering to the reference cell)"
      DO bin=1, ana_env%pair_correl%nr_bins
        voldr = 4.0/3.0* PI* ana_env%pair_correl%step_lenght**3 * &
                  (REAL(bin,KIND=dp)**3-REAL(bin-1,KIND=dp)**3)
      WRITE(file_ptr,*) (bin-0.5)*ana_env%pair_correl%step_lenght*au2a , &
...
! *****************************************************************************
  SUBROUTINE ana_dipole_moment_init(ana_dip_mom, atoms)
    ...
    IF(.NOT.(ASSOCIATED(ana_dip_mom)))CALL cp__a("tmc/tmc_analysis.F",1241)
    IF(.NOT.(ASSOCIATED(ana_dip_mom%charges_inp)))CALL cp__a("tmc/tmc_analysis.F",1242)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_analysis.F",1243)
...
! *****************************************************************************
  SUBROUTINE calc_dipole_moment(elem, weight, ana_env)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",1285)
    IF(.NOT.(ASSOCIATED(elem%pos)))CALL cp__a("tmc/tmc_analysis.F",1286)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1287)
    IF(.NOT.(ASSOCIATED(ana_env%dip_mom)))CALL cp__a("tmc/tmc_analysis.F",1288)
    IF(.NOT.(ASSOCIATED(ana_env%dip_mom%charges)))CALL cp__a("tmc/tmc_analysis.F",1289)
...
! *****************************************************************************
  SUBROUTINE print_dipole_moment(ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'print_dipole_moment', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE calc_dipole_analysis(elem, weight, ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'calc_dipole_analysis', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",1363)
    IF(.NOT.(ASSOCIATED(elem%dipole)))CALL cp__a("tmc/tmc_analysis.F",1364)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1365)
    IF(.NOT.(ASSOCIATED(ana_env%dip_ana)))CALL cp__a("tmc/tmc_analysis.F",1366)
...
! *****************************************************************************
  SUBROUTINE print_act_dipole_analysis(elem, ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'print_act_dipole_analysis', &
      routineP = moduleN//':'//routineN
    ...
    CASE DEFAULT
      CALL cp__w("tmc/tmc_analysis.F",1465,'unknown analysis type "'//cp_to_string(ana_env%dip_ana%ana_type)//'" used.')
    END SELECT
...
! *****************************************************************************
  SUBROUTINE print_dipole_analysis(ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: fmt_my = '(T2,A,"| ",A,T41,A40)', &
      plabel = "TMC_ANA", routineN = 'print_dipole_analysis', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1535)
    IF(.NOT.(ASSOCIATED(ana_env%dip_ana)))CALL cp__a("tmc/tmc_analysis.F",1536)
...
! *****************************************************************************
  SUBROUTINE calc_displacement(elem, ana_env)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_analysis.F",1626)
    IF(.NOT.(ASSOCIATED(elem%pos)))CALL cp__a("tmc/tmc_analysis.F",1627)
    IF(.NOT.(ASSOCIATED(ana_env)))CALL cp__a("tmc/tmc_analysis.F",1628)
    IF(.NOT.(ASSOCIATED(ana_env%displace)))CALL cp__a("tmc/tmc_analysis.F",1629)
    IF(.NOT.(ASSOCIATED(ana_env%last_elem)))CALL cp__a("tmc/tmc_analysis.F",1630)
...
! *****************************************************************************
  SUBROUTINE print_average_displacement(ana_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: fmt_my = '(T2,A,"| ",A,T41,A40)', &
      plabel = "TMC_ANA", routineN = 'print_average_displacement', &
      routineP = moduleN//':'//routineN
