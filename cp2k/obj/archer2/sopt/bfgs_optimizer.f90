# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/bfgs_optimizer.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/bfgs_optimizer.F"
!-----------------------------------------------------------------------------!
...
# 73 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/bfgs_optimizer.F" 2
...
END INTERFACE
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/bfgs_optimizer.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE geoopt_bfgs(force_env,gopt_param,globenv,geo_section,gopt_env,x0)
    ...
    INTEGER                                  :: handle, hesunit_read, indf, &
                                                ...
                                                ndf, nfree, output_unit
    LOGICAL                                  :: cell_opt, conv, hesrest, &
                                                ionode, shell_present, &
                                                should_stop, use_mod_hes, &
                                                use_rfo
    ...
    CASE (default_ts_method_id)
       CALL cp__b("motion/bfgs_optimizer.F",151,"BFGS method not yet working with DIMER")
    END SELECT
    ...
    IF(ndf > 3000)&
       CALL cp_warn(cp__l("motion/bfgs_optimizer.F",163),&
    ...
    IF (shell_present) THEN
      IF(use_mod_hes)&
         CALL cp_warn(cp__l("motion/bfgs_optimizer.F",186),&
    ...
    IF (use_mod_hes) THEN
       ...
       ELSE
          ...
          CALL cp_gemm("N","T",ndf,ndf,ndf,one,hess_tmp,eigvec_mat,zero,hess_mat)
       ENDIF
    ELSE
       CALL cp_fm_set_all(hess_mat,alpha=zero,beta=one)
    END IF
...
! *****************************************************************************
  SUBROUTINE geoopt_get_step(ndf,eigval,eigvec_mat,hess_tmp,dr,g,para_env,use_rfo)
    ...
    CALL cp_gemm("N","T",ndf,ndf,ndf,one,hess_tmp,eigvec_mat,zero,tmp)
...
! *****************************************************************************
  SUBROUTINE construct_initial_hess(force_env,hess_mat)
    ...
    INTEGER                                  :: i, iat_col, iat_row, iglobal, &
                                                iind, j, jat_row, jglobal, &
                                                jind, k, natom, ncol_local, &
                                                nrow_local, z
