# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_acceptance.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_acceptance.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_acceptance.F" 2
...
! *****************************************************************************
  SUBROUTINE acceptance_check(tree_element, parent_element, tmc_params, &
    ...
    IF(.NOT.(ASSOCIATED(tree_element)))CALL cp__a("tmc/tmc_tree_acceptance.F",97)
    IF(.NOT.(ASSOCIATED(parent_element)))CALL cp__a("tmc/tmc_tree_acceptance.F",98)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_tree_acceptance.F",99)
    IF(.NOT.(temperature.GT.0.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",100)
    IF(.NOT.(rnd_nr.GE.0.0_dp.AND.rnd_nr.LE.1.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",101)
...
CALL cp__b("tmc/tmc_tree_acceptance.F",109,"")
...
!DO NOT DO       RETURN
    ...
    IF(diff_pot_check) THEN
      IF(.NOT.(ASSOCIATED(tmc_params%prior_NMC_acc)))CALL cp__a("tmc/tmc_tree_acceptance.F",181)
...
! *****************************************************************************
  SUBROUTINE swap_acceptance_check(tree_elem, conf1, conf2, tmc_params, accept)
    ...
    IF(.NOT.(ASSOCIATED(tree_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",222)
    IF(.NOT.(tree_elem%rnd_nr.GE.0.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",223)
    IF(.NOT.(ASSOCIATED(conf1)))CALL cp__a("tmc/tmc_tree_acceptance.F",224)
    IF(.NOT.(ASSOCIATED(conf2)))CALL cp__a("tmc/tmc_tree_acceptance.F",225)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_tree_acceptance.F",226)
...
! *****************************************************************************
  SUBROUTINE volume_acceptance_check(parent_elem, new_elem, temperature, &
    ...
    IF(.NOT.(ASSOCIATED(parent_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",297)
    IF(.NOT.(ASSOCIATED(new_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",298)
    IF(.NOT.(temperature.GT.0.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",299)
    IF(.NOT.(rnd_nr.GT.0.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",300)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_tree_acceptance.F",301)
    IF(.NOT.(tmc_params%pressure.GE.0.0_dp))CALL cp__a("tmc/tmc_tree_acceptance.F",302)
...
! *****************************************************************************
  SUBROUTINE check_acceptance_of_depending_subtree_nodes(tree_elem, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",373)
    IF(.NOT.(ASSOCIATED(tree_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",374)
    IF(.NOT.(tree_elem%stat.EQ.status_calculated))CALL cp__a("tmc/tmc_tree_acceptance.F",375)
    IF(.NOT.(ASSOCIATED(tree_elem%parent)))CALL cp__a("tmc/tmc_tree_acceptance.F",376)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(act_elem, parent_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",386)
    ...
    CASE(status_accepted_result, status_rejected_result, &
      ...
      parent_ready = .TRUE.
    CASE DEFAULT
      CALL cp__b("tmc/tmc_tree_acceptance.F",398,"unknown parent element status"//cp_to_string(parent_elem%stat))
    END SELECT
...
! *****************************************************************************
  SUBROUTINE check_and_change_status_of_subtree_elem(act_elem, parent_elem, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",457)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_tree_acceptance.F",458)
    IF(.NOT.(ASSOCIATED(act_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",459)
    IF(.NOT.(ASSOCIATED(parent_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",460)
    IF(.NOT.(act_elem%stat.EQ.status_calculated))CALL cp__a("tmc/tmc_tree_acceptance.F",461)
    ...
    DO WHILE(ASSOCIATED(tmp_gt_list_ptr))
      IF(.NOT.(tmp_gt_list_ptr%gt_elem%stat.NE.status_accepted_result))CALL cp__a("tmc/tmc_tree_acceptance.F",469)
      IF(.NOT.(tmp_gt_list_ptr%gt_elem%stat.NE.status_rejected_result))CALL cp__a("tmc/tmc_tree_acceptance.F",470)
...
! *****************************************************************************
  SUBROUTINE subtree_configuration_stat_change(gt_ptr, stat, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(gt_ptr)))CALL cp__a("tmc/tmc_tree_acceptance.F",533)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_tree_acceptance.F",534)
    ...
    IF(.NOT.gt_ptr%swaped) THEN
      ...
      CASE(status_accepted, status_rejected)
        ptr%stat = stat
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_tree_acceptance.F",553),&
...
! *****************************************************************************
  SUBROUTINE elem_ready_to_check(elem, ready)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",581)
    ...
    CASE(status_calculated, status_accepted_result, status_accepted, status_rejected, status_rejected_result)
       ready = .TRUE.
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_tree_acceptance.F",596),&
...
! *****************************************************************************
  SUBROUTINE check_elements(gt_act_elem, tmc_env, check_done, result_acc)
    ...
    IF(.NOT.(ASSOCIATED(gt_act_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",627)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",628)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",629)
    ...
    CASE(status_accepted, status_rejected)
      ...
      ELSE IF(gt_act_elem%stat .EQ. status_rejected) THEN
        result_acc = .FALSE.
      ELSE
        CALL cp_abort(cp__l("tmc/tmc_tree_acceptance.F",644),&
    ...
    IF(.NOT.check_done) THEN
      ...
      CALL get_subtree_elements_to_check(gt_act_elem=gt_act_elem, elem1=act_element, &
                                         elem2=tmp_element)
      IF(.NOT.(ASSOCIATED(act_element)))CALL cp__a("tmc/tmc_tree_acceptance.F",656)
      IF(.NOT.(ASSOCIATED(tmp_element)))CALL cp__a("tmc/tmc_tree_acceptance.F",657)
      ...
      ELSE
        ...
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_tree_acceptance.F",703),&
...
! *****************************************************************************
  SUBROUTINE tree_update(tmc_env, result_acc, something_updated)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",737)
...
! *****************************************************************************
  FUNCTION ready_for_update_acc_prob(elem) RESULT(ready)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",894)
    ready = .FALSE.
...
! *****************************************************************************
  SUBROUTINE check_elements_for_acc_prob_update(tree_elem, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tree_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",923)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_acceptance.F",924)
...
! *****************************************************************************
  SUBROUTINE update_prob_gt_node_list(reference_list, act_elem, parent_elem, act_parent, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(reference_list%gt_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",1002)
    IF(.NOT.(ASSOCIATED(act_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",1003)
    IF(.NOT.(ASSOCIATED(parent_elem)))CALL cp__a("tmc/tmc_tree_acceptance.F",1004)
