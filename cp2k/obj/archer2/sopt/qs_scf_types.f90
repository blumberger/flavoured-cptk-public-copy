# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_types.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_types.F" 2
...
!NOTE: the following matrices are small and could be used as standard array rather than distributed fm
  ...
  TYPE subspace_env_type
      ...
      TYPE(cp_dbcsr_p_type), DIMENSION(:), POINTER :: p_matrix_store
      TYPE(cp_dbcsr_p_type), DIMENSION(:,:), POINTER :: p_matrix_mix
   ...
   TYPE qs_scf_env_type
      ...
      TYPE(cp_dbcsr_type),  POINTER :: ortho_dbcsr, buf1_dbcsr, buf2_dbcsr
...
! *****************************************************************************
  SUBROUTINE scf_env_retain(scf_env)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_types.F",218)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_types.F",219)
...
! *****************************************************************************
  SUBROUTINE scf_env_did_change(scf_env)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_types.F",244)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_types.F",245)
...
! *****************************************************************************
  SUBROUTINE scf_env_release(scf_env)
    ...
    IF (ASSOCIATED(scf_env)) THEN
       IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_types.F",278)
       scf_env%ref_count = scf_env%ref_count - 1
       IF (scf_env%ref_count < 1) THEN
          ...
          IF (ASSOCIATED(scf_env%p_mix_new)) THEN
             ! we should not end up here, and give back using the pools
             IF(.NOT.(.TRUE.))CALL cp__a("qs_scf_types.F",304)
             CALL cp_dbcsr_deallocate_matrix_set(scf_env%p_mix_new)
          END IF
          IF (ASSOCIATED(scf_env%p_delta)) THEN
             ! we should not end up here, and give back using the pools
             IF(.NOT.(.TRUE.))CALL cp__a("qs_scf_types.F",309)
             CALL cp_dbcsr_deallocate_matrix_set(scf_env%p_delta)
          END IF
...
! *****************************************************************************
 SUBROUTINE krylov_space_create(krylov_space,scf_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(krylov_space)))CALL cp__a("qs_scf_types.F",382)
...
! *****************************************************************************
 SUBROUTINE diag_subspace_env_create(subspace_env,scf_section,ecut)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(subspace_env)))CALL cp__a("qs_scf_types.F",489)
 ...
 SUBROUTINE diag_subspace_env_release(subspace_env)
    ...
    IF(ASSOCIATED(subspace_env)) THEN
       ...
       IF (ASSOCIATED(subspace_env%p_matrix_store)) THEN
           ...
           IF(.NOT.(.TRUE.))CALL cp__a("qs_scf_types.F",544)
           CALL cp_dbcsr_deallocate_matrix_set(subspace_env%p_matrix_store)
       ENDIF
