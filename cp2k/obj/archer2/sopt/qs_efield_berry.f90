# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_berry.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_berry.F"
!-----------------------------------------------------------------------------!
...
# 81 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_berry.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_efield_integrals (qs_env)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_efield_berry.F",155)
    ...
    DO i=1,3
       ...
       CALL cp_dbcsr_init(cosmat(i)%matrix)
       CALL cp_dbcsr_init(sinmat(i)%matrix)

       CALL cp_dbcsr_copy(cosmat(i)%matrix,matrix_s(1)%matrix,'COS MAT')
       CALL cp_dbcsr_copy(sinmat(i)%matrix,matrix_s(1)%matrix,'SIN MAT')
       CALL cp_dbcsr_set(cosmat(i)%matrix,0.0_dp)
       CALL cp_dbcsr_set(sinmat(i)%matrix,0.0_dp)
...
! *****************************************************************************
  SUBROUTINE qs_efield_derivatives(qs_env,just_energy,calculate_forces)
    ...
    INTEGER :: atom_a, atom_b, handle, i, ia, iatom, icol, idir, ikind, &
      inode, irow, iset, ispin, j, jatom, jkind, jset, ldab, ldsa, ldsb, &
      lsab, n1, n2, nao, natom, ncoa, ncob, nkind, nmo, nseta, nsetb, sgfa, &
      sgfb
    ...
    REAL(dp) :: charge, ci(3), cqi(3), dab, dd, ener_field, f0, fab, &
      fieldpol(3), focc, fpolvec(3), hmat(3,3), occ, qi(3), ti(3)
    ...
    IF (use_virial) THEN
       CALL cp__b("qs_efield_berry.F",271,"Stress tensor for periodic E-field not implemented")
    END IF
    ...
    DO ispin = 1,dft_control%nspins
       ...
       IF (.NOT.uniform) THEN
          CALL cp__b("qs_efield_berry.F",321,"Berry phase moments for non uniform MOs' occupation numbers not implemented")
       END IF
    END DO
    ...
    IF(calculate_forces)THEN
       ...
       DO ispin=1,dft_control%nspins
          ...
          CALL cp_dbcsr_init(tempmat(1,ispin)%matrix)
          CALL cp_dbcsr_init(tempmat(2,ispin)%matrix)
          CALL cp_dbcsr_copy(tempmat(1,ispin)%matrix,matrix_s(1)%matrix,'TEMPMAT')
          CALL cp_dbcsr_copy(tempmat(2,ispin)%matrix,matrix_s(1)%matrix,'TEMPMAT')
          CALL cp_dbcsr_set(tempmat(1,ispin)%matrix,0.0_dp)
          CALL cp_dbcsr_set(tempmat(2,ispin)%matrix,0.0_dp)
       END DO
       ! integration
    ...
    DO idir=1,3
       ...
       IF(ABS(fpolvec(idir)).GT.1.0E-12_dp)THEN
          ...
          zdet = one
          DO ispin = 1, dft_control%nspins
             CALL cp_cfm_add_fm(zero,eigrmat(ispin)%matrix,one,op_fm_set(1,ispin)%matrix)
             CALL cp_cfm_add_fm(one,eigrmat(ispin)%matrix,-zone,op_fm_set(2,ispin)%matrix)
             CALL cp_cfm_set_all(inv_mat(ispin)%matrix,zero,one)
          ...
          IF(calculate_forces)THEN
             ...
             DO WHILE (neighbor_list_iterate(nl_iterator)==0)
                ...
                DO ispin=1,dft_control%nspins
                   ...
                   IF(.NOT.(found))CALL cp__a("qs_efield_berry.F",562)
                   CALL cp_dbcsr_get_block_p(matrix=tempmat(2,ispin)%matrix,&
                        row=irow,col=icol,BLOCK=iblock,found=found)
                   IF(.NOT.(found))CALL cp__a("qs_efield_berry.F",565)
                   n1 = SIZE(rblock,1)
                   n2 = SIZE(rblock,2)
                   IF(.NOT.(SIZE(iblock,1)==n1))CALL cp__a("qs_efield_berry.F",568)
                   IF(.NOT.(SIZE(iblock,2)==n2))CALL cp__a("qs_efield_berry.F",569)
                   IF(.NOT.(lsab>=n1))CALL cp__a("qs_efield_berry.F",570)
                   IF(.NOT.(lsab>=n2))CALL cp__a("qs_efield_berry.F",571)
                   IF (iatom <= jatom) THEN
                      DO i=1,3
    ...
    IF(calculate_forces)THEN
       ...
       IF( ABS(efield%field_energy-ener_field) > pi*ABS(SUM(fpolvec))) THEN
          CALL cp__w("qs_efield_berry.F",627,"Large change of e-field energy detected. Correct for non-smooth energy surface")
       END IF
    ...
    IF(calculate_forces)THEN
       ...
       END DO
       CALL cp_dbcsr_deallocate_matrix_set(tempmat)
    END IF
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE qs_dispfield_derivatives(qs_env,just_energy,calculate_forces)
    ...
    INTEGER :: handle, i, ia, iatom, icol, idir, ikind, inode, irow, iset, &
      ispin, jatom, jkind, jset, ldab, ldsa, ldsb, lsab, n1, n2, nao, natom, &
      ncoa, ncob, nkind, nmo, nseta, nsetb, sgfa, sgfb
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: atom_of_kind
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, lb_max, &
                                                lb_min, npgfa, npgfb, nsgfa, &
                                                nsgfb
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgfa, first_sgfb
    LOGICAL                                  :: found, uniform, use_virial
    REAL(dp) :: charge, ci(3), cqi(3), dab, dd, di(3), ener_field, fab, &
      fieldpol(3), focc, hmat(3,3), occ, omega, qi(3), rlog(3), zlog(3)
    REAL(dp), DIMENSION(3)                   :: forcea, forceb, kvec, ra, &
                                                rab, rb, ria
    REAL(dp), DIMENSION(:, :), POINTER       :: cosab, iblock, rblock, sinab, &
                                                work
    ...
    ! disable stress calculation
    IF (use_virial) THEN
       CALL cp__b("qs_efield_berry.F",774,"Stress tensor for periodic D-field not implemented")
    END IF
    ...
    DO ispin = 1,dft_control%nspins
       CALL get_mo_set(mo_set=mos(ispin)%mo_set,maxocc=occ,uniform_occupation=uniform)
       IF (.NOT.uniform) THEN
          CALL cp__b("qs_efield_berry.F",821,"Berry phase moments for non uniform MOs' occupation numbers not implemented")
       END IF
    END DO
       ...
       IF(calculate_forces)THEN
          ...
          DO WHILE (neighbor_list_iterate(nl_iterator)==0)
             ...
             DO ispin=1,dft_control%nspins
                ...
                CALL cp_dbcsr_get_block_p(matrix=tempmat(1,ispin)%matrix,&
                     row=irow,col=icol,BLOCK=rblock,found=found)
                IF(.NOT.(found))CALL cp__a("qs_efield_berry.F",1061)
                CALL cp_dbcsr_get_block_p(matrix=tempmat(2,ispin)%matrix,&
                     row=irow,col=icol,BLOCK=iblock,found=found)
                IF(.NOT.(found))CALL cp__a("qs_efield_berry.F",1064)
                n1 = SIZE(rblock,1)
                n2 = SIZE(rblock,2)
                IF(.NOT.(SIZE(iblock,1)==n1))CALL cp__a("qs_efield_berry.F",1067)
                IF(.NOT.(SIZE(iblock,2)==n2))CALL cp__a("qs_efield_berry.F",1068)
                IF(.NOT.(lsab>=n1))CALL cp__a("qs_efield_berry.F",1069)
                IF(.NOT.(lsab>=n2))CALL cp__a("qs_efield_berry.F",1070)
                IF (iatom <= jatom) THEN
                   DO i=1,3
                      forcea(i) = forcea(i) + SUM(rblock(1:n1,1:n2)*dsint(i,1)%block(1:n1,1:n2)) &
                                            - SUM(iblock(1:n1,1:n2)*dcost(i,1)%block(1:n1,1:n2))
    ...
    IF(calculate_forces)THEN
       ...
       IF( ABS(efield%field_energy-ener_field) > pi*ABS(SUM(hmat)) ) THEN
          CALL cp__w("qs_efield_berry.F",1124,"Large change of e-field energy detected. Correct for non-smooth energy surface")
       END IF
       efield%field_energy = ener_field
       efield%polarisation(:) = cqi(:)
    END IF
    ...
    ener_field = 0.0_dp
    DO i=1,3
       ener_field = ener_field + (fieldpol(i) - 2._dp*twopi*ci(i))**2
    END DO
    energy%efield = 0.25_dp*omega/twopi * ener_field

    IF(.NOT.just_energy) THEN
       DO i=1,3
          di(i) = -omega * (fieldpol(i) - 2._dp*twopi*ci(i))
       END DO
       ! Add the result to mo_derivativs
       DO ispin=1, dft_control%nspins
