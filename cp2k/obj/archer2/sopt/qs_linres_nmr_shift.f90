# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_nmr_shift.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_nmr_shift.F"
!-----------------------------------------------------------------------------!
...
# 80 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_nmr_shift.F" 2
...
! *****************************************************************************
  SUBROUTINE nmr_shift_gapw(nmr_env,current_env,qs_env,iB,idir)
    ...
    INTEGER :: handle, ia, iat, iatom, idir2_1, idir3_1, ikind, ir, ira, &
      ispin, j, jatom, mepos, n_nics, na, natom, natom_local, natom_tot, &
      nkind, nnics_local, nr, nra, nspins, num_pe, output_unit
...
!
    ...
    IF(.NOT.(ASSOCIATED(shift_pw_rspace%pw)))CALL cp__a("qs_linres_nmr_shift.F",558)
    ...
    IF(.NOT.(success))CALL cp__a("qs_linres_nmr_shift.F",581)
...
!
    ...
    !
    NULLIFY(cs_atom_list,chemical_shift,chemical_shift_nics,r_nics)
    IF(.NOT.(ASSOCIATED(shift_pw_gspace%pw)))CALL cp__a("qs_linres_nmr_shift.F",658)
    !
    CALL get_nmr_env(nmr_env=nmr_env, cs_atom_list=cs_atom_list,&
                     chemical_shift=chemical_shift,&
                     chemical_shift_nics=chemical_shift_nics,&
...
! *****************************************************************************
  SUBROUTINE nmr_shift_print(nmr_env,current_env,qs_env)
    ...
    INTEGER                                  :: iatom, ir, n_nics, nat_print, &
                                                natom, output_unit, &
                                                unit_atoms, unit_nics
    INTEGER, DIMENSION(:), POINTER           :: cs_atom_list
