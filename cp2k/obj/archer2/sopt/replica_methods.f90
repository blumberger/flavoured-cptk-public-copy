# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_methods.F"
!-----------------------------------------------------------------------------!
...
# 65 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE rep_env_create(rep_env, para_env, input, input_declaration, nrep, prep,&
    ...
    INTEGER :: comm_cart, comm_f, comm_inter_rep, forcedim, i, i0, ierr, ip, &
      ir, irep, lp, my_prep, new_env_id, nparticle, nrep_local, unit_nr
    ...
    IF(.NOT.(.NOT.ASSOCIATED(rep_env)))CALL cp__a("replica_methods.F",117)
    IF(.NOT.(ASSOCIATED(input_declaration)))CALL cp__a("replica_methods.F",118)
    ...
    IF (ASSOCIATED(rep_env)) THEN
       ...
       IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",222)
       ...
       IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",233)
       ...
       DO irep=1,nrep
          ...
          END IF
       END DO
       IF(.NOT.(nrep_local==SIZE(rep_env%local_rep_indices)))CALL cp__a("replica_methods.F",259)
       ...
       IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",273)
       DO i=1,nrep
          IF (rep_env%rep_is_local(i)) THEN
             CALL get_pos(rep_env%f_env_id,rep_env%r(:,i),rep_env%ndim,ierr)
             IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",277)
          END IF
       END DO
    END IF
    IF (ASSOCIATED(rep_env)) THEN
       ...
       IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",284)
    END IF
...
! *****************************************************************************
  SUBROUTINE rep_env_init_low(rep_env_id, ierr)
    ...
    IF(.NOT.ASSOCIATED(rep_env))&
       CALL cp__b("replica_methods.F",313,"could not find rep_env with id_nr"//cp_to_string(rep_env_id))
    ...
    IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",366)
...
! *****************************************************************************
  SUBROUTINE rep_env_calc_e_f(rep_env,calc_f)
    ...
    IF(.NOT.(ASSOCIATED(rep_env)))CALL cp__a("replica_methods.F",390)
    IF(.NOT.(rep_env%ref_count>0))CALL cp__a("replica_methods.F",391)
    my_calc_f=0
    IF (PRESENT(calc_f)) THEN
       IF (calc_f) my_calc_f=1
    END IF
    CALL rep_env_calc_e_f_low(rep_env%id_nr,my_calc_f,ierr)
    IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",397)
...
! *****************************************************************************
  SUBROUTINE rep_env_calc_e_f_int(rep_env,calc_f)
    ...
    IF(.NOT.(ASSOCIATED(rep_env)))CALL cp__a("replica_methods.F",458)
    IF(.NOT.(rep_env%ref_count>0))CALL cp__a("replica_methods.F",459)
    ...
    CALL f_env_rm_defaults(f_env,ierr)
    IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",470)
    DO i=1,SIZE(rep_env%local_rep_indices)
       ...
       IF (rep_env%sync_v) THEN
          ...
          IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",476)
       END IF
       ...
       IF (rep_env%keep_wf_history) THEN
          ...
          CALL set_qs_env(qs_env,&
               wf_history=rep_env%wf_history(i)%wf_history)
          CALL f_env_rm_defaults(f_env,ierr)
          IF(.NOT.(ierr==0))CALL cp__a("replica_methods.F",488)
       END IF
