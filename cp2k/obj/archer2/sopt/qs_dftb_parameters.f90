# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_parameters.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_parameters.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_parameters.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_dftb_param_init (atomic_kind_set,qs_kind_set,dftb_control,dftb_potential,&
    ...
    CHARACTER(LEN=default_string_length)     :: iname, jname, name_a, name_b, &
                                                skfn
    INTEGER :: ikind, isp, jkind, k, l, l1, l2, llm, lmax, lmax_a, lmax_b, &
      lp, m, n_urpoly, ngrd, nkind, output_unit, runit, spdim, z
    LOGICAL                                  :: at_end, found, ldum, search, &
                                                sklist
    ...
    DO ikind = 1, nkind
      ...
      DO jkind = 1, nkind
        ...
        END IF
        IF(.NOT.found)&
           CALL cp_abort(cp__l("qs_dftb_parameters.F",212),&
...
! This is creative guessing!
      ...
      IF ( lmax < 0 ) THEN
        ...
        DO l=0,3
          SELECT CASE (l)
            CASE DEFAULT
              CALL cp__b("qs_dftb_parameters.F",313,"")
            CASE (0)
              lp = 10
            CASE (1)
              lp = 7
      ...
      END IF
      IF ( lmax > 2 ) THEN
        CALL cp_abort(cp__l("qs_dftb_parameters.F",332),"Maximum L allowed is d. "//&
...
1   CONTINUE
    CALL cp__b("qs_dftb_parameters.F",705,"")
