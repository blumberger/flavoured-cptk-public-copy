# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_occupation.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_occupation.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_occupation.F" 2
...
! *****************************************************************************
   SUBROUTINE set_mo_occupation_3(mo_array,smear)
    ...
    IF(.NOT.(ASSOCIATED(mo_array)))CALL cp__a("qs_mo_occupation.F",78)
    IF(.NOT.((SIZE(mo_array) == 2)))CALL cp__a("qs_mo_occupation.F",79)
    ...
    ! this is not a real problem, but the temperature might be a bit large
        IF(is_large)&
           CALL cp__w("qs_mo_occupation.F",121,"Fermi-Dirac smearing includes the first MO")

    is_large=ABS(MINVAL(all_occ))> smear%eps_fermi_dirac
    IF(is_large)&
       CALL cp_warn(cp__l("qs_mo_occupation.F",125),&
    ...
    IF(is_large)&
       CALL cp__w("qs_mo_occupation.F",132,"Total number of electrons is not accurate")
...
! *****************************************************************************
  SUBROUTINE set_mo_occupation_2(mo_array,smear,eval_deriv)
    ...
    IF(.NOT.(ASSOCIATED(mo_array)))CALL cp__a("qs_mo_occupation.F",213)
    mo_set_a => mo_array(1)%mo_set
    IF(.NOT.(ASSOCIATED(mo_set_a)))CALL cp__a("qs_mo_occupation.F",215)
    ...
    IF (SIZE(mo_array) == 1) THEN
       ...
       RETURN
    END IF
    IF(.NOT.((SIZE(mo_array) == 2)))CALL cp__a("qs_mo_occupation.F",226)
    mo_set_b => mo_array(2)%mo_set

    IF (smear%do_smear) THEN
       IF (smear%fixed_mag_mom < 0.0_dp) THEN
          IF (smear%fixed_mag_mom /= -1.0_dp) THEN
             IF(.NOT.(.NOT.(PRESENT(eval_deriv))))CALL cp__a("qs_mo_occupation.F",232)
       ...
       ELSE
          ...
          END IF
          IF(.NOT.(.NOT.(PRESENT(eval_deriv))))CALL cp__a("qs_mo_occupation.F",243)
          CALL set_mo_occupation_1(mo_set_a,smear=smear)
    ...
    IF(mo_set_a%nelectron >= mo_set_a%nmo)&
       CALL cp_warn(cp__l("qs_mo_occupation.F",267),&
    ...
    IF((mo_set_b%nelectron >= mo_set_b%nmo).AND. (mo_set_b%nelectron /= mo_set_a%nelectron))&
       CALL cp_warn(cp__l("qs_mo_occupation.F",271),"All beta MOs are occupied. Add more beta MOs to "//&
    ...
    DO i=1,nelec
       ...
       IF (lumo_a > mo_set_a%nmo) THEN
          IF(i /= nelec)&
             CALL cp_warn(cp__l("qs_mo_occupation.F",292),&
                  "All alpha MOs are occupied. Add more alpha MOs to "//&
                  "allow for a higher multiplicity")
          IF (i < nelec) THEN
             lumo_a = lumo_a - 1
       ...
       IF (lumo_b > mo_set_b%nmo) THEN
          IF(lumo_b < lumo_a)&
             CALL cp_warn(cp__l("qs_mo_occupation.F",302),&
    ...
    IF (mo_set_b%homo > mo_set_a%homo) THEN
       CALL cp_warn(cp__l("qs_mo_occupation.F",316),&
            "More beta ("//&
    ...
    IF(multiplicity_new /= multiplicity_old)&
       CALL cp_warn(cp__l("qs_mo_occupation.F",331),&
...
! *****************************************************************************
  SUBROUTINE set_mo_occupation_1(mo_set,smear,eval_deriv,xas_env)
    ...
    IF(.NOT.(ASSOCIATED(mo_set)))CALL cp__a("qs_mo_occupation.F",383)
    IF(.NOT.(ASSOCIATED(mo_set%eigenvalues)))CALL cp__a("qs_mo_occupation.F",384)
    IF(.NOT.(ASSOCIATED(mo_set%occupation_numbers)))CALL cp__a("qs_mo_occupation.F",385)
    ...
    IF(PRESENT(xas_env)) THEN
      ...
      IF(.NOT.(.NOT.is_large))CALL cp__a("qs_mo_occupation.F",407)
    ELSE
    ...
    IF(.NOT.(nmo >= nomo))CALL cp__a("qs_mo_occupation.F",422)
    IF(.NOT.((SIZE(mo_set%occupation_numbers) == nmo)))CALL cp__a("qs_mo_occupation.F",423)
    ...
    IF (PRESENT(eval_deriv)) THEN
      ...
      IF(.NOT.(equal_size))CALL cp__a("qs_mo_occupation.F",432)
    END IF
    ...
    IF (smear%do_smear) THEN
      ...
      CASE (smear_fermi_dirac)
        ...
        ! this is not a real problem, but the temperature might be a bit large
        IF(is_large)&
           CALL cp__w("qs_mo_occupation.F",501,"Fermi-Dirac smearing includes the first MO")
        ...
        IF(is_large)&
           CALL cp_warn(cp__l("qs_mo_occupation.F",512),&
                "Fermi-Dirac smearing includes the last MO => "//&
                "Add more MOs for proper smearing.")
        ...
        IF(is_large)&
           CALL cp__w("qs_mo_occupation.F",519,"Total number of electrons is not accurate")

      CASE (smear_energy_window)
        ! not implemented
        IF(.NOT.(.NOT.PRESENT(eval_deriv)))CALL cp__a("qs_mo_occupation.F",523)
        ...
        IF(e1 <= mo_set%eigenvalues(1))&
           CALL cp__w("qs_mo_occupation.F",528,"Energy window for smearing includes the first MO")

        e2 = mo_set%eigenvalues(mo_set%homo) + 0.5_dp*smear%window_size
        IF(e2 >= mo_set%eigenvalues(nmo))&
           CALL cp_warn(cp__l("qs_mo_occupation.F",532),&
      ...
      CASE (smear_list)
        ...
        IF(.NOT.(equal_size))CALL cp__a("qs_mo_occupation.F",571)
