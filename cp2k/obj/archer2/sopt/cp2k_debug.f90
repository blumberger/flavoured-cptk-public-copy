# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/cp2k_debug.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/cp2k_debug.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/cp2k_debug.F" 2
  ...
  SUBROUTINE cp2k_debug_energy_and_forces(force_env)
    ...
    IF (debug_stress_tensor) THEN
       ...
       CASE (do_stress_numerical, do_stress_diagonal_numer)
          ! Nothing to check
          CALL cp_warn(cp__l("cp2k_debug.F",139), "Numerical stress tensor was requested in "//&
                      "the FORCE_EVAL section. Nothing to debug!")
          skip = .TRUE.
       CASE DEFAULT
          CALL cp_warn(cp__l("cp2k_debug.F",143), "Stress tensor calculation was not enabled in "//&
                       "the FORCE_EVAL section. Nothing to debug!")
          skip = .TRUE.
       END SELECT
       ...
       IF (.NOT.skip) THEN
          ...
          IF (check_failed) THEN
             ...
             IF (stop_on_mismatch) THEN
                CALL cp__b("cp2k_debug.F",210,TRIM(message))
             ELSE
                CALL cp__w("cp2k_debug.F",212,TRIM(message))
             END IF
          END IF
       END IF
    END IF
    ...
    IF (debug_forces) THEN
       ...
       Atom: DO ip=1,np
          ...
          IF (ANY(ABS(err(1:3)) > my_maxerr(1:3))) THEN
             ...
             IF (stop_on_mismatch) THEN
                CALL cp__b("cp2k_debug.F",298,message)
             ELSE
                CALL cp__w("cp2k_debug.F",300,message)
             END IF
          END IF
       END DO Atom
       ! Print summary
