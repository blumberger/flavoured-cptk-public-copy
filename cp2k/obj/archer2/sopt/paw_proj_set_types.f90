# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/paw_proj_set_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/paw_proj_set_types.F"
!-----------------------------------------------------------------------------!
...
# 38 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/paw_proj_set_types.F" 2
...
! *****************************************************************************
  SUBROUTINE build_projector(paw_proj,orb_basis,eps_fit,eps_iso,eps_svd, &
    ...
    IF(.NOT.(ASSOCIATED(paw_proj)))CALL cp__a("paw_proj_set_types.F",275)
    IF(.NOT.(ASSOCIATED(orb_basis)))CALL cp__a("paw_proj_set_types.F",276)
    ...
    paw_proj%maxl = maxl
    IF(.NOT.(.NOT. ASSOCIATED(paw_proj%zisomin)))CALL cp__a("paw_proj_set_types.F",288)
    IF(.NOT.(.NOT. ASSOCIATED(paw_proj%zprjisomin)))CALL cp__a("paw_proj_set_types.F",289)
    IF(.NOT.(.NOT. ASSOCIATED(paw_proj%nprj)))CALL cp__a("paw_proj_set_types.F",290)
    ...
    DO lshell = 0,maxl    ! lshell
       ...
       DO ip = 1, np
          ...
          DO ipp =1,ip-1
             IF(zet(ip)==zet(ipp)) THEN
                CALL cp_abort(cp__l("paw_proj_set_types.F",399),&
       ...
       IF(.NOT.(np > 0))CALL cp__a("paw_proj_set_types.F",461)
...
! *****************************************************************************
  SUBROUTINE get_paw_proj_set(paw_proj_set,cprj,cprj_s,csprj,&
    ...
    ELSE
       CALL cp__b("paw_proj_set_types.F",858,"The pointer  paw_proj_set is not associated")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE set_paw_proj_set(paw_proj_set, rzetprj, rcprj)
    ...
    ELSE
       CALL cp__b("paw_proj_set_types.F",884,"The pointer paw_proj_set is not associated")
    END IF
