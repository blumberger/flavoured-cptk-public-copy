# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_output_handling.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_output_handling.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_output_handling.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_print_key_section_create(print_key_section, name, description, &
    ...
    IF(.NOT.(.NOT.ASSOCIATED(print_key_section)))CALL cp__a("input/cp_output_handling.F",188)
    ...
    IF(.NOT.(check))CALL cp__a("input/cp_output_handling.F",228)
    ...
    DO i_each = 1, SIZE(each_possible_labels)
       ...
       IF (ext_each) THEN
          ...
          IF(.NOT.(check))CALL cp__a("input/cp_output_handling.F",235)
...
! *****************************************************************************
  FUNCTION cp_print_key_should_output(iteration_info,basis_section,&
    ...
    IF(.NOT.(ASSOCIATED(basis_section)))CALL cp__a("input/cp_output_handling.F",357)
    IF(.NOT.(basis_section%ref_count>0))CALL cp__a("input/cp_output_handling.F",358)
    ...
    IF (PRESENT(print_key_path)) THEN
       ...
       END IF
       IF(.NOT.(ASSOCIATED(print_key)))CALL cp__a("input/cp_output_handling.F",378)
       IF(.NOT.(print_key%ref_count>0))CALL cp__a("input/cp_output_handling.F",379)
...
! *****************************************************************************
  FUNCTION cp_printkey_is_on(iteration_info,print_key) RESULT(res)
    ...
    IF(.NOT.(ASSOCIATED(iteration_info)))CALL cp__a("input/cp_output_handling.F",441)
    IF(.NOT.(iteration_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",442)
    IF (.NOT.ASSOCIATED(print_key)) THEN
       res=(iteration_info%print_level > debug_print_level)
    ELSE
       IF(.NOT.(print_key%ref_count>0))CALL cp__a("input/cp_output_handling.F",446)
...
! *****************************************************************************
  FUNCTION cp_printkey_is_iter(iteration_info,print_key,first_time)&
    ...
    IF(.NOT.(ASSOCIATED(iteration_info)))CALL cp__a("input/cp_output_handling.F",478)
    IF(.NOT.(iteration_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",479)
    IF (.NOT.ASSOCIATED(print_key)) THEN
       res=(iteration_info%print_level > debug_print_level)
       first=ALL(iteration_info%iteration(1:iteration_info%n_rlevel)==1)
    ELSE
       IF(.NOT.(print_key%ref_count>0))CALL cp__a("input/cp_output_handling.F",484)
       res   = .FALSE.
       first = .FALSE.
...
! *****************************************************************************
  FUNCTION cp_iter_string(iter_info,print_key,for_file) RESULT(res)
    ...
    IF(.NOT.(ASSOCIATED(iter_info)))CALL cp__a("input/cp_output_handling.F",547)
    IF(.NOT.(iter_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",548)
...
! *****************************************************************************
  SUBROUTINE cp_iterate(iteration_info,last,iter_nr,increment,iter_nr_out)
    ...
    IF(.NOT.(ASSOCIATED(iteration_info)))CALL cp__a("input/cp_output_handling.F",608)
    IF(.NOT.(iteration_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",609)
...
! *****************************************************************************
  SUBROUTINE cp_add_iter_level(iteration_info,level_name,n_rlevel_new)
    ...
    IF(.NOT.(ASSOCIATED(iteration_info)))CALL cp__a("input/cp_output_handling.F",642)
    IF(.NOT.(iteration_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",643)
    found = .FALSE.
    ...
    ELSE
       CALL cp_abort(cp__l("input/cp_output_handling.F",662),&
...
! *****************************************************************************
  SUBROUTINE cp_rm_iter_level(iteration_info,level_name,n_rlevel_att)
    ...
    IF(.NOT.(ASSOCIATED(iteration_info)))CALL cp__a("input/cp_output_handling.F",687)
    IF(.NOT.(iteration_info%ref_count>0))CALL cp__a("input/cp_output_handling.F",688)
    IF (PRESENT(n_rlevel_att)) THEN
       IF(.NOT.(n_rlevel_att==iteration_info%n_rlevel))CALL cp__a("input/cp_output_handling.F",690)
    END IF
    ...
    IF(.NOT.(check))CALL cp__a("input/cp_output_handling.F",696)
    iteration_info%n_rlevel=iteration_info%n_rlevel-1
    CALL reallocate(iteration_info%iteration,  1, iteration_info%n_rlevel)
    CALL reallocate(iteration_info%level_name, 1, iteration_info%n_rlevel)
    CALL reallocate(iteration_info%last_iter,  1, iteration_info%n_rlevel)
...
! *****************************************************************************
  FUNCTION cp_print_key_unit_nr(logger, basis_section, print_key_path, extension,&
       ...
       file_action, file_status, do_backup, on_file, is_new_file) RESULT(res)
    TYPE(cp_logger_type), POINTER            :: logger
    TYPE(section_vals_type), POINTER         :: basis_section
    CHARACTER(len=*), INTENT(IN), OPTIONAL   :: print_key_path
    CHARACTER(len=*), INTENT(IN)             :: extension
    ...
    LOGICAL                                  :: do_log, found, my_do_backup, &
                                                my_local, my_on_file, &
                                                my_should_output
    TYPE(cp_iteration_info_type), POINTER    :: iteration_info
    TYPE(section_vals_type), POINTER         :: print_key
    ...
    NULLIFY(print_key)
    IF(.NOT.(ASSOCIATED(basis_section)))CALL cp__a("input/cp_output_handling.F",867)
    IF(.NOT.(ASSOCIATED(logger)))CALL cp__a("input/cp_output_handling.F",868)
    IF(.NOT.(basis_section%ref_count>0))CALL cp__a("input/cp_output_handling.F",869)
    IF(.NOT.(logger%ref_count>0))CALL cp__a("input/cp_output_handling.F",870)
...
! *****************************************************************************
  SUBROUTINE cp_print_key_finished_output(unit_nr, logger, basis_section,&
    ...
    IF (PRESENT(local)) my_local=local
    IF (PRESENT(on_file)) my_on_file=on_file
    IF(.NOT.(ASSOCIATED(basis_section)))CALL cp__a("input/cp_output_handling.F",1017)
    IF(.NOT.(ASSOCIATED(logger)))CALL cp__a("input/cp_output_handling.F",1018)
    IF(.NOT.(basis_section%ref_count>0))CALL cp__a("input/cp_output_handling.F",1019)
    IF(.NOT.(logger%ref_count>0))CALL cp__a("input/cp_output_handling.F",1020)
    ...
    IF (my_should_output.and.(my_local.OR.&
       ...
       IF (my_on_file.OR.outPath.NE.'__STD_OUT__') THEN
          IF(.NOT.(unit_nr>0))CALL cp__a("input/cp_output_handling.F",1028)
       ...
       ENDIF
    END IF
    IF(.NOT.(unit_nr==-1))CALL cp__a("input/cp_output_handling.F",1035)
    unit_nr=-1
