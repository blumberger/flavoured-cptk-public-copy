# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_methods.F"
!-----------------------------------------------------------------------------!
...
# 35 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE build_A_matrix(Am, gfunc, w, particle_set, radii, rho_tot_g, gcut, g_dot_rvec_sin, g_dot_rvec_cos)
    ...
    INTEGER                                  :: e_dim, handle, idim1, idim2, &
                                                ig, igauss1, igauss2, igmax, &
...
!NB Use DGEMM to speed up calculation, can't do accurate_sum() anymore because dgemm does the sum over g
    ...
    DO ig = s_dim, e_dim
       ...
       igmax  = ig
    ENDDO
    ALLOCATE(arg1_v_sin(s_dim:igmax))
...
! *****************************************************************************
  RECURSIVE SUBROUTINE ewald_ddapc_pot(cp_para_env, coeff, factor, cell, multipole_section,&
    ...
    INTEGER :: ewmdim, handle, idim, idim1, idim2, idimo, igauss1, igauss2, &
      ...
      nmax1, nmax2, nmax3, r1, r2, r3, rmax1, rmax2, rmax3
    LOGICAL                                  :: analyt
    ...
    NULLIFY(EwM)
    CALL timeset(routineN,handle)
    IF(.NOT.(.NOT.ASSOCIATED(M)))CALL cp__a("cp_ddapc_methods.F",670)
    IF(.NOT.(ASSOCIATED(radii)))CALL cp__a("cp_ddapc_methods.F",671)
    IF(.NOT.(cell%orthorhombic))CALL cp__a("cp_ddapc_methods.F",672)
...
! *****************************************************************************
  SUBROUTINE solvation_ddapc_pot( solvation_section, particle_set, M,  radii)
    ...
    ELSE
       ...
       END IF
    END IF
    IF(.NOT.(n_rep1/=0.OR.n_rep2/=0))CALL cp__a("cp_ddapc_methods.F",897)
    ...
    DO i = 1, SIZE(particle_set)
       ...
       IF (r1>=Rs) THEN
          ...
          END IF
          CALL cp__b("cp_ddapc_methods.F",912,"")
       END IF
       LocP(:,i) = 0.0_dp
       IF (r1/=0.0_dp) THEN
          DO l = 0, lmax
