# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_dftb.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_dftb.F"
!-----------------------------------------------------------------------------!
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_dftb.F" 2
...
! *****************************************************************************
  SUBROUTINE scf_post_calculation_dftb(dft_section, scf_env, qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_dftb.F",123)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",200,"Lowdin population analysis not implemented for DFTB method.")
    END IF

    ! Hirshfeld
    print_key => section_vals_get_subs_vals(print_section,"HIRSHFELD")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",206,"Hirshfeld charges not available for DFTB method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",224,"Hyperfine Coupling not implemented for DFTB method.")
    END IF

    ! EFIELD CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"EFIELD_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",230,"Efield cube file not implemented for DFTB method.")
    END IF

    ! DENSITY CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"E_DENSITY_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",236,"Electronic density cube file not implemented for DFTB method.")
    END IF

    ! TOTAL DENSITY CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"TOT_DENSITY_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",242,"Total density cube file not implemented for DFTB method.")
    END IF

    ! V_Hartree CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"V_HARTREE_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",248,"Hartree potential cube file not implemented for DFTB method.")
    END IF

    ! V_XC CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"V_XC_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",254,"XC potential cube file not available for DFTB method.")
    END IF

    ! ELF
    print_key => section_vals_get_subs_vals(print_section,"ELF_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",260,"ELF not implemented for DFTB method.")
    END IF

    ! KINETIC ENERGY 
    print_key => section_vals_get_subs_vals(print_section,"KINETIC_ENERGY")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",266,"Kinetic energy not available for DFTB method.")
    END IF

    ! Xray diffraction spectrum
    print_key => section_vals_get_subs_vals(print_section,"XRAY_DIFFRACTION_SPECTRUM")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",272,"Xray diffraction spectrum not implemented for DFTB method.")
    END IF

    ! Electric field gradients
    print_key => section_vals_get_subs_vals(print_section,"ELECTRIC_FIELD_GRADIENT")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",278,"Electric field gradient not implemented for DFTB method.")
    END IF

    ! STM
    print_key => section_vals_get_subs_vals(print_section,"STM")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",284,"STM not implemented for DFTB method.")
    END IF

    ! MO 
    print_key => section_vals_get_subs_vals(print_section,"MO")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",290,"Printing of MO properties not implemented for DFTB method.")
    END IF

    ! MO CUBES
    print_key => section_vals_get_subs_vals(print_section,"MO_CUBES")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",296,"Printing of MO cube files not implemented for DFTB method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",307,"DFT+U method not implemented for DFTB method.")
    END IF

    ! PDOS
    print_key => section_vals_get_subs_vals(print_section,"PDOS")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_dftb.F",313,"Projected DOS not implemented for DFTB method.")
    END IF
