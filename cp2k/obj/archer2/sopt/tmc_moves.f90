# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_moves.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_moves.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_moves.F" 2
...
! *****************************************************************************
  SUBROUTINE change_pos(tmc_params, move_types, rng_stream, elem, mv_conf, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",91)
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_moves.F",92)
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("tmc/tmc_moves.F",93)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",94)
    ...
    IF(.NOT.(ANY(elem%elem_stat(:).EQ.status_ok)))CALL cp__a("tmc/tmc_moves.F",112)
    ...
    CASE(mv_type_gausian_adapt)
       ...
       CALL cp__b("tmc/tmc_moves.F",127,"gaussian adaptation is not imlemented yet.")
...
!                    rng_seed=elem%rng_seed, rng_seed_last_acc=last_acc_elem%rng_seed)
    ...
    CASE(mv_type_MD)
      IF(.NOT.(ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_moves.F",318)
    ...
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_moves.F",353),&
           "unknown move type "//&
...
! *****************************************************************************
  SUBROUTINE get_mol_indeces(tmc_params, mol_arr, mol, start_ind, end_ind)
    ...
    IF(.NOT.(ASSOCIATED(mol_arr)))CALL cp__a("tmc/tmc_moves.F",387)
    IF(.NOT.(mol.LE.MAXVAL(mol_arr(:))))CALL cp__a("tmc/tmc_moves.F",388)
    ! get start index
    ...
    IF(.NOT.(ALL(mol_arr(start_ind:end_ind).EQ.mol)))CALL cp__a("tmc/tmc_moves.F",404)
    IF(.NOT.(start_ind.GT.0))CALL cp__a("tmc/tmc_moves.F",405)
    IF(.NOT.(end_ind.GT.0))CALL cp__a("tmc/tmc_moves.F",406)
...
! *****************************************************************************
  FUNCTION check_pos_in_subbox(pos, subbox_center, box_scale, tmc_params)&
    ...
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("tmc/tmc_moves.F",435)
    IF(.NOT.(ASSOCIATED(subbox_center)))CALL cp__a("tmc/tmc_moves.F",436)
    IF(.NOT.(ASSOCIATED(box_scale)))CALL cp__a("tmc/tmc_moves.F",437)
    ! if pressure is defined, no scale should be 0
    flag = .NOT.((tmc_params%pressure.GT.0.0_dp).AND.(ANY(box_scale.EQ.0.0_dp)))
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_moves.F",440)
    IF(.NOT.(SIZE(pos).EQ.3))CALL cp__a("tmc/tmc_moves.F",441)
    IF(.NOT.(SIZE(pos).EQ.SIZE(subbox_center)))CALL cp__a("tmc/tmc_moves.F",442)
...
! *****************************************************************************
  SUBROUTINE elements_in_new_subbox(tmc_params, rng_stream, elem, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",493)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",494)

    ! start the timing
    CALL timeset(routineN,handle)

    IF(ANY(tmc_params%sub_box_size(:).LE.0.1_dp)) THEN
      !CALL cp__w("tmc/tmc_moves.F",500,"try to count elements in sub box without sub box.")
      elem%elem_stat = status_ok
...
! *****************************************************************************
  SUBROUTINE do_mol_rot(pos, ind_start, ind_end, max_angle, move_types, &
    ...
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("tmc/tmc_moves.F",573)
    IF(.NOT.(dim_per_elem.EQ.3))CALL cp__a("tmc/tmc_moves.F",574)
    IF(.NOT.(ind_start.GT.0.AND.ind_start.LT.SIZE(pos)))CALL cp__a("tmc/tmc_moves.F",575)
    IF(.NOT.(ind_end.GT.0.AND.ind_end.LT.SIZE(pos)))CALL cp__a("tmc/tmc_moves.F",576)
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_moves.F",577)
...
! *****************************************************************************
  SUBROUTINE vel_change(vel, atom_kind, phi, temp, rnd_sign_change, rng_stream)
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("tmc/tmc_moves.F",635)
...
! *****************************************************************************
  SUBROUTINE search_and_do_proton_displace_loop(elem, short_loop, rng_stream, &
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",695)
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("tmc/tmc_moves.F",696)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",697)
    ...
    IF(counter.LT.6) THEN
       CALL cp_warn(cp__l("tmc/tmc_moves.F",749),&
            "short proton loop with"//cp_to_string(counter)//&
            "molecules.")
       tmp_chr=""
       WRITE(tmp_chr, *) mol_arr(1:counter)
       CALL cp__w("tmc/tmc_moves.F",754,"selected molecules:"//TRIM(tmp_chr))
       short_loop = .TRUE.
    ENDIF
...
! *****************************************************************************
  SUBROUTINE find_nearest_proton_acceptor_donator(elem, mol, donor_acceptor, &
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",798)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",799)
    ...
    list_distances: DO mol_tmp=1, nr_mol
      ...
      IF(MOD(ind_e-ind_n,3).GT.0) THEN
        CALL cp_warn(cp__l("tmc/tmc_moves.F",829),&
...
! *****************************************************************************
  FUNCTION check_donor_acceptor(elem, i_orig, i_neighbor, tmc_params) &
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",933)
    IF(.NOT.(i_orig.GE.1.AND.i_orig.LE.SIZE(elem%pos)))CALL cp__a("tmc/tmc_moves.F",934)
    IF(.NOT.(i_neighbor.GE.1.AND.i_neighbor.LE.SIZE(elem%pos)))CALL cp__a("tmc/tmc_moves.F",935)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",936)
...
! *****************************************************************************
  SUBROUTINE rotate_molecules_in_chain(tmc_params, elem, mol_arr_in, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",1001)
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_moves.F",1002)
...
! *****************************************************************************
  SUBROUTINE change_volume(conf, T_ind, move_types, rng_stream, tmc_params, &
    ...
    IF(.NOT.(ASSOCIATED(conf)))CALL cp__a("tmc/tmc_moves.F",1155)
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_moves.F",1156)
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("tmc/tmc_moves.F",1157)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",1158)
    IF(.NOT.(T_ind.GT.0.AND.T_ind.LE.tmc_params%nr_temp))CALL cp__a("tmc/tmc_moves.F",1159)
    IF(.NOT.(tmc_params%dim_per_elem.EQ.3))CALL cp__a("tmc/tmc_moves.F",1160)
    IF(.NOT.(tmc_params%cell%orthorhombic))CALL cp__a("tmc/tmc_moves.F",1161)
    ...
    ELSE
       DO mol=1, MAXVAL(conf%mol(:))
         ...
         ! get center of mass
         IF(.NOT.(ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_moves.F",1233)
...
! *****************************************************************************
  SUBROUTINE swap_atoms(conf, move_types, rng_stream, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(conf)))CALL cp__a("tmc/tmc_moves.F",1281)
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_moves.F",1282)
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("tmc/tmc_moves.F",1283)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_moves.F",1284)
    IF(.NOT.(ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_moves.F",1285)
