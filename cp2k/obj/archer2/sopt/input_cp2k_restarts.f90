# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/input_cp2k_restarts.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/input_cp2k_restarts.F"
!-----------------------------------------------------------------------------!
...
# 89 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/input_cp2k_restarts.F" 2
...
! *****************************************************************************
  SUBROUTINE write_restart(md_env,force_env,root_section,&
    ...
    INTEGER                                  :: handle, ikey, ires, log_unit, &
                                                nforce_eval
    LOGICAL                                  :: save_mem, &
...
! *****************************************************************************
  SUBROUTINE update_motion(motion_section,md_env,force_env,logger,&
    ...
    ELSE
       ...
       ELSE
          CALL cp__b("motion/input_cp2k_restarts.F",567,"")
       END IF
    END IF
    ...
    IF (ASSOCIATED(itimes)) THEN
       IF (itimes>=0) THEN
          ...
          IF(.NOT.(ASSOCIATED(t)))CALL cp__a("motion/input_cp2k_restarts.F",591)
...
! *****************************************************************************
  SUBROUTINE update_motion_helium(helium)
    ...
    IF(.NOT.(ASSOCIATED(helium)))CALL cp__a("motion/input_cp2k_restarts.F",1067)
    ...
    ALLOCATE(int_msg_gather(msglen))
    ...
    IF (helium%solute_present) THEN
      ...
      err_str = TRIM(ADJUSTL(err_str)) // &
                TRIM(ADJUSTL(stmp)) // "'."
      IF(msgLEN/=reqlen)&
         CALL cp__b("motion/input_cp2k_restarts.F",1293,err_str)
...
! *****************************************************************************
  SUBROUTINE dump_csvr_energy_info(thermostat_energy, nsize, work_section)
    ...
    IF(.NOT.(ASSOCIATED(work_section)))CALL cp__a("motion/input_cp2k_restarts.F",1408)
    IF(.NOT.(work_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",1409)
    ...
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",1418),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
...
! *****************************************************************************
  SUBROUTINE collect_nose_restart_info(nhc, para_env, eta, veta, fnhc, mnhc)
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1739,&
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1752,&
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1785,&
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1818,&
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1851,&
...
! *****************************************************************************
  SUBROUTINE section_neb_coord_val_set(coord_section,array,narray,nsize,nfield,&
    ...
    NULLIFY (my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(coord_section)))CALL cp__a("motion/input_cp2k_restarts.F",1919)
    IF(.NOT.(coord_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",1920)
    section => coord_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",1924),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
    DO
...
! *****************************************************************************
  SUBROUTINE meta_hills_val_set_ss(ss_section, meta_env)
    ...
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(ss_section)))CALL cp__a("motion/input_cp2k_restarts.F",2050)
    IF(.NOT.(ss_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",2051)
    section => ss_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",2055),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
    DO
...
! *****************************************************************************
  SUBROUTINE meta_hills_val_set_ds(ds_section, meta_env)
    ...
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(ds_section)))CALL cp__a("motion/input_cp2k_restarts.F",2121)
    IF(.NOT.(ds_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",2122)
    section => ds_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",2126),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
    DO
...
! *****************************************************************************
  SUBROUTINE meta_hills_val_set_ww(ww_section, meta_env)
    ...
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(ww_section)))CALL cp__a("motion/input_cp2k_restarts.F",2191)
    IF(.NOT.(ww_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",2192)
    section => ww_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",2196),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
    DO
...
! *****************************************************************************
  SUBROUTINE meta_hills_val_set_dt(invdt_section, meta_env)
    ...
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(invdt_section)))CALL cp__a("motion/input_cp2k_restarts.F",2258)
    IF(.NOT.(invdt_section%ref_count>0))CALL cp__a("motion/input_cp2k_restarts.F",2259)
    section => invdt_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",2263),"section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
    DO
  ...
  SUBROUTINE write_binary_restart(output_unit,log_unit,root_section,md_env,force_env)
    ...
    IF (output_unit > 0) THEN ! only ionode
       ...
       IF (print_info) THEN
          ...
          IF (istat /= 0) THEN
             CALL cp_abort(cp__l("motion/input_cp2k_restarts.F",2427),&
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,2465,&
                                        "ibuf",int_size*natom)
       DO iatom=1,natom
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,2477,&
                                        "rbuf",dp_size*3*natom)
       DO iatom=1,natom
       ...
       IF (nmolecule > 0) THEN
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2502,&
                                           "imol",int_size*natom)
          imol(:) = 0
       ...
       IF (nshell > 0) THEN
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2541,&
                                           "rbuf",dp_size*3*nshell)
          DO ishell=1,nshell
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2554,&
                                           "ibuf",int_size*nshell)
          DO ishell=1,nshell
       ...
       IF (ncore > 0) THEN
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2577,&
                                           "rbuf",dp_size*3*ncore)
          DO icore=1,ncore
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2590,&
                                           "ibuf",int_size*ncore)
          DO icore=1,ncore
    ...
    IF (output_unit > 0) THEN ! only ionode
       ...
       IF (write_velocities) THEN
          ALLOCATE (rbuf(3,natom),STAT=istat)
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,2681,&
                                           "rbuf",dp_size*3*natom)
       ...
       IF (nshell > 0) THEN
          IF (write_velocities) THEN
             ALLOCATE (rbuf(3,nshell),STAT=istat)
             IF (istat /= 0) CALL stop_memory(routineN,moduleN,2709,&
                                              "rbuf",dp_size*3*nshell)
             DO ishell=1,nshell
       ...
       IF (ncore > 0) THEN
          IF (write_velocities) THEN
             ALLOCATE (rbuf(3,ncore),STAT=istat)
             IF (istat /= 0) CALL stop_memory(routineN,moduleN,2737,&
                                              "rbuf",dp_size*3*ncore)
             DO icore=1,ncore
...
! *****************************************************************************
  SUBROUTINE stop_write(object,unit_number)
    ...
    CALL cp__b("motion/input_cp2k_restarts.F",2910,message)
