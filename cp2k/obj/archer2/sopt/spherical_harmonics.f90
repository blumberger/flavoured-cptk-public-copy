# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/spherical_harmonics.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/spherical_harmonics.F"
!-----------------------------------------------------------------------------!
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/spherical_harmonics.F" 2
...
! *****************************************************************************
  SUBROUTINE clebsch_gordon_complex (l1,m1,l2,m2,clm)
    ...
    INTEGER                                  :: icase, ind, l, lm, lp, n
     ...
     n = l/2 + 1
     IF (n > SIZE(clm)) CALL cp__b("common/spherical_harmonics.F",86,"Array too small")
...
! *****************************************************************************
  SUBROUTINE clebsch_gordon_real (l1,m1,l2,m2,rlm)
    ...
    REAL(KIND=dp)                            :: xsi
     ...
     IF (l > lmax) CALL clebsch_gordon_init ( l )
     n = l/2 + 1
     IF (n > SIZE(rlm,1)) CALL cp__b("common/spherical_harmonics.F",125,"Array too small")
...
! *****************************************************************************
  SUBROUTINE clebsch_gordon_init ( l )
    ...
    INTEGER                                  :: handle, i1, i2, ierr, ix, iy, &
     ...
     IF ( l < 0 ) CALL cp__b("common/spherical_harmonics.F",257,"l < 0")
     IF ( ALLOCATED ( cg_table ) ) THEN
        DEALLOCATE ( cg_table)
     END IF
     ...
     IF (ierr /= 0) CALL stop_memory(routineN,moduleN,265,&
...
! m1 >= 0; m2 >= 0; mp >= 0
    ...
    IF ( m1<0 .OR. m2<0 .OR. mp<0 ) THEN
       WRITE(*,*) l1,l2,lp
       WRITE(*,*) m1,m2,mp
       CALL cp__b("common/spherical_harmonics.F",324,"Illegal input values")
    END IF
    IF ( l2 < l1 ) THEN
      la = l2
      ma = m2
...
!
  ...
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",443,"Negative l value")
  CASE ( 0 )
    pf = SQRT ( 1.0_dp / ( 4.0_dp * pi ) )
    IF ( m /= 0 )  CALL cp__b("common/spherical_harmonics.F",446,"l = 0 and m value out of bounds")
    y(:) = pf
  CASE ( 1 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",451,"l = 1 and m value out of bounds")
    CASE ( 1 )
  ...
  CASE ( 2 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",465,"l = 2 and m value out of bounds")
    CASE ( 2 )
  ...
  CASE ( 3 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",485,"l = 3 and m value out of bounds")
    CASE ( 3 )
    ...
    END SELECT
  CASE DEFAULT
    IF ( m < -l .OR. m > l ) CALL cp__b("common/spherical_harmonics.F",509,"m value out of bounds")
    lpm = fac ( l + ABS ( m ) )
    lmm = fac ( l - ABS ( m ) )
    IF ( m == 0 ) THEN
      t = 4.0_dp * pi
...
!
  ...
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",577,"Negative l value")
  CASE ( 0 )
    pf = SQRT ( 1.0_dp / ( 4.0_dp * pi ) )
    IF ( m /= 0 )  CALL cp__b("common/spherical_harmonics.F",580,"l = 0 and m value out of bounds")
    y = pf
  CASE ( 1 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",585,"l = 1 and m value out of bounds")
    CASE ( 1 )
      pf = SQRT ( 3.0_dp / ( 4.0_dp * pi ) )
      y = pf * r ( 1 )
    CASE ( 0 )
  ...
  CASE ( 2 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",599,"l = 2 and m value out of bounds")
    CASE ( 2 )
      pf = SQRT ( 15.0_dp / ( 16.0_dp * pi ) )
      y = pf * ( r ( 1 ) * r ( 1 ) - r ( 2 ) * r ( 2 ) )
    CASE ( 1 )
  ...
  CASE ( 3 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",619,"l = 3 and m value out of bounds")
    CASE ( 3 )
      pf = SQRT ( 35.0_dp / ( 32.0_dp * pi ) )
      y = pf * r ( 1 ) * ( r ( 1 )**2 - 3.0_dp * r ( 2 )**2 )
    CASE ( 2 )
    ...
    END SELECT
  CASE DEFAULT
    IF ( m < -l .OR. m > l ) CALL cp__b("common/spherical_harmonics.F",643,"m value out of bounds")
    lpm = fac ( l + ABS ( m ) )
    lmm = fac ( l - ABS ( m ) )
    IF ( m == 0 ) THEN
      t = 4.0_dp * pi
...
!
  ...
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",712,"Negative l value")
  CASE ( 0 )
    pf = SQRT ( 1.0_dp / ( 4.0_dp * pi ) )
    IF ( m /= 0 )  CALL cp__b("common/spherical_harmonics.F",715,"l = 0 and m value out of bounds")
    y(:) = pf
  CASE ( 1 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",720,"l = 1 and m value out of bounds")
    CASE ( 1 )
  ...
  CASE ( 2 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",742,"l = 2 and m value out of bounds")
    CASE ( 2 )
  ...
  CASE ( 3 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",778,"l = 3 and m value out of bounds")
    CASE ( 3 )
    ...
    END SELECT
  CASE DEFAULT
    IF ( m < -l .OR. m > l ) CALL cp__b("common/spherical_harmonics.F",826,"m value out of bounds")
    lpm = fac ( l + ABS ( m ) )
    lmm = fac ( l - ABS ( m ) )
    t = 4.0_dp * pi
    IF ( ABS ( lpm ) < EPSILON ( 1.0_dp ) ) THEN
...
!
  ...
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",893,"Negative l value")
  CASE ( 0 )
    pf = SQRT ( 1.0_dp / ( 4.0_dp * pi ) )
    IF ( m /= 0 )  CALL cp__b("common/spherical_harmonics.F",896,"l = 0 and m value out of bounds")
    y = pf
  CASE ( 1 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",901,"l = 1 and m value out of bounds")
    CASE ( 1 )
      pf = SQRT ( 3.0_dp / ( 8.0_dp * pi ) )
      yp = r ( 1 )
      ym = r ( 2 )
  ...
  CASE ( 2 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",919,"l = 2 and m value out of bounds")
    CASE ( 2 )
      pf = SQRT ( 15.0_dp / ( 32.0_dp * pi ) )
      yp = ( r ( 1 ) * r ( 1 ) - r ( 2 ) * r ( 2 ) )
      ym = 2.0_dp * r ( 1 ) * r ( 2 )
  ...
  CASE ( 3 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",947,"l = 3 and m value out of bounds")
    CASE ( 3 )
      pf = SQRT ( 35.0_dp / ( 64.0_dp * pi ) )
    ...
    END SELECT
  CASE DEFAULT
    IF ( m < -l .OR. m > l ) CALL cp__b("common/spherical_harmonics.F",983,"m value out of bounds")
    lpm = fac ( l + ABS ( m ) )
    lmm = fac ( l - ABS ( m ) )
    t = 4.0_dp * pi
    IF ( ABS ( lpm ) < EPSILON ( 1.0_dp ) ) THEN
...
! *****************************************************************************
  ...
  IF ( ABS ( x ) > 1.0_dp ) CALL cp__b("common/spherical_harmonics.F",1038,"x value > 1")
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",1041,"Negative l value")
  CASE ( 0 )
    plm = 1.0_dp
  CASE ( 1 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1047,"l = 1 and m value out of bounds")
    CASE ( 1 )
      plm = SQRT ( 1.0_dp - x * x )
    CASE ( 0 )
      plm = x
  ...
  CASE ( 2 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1056,"l = 2 and m value out of bounds")
    CASE ( 2 )
  ...
  CASE ( 3 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1067,"l = 3 and m value out of bounds")
    CASE ( 3 )
  ...
  CASE ( 4 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1080,"l = 4 and m value out of bounds")
    CASE ( 4 )
  ...
  CASE ( 5 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1095,"l = 5 and m value out of bounds")
    CASE ( 5 )
  ...
  CASE ( 6 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1113,"l = 6 and m value out of bounds")
    CASE ( 6 )
  ...
  CASE DEFAULT
    mm = ABS ( m )
    IF ( mm > l ) CALL cp__b("common/spherical_harmonics.F",1135,"m out of bounds")
...
! *****************************************************************************
  ...
  IF ( ABS ( x ) > 1.0_dp ) CALL cp__b("common/spherical_harmonics.F",1186,"x value > 1")
  SELECT CASE ( l )
  CASE ( 0 )
    dplm = 0.0_dp
  CASE ( 1 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1193,"l = 1 and m value out of bounds")
    CASE ( 1 )
  ...
  CASE ( 2 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1202,"l = 2 and m value out of bounds")
    CASE ( 2 )
      dplm = -6.0_dp * x
    CASE ( 1 )
  ...
  CASE ( 3 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1213,"l = 3 and m value out of bounds")
    CASE ( 3 )
  ...
  CASE ( 4 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1226,"l = 4 and m value out of bounds")
    CASE ( 4 )
      dplm = - 420 * x * (1 - x*x)
    CASE ( 3 )
  ...
  CASE ( 5 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1246,"l = 5 and m value out of bounds")
    CASE ( 5 )
  ...
  CASE ( 6 )
    SELECT CASE ( ABS ( m ) )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1270,"l = 6 and m value out of bounds")
    CASE ( 6 )
    ...
    END SELECT
  CASE DEFAULT
!    CALL cp__b("common/spherical_harmonics.F",1296,"l > 6 dplm not implemented")
    mm = ABS ( m )
    IF ( mm > l ) CALL cp__b("common/spherical_harmonics.F",1298,"m out of bounds")
! use recurence from numerical recipies
    pmm = 1.0_dp
    dpmm = 0.0_dp
    IF ( mm > 0 ) THEN
...
! *****************************************************************************
  FUNCTION dPof1(x, l)
    ...
    IF ( ABS ( x ) -1.0_dp > EPSILON(1.0_dp)) THEN
       CALL cp__b("common/spherical_harmonics.F",1357,"|x| is not 1")
    END IF
    IF (x > 0.0_dp) THEN
      SELECT CASE ( l )
      CASE ( : -1 )
        CALL cp__b("common/spherical_harmonics.F",1362,"Negative l value")
      CASE ( 0 )
        dPof1 = 0.0_dp
      CASE ( 1 )
        dPof1 = 1.0_dp
      ...
      CASE ( 6 )
        dPof1 = 21.0_dp
      CASE ( 7 :  )
        CALL cp__b("common/spherical_harmonics.F",1378,"Not implemented")
      END SELECT
    ELSE
      SELECT CASE ( l )
      CASE ( : -1 )
        CALL cp__b("common/spherical_harmonics.F",1383,"Negative l value")
      CASE ( 0 )
        dPof1 = 0.0_dp
      CASE ( 1 )
        dPof1 = 1.0_dp
      ...
      CASE ( 6 )
        dPof1 = -21.0_dp
      CASE ( 7 :  )
        CALL cp__b("common/spherical_harmonics.F",1399,"Not implemented")
      END SELECT
    END IF
...
!
  ...
  SELECT CASE ( l )
  CASE ( : -1 )
    CALL cp__b("common/spherical_harmonics.F",1695,"Negative l value")
  CASE ( 0 )
    pf = SQRT ( 1.0_dp / ( 4.0_dp * pi ) )
    IF ( m /= 0 )  CALL cp__b("common/spherical_harmonics.F",1698,"l = 0 and m value out of bounds")
    dy(1:3,:) = 0.0_dp
  CASE ( 1 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1703,"l = 1 and m value out of bounds")
    CASE ( 1 )
      pf = SQRT ( 3.0_dp / ( 4.0_dp * pi ) )
      dy (1, : ) = pf
      dy (2, : ) = 0.0_dp
  ...
  CASE ( 2 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1723,"l = 2 and m value out of bounds")
    CASE ( 2 )
      pf = SQRT ( 15.0_dp / ( 16.0_dp * pi ) )
      dy (1, : ) =  pf * 2.0_dp * r ( 1, : )
      dy (2, : ) = -pf * 2.0_dp * r ( 2, : )
  ...
  CASE ( 3 )
    SELECT CASE ( m )
    CASE DEFAULT
      CALL cp__b("common/spherical_harmonics.F",1753,"l = 3 and m value out of bounds")
    CASE ( 3 )
      pf = SQRT ( 35.0_dp / ( 32.0_dp * pi ) )
    ...
    END SELECT
  CASE DEFAULT
    IF ( m < -l .OR. m > l ) CALL cp__b("common/spherical_harmonics.F",1791,"m value out of bounds")
    lpm = fac ( l + ABS ( m ) )
    lmm = fac ( l - ABS ( m ) )
    IF ( m == 0 ) THEN
      t = 4.0_dp * pi
