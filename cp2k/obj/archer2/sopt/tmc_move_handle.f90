# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_move_handle.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_move_handle.F"
!-----------------------------------------------------------------------------!
...
# 50 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_move_handle.F" 2
  ...
  SUBROUTINE read_init_move_types(tmc_params, tmc_section)
    ...
    IF(explicit) THEN
      ...
      IF(tmc_params%NMC_inp_file.EQ."")&
         CALL cp__b("tmc/tmc_move_handle.F",120,"Please specify a valid approximate potential.")

      CALL section_vals_val_get(nmc_section,"NR_NMC_STEPS",&
                                i_val=nmc_steps)
      IF(nmc_steps.LE.0)&
         CALL cp__b("tmc/tmc_move_handle.F",125,"Please specify a valid amount of NMC steps (NR_NMC_STEPS {INTEGER}).")
      ...
      IF(nmc_init_acc_prob.LE.0.0_dp)&
         CALL cp_abort(cp__l("tmc/tmc_move_handle.F",132),&
    ...
    IF(n_items+n_NMC_items.GT.0) THEN
      ...
      IF(mv_prob_sum.LE.0.0)&
         CALL cp_abort(cp__l("tmc/tmc_move_handle.F",157),&
      ...
      DO i_tmp = 1, n_items+n_NMC_items
        ...
        IF(mv_prob.LT.0.0_dp)&
           CALL cp_abort(cp__l("tmc/tmc_move_handle.F",196),&
        ...
        IF(init_acc_prob.LT.0.0_dp)&
           CALL cp_abort(cp__l("tmc/tmc_move_handle.F",202),&
                "Please select a valid initial acceptance probability (>0.0) "//&
                "for the move type "//inp_kind_name)
        ...
        CASE("ATOM_TRANS", "MOL_TRANS")
          ...
          CASE DEFAULT
            CALL cp__b("tmc/tmc_move_handle.F",215,"move type is not defined in the translation types")
          END SELECT
          ! convert units
          ...
          CASE DEFAULT
            CALL cp__b("tmc/tmc_move_handle.F",224,"move type atom / mol trans is not defined for this TMC run type")
          END SELECT
        ! molecule rotation
        CASE("MOL_ROT")
          ind = mv_type_mol_rot
          ...
          CASE DEFAULT
            CALL cp__b("tmc/tmc_move_handle.F",234,"move type MOL_ROT is not defined for this TMC run type")
          END SELECT
        ! proton reordering
        CASE("PROT_REORDER")
        ...
        CASE("PT_SWAP")
          ...
          IF(tmc_params%nr_temp.LE.1) THEN
            ...
            mv_prob = 0.0_dp
            CALL cp_warn(cp__l("tmc/tmc_move_handle.F",254),&
        ...
        CASE("VOL_MOVE")
          ...
          ELSE
            CALL cp_warn(cp__l("tmc/tmc_move_handle.F",266),&
          ...
          IF(n_rep_val.GT.0) THEN
            ...
            DO i= 1, n_rep_val
              ...
              IF(SIZE(move_types%atom_lists(i)%atoms).LE.1)&
                 CALL cp__b("tmc/tmc_move_handle.F",286,"ATOM_SWAP requires minimum two atom kinds selected. ")
            END DO
          END IF
        ! gaussian adaptation
        CASE("GAUSS_ADAPT")
        ...
        CASE DEFAULT
          CALL cp__b("tmc/tmc_move_handle.F",294,"A unknown move type is selected: "//inp_kind_name)
        END SELECT
        ! check for valid move sizes
        IF(delta_x.LT.0.0_dp)&
           CALL cp_abort(cp__l("tmc/tmc_move_handle.F",298),&
                "Please select a valid move size (>0.0) "//&
                "for the move type "//inp_kind_name)
        ! check if not already set
        IF(move_types%mv_weight(ind).GT.0.0) THEN
          CALL cp__b("tmc/tmc_move_handle.F",303,"TMC: Each move type can be set only once. ")
        END IF
      ...
      END DO
    ELSE
      CALL cp__b("tmc/tmc_move_handle.F",314,"No move type selected, please select at least one.")
    END IF
    mv_prob_sum = SUM(tmc_params%move_types%mv_weight(:))
    flag = .TRUE.
    IF(.NOT.(ABS(mv_prob_sum-1.0_dp).LT.0.01_dp))CALL cp__a("tmc/tmc_move_handle.F",318)
    IF(ASSOCIATED(tmc_params%nmc_move_types)) THEN
      mv_prob_sum = SUM(tmc_params%nmc_move_types%mv_weight(:))
      IF(.NOT.(ABS(mv_prob_sum-1.0_dp)<10*EPSILON(1.0_dp)))CALL cp__a("tmc/tmc_move_handle.F",321)
    END IF
...
! *****************************************************************************
  SUBROUTINE check_moves(tmc_params, move_types, mol_array)
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_move_handle.F",343)
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_move_handle.F",344)
    ...
    IF(move_types%mv_weight(mv_type_mol_trans).GT.0.0_dp .OR. &
      ...
      IF(mol_array(SIZE(mol_array)).EQ.SIZE(mol_array)) &
        CALL cp_abort(cp__l("tmc/tmc_move_handle.F",352),&
    ...
    IF(move_types%mv_weight(mv_type_atom_swap).GT.0.0_dp) THEN
      ...
      IF(ASSOCIATED(move_types%atom_lists)) THEN
        DO list_i=1, SIZE(move_types%atom_lists(:))
          DO atom_j=1, SIZE(move_types%atom_lists(list_i)%atoms(:))
            ...
            END DO ref_loop
            IF(.NOT.found)&
               CALL cp_abort(cp__l("tmc/tmc_move_handle.F",374),&
            ...
            IF(ANY(move_types%atom_lists(list_i)%atoms(atom_j).EQ.&
              ...
              CALL cp_abort(cp__l("tmc/tmc_move_handle.F",381),&
      ...
      ELSE
        ...
        END IF
        IF(.NOT.found)&
           CALL cp_abort(cp__l("tmc/tmc_move_handle.F",400),&
...
! *****************************************************************************
  SUBROUTINE finalize_mv_types(tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_move_handle.F",419)
...
! *****************************************************************************
  SUBROUTINE print_move_types(init, file_io, tmc_params)
    ...
    subbox_out = .FALSE.
    type_title = .FALSE.
    IF(.NOT.(file_io.GT.0))CALL cp__a("tmc/tmc_move_handle.F",461)
    IF(.NOT.(ASSOCIATED(tmc_params%move_types)))CALL cp__a("tmc/tmc_move_handle.F",462)
          ...
          ! certain move types
             IF(move_types%mv_weight(typ).GT.0.0_dp)THEN 
                ...
                CASE(mv_type_mol_rot)
                   ...
                   END IF
                CASE(mv_type_MD)
                    CALL cp__w("tmc/tmc_move_handle.F",621,"md_time_step and nr md_steps not implemented...")
...
!                   END IF
                ...
                CASE(mv_type_gausian_adapt)
                   ...
                   END IF
                CASE DEFAULT
                   CALL cp_warn(cp__l("tmc/tmc_move_handle.F",689),&
...
! *****************************************************************************
  SUBROUTINE prob_update(move_types, pt_el, elem, acc, subbox, prob_opt)
    ...
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_move_handle.F",734)
    IF(.NOT.(.NOT.(PRESENT(pt_el).AND.PRESENT(subbox))))CALL cp__a("tmc/tmc_move_handle.F",735)
    ...
    IF(PRESENT(pt_el)) THEN
      IF(.NOT.(ASSOCIATED(pt_el)))CALL cp__a("tmc/tmc_move_handle.F",748)
      ...
      CASE(status_rejected_result)
        ...
        END IF
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_move_handle.F",766),&
    ...
    IF(PRESENT(elem)) THEN
      IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_move_handle.F",773)
      ...
      IF(.NOT.(PRESENT(acc)))CALL cp__a("tmc/tmc_move_handle.F",778)
...
! *****************************************************************************
  SUBROUTINE add_mv_prob(move_types, prob_opt, mv_counter, acc_counter, &
    ...
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_move_handle.F",843)
    IF(.NOT.(PRESENT(mv_counter).OR.PRESENT(subbox_counter)))CALL cp__a("tmc/tmc_move_handle.F",844)
 
    IF(PRESENT(mv_counter)) THEN
       IF(.NOT.(PRESENT(acc_counter)))CALL cp__a("tmc/tmc_move_handle.F",847)
    ...
    IF(PRESENT(subbox_counter)) THEN
       IF(.NOT.(PRESENT(subbox_acc_counter)))CALL cp__a("tmc/tmc_move_handle.F",857)
...
! *****************************************************************************
  SUBROUTINE clear_move_probs(move_types)
    ...
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_move_handle.F",875)
...
!> \retval mv_type (result) move type
!> \author Mandes 12.2012
...
! *****************************************************************************
  FUNCTION select_random_move_type(move_types, rnd) RESULT(mv_type)
    ...
    IF(.NOT.(ASSOCIATED(move_types)))CALL cp__a("tmc/tmc_move_handle.F",904)
    IF(.NOT.(rnd.GE.0.0_dp.AND.rnd.LT.1.0_dp))CALL cp__a("tmc/tmc_move_handle.F",905)
