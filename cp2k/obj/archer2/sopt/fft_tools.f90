# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F"
!-----------------------------------------------------------------------------!
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F" 2
...
# 181 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F"
...
! *****************************************************************************
  SUBROUTINE init_fft ( fftlib, alltoall, fftsg_sizes, pool_limit, wisdom_file,&
    ...
    IF ( fft_type <= 0 ) CALL cp__b("pw/fft_tools.F",214,"Unknown FFT library: "//TRIM(fftlib))
...
! *****************************************************************************
  SUBROUTINE fft_radix_operations ( radix_in, radix_out, operation )
    ...
    IF ( iloc == 0 ) THEN
       CALL cp__b("pw/fft_tools.F",297,"Index to radix array not found.")
    END IF
    ...
    ELSE IF ( OPERATION == FFT_RADIX_NEXT_ODD ) THEN
       ...
       IF ( MOD ( radix_out, 2 ) == 0 ) THEN
          CALL cp__b("pw/fft_tools.F",330,"No odd radix found.")
       END IF

    ELSE
       CALL cp__b("pw/fft_tools.F",334,"Disallowed radix operation.")
    END IF
...
! *****************************************************************************
  SUBROUTINE fft3d_s ( fsign, n, zin, zout, scale, status, debug )
    ...
    COMPLEX(KIND=dp), DIMENSION(:, :, :), &
      POINTER                                :: zoptr
    INTEGER                                  :: handle, ld(3), lo(3), &
                                                output_unit, sign, stat
    ...
    IF ( n(1) /= ld(1) .OR. n(2) /= ld(2) .OR. n(3) /= ld(3) ) THEN
       CALL cp__b("pw/fft_tools.F",409,"Size and dimension (zin) have to be the same.")
    END IF
    ...
    IF ( PRESENT ( zout ) ) THEN
       ...
       IF ( n(1) /= lo(1) .OR. n(2) /= lo(2) .OR. n(3) /= lo(3) ) THEN
          CALL cp__b("pw/fft_tools.F",437,"Size and dimension (zout) have to be the same.")
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE fft3d_ps ( fsign, n, cin, gin, gs_group, rs_group, yzp, nyzray, &
    ...
    INTEGER :: g_pos, handle, ierr, iout, lg, lmax, mcx2, mcz1, mcz2, mg, &
      ...
      nx, ny, nz, output_unit, r_dim(2), r_pos(2), rp, sign, stat
    ...
    IF ( numtask_g /= numtask_r ) THEN
       CALL cp__b("pw/fft_tools.F",529,"Real space and G space groups are different.")
    END IF
    numtask = numtask_r
    CALL mp_comm_compare ( rs_group, gs_group, iout )
    IF ( iout >3 ) THEN
       CALL cp__b("pw/fft_tools.F",534,"Real space and G space groups are different.")
    END IF
    ...
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,560,&
    ...
    IF ( r_dim ( 2 ) > 1 ) THEN
       ...
       IF ( r_dim ( 1 ) == 1 ) THEN
          CALL cp__b("pw/fft_tools.F",624,"This processor distribution is not supported.")
       END IF
       ...
       ELSE
          ...
          CALL cp__b("pw/fft_tools.F",791,"Illegal fsign parameter.")
    ...
    ELSE
       ...
       ELSE IF ( sign == BWFFT ) THEN
          ...
          END IF
       ELSE
          CALL cp__b("pw/fft_tools.F",911,"Illegal fsign parameter.")
       ENDIF
...
! *****************************************************************************
  SUBROUTINE fft3d_pb ( fsign, n, zin, gin, group, bo, scale, status, debug )
    ...
    COMPLEX(KIND=dp), DIMENSION(:, :), &
      POINTER                                :: abuf, bbuf
    INTEGER :: handle, lg(2), lz(3), mcx2, mcy3, mcz1, mcz2, mx1, mx2, mx3, &
      my1, my2, my3, my_pos, mz1, mz2, mz3, np, output_unit, sign, stat
...
!------------------------------------------------------------------------------
    ...
    IF ( DIM ( 1 ) > 1 .AND. DIM ( 2 ) > 1 ) THEN
       ...
       ELSE
          CALL cp__b("pw/fft_tools.F",1215,"Illegal fsign parameter.")
       END IF
    ...
    ELSEIF ( DIM ( 2 ) == 1 ) THEN
       ...
       ELSE
          CALL cp__b("pw/fft_tools.F",1337,"Illegal fsign parameter.")
       ENDIF
    ...
    ELSE
       ...
       CALL cp__b("pw/fft_tools.F",1344,"This partition not implemented.")
...
! *****************************************************************************
  SUBROUTINE resize_fft_scratch_pool()
    ...
    IF (nscratch>fft_pool_scratch_limit) THEN
       ...
       ELSE
          CALL cp__w("pw/fft_tools.F",2923,"The number of the scratches exceeded the limit, but none could be deallocated")
       ENDIF
    ENDIF
  ...
  SUBROUTINE get_fft_scratch(fft_scratch,tf_type,n,fft_sizes)
    ...
    DO
       ...
       ELSE
          ...
          SELECT CASE (tf_type)
          CASE DEFAULT
             CALL cp__b("pw/fft_tools.F",3063,"")
...
# 3181 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F"
...
# 3247 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft_tools.F"
...
! *****************************************************************************
  SUBROUTINE release_fft_scratch(fft_scratch)
    ...
    DO
       ...
       ELSE
          ! We cannot find the scratch type in this pool
          CALL cp__b("pw/fft_tools.F",3383,"")
          EXIT
       END IF
    END DO
...
! *****************************************************************************
 SUBROUTINE sparse_alltoall ( rs, scount, sdispl, rq, rcount, rdispl, group )
    ...
    IF ( rcount(pos) /= 0 ) THEN
       IF( rcount(pos) /= scount(pos) ) CALL cp__b("pw/fft_tools.F",3434,"")
