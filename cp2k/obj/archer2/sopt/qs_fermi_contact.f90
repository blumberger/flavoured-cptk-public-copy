# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fermi_contact.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fermi_contact.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fermi_contact.F" 2
...
! *****************************************************************************
  ...
  SUBROUTINE build_fermi_contact_matrix(qs_env,matrix_fc,rc)
    ...
    INTEGER :: after, handle, iatom, icol, ikind, inode, irow, iset, istat, &
      iw, jatom, jkind, jset, last_jatom, ldai, ldfc, maxco, maxlgto, maxsgf, &
      natom, ncoa, ncob, nkind, nseta, nsetb, sgfa, sgfb
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,148,&
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,158,&
                                     "fcint",1*int_size)
    NULLIFY(fcint(1)%block)

    ALLOCATE (basis_set_list(nkind),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,163,&
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (new_atom_b) THEN
          ...
          NULLIFY (fcint(1)%block)
          CALL cp_dbcsr_get_block_p(matrix=matrix_fc(1)%matrix,&
