# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_csr.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_csr.F"
!-----------------------------------------------------------------------------!
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_csr.F" 2
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/../base/hash_table_types.f90" 1
! *****************************************************************************
...
# 87 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_csr.F" 2
             ...
             IF (careful_mod) THEN
                IF (c_nze .EQ. 0 .OR. k_size .EQ. 0) THEN
                   CALL dbcsr_assert (.FALSE.,&
                        ...
                        "Can not call MM with LDx=0.", 446)
                   CYCLE
                ENDIF
             ENDIF
...
! *****************************************************************************
  SUBROUTINE dbcsr_mm_csr_init(this, left, right, product, &
    ...
    INTEGER :: default_stack, handle, istack, ithread, k_map, k_size, m_map, &
      m_size, n_map, n_size, nstacks, nthreads, ps_g
    ...
    CALL dbcsr_assert (.NOT.(PRESENT(left).NEQV.PRESENT(right)),&
         ...
         "Must both left and right provided or not.", 516)
    ...
    IF (PRESENT(left).AND.PRESENT(right)) THEN
       ...
       CALL dbcsr_assert (right%local_indexing, "EQV", left%local_indexing,&
            dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Local index useage must be consistent.", 522)
    ...
    ! Setup the MM stack
    CALL dbcsr_get_conf_nstacks (nxstacks)
...
! *****************************************************************************
  SUBROUTINE fill_hash_tables(hashes, matrix, block_estimate, row_map, col_map)
    ...
    INTEGER                                  :: col, handle, i, imat, n_rows, &
                                                row
...
!   ---------------------------------------------------------------------------
    ...
    IF (PRESENT (row_map)) THEN
       ...
       CALL dbcsr_assert (SIZE(hashes), "EQ", n_rows,&
            dbcsr_fatal_level, dbcsr_internal_error, routineN,&
            "Local row count mismatch", 705)
    ELSE
       n_rows = matrix%nblkrows_total
       CALL dbcsr_assert (SIZE(hashes), "EQ", n_rows,&
            dbcsr_fatal_level, dbcsr_internal_error, routineN,&
            "Global row count mismatch", 710)
    ENDIF
    DO row = 1, n_rows
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/../base/hash_table.f90" 1
! -----------------------------------------------------------------------------
...
! End of hashtable
! -----------------------------------------------------------------------------
# 935 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_csr.F" 2
