# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/et_coupling.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/et_coupling.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/et_coupling.F" 2
...
! *****************************************************************************
  SUBROUTINE becke_restraint(qs_env,becke_const ,calc_pot,calculate_forces)
    ...
    IF (dft_control%qs_control%becke_restraint) THEN
          ...
          IF (istat /= 0) CALL stop_memory(routineN,moduleN,352,&
                                           "catom",int_size*&
...
! *****************************************************************************
  SUBROUTINE becke_force(qs_env,becke_const)
    ...
    IF(dft_control%qs_control%becke_restraint)THEN
       ...
       ALLOCATE (catom(dft_control%qs_control%becke_control%natoms),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,531,&
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,544,&
                                        "atom_of_kind",natom*int_size)

       ALLOCATE (kind_of(natom),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,548,&
