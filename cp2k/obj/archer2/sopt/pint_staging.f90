# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_staging.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_staging.F"
!-----------------------------------------------------------------------------!
...
# 20 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_staging.F" 2
...
! *****************************************************************************
  SUBROUTINE staging_env_create(staging_env,staging_section,p,kT)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(staging_env)))CALL cp__a("motion/pint_staging.F",53)
    ALLOCATE(staging_env)
...
! *****************************************************************************
  SUBROUTINE staging_release(staging_env)
    TYPE(staging_env_type), POINTER          :: staging_env
    ...
    IF (ASSOCIATED(staging_env)) THEN
     IF(.NOT.(staging_env%ref_count>0))CALL cp__a("motion/pint_staging.F",85)
     staging_env%ref_count=staging_env%ref_count-1
     IF (staging_env%ref_count==0) THEN
        DEALLOCATE(staging_env)
     END IF
    END IF
    NULLIFY(staging_env)
    RETURN
...
!> \author Fawzi Mohamed
! *****************************************************************************
  SUBROUTINE staging_retain(staging_env)
    TYPE(staging_env_type), POINTER          :: staging_env

    CHARACTER(len=*), PARAMETER :: routineN = 'staging_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(staging_env)))CALL cp__a("motion/pint_staging.F",106)
    IF(.NOT.(staging_env%ref_count>0))CALL cp__a("motion/pint_staging.F",107)
    staging_env%ref_count=staging_env%ref_count+1
    RETURN
...
! *****************************************************************************
  SUBROUTINE staging_x2u(staging_env,ux,x)
    ...
    IF(.NOT.(ASSOCIATED(staging_env)))CALL cp__a("motion/pint_staging.F",197)
    IF(.NOT.(staging_env%ref_count>0))CALL cp__a("motion/pint_staging.F",198)
    ux=x
...
! *****************************************************************************
  SUBROUTINE staging_u2x(staging_env,ux,x)
    ...
    IF(.NOT.(ASSOCIATED(staging_env)))CALL cp__a("motion/pint_staging.F",233)
    IF(.NOT.(staging_env%ref_count>0))CALL cp__a("motion/pint_staging.F",234)
    j=staging_env%j
...
! *****************************************************************************
  SUBROUTINE staging_f2uf(staging_env,uf,f)
    ...
    IF(.NOT.(ASSOCIATED(staging_env)))CALL cp__a("motion/pint_staging.F",283)
    IF(.NOT.(staging_env%ref_count>0))CALL cp__a("motion/pint_staging.F",284)
