# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_epr_ownutils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_epr_ownutils.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_epr_ownutils.F" 2
...
! *****************************************************************************
  SUBROUTINE epr_g_so(epr_env,current_env,qs_env,iB)
    ...
    INTEGER :: aint_precond, ia, iat, iatom, idir1, idir2, idir3, ikind, ir, &
      ispin, max_iter, natom, nkind, nspins, output_unit, precond_kind
    ...
    IF (gapw) THEN
       ...
       DO ispin = 1,nspins
          DO idir1 = 1,3
             ...
             IF(.NOT.(success))CALL cp__a("qs_linres_epr_ownutils.F",419)
...
! *****************************************************************************
  SUBROUTINE epr_g_soo(epr_env,current_env,qs_env,iB)
    ...
    INTEGER :: aint_precond, ia, iat, iatom, idir1, ikind, ir, iso, ispin, &
      max_iter, natom, nkind, nspins, output_unit, precond_kind
    INTEGER, DIMENSION(2)                    :: bo
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: gapw, paw_atom, soo_rho_hard, &
                                                success
    ...
    IF (gapw .AND. soo_rho_hard) THEN
       ...
       DO idir1 = 1,3
          ...
          success=find_coeffs(values=brho1_r(1)%pw,&
                  ...
                  preconditioner=precond,pool=auxbas_pw_pool,&
                  eps_r=eps_r,eps_x=eps_x,max_iter=max_iter)
          IF(.NOT.(success))CALL cp__a("qs_linres_epr_ownutils.F",702)
          CALL pw_spline_precond_release(precond)
       END DO ! idir1
