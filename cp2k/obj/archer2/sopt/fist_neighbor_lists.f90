# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_neighbor_lists.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_neighbor_lists.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_neighbor_lists.F" 2
...
! *****************************************************************************
  SUBROUTINE build_fist_neighbor_lists(atomic_kind_set, particle_set, &
    ...
    IF (PRESENT(exclusions)) THEN
      IF(.NOT.(present_local_particles))CALL cp__a("fist_neighbor_lists.F",156)
    ENDIF
...
! *****************************************************************************
  SUBROUTINE build_neighbor_lists(nonbonded, particle_set, atom,cell, &
    ...
    INTEGER :: a_i, a_j, a_k, atom_a, atom_b, b_i, b_j, b_k, b_pi, b_pj, &
      ...
      kcell, kx, natom_local_a, ncellmax, nkind, nkind00, tmpdim, xdim, ydim, &
      zdim
          ...
          DO jcell=-imax_cell,imax_cell
             DO icell=-imax_cell,imax_cell
                IF (cellmap(icell,jcell,kcell)==-1) THEN
                   ...
                   IF(.NOT.(imap<=nonbonded%nlists))CALL cp__a("fist_neighbor_lists.F",389)
                   ...
                   IF (check_spline) THEN
                      ...
                      IF (rab2 < rab2_min) THEN
                         ...
                         IF (rab2 < rab2_min/(1.06_dp)**2) THEN
                            IF (geo_check) THEN
                               CALL cp__b("fist_neighbor_lists.F",626,"GEOMETRY wrong or EMAX_SPLINE too small!")
                            END IF
                         END IF
                      END IF
                   END IF
