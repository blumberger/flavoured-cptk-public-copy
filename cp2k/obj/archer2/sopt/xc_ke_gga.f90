# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_ke_gga.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_ke_gga.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_ke_gga.F" 2
...
! *****************************************************************************
  SUBROUTINE ke_gga_lda_eval ( functional, rho_set, deriv_set, order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_ke_gga.F",201)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_ke_gga.F",202)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_ke_gga.F",203)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_ke_gga.F",204)
...
!       tact = t13
    ...
    CASE (ke_ol1)
       ...
       CALL cp__b("xc/xc_ke_gga.F",227,"OL1 functional currently not working properly")
    CASE (ke_ol2)
       CALL efactor_ol2 ( s, fs, m )
       CALL cp__b("xc/xc_ke_gga.F",230,"OL2 functional currently not working properly")
    CASE (ke_llp)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_ke_gga.F",301,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE ke_gga_lsd_eval ( functional, rho_set, deriv_set, order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_ke_gga.F",352)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_ke_gga.F",353)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_ke_gga.F",354)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_ke_gga.F",355)
    ...
    DO ispin=1,2
       ...
       END IF
       IF ( order>3.OR.order<-3) THEN
          CALL cp__b("xc/xc_ke_gga.F",467,"derivatives bigger than 3 not implemented")
       END IF
  ...
  SUBROUTINE efactor_ol1 ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE (2:3)
         ...
         fs(ip,3) = 2.0_dp*t1
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",722,"Illegal order.")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_ol2 ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",775,"Illegal order.")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_llp ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",946,"Illegal order.")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_pw86 ( s, fs, m , f2_lsd)
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",1020,"Illegal order.")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_t92 ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",1114,"Illegal order")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_pbex ( s, fs, m, pset,f2_lsd )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_ke_gga.F",1176,"Illegal order.")
      END SELECT
    END DO
