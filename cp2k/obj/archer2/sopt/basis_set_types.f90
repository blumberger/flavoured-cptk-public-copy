# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/basis_set_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/basis_set_types.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/basis_set_types.F" 2
...
! *****************************************************************************
  SUBROUTINE copy_gto_basis_set(basis_set_in, basis_set_out)
    ...
    IF(.NOT.(ASSOCIATED(basis_set_in)))CALL cp__a("aobasis/basis_set_types.F",253)
...
! *****************************************************************************
  SUBROUTINE combine_basis_sets(basis_set, basis_set_add)
    ...
    INTEGER :: iset, ishell, lshell, maxco, maxpgf, maxshell, nc, ncgf, &
      ncgfn, ncgfo, ns, nset, nsetn, nseto, nsgf, nsgfn, nsgfo
    TYPE(gto_basis_set_type), POINTER        :: bad, bout

    IF(.NOT.(ASSOCIATED(basis_set)))CALL cp__a("aobasis/basis_set_types.F",334)
    IF(.NOT.(ASSOCIATED(basis_set_add)))CALL cp__a("aobasis/basis_set_types.F",335)
...
! -------------------------------------------------------------------------
    ...
    ELSE
       CALL cp__b("aobasis/basis_set_types.F",611,"The pointer gto_basis_set is not associated")
    END IF
...
! -------------------------------------------------------------------------
    ...
    CASE DEFAULT
       CALL cp__b("aobasis/basis_set_types.F",648,"Normalization method not specified")
    END SELECT
...
! -------------------------------------------------------------------------
    ...
    CASE DEFAULT
       CALL cp__b("aobasis/basis_set_types.F",982,"Normalization method not specified")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE read_gto_basis_set1(element_symbol,basis_set_name,gto_basis_set,&
    ...
    INTEGER :: i, ibasis, ico, ipgf, irep, iset, ishell, lshell, m, maxco, &
      maxl, maxpgf, maxshell, nbasis, ncgf, nmin, nset, nsgf, strlen1, strlen2
    ...
    IF (tmp .NE. "NONE") THEN
      IF( .NOT. basis_found ) THEN
        ...
        END DO
        CALL cp_abort(cp__l("aobasis/basis_set_types.F",1346),&
...
! *****************************************************************************
  SUBROUTINE read_gto_basis_set2(element_symbol,basis_type,gto_basis_set,&
    ...
    IF (.NOT.is_ok) CALL cp__b("aobasis/basis_set_types.F",1459,"Error reading the Basis set from input file!!")
    ...
    DO iset=1,nset
       is_ok=cp_sll_val_next(list,val)
       IF (.NOT.is_ok) CALL cp__b("aobasis/basis_set_types.F",1475,"Error reading the Basis set from input file!!")
       CALL val_get(val,c_val=line_att)
       ...
       IF (LEN_TRIM(line_att)/=0)&
          CALL cp__b("aobasis/basis_set_types.F",1509,"Error reading the Basis from input file!!")
       DO ipgf=1,npgf(iset)
          is_ok=cp_sll_val_next(list,val)
          IF (.NOT.is_ok) CALL cp__b("aobasis/basis_set_types.F",1512,"Error reading the Basis set from input file!!")
          CALL val_get(val,c_val=line_att)
...
! -------------------------------------------------------------------------
    ...
    ELSE
       CALL cp__b("aobasis/basis_set_types.F",1743,"The pointer gto_basis_set is not associated")
    END IF
...
! -------------------------------------------------------------------------
    ...
    IF (ASSOCIATED(sto_basis_set)) THEN
       ...
       END IF
    ELSE
       CALL cp__b("aobasis/basis_set_types.F",1983,"The pointer sto_basis_set is not associated")
    END IF
...
! -------------------------------------------------------------------------
    ...
    IF (ASSOCIATED(sto_basis_set)) THEN
       ...
       END IF
    ELSE
       CALL cp__b("aobasis/basis_set_types.F",2042,"The pointer sto_basis_set is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE create_gto_from_sto_basis(sto_basis_set,gto_basis_set,ngauss)
    ...
    IF (ng > maxng) CALL cp__b("aobasis/basis_set_types.F",2073,"Too many Gaussian primitives requested")
