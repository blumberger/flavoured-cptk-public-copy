# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_exchange.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_exchange.F"
!-----------------------------------------------------------------------------!
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_exchange.F" 2
...
! *****************************************************************************
  SUBROUTINE build_fock_matrix_exchange (qs_env, ks_matrix, matrix_p, calculate_forces,&
    ...
    LOGICAL                                  :: anag, check, defined, found, &
                                                switch, use_virial
    ...
    IF(.NOT.(ASSOCIATED(matrix_p)))CALL cp__a("se_fock_matrix_exchange.F",136)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix_exchange.F",137)
  ...
  DO WHILE (neighbor_list_iterate(nl_iterator)==0)
     ...
     IF(.NOT.(ASSOCIATED(ks_block_a)))CALL cp__a("se_fock_matrix_exchange.F",188)
     CALL cp_dbcsr_get_block_p(matrix=matrix_p(1)%matrix,&
          row=irow,col=icol,BLOCK=p_block_a,found=found)
     IF(.NOT.(ASSOCIATED(p_block_a)))CALL cp__a("se_fock_matrix_exchange.F",191)
     ...
     IF ( nspins == 2 ) THEN
        ...
        IF(.NOT.(ASSOCIATED(ks_block_b)))CALL cp__a("se_fock_matrix_exchange.F",200)
        CALL cp_dbcsr_get_block_p(matrix=matrix_p(2)%matrix,&
             row=irow,col=icol,BLOCK=p_block_b,found=found)
        IF(.NOT.(ASSOCIATED(p_block_b)))CALL cp__a("se_fock_matrix_exchange.F",203)
        check = (size_p_block_a(1)==SIZE(p_block_b,1)).AND.(size_p_block_a(2)==SIZE(p_block_b,2))
        IF(.NOT.(check))CALL cp__a("se_fock_matrix_exchange.F",205)
