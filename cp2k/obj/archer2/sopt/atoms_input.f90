# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atoms_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atoms_input.F"
!-----------------------------------------------------------------------------!
...
# 50 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atoms_input.F" 2
...
! *****************************************************************************
  SUBROUTINE read_atoms_input ( topology, overwrite, subsys_section, save_mem)
    ...
    IF (my_overwrite) THEN
       IF(.NOT.(SIZE(atom_info%r,2)==natom))CALL cp__a("atoms_input.F",124)
       CALL cp_warn(cp__l("atoms_input.F",125),&
       ...
       DO iatom=1,natom
          ...
          DO wrd=1,4
             ...
             IF(LEN_TRIM(line_att(start_c:end_c-1))==0)&
                CALL cp__b("atoms_input.F",149,"incorrectly formatted line in coord section'"//line_att//"'")
             IF (wrd==1) THEN
    ...
    ELSE
       ...
       DO iatom=1,natom
          ...
          DO wrd=1,6
             ...
             END DO
             IF(LEN_TRIM(line_att(start_c:end_c-1)) == 0)&
                CALL cp_abort(cp__l("atoms_input.F",200),&
             ...
             CASE (2:4)
                ...
                IF(LEN_TRIM(error_message) /= 0)&
                   CALL cp_abort(cp__l("atoms_input.F",212),&
                        "Incorrectly formatted input line for atom "//&
                        TRIM(ADJUSTL(cp_to_string(iatom)))//&
...
! *****************************************************************************
  SUBROUTINE read_shell_coord_input ( particle_set, shell_particle_set, cell,&
    ...
    INTEGER                                  :: end_c, handle, ishell, j, &
                                                nshell, output_unit, &
                                                sh_index, start_c, wrd
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: at_index, at_index_c
    LOGICAL :: core_scaled_coordinates, explicit, is_ok, is_shell, &
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("atoms_input.F",315)
    !-----------------------------------------------------------------------------
    !-----------------------------------------------------------------------------
    ...
    IF (ASSOCIATED(shell_particle_set)) THEN
      IF(.NOT.((SIZE(shell_particle_set,1) == nshell)))CALL cp__a("atoms_input.F",327)
      ALLOCATE (r(3,nshell),at_name(nshell),at_index(nshell))
      CALL cp_warn(cp__l("atoms_input.F",329),&
      ...
      DO ishell=1,nshell
         ...
         DO wrd=1,5
            ...
            IF(wrd/=5.AND.end_c>=LEN(line_att)+1)&
               CALL cp__b("atoms_input.F",353,"incorrectly formatted line in coord section'"//line_att//"'")
            IF (wrd==1) THEN
      ...
      IF (PRESENT(core_particle_set)) THEN
        IF(.NOT.(ASSOCIATED(core_particle_set)))CALL cp__a("atoms_input.F",367)
        ...
        IF (explicit) THEN
          ...
          IF(.NOT.((SIZE(core_particle_set,1) == nshell)))CALL cp__a("atoms_input.F",377)
          ALLOCATE (rc(3,nshell),at_name_c(nshell),at_index_c(nshell))
          CALL cp_warn(cp__l("atoms_input.F",379),&
          ...
          DO ishell=1,nshell
             ...
             DO wrd=1,5
                ...
                END DO
                IF(wrd/=5.AND.end_c>=LEN(line_att)+1)&
                   CALL cp__b("atoms_input.F",403,"incorrectly formatted line in coord section'"//line_att//"'")
                IF (wrd==1) THEN
      ...
      DO ishell=1,nshell
         ...
         IF((TRIM(at_name(ishell))==TRIM(name_kind)).AND. is_shell) THEN
           ...
           ELSE IF(explicit) THEN
             ...
             rab = pbc(shell_particle_set(sh_index)%r,core_particle_set(sh_index)%r,cell)
             IF(.NOT.(TRIM(at_name(ishell))==TRIM(at_name_c(ishell))))CALL cp__a("atoms_input.F",453)
             IF(.NOT.(at_index(ishell)==at_index_c(ishell)))CALL cp__a("atoms_input.F",454)
           ELSE
         ...
         ELSE
           CALL cp__b("atoms_input.F",467,"shell coordinate assigned to the wrong atom. check the shell indexes in the input")
         END IF
      END DO
