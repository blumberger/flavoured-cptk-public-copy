# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/gopt_f77_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/gopt_f77_methods.F"
!-----------------------------------------------------------------------------!
...
# 81 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/gopt_f77_methods.F" 2
    IMPLICIT NONE
    ...
    CASE (default_minimization_method_id,default_ts_method_id)
       ...
       CASE (default_ts_method_id)
          ...
          IF (PRESENT(gradient)) THEN
             IF (master == para_env%mepos) THEN ! we are on the master
                IF(.NOT.(ASSOCIATED(gradient)))CALL cp__a("motion/gopt_f77_methods.F",165)
    ...
    CASE (default_cell_method_id)
       ...
       IF(.NOT.virial%pv_availability)&
          CALL cp_abort(cp__l("motion/gopt_f77_methods.F",177),&
       ...
       CASE (default_cell_direct_id)
          ...
          ! Possibly take the gradients
          IF (PRESENT(gradient)) THEN
             IF(.NOT.(ANY(virial_avg%pv_total /= 0)))CALL cp__a("motion/gopt_f77_methods.F",199)
             ! Convert the average ptens
             av_ptens(:,:) = virial_avg%pv_total(:,:)/cell%deth
             IF (master == para_env%mepos) THEN ! we are on the master
                IF(.NOT.(ASSOCIATED(gradient)))CALL cp__a("motion/gopt_f77_methods.F",203)
                nparticle = force_env_get_nparticle(gopt_env%force_env)
                nsize = 3*nparticle
                IF(.NOT.((SIZE(gradient) == nsize + 6)))CALL cp__a("motion/gopt_f77_methods.F",206)
       ...
       CASE (default_cell_geo_opt_id,default_cell_md_id)
          ...
          ELSE
             ...
             CASE (default_cell_geo_opt_id)
                ...
                IF(.NOT.explicit)&
                   CALL cp_abort(cp__l("motion/gopt_f77_methods.F",240),&
             ...
             CASE(default_cell_md_id)
                ...
                CALL section_vals_get(work,explicit=explicit)
                IF(.NOT.explicit)&
                   CALL cp_abort(cp__l("motion/gopt_f77_methods.F",254),&
                ...
                IF(ensemble/=nvt_ensemble)&
                 CALL cp__b("motion/gopt_f77_methods.F",259,"Cell optimization at finite temperature requires the NVT MD ensemble!")
             ...
             CASE DEFAULT
                CALL cp__b("motion/gopt_f77_methods.F",271,"")
             END SELECT
          ...
          IF (PRESENT(gradient)) THEN
             IF(.NOT.(ANY(virial_avg%pv_total/=0)))CALL cp__a("motion/gopt_f77_methods.F",285)
             ! Convert the average ptens
             av_ptens(:,:) = virial_avg%pv_total(:,:)/cell%deth
             IF (master == para_env%mepos) THEN ! we are on the master
                IF(.NOT.(ASSOCIATED(gradient)))CALL cp__a("motion/gopt_f77_methods.F",289)
          ...
          CALL virial_release(virial_avg)
       CASE DEFAULT
          CALL cp__b("motion/gopt_f77_methods.F",302,"")
       END SELECT
    CASE (default_shellcore_method_id)
       ...
       IF (PRESENT(gradient)) THEN
         IF (master == para_env%mepos) THEN ! we are on the master
            IF(.NOT.(ASSOCIATED(gradient)))CALL cp__a("motion/gopt_f77_methods.F",330)
            idg = 0
       ...
       END IF
    CASE DEFAULT
       CALL cp__b("motion/gopt_f77_methods.F",341,"")
    END SELECT
