# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_input.F"
!-----------------------------------------------------------------------------!
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_input.F" 2
...
SUBROUTINE read_topology_section ( topology, topology_section)
  ...
  IF(ival>1)&
     CALL cp__b("topology_input.F",67,"Only one between <CHARGE_OCCUP,CHARGE_BETA,CHARGE_EXTENDED> can be defined! ")
  ...
  IF (ASSOCIATED(constraint_section)) THEN
     ...
     IF (explicit) THEN
        ...
        DO ig = 1, ncons
           ...
           IF  ((cons_info%const_vsite_mol(ig)/=0).AND.(cons_info%const_vsite_molname(ig)/="UNDEF")) THEN
              CALL cp__b("topology_input.F",354,"")
           END IF
           IF  ((cons_info%const_vsite_mol(ig)==0).AND.(cons_info%const_vsite_molname(ig)=="UNDEF").AND.&
              ...
              CALL cp__b("topology_input.F",358,"")
           END IF
     ...
     IF (explicit) THEN
        ...
        DO ig = 1, ncons
           ...
           IF (nrep==1 .AND. explicit) THEN
              IF(.NOT.(cons_info%freeze_mm==do_constr_none))CALL cp__a("topology_input.F",453)
           ...
           CALL section_vals_val_get(fix_atom_section,"QM_SUBSYS",i_rep_section=ig,&
                n_rep_val=nrep,explicit=explicit)
           IF (nrep==1 .AND. explicit) THEN
              IF(.NOT.(cons_info%freeze_qm==do_constr_none))CALL cp__a("topology_input.F",461)
     ...
     IF (explicit) THEN
        ...
        DO ig = 1, ncons
           ...
           IF(.NOT.(icolvar<=SIZE(colvar_p)))CALL cp__a("topology_input.F",494)
        END DO
        ...
        DO ig = 1, ncons
           ...
           IF  (((cons_info%const_colv_mol(ig)/=0).AND.(cons_info%const_colv_molname(ig)/="UNDEF"))) THEN
              CALL cp__b("topology_input.F",543,"Both MOLNAME and MOLECULE specified for CONSTRAINT section. ")
           END IF
           IF  ((cons_info%const_colv_mol(ig)==0).AND.(cons_info%const_colv_molname(ig)=="UNDEF").AND.&
                (.NOT.cons_info%colv_intermolecular(ig))) THEN
              CALL cp_abort(cp__l("topology_input.F",547),&
