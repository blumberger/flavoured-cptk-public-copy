# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_util.F"
!-----------------------------------------------------------------------------!
...
MODULE force_fields_util
  ...
  USE force_field_kind_types,          ONLY: &
       ...
       bond_kind_type, deallocate_bend_kind_set, deallocate_bond_kind_set, &
       do_ff_undef, impr_kind_dealloc_ref, impr_kind_type, opbend_kind_type, &
...
# 79 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_util.F" 2
...
! *****************************************************************************
  SUBROUTINE force_field_pack (particle_set,atomic_kind_set,molecule_kind_set, &
    ...
    INTEGER                                  :: handle, iw, iw2, iw3, iw4, &
                                                output_unit
    LOGICAL                                  :: do_zbl, explicit, fatal, &
                                                ignore_fatal, my_qmmm
  ...
  SUBROUTINE release_FF_missing_par(fatal, ignore_fatal, array, output_unit, iw)
    ...
    IF (fatal) THEN
      IF (ignore_fatal) THEN
        ...
        END IF
      ELSE
        CALL cp__b("force_fields_util.F",401,"Missing critical ForceField parameters!")
      END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE get_generic_info(gen_section, func_name, xfunction, parameters, values,&
    ...
    IF (PRESENT(size_variables)) THEN
       IF(.NOT.(mydim==size_variables))CALL cp__a("force_fields_util.F",1321)
    END IF
    ...
    IF(.NOT.(check))CALL cp__a("force_fields_util.F",1327)
    ...
    IF (n_par>0) THEN
       ...
       END DO
       IF(.NOT.(SIZE(my_par)==SIZE(my_val)))CALL cp__a("force_fields_util.F",1351)
       ...
       IF (n_units > 0) THEN
          DO i=1,n_units
             ...
             END DO
          END DO
          IF(.NOT.(SIZE(my_units)==SIZE(my_val)))CALL cp__a("force_fields_util.F",1367)
       END IF
    ...
    IF (mydim>0) THEN
       ...
       IF (PRESENT(var_values)) THEN
          IF(.NOT.(SIZE(var_values)==SIZE(my_var)))CALL cp__a("force_fields_util.F",1383)
