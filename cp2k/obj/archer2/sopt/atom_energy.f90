# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_energy.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_energy.F"
!-----------------------------------------------------------------------------!
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_energy.F" 2
...
! *****************************************************************************
  SUBROUTINE atom_energy_opt(atom_section)
    ...
    INTEGER :: do_eric, do_erie, handle, i, im, in, iw, k, l, maxl, mb, &
    ...
    DO in = 1, n_rep
      DO im = 1, n_meth
        ...
        IF ( pp_calc ) THEN
          ...
          IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_energy.F",292)
          had_pp = .TRUE.
        ELSE
          ! general integrals
          ...
          IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_energy.F",305)
          had_ae = .TRUE.
        END IF
        ...
        IF(atom_consistent_method(method,state%multiplicity)) THEN
          ...
          iw = cp_print_key_unit_nr(logger,atom_section,"PRINT%ENERGIES_MINUS_KINETIC",extension=".log")
          IF (iw > 0) THEN
...
! *****************************************************************************
  SUBROUTINE atom_response_basis(atom,delta,nder,iw)
    ...
    ALLOCATE(wfn(s1,s2,0:3,-nder:nder))
    s2=MAXVAL(state%maxn_occ)+nder
    ALLOCATE(rbasis(s1,s2,0:3))
    rbasis = 0._dp

    DO ider=-nder,nder
      dene = REAL(ider,KIND=dp)*delta
      IF(.NOT.(fhomo>ABS(dene)))CALL cp__a("atom_energy.F",529)
...
! *****************************************************************************
  SUBROUTINE atom_write_upf(atom,iw)
    ...
    IF(.NOT.(.NOT.pot%lsdpot))CALL cp__a("atom_energy.F",626)
    ...
    IF(pot%nlcc) THEN
       ...
       ALLOCATE(corden(nr))
       CALL atom_core_density(corden,pot,"RHO",atom%basis%grid%rad)
       IF(up) THEN
