# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner_makes.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner_makes.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/preconditioner_makes.F" 2
  ...
  SUBROUTINE make_preconditioner_matrix(preconditioner_env, matrix_h, matrix_s, matrix_t, mo_coeff,&
    ...
    CASE (ot_precond_full_single)
       IF(my_solver_type.NE.ot_precond_solver_default) &
          CALL cp__b("preconditioner_makes.F",97,"Only PRECOND_SOLVER DEFAULT for the moment")
    ...
    CASE (ot_precond_s_inverse)
       ...
       IF (.NOT. PRESENT(matrix_s) ) &
          CALL cp__b("preconditioner_makes.F",109,"Type for S=1 not implemented")
    ...
    CASE (ot_precond_full_kinetic)
       ...
       IF (.NOT.( PRESENT(matrix_s) .AND. PRESENT(matrix_t) )) &
          CALL cp__b("preconditioner_makes.F",115,"Type for S=1 not implemented")
    ...
    CASE (ot_precond_full_all)
       IF(my_solver_type.NE.ot_precond_solver_default) THEN
          CALL cp__b("preconditioner_makes.F",123,"Only PRECOND_SOLVER DEFAULT for the moment")
       ENDIF
       IF ( PRESENT(matrix_s) ) THEN
    ...
    CASE DEFAULT
       CALL cp__b("preconditioner_makes.F",134,"Type not implemented")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE make_full_s_inverse(preconditioner_env, matrix_s)
    ...
    IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("preconditioner_makes.F",155)
...
! *****************************************************************************
  SUBROUTINE make_full_kinetic(preconditioner_env, matrix_t, matrix_s, &
    ...
    IF(.NOT.(ASSOCIATED(matrix_t)))CALL cp__a("preconditioner_makes.F",189)
    IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("preconditioner_makes.F",190)
...
! *****************************************************************************
  SUBROUTINE make_full_single(preconditioner_env, fm, matrix_h, matrix_s, &
    ...
    CALL cp_gemm('N','T',n,n,n,1.0_dp,fm,fm_h,0.0_dp,fm_s)
...
SUBROUTINE make_full_all(preconditioner_env, matrix_c0, matrix_h, matrix_s, c0_evals, energy_gap)
  ...
  CALL timeset(routineN,handle)
    ...
    ! tmp = H ( 1 - PS )
    CALL cp_gemm('N','T',n,n,k,-1.0_dp,matrix_hc0,matrix_sc0,1.0_dp,matrix_tmp)
    ...
    CALL cp_fm_struct_release(fm_struct_tmp)
    CALL cp_gemm('T','N',k,n,n,1.0_dp,matrix_c0,matrix_tmp,0.0_dp,matrix_left)
    ! tmp = (1 - PS)^T H (1-PS)
    CALL cp_gemm('N','N',n,n,k,-1.0_dp,matrix_sc0,matrix_left,1.0_dp,matrix_tmp)
    ...
    IF (.FALSE.) THEN
        ...
        CALL cp_fm_struct_release(fm_struct_tmp)
        ALLOCATE(norms(k))
        CALL cp_gemm('T','N',k,k,n,1.0_dp,matrix_sc0,matrix_tmp,0.0_dp,matrix_s1)
...
SUBROUTINE make_full_all_ortho(preconditioner_env, matrix_c0, matrix_h, c0_evals, energy_gap)
  ...
  CALL timeset(routineN,handle)
    ...
    CALL cp_dbcsr_sm_fm_multiply(matrix_h,matrix_c0,matrix_hc0,k)
       ...
       CALL cp_fm_struct_release(fm_struct_tmp)
       ! since we only use diagonal elements this is a bit of a waste
       CALL cp_gemm('T','N',k,k,n,1.0_dp,matrix_hc0,matrix_hc0,0.0_dp,matrix_s1)
       ALLOCATE(diag(k))
       CALL cp_fm_get_diag(matrix_s1,diag)
       error_estimate=MAXVAL(SQRT(ABS(diag-c0_evals**2)))
       DEALLOCATE(diag)
...
SUBROUTINE make_full_single_inverse(preconditioner_env, matrix_c0, matrix_h, energy_gap, matrix_s)
    ...
    CALL cp_dbcsr_init(dbcsr_sc)
    CALL cp_dbcsr_create(dbcsr_sc,template=mo_dbcsr)
    CALL cp_dbcsr_init(dbcsr_hc)
    CALL cp_dbcsr_create(dbcsr_hc,template=mo_dbcsr)
    CALL cp_dbcsr_init(dbcsr_cThc)
    ...
    CALL cp_dbcsr_copy(preconditioner_env%sparse_matrix,matrix_h)

    ! Precompute some matrices
    ! S*C, if orthonormal this will be simply C so a copy will do
    IF(PRESENT(matrix_s))THEN
       CALL cp_dbcsr_multiply("N", "N",1.0_dp,matrix_s,mo_dbcsr,0.0_dp,dbcsr_sc)
    ELSE
       CALL cp_dbcsr_copy(dbcsr_sc,mo_dbcsr)
    END IF

!----------------------------compute the occupied subspace and shift it ------------------------------------
    ! cT*H*C which will be used to shift the occupied states to 0
    CALL cp_dbcsr_multiply("N", "N",1.0_dp,matrix_h,mo_dbcsr,0.0_dp,dbcsr_hc)
    CALL cp_dbcsr_multiply("T", "N",1.0_dp,mo_dbcsr,dbcsr_hc,0.0_dp,dbcsr_cThc)
    ...
    CALL get_selected_ritz_vec(my_arnoldi,1,matrices(1)%matrix,preconditioner_env%max_ev_vector)
    ...
    CALL cp_dbcsr_add_on_diag(dbcsr_cThc,-0.5_dp)
    ! Get the AO representation of the shift (see above why S is needed), W-matrix like object
    CALL cp_dbcsr_multiply("N", "N",2.0_dp,dbcsr_sc,dbcsr_cThc,0.0_dp,dbcsr_hc)
...
!-------------------------------------compute eigenvalues of H ----------------------------------------------
    ...
    END IF
    IF(PRESENT(matrix_s))THEN
       CALL cp_dbcsr_add(preconditioner_env%sparse_matrix,matrix_s,1.0_dp,pre_shift)
    ELSE
       CALL cp_dbcsr_add_on_diag(preconditioner_env%sparse_matrix,pre_shift)
    END IF

    CALL cp_dbcsr_release(mo_dbcsr)
    CALL cp_dbcsr_release(dbcsr_hc)
    CALL cp_dbcsr_release(dbcsr_sc)
    CALL cp_dbcsr_release(dbcsr_cThc)
