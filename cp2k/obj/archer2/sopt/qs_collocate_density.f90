# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F"
!-----------------------------------------------------------------------------!
...
# 138 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F" 2
...
!> \param scpv ...
! *****************************************************************************
  SUBROUTINE calculate_scp_charge(scp,qs_env,scpv)
    ...
    INTEGER                                  :: atom_a, handle, i, iatom, ii, &
                                                ikind, ithread, j, jj, l, &
                                                lmax_global, lmaxscp, natom, &
                                                ni, nj, npme, nthread
    ...
    NULLIFY(cell,dft_control,pab,qs_kind_set,particle_set,atomic_kind_set,&
    ...
    DO ikind=1,SIZE(atomic_kind_set)
       ...
       CALL get_scptb_parameter(scptb_kind,defined=defined,lmaxscp=lmaxscp)
       lmax_global=MAX(lmaxscp,lmax_global)
    END DO
    ...
    CALL rs_pw_transfer(rs_rho,scp%pw,rs2pw)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_nlcc(rho_nlcc,qs_env)
    ...
    INTEGER :: atom_a, handle, iatom, iexp_nlcc, ikind, ithread, j, &
      lmax_global, n, natom, nc, nexp_nlcc, ni, npme, nthread
    INTEGER(KIND=int_8)                      :: subpatch_pattern
    INTEGER, DIMENSION(:), POINTER           :: atom_list, cores, mylmax, &
                                                nct_nlcc
    LOGICAL                                  :: nlcc
    ...
    DO ikind=1,SIZE(atomic_kind_set)
      ...
      CALL get_potential(potential=gth_potential,nct_nlcc=mylmax)
      ...
      DO iexp_nlcc=1,nexp_nlcc
         ...
         DO j=1,nc
           SELECT CASE (j)
             ...
             CASE DEFAULT
               CALL cp__b("qs_collocate_density.F",449,"")
           END SELECT
         END DO
...
! *****************************************************************************
  SUBROUTINE calculate_ppl_grid(vppl,qs_env)
    ...
    INTEGER                                  :: atom_a, handle, iatom, ikind, &
                                                ithread, j, lmax_global, &
                                                lppl, n, natom, ni, npme, &
                                                nthread
    INTEGER(KIND=int_8)                      :: subpatch_pattern
    INTEGER, DIMENSION(:), POINTER           :: atom_list, cores
    REAL(KIND=dp)                            :: alpha, eps_rho_rspace
    ...
    DO ikind=1, SIZE(atomic_kind_set)
      ...
      DO j=1,lppl
        SELECT CASE (j)
          ...
          CASE DEFAULT
            CALL cp__b("qs_collocate_density.F",639,"")
        END SELECT
      END DO
...
! *****************************************************************************
  SUBROUTINE calculate_lri_rho_elec(lri_rho_g, lri_rho_r, qs_env, &
    ...
    INTEGER :: atom_a, dir, group_size, handle, iatom, ierr, igrid_level, &
      ikind, ipgf, iset, lmax_global, m1, maxco, my_pos, na1, natom, ncoa, &
      nseta, offset, sgfa
    ...
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_collocate_density.F",765)
    ...
    CALL get_qs_kind_set(qs_kind_set=qs_kind_set,&
                             maxco=maxco,basis_type="LRI")

    ALLOCATE (pab(maxco,1),STAT=ierr)
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,787,&
                                    "pab",maxco*dp_size)
    offset = 0
...
! *****************************************************************************
  SUBROUTINE calculate_rho_core(rho_core,total_rho,qs_env,only_nopaw)
    ...
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,959,&
                                    "pab",dp_size)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_single_gaussian(rho_gb,qs_env,iatom_in)
    ...
    ALLOCATE (pab(1,1),STAT=ierr)
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,1085,&
                                    "pab",dp_size)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_metal(rho_metal,coeff,total_rho_metal,qs_env)
    ...
    ALLOCATE (pab(1,1),STAT=ierr)
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,1179,&
                                      "pab",dp_size)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_resp_single(rho_gb,qs_env,eta,iatom_in)
    ...
    ALLOCATE (pab(1,1),STAT=ierr)
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,1284,&
                                    "pab",dp_size)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_resp_all(rho_resp,coeff,natom,eta,qs_env)
    ...
    ALLOCATE (pab(1,1),STAT=ierr)
    IF (ierr /= 0) CALL stop_memory(routineN,moduleN,1382,&
                                      "pab",dp_size)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_elec(matrix_p, matrix_p_kp, rho, rho_gspace, total_rho,&
    ...
    INTEGER :: bcol, brow, ga_gb_function, handle, iatom, iatom_old, &
      igrid_level, igrid_level_dummy, ikind, ikind_old, img, img_old, ipair, &
      ipgf, iset, iset_old, itask, ithread, jatom, jatom_old, jkind, &
      jkind_old, jpgf, jset, jset_old, lb, lbr, lbw, lmax_global, maxco, &
      maxpgf, maxset, maxsgf, maxsgf_set, my_der_type, my_idir, n, na1, na2, &
      natoms, nb1, nb2, nblock, ncoa, ncob, nimages, nr, nrlevel, nseta, &
      nsetb, ntasks, nthread, nxy, nz, nzsize, sgfa, sgfb, ub
    INTEGER(kind=int_8), DIMENSION(:), &
      POINTER                                :: atom_pair_recv, atom_pair_send
    INTEGER(kind=int_8), DIMENSION(:, :), &
      POINTER                                :: tasks
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, lb_max, &
                                                lb_min, mylmax, npgfa, npgfb, &
                                                nsgfa, nsgfb
    ...
    IF(.NOT.(PRESENT(matrix_p).OR.PRESENT(matrix_p_kp)))CALL cp__a("qs_collocate_density.F",1534)
    ...
    IF(.NOT.(nimages==1.OR.do_kp))CALL cp__a("qs_collocate_density.F",1603)
...
!$  nthread = omp_get_max_threads()
    ...
    CASE(FUNC_AB)
        lmax_global=lmax_global
    CASE DEFAULT
        CALL cp__b("qs_collocate_density.F",1667,"")
    END SELECT

    ! get the task lists
    IF (my_soft) task_list=>task_list_soft
    IF(.NOT.(ASSOCIATED(task_list)))CALL cp__a("qs_collocate_density.F",1672)
    ...
    ! *** set up the pw multi-grids
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_collocate_density.F",1680)
    CALL pw_env_get(pw_env, rs_descs=rs_descs, rs_grids=rs_rho, lgrid=lgrid)
    ! *** set up the rs multi-grids
    IF(.NOT.(ASSOCIATED(rs_rho)))CALL cp__a("qs_collocate_density.F",1683)
    IF(.NOT.(ASSOCIATED(rs_descs)))CALL cp__a("qs_collocate_density.F",1684)
    IF(.NOT.(ASSOCIATED(lgrid)))CALL cp__a("qs_collocate_density.F",1685)
    ...
    NULLIFY(deltap)
    CALL cp_dbcsr_allocate_matrix_set(deltap,nimages)
    IF (distributed_rs_grids) THEN
       DO img=1,nimages
          ALLOCATE(deltap(img)%matrix)
          CALL cp_dbcsr_init(deltap(img)%matrix)
       END DO
       ! this matrix has no strict sparsity pattern in parallel
       ! deltap%sparsity_id=-1
       IF(do_kp) THEN
          DO img=1,nimages
             CALL cp_dbcsr_copy(deltap(img)%matrix,matrix_p_kp(img)%matrix,&
                                name="DeltaP")
          END DO
       ELSE
          CALL cp_dbcsr_copy(deltap(1)%matrix,matrix_p,name="DeltaP")
       END IF
    ELSE
       IF(do_kp) THEN
          DO img=1,nimages
...
!$omp          private(na1,na2,nb1,nb2,scale,use_subpatch,rab_inv,ithread,lb,ub,n), &
...
!$omp do schedule(dynamic, MAX(1,task_list%npairs(igrid_level)/(nthread*50)))
    ...
    loop_tasks: DO itask = task_list%taskstart(ipair,igrid_level), task_list%taskstop(ipair,igrid_level)
       ...
       IF (iatom .NE. iatom_old .OR. jatom .NE. jatom_old .OR. img .NE. img_old) THEN
          ...
          IF(.NOT.(found))CALL cp__a("qs_collocate_density.F",1827)
...
!$omp end parallel
    ...
    IF (distributed_rs_grids) THEN
       CALL cp_dbcsr_deallocate_matrix_set ( deltap)
    ELSE
       DO img=1,nimages
...
! *****************************************************************************
  SUBROUTINE calculate_drho_elec(matrix_p,matrix_p_kp,drho,drho_gspace,qs_env,&
    ...
    INTEGER :: bcol, brow, handle, i, iatom, iatom_old, idir, igrid_level, &
      ikind, ikind_old, img, img_old, ipgf, iset, iset_old, itask, ithread, &
      jatom, jatom_old, jkind, jkind_old, jpgf, jset, jset_old, lmax_global, &
      maxco, maxpgf, maxset, maxsgf, maxsgf_set, na1, na2, natoms, nb1, nb2, &
      ncoa, ncob, nimages, nseta, nsetb, ntasks, nthread, sgfa, sgfb
    INTEGER(kind=int_8), DIMENSION(:), &
      POINTER                                :: atom_pair_recv, atom_pair_send
    INTEGER(kind=int_8), DIMENSION(:, :), &
      POINTER                                :: tasks
    ...
    IF(.NOT.(PRESENT(matrix_p).OR.PRESENT(matrix_p_kp)))CALL cp__a("qs_collocate_density.F",2104)
    do_kp = PRESENT(matrix_p_kp)
    ...
    ! find maximum numbers
    nimages = dft_control%nimages
    IF(.NOT.(nimages==1 .OR. do_kp))CALL cp__a("qs_collocate_density.F",2161)
    ...
    ! get the task lists
    IF (my_soft) task_list=>task_list_soft
    IF(.NOT.(ASSOCIATED(task_list)))CALL cp__a("qs_collocate_density.F",2189)
    tasks  =>task_list%tasks
    dist_ab=>task_list%dist_ab
    atom_pair_send=>task_list%atom_pair_send
    atom_pair_recv=>task_list%atom_pair_recv
    ...
    ! *** set up the rs multi-grids
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_collocate_density.F",2197)
    ...
    loop_xyz: DO idir=1,3
      ...
      loop_tasks: DO itask=1,ntasks
        ...
        IF (iatom .NE. iatom_old .OR. jatom .NE. jatom_old .OR. img .NE. img_old) THEN
          ...
          CALL cp_dbcsr_get_block_p(matrix=deltap(img)%matrix,&
               row=brow,col=bcol,BLOCK=p_block,found=found)
          IF(.NOT.(found))CALL cp__a("qs_collocate_density.F",2306)
...
! *****************************************************************************
  SUBROUTINE calculate_wavefunction(mo_vectors,ivector,rho,rho_gspace, &
    ...
    CHARACTER(LEN=default_string_length)     :: my_basis_type
    INTEGER :: dir, group, group_size, handle, i, iatom, igrid_level, ikind, &
      ipgf, iset, lmax_global, maxco, maxsgf_set, my_pos, na1, na2, nao, &
      natom, ncoa, nseta, offset, sgfa, stat
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: where_is_the_point
    INTEGER, DIMENSION(3)                    :: lb, location, tp, ub
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, mylmax, &
                                                npgfa, nsgfa
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgfa
    ...
    IF (PRESENT(external_vector)) THEN
       ...
       IF (stat.NE.0) CALL stop_memory(routineN,moduleN,2524,"eigenvector",dp_size*nao)
       eigenvector=external_vector
    ELSE
       CALL cp_fm_get_info(matrix=mo_vectors,nrow_global=nao)
       ALLOCATE (eigenvector(nao),STAT=stat)
       IF (stat.NE.0) CALL stop_memory(routineN,moduleN,2529,"eigenvector",dp_size*nao)
       DO i=1,nao
    ...
    ! *** set up the pw multi-grids
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_collocate_density.F",2536)
...
!   *** Allocate work storage ***
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,2563,&
                                    "pab",maxco*dp_size)
    ALLOCATE (work(maxco,1),STAT=stat)
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,2566,&
                                    "work",maxco*dp_size)
  ...
  SUBROUTINE collocate_pgf_product_rspace(la_max,zeta,la_min,&
    ...
    IF(PRESENT(use_subpatch)) THEN
       IF(use_subpatch)THEN
         ...
         IF(.NOT.(PRESENT(subpatch_pattern)))CALL cp__a("qs_collocate_density.F",2806)
       ENDIF
    ENDIF
    ...
    CASE(FUNC_ADBmDAB)
        IF(.NOT.(PRESENT(idir)))CALL cp__a("qs_collocate_density.F",2892)
        la_max_local=la_max+1
        la_min_local=MAX(la_min-1,0)
        lb_max_local=lb_max+1
        lb_min_local=MAX(lb_min-1,0)
        ...
        o2_local=0
    CASE(FUNC_DABpADB)
        IF(.NOT.(PRESENT(idir)))CALL cp__a("qs_collocate_density.F",2924)
        la_max_local=la_max+1
        la_min_local=MAX(la_min-1,0)
        lb_max_local=lb_max+1
        lb_min_local=MAX(lb_min-1,0)
    ...
    CASE(FUNC_DXDX,FUNC_DYDY,FUNC_DZDZ)
        ...
        o2_local=0
    CASE(FUNC_ARDBmDARB)
        IF(.NOT.(PRESENT(idir)))CALL cp__a("qs_collocate_density.F",3047)
        IF(.NOT.(PRESENT(ir)))CALL cp__a("qs_collocate_density.F",3048)
        la_max_local=la_max+1
        la_min_local=MAX(la_min-1,0)
        lb_max_local=lb_max+2
        lb_min_local=MAX(lb_min-1,0)
        ...
        o2_local=0
    CASE(FUNC_ARB)
        IF(.NOT.(PRESENT(ir)))CALL cp__a("qs_collocate_density.F",3081)
        la_max_local=la_max
        la_min_local=la_min
        lb_max_local=lb_max+1
        lb_min_local=lb_min
    ...
    CASE(FUNC_AB)
        ...
        o2_local=o2
    CASE DEFAULT
        IF(.NOT.(.FALSE.))CALL cp__a("qs_collocate_density.F",3116)
    END SELECT
    ...
    DO i=1,3
      ...
      ELSE
        ...
        IF (ub_cube(i)+offset>UBOUND(grid,i).OR.lb_cube(i)+offset<LBOUND(grid,i)) THEN
           IF(.NOT.(.FALSE.))CALL cp__a("qs_collocate_density.F",3307)
        ENDIF
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/prep.f90" 1
!
...
# 3321 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F" 2

    IF ( PRESENT ( lgrid ) ) THEN
       ig = lgrid%ldim * ithread_l + 1

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/call_collocate_omp.f90" 1
  SELECT CASE(lp)
 CASE(0)
 CALL collocate_core_0(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(1)
 CALL collocate_core_1(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(2)
 CALL collocate_core_2(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(3)
 CALL collocate_core_3(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(4)
 CALL collocate_core_4(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(5)
 CALL collocate_core_5(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(6)
 CALL collocate_core_6(lgrid%r(ig),coef_xyz(1),pol_x(0,-cmax), pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),cmax,gridbounds(1,1))
 CASE(7)
 ...
 CALL collocate_core_default(grid(1,1,1),coef_xyz(1),pol_x(0,-cmax),pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
  ...
  END SELECT
# 3325 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F" 2
    ELSE

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/call_collocate.f90" 1
  SELECT CASE(lp)
 CASE(0)
 ...
 CALL collocate_core_default(grid(1,1,1),coef_xyz(1),pol_x(0,-cmax),pol_y(1,0,-cmax),pol_z(1,0,-cmax), &
                        map(-cmax,1),sphere_bounds(1),lp,cmax,gridbounds(1,1))
  END SELECT
# 3327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_collocate_density.F" 2
    END IF
...
! still hard-wired (see MODULO)
      ...
      IF(.NOT.(ALL(rsgrid % desc % perd==1)))CALL cp__a("qs_collocate_density.F",3742)
...
! *****************************************************************************
  SUBROUTINE collocate_pgf_product_gspace(la_max,zeta,la_min,&
    ...
    INTEGER                                  :: ax, ay, az, bx, by, bz, &
                                                handle, i, ico, ig, ig2, jco, &
                                                jg, kg, la, lb, lb_cube_min, &
                                                lb_grid, ub_cube_max, ub_grid
