# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_vertex_coloring_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_vertex_coloring_methods.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_vertex_coloring_methods.F" 2
...
!> \param graph ...
! *****************************************************************************
  SUBROUTINE kg_heap_fill(heap, graph)
    ...
    IF(heap_error)&
       CALL cp__w("kg_vertex_coloring_methods.F",310,"Error initially filling heap.")
...
!> \param key ...
! *****************************************************************************
  SUBROUTINE kg_heap_pop(heap, key)
    TYPE(heap_t)                             :: heap
    ...
    IF(heap_error)&
       CALL cp__w("kg_vertex_coloring_methods.F",333,"Error getting topmost heap element.")
...
!> \param value ...
! *****************************************************************************
  SUBROUTINE kg_heap_reset_node(heap, key, value)
    TYPE(heap_t)                             :: heap
    ...
    IF(heap_error)&
       CALL cp__w("kg_vertex_coloring_methods.F",355,"Error changing value of heap element.")
...
! *****************************************************************************
  SUBROUTINE kg_update_node(heap, node)
    ...
    IF (heap%index(id)>0) THEN
      ...
      CALL kg_heap_reset_node(heap, id, value)
...
! *****************************************************************************
  SUBROUTINE kg_dsatur(kg_env, graph, ncolors)
    ...
    INTEGER                                  :: color_limit, handle, i, j, &
                                                key, maxdegree, nneighbors, &
                                                nnodes
    LOGICAL, ALLOCATABLE, DIMENSION(:)       :: color_present
    TYPE(heap_t)                             :: heap
    ...
    ELSE
      ...
      CALL heap_new(heap, nnodes)

      CALL kg_heap_fill(heap, graph)

      DO WHILE (heap%n>0)

        CALL kg_heap_pop(heap, key)
...
! get a useful output_unit
    ...
    CASE DEFAULT
       CALL cp__b("kg_vertex_coloring_methods.F",732,"Coloring method not known.")
    END SELECT
    ...
    IF(.NOT.valid)&
       CALL cp__b("kg_vertex_coloring_methods.F",740,"Coloring not valid.")
