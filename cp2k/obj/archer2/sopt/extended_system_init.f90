# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_init.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_init.F"
!-----------------------------------------------------------------------------!
...
# 50 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_init.F" 2
...
! *****************************************************************************
  SUBROUTINE initialize_npt ( simpar, globenv, npt_info, cell, work_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(npt_info)))CALL cp__a("motion/thermostat/extended_system_init.F",96)
    ...
    IF (ASSOCIATED(npt_info)) THEN
       IF (ASSOCIATED(work_section)) THEN
          ...
          IF(restart.NEQV.explicit)&
             CALL cp_abort(cp__l("motion/thermostat/extended_system_init.F",130),"You need to define both VELOCITY and "//&
...
! *****************************************************************************
  SUBROUTINE set_yoshida_coef ( nhc, dt )
    ...
    CASE DEFAULT
       CALL cp__b("motion/thermostat/extended_system_init.F",515,'Value not available.')
    CASE (1)
       yosh_wt(1) = 1.0_dp
    CASE (3)
    ...
    ELSE
       ...
       IF (ASSOCIATED(nose_section)) THEN
          ...
          IF(.NOT.restart.AND.explicit)&
             CALL cp_abort(cp__l("motion/thermostat/extended_system_init.F",630),"You need to define both VELOCITY and "//&
                  "COORD and MASS and FORCE section (or none) in the NOSE section")
          restart=explicit.and.restart
          work_section => section_vals_get_subs_vals(nose_section,"MASS")
          CALL section_vals_get(work_section, explicit=explicit)
          IF(.NOT.restart.AND.explicit)&
             CALL cp_abort(cp__l("motion/thermostat/extended_system_init.F",636),"You need to define both VELOCITY and "//&
                  "COORD and MASS and FORCE section (or none) in the NOSE section")
          restart=explicit.and.restart
          work_section => section_vals_get_subs_vals(nose_section,"FORCE")
          CALL section_vals_get(work_section, explicit=explicit)
          IF(.NOT.restart.AND.explicit)&
             CALL cp_abort(cp__l("motion/thermostat/extended_system_init.F",642),"You need to define both VELOCITY and "//&
                  "COORD and MASS and FORCE section (or none) in the NOSE section")
          restart=explicit.and.restart
       END IF
...
! *****************************************************************************
  SUBROUTINE init_nhc_forces ( nhc)
    ...
    IF(.NOT.(ASSOCIATED(nhc)))CALL cp__a("motion/thermostat/extended_system_init.F",954)
    ! assign the forces
    DO i = 1, SIZE ( nhc % nvt, 2 )
