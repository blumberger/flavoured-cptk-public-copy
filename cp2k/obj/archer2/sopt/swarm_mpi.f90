# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_mpi.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_mpi.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_mpi.F" 2
...
! *****************************************************************************
   SUBROUTINE swarm_mpi_init(swarm_mpi, world_para_env, root_section, n_workers, worker_id, iw)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'swarm_mpi_init', &
...
! ====== Setup of MPI-Groups ======
    ...
    IF (MOD(swarm_mpi%world%num_pe-1, n_workers) /= 0) THEN
       CALL cp__b("swarm/swarm_mpi.F",103,"number of processors-1 is not divisible by n_workers.")
    ENDIF
    IF (swarm_mpi%world%num_pe < n_workers + 1) THEN
       CALL cp__b("swarm/swarm_mpi.F",106,"There are not enough processes for n_workers + 1. Aborting.")
    ENDIF
    ...
    IF (im_the_master) THEN
       ...
       IF(subgroup_size/=1) CALL cp__b("swarm/swarm_mpi.F",123,"mp_comm_split_direct failed (master)")
       CALL cp_para_env_create(swarm_mpi%master, group=subgroup)
    ...
    ELSE
       ...
       IF(subgroup_size/=swarm_mpi%world%num_pe-1) CALL cp__b("swarm/swarm_mpi.F",130,"mp_comm_split_direct failed (worker)")
    ENDIF
    ...
    IF(.NOT. im_the_master) THEN
       ...
       IF(n_groups_created/=n_workers) CALL cp__b("swarm/swarm_mpi.F",141,"mp_comm_split failed.")
       CALL cp_para_env_create(swarm_mpi%worker, group=worker_group)
...
! *****************************************************************************
  SUBROUTINE logger_init_worker(swarm_mpi, root_section, worker_id)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'logger_init_worker', &
      routineP = moduleN//':'//routineN
    ...
    IF(ASSOCIATED(swarm_mpi%worker)) THEN
       ...
       IF(worker_id > 99999) THEN
          CALL cp__b("swarm/swarm_mpi.F",232,"Did not expect so many workers.")
       ENDIF
       WRITE(worker_name,"(A,I5.5)") 'WORKER', worker_id
       IF(LEN_TRIM(project_name)+1+LEN_TRIM(worker_name)>default_string_length) THEN
          CALL cp__b("swarm/swarm_mpi.F",236,"project name too long")
       ENDIF
...
! *****************************************************************************
  SUBROUTINE swarm_mpi_finalize(swarm_mpi, root_section)
    ...
    IF(ASSOCIATED(swarm_mpi%worker)) CALL cp_para_env_release(swarm_mpi%worker)
    IF(ASSOCIATED(swarm_mpi%master)) CALL cp_para_env_release(swarm_mpi%master)
