# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_qmmm_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_qmmm_methods.F"
!-----------------------------------------------------------------------------!
...
# 38 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_qmmm_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE ks_qmmm_env_rebuild(qs_env,qmmm_env)
    ...
    IF (.NOT.ASSOCIATED(ks_qmmm_env)) THEN
       ...
       CALL qs_ks_qmmm_release(ks_qmmm_env=ks_qmmm_env)
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_ks_qmmm_create(ks_qmmm_env, qs_env, qmmm_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ks_qmmm_env)))CALL cp__a("qs_ks_qmmm_methods.F",105)
...
! *****************************************************************************
  SUBROUTINE qmmm_calculate_energy(qs_env, rho, v_qmmm, qmmm_energy)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("qs_ks_qmmm_methods.F",173)
    ...
    IF(dft_control%qs_control%gapw) THEN
      CALL get_qs_env(qs_env=qs_env,&
                    rho0_s_rs=rho0_s_rs)
       IF(.NOT.(ASSOCIATED(rho0_s_rs)))CALL cp__a("qs_ks_qmmm_methods.F",194)
