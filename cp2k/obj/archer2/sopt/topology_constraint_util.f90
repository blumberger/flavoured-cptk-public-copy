# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_constraint_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_constraint_util.F"
!-----------------------------------------------------------------------------!
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_constraint_util.F" 2
...
! *****************************************************************************
  SUBROUTINE topology_constraint_pack ( molecule_kind_set, molecule_set,&
    ...
    INTEGER, DIMENSION(:), POINTER           :: constr_x_glob, inds, &
                                                molecule_list
    LOGICAL :: exclude_mm, exclude_qm, fix_atom_mm, fix_atom_molname, &
      fix_atom_qm, fix_atom_qmmm, fix_fixed_atom, found_molname, is_qm, &
      ishbond, ldummy, restart_restraint_clv, restart_restraint_pos, &
      use_clv_info
    ...
    IF(topology%const_hydr) THEN
       ...
       IF (ASSOCIATED(hdist)) THEN
          IF(.NOT.(SIZE(hdist)==SIZE(atom_typeh)))CALL cp__a("topology_constraint_util.F",264)
       END IF
    ...
    IF(topology%const_atom) THEN
                ...
                DO k=first,last
                   ...
                   IF (PRESENT(qmmm_env)) THEN
                      ...
                      IF (fix_atom_qm.AND.fix_atom_mm) THEN
                         CALL cp_abort(cp__l("topology_constraint_util.F",736),&
                   ...
                   IF ((fix_fixed_atom.AND.fix_atom_qmmm).OR.(fix_fixed_atom.AND.fix_atom_molname)&
                      ...
                      CALL cp_abort(cp__l("topology_constraint_util.F",744),&
                           "Atom number: "//cp_to_string(k)//&
                   ...
                   IF (fix_fixed_atom.OR.fix_atom_qmmm.OR.fix_atom_molname) THEN
                      ...
                      ELSE
                         ...
                         CALL cp__b("topology_constraint_util.F",770,"")
                      END IF
                      IF (fixd_list(kk)%restraint%active) THEN
                         ...
                         IF (restart_restraint_pos) THEN
                            ...
                            SELECT CASE(itype)
                            CASE(use_perd_x)
                               IF(.NOT.(SIZE(r)==1))CALL cp__a("topology_constraint_util.F",799)
                               fixd_list(kk)%coord(1) = r(1)
                            CASE(use_perd_y)
                               IF(.NOT.(SIZE(r)==1))CALL cp__a("topology_constraint_util.F",802)
                               fixd_list(kk)%coord(2) = r(1)
                            CASE(use_perd_z)
                               IF(.NOT.(SIZE(r)==1))CALL cp__a("topology_constraint_util.F",805)
                               fixd_list(kk)%coord(3) = r(1)
                            CASE(use_perd_xy)
                               IF(.NOT.(SIZE(r)==2))CALL cp__a("topology_constraint_util.F",808)
                               fixd_list(kk)%coord(1) = r(1)
                               fixd_list(kk)%coord(2) = r(2)
                            CASE(use_perd_xz)
                               IF(.NOT.(SIZE(r)==2))CALL cp__a("topology_constraint_util.F",812)
                               fixd_list(kk)%coord(1) = r(1)
                               fixd_list(kk)%coord(3) = r(2)
                            CASE(use_perd_yz)
                               IF(.NOT.(SIZE(r)==2))CALL cp__a("topology_constraint_util.F",816)
                               fixd_list(kk)%coord(2) = r(1)
                               fixd_list(kk)%coord(3) = r(2)
                            CASE(use_perd_xyz)
                               IF(.NOT.(SIZE(r)==3))CALL cp__a("topology_constraint_util.F",820)
       ...
       END IF
       IF(.NOT.(COUNT(missed_molname)==0))CALL cp__a("topology_constraint_util.F",870)
  ...
  SUBROUTINE setup_lcolv(lcolv, ilist, first_atom, last_atom, cons_info,&
    ...
    DO kk=1,SIZE(ilist)
       ...
       IF  ((lcolv(kk)%colvar%type_id==xyz_diag_colvar_id).OR.&
          ...
          IF (use_clv_info) THEN
             ...
             CASE(xyz_diag_colvar_id)
                IF(.NOT.(SIZE(r_vals)==3))CALL cp__a("topology_constraint_util.F",1156)
                lcolv(kk)%colvar%xyz_diag_param%r0=r_vals
             CASE(xyz_outerdiag_colvar_id)
                IF(.NOT.(SIZE(r_vals)==6))CALL cp__a("topology_constraint_util.F",1159)
       ...
       IF  (ANY(lcolv(kk)%colvar%i_atom > last_atom).OR.&
          ...
          CALL cp__b("topology_constraint_util.F",1197,"")
       END IF
    END DO
    ...
    DO kk = 1, SIZE(lg3x3)
       ...
       IF  ((g3x3_list(kk)%a+first_atom-1 < first_atom).OR.&
          ...
          WRITE(*,'(T5,"|",T8,A)')"A constraint has been defined for a molecule type",&
               ...
               " in the constraint specification in the input file.",&
               " Please check it carefully!"
          CALL cp__b("topology_constraint_util.F",1251,"")
       END IF
    END DO
    ...
    DO kk = 1, SIZE(lg4x6)
       ...
       IF  ((g4x6_list(kk)%a+first_atom-1 < first_atom).OR.&
          ...
          WRITE(*,'(T5,"|",T8,A)')"A constrained has been defined for a molecule type",&
               ...
               " in the constraint specification in the input file.",&
               " Please check it carefully!"
          CALL cp__b("topology_constraint_util.F",1317,"")
       END IF
    END DO
...
! *****************************************************************************
  SUBROUTINE give_constraint_array(const_mol, const_molname, const_intermolecular,&
    ...
    END DO
    IF(.NOT.(SIZE(const_mol)==SIZE(const_molname)))CALL cp__a("topology_constraint_util.F",1366)
    iglob = 0
       ...
       ELSE
          ...
          IF (const_mol(i)/=0) THEN
             ...
             IF(k>SIZE(molecule_kind_set))&
                CALL cp_abort(cp__l("topology_constraint_util.F",1379),&
...
! *****************************************************************************
  SUBROUTINE print_warning_molname(found, name)
    ...
    IF(.NOT.found)&
       CALL cp_warn(cp__l("topology_constraint_util.F",1423),&
