# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/vibrational_analysis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/vibrational_analysis.F"
!-----------------------------------------------------------------------------!
...
MODULE vibrational_analysis
  ...
  USE physcon,                         ONLY: &
       a_bohr, boltzmann, e_mass, h_bar, hertz, kelvin, kjmol, massunit, &
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/vibrational_analysis.F" 2
...
! *****************************************************************************
  SUBROUTINE vb_anal(input, input_declaration, para_env, globenv)
    ...
    INTEGER :: handle, i, icoord, icoordm, icoordp, ierr, imap, ip1, ip2, &
      ...
      nRotTrM, nvib, output_unit, prep, proc_dist_type
    INTEGER, DIMENSION(:), POINTER           :: Clist, Mlist
    LOGICAL :: calc_intens, calc_thchdata, do_mode_tracking, keep_rotations, &
      row_force, something_frozen
    REAL(KIND=dp)                            :: dx, inertia(3), &
                                                minimum_energy, norm, &
    ...
    output_unit=cp_print_key_unit_nr(logger,print_section,"PROGRAM_RUN_INFO",extension=".vibLog")
    ...
    IF (ASSOCIATED(rep_env)) THEN
       ...
       ELSE
          ...
          DO i = 1, natoms
             ...
             IF(.NOT.(mass(i)>0.0_dp))CALL cp__a("motion/vibrational_analysis.F",186)
             mass(i)= SQRT(mass(i))
...
! *****************************************************************************
  SUBROUTINE get_moving_atoms(force_env, Ilist)
    ...
    IF(.NOT.(ndim>=0))CALL cp__a("motion/vibrational_analysis.F",523)
    ALLOCATE(Ilist(ndim))
  ...
  SUBROUTINE build_D_matrix(mat,dof,Dout,full,natoms)
    ...
    DO i = 1, dof
       ...
       DO j = i+1, dof
          ...
          IF(.NOT.(ABS(norm)<thrs_motion))CALL cp__a("motion/vibrational_analysis.F",669)
       END DO
    END DO
    ...
    DO WHILE (ifound /= nvib)
       iseq = iseq + 1
       IF(.NOT.(iseq<=3*natoms))CALL cp__a("motion/vibrational_analysis.F",677)
       work       = 0.0_dp
       work(iseq) = 1.0_dp
       ...
       END IF
    END DO
    IF(.NOT.(dof+ifound==3*natoms))CALL cp__a("motion/vibrational_analysis.F",693)
    IF (my_full) THEN
...
! *****************************************************************************
  ...
  SUBROUTINE get_thch_values(freqs,iw,mass,nvib,inertia,spin,totene,temp,pressure)
    ...
    REAL(KIND=dp) :: el_entropy, entropy, exp_min_one, fact, fact2, freq_arg, &
      freq_arg2, freqsum, Gibbs, heat_capacity, inertia_kg(3), mass_tot, &
      one_min_exp, partition_function, rot_cv, rot_energy, rot_entropy, &
      rot_part_func, rotvibtra, tran_cv, tran_energy, tran_enthalpy, &
      tran_entropy, tran_part_func, vib_cv, vib_energy, vib_entropy, &
      vib_part_func, zpe
