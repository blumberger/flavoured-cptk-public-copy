# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/lebedev.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/lebedev.F"
!-----------------------------------------------------------------------------!
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/lebedev.F" 2
...
!      lgnum(:) = MINLOC(lebedev_grid(:)%n,MASK=(lebedev_grid(:)%n >= n))
    ...
    ELSE
      CALL cp__b("common/lebedev.F",139,"*** ERROR in FUNCTION get_number_of_lebedev_grid ***")
    END IF

    IF (lgnum(1) == 0) THEN
      CALL cp__b("common/lebedev.F",143,"*** ERROR in FUNCTION get_number_of_lebedev_grid ***")
    ELSE
...
! *****************************************************************************
  SUBROUTINE load_sub_grid(subsystem,lgnum,np)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'load_sub_grid', &
      routineP = moduleN//':'//routineN
...
!   *** Check argument values ***
    ...
    IF ((lgnum < 1).OR.(lgnum > nlg)) THEN
      CALL cp__b("common/lebedev.F",176,"Argument value #2 (lgnum) is out of range")
    END IF

    IF ((np < 0).OR.(np > max_np)) THEN
      CALL cp__b("common/lebedev.F",180,"Argument value #3 (np) is out of range")
    END IF
    ...
    CASE ("D")
      IF (MODULO(np,3) == 0) THEN
        ...
        END DO
      ELSE
        CALL cp__b("common/lebedev.F",309,"Subsytem D: np is not modulo 3 (check argument #3)")
      END IF
      nlgp = nlgp + nd*np/3
    CASE DEFAULT
      CALL cp_abort(cp__l("common/lebedev.F",313),&
