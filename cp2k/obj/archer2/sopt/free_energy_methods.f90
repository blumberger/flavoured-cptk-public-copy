# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/free_energy_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/free_energy_methods.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/free_energy_methods.F" 2
    ...
    IF (.NOT.ASSOCIATED(force_env%meta_env).AND.ASSOCIATED(fe_env)) THEN
       ...
       CASE(do_fe_ac)
          ...
          IF(.NOT.ASSOCIATED(force_env%mixed_env))&
             CALL cp_abort(cp__l("motion/free_energy_methods.F",140),&
                  'ASSERTION (cond) failed at line '//cp_to_string(141)//&
...
! *****************************************************************************
  SUBROUTINE dump_ac_info(my_val, my_par, dx, lerr, fe_section, nforce_eval, cum_res,&
    ...
    END DO
    IF(.NOT.(i<=SIZE(my_par)))CALL cp__a("motion/free_energy_methods.F",840)
    ipar = i
    ...
    IF (ABS(err)>lerr) THEN
       ...
       CALL cp_warn(cp__l("motion/free_energy_methods.F",848),&
            'ASSERTION (cond) failed at line '//cp_to_string(849)//&
    ...
    IF (ABS(err)>lerr) THEN
       ...
       CALL compress(this_error,.TRUE.)
       CALL compress(def_error,.TRUE.)
       CALL cp_warn(cp__l("motion/free_energy_methods.F",867),&
            'ASSERTION (cond) failed at line '//cp_to_string(868)//&
            ' Error '//TRIM(this_error)//' in computing numerical derivatives larger then'//&
            TRIM(def_error)//' .')
    END IF
