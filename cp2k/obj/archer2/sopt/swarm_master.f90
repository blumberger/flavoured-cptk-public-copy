# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_master.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_master.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/swarm_master.F" 2
...
! *****************************************************************************
 SUBROUTINE swarm_master_init(master, para_env, globenv, root_section, n_workers)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'swarm_master_init', &
    ...
    SELECT CASE(master%behavior)
      ...
      CASE DEFAULT
         CALL cp__b("swarm/swarm_master.F",125,"got unknown behavior")
    END SELECT
...
! *****************************************************************************
 SUBROUTINE replay_comlog(master)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'replay_comlog', &
      routineP = moduleN//':'//routineN
    ...
    DO
      ...
      IF(TRIM(command_log) /= "shutdown") THEN
        IF(.NOT. commands_equal(cmd_now, cmd_log, master%iw)) CALL cp__b("swarm/swarm_master.F",180,"wrong behaviour")
      END IF
 ...
 SUBROUTINE swarm_master_steer(master, report, cmd)
    ...
    ELSE
       SELECT CASE(master%behavior)
         ...
         CASE DEFAULT
            CALL cp__b("swarm/swarm_master.F",297,"got unknown behavior")
       END SELECT
    ...
    IF(master%n_waiting < 0) CALL cp__b("swarm/swarm_master.F",316,"master%n_waiting < 0")
...
! *****************************************************************************
 SUBROUTINE swarm_master_finalize(master)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'swarm_master_finalize', &
      routineP = moduleN//':'//routineN
    ...
    SELECT CASE(master%behavior)
      ...
      CASE DEFAULT
         CALL cp__b("swarm/swarm_master.F",353,"got unknown behavior")
    END SELECT
