# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_se.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_se.F"
!-----------------------------------------------------------------------------!
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_se.F" 2
...
! *****************************************************************************
  SUBROUTINE scf_post_calculation_se(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_se.F",119)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_se.F",140,"Printing of MO cube files not implemented for Semi-Empirical method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_se.F",147,"STM not implemented for Semi-Empirical method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_se.F",154,"DFT+U not available for Semi-Empirical method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_se.F",161,"Kinetic energy not available for Semi-Empirical method.")
    END IF
    ...
    IF(explicit.AND..NOT.qs_env%run_rtp) THEN
      CALL cp__w("qs_scf_post_se.F",168,"Wavefunction mixing not implemented for Semi-Empirical  method.")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",175,"XRAY_DIFFRACTION_SPECTRUM  not implemented for Semi-Empirical calculations!!")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_se.F",182,"ELECTRIC_FIELD_GRADIENT not implemented for Semi-Empirical calculations!!")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),&
           cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",190,"HYPERFINE_COUPLING_TENSOR  not implemented for Semi-Empirical calculations!!")
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_moments(input, logger, qs_env)
    ...
    COMPLEX(KIND=dp), DIMENSION(3)           :: dggamma, dzphase, ggamma, &
                                                zphase
    INTEGER                                  :: i, iat, iatom, ikind, ix, j, &
                                                nat, natom, natorb, nkind, &
                                                nspin, reference, unit_nr
          ...
          DO iatom=1,nat
             ...
             DO i=1,nspin
                CALL cp_dbcsr_get_block_p(matrix=matrix_p(i)%matrix,row=iat,col=iat,&
                  block=pblock,found=found)
                IF(found) THEN
                  IF(.NOT.(natorb==SIZE(pblock,1)))CALL cp__a("qs_scf_post_se.F",371)
                  ix=coset(1,0,0) - 1
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_charges(input, logger, qs_env, rho, &
    ...
    INTEGER                                  :: i, iat, iatom, ikind, j, nat, &
                                                natom, natorb, nkind, nspin, &
                                                unit_nr
    LOGICAL                                  :: found
    REAL(KIND=dp)                            :: zeff
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",590,"Lowdin charges not available for semi-empirical calculations!")
    END IF

    ! Hirshfeld charges
    print_key => section_vals_get_subs_vals(input,"DFT%PRINT%HIRSHFELD")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",596,"Hirshfeld charges not available for semi-empirical calculations!")
    END IF
...
! *****************************************************************************
  SUBROUTINE write_available_results(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_se.F",640)
    CALL get_qs_env(qs_env,&
                    dft_control=dft_control,&
                    mos=mos,&
                    atomic_kind_set=atomic_kind_set,&
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,dft_section,"PRINT%PDOS")&
        ,cp_p_file) ) THEN
       CALL cp__w("qs_scf_post_se.F",671,"PDOS not implemented for Semi-Empirical calculations!!")
    ENDIF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       CALL cp__w("qs_scf_post_se.F",677,"TOT_DENSITY_CUBE  not implemented for Semi-Empirical calculations!!")
    END IF

    ! Write cube file with electron density
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
              "DFT%PRINT%E_DENSITY_CUBE"),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",683,"E_DENSITY_CUBE not implemented for Semi-Empirical calculations!!")
    END IF ! print key

    ! Write cube file with EFIELD 
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
              "DFT%PRINT%EFIELD_CUBE"),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",689,"EFIELD_CUBE not implemented for Semi-Empirical calculations!!")
    END IF ! print key

    ! Write cube file with ELF
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
              "DFT%PRINT%ELF_CUBE"),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",695,"ELF function not implemented for Semi-Empirical calculations!!")
    END IF ! print key

    ! Print the hartree potential
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
         "DFT%PRINT%V_HARTREE_CUBE"),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",701,"V_HARTREE_CUBE not implemented for Semi-Empirical calculations!!")
    ENDIF

    ! Print the XC potential
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
         "DFT%PRINT%V_XC_CUBE"),cp_p_file)) THEN
       CALL cp__w("qs_scf_post_se.F",707,"V_XC_CUBE not available for Semi-Empirical calculations!!")
    ENDIF
