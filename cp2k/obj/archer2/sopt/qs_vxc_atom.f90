# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc_atom.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc_atom.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc_atom.F" 2
...
! *****************************************************************************
  SUBROUTINE calculate_vxc_atom(qs_env,energy_only,gradient_atom_set,adiabatic_rescale_factor)
    ...
    INTEGER                                  :: bo(2), handle, ia, iat, &
                                                iatom, idir, ikind, ir, &
                                                ispin, mepos, myfun, na, &
                                                natom, nr, nspins, num_pe
    INTEGER, DIMENSION(2, 3)                 :: bounds
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: epr_xc, gradient_f, lsd, &
                                                paw_atom, tau_f
...
! *****************************************************************************
  SUBROUTINE calculate_xc_2nd_deriv_atom(p_env,qs_env,xc_section,do_tddft,do_triplet)
    ...
    INTEGER, DIMENSION(2, 3)                 :: bounds
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: gradient_functional, lsd, &
                                                lsd_singlets, my_do_tddft, &
                                                paw_atom, tau_f
...
! -------------------------------------------------------------------------
   ...
   rho1_atom_set => p_env%local_rho_set%rho_atom_set
    ...
    tau_f = (needs%tau.OR.needs%tau_spin)
    IF ( tau_f ) THEN
      CALL cp__b("qs_vxc_atom.F",547,"Tau functionals not implemented for GAPW 2nd derivatives")
    END IF
...
! *****************************************************************************
  SUBROUTINE calc_rho_angular(grid_atom, harmonics, nspins, grad_func, &
   ...
   IF(.NOT.(ASSOCIATED(r_h)))CALL cp__a("qs_vxc_atom.F",760)
   IF(.NOT.(ASSOCIATED(r_s)))CALL cp__a("qs_vxc_atom.F",761)
   IF(.NOT.(ASSOCIATED(rho_h)))CALL cp__a("qs_vxc_atom.F",762)
   IF(.NOT.(ASSOCIATED(rho_s)))CALL cp__a("qs_vxc_atom.F",763)
   IF (grad_func) THEN
      IF(.NOT.(ASSOCIATED(dr_h)))CALL cp__a("qs_vxc_atom.F",765)
      IF(.NOT.(ASSOCIATED(dr_s)))CALL cp__a("qs_vxc_atom.F",766)
      IF(.NOT.(ASSOCIATED(r_h_d)))CALL cp__a("qs_vxc_atom.F",767)
      IF(.NOT.(ASSOCIATED(r_s_d)))CALL cp__a("qs_vxc_atom.F",768)
      IF(.NOT.(ASSOCIATED(drho_h)))CALL cp__a("qs_vxc_atom.F",769)
      IF(.NOT.(ASSOCIATED(drho_s)))CALL cp__a("qs_vxc_atom.F",770)
   END IF
...
! *****************************************************************************
  SUBROUTINE calc_tau_angular(grid_atom, harmonics, nspins, ir, &
    ...
    IF(.NOT.(ASSOCIATED(trho_h)))CALL cp__a("qs_vxc_atom.F",875)
    IF(.NOT.(ASSOCIATED(trho_s)))CALL cp__a("qs_vxc_atom.F",876)
    IF(.NOT.(ASSOCIATED(tau_h)))CALL cp__a("qs_vxc_atom.F",877)
    IF(.NOT.(ASSOCIATED(tau_s)))CALL cp__a("qs_vxc_atom.F",878)
...
! -------------------------------------------------------------------------
    ...
    DO iset1 = 1,nset
      ...
      DO iset2 = 1,nset
         ...
         IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_vxc_atom.F",999)
            ...
            DO ispin=1,nspins
              ...
              DO iso = 1,max_iso_not0_local
                 DO icg = 1,cg_n_list(iso)
                    ...
                    IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_vxc_atom.F",1053)
                    DO ia = 1,na
...
! -------------------------------------------------------------------------
    ...
    ALLOCATE(matso_h(nsoset(maxl),nsoset(maxl)),&
             matso_s(nsoset(maxl),nsoset(maxl)),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,1181,&
    ...
    DO iset1 = 1,nset
       ...
       DO iset2 = 1,nset
          CALL get_none0_cg_list(my_CG,lmin(iset1),lmax(iset1),lmin(iset2),lmax(iset2),&
                                 max_s_harm,lmax_expansion,cg_list,cg_n_list,max_iso_not0_local)
          IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_vxc_atom.F",1197)
          ...
          !IF(.NOT.(dmax_iso_not0_local.LE.dmax_iso_not0))CALL cp__a("qs_vxc_atom.F",1200)
             ...
             DO ipgf2 = 1,npgf(iset2)
              ...
              DO iso = 1,max_iso_not0_local
                 DO icg = 1,cg_n_list(iso)
                    ...
                    !test reduce expansion local densities
                    IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_vxc_atom.F",1303)
                    DO ia = 1,na
                       matso_h(iso1,iso2) = matso_h(iso1,iso2) + &
                                            gVXCg_h(ia,l)*&
                                            harmonics%slm(ia,iso)*&
              ...
              DO iso = 1,dmax_iso_not0_local
                 DO icg = 1,dcg_n_list(iso)
                    ...
                    l = indso(1,iso1) + indso(1,iso2)
                    !test reduce expansion local densities
                    IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_vxc_atom.F",1328)
                    DO ia = 1,na
                       matso_h(iso1,iso2) = matso_h(iso1,iso2) + &
...
! *****************************************************************************
  SUBROUTINE  dgaVtaudgb(vtau_h,vtau_s,qs_kind,rho_atom,nspins)
    ...
    INTEGER :: dmax_iso_not0_local, handle, ic, icg, ipgf1, ipgf2, iset1, &
      iset2, iso, iso1, iso2, isom1, isom2, ispin, l1, l2, lmax12, &
      lmax_expansion, lmin12, m1, m2, max_iso_not0, max_iso_not0_local, &
      max_s_harm, maxiso, maxl, maxso, mm1, mm2, n1, n2, na, ngau1, ngau2, &
      nngau1, nr, nset, size1
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: cg_n_list, dcg_n_list
    INTEGER, ALLOCATABLE, DIMENSION(:, :, :) :: cg_list, dcg_list
    INTEGER, DIMENSION(:), POINTER           :: lmax, lmin, npgf
    ...
    DO ispin=1,nspins
      ...
      DO iset1 = 1,nset
        ...
        DO iset2 = 1,nset
           CALL get_none0_cg_list(my_CG,lmin(iset1),lmax(iset1),lmin(iset2),lmax(iset2),&
                                  max_s_harm,lmax_expansion,cg_list,cg_n_list,max_iso_not0_local)
           IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_vxc_atom.F",1471)
