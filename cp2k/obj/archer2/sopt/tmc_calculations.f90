# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_calculations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_calculations.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_calculations.F" 2
...
! *****************************************************************************
  SUBROUTINE calc_potential_energy(conf, env_id, exact_approx_pot, &
    ...
    IF(.NOT.(ASSOCIATED(conf)))CALL cp__a("tmc/tmc_calculations.F",84)
    IF(.NOT.(env_id.GT.0))CALL cp__a("tmc/tmc_calculations.F",85)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_calculations.F",86)
    ...
    CASE(task_type_MC)
      IF(tmc_env%params%pressure.GE.0.0_dp) THEN
        ...
        IF(.NOT.(ierr.EQ.0))CALL cp__a("tmc/tmc_calculations.F",97)
        DEALLOCATE(tmp_cell)
      END IF
    ...
    CASE(task_type_ideal_gas)
      e_pot = 0.0_dp
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_calculations.F",121),&
...
! *****************************************************************************
  SUBROUTINE get_scaled_cell(cell, box_scale, scaled_hmat, scaled_cell, vol, &
    ...
    IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("tmc/tmc_calculations.F",173)
    IF(.NOT.(ASSOCIATED(box_scale)))CALL cp__a("tmc/tmc_calculations.F",174)
...
! *****************************************************************************
  SUBROUTINE get_cell_scaling(cell, scaled_hmat, box_scale)
    ...
    IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("tmc/tmc_calculations.F",221)
...
! *****************************************************************************
  FUNCTION nearest_distance(x1, x2, cell, box_scale) RESULT(res)
    ...
    IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("tmc/tmc_calculations.F",259)
    IF(.NOT.(SIZE(x1).EQ.3))CALL cp__a("tmc/tmc_calculations.F",260)
    IF(.NOT.(SIZE(x2).EQ.3))CALL cp__a("tmc/tmc_calculations.F",261)

    dist_vec(:) = x2(:)-x1(:) ! distance vector between atoms
    ALLOCATE(tmp_box_scale(3))
    IF(PRESENT(box_scale)) THEN
      IF(.NOT.(SIZE(box_scale).EQ.3))CALL cp__a("tmc/tmc_calculations.F",266)
...
! *****************************************************************************
  SUBROUTINE geometrical_center(pos, center)
    ...
    IF(.NOT.(ASSOCIATED(center)))CALL cp__a("tmc/tmc_calculations.F",292)
    IF(.NOT.(SIZE(pos).GE.SIZE(center)))CALL cp__a("tmc/tmc_calculations.F",293)
...
! *****************************************************************************
  SUBROUTINE center_of_mass(pos, atoms, center)
    ...
    IF(.NOT.(ASSOCIATED(center)))CALL cp__a("tmc/tmc_calculations.F",329)
    IF(.NOT.(SIZE(pos).GE.SIZE(center)))CALL cp__a("tmc/tmc_calculations.F",330)
    ...
    DO i=1, SIZE(pos), SIZE(center)
      IF(PRESENT(atoms)) THEN
        IF(.NOT.(SIZE(atoms).EQ.SIZE(pos)/SIZE(center)))CALL cp__a("tmc/tmc_calculations.F",339)
      ...
      ELSE
        CALL cp__w("tmc/tmc_calculations.F",345,"try to calculate center of mass without any mass.")
        center(:) = center(:) + pos(i:i+SIZE(center)-1)/&
...
! *****************************************************************************
  SUBROUTINE init_vel(vel, atoms, temerature, rng_stream, rnd_seed)
    ...
    IF(.NOT.(ASSOCIATED(vel)))CALL cp__a("tmc/tmc_calculations.F",382)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_calculations.F",383)
...
! *****************************************************************************
  FUNCTION calc_e_kin(vel, atoms) RESULT(ekin)
    ...
    IF(.NOT.(ASSOCIATED(vel)))CALL cp__a("tmc/tmc_calculations.F",421)
    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_calculations.F",422)
    ekin = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE three_point_extrapolate(v1,v2,v3, extrapolate, res_err)
    ...
    ENDIF
    IF(.NOT.(extrapolate.NE.HUGE(extrapolate)))CALL cp__a("tmc/tmc_calculations.F",483)
  CONTAINS
! *****************************************************************************
!> \brief ...
!> \param x1 ...
...
! *****************************************************************************
  FUNCTION compute_estimated_prob(elem_old, elem_new, E_classical_diff, &
    ...
    IF(.NOT.(ASSOCIATED(elem_old)))CALL cp__a("tmc/tmc_calculations.F",568)
    IF(.NOT.(ASSOCIATED(elem_new)))CALL cp__a("tmc/tmc_calculations.F",569)
    IF(.NOT.(rnd_nr.GT.0.0_dp))CALL cp__a("tmc/tmc_calculations.F",570)
...
! *****************************************************************************
  SUBROUTINE get_subtree_efficiency(tmc_env, eff)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_calculations.F",645)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_calculations.F",646)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_calculations.F",647)
