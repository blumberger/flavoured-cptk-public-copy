# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_tddfpt_eigensolver.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_tddfpt_eigensolver.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_tddfpt_eigensolver.F" 2
...
! *****************************************************************************
  SUBROUTINE eigensolver(p_env, qs_env, t_env)
    ...
    INTEGER                                  :: handle, n_ev, nspins, &
                                                output_unit, restarts
    ...
    ELSE
       ...
       CALL cp__b("qs_tddfpt_eigensolver.F",111,"")
    END IF
...
! *****************************************************************************
  FUNCTION iterative_solver(in_evals, &
    ...
    CHARACTER                                :: mode
    INTEGER :: col, handle, i, iev, iter, j, k, max_krylovspace_dim, max_kv, &
      n_ev, n_kv, nspins, output_unit, row, spin
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: must_improve
    REAL(dp)                                 :: Atilde_ij, convergence, tmp, &
                                                tmp2
    ...
    NULLIFY(ao_mo_fm_pools, X, R, b, Ab, Sb, tddfpt_control, &
            tilde_fm_struct, Atilde%matrix, Us%matrix, matrix_s, dft_control,&
            para_env, blacs_env)
...
!    CPPostcondition(max_krylovspace_dim>=max_kv,cp_failure_level,routineP,failure)
    ...
    iteration: DO
       ...
       DO i=1, n_kv
          ...
          ELSE
             ...
             ELSE
                ...
                CALL cp__b("qs_tddfpt_eigensolver.F",364,"")
             END IF
       ...
       CALL cp_fm_syevd(Atilde%matrix, Us%matrix, evals_tmp(:))
...
! *****************************************************************************
  SUBROUTINE apply_op(X, R,  p_env, qs_env, do_kernel)
    ...
    IF (do_kernel) THEN
       DO spin=1, nspins
          CALL cp_dbcsr_set(p_env%p1(spin)%matrix, 0.0_dp) ! optimize?
    ...
    DO spin=1, nspins
       DO index=startv, startv + n_v - 1
          ...
          IF (.NOT.ASSOCIATED(vectors(index,spin)%matrix)) &
               CALL cp__b("qs_tddfpt_eigensolver.F",638,"Could not allocate "//TRIM(mat_name)//".")
       END DO
    END DO
