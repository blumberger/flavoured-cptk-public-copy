# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_electric_field_gradient.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_electric_field_gradient.F"
!-----------------------------------------------------------------------------!
...
# 75 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_electric_field_gradient.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_efg_calc(qs_env)
    ...
    INTEGER :: aint_precond, handle, i, iat, iatom, ij, ikind, j, max_iter, &
      natom, natomkind, nkind, nspins, precond_kind, unit_nr
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: efg_debug, efg_interpolation, &
                                                gapw, paw_atom, smoothing, &
                                                success
    ...
    ELSE
      smoothing = .TRUE.
    END IF
    IF(.NOT.(ecut > 0._dp))CALL cp__a("qs_electric_field_gradient.F",181)
    IF(.NOT.(sigma > 0._dp))CALL cp__a("qs_electric_field_gradient.F",182)
    ...
    ELSE
      ...
      DO i=1,6
        ...
        IF(.NOT.(success))CALL cp__a("qs_electric_field_gradient.F",281)
...
! *****************************************************************************
  ...
  ! calculate : Limit(r->0) V_hartree(r)/r^2
    ...
    INTEGER :: i, i1, i2, iat, iatom, icg, ipgf1, ipgf2, iset1, iset2, iso, &
      iso1, iso1_first, iso1_last, iso2, iso2_first, iso2_last, l, l_iso, &
      llmax, m1s, m2s, m_iso, max_iso_not0, max_iso_not0_local, max_s_harm, &
      maxl, maxso, mepos, n1s, n2s, nset, num_pe, size1, size2
    ...
    DO iset1 = 1,nset
       ...
       DO iset2 = 1,nset
          ...
          IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_electric_field_gradient.F",497)
          ...
          DO ipgf1  = 1,npgf(iset1)
             ...
             IF(.NOT.(size1==i1))CALL cp__a("qs_electric_field_gradient.F",507)
          ...
          DO ipgf2 = 1,npgf(iset2)
            ...
            IF(.NOT.(size2==i2))CALL cp__a("qs_electric_field_gradient.F",518)
