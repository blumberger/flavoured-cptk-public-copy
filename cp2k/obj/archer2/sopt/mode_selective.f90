# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mode_selective.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mode_selective.F"
!-----------------------------------------------------------------------------!
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mode_selective.F" 2
...
! *****************************************************************************
  SUBROUTINE MS_initial_moves(para_env, nrep , input, globenv ,ms_vib,particles ,&
    ...
    IF(ms_vib%select_id==0)&
         CALL cp__b("mode_selective.F",337,"no frequency, range or involved atoms specified ")
...
! *****************************************************************************
  SUBROUTINE rest_guess(ms_vib_section,para_env,ms_vib,mass,ionode,particles,nrep,calc_intens)
    ...
    IF(ionode)THEN
       ...
       IF(.NOT.(stat==0))CALL cp__a("mode_selective.F",604)
       ms_vib%mat_size=mat
    END IF
  ...
  SUBROUTINE molden_guess(ms_vib_section,input,para_env,ms_vib,mass,ncoord,nrep,logger)
    ...
    IF(para_env%mepos==para_env%source)THEN
       ...
       DO
          ...
          IF(.NOT.(stat==0))CALL cp__a("mode_selective.F",731)
  ...
  SUBROUTINE  evaluate_H_update_b(rep_env,ms_vib,input,nrep,&
    ...
    IF(converged)THEN
       ...
       CALL molden_out(input,particles,eigenval,tmp_b,intensities,calc_intens,&
                       dump_only_positive=.TRUE., logger=logger)
       IF (calc_intens) THEN
