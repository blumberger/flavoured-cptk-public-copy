# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/eigenvalueproblems.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/eigenvalueproblems.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/eigenvalueproblems.F" 2
...
SUBROUTINE diagonalise_ssyev( matrix, mysize, storageform, eigenvalues, &
  ...
  ELSE IF ( storageform ( 1:5 ) == "Upper" .OR. &
     ...
     uplo = "U"
  ELSE
     CALL cp__b("common/eigenvalueproblems.F",79,"Unknown form of storage")
  END IF
  ...
  CALL lapack_ssyev ( jobz, uplo, mysize, eigenvectors, lda, eigenvalues, &
       work, lwork, info )
  IF ( info /= 0 ) THEN
     CALL cp__b("common/eigenvalueproblems.F",90,"Error in diagonalisation")
  END IF
...
SUBROUTINE diagonalise_chpev ( matrix, mysize, storageform, eigenvalues, &
  ...
  ELSE IF ( storageform ( 1:5 ) == "Upper" .OR. &
     ...
     uplo = "U"
  ELSE
     CALL cp__b("common/eigenvalueproblems.F",130,"Unknown form of storage")
  END IF

  CALL lapack_chpev ( jobz, uplo, mysize, matrix, eigenvalues, &
       eigenvectors, mysize, work, rwork, info )
  IF ( info /= 0 ) THEN
     CALL cp__b("common/eigenvalueproblems.F",136,"Error in diagonalisation")
  END IF
...
SUBROUTINE cp2k_sgesvd ( matrix, svalues, mrow, ncol, uvec, vtvec )
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'cp2k_sgesvd', &
      routineP = moduleN//':'//routineN
  ...
  IF ( info /= 0 ) THEN
     CALL cp__b("common/eigenvalueproblems.F",172,"Error in singular value decomposition.")
  END IF
...
SUBROUTINE cp2k_cgesvd ( matrix, svalues, mrow, ncol, uvec, vtvec )
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'cp2k_cgesvd', &
      routineP = moduleN//':'//routineN
    CHARACTER, PARAMETER                     :: jobu = "A", jobvt = "A"
  ...
  IF ( info /= 0 ) THEN
     CALL cp__b("common/eigenvalueproblems.F",209,"Error in singular value decomposition.")
  END IF
