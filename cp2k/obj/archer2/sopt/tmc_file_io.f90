# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_file_io.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_file_io.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_file_io.F" 2
...
!> \retval result_file_name the new filename
...
! *****************************************************************************
  FUNCTION expand_file_name_ending(file_name, extra) RESULT(result_file_name)
    ...
    IF(.NOT.(file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",81)
    ...
    IF(.NOT.(result_file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",91)
...
! *****************************************************************************
  FUNCTION expand_file_name_char(file_name, extra) RESULT(result_file_name)
    ...
    IF(.NOT.(file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",111)
    ...
    END IF
    result_file_name=TRIM(ADJUSTL(result_file_name))
    IF(.NOT.(result_file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",121)
...
! *****************************************************************************
  FUNCTION expand_file_name_temp(file_name,rvalue) RESULT(result_file_name)
    ...
    IF(.NOT.(file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",143)
    ...
    END IF
    result_file_name=TRIM(ADJUSTL(result_file_name))
    IF(.NOT.(result_file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",161)
...
! *****************************************************************************
  FUNCTION expand_file_name_int(file_name,ivalue) RESULT(result_file_name)
    ...
    IF(.NOT.(file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",183)
    ...
    END IF
    result_file_name=TRIM(ADJUSTL(result_file_name))
    IF(.NOT.(result_file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",202)
...
! *****************************************************************************
  SUBROUTINE print_restart_file(tmc_env, job_counts, timings)
    ...
    c_tmp=""
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_file_io.F",229)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_file_io.F",230)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_file_io.F",231)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_act)))CALL cp__a("tmc/tmc_file_io.F",232)
...
! *****************************************************************************
  SUBROUTINE read_restart_file(tmc_env, job_counts, timings, file_name)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_file_io.F",303)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_file_io.F",304)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_file_io.F",305)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_act)))CALL cp__a("tmc/tmc_file_io.F",306)

    IF(file_name.EQ.tmc_default_restart_in_file_name) THEN
      INQUIRE(FILE=tmc_default_restart_in_file_name, EXIST=flag)
      IF(.NOT.(flag))CALL cp__a("tmc/tmc_file_io.F",310)
    ...
    IF(temp_size.NE. SIZE(tmc_env%params%Temp))&
       CALL cp_abort(cp__l("tmc/tmc_file_io.F",321),&
    ...
    IF(ANY(ABS(tmc_env%params%Temp(:)-tmp_temp(:)) .GE. 0.005))&
       CALL cp_abort(cp__l("tmc/tmc_file_io.F",343),"the temperatures differ from the previous calculation. "//&
            "There were the following temperatures used:")
    IF(ANY(mv_weight_tmp(:).NE.tmc_env%params%move_types%mv_weight(:)))&
       CALL cp__w("tmc/tmc_file_io.F",346,"The amount of mv types differs between the original and the restart run.")
...
! *****************************************************************************
  SUBROUTINE write_result_list_element(result_list, result_count,conf_updated,&
    ...
    IF(.NOT.(ASSOCIATED(result_list)))CALL cp__a("tmc/tmc_file_io.F",396)
    IF(.NOT.(ASSOCIATED(result_count)))CALL cp__a("tmc/tmc_file_io.F",397)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_file_io.F",398)
    IF(.NOT.(ASSOCIATED(tmc_params%Temp)))CALL cp__a("tmc/tmc_file_io.F",399)
    IF(.NOT.(conf_updated.GE.0))CALL cp__a("tmc/tmc_file_io.F",400)
    IF(.NOT.(conf_updated.LE.SIZE(tmc_params%Temp)))CALL cp__a("tmc/tmc_file_io.F",401)
...
! *****************************************************************************
  SUBROUTINE write_element_in_file(elem, tmc_params, temp_index, file_name, conf_nr, &
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_file_io.F",461)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_file_io.F",462)
    IF(.NOT.(ASSOCIATED(tmc_params%atoms)))CALL cp__a("tmc/tmc_file_io.F",463)
    IF(.NOT.(PRESENT(conf_nr).OR.PRESENT(conf_info)))CALL cp__a("tmc/tmc_file_io.F",464)
    ...
    IF(print_it) THEN
      ...
      IF(PRESENT(file_name)) THEN
        IF(.NOT.(file_name.NE.""))CALL cp__a("tmc/tmc_file_io.F",472)
        file_name_act = file_name
      ELSE
        IF(.NOT.(ASSOCIATED(tmc_params%Temp)))CALL cp__a("tmc/tmc_file_io.F",475)
        IF(.NOT.(PRESENT(temp_index)))CALL cp__a("tmc/tmc_file_io.F",476)
...
! *****************************************************************************
  SUBROUTINE write_dipoles_in_file(file_name, conf_nr, dip, file_ext)
    ...
    IF(.NOT.(ASSOCIATED(dip)))CALL cp__a("tmc/tmc_file_io.F",604)

    IF(PRESENT(file_ext)) THEN
      IF(.NOT.(file_ext.NE.""))CALL cp__a("tmc/tmc_file_io.F",607)
...
! *****************************************************************************
  SUBROUTINE read_element_from_file(elem, tmc_ana, conf_nr, stat)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_file_io.F",655)
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",656)
    IF(.NOT.(ASSOCIATED(tmc_ana%atoms)))CALL cp__a("tmc/tmc_file_io.F",657)
    ...
    IF(tmc_ana%id_traj.GT.0) THEN
      ...
      IF(stat.EQ.TMC_STATUS_WAIT_FOR_NEW_TASK) THEN
        CALL cp_warn(cp__l("tmc/tmc_file_io.F",668),&
      ...
      ELSE
        IF(.NOT.(i_tmp.GT.conf_nr_old))CALL cp__a("tmc/tmc_file_io.F",673)
        conf_nr = i_tmp
        elem%nr = i_tmp
      END IF
    END IF
    ...
    IF(tmc_ana%id_dip.GT.0 .AND. stat.EQ.TMC_STATUS_OK) THEN
      ...
      search_conf_dip: DO
        CALL read_dipole_from_file(elem=elem, tmc_ana=tmc_ana, stat=stat, &
                                   conf_nr=i_tmp)
        IF(stat.EQ.TMC_STATUS_WAIT_FOR_NEW_TASK) THEN 
          CALL cp_warn(cp__l("tmc/tmc_file_io.F",689),&
    ...
    IF(tmc_ana%id_cell.GT.0 .AND. stat.EQ.TMC_STATUS_OK) THEN
      search_conf_cell: DO
        CALL read_cell_from_file(elem=elem, tmc_ana=tmc_ana, stat=stat, &
                                 conf_nr=i_tmp)
        IF(stat.EQ.TMC_STATUS_WAIT_FOR_NEW_TASK) THEN
          CALL cp_warn(cp__l("tmc/tmc_file_io.F",719),&
    ...
    IF(files_conf_missmatch)&
       CALL cp_warn(cp__l("tmc/tmc_file_io.F",748),&
...
! *****************************************************************************
  SUBROUTINE read_pos_from_file(elem, tmc_ana, stat, conf_nr, header_info)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_file_io.F",783)
    IF(.NOT.(ASSOCIATED(elem%pos)))CALL cp__a("tmc/tmc_file_io.F",784)
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",785)
    IF(.NOT.(tmc_ana%id_traj.GT.0))CALL cp__a("tmc/tmc_file_io.F",786)
    ...
    search_next_conf:DO
      ...
      IF(status.GT.0)&
         CALL cp_abort(cp__l("tmc/tmc_file_io.F",796),&
      ...
      IF(INDEX(c_tmp,"=").GT.0)THEN
        ...
        IF(status.NE.0)&
           CALL cp_abort(cp__l("tmc/tmc_file_io.F",806),&
    ...
    IF(stat.EQ. TMC_STATUS_OK) THEN
      pos_loop:DO i=1, SIZE(elem%pos), tmc_ana%dim_per_elem
        ...
        IF(status.NE.0) THEN
          CALL cp_abort(cp__l("tmc/tmc_file_io.F",825),&
...
! *****************************************************************************
  SUBROUTINE read_dipole_from_file(elem, tmc_ana, stat, conf_nr)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_file_io.F",859)
    IF(.NOT.(ASSOCIATED(elem%dipole)))CALL cp__a("tmc/tmc_file_io.F",860)
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",861)
    IF(.NOT.(tmc_ana%id_dip.GT.0))CALL cp__a("tmc/tmc_file_io.F",862)
    ...
    ELSE
      IF(status.NE.0)&
         CALL cp__w("tmc/tmc_file_io.F",885,"configuration dipole read error at line: "//cp_to_string(tmc_ana%lc_dip))
      stat = TMC_STATUS_FAILED
    END IF
...
! *****************************************************************************
  SUBROUTINE read_cell_from_file(elem, tmc_ana, stat, conf_nr)
    ...
    IF(.NOT.(ASSOCIATED(elem)))CALL cp__a("tmc/tmc_file_io.F",917)
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",918)
    IF(.NOT.(ASSOCIATED(tmc_ana%cell)))CALL cp__a("tmc/tmc_file_io.F",919)
    IF(.NOT.(tmc_ana%id_cell.GT.0))CALL cp__a("tmc/tmc_file_io.F",920)
    ...
    ELSE IF(status.GT.0) THEN
      IF(status.NE.0)&
         CALL cp__b("tmc/tmc_file_io.F",942,"configuration cell read error at line: "//cp_to_string(tmc_ana%lc_cell))
      stat = TMC_STATUS_FAILED
    ELSE
...
! *****************************************************************************
  SUBROUTINE analyse_files_open(tmc_ana, stat, dir_ind)
    ...
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",981)
    ...
    IF(PRESENT(dir_ind)) THEN
      IF(.NOT.(ASSOCIATED(tmc_ana%dirs)))CALL cp__a("tmc/tmc_file_io.F",989)
      IF(.NOT.(dir_ind.GT.0))CALL cp__a("tmc/tmc_file_io.F",990)
      IF(.NOT.(dir_ind.LE.SIZE(tmc_ana%dirs)))CALL cp__a("tmc/tmc_file_io.F",991)
    ...
    IF(tmc_ana%id_traj.GT.0 .OR. tmc_ana%id_cell.GT.0 .OR. &
      ...
      stat = TMC_STATUS_OK
    ELSE
      CALL cp_warn(cp__l("tmc/tmc_file_io.F",1056),&
...
! *****************************************************************************
  SUBROUTINE analyse_files_close(tmc_ana)
    ...
    IF(.NOT.(ASSOCIATED(tmc_ana)))CALL cp__a("tmc/tmc_file_io.F",1078)
