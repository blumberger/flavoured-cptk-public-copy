# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_multrec.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_multrec.F"
!-----------------------------------------------------------------------------!
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_multrec.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    !$      "Missing thread distribution.", 278)
    ...
    ELSE
       ...
       DO i = 1 , product%nblkrows_total
          this%c_local_rows(i) = i
       END DO
       DO i = 1 , product%nblkcols_total
    ...
    IF (this%use_eps) THEN
       ...
       ELSE
          IF (careful_mod) &
               ...
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "max epss local/global mismatch.", 350)
          this%row_max_epss(:) = row_max_epss(:)
       ENDIF
    ELSE
...
!> \param this ...
! *****************************************************************************
  SUBROUTINE dbcsr_mm_multrec_finalize(this)
    TYPE(dbcsr_mm_multrec_type), &
      INTENT(inout)                          :: this
