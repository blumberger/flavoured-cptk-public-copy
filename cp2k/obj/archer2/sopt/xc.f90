# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc.F"
!-----------------------------------------------------------------------------!
...
# 80 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc.F" 2
...
! *****************************************************************************
  SUBROUTINE xc_vxc_pw_create1(vxc_rho,vxc_tau,rho_r,rho_g,tau,exc,xc_section,&
    ...
    IF(.NOT.(ASSOCIATED(rho_r)))CALL cp__a("xc/xc.F",131)
    IF(.NOT.(ASSOCIATED(xc_section)))CALL cp__a("xc/xc.F",132)
    IF(.NOT.(.NOT.ASSOCIATED(vxc_rho)))CALL cp__a("xc/xc.F",133)
    IF(.NOT.(.NOT.ASSOCIATED(vxc_tau)))CALL cp__a("xc/xc.F",134)
...
SUBROUTINE xc_vxc_pw_create_test_lsd(vxc_rho,vxc_tau,rho_r,rho_g,tau,&
  ...
  END IF
    ...
    ! IF(.NOT.(maxdiff<5.e-11))CALL cp__a("xc/xc.F",373)
    ! IF(.NOT.(ABS(exc-exc2)<1.e-14))CALL cp__a("xc/xc.F",374)
    ...
    DO WHILE (cp_sll_xc_deriv_next(deriv_iter,el_att=deriv))
       ...
       CASE(1)
          ...
          ELSEIF (split_desc(1)=="norm_drho") THEN
             ...
             END IF
          ELSE
             CALL cp__b("xc/xc.F",432,"")
          END IF
       CASE default
          CALL cp__b("xc/xc.F",435,"")
       END SELECT
       CALL xc_derivative_get(deriv2,&
            deriv_data=pot2)
...
SUBROUTINE xc_rho_set_and_dset_create(rho_set,deriv_set,deriv_order,&
  ...
  IF(.NOT.(.NOT.ASSOCIATED(rho_set)))CALL cp__a("xc/xc.F",832)
  IF(.NOT.(.NOT.ASSOCIATED(deriv_set)))CALL cp__a("xc/xc.F",833)
  IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("xc/xc.F",834)
...
! *****************************************************************************
  SUBROUTINE smooth_cutoff(pot,rho,rhoa,rhob,rho_cutoff,&
    ...
    IF(.NOT.(ASSOCIATED(pot)))CALL cp__a("xc/xc.F",901)
    bo(1,:)=LBOUND(pot)
    bo(2,:)=UBOUND(pot)
    ...
    IF (rho_smooth_cutoff_range>0.0_dp) THEN
       IF (PRESENT(e_0)) THEN
          IF(.NOT.(ASSOCIATED(e_0)))CALL cp__a("xc/xc.F",912)
       ...
       ELSE
          IF (ASSOCIATED(rho)) THEN
             ...
             END DO
          ELSE
             IF(.NOT.(ASSOCIATED(rhoa)))CALL cp__a("xc/xc.F",1009)
             IF(.NOT.(ASSOCIATED(rhob)))CALL cp__a("xc/xc.F",1010)
             !$omp parallel do default(none) shared(bo,pot,&
             !$omp             rho_cutoff,rho_smooth_cutoff,rho_smooth_cutoff_2,&
...
SUBROUTINE xc_vxc_pw_create(vxc_rho,vxc_tau,exc,rho_r,rho_g,tau,xc_section,&
    ...
    INTEGER :: handle, i, idir, ispin, j, jdir, k, n_deriv, npoints, nspins, &
      order, xc_deriv_method_id, xc_rho_smooth_id
    INTEGER, DIMENSION(2, 3)                 :: bo
    INTEGER, DIMENSION(3, 3)                 :: nd, nd_laplace
    LOGICAL                                  :: dealloc_pw_to_deriv, &
  ...
  IF(.NOT.(ASSOCIATED(xc_section)))CALL cp__a("xc/xc.F",1129)
  IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("xc/xc.F",1130)
  IF(.NOT.(.NOT.ASSOCIATED(vxc_rho)))CALL cp__a("xc/xc.F",1131)
  IF(.NOT.(.NOT.ASSOCIATED(vxc_tau)))CALL cp__a("xc/xc.F",1132)
  nspins=SIZE(rho_r)
  IF(.NOT.(ASSOCIATED(rho_r(SIZE(rho_r))%pw)))CALL cp__a("xc/xc.F",1134)
  lsd=(nspins/=1)
  IF (lsd) THEN
     IF(.NOT.(nspins==2))CALL cp__a("xc/xc.F",1137)
  END IF
     ...
     IF (order==1) THEN
        IF (lsd) THEN
           ...
           CASE default
              ...
              CALL cp__b("xc/xc.F",1189,"unknown functional derivative (LSD): '"//TRIM(desc)//"'")
           END SELECT
        ELSE
           SELECT CASE(desc)
           CASE("(rho)","(norm_drho)")
           ...
           CASE("(laplace_rho)")
              ...
              n_deriv = n_deriv + 1
           CASE default
              CALL cp__b("xc/xc.F",1202,"unknown functional derivative (LDA): '"//TRIM(desc)//"'")
           END SELECT
        END IF
     END IF
  END DO
  ...
  IF (ASSOCIATED(deriv_att)) THEN
     ...
     ELSE IF (ASSOCIATED(drho)) THEN
        ...
        END DO
     ELSE
        IF(.NOT.(ASSOCIATED(drhoa)))CALL cp__a("xc/xc.F",1310)
        IF(.NOT.(ASSOCIATED(drhob)))CALL cp__a("xc/xc.F",1311)
     ...
     IF (ASSOCIATED(drho).AND.ASSOCIATED(drho(1)%array)) THEN
        IF(.NOT.(ASSOCIATED(deriv_data)))CALL cp__a("xc/xc.F",1327)
        IF (use_virial) THEN
        ...
        DO idir=1,3
           CALL pw_create(pw_to_deriv_rho(idir)%pw,pw_grid=pw_grid,&
                ...
                use_data=REALDATA3D,in_space=REALSPACE)
           IF(.NOT.(ASSOCIATED(drho(idir)%array)))CALL cp__a("xc/xc.F",1354)
  ...
  DO ispin=1,nspins
     ...
     IF (lsd) THEN
        ...
        IF (ASSOCIATED(deriv_att)) THEN
           IF(.NOT.(lsd))CALL cp__a("xc/xc.F",1427)
           CALL xc_derivative_get(deriv_att,deriv_data=deriv_data)
           ...
           DO idir=1,3
              IF(.NOT.(ASSOCIATED(drho_spin(idir)%array)))CALL cp__a("xc/xc.F",1486)
              IF(.NOT.(ASSOCIATED(vxc_to_deriv(ispin)%pw%cr3d)))CALL cp__a("xc/xc.F",1487)
     ...
     IF( has_laplace ) THEN
       IF (use_virial) THEN
         CALL cp__b("xc/xc.F",1626,"Stress tensor not available for Laplace functionals.")
       END IF
       nd_laplace = RESHAPE((/2,0,0,0,2,0,0,0,2/),(/3,3/))
       IF(lsd) THEN
         IF( ispin == 1) THEN
           deriv_att => xc_dset_get_derivative(deriv_set, "(laplace_rhoa)")
           IF(.NOT.(ASSOCIATED(deriv_att)))CALL cp__a("xc/xc.F",1632)
           CALL xc_derivative_get(deriv_att,deriv_data=deriv_data)
         ELSE
           deriv_att => xc_dset_get_derivative(deriv_set, "(laplace_rhob)")
           IF(.NOT.(ASSOCIATED(deriv_att)))CALL cp__a("xc/xc.F",1636)
           CALL xc_derivative_get(deriv_att,deriv_data=deriv_data)
         END IF

       ELSE
         deriv_att => xc_dset_get_derivative(deriv_set, "(laplace_rho)")
         IF(.NOT.(ASSOCIATED(deriv_att)))CALL cp__a("xc/xc.F",1642)
         CALL xc_derivative_get(deriv_att,deriv_data=deriv_data)
       END IF
       DO idir=1,3
  ...
  DO idir=1,3
     IF(.NOT.(.NOT.ASSOCIATED(pw_to_deriv(idir)%pw)))CALL cp__a("xc/xc.F",1725)
     IF(.NOT.(.NOT.ASSOCIATED(pw_to_deriv_rho(idir)%pw)))CALL cp__a("xc/xc.F",1726)
  END DO
  ...
  IF (n_deriv>0) THEN
     ...
     IF(.NOT.(ASSOCIATED(deriv_att)))CALL cp__a("xc/xc.F",1733)
     CALL xc_derivative_get(deriv_att,deriv_data=deriv_data)
  ...
  IF (has_tau) THEN
     IF (use_virial) THEN
       CALL cp__b("xc/xc.F",1759,"Stress tensor not available for Meta-functionals.")
     END IF
     ...
     IF (lsd) THEN
        ...
        IF (ASSOCIATED(deriv_att)) THEN
           ...
           IF (ASSOCIATED(vxc_tau(1)%pw)) THEN
              DO ispin=1,2
                 IF(.NOT.(ASSOCIATED(vxc_tau(ispin)%pw)))CALL cp__a("xc/xc.F",1789)
     ...
     END IF
     DO ispin=1,nspins
        IF(.NOT.(ASSOCIATED(vxc_tau(ispin)%pw)))CALL cp__a("xc/xc.F",1812)
     END DO
  END IF
...
SUBROUTINE xc_calc_2nd_deriv(v_xc, deriv_set, rho_set, rho1_set, &
    ...
    INTEGER, DIMENSION(2, 3)                 :: bo
    LOGICAL                                  :: gradient_f, lsd, my_gapw, &
                                                unknown_deriv
...
! PARAMETER
  ...
  IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc.F",1973)
  IF(.NOT.(ASSOCIATED(rho1_set)))CALL cp__a("xc/xc.F",1974)
  IF(.NOT.(ASSOCIATED(v_xc)))CALL cp__a("xc/xc.F",1975)
  IF(.NOT.(ASSOCIATED(xc_section)))CALL cp__a("xc/xc.F",1976)
  IF (my_gapw) THEN
     IF(.NOT.(PRESENT(vxg)))CALL cp__a("xc/xc.F",1978)
  END IF
  ...
  DO WHILE (cp_sll_xc_deriv_next(pos,el_att=deriv_att))
     ...
     END SELECT
     IF(unknown_deriv)&
        CALL cp_abort(cp__l("xc/xc.F",2067),&
        ...
        DO ispin=1, nspins
           ...
           ELSE
              ...
              ALLOCATE(tmp_a(ispin)%pw%cr3d(bo(1,1):bo(2,1),bo(1,2):bo(2,2),bo(1,3):bo(2,3)), &
                   ...
                   stat=stat)
              IF(.NOT.(stat==0))CALL cp__a("xc/xc.F",2103)
              ...
              DO ispin=1, nspins
                 ...
                 CASE (xc_deriv_nn50_smooth)
                    CALL pw_nn_deriv_r ( pw_in=tmp_r(ispin)%pw,&
                         ...
                         idir=idir)
                 CASE default
                    CALL cp__b("xc/xc.F",2699,"XC_DERIV method not implemented for GPW-LSD!")
                 END SELECT
              END DO ! ispin
        ...
        ELSE
           ...
           ELSE
              ...
              ALLOCATE(tmp_r(1)%pw%cr3d(bo(1,1):bo(2,1),bo(1,2):bo(2,2),bo(1,3):bo(2,3)), &
                   stat=stat)
              IF(.NOT.(stat==0))CALL cp__a("xc/xc.F",2848)
           END IF
           ...
           DO idir=1,3
              ...
              CASE (xc_deriv_nn50_smooth)
                 CALL pw_nn_deriv_r ( pw_in=tmp_r(1)%pw,&
                      ...
                      idir=idir)
              CASE default
                 CALL cp__b("xc/xc.F",2881,"XC_DERIV method not implemented for GPW!")
              END SELECT
...
! *****************************************************************************
  SUBROUTINE xc_prep_2nd_deriv(deriv_set, &
       rho_set, rho_r, pw_pool, xc_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(rho_set)))CALL cp__a("xc/xc.F",2944)
    IF(.NOT.(ASSOCIATED(xc_section)))CALL cp__a("xc/xc.F",2945)
    IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("xc/xc.F",2946)
    ...
    DO WHILE (cp_sll_xc_deriv_next(pos,el_att=deriv_att))
       ...
       IF (order==1 .OR. order==2) THEN
          DO idesc=1,SIZE(split_desc)
             ...
             CASE("rho")
                IF(lsd)&
                   CALL cp__b("xc/xc.F",3038,"rho not handled in lsd: '"//TRIM(desc)//"'")
             CASE("rhoa","rhob")
             CASE default
                CALL cp_abort(cp__l("xc/xc.F",3041),&
