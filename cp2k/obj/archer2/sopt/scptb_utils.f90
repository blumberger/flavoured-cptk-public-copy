# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_utils.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE scptb_global_parameter_broadcast(scptb_control,para_env)
    ...
    IF(para_env%ionode) THEN
       IF(.NOT.(source==para_env%mepos))CALL cp__a("scptb_utils.F",576)
    ELSE
       IF(.NOT.(source/=para_env%mepos))CALL cp__a("scptb_utils.F",578)
    END IF
...
! *****************************************************************************
  SUBROUTINE scptb_parameter_broadcast(scptb_parameter,para_env)
    ...
    IF(para_env%ionode) THEN
       IF(.NOT.(source==para_env%mepos))CALL cp__a("scptb_utils.F",605)
    ELSE
       IF(.NOT.(source/=para_env%mepos))CALL cp__a("scptb_utils.F",607)
    END IF
