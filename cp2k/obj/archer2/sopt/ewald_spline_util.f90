# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_spline_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_spline_util.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_spline_util.F" 2
...
!
    ...
    IF(.NOT.(.NOT.ASSOCIATED(pw_grid)))CALL cp__a("ewald_spline_util.F",100)
    IF(.NOT.(.NOT.ASSOCIATED(pw_pool)))CALL cp__a("ewald_spline_util.F",101)
    IF(.NOT.(.NOT.ASSOCIATED(coeff)))CALL cp__a("ewald_spline_util.F",102)
    NULLIFY(cell, pw)

    CALL cell_create(cell, hmat=hmat, periodic=(/1,1,1/))
    CALL pw_grid_create( pw_grid, para_env%group, local=.TRUE.)
...
! *****************************************************************************
  SUBROUTINE eval_pw_TabLR( grid, pw_pool, TabLR, Lg, gx, gy, gz, hmat_mm,&
    ...
    INTEGER :: act_nx, act_ny, aint_precond, handle, i, iii, is, j, js, k, &
      ks, Lg_loc_max, Lg_loc_min, max_iter, my_i, my_j, my_k, n1, n2, n3, &
      n_extra, NLg_loc, nxlim, nylim, nzlim, precond_kind
    INTEGER, DIMENSION(2, 3)                 :: gbo
    LOGICAL                                  :: success
    REAL(KIND=dp)                            :: dr1, dr2, dr3, eps_r, eps_x, &
                                                xs1, xs2, xs3
    REAL(KIND=dp), ALLOCATABLE :: cos_gx(:,:), cos_gy(:,:), cos_gz(:,:), &
      lhs(:,:), rhs(:,:), sin_gx(:,:), sin_gy(:,:), sin_gz(:,:)
...
!NB temporaries for holding stuff so that dgemm can be used 
    ...
    success=find_coeffs(values=grid,coeffs=TabLR,&
            ...
            max_iter=max_iter)
    IF(.NOT.(success))CALL cp__a("ewald_spline_util.F",334)
