# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kinetic.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kinetic.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kinetic.F" 2
...
! *****************************************************************************
  SUBROUTINE build_kinetic_matrix(ks_env,matrix_t,matrixkp_t,matrix_name,&
    ...
    INTEGER :: atom_a, atom_b, handle, iatom, ic, icol, ikind, inode, irow, &
      iset, jatom, jkind, jset, ldsab, natom, ncoa, ncob, nimg, nkind, nseta, &
      nsetb, sgfa, sgfb
    ...
    TYPE(virial_type), POINTER               :: virial
     ...
     ! check for symmetry
     IF(.NOT.(SIZE(sab_nl) > 0))CALL cp__a("qs_kinetic.F",182)
     ...
     IF(dokp) THEN
        CALL cp_dbcsr_allocate_matrix_set(matrixkp_t,1,nimg)
        CALL create_sab_matrix(ks_env, matrixkp_t, matrix_name, basis_set_list, basis_set_list, &
                               sab_nl, do_symmetric)
     ELSE
        CALL cp_dbcsr_allocate_matrix_set(matrix_t,1)
     ...
     IF (do_forces) THEN
        ...
        IF(dokp) THEN
           IF(.NOT.(PRESENT(matrixkp_p)))CALL cp__a("qs_kinetic.F",205)
        ELSE
           IF(.NOT.(PRESENT(matrix_p)))CALL cp__a("qs_kinetic.F",207)
        END IF
     END IF
     ...
     DO WHILE (neighbor_list_iterate(nl_iterator)==0)
        ...
        IF(use_cell_mapping) THEN
           ...
           IF(.NOT.(ic > 0))CALL cp__a("qs_kinetic.F",254)
        ELSE
           ic = 1
        END IF
        ...
        IF(dokp) THEN
           ...
           IF(.NOT.(found))CALL cp__a("qs_kinetic.F",280)
        ELSE
           CALL cp_dbcsr_get_block_p(matrix=matrix_t(1)%matrix,&
                row=irow,col=icol,BLOCK=k_block,found=found)
           IF(.NOT.(found))CALL cp__a("qs_kinetic.F",284)
        END IF
     ...
     IF(dokp) THEN
        DO ic=1,nimg
           CALL cp_dbcsr_finalize(matrixkp_t(1,ic)%matrix)
           IF (PRESENT(eps_filter)) THEN
              CALL cp_dbcsr_filter(matrixkp_t(1,ic)%matrix, eps_filter)
           END IF
        END DO
     ELSE
        CALL cp_dbcsr_finalize(matrix_t(1)%matrix)
        IF (PRESENT(eps_filter)) THEN
           CALL cp_dbcsr_filter(matrix_t(1)%matrix, eps_filter)
        END IF
     END IF
