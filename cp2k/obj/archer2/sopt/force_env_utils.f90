# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_utils.F"
!-----------------------------------------------------------------------------!
...
# 36 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE force_env_shake(force_env,dt,shake_tol,log_unit,lagrange_mult,dump_lm,&
    ...
    INTEGER :: handle, i, iparticle, iparticle_kind, iparticle_local, j, &
      my_lagrange_mult, my_log_unit, nparticle_kind, nparticle_local
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_utils.F",98)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_utils.F",99)
    my_log_unit=-1
...
! *****************************************************************************
  SUBROUTINE force_env_rattle(force_env,dt,shake_tol,log_unit,lagrange_mult,dump_lm,&
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_utils.F",268)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_utils.F",269)
    my_log_unit=-1
    IF (PRESENT(log_unit)) my_log_unit=log_unit
    my_lagrange_mult=-1
    IF (PRESENT(lagrange_mult)) my_lagrange_mult=lagrange_mult
...
! *****************************************************************************
  SUBROUTINE rescale_forces (force_env)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_utils.F",386)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_utils.F",387)
...
! *****************************************************************************
  SUBROUTINE write_stress_tensor(pv_virial,output_unit,cell,ndigits,numerical)
    ...
    IF (output_unit > 0) THEN
      IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("force_env_utils.F",438)
...
! *****************************************************************************
  SUBROUTINE write_forces(particles,output_unit,label,ndigits,total_force,&
    ...
    IF (output_unit > 0) THEN
      IF(.NOT.(ASSOCIATED(particles)))CALL cp__a("force_env_utils.F",516)
      n = MIN(MAX(1,ndigits),20)
