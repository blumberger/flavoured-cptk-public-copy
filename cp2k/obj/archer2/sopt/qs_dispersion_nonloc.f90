# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_nonloc.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_nonloc.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_nonloc.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_dispersion_nonloc_init(dispersion_env,para_env)
    ...
    SELECT CASE (dispersion_env%nl_type)
       CASE DEFAULT
          CALL cp__b("qs_dispersion_nonloc.F",88,"Unknown vdW-DF functional")
...
! *****************************************************************************
  SUBROUTINE calculate_dispersion_nonloc(vxc_rho,rho_r,rho_g,edispersion,&
    ...
    IF(.NOT.(ASSOCIATED(rho_r)))CALL cp__a("qs_dispersion_nonloc.F",204)
    IF(.NOT.(ASSOCIATED(rho_g)))CALL cp__a("qs_dispersion_nonloc.F",205)
    IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("qs_dispersion_nonloc.F",206)
    ...
    ENDIF
    IF(use_virial) THEN
       IF(.NOT.(.NOT.energy_only))CALL cp__a("qs_dispersion_nonloc.F",214)
    END IF
    IF (.NOT. energy_only) THEN
       IF(.NOT.(ASSOCIATED(vxc_rho)))CALL cp__a("qs_dispersion_nonloc.F",217)
    END IF
    ...
    drho = 0.0_dp
    DO ispin=1,nspin
       IF(.NOT.(rho_r(ispin)%pw%in_use==REALDATA3D))CALL cp__a("qs_dispersion_nonloc.F",246)
       CALL pw_transfer ( rho_g(ispin)%pw, tmp_g)
       CALL pw_transfer ( tmp_g, tmp_r)
       rho(:) = rho(:) + RESHAPE(tmp_r%cr3d,(/np/))
       drho(:,1) = drho(:,1) + RESHAPE(drho_r(1,ispin)%pw%cr3d,(/np/))
       drho(:,2) = drho(:,2) + RESHAPE(drho_r(2,ispin)%pw%cr3d,(/np/))
       drho(:,3) = drho(:,3) + RESHAPE(drho_r(3,ispin)%pw%cr3d,(/np/))
    END DO
    !! ---------------------------------------------------------------------------------
    ...
    IF (energy_only) THEN
       ...
       SELECT CASE (dispersion_env%nl_type)
          CASE DEFAULT
             CALL cp__b("qs_dispersion_nonloc.F",266,"Unknown vdW-DF functional")
          CASE (vdw_nl_DRSLL,vdw_nl_LMKLL)
    ...
    ELSE
       ...
       SELECT CASE (dispersion_env%nl_type)
          CASE DEFAULT
             CALL cp__b("qs_dispersion_nonloc.F",276,"Unknown vdW-DF functional")
          CASE (vdw_nl_DRSLL,vdw_nl_LMKLL)
    ...
    SELECT CASE (dispersion_env%nl_type)
       CASE DEFAULT
          CALL cp__b("qs_dispersion_nonloc.F",308,"Unknown vdW-DF functional")
       CASE (vdw_nl_DRSLL,vdw_nl_LMKLL)
    ...
    IF (.NOT.energy_only) THEN
       ...
       DO idir=1,3
          tmp_r%cr3d = 0.0_dp
          DO ispin=1,nspin
             tmp_r%cr3d = tmp_r%cr3d + RESHAPE(hpot,(/n1,n2,n3/)) * drho_r(idir,ispin)%pw%cr3d
          END DO
       ...
       DO ispin=1,nspin
          CALL pw_axpy(vxc_r,vxc_rho(ispin)%pw,1._dp)
       END DO
...
! *****************************************************************************
  SUBROUTINE vdW_energy(thetas_g, dispersion_env, vdW_xc_energy, energy_only, virial)
    ...
    ALLOCATE(theta(nqs),thetam(nqs),theta_g(nqs))
    ...
    IF (.NOT.energy_only) THEN
       ...
       DEALLOCATE(u_vdW)
    END IF
    DEALLOCATE(theta,thetam,theta_g)
...
! *****************************************************************************
  SUBROUTINE get_potential(q0, dq0_drho, dq0_dgradrho, total_rho, u_vdW, potential, h_prefactor, &
    ...
    IF (PRESENT(virial)) THEN
       use_virial = .TRUE.
       IF(.NOT.(PRESENT(drho)))CALL cp__a("qs_dispersion_nonloc.F",616)
       IF(.NOT.(PRESENT(dvol)))CALL cp__a("qs_dispersion_nonloc.F",617)
    ELSE
       use_virial = .FALSE.
    END IF
    ...
    DO i_grid = 1,SIZE(u_vdw,1)
       ...
       DO WHILE ( (q_hi - q_low) > 1)
          ...
          END IF
       END DO
       IF (q_hi == q_low) CALL cp__b("qs_dispersion_nonloc.F",644,"get_potential:  qhi == qlow")
       ...
       DO P_i = 1, Nqs
          ...
          SELECT CASE (dispersion_env%nl_type)
             CASE DEFAULT
                CALL cp__b("qs_dispersion_nonloc.F",663,"Unknown vdW-DF functional")
             CASE (vdw_nl_DRSLL,vdw_nl_LMKLL)
...
! *****************************************************************************
  ...
  !!
    ...
    SELECT CASE (dispersion_env%nl_type)
       CASE DEFAULT
          CALL cp__b("qs_dispersion_nonloc.F",746,"Unknown vdW-DF functional")
       CASE (vdw_nl_DRSLL)
          Z_ab = -0.8491_dp
       CASE (vdw_nl_LMKLL)
          Z_ab = -1.887_dp
...
! *****************************************************************************
  SUBROUTINE get_q0_on_grid_eo_vdw (total_rho, gradient_rho, q0, dispersion_env)
    ...
    SELECT CASE (dispersion_env%nl_type)
       CASE DEFAULT
          CALL cp__b("qs_dispersion_nonloc.F",953,"Unknown vdW-DF functional")
       CASE (vdw_nl_DRSLL)
          Z_ab = -0.8491_dp
       CASE (vdw_nl_LMKLL)
          Z_ab = -1.887_dp
...
! *****************************************************************************
  SUBROUTINE interpolate_kernel(k, kernel_of_k, dispersion_env)
    ...
    !! of radial points.
    !! -------------------------------------------------------------------------------------
    IF(.NOT.(k < Nr_points*dk))CALL cp__a("qs_dispersion_nonloc.F",1223)
    !! -------------------------------------------------------------------------------------
    kernel_of_k = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE interpolate_dkernel_dk(k, dkernel_of_dk, dispersion_env)
    ...
    IF(.NOT.(k < Nr_points*dk))CALL cp__a("qs_dispersion_nonloc.F",1285)
