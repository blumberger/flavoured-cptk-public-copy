# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_types.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_types.F" 2
...
! *****************************************************************************
  SUBROUTINE metadyn_create(meta_env, n_colvar, dt, para_env, metadyn_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(meta_env)))CALL cp__a("metadynamics_types.F",159)
    ALLOCATE(meta_env)
...
! *****************************************************************************
  SUBROUTINE meta_env_retain(meta_env)
    ...
    IF(.NOT.(ASSOCIATED(meta_env)))CALL cp__a("metadynamics_types.F",269)
    IF(.NOT.(meta_env%ref_count>0))CALL cp__a("metadynamics_types.F",270)
...
! *****************************************************************************
  SUBROUTINE meta_env_release(meta_env)
    ...
    IF (ASSOCIATED(meta_env)) THEN
       IF(.NOT.(meta_env%ref_count>0))CALL cp__a("metadynamics_types.F",288)
       meta_env%ref_count=meta_env%ref_count-1
