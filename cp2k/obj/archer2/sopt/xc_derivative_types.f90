# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivative_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivative_types.F"
!-----------------------------------------------------------------------------!
...
# 20 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivative_types.F" 2
...
SUBROUTINE xc_derivative_retain(deriv)
  ...
  IF(.NOT.(ASSOCIATED(deriv)))CALL cp__a("xc/xc_derivative_types.F",101)
  IF(.NOT.(deriv%ref_count>0))CALL cp__a("xc/xc_derivative_types.F",102)
...
! *****************************************************************************
  SUBROUTINE xc_derivative_release(derivative, pw_pool)
    ...
    IF(.NOT.(ASSOCIATED(derivative)))CALL cp__a("xc/xc_derivative_types.F",120)
    IF(.NOT.(derivative%ref_count>=1))CALL cp__a("xc/xc_derivative_types.F",121)
...
SUBROUTINE xc_derivative_get(deriv,desc,split_desc,&
  ...
  IF(.NOT.(ASSOCIATED(deriv)))CALL cp__a("xc/xc_derivative_types.F",176)
  IF(.NOT.(deriv%ref_count>0))CALL cp__a("xc/xc_derivative_types.F",177)
  ...
  IF (PRESENT(deriv_data)) THEN
     ...
     IF (.NOT.my_accept_null_data) THEN
        IF(.NOT.(ASSOCIATED(deriv_data)))CALL cp__a("xc/xc_derivative_types.F",183)
     END IF
  END IF
