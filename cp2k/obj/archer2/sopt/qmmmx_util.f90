# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_util.F"
!-----------------------------------------------------------------------------!
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_util.F" 2
...
! *****************************************************************************
  SUBROUTINE add_new_label(ip, label, n_new, new_indices, new_labels, new_full_labels, max_n_qm)
    ...
    IF (new_full_labels(ip) > force_mixing_label_none) THEN ! already marked, just change mark
         ...
         END DO
         IF(old_index <= 0)&
            CALL cp_abort(cp__l("qmmmx_util.F",397),&
    ...
    ELSE
         n_new = n_new + 1
         IF(n_new > max_n_qm)&
            CALL cp_abort(cp__l("qmmmx_util.F",404),&
...
! *****************************************************************************
  SUBROUTINE setup_force_mixing_qmmm_sections(subsys, qmmm_section, qmmm_core_section, qmmm_extended_section)
    ...
    IF(SIZE(cur_indices) <= 0)&
       CALL cp__b("qmmmx_util.F",680,"cur_indices is empty, found no QM atoms")
    ...
    DO ip=1, SIZE(cur_indices)
       IF (cur_labels(ip) > force_mixing_label_none .AND. cur_labels(ip) < force_mixing_label_QM_core_list .AND. &
          ...
          END DO
          IF(.NOT.mapped)&
             CALL cp_abort(cp__l("qmmmx_util.F",701),&
    ...
    IF(i_rep_section_core <= 0)&
       CALL cp_abort(cp__l("qmmmx_util.F",712),&
...
! *****************************************************************************
  SUBROUTINE get_force_mixing_indices(force_mixing_section, indices, labels)
    ...
    IF(n_indices /= n_labels)&
       CALL cp__b("qmmmx_util.F",841,"got unequal numbers of force_mixing indices and labels!")
