# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_methods.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE pao_print_atom_info(pao)
    ...
    IF(.NOT.(SIZE(pao_basis) == SIZE(pri_basis)))CALL cp__a("pao_methods.F",85)
    natoms = SIZE(pao_basis)

    param_rows => cp_dbcsr_row_block_sizes(pao%matrix_X)
    param_cols => cp_dbcsr_col_block_sizes(pao%matrix_X)
    IF(.NOT.(SIZE(param_rows)==natoms.AND.SIZE(param_cols)==natoms))CALL cp__a("pao_methods.F",90)
...
! *****************************************************************************
  SUBROUTINE pao_build_orthogonalizer(pao, qs_env)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(arow /= acol) CALL cp__b("pao_methods.F",142,"got non-diagonal block")
       iatom = arow

       CALL cp_dbcsr_get_block_p(matrix=pao%matrix_N_inv, row=arow, col=acol, block=block_N_inv, found=found)
       IF(.NOT.(ASSOCIATED(block_N_inv)))CALL cp__a("pao_methods.F",146)

       CALL cp_dbcsr_get_block_p(matrix=matrix_s(1)%matrix, row=arow, col=acol, block=block_S, found=found)
       IF(.NOT.(ASSOCIATED(block_S)))CALL cp__a("pao_methods.F",149)

       IF(SIZE(block_S,1)/=SIZE(block_S,2)) CALL cp__b("pao_methods.F",151,"Expected symmetric block")
       N = SIZE(block_S,1)
...
! *****************************************************************************
  SUBROUTINE pao_build_selector(qs_env, matrix_Y)
    ...
    IF(nblocks /= SIZE(particle_set, 1)) CALL cp__b("pao_methods.F",214,"nblocks wrong")
    ...
    DO i=1, nblocks
      ...
      IF(pao_basis_size < 1) CALL cp__b("pao_methods.F",220,"PAO_BASIS_SIZE too small.")
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(SIZE(block,2)>SIZE(block,1))&
         CALL cp__b("pao_methods.F",239,"PAO basis size exceeds primary basis size.")
       block = 0.0_dp
       DO i=1, SIZE(block, 2)
          block(i,i) = 1.0_dp
       ENDDO
...
! *****************************************************************************
  SUBROUTINE pao_test_convergence(pao, ls_scf_env, new_energy, is_converged)
    ...
    IF(pao%istep>1) THEN
       ...
       ! IF(energy_diff>0.0_dp) CALL cp__w("pao_methods.F",281,"PAO| energy increased")
...
! *****************************************************************************
  SUBROUTINE pao_check_trace_PS(ls_scf_env)
    ...
    IF(ABS(ls_scf_env%nelectron_total - trace_PS) > 0.5)&
         CALL cp__b("pao_methods.F",418,"Number of electrons wrong. Trace(PS) ="//cp_to_string(trace_PS))
...
! *****************************************************************************
  SUBROUTINE pao_read_preopt_dm(pao, qs_env)
    ...
    IF(dft_control%nspins/=1) CALL cp__b("pao_methods.F",455,"open shell not yet implemented")
...
! *****************************************************************************
  SUBROUTINE pao_dm_trs4(qs_env, ls_scf_env)
    ...
    DO ispin=1, nspin
       ...
       IF(.NOT.converged) CALL cp__b("pao_methods.F",548,"TRS4 did not converge")
    ENDDO
...
! *****************************************************************************
   SUBROUTINE pao_calc_outer_grad_lnv(qs_env,ls_scf_env,matrix_M)
    ...
    IF(nspin /= 1) CALL cp__b("pao_methods.F",598,"open shell not yet implemented")
  ...
  SUBROUTINE pao_check_grad(pao, qs_env, ls_scf_env)
    ...
    DO iatom=1, natoms
       ...
       IF(ASSOCIATED(block_X)) THEN !only one node actually has the block
          ...
          IF(.NOT.(ASSOCIATED(block_G)))CALL cp__a("pao_methods.F",813)
       ENDIF
       ...
       DO i=1, blk_sizes_row(iatom)
          DO j=1, blk_sizes_col(iatom)
             ...
             CASE DEFAULT
                CALL cp__b("pao_methods.F",841,"Unsupported numerical derivative order: "//cp_to_string(pao%num_grad_order))
             END SELECT
    ...
    IF(delta_max > pao%check_grad_tol)  CALL cp_abort(cp__l("pao_methods.F",855),&
