# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/molecule_types_new.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/molecule_types_new.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/molecule_types_new.F" 2
    ...
    IF (ASSOCIATED(gci)) THEN
       ...
       IF (ASSOCIATED(gci%lcolv)) THEN
          DO i=1, SIZE(gci%lcolv)
             ...
             NULLIFY(gci%lcolv(i)%colvar, gci%lcolv(i)%colvar_old)
          END DO
...
! *****************************************************************************
  SUBROUTINE deallocate_molecule_set(molecule_set)
      ...
      DO imolecule=1,SIZE(molecule_set)
         IF (ASSOCIATED(molecule_set(imolecule)%lmi)) THEN
             IF (ASSOCIATED(molecule_set(imolecule)%lmi%states)) THEN
                 DEALLOCATE (molecule_set(imolecule)%lmi%states)
             ENDIF
    ...
    ELSE
      ...
      CALL cp_abort(cp__l("subsys/molecule_types_new.F",274),&
  ...
  SUBROUTINE get_molecule(molecule,molecule_kind,lmi,lci,lg3x3,lg4x6,lcolv,&
    ...
    IF (ASSOCIATED(molecule)) THEN
      ...
      IF ( PRESENT ( lcolv ) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",334,"The pointer lci is not associated")
         ENDIF
      ENDIF
      IF ( PRESENT ( lg3x3 ) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",341,"The pointer lci is not associated")
         ENDIF
      ENDIF
      IF ( PRESENT ( lg4x6 ) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",348,"The pointer lci is not associated")
         ENDIF
      ENDIF

    ELSE

      CALL cp__b("subsys/molecule_types_new.F",354,"The pointer lci is not associated")
  ...
  SUBROUTINE set_molecule(molecule,molecule_kind,lmi,lci,lcolv,lg3x3,lg4x6)
    ...
    IF (ASSOCIATED(molecule)) THEN
      ...
      IF (PRESENT(lcolv) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",400,"The pointer lci is not associated")
         ENDIF
      ENDIF
      IF (PRESENT(lg3x3)) THEN
         IF ( ASSOCIATED ( molecule%lci ) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",407,"The pointer lci is not associated")
         ENDIF
      ENDIF
      IF (PRESENT(lg4x6)) THEN
         IF ( ASSOCIATED ( molecule%lci ) ) THEN
         ...
         ELSE
           CALL cp__b("subsys/molecule_types_new.F",414,"The pointer lci is not associated")
         ENDIF
      ENDIF
    ELSE

      CALL cp__b("subsys/molecule_types_new.F",419,"The pointer molecule is not associated")
...
! *****************************************************************************
  SUBROUTINE set_molecule_set(molecule_set,first_atom,last_atom)
    ...
    IF (ASSOCIATED(molecule_set)) THEN
      ...
      IF (PRESENT(first_atom)) THEN
        ...
        IF (SIZE(first_atom) /= SIZE(molecule_set)) THEN
          CALL cp_abort(cp__l("subsys/molecule_types_new.F",450),&
      ...
      IF (PRESENT(last_atom)) THEN
        ...
        IF (SIZE(last_atom) /= SIZE(molecule_set)) THEN
          CALL cp_abort(cp__l("subsys/molecule_types_new.F",464),&
    ...
    ELSE
      ...
      CALL cp__b("subsys/molecule_types_new.F",477,"The pointer molecule_set is not associated")
...
! *****************************************************************************
  SUBROUTINE get_molecule_set_info(molecule_set,atom_to_mol,mol_to_first_atom,&
    ...
    INTEGER, DIMENSION(:), INTENT(OUT), OPTIONAL :: atom_to_mol, &
      mol_to_first_atom, mol_to_last_atom, mol_to_nelectrons, mol_to_nbasis, &
      mol_to_charge, mol_to_multiplicity

    CHARACTER(len=*), PARAMETER :: routineN = 'get_molecule_set_info', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: first_atom, iatom, imol, &
                                                last_atom, nbasis, nelec
    REAL(KIND=dp)                            :: charge
