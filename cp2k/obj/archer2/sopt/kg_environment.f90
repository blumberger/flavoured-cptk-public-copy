# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_environment.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_environment.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kg_environment.F" 2
...
!> \author Martin Haeufel
! *****************************************************************************
  SUBROUTINE kg_env_create(kg_env, input)
...
!> \author Martin Haeufel
! *****************************************************************************
  SUBROUTINE init_kg_env(kg_env, input)
    TYPE(kg_environment_type), POINTER       :: kg_env
    TYPE(section_vals_type), OPTIONAL, &
      POINTER                                :: input
    ...
    IF (kg_env%tnadd_method == kg_tnadd_embed) THEN
       ...
       IF(ke_sections/=1)&
          CALL cp_abort(cp__l("kg_environment.F",204),&
    ...
    ELSE
       CALL cp__b("kg_environment.F",217,"")
    END IF
  ...
  SUBROUTINE kg_build_neighborlist(qs_env, sab_orb, sac_kin,&
    ...
    IF (PRESENT(subset_of_mol)) THEN
      CALL atom2d_build(atom2d,orb_radius,orb_present,distribution_1d,distribution_2d,&
                     ...
                     dftb=.FALSE.,particle_set=particle_set)
    ELSE
      CALL atom2d_build(atom2d,orb_radius,orb_present,distribution_1d,distribution_2d,&
...
! *****************************************************************************
  SUBROUTINE write_to_file(pairs, nnodes)
    ...
    IF(.NOT.(npairs==SIZE(pairs,2)/2))CALL cp__a("kg_environment.F",475)
