# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_create.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_create.F"
!-----------------------------------------------------------------------------!
...
MODULE qmmm_create
  ...
  USE qmmm_types_low,                  ONLY: &
       add_set_release, add_set_type, add_shell_type, qmmm_env_mm_create, &
       qmmm_env_mm_release, qmmm_env_mm_type, qmmm_env_qm_create, &
       qmmm_env_qm_type, qmmm_links_type
...
# 70 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_create.F" 2
...
! *****************************************************************************
  SUBROUTINE qmmm_env_create(qmmm_env, root_section, para_env, globenv,&
    ...
    INTEGER                                  :: center_i, delta_charge, &
                                                handle, iw, iw2, orig_charge, &
                                                qmmm_coupl_type, use_multipole
    INTEGER, DIMENSION(:), POINTER           :: mm_atom_index, mm_link_atoms, &
                                                qm_atom_index
    LOGICAL :: add_mm_charges, explicit, move_mm_charges, nocompatibility, &
    ...
    ELSE IF (center_i == do_qmmm_center_every_step) THEN
       ...
       qmmm_env_qm%center_qm_subsys0  = .TRUE.
    ELSE
       CALL cp__b("qmmm_create.F",202,"Unknown type of CENTER! ")
    ENDIF
    ...
    IF (qmmm_coupl_type == do_qmmm_none) THEN
       IF(qmmm_env_qm%periodic)&
          CALL cp_warn(cp__l("qmmm_create.F",228),&
    ...
    IF ( iw > 0 ) THEN
       ...
       ENDIF
       WRITE(iw,'(A)')" QM cell ::"
       CALL write_cell(qm_cell_small, subsys_section)
    END IF
    ...
    IF (qmmm_env_qm%image_charge) THEN
       IF(ANY(ABS(abc_mm-abc_qm) > 1.0E-12))&
        CALL cp__b("qmmm_create.F",290,"QM and MM box need to have the same size when using image charges")
    ENDIF
    ...
    CALL qs_init(qs_env, para_env, globenv, root_section, cp_subsys=subsys_qm,&
    ...
    ! assemble the actuall qmmm_env
    ALLOCATE(qmmm_env)
    qmmm_env%qs_env => qs_env
    qmmm_env%fist_env => fist_env
    qmmm_env%qm => qmmm_env_qm

    ! The qmmm_env inherits our ref_cout for qmmm_env_qm, fist_env, qs_env
    ! An expection is qmmm_env_mm, because it's buried in the fist_env
    CALL qmmm_env_mm_release(qmmm_env_mm)
