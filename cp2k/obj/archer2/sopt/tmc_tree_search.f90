# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_search.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_search.F"
!-----------------------------------------------------------------------------!
...
# 29 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_search.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE most_prob_end(global_tree_elem, prob, n_acc, &
    ...
    IF(.NOT.(ASSOCIATED(global_tree_elem)))CALL cp__a("tmc/tmc_tree_search.F",92)
    st_elem => global_tree_elem%conf(global_tree_elem%mv_conf)%elem
    IF(.NOT.(ASSOCIATED(st_elem)))CALL cp__a("tmc/tmc_tree_search.F",94)
    ...
    CASE DEFAULT
      ...
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_tree_search.F",145),&
...
! *****************************************************************************
  SUBROUTINE search_next_energy_calc(gt_head, new_gt_elem, stat, react_count)
    ...
    prob = 0.0_dp
    flag = .FALSE.
    IF(.NOT.(ASSOCIATED(gt_head)))CALL cp__a("tmc/tmc_tree_search.F",243)
...
!> \retval parent parent tree element (last accepted one)
!> \author Mandes 12.2012
...
! *****************************************************************************
  RECURSIVE FUNCTION search_parent_element(current) RESULT(parent)
    ...
    IF(.NOT.(ASSOCIATED(current)))CALL cp__a("tmc/tmc_tree_search.F",290)
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(parent)))CALL cp__a("tmc/tmc_tree_search.F",307)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE search_next_gt_element_to_check(ptr, found)
    ...
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_search.F",328)
    ...
    CASE(status_cancel_nmc, status_cancel_ener, status_canceled_nmc, &
      ...
      ! nothing to do
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_tree_search.F",354),&
    ...
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_search.F",361)
...
! *****************************************************************************
  SUBROUTINE get_subtree_elements_to_check(gt_act_elem, elem1, elem2)
    ...
    IF(.NOT.(ASSOCIATED(gt_act_elem)))CALL cp__a("tmc/tmc_tree_search.F",382)
    ...
    IF(.NOT.(ASSOCIATED(gt_act_elem)))CALL cp__a("tmc/tmc_tree_search.F",410)
    IF(.NOT.(ASSOCIATED(elem1)))CALL cp__a("tmc/tmc_tree_search.F",411)
    IF(.NOT.(ASSOCIATED(elem2)))CALL cp__a("tmc/tmc_tree_search.F",412)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE search_end_of_clean_g_tree(last_acc, tree_ptr)
    ...
    IF(.NOT.(ASSOCIATED(last_acc)))CALL cp__a("tmc/tmc_tree_search.F",431)
    IF(.NOT.(ASSOCIATED(tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",432)
    ...
    CASE(status_calculated, status_calculate_energy, status_created, status_accepted, status_rejected, &
       ...
       ! nothing to do
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_tree_search.F",455),&
           "the global tree element "//cp_to_string(tree_ptr%nr)//&
           " stat "//cp_to_string(tree_ptr%stat)//" is UNknown")
    END SELECT
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(last_acc)))CALL cp__a("tmc/tmc_tree_search.F",461)
    IF(.NOT.(ASSOCIATED(tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",462)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE search_end_of_clean_tree(tree_ptr, last_acc)
    ...
    IF(.NOT.(ASSOCIATED(tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",484)
    IF(.NOT.(ASSOCIATED(last_acc)))CALL cp__a("tmc/tmc_tree_search.F",485)
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",501)
    IF(.NOT.(ASSOCIATED(last_acc)))CALL cp__a("tmc/tmc_tree_search.F",502)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE search_canceling_elements(pt_elem_in, prob, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(pt_elem_in)))CALL cp__a("tmc/tmc_tree_search.F",529)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_search.F",530)
    ...
    IF(ready) THEN
      IF(ASSOCIATED(pt_elem%conf(pt_elem%mv_conf)%elem))THEN
         ...
         CASE DEFAULT
           CALL cp_abort(cp__l("tmc/tmc_tree_search.F",563),&
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(pt_elem_in)))CALL cp__a("tmc/tmc_tree_search.F",581)
...
! *****************************************************************************
  SUBROUTINE count_prepared_nodes_in_trees(global_tree_ptr, counters)
    ...
    IF(.NOT.(ASSOCIATED(global_tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",604)
    IF(.NOT.(ASSOCIATED(counters)))CALL cp__a("tmc/tmc_tree_search.F",605)
    IF(.NOT.(SIZE(counters(1:)).EQ.SIZE(global_tree_ptr%conf(:))))CALL cp__a("tmc/tmc_tree_search.F",606)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE count_prepared_nodes_in_subtree(tree_ptr, counter)
    ...
    IF(.NOT.(ASSOCIATED(tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",641)
    ...
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_tree_search.F",669),&
...
! *****************************************************************************
  SUBROUTINE count_nodes_in_trees(global_tree_ptr, end_of_clean_trees, &
    ...
    IF(.NOT.(ASSOCIATED(global_tree_ptr)))CALL cp__a("tmc/tmc_tree_search.F",701)
    IF(.NOT.(ASSOCIATED(end_of_clean_trees)))CALL cp__a("tmc/tmc_tree_search.F",702)
    IF(.NOT.(ASSOCIATED(counters)))CALL cp__a("tmc/tmc_tree_search.F",703)
    IF(.NOT.(SIZE(counters(1:)).EQ.SIZE(global_tree_ptr%conf(:))))CALL cp__a("tmc/tmc_tree_search.F",704)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE count_nodes_in_global_tree(ptr, counter)
    ...
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_search.F",745)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE count_nodes_in_tree(ptr, counter)
    ...
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_search.F",768)
