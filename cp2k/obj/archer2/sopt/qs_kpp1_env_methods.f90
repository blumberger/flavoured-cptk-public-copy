# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_methods.F"
!-----------------------------------------------------------------------------!
...
# 88 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE kpp1_calc_k_p_p1(kpp1_env, p_env, qs_env, k_p_p1, rho, rho1, rho1_xc)
    ...
    INTEGER                                  :: excitations, handle, ispin, &
                                                ns, nspins, output_unit, &
                                                res_etype
    INTEGER, DIMENSION(2, 3)                 :: bo
    LOGICAL                                  :: explicit, gapw, gapw_xc, &
                                                ionode, lsd, lsd_singlets
    ...
    IF(.NOT.(ASSOCIATED(kpp1_env)))CALL cp__a("qs_kpp1_env_methods.F",191)
    IF(.NOT.(ASSOCIATED(k_p_p1)))CALL cp__a("qs_kpp1_env_methods.F",192)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("qs_kpp1_env_methods.F",193)
    IF(.NOT.(ASSOCIATED(rho1)))CALL cp__a("qs_kpp1_env_methods.F",194)

    IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_kpp1_env_methods.F",196)
    ...
    IF(gapw_xc) THEN
       IF(.NOT.(ASSOCIATED(rho1_xc)))CALL cp__a("qs_kpp1_env_methods.F",208)
    END IF
...
! gets the tmp grids
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_kpp1_env_methods.F",235)
    ...
    DO ispin=1,nspins
       ...
       ! XC uses the sofft basis set only
         ...
         IF (excitations==tddfpt_excitations .AND. nspins==1) THEN
             ...
             CALL cp_dbcsr_set(kpp1_env%v_ao(ispin)%matrix,0.0_dp)
       ...
       CALL cp_dbcsr_copy(k_p_p1(ispin)%matrix,kpp1_env%v_ao(ispin)%matrix)
    END DO
...
! *****************************************************************************
  SUBROUTINE kpp1_calc_k_p_p1_fdiff(qs_env,k_p_p1,rho,rho1,&
    ...
    DO ispin=1,SIZE(k_p_p1)
       CALL cp_dbcsr_add(rho_ao(ispin)%matrix,rho1_ao(ispin)%matrix,&
    ...
    CALL cp_dbcsr_allocate_matrix_set(ks_2,nspins)
    DO ispin=1,nspins
       ALLOCATE(ks_2(ispin)%matrix)
       CALL cp_dbcsr_init(ks_2(ispin)%matrix)
    ...
    DO ispin=1,nspins
       ...
       CALL cp_dbcsr_scale(k_p_p1(ispin)%matrix,alpha_scalar=0.5_dp/my_diff)
    END DO

    CALL cp_dbcsr_deallocate_matrix_set(ks_2)
...
! ------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(kpp1_env)))CALL cp__a("qs_kpp1_env_methods.F",625)
    IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_kpp1_env_methods.F",626)
...
! *****************************************************************************
  SUBROUTINE kpp1_did_change(kpp1_env, s_struct_changed, grid_changed,&
    ...
    IF(.NOT.(ASSOCIATED(kpp1_env)))CALL cp__a("qs_kpp1_env_methods.F",734)
    IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_kpp1_env_methods.F",735)
    IF (my_s_struct_changed) THEN
       IF (ASSOCIATED(kpp1_env%v_ao)) THEN
          CALL cp_dbcsr_deallocate_matrix_set(kpp1_env%v_ao)
       END IF
    END IF
