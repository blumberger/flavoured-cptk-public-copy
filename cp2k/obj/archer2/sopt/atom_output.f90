# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_output.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_output.F"
!-----------------------------------------------------------------------------!
...
MODULE atom_output
  ...
  USE input_constants,                 ONLY: &
       ...
       do_rohf_atom, do_sczoramp_atom, do_uhf_atom, do_uks_atom, &
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_output.F" 2
  ...
  SUBROUTINE atom_print_state(state,iw)
    ...
    IF (state%multiplicity == -1) THEN
      DO l=0,MAX(mlo,mlc)
        ...
        ELSE
           mc = mm(l)
           IF(.NOT.(SUM(state%occ(l,1:mc))==0))CALL cp__a("atom_output.F",127)
  ...
  SUBROUTINE atom_print_basis(atom_basis,iw,title)
    ...
    SELECT CASE (atom_basis%basis_type)
      ...
      CASE (STO_BASIS)
        ...
        WRITE(iw,'(" ",79("*"))')
      CASE (NUM_BASIS)
        CALL cp__b("atom_output.F",351,"")
      CASE DEFAULT
        CALL cp__b("atom_output.F",353,"")
    END SELECT
  ...
  SUBROUTINE atom_print_basis_file(atom_basis)
    ...
    SELECT CASE (atom_basis%basis_type)
      ...
      CASE (STO_BASIS)
        ...
        END IF
      CASE (NUM_BASIS)
        CALL cp__b("atom_output.F",424,"")
      CASE DEFAULT
        CALL cp__b("atom_output.F",426,"")
    END SELECT
