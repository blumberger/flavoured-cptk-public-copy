# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ge_moves.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ge_moves.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ge_moves.F" 2
...
! allocate some stuff
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,172,&
         "subsys_bias",nboxes)
...
! find the molecule bounds
                  ...
                  IF(loverlap)&
                     CALL cp__b("motion/mc/mc_ge_moves.F",260,'Quickstep move found an overlap in the old config')
               ENDIF
...
! allocate some stuff
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,536,&
         "oldsys",2)
      ALLOCATE(particles_old(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,539,&
         "particles_old",2)
...
! check for overlap
         ...
         IF(loverlap) CALL cp_abort(cp__l("motion/mc/mc_ge_moves.F",614),&
         ...
         IF(loverlap) CALL cp_abort(cp__l("motion/mc/mc_ge_moves.F",623),&
...
! allocate some arrays we'll be using
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,687,&
...
! of molecules
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,843,&
...
! allocate an array we'll need
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,852,&
      "r_cbmc",3*ins_atoms*dp_size)
      ALLOCATE(cbmc_energies(1:nswapmoves,1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,855,&
...
! potentials, and calculate the energies
      IF(lbias) THEN
         ...
         IF (istat /= 0) CALL stop_memory(routineN,moduleN,950,&
              "test_env_bias",2)
...
! allocate some stuff
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1260,&
         "r",2*3*max_atoms*dp_size)
      ALLOCATE(oldsys(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1263,&
         "oldsys",2)
      ALLOCATE(particles_old(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1266,&
         "particles_old",2)
      ALLOCATE(cell(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1269,&
         "cell",2)
      ALLOCATE(cell_test(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1272,&
         "cell_test",2)
      ALLOCATE(cell_old(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1275,&
         "cell_old",2)
      ALLOCATE(loverlap(1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1278,&
         "loverlap",2)
...
! now let's grab the cell length and particle positions
      DO ibox=1,2
         ...
         CALL cell_clone(cell(ibox)%cell,cell_old(ibox)%cell)
         CALL cp_subsys_get(oldsys(ibox)%subsys, &
            particles=particles_old(ibox)%list)
...
! add to one box, subtract from the other
      IF(old_cell_length(1,1)*old_cell_length(2,1)*&
         ...
         CALL cp__b("motion/mc/mc_ge_moves.F",1332,'GE_volume moves are trying to make box 1 smaller than 3')
      IF(old_cell_length(1,2)*old_cell_length(2,2)*&
         old_cell_length(3,2)+vol_dis .LE. (3.0E0_dp/angstrom)**3) &
         CALL cp__b("motion/mc/mc_ge_moves.F",1335,'GE_volume moves are trying to make box 2 smaller than 3')
