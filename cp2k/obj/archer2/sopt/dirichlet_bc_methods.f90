# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dirichlet_bc_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dirichlet_bc_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE dirichlet_bc_methods
  ...
  USE dirichlet_bc_types,              ONLY: &
       ...
       dbc_parameters_dealloc, dirichlet_bc_p_type, dirichlet_bc_type, &
       tile_p_type, xy_plane, xz_plane, yz_plane
  USE erf_fn,                          ONLY: erf
  USE kinds,                           ONLY: dp
...
# 48 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dirichlet_bc_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE dirichlet_boundary_region_setup(pw_pool, poisson_params, dbcs)
    ...
    INTEGER :: apx_type, dbc_id, handle, ind_end, ind_start, j, l, &
      n_aa_cuboidal, n_aa_planar, n_dbcs, n_planar, n_xaa_cylindrical, &
      parallel_plane, unit_nr
    INTEGER, DIMENSION(3)                    :: n_prtn, npts
    INTEGER, DIMENSION(:), POINTER           :: xaa_cylindrical_nsides
    LOGICAL                                  :: verbose
    REAL(dp)                                 :: base_radius, delta_alpha, &
                                                sigma, thickness, v_D
    ...
    CASE (MIXED_BC, MIXED_PERIODIC_BC)
      IF (n_dbcs .EQ. 0) THEN 
         CALL cp__b("pw/dirichlet_bc_methods.F",128,"No Dirichlet region is defined.")
      END IF
      ...
      DO j = 1, n_aa_planar
         ...
         CALL aa_planar_dbc_setup(cell_xtnts, parallel_plane, &
                                  ...
                                  thickness, sigma, v_D, dbc_id, verbose, dbcs(j)%dirichlet_bc)

         CALL dirichlet_bc_partition(sigma, n_prtn, pw_pool, cell_xtnts, &
                                  x_locl, y_locl, z_locl, verbose, dbcs(j)%dirichlet_bc)
      END DO
      ...
      DO j = l + 1, l + n_aa_cuboidal
         ...
         CALL aa_cuboidal_dbc_setup(cell_xtnts, &
                                  ...
                                  sigma, v_D, dbc_id, verbose, dbcs(j)%dirichlet_bc)
      ...
      DO j = l + 1, l + n_planar
         ...
         thickness = poisson_params%dbc_params%planar_thickness(j)
      ...
      DO j = 1, n_xaa_cylindrical
         ...
         n_prtn = 1
         n_prtn(1:2) = poisson_params%dbc_params%xaa_cylindrical_nprtn(:,j)
         ...
         thickness = poisson_params%dbc_params%xaa_cylindrical_thickness(j)
         delta_alpha = poisson_params%dbc_params%xaa_cylindrical_sgap(j)
         sigma = poisson_params%dbc_params%xaa_cylindrical_sigma(j)
         ...
         IF (unit_nr .GT. 0) THEN
            ...
            WRITE(unit_nr,'(T3,A,E13.4,2X,A)') "POISSON|     applied potential :", v_D, "[Eh/e]"
         END IF
         CALL xaa_cylindrical_dbc_setup(pw_pool, cell_xtnts, x_locl, y_locl, z_locl, apx_type, &
                                  poisson_params%dbc_params%xaa_cylindrical_xxtnt(:,j), &
                                  base_center, base_radius, thickness, delta_alpha, sigma, v_D, &
                                  n_prtn, verbose, dbcs(ind_start : ind_end))
...
! *****************************************************************************
  SUBROUTINE dirichlet_bc_partition(sigma, n_prtn, pw_pool, cell_xtnts, &
                                    x_locl, y_locl, z_locl, verbose, dirichlet_bc)
    ...
    TYPE(cp_logger_type), POINTER            :: logger
    TYPE(pw_type), POINTER                   :: cylinder_pw
    ...
    CASE (PLANAR)
      ...
      DO k = 1, n_tiles
         ...
         tile_volume = SUM(dirichlet_bc%tiles(k)%tile%tile_pw%cr3d)
         CALL mp_sum(tile_volume, pw_pool%pw_grid%para%group)
    ...
    CASE (CYLINDRICAL)
      ...
      cyl_maxval = MAXVAL(cylinder_pw%cr3d)
      CALL mp_max(cyl_maxval, pw_pool%pw_grid%para%group)
      IF (cyl_maxval .GT. 1.01_dp) THEN
         CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",366),&
...
! *****************************************************************************
  SUBROUTINE aa_planar_dbc_setup(cell_xtnts, parallel_plane, x_xtnt, y_xtnt, z_xtnt, &
                                 thickness, sigma, v_D, dbc_id, verbose, dirichlet_bc)

    REAL(dp), DIMENSION(2, 3), INTENT(IN)    :: cell_xtnts
    INTEGER, INTENT(IN)                      :: parallel_plane
    REAL(dp), DIMENSION(2), INTENT(IN)       :: x_xtnt, y_xtnt, z_xtnt
    REAL(dp), INTENT(IN)                     :: thickness, sigma, v_D
    ...
    IF (n_forb_xtnts .GT. 0) THEN
       CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",491),&
...
! *****************************************************************************
  SUBROUTINE aa_cuboidal_dbc_setup(cell_xtnts, x_xtnt, y_xtnt, z_xtnt, &
                                   sigma, v_D, dbc_id, verbose, dirichlet_bc)

    REAL(dp), DIMENSION(2, 3), INTENT(IN)    :: cell_xtnts
    REAL(dp), DIMENSION(2), INTENT(IN)       :: x_xtnt, y_xtnt, z_xtnt
    REAL(dp), INTENT(IN)                     :: sigma, v_D
    INTEGER, INTENT(IN)                      :: dbc_id
    LOGICAL, INTENT(IN)                      :: verbose
    TYPE(dirichlet_bc_type), INTENT(INOUT), &
      POINTER                                :: dirichlet_bc
    ...
    n_forb_xtnts = COUNT((/forb_xtnt1, forb_xtnt2, forb_xtnt3, &
                           forb_xtnt4, forb_xtnt5, forb_xtnt6/) .EQV. test_forb_xtnts)
    IF (n_forb_xtnts .GT. 0) THEN
       CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",577),&
            "The given extents for the Dirichlet region are outside the range of "//&
            "the simulation cell.")
    END IF 
...
! *****************************************************************************
  SUBROUTINE arbitrary_planar_dbc_setup(cell_xtnts, A, B, C, thickness, &
    ...
    IF (.NOT. are_orthogonal) THEN
       CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",660),"The given vertices for defining a "//&
    ...
    IF (.NOT. (A_is_inside .AND. B_is_inside .AND. C_is_inside .AND. D_is_inside)) THEN
       CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",680),&
    ...
    IF (.NOT. are_coplanar) THEN
       CALL cp__b("pw/dirichlet_bc_methods.F",687,"The given vertices for defining a planar Dirichlet region are not coplanar.")
    END IF
    ...
    IF (.NOT. is_rectangle) THEN
        CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",699),"The given vertices for defining "//&
...
!> \author Mohammad Hossein Bani-Hashemian
! *****************************************************************************
  SUBROUTINE xaa_cylindrical_dbc_setup(pw_pool, cell_xtnts, x_locl, y_locl, z_locl, apx_type, &
                                       x_xtnt, base_center, base_radius, thickness, delta_alpha, &
                                       sigma, v_D, n_prtn, verbose, dbcs)
    ...
    REAL(dp), INTENT(IN)                     :: base_radius, thickness, &
                                                delta_alpha, sigma, v_D
    INTEGER, DIMENSION(3), INTENT(IN)        :: n_prtn
    LOGICAL, INTENT(IN)                      :: verbose
    TYPE(dirichlet_bc_p_type), &
      DIMENSION(:), INTENT(INOUT)            :: dbcs

    CHARACTER(LEN=*), PARAMETER :: routineN = 'xaa_cylindrical_dbc_setup', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: handle, i, intern_apx_type, &
                                                j, n_dbcs, unit_nr
    LOGICAL                                  :: forb_xtnt1, forb_xtnt2, &
                                                forb_xtnt3, forb_xtnt4
    REAL(dp)                                 :: h, Lx, Ly, Lz, theta, xlb, xub
    REAL(dp), DIMENSION(3)                   :: A, B, C, D, normal_vec, &
                                                unit_normal
    TYPE(cp_logger_type), POINTER            :: logger
    ...
    IF(x_xtnt(1) .LT. cell_xtnts(1,1) .OR. x_xtnt(2) .GT. cell_xtnts(2,1)) THEN
       CALL cp_abort(cp__l("pw/dirichlet_bc_methods.F",803),&
    ...
    IF( forb_xtnt1 .OR. forb_xtnt2 .OR. forb_xtnt3 .OR. forb_xtnt4 ) THEN
       CALL cp__b("pw/dirichlet_bc_methods.F",812,"The cylinder does not fit entirely inside the simulation cell.")
    END IF
    ...
    ELSE 
       CALL cp__b("pw/dirichlet_bc_methods.F",830,"Unknown approximation type for cylinder.")
    END IF
    IF( h .GT. MINVAL((/Ly, Lz/)) ) THEN
       CALL cp__b("pw/dirichlet_bc_methods.F",833,"Reduce the base radius!")
    END IF
  ...
  END SUBROUTINE xaa_cylindrical_dbc_setup
...
! *****************************************************************************
  SUBROUTINE rotate_translate_cuboid(cuboid_vtx, Rmat, Tpnt, cuboid_transfd_vtx)
    ...
    INTEGER                                  :: handle
    REAL(dp), DIMENSION(3)                   :: A, A2, AA2, AB, AD, B, B2, C, &
                                                C2, D, D2, e1_locl, e2_locl, &
                                                e3_locl
...
!> \retval vec_transfd transformed vector
!> \par History
!>       11.2015 created [Hossein Bani-Hashemian]
!> \author Mohammad Hossein Bani-Hashemian
! *****************************************************************************
  FUNCTION rotate_translate_vector(Rmat, Tp, direction, vec) RESULT(vec_transfd)
    ...
    IF (direction .EQ. FWROT) THEN
       CALL matvec_3x3(vec_tmp, Rmat, vec)
    ...
    ELSEIF (direction .EQ. BWROT) THEN
       ...
       CALL matvec_3x3(vec_tmp, Rmat_inv, vec)
...
! *****************************************************************************
  SUBROUTINE aa_dbc_partition(dbc_vertices, n_prtn, tiles)
    ...
    REAL(dp)                                 :: step
    REAL(dp), ALLOCATABLE, DIMENSION(:)      :: xprtn_lb, xprtn_ub, yprtn_lb, &
                                                yprtn_ub, zprtn_lb, zprtn_ub
...
! devide the x, y and z extents into n_prtn partitions
...
! *****************************************************************************
  SUBROUTINE aa_tile_pw_compute(cell_xtnts, x_locl, y_locl, z_locl, tile_vertices,&
    ...
    REAL(dp), INTENT(IN)                     :: sigma
    TYPE(pw_type), INTENT(INOUT), POINTER    :: tile_pw
    ...
    LOGICAL                                  :: xleft_ok, xright_ok, &
                                                yleft_ok, yright_ok, &
                                                zleft_ok, zright_ok
    REAL(dp) :: fx, fy, fz, gx, gy, gz, hx, hy, hz, xi0, xil, xir, xleft, &
      xright, yj0, yjl, yjr, yleft, yright, zk0, zkl, zkr, zleft, zright
    REAL(dp), ALLOCATABLE, DIMENSION(:)      :: xlocll, xloclr, ylocll, &
                                                yloclr, zlocll, zloclr
    REAL(dp), DIMENSION(2)                   :: x_xtnt, y_xtnt, z_xtnt
...
! periodicities
    ...
    xright = 0.5_dp*(erf((cell_xtnts(2,1) - x_xtnt(1))/sigma) - erf((cell_xtnts(2,1) - x_xtnt(2))/sigma))
    xleft  = 0.5_dp*(erf((cell_xtnts(1,1) - x_xtnt(1))/sigma) - erf((cell_xtnts(1,1) - x_xtnt(2))/sigma))
    yright = 0.5_dp*(erf((cell_xtnts(2,2) - y_xtnt(1))/sigma) - erf((cell_xtnts(2,2) - y_xtnt(2))/sigma))
    yleft  = 0.5_dp*(erf((cell_xtnts(1,2) - y_xtnt(1))/sigma) - erf((cell_xtnts(1,2) - y_xtnt(2))/sigma))
    zright = 0.5_dp*(erf((cell_xtnts(2,3) - z_xtnt(1))/sigma) - erf((cell_xtnts(2,3) - z_xtnt(2))/sigma))
    zleft  = 0.5_dp*(erf((cell_xtnts(1,3) - z_xtnt(1))/sigma) - erf((cell_xtnts(1,3) - z_xtnt(2))/sigma))

    xright_ok = .FALSE.;      xleft_ok = .FALSE.
    yright_ok = .FALSE.;      yleft_ok = .FALSE.
    zright_ok = .FALSE.;      zleft_ok = .FALSE.

    xright_ok = xright .LT. 0.5_dp
    yright_ok = yright .LT. 0.5_dp
    zright_ok = zright .LT. 0.5_dp
    xleft_ok  = xleft  .LT. 0.5_dp
    yleft_ok  = yleft  .LT. 0.5_dp
    zleft_ok  = zleft  .LT. 0.5_dp

! prefactor: columns: left, original, right - rows: x , y, z directions
    prefactor = 0.5_dp 

    IF (.NOT. xright_ok) prefactor(1,1) = 0.0_dp
    IF (.NOT. yright_ok) prefactor(2,1) = 0.0_dp
    IF (.NOT. zright_ok) prefactor(3,1) = 0.0_dp
    IF (.NOT. xleft_ok ) prefactor(1,3) = 0.0_dp
    IF (.NOT. yleft_ok ) prefactor(2,3) = 0.0_dp
    IF (.NOT. zleft_ok ) prefactor(3,3) = 0.0_dp
...
! periodically replicated cell on the right, points local to the current processor
          ...
          tile_pw%cr3d(i,j,k) = (fx*fy*fz) + (gx*gy*gz) + (hx*hy*hz)
        END DO
      END DO
    END DO
...
! *****************************************************************************
  SUBROUTINE arbitrary_tile_pw_compute(cell_xtnts, x_locl, y_locl, z_locl, tile_vertices, &
    ...
    LOGICAL                                  :: xleft_ok, xright_ok, &
                                                yleft_ok, yright_ok, &
                                                zleft_ok, zright_ok
    REAL(dp) :: cm_x, cm_y, cm_z, fx, fy, fz, gx, gy, gz, hx, hy, hz, xi0, &
      xil, xir, xleft, xright, yj0, yjl, yjr, yleft, yright, zk0, zkl, zkr, &
      zleft, zright
    REAL(dp), ALLOCATABLE, DIMENSION(:)      :: xlocll, xloclr, ylocll, &
                                                yloclr, zlocll, zloclr
    REAL(dp), DIMENSION(2)                   :: transfdx_xtnt, transfdy_xtnt, &
                                                transfdz_xtnt, x_xtnt, &
                                                y_xtnt, z_xtnt
    REAL(dp), DIMENSION(3)                   :: per, pnt0, pntl, pntr, Tpnt
    ...
    DO k = lb3, ub3
      DO j = lb2, ub2
        DO i = lb1, ub1
          ...
          pntl = rotate_translate_vector(Rmat, Tpnt, BWROT, (/x_locl(i)-per(1), y_locl(j)-per(2), z_locl(k)-per(3)/))
          pntr = rotate_translate_vector(Rmat, Tpnt, BWROT, (/x_locl(i)+per(1), y_locl(j)+per(2), z_locl(k)+per(3)/))

          xi0 = pnt0(1);    yj0 = pnt0(2);    zk0 = pnt0(3)
          xil = pntl(1);    yjl = pntl(2);    zkl = pntl(3)
          xir = pntr(1);    yjr = pntr(2);    zkr = pntr(3)
