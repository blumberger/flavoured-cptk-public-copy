# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_optimizer.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_optimizer.F"
!-----------------------------------------------------------------------------!
...
# 87 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_optimizer.F" 2
      ...
      IF (.NOT.prepare_to_exit) THEN ! update the ALMOs and density matrix
         ...
         CASE (almo_scf_dm_sign)
            ...
            DO ispin=1,nspin
               ...
               CALL cp__b("almo_scf_optimizer.F",288,"Density_matrix_sign has not been tested yet")
...
! *****************************************************************************
  SUBROUTINE almo_scf_xalmo_eigensolver(qs_env,almo_scf_env,optimizer)
    ...
    IF (.NOT.converged)  THEN
      CALL cp__b("almo_scf_optimizer.F",717,"SCF for ALMOs on overlapping domains not converged! ")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE almo_scf_xalmo_pcg(qs_env,almo_scf_env,optimizer,quench_t,&
    ...
    IF (almo_scf_env%nspins.gt.1) THEN
       CALL cp__b("almo_scf_optimizer.F",861,"UNRESTRICTED ALMO SCF IS NYI(!)")
    ENDIF
    ...
    DO ispin=1,almo_scf_env%nspins
       ...
       ELSE
          IF (optimize_theta) THEN
             ...
             IF (grad_norm.gt.almo_scf_env%envelope_amplitude) THEN
                CALL cp__b("almo_scf_optimizer.F",991,"Max norm of the initial guess is too large")
             ENDIF
          ...
          DO
             ...
             IF (assume_t0_q0x.AND.(.NOT.just_started)) THEN
                ...
                ELSE IF (my_special_case.eq.xalmo_case_block_diag) THEN
                   ...
                   CALL cp__b("almo_scf_optimizer.F",1120,"")
                ELSE
                   ! no special case
             ...
             IF (.NOT.skip_grad) THEN
                ...
                IF (assume_t0_q0x) THEN
                   ...
                   ELSE IF (my_special_case.eq.xalmo_case_block_diag) THEN
                      ...
                      CALL cp__b("almo_scf_optimizer.F",1383,"")
                   ELSE
                   ...
                   IF (.NOT.line_search) THEN
                      ...
                      IF (use_preconditioner) THEN
                         ...
                         CASE (1)
                            ...
                            IF (ncores.gt.1) THEN                            
                               CALL cp__b("almo_scf_optimizer.F",1765,"serial code only")
                            ENDIF
                         ...
                         CASE (4)
                            ...
                            ELSE
                               ...
                               IF (optimize_theta) THEN
                                  CALL cp__b("almo_scf_optimizer.F",1864,"theta is NYI")
                               ENDIF
                      ...
                      IF (.NOT.reset_conjugator) THEN
                         ...
                         CASE (cg_zero)
                            beta=0.0_dp
                         CASE DEFAULT
                            CALL cp__b("almo_scf_optimizer.F",1979,"illegal conjugator")
                         END SELECT
       ...
       IF (.NOT.converged)  THEN
          CALL cp__b("almo_scf_optimizer.F",2280,"Optimization not converged! ")
       ENDIF
    ...
    DO ispin=1,almo_scf_env%nspins
       ...
       DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          IF (iblock_row.ne.iblock_col) THEN
             CALL cp__b("almo_scf_optimizer.F",2328,"off-diagonal block found")
          ENDIF

          retained_v=almo_scf_env%nvirt_of_domain(iblock_col,ispin)
          discarded_v=almo_scf_env%nvirt_disc_of_domain(iblock_col,ispin)
          IF(.NOT.(retained_v.gt.0))CALL cp__a("almo_scf_optimizer.F",2333)
          IF(.NOT.(discarded_v.gt.0))CALL cp__a("almo_scf_optimizer.F",2334)

          NULLIFY (p_new_block)
          CALL cp_dbcsr_reserve_block2d(almo_scf_env%matrix_v_disc_blk(ispin),&
                  iblock_row,iblock_col,p_new_block)
          IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_optimizer.F",2339)
          IF(.NOT.(retained_v+discarded_v.eq.iblock_col_size))CALL cp__a("almo_scf_optimizer.F",2340)
          p_new_block(:,:) = data_p(:,(retained_v+1):iblock_col_size)

          NULLIFY (p_new_block)
          CALL cp_dbcsr_reserve_block2d(almo_scf_env%matrix_v_blk(ispin),&
                  iblock_row,iblock_col,p_new_block)
          IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_optimizer.F",2346)
...
!INTEGER             :: ppp
                      ...
                      IF (.NOT.prepare_to_exit) THEN
                         ...
                         IF (.NOT.line_search) THEN
                            ...
                            IF (.NOT.reset_conjugator) THEN
                               ...
                               CASE (cg_zero)
                                  beta=0.0_dp
                               CASE DEFAULT
                                  CALL cp__b("almo_scf_optimizer.F",3296,"illegal conjugator")
                               END SELECT
                         ...
                         IF (line_search) THEN
                            ...
                            IF (ABS(fun1-fun0-gfun0*step_size).lt.num_threshold) THEN
                               ...
                               ENDIF
                               CALL cp__b("almo_scf_optimizer.F",3360,"")
                            ENDIF
                            IF (ABS(gfun0).lt.num_threshold) THEN
                               IF (unit_nr>0) THEN
                                  WRITE(unit_nr,'(T3,A,1X,E17.7,A,1X,E12.3)') "Linear gradient",&
                                     ...
                                     " is smaller than the threshold",num_threshold
                               ENDIF
                               CALL cp__b("almo_scf_optimizer.F",3368,"")
                            ENDIF
                            ...
                            IF (step_size.lt.0.0_dp) THEN
                               CALL cp__b("almo_scf_optimizer.F",3477,"Negative step proposed")
                            ENDIF
    ...
    CASE(dm_ls_step)
       ...
       DO ispin=1,nspin
          ...
          ! RZK UPDATE! it requires updating core LS_SCF routines
          ! RZK UPDATE! (the code exists in the CVS version)
          CALL cp__b("almo_scf_optimizer.F",3969,"CVS only: density_matrix_sign has not been updated in SVN")
