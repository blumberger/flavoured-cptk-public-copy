# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/external_potential_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/external_potential_types.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/external_potential_types.F" 2
...
! *****************************************************************************
  SUBROUTINE deallocate_all_potential(potential)
    ...
    ELSE
       CALL cp__b("subsys/external_potential_types.F",380,"The pointer potential is not associated.")
    END IF
...
! *****************************************************************************
  SUBROUTINE deallocate_fist_potential(potential)
    ...
    IF (ASSOCIATED(potential)) THEN
       ...
       DEALLOCATE (potential)
    ELSE
       CALL cp__b("subsys/external_potential_types.F",401,"The pointer potential is not associated.")
    END IF
...
! *****************************************************************************
  SUBROUTINE deallocate_local_potential(potential)
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",432,"The pointer potential is not associated.")
...
! *****************************************************************************
  SUBROUTINE deallocate_gth_potential(potential)
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",491,"The pointer potential is not associated.")
    END IF
...
! *****************************************************************************
  SUBROUTINE get_all_potential(potential,name,alpha_core_charge,&
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",542,"The pointer potential is not associated.")
...
! *****************************************************************************
  SUBROUTINE get_fist_potential(potential, name, apol, cpol, mm_radius, qeff, &
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",585,"The pointer potential is not associated.")
...
! *****************************************************************************
  SUBROUTINE get_local_potential(potential,name,ngau,npol,alpha,cval,radius)
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",629,"The pointer potential is not associated.")
...
! *****************************************************************************
  SUBROUTINE get_gth_potential(potential,name,alpha_core_charge,&
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",788,"The pointer potential is not associated.")
...
! *****************************************************************************
  SUBROUTINE read_all_potential(element_symbol,potential_name,potential,zeff_correction,&
       para_env, dft_section, potential_section)
    ...
    search_loop: DO
       ...
       IF (found) THEN
          ...
          IF (match) THEN
             ...
             IF (read_from_input) THEN
                ...
                IF (.NOT.is_ok)&
                   CALL cp__b("subsys/external_potential_types.F",1129,"Error reading the Potential from input file!!")
             ...
             IF (read_from_input) THEN
                is_ok=cp_sll_val_next(list,val)
                IF (.NOT.is_ok) CALL cp__b("subsys/external_potential_types.F",1165,"Error reading the Potential from input file!!")
                CALL val_get(val,c_val=line_att)
                READ(line_att,*)r
             ELSE
       ...
       ELSE
          ...
          CALL cp_abort(cp__l("subsys/external_potential_types.F",1184),&
    ...
    search_loop: DO
       ...
       IF (found) THEN
          ...
          IF (match) THEN
             ...
             IF (read_from_input) THEN
                is_ok=cp_sll_val_next(list,val)
                IF (.NOT.is_ok) CALL cp__b("subsys/external_potential_types.F",1303,"Error reading the Potential from input file!!")
                CALL val_get(val,c_val=line_att)
             ...
             DO igau=1,ngau
                IF (read_from_input) THEN
                   is_ok=cp_sll_val_next(list,val)
                   IF (.NOT.is_ok)&
                      CALL cp__b("subsys/external_potential_types.F",1322,"Error reading the Potential from input file!!")
                   CALL val_get(val,c_val=line_att)
       ...
       ELSE
          ! Stop program, if the end of file is reached
          CALL cp_abort(cp__l("subsys/external_potential_types.F",1357),&
...
! *****************************************************************************
  SUBROUTINE read_gth_potential(element_symbol,potential_name,potential,zeff_correction,&
    ...
    INTEGER :: i, ic, ipot, irep, istr, j, l, lppnl, lprj_ppnl_max, maxlppl, &
      n, nppnl, nprj_ppnl, nprj_ppnl_max, strlen1, strlen2
    INTEGER, DIMENSION(:), POINTER           :: elec_conf
    LOGICAL                                  :: found, is_ok, match, &
                                                read_from_input
    ...
    search_loop: DO
       ...
       IF (found) THEN
          ...
          IF (match) THEN
             ...
             IF (read_from_input) THEN
                is_ok=cp_sll_val_next(list,val)
                IF (.NOT.is_ok) CALL cp__b("subsys/external_potential_types.F",1498,"Error reading the Potential from input file!!")
                CALL val_get(val,c_val=line_att)
                READ(line_att,*)elec_conf(l)
                CALL remove_word(line_att)
                DO WHILE (LEN_TRIM(line_att) /= 0)
             ...
             IF (read_from_input) THEN
                is_ok=cp_sll_val_next(list,val)
                IF (.NOT.is_ok) CALL cp__b("subsys/external_potential_types.F",1534,"Error reading the Potential from input file!!")
                CALL val_get(val,c_val=line_att)
                READ(line_att,*)r
                CALL remove_word(line_att)
             ELSE
             ...
             ELSE
                IF (LEN_TRIM(line_att)/= 0) THEN
                   CALL cp__b("subsys/external_potential_types.F",1586,"Error reading the Potential from input file!!")
                END IF
             END IF
             maxlppl = 2*(n - 1)
             ...
             IF (read_from_input) THEN
               DO
                 is_ok=cp_sll_val_next(list,val)
                 IF(.NOT.(is_ok))CALL cp__a("subsys/external_potential_types.F",1598)
                 CALL val_get(val,c_val=line_att)
                 IF(INDEX(line_att,"LPOT") /= 0) THEN
                   ...
                   DO ipot=1,potential%nexp_lpot
                      is_ok=cp_sll_val_next(list,val)
                      IF(.NOT.(is_ok))CALL cp__a("subsys/external_potential_types.F",1613)
                      CALL val_get(val,c_val=line_att)
                      READ(line_att,*) r
                 ...
                 ELSEIF(INDEX(line_att,"NLCC") /= 0) THEN
                   ...
                   DO ipot=1,potential%nexp_nlcc
                      is_ok=cp_sll_val_next(list,val)
                      IF(.NOT.(is_ok))CALL cp__a("subsys/external_potential_types.F",1638)
                      CALL val_get(val,c_val=line_att)
                 ...
                 ELSEIF(INDEX(line_att,"LSD") /= 0) THEN
                   ...
                   DO ipot=1,potential%nexp_lsd
                      is_ok=cp_sll_val_next(list,val)
                      IF(.NOT.(is_ok))CALL cp__a("subsys/external_potential_types.F",1662)
                      CALL val_get(val,c_val=line_att)
                      READ(line_att,*) r
               ...
               DO
                 ...
                 ELSEIF(parser_test_next_token(parser) == "STR") THEN
                   ...
                   ELSEIF(INDEX(LINE,"LSD") /= 0) THEN
                     ...
                     END DO
                   ELSE
                     CALL cp__b("subsys/external_potential_types.F",1783,"")
                   END IF
                 ELSE
                   CALL cp__b("subsys/external_potential_types.F",1786,"")
                 END IF
               END DO
             END IF
             ...
             IF (n > 0) THEN
                ...
                DO l=0,lppnl
                   IF (read_from_input) THEN
                      is_ok=cp_sll_val_next(list,val)
                      IF (.NOT.is_ok)&
                         CALL cp__b("subsys/external_potential_types.F",1829,"Error reading the Potential from input file!!")
                      CALL val_get(val,c_val=line_att)
                      READ(line_att,*)r
                      CALL remove_word(line_att)
                   ...
                   IF (r==0.0_dp.AND.nprj_ppnl/=0) THEN
                      CALL cp_abort(cp__l("subsys/external_potential_types.F",1843),&
                      ...
                      ELSE
                         IF (read_from_input) THEN
                            IF (LEN_TRIM(line_att)/=0)&
                               CALL cp__b("subsys/external_potential_types.F",1872,"Error reading the Potential from input file!!")
                            is_ok=cp_sll_val_next(list,val)
                            IF (.NOT.is_ok)&
                               CALL cp__b("subsys/external_potential_types.F",1875,"Error reading the Potential from input file!!")
                            CALL val_get(val,c_val=line_att)
                            READ(line_att,*)hprj_ppnl(i,i,l)
                            CALL remove_word(line_att)
                         ELSE
                   ...
                   ELSE
                      IF (LEN_TRIM(line_att) /= 0) THEN
                         CALL cp__b("subsys/external_potential_types.F",1907,"Error reading the Potential from input file!!")
                      END IF
                   END IF
                   IF (nprj_ppnl > 1) THEN
       ...
       ELSE
          ! Stop program, if the end of file is reached
          CALL cp_abort(cp__l("subsys/external_potential_types.F",1936),&
               "The requested atomic potential <"//&
               TRIM(potential_name)//&
               "> for element <"//&
               TRIM(symbol)//&
...
! *****************************************************************************
  SUBROUTINE set_all_potential(potential,name,alpha_core_charge,&
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",2066,"The pointer potential is not associated")
...
! *****************************************************************************
  SUBROUTINE set_local_potential(potential,name,alpha,cval,radius)
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",2106,"The pointer potential is not associated")
...
! *****************************************************************************
  SUBROUTINE set_fist_potential(potential, apol, cpol, qeff, mm_radius, &
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",2145,"The pointer potential is not associated")
...
! *****************************************************************************
  SUBROUTINE set_gth_potential(potential,name,alpha_core_charge,alpha_ppl,&
    ...
    ELSE
       ...
       CALL cp__b("subsys/external_potential_types.F",2239,"The pointer potential is not associated")
