# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_parsing.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_parsing.F"
!-----------------------------------------------------------------------------!
...
MODULE input_parsing
  ...
  USE input_section_types,             ONLY: &
       ...
       section_get_subsection_index, section_type, section_typo_match, &
       section_vals_add_values, section_vals_type, typo_match_section, &
       typo_matching_line, typo_matching_rank
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_parsing.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE section_vals_parse(section_vals,parser,default_units,root_section)
    ...
    INTEGER                                  :: desc_level, handle, ik, &
                                                imatch, irs, is, nsub, &
                                                output_unit
    LOGICAL                                  :: at_end, compatible_end, &
                                                root_sect, whole_section
    ...
    IF(.NOT.(ASSOCIATED(section_vals)))CALL cp__a("input/input_parsing.F",109)
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/input_parsing.F",110)
    output_unit= cp_logger_get_default_io_unit()

    IF(.NOT.(section_vals%ref_count>0))CALL cp__a("input/input_parsing.F",113)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/input_parsing.F",114)
    IF (root_sect.and.parser%icol1>parser%icol2) &
         CALL cp_abort(cp__l("input/input_parsing.F",116),&
    ...
    IF (root_sect) THEN
       ...
       IF (token/=parser%section_character//section%name)&
            CALL cp_abort(cp__l("input/input_parsing.F",125),&
    ...
    IF(.NOT.section%repeats.AND.SIZE(section_vals%values,2)/=0)&
       CALL cp_abort(cp__l("input/input_parsing.F",130), "Section "//TRIM(section%name)//&
    ...
    DO WHILE (.TRUE.)
       ...
       IF (at_end) THEN
          IF(root_sect)&
             CALL cp_abort(cp__l("input/input_parsing.F",154),&
       ...
       IF (token(1:1)==parser%section_character) THEN
          IF (token=="&END") THEN
             ...
             IF (.NOT.compatible_end) THEN
                CALL cp_abort(cp__l("input/input_parsing.F",179),&
          ...
          ELSE
             ...
             IF (output_unit>0) THEN
                ...
                ENDDO
             ENDIF
             CALL cp_abort(cp__l("input/input_parsing.F",204),&
          ...
          IF (token=="DESCRIBE") THEN
             ...
             DO WHILE (parser_test_next_token(parser)=="STR")
                ...
                IF (.NOT.ASSOCIATED(keyword)) THEN
                   CALL cp_warn(cp__l("input/input_parsing.F",234),&
             ...
             IF (ik<1) THEN ! don't accept pseudo keyword names
                ...
                IF (.NOT.ASSOCIATED(section%keywords(0)%keyword)) THEN
                   IF (output_unit>0) THEN
                      ...
                      ENDDO
                   ENDIF
                   CALL cp_abort(cp__l("input/input_parsing.F",264),&
                ...
                IF (ASSOCIATED(el)) THEN
                   ...
                   ELSE
                      IF(.NOT.keyword%repeats)&
                         CALL cp_abort(cp__l("input/input_parsing.F",286),&
                              "Keyword "//TRIM(token)//&
...
! *****************************************************************************
  SUBROUTINE val_create_parsing(val,type_of_var, n_var,enum,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(val)))CALL cp__a("input/input_parsing.F",351)
    ...
    CASE (logical_t)
       ...
       IF (parser_test_next_token(parser)=="EOL") THEN
          IF (.NOT.ASSOCIATED(default_value)) THEN
             ...
             ELSE
                CALL cp_abort(cp__l("input/input_parsing.F",362),&
                     "no value was given and there is no default value"//&
                     TRIM(parser_location(parser)))
             END IF
          ELSE
             IF(.NOT.(ASSOCIATED(default_value%l_val)))CALL cp__a("input/input_parsing.F",367)
    ...
    CASE (integer_t)
       ...
       IF (parser_test_next_token(parser)=="EOL") THEN
          IF (.NOT.ASSOCIATED(default_value)) THEN
             ...
             ELSE
                CALL cp_abort(cp__l("input/input_parsing.F",402),&
                     "no value was given and there is no default value"//&
                     TRIM(parser_location(parser)))
             END IF
          ELSE
             check = ASSOCIATED(default_value%i_val)
             IF(.NOT.(check))CALL cp__a("input/input_parsing.F",408)
    ...
    CASE (real_t)
       ...
       IF (parser_test_next_token(parser)=="EOL") THEN
          IF (.NOT.ASSOCIATED(default_value)) THEN
             ...
             ELSE
                CALL cp_abort(cp__l("input/input_parsing.F",443),&
                     "no value was given and there is no default value"//&
                     TRIM(parser_location(parser)))
             END IF
          ELSE
             IF(.NOT.(ASSOCIATED(default_value%r_val)))CALL cp__a("input/input_parsing.F",448)
    ...
    CASE (char_t)
       ...
       IF (parser_test_next_token(parser)=="EOL") THEN
          ...
          ELSE
             IF (.NOT.ASSOCIATED(default_value)) THEN
                CALL cp_abort(cp__l("input/input_parsing.F",487),&
                     "no value was given and there is no default value"//&
                     TRIM(parser_location(parser)))
             ELSE
                IF(.NOT.(ASSOCIATED(default_value%c_val)))CALL cp__a("input/input_parsing.F",491)
                CALL val_create(val,c_vals=default_value%c_val)
             END IF
          END IF
       ELSE
          IF (n_var<1) THEN
             IF(.NOT.(n_var==-1))CALL cp__a("input/input_parsing.F",497)
    ...
    CASE (lchar_t)
       ...
       ELSE
          ...
          CALL cp_dealloc(c_first)
       END IF
       IF(.NOT.(ASSOCIATED(c_val_p)))CALL cp__a("input/input_parsing.F",548)
       CALL val_create(val,lc_vals_ptr=c_val_p)
    CASE (enum_t)
       IF(.NOT.(ASSOCIATED(enum)))CALL cp__a("input/input_parsing.F",551)
       NULLIFY(i_val_p)
       IF (parser_test_next_token(parser)=="EOL") THEN
          IF (.NOT.ASSOCIATED(default_value)) THEN
             IF (n_var<1) THEN
                ...
                CALL val_create(val,i_vals_ptr=i_val_p)
             ELSE
                CALL cp_abort(cp__l("input/input_parsing.F",559),&
                     "no value was given and there is no default value"//&
                     TRIM(parser_location(parser)))
             END IF
          ELSE
             IF(.NOT.(ASSOCIATED(default_value%i_val)))CALL cp__a("input/input_parsing.F",564)
             CALL val_create(val,i_vals=default_value%i_val,&
       ...
       END IF
    CASE default
       CALL cp_abort(cp__l("input/input_parsing.F",594),&
    ...
    IF (parser_test_next_token(parser).NE."EOL") THEN
       ...
       CALL cp_abort(cp__l("input/input_parsing.F",600),&
  ...
  SUBROUTINE get_r_val(r_val, parser, unit, default_units, c_val)
    ...
    IF (ASSOCIATED(unit)) THEN
       IF ('STR'==parser_test_next_token(parser)) THEN
          CALL parser_get_object(parser,c_val)
          check = c_val(1:1)=="["
          IF(.NOT.(check))CALL cp__a("input/input_parsing.F",636)
          check = c_val(LEN_TRIM(c_val):LEN_TRIM(c_val))=="]"
          IF(.NOT.(check))CALL cp__a("input/input_parsing.F",638)
       ...
       IF(.NOT.cp_unit_compatible(unit,my_unit))&
          CALL cp_abort(cp__l("input/input_parsing.F",648),&
