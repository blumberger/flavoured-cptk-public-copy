# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_wang.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_wang.F"
!-----------------------------------------------------------------------------!
...
# 33 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_wang.F" 2
...
! *****************************************************************************
  SUBROUTINE perdew_wang_info ( method, lsd, reference, shortform, needs, &
    ...
    SELECT CASE (method)
    CASE DEFAULT
       CALL cp__b("xc/xc_perdew_wang.F",78,"")
    CASE (pw_orig)
       p_string='PWO'
    CASE (pw_dmc)
       p_string='DMC'
...
! *****************************************************************************
  SUBROUTINE perdew_wang_init(method, cutoff)
    ...
    CASE DEFAULT
       CALL cp__b("xc/xc_perdew_wang.F",157,"Unknown method")
...
! *****************************************************************************
  SUBROUTINE perdew_wang_lda_eval( method, rho_set, deriv_set, order, scale)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_perdew_wang.F",235)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_perdew_wang.F",236)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_perdew_wang.F",237)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_perdew_wang.F",238)
    ...
    END IF
    IF (order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_perdew_wang.F",273,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE perdew_wang_lsd_eval( method, rho_set, deriv_set, order, scale)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_perdew_wang.F",374)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_perdew_wang.F",375)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_perdew_wang.F",376)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_perdew_wang.F",377)
    ...
    END IF
    IF (order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_perdew_wang.F",432,"derivatives bigger than 3 not implemented")
    END IF
