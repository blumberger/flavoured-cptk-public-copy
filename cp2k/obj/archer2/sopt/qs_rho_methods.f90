# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_methods.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_rho_rebuild(rho, qs_env, rebuild_ao, rebuild_grids, admm)
    ...
    TYPE(pw_p_type), DIMENSION(:), POINTER   :: drho_g, drho_r, rho_g, rho_r, &
                                                tau_g, tau_r
    ...
    IF (my_admm) THEN
       IF(.NOT.(.NOT.do_kpoints))CALL cp__a("qs_rho_methods.F",138)
    ...
    IF (.NOT.ASSOCIATED(rho)) CALL cp__b("qs_rho_methods.F",149,"rho not associated")
    CALL qs_rho_get(rho,&
                    tot_rho_r=tot_rho_r,&
                    rho_ao_kp=rho_ao_kp,&
                    rho_r=rho_r,&
    ...
    IF (my_rebuild_ao.OR.(.NOT.ASSOCIATED(rho_ao_kp))) THEN
       IF (ASSOCIATED(rho_ao_kp)) &
          CALL cp_dbcsr_deallocate_matrix_set(rho_ao_kp)
       ! Create a new density matrix set
       CALL cp_dbcsr_allocate_matrix_set(rho_ao_kp,nspins,nimg)
       ...
       DO i=1,nspins
          DO ic=1,nimg
             ...
             CALL cp_dbcsr_set(tmatrix,0.0_dp)
          END DO
       END DO
    END IF
...
!> \author Fawzi Mohamed
! *****************************************************************************
SUBROUTINE qs_rho_update_rho(rho_struct, qs_env, local_rho_set, task_list_external)
    ...
    TYPE(pw_p_type), DIMENSION(:), POINTER   :: drho_g, drho_r, drho_xc_g, &
                                                rho_g, rho_r, rho_xc_g, &
                                                rho_xc_r, tau_g, tau_r, &
                                                tau_xc_g, tau_xc_r
    TYPE(pw_p_type), POINTER                 :: rho_r_sccs
    ...
    IF(.NOT.(ASSOCIATED(rho_struct)))CALL cp__a("qs_rho_methods.F",372)
    ...
    ELSEIF(dft_control%qs_control%lrigpw) THEN
      ...
      CALL calculate_lri_densities(lri_env,lri_density,qs_env,rho_ao,&
    ...
    IF (dft_control%drho_by_collocation) THEN
       CALL qs_rho_get(rho_struct, rho_ao_kp=rho_ao_kp)
       DO ispin=1,nspins
         IF(.NOT.(.NOT.PRESENT(task_list_external)))CALL cp__a("qs_rho_methods.F",434)
         rho_ao => rho_ao_kp(ispin,:)
    ...
    IF (dft_control%use_kinetic_energy_density) THEN
       IF(dft_control%qs_control%lrigpw) THEN
          CALL cp__b("qs_rho_methods.F",447,"LRIGPW not implemented for meta-GGAs")
       ENDIF
       CALL qs_rho_get(rho_struct, rho_ao_kp=rho_ao_kp)
       DO ispin=1,nspins
          IF(.NOT.(.NOT.PRESENT(task_list_external)))CALL cp__a("qs_rho_methods.F",451)
          rho_ao => rho_ao_kp(ispin,:)
          CALL calculate_rho_elec(matrix_p_kp=rho_ao,&
               rho=tau_r(ispin),&
   ...
   ! GAPW o GAPW_XC require the calculation of hard and soft local densities
    IF(gapw ) THEN
       IF(.NOT.(.NOT.PRESENT(task_list_external)))CALL cp__a("qs_rho_methods.F",465)
       CALL qs_rho_get(rho_struct, rho_ao_kp=rho_ao_kp)
    ...
    ENDIF
    IF(gapw_xc) THEN
       IF(.NOT.(.NOT.PRESENT(task_list_external)))CALL cp__a("qs_rho_methods.F",474)
       ...
       DO ispin=1,nspins
          DO img=1,nimg
             CALL cp_dbcsr_copy(rho_xc_ao(ispin,img)%matrix,rho_ao_kp(ispin,img)%matrix)
          END DO
       END DO
       DO ispin=1,nspins
...
! *****************************************************************************
  SUBROUTINE duplicate_rho_type(rho_input, rho_output, qs_env)
    ...
    INTEGER                                  :: handle, i, nspins, &
                                                rebuild_each_in
    LOGICAL :: drho_g_valid_in, drho_r_valid_in, rho_g_valid_in, &
    ...
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
      POINTER                                :: rho_ao_in, rho_ao_out
    TYPE(dft_control_type), POINTER          :: dft_control
    TYPE(pw_env_type), POINTER               :: pw_env
    TYPE(pw_p_type), DIMENSION(:), POINTER :: drho_g_in, drho_g_out, &
      drho_r_in, drho_r_out, rho_g_in, rho_g_out, rho_r_in, rho_r_out, &
      tau_g_in, tau_g_out, tau_r_in, tau_r_out
    ...
    IF(.NOT.(ASSOCIATED(rho_input)))CALL cp__a("qs_rho_methods.F",575)
    IF(.NOT.(ASSOCIATED(rho_output)))CALL cp__a("qs_rho_methods.F",576)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_rho_methods.F",577)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_rho_methods.F",578)
    ...
    IF (ASSOCIATED(rho_ao_in)) THEN
      CALL cp_dbcsr_allocate_matrix_set(rho_ao_out, nspins)
      CALL qs_rho_set(rho_output, rho_ao=rho_ao_out)
      DO i = 1, nspins
         ALLOCATE(rho_ao_out(i)%matrix)
         CALL cp_dbcsr_init(rho_ao_out(i)%matrix)
         CALL cp_dbcsr_copy(rho_ao_out(i)%matrix,rho_ao_in(i)%matrix, &
              name="myDensityMatrix_for_Spin_"//TRIM(ADJUSTL(cp_to_string(i))))
         CALL cp_dbcsr_set(rho_ao_out(i)%matrix,0.0_dp)
      END DO
    END IF
