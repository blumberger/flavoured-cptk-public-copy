# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_move_control.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_move_control.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_move_control.F" 2
...
! allocate all the structures
      ...
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,63,&
                                     "moves",0)
      ALLOCATE (moves%bond,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,66,&
                                     "moves%bond",0)
      ALLOCATE (moves%angle,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,69,&
                                     "moves%angle",0)
      ALLOCATE (moves%dihedral,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,72,&
                                     "moves%dihedral",0)
      ALLOCATE (moves%trans,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,75,&
                                     "moves%trans",0)
      ALLOCATE (moves%rot,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,78,&
                                     "moves%rot",0)
      ALLOCATE (moves%bias_bond,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,81,&
                                     "moves%bias_bond",0)
      ALLOCATE (moves%bias_angle,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,84,&
                                     "moves%bias_angle",0)
      ALLOCATE (moves%bias_dihedral,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,87,&
                                     "moves%bias_dihedral",0)
      ALLOCATE (moves%bias_trans,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,90,&
                                     "moves%bias_trans",0)
      ALLOCATE (moves%bias_rot,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,93,&
                                     "moves%bias_rot",0)
      ALLOCATE (moves%volume,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,96,&
                                     "moves%volume",0)
      ALLOCATE (moves%hmc,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,99,&
                                     "moves%hmc",0)
      ALLOCATE (moves%avbmc_inin,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,102,&
                                     "moves%avbmc_inin",0)
      ALLOCATE (moves%avbmc_inout,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,105,&
                                     "moves%avbmc_inout",0)
      ALLOCATE (moves%avbmc_outin,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,108,&
                                     "moves%avbmc_outin",0)
      ALLOCATE (moves%avbmc_outout,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,111,&
                                     "moves%avbmc_outout",0)
      ALLOCATE (moves%swap,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,114,&
                                     "moves%swap",0)
      ALLOCATE (moves%Quickstep,stat=stat)
      IF (stat /= 0) CALL stop_memory(routineN,moduleN,117,&
                                     "moves%Quickstep",0)
...
! grab some stuff from mc_par
      ...
      CASE DEFAULT
         WRITE(*,*) 'flag =',flag
         CALL cp__b("motion/mc/mc_move_control.F",481,"Wrong option passed")
      CASE ("trans")
