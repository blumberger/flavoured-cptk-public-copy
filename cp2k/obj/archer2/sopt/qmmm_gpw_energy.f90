# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_energy.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_energy.F"
!-----------------------------------------------------------------------------!
...
# 72 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_gpw_energy.F" 2
  ...
  SUBROUTINE  qmmm_el_coupling(qs_env,qmmm_env,mm_particles,mm_cell)
    ...
    IF (dft_control%qs_control%semi_empirical) THEN
       ...
       CASE (do_qmmm_pcharge)
          CALL cp__b("qmmm_gpw_energy.F",150,"Point charge QM/MM electrostatic coupling not yet implemented for SE.")
       CASE (do_qmmm_gauss,do_qmmm_swave)
          CALL cp__b("qmmm_gpw_energy.F",152,"GAUSS or SWAVE QM/MM electrostatic coupling not yet implemented for SE.")
       CASE DEFAULT
          IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')"Unknown Coupling..."
          CALL cp__b("qmmm_gpw_energy.F",155,"")
       END SELECT
    ELSEIF (dft_control%qs_control%dftb) THEN
       ...
       CASE (do_qmmm_gauss,do_qmmm_swave)
          CALL cp__b("qmmm_gpw_energy.F",169,"GAUSS or SWAVE QM/MM electrostatic coupling not implemented for DFTB.")
       CASE DEFAULT
          IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')"Unknown Coupling..."
          CALL cp__b("qmmm_gpw_energy.F",172,"")
       END SELECT
    ELSE
       ! QS
       SELECT CASE(qmmm_env%qmmm_coupl_type)
       CASE(do_qmmm_coulomb)
          CALL cp__b("qmmm_gpw_energy.F",178,"Coulomb QM/MM electrostatic coupling not implemented for GPW/GAPW.")
       CASE(do_qmmm_pcharge)
          CALL cp__b("qmmm_gpw_energy.F",180,"Point Charge QM/MM electrostatic coupling not implemented for GPW/GAPW.")
       CASE(do_qmmm_gauss,do_qmmm_swave)
          IF (iw>0) &
       ...
       CASE DEFAULT
          IF (iw>0) WRITE(iw,'(T2,"QMMM|",1X,A)')"Unknown Coupling..."
          CALL cp__b("qmmm_gpw_energy.F",204,"")
       END SELECT
...
! *****************************************************************************
  SUBROUTINE qmmm_elec_with_gaussian(qmmm_env, v_qmmm, mm_particles,&
    ...
    IF(.NOT.(ASSOCIATED(mm_particles)))CALL cp__a("qmmm_gpw_energy.F",271)
    IF(.NOT.(ASSOCIATED(qmmm_env%mm_atom_chrg)))CALL cp__a("qmmm_gpw_energy.F",272)
    IF(.NOT.(ASSOCIATED(qmmm_env%mm_atom_index)))CALL cp__a("qmmm_gpw_energy.F",273)
    IF(.NOT.(ASSOCIATED(aug_pools)))CALL cp__a("qmmm_gpw_energy.F",274)
    IF(.NOT.(ASSOCIATED(pw_pools)))CALL cp__a("qmmm_gpw_energy.F",275)
    !Statements
    CALL timeset(routineN,handle)
    ...
    CASE DEFAULT
       CALL cp__b("qmmm_gpw_energy.F",337,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE qmmm_elec_with_gaussian_LG(pgfs, cgrid, mm_charges, mm_atom_index,&
    ...
    REAL(KIND=dp) :: a1, a2, a3, abc_X(4,4), abc_X_Y(4), b1, b2, b3, c1, c2, &
      c3, d1, d2, d3, dr1, dr1c, dr2, dr2c, dr3, dr3c, e1, e2, e3, f1, f2, &
      f3, g1, g2, g3, h1, h2, h3, p1, p2, p3, q1, q2, q3, qt, r1, r2, r3, &
      rt1, rt2, rt3, rv1, rv2, rv3, s1, s2, s3, s4, sph_chrg_factor, t1, t2, &
      t3, t4, u1, u2, u3, v1, v2, v3, v4, val, xd1, xd2, xd3, xs1, xs2, xs3
