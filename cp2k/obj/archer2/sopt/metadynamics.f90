# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"
!-----------------------------------------------------------------------------!
...
USE cp_log_handling, ONLY: cp_logger_type, cp_get_default_logger
  ...
  USE kinds,                           ONLY: dp
  USE message_passing,                 ONLY: mp_bcast, cp2k_is_parallel
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"
...
# 70 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F" 2
...
! *****************************************************************************
  SUBROUTINE metadyn_initialise_plumed(force_env, simpar, itimes)
    ...
    IF(.NOT.(ASSOCIATED(simpar)))CALL cp__a("metadynamics.F",117)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("metadynamics.F",118)
...
# 140 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(itimes))==-1) EXIT ;  END DO ; ENDIF
# 161 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"


    CALL cp_abort(cp__l("metadynamics.F",163),&
...
# 234 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"
...
# 338 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics.F"


                CALL cp_abort(cp__l("metadynamics.F",340),&
                     "Requested to use plumed for metadynamics, but cp2k was"//&
                     " not compiled with plumed support.")
        ...
        ELSE
            IF(force_env%meta_env%langevin) THEN
              IF (.NOT.PRESENT(rand)) THEN
                 CALL cp__b("metadynamics.F",349,"Langevin on COLVAR not implemented for this MD ensemble!")
              END IF
