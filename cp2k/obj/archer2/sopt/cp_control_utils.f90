# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_control_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_control_utils.F"
!-----------------------------------------------------------------------------!
...
MODULE cp_control_utils
  USE bibliography,                    ONLY: &
       Andreussi2012, Dewar1977, Dewar1985, Elstner1998, Fattebert2002, &
       Hu2007, Krack2000, Lippert1997, Lippert1999, Porezag1995, Repasky2002, &
       Rocha2006, Schenter2008, Seifert1996, Souza2002, Stengel2009, &
       Stewart1989, Stewart2007, Thiel1992, Umari2002, VandeVondele2005a, &
       VandeVondele2005b, Zhechkov2005, cite_reference
  USE cp_control_types,                ONLY: &
       admm_control_create, admm_control_type, ddapc_control_create, &
       ddapc_restraint_type, dft_control_create, dft_control_type, &
       efield_type, qs_control_type, sccs_control_create, &
  ...
  USE input_constants,                 ONLY: &
       ...
       do_admm_exch_scaling_merlot, do_admm_purify_mcweeny, &
       do_admm_purify_mo_diag, do_admm_purify_mo_no_diag, &
       do_admm_purify_none, do_admm_purify_none_dm, do_ddapc_constraint, &
       do_ddapc_restraint, do_method_am1, do_method_dftb, do_method_gapw, &
       do_method_gapw_xc, do_method_gpw, do_method_lrigpw, do_method_mndo, &
       ...
       real_time_propagation, sccs_andreussi, sccs_derivative_cd3, &
       sccs_derivative_cd5, sccs_derivative_cd7, sccs_derivative_fft, &
       sccs_fattebert_gygi, sic_ad, sic_eo, sic_list_all, sic_list_unpaired, &
       sic_mauri_spz, sic_mauri_us, sic_none, slater, tddfpt_excitations
  ...
  USE input_section_types,             ONLY: &
       ...
       section_vals_get, section_vals_get_subs_vals, section_vals_type, &
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_control_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE read_dft_control(dft_control,dft_section)
    ...
    IF (.NOT.is_present.AND.not_SE) THEN
       CALL cp__b("cp_control_utils.F",127,"XC section missing.")
    END IF
    IF (is_present) THEN
       ...
       IF(density_cut<=EPSILON(0.0_dp)*100.0_dp)&
          CALL cp_warn(cp__l("cp_control_utils.F",135),&
       ...
       IF(gradient_cut<=EPSILON(0.0_dp)*100.0_dp)&
          CALL cp_warn(cp__l("cp_control_utils.F",140),&
       ...
       IF(tau_cut<=EPSILON(0.0_dp)*100.0_dp)&
          CALL cp_warn(cp__l("cp_control_utils.F",145),&
    ...
    IF (.NOT.is_present.AND.not_SE) THEN
       CALL cp__b("cp_control_utils.F",156,"XC_FUNCTIONAL section missing.")
    END IF
    ...
    IF (dft_control%drho_by_collocation) THEN
      CALL cp__b("cp_control_utils.F",176,"derivatives by collocation not implemented")
    END IF
    ...
    IF(is_present) THEN
      ...
      IF( (dft_control%admm_control%method == do_admm_blocking_purify_full .OR. &
         ...
         CALL cp__b("cp_control_utils.F",213,"ADMM: Blocking and Merlot scaling are mutually exclusive.")
      END IF
      ...
      IF( dft_control%admm_control%method == do_admm_charge_constrained_projection .AND. &
         ...
         CALL cp_abort(cp__l("cp_control_utils.F",219),&
      ...
      IF(dft_control%admm_control%purification_method == do_admm_purify_mo_diag    .OR. &
         dft_control%admm_control%purification_method == do_admm_purify_mo_no_diag) THEN
          IF( dft_control%admm_control%method /= do_admm_basis_projection ) &
            CALL cp__b("cp_control_utils.F",229,"ADMM: Chosen purification requires BASIS_PROJECTION")

           IF( .NOT. do_ot) CALL cp__b("cp_control_utils.F",231,"ADMM: MO-based purification requires OT.")
      END IF
    ...
    IF (dft_control%relax_multiplicity > 0.0_dp) THEN
       IF(.NOT.dft_control%uks)&
          CALL cp_abort(cp__l("cp_control_utils.F",253),"The option RELAX_MULTIPLICITY is only valid for "//&
    ...
    IF (is_present) THEN
       ...
       ELSE
          ...
          IF(.NOT.(nrep==1))CALL cp__a("cp_control_utils.F",333)
       END IF
    END IF
    ...
    IF (dft_control%apply_period_efield) THEN
       ...
       IF(.NOT.(.NOT.do_rtp))CALL cp__a("cp_control_utils.F",352)
    ...
    !MK Read the SCCS input section if present
       ...
       IF (dft_control%do_sccs) THEN
          ...
          CASE (sccs_andreussi)
             ...
             IF (dft_control%sccs_control%rho_max < dft_control%sccs_control%rho_min) THEN
                CALL cp_abort(cp__l("cp_control_utils.F",404),&
          ...
          CASE (sccs_fattebert_gygi)
             ...
             IF (dft_control%sccs_control%beta < 0.5_dp) THEN
                CALL cp_abort(cp__l("cp_control_utils.F",413),&
          ...
          CASE DEFAULT
             CALL cp__b("cp_control_utils.F",420,"Invalid SCCS model specified. Please, check your input!")
          END SELECT
...
!> \param para_env ...
! *****************************************************************************
  SUBROUTINE read_mgrid_section(qs_control,dft_section,para_env)
    ...
    CALL section_vals_val_get(mgrid_section,"SKIP_LOAD_BALANCE_DISTRIBUTED", &
                                 l_val=qs_control%skip_load_balance_distributed,explicit=explicit)
    ...
    IF (multigrid_set) THEN
      ...
      IF (qs_control%commensurate_mgrids) THEN
         CALL cp__b("cp_control_utils.F",503,"Do not specify cutoffs for the commensurate grids (NYI)")
      END IF
      ...
      IF(ASSOCIATED(cutofflist)) THEN
         IF(SIZE(cutofflist,1) /= ngrid_level)&
            CALL cp__b("cp_control_utils.F",509,"Inconsistent values for number of multi grids")
    ...
    DO igrid_level=2,ngrid_level
      IF ( qs_control%e_cutoff(igrid_level) > &
         ...
         CALL cp__b("cp_control_utils.F",537,"Multi-grids not ordered")
      END IF
    END DO
...
! *****************************************************************************
  SUBROUTINE read_qs_section(qs_control,qs_section)
    ...
    TYPE(section_vals_type), POINTER :: becke_restraint_section, &
      ddapc_restraint_section, dftb_parameter, dftb_section, &
      lri_optbas_section, mull_section, s2_restraint_section, scptb_section, &
      se_section
    ...
    becke_restraint_section=> section_vals_get_subs_vals(qs_section,"BECKE_RESTRAINT")
    ...
    IF (qs_control%mulliken_restraint) THEN
       ...
       IF (qs_control%mulliken_restraint_control%natoms<1) &
            CALL cp__b("cp_control_utils.F",785,"Need at least 1 atom to use mulliken contraints")
    ...
    CALL section_vals_get(becke_restraint_section,explicit=qs_control%becke_restraint)
    ...
    IF (qs_control%semi_empirical) THEN
       ...
       IF (qs_control%method_id==do_method_pnnl) THEN
          IF(qs_control%se_control%integral_screening/=do_se_IS_slater)&
             CALL cp_warn(cp__l("cp_control_utils.F",835),&
       ...
       CASE(do_se_lr_ewald_gks)
          ...
          IF(qs_control%method_id/=do_method_pnnl)&
             CALL cp_abort(cp__l("cp_control_utils.F",903),&
       ...
       CASE(do_se_lr_ewald_r3)
          ...
          IF(qs_control%se_control%integral_screening/=do_se_IS_kdso)&
             CALL cp_abort(cp__l("cp_control_utils.F",912),&
                  "A periodic semi-empirical calculation was requested with a long-range  "//&
       ...
       IF (qs_control%se_control%periodic_type==do_se_lr_ewald_r3) THEN
          CALL cp__b("cp_control_utils.F",935,"EWALD_R3 not implemented yet!")
       END IF
    ...
    IF (qs_control%scptb) THEN
       CALL section_vals_val_get(scptb_section,"DISPERSION",&
            l_val=qs_control%scptb_control%dispersion)
       CALL section_vals_val_get(scptb_section,"PARAMETER_FILE_NAME",&
            c_val=qs_control%scptb_control%parameter_file)
       CALL section_vals_val_get(scptb_section,"DISPERSION_RADIUS",&
            r_val=qs_control%scptb_control%rcdisp)
       CALL section_vals_val_get(scptb_section,"COORDINATION_CUTOFF",&
            r_val=qs_control%scptb_control%epscn)
       CALL section_vals_val_get(scptb_section,"D3_SCALING",r_vals=scal)
  ...
  SUBROUTINE write_dft_control(dft_control, dft_section)
    ...
    IF (output_unit > 0) THEN
       ...
       CASE DEFAULT
         ...
         CALL cp__b("cp_control_utils.F",1147,"SIC option unknown")
       END SELECT
       ...
       IF (dft_control%do_sccs) THEN
          IF (dft_control%qs_control%gapw) THEN
             CALL cp__b("cp_control_utils.F",1204,"SCCS is not yet implemented with GAPW")
          END IF
          WRITE (UNIT=output_unit,FMT="(/,T2,A)")&
          ...
          CASE DEFAULT
             CALL cp__b("cp_control_utils.F",1220,"Invalid SCCS model specified. Please, check your input!")
          END SELECT
          ...
          CASE DEFAULT
             CALL cp_abort(cp__l("cp_control_utils.F",1236),&
    ...
    IF (output_unit>0) THEN
       ...
       IF (qs_control%ddapc_restraint) THEN
          DO i=1,SIZE( qs_control%ddapc_restraint_control )
             ...
             CASE DEFAULT
                CALL cp__b("cp_control_utils.F",1441,"Unknown ddapc restraint")
             END SELECT
          END DO
       END IF
       IF (qs_control%s2_restraint) THEN
          ...
          CASE (do_s2_restraint)
             ...
             CALL cp__b("cp_control_utils.F",1454,"Not yet implemented")
          CASE (do_s2_constraint)
             WRITE (UNIT=output_unit,FMT="(T2,A,T61,A20)")&
                  "QS| s2 restraint functional form :","CONSTRAINT"
          CASE DEFAULT
             CALL cp__b("cp_control_utils.F",1459,"Unknown ddapc restraint")
          END SELECT
       END IF
    END IF
    CALL cp_print_key_finished_output(output_unit,logger,dft_section,&
  ...
  SUBROUTINE read_ddapc_section(qs_control,qs_section,ddapc_restraint_section)
    ...
    IF(PRESENT(ddapc_restraint_section))THEN
       IF(ASSOCIATED(qs_control%ddapc_restraint_control))THEN
          IF(SIZE(qs_control%ddapc_restraint_control).GE.2)&
               CALL cp__b("cp_control_utils.F",1501,"ET_COUPLING cannot be used in combination with a normal restraint")
       ELSE
    ...
    DO i=1,SIZE(qs_control%ddapc_restraint_control)
       ...
       DO k = 1,n_rep
          ...
          END DO
       END DO
       IF (jj < 1) CALL cp__b("cp_control_utils.F",1539,"Need at least 1 atom to use ddapc contraints")
       ...
       DO k = 1,n_rep
          ...
          DO  j = 1,SIZE(rtmplist)
             ...
             IF (jj>ddapc_restraint_control%natoms) &
                CALL cp__b("cp_control_utils.F",1568,"Need the same number of coeff as there are atoms ")
             ddapc_restraint_control%coeff (jj) = rtmplist(j)
          END DO
       END DO
       IF (jj<ddapc_restraint_control%natoms .AND. jj.NE.0) &
            CALL cp__b("cp_control_utils.F",1573,"Need no or the same number of coeff as there are atoms.")
    END DO
    k=0
    DO i=1,SIZE(qs_control%ddapc_restraint_control)
    ...
    END DO
    IF(k==2) CALL cp_abort(cp__l("cp_control_utils.F",1580),&
...
! *****************************************************************************
  SUBROUTINE read_becke_section(qs_control,becke_section)
    ...
    jj = 0
    DO k = 1,n_rep
       CALL section_vals_val_get(becke_section,"ATOMS",i_rep_val=k,i_vals=tmplist)
       DO  j = 1,SIZE(tmplist)
          jj = jj+1
       END DO
    END DO
    IF (jj < 1) CALL cp__b("cp_control_utils.F",1618,"Need at least 1 atom to use ddapc contraints.")
    ...
    DO k = 1,n_rep
       ...
       DO  j = 1,SIZE(rtmplist)
          ...
          IF (jj>qs_control%becke_control%natoms) &
               CALL cp__b("cp_control_utils.F",1644,"Need the same number of coeff as there are atoms.")
          qs_control%becke_control%coeff (jj) = rtmplist(j)
       END DO
    END DO
    IF (jj < qs_control%becke_control%natoms .AND. jj.NE.0) &
       CALL cp__b("cp_control_utils.F",1649,"Need no or the same number of coeff as there are atoms.")
...
! *****************************************************************************
  SUBROUTINE read_rtp_section(dft_control,rtp_section)
    ...
    CALL section_vals_val_get(rtp_section,"ORTHONORMAL",&
      l_val=dft_control%rtp_control%orthonormal)
