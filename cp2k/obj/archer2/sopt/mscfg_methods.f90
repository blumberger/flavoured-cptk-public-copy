# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mscfg_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mscfg_methods.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mscfg_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE loop_over_molecules(globenv,force_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("mscfg_methods.F",93)
...
! *****************************************************************************
  SUBROUTINE calcs_on_isolated_molecules(force_env, globenv, nfrags,&
    ...
    INTEGER :: first_atom, force_method, global_charge, global_multpl, &
      handle, i, ifrag, imo, isize, j, k, last_atom, my_targ, nmosets_of_frag
    ...
    IF(.NOT.(force_method.eq.do_qs))CALL cp__a("mscfg_methods.F",177)
    ...
    IF(.NOT.(ASSOCIATED(mscfg_env)))CALL cp__a("mscfg_methods.F",196)
    !
    ...
    DO ifrag = 1, nfrags
       ...
       IF(.NOT.(nmosets_of_frag.le.mscfg_max_moset_size))CALL cp__a("mscfg_methods.F",285)
...
! *****************************************************************************
  FUNCTION do_mol_loop(force_env)
    ...
    IF (method_name_id.eq.do_qs) THEN
       ...
       CALL force_env_get(force_env, qs_env=qs_env)
       IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("mscfg_methods.F",408)
       
       CALL get_qs_env(qs_env, mscfg_env=mscfg_env)
       IF(.NOT.(ASSOCIATED(mscfg_env)))CALL cp__a("mscfg_methods.F",411)
