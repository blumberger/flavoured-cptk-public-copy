# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_kernel.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_kernel.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_kernel.F" 2
...
! *****************************************************************************
  ...
  ELSE
     ...
     CALL cp__b("pw/ps_wavelet_kernel.F",139,"No wavelet based poisson solver for given geometry")
...
SUBROUTINE Surfaces_Kernel(n1,n2,n3,m3,nker1,nker2,nker3,h1,h2,h3,&
    ...
    INTEGER :: i, i1, i2, i3, ic, iend, imu, ind1, ind2, inzee, ipolyord, &
      ireim, istart, j2, j2nd, j2st, jnd1, jp2, jreim, n_cell, n_range, &
      n_scf, nact2, ncache, nfft, num_of_mus, shift
...
SUBROUTINE Free_Kernel(n01,n02,n03,nfft1,nfft2,nfft3,n1k,n2k,n3k,&
    ...
    INTEGER :: i, i01, i02, i03, i1, i2, i3, i_gauss, i_kern, iend, istart, &
      istart1, n1h, n2h, n3h, n_cell, n_iter, n_range, n_scf, nker1, nker2, &
      nker3
...
!check input
  ...
  IF(.NOT.(nd1.GE.n1))CALL cp__a("pw/ps_wavelet_kernel.F",1148)
  IF(.NOT.(nd2.GE.n2))CALL cp__a("pw/ps_wavelet_kernel.F",1149)
  IF(.NOT.(nd3.GE.n3/2+1))CALL cp__a("pw/ps_wavelet_kernel.F",1150)
  IF(.NOT.(MOD(nd3,nproc).EQ.0))CALL cp__a("pw/ps_wavelet_kernel.F",1151)
  IF(.NOT.(MOD(nd2,nproc).EQ.0))CALL cp__a("pw/ps_wavelet_kernel.F",1152)
  ...
  lot=ncache/(2*n3)
  IF(.NOT.(lot.GE.1))CALL cp__a("pw/ps_wavelet_kernel.F",1199)
  ...
  DO j3=1,nd3/nproc
     ...
     IF (iproc*(nd3/nproc)+j3.LE.n3/2+1) THEN
        ...
        lot=ncache/(4*n1)
        IF(.NOT.(lot.GE.1))CALL cp__a("pw/ps_wavelet_kernel.F",1250)
        ...
        lot=ncache/(4*n2)
        IF(.NOT.(lot.GE.1))CALL cp__a("pw/ps_wavelet_kernel.F",1283)
