# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_methods.F"
!-----------------------------------------------------------------------------!
...
# 104 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_methods.F" 2
  ...
  SUBROUTINE optimize_loc_berry( method, qs_loc_env, vectors, op_sm_set, &
    ...
    CASE( do_loc_l1_norm_sd )
       IF(.NOT. cell%orthorhombic ) THEN
           CALL cp__b("qs_loc_methods.F",234,"Non-orthorhombic cell with the selected method NYI")
       ELSE
    ...
    CASE ( do_loc_none )
       ...
       ENDIF
    CASE DEFAULT
       CALL cp__b("qs_loc_methods.F",252,"Unknown localization method")
    END SELECT
    IF (output_unit>0) THEN
...
! *****************************************************************************
  SUBROUTINE  optimize_loc_pipek(qs_env, method, qs_loc_env, vectors, zij_fm_set,&
    ...
    INTEGER                                  :: handle, iatom, isgf, ldz, &
                                                nao, natom, ncol, nmoloc, &
                                                output_unit, sweeps
    ...
    DO iatom = 1, natom
       ...
       CALL cp_gemm('N','N',nao,nmoloc,nao,1.0_dp,ov_fm,vectors,0.0_dp,opvec,&
            b_first_col=1,a_first_row=1,b_first_row=1)
       CALL cp_gemm('T','N',nmoloc,nmoloc,ncol,0.5_dp,vectors,opvec,&
            0.0_dp,zij_fm_set(iatom,1)%matrix,&
            b_first_col=1,a_first_row=isgf,b_first_row=isgf)
    ...
    CASE ( do_loc_crazy )
       CALL cp__b("qs_loc_methods.F",364,"Crazy and Pipek not implemented.")
    CASE( do_loc_l1_norm_sd )
       CALL cp__b("qs_loc_methods.F",366,"L1 norm and Pipek not implemented.")
    CASE ( do_loc_direct )
       CALL cp__b("qs_loc_methods.F",368,"Direct and Pipek not implemented.")
    CASE ( do_loc_none )
       IF (output_unit>0) WRITE(output_unit,'(A,I6,A)') " No MOS localization applied "
    CASE DEFAULT
       CALL cp__b("qs_loc_methods.F",372,"Unknown localization method")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE centers_spreads_berry( qs_loc_env, zij, nmoloc, cell, weights, ispin,&
    ...
    IF(.NOT.(nstates>=nmoloc))CALL cp__a("qs_loc_methods.F",445)

    centers => qs_loc_env%localized_wfn_control%centers_set(ispin)%array
    IF(.NOT.(ASSOCIATED(centers)))CALL cp__a("qs_loc_methods.F",448)
    IF(.NOT.(SIZE(centers,2)==nmoloc))CALL cp__a("qs_loc_methods.F",449)
    sum_spread_i  = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE centers_spreads_pipek(qs_loc_env,zij_fm_set,particle_set,ispin,&
    ...
    CHARACTER(len=default_path_length)       :: file_tmp, iter
    INTEGER                                  :: iatom, istate, natom, nstate, &
                                                unit_out_s
    ...
    centers => qs_loc_env%localized_wfn_control%centers_set(ispin)%array
    IF(.NOT.(ASSOCIATED(centers)))CALL cp__a("qs_loc_methods.F",545)
    IF(.NOT.(SIZE(centers,2)==nstate))CALL cp__a("qs_loc_methods.F",546)
...
! *****************************************************************************
  SUBROUTINE qs_loc_driver(qs_env,qs_loc_env,print_loc_section,myspin,&
    ...
    INTEGER :: dim_op, handle, i, imo, imoloc, ir, ispin, istate, j, jstate, &
      l_spin, lb, loc_method, n_rep, nao, ncubes, nmo, nmosub, s_spin, ub
    ...
    DO ispin = s_spin,l_spin
       ...
       CASE(op_loc_berry)
          ...
          DO i = 1, dim_op
             DO j = 1, SIZE ( op_fm_set, 1 )
                ...
                IF(.NOT.(nmo>=nmosub))CALL cp__a("qs_loc_methods.F",691)
       ...
       CASE(op_loc_boys)
          CALL cp__b("qs_loc_methods.F",711,"Boys localization not implemented")
  ...
  SUBROUTINE qs_print_cubes(qs_env,mo_coeff,nstates,state_list,centers,&
    ...
    DO istate = 1, nstates
       ...
       CALL cp_pw_to_cube(wf_r%pw,unit_out_c,title,&
            particles=particles,&
            stride=section_get_ivals(print_key,"STRIDE"))
       CALL cp_print_key_finished_output(unit_out_c,logger,print_key,"")
    END DO
