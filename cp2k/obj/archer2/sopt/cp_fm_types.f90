# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_fm_create(matrix,matrix_struct,name,use_sp)
    ...
    IF(.NOT.(ASSOCIATED(matrix_struct)))CALL cp__a("fm/cp_fm_types.F",160)
    ALLOCATE(matrix)
...
! *****************************************************************************
  SUBROUTINE cp_fm_retain(matrix)
    ...
    IF(.NOT.(ASSOCIATED(matrix)))CALL cp__a("fm/cp_fm_types.F",219)
    IF(.NOT.(matrix%ref_count>0))CALL cp__a("fm/cp_fm_types.F",220)
...
! *****************************************************************************
  SUBROUTINE cp_fm_release(matrix)
    ...
    IF (ASSOCIATED(matrix)) THEN
       IF(.NOT.(matrix%ref_count>0))CALL cp__a("fm/cp_fm_types.F",243)
       matrix%ref_count=matrix%ref_count-1
...
! *****************************************************************************
  SUBROUTINE cp_fm_init_random(matrix,ncol,start_col)
    ...
    IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",302)
    ...
    IF(ncol_global<(my_start_col+my_ncol-1))&
       CALL cp__b("fm/cp_fm_types.F",315,"ncol_global>=(my_start_col+my_ncol-1)")
    ...
    DO icol_local=1,ncol_local
       IF(.NOT.(col_indices(icol_local)>icol_global))CALL cp__a("fm/cp_fm_types.F",324)
       DO
...
! *****************************************************************************
  SUBROUTINE cp_fm_set_all(matrix,alpha,beta)
    ...
    IF (PRESENT(beta)) THEN
       IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",379)
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"


    CALL cp_fm_get_info(matrix,nrow_global=nrow_global)

# 445 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
    DO i=1, nrow_global
       IF(matrix%use_sp) THEN
...
# 518 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
...
! *****************************************************************************
  SUBROUTINE cp_fm_set_element(matrix,irow_global,icol_global,alpha)
    ...
    IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",558)

# 574 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
...
! *****************************************************************************
  SUBROUTINE cp_fm_set_submatrix(fm,new_values,start_row,&
    ...
    IF(.NOT.(ASSOCIATED(fm)))CALL cp__a("fm/cp_fm_types.F",630)
    IF(.NOT.(fm%ref_count>0))CALL cp__a("fm/cp_fm_types.F",631)

    IF(.NOT.(.NOT.fm%use_sp))CALL cp__a("fm/cp_fm_types.F",633)
...
! *****************************************************************************
  SUBROUTINE cp_fm_get_submatrix(fm,target_m, start_row,&
    ...
    IF(.NOT.(ASSOCIATED(fm)))CALL cp__a("fm/cp_fm_types.F",768)
    IF(.NOT.(fm%ref_count>0))CALL cp__a("fm/cp_fm_types.F",769)

    IF(.NOT.(.NOT.fm%use_sp))CALL cp__a("fm/cp_fm_types.F",771)
...
! *****************************************************************************
  SUBROUTINE cp_fm_maxabsval(matrix,a_max,ir_max,ic_max)
    ...
    IF(PRESENT(ir_max)) THEN
      ...
      IF(.NOT.(ic_max>0))CALL cp__a("fm/cp_fm_types.F",1001)
      IF(.NOT.(ir_max>0))CALL cp__a("fm/cp_fm_types.F",1002)
...
! *****************************************************************************
  SUBROUTINE cp_fm_maxabsrownorm(matrix, a_max)
    ...
    IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",1040)
...
! *****************************************************************************
  SUBROUTINE cp_fm_vectorsnorm(matrix, norm_array)
    ...
    IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",1081)
...
! *****************************************************************************
  ...
  USE message_passing,  ONLY: mp_sum
    ...
    IF(.NOT.(.NOT.matrix%use_sp))CALL cp__a("fm/cp_fm_types.F",1124)
    ...
    IF ((.NOT. cp2k_is_parallel).OR.&
       ...
       IF(source%use_sp.AND.destination%use_sp) THEN
          IF(SIZE(source%local_data_sp,1)/=SIZE(destination%local_data_sp,1).OR.&
             ...
             CALL cp_abort(cp__l("fm/cp_fm_types.F",1170),&
       ...
       ELSEIF(source%use_sp.AND..NOT.destination%use_sp) THEN
          IF(SIZE(source%local_data_sp,1)/=SIZE(destination%local_data,1).OR.&
             ...
             CALL cp_abort(cp__l("fm/cp_fm_types.F",1179),&
                  "Cannot copy full matrix <"//TRIM(source%name)//&
                  "> to full matrix <"//TRIM(destination%name)//&
                  ">. The local_data blocks have different sizes.")
       ...
       ELSEIF(.NOT.source%use_sp.AND.destination%use_sp) THEN
          IF(SIZE(source%local_data,1)/=SIZE(destination%local_data_sp,1).OR.&
             ...
             CALL cp_abort(cp__l("fm/cp_fm_types.F",1187),&
                  "Cannot copy full matrix <"//TRIM(source%name)//&
                  "> to full matrix <"//TRIM(destination%name)//&
                  ">. The local_data blocks have different sizes.")
       ...
       ELSE
          IF(SIZE(source%local_data,1)/=SIZE(destination%local_data,1).OR.&
             ...
             CALL cp_abort(cp__l("fm/cp_fm_types.F",1195),&
                  "Cannot copy full matrix <"//TRIM(source%name)//&
                  "> to full matrix <"//TRIM(destination%name)//&
                  ">. The local_data blocks have different sizes.")
       ...
       ENDIF
    ELSE
      CALL cp__b("fm/cp_fm_types.F",1203,"")
    END IF
...
!    nrow must be <= na and nb
    IF(nrow>na)&
       CALL cp__b("fm/cp_fm_types.F",1343,"cannot copy because nrow > number of rows of source matrix")
    IF(nrow>nb)&
       CALL cp__b("fm/cp_fm_types.F",1345,"cannot copy because nrow > number of rows of target matrix")
    na = msource%matrix_struct%ncol_global
    nb = mtarget%matrix_struct%ncol_global
!    ncol must be <= na_col and nb_col
    IF(ncol>na)&
       CALL cp__b("fm/cp_fm_types.F",1350,"cannot copy because nrow > number of rows of source matrix")
    IF(ncol>nb)&
       CALL cp__b("fm/cp_fm_types.F",1352,"cannot copy because nrow > number of rows of target matrix")

# 1363 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
    DO i = 0,ncol-1
      ss = s_firstcol+i
      ts = t_firstcol+i
...
!>        Matrices have to have the same global shape.
!>
...
!>
!>        Here A and B are global matrices, Aloc, and Bloc, are local matrices, 
!>        and context is a process grid that covers all parts of either A or B. 
!>        If a whole global matrix is to be distributed, use the call:
!>
!>        call pdgemr2d(M,N,A,1,1,descA,Bloc,1,1,descB,context)
!>
...
! *****************************************************************************
  SUBROUTINE cp_fm_copy_general(source,destination,global_context)

    TYPE(cp_fm_type), POINTER                :: source, destination
    INTEGER, INTENT(IN)                      :: global_context
    ...
    IF (.NOT. cp2k_is_parallel) THEN
       CALL cp_fm_to_fm(source,destination)
    ELSE
# 1462 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
       IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(global_context))==-1) EXIT ;  END DO ; ENDIF
       CALL cp__b("fm/cp_fm_types.F",1463,"")
...
! *****************************************************************************
  SUBROUTINE cp_fm_add_to_element(matrix,irow_global,icol_global,alpha)
    ...
    IF(.NOT.(ASSOCIATED(matrix)))CALL cp__a("fm/cp_fm_types.F",1502)
...
# 1526 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
...
! *****************************************************************************
 SUBROUTINE cp_fm_write_unformatted(fm,unit)
    ...
    TYPE(cp_para_env_type), POINTER          :: para_env
# 1559 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"

   CALL timeset(routineN,handle)
   CALL cp_fm_get_info(fm, nrow_global=nrow_global,ncol_global=ncol_global,ncol_block=max_block,&
        para_env=para_env)

# 1622 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
...
# 1681 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_types.F"
...
! *****************************************************************************
 FUNCTION  cp_fm_indxg2p( INDXGLOB, NB, IPROC, ISRCPROC, NPROCS ) RESULT(G2P)
   ...
   IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(indxglob))==-1) EXIT ;  END DO ; ENDIF
   IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(iproc))==-1) EXIT ;  END DO ; ENDIF
   IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(isrcproc))==-1) EXIT ;  END DO ; ENDIF
   IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(nb))==-1) EXIT ;  END DO ; ENDIF
   IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(nprocs))==-1) EXIT ;  END DO ; ENDIF
