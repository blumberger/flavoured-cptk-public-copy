# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dft_plus_u.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dft_plus_u.F"
!-----------------------------------------------------------------------------!
...
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dft_plus_u.F" 2
...
! *****************************************************************************
  SUBROUTINE plus_u(qs_env,matrix_h,matrix_w)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("dft_plus_u.F",116)
    ...
    CASE (plus_u_mulliken_charges)
      CALL mulliken_charges(qs_env,orthonormal_basis,matrix_h,matrix_w,&
                            should_output,output_unit,print_level)
    CASE DEFAULT
      CALL cp__b("dft_plus_u.F",165,"Invalid DFT+U method requested")
    END SELECT
...
!> \par
!>  \f{eqnarray*}{
!>   E^{\rm DFT+U} & = & E^{\rm DFT} +  E^{\rm U}\\!>                 & = & E^{\rm DFT} +  \frac{1}{2}(U - J)\sum_\mu (q_\mu - q_\mu^2)\\[1ex]

!>   V_{\mu\nu}^{\rm DFT+U} & = & V_{\mu\nu}^{\rm DFT} + V_{\mu\nu}^{\rm U}\\!>                          & = & \frac{\partial E^{\rm DFT}}
...
! *****************************************************************************
  SUBROUTINE lowdin(qs_env,matrix_h,matrix_w,should_output,output_unit,&
    ...
    LOGICAL :: debug, dft_plus_u_atom, fm_work1_local_alloc, &
    ...
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("dft_plus_u.F",310)
    IF(.NOT.(ASSOCIATED(dft_control)))CALL cp__a("dft_plus_u.F",311)
    IF(.NOT.(ASSOCIATED(energy)))CALL cp__a("dft_plus_u.F",312)
    IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("dft_plus_u.F",313)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("dft_plus_u.F",314)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("dft_plus_u.F",315)
    ...
    IF(.NOT.(ASSOCIATED(fm_s_half)))CALL cp__a("dft_plus_u.F",353)
    ...
    DO ispin=1,nspin
          ...
          IF (ASSOCIATED(q_block)) THEN
              ...
              IF ((qs_env%scf_env%iter_delta >= eps_scf).OR.&
                ...
                DO isb=1,nsb
                  ...
                  DO j=i0,jsb*nsbsize
                    ...
                    END IF
                    IF(.NOT.(iorb /= -1000))CALL cp__a("dft_plus_u.F",635)
            ...
            IF (.NOT.just_energy) THEN
              ...
              CALL cp_dbcsr_get_block_p(matrix=sm_v,&
                                        ...
                                        block=v_block,&
                                        found=found)
              IF(.NOT.(ASSOCIATED(v_block)))CALL cp__a("dft_plus_u.F",675)
      ...
      IF (ASSOCIATED(sm_w)) THEN
        ...
        CALL cp__w("dft_plus_u.F",801,"Forces are not yet fully implemented for DFT+U method LOWDIN")
    ...
    IF(energy%dft_plus_u < 0.0_dp)&
       CALL cp_warn(cp__l("dft_plus_u.F",812),&
...
!> \par
!>  \f{eqnarray*}{
!>   E^{\rm DFT+U} & = & E^{\rm DFT} + E^{\rm U}\\!>                 & = & E^{\rm DFT} + \frac{1}{2}\sum_A(U_A - J_A)(Tr(q_A) - Tr(q^2_A))\\[1ex]
...
! *****************************************************************************
  SUBROUTINE mulliken(qs_env,orthonormal_basis,matrix_h,should_output,&
    ...
    INTEGER :: atom_a, handle, i, i0, iatom, ikind, iorb, isb, iset, isgf, &
      ishell, ispin, j, jsb, jset, jsgf, jshell, lu, m, max_scf, n, natom, &
      natom_of_kind, nkind, norb, nsb, nsbsize, nset, nsgf_kind, nspin
    INTEGER, DIMENSION(1)                    :: iloc
    INTEGER, DIMENSION(:), POINTER           :: atom_list, nshell, orbitals
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgf, l, last_sgf
    ...
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("dft_plus_u.F",962)
    IF(.NOT.(ASSOCIATED(dft_control)))CALL cp__a("dft_plus_u.F",963)
    IF(.NOT.(ASSOCIATED(energy)))CALL cp__a("dft_plus_u.F",964)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("dft_plus_u.F",965)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("dft_plus_u.F",966)
    ...
    ELSE
      ...
      CALL get_qs_env(qs_env=qs_env,&
                      matrix_s=matrix_s)
      IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("dft_plus_u.F",974)
    ...
    DO ispin=1,nspin
      ...
      DO iatom=1,natom
        ...
        CALL cp_dbcsr_get_block_p(matrix=sm_q,&
                                  ...
                                  block=q_block,&
                                  found=found)
        IF(.NOT.(ASSOCIATED(q_block)))CALL cp__a("dft_plus_u.F",1053)
        ...
        ELSE
          CALL cp_dbcsr_get_block_p(matrix=sm_s,&
                                    ...
                                    block=s_block,&
                                    found=found)
          IF(.NOT.(ASSOCIATED(s_block)))CALL cp__a("dft_plus_u.F",1066)
          ...
          IF (ASSOCIATED(q_block)) THEN
                ...
                DO isb=1,nsb
                  ...
                  DO j=i0,jsb*nsbsize
                    ...
                    ELSE
                      ...
                      orb_occ(lu+orbitals(i)+1) = .TRUE.
                    END IF
                    IF(.NOT.(iorb /= -1000))CALL cp__a("dft_plus_u.F",1261)
                    iloc = MAXLOC(q_eigvec(iorb,:))
                    q_eigval(iloc(1)) = MIN(occ,trq)
                    q_matrix(:,iloc(1)) = q_eigval(iloc(1))*q_eigvec(:,iloc(1)) ! backtransform left
                    trq = trq - q_eigval(iloc(1))
            ...
            IF (.NOT.just_energy) THEN
              ...
              CALL cp_dbcsr_get_block_p(matrix=sm_v,&
                                        ...
                                        block=v_block,&
                                        found=found)
              IF(.NOT.(ASSOCIATED(v_block)))CALL cp__a("dft_plus_u.F",1301)
        ...
        DO ikind=1,nkind
          ...
          DO iatom=1,natom_of_kind
            ...
            CALL cp_dbcsr_get_block_p(matrix=sm_v,&
                                      ...
                                      block=v_block,&
                                      found=found)
            IF(.NOT.(ASSOCIATED(v_block)))CALL cp__a("dft_plus_u.F",1444)
            ...
            ELSE
               CALL cp_dbcsr_get_block_p(matrix=sm_s,&
                                         ...
                                         block=s_block,&
                                         found=found)
              IF(.NOT.(ASSOCIATED(s_block)))CALL cp__a("dft_plus_u.F",1456)
    ...
    IF(energy%dft_plus_u < 0.0_dp)&
       CALL cp_warn(cp__l("dft_plus_u.F",1477),&
            "DFT+U energy contibution is negative possibly due "//&
...
! *****************************************************************************
  SUBROUTINE mulliken_charges(qs_env,orthonormal_basis,matrix_h,matrix_w,&
    ...
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("dft_plus_u.F",1609)
    IF(.NOT.(ASSOCIATED(dft_control)))CALL cp__a("dft_plus_u.F",1610)
    IF(.NOT.(ASSOCIATED(energy)))CALL cp__a("dft_plus_u.F",1611)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("dft_plus_u.F",1612)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("dft_plus_u.F",1613)
    ...
    ELSE
      ...
      CALL get_qs_env(qs_env=qs_env,&
                      matrix_s=matrix_s)
      IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("dft_plus_u.F",1621)
      sm_s => matrix_s(1)%matrix
    END IF
      ...
      DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
        ...
        ELSE
          ...
          CALL cp_dbcsr_get_block_p(matrix=sm_s,&
                                    ...
                                    block=s_block,&
                                    found=found)
          IF(.NOT.(ASSOCIATED(s_block)))CALL cp__a("dft_plus_u.F",1714)
        ...
        DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          ELSE
            ...
            CALL cp_dbcsr_get_block_p(matrix=sm_s,&
                                      ...
                                      block=s_block,&
                                      found=found)
            IF(.NOT.(ASSOCIATED(s_block)))CALL cp__a("dft_plus_u.F",1938)
      ...
      IF (ASSOCIATED(sm_w).AND.(.NOT.orthonormal_basis)) THEN
        ...
        DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          CALL cp_dbcsr_get_block_p(matrix=sm_w,&
                                    ...
                                    block=w_block,&
                                    found=found)
          IF(.NOT.(ASSOCIATED(w_block)))CALL cp__a("dft_plus_u.F",1997)
   ...
   END DO ! next spin "ispin"
    ...
    IF(energy%dft_plus_u < 0.0_dp)&
       CALL cp_warn(cp__l("dft_plus_u.F",2038),&
            "DFT+U energy contibution is negative possibly due "//&
            "to unphysical Mulliken charges. Check your input, "//&
            "if this warning persists or try a different method!")
