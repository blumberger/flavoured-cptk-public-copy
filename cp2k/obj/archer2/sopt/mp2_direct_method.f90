# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2_direct_method.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2_direct_method.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2_direct_method.F" 2
...
! *****************************************************************************
  SUBROUTINE mp2_canonical_direct_single_batch(Emp2,Emp2_Cou,Emp2_ex,mp2_env,qs_env,rho,para_env,&
    ...
    INTEGER :: max_am, max_num_call_sec_transf, max_pgf, max_set, multiple, &
      ...
      sphi_c_u3, sphi_d_u1, sphi_d_u2, sphi_d_u3, Ssize_rec, step_size, &
          ...
          !  proc_receive=proc_send
              ...
              DO index_proc_ij=proc_send+1,multiple,para_env%num_pe
                ...
                SELECT CASE(case_send_receive)
                  ...
                  CASE(1)
                    ...
                    END IF
                  CASE DEFAULT
                    CALL cp__b("mp2_direct_method.F",1074,"")
                END SELECT
            ...
            ELSE
              ...
              SELECT CASE(case_send_receive)
                ...
                CASE DEFAULT
                  CALL cp__b("mp2_direct_method.F",1103,"")
              END SELECT
            END IF
...
! *****************************************************************************
  SUBROUTINE transform_occupied_orbitals_second(dimen,latom,katom,lset,kset,&
    ...
    INTEGER :: case_send_receive, elements_ij_proc_rec, handle, i, &
      ...
      Rsize_rec, s, S_global, S_offset, S_offset_rec, Ssize_rec
    REAL(KIND=dp), &
        ...
        IF(index_proc_ij<=Ni_occupied*Nj_occupied) THEN
         ...
         SELECT CASE(case_send_receive)
           ...
           CASE(1)
             ...
             END IF
           CASE DEFAULT
             CALL cp__b("mp2_direct_method.F",1439,"")
         END SELECT
...
! *****************************************************************************
  SUBROUTINE transform_occupied_orbitals_second_big(dimen,latom,katom,lset,kset,&
    ...
    DO index_proc_shift=0, para_env%num_pe-1
      ...
      CASE DEFAULT
        CALL cp__b("mp2_direct_method.F",1612,"")
      END SELECT
