# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_linpot.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_linpot.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param_linpot.F" 2
...
! *****************************************************************************
  SUBROUTINE pao_param_init_linpot(pao, qs_env, reuse_matrix_X)
    ...
    IF(dft_control%nspins /= 1) CALL cp__b("pao_param_linpot.F",75,"open shell not yet implemented")
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(n*n == SIZE(block_V_terms,1)))CALL cp__a("pao_param_linpot.F",119)
...
! *****************************************************************************
  SUBROUTINE pao_calc_U_linpot(pao, iatom, penalty, min_eval, block_X, block_U, block_M1, block_G)
    ...
    IF(MAXVAL(ABS(block_V-TRANSPOSE(block_V))/MAX(1.0_dp,MAXVAL(ABS(block_V)))) > 1e-12)&
        CALL cp__b("pao_param_linpot.F",195,"block_V not symmetric")
    ...
    ELSE  ! TURNING POINT (if calc grad) --------------------------------------
       ...
       IF(.NOT.(PRESENT(block_M1)))CALL cp__a("pao_param_linpot.F",203)
       ALLOCATE(vec_M2(n*n))
       ...
       IF(MAXVAL(ABS(block_M2-TRANSPOSE(block_M2)))>1e-14_dp) CALL cp__b("pao_param_linpot.F",207,"Expect symmetric matrix")
       IF(SIZE(block_V_terms)>0)& ! protect against corner-case of zero pao parameters
  ...
  SUBROUTINE linpot_count_terms(pao, qs_env, iatom, nterms)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'linpot_count_terms', &
     ...
     SELECT CASE(pao%parameterization)
      ...
      CASE DEFAULT
        CALL cp__b("pao_param_linpot.F",241,"unkown parameterization")
     END SELECT
  ...
  SUBROUTINE linpot_calc_term(pao, qs_env, iatom, kterm, block_V)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'linpot_calc_term', &
     ...
     SELECT CASE(pao%parameterization)
      ...
      CASE DEFAULT
        CALL cp__b("pao_param_linpot.F",270,"unkown parameterization")
     END SELECT
...
! *****************************************************************************
  SUBROUTINE linepot_initial_guess(pao, qs_env)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(ASSOCIATED(block_V_terms)))CALL cp__a("pao_param_linpot.F",309)
       ...
       IF(.NOT.(ASSOCIATED(block_H0) .AND. ASSOCIATED(block_ks)))CALL cp__a("pao_param_linpot.F",315)
...
! *****************************************************************************
   SUBROUTINE dgelsy_wrapper(matrix_A, vec_B, vec_X)
    ...
    IF(SIZE(matrix_A, 1) /= SIZE(vec_B)) CALL cp__b("pao_param_linpot.F",364,"DGELSY: matrix_A and vec_B not compatible")
    IF(SIZE(matrix_A, 2) /= SIZE(vec_X)) CALL cp__b("pao_param_linpot.F",365,"DGELSY: matrix_A and vec_X not compatible")
    m = SIZE(matrix_A, 1)
    n = SIZE(matrix_A, 2)
    lda = m
    ldb = MAX(m,n)
    ...
    IF (info /= 0) CALL cp__b("pao_param_linpot.F",387,"DGELSY work space query failed")
    ...
    CALL DGELSY(m, n, nrhs, A(1,1), lda, B(1), ldb, jpvt, rcon, rank, work(1), lwork, info)
    IF (info /= 0) CALL cp__b("pao_param_linpot.F",396,"DGELSY optimization failed")
