# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_init.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_init.F"
!-----------------------------------------------------------------------------!
...
MODULE qmmm_init
  ...
  USE input_constants,                 ONLY: &
       RADIUS_QMMM_DEFAULT, calc_always, calc_once, do_qmmm_gauss, &
       do_qmmm_image_calcmatrix, do_qmmm_image_iter, do_qmmm_image_mme, &
       do_qmmm_link_gho, do_qmmm_link_imomm, do_qmmm_link_pseudo, &
       do_qmmm_pcharge, do_qmmm_swave
...
# 73 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_init.F" 2
...
! *****************************************************************************
 SUBROUTINE assign_mm_charges_and_radius(subsys, charges, mm_atom_chrg,mm_el_pot_radius,&
    ...
    INTEGER                                  :: I, ilink, IndMM, IndShell, &
                                                ishell
    LOGICAL                                  :: is_shell
 ...
 SUBROUTINE qmmm_init_periodic_potential(qmmm_env_qm, qm_cell_small, mm_cell, para_env, qs_env,&
   ...
   IF (qmmm_env_qm%periodic) THEN
      ...
      IF(dft_control%qs_control%semi_empirical) THEN
         CALL cp__b("qmmm_init.F",563,"QM/MM periodic calculations not implemented for semi empirical methods")
      ...
      ELSE IF(dft_control%qs_control%scptb) THEN
         CALL cp__b("qmmm_init.F",569,"QM/MM periodic calculations not implemented for SCPTB method")
      ELSE
...
! *****************************************************************************
  SUBROUTINE setup_qmmm_vars_qm( qmmm_section, qmmm_env, subsys_mm, qm_atom_type,&
    ...
    IF (explicit) THEN
       ...
       IF(.NOT.(SIZE(tmp_sph_cut)==2))CALL cp__a("qmmm_init.F",698)
       ...
       ELSE
          ...
          IF(tmp_radius<=0.0_dp)&
             CALL cp_abort(cp__l("qmmm_init.F",706),&
    ...
    ELSE
     CALL cp__b("qmmm_init.F",784,"QMMM section not present in input file!")
    ENDIF
    !
    ! Build MM atoms list
    !
    ...
    DO i = 1, SIZE(subsys_mm%particles%els)
       ...
       END IF
    END DO
    IF(.NOT.(icount==size_mm_system))CALL cp__a("qmmm_init.F",807)
    IF (ASSOCIATED(mm_link_atoms)) THEN
    ...
    IF (qmmm_env%image_charge) THEN
       ...
       IF (qmmm_env%image_charge_pot%image_matrix_method .EQ. do_qmmm_image_mme) THEN
         eri_mme_section => section_vals_get_subs_vals(image_charge_section, "ERI_MME")
         CALL eri_mme_init_read_input(eri_mme_section, qmmm_env%image_charge_pot%eri_mme_param)
         CALL eri_mme_set_params(qmmm_env%image_charge_pot%eri_mme_param, &
                                 qm_cell_small, &
    ...
    IF (explicit) THEN
       DO ikind = 1, nlinks
          ...
          IF (PRESENT(qm_atom_index)) THEN
             ...
             IF(.NOT.(ALL(qm_atom_index/=mm_index)))CALL cp__a("qmmm_init.F",1118)
       ...
       END DO
    END IF
    IF(.NOT.(num_qm_atom_tot-1==SIZE(qm_atom_index)))CALL cp__a("qmmm_init.F",1136)
...
! *****************************************************************************
  SUBROUTINE  setup_qmmm_links(qmmm_section, qmmm_links, mm_el_pot_radius, mm_el_pot_radius_corr,&
    ...
    IF(.NOT.(nlinks /= 0))CALL cp__a("qmmm_init.F",1178)
    DO ikind= 1, nlinks
    ...
    IF(.NOT.(n_tot /= 0))CALL cp__a("qmmm_init.F",1186)
    ALLOCATE(qmmm_links)
...
! *****************************************************************************
  SUBROUTINE move_or_add_atoms(qmmm_section, move_mm_charges, add_mm_charges, &
    ...
    qmmm_link_section => section_vals_get_subs_vals(qmmm_section,"LINK")
    CALL section_vals_get(qmmm_link_section,n_repetition=nlinks)
    IF(.NOT.(nlinks /= 0))CALL cp__a("qmmm_init.F",1305)
    icount     = 0
    n_move_tot = 0
    n_add_tot  = 0
    DO ikind= 1, nlinks
...
! *****************************************************************************
   SUBROUTINE set_add_set_type(added_charges, icount, Index1, Index2, alpha, radius, c_radius, charge,&
    ...
    REAL(KIND=dp)                            :: my_c_radius, my_charge, &
     ...
     IF(.NOT.(i<=SIZE(mm_atom_index)))CALL cp__a("qmmm_init.F",1437)
...
! This is the vector that corrects position to apply properly the PBC
     ...
     IF(.NOT.(ALL(tmp>0)))CALL cp__a("qmmm_init.F",1482)
...
! *****************************************************************************
   ...
   IF(.NOT.(SIZE(qmmm_env%image_charge_pot%image_mm_list) /= 0))CALL cp__a("qmmm_init.F",1557)
   ...
   IF(.NOT.(imageind_in_range))CALL cp__a("qmmm_init.F",1560)

   DO i=1,num_image_mm_atom
      atom_a=qmmm_env%image_charge_pot%image_mm_list(i)
      IF(ANY(qm_atom_index==atom_a)) THEN
         CALL cp__b("qmmm_init.F",1565,"Image atom list must only contain MM atoms")
      ENDIF
      DO j=i+1,num_image_mm_atom
         atom_b=qmmm_env%image_charge_pot%image_mm_list(j)
         IF(atom_a==atom_b) &
         CALL cp__b("qmmm_init.F",1570,"There are atoms doubled in image list.")
      ENDDO
   ENDDO
   ...
   num_const_atom = 0
   fix_xyz = .TRUE.
   IF(ASSOCIATED(subsys_mm%molecule_kinds_new)) THEN 
     IF(ASSOCIATED(subsys_mm%molecule_kinds_new%els)) THEN 
       molecule_kind => subsys_mm%molecule_kinds_new%els
