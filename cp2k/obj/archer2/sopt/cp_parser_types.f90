# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_types.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_types.F" 2
...
!> \author fawzi
! *****************************************************************************
  SUBROUTINE parser_retain(parser)
    TYPE(cp_parser_type), POINTER            :: parser

    CHARACTER(len=*), PARAMETER :: routineN = 'parser_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_types.F",112)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_types.F",113)
...
! *****************************************************************************
  SUBROUTINE parser_release(parser)
    ...
    IF (ASSOCIATED(parser)) THEN
       IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_types.F",131)
       parser%ref_count=parser%ref_count-1
...
! *****************************************************************************
  SUBROUTINE parser_create(parser, file_name, unit_nr, para_env, end_section_label,&
       ...
       initial_variables, apply_preprocessing)
    TYPE(cp_parser_type), POINTER            :: parser
    ...
    IF(.NOT.(.NOT.ASSOCIATED(parser)))CALL cp__a("input/cp_parser_types.F",189)
    ALLOCATE(parser)
    ...
    IF (parser%para_env%mepos==parser%para_env%source) THEN
       ...
       ELSE
          IF(.NOT.PRESENT(file_name))&
             CALL cp__b("input/cp_parser_types.F",240,"at least one of filename and unit_nr must be present")
...
! *****************************************************************************
  SUBROUTINE parser_reset(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_types.F",282)
    ! Rewind units
