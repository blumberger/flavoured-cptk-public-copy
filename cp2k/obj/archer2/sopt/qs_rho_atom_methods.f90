# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_atom_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_atom_methods.F"
!-----------------------------------------------------------------------------!
...
# 68 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_atom_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE calculate_rho_atom(para_env,rho_atom_set,qs_kind,atom_list,&
    ...
    INTEGER :: damax_iso_not0_local, dbmax_iso_not0_local, dmax_iso_not0, &
      ...
      iso, iso1, iso1_first, iso1_last, iso2, iso2_first, iso2_last, isom1, &
      isom2, istat, j, l, l1, l2, l_iso, l_sub, l_sum, lmax12, &
      lmax_expansion, lmin12, m1, m1s, m2, m2s, max_iso_not0, &
      max_iso_not0_local, max_s_harm, maxl, maxso, mepos, n1s, n2s, nr, nset, &
      num_pe, size1, size2
...
!   Distribute the atoms of this kind
    ...
    ALLOCATE(cg_list(2,nsoset(maxl)**2,max_s_harm),cg_n_list(max_s_harm),&
             ...
             STAT=istat)
    IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",182)
    ...
    DO iset1 = 1,nset
        ...
        DO iset2 = 1,nset
           ...
           IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_rho_atom_methods.F",202)
           ...
           DO ipgf1  = 1,npgf(iset1)
              ...
              IF(.NOT.(size1==i1))CALL cp__a("qs_rho_atom_methods.F",216)
              ...
              DO ipgf2 = 1,npgf(iset2)
                 ...
                 IF(.NOT.(size2==i2))CALL cp__a("qs_rho_atom_methods.F",229)
                 ...
                 IF(lmin12 .LE. lmax_expansion) THEN
                    ...
                    DO iso = 1,max_iso_not0_local
                       ...
                       DO icg = 1,cg_n_list(iso)
                          ...
                          IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_rho_atom_methods.F",283)
                    ...
                    DO i=1,nspins
                       ...
                       DO iso = 1,max_iso_not0_local
                          ...
                          DO icg = 1,cg_n_list(iso)
                             ...
                             l = indso(1,iso1) + indso(1,iso2)
                             IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_rho_atom_methods.F",335)
                       ...
                       DO iso = 1,max_iso_not0 !damax_iso_not0_local
                          ...
                          DO icg = 1,dacg_n_list(iso)
                             ...
                             l = indso(1,iso1) + indso(1,iso2)
                             IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_rho_atom_methods.F",388)
                             DO j = 1,3
                       ...
                       DO iso = 1,max_iso_not0!dbmax_iso_not0_local
                          ...
                          DO icg = 1,dbcg_n_list(iso)
                             ...
                             l = indso(1,iso1) + indso(1,iso2)
                             IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_rho_atom_methods.F",412)
...
! *****************************************************************************
  SUBROUTINE calculate_rho_atom_coeff(qs_env, rho_ao, rho_atom_set)
    ...
    LOGICAL                                  :: dista, distab, distb, found, &
                                                paw_atom
    ...
    ALLOCATE (PC1(max_nsgf*max_gau),CPC(max_gau*max_gau),p_matrix(max_nsgf,max_nsgf),&
              STAT=istat)
    IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",565)
...
!   Inizialize to 0 the CPC coefficients and the local density arrays
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF(nimages > 1) THEN
          ...
          IF(.NOT.(img > 0))CALL cp__a("qs_rho_atom_methods.F",625)
       ELSE
          img = 1
       END IF
    ...
    DEALLOCATE(PC1,CPC,p_block_spin,p_matrix,kind_of,basis_set_list)
...
! *****************************************************************************
  SUBROUTINE init_rho_atom(qs_env,gapw_control)
    ...
    INTEGER :: handle, ikind, il, iso, iso1, iso2, l1, l1l2, l2, la, lc1, &
      lc2, lcleb, ll, llmax, lmax_sphere, lp, m1, m2, max_s_harm, max_s_set, &
      maxl, maxlgto, maxs, mm, mp, na, nat, natom, nr, nspins, quadrature
    ...
    DO ikind = 1, SIZE(atomic_kind_set)
       ...
       DO iat = 1,nat
          ...
          ALLOCATE (rho_atom_set(iatom)%cpc_h(nspins), &
                    ...
                    STAT=istat)
          IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1008)
          ...
          IF (paw_atom) THEN
             DO ispin = 1,nspins
                ...
                ALLOCATE(rho_atom_set(iatom)%cpc_h(ispin)%r_coef(1:nsatbas,1:nsatbas), &
                         ...
                         STAT=istat)
                IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1023)
       ...
       DO iat = bo(1), bo(2)
          ...
          ALLOCATE (rho_atom_set(iatom)%ga_Vlocal_gb_h(nspins), &
                    ...
                    STAT=istat)
          IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1055)
          IF (paw_atom) THEN
             DO ispin = 1,nspins
  ...
  SUBROUTINE allocate_rho_atom_rad(rho_atom_set,iatom,ispin,nr,max_iso_not0)
    ...
    CALL timeset(routineN,handle)
         ...
         ALLOCATE(rho_atom_set(iatom)%rho_rad_h(ispin)%r_coef(1:nr,1:max_iso_not0), &
                  ...
                  STAT=istat)
         IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1110)
         ...
         ALLOCATE(rho_atom_set(iatom)%drho_rad_h(ispin)%r_coef(nr,max_iso_not0),&
                  ...
                  STAT=istat)
         IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1120)
         ...
         DO j = 1,3
           ALLOCATE(rho_atom_set(iatom)%trho_rad_h(j,ispin)%r_coef(nr,max_iso_not0),&
                    ...
                    STAT=istat)
           IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1128)
         ...
         DO j = 1,3
            ...
            ALLOCATE(rho_atom_set(iatom)%rho_rad_h_d(j,ispin)%r_coef(nr,max_iso_not0), &
                     ...
                     STAT=istat)
            IF(.NOT.(istat==0))CALL cp__a("qs_rho_atom_methods.F",1139)
