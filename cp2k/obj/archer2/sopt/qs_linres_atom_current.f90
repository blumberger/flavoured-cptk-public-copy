# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_atom_current.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_atom_current.F"
!-----------------------------------------------------------------------------!
...
# 69 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_atom_current.F" 2
...
! *****************************************************************************
  SUBROUTINE calculate_jrho_atom_coeff(qs_env,current_env,mat_d0,mat_jp,mat_jp_rii,&
    ...
    INTEGER :: bo(2), handle, iac, iat, iatom, ibc, idir2, ii, iii, ikind, &
      ispin, jatom, jkind, kac, katom, kbc, kkind, max_gau, max_nsgf, mepos, &
      n_cont_a, n_cont_b, nat, natom, nkind, nsatbas, nsgfa, nsgfb, nso, &
      nsoctot, nspins, num_pe, output_unit
    INTEGER, DIMENSION(3)                    :: cell_b
    INTEGER, DIMENSION(:), POINTER           :: atom_list, list_a, list_b
    LOGICAL :: den_found, dista, distab, distb, is_not_associated, paw_atom, &
    ...
    !
    NULLIFY(atom_list,atomic_kind_set,qs_kind_set,dft_control,orb_basis_set,sab_all,&
    ...
    !
    IF(.NOT.(ASSOCIATED(oce)))CALL cp__a("qs_linres_atom_current.F",167)
    !
    CALL get_current_env(current_env=current_env,&
                     jrho1_atom_set=jrho1_atom_set)
    !
    IF(.NOT.(ASSOCIATED(jrho1_atom_set)))CALL cp__a("qs_linres_atom_current.F",172)
    !
...
! *****************************************************************************
  SUBROUTINE calculate_jrho_atom_rad(qs_env,current_env,idir)
    ...
    INTEGER :: damax_iso_not0, damax_iso_not0_local, handle, i1, i2, iat, &
      ...
      iso1_first, iso1_last, iso2, iso2_first, iso2_last, ispin, l, l_iso, &
      llmax, lmax12, lmax_expansion, lmin12, m1s, m2s, m_iso, max_iso_not0, &
      max_iso_not0_local, max_max_iso_not0, max_nso, max_s_harm, maxl, &
      maxlgto, maxso, mepos, n1s, n2s, na, natom, natom_tot, nkind, nr, nset, &
...
!
    ...
    !
    DO ikind = 1,nkind
       NULLIFY(atom_list,grid_atom,harmonics,orb_basis_set,&
       ...
       CASE DEFAULT
          CALL cp__b("qs_linres_atom_current.F",698,"Unknown gauge, try again...")
       END SELECT
       !
       !
       m1s = 0
       DO iset1 = 1,nset
          ...
          DO iset2 = 1,nset
             ...
             IF(.NOT.(max_iso_not0_local.LE.max_iso_not0))CALL cp__a("qs_linres_atom_current.F",708)
             ...
             IF(.NOT.(damax_iso_not0_local.LE.damax_iso_not0))CALL cp__a("qs_linres_atom_current.F",711)
             ...
             DO ipgf1 = 1,npgf(iset1)
                ...
                IF(.NOT.(size1==i1))CALL cp__a("qs_linres_atom_current.F",721)
                ...
                DO ipgf2 = 1,npgf(iset2)
                   ...
                   IF(.NOT.(size2==i2))CALL cp__a("qs_linres_atom_current.F",734)
                         ...
                         DO iso = 1,max_iso_not0_local
                            ...
                            DO icg = 1,cg_n_list(iso)
                               ...
                               IF(.NOT.(iso2>0.AND.iso1>0))THEN
                                  ...
                                  WRITE(*,*) '.... will stop!'
                               ENDIF
                               IF(.NOT.(iso2>0.AND.iso1>0))CALL cp__a("qs_linres_atom_current.F",884)
                               !
                               ...
                               IF(l.GT.lmax_expansion.OR.l.LT..0) THEN
                                  ...
                                  WRITE(*,*) '.... will stop!'
                               ENDIF
                               IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_linres_atom_current.F",892)
                               !------------------------------------------------------------------
                               ! P0
                               !
                         ...
                         DO iso = 1,damax_iso_not0_local
                            ...
                            DO icg = 1,dacg_n_list(iso)
                               ...
                               IF(.NOT.(iso2>0.AND.iso1>0))THEN
                                  ...
                                  WRITE(*,*) '.... will stop!'
                               ENDIF
                               IF(.NOT.(iso2>0.AND.iso1>0))CALL cp__a("qs_linres_atom_current.F",998)
                               !
                               l = indso(1,iso1) + indso(1,iso2)
                               IF(l.GT.lmax_expansion) THEN
                                  ...
                                  WRITE(*,*) '.... will stop!'
                               ENDIF
                               IF(.NOT.(l<=lmax_expansion))CALL cp__a("qs_linres_atom_current.F",1006)
                               !------------------------------------------------------------------
                               ! Daij
                               !
                               ! Hard term
...
!
    NULLIFY(Fr_h,Fr_s,Fr_a_h,Fr_a_s,Fr_a_h_ii,Fr_a_s_ii,Fr_a_h_iii,Fr_a_s_iii,&
            ...
            a,slm)
    !
    IF(.NOT.(ASSOCIATED(jrho_h)))CALL cp__a("qs_linres_atom_current.F",1144)
    IF(.NOT.(ASSOCIATED(jrho_s)))CALL cp__a("qs_linres_atom_current.F",1145)
    IF(.NOT.(ASSOCIATED(jrho1_atom)))CALL cp__a("qs_linres_atom_current.F",1146)
    ! just to be sure...
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h)))CALL cp__a("qs_linres_atom_current.F",1148)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s)))CALL cp__a("qs_linres_atom_current.F",1149)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h)))CALL cp__a("qs_linres_atom_current.F",1150)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s)))CALL cp__a("qs_linres_atom_current.F",1151)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1152)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1153)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1154)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1155)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h_ii)))CALL cp__a("qs_linres_atom_current.F",1156)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s_ii)))CALL cp__a("qs_linres_atom_current.F",1157)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h_ii)))CALL cp__a("qs_linres_atom_current.F",1158)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s_ii)))CALL cp__a("qs_linres_atom_current.F",1159)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h_ii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1160)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s_ii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1161)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h_ii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1162)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s_ii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1163)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h_iii)))CALL cp__a("qs_linres_atom_current.F",1164)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s_iii)))CALL cp__a("qs_linres_atom_current.F",1165)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h_iii)))CALL cp__a("qs_linres_atom_current.F",1166)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s_iii)))CALL cp__a("qs_linres_atom_current.F",1167)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_h_iii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1168)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_a_s_iii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1169)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_h_iii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1170)
    IF(.NOT.(ASSOCIATED(jrho1_atom%jrho_b_s_iii(ispin)%r_coef)))CALL cp__a("qs_linres_atom_current.F",1171)
    !
    !
    nr = grid_atom%nr
    na = grid_atom%ng_sphere
