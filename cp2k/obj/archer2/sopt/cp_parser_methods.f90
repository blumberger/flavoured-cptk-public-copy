# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_methods.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_methods.F" 2
...
! *****************************************************************************
  FUNCTION parser_location(parser) RESULT(res)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",76)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",77)
...
! *****************************************************************************
  SUBROUTINE parser_store_status(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",103)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",104)
    IF(.NOT.(ASSOCIATED(parser%status)))CALL cp__a("input/cp_parser_methods.F",105)
...
! *****************************************************************************
  SUBROUTINE parser_retrieve_status(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",130)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",131)
...
! *****************************************************************************
  SUBROUTINE parser_read_line(parser,nline,at_end)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",173)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",174)
       ...
       IF (istat /= 0) THEN
          IF (istat < 0) THEN ! EOF/EOR is negative other errors positive
             IF (PRESENT(at_end)) THEN
                at_end = .TRUE.
             ELSE
                CALL cp__b("input/cp_parser_methods.F",187,"Unexpected EOF"//TRIM(parser_location(parser)))
             END IF
             parser%icol  = -1
             parser%icol1 =  0
             parser%icol2 = -1
          ELSE
             CALL cp_abort(cp__l("input/cp_parser_methods.F",193),&
...
! *****************************************************************************
  SUBROUTINE parser_read_line_low(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",267)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",268)
...
! *****************************************************************************
  SUBROUTINE broadcast_input_information(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",370)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",371)
...
! *****************************************************************************
  SUBROUTINE parser_get_next_line(parser,nline,at_end)
    ...
    IF (nline > 0) THEN
       ...
       ELSE
          IF (my_at_end) THEN
             CALL cp__b("input/cp_parser_methods.F",470,"Unexpected EOF"//TRIM(parser_location(parser)))
          END IF
       END IF
...
! *****************************************************************************
  SUBROUTINE parser_skip_space(parser)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",494)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",495)
       ...
       IF (.NOT.is_whitespace(parser%input_line(parser%icol:parser%icol))) THEN
          ...
          IF (parser%input_line(parser%icol:parser%icol) == parser%continuation_character) THEN
             ...
             ELSE
                parser%icol1 = i
                parser%icol2 = i
                CALL cp_abort(cp__l("input/cp_parser_methods.F",535),&
...
! *****************************************************************************
  SUBROUTINE parser_next_token(parser,string_length)
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",572)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",573)
    ...
    IF (length > 0) THEN
       ...
       IF(parser%icol==-1)&
          CALL cp__b("input/cp_parser_methods.F",583,"Unexpectetly reached EOF"//TRIM(parser_location(parser)))
       ...
       DO
          ...
          IF (parser%input_line(parser%icol:parser%icol) == &
             ...
             ELSE
                parser%icol1 = i
                parser%icol2 = i
                CALL cp_abort(cp__l("input/cp_parser_methods.F",631),&
                     "Found non-blank tokens after the line continuation character '"//&
          ...
          ELSE IF (INDEX(parser%separators,parser%input_line(parser%icol:parser%icol)) > 0) THEN
             ...
             ELSE
                parser%icol1 = parser%icol
                parser%icol2 = parser%icol
                CALL cp_abort(cp__l("input/cp_parser_methods.F",648),&
          ...
          ELSE IF (parser%input_line(parser%icol:parser%icol) == '"') THEN
             ...
             IF (parser%icol2 == parser%icol) THEN
                parser%icol1 = parser%icol
                parser%icol2 = parser%icol
                CALL cp__b("input/cp_parser_methods.F",661,"Unmatched quotation mark found"//TRIM(parser_location(parser)))
             ELSE
...
! *****************************************************************************
  FUNCTION parser_test_next_token(parser,string_length) RESULT(test_result)
    ...
    test_result = ""
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",730)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",731)
...
! *****************************************************************************
  SUBROUTINE parser_search_string(parser,string,ignore_case,found,line,begin_line,&
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",836)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",837)
       ...
       IF (ipattern > 0) THEN
          ...
          IF (PRESENT(line)) THEN
             IF(LEN(line) < LEN_TRIM(parser%input_line)) THEN
                CALL cp_warn(cp__l("input/cp_parser_methods.F",861),&
...
! *****************************************************************************
  SUBROUTINE parser_get_integer(parser,object,newline,skip_lines,&
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",1001)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",1002)
    ...
    ELSE IF (my_at_end) THEN
       CALL cp__b("input/cp_parser_methods.F",1018,"Unexpected EOF"//TRIM(parser_location(parser)))
    END IF
    ...
    ELSE
       ...
       IF (parser%icol1 > parser%icol2) THEN
          parser%icol1 = parser%icol
          parser%icol2 = parser%icol
          CALL cp_abort(cp__l("input/cp_parser_methods.F",1028),&
    ...
    IF (integer_object(parser%input_line(parser%icol1:parser%icol2))) THEN
       ...
       ELSE
          ...
          IF (LEN_TRIM(error_message) > 0) THEN
             CALL cp__b("input/cp_parser_methods.F",1045,TRIM(error_message)//TRIM(parser_location(parser)))
          END IF
       END IF
    ELSE
       CALL cp_abort(cp__l("input/cp_parser_methods.F",1049),&
...
! *****************************************************************************
  SUBROUTINE parser_get_logical(parser,object,newline,skip_lines,&
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",1088)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",1089)
    IF(.NOT.(.NOT.parser%ilist%in_use))CALL cp__a("input/cp_parser_methods.F",1090)
    IF (PRESENT(skip_lines)) THEN
       nline = skip_lines
    ELSE
       nline = 0
    ...
    IF (PRESENT(at_end)) THEN
       ...
       IF (my_at_end) RETURN
    ELSE IF (my_at_end) THEN
       CALL cp__b("input/cp_parser_methods.F",1106,"Unexpected EOF"//TRIM(parser_location(parser)))
    END IF
    ...
    IF (input_string_length == 0) THEN
       parser%icol1 = parser%icol
       parser%icol2 = parser%icol
       CALL cp_abort(cp__l("input/cp_parser_methods.F",1116),&
    ...
    CASE ("1","T",".T.","TRUE",".TRUE.","Y","YES","ON")
       object = .TRUE.
    CASE DEFAULT
       CALL cp_abort(cp__l("input/cp_parser_methods.F",1131),&
...
! *****************************************************************************
  SUBROUTINE parser_get_real(parser,object,newline,skip_lines,string_length,&
    ...
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",1166)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",1167)
    IF(.NOT.(.NOT.parser%ilist%in_use))CALL cp__a("input/cp_parser_methods.F",1168)
    ...
    IF (PRESENT(at_end)) THEN
       ...
       IF (my_at_end) RETURN
    ELSE IF (my_at_end) THEN
       CALL cp__b("input/cp_parser_methods.F",1186,"Unexpected EOF"//TRIM(parser_location(parser)))
    END IF
    ...
    IF (parser%icol1 > parser%icol2) THEN
       parser%icol1 = parser%icol
       parser%icol2 = parser%icol
       CALL cp_abort(cp__l("input/cp_parser_methods.F",1194),&
    ...
    IF (LEN_TRIM(error_message) > 0) THEN
       CALL cp__b("input/cp_parser_methods.F",1202,TRIM(error_message)//TRIM(parser_location(parser)))
    END IF
...
! *****************************************************************************
  SUBROUTINE parser_get_string(parser,object,lower_to_upper,newline,skip_lines,&
    ...
    object = ""
    IF(.NOT.(ASSOCIATED(parser)))CALL cp__a("input/cp_parser_methods.F",1237)
    IF(.NOT.(parser%ref_count>0))CALL cp__a("input/cp_parser_methods.F",1238)
    IF(.NOT.(.NOT.parser%ilist%in_use))CALL cp__a("input/cp_parser_methods.F",1239)
    IF (PRESENT(skip_lines)) THEN
       nline = skip_lines
    ELSE
       nline = 0
    ...
    IF (PRESENT(at_end)) THEN
       ...
       IF (my_at_end) RETURN
    ELSE IF (my_at_end) THEN
       CALL cp__b("input/cp_parser_methods.F",1255,"Unexpected EOF"//TRIM(parser_location(parser)))
    END IF
    ...
    IF (input_string_length <= 0) THEN
       CALL cp_abort(cp__l("input/cp_parser_methods.F",1263),&
            "A string type object was expected, found end of line"//&
            TRIM(parser_location(parser)))
    ELSE IF (input_string_length > LEN(object)) THEN
       CALL cp_warn(cp__l("input/cp_parser_methods.F",1267),&
