# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_env_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_env_types.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_env_types.F" 2
...
! *****************************************************************************
  SUBROUTINE fb_env_retain(fb_env)
    ...
    IF(.NOT.(ASSOCIATED(fb_env%obj)))CALL cp__a("qs_fb_env_types.F",110)
    IF(.NOT.(fb_env%obj%ref_count>0))CALL cp__a("qs_fb_env_types.F",111)
...
! *****************************************************************************
  SUBROUTINE fb_env_release(fb_env)
    ...
    IF (ASSOCIATED(fb_env%obj)) THEN
       IF(.NOT.(fb_env%obj%ref_count>0))CALL cp__a("qs_fb_env_types.F",129)
       fb_env%obj%ref_count = fb_env%obj%ref_count - 1
...
! *****************************************************************************
  SUBROUTINE fb_env_create(fb_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(fb_env%obj)))CALL cp__a("qs_fb_env_types.F",216)
    ALLOCATE(fb_env%obj)
...
! *****************************************************************************
  SUBROUTINE fb_env_init(fb_env)
    ...
    IF(.NOT.(ASSOCIATED(fb_env%obj)))CALL cp__a("qs_fb_env_types.F",246)
    IF (ASSOCIATED(fb_env%obj%rcut)) THEN
       DEALLOCATE(fb_env%obj%rcut)
    END IF
    CALL fb_atomic_halo_list_release(fb_env%obj%atomic_halos)
...
! *****************************************************************************
  SUBROUTINE fb_env_get(fb_env, &
    ...
    IF(.NOT.(ASSOCIATED(fb_env%obj)))CALL cp__a("qs_fb_env_types.F",309)
    IF(.NOT.(fb_env%obj%ref_count>0))CALL cp__a("qs_fb_env_types.F",310)
    IF (PRESENT(rcut)) &
...
! *****************************************************************************
  SUBROUTINE fb_env_set(fb_env, &
    ...
    IF(.NOT.(ASSOCIATED(fb_env%obj)))CALL cp__a("qs_fb_env_types.F",373)
    ...
    IF (PRESENT(atomic_halos)) THEN
       IF (fb_atomic_halo_list_has_data(atomic_halos)) &
            CALL fb_atomic_halo_list_retain(atomic_halos)
       CALL fb_atomic_halo_list_release(fb_env%obj%atomic_halos)
