# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_ks_matrix.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_ks_matrix.F"
!-----------------------------------------------------------------------------!
...
# 100 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_ks_matrix.F" 2
...
! *****************************************************************************
  SUBROUTINE build_scptb_ks_matrix(qs_env,calculate_forces,just_energy)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("scptb_ks_matrix.F",160)
    ...
    IF(.NOT.(SIZE(ks_matrixkp,1)>0))CALL cp__a("scptb_ks_matrix.F",174)
    IF(.NOT.(SIZE(ks_matrixkp,2)>0))CALL cp__a("scptb_ks_matrix.F",175)

    scf_section => section_vals_get_subs_vals(qs_env%input,"DFT%SCF")
    nspins=dft_control%nspins
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("scptb_ks_matrix.F",179)
    ...
    IF(.NOT.(ASSOCIATED(matrixkp_h)))CALL cp__a("scptb_ks_matrix.F",198)
    DO ispin=1,nspins
    ...
    IF (qs_env%requires_mo_derivs .AND. .NOT. just_energy) THEN
       ...
       DO ispin=1,SIZE(mo_derivs)
          ...
          IF(.NOT.mo_array(ispin)%mo_set%use_mo_coeff_b) THEN
             CALL cp__b("scptb_ks_matrix.F",271,"")
          ENDIF
  ...
  SUBROUTINE tb_coulomb(qs_env,ks_matrix,matrix_s,rho,mcharge,calculate_forces,just_energy)
    ...
    IF ( chabs > 1.e-8_dp ) THEN
       IF(scptb_control%do_ewald) THEN
          ...
          CASE DEFAULT
            CALL cp__b("scptb_ks_matrix.F",393,"Invalid Ewald type")
          CASE(do_ewald_none)
            CALL cp__b("scptb_ks_matrix.F",395,"Not allowed with SCPTB")
          CASE(do_ewald_ewald)
            CALL cp__b("scptb_ks_matrix.F",397,"Standard Ewald not implemented in SCPTB")
          CASE(do_ewald_pme)
            CALL cp__b("scptb_ks_matrix.F",399,"PME not implemented in SCPTB")
          CASE(do_ewald_spme)
          ...
          IF ( .NOT. just_energy ) THEN
            ...
            DO WHILE (neighbor_list_iterate(nl_iterator)==0)
               ...
               ELSE
                  ...
                  IF(.NOT.(ic > 0))CALL cp__a("scptb_ks_matrix.F",452)
               END IF
          ...
          IF(.NOT.just_energy.OR.calculate_forces) THEN
            ...
            DO WHILE (neighbor_list_iterate(nl_iterator)==0)
               ...
               ELSE
                  ic = cell_to_index(cellind(1),cellind(2),cellind(3))
                  IF(.NOT.(ic > 0))CALL cp__a("scptb_ks_matrix.F",593)
               END IF
               ...
               IF(.NOT.just_energy) THEN
                  CALL cp_dbcsr_get_block_p(matrix=matrix_s(1,ic)%matrix,&
                       row=irow,col=icol,block=sblock,found=found)
                  IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",600)
                  DO is=1,nsize
                     NULLIFY(ksblock)
                     CALL cp_dbcsr_get_block_p(matrix=ks_matrix(is,ic)%matrix,&
                          row=irow,col=icol,block=ksblock,found=found)
                     IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",605)
               ...
               IF ( calculate_forces .AND. iatom /= jatom ) THEN
                  ...
                  CALL cp_dbcsr_get_block_p(matrix=matrix_p(1,ic)%matrix,&
                       row=irow,col=icol,block=pblock,found=found)
                  IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",617)
                  DO i=1,3
                     NULLIFY(dsblock)
                     CALL cp_dbcsr_get_block_p(matrix=matrix_s(1+i,ic)%matrix,&
                          row=irow,col=icol,block=dsblock,found=found)
                     IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",622)
       ...
       ELSE
         ! direct sum
         IF (nimg > 1) THEN
            CALL cp__b("scptb_ks_matrix.F",663,"SCPTB direct sum electrostatic cannot be used with k-points.")
         END IF
         IF(.NOT.just_energy .OR. calculate_forces) THEN
            ALLOCATE(qcharge(natom))
            qcharge = 0._dp
            ...
            DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
               ...
               IF(.NOT.just_energy) THEN
                  DO is=1,nsize
                     ...
                     IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",742)
                     ksblock = ksblock - 0.5_dp*dq*sblock
                  END DO
               END IF
               IF ( calculate_forces ) THEN
...
! *****************************************************************************
  SUBROUTINE scp_coulomb(qs_env,ks_matrixkp,matrixkp_s,rho,mcharge,&
    ...
    IF(do_kpoints) THEN
       CALL cp__b("scptb_ks_matrix.F",857,"SCPTB not implemented with k-points.")
    END IF
       ...
       DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          IF ( calculate_forces .AND. iatom /= jatom ) THEN
             ...
             CALL cp_dbcsr_get_block_p(matrix=matrix_p(1)%matrix,&
                  row=iatom,col=jatom,block=pblock,found=found)
             IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",1012)
             ...
             DO i=1,3
                ...
                CALL cp_dbcsr_get_block_p(matrix=matrix_s(1+i)%matrix,&
                     row=iatom,col=jatom,block=dsblock,found=found)
                IF(.NOT.(found))CALL cp__a("scptb_ks_matrix.F",1021)
                fi = dq*SUM(pblock*dsblock)
