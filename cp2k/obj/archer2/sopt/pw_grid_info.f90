# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grid_info.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grid_info.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grid_info.F" 2
...
! *****************************************************************************
  FUNCTION pw_grid_init_setup ( hmat, cutoff, spherical, odd, fft_usage, ncommensurate,&
    ...
    IF (ncommensurate>0) THEN
       ...
       IF(.NOT.(icommensurate>0))CALL cp__a("pw/pw_grid_info.F",63)
       IF(.NOT.(icommensurate<=ncommensurate))CALL cp__a("pw/pw_grid_info.F",64)
    ELSE
       my_icommensurate=0
    END IF

    IF (my_icommensurate>1) THEN
       IF(.NOT.(PRESENT(ref_grid)))CALL cp__a("pw/pw_grid_info.F",70)
       n=ref_grid%npts/2**(my_icommensurate-1)
       IF(.NOT.(ALL(ref_grid%npts==n*2**(my_icommensurate-1))))CALL cp__a("pw/pw_grid_info.F",72)
       IF(.NOT.(ALL(pw_grid_n_for_fft(n)==n)))CALL cp__a("pw/pw_grid_info.F",73)
    ELSE
...
! *****************************************************************************
  FUNCTION pw_grid_find_n ( hmat, cutoff, fft_usage, spherical, odd, ncommensurate,&
    ...
    INTEGER :: idir, my_icommensurate, my_ncommensurate, nsubgrid, &
...
! is k*2**(ncommensurate-icommensurate)
    ...
    ELSE
       my_icommensurate=0
    ENDIF
    IF(.NOT.(my_icommensurate <= my_ncommensurate))CALL cp__a("pw/pw_grid_info.F",123)
    IF(.NOT.(my_icommensurate > 0 .OR. my_ncommensurate <= 0))CALL cp__a("pw/pw_grid_info.F",124)
    IF(.NOT.(my_ncommensurate >= 0))CALL cp__a("pw/pw_grid_info.F",125)

    IF (PRESENT(n_orig)) THEN
       n = n_orig
    ELSE
       IF(.NOT.(cutoff>0.0_dp))CALL cp__a("pw/pw_grid_info.F",130)
    ...
    IF (my_ncommensurate>0) THEN
       DO my_icommensurate=1,my_ncommensurate
          ...
          IF(.NOT.(.NOT. ftest))CALL cp__a("pw/pw_grid_info.F",175)
       END DO
    ENDIF
...
! *****************************************************************************
  FUNCTION pw_grid_n_for_fft(n,odd) RESULT(nout)
    ...
    IF(.NOT.(ALL(n>=0)))CALL cp__a("pw/pw_grid_info.F",203)
    IF (my_odd) THEN
...
! *****************************************************************************
  FUNCTION pw_grid_n_from_cutoff ( hmat, cutoff) RESULT(n)
    ...
    DO i=1,3
       alat(i) = SUM ( hmat(:,i)**2 )
    ENDDO
    IF(.NOT.(ALL(alat/=0._dp)))CALL cp__a("pw/pw_grid_info.F",248)
