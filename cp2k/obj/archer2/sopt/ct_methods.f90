# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ct_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ct_methods.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ct_methods.F" 2
...
!REAL(KIND=dp), DIMENSION(:,:), POINTER :: data_p
    ...
    IF (cts_env%update_q.AND.(.NOT.cts_env%update_p)) THEN
       CALL cp__b("ct_methods.F",94,"q-update is possible only with p-update")
    ENDIF

    IF (cts_env%tensor_type.eq.tensor_up_down) THEN
       CALL cp__b("ct_methods.F",98,"riccati is not implemented for biorthogonal basis")
    ENDIF

    IF (.NOT.ASSOCIATED(cts_env%matrix_ks)) THEN
       CALL cp__b("ct_methods.F",102,"KS matrix is not associated")
    ENDIF

    IF (cts_env%use_virt_orbs.AND.(.NOT.cts_env%use_occ_orbs)) THEN
       CALL cp__b("ct_methods.F",106,"virtual orbs can be used only with occupied orbs")
    ENDIF

    IF (cts_env%use_occ_orbs) THEN
     IF (.NOT.ASSOCIATED(cts_env%matrix_t)) THEN
       CALL cp__b("ct_methods.F",111,"T matrix is not associated")
     ENDIF
     IF (.NOT.ASSOCIATED(cts_env%matrix_qp_template)) THEN
       CALL cp__b("ct_methods.F",114,"QP template is not associated")
     ENDIF
     IF (.NOT.ASSOCIATED(cts_env%matrix_pq_template)) THEN
       CALL cp__b("ct_methods.F",117,"PQ template is not associated")
     ENDIF
    ENDIF

    IF (cts_env%use_virt_orbs) THEN
     IF (.NOT.ASSOCIATED(cts_env%matrix_v)) THEN
       CALL cp__b("ct_methods.F",123,"V matrix is not associated")
     ENDIF
    ELSE
     IF (.NOT.ASSOCIATED(cts_env%matrix_p)) THEN
       CALL cp__b("ct_methods.F",127,"P matrix is not associated")
     ENDIF
    ENDIF

    IF (cts_env%tensor_type.NE.tensor_up_down.AND.&
       cts_env%tensor_type.NE.tensor_orthogonal) THEN
       CALL cp__b("ct_methods.F",133,"illegal tensor flag")
    ENDIF
...
!RZK-warning diagonalization works only with orthogonal tensor!!!
        ...
        IF ( cts_env%max_iter.ge.0 ) THEN
           ...
           IF (cts_env%converged) THEN
             ...
             !ENDIF
           ELSE
             CALL cp__b("ct_methods.F",275,"RICCATI: CG algorithm has NOT converged")
           ENDIF
...
!CALL cp_dbcsr_print(matrix_pp)
        ...
        IF ( cts_env%max_iter.ge.0 ) THEN
           ...
           IF (cts_env%converged) THEN
             ...
             !ENDIF
           ELSE
             CALL cp__b("ct_methods.F",398,"RICCATI: CG algorithm has NOT converged")
           ENDIF
     ...
     CASE DEFAULT
        CALL cp__b("ct_methods.F",421,"illegal preconditioner type")
     END SELECT ! preconditioner type

     IF (cts_env%update_p) THEN
       
        IF (cts_env%tensor_type.eq.tensor_up_down) THEN
           CALL cp__b("ct_methods.F",427,"orbital update is NYI for this tensor type")
        ENDIF
    ...
    ELSE
     CALL cp__b("ct_methods.F",524,"illegal occ option")
    ENDIF
...
! *****************************************************************************
   RECURSIVE SUBROUTINE solve_riccati_equation(pp,qq,qp,pq,oo,vv,x,res,&
    ...
    INTEGER                                  :: handle, istep, iteration, &
                                                nsteps, unit_nr, &
                                                update_prec_freq
    LOGICAL                                  :: prepare_to_exit, present_oo, &
                                                present_vv, quadratic_term, &
                                                restart_conjugator
...
!CALL cp_dbcsr_print(prec)
          ...
          ELSE
             ...
             CASE (cg_zero)
                beta=0.0_dp
             CASE DEFAULT
                CALL cp__b("ct_methods.F",1064,"illegal conjugator")
             END SELECT
...
!CALL cp_dbcsr_release(zero_pq)
          ...
          IF (nsteps.eq.0) THEN
             CALL cp__b("ct_methods.F",1164,"no step sizes!")
          ENDIF
    ...
    DO row = 1, nblkrows_tot
       DO col = 1, nblkcols_tot
          ...
          IF (hold.EQ.mynode) THEN
             ...
             IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("ct_methods.F",1298)
             p_new_block(:,:) = 1.0_dp
          ENDIF ! mynode
       ENDDO
    ENDDO
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF (block_needed) THEN
          ...
          IF (do_eigenvalues) THEN
             ...
             IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("ct_methods.F",1624)
             p_new_block(:,:) = 0.0_dp
