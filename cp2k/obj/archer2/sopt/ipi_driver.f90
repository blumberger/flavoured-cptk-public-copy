# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/ipi_driver.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/ipi_driver.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/ipi_driver.F" 2
...
! access cp2k structures
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("ipi_driver.F",353)
    CALL force_env_get(force_env,para_env=para_env)

    hasdata = .FALSE.
    ionode=(para_env%source==para_env%mepos)
    ...
    driver_loop: DO
      ...
      ELSE IF (TRIM(header) == "POSDATA") THEN              
         ...
         IF (nat/=subsys%particles%n_els) &
                CALL cp__b("ipi_driver.F",448,"@DRIVER MODE: Uh-oh! Particle number mismatch between i-PI and cp2k input!")
         ii=0
