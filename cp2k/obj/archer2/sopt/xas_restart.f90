# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_restart.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_restart.F"
!-----------------------------------------------------------------------------!
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_restart.F" 2
...
! *****************************************************************************
  SUBROUTINE xas_read_restart(xas_env,xas_section,qs_env, xas_method,iatom,estate)
    ...
    INTEGER :: group, handle, i, ia, ie, ispin, nao, nao_read, nelectron, &
      nexc_atoms, nexc_atoms_read, nexc_search, nexc_search_read, nmo, &
      nmo_read, output_unit, rst_unit, source, xas_estate, xas_estate_read, &
      xas_method_read
    LOGICAL                                  :: file_exists
    ...
    IF(file_exists) THEN
      ...
      IF(rst_unit>0) THEN
         ...
         IF(xas_method_read/=xas_method)&
            CALL cp__b("xas_restart.F",185," READ XAS RESTART: restart with different XAS method is not possible. ")
         IF(nexc_atoms_read/=nexc_atoms)&
            CALL cp_abort(cp__l("xas_restart.F",187),&
      ...
      DO ispin = 1, SIZE(mos)
        ...
        IF (para_env%ionode) THEN
          ...
          IF(nao/=nao_read)&
             CALL cp__b("xas_restart.F",208,"To change basis is not possible. ")
          ...
          IF(nmo_read>nmo) THEN
            IF(occupation_numbers(nmo)>=EPSILON(0.0_dp))&
               CALL cp_warn(cp__l("xas_restart.F",218),&
    ...
    DO ispin=1,SIZE(mos)
       IF(ispin==1) THEN
         IF(xas_env%homo_occ == 0) THEN
           ...
           CALL set_mo_set(mos(ispin)%mo_set,nelectron=nelectron)
         END IF
         CALL set_mo_occupation(mo_set=qs_env%mos(ispin)%mo_set, smear=scf_control%smear,&
              xas_env=xas_env)
       ELSE
         CALL set_mo_occupation(mo_set=qs_env%mos(ispin)%mo_set, smear=scf_control%smear)
       END IF
...
! *****************************************************************************
  ...
  SUBROUTINE find_excited_core_orbital(xas_env,mos,matrix_s)
    ...
    IF(.NOT.(ASSOCIATED(excvec_overlap)))CALL cp__a("xas_restart.F",467)
  ...
  ! ** use the maximum overlap criterion to find the index of the excited orbital
    ...
    CALL cp_gemm("T","N",1,xas_env%nexc_search,nao,1.0_dp,excvec_coeff,&
