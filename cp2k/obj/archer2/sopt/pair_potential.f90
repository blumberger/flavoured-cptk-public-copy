# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential.F" 2
       ...
       DO j = 1, i
          ...
          DO k = 1, SIZE(pot%type)
             ...
             CASE DEFAULT
                ...
                CALL cp__b("pair_potential.F",203,"")
             END SELECT
  ...
  SUBROUTINE generate_spline_low(spl_p, npoints, locut, hicut,  eps_spline, &
    ...
    DO WHILE (.TRUE.)
       ...
       IF (npoints > MAX_POINTS) THEN
          ...
          CALL cp_abort(cp__l("pair_potential.F",436),TRIM(message))
       END IF
  ...
  SUBROUTINE get_nonbond_storage(spline_env, potparm, atomic_kind_set, do_zbl, &
    ...
    DO pot_target = MINVAL(list_pot), MAXVAL(list_pot)
       ...
       CASE (nn_type)
          nvar = nvar
       CASE DEFAULT
          CALL cp__b("pair_potential.F",640,"")
       END SELECT
             ...
             DO j = 1, i
                ...
                IF (potparm%pot(i,j)%pot%type(1)==pot_target) THEN
                   ...
                   CASE (nn_type)
                      ! no checks
                   CASE DEFAULT
                      CALL cp__b("pair_potential.F",750,"")
                   END SELECT
  ...
  SUBROUTINE set_potparm_index(potparm, my_index, pot_target, ntype, tmpij_out,&
    ...
    IF ((pot_target==lj_type).OR.(pot_target==lj_charmm_type)) THEN
       ...
       DO i = 1, nvalues
          ...
          IF(.NOT.(check))CALL cp__a("pair_potential.F",967)
    ...
    IF(ANY(potential_single_allocation==pot_target)) THEN
       DO i = 1, nvalues
          ...
          check = SIZE(pot%type)==1
          IF(.NOT.(check))CALL cp__a("pair_potential.F",1014)
