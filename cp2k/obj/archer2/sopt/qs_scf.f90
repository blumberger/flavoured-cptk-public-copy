# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf.F"
!-----------------------------------------------------------------------------!
...
# 139 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf.F" 2
...
! *****************************************************************************
  SUBROUTINE scf(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf.F",177)
...
! *****************************************************************************
  SUBROUTINE scf_env_do_scf(scf_env,scf_control,qs_env,converged,should_stop)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf.F",294)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf.F",295)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf.F",296)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_scf.F",297)
    ...
    IF(test_for_result(results,description=description)) THEN
      ...
      IF(.NOT.(i_tmp.EQ.3))CALL cp__a("qs_scf.F",344)
      IF(ALL(res_val_3(:).LE.0.0))&
         CALL cp_abort(cp__l("qs_scf.F",346),&
    ...
    IF (.NOT. converged) CALL cp__w("qs_scf.F",479,"SCF run NOT converged")
    ...
    DO ispin=1,SIZE(mos)!fm -> dbcsr
       IF(mos(ispin)%mo_set%use_mo_coeff_b) THEN!fm->dbcsr
          IF (.NOT.ASSOCIATED(mos(ispin)%mo_set%mo_coeff_b)) &!fm->dbcsr
             CALL cp__b("qs_scf.F",485,"mo_coeff_b is not allocated")!fm->dbcsr
...
! *****************************************************************************
  SUBROUTINE init_scf_loop(scf_env,qs_env,scf_section)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf.F",540)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf.F",541)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf.F",542)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_scf.F",543)
    ...
    CASE DEFAULT
       ...
       CALL cp__b("qs_scf.F",568,"unknown scf method method:"//cp_to_string(scf_env%method))
    ...
    CASE (ot_diag_method_nr)
       ...
       IF (dft_control%qs_control%dftb .OR. dft_control%qs_control%semi_empirical) THEN
         CALL cp__b("qs_scf.F",599,"DFTB and SE not available with OT/DIAG")
       END IF
    ...
    CASE (ot_method_nr)
       ...
       IF (.NOT.ASSOCIATED(scf_env%qs_ot_env)) THEN
          ...
          IF (dft_control%restricted) THEN
              ...
              IF(.NOT.scf_env%qs_ot_env(1)%settings%do_rotation)&
                 CALL cp_abort(cp__l("qs_scf.F",658),&
          ...
          SELECT CASE(scf_env%qs_ot_env(1)%settings%preconditioner_type)
             ...
             CASE(ot_precond_full_all,ot_precond_full_single_inverse)
                ...
                IF(.NOT.(.NOT.(do_rotation.AND.is_full_all)))CALL cp__a("qs_scf.F",707)
       ...
       DO ispin=1,SIZE(mos)
          IF (.NOT. mos(ispin)%mo_set%uniform_occupation) THEN
             IF(.NOT.(do_rotation))CALL cp__a("qs_scf.F",729)
          ENDIF
       ENDDO
    END SELECT
    ...
    IF (dft_control%low_spin_roks) THEN
       IF(.NOT.(scf_env%method==ot_method_nr))CALL cp__a("qs_scf.F",736)
       do_rotation=scf_env%qs_ot_env(1)%settings%do_rotation
       IF(.NOT.(do_rotation))CALL cp__a("qs_scf.F",738)
    ENDIF
...
! *****************************************************************************
  SUBROUTINE scf_env_cleanup(scf_env)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf.F",765)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf.F",766)
...
! *** method dependent cleanup
   ...
   !
       CASE DEFAULT
          CALL cp__b("qs_scf.F",814,"unknown scf method method:"//cp_to_string(scf_env%method))
       END SELECT
