# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_main.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_main.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_main.F" 2
  ...
  SUBROUTINE pao_update(qs_env,ls_scf_env,pao_is_done)
    ...
    DO WHILE(.TRUE.)
       ...
       IF (should_stop .OR. pao%istep >= pao%max_pao) THEN
          CALL cp__w("pao_main.F",247,"PAO not converged!")
          pao_is_done = .TRUE.
          EXIT
       ENDIF
       ...
       IF(pao%linesearch%step_size < 1e-10_dp) CALL cp__b("pao_main.F",256,"PAO gradient is wrong.")
