# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_image_charge.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_image_charge.F"
!-----------------------------------------------------------------------------!
...
MODULE  qmmm_image_charge
  ...
  USE eri_mme_methods,                 ONLY: eri_mme_2c_integrate
...
# 74 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_image_charge.F" 2
...
! *****************************************************************************
  SUBROUTINE calc_image_coeff_gaussalgorithm(v_hartree_rspace,coeff,qmmm_env,&
    ...
    IF(.NOT.(info==0))CALL cp__a("qmmm_image_charge.F",198)
...
! *****************************************************************************
  SUBROUTINE integrate_potential_devga_rspace(potential,coeff,forces,qmmm_env,&
    ...
    IF (use_virial) THEN
       CALL cp__b("qmmm_image_charge.F",521,"Virial not implemented for image charge method")
    END IF
    ...
    IF(.NOT.qmmm_env%image_charge_pot%coeff_iterative) THEN
      ...
      CASE(calc_once)
          ...
          IF(qmmm_env%center_qm_subsys0)&
             CALL cp_warn(cp__l("qmmm_image_charge.F",615),&
      ...
      CASE DEFAULT
         CALL cp__b("qmmm_image_charge.F",622,"No initialization for image charges available?")
      END SELECT
    ENDIF
  ...
  SUBROUTINE calculate_image_matrix(image_matrix,ipiv,qs_env,qmmm_env)
    ...
    ELSE
       ...
       IF(PRESENT(ipiv)) THEN
          ...
          IF(.NOT.(stat==0))CALL cp__a("qmmm_image_charge.F",709)
       ENDIF
    ENDIF
...
! *****************************************************************************
  SUBROUTINE calculate_image_matrix_gpw(image_matrix, qs_env, qmmm_env)
    ...
    INTEGER                                  :: handle, iatom, iatom_ref, &
                                                natom
    REAL(KIND=dp), DIMENSION(:), POINTER     :: int_res
    TYPE(cp_para_env_type), POINTER          :: para_env
    TYPE(pw_env_type), POINTER               :: pw_env
...
! *****************************************************************************
  SUBROUTINE calculate_image_matrix_mme(image_matrix,qs_env,qmmm_env)
    ...
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: la_max, la_min
    ...
    natom=SIZE(qmmm_env%image_charge_pot%image_mm_list)
    ALLOCATE(la_min(natom),la_max(natom))
    ...
    CALL eri_mme_2c_integrate(qmmm_env%image_charge_pot%eri_mme_param, la_min, la_max, la_min, la_max, &
...
! *****************************************************************************
  SUBROUTINE restart_image_matrix(image_matrix,qs_env,qmmm_env)
    ...
    ELSE
      CALL cp__b("qmmm_image_charge.F",1095,"Restart file for image matrix not found")
    ENDIF
