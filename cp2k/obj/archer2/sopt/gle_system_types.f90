# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/gle_system_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/gle_system_types.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/gle_system_types.F" 2
  ...
  SUBROUTINE gle_init(gle, dt,temp, section)
    ...
    DO ir = 1,n_rep
      ...
      IF(ASSOCIATED(list)) THEN
         DO i = 1,SIZE(list)
            IF(j>gle%ndim) THEN
               CALL cp__b("gle_system_types.F",111,"GLE: Too many elements in A_LIST")
            END IF
    ...
    IF(j<gle%ndim+1) THEN
       CALL cp__b("gle_system_types.F",123,"GLE: Too few elements in A_LIST")
    END IF
    ...
    IF(n_rep>0) THEN
      ...
      DO ir = 1,n_rep
        ...
        IF(ASSOCIATED(list)) THEN
          DO i = 1,SIZE(list)
            IF(j>gle%ndim) THEN
                 CALL cp__b("gle_system_types.F",139,"GLE: Too many elements in C_LIST")
            END IF
      ...
      END DO ! ir
      IF(j<gle%ndim+1) THEN
         CALL cp__b("gle_system_types.F",151,"GLE: Too few elements in C_LIST")
      END IF
    ELSE
      gle%c_mat = 0.0_dp
      DO i = 1,gle%ndim
...
! *****************************************************************************
  SUBROUTINE gle_thermo_create(gle, mal_size)
    ...
    IF(.NOT.(ASSOCIATED(gle)))CALL cp__a("gle_system_types.F",181)
    IF(.NOT.(.NOT.ASSOCIATED(gle%nvt)))CALL cp__a("gle_system_types.F",182)
