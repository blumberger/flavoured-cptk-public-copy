# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_util.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_util.F" 2
...
! *****************************************************************************
  SUBROUTINE topology_reorder_atoms(topology,qmmm,qmmm_env_mm,subsys_section,force_env_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(atom_info%map_mol_num)))CALL cp__a("topology_util.F",149)
    IF(.NOT.(.NOT.ASSOCIATED(atom_info%map_mol_typ)))CALL cp__a("topology_util.F",150)
    IF(.NOT.(.NOT.ASSOCIATED(atom_info%map_mol_res)))CALL cp__a("topology_util.F",151)
    ...
    END IF
    IF(.NOT.(natom_loc==natom))CALL cp__a("topology_util.F",302)
    ...
    IF (my_qmmm) THEN
       ...
       IF(.NOT.(SIZE(qmmm_env_mm%qm_atom_index)/=0))CALL cp__a("topology_util.F",365)
       IF(.NOT.(ALL(qmmm_env_mm%qm_atom_index /= 0)))CALL cp__a("topology_util.F",366)
       ...
       IF(.NOT.(ALL(qmmm_env_mm%qm_atom_index /= 0)))CALL cp__a("topology_util.F",372)
       ...
       IF (qmmm_env_mm%qmmm_link) THEN
          ! Update the qmmm_env_mm
          IF(.NOT.(SIZE(qmmm_env_mm%mm_link_atoms)>0))CALL cp__a("topology_util.F",394)
          ...
          IF(.NOT.(ALL(qmmm_env_mm%mm_link_atoms /= 0)))CALL cp__a("topology_util.F",400)
          ...
          IF(.NOT.(nlinks /= 0))CALL cp__a("topology_util.F",405)
          DO ikind= 1, nlinks
...
! *****************************************************************************
  SUBROUTINE setup_graph_set(graph_set, idim, ind, array2, atom_bond_list, map_mol,&
    ...
    IF (ASSOCIATED(graph_set)) THEN
       ldim = SIZE(graph_set)
       IF(.NOT.(ldim+1==idim))CALL cp__a("topology_util.F",488)
       NULLIFY(tmp_graph_set)
...
! *****************************************************************************
  SUBROUTINE allocate_graph_set(graph_set_in, graph_set_out, ldim_in, ldim_out)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(graph_set_out)))CALL cp__a("topology_util.F",516)
    mydim_in  = 0
    mydim_out = 0
...
! *****************************************************************************
  SUBROUTINE setup_graph(ind, graph, array2, atom_bond_list, map_mol,&
    ...
    IF (PRESENT(atm_map3)) THEN
       IF(.NOT.(.NOT.ASSOCIATED(atm_map3)))CALL cp__a("topology_util.F",578)
    END IF
    IF(.NOT.(.NOT.ASSOCIATED(graph)))CALL cp__a("topology_util.F",580)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE reorder_list_array(Ilist1, Ilist2, Ilist3, Ilist4, nsize, ndim)
    ...
    IF(.NOT.(nsize>0))CALL cp__a("topology_util.F",626)
    ALLOCATE(wrk(Ndim))
  ...
  RECURSIVE SUBROUTINE spread_mol(atom_bond_list,mol_info,iatom,imol, &
    ...
    DO I=1,SIZE(atom_bond_list(iatom)%array1)
       ...
       IF (mol_info(atom_b)==-1.AND.my_mol_name==mol_name(atom_b)) THEN
          ...
          IF (mol_info(atom_b)/=imol) CALL cp__b("topology_util.F",946,"internal error")
       END IF
    ENDDO
    ...
    DO iatom = 2, natom
       ...
       IF ((icheck_typ.AND.(.NOT.icheck_num)).OR.(.NOT.icheck_typ)) THEN
          ...
          IF (counter /= loc_counter) THEN
             CALL cp_abort(cp__l("topology_util.F",1091),&
       ...
       IF (icheck_num) THEN
          ...
          IF( atom_info%id_atmname(iatom) /= &
             ...
             CALL cp_abort(cp__l("topology_util.F",1114),&
          ...
          IF (SIZE(atom_bond_list(iatom)%array1)/=SIZE(atom_bond_list(first+loc_counter-1)%array1)) THEN
             CALL cp_abort(cp__l("topology_util.F",1127),&
          ...
          DO k = 1,SIZE(atom_bond_list(iatom)%array1)
             IF (ALL(atom_bond_list(first+loc_counter-1)%array1-first/=&
                ...
                CALL cp_abort(cp__l("topology_util.F",1140),&
...
! *****************************************************************************
  SUBROUTINE check_subsys_element(element_in, atom_name_in, element_out, subsys_section, use_mm_map_first)
    ...
    LOGICAL                                  :: defined_kind_section, found, &
                                                in_ptable
    TYPE(section_vals_type), POINTER         :: kind_section
    ...
    IF(.NOT.found.AND.defined_kind_section)&
       CALL cp_abort(cp__l("topology_util.F",1240), "Element <"//TRIM(element_symbol)//&
    ...
    IF (.NOT.found) THEN
       CALL cp_abort(cp__l("topology_util.F",1297),&
