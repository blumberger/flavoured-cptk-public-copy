# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_integrate_potential_product.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_integrate_potential_product.F"
!-----------------------------------------------------------------------------!
...
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_integrate_potential_product.F" 2
...
! *****************************************************************************
  SUBROUTINE integrate_v_rspace(v_rspace, hmat, hmat_kp, pmat, pmat_kp, &
    ...
    LOGICAL :: atom_pair_changed, atom_pair_done, distributed_grids, do_kp, &
      found, h_duplicated, map_consistent, my_compute_tau, my_force_adm, &
    ...
    IF(.NOT.(PRESENT(hmat) .OR. PRESENT(hmat_kp)))CALL cp__a("qs_integrate_potential_product.F",219)
    do_kp = .FALSE.
    IF(PRESENT(hmat_kp)) do_kp = .TRUE.
    IF(PRESENT(pmat)) THEN
       IF(.NOT.(PRESENT(hmat)))CALL cp__a("qs_integrate_potential_product.F",223)
    ELSE IF(PRESENT(pmat_kp)) THEN
       IF(.NOT.(PRESENT(hmat_kp)))CALL cp__a("qs_integrate_potential_product.F",225)
    END IF
    ...
    IF(.NOT.(ASSOCIATED(task_list)))CALL cp__a("qs_integrate_potential_product.F",269)
    ...
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_integrate_potential_product.F",306)
    ...
    IF(nimages > 1) THEN
       IF(.NOT.(do_kp))CALL cp__a("qs_integrate_potential_product.F",321)
    END IF
    ...
    IF (distributed_grids) THEN
       ...
       DO img=1,nimages
          ...
          CALL cp_dbcsr_init(dhmat(img)%matrix)
    ...
    IF ( pab_required ) THEN
       ...
       IF (distributed_grids) THEN
          ...
          IF(do_kp) THEN
             DO img=1,nimages
                CALL cp_dbcsr_copy(deltap(img)%matrix,pmat_kp(img)%matrix,name="LocalP")
             END DO
          ELSE
             CALL cp_dbcsr_copy(deltap(1)%matrix,pmat%matrix,name="LocalP")
          END IF
       END IF
    END IF
...
!$  nthread = omp_get_max_threads()
    ...
    ELSE
       IF(.NOT.(.NOT.calculate_forces))CALL cp__a("qs_integrate_potential_product.F",417)
    END IF
...
!$  ithread = omp_get_thread_num()
    ...
    DO img=1,nimages
       CALL cp_dbcsr_work_create(dhmat(img)%matrix,work_mutable=.TRUE.,n=nthread)
!$  IF(.NOT.dbcsr_distribution_has_threads(cp_dbcsr_distribution(dhmat(img)%matrix)))&
!$     CALL cp__b("qs_integrate_potential_product.F",488,"No thread distribution defined.")
    END DO
!$omp barrier
...
!$omp do schedule (dynamic, MAX(1,task_list%npairs(igrid_level)/(nthread*50)))
    ...
    loop_tasks: DO itask = task_list%taskstart(ipair,igrid_level), task_list%taskstop(ipair,igrid_level)
       ...
       IF(.NOT.(img==1 .OR. do_kp))CALL cp__a("qs_integrate_potential_product.F",505)
...
!$omp critical (block_touched_critical)
            IF ((block_touched(brow,bcol).NE.ithread) .AND. (block_touched(brow,bcol).NE. -1) ) THEN
              CALL cp__b("qs_integrate_potential_product.F",562,"Block has been modified by another thread")
            END IF
...
!$omp end critical (block_touched_critical)
          ...
          IF (.NOT.ASSOCIATED(h_block)) THEN
             CALL cp_dbcsr_add_block_node ( dhmat(img)%matrix, brow, bcol, h_block)
          END IF

          IF (pab_required) THEN
             CALL cp_dbcsr_get_block_p(matrix=deltap(img)%matrix,&
                  row=brow,col=bcol,BLOCK=p_block,found=found)
             IF(.NOT.(found))CALL cp__a("qs_integrate_potential_product.F",577)
          END IF
...
!$omp end do
    ...
    DO img=1,nimages
       CALL cp_dbcsr_finalize(dhmat(img)%matrix)
    END DO
...
!$omp end parallel
    ...
    IF ( h_duplicated ) THEN
       ...
       ELSE
          ...
          DEALLOCATE(htemp)
       END IF
       CALL cp_dbcsr_deallocate_matrix_set ( dhmat)
    ELSE
       DO img=1,nimages
          NULLIFY ( dhmat(img)%matrix )
       END DO
