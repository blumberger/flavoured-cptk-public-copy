# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/cp_linked_list_xc_deriv.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/cp_linked_list_xc_deriv.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/../common/../base/base_uses.f90" 1
...
# 50 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/cp_linked_list_xc_deriv.F" 2
...
FUNCTION cp_sll_deriv_less_q(el1,el2) RESULT(res)
  ...
  IF(.NOT.(ASSOCIATED(el1)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",190)
  IF(.NOT.(ASSOCIATED(el1%split_desc)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",191)
  IF(.NOT.(ASSOCIATED(el2)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",192)
  IF(.NOT.(ASSOCIATED(el2%split_desc)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",193)
       ...
       IF (PRESENT(iter)) THEN
          ...
          DO i=1,iter
             IF (ASSOCIATED(res%rest)) THEN
                res => res%rest
             ELSE
                CALL cp__b("xc/cp_linked_list_xc_deriv.F",319,"tried to go past end")
             END IF
          END DO
          IF (iter==-1) THEN
             DO
...
! *****************************************************************************
  FUNCTION cp_sll_xc_deriv_get_el_at(sll,index) RESULT(res)
    ...
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",391)
...
! *****************************************************************************
  SUBROUTINE cp_sll_xc_deriv_set_el_at(sll,index,value)
    ...
    ELSE
       pos => cp_sll_xc_deriv_get_rest(sll, iter=index-1)
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",423)
...
! *****************************************************************************
  SUBROUTINE cp_sll_xc_deriv_rm_first_el(sll)
    ...
    ELSE
       CALL cp__b("xc/cp_linked_list_xc_deriv.F",510,"tried to remove first el of an empty list")
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_sll_xc_deriv_insert_el_at(sll,el,index)
    ...
    ELSE
       ...
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",544)
  ...
  SUBROUTINE cp_sll_xc_deriv_rm_el_at(sll,index)
    ...
    ELSE
       ...
       ELSE
          pos => cp_sll_xc_deriv_get_rest(sll, iter=index-2)
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",576)
...
FUNCTION cp_sll_xc_deriv_to_array(sll) RESULT(res)
  ...
  DO i=1,len
     ...
     IF(.NOT.(cp_sll_xc_deriv_next(iter).OR.i==len))CALL cp__a("xc/cp_linked_list_xc_deriv.F",633)
  END DO
...
!     #define ( el1%desc == el2%desc ) ( el1%desc == el2%desc )
!     "
...
!           routineP=moduleN//':'//routineN
!     
!       IF(.NOT.(associated(el1)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",670)
!       IF(.NOT.(associated(el1%split_desc)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",671)
!       IF(.NOT.(associated(el2)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",672)
!       IF(.NOT.(associated(el2%split_desc)))CALL cp__a("xc/cp_linked_list_xc_deriv.F",673)
!       res=size(el1%split_desc)<size(el2%split_desc).or.&
!           (size(el1%split_desc)==size(el2%split_desc).and.el1%desc<el2%desc)
!     end function
!     "
