# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_initial_guess.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_initial_guess.F"
!-----------------------------------------------------------------------------!
...
# 93 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_initial_guess.F" 2
...
! *****************************************************************************
  SUBROUTINE calculate_first_density_matrix(scf_env,qs_env)
    ...
    IF (scf_control%use_ot.AND.&
       ...
       CALL cp__b("qs_initial_guess.F",255,"OT needs GUESS ATOMIC / RANDOM / SPARSE / RESTART / HISTORY RESTART: other options NYI")
    END IF
    ...
    IF (density_guess == restart_guess) THEN
       ...
       IF (.NOT.exist) THEN
          CALL cp_warn(cp__l("qs_initial_guess.F",278),&
    ...
    ELSE IF (density_guess == history_guess) THEN
       IF(do_kpoints) THEN
          CALL cp__b("qs_initial_guess.F",287,"calculate_first_density_matrix: history_guess not implemented for k-points")
       ENDIF
       IF (para_env%ionode) &
          CALL wfn_restart_file_name(file_name,exist,dft_section,logger)
       CALL mp_bcast(exist, para_env%source, para_env%group)
       ...
       IF (not_read == 1) THEN
          ...
          CALL mp_bcast(exist, para_env%source, para_env%group)
          IF (.NOT. exist) THEN
             CALL cp_warn(cp__l("qs_initial_guess.F",313),&
    ...
    IF (density_guess == restart_guess) THEN
       ...
       IF (scf_control%diagonalization%mom) THEN
          ! activate MOM immediately as we already have old molecular orbitals;
          ! this behaviour can be modified via your input file
          CALL do_mom_guess(nspins=nspin, mos=mo_array, &
               scf_control=scf_control, p_rmpv=p_rmpv, scf_iter=1)
       ENDIF
    ...
    IF (density_guess == core_guess) THEN
       ...
       IF(do_kpoints) THEN
          CALL cp__b("qs_initial_guess.F",481,"calculate_first_density_matrix: core_guess not implemented for k-points")
       ENDIF
    ...
    IF (density_guess == atomic_guess) THEN
       ...
       IF (ofgpw .AND. (scf_control%use_ot .OR. scf_env%method==ot_diag_method_nr)) THEN
          ...
          IF(.NOT.(1==0))CALL cp__a("qs_initial_guess.F",595)
!         CALL cp_fm_create(sv,mo_coeff%matrix_struct,"SV")
!         DO ispin=1,nspin
...
!         CALL cp_fm_release(sv)
       ...
       IF (scf_control%diagonalization%mom) THEN
          ! by default activate MOM since the second SCF iteration, as 'old'
          ! molecular orbitals will be available after the first iteration
          CALL do_mom_guess(nspins=nspin, mos=mo_array, &
               scf_control=scf_control, p_rmpv=p_rmpv, scf_iter=2)
       ENDIF
    ...
    IF (density_guess == sparse_guess) THEN
       ...
       IF (ofgpw) CALL cp__b("qs_initial_guess.F",622,"SPARSE_GUESS not implemented for OFGPW")
       IF(.NOT.scf_control%use_ot) CALL cp__b("qs_initial_guess.F",623,"OT needed!")
       IF(do_kpoints) THEN
          CALL cp__b("qs_initial_guess.F",625,"calculate_first_density_matrix: sparse_guess not implemented for k-points")
       ENDIF
       ...
       DO ispin=1,nspin
          ...
          ELSE
             ...
             DO i = 1,size_atomic_kind_set
                ...
                DO iatom = 1,natom
                   ...
                   IF (nmo_tmp.LT.0.OR.natoms_tmp.LT.0) THEN
                      CALL cp__b("qs_initial_guess.F",745,"Wrong1!")
                   END IF
                   DO j = 1,n_cols
             ...
             IF (istart_col.LE.nmo) THEN
                CALL cp__b("qs_initial_guess.F",757,"Wrong2!")
             END IF
                   ...
                   DO j = i+1,nmo
                      ...
                      IF(ABS(DOT_PRODUCT(buff(:,1),buff2(:,1))-1.0_dp).LT.1E-10_dp) THEN
                         ...
                         DO ikind=1,nao
                            ...
                            ENDIF
                         ENDDO
                         CALL cp__b("qs_initial_guess.F",785,"")
                      ENDIF
                   ENDDO
                ENDDO
             ...
             ! multiply times PS
             IF (has_unit_metric) THEN
                CALL cp__b("qs_initial_guess.F",808,"has_unit_metric will be removed soon")
             END IF
             !
             ! S*C
    ...
    IF (.NOT. did_guess) THEN
       CALL cp__b("qs_initial_guess.F",931,"An invalid keyword for the initial density guess was specified")
    END IF
          ...
          IF (dft_control%qs_control%dftb) THEN
             DO iatom=1,natom
                ...
                DO la=0,maxll
                   ...
                   CASE DEFAULT
                      CALL cp__b("qs_initial_guess.F",1258,"")
                   END SELECT
                END DO
             END DO
          ELSEIF (dft_control%qs_control%semi_empirical) THEN
            ...
            DO iatom=1,natom
              ...
              CASE (9) ! spd-basis
                ...
                END IF
              CASE DEFAULT
                CALL cp__b("qs_initial_guess.F",1315,"")
              END SELECT
            END DO
          ELSE
