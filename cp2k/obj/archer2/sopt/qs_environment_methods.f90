# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_methods.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_methods.F" 2
...
SUBROUTINE qs_env_setup(qs_env,unit_nr)
    ...
    IF(.NOT.(ASSOCIATED(qs_kind_set)))CALL cp__a("qs_environment_methods.F",158)
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("qs_environment_methods.F",159)
    IF(.NOT.(ASSOCIATED(dft_control)))CALL cp__a("qs_environment_methods.F",160)
    IF(.NOT.(ASSOCIATED(scf_control)))CALL cp__a("qs_environment_methods.F",161)
    ! allocate qs_charges
...
SUBROUTINE qs_env_update_s_mstruct(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_environment_methods.F",244)
    ...
    IF(dft_control%qs_control%gapw) THEN
       ...
       IF(dft_control%qs_control%gapw_control%nopaw_as_gpw) THEN
          IF(.NOT.(ASSOCIATED(rho_core)))CALL cp__a("qs_environment_methods.F",256)
    ...
    ELSE IF(dft_control%qs_control%scptb) THEN
       !??
    ELSE
       IF(.NOT.(ASSOCIATED(rho_core)))CALL cp__a("qs_environment_methods.F",274)
       CALL calculate_rho_core(rho_core, &
    ...
    IF ( do_ppl ) THEN
       ...
       IF(.NOT.(ASSOCIATED(vppl)))CALL cp__a("qs_environment_methods.F",284)
...
SUBROUTINE qs_env_rebuild_pw_env(qs_env)
    ...
    IF(ANY(new_pw_env%cell_hmat/=cell%hmat)) THEN
       ...
       IF(.NOT.(ASSOCIATED(new_pw_env)))CALL cp__a("qs_environment_methods.F",468)
       IF(dft_control%qs_control%gapw) THEN
         IF (ASSOCIATED(rho_core)) THEN
            CALL pw_release(rho_core%pw)
            DEALLOCATE(rho_core)
       ...
       IF (nlcc) THEN
         ...
         IF (dft_control%qs_control%gapw_xc .OR. dft_control%qs_control%gapw) THEN
             CALL cp__b("qs_environment_methods.F",558,"Non linear core correction for GAPW not implemented")
         ENDIF
