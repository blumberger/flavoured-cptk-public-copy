# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_pseudo.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_pseudo.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_pseudo.F" 2
...
! *****************************************************************************
  SUBROUTINE atom_pseudo_opt(atom_section)
    ...
    INTEGER                                  :: ads, do_eric, do_erie, &
                                                handle, i, im, in, iw, k, l, &
                                                maxl, mb, method, mo, n_meth, &
                                                n_rep, reltyp, zcore, zval, zz
    INTEGER, DIMENSION(0:3)                  :: maxn
    INTEGER, DIMENSION(:), POINTER           :: cn
    LOGICAL                                  :: eri_c, eri_e, pp_calc
    REAL(KIND=dp)                            :: ne, nm
    ...
    DO in = 1, n_rep
      DO im = 1, n_meth
        ...
        IF(.NOT.(.NOT.pp_calc))CALL cp__a("atom_pseudo.F",215)
        ...
        END DO
        IF(state%multiplicity/=-1) THEN
          statepp%occa = state%occa - statepp%core/2
          statepp%occb = state%occb - statepp%core/2
        END IF
        ...
        IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_pseudo.F",306)
        ...
        IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_pseudo.F",319)
