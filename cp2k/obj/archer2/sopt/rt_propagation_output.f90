# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/emd/rt_propagation_output.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/emd/rt_propagation_output.F"
!-----------------------------------------------------------------------------!
...
# 91 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/emd/rt_propagation_output.F" 2
...
! *****************************************************************************
  SUBROUTINE rt_prop_output(qs_env,run_type,delta_iter,used_time)
    ...
    IF(rtp%converged)THEN
       ...
       IF (rtp%linear_scaling) THEN
          ...
          IF(BTEST(cp_print_key_should_output(logger%iter_info,&
             ...
             CALL write_rt_p_to_restart(rho_new)
          ENDIF
          IF(.NOT.dft_control%qs_control%dftb) THEN
             ...
             IF(BTEST(cp_print_key_should_output(logger%iter_info,&
                ...
                DO spin=1,SIZE(rho_new)/2
                   CALL rt_current(qs_env,rho_new(2*spin)%matrix,dft_section,spin)
                END DO
             ENDIF
          ENDIF
       ELSE
          CALL get_rtp(rtp=rtp,mos_new=mos_new)
          IF(.NOT.dft_control%qs_control%dftb) THEN
             CALL write_available_results(qs_env=qs_env)
             IF(BTEST(cp_print_key_should_output(logger%iter_info,&
                dft_section,"REAL_TIME_PROPAGATION%PRINT%CURRENT"),cp_p_file)) THEN
                NULLIFY(P_im)
                nspin=SIZE(mos_new)/2
                CALL cp_dbcsr_allocate_matrix_set(P_im,nspin)
                DO spin=1,nspin
                   CALL cp_dbcsr_init_p(P_im(spin)%matrix)
                   CALL cp_dbcsr_create(P_im(spin)%matrix,template=matrix_s(1)%matrix,matrix_type="N")
                END DO
                CALL calculate_P_imaginary(rtp,P_im)
                DO spin=1,nspin
                   CALL rt_current(qs_env,P_im(spin)%matrix,dft_section,spin)
                END DO
                CALL cp_dbcsr_deallocate_matrix_set(P_im)
             ENDIF
          ENDIF
    ...
    IF(.NOT.rtp%converged .AND. rtp%iter>=dft_control%rtp_control%max_iter)&
       CALL cp_abort(cp__l("emd/rt_propagation_output.F",259),"EMD did not converge, either increase MAX_ITER "//&
...
! *****************************************************************************
  SUBROUTINE rt_calculate_orthonormality(orthonormality,mos_new,matrix_s)
   ...
   DO ispin=1,nspin
      ...
      im=ispin*2
      ! get S*C
      CALL cp_fm_create(svec_re,mos_new(im)%matrix%matrix_struct)
      CALL cp_fm_create(svec_im,mos_new(im)%matrix%matrix_struct)
      ...
      CALL cp_fm_struct_create(tmp_fm_struct,nrow_global=k,ncol_global=k, &
           ...
           context=mos_new(re)%matrix%matrix_struct%context)
...
! *****************************************************************************
 ...
 SUBROUTINE rt_convergence(rtp,matrix_s,delta_mos,delta_eps)
   ...
   DO i=1,SIZE(mos_new)
      CALL cp_fm_scale_and_add(-one,delta_mos(i)%matrix,one,mos_new(i)%matrix)
   END DO
   ...
   DO ispin=1,nspin
      ...
      CALL cp_fm_struct_create(tmp_fm_struct,nrow_global=nmo,ncol_global=nmo, &
           ...
           context=delta_mos(re)%matrix%matrix_struct%context)
...
! *****************************************************************************
 ...
 SUBROUTINE rt_convergence_density(rtp,delta_P,delta_eps)
   ...
   DO i=1,SIZE(rho_new)
      CALL cp_dbcsr_add(delta_P(i)%matrix,rho_new(i)%matrix,one,-one)
   ENDDO
   ...
   CALL cp_dbcsr_create(tmp,template=delta_P(1)%matrix,matrix_type="N")
   DO ispin=1,SIZE(delta_P)/2
      CALL cp_dbcsr_desymmetrize(delta_P(2*ispin-1)%matrix,tmp)
      CALL cp_dbcsr_add(delta_P(2*ispin)%matrix,tmp,one,one)
   END DO
...
! *****************************************************************************
  ...
  SUBROUTINE report_density_occupation(filter_eps,rho)
    ...
    NULLIFY(tmp)
    CALL cp_dbcsr_allocate_matrix_set(tmp,SIZE(rho))
    DO i=1,SIZE(rho)
      CALL cp_dbcsr_init_p(tmp(i)%matrix)
      CALL cp_dbcsr_create(tmp(i)%matrix,template=rho(i)%matrix)
      CALL cp_dbcsr_copy(tmp(i)%matrix,rho(i)%matrix)
    ENDDO
    DO ispin=1,SIZE(rho)/2
      ...
      DO WHILE(eps<1.1_dp)
         CALL cp_dbcsr_filter(tmp(re)%matrix,eps)
      ...
      eps=MAX(filter_eps,10E-12_dp)
      DO WHILE(eps<1.1_dp)
         CALL cp_dbcsr_filter(tmp(im)%matrix,eps)
...
! *****************************************************************************
  ...
  SUBROUTINE write_rt_p_to_restart(rho_new)
    ...
    DO ispin=1,SIZE(rho_new)/2
       ...
       cs_pos = cp_dbcsr_checksum (rho_new(re)%matrix, pos=.TRUE.)
       IF (unit_nr>0) THEN
       ...
       cs_pos = cp_dbcsr_checksum (rho_new(im)%matrix, pos=.TRUE.)
       IF (unit_nr>0) THEN
          WRITE(unit_nr,'(T2,A,E20.8)') "Writing restart DM "//TRIM(file_name)//" with checksum: ",cs_pos
       ENDIF
...
! *****************************************************************************
  SUBROUTINE rt_current(qs_env,P_im,dft_section,spin)
    TYPE(qs_environment_type), POINTER       :: qs_env
    ...
    logger => cp_get_default_logger()
    CALL get_qs_env(qs_env=qs_env,subsys=subsys,pw_env=pw_env,rho=rho,ks_env=ks_env)
    CALL qs_subsys_get(subsys,particles=particles) 
    CALL pw_env_get(pw_env,auxbas_pw_pool=auxbas_pw_pool,rs_grids=rs_rho,rs_descs=rs_descs)
    ...
    NULLIFY(rs%pw,gs%pw)
    ALLOCATE(zero)
    CALL cp_dbcsr_init(zero)
    CALL cp_dbcsr_create(zero,template=P_im)
    CALL cp_dbcsr_copy(zero,P_im)
    CALL cp_dbcsr_set(zero,0.0_dp)
    current_env%gauge=-1
    ...
    DO dir=1,3
       ...
       CALL calculate_jrho_resp(zero,P_im,zero,zero,dir,dir,rs,gs,qs_env,current_env,rtp=.TRUE.)
    ...
    CALL cp_dbcsr_deallocate_matrix(zero)
