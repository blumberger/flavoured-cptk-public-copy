# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_data_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_data_methods.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_data_methods.F" 2
    ...
    SELECT CASE (area%d%data_type)
       ...
       CASE default
         CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
               routineN, "Invalid data type.",135)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_data_dev2host(area)
    ...
    SELECT CASE (area%d%data_type)
       ...
       CASE default
         CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
               routineN, "Invalid data type.",177)
    END SELECT
...
!   ---------------------------------------------------------------------------
    ...
    IF (dbcsr_type_is_2d(data_type)) THEN
       ...
       CALL dbcsr_assert(PRESENT(data_size).EQV.PRESENT(data_size2), &
               ...
               "Must specify 2 sizes for 2-D data", 229)
    ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert(ASSOCIATED (area%d), dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Data area must be setup.",288)
    ...
    !          routineN, "Host and device buffer differ in size.",304)
    ...
    ELSE
       SELECT CASE (area%d%data_type)
          ...
          CASE default
             CALL dbcsr_assert(.FALSE., dbcsr_failure_level,&
                  ...
                  "Invalid data type are supported",372)
       END SELECT
       ...
       IF(area%d%memory_type%acc_devalloc) THEN
          ...
          CALL dbcsr_assert(&
                 ...
                 dbcsr_fatal_level, dbcsr_caller_error,&
                 routineN, "Host and device buffer differ in size.",392)
       ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.ASSOCIATED(area%d))&
       CALL cp__w("dbcsr/data/dbcsr_data_methods.F",418,"Data seems to be unreferenced.")
    IF (ASSOCIATED (area%d)) THEN
       !
       IF (careful_mod) THEN
          IF(area%d%refcount.LE.0)&
             CALL cp__w("dbcsr/data/dbcsr_data_methods.F",423,"Data seems to be unreferenced.")
       ENDIF
       !
