# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/../../base/base_uses.f90" 1
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/../base/hash_table_types.f90" 1
! *****************************************************************************
...
# 58 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/../base/hash_table.f90" 1
! -----------------------------------------------------------------------------
...
! End of hashtable
! -----------------------------------------------------------------------------
# 426 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2


# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low_d.f90" 1
! *****************************************************************************
...
!> \param work_col ...
! *****************************************************************************
  SUBROUTINE dbcsr_matrix_colvec_multiply_low_d(matrix, vec_in, vec_out, alpha, beta, work_row, work_col)
    TYPE(dbcsr_obj)                          :: matrix, vec_in, vec_out
    ...
    CASE(dbcsr_type_antisymmetric)
       ...
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error, &
            routineN, "NYI, antisymmetric matrix not permitted", 30)
    CASE DEFAULT
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error, &
            routineN, "Unknown matrix type, ...", 33)
    END SELECT
...
!$OMP          SHARED(matrix,fast_vec_col,fast_vec_row)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       fast_vec_col%blk_map_d(prow)%ptr=fast_vec_col%blk_map_d(prow)%ptr+&
...
! sum all the data onto the first processor col where the original vector is stored
    data_vec => dbcsr_get_data_p (work_col%m%data_area, select_data_type=0.0_real_8)
...
! sum all the data within a processor column to obtain the replicated result
    data_vec => dbcsr_get_data_p (work_row%m%data_area, select_data_type=0.0_real_8)
...
! Copy the local vector to the replicated on the first processor column (this is where vec_in lives)
    ...
    data_vec_rep => dbcsr_get_data_p (rep_col_vec%m%data_area, select_data_type=0.0_real_8)
    data_vec => dbcsr_get_data_p (vec_in%m%data_area, select_data_type=0.0_real_8)
...
! Hope this clarifies the idea
    ...
    data_vec_rep => dbcsr_get_data_p (rep_row_vec%m%data_area, select_data_type=0.0_real_8)
...
!$OMP END PARALLEL
    ...
    IF (col.GT.1) CALL dbcsr_abort(routineP,415,"BUG")
...
!$OMP END PARALLEL
    ...
    IF (row.GT.1) CALL dbcsr_abort(routineP,469,"BUG")
...
!$OMP END PARALLEL
    ...
    data_vec => dbcsr_get_data_p (result_row%m%data_area, select_data_type=0.0_real_8)
...
# 428 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low_z.f90" 1
! *****************************************************************************
!> \brief the real driver routine for the multiply, not all symmetries implemented yet
!> \param matrix ...
!> \param vec_in ...
...
!> \param work_col ...
! *****************************************************************************
  SUBROUTINE dbcsr_matrix_colvec_multiply_low_z(matrix, vec_in, vec_out, alpha, beta, work_row, work_col)
    TYPE(dbcsr_obj)                          :: matrix, vec_in, vec_out
...
! sum all the data onto the first processor col where the original vector is stored
    data_vec => dbcsr_get_data_p (work_col%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    CALL dbcsr_get_info(matrix=work_col, nfullrows_local=nrows, nfullcols_local=ncols)
    CALL mp_sum(data_vec(1:nrows*ncols), prow_group)
...
! sum all the data within a processor column to obtain the replicated result
    data_vec => dbcsr_get_data_p (work_row%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
! we use the replicated vector but the final answer is only summed to proc_col 0 for efficiency
    CALL dbcsr_get_info(matrix=work_row, nfullrows_local=nrows, nfullcols_local=ncols)
    CALL mp_sum(data_vec(1:nrows*ncols), pcol_group)
...
! Copy the local vector to the replicated on the first processor column (this is where vec_in lives)
    CALL dbcsr_get_info(matrix=rep_col_vec, nfullrows_local=nrows, nfullcols_local=ncols)
    data_vec_rep => dbcsr_get_data_p (rep_col_vec%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    data_vec => dbcsr_get_data_p (vec_in%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    IF(mypcol==0)data_vec_rep(1:nrows*ncols)=data_vec(1:nrows*ncols)
! Replicate the data along the row
    CALL mp_bcast(data_vec_rep(1:nrows*ncols), 0, prow_group)
...
! Hope this clarifies the idea
    ...
    CALL dbcsr_iterator_stop(iter)
    CALL dbcsr_get_info(matrix=rep_row_vec, nfullrows_local=nrows, nfullcols_local=ncols)
    data_vec_rep => dbcsr_get_data_p (rep_row_vec%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    CALL mp_sum(data_vec_rep(1:ncols*nrows), pcol_group)
...
!$OMP END PARALLEL
    ...
    ! sum all the data within a processor column to obtain the replicated result from lower
    data_vec => dbcsr_get_data_p (result_row%m%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    CALL dbcsr_get_info(matrix=result_row, nfullrows_local=nrows, nfullcols_local=ncols)
...
# 429 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low_s.f90" 1
! *****************************************************************************
!> \brief the real driver routine for the multiply, not all symmetries implemented yet
!> \param matrix ...
!> \param vec_in ...
...
! *****************************************************************************
  SUBROUTINE dbcsr_matrix_colvec_multiply_low_s(matrix, vec_in, vec_out, alpha, beta, work_row, work_col)
    ...
    SELECT CASE(matrix_type)
    CASE(dbcsr_type_no_symmetry)
       CALL dbcsr_matrix_vector_mult_s(matrix, vec_in, vec_out, alpha, beta, work_row, work_col)
    CASE(dbcsr_type_symmetric)
       CALL dbcsr_sym_matrix_vector_mult_s(matrix, vec_in, vec_out, alpha, beta, work_row, work_col)
    CASE(dbcsr_type_antisymmetric)
        ! Not yet implemented, should mainly be some prefactor magic, but who knows how antisymmetric matrices are stored???
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error, &
            routineN, "NYI, antisymmetric matrix not permitted", 30)
...
! Transfer the correct parts of the input vector to the correct locations so we can do a local multiply
    CALL dbcsr_col_vec_to_rep_row_s(vec_in, work_col, work_row, fast_vec_col)

! Set the work vector for the results to 0
    CALL dbcsr_set(work_col, 0.0_real_4)
...
! Set the work vector for the results to 0
    CALL dbcsr_set(work_row, 0.0_real_4)
...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_s(work_col, work_row, fast_vec_row)
...
! on every replica in the processor column, by summing along the column we end up with the complete vector everywhere
! Hope this clarifies the idea
    CALL dbcsr_set(rep_row_vec, 0.0_real_4)
    CALL dbcsr_get_info(matrix=rep_row_vec, nfullrows_local=nrows, local_cols=local_cols, nfullcols_local=ncols)
    CALL dbcsr_iterator_start(iter, rep_row_vec)
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       CALL dbcsr_iterator_next_block(iter, row, col, vec_row, transposed)
...
! The same trick as described above with opposite direction
    CALL dbcsr_set(rep_col_vec, 0.0_real_4)
    CALL dbcsr_iterator_start(iter, rep_col_vec)
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       CALL dbcsr_iterator_next_block(iter, row, col, vec_col, transposed)
       IF(col_dist(row)==mypcol)THEN
...
!! mp_sum. Of the res_row vectors are created only taking the approriate element (0 otherwise) while the res_col
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_s(work_col, result_row, res_fast_vec_row, res_fast_vec_col)
...
# 430 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low_c.f90" 1
! *****************************************************************************
!> \brief the real driver routine for the multiply, not all symmetries implemented yet
!> \param matrix ...
!> \param vec_in ...
...
# 431 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/vec/dbcsr_vector_operations_low.F" 2
