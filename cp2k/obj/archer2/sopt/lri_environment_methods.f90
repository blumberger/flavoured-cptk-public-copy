# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_environment_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_environment_methods.F"
!-----------------------------------------------------------------------------!
...
# 73 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_environment_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE calculate_avec(lri_env,lri_density,qs_env,pmatrix)
    ...
    INTEGER :: handle, i, iac, iatom, ikind, ilist, ispin, jatom, jkind, &
      jneighbor, nba, nbb, nfa, nfb, nkind, nlist, nn, nneighbor, nspin
    LOGICAL                                  :: found, trans
      ...
      DO ispin = 1, nspin
         ...
         DO WHILE (neighbor_list_iterate(nl_iterator)==0)
            ...
            ELSE
              ...
              trans = .TRUE.
            END IF
            IF(.NOT.(found))CALL cp__a("lri_environment_methods.F",443)
...
! *****************************************************************************
  SUBROUTINE distribute_lri_density_on_the_grid(lri_env,lri_density,qs_env,&
    ...
    INTEGER :: atom_a, atom_b, handle, iac, iatom, ikind, ilist, ispin, &
      jatom, jkind, jneighbor, nat, natom, nfa, nfb, nkind, nspin
  ...
  SUBROUTINE inverse_lri_overlap(lri_env,sinv)
    ...
    CASE(do_lri_inv_auto)
       ...
       IF (info == 0) THEN
           ...
           IF (info /= 0) THEN
             CALL cp__b("lri_environment_methods.F",942,"DPOCON failed")
           ENDIF
    ...
    CASE DEFAULT
       CALL cp__b("lri_environment_methods.F",954,"No initialization for LRI overlap available?")
    END SELECT
