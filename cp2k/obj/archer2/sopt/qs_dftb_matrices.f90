# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_matrices.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_matrices.F"
!-----------------------------------------------------------------------------!
...
# 103 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_matrices.F" 2
...
! *****************************************************************************
  SUBROUTINE build_dftb_matrices(qs_env,para_env,calculate_forces)
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       ddr = dgrd*0.1_dp
       IF(.NOT.(dftb_param_ij%llm==dftb_param_ji%llm))CALL cp__a("qs_dftb_matrices.F",330)
       ...
       IF (NINT(dr/dgrd) <= ngrdcut) THEN
         ...
         ELSE
            ...
            IF(.NOT.(ic > 0))CALL cp__a("qs_dftb_matrices.F",354)
         END IF
         ...
         IF (calculate_forces) THEN
            ...
            IF(.NOT.(ASSOCIATED(pblock)))CALL cp__a("qs_dftb_matrices.F",377)
            NULLIFY (wblock)
            CALL cp_dbcsr_get_block_p(matrix=matrix_w(1,ic)%matrix,&
                 row=irow,col=icol,block=wblock,found=found)
            IF(.NOT.(ASSOCIATED(wblock)))CALL cp__a("qs_dftb_matrices.F",381)
            IF ( dftb_control%self_consistent ) THEN
               DO i=2,4
                  ...
                  IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",387)
                  ...
                  IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",391)
               END DO
            END IF
         END IF
         ...
         ELSE
           ...
           IF(calculate_forces) THEN
             ...
             DO i=1,3 
               ...
               IF ( dftb_control%self_consistent ) THEN
                  IF(.NOT.(ASSOCIATED(dsblocks(i+1)%block)))CALL cp__a("qs_dftb_matrices.F",442)
         ...
         IF ( dftb_control%self_consistent ) THEN
            ...
            IF(calculate_forces .AND. (iatom/=jatom .OR. dr > 0.001_dp)) THEN
               ...
               DO i=1,3
                  IF(.NOT.(ASSOCIATED(dgblocks(i+1)%block)))CALL cp__a("qs_dftb_matrices.F",493)
                  IF ( irow == iatom ) THEN
...
! *****************************************************************************
  SUBROUTINE build_dftb_ks_matrix(qs_env,calculate_forces,just_energy)
    ...
    logger => cp_get_default_logger()
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_dftb_matrices.F",655)
    ...
    IF(.NOT.(ASSOCIATED(matrix_h)))CALL cp__a("qs_dftb_matrices.F",673)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("qs_dftb_matrices.F",674)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("qs_dftb_matrices.F",675)
    ...
    IF (qs_env%qmmm) THEN
       IF(.NOT.(SIZE(ks_matrix,2)==1))CALL cp__a("qs_dftb_matrices.F",715)
       DO ispin = 1, nspins
    ...
    IF (qs_env%requires_mo_derivs .AND. .NOT. just_energy) THEN
       IF(.NOT.(SIZE(ks_matrix,2)==1))CALL cp__a("qs_dftb_matrices.F",754)
       ...
       DO ispin=1,SIZE(mo_derivs)
          ...
          IF(.NOT.mo_array(ispin)%mo_set%use_mo_coeff_b) THEN
             CALL cp__b("qs_dftb_matrices.F",760,"")
          ENDIF
...
! *****************************************************************************
  SUBROUTINE build_dftb_overlap(qs_env,nderivative,matrix_s)
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       dgrd = dftb_param_ij%dgrd
       ddr = dgrd*0.1_dp
       IF(.NOT.(dftb_param_ij%llm==dftb_param_ji%llm))CALL cp__a("qs_dftb_matrices.F",881)
       llm = dftb_param_ij%llm
       smatij => dftb_param_ij%smat
       smatji => dftb_param_ji%smat
       ...
       IF (NINT(dr/dgrd) <= ngrdcut) THEN
          ...
          CALL cp_dbcsr_get_block_p(matrix=matrix_s(1)%matrix,&
               row=irow,col=icol,BLOCK=sblock,found=found)
          IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",894)
          ...
          ELSE
             ...
             IF(nderivative.ge.1) THEN
                ...
                DO i=1,3
                   ...
                   IF(.NOT.(ASSOCIATED(dsblocks(i+1)%block)))CALL cp__a("qs_dftb_matrices.F",937)
                   dsblocks(i+1)%block = dsblocks(i+1)%block + dsblock
...
! *****************************************************************************
  SUBROUTINE build_dftb_qmmm_matrix(qs_env,qmmm_env,particles_mm,mm_cell,para_env)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",1091)
...
! *****************************************************************************
  SUBROUTINE build_dftb_qmmm_matrix_pc(qs_env,qmmm_env,particles_mm,mm_cell,para_env)
    ...
    IF(do_multipoles) CALL cp__b("qs_dftb_matrices.F",1231,"No multipole force fields allowed in DFTB QM/MM")
    IF(do_ipol /= do_fist_pol_none) CALL cp__b("qs_dftb_matrices.F",1232,"No polarizable force fields allowed in DFTB QM/MM")

    SELECT CASE(ewald_type)
       CASE(do_ewald_pme)
          CALL cp__b("qs_dftb_matrices.F",1236,"PME Ewald type not implemented for DFTB/QMMM")
       CASE(do_ewald_ewald,do_ewald_spme)
          DO ipot = 1,SIZE(qmmm_env%Potentials)
             ...
             IF(ewald_type == do_ewald_ewald) THEN
                CALL cp__b("qs_dftb_matrices.F",1253,"Ewald not implemented for DFTB/QMMM")
          ...
          IF (qmmm_env%move_mm_charges.OR.qmmm_env%add_mm_charges) THEN
             DO ipot = 1,SIZE(qmmm_env%added_charges%Potentials)
                ...
                END DO
                IF(ewald_type == do_ewald_ewald) THEN
                   CALL cp__b("qs_dftb_matrices.F",1277,"Ewald not implemented for DFTB/QMMM")
                ELSE IF(ewald_type == do_ewald_spme) THEN
                   ! spme electrostatic potential
                   CALL spme_potential(ewald_env,ewald_pw,mm_cell,atoms_mm,charges_mm,particles_qm,qpot)
                END IF
       ...
       CASE(do_ewald_none)
          ...
          END DO
       CASE DEFAULT
          CALL cp__b("qs_dftb_matrices.F",1351,"Unknown Ewald type!")
    END SELECT
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       CALL cp_dbcsr_get_block_p(matrix=matrix_h(1)%matrix,&
            row=iatom,col=jatom,block=hblock,found=found)
       IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",1370)
       hblock = hblock - 0.5_dp*sblock*(qpot(iatom)+qpot(jatom))
    END DO
    CALL cp_dbcsr_iterator_stop(iter)
  ...
  SUBROUTINE deriv_dftb_qmmm_matrix_pc(qs_env,qmmm_env,particles_mm,mm_cell,para_env,&
    ...
    IF (calc_force) THEN
      ...
      ! Mulliken charges
       ...
       CALL ewald_env_get(ewald_env,ewald_type=ewald_type,do_multipoles=do_multipoles,do_ipol=do_ipol)
       IF(do_multipoles) CALL cp__b("qs_dftb_matrices.F",1702,"No multipole force fields allowed in DFTB QM/MM")
       IF(do_ipol /= do_fist_pol_none) CALL cp__b("qs_dftb_matrices.F",1703,"No polarizable force fields allowed in DFTB QM/MM")

       SELECT CASE(ewald_type)
          CASE(do_ewald_pme)
             CALL cp__b("qs_dftb_matrices.F",1707,"PME Ewald type not implemented for DFTB/QMMM")
          CASE(do_ewald_ewald,do_ewald_spme)
             DO ipot = 1,SIZE(qmmm_env%Potentials)
                Pot => qmmm_env%Potentials(ipot)%Pot
                nmm = SIZE(Pot%mm_atom_index)
                ...
                Forces_MM= 0.0_dp
                IF(ewald_type == do_ewald_ewald) THEN
                   CALL cp__b("qs_dftb_matrices.F",1727,"Ewald not implemented for DFTB/QMMM")
                ELSE IF(ewald_type == do_ewald_spme) THEN
                   ! spme electrostatic potential
             ...
             IF (qmmm_env%move_mm_charges.OR.qmmm_env%add_mm_charges) THEN
                DO ipot = 1,SIZE(qmmm_env%added_charges%Potentials)
                   ...
                   Forces_MM= 0.0_dp
                   IF(ewald_type == do_ewald_ewald) THEN
                      CALL cp__b("qs_dftb_matrices.F",1769,"Ewald not implemented for DFTB/QMMM")
                   ELSE IF(ewald_type == do_ewald_spme) THEN
                      ! spme electrostatic potential
          ...
          CASE(do_ewald_none)
             ...
             END DO
          CASE DEFAULT
             CALL cp__b("qs_dftb_matrices.F",1879,"Unknown Ewald type!")
       END SELECT
       ...
       DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          CALL cp_dbcsr_get_block_p(matrix=matrix_p(1)%matrix,&
               row=iatom,col=jatom,block=pblock,found=found)
          IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",1914)
          DO i=1,3
             NULLIFY(dsblock)
             CALL cp_dbcsr_get_block_p(matrix=matrix_s(1+i)%matrix,&
                  row=iatom,col=jatom,block=dsblock,found=found)
             IF(.NOT.(found))CALL cp__a("qs_dftb_matrices.F",1919)
             fi = -2.0_dp*gmij*SUM(pblock*dsblock)
             Forces_QM(i,iatom) = Forces_QM(i,iatom) + fi
             Forces_QM(i,jatom) = Forces_QM(i,jatom) - fi
          END DO
       ...
       LoopMM: DO Imp = 1, SIZE(Pot%mm_atom_index)
          ...
          IF(dr > rtiny) THEN
             ...
             ELSE IF (pot_type==2) THEN
                ...
                qpot = qpot + qeff*sr
             ELSE 
                CALL cp__b("qs_dftb_matrices.F",2038,"")
             END IF
          END IF
       END DO LoopMM
    END DO MainLoopPot
       ...
       LoopMM: DO Imp = 1, SIZE(Pot%mm_atom_index)
          ...
          IF(dr > rtiny) THEN
             ...
             ELSE IF (pot_type==2) THEN
                ...
                fsr = qeff*qcharge*dsr
             ELSE
                CALL cp__b("qs_dftb_matrices.F",2132,"")
             END IF
             force_ab = -fsr * rij/dr
          ELSE
             force_ab = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE setup_matrices1(qs_env,nderivative,matrices,mnames,sab_nl)
    ...
    IF(.NOT.(nmat>0))CALL cp__a("qs_dftb_matrices.F",2246)
...
! *****************************************************************************
  SUBROUTINE setup_matrices2(qs_env,nderivative,nimg,matrices,mnames,sab_nl)
    ...
    IF(nderivative==1)nmat=4
    IF(nderivative==2)nmat=10
    IF(.NOT.(nmat>0))CALL cp__a("qs_dftb_matrices.F",2342)
...
! *****************************************************************************
  SUBROUTINE turnsk(mat,skab1,skab2,dxv,dx,lmaxa,lmaxb)
    ...
    IF (lmaxab.gt.2)  CALL cp__b("qs_dftb_matrices.F",2618,'lmax=2')
...
!
    ...
    DO m = 1,n-1
      DO i = 1,n-m
        ...
        w=c(i+1)-d(i)
        den=ho-hp
        IF(.NOT.(den /= 0.0_dp))CALL cp__a("qs_dftb_matrices.F",3035)
        den=w/den
        d(i)=hp*den
        c(i)=ho*den
      END DO
