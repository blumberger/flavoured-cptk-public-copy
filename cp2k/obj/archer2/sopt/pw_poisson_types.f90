# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_types.F"
!-----------------------------------------------------------------------------!
...
# 58 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_types.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_green_create ( green, poisson_params, cell_hmat, pw_pool, &
    ...
    IF(.NOT.(.NOT.(ASSOCIATED(green))))CALL cp__a("pw/pw_poisson_types.F",214)
    ALLOCATE(green)
    green%p3m=.FALSE.
    ...
    !IF(.NOT.(cell%orthorhombic))CALL cp__a("pw/pw_poisson_types.F",239)
    DO i=1,3
       abc(i)=cell_hmat(i,i)
    END DO
    ...
    CASE (pw_poisson_periodic)
       green%method = PERIODIC3D
       IF (dim /= 3) THEN
          CALL cp__b("pw/pw_poisson_types.F",249,"Illegal combination of periodicity and Poisson solver periodic3d")
       END IF
    CASE (pw_poisson_multipole)
       green%method = MULTIPOLE0D
       IF (dim /= 0) THEN
          CALL cp__b("pw/pw_poisson_types.F",254,"Illegal combination of periodicity and Poisson solver mulipole0d")
       END IF
    CASE (pw_poisson_analytic)
       ...
       CASE (3)
          green%method = PERIODIC3D
       CASE DEFAULT
          CALL cp__b("pw/pw_poisson_types.F",277,"")
       END SELECT
    CASE (pw_poisson_mt)
       ...
       CASE (2)
          ...
          green%slab_size = abc(i)
       CASE (3)
          CALL cp__b("pw/pw_poisson_types.F",301,"Illegal combination of periodicity and Poisson solver (MT)")
       CASE DEFAULT
          CALL cp__b("pw/pw_poisson_types.F",303,"")
       END SELECT
    CASE (pw_poisson_implicit)
       green%method = PS_IMPLICIT
    CASE DEFAULT
       CALL cp__b("pw/pw_poisson_types.F",308,"An unknown Poisson solver was specified")
    END SELECT
    ...
    CASE DEFAULT
       CALL cp__b("pw/pw_poisson_types.F",349,"")
    END SELECT
    ...
    CASE DEFAULT
       CALL cp__b("pw/pw_poisson_types.F",444,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE pw_green_retain(gftype)
    TYPE(greens_fn_type), POINTER            :: gftype

    CHARACTER(len=*), PARAMETER :: routineN = 'pw_green_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(gftype)))CALL cp__a("pw/pw_poisson_types.F",460)
    IF(.NOT.(gftype%ref_count>0))CALL cp__a("pw/pw_poisson_types.F",461)
...
! *****************************************************************************
  SUBROUTINE pw_green_release ( gftype, pw_pool)
    ...
    IF (ASSOCIATED(gftype)) THEN
       IF(.NOT.(gftype%ref_count>0))CALL cp__a("pw/pw_poisson_types.F",484)
       gftype%ref_count=gftype%ref_count-1
...
! *****************************************************************************
  SUBROUTINE influence_factor ( gftype)
    ...
    IF(.NOT.(ASSOCIATED(gftype)))CALL cp__a("pw/pw_poisson_types.F",535)
    IF(.NOT.(gftype%ref_count>0))CALL cp__a("pw/pw_poisson_types.F",536)
    n = gftype % p3m_order
...
! *****************************************************************************
  SUBROUTINE pw_poisson_create ( poisson_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_types.F",632)
...
! *****************************************************************************
  SUBROUTINE pw_poisson_retain(poisson_env)
    ...
    IF(.NOT.(ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_types.F",662)
    IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_types.F",663)
...
! *****************************************************************************
  SUBROUTINE pw_poisson_release ( poisson_env)
    ...
    IF (ASSOCIATED(poisson_env)) THEN
       IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_types.F",682)
       poisson_env%ref_count=poisson_env%ref_count-1
