# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/molecular_states.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/molecular_states.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/molecular_states.F" 2
...
! *****************************************************************************
  SUBROUTINE construct_molecular_states(molecule_set, mo_localized, &
    ...
    IF (isos /= 0) CALL stop_memory(routineN,moduleN,208,&
                                    "nstates",int_size*2)
    ...
    DO imol = 1, SIZE(molecule_set)
       ...
       IF (isos /= 0) CALL stop_memory(routineN,moduleN,226,&
       ...
       IF (isos /= 0) CALL stop_memory(routineN,moduleN,240,&
                                       "evals",dp_size*ns)
       ...
       CALL cp_gemm('T','N',ns,ns,nrow_global,1.0_dp, &
            b,c,0.0_dp,d)
       ...
       CALL cp_gemm('N','N',nrow_global,ns,ns,1.0_dp, &
       ...
       IF (.FALSE.) THEN ! this is too much data for large systems
         ...
         CALL cp_fm_struct_release(fm_struct_tmp)
         CALL cp_gemm('T','N',ns,ncol_global,nrow_global,1.0_dp, &
