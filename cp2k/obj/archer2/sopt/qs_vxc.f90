# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_vxc.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_vxc_create( ks_env, rho_struct, xc_section, vxc_rho, vxc_tau, exc,&
    ...
    INTEGER                                  :: handle, ispin, myfun, &
                                                nelec_spin(2), vdw
    LOGICAL :: compute_virial, do_adiabatic_rescaling, my_just_energy, &
      rho_g_valid, sic_scaling_b_zero, tau_r_valid, uf_grid, vdW_nl
    ...
    IF(.NOT.(.NOT.ASSOCIATED(vxc_rho)))CALL cp__a("qs_vxc.F",137)
    IF(.NOT.(.NOT.ASSOCIATED(vxc_tau)))CALL cp__a("qs_vxc.F",138)
    ...
    IF(.NOT.(.NOT.(do_adiabatic_rescaling .AND. vdW_nl)))CALL cp__a("qs_vxc.F",176)
    ...
    IF (myfun/=xc_none .OR. vdW_nl) THEN
       ...
       IF(.NOT.(ASSOCIATED(rho_struct)))CALL cp__a("qs_vxc.F",186)
       IF(dft_control%nspins/=1 .AND. dft_control%nspins/=2)&
          CALL cp__b("qs_vxc.F",188,"nspins must be 1 or 2")
       ...
       CASE ( sic_mauri_spz, sic_ad )
          ...
          IF(.NOT.(.NOT.tau_r_valid))CALL cp__a("qs_vxc.F",201)
       CASE ( sic_mauri_us )
          my_scaling=1.0_dp-dft_control%sic_scaling_b
          ! no idea yet what to do here in that case
          IF(.NOT.(.NOT.tau_r_valid))CALL cp__a("qs_vxc.F",205)
       CASE ( sic_eo )
          ! NOTHING TO BE DONE
       CASE DEFAULT
          ! this case has not yet been treated here
          CALL cp__b("qs_vxc.F",210,"NYI")
       END SELECT
       ...
       IF (.not.uf_grid) THEN
          ...
          END IF
       ELSE
          IF(.NOT.(rho_g_valid))CALL cp__a("qs_vxc.F",244)
          ALLOCATE(rho_g(dft_control%nspins))
          DO ispin=1,dft_control%nspins
          ...
          IF (tau_r_valid) THEN
            ...
            CALL cp__b("qs_vxc.F",258,"tau with finer grids")
...
!                END DO
       ...
       IF (vdW_nl) THEN
          ...
          IF(.NOT.(dft_control%sic_method_id==sic_none))CALL cp__a("qs_vxc.F",321)
          ! 
       ...
       IF (dft_control%sic_method_id .EQ. sic_mauri_spz .AND. .NOT. sic_scaling_b_zero) THEN
          ...
          ELSE
             ! virial untested
             IF(.NOT.(.NOT.compute_virial))CALL cp__a("qs_vxc.F",409)
             CALL xc_vxc_pw_create1(vxc_rho=my_vxc_rho,vxc_tau=my_vxc_tau, rho_r=rho_r,&
       ...
       IF ( dft_control%sic_method_id .EQ. sic_ad  .AND. .NOT. sic_scaling_b_zero ) THEN
          ...
          DO ispin=1,2
             ...
             ELSE
                ! virial untested
                IF(.NOT.(.NOT.compute_virial))CALL cp__a("qs_vxc.F",481)
                CALL xc_vxc_pw_create1(vxc_rho=my_vxc_rho,vxc_tau=my_vxc_tau, rho_r=rho_r,&
                     rho_g=rho_g,tau=tau,exc=exc_m,&
                     xc_section=xc_section,&
                     pw_pool=xc_pw_pool,&
       ...
       IF (dft_control%sic_method_id .EQ. sic_mauri_us .AND. .NOT. sic_scaling_b_zero ) THEN
          ...
          ELSE
             ! virial untested
             IF(.NOT.(.NOT.compute_virial))CALL cp__a("qs_vxc.F",526)
             CALL xc_vxc_pw_create1(vxc_rho=my_vxc_rho,vxc_tau=my_vxc_tau, rho_r=rho_r,&
                  rho_g=rho_g,tau=tau,exc=exc_m,&
                  xc_section=xc_section,&
                  pw_pool=xc_pw_pool,&
