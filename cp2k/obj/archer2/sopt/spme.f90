# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/spme.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/spme.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/spme.F" 2
...
! *****************************************************************************
  SUBROUTINE spme_evaluate ( ewald_env, ewald_pw, box, particle_set, &
    ...
    INTEGER                                  :: group, handle, i, ig, ipart, &
                                                j, n, ncore, npart, nshell, &
                                                o_spline, p1, p1_shell
    ...
    IF(PRESENT(shell_particle_set).AND.(PRESENT(core_particle_set))) THEN
       IF(.NOT.(ASSOCIATED(shell_particle_set)))CALL cp__a("spme.F",168)
       IF(.NOT.(ASSOCIATED(core_particle_set)))CALL cp__a("spme.F",169)
       nshell=SIZE(shell_particle_set)
       ncore =SIZE(core_particle_set)
       IF(.NOT.(nshell==ncore))CALL cp__a("spme.F",172)
...
! *****************************************************************************
  SUBROUTINE spme_potential ( ewald_env, ewald_pw, box, particle_set_a, charges_a, &
    ...
    INTEGER                                  :: group, handle, ipart, n, &
                                                npart_a, npart_b, o_spline, p1
...
! *****************************************************************************
  SUBROUTINE spme_forces ( ewald_env, ewald_pw, box, particle_set_a, charges_a, &
    ...
    INTEGER                                  :: group, handle, i, ipart, n, &
                                                npart_a, npart_b, o_spline, p1
    INTEGER, ALLOCATABLE, DIMENSION(:, :)    :: center
    INTEGER, DIMENSION(3)                    :: npts
...
! *****************************************************************************
  SUBROUTINE get_patch_a ( part, box, green, npts, p, rhos, is_core, is_shell,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'get_patch_a', &
      routineP = moduleN//':'//routineN
    ...
    IF(is_core .AND. is_shell) THEN
       CALL cp__b("spme.F",803,"Shell-model: cannot be core and shell simultaneously")
    END IF
...
! *****************************************************************************
  SUBROUTINE get_patch_b ( part, box, green, npts, p, rhos, charges )
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'get_patch_b', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE spme_get_patch ( rhos, n, delta, q, coeff )
    ...
    IF ( n > nmax ) THEN
       CALL cp__b("spme.F",899,"nmax value too small")
    END IF
