# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_hash_table_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_hash_table_types.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_hash_table_types.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE fb_hash_table_add(hash_table, key, val)
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",117)
    ...
    IF(.NOT.(islot>0))CALL cp__a("qs_fb_hash_table_types.F",126)
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_create(hash_table, nmax)
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",168)
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_get(hash_table, key, val, found)
    ...
    check_ok = fb_hash_table_has_data(hash_table)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",205)
    found = .FALSE.
    val = 0
    islot = fb_hash_table_linear_probe(hash_table, key)
    IF (islot > 0) THEN
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_init(hash_table, nmax)
    ...
    check_ok = fb_hash_table_has_data(hash_table)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",249)
...
!> \author Lianheng Tong (LT) lianheng.tong@kcl.ac.uk
! *****************************************************************************
  SUBROUTINE fb_hash_table_rehash(hash_table, nmax)
    TYPE(fb_hash_table_obj), INTENT(INOUT)   :: hash_table
    INTEGER, INTENT(IN), OPTIONAL            :: nmax
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_release(hash_table)
    ...
    IF (ASSOCIATED(hash_table%obj)) THEN
       ...
       IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",345)
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_remove(hash_table, key)
    ...
    check_ok = fb_hash_table_has_data(hash_table)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",377)
    islot = fb_hash_table_linear_probe(hash_table, key)
...
!> \author Lianheng Tong (LT) lianheng.tong@kcl.ac.uk
! *****************************************************************************
  SUBROUTINE fb_hash_table_retain(hash_table)
    TYPE(fb_hash_table_obj), INTENT(INOUT)   :: hash_table
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",411)
    check_ok = hash_table%obj%ref_count > 0
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",413)
    hash_table%obj%ref_count = hash_table%obj%ref_count + 1
...
! *****************************************************************************
  SUBROUTINE fb_hash_table_status(hash_table, nelements, nmax, prime)
    ...
    check_ok = fb_hash_table_has_data(hash_table)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_hash_table_types.F",435)
