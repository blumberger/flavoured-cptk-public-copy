# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf.F"
!-----------------------------------------------------------------------------!
...
# 69 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf.F" 2
...
! *****************************************************************************
  SUBROUTINE almo_entry_scf(qs_env, calc_forces)
    ...
    ! no forces so far 
    IF(calc_forces)&
       CALL cp__b("almo_scf.F",107," No forces/gradients with ALMOs.... yet(!) ")
       ...
       DO idomain=1,ndomains
          ...
          IF (nelec_a.ne.nelec_b) THEN 
             IF (nspins.eq.1) THEN
                ...
                CALL cp__b("almo_scf.F",245,"odd e- -- use unrestricted methods")
             ENDIF
             ...
             CALL cp__b("almo_scf.F",250,"Unrestricted ALMO methods are NYI")
          ENDIF
       ENDDO
       DO ispin=1,nspins
          ...
          CASE (virt_occ_size)
             ...
             ENDDO
          CASE DEFAULT
             CALL cp__b("almo_scf.F",283,"illegal method for virtual space truncation")
          END SELECT
       ENDDO ! spin
  ...
  SUBROUTINE almo_scf_delocalization(qs_env,almo_scf_env)
       ...
       DO row = 1, nblkrows_tot
          DO col = 1, nblkcols_tot
             ...
             IF (hold.EQ.mynode) THEN
                ...
                IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf.F",1078)
                p_new_block(:,:) = 1.0_dp
             ENDIF
          ENDDO
       ENDDO
    ...
    CASE (almo_deloc_xalmo_1diag)
       ...
       ELSE
          ...
          CALL cp__b("almo_scf.F",1153,"Other algorithms do not exist")
    ...
    CASE (almo_deloc_xalmo_x)
       ...
       ELSE
          ...
          CALL cp__b("almo_scf.F",1173,"Other algorithms do not exist")
    ...
    CASE (almo_deloc_xalmo_scf)
       ...
       IF (almo_scf_env%xalmo_update_algorithm.eq.almo_scf_diag) THEN
          ...
          CALL cp__b("almo_scf.F",1181,"Should not be here: convergence will fail!")
    ...
    CASE DEFAULT
       ...
       CALL cp__b("almo_scf.F",1219,"Illegal delocalization method")
    ...
    CASE (almo_deloc_scf,almo_deloc_x_then_scf)
       ...
       IF (almo_scf_env%deloc_truncate_virt.ne.virt_full) THEN
          CALL cp__b("almo_scf.F",1227,"full scf is NYI for truncated virtual space")
       ENDIF
