# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/reftraj_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/reftraj_util.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/reftraj_util.F" 2
...
! *****************************************************************************
  SUBROUTINE initialize_reftraj(reftraj,reftraj_section,md_env)
    ...
    IF(.NOT.(nskip>=0))CALL cp__a("motion/reftraj_util.F",108)
    ...
    reftraj%isnap = nskip
    IF(my_end)&
       CALL cp_abort(cp__l("motion/reftraj_util.F",117),&
    ...
    IF(reftraj%info%variable_volume) THEN
       ...
       END IF
       IF(my_end)&
          CALL cp_abort(cp__l("motion/reftraj_util.F",127),&
...
! *****************************************************************************
  SUBROUTINE initialize_msd_reftraj(msd,msd_section,reftraj,md_env)
    ...
    INTEGER                                  :: first_atom, iatom, ikind, &
                                                imol, last_atom, natom_read, &
                                                nkind, nmol, nmolecule, &
                                                nmolkind, npart
    ...
    IF(.NOT.(.NOT. ASSOCIATED(msd)))CALL cp__a("motion/reftraj_util.F",192)
    ...
    IF (para_env%mepos==para_env%source) THEN
      ...
      IF(.NOT.(natom_read==reftraj%natom))CALL cp__a("motion/reftraj_util.F",219)
      ...
      DO iatom = 1,natom_read
          ...
          IF(.NOT.((TRIM(AA)==name)))CALL cp__a("motion/reftraj_util.F",226)
    ...
    CALL mp_bcast(msd%total_mass,para_env%source,para_env%group)
    CALL mp_bcast(msd%ref0_pos,para_env%source,para_env%group)
    CALL mp_bcast(msd%ref0_com,para_env%source,para_env%group)
...
998 CONTINUE ! end of file
    CALL cp__b("motion/reftraj_util.F",311,"End of reference positions file reached")
999 CONTINUE ! error
    CALL cp__b("motion/reftraj_util.F",313,"Error reading reference positions file")
...
! *****************************************************************************
  SUBROUTINE compute_msd_reftraj(reftraj,md_env,particle_set)
    ...
    INTEGER :: atom, bo(2), first_atom, iatom, ikind, imol, imol_global, &
      last_atom, mepos, natom_kind, nmol_per_kind, nmolecule, nmolkind, num_pe
...
! compute com
      ...
      DO ikind=1,atomic_kinds%n_els
          ...
          END DO
      END DO
      CALL mp_sum(rcom,para_env%group)
...
!      IF(para_env%ionode) WRITE(*,'(A,T50,3f10.5)') ' COM displacement (dx,dy,dz) [angstrom]:  ', &
...
! compute_com
      ...
      END DO  ! ikind
    ENDIF
    CALL mp_sum(reftraj%msd%val_msd_kind,para_env%group)
    CALL mp_sum(reftraj%msd%num_disp_atom,para_env%group)
    CALL mp_sum(reftraj%msd%disp_atom_dr,para_env%group)
