# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_types.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_types.F" 2
...
! *****************************************************************************
  SUBROUTINE semi_empirical_create(sep)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(sep)))CALL cp__a("semi_empirical_types.F",192)
    ALLOCATE (sep)
...
! *****************************************************************************
  SUBROUTINE zero_se_param(sep)
    ...
    IF(.NOT.(ASSOCIATED(sep)))CALL cp__a("semi_empirical_types.F",254)
    sep%defined            = .FALSE.
    sep%dorb               = .FALSE.
...
! *****************************************************************************
  SUBROUTINE get_se_param(sep,name,typ,defined,z,zeff,natorb,eheat,&
    ...
    ELSE
       CALL cp__b("semi_empirical_types.F",428,"The pointer sep is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE set_se_param(sep,name,typ,defined,z,zeff,natorb,eheat,&
    ...
    ELSE
       CALL cp__b("semi_empirical_types.F",530,"The pointer sep is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE rotmat_create(rotmat)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(rotmat)))CALL cp__a("semi_empirical_types.F",545)
    ALLOCATE (rotmat)
...
! *****************************************************************************
  SUBROUTINE se_taper_create(se_taper, integral_screening, do_ewald, &
    ...
    IF(.NOT.(.NOT.ASSOCIATED(se_taper)))CALL cp__a("semi_empirical_types.F",636)
    ALLOCATE(se_taper)
  ...
  SUBROUTINE write_se_param(sep, subsys_section)
    ...
    IF (ASSOCIATED(sep).AND.BTEST(cp_print_key_should_output(logger%iter_info,subsys_section,&
       ...
       IF (output_unit >0) THEN
          ...
          SELECT CASE (typ)
          CASE DEFAULT
             CALL cp__b("semi_empirical_types.F",737,"Semiempirical method unknown")
          CASE(do_method_am1)
