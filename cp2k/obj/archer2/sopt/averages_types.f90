# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/averages_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/averages_types.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/averages_types.F" 2
  ...
  SUBROUTINE create_averages(averages, averages_section, virial_avg, force_env)
    ...
    IF (averages%do_averages) THEN
       ...
       IF (PRESENT(virial_avg)) THEN
          IF (virial_avg) CALL virial_create(averages%virial)
       END IF
...
! *****************************************************************************
  SUBROUTINE retain_averages(averages)
    ...
    IF(.NOT.(ASSOCIATED(averages)))CALL cp__a("motion/averages_types.F",154)
    IF(.NOT.(averages%ref_count>0))CALL cp__a("motion/averages_types.F",155)
...
! *****************************************************************************
  SUBROUTINE release_averages(averages)
    ...
    IF (ASSOCIATED(averages)) THEN
       IF(.NOT.(averages%ref_count>0))CALL cp__a("motion/averages_types.F",173)
       averages%ref_count=averages%ref_count-1
       IF (averages%ref_count==0) THEN
          CALL virial_release(averages%virial)
          ...
          IF (averages%itimes_start==-1) THEN
             ...
             IF (restart_averages) THEN
                ...
                IF (nint>0) THEN
                   ...
                   IF(.NOT.(nint==SIZE(cvalues)))CALL cp__a("motion/averages_types.F",301)
                   IF(.NOT.(nint*nint==SIZE(Mmatrix)))CALL cp__a("motion/averages_types.F",302)
...
! *****************************************************************************
  SUBROUTINE get_averages_rv(avg, add, delta_t)
    ...
    IF(.NOT.(check))CALL cp__a("motion/averages_types.F",480)
    DO i = 1, SIZE(avg)
...
! *****************************************************************************
  SUBROUTINE get_averages_rm(avg, add, delta_t)
    ...
    IF(.NOT.(check))CALL cp__a("motion/averages_types.F",507)
    check = SIZE(avg,2)==SIZE(add,2)
    IF(.NOT.(check))CALL cp__a("motion/averages_types.F",509)
    DO i = 1, SIZE(avg,2)
       DO j = 1, SIZE(avg,1)
