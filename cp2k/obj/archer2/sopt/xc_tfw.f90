# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_tfw.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_tfw.F"
!-----------------------------------------------------------------------------!
...
# 26 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_tfw.F" 2
...
! *****************************************************************************
  SUBROUTINE tfw_lda_eval (rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_tfw.F",147)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_tfw.F",148)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_tfw.F",149)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_tfw.F",150)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_tfw.F",205,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE tfw_lsd_eval(rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_tfw.F",271)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_tfw.F",272)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_tfw.F",273)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_tfw.F",274)
    ...
    DO ispin=1,2
       ...
       END IF
       IF ( order>3.OR.order<-3) THEN
          CALL cp__b("xc/xc_tfw.F",341,"derivatives bigger than 3 not implemented")
       END IF
    END DO
