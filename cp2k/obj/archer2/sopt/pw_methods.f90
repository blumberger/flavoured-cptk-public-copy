# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_methods.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_zero ( pw)
    ...
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",96)
    ...
    ELSE
       CALL cp__b("pw/pw_methods.F",110,"No possible data field!")
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_copy ( pw1, pw2)
    ...
    IF(.NOT.(pw1%ref_count>0))CALL cp__a("pw/pw_methods.F",144)
    IF(.NOT.(pw2%ref_count>0))CALL cp__a("pw/pw_methods.F",145)
...
!$omp parallel do private(i) default(none) shared(ng,ng2,pw2)
                ...
                ns = 2 * MAX ( ng1, ng2 )
             ELSE
                CALL cp__b("pw/pw_methods.F",170,"No suitable data field")
             END IF

          ELSE
             IF (output_unit > 0) THEN
                 WRITE ( output_unit, "(A,I5,T30,A,L1,T60,A,I5)" ) &
             ...
             END IF
             CALL cp__b("pw/pw_methods.F",185,"Incompatible grids")
          END IF
             ...
             ELSE
                IF (output_unit > 0) THEN 
                    ...
                    WRITE ( output_unit, "(A,I5,T30,A,L1,T60,A,I5)" ) &
                               ...
                               " reference :",pw2%pw_grid%reference
                ENDIF
                CALL cp__b("pw/pw_methods.F",240,"Incompatible grids")
             END IF

          ELSE
             CALL cp__b("pw/pw_methods.F",244,"No suitable data field")
          END IF
       ...
       ELSE
          IF (output_unit > 0) THEN
              ...
              WRITE ( output_unit, "(A,I5,T30,A,L1,T60,A,I5)" ) &
                         ...
                         " reference :",pw2%pw_grid%reference
          ENDIF
          CALL cp__b("pw/pw_methods.F",260,"Incompatible grids")
       END IF
...
!$omp parallel do private(i) default(none) shared(ns,pw2,pw1)
       ...
       ELSE IF ( pw1%in_use == COMPLEXDATA3D .AND. &
          ...
          pw2%cc3d(:,:,:) = pw1%cc3d(:,:,:)
       ELSE
          CALL cp__b("pw/pw_methods.F",286,"No suitable data field")
       END IF
...
! *****************************************************************************
  SUBROUTINE pw_scale( pw, a)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",316)
    ...
    CASE (COMPLEXDATA3D )
       ...
       pw%cc3d(:,:,:) = a*pw%cc3d(:,:,:)
    CASE DEFAULT
       CALL cp__b("pw/pw_methods.F",332,"No suitable data field" )
    END SELECT
...
! *****************************************************************************
  SUBROUTINE pw_derive ( pw, n)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",364)
    IF(.NOT.(ALL(n>=0)))CALL cp__a("pw/pw_methods.F",365)
    ...
    IF ( pw%in_space == RECIPROCALSPACE .AND. &
       ...
       IF ( n ( 1 ) == 1 ) THEN
!$omp parallel do private (i) default(none) shared(cnt, pw)
          DO i = 1, cnt
             pw%cc ( i ) = pw%cc ( i ) * pw%pw_grid%g ( 1, i )
          END DO
          flop = flop + 6 * cnt
       ELSE IF ( n ( 1 ) > 1 ) THEN
!$omp parallel do private (i) default(none) shared(cnt, pw,n)
          DO i = 1, cnt
       ...
       END IF
       IF ( n ( 2 ) == 1 ) THEN
!$omp parallel do private (i) default(none) shared(pw, cnt)
          DO i = 1, cnt
             pw%cc ( i ) = pw%cc ( i ) * pw%pw_grid%g ( 2, i )
          END DO
          flop = flop + 6 * cnt
       ELSE IF ( n ( 2 ) > 1 ) THEN
!$omp parallel do private (i) default(none) shared(pw, cnt,n)
          DO i = 1, cnt
...
!$omp parallel do private (i) default(none) shared(pw,cnt,im)
          DO i = 1, cnt
             pw%cc ( i ) = im * pw%cc ( i )
          END DO
          flop = flop + 6 * cnt
    ...
    ELSE
       ...
       CALL cp__b("pw/pw_methods.F",429,"No suitable data field")
...
! *****************************************************************************
  SUBROUTINE pw_dr2 ( pw, pwdr2, i, j)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",463)
...
!$omp parallel do private (ig) default(none) shared(pwdr2,pw,i,j,cnt)
    ...
    ELSE
       ...
       CALL cp__b("pw/pw_methods.F",490,"No suitable data field")
...
! *****************************************************************************
  SUBROUTINE pw_dr2_gg ( pw, pwdr2_gg, i, j)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",525)
...
!$omp parallel do private (ig) default(none) shared(pwdr2_gg,pw,i,j,cnt)
    ...
    ELSE
       ...
       CALL cp__b("pw/pw_methods.F",555,"No suitable data field")
...
! *****************************************************************************
  SUBROUTINE pw_smoothing ( pw, ecut, sigma)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",588)
...
!$omp parallel do default(none) private(ig,arg,f) shared(sigma,ecut,pw,cnt)
    ...
    ELSE
       ...
       CALL cp__b("pw/pw_methods.F",607,"No suitable data field")
...
! *****************************************************************************
  SUBROUTINE pw_transfer ( pw1, pw2, debug)
    ...
    IF(.NOT.(pw1%ref_count>0))CALL cp__a("pw/pw_methods.F",642)
    IF(.NOT.(pw2%ref_count>0))CALL cp__a("pw/pw_methods.F",643)
    ...
    ELSEIF ( pw1%in_space == RECIPROCALSPACE .AND. &
       ...
       ELSE
          ...
          ELSE
             CALL cp__b("pw/pw_methods.F",666,"Do not know what to do" )
          END IF
...
! *****************************************************************************
  SUBROUTINE pw_axpy ( pw1, pw2, alpha)
    ...
    IF(.NOT.(pw1%ref_count>0))CALL cp__a("pw/pw_methods.F",712)
    IF(.NOT.(pw2%ref_count>0))CALL cp__a("pw/pw_methods.F",713)
...
!$omp parallel do private(i) default(none) shared(pw2,my_alpha,pw1)
       ...
       ELSE IF ( pw1%in_use == COMPLEXDATA3D .AND. &
          ...
          END IF
       ELSE
          CALL cp__b("pw/pw_methods.F",767,"No suitable data field")
       END IF
          ...
          ELSE
             IF (output_unit > 0) THEN
                ...
                WRITE ( output_unit, "(A,I5,T30,A,L1,T60,A,I5)" ) &
                           ...
                           " reference :",pw2%pw_grid%reference
             ENDIF
             CALL cp__b("pw/pw_methods.F",869,"Grids not compatible")

          END IF

       ELSE
          CALL cp__b("pw/pw_methods.F",874,"No suitable data field")
       END IF
    ...
    ELSE
       IF (output_unit > 0) THEN
          ...
          WRITE ( output_unit, "(A,I5,T30,A,L1,T60,A,I5)" ) &
                     ...
                     " reference :",pw2%pw_grid%reference
       ENDIF
       CALL cp__b("pw/pw_methods.F",888,"Grids not compatible")
...
! *****************************************************************************
  SUBROUTINE pw_gather_s ( pw, c, scale)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",922)

    IF ( pw%in_use /= COMPLEXDATA1D ) THEN
       CALL cp__b("pw/pw_methods.F",925,"Data field has to be COMPLEXDATA1D")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE pw_gather_p ( pw, c, scale)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",993)

    IF ( pw%in_use /= COMPLEXDATA1D ) THEN
       CALL cp__b("pw/pw_methods.F",996,"Data field has to be COMPLEXDATA1D")
    ENDIF

    IF ( pw%pw_grid%para%mode /= PW_MODE_DISTRIBUTED ) THEN
       CALL cp__b("pw/pw_methods.F",1000,"This grid type is not distributed")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE pw_scatter_s ( pw, c, scale)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",1075)

    IF ( pw%in_use /= COMPLEXDATA1D ) THEN
       CALL cp__b("pw/pw_methods.F",1078,"Data field has to be COMPLEXDATA1D")
    ENDIF

    IF ( pw%in_space /= RECIPROCALSPACE ) THEN
       CALL cp__b("pw/pw_methods.F",1082,"Data has to be in RECIPROCALSPACE")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE pw_scatter_p ( pw, c, scale)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_methods.F",1184)

    IF ( pw%in_use /= COMPLEXDATA1D ) THEN
       CALL cp__b("pw/pw_methods.F",1187,"Data field has to be COMPLEXDATA1D")
    ENDIF

    IF ( pw%in_space /= RECIPROCALSPACE ) THEN
       CALL cp__b("pw/pw_methods.F",1191,"Data has to be in RECIPROCALSPACE")
    ENDIF

    IF ( pw%pw_grid%para%mode /= PW_MODE_DISTRIBUTED ) THEN
       CALL cp__b("pw/pw_methods.F",1195,"This grid type is not distributed")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE fft_wrap_pw1pw2 ( pw1, pw2, debug)
    ...
    IF(.NOT.(pw1%ref_count>0))CALL cp__a("pw/pw_methods.F",1323)
    IF(.NOT.(pw2%ref_count>0))CALL cp__a("pw/pw_methods.F",1324)
    NULLIFY ( c_in )
    NULLIFY ( c_out )
    ...
    IF ( pw1%pw_grid %id_nr /= pw2%pw_grid %id_nr ) THEN
       IF ( pw1%pw_grid%dvol /= pw2%pw_grid%dvol ) THEN
          CALL cp__b("pw/pw_methods.F",1337,"PW grids not compatible")
       END IF
       IF ( pw1%pw_grid%para%group /= pw2%pw_grid%para%group) THEN
          CALL cp__b("pw/pw_methods.F",1340,"PW grids have not compatible MPI groups")
       END IF
    END IF
    ...
    ELSE IF ( pw1%in_space == RECIPROCALSPACE ) THEN
       ...
       out_space = REALSPACE
    ELSE
       CALL cp__b("pw/pw_methods.F",1354,"Error in space tag")
    END IF
    ...
    IF ( pw1%pw_grid%para%mode == PW_MODE_LOCAL ) THEN
       ...
       IF ( test .AND. output_unit > 0 ) THEN
          WRITE ( output_unit,'(A)') " FFT Protocol "
          IF ( dir == FWFFT ) WRITE ( output_unit,'(A,T76,A)') "  Transform direction ","FWFFT"
          IF ( dir == BWFFT ) WRITE ( output_unit,'(A,T76,A)') "  Transform direction ","BWFFT"
          IF ( pw1%in_space == REALSPACE ) &
               WRITE ( output_unit,'(A,T72,A)') "  in space ","REALSPACE"
          IF ( pw1%in_space == RECIPROCALSPACE ) &
               WRITE ( output_unit,'(A,T66,A)') "  in space ","RECIPROCALSPACE"
          IF ( out_space == REALSPACE ) &
               WRITE ( output_unit,'(A,T72,A)') "  out space ","REALSPACE"
          IF ( out_space == RECIPROCALSPACE ) &
               WRITE ( output_unit,'(A,T66,A)') "  out space ","RECIPROCALSPACE"
          WRITE ( output_unit,'(A,T66,E15.6)') "  scale factor",norm
       END IF

       SELECT CASE ( mode )
       CASE DEFAULT
          CALL cp__b("pw/pw_methods.F",1384,"Illegal combination of in_use and in_space")
       CASE ( "FW_C3DC3D" )
          c_in => pw1%cc3d
          c_out => pw2%cc3d
       ...
       CASE ( "FW_C3DC1D" )
          ...
          IF (stat /= 0) CALL stop_memory(routineN,moduleN,1396,&
                                          "c_out",2*dp_size*PRODUCT(n(1:3)))
          ! transform
          CALL fft3d ( dir, n, c_in, c_out, scale = norm, debug = test )
          ! gather results
          IF ( test .AND. output_unit > 0 ) WRITE ( output_unit,'(A)') "  PW_GATHER : 3d -> 1d "
       ...
       CASE ( "FW_R3DC1D" )
          ...
          ALLOCATE ( c_out( n(1), n(2), n(3) ), STAT = stat )
          IF (stat /= 0) CALL stop_memory(routineN,moduleN,1409,&
                                          "c_out",2*dp_size*PRODUCT(n(1:3)))
       ...
       CASE ( "BW_C3DR3D" )
          c_in => pw1%cc3d
          ALLOCATE ( c_out( n(1), n(2), n(3) ), STAT = stat )
          IF (stat /= 0) CALL stop_memory(routineN,moduleN,1423,&
                                          "c_out",2*dp_size*PRODUCT(n(1:3)))
          CALL fft3d ( dir, n, c_in, c_out, scale = norm, debug = test )
          ! use real part only
          IF ( test .AND. output_unit > 0 ) WRITE ( output_unit,'(A)') "  REAL part "
          pw2%cr3d = REAL ( c_out,KIND=dp)
          DEALLOCATE ( c_out)
       CASE ( "BW_C1DC3D" )
          c_out => pw2%cc3d
          IF ( test .AND. output_unit > 0 ) WRITE ( output_unit,'(A)') "  PW_SCATTER : 3d -> 1d "
       ...
       CASE ( "BW_C1DR3D" )
          ...
          ALLOCATE ( c_out( n(1), n(2), n(3) ), STAT = stat)
          IF (stat /= 0) CALL stop_memory(routineN,moduleN,1440,&
                                          "c_out",2*dp_size*PRODUCT(n(1:3)))
          IF ( test .AND. output_unit > 0 ) WRITE ( output_unit,'(A)') "  PW_SCATTER : 3d -> 1d "
          CALL pw_scatter ( pw1, c_out)
          ! transform
       ...
       IF ( test .AND. output_unit > 0 ) WRITE ( output_unit,'(A)') " End of FFT Protocol "
    ...
    ELSE
       ...
       IF ( test .AND. pw1%pw_grid%para%group_head .AND. output_unit > 0 ) THEN
          WRITE ( output_unit,'(A)') " FFT Protocol "
          IF ( dir == FWFFT ) WRITE ( output_unit,'(A,T76,A)') "  Transform direction ","FWFFT"
          IF ( dir == BWFFT ) WRITE ( output_unit,'(A,T76,A)') "  Transform direction ","BWFFT"
          IF ( pw1%in_space == REALSPACE ) &
       ...
       IF(.NOT.(SIZE(grays,1)==n(1)))CALL cp__a("pw/pw_methods.F",1479)
       IF(.NOT.(SIZE(grays,2)==nrays))CALL cp__a("pw/pw_methods.F",1480)

       SELECT CASE ( mode )
       CASE DEFAULT
          CALL cp_abort(cp__l("pw/pw_methods.F",1484),&
       ...
       CASE ( "FW_C3DC1D" )
          ...
          END IF
          !..prepare output
          IF ( test .AND. pw1%pw_grid%para%group_head .AND. output_unit > 0 ) &
               WRITE ( output_unit,'(A)') "  PW_GATHER : 2d -> 1d "
       ...
       CASE ( "FW_R3DC1D" )
          ...
          IF (use_pw_cuda) THEN
             CALL pw_cuda_r3dc1d_3d_ps(pw1, pw2, scale = norm)
          ELSE
             !..prepare input
             nloc = pw1%pw_grid%npts_local
             ALLOCATE ( c_in( nloc(1), nloc(2), nloc(3) ), STAT = stat)
             IF (stat /= 0) CALL stop_memory(routineN,moduleN,1518,&
                                             "c_in",2*dp_size*PRODUCT(nloc(1:3)))
       ...
       CASE ( "BW_C1DC3D" )
          !..prepare input
          IF ( test .AND. pw1%pw_grid%para%group_head .AND. output_unit > 0 ) &
               WRITE ( output_unit,'(A)') "  PW_SCATTER : 2d -> 1d "
          CALL zero_c(grays)
       ...
       CASE ( "BW_C1DR3D" )
          ...
          IF (use_pw_cuda) THEN
             CALL pw_cuda_c1dr3d_3d_ps(pw1, pw2, scale = norm)
          ELSE
             !..prepare input
             IF ( test .AND. pw1%pw_grid%para%group_head .AND. output_unit > 0 ) &
                  WRITE ( output_unit,'(A)') "  PW_SCATTER : 2d -> 1d "
             ...
             ALLOCATE ( c_in( nloc(1), nloc(2), nloc(3) ), STAT = stat )
             IF (stat /= 0) CALL stop_memory(routineN,moduleN,1573,&
                                             "c_in",2*dp_size*PRODUCT(nloc(1:3)))
             !..transform
             IF ( pw1%pw_grid%para%ray_distribution ) THEN
                CALL fft3d ( dir, n, c_in, grays, pw1%pw_grid%para%group, &
    ...
    IF ( test .AND. pw1%pw_grid%para%group_head .AND. output_unit > 0 ) THEN
       WRITE ( output_unit,'(A)') " End of FFT Protocol "
    END IF
    CALL timestop(handle)
...
! *****************************************************************************
  FUNCTION fftselect ( use1, use2, space1) RESULT ( mode )
    ...
    ELSE IF ( space1 == RECIPROCALSPACE ) THEN
       mode ( 1 : 3 ) = "BW_"
    ELSE
       CALL cp__b("pw/pw_methods.F",1626,"Error in space tag")
    END IF
    ...
    CASE ( REALDATA1D )
       mode ( 4 : 6 ) = "R1D"
    CASE DEFAULT
       CALL cp__b("pw/pw_methods.F",1639,"Error in use1 tag")
    END SELECT
    ...
    CASE ( REALDATA1D )
       mode ( 7 : 9 ) = "R1D"
    CASE DEFAULT
       CALL cp__b("pw/pw_methods.F",1652,"Error in use1 tag")
    END SELECT
...
! *****************************************************************************
  FUNCTION pw_integral_ab ( pw1, pw2) RESULT ( integral_value )
    ...
    IF ( pw1%pw_grid %id_nr /= pw2%pw_grid %id_nr ) THEN
       CALL cp__b("pw/pw_methods.F",1779,"Grids incompatible")
    END IF
    ...
    ELSE
       CALL cp__b("pw/pw_methods.F",1812,"No possible DATA")
    END IF
...
! *****************************************************************************
  FUNCTION pw_integral_a2b ( pw1, pw2) RESULT ( integral_value )
    ...
    IF ( pw1%pw_grid %id_nr /= pw2%pw_grid %id_nr ) THEN
       CALL cp__b("pw/pw_methods.F",1848,"Grids incompatible")
    END IF
    ...
    ELSE IF ( pw1%in_use == COMPLEXDATA1D .AND. &
       ...
       END IF
    ELSE
       CALL cp__b("pw/pw_methods.F",1862,"No possible DATA")
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_structure_factor ( sf, r)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(sf%ref_count>0))CALL cp__a("pw/pw_methods.F",1898)
    flop = 0.0_dp
...
!$omp parallel do private (ig,arg) default(none) shared(sf,r,cnt)
    ...
    ELSE
       ...
       CALL cp__b("pw/pw_methods.F",1914,"No suitable data field")
  ...
  FUNCTION pw_integrate_function(fun,isign,oprt) RESULT(total_fun)
    ...
    IF ( PRESENT(oprt) ) THEN
       ...
       CASE ("ABS","abs")
          iop = 1
       CASE DEFAULT
          CALL cp__b("pw/pw_methods.F",1948,"Unknown operator")
       END SELECT
    END IF
    ...
    IF (fun%in_space == REALSPACE) THEN
       IF (fun%in_use == REALDATA3D) THEN
          ...
          END IF
       ELSE
          CALL cp__b("pw/pw_methods.F",1963,"In_space/in_use combination not implemented")
       END IF
    ELSEIF (fun%in_space == RECIPROCALSPACE) THEN
       IF (iop==1) &
            CALL cp__b("pw/pw_methods.F",1967,"Operator ABS not implemented")
       IF (fun%in_use == COMPLEXDATA1D) THEN
          IF ( fun%pw_grid%have_g0 ) total_fun = REAL(fun%pw_grid%vol*fun%cc(1),KIND=dp)
       ELSE
          CALL cp__b("pw/pw_methods.F",1971,"In_space/in_use combination not implemented")
       END IF
    ELSE
       CALL cp__b("pw/pw_methods.F",1974,"No space defined")
    END IF
