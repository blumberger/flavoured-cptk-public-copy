# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_qs.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_qs.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/almo_scf_qs.F" 2
    ...
    DO dimen=1,2 ! 1 - row, 2 - column dimension
       ...
       IF (size_keys(dimen)==almo_mat_dim_aobasis) THEN ! this dimension is AO
          ...
          ELSE IF (almo_scf_env%mat_distr_aos==almo_mat_distr_molecular) THEN
             ...
             END IF
          ELSE
             CALL cp__b("almo_scf_qs.F",212,"Illegal distribution")
          ENDIF
       ...
       ELSE ! this dimension is not AO
          ...
          IF (size_keys(dimen)==almo_mat_dim_occ .OR.&
             ...
             ELSE IF (almo_scf_env%mat_distr_mos==almo_mat_distr_molecular) THEN
                ...
                ENDIF
             ELSE
                CALL cp__b("almo_scf_qs.F",248,"Illegal distribution")
             ENDIF
             
          ELSE
 
             CALL cp__b("almo_scf_qs.F",253,"Illegal dimension")
       ...
       DO row = 1, nblkrows_tot
          ...
          IF(hold.EQ.mynode) THEN
             ...
             IF (active) THEN
                ...
                IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_qs.F",388)
                p_new_block(:,:) = 1.0_dp
             ENDIF
...
! *****************************************************************************
  SUBROUTINE almo_scf_init_qs(qs_env,almo_scf_env)
    ...
    IF(.NOT.(ASSOCIATED(mscfg_env)))CALL cp__a("almo_scf_qs.F",584)
                ...
                CASE (almo_constraint_block_diagonal)
                   ...
                   IF (almo_scf_env%domain_layout_mos==almo_domain_layout_molecular) THEN
                      ...
                      ELSE ! ao domains are atomic
                         ...
                         CALL cp__b("almo_scf_qs.F",1035,"Illegal: atomic domains and molecular groups")
                   ...
                   ELSE ! electron groups are atomic
                      ...
                      IF (almo_scf_env%domain_layout_aos==almo_domain_layout_molecular) THEN
                         ...
                         CALL cp__b("almo_scf_qs.F",1045,"Illegal: molecular domains and atomic groups")
                   ...
                   IF ( block_active ) THEN
                      ...
                      IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_qs.F",1063)
                      p_new_block(:,:) = 1.0_dp
                      
                      IF (domain_map_local_entries.ge.max_domain_neighbors*almo_scf_env%ndomains) THEN
                         CALL cp__b("almo_scf_qs.F",1067,"weird... max_domain_neighbors is exceeded")
                      ENDIF
                ...
                CASE (almo_constraint_ao_overlap)
                   ...
                   IF (almo_scf_env%domain_layout_mos==almo_domain_layout_molecular) THEN
                      ...
                      ELSE ! ao domains are atomic
                         ...
                         CALL cp__b("almo_scf_qs.F",1100,"atomic domains and molecular groups - NYI")
                   ...
                   ELSE ! electron groups are atomic
                      ...
                      IF (almo_scf_env%domain_layout_aos==almo_domain_layout_molecular) THEN
                         ...
                         CALL cp__b("almo_scf_qs.F",1111,"molecular domains and atomic groups - NYI")
                   ...
                   IF ( s0.lt.0.0_dp ) THEN
                      CALL cp__b("almo_scf_qs.F",1139,"S0 is less than zero")
                   ENDIF
                   IF ( s1.le.0.0_dp ) THEN
                      CALL cp__b("almo_scf_qs.F",1142,"S1 is less than or equal to zero")
                   ENDIF
                   IF ( s0.ge.s1 ) THEN
                      CALL cp__b("almo_scf_qs.F",1145,"S0 is greater than or equal to S1")
                   ENDIF
                   ...
                   IF ( overlap.lt.s1 ) THEN
                      ...
                      CALL cp_dbcsr_reserve_block2d(almo_scf_env%quench_t(ispin),&
                              iblock_row, iblock_col, p_new_block)
                      IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_qs.F",1153)
                      ...
                      IF (ABS(p_new_block(1,1)).gt.ABS(almo_scf_env%eps_filter)) THEN
                         IF (domain_map_local_entries.ge.max_domain_neighbors*almo_scf_env%ndomains) THEN
                            CALL cp__b("almo_scf_qs.F",1166,"weird... max_domain_neighbors is exceeded")
                         ENDIF
                         almo_scf_env%domain_map(ispin)%pairs(domain_map_local_entries+1,1)=iblock_row
                         almo_scf_env%domain_map(ispin)%pairs(domain_map_local_entries+1,2)=iblock_col
                         domain_map_local_entries=domain_map_local_entries+1
                ...
                CASE (almo_constraint_distance)
                   ...
                   IF (almo_scf_env%domain_layout_mos==almo_domain_layout_molecular) THEN
                      ...
                      IF (almo_scf_env%domain_layout_aos==almo_domain_layout_molecular) THEN
                         ...
                         ELSE
                            ...
                            DO iatom=first_atom_of_molecule(domain_row),last_atom_of_molecule(domain_row)
                               ...
                               ENDDO ! jatom
                            ENDDO ! iatom
                            IF(.NOT.(contact_atom_1.gt.0))CALL cp__a("almo_scf_qs.F",1207)
                      ...
                      ELSE ! ao domains are atomic
                         ...
                         CALL cp__b("almo_scf_qs.F",1215,"atomic domains and molecular groups - NYI")
                   ...
                   ELSE ! electron groups are atomic
                      ...
                      IF (almo_scf_env%domain_layout_aos==almo_domain_layout_molecular) THEN
                         ...
                         CALL cp__b("almo_scf_qs.F",1226,"molecular domains and atomic groups - NYI")
                   ...
                   ELSE
                      CALL cp__b("almo_scf_qs.F",1253,"Illegal quencher_radius_type")
                   END IF
...
! the element specific radii
                   ...
                   IF ( r0.lt.0.0_dp ) THEN
                      CALL cp__b("almo_scf_qs.F",1268,"R0 is less than zero")
                   ENDIF
                   IF ( r1.le.0.0_dp ) THEN
                      CALL cp__b("almo_scf_qs.F",1271,"R1 is less than or equal to zero")
                   ENDIF
                   IF ( r0.gt.r1 ) THEN
                      CALL cp__b("almo_scf_qs.F",1274,"R0 is greater than or equal to R1")
                   ENDIF
                   ...
                   IF ( distance.lt.r1 ) THEN
                      ...
                      CALL cp_dbcsr_reserve_block2d(almo_scf_env%quench_t(ispin),&
                              iblock_row, iblock_col, p_new_block)
                      IF(.NOT.(ASSOCIATED(p_new_block)))CALL cp__a("almo_scf_qs.F",1282)
                      ...
                      IF (ABS(p_new_block(1,1)).gt.ABS(almo_scf_env%eps_filter)) THEN
                         IF (domain_map_local_entries.ge.max_domain_neighbors*almo_scf_env%ndomains) THEN
                            CALL cp__b("almo_scf_qs.F",1299,"weird... max_domain_neighbors is exceeded")
                         ENDIF
                         almo_scf_env%domain_map(ispin)%pairs(domain_map_local_entries+1,1)=iblock_row
                         almo_scf_env%domain_map(ispin)%pairs(domain_map_local_entries+1,2)=iblock_col
                         domain_map_local_entries=domain_map_local_entries+1
                ...
                CASE DEFAULT
                   CALL cp__b("almo_scf_qs.F",1309,"Illegal constraint type")
                END SELECT
       ...
       IF (nblks.ne.domain_map_local_entries) THEN
          CALL cp__b("almo_scf_qs.F",1329,"number of blocks is wrong")
       ENDIF
       ...
       max_neig_loop: DO WHILE (max_neig_fails)
          ...
          DO ientry=1,global_entries
             ...
             ELSE
                ...
                CALL cp__b("almo_scf_qs.F",1419,"all records must be inserted")
             ENDIF
          ENDDO
