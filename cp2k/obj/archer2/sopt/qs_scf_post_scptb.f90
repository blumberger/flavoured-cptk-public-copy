# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_scptb.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_scptb.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_scptb.F" 2
...
! *****************************************************************************
  SUBROUTINE write_scptb_mo_free_results(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_scptb.F",112)
    ...
    IF(BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file))THEN
       CALL cp_warn(cp__l("qs_scf_post_scptb.F",130),&
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_section,&
        ...
        CALL cp_warn(cp__l("qs_scf_post_scptb.F",139),&
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_section,&
        ...
        CALL cp_warn(cp__l("qs_scf_post_scptb.F",148),&
             "Printing of cube files from SCPTB calculation currently " //&
             "not supported")
    END IF ! print key
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_section,&
        ...
        CALL cp_warn(cp__l("qs_scf_post_scptb.F",158),&
             "Printing of cube files from SCPTB calculation currently " //&
             "not supported")
    ENDIF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_section,&
        ...
        CALL cp_warn(cp__l("qs_scf_post_scptb.F",168),&
             "Printing of cube files from SCPTB calculation currently " //&
             "not supported")
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",206,"Hirshfeld charges not available for SCPTB method.")
    END IF

    ! KINETIC ENERGY 
    print_key => section_vals_get_subs_vals(print_section,"KINETIC_ENERGY")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",212,"Kinetic energy not available for SCPTB method.")
    END IF

    ! Xray diffraction spectrum
    print_key => section_vals_get_subs_vals(print_section,"XRAY_DIFFRACTION_SPECTRUM")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",218,"Xray diffraction spectrum not implemented for SCPTB method.")
    END IF

    ! Electric field gradients
    print_key => section_vals_get_subs_vals(print_section,"ELECTRIC_FIELD_GRADIENT")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",224,"Electric field gradient not implemented for SCPTB method.")
    END IF

    ! STM
    print_key => section_vals_get_subs_vals(print_section,"STM")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",230,"STM not implemented for SCPTB method.")
    END IF

    ! MO 
    print_key => section_vals_get_subs_vals(print_section,"MO")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",236,"Printing of MO properties not implemented for SCPTB method.")
    END IF

    ! MO CUBES
    print_key => section_vals_get_subs_vals(print_section,"MO_CUBES")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",242,"Printing of MO cube files not implemented for SCPTB method.")
    END IF
    ...
    IF(explicit.AND..NOT.qs_env%run_rtp) THEN
      CALL cp__w("qs_scf_post_scptb.F",249,"Wavefunction mixing not implemented for SCPTB method.")
    END IF

    ! PLUS_U
    print_key => section_vals_get_subs_vals(print_section,"PLUS_U")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",255,"DFT+U method not implemented for SCPTB method.")
    END IF

    ! PDOS
    print_key => section_vals_get_subs_vals(print_section,"PDOS")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",261,"Projected DOS not implemented for SCPTB method.")
    END IF

    ! V_XC CUBE FILE 
    print_key => section_vals_get_subs_vals(print_section,"V_XC_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",267,"XC potential cube file not available for SCPTB method.")
    END IF

    ! ELF
    print_key => section_vals_get_subs_vals(print_section,"ELF_CUBE")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",273,"ELF not implemented for SCPTB method.")
    END IF

    ! EPR Hyperfine Coupling
    print_key => section_vals_get_subs_vals(print_section,"HYPERFINE_COUPLING_TENSOR")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",279,"Hyperfine Coupling not implemented for SCPTB method.")
    END IF

    ! Lowdin
    print_key => section_vals_get_subs_vals(print_section,"LOWDIN")
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
      CALL cp__w("qs_scf_post_scptb.F",285,"Lowdin population analysis not implemented for SCPTB method.")
    END IF
