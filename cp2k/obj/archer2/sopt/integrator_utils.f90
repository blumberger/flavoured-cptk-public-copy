# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/integrator_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/integrator_utils.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/integrator_utils.F" 2
...
! *****************************************************************************
  ...
  PUBLIC :: old_variables_type, tmp_variables_type, allocate_old, deallocate_old,&
       ...
       variable_timestep 
!> CHANGE_AC 
!       vv_second_if_sh_rescaling, update_forces_newstate
...
! *****************************************************************************
  SUBROUTINE allocate_old ( old, particle_set, npt)
    ...
    idim = SIZE ( npt, 1)
    jdim = SIZE ( npt, 2)
    IF(.NOT.(.NOT.ASSOCIATED(old)))CALL cp__a("motion/integrator_utils.F",131)
    ALLOCATE ( old)
...
! *****************************************************************************
  SUBROUTINE allocate_tmp ( md_env, tmp, nparticle, nshell, shell_adiabatic)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(tmp)))CALL cp__a("motion/integrator_utils.F",202)
    ALLOCATE (tmp)
...
! *****************************************************************************
  SUBROUTINE update_dealloc_tmp ( tmp, particle_set, shell_particle_set, &
    ...
    IF(.NOT.(ASSOCIATED(tmp)))CALL cp__a("motion/integrator_utils.F",288)
    my_pos =.FALSE.
    my_vel =.FALSE.
    my_deall=.TRUE.
    ...
    IF(my_vel) THEN
       ...
       IF(my_deall)THEN
          ...
          END IF
          IF(.NOT.(.NOT.ASSOCIATED(tmp%pos)))CALL cp__a("motion/integrator_utils.F",322)
          IF(.NOT.(.NOT.ASSOCIATED(tmp%shell_pos)))CALL cp__a("motion/integrator_utils.F",323)
          IF(.NOT.(.NOT.ASSOCIATED(tmp%core_pos)))CALL cp__a("motion/integrator_utils.F",324)
          DEALLOCATE (tmp)
       END IF
    END IF
...
! *****************************************************************************
!> \brief Second half of the velocity-verlet algorithm : when a hop occurs in 
!>        surface hopping calculation, the SH subroutine calculate the second 
!>        half of the velocities
!> \param tmp ...
!> \param atomic_kind_set ...
!> \param local_particles ...
!> \param particle_set ...
...
!> \author AC (June 2016)
! *****************************************************************************
  SUBROUTINE vv_second_if_sh_rescaling ( tmp, atomic_kind_set, local_particles, particle_set,&
       core_particle_set, shell_particle_set, nparticle_kind,&
       shell_adiabatic, dt, u, new_velocities)
    ...
    REAL(KIND=dp)                            :: dt
    REAL(KIND=dp), DIMENSION(3, 3), OPTIONAL :: u
    REAL(KIND=dp), DIMENSION(:,:), POINTER   :: new_velocities
    ...
    DO iparticle_kind=1,nparticle_kind
       ...
       IF (mass/=0.0_dp) THEN
          dm = 0.5_dp*dt/mass
          IF(is_shell .AND. shell_adiabatic) THEN
             CALL cp__b("motion/integrator_utils.F",1876,'NOT IMPLEMENTED WITH SURFACE HOPPING')
             !CALL get_atomic_kind(atomic_kind=atomic_kind, shell=shell)
             !dms = 0.5_dp*dt/shell%mass_shell
             !dmc = 0.5_dp*dt/shell%mass_core
             !fac_masss = shell%mass_shell/mass
          ...
          ELSE
             ...
             ELSE
                DO iparticle_local=1,nparticle_local
                   iparticle = local_particles%list(iparticle_kind)%array(iparticle_local)
                   tmp%vel = new_velocities
                   !tmp%vel( 1, iparticle) = new_velocities( 1, iparticle, 1)
                   !tmp%vel( 2, iparticle) = new_velocities( iparticle, 2)
                   !tmp%vel( 3, iparticle) = new_velocities( iparticle, 3)
                   !tmp%vel ( 1, iparticle) = &
                   !     tmp%vel ( 1, iparticle) * tmp%scale_v(1) * tmp%scale_v(1)  + &
                   !     tmp%scale_v(1) * tmp%poly_v(1) * dm*particle_set(iparticle)%f(1)
                   !tmp%vel ( 2, iparticle) = &
  ...
  END SUBROUTINE vv_second_if_sh_rescaling

!  SUBROUTINE update_forces_newstate ( atomic_kind_set, local_particles, particle_set,&
!       nparticle_kind, new_forces)
!
!    TYPE(atomic_kind_type), DIMENSION(:), &
!      POINTER                                :: atomic_kind_set
!    TYPE(distribution_1d_type), POINTER      :: local_particles
...
!    REAL(KIND=dp), DIMENSION(:,:), POINTER   :: new_velocities
!
!    CHARACTER(len=*), PARAMETER :: routineN = 'update_forces_newstate', &
!      routineP = moduleN//':'//routineN
!
!    INTEGER                                  :: iparticle, iparticle_kind, &
!                                                iparticle_local, &
...
!       DO iparticle_local=1,nparticle_local
!                   iparticle = local_particles%list(iparticle_kind)%array(iparticle_local)
!                   particle_set(iparticle)%f(1) =  new_forces( 1, iparticle)
!                   particle_set(iparticle)%f(2) =  new_forces( 2, iparticle)
!                   particle_set(iparticle)%f(3) =  new_forces( 3, iparticle)
!       END DO
!    END DO
!
!  END SUBROUTINE update_forces_newstate
