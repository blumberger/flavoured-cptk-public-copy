# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_force.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_force.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_nonbond_force.F" 2
...
! *****************************************************************************
  SUBROUTINE force_nonbond(fist_nonbond_env, ewald_env, particle_set, cell, &
    ...
    IF (PRESENT(fshell_nonbond)) THEN
       IF(.NOT.(PRESENT(fcore_nonbond)))CALL cp__a("fist_nonbond_force.F",165)
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,172,"mm_radius",dp_size*nkind)
    ALLOCATE (qeff(nkind),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,174,"qeff",dp_size*nkind)
    ALLOCATE (qcore(nkind),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,176,"qcore",dp_size*nkind)
    ALLOCATE (qshell(nkind),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,178,"qshell",dp_size*nkind)
    ALLOCATE (is_shell_kind(nkind),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,180,"is_shell_kind",int_size*nkind)
    IF(.NOT.(istat==0))CALL cp__a("fist_nonbond_force.F",181)
    DO ikind=1,nkind
       ...
       Kind_Group_Loop: DO igrp=1,neighbor_kind_pair%ngrp_kind
          ...
          Pairs: DO ipair=istart,iend
             ...
             IF (shell_type /= nosh_nosh) THEN
                IF(.NOT.(.NOT.do_multipoles))CALL cp__a("fist_nonbond_force.F",316)
                IF(.NOT.(shell_present))CALL cp__a("fist_nonbond_force.F",317)
             END IF
             rab2_max = pot%rcutsq
