# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_neighbor_list_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_neighbor_list_types.F"
!-----------------------------------------------------------------------------!
...
# 23 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_neighbor_list_types.F" 2
...
! *****************************************************************************
  SUBROUTINE neighbor_list_iterator_create(iterator_set,nl,search,nthread)
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,167,&
    ...
    IF (PRESENT(search)) THEN
      IF (search) THEN
         ...
         IF (istat /= 0) CALL stop_memory(routineN,moduleN,201,&
                                          "list_search",0)
         DO il=1,SIZE(nl)
...
! *****************************************************************************
  SUBROUTINE nl_set_sub_iterator(iterator_set,ikind,jkind,iatom)
    ...
    IF (ASSOCIATED(iterator%list_search)) THEN
       ...
       END IF
    ELSE
       CALL cp__b("qs_neighbor_list_types.F",323,"")
    END IF
...
! Sub-iterator single-threaded, or with private sub-iterators only
    ...
    IF (ASSOCIATED(iterator%neighbor_list)) THEN
       ...
       ELSEIF (iterator%inode > 0) THEN
          ...
          iterator%neighbor_node => iterator%neighbor_node%next_neighbor_node
       ELSE
          CALL cp__b("qs_neighbor_list_types.F",479,"wrong")
       END IF
    ELSE
       ! no list available
       istat  = 1
...
!         *** Allocate a new neighbor list ***
          ...
          IF (istat /= 0) THEN
            CALL stop_memory(routineN,moduleN,586,&
                             "new_neighbor_list",0)
          END IF
...
!         *** Allocate a new first neighbor list ***
          ...
          ALLOCATE (new_neighbor_list,STAT=istat)
          IF (istat /= 0) THEN
            CALL stop_memory(routineN,moduleN,609,&
                             "new_neighbor_list",0)
          END IF
...
!     *** Return a pointer to the new neighbor list ***
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",645,"The requested neighbor list set is not associated")
...
!         *** Allocate a new neighbor node ***
          ...
          IF (istat /= 0) THEN
            CALL stop_memory(routineN,moduleN,704,&
                             "new_neighbor_node",0)
          END IF
...
!         *** Allocate a new first neighbor node ***
          ...
          ALLOCATE (new_neighbor_node,STAT=istat)
          IF (istat /= 0) THEN
            CALL stop_memory(routineN,moduleN,726,&
                             "new_neighbor_node",0)
          END IF
...
!     *** Increment the neighbor node counter ***
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",756,"The requested neighbor list is not associated")
...
!   *** Allocate a set of neighbor lists ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,794,&
                       "neighbor_list_set",0)
    END IF
...
! *****************************************************************************
  SUBROUTINE get_neighbor_list(neighbor_list,atom,nnode)
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",929,"The requested neighbor list is not associated")
...
! *****************************************************************************
  SUBROUTINE get_neighbor_list_set(neighbor_list_set,r_max,nlist,symmetric,mic,molecular)
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",967,"The requested neighbor list set is not associated")
  ...
  SUBROUTINE get_neighbor_list_set_p(neighbor_list_sets,r_max,nlist,symmetric,mic,molecular)
    ...
    IF (ASSOCIATED(neighbor_list_sets)) THEN
      ...
      IF (ASSOCIATED(neighbor_list_set)) THEN
         ...
         IF (PRESENT(molecular)) molecular = neighbor_list_set%molecular
      ELSE
         CALL cp_abort(cp__l("qs_neighbor_list_types.F",1014),"No neighbor list set is associated. "//&
              "Did you specify *all* required basis-sets, eg. for ADMM?")
      END IF

    ELSE

      CALL cp__b("qs_neighbor_list_types.F",1020,"The requested neighbor list sets are not associated")
...
! *****************************************************************************
  SUBROUTINE get_neighbor_node(neighbor_node,neighbor,cell,r)
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",1056,"The requested neighbor node is not associated")
...
! *****************************************************************************
  SUBROUTINE init_neighbor_list_set(neighbor_list_set,r_max,symmetric,mic,molecular)
    ...
    ELSE
      ...
      CALL cp__b("qs_neighbor_list_types.F",1102,"The requested neighbor list set is not associated")
