# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_symmetry.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_symmetry.F"
!-----------------------------------------------------------------------------!
...
# 36 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_symmetry.F" 2
...
! *****************************************************************************
  SUBROUTINE write_symmetry(particle_set,cell,input_section)
    ...
    LOGICAL                                  :: check, molecular, pall, &
                                                pcoor, pinertia, prmat, &
                                                psymmele
    ...
    IF (iw > 0) THEN
      ...
      IF ( molecular ) THEN
        ...
        IF ( TRIM(esymm) /= "NONE" ) THEN
          ...
          IF(.NOT.(check))CALL cp__a("cp_symmetry.F",143)
        END IF
      ...
      ELSE
        ...
        IF ( TRIM(esymm) /= "NONE" ) THEN
          ...
          IF(.NOT.(check))CALL cp__a("cp_symmetry.F",188)
        END IF
