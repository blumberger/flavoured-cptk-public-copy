# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_core_matrix.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_core_matrix.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scptb_core_matrix.F" 2
...
! *****************************************************************************
  SUBROUTINE build_scptb_core_matrix(qs_env,calculate_forces)
    ...
    INTEGER :: atom_a, atom_b, handle, i, iatom, ic, icol, icor, ikind, &
      inode, irow, j, jatom, jkind, l, li, ll, natom, nder, nimg, nkind, &
      nspins
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF(use_cell_mapping) THEN
          ...
          IF(.NOT.(ic > 0))CALL cp__a("scptb_core_matrix.F",263)
       ELSE
          ic = 1
       END IF
       ...
       IF(.NOT.(ASSOCIATED(h_block)))CALL cp__a("scptb_core_matrix.F",277)
       ...
       ELSE
          ...
          IF(.NOT.(ASSOCIATED(s_block)))CALL cp__a("scptb_core_matrix.F",288)
          IF ( irow == iatom ) THEN
            DO i=1,SIZE(h_block,1)
          ...
          IF(calculate_forces) THEN
            ...
            IF(.NOT.(ASSOCIATED(p_block)))CALL cp__a("scptb_core_matrix.F",308)
            DO icor=1,3
              force_ab(icor) = 0._dp
              CALL cp_dbcsr_get_block_p(matrix_s(icor+1,ic)%matrix,irow,icol,ds_block,found)
              IF(.NOT.(ASSOCIATED(ds_block)))CALL cp__a("scptb_core_matrix.F",312)
              IF ( irow == iatom ) THEN
                 DO i=1,SIZE(h_block,1)
                   DO j=1,SIZE(h_block,2)
