# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_types.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_types.F" 2
...
! now read in and store the input file, first finding out how many lines it is
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,1035,&
...
! first, Let's find the first END after GLOBAL
    ...
    IF(mc_input_file%global_row_end == 0) THEN
       ...
       END IF
       CALL cp_abort(cp__l("motion/mc/mc_types.F",1055),&
...
! be attached to the force env
    ...
    IF(mc_input_file%motion_row_end == 0) THEN
       ...
       END IF
       CALL cp_abort(cp__l("motion/mc/mc_types.F",1069),&
...
! first, let's find the first END after &COORD, and the line of &COORD
    ...
    IF(mc_input_file%coord_row_end == 0) THEN
       IF (iw>0) THEN
          ...
          WRITE(iw,*) 'File name ',input_file_name
       END IF
       CALL cp_abort(cp__l("motion/mc/mc_types.F",1084),&
...
! now the RUN_TYPE
    ...
    IF(mc_input_file%run_type_row == 0) THEN
       IF (iw>0) THEN
          ...
          WRITE(iw,*) 'File name ',input_file_name
       END IF
       CALL cp_abort(cp__l("motion/mc/mc_types.F",1097),&
...
! and we need to find the next one
    ...
    IF(mc_input_file%cell_row == 0) THEN
       IF (iw>0) THEN
          ...
          WRITE(iw,*) 'File name ',input_file_name
       END IF
       CALL cp_abort(cp__l("motion/mc/mc_types.F",1124),&
...
! now we need all the MOL_SET NMOLS indicies
    IF(.NOT. lhmc)THEN
       ...
       IF(irep .NE. nmol_types) THEN
          ...
          END IF
          CALL cp_abort(cp__l("motion/mc/mc_types.F",1151),&
...
! QS, and Fist, though it should be trivial to add others
    ...
    CASE default
       CALL cp__b("motion/mc/mc_types.F",1168,'Cannot determine the type of force_env we are using (check METHOD)')
    END SELECT
...
! begin the timing of the subroutine
    ...
    IF(.NOT.(ASSOCIATED(input_file)))CALL cp__a("motion/mc/mc_types.F",1294)
...
! now an integer array
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_i_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1396,'AVBMC_ATOM must have as many values as there are molecule types.')
         ENDIF
      ENDIF
...
! yes, I'm allocating here...I cannot find a better place to do it, though
        ...
        IF (istat /= 0) CALL stop_memory(routineN,moduleN,1432,&
...
! all of these arrays should have one value for each type of molecule...so check that
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1442,'AVBMC_RMIN must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1451,'AVBMC_RMAXL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1460,'PBIAS must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1470,'PMAVBMC_MOL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1479,'ETA must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1489,'RMBOND must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1499,'RMANGLE must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1509,'RMDIHEDRAL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1519,'RMROT must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1529,'RMTRANS must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1540,'PMTRAION_MOL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1551,'PMTRANS_MOL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1562,'PMROT_MOL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
      ...
      IF( .NOT. mc_par%lhmc)THEN
         IF(SIZE(temp_r_array) .NE. nmol_types)THEN
            CALL cp__b("motion/mc/mc_types.F",1573,'PMSWAP_MOL must have as many values as there are molecule types.')
         ENDIF
      ENDIF
      DO imol=1,SIZE(temp_r_array)
...
! box
    ...
    ALLOCATE(particles(1:nboxes),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1890,&
         "particles",nboxes)
    ALLOCATE (molecule_kinds_new(1:nboxes),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,1893,&
 ...
 ! find out how many types we have
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,1909,&
...
! the conformational probabilities
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,1963,&
...
! figure out the empty coordinates
    IF(PRESENT(coordinates_empty)) THEN
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,2004,&
...
! and which force_env (box) it's in
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,2040,&
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,2043,&
...
! a chance we won't select a molecule type for a move
    IF(mc_par%pmavbmc_mol(nmol_types) .LT. 0.9999E0_dp) THEN
       CALL cp__b("motion/mc/mc_types.F",2140,'The last value of PMAVBMC_MOL needs to be 1.0')
    ENDIF
    IF(mc_par%pmswap_mol(nmol_types) .LT. 0.9999E0_dp) THEN
       CALL cp__b("motion/mc/mc_types.F",2143,'The last value of PMSWAP_MOL needs to be 1.0')
    ENDIF
    IF(mc_par%pmtraion_mol(nmol_types) .LT. 0.9999E0_dp) THEN
       CALL cp__b("motion/mc/mc_types.F",2146,'The last value of PMTRAION_MOL needs to be 1.0')
    ENDIF
    IF(mc_par%pmtrans_mol(nmol_types) .LT. 0.9999E0_dp) THEN
       CALL cp__b("motion/mc/mc_types.F",2149,'The last value of PMTRANS_MOL needs to be 1.0')
    ENDIF
    IF(mc_par%pmrot_mol(nmol_types) .LT. 0.9999E0_dp) THEN
       CALL cp__b("motion/mc/mc_types.F",2152,'The last value of PMROT_MOL needs to be 1.0')
    ENDIF
...
! check to make sure we have all the desired values for various
 ...
 ! some ensembles require multiple boxes or molecule types
    ...
    CASE("GEMC_NPT")
       IF(nmol_types .LE. 1) &
            CALL cp__b("motion/mc/mc_types.F",2161,'Cannot have GEMC-NPT simulation with only one molecule type')
       IF(nboxes .LE. 1) &
            CALL cp__b("motion/mc/mc_types.F",2163,'Cannot have GEMC-NPT simulation with only one box')
    CASE("GEMC_NVT")
       IF(nboxes .LE. 1) &
            CALL cp__b("motion/mc/mc_types.F",2166,'Cannot have GEMC-NVT simulation with only one box')
    CASE("TRADITIONAL")
       IF(mc_par%pmswap .GT. 0.0E0_dp)&
            CALL cp__b("motion/mc/mc_types.F",2169,'You cannot do swap moves in a system with only one box')
    CASE("VIRIAL")
       IF(nchain_total .NE. 2)&
            CALL cp__b("motion/mc/mc_types.F",2172,'You need exactly two molecules in the box to compute the second virial.')
    END SELECT
...
! of units in that molecule
    DO imol_type=1,nmol_types
       IF(mc_par%avbmc_atom(imol_type) .GT. nunits(imol_type)) THEN
          CALL cp__b("motion/mc/mc_types.F",2179,'Cannot have avbmc_atom higher than the number of atoms for that molecule type!')
       ENDIF
    ENDDO
