# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_clv.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_clv.F"
!-----------------------------------------------------------------------------!
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_clv.F" 2
...
! *****************************************************************************
  SUBROUTINE  update_con_colv(wrk, fac, lcolv, lambda, roll, rmat, imass)
    ...
    IF (PRESENT(roll)) THEN
       my_roll = roll
       IF (my_roll) THEN
          IF(.NOT.(PRESENT(rmat)))CALL cp__a("constraint_clv.F",929)
       END IF
    END IF
...
! *****************************************************************************
  FUNCTION eval_Jac_colvar(colvar, colvar_old, roll, rmat, imass) RESULT(res)
    ...
    IF (PRESENT(roll)) THEN
       my_roll = roll
       IF (my_roll) THEN
          IF(.NOT.(PRESENT(rmat)))CALL cp__a("constraint_clv.F",976)
       END IF
    END IF
...
! *****************************************************************************
  FUNCTION rattle_con_eval(colvar, vel, roll, veps, rmat, particles) RESULT(res)
    ...
    IF (PRESENT(roll)) THEN
       my_roll = roll
       IF (my_roll) THEN
          IF(.NOT.(PRESENT(rmat)))CALL cp__a("constraint_clv.F",1030)
          IF(.NOT.(PRESENT(veps)))CALL cp__a("constraint_clv.F",1031)
          IF(.NOT.(PRESENT(particles)))CALL cp__a("constraint_clv.F",1032)
       END IF
    END IF
    res = 0.0_dp
