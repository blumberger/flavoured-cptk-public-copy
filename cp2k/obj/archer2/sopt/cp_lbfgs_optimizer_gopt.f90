# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cp_lbfgs_optimizer_gopt.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cp_lbfgs_optimizer_gopt.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cp_lbfgs_optimizer_gopt.F" 2

  IMPLICIT NONE
  PRIVATE

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/gopt_f77_methods.h" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
END INTERFACE
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cp_lbfgs_optimizer_gopt.F" 2
...
!> \author Fawzi Mohamed
! *****************************************************************************
SUBROUTINE cp_opt_gopt_retain(optimizer)
    TYPE(cp_lbfgs_opt_gopt_type), POINTER    :: optimizer

    CHARACTER(len=*), PARAMETER :: routineN = 'cp_opt_gopt_retain', &
      routineP = moduleN//':'//routineN

  IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",309)
  IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",310)
...
SUBROUTINE cp_opt_gopt_release(optimizer)
  ...
  IF (ASSOCIATED(optimizer)) THEN
     IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",333)
     optimizer%ref_count=optimizer%ref_count-1
     IF (optimizer%ref_count==0) THEN
        ...
        END IF
        CALL cp_para_env_release(optimizer%para_env)
...
!  call timeset(routineN,handle)
    ...
    IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",432)
    IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",433)
    ...
    IF (optimizer%master==optimizer%para_env%mepos) THEN
       ...
       ELSE
          IF(.NOT.(.NOT.PRESENT(last_f)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",467)
          IF(.NOT.(.NOT.PRESENT(actual_projected_gradient)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",468)
       END IF
    ELSE
       IF(PRESENT(lower_bound).OR.PRESENT(upper_bound).OR.PRESENT(kind_of_bound))&
          CALL cp__w("motion/cp_lbfgs_optimizer_gopt.F",472,"asked undefined types")
    END IF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE cp_opt_gopt_step(optimizer,n_iter,f,last_f,&
    ...
    IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",527)
    IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",528)

    IF (optimizer%status>=4) THEN
       CALL cp__w("motion/cp_lbfgs_optimizer_gopt.F",531,"status>=4, trying to restart")
       optimizer%status=0
       IF (is_master) THEN
          optimizer%task='START'
          CALL setulb (SIZE(optimizer%x), optimizer%m, optimizer%x, &
    ...
    DO
       ifMaster: IF (is_master) THEN
          ...
          ELSE IF (optimizer%task(1:4)=='STOP') THEN
             optimizer%status=5
             CALL cp__w("motion/cp_lbfgs_optimizer_gopt.F",594,"task became stop in an unknown way")
          ELSE IF (optimizer%task(1:5)=='ERROR') THEN
             optimizer%status=5
          ELSE
             CALL cp__w("motion/cp_lbfgs_optimizer_gopt.F",598,"unknown task '"//optimizer%task//"'")
          END IF
       END IF ifMaster
       ...
       CASE (6)
          ! deallocated
          CALL cp__b("motion/cp_lbfgs_optimizer_gopt.F",683,"step on a deallocated opt structure ")
       CASE default
          CALL cp_abort(cp__l("motion/cp_lbfgs_optimizer_gopt.F",685),&
...
!  call timeset(routineN,handle)
    ...
    IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",740)
    IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",741)
...
!call timeset(routineN,handle)
    ...
    IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",795)
    IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",796)
...
!  call timeset(routineN,handle)
    ...
    IF(.NOT.(ASSOCIATED(optimizer)))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",823)
    IF(.NOT.(optimizer%ref_count>0))CALL cp__a("motion/cp_lbfgs_optimizer_gopt.F",824)
