# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings.F" 2
...
! *****************************************************************************
  SUBROUTINE add_timer_env(timer_env)
    ...
    IF(.NOT. ASSOCIATED(timer_env_))&
       CALL cp__b("common/timings.F",95,"add_timer_env: not associated")
...
! *****************************************************************************
  SUBROUTINE timer_env_create(timer_env) 
    ...
    IF (stat/=0)&
       CALL cp__b("common/timings.F",114,"timer_env_create: allocation failed")
...
! *****************************************************************************
  SUBROUTINE timer_env_retain(timer_env)
    ...
    IF(.NOT. ASSOCIATED(timer_env))&
       CALL cp__b("common/timings.F",160,"timer_env_retain: not associated")
    IF (timer_env%ref_count < 0)&
       CALL cp__b("common/timings.F",162,"timer_env_retain: negativ ref_count")
...
! *****************************************************************************
  SUBROUTINE timer_env_release(timer_env)
    ...
    IF(.NOT. ASSOCIATED(timer_env))&
       CALL cp__b("common/timings.F",180,"timer_env_release: not associated")
    IF (timer_env%ref_count < 0)&
       CALL cp__b("common/timings.F",182,"timer_env_release: negativ ref_count")
    timer_env%ref_count = timer_env%ref_count - 1
...
! *****************************************************************************
  SUBROUTINE timeset_handler(routineN, handle)
    ...
    INTEGER(KIND=int_8)                      :: cpumem, gpumem_free, &
                                                gpumem_total
...
!$OMP CRITICAL(time_setstop)
    ...
    IF (LEN_TRIM(routineN) > default_string_length) THEN
       CALL cp__b("common/timings.F",237,'timings_timeset: routineN too long: "'//TRIM(routineN)//"'")
    END IF
...
!$OMP CRITICAL(time_setstop)
    ...
    IF (handle /= cs_entry%routine_id) THEN
       ...
       CALL cp__b("common/timings.F",324,'mismatched timestop '//TRIM(r_stat%routineN)//' in routine timestop')
    END IF
...
! *****************************************************************************
 FUNCTION routine_name2id(routineN) RESULT(routine_id)
    ...
    IF (INDEX(routineN(1:LEN_TRIM(routineN)),' ') /= 0) THEN 
       CALL cp__b("common/timings.F",478,"timings_name2id: routineN contains spaces: "//routineN)
    END IF
    ...
    IF(stat /= 0)&
       CALL cp__b("common/timings.F",487,"timings_name2id: allocation failed")
    ...
    IF(list_size(timer_env%routine_stats) /= dict_size(timer_env%routine_names)) &
       CALL cp__b("common/timings.F",500,"timings_name2id: assertion failed")
