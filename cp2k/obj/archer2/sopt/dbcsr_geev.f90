# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/arnoldi/dbcsr_geev.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/arnoldi/dbcsr_geev.F"
!-----------------------------------------------------------------------------!
...
# 18 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/arnoldi/dbcsr_geev.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_ssyevd(jobvr,matrix,ndim,evals,revec)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(jobvr))==-1) EXIT ;  END DO ; ENDIF !the argument has to be here for the template to work
    lwork=1+6*ndim+2*ndim**2
    liwork=3+5*ndim
...
! *****************************************************************************
  SUBROUTINE dbcsr_sstev(jobvl,jobvr,matrix,ndim,evals,revec,levec)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(jobvl))==-1) EXIT ;  END DO ; ENDIF !the argument has to be here for the template to work
  ...
  SUBROUTINE dbcsr_sgeev(jobvl,jobvr,matrix,ndim,evals,revec,levec)
    ...
    DO WHILE (i.le.ndim)
      ...
      ELSE IF(eval2(i).GT.EPSILON(REAL(0.0,real_4)))THEN
        ...
        i=i+2
      ELSE
        CALL dbcsr_abort(routineP,340,'something went wrong while sorting the EV in dbcsr_geev')
      END IF
    END DO
  ...
  SUBROUTINE dbcsr_dgeev(jobvl,jobvr,matrix,ndim,evals,revec,levec)
    ...
    DO WHILE (i.le.ndim)
      ...
      ELSE IF(eval2(i).GT.EPSILON(REAL(0.0,real_8)))THEN
        ...
        i=i+2
      ELSE
        CALL dbcsr_abort(routineP,410,'something went wrong while sorting the EV in dbcsr_geev')
      END IF
    END DO
