# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix.F" 2
...
! *****************************************************************************
  SUBROUTINE build_se_fock_matrix (qs_env,calculate_forces,just_energy)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("se_fock_matrix.F",123)
    ...
    CASE DEFAULT
       ...
       CALL cp__b("se_fock_matrix.F",141,"Fock Matrix not available for the chosen parameterization! ")
    ...
    CASE (do_method_am1,do_method_rm1,do_method_mndo,do_method_pdg,&
       ...
       IF(.NOT.(((nspins>=1).AND.(nspins<=2))))CALL cp__a("se_fock_matrix.F",148)
       IF(.NOT.(ASSOCIATED(matrix_h)))CALL cp__a("se_fock_matrix.F",149)
       IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("se_fock_matrix.F",150)
       IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix.F",151)
       ...
       IF (qs_env%requires_mo_derivs .AND. .NOT. just_energy) THEN
          ...
          DO ispin=1,SIZE(mo_derivs)
             ...
             IF(.NOT.mo_array(ispin)%mo_set%use_mo_coeff_b) THEN
                CALL cp__b("se_fock_matrix.F",262,"")
             ENDIF
