# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_update.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_update.F"
!-----------------------------------------------------------------------------!
...
# 31 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_update.F" 2
...
! *****************************************************************************
  SUBROUTINE qmmmx_update_force_env(force_env, root_section)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'qmmmx_update_force_env', &
      routineP = moduleN//':'//routineN

    LOGICAL                                  :: force_mixing_active, &
                                                labels_changed
...
! check everything for not null, because sometimes (e.g. metadynamics in parallel) it happens
    ...
    IF(.NOT.ASSOCIATED(force_env%qmmmx_env)) CALL cp__b("qmmmx_update.F",75,"force_env%qmmmx not associated")

    CALL force_env_get(force_env, subsys=subsys)
    CALL update_force_mixing_labels(subsys, qmmm_section, labels_changed=labels_changed)
    IF (.NOT. labels_changed) RETURN
    CALL cp__w("qmmmx_update.F",80,"Adaptive force-mixing labels changed, rebuilding QM/MM calculations! ")
...
! *****************************************************************************
  SUBROUTINE copy_wiener_process(from_local_particle_kinds, from_local_particles,&
    ...
    IF(.NOT.(ASSOCIATED(from_local_particles)))CALL cp__a("qmmmx_update.F",152)
    IF(.NOT.(ASSOCIATED(to_local_particles)))CALL cp__a("qmmmx_update.F",153)

    IF (.NOT. ASSOCIATED(from_local_particles%local_particle_set)) RETURN
    IF(.NOT.(.NOT. ASSOCIATED(to_local_particles%local_particle_set)))CALL cp__a("qmmmx_update.F",156)
    ...
    END DO
    IF(.NOT.(tot_from_nparticle_local == tot_to_nparticle_local))CALL cp__a("qmmmx_update.F",170)
    ...
    DO to_iparticle_kind=1, to_nparticle_kind
       ...
       DO to_iparticle_local=1, to_nparticle_local
          ...
          DO from_iparticle_kind=1, from_nparticle_kind
             ...
             ENDIF
          END DO
          IF(.NOT.(found_it))CALL cp__a("qmmmx_update.F",196)
