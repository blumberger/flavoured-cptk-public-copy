# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/fparser.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/fparser.F"
! *****************************************************************************
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/fparser.F" 2
...
! *****************************************************************************
  SUBROUTINE finalizef()
    ...
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    CHARACTER(len=*), PARAMETER :: routineN = 'finalizef', &
...
! *****************************************************************************
  SUBROUTINE parsef (i, FuncStr, Var)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'parsef', &
      routineP = moduleN//':'//routineN
...
!----- -------- --------- --------- --------- --------- --------- --------- -------
    ...
    IF (i < 1 .OR. i > SIZE(Comp)) THEN
       CALL cp__b("common/fparser.F",168,"Function number is out of range")
    END IF
    IF (SIZE(var)>HUGE(0_is)) THEN
       CALL cp__b("common/fparser.F",171,"Too many variables")
    END IF
...
! *****************************************************************************
  FUNCTION evalf (i, Val) RESULT (res)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'evalf', &
      routineP = moduleN//':'//routineN
    ...
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE   (cPow)
          ...
          IF (Comp(i)%Stack(SP-1)<0.0_rn) THEN
             ...
             ELSE
                CALL cp__b("common/fparser.F",230,"Negative floating-point value raised to a real power!")
             END IF
          ELSE
...
! *****************************************************************************
  SUBROUTINE ParseErrMsg (j, FuncStr, Msg)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'ParseErrMsg', &
      routineP = moduleN//':'//routineN
...
!----- -------- --------- --------- --------- --------- --------- --------- -------
    ...
    END IF
    CALL cp__b("common/fparser.F",413,TRIM(message))
...
! *****************************************************************************
  SUBROUTINE Compile (i, F, Var)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'Compile', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  FUNCTION evalfd(id_fun,ipar,vals,h,err) RESULT(derivative)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'evalfd', &
      routineP = moduleN//':'//routineN
    ...
    IF(h/=0._rn) THEN
       ...
       END DO
    ELSE
       CALL cp__b("common/fparser.F",1046,"DX provided equals zero!")
    END IF
    vals(ipar)=xval
  END FUNCTION evalfd
