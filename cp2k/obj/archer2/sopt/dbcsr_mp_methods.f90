# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_methods.F"
!-----------------------------------------------------------------------------!
...
!   ---------------------------------------------------------------------------
    ...
    IF (.NOT. mp_env%mp%subgroups_defined) THEN
       ...
       CALL dbcsr_assert (my_pos(1) .EQ. mp_env%mp%myprow,&
            ...
            "Got different MPI process grid",161)
       CALL dbcsr_assert (my_pos(2) .EQ. mp_env%mp%mypcol,&
            dbcsr_fatal_level, dbcsr_internal_error, "mp_grid_setup",&
            "Got different MPI process grid",164)
       !
       remain = (/.FALSE., .TRUE./)
