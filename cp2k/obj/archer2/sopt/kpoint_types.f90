# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_types.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_types.F" 2
...
! *****************************************************************************
  SUBROUTINE kpoint_create(kpoint)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(kpoint)))CALL cp__a("kpoint_types.F",161)
...
! *****************************************************************************
  SUBROUTINE get_kpoint_info(kpoint, kp_scheme, nkp_grid, kp_shift, symmetry, verbose,&
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_types.F",326)
...
! *****************************************************************************
  SUBROUTINE set_kpoint_info(kpoint, kp_scheme, nkp_grid, kp_shift, symmetry, verbose,&
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_types.F",443)
...
!> \author JGH
! *****************************************************************************
  SUBROUTINE read_kpoint_section(kpoint,kpoint_section)
    TYPE(kpoint_type), POINTER               :: kpoint
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_types.F",502)
    ...
    IF (available) THEN
       ...
       IF(.NOT.(nval >= 1))CALL cp__a("kpoint_types.F",509)
       ...
       SELECT CASE (kpoint%kp_scheme)
          ...
          CASE ("GAMMA")
             ! do nothing
          CASE ("MONKHORST-PACK")
             IF(.NOT.(nval >= 4))CALL cp__a("kpoint_types.F",520)
             DO i=2,4
                READ(tmpstringlist(i),*) kpoint%nkp_grid(i-1)
             END DO
          CASE ("MACDONALD")
             IF(.NOT.(nval >= 7))CALL cp__a("kpoint_types.F",525)
             DO i=2,4
                READ(tmpstringlist(i),*) kpoint%nkp_grid(i-1)
             END DO
             DO i=5,7
          ...
          CASE ("GENERAL")
             ...
             DO i=1,n_rep
                CALL section_vals_val_get(kpoint_section,"KPOINT", i_rep_val=i,&
                                  r_vals=reallist)
                nval = SIZE(reallist)
                IF(.NOT.(nval>=4))CALL cp__a("kpoint_types.F",540)
                kpoint%xkp(1:3,i) = reallist(1:3)
                kpoint%wkp(i) = reallist(4)
             END DO
          CASE DEFAULT
            CALL cp__b("kpoint_types.F",545,"")
       END SELECT
...
! *****************************************************************************
  SUBROUTINE write_kpoint_info(kpoint,dft_section)
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_types.F",586)
...
! *****************************************************************************
  SUBROUTINE kpoint_env_create(kp_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(kp_env)))CALL cp__a("kpoint_types.F",658)
...
! *****************************************************************************
  SUBROUTINE get_kpoint_env(kpoint_env, nkpoint, wkp, xkp, is_local, mos)
    ...
    IF(.NOT.(ASSOCIATED(kpoint_env)))CALL cp__a("kpoint_types.F",743)
