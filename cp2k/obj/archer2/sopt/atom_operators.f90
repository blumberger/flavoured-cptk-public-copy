# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_operators.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_operators.F"
!-----------------------------------------------------------------------------!
...
MODULE atom_operators
  ...
  USE input_constants,                 ONLY: &
       barrier_conf, do_dkh0_atom, do_dkh1_atom, do_dkh2_atom, do_dkh3_atom, &
       do_dkh4_atom, do_dkh5_atom, do_nonrel_atom, do_sczoramp_atom, &
       do_zoramp_atom, poly_conf
  USE kinds,                           ONLY: dp
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_operators.F" 2
...
! *****************************************************************************
  SUBROUTINE atom_int_setup(integrals,basis,potential,&
    ...
    IF ( integrals%status == 0 ) THEN
      ...
      ALLOCATE (integrals%ovlp(n,n,0:3))
      integrals%ovlp = 0._dp

      ALLOCATE (integrals%kin(n,n,0:3))
      integrals%kin = 0._dp
      ...
      IF ( PRESENT(potential) ) THEN
        IF ( potential%confinement ) THEN
          ALLOCATE (integrals%conf(n,n,0:3))
          ...
          ELSEIF ( potential%conf_type == barrier_conf ) THEN
             ...
             END DO
          ELSE
             CALL cp__b("atom_operators.F",148,"")
          END IF
      ...
      SELECT CASE (basis%basis_type)
        CASE DEFAULT
          CALL cp__b("atom_operators.F",157,"")
        CASE (GTO_BASIS)
          DO l=0,3
            n = integrals%n(l)
        ...
        CASE (STO_BASIS)
          DO l=0,3
            ...
            CALL sto_kinetic ( integrals%kin(1:n,1:n,l), l, basis%ns(1:n,l), basis%as(1:n,l), &
                               basis%ns(1:n,l), basis%as(1:n,l) )
          END DO
          IF(.NOT.(.NOT.integrals%eri_coulomb))CALL cp__a("atom_operators.F",286)
          IF(.NOT.(.NOT.integrals%eri_exchange))CALL cp__a("atom_operators.F",287)
        CASE (NUM_BASIS)
          CALL cp__b("atom_operators.F",289,"")
      END SELECT
      ...
      n = MAXVAL(basis%nbas)
      ALLOCATE (integrals%utrans(n,n,0:3),integrals%uptrans(n,n,0:3))
      ...
      DO l = 0, 3
        ...
        IF ( n > 0 ) THEN
          ...
          IF(.NOT.(info==0))CALL cp__a("atom_operators.F",320)
        END IF
      END DO
...
! *****************************************************************************
  SUBROUTINE atom_ppint_setup(integrals,basis,potential)
    ...
    IF ( integrals%ppstat == 0 ) THEN
      ...
      ALLOCATE (integrals%hnl(n,n,0:3))
      integrals%hnl = 0._dp

      ALLOCATE (integrals%core(n,n,0:3))
      integrals%core = 0._dp

      ALLOCATE (integrals%clsd(n,n,0:3))
      ...
      SELECT CASE (basis%basis_type)
        CASE DEFAULT
          CALL cp__b("atom_operators.F",372,"")
        CASE (GTO_BASIS)
          ...
          SELECT CASE (potential%ppot_type)
            ...
            CASE (GTH_PSEUDO)
              ...
              END DO
            CASE DEFAULT
              CALL cp__b("atom_operators.F",430,"")
          END SELECT
        ...
        CASE (CGTO_BASIS)
          ...
          SELECT CASE (potential%ppot_type)
            ...
            CASE (GTH_PSEUDO)
              ...
              END DO
            CASE DEFAULT
              CALL cp__b("atom_operators.F",503,"")
          END SELECT
        ...
        CASE (STO_BASIS)
          ...
          SELECT CASE (potential%ppot_type)
            ...
            CASE DEFAULT
              CALL cp__b("atom_operators.F",586,"")
          END SELECT

        CASE (NUM_BASIS)
          CALL cp__b("atom_operators.F",590,"")
      END SELECT
...
! *****************************************************************************
  SUBROUTINE atom_relint_setup(integrals,basis,reltyp,zcore,alpha)
    ...
    INTEGER                                  :: dkhorder, handle, i, k1, k2, &
                                                l, m, n, nl
    ...
    SELECT CASE (reltyp)
      CASE DEFAULT
        CALL cp__b("atom_operators.F",632,"")
    ...
    SELECT CASE (reltyp)
      CASE DEFAULT
        CALL cp__b("atom_operators.F",651,"")
      CASE (do_nonrel_atom)
        ! nothing to do
      ...
      CASE (do_zoramp_atom, do_sczoramp_atom)
        ...
        IF ( integrals%zorastat == 0 ) THEN
          n = MAXVAL(basis%nbas)
          ALLOCATE (integrals%tzora(n,n,0:3))
          ...
          IF( reltyp == do_sczoramp_atom ) THEN
             ALLOCATE (hmat(n,n,0:3),wfn(n,n,0:3),ener(n,0:3),pvp(n,n,0:3),sps(n,n))
             hmat(:,:,:) = integrals%kin + integrals%tzora
             ! model potential
             CALL numpot_matrix(hmat,modpot,basis,0)
             ! eigenvalues and eigenvectors
             CALL atom_solve(hmat,integrals%utrans,wfn,ener,basis%nbas,integrals%nne,3)
      ...
      CASE (do_dkh0_atom, do_dkh1_atom, do_dkh2_atom, do_dkh3_atom, do_dkh4_atom, do_dkh5_atom)
        ...
        IF ( integrals%dkhstat == 0 ) THEN
          n = MAXVAL(basis%nbas)
          ALLOCATE (integrals%hdkh(n,n,0:3))
          integrals%hdkh = 0._dp

          m = MAXVAL(basis%nprim)
          ALLOCATE (tp(m,m,0:3),sp(m,m,0:3),vp(m,m,0:3),pvp(m,m,0:3))
          tp = 0._dp
          sp = 0._dp
          vp = 0._dp
          pvp = 0._dp
          ...
          SELECT CASE (basis%basis_type)
            CASE DEFAULT
              CALL cp__b("atom_operators.F",754,"")
            ...
            CASE (STO_BASIS)
              CALL cp__b("atom_operators.F",774,"")
            CASE (NUM_BASIS)
              CALL cp__b("atom_operators.F",776,"")
          END SELECT
        ...
        ELSE
          IF(.NOT.(ASSOCIATED(integrals%hdkh)))CALL cp__a("atom_operators.F",786)
        END IF
...
! *****************************************************************************
  SUBROUTINE dkh_integrals(integrals,basis,order,sp,tp,vp,pvp)
    ...
    IF(.NOT.(order>=0))CALL cp__a("atom_operators.F",816)
    ...
    DO l=0,3
      ...
      IF ( n > 0 ) THEN
        ...
        SELECT CASE (basis%basis_type)
          CASE DEFAULT
            CALL cp__b("atom_operators.F",827,"")
          CASE (GTO_BASIS)
            IF(.NOT.(n==m))CALL cp__a("atom_operators.F",829)
          ...
          CASE (STO_BASIS)
            CALL cp__b("atom_operators.F",835,"")
          CASE (NUM_BASIS)
            CALL cp__b("atom_operators.F",837,"")
        END SELECT
      ELSE
...
! *****************************************************************************
  SUBROUTINE contract4 ( eri, omat, cm1, cm2)
    ...
    INTEGER                                  :: handle, i1, i2, m1, m2, mm1, &
                                                mm2, n1, n2, nn1, nn2
