# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_linked_list_real.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_linked_list_real.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_linked_list_real.F" 2
       ...
       IF (PRESENT(iter)) THEN
          ...
          DO i=1,iter
             IF (ASSOCIATED(res%rest)) THEN
                res => res%rest
             ELSE
                CALL cp__b("input/cp_linked_list_real.F",296,"tried to go past end")
             END IF
          END DO
          IF (iter==-1) THEN
             DO
...
! *****************************************************************************
  FUNCTION cp_sll_real_get_el_at(sll,index) RESULT(res)
    ...
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("input/cp_linked_list_real.F",368)
...
! *****************************************************************************
  SUBROUTINE cp_sll_real_set_el_at(sll,index,value)
    ...
    ELSE
       pos => cp_sll_real_get_rest(sll, iter=index-1)
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("input/cp_linked_list_real.F",400)
...
! *****************************************************************************
  SUBROUTINE cp_sll_real_rm_first_el(sll)
    ...
    ELSE
       CALL cp__b("input/cp_linked_list_real.F",486,"tried to remove first el of an empty list")
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_sll_real_insert_el_at(sll,el,index)
    ...
    ELSE
       ...
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("input/cp_linked_list_real.F",520)
  ...
  SUBROUTINE cp_sll_real_rm_el_at(sll,index)
    ...
    ELSE
       ...
       ELSE
          pos => cp_sll_real_get_rest(sll, iter=index-2)
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("input/cp_linked_list_real.F",552)
...
FUNCTION cp_sll_real_to_array(sll) RESULT(res)
  ...
  DO i=1,len
     res(i) = iter%first_el
     IF(.NOT.(cp_sll_real_next(iter).OR.i==len))CALL cp__a("input/cp_linked_list_real.F",608)
  END DO
