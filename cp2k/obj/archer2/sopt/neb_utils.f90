# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/neb_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/neb_utils.F"
!-----------------------------------------------------------------------------!
...
# 75 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/neb_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE neb_replica_distance(particle_set,coords,i0,i,distance,iw,rotate)
    ...
    IF (my_rotate.AND.(coords%in_use == do_band_cartesian)) THEN
       IF(.NOT.(PRESENT(particle_set)))CALL cp__a("motion/neb_utils.F",119)
...
! *****************************************************************************
  SUBROUTINE build_replica_coords(neb_section, particle_set,&
    ...
    IF(.NOT.(ASSOCIATED(coords)))CALL cp__a("motion/neb_utils.F",170)
    IF(.NOT.(ASSOCIATED(vels)))CALL cp__a("motion/neb_utils.F",171)
    ...
    IF(.NOT.(input_nr_replica<=neb_nr_replica))CALL cp__a("motion/neb_utils.F",176)
    ...
    DO i_rep=1,input_nr_replica
       ...
       IF (explicit) THEN
          CALL section_vals_val_get(coord_section,"_DEFAULT_KEYWORD_",&
                                    n_rep_val=natom)
          IF(.NOT.((natom == SIZE(particle_set))))CALL cp__a("motion/neb_utils.F",201)
          DO iatom=1,natom
             CALL section_vals_val_get(coord_section,"_DEFAULT_KEYWORD_",&
       ...
       ELSE
          ...
          IF(.NOT.(TRIM(filename)/=""))CALL cp__a("motion/neb_utils.F",217)
          ...
          IF(.NOT.((natom == SIZE(particle_set))))CALL cp__a("motion/neb_utils.F",222)
          CALL parser_get_next_line(parser,1)
          DO iatom=1,natom
             ! Atom coordinates
             CALL parser_get_next_line(parser,1,at_end=my_end)
             IF(my_end)&
                CALL cp_abort(cp__l("motion/neb_utils.F",228),&
       ...
       IF (neb_env%use_colvar) THEN
          ...
          IF (n_rep /= 0) THEN
             ...
             IF(.NOT.(check))CALL cp__a("motion/neb_utils.F",254)
       ...
       ELSE
          ...
          IF (neb_env%use_colvar) THEN
             ...
             IF(.NOT.(nval == nvar))CALL cp__a("motion/neb_utils.F",275)
             DO ivar=1,nvar
                CALL section_vals_val_get(vel_section,"_DEFAULT_KEYWORD_",&
          ...
          ELSE
             ...
             IF(.NOT.(nval == natom))CALL cp__a("motion/neb_utils.F",283)
             DO iatom=1,natom
                CALL section_vals_val_get(vel_section,"_DEFAULT_KEYWORD_",&
                                          i_rep_val=iatom,r_vals=rptr)
                ic = 3*(iatom - 1)
...
! *****************************************************************************
  SUBROUTINE perform_replica_md(rep_env, coords, irep, n_rep_neb, cvalues, Mmatrix)
    ...
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_utils.F",544)
    !
    IF (irep+j <= n_rep_neb) THEN
       ...
       IF(.NOT.(explicit))CALL cp__a("motion/neb_utils.F",551)
    ...
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_utils.F",577)
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE perform_replica_geo(rep_env, coords, irep, n_rep_neb, cvalues, Mmatrix)
    ...
    ! Syncronize position on the replica procs
    CALL set_pos(rep_env%f_env_id,rep_env%r(:,j+1),n_el,ierr)
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_utils.F",621)
    IF (irep+j <= n_rep_neb) THEN
       logger%iter_info%iteration(2)=irep+j
       CALL remove_restart_info(root_section)
       ...
       IF(.NOT.(explicit))CALL cp__a("motion/neb_utils.F",627)
       ! Let's syncronize the target of Collective Variables for this run
       CALL set_colvars_target(coords%int(:,irep+j), f_env%force_env)
    ...
    CALL rep_env_sync(rep_env,Mmatrix)
    CALL f_env_rm_defaults(f_env=f_env,ierr=ierr,handle=handle2)
    IF(.NOT.(ierr==0))CALL cp__a("motion/neb_utils.F",659)
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE get_tangent(neb_env, coords, i, tangent, energies, iw)
    ...
    IF(.NOT.(ASSOCIATED(coords)))CALL cp__a("motion/neb_utils.F",687)
    tangent(:) = 0.0_dp
...
! *****************************************************************************
  RECURSIVE SUBROUTINE get_neb_force(neb_env,tangent,coords,i,forces,tag,Mmatrix,&
    ...
    IF(.NOT.(ASSOCIATED(forces)))CALL cp__a("motion/neb_utils.F",757)
    IF(.NOT.(ASSOCIATED(coords)))CALL cp__a("motion/neb_utils.F",758)
    nsize_wrk = coords%size_wrk(1)
...
! *****************************************************************************
  FUNCTION dot_product_band(neb_env, array1, array2, array3) RESULT(value)
    ...
    IF (neb_env%use_colvar) THEN
       ...
       IF(.NOT.(check))CALL cp__a("motion/neb_utils.F",879)
  ...
  SUBROUTINE reorient_images(rotate_frames, particle_set, coords, vels, iw,&
    ...
    DO i = 2, number_of_replica
       ...
       IF (PRESENT(distances)) THEN
          ...
          IF(.NOT.(check))CALL cp__a("motion/neb_utils.F",936)
  ...
  SUBROUTINE reparametrize_images(reparametrize_frames, spline_order, smoothing,&
    ...
    IF (reparametrize_frames) THEN
       ...
       CASE(1)
          ...
          END DO
       CASE DEFAULT
          CALL cp__w("motion/neb_utils.F",1008,"String Method: Spline order greater than 1 not implemented.")
       END SELECT
       sline = coords - tmp_coords + sline
