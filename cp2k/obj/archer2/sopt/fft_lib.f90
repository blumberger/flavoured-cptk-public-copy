# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft/fft_lib.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft/fft_lib.F"
!-----------------------------------------------------------------------------!
...
# 23 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/fft/fft_lib.F" 2
...
SUBROUTINE fft_do_init(fft_type, wisdom_file)
  ...
  SELECT CASE ( fft_type )
    CASE DEFAULT
      CALL cp__b("pw/fft/fft_lib.F",69,"fft_do_init")
    CASE ( 1 )
      CALL fftsg_do_init ()
    CASE ( 3 )
...
SUBROUTINE fft_do_cleanup(fft_type, wisdom_file, ionode)
  ...
  SELECT CASE ( fft_type )
    CASE DEFAULT
      CALL cp__b("pw/fft/fft_lib.F",91,"fft_do_cleanup")
    CASE ( 1 )
...
SUBROUTINE fft_get_lengths ( fft_type, DATA, max_length )
  ...
  SELECT CASE ( fft_type )
    CASE DEFAULT
      CALL cp__b("pw/fft/fft_lib.F",114,"fft_get_lengths")
    CASE ( 1 )
...
SUBROUTINE fft_3d ( plan, scale, zin, zout, stat )
  ...
  IF ( plan%n_3d(1)*plan%n_3d(2)*plan%n_3d(3) > 0 ) THEN
    SELECT CASE ( plan%fft_type )
      CASE DEFAULT
        CALL cp__b("pw/fft/fft_lib.F",187,"fft_3d")
      CASE ( 1 )
...
SUBROUTINE fft_1dm ( plan, zin, zout, scale, stat)
  ...
  IF ( plan%n * plan%m > 0 ) THEN
    SELECT CASE ( plan%fft_type )
      CASE DEFAULT
        CALL cp__b("pw/fft/fft_lib.F",286,"fft_1dm")
      CASE ( 1 )
