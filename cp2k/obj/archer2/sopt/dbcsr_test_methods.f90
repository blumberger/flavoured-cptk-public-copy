# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_test_methods
  ...
  USE dbcsr_block_operations,          ONLY: dbcsr_block_conjg,&
                                             ...
                                             dbcsr_data_clear,&
                                             dbcsr_data_set
  USE dbcsr_config,                    ONLY: randmat_counter
  USE dbcsr_data_methods,              ONLY: &
       dbcsr_data_clear_pointer, dbcsr_data_get_sizes, dbcsr_data_get_type, &
       dbcsr_data_init, dbcsr_data_new, dbcsr_data_release, dbcsr_scalar, &
  ...
  USE dbcsr_types,                     ONLY: &
       ...
       dbcsr_obj, dbcsr_scalar_type, dbcsr_type_antihermitian, &
...
# 75 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_methods.F" 2
  ...
  PUBLIC :: dbcsr_random_dist, dbcsr_make_random_matrix,&
            ...
            dbcsr_test_read_args, atoi, atol, ator
...
! *****************************************************************************
  SUBROUTINE dbcsr_impose_sparsity(sparse, dense)
    ...
    INTEGER :: blk, col, col_offset, col_size, data_type, dense_col_size, &
      dense_row_size, handle, row, row_offset, row_size
    LOGICAL                                  :: valid
    ...
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",168)
       ...
       IF(symm.NE.dbcsr_type_no_symmetry) THEN
          ...
          CASE DEFAULT
             CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, &
                  dbcsr_wrong_args_error, routineN, "wrong matrix symmetry",244)
          END SELECT
       ENDIF
    ENDDO
...
! *****************************************************************************
  SUBROUTINE dbcsr_to_dense_local (sparse, dense)
    ...
    CALL dbcsr_data_get_sizes (dense, dense_row_size, dense_col_size, valid)
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",283)
    ...
    DO WHILE( dbcsr_iterator_blocks_left(iter) )
       ...
       IF(symm.NE.dbcsr_type_no_symmetry.AND.row.NE.col) THEN
          ...
          CASE DEFAULT
             CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, &
                  dbcsr_wrong_args_error, routineN, "wrong matrix symmetry",341)
          END SELECT
       ENDIF
    END DO
    CALL dbcsr_iterator_stop(iter)
...
!
    ...
    CALL dbcsr_assert (current_sum, "EQ", size_sum, dbcsr_fatal_level,&
         dbcsr_internal_error, routineN, "Incorrect block sizes",593)
