# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_methods.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE scf_env_density_mixing(p_mix_new,mixing_store,rho_ao,para_env,&
    ...
    iter_delta = 0.0_dp
    IF(.NOT.(ASSOCIATED(p_mix_new)))CALL cp__a("qs_scf_methods.F",121)
    DO ic=1,SIZE(p_mix_new,2)
       DO ispin=1,SIZE(p_mix_new,1)
          IF (my_invert) THEN
             IF(.NOT.(my_p_mix/=0.0_dp))CALL cp__a("qs_scf_methods.F",125)
...
!    Reduce KS matrix
      CALL cp_dbcsr_desymmetrize(matrix_ks,ksbuf2)
      CALL cp_dbcsr_multiply('N','N',1.0_dp,ksbuf2,ortho_dbcsr,0.0_dp,ksbuf1)
      CALL cp_dbcsr_multiply('T','N',1.0_dp,ortho_dbcsr,ksbuf1,0.0_dp,ksbuf2)
...
!    Restore the eigenvector of the general eig. problem
      CALL cp_dbcsr_sm_fm_multiply(ortho_dbcsr,work,mo_coeff,nmo)
...
! *****************************************************************************
  SUBROUTINE eigensolver_symm(matrix_ks_fm,mo_set,ortho,work,do_level_shift,&
    ...
    ELSE ! full S^(-1/2) has been computed
       ...
       CALL cp_gemm("T","N",nao,nao,nao,1.0_dp,ortho,work,0.0_dp,matrix_ks_fm)
       ...
       IF (do_level_shift) THEN
          DO imo=homo+1,nmo
             ...
             CALL cp_fm_set_element(matrix_ks_fm,imo,imo,alpha)
          END DO
       END IF
...
! *****************************************************************************
  SUBROUTINE cp_sm_mix(m1,m2,p_mix,delta,para_env,m3)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(ASSOCIATED(p_old_block)))CALL cp__a("qs_scf_methods.F",489)
       IF(PRESENT(m3)) THEN
          CALL cp_dbcsr_get_block_p(matrix=m3,row=iblock_row,col=iblock_col,&
               BLOCK=p_delta_block,found=found)
          IF(.NOT.(ASSOCIATED(p_delta_block)))CALL cp__a("qs_scf_methods.F",493)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(ksa)))CALL cp__a("qs_scf_methods.F",560)
    IF(.NOT.(ASSOCIATED(ksb)))CALL cp__a("qs_scf_methods.F",561)
    ...
    IF(.NOT.(compatible_matrices))CALL cp__a("qs_scf_methods.F",576)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(ksa)))CALL cp__a("qs_scf_methods.F",640)
    IF(.NOT.(ASSOCIATED(ksb)))CALL cp__a("qs_scf_methods.F",641)
    ...
    compatible_matrices = cp_fm_struct_equivalent(ksa_struct,ksb_struct)
    IF(.NOT.(compatible_matrices))CALL cp__a("qs_scf_methods.F",656)
