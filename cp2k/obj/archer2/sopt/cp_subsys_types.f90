# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/cp_subsys_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/cp_subsys_types.F"
!-----------------------------------------------------------------------------!
...
# 58 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/cp_subsys_types.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_subsys_retain(subsys)
    ...
    IF(.NOT.(ASSOCIATED(subsys)))CALL cp__a("subsys/cp_subsys_types.F",141)
    IF(.NOT.(subsys%ref_count>0))CALL cp__a("subsys/cp_subsys_types.F",142)
...
! *****************************************************************************
  SUBROUTINE cp_subsys_release(subsys)
    ...
    IF (ASSOCIATED(subsys)) THEN
       IF(.NOT.(subsys%ref_count>0))CALL cp__a("subsys/cp_subsys_types.F",160)
       subsys%ref_count=subsys%ref_count-1
       IF (subsys%ref_count==0) THEN
          ...
          CALL mol_kind_new_list_release(subsys%molecule_kinds_new)
          CALL mol_new_list_release(subsys%molecules_new)
          CALL distribution_1d_release(subsys%local_molecules_new)
          ...
          CALL virial_release(subsys%virial)
...
! *****************************************************************************
  SUBROUTINE cp_subsys_set(subsys, atomic_kinds, particles, local_particles,&
    ...
    IF(.NOT.(ASSOCIATED(subsys)))CALL cp__a("subsys/cp_subsys_types.F",238)
    IF(.NOT.(subsys%ref_count>0))CALL cp__a("subsys/cp_subsys_types.F",239)
    ...
    IF (PRESENT(colvar_p)) THEN
       IF(.NOT.(.NOT.ASSOCIATED(subsys%colvar_p)))CALL cp__a("subsys/cp_subsys_types.F",281)
    ...
    END IF
    IF (PRESENT(gci)) THEN
       IF(.NOT.(.NOT.ASSOCIATED(subsys%gci)))CALL cp__a("subsys/cp_subsys_types.F",299)
       subsys%gci => gci
    ENDIF
...
! *****************************************************************************
  SUBROUTINE cp_subsys_get(subsys, ref_count, atomic_kinds, atomic_kind_set, &
    ...
    IF(.NOT.(ASSOCIATED(subsys)))CALL cp__a("subsys/cp_subsys_types.F",407)
    IF(.NOT.(subsys%ref_count>0))CALL cp__a("subsys/cp_subsys_types.F",408)
    ...
    IF (PRESENT(natom).OR.PRESENT(nparticle).OR.PRESENT(nshell)) THEN
       ...
       IF(.NOT.(ASSOCIATED(subsys%particles)))CALL cp__a("subsys/cp_subsys_types.F",435)
       ...
       IF (ASSOCIATED(subsys%shell_particles)) THEN
          ...
          IF(.NOT.(ASSOCIATED(subsys%core_particles)))CALL cp__a("subsys/cp_subsys_types.F",440)
          n_core = subsys%core_particles%n_els
          ! The same number of shell and core particles is assumed
          IF(.NOT.(n_core == n_shell))CALL cp__a("subsys/cp_subsys_types.F",443)
       ELSE IF (ASSOCIATED(subsys%core_particles)) THEN
          ! This case should not occur at the moment
          IF(.NOT.(ASSOCIATED(subsys%shell_particles)))CALL cp__a("subsys/cp_subsys_types.F",446)
       ELSE
          n_core = 0
          n_shell = 0
       END IF
...
! *****************************************************************************
  SUBROUTINE pack_subsys_particles(subsys,f,r,s,v,fscale,cell)
    ...
    IF(.NOT.(ASSOCIATED(subsys)))CALL cp__a("subsys/cp_subsys_types.F",490)

    IF (PRESENT(s)) THEN
      IF(.NOT.(PRESENT(cell)))CALL cp__a("subsys/cp_subsys_types.F",493)
    END IF
    ...
    IF (PRESENT(f)) THEN
      IF(.NOT.((SIZE(f) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",512)
      j = 0
      DO iatom=1,natom
        ...
        ELSE
          DO i=1,3
            j = j + 1
            f(j) = core_particles%els(shell_index)%f(i) + particles%els(iatom)%f(i)
          END DO
    ...
    IF (PRESENT(r)) THEN
      IF(.NOT.((SIZE(r) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",538)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
    ...
    IF (PRESENT(s)) THEN
      IF(.NOT.((SIZE(s) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",563)
      IF(.NOT.(PRESENT(cell)))CALL cp__a("subsys/cp_subsys_types.F",564)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
    ...
    IF (PRESENT(v)) THEN
      IF(.NOT.((SIZE(v) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",592)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
...
! *****************************************************************************
  SUBROUTINE unpack_subsys_particles(subsys,f,r,s,v,fscale,cell)
    ...
    IF(.NOT.(ASSOCIATED(subsys)))CALL cp__a("subsys/cp_subsys_types.F",647)
    ...
    IF (PRESENT(f)) THEN
      IF(.NOT.((SIZE(f) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",665)
    ...
    IF (PRESENT(r)) THEN
      IF(.NOT.((SIZE(r) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",695)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
    ...
    IF (PRESENT(s)) THEN
      IF(.NOT.((SIZE(s) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",726)
      IF(.NOT.(PRESENT(cell)))CALL cp__a("subsys/cp_subsys_types.F",727)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
    ...
    IF (PRESENT(v)) THEN
      IF(.NOT.((SIZE(v) >= nsize)))CALL cp__a("subsys/cp_subsys_types.F",761)
      j = 0
      DO iatom=1,natom
        shell_index = particles%els(iatom)%shell_index
        IF (shell_index == 0) THEN
