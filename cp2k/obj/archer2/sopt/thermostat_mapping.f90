# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_mapping.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_mapping.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_mapping.F" 2
...
! *****************************************************************************
  SUBROUTINE adiabatic_mapping_region ( map_info, deg_of_freedom, massive_atom_list,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(deg_of_freedom)))CALL cp__a("motion/thermostat/thermostat_mapping.F",108)
    IF(.NOT.(.NOT.ASSOCIATED(massive_atom_list)))CALL cp__a("motion/thermostat/thermostat_mapping.F",109)
...
! *****************************************************************************
  SUBROUTINE adiabatic_mapping_region_low( region, map_info, nkind, point,&
    ...
    IF ( region==do_region_global) THEN
       ...
       IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",214)
       DO ikind = 1, nkind
...
! If molecule has no thermostat, then nullify pointers to that atom
       ...
       ELSE IF ( map_info%dis_type == do_thermo_communication ) THEN
          ...
          IF(.NOT.(nkind==1))CALL cp__a("motion/thermostat/thermostat_mapping.F",286)
          number = number + 1
          ntherm = ntherm + 1
          ...
          END DO
       ELSE
          CALL cp__b("motion/thermostat/thermostat_mapping.F",306,"")
       END IF
       IF (nglob_cns/=0) THEN
          CALL cp__b("motion/thermostat/thermostat_mapping.F",309,"Molecular thermostats with global constraints are impossible!")
       END IF
    ELSE IF ( region==do_region_massive) THEN
       ! Massive Region
       check = ( map_info%dis_type == do_thermo_no_communication )
       IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",314)
       DO ikind = 1, nkind
          nmol_local =  local_molecules % n_el ( ikind )
          DO imol_local = 1, nmol_local
             icount = icount + 1
             ...
             ELSE
                IF ((tot_const(icount)>0).OR.(nglob_cns/=0)) THEN
                   CALL cp__b("motion/thermostat/thermostat_mapping.F",328,"Massive thermostats with constraints are impossible!")
                END IF
             END IF
             k = 0
             ...
             IF ( first_atom + k -1 /= last_atom ) THEN
                CALL cp__b("motion/thermostat/thermostat_mapping.F",365,"Inconsistent mapping of particles")
             END IF
          END DO
       END DO
    ELSE
       CALL cp__b("motion/thermostat/thermostat_mapping.F",370,"Invalid region!")
    END IF
...
! *****************************************************************************
  SUBROUTINE adiabatic_region_evaluate(dis_type, natoms_local, nmol_local, const_mol,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(const_mol)))CALL cp__a("motion/thermostat/thermostat_mapping.F",438)
    IF(.NOT.(.NOT.ASSOCIATED(tot_const)))CALL cp__a("motion/thermostat/thermostat_mapping.F",439)
    IF(.NOT.(.NOT.ASSOCIATED(point)))CALL cp__a("motion/thermostat/thermostat_mapping.F",440)
...
! *****************************************************************************
  SUBROUTINE thermostat_mapping_region ( map_info, deg_of_freedom, massive_atom_list,&
    ...
    NULLIFY ( const_mol, tot_const, point)
    IF(.NOT.(.NOT.ASSOCIATED(deg_of_freedom)))CALL cp__a("motion/thermostat/thermostat_mapping.F",572)
    IF(.NOT.(.NOT.ASSOCIATED(massive_atom_list)))CALL cp__a("motion/thermostat/thermostat_mapping.F",573)
    ...
    check = (number==number_of_thermostats)
    IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",605)
    DEALLOCATE (const_mol)
    DEALLOCATE (tot_const)
    DEALLOCATE (point)
...
! *****************************************************************************
  SUBROUTINE  thermostat_mapping_region_low(region, map_info, nkind, point,&
    ...
    INTEGER :: first_atom, first_shell, handle, i, icount, ielement, ii, &
      ...
      last_shell, nglob_cns, nmol_local
    ...
    IF ( region==do_region_global) THEN
       ! Global Region
       check = ( map_info%dis_type == do_thermo_communication )
       IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",672)
       DO ikind = 1, nkind
          DO jj = point ( 1, ikind ), point ( 2, ikind )
             DO ii = 1, 3
                map_info%p_kin(ii,jj)%point => map_info%s_kin(1)
    ...
    ELSE IF ( region==do_region_defined) THEN
       ...
       check = ( map_info%dis_type == do_thermo_communication )
       IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",689)
       ...
       END DO
       IF (nglob_cns/=0) THEN
          CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",718),&
    ...
    ELSE IF ( region==do_region_molecule) THEN
       ...
       ELSE IF ( map_info%dis_type == do_thermo_communication ) THEN
          ! This case is quite rare and happens only when we have one molecular
          ! kind and one molecule..
          IF(.NOT.(nkind==1))CALL cp__a("motion/thermostat/thermostat_mapping.F",744)
          number = number + 1
          ...
          END DO
       ELSE
          CALL cp__b("motion/thermostat/thermostat_mapping.F",756,"")
       END IF
       IF (nglob_cns/=0) THEN
          CALL cp__b("motion/thermostat/thermostat_mapping.F",759,"Molecular thermostats with global constraints are impossible!")
       END IF
    ELSE IF ( region==do_region_massive) THEN
       ! Massive Region
       check = ( map_info%dis_type == do_thermo_no_communication )
       IF(.NOT.(check))CALL cp__a("motion/thermostat/thermostat_mapping.F",764)
       DO ikind = 1, nkind
          nmol_local =  local_molecules % n_el ( ikind )
          DO imol_local = 1, nmol_local
             icount = icount + 1
             ...
             ELSE
                IF ((tot_const(icount)>0).OR.(nglob_cns/=0)) THEN
                   CALL cp__b("motion/thermostat/thermostat_mapping.F",778,"Massive thermostats with constraints are impossible!")
                END IF
             END IF
             k = 0
             DO ii =  point ( 1, icount ), point ( 2, icount )
             ...
             END DO
             IF ( first_atom + k -1 /= last_atom ) THEN
                CALL cp__b("motion/thermostat/thermostat_mapping.F",795,"Inconsistent mapping of particles")
             END IF
          END DO
       END DO
    ELSE
       CALL cp__b("motion/thermostat/thermostat_mapping.F",800,"Invalid region!")
    END IF
...
! *****************************************************************************
  SUBROUTINE mapping_region_evaluate(dis_type, natoms_local, nmol_local, const_mol,&
    ...
    INTEGER :: atm_offset, first_atom, handle, i, iatm, icount, id_region, &
      ...
      nfixd, nkind, nmol_per_kind, nmolecule, nshell
    ...
    IF(.NOT.(.NOT.ASSOCIATED(const_mol)))CALL cp__a("motion/thermostat/thermostat_mapping.F",882)
    IF(.NOT.(.NOT.ASSOCIATED(tot_const)))CALL cp__a("motion/thermostat/thermostat_mapping.F",883)
    IF(.NOT.(.NOT.ASSOCIATED(point)))CALL cp__a("motion/thermostat/thermostat_mapping.F",884)
    IF ( dis_type == do_thermo_no_communication ) THEN
       ALLOCATE ( const_mol (nmol_local))
       ALLOCATE ( tot_const (nmol_local))
       ALLOCATE ( point (2, nmol_local))
                            ...
                            IF (.NOT.colv_list(i)%restraint%active) THEN
                               ...
                               DO j = 2, SIZE(colv_list(i)%i_atoms)
                                  ...
                                  IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                     CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",972),&
                                          "User Defined Region: "//&
                      ...
                      IF (ASSOCIATED(g3x3_list)) THEN
                         DO i = 1, SIZE(g3x3_list)
                            IF (.NOT.g3x3_list(i)%restraint%active) THEN
                               ...
                               IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                  CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",989),&
                                       "User Defined Region: "//&
                               ...
                               IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                  CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",996),&
                                       "User Defined Region: "//&
                                       "A constraint (G3X3) was defined between two thermostatting regions! "//&
                                       "This is not allowed!")
                               END IF
                      ...
                      IF (ASSOCIATED(g4x6_list)) THEN
                         DO i = 1, SIZE(g4x6_list)
                            IF (.NOT.g4x6_list(i)%restraint%active) THEN
                               ...
                               IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                  CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",1012),&
                                       " User Defined Region: "//&
                               ...
                               IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                  CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",1019),&
                                       " User Defined Region: "//&
                                       "A constraint (G4X6) was defined between two thermostatting regions! "//&
                                       "This is not allowed!")
                               END IF
                               ...
                               IF (map_loc_thermo_gen(iatm)/=map_loc_thermo_gen(jatm)) THEN
                                  CALL cp_abort(cp__l("motion/thermostat/thermostat_mapping.F",1026),&
                                       " User Defined Region: "//&
                                       "A constraint (G4X6) was defined between two thermostatting regions! "//&
                                       "This is not allowed!")
                               END IF
...
! *****************************************************************************
  SUBROUTINE massive_list_generate ( molecule_set, molecule_kind_set, &
    ...
    IF (stat /= 0) CALL stop_memory(routineN,moduleN,1188,&
