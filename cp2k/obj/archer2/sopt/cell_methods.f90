# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cell_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cell_methods.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cell_methods.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE read_cell( cell, cell_ref, use_ref_cell, cell_section,&
    ...
    LOGICAL                                  :: cell_read_a, cell_read_abc, &
                                                cell_read_b, cell_read_c, &
                                                cell_read_file, check, &
                                                my_check
    ...
    IF(.NOT.check)&
       CALL cp_warn(cp__l("cell_methods.F",132),&
    ...
    IF (cell_read_abc) THEN
       ...
       IF(check)&
          CALL cp_warn(cp__l("cell_methods.F",141),&
...
! *****************************************************************************
  FUNCTION parsed_cp2k_input(input_file,check_this_section) RESULT(res)
    ...
    IF (res) THEN
       IF(.NOT.(input_file%ref_count>0))CALL cp__a("cell_methods.F",219)
...
! Fail is one of the value is set to zero..
    ...
    IF(ANY(multiple_unit_cell<=0))&
       CALL cp_abort(cp__l("cell_methods.F",248),&
...
! *****************************************************************************
  SUBROUTINE read_cell_from_external_file(cell_section, para_env)
    ...
    IF(explicit)&
       CALL cp_warn(cp__l("cell_methods.F",319),&
