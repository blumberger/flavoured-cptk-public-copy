# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_harmonics_atom.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_harmonics_atom.F"
!-----------------------------------------------------------------------------!
...
# 23 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_harmonics_atom.F" 2
...
! *****************************************************************************
  SUBROUTINE deallocate_harmonics_atom(harmonics)
    ...
    ELSE
       CALL cp_abort(cp__l("qs_harmonics_atom.F",150),&
...
! *****************************************************************************
  SUBROUTINE create_harmonics_atom(harmonics,my_CG,na,&
       llmax,maxs,max_s_harm,ll,wa,pol,cos_pol,cotan_pol)

    TYPE(harmonics_atom_type), POINTER       :: harmonics
    REAL(dp), DIMENSION(:, :, :), POINTER    :: my_CG
    INTEGER, INTENT(IN)                      :: na, llmax, maxs, max_s_harm, &
                                                ll
    REAL(dp), DIMENSION(:), INTENT(IN)       :: wa, pol, cos_pol, cotan_pol
    ...
    IF(.NOT.(ASSOCIATED(harmonics)))CALL cp__a("qs_harmonics_atom.F",198)
...
! *****************************************************************************
  SUBROUTINE get_maxl_CG(harmonics,orb_basis,llmax,max_s_harm)
    ...
    IF(.NOT.(ASSOCIATED(harmonics)))CALL cp__a("qs_harmonics_atom.F",514)
...
! *****************************************************************************
  SUBROUTINE get_none0_cg_list4(cgc,lmin1,lmax1,lmin2,lmax2,max_s_harm,llmax,&
    ...
    IF(.NOT.(nsoset(lmax1).LE.SIZE(cgc,2)))CALL cp__a("qs_harmonics_atom.F",577)
    IF(.NOT.(nsoset(lmax2).LE.SIZE(cgc,3)))CALL cp__a("qs_harmonics_atom.F",578)
    IF(.NOT.(max_s_harm.LE.SIZE(cgc,4)))CALL cp__a("qs_harmonics_atom.F",579)
    IF(PRESENT(n_list).AND.PRESENT(list)) THEN
       IF(.NOT.(max_s_harm.LE.SIZE(list,3)))CALL cp__a("qs_harmonics_atom.F",581)
    ENDIF
    max_iso_not0 = 0
...
! *****************************************************************************
  SUBROUTINE get_none0_cg_list3(cgc,lmin1,lmax1,lmin2,lmax2,max_s_harm,llmax,&
    ...
    IF(.NOT.(nsoset(lmax1).LE.SIZE(cgc,1)))CALL cp__a("qs_harmonics_atom.F",640)
    IF(.NOT.(nsoset(lmax2).LE.SIZE(cgc,2)))CALL cp__a("qs_harmonics_atom.F",641)
    IF(.NOT.(max_s_harm.LE.SIZE(cgc,3)))CALL cp__a("qs_harmonics_atom.F",642)
    IF(PRESENT(n_list).AND.PRESENT(list)) THEN
       IF(.NOT.(max_s_harm.LE.SIZE(list,3)))CALL cp__a("qs_harmonics_atom.F",644)
    ENDIF
    max_iso_not0 = 0
    IF(PRESENT(n_list).AND.PRESENT(list)) n_list = 0
    DO iso = 1,max_s_harm
