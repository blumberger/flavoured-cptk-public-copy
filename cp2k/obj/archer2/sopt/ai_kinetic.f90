# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_kinetic.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_kinetic.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_kinetic.F" 2
...
! *****************************************************************************
  SUBROUTINE kinetic(la_max,la_min,npgfa,rpgfa,zeta,&
    ...
    REAL(KIND=dp), DIMENSION(:, :), &
      INTENT(INOUT), OPTIONAL                :: kab
    REAL(KIND=dp), DIMENSION(:, :, :), &
      INTENT(INOUT), OPTIONAL                :: dab

    CHARACTER(len=*), PARAMETER :: routineN = 'kinetic', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: ax, ay, az, bx, by, bz, coa, &
                                                cob, ia, ib, idx, idy, idz, &
                                                ipgf, jpgf, la, lb, ldrr, &
                                                lma, lmb, ma, mb, na, nb, &
                                                ofa, ofb
    REAL(KIND=dp)                            :: a, b, dsx, dsy, dsz, dtx, &
                                                dty, dtz, f0, rab2, tab, xhi, &
                                                zet
...
! Distance of the centers a and b
    ...
    IF (PRESENT(kab)) THEN
       IF(.NOT.((SIZE(kab,1) >= na*npgfa)))CALL cp__a("aobasis/ai_kinetic.F",113)
       IF(.NOT.((SIZE(kab,2) >= nb*npgfb)))CALL cp__a("aobasis/ai_kinetic.F",114)
    END IF
    IF (PRESENT(dab)) THEN
       IF(.NOT.((SIZE(dab,1) >= na*npgfa)))CALL cp__a("aobasis/ai_kinetic.F",117)
       IF(.NOT.((SIZE(dab,2) >= nb*npgfb)))CALL cp__a("aobasis/ai_kinetic.F",118)
       IF(.NOT.((SIZE(dab,3) >= 3)))CALL cp__a("aobasis/ai_kinetic.F",119)
    END IF
    ...
    DO ipgf=1,npgfa
       ...
       DO jpgf=1,npgfb
          ...
          ! Calculate the reccurence relation, overlap
