# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_util.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_util.F" 2
...
! *****************************************************************************
  SUBROUTINE PSolver(geocode,iproc,nproc,n01,n02,n03,hx,hy,hz,&
    ...
    INTEGER :: i1, i2, i3, iend, istart, j2, m1, m2, m3, md1, md2, md3, n1, &
      n2, n3, nd1, nd2, nd3, nlim, nwb, nwbl, nwbr, nxc, nxcl, nxcr, nxt
...
!calculate the dimensions wrt the geocode
    ...
    ELSE
       CALL cp__b("pw/ps_wavelet_util.F",106,"PSolver: geometry code not admitted")
    END IF
    !array allocations
...
!dimensions of the density in the real space
    ...
    ELSE
       ...
       CALL cp__b("pw/ps_wavelet_util.F",259,"")
    END IF
    l1=l1+1
    ...
    ELSE
       ...
       CALL cp__b("pw/ps_wavelet_util.F",267,"")
    END IF
    ...
    ELSE
       ...
       CALL cp__b("pw/ps_wavelet_util.F",274,"")
    END IF
...
!dimensions of the density in the real space
    ...
    ELSE
       PRINT *,'the FFT in the x direction is not allowed'
       PRINT *,'n01 dimension',n01
       CALL cp__b("pw/ps_wavelet_util.F",359,"")
    END IF
    l1=l1+1
    CALL fourier_dim(l2,n2)
    IF (n2 == m2) THEN
    ELSE
       PRINT *,'the FFT in the z direction is not allowed'
       PRINT *,'n03 dimension',n03
       CALL cp__b("pw/ps_wavelet_util.F",367,"")
    END IF
    DO
       CALL fourier_dim(l3,n3)
...
! *****************************************************************************
  SUBROUTINE  scale_and_distribute(m1,m3,md1,md2,md3,nxc,&
    ...
    !Arguments----------------------
    INTEGER, INTENT(in)                      :: m1, m3, md1, md2, md3, nxc, &
                                                nproc
    REAL(KIND=dp), &
