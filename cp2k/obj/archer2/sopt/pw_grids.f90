# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F" 2
...
# 94 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F"
...
! *****************************************************************************
  SUBROUTINE pw_grid_create ( pw_grid, pe_group, local)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(pw_grid)))CALL cp__a("pw/pw_grids.F",124)
    ALLOCATE(pw_grid)
    pw_grid % bounds = 0
...
! *****************************************************************************
  SUBROUTINE get_pw_grid_info(pw_grid, id_nr, mode, vol, dvol,  npts, ngpts,&
    ...
    IF(.NOT.(pw_grid%ref_count>0))CALL cp__a("pw/pw_grids.F",237)
...
! *****************************************************************************
  SUBROUTINE set_pw_grid_info(pw_grid, grid_span, npts, bounds, cutoff, spherical)
    ...
    IF(.NOT.(pw_grid%ref_count>0))CALL cp__a("pw/pw_grids.F",280)
    ...
    IF ( PRESENT(bounds) .AND. PRESENT(npts) ) THEN
       ...
       pw_grid%npts = npts
       IF(.NOT.(ALL(npts==bounds(2,:)-bounds(1,:)+1)))CALL cp__a("pw/pw_grids.F",288)
...
! *****************************************************************************
  SUBROUTINE pw_grid_setup (cell_hmat, pw_grid, grid_span, cutoff, bounds, bounds_local, npts, &
    ...
    IF (cell_deth < 1.0E-10_dp) THEN
       CALL cp_abort(cp__l("pw/pw_grids.F",365),&
    ...
    ELSE IF ( PRESENT(cutoff) ) THEN
       ...
       CALL set_pw_grid_info ( pw_grid, npts=n, cutoff=cutoff, &
            spherical=my_spherical)
    ELSE
       CALL cp__b("pw/pw_grids.F",441,"BOUNDS, NPTS or CUTOFF have to be specified")
    END IF
...
# 544 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F"
...
!------------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(pw_grid)))CALL cp__a("pw/pw_grids.F",596)
    IF(.NOT.(pw_grid%ref_count>0))CALL cp__a("pw/pw_grids.F",597)
    ...
    IF ( PRESENT ( ref_grid ) ) THEN
       IF(.NOT.(pw_grid % para % mode == ref_grid % para % mode))CALL cp__a("pw/pw_grids.F",622)
       IF ( pw_grid % para % mode == PW_MODE_DISTRIBUTED ) THEN
          CALL mp_comm_compare(pw_grid % para % group, ref_grid % para % group, ires )
          IF(.NOT.(ires <= 3))CALL cp__a("pw/pw_grids.F",625)
       END IF
       IF(.NOT.(pw_grid % grid_span == ref_grid % grid_span))CALL cp__a("pw/pw_grids.F",627)
       IF(.NOT.(pw_grid % spherical .EQV. ref_grid % spherical))CALL cp__a("pw/pw_grids.F",628)
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_grid_distribute ( pw_grid, yz_mask, bounds_local, ref_grid, blocked, rs_dims)
    ...
    INTEGER :: blocked_local, coor( 2 ), gmax, handle, i, i1, i2, ip, ipl, &
      ipp, itmp, j, l, lby, lbz, lo( 2 ), m, n, np, ns, nx, ny, nz, rsd( 2 )
...
!------------------------------------------------------------------------------
    ...
    IF(.NOT.(ALL(pw_grid%para%rs_dims==0)))CALL cp__a("pw/pw_grids.F",860)
...
IF(.NOT.(pw_grid%ngpts_cut<HUGE(pw_grid%ngpts_cut_local)))CALL cp__a("pw/pw_grids.F",881)
       pw_grid % ngpts_cut_local = INT(pw_grid % ngpts_cut)
IF(.NOT.(pw_grid%ngpts<HUGE(pw_grid%ngpts_local)))CALL cp__a("pw/pw_grids.F",883)
       ...
       ELSE
          ...
          DO ip = 0, np - 1
             ...
             DO n = pw_grid % para % bo ( 1, 3, ipp, 3 ), &
                ...
                END DO
             END DO
             IF(.NOT.(ns == pw_grid%para%nyzray(ip)))CALL cp__a("pw/pw_grids.F",1197)
          END DO
...
! *****************************************************************************
  SUBROUTINE pw_grid_count ( h_inv, pw_grid, cutoff, yz_mask )
    ...
    ELSE
       CALL cp__b("pw/pw_grids.F",1457,"No type set for the grid")
    END IF
    ...
    ELSE
       CALL cp__b("pw/pw_grids.F",1470,"para % mode not specified")
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_grid_assign ( h_inv, pw_grid, cutoff)
    ...
    ELSE IF ( pw_grid % grid_span == FULLSPACE ) THEN
       n_upperlimit = bounds ( 2, 3 )
    ELSE
       CALL cp__b("pw/pw_grids.F",1542,"No type set for the grid")
    END IF
    ...
    IF(.NOT.(pw_grid % ngpts_cut_local == gpt))CALL cp__a("pw/pw_grids.F",1649)
    IF ( pw_grid % para % mode == PW_MODE_DISTRIBUTED ) THEN
       gpt_global = gpt
       CALL mp_sum ( gpt_global, pw_grid % para % group )
       IF(.NOT.(pw_grid % ngpts_cut == gpt_global))CALL cp__a("pw/pw_grids.F",1653)
    ENDIF
...
# 1793 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_grids.F"
  ...
  SUBROUTINE pw_grid_sort ( pw_grid, ref_grid)
    ...
    IF ( PRESENT ( ref_grid ) ) THEN
       ...
       IF ( pw_grid % spherical ) THEN
          IF ( .NOT. ALL ( pw_grid % g_hat ( 1:3, 1:ngr ) &
             ...
             CALL cp__b("pw/pw_grids.F",1895,"G space sorting not compatible")
          END IF
       ELSE
          ...
          IF ( ng == ngr ) THEN
             ...
             DO ig = it + 1, ngr
                ...
                IF ( .NOT. g_found ) THEN
                   ...
                   CALL cp__b("pw/pw_grids.F",1923,"G vector not found")
                END IF
                ip = ih - 1
                ...
                IF ( pw_grid%gidx(ig) == 0 ) THEN
                   ...
                   END DO
                   CALL cp__b("pw/pw_grids.F",1945,"G vector not found")
                END IF
                is = ip
             END DO
          ELSE
             ...
             DO ig = it + 1, ngr
                ...
                IF ( .NOT. g_found ) THEN
                   ...
                   CALL cp__b("pw/pw_grids.F",1963,"G vector not found")
                END IF
                ip = ih - 1
                DO ih = ip + 1, ng
                   IF ( ABS ( pw_grid % gsq(ih) - gig )/gigr > 1.e-12_dp ) CYCLE
                ...
                IF ( pw_grid%gidx(ig) == 0 ) THEN
                   ...
                   END DO
                   CALL cp__b("pw/pw_grids.F",1985,"G vector not found")
                END IF
                is = ip
             END DO
          END IF
          ...
          IF ( ANY ( pw_grid%gidx == 0 ) ) THEN
             CALL cp__b("pw/pw_grids.F",1992,"G space sorting not compatible")
          END IF
       END IF
    END IF
    ...
    IF ( pw_grid % have_g0 ) THEN
       IF ( pw_grid % gsq ( 1 ) /= 0.0_dp ) THEN
          CALL cp__b("pw/pw_grids.F",2000,"G = 0 not in first position")
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_grid_remap ( pw_grid, yz)
    ...
    IF(.NOT.(is == pw_grid % para % nyzray ( ip )))CALL cp__a("pw/pw_grids.F",2164)
...
! *****************************************************************************
  SUBROUTINE pw_grid_change ( cell_hmat, pw_grid )
    ...
    cell_deth = ABS(det_3x3(cell_hmat))
    IF (cell_deth < 1.0E-10_dp) THEN
       CALL cp_abort(cp__l("pw/pw_grids.F",2200),&
            "An invalid set of cell vectors was specified. "//&
            "The determinant det(h) is too small")
    END IF
    cell_h_inv = inv_3x3(cell_hmat)
...
! *****************************************************************************
  SUBROUTINE pw_grid_retain(pw_grid)
    ...
    IF(.NOT.(ASSOCIATED(pw_grid)))CALL cp__a("pw/pw_grids.F",2243)
    IF(.NOT.(pw_grid%ref_count>0))CALL cp__a("pw/pw_grids.F",2244)
...
! *****************************************************************************
  SUBROUTINE pw_grid_release(pw_grid)
    ...
    IF (ASSOCIATED(pw_grid)) THEN
       IF(.NOT.(pw_grid%ref_count>0))CALL cp__a("pw/pw_grids.F",2270)
       pw_grid%ref_count=pw_grid%ref_count-1
