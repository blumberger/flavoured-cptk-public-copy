# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_input.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_input.F" 2
...
! *****************************************************************************
  SUBROUTINE run_optimize_input(input_declaration, root_section, para_env)
    ...
    CASE DEFAULT
      CALL cp__b("optimize_input.F",148,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE read_reference_data(oi_env,para_env,force_traj,pos_traj,energy_traj,cell_traj)
    ...
    IF(filename.EQ."")&
       CALL cp__b("optimize_input.F",547,"The reference trajectory file name is empty")
    ...
    IF(filename.EQ."")&
       CALL cp__b("optimize_input.F",581,"The reference force file name is empty")
    CALL parser_create(local_parser,filename,para_env=para_env)
...
999    CONTINUE
       IF (.NOT. test_ok) THEN
           CALL cp__b("optimize_input.F",594,"Could not parse the title line of the trajectory file")
       END IF
...
! *****************************************************************************
  SUBROUTINE parse_input(oi_env,root_section)
    ...
    CASE DEFAULT
      CALL cp__b("optimize_input.F",692,"")
    END SELECT
