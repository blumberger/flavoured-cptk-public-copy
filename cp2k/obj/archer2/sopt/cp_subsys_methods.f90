# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_subsys_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_subsys_methods.F"
!-----------------------------------------------------------------------------!
...
# 68 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_subsys_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_subsys_create(subsys, para_env, &
    ...
    LOGICAL :: atomic_energy, atomic_stress, my_use_motion_section, &
    ...
    IF(.NOT.(.NOT.ASSOCIATED(subsys)))CALL cp__a("cp_subsys_methods.F",130)
    ALLOCATE(subsys)
    ...
    CALL mol_new_list_create(mols,els_ptr=molecule_set)
    CALL mol_kind_new_list_create(mol_kinds,els_ptr=molecule_kind_set)
    ...
    CALL mol_new_list_release(mols)
    CALL mol_kind_new_list_release(mol_kinds)
    ...
    IF (atomic_stress) THEN
       IF(.NOT.(pv_availability))CALL cp__a("cp_subsys_methods.F",210)
       IF(.NOT.(.NOT.pv_numerical))CALL cp__a("cp_subsys_methods.F",211)
    END IF
...
! *****************************************************************************
  SUBROUTINE create_small_subsys(small_subsys, big_subsys, small_cell,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(small_subsys)))CALL cp__a("cp_subsys_methods.F",299)
    IF(.NOT.(ASSOCIATED(big_subsys)))CALL cp__a("cp_subsys_methods.F",300)
    IF(big_subsys%para_env%group/=small_para_env%group)&
       CALL cp__b("cp_subsys_methods.F",302,"big_subsys%para_env%group==small_para_env%group")
    ...
    topology%natoms=nat
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%r)))CALL cp__a("cp_subsys_methods.F",323)
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%id_atmname)))CALL cp__a("cp_subsys_methods.F",324)
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%id_molname)))CALL cp__a("cp_subsys_methods.F",325)
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%id_resname)))CALL cp__a("cp_subsys_methods.F",326)
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%atm_mass)))CALL cp__a("cp_subsys_methods.F",327)
    IF(.NOT.(.NOT.ASSOCIATED(topology%atom_info%atm_charge)))CALL cp__a("cp_subsys_methods.F",328)
