# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/parallel_rng_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/parallel_rng_types.F"
!-----------------------------------------------------------------------------!
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/parallel_rng_types.F" 2
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",193)
...
! -------------------------------------------------------------------------
    ...
    DO i=1,3
      ...
      IF (seed(i,1) < 0.0_dp) THEN
        ...
        CALL cp__b("common/parallel_rng_types.F",455,message)
      END IF
      IF (seed(i,1) >= m1) THEN
        WRITE (UNIT=message,FMT=fmtstr)&
          "seed(",i,",1) = ",seed(i,1)," >= ",m1
        CALL compress(message)
        CALL cp__b("common/parallel_rng_types.F",461,message)
      END IF
      ...
      IF (seed(i,2) < 0.0_dp) THEN
        ...
        CALL compress(message)
        CALL cp__b("common/parallel_rng_types.F",470,message)
      END IF
      IF (seed(i,2) >= m2) THEN
        WRITE (UNIT=message,FMT=fmtstr)&
          "seed(",i,",2) = ",seed(i,2)," >= ",m2
        CALL compress(message)
        CALL cp__b("common/parallel_rng_types.F",476,message)
      END IF
    ...
    IF (ALL(seed(:,1) < 1.0_dp)) THEN
      CALL cp__b("common/parallel_rng_types.F",484,"First seed = 0")
    END IF

    IF (ALL(seed(:,2) < 1.0_dp)) THEN
      CALL cp__b("common/parallel_rng_types.F",488,"Second seed = 0")
    END IF
...
! -------------------------------------------------------------------------
    ...
    IF (PRESENT(distribution_type)) THEN
      ...
      CASE DEFAULT
        CALL cp__b("common/parallel_rng_types.F",554,"Invalid distribution type specified")
      END SELECT
    ELSE IF (PRESENT(last_rng_stream)) THEN
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",601)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",625)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",680)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",996)
    IF(.NOT.(rng_stream%distribution_type == UNIFORM))CALL cp__a("common/parallel_rng_types.F",997)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1026)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1111)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1138)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1167)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1229)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1269)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1398)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(rng_stream)))CALL cp__a("common/parallel_rng_types.F",1489)
