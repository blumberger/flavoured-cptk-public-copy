# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_wf_history_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_wf_history_types.F"
!-----------------------------------------------------------------------------!
...
# 26 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_wf_history_types.F" 2
...
! *****************************************************************************
  TYPE qs_wf_history_type
     ...
     LOGICAL :: store_wf, store_rho_r, store_rho_g, store_rho_ao, store_rho_ao_kp, &
...
SUBROUTINE wfs_release(snapshot)
  ...
  IF (ASSOCIATED(snapshot)) THEN
     IF(.NOT.(snapshot%ref_count>0))CALL cp__a("qs_wf_history_types.F",135)
     snapshot%ref_count=snapshot%ref_count-1
     IF (snapshot%ref_count==0) THEN
        IF (ASSOCIATED(snapshot%wf)) THEN
           DO i=1,SIZE(snapshot%wf)
              CALL cp_fm_release(snapshot%wf(i)%matrix)
           END DO
           DEALLOCATE(snapshot%wf)
        END IF
...
SUBROUTINE wfi_retain(wf_history)
  ...
  IF(.NOT.(ASSOCIATED(wf_history)))CALL cp__a("qs_wf_history_types.F",177)
...
SUBROUTINE wfi_release(wf_history)
  ...
  IF (ASSOCIATED(wf_history)) THEN
     IF(.NOT.(wf_history%ref_count>0))CALL cp__a("qs_wf_history_types.F",199)
     wf_history%ref_count=wf_history%ref_count-1
...
FUNCTION wfi_get_snapshot(wf_history, index) RESULT(res)
  ...
  IF(.NOT.(ASSOCIATED(wf_history)))CALL cp__a("qs_wf_history_types.F",233)
  IF(.NOT.(ASSOCIATED(wf_history%past_states)))CALL cp__a("qs_wf_history_types.F",234)
  IF (index>wf_history%memory_depth.OR.index>wf_history%snapshot_count) THEN
     CALL cp__b("qs_wf_history_types.F",236,"")
  END IF
