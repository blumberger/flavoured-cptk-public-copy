# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings_report.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings_report.F"
!-----------------------------------------------------------------------------!
...
# 45 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/timings_report.F" 2
...
! *****************************************************************************
  SUBROUTINE collect_reports_from_ranks(reports, cost_type, para_env)
    ...
    IF(.NOT. list_isready(reports))&
       CALL cp__b("common/timings_report.F",117,"BUG")
    ...
    DO
      ...
      IF(dict_haskey(timer_env%routine_names, routineN)) THEN
        ...
        SELECT CASE(cost_type)
          ...
          CASE DEFAULT
            CALL cp__b("common/timings_report.F",177,"BUG")
        END SELECT
      END IF
...
! *****************************************************************************
  SUBROUTINE print_reports(reports, iw,threshold,sort_by_exclusiv_cost,cost_type, para_env)
    ...
    IF(.NOT. list_isready(reports))&
       CALL cp__b("common/timings_report.F",225,"BUG")
    ...
    SELECT CASE(cost_type)
      ...
      CASE(cost_type_time)
         ...
         label = "TIME"
      CASE DEFAULT
        CALL cp__b("common/timings_report.F",236,"BUG")
    END SELECT
