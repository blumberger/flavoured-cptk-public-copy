# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_io.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_io.F"
!-----------------------------------------------------------------------------!
...
# 48 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_io.F" 2
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,&
       ...
       DO ikey=1,SIZE(keys)
          ...
          IF (BTEST(cp_print_key_should_output(logger%iter_info,&
             ...
             END IF
             IF(.NOT.(ASSOCIATED(scf_env%scf_work1)))CALL cp__a("kpoint_io.F",123)
  ...
  SUBROUTINE write_kpoints_file_header(qs_kind_set, particle_set, ires)
    ...
    IF (ires>0) THEN
       ...
       DO iatom=1,natom
          ...
          ELSE
             CALL cp__b("kpoint_io.F",235,"Unknown basis type.")
          END IF
       END DO
       ...
       DO iatom=1,natom
          ...
          ELSEIF (ASSOCIATED(dftb_parameter)) THEN
             ...
             nao = nao + (lmax+1)**2
          ELSE
             CALL cp__b("kpoint_io.F",275,"Unknown basis set type.")
          END IF
       END DO
...
! *****************************************************************************
  SUBROUTINE read_kpoints_restart(denmat,kpoints,fmwork,natom,&
    ...
    IF(.NOT.(ASSOCIATED(fmwork)))CALL cp__a("kpoint_io.F",350)
    IF(.NOT.(ASSOCIATED(fmwork(1)%matrix)))CALL cp__a("kpoint_io.F",351)
...
! *****************************************************************************
  SUBROUTINE read_kpoints_restart_low (denmat,kpoints,fmat,para_env,&
    ...
    IF(.NOT.(ASSOCIATED(denmat)))CALL cp__a("kpoint_io.F",402)
    ...
    IF (para_env%ionode) THEN
       ...
       IF (version_read /= version) THEN
            CALL cp_abort(cp__l("kpoint_io.F",415),&
       ...
       IF (natom_match) THEN 
         IF (nao_read /= nao) THEN
            CALL cp_abort(cp__l("kpoint_io.F",422),&
    ...
    IF (natom_mismatch) THEN
          CALL cp_warn(cp__l("kpoint_io.F",436),&
    ...
    ELSE
       ...
       DO
          ...
          IF (nspin_read /= nspin) THEN
             CALL cp_abort(cp__l("kpoint_io.F",449),&
