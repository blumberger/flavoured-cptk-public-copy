# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_resp.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_resp.F"
!-----------------------------------------------------------------------------!
...
# 94 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_resp.F" 2
...
! *****************************************************************************
  SUBROUTINE resp_fit(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_resp.F",167)
    ...
    IF (has_resp) THEN
       ...
       CASE DEFAULT
        CALL cp_abort(cp__l("qs_resp.F",204),&
       ...
       IF(.NOT.(info==0))CALL cp__a("qs_resp.F",223)

       CALL DGETRS('N',nvar,1,resp_env%matrix,nvar,ipiv,resp_env%rhs,nvar,info)
       IF(.NOT.(info==0))CALL cp__a("qs_resp.F",226)
...
! *****************************************************************************
  SUBROUTINE resp_allocate(resp_env,natom,nvar)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'resp_allocate', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE init_resp(resp_env, rep_sys, subsys, atomic_kind_set, &
    ...
    IF(SIZE(my_stride)/=1.AND.SIZE(my_stride)/=3)&
       CALL cp_abort(cp__l("qs_resp.F",413), "STRIDE keyword can accept only 1 (the same for X,Y,Z) "//&
    ...
    IF (explicit) THEN
       IF(resp_env%molecular_sys) THEN 
         CALL cp_abort(cp__l("qs_resp.F",442),&
       ...
       DO i=1, nrep
          ...
          IF(ANY(rep_sys(i)%p_resp%range_surf<0.0_dp)) THEN
             CALL cp__b("qs_resp.F",457,"Numbers in RANGE in SLAB_SAMPLING cannot be negative.")
          END IF
          IF(rep_sys(i)%p_resp%length<=EPSILON(0.0_dp)) THEN
             CALL cp__b("qs_resp.F",460,"Parameter LENGTH in SLAB_SAMPLING has to be larger than zero.")
          END IF
...
! *****************************************************************************
  SUBROUTINE get_parameter_molecular_sys(resp_env,sphere_section,cell,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'get_parameter_molecular_sys', &
      routineP = moduleN//':'//routineN
    ...
    INTEGER                                  :: ikind, j, kind_number, &
                                                n_rmax_missing, &
                                                n_rmin_missing, nkind, &
                                                nrep_rmax, nrep_rmin, z
    LOGICAL                                  :: explicit, has_rmax, has_rmin
    LOGICAL, ALLOCATABLE, DIMENSION(:)       :: rmax_is_set, rmin_is_set
    REAL(KIND=dp)                            :: auto_rmax_scale, &
                                                auto_rmin_scale, rmax, rmin
    ...
    IF (explicit) THEN
       ...
       IF (n_rmin_missing > 0) THEN
          CALL cp_warn(cp__l("qs_resp.F",644), &
       ...
       IF (n_rmax_missing > 0) THEN
          CALL cp_warn(cp__l("qs_resp.F",651), &
       ...
       IF (n_rmin_missing > 0 .OR. &
          ...
          CALL cp__b("qs_resp.F",659,"Insufficient data for RMIN or RMAX")
       END IF
...
! *****************************************************************************
  SUBROUTINE build_atom_list(section,subsys,atom_list,rep)
    ...
    IF(.NOT.(SIZE(atom_list) /= 0))CALL cp__a("qs_resp.F",741)
    index_in_range=(MAXVAL(atom_list)<= max_index)&
                         .AND.(MINVAL(atom_list) > 0)
    IF(.NOT.(index_in_range))CALL cp__a("qs_resp.F",744)
    DO i=1,num_atom
     DO j=i+1,num_atom
      atom_a=atom_list(i)
      atom_b=atom_list(j)
      IF(atom_a==atom_b) &
      CALL cp__b("qs_resp.F",750,"There are atoms doubled in atom list for RESP.")
     ENDDO
    ENDDO
...
! *****************************************************************************
  SUBROUTINE calc_resp_matrix_nonper(qs_env,resp_env,atomic_kind_set,particles,&
    ...
    REAL(KIND=dp)                            :: delta, dh(3,3), dvol, r(3), &
                                                rmax, rmin, vec(3), &
                                                vec_pbc(3), vj
...
! *****************************************************************************
  SUBROUTINE calc_resp_matrix_periodic(qs_env,resp_env,rep_sys,particles,cell,&
    ...
    IF(.NOT.ALL(periodic/=0)) THEN
        CALL cp_abort(cp__l("qs_resp.F",961),&
    ...
    IF(resp_env%use_repeat_method) THEN
      ...
      ENDDO
      CALL mp_sum(resp_env%sum_vpot,para_env%group)
      CALL mp_sum(resp_env%sum_vhartree,para_env%group)
...
! *****************************************************************************
  SUBROUTINE get_fitting_points(qs_env,resp_env,rep_sys,particles,cell)
    ...
    INTEGER :: bo(2,3), gbo(2,3), handle, i, iatom, ikind, in_x, in_y, in_z, &
      jx, jy, jz, k, kind_number, l, m, natom, nkind, now, p
    LOGICAL, ALLOCATABLE, DIMENSION(:, :)    :: not_in_range
    REAL(KIND=dp)                            :: delta, dh(3,3), r(3), rmax, &
                                                rmin, vec_pbc(3)
    REAL(KIND=dp), ALLOCATABLE, DIMENSION(:) :: dist
    TYPE(atomic_kind_type), DIMENSION(:), &
      POINTER                                :: atomic_kind_set
...
! *****************************************************************************
  SUBROUTINE print_fitting_points(qs_env,resp_env)
    ...
    INTEGER                                  :: gbo(2,3), handle, i, iatom, &
                                                ip, jx, jy, jz, k, l, my_pos, &
                                                nobjects, output_unit, p, &
                                                req(6)
  ...
  SUBROUTINE add_restraints_and_constraints(qs_env,resp_env,rest_section,&
    ...
    DO i=1,resp_env%nrest_sec
       ...
       IF (explicit_coeff) THEN
        ...
        IF(.NOT.(SIZE(atom_list_res)==SIZE(atom_coef)))CALL cp__a("qs_resp.F",1436)
       ENDIF
    ...
    DO i=1,resp_env%ncons_sec
       ...
       IF(.NOT.resp_env%equal_charges) THEN
         ...
         IF(.NOT.(SIZE(atom_list_cons)==SIZE(atom_coef)))CALL cp__a("qs_resp.F",1496)
...
! *****************************************************************************
  SUBROUTINE print_pot_from_resp_charges(qs_env,resp_env,particles,natom,output_runinfo)
    ...
    INTEGER                                  :: handle, ip, jx, jy, jz, &
                                                unit_nr
    LOGICAL                                  :: append_cube
    REAL(KIND=dp)                            :: dvol, normalize_factor, rms, &
                                                rrms, sum_diff, sum_hartree, &
                                                udvol
    TYPE(cp_logger_type), POINTER            :: logger
    TYPE(cp_para_env_type), POINTER          :: para_env
    TYPE(pw_env_type), POINTER               :: pw_env
