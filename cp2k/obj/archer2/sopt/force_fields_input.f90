# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_input.F"
!-----------------------------------------------------------------------------!
...
MODULE force_fields_input
  ...
  USE pair_potential_types,            ONLY: &
       ...
       eam_pot_type, ft_pot_type, ft_type, ftd_type, gp_type, gw_type, &
       ip_type, ipbv_pot_type, lj_charmm_type, &
...
# 67 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_input.F" 2
...
INTEGER::i
    ...
    IF(.NOT.(ff_type%max_energy<=ff_type%emax_spline))CALL cp__a("force_fields_input.F",120)
    ...
    CASE(do_ff_charmm,do_ff_amber,do_ff_g96,do_ff_g87)
       ...
       IF(TRIM(ff_type%ff_file_name)=="")&
          CALL cp__b("force_fields_input.F",126,"Force Field Parameter's filename is empty! Please check your input file.")
    CASE(do_ff_undef)
       ! Do Nothing
    CASE DEFAULT
       CALL cp__b("force_fields_input.F",130,"Force field type not implemented")
    END SELECT
    ! Numerical Accuracy:
    ...
    IF (ff_type%eps_spline < min_eps_spline_allowed) THEN
       CALL cp_warn(cp__l("force_fields_input.F",140),&
...
! *****************************************************************************
  SUBROUTINE set_IPBV_ff(at1, at2, ipbv )
    ...
    ELSE
       CALL cp__b("force_fields_input.F",515,"IPBV only for WATER")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE set_BMHFT_ff(at1, at2, ft)
    ...
    ELSE
       CALL cp__b("force_fields_input.F",548,"BMHFT only for NaCl")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE set_BMHFTD_ff()
    ...
    CALL cp__b("force_fields_input.F",562,"No default parameters present for BMHFTD")
...
! *****************************************************************************
  SUBROUTINE read_b4_section(nonbonded,section,start)
    ...
    CHARACTER(LEN=default_string_length), &
      DIMENSION(:), POINTER                  :: atm_names
    INTEGER                                  :: i, ir, isec, n_items, n_rep, &
                                                np1, np2
    ...
    DO isec=1,n_items
      ...
      IF ((.NOT.explicit_poly1).OR.(.NOT.explicit_poly2)) THEN
        ...
        IF(eval_error >= 1.0E-8_dp)&
           CALL cp_warn(cp__l("force_fields_input.F",1114),&
      ...
      ELSE
        ...
        IF(.NOT.(np1 - 1 <= 10))CALL cp__a("force_fields_input.F",1142)
        ...
        IF(.NOT.(np2 - 1 <= 10))CALL cp__a("force_fields_input.F",1145)
...
! *****************************************************************************
  SUBROUTINE read_shell_section(shell_list,section,start)
    ...
    DO i_rep = 1, n_rep
       ...
       IF (k < 0.0_dp) THEN
          CALL cp_abort(cp__l("force_fields_input.F",1561),&
       ...
       IF (k < 0.0_dp) THEN
          CALL cp_abort(cp__l("force_fields_input.F",1568),&
...
! *****************************************************************************
  SUBROUTINE read_bonds_section(bond_kind,bond_a, bond_b, bond_k, bond_r0, bond_cs, section, start)
    ...
    DO isec = 1, n_items
       ...
       IF(.NOT.(SIZE(Kvals) <= 3))CALL cp__a("force_fields_input.F",1620)
...
! *****************************************************************************
  SUBROUTINE read_bends_section(bend_kind,bend_a, bend_b, bend_c, bend_k, bend_theta0, bend_cb, &
    ...
    REAL(KIND=dp), DIMENSION(:), POINTER     :: bend_k, bend_theta0, bend_cb, &
                                                ...
                                                bend_kss
    TYPE(legendre_data_type), DIMENSION(:), &
    ...
    DO isec = 1, n_items
       ...
       CALL section_vals_val_get(section,"K",i_rep_section=isec,r_vals=Kvals)
       IF(.NOT.(SIZE(Kvals) == 1))CALL cp__a("force_fields_input.F",1685)
  ...
  SUBROUTINE read_ubs_section(ub_kind,ub_a, ub_b, ub_c, ub_k, ub_r0, section, start)
    ...
    DO isec = 1, n_items
       ...
       IF (explicit) THEN
          ...
          IF(.NOT.(SIZE(Kvals) <= 3))CALL cp__a("force_fields_input.F",1753)
    ...
    DO isec = 1, n_items
       ...
       IF (torsion_kind(start+isec) .EQ. do_ff_opls) THEN
          IF (torsion_phi0(start+isec) .NE. 0.0_dp) THEN
             CALL cp_warn(cp__l("force_fields_input.F",1814),"PHI0 parameter was non-zero "//&
