# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_files.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_files.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_files.F" 2
...
! *****************************************************************************
  SUBROUTINE assign_preconnection(file_name,unit_number)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'assign_preconnection', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: ic, islot, nc

    IF ((unit_number < 1).OR.(unit_number > max_unit_number)) THEN
       CALL cp__b("common/cp_files.F",67,"An invalid logical unit number was specified.")
    END IF

    IF (LEN_TRIM(file_name) == 0) THEN
       CALL cp__b("common/cp_files.F",71,"No valid file name was specified")
    END IF
    ...
    DO ic=1,nc
       IF (TRIM(preconnected(ic)%file_name) == TRIM(file_name)) THEN
          ...
          ELSE
             ...
             CALL cp_abort(cp__l("common/cp_files.F",84),&
    ...
    IF (islot == -1) THEN
       CALL print_preconnection_list()
       CALL cp__b("common/cp_files.F",102,"No free slot found in the list of preconnected units")
    END IF
    ...
    IF (is_open) THEN
       ...
       IF (istat /= 0) THEN
          WRITE (UNIT=message,FMT="(A,I0,A,I0,A)")&
            "An error occurred inquiring the unit with the number ",unit_number,&
            " (IOSTAT = ",istat,")"
          CALL cp__b("common/cp_files.F",177,TRIM(message))
       END IF
       ...
       ELSE
          ...
          IF (istat /= 0) THEN
             ...
             CALL cp__b("common/cp_files.F",189,TRIM(message))
          END IF
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE delete_preconnection(file_name,unit_number)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'delete_preconnection', &
      routineP = moduleN//':'//routineN
    ...
    DO ic=1,nc
       IF (TRIM(preconnected(ic)%file_name) == TRIM(file_name)) THEN
          ...
          ELSE
             CALL print_preconnection_list()
             CALL cp_abort(cp__l("common/cp_files.F",225),&
...
! *****************************************************************************
  SUBROUTINE init_preconnection_list()
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'init_preconnection_list', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE open_file(file_name,file_status,file_form,file_action,&
    ...
    CHARACTER(LEN=11) :: access_string, action_string, current_action, &
      current_form, form_string, pad_string, position_string, status_string
    CHARACTER(LEN=2*default_path_length)     :: message
    CHARACTER(LEN=default_path_length)       :: cwd, iomsgstr, real_file_name
    INTEGER                                  :: debug_unit, istat
    LOGICAL                                  :: exists, get_a_new_unit, &
                                                is_open
    ...
    IF (PRESENT(file_pad)) THEN
       ...
       IF (form_string=="UNFORMATTED") THEN
          ...
          CALL cp__b("common/cp_files.F",356,TRIM(message))
       END IF
    ELSE
       pad_string = "YES"
    END IF
    ...
    IF (istat /= 0) THEN
       WRITE (UNIT=message,FMT="(A,I0,A)")&
         ...
         "> (IOSTAT = ",istat,")"
       CALL cp__b("common/cp_files.F",391,TRIM(message))
    ELSE IF (status_string == "OLD") THEN
       IF (.NOT.exists) THEN
          ...
          CALL cp__b("common/cp_files.F",398,TRIM(message))
       END IF
    END IF
    ...
    IF (is_open) THEN
       ...
       IF (TRIM(status_string) == "NEW") THEN
          CALL cp_abort(cp__l("common/cp_files.F",408),&
       ...
       IF (TRIM(current_form) /= TRIM(form_string)) THEN
          CALL cp_abort(cp__l("common/cp_files.F",413),&
       ...
       IF (TRIM(current_action) /= TRIM(action_string)) THEN
          CALL cp_abort(cp__l("common/cp_files.F",419),&
    ...
    ELSE
       ...
       IF (unit_number < 1) THEN
          ...
          CALL cp__b("common/cp_files.F",436,TRIM(message))
       END IF
       ...
       IF (istat /= 0) THEN
          ...
          CALL cp__b("common/cp_files.F",467,TRIM(message))
       END IF
    END IF
...
! *****************************************************************************
  FUNCTION get_data_dir() RESULT(data_dir_path)
    ...
    data_dir_path = "/work/e05/e05/fivanovic/flavoured-cptk-X-SH/cp2k/data"
...
! *****************************************************************************
  SUBROUTINE print_preconnection_list(output_unit)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'print_preconnection_list', &
      routineP = moduleN//':'//routineN
