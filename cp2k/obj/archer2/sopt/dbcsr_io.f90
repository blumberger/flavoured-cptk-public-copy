# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_io.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_io.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_io
  ...
  USE dbcsr_methods,                   ONLY: &
       dbcsr_data_area, dbcsr_distribution, dbcsr_get_data_size, &
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_io.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_print(matrix, nodata, matlab_format, variable_name, unit_nr)
    ...
    COMPLEX(KIND=real_8), DIMENSION(:), &
      POINTER                                :: c_dp
    INTEGER                                  :: ablk_p, bc, blk, blk_p, br, &
                                                ebr, fblk, handle, ibr, &
                                                iunit, lblk, m, mn, n, sblk
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.dbcsr_valid_index(matrix))&
       CALL cp__w("dbcsr/ops/dbcsr_io.F",126,"Can not print invalid matrix.")
    ...
    WRITE(iunit,'(1X,A,3(1X,I9,1X,A))')routineP, matrix%m%nblks, "blocks", &
         ...
         dbcsr_data_get_size_referenced (matrix%m%data_area), "used"
    ...
    IF (matrix%m%nblks .GT. 0) THEN
       IF (matrix%m%list_indexing) THEN
          CALL dbcsr_assert(SIZE(matrix%m%coo_l).EQ.3*matrix%m%nblks,&
          ...
          sblk = 3
       ELSE
          ebr = matrix%m%nblkrows_total
          sblk = 1
       ENDIF
       DO ibr = 1, ebr
          IF (matrix%m%list_indexing) THEN
             fblk = 1
             lblk = SIZE(matrix%m%coo_l)
          ELSE
             br = ibr
             fblk = matrix%m%row_p(br)+1
             lblk = matrix%m%row_p(br+1)
             m = row_blk_size(br)
          ENDIF
          DO blk = fblk, lblk, sblk
             IF (matrix%m%list_indexing) THEN
                br = matrix%m%coo_l(blk)
                bc = matrix%m%coo_l(blk+1)
                IF (matrix%m%local_indexing) THEN
                   br = local_rows(br)
                   bc = local_cols(bc)
                ENDIF
                m = row_blk_size(br)
                ablk_p = matrix%m%coo_l(blk+2)
             ELSE
                bc = matrix%m%col_i(blk)
                ablk_p = matrix%m%blk_p(blk)
             ENDIF
             n = col_blk_size(bc)
             mn = m*n
             blk_p = ABS(ablk_p)
  ...
  SUBROUTINE dbcsr_binary_write(matrix, filepath)
   ...
   CALL timeset(routineN, handle)
    ...
    IF(default_string_length/=80)&
       CALL cp_warn(cp__l("dbcsr/ops/dbcsr_io.F",750), "Changing the default string length affects "//&
    ...
    data_area_size = dbcsr_data_get_size_referenced(matrix%m%data_area)
...
! read version, matrix name and matrix type
    ...
    IF(version/=version_v_1_0)&
       CALL cp__w("dbcsr/ops/dbcsr_io.F",958,"Trying to read an unknown version of the matrix data file. Good luck!")
...
! compute the offsets in sub-header2 and read 2 integers nblocks, data_area_size
    ...
    DO i = 1, share_size
       ...
       CALL dbcsr_assert (data_area_size < HUGE(val_data(2,i)),&
            dbcsr_failure_level, dbcsr_caller_error,&
            routineN, "Data area too large, fix code.",1004)
...
! *****************************************************************************
  SUBROUTINE dbcsr_print_block_sum(matrix, unit_nr)
    ...
    COMPLEX(KIND=real_8), DIMENSION(:), &
      POINTER                                :: c_dp
    INTEGER                                  :: bc, blk, blk_p, br, handle, &
                                                iunit, m, mn, n
    INTEGER, DIMENSION(:), POINTER           :: col_blk_offset, col_blk_size, &
                                                row_blk_offset, row_blk_size
...
!   ---------------------------------------------------------------------------
    ...
    CALL timeset(routineN, handle)
    IF(.NOT.dbcsr_valid_index(matrix))&
       CALL cp__w("dbcsr/ops/dbcsr_io.F",1248,"Can not print invalid matrix.")
