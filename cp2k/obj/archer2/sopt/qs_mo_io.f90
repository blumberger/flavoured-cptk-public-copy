# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_io.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_io.F"
!-----------------------------------------------------------------------------!
...
# 81 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_mo_io.F" 2
       ...
       IF(mo_array(1)%mo_set%use_mo_coeff_b) THEN
          ...
          DO ispin=1,SIZE(mo_array)
             IF(.not.ASSOCIATED(mo_array(ispin)%mo_set%mo_coeff_b)) THEN
                IF(.NOT.(.FALSE.))CALL cp__a("qs_mo_io.F",143)
             ENDIF
  ...
  SUBROUTINE write_dm_binary_restart(mo_array, dft_section, tmpl_matrix)
    ...
    IF (do_dm_restart) THEN
       ...
       DO ispin = 1, SIZE(mo_array)
          ...
          IF (.NOT. ASSOCIATED(mo_array(ispin)%mo_set%mo_coeff_b)) CALL cp__b("qs_mo_io.F",215,"mo_coeff_b NOT ASSOCIATED")
...
! *****************************************************************************
  SUBROUTINE write_mo_set_low(mo_array, qs_kind_set, particle_set, ires, rt_mos)
    ...
    INTEGER :: handle, iatom, ikind, imat, iset, ishell, ispin, lmax, lshell, &
    ...
    IF (ires>0) THEN
       ...
       DO iatom=1,natom
          ...
          ELSE
          CALL cp__b("qs_mo_io.F",355,"Unknown basis type. ")
          END IF
       END DO
       ...
       DO iatom=1,natom
          ...
          ELSEIF (ASSOCIATED(dftb_parameter)) THEN
             ...
             END DO
          ELSE
          CALL cp__b("qs_mo_io.F",395,"Unknown basis set type. ")
          END IF
       END DO
...
! *****************************************************************************
  SUBROUTINE read_mos_restart_low (mos,  para_env, qs_kind_set, particle_set, natom, rst_unit, &
    ...
    INTEGER :: group, homo, homo_read, i, iatom, ikind, imat, irow, iset, &
      iset_read, ishell, ishell_read, iso, ispin, lfomo_read, lmax, lshell, &
    ...
    IF (para_env%ionode) THEN
       ...
       IF(PRESENT(rt_mos)) THEN
          IF (nspin_read /= nspin) THEN
            CALL cp__b("qs_mo_io.F",728,"To change nspin is not possible. ")
         END IF
      ELSE
         ...
         IF (nspin_read > nspin) THEN
            CALL cp__b("qs_mo_io.F",738,"Reducing nspin is not possible. ")
         ENDIF
      END IF
      ...
      IF (natom_match) THEN ! actually do the read read
         ...
         IF (nao_read /= nao) THEN
            ...
            IF(PRESENT(rt_mos))&
                 CALL cp__b("qs_mo_io.F",756,"To change basis is not possible. ")
         END IF
   ...
   IF (.NOT. natom_match) THEN
      IF (PRESENT(natom_mismatch)) THEN
         ...
         RETURN
      ELSE
         CALL cp__b("qs_mo_io.F",788,"Incorrect number of atoms in restart file. ")
      ENDIF
   ENDIF
   ...
   DO ispin=1,nspin
      ...
      IF (para_env%ionode.AND.(nmo > 0)) THEN
         ...
         IF(nmo_read < nmo)&
            CALL cp_warn(cp__l("qs_mo_io.F",812),&
        ...
        IF(nmo_read>nmo)&
           CALL cp_warn(cp__l("qs_mo_io.F",816),&
        ...
        IF (homo_read > nmo) THEN
          IF(nelectron_read==mos(ispin)%mo_set%nelectron) THEN
             CALL cp_warn(cp__l("qs_mo_io.F",829),&
          ...
          ELSE
              ...
              CALL cp__b("qs_mo_io.F",836,"Number of occupied MOs on restart unit larger than allocated MOs. ")
          END IF
        END IF
      END IF
            ...
            IF (para_env%ionode) THEN
               ...
               DO iatom=1,natom
                  ...
                  ELSEIF (ASSOCIATED(dftb_parameter)) THEN
                     ...
                     minbas=.TRUE.
                  ELSE
                     CALL cp__b("qs_mo_io.F",883,"Unknown basis set type. ")
                  END IF
      ...
      IF(.NOT.PRESENT(rt_mos)) THEN
        IF(ispin==1 .AND. nspin_read<nspin) THEN
          ...
          IF (my_mult.NE.1) THEN
             CALL cp__b("qs_mo_io.F",959,"Restarting an LSD calculation from an LDA wfn only works for multiplicity=1 (singlets).")
          END IF
          IF(mos(ispin+1)%mo_set%nelectron < 0)THEN
             CALL cp__b("qs_mo_io.F",962,"LSD: too few electrons for this multiplisity. ")
          END IF
...
! *****************************************************************************
  SUBROUTINE write_mo_set_to_output_unit(mo_set,atomic_kind_set,qs_kind_set,particle_set,&
    ...
    INTEGER :: after, first_mo, from, iatom, icgf, ico, icol, ikind, imo, &
      irow, iset, isgf, ishell, iso, iw, jcol, last_mo, left, lmax, lshell, &
          ...
          IF (p_cart) THEN
             ...
             DO iatom=1,natom
                ...
                ELSE IF (ASSOCIATED(dftb_parameter)) THEN
                   ...
                   END DO
                ELSE
                   CALL cp__b("qs_mo_io.F",1175,"Unknown basis set type. ")
                END IF
             END DO ! iatom
             ...
             DO iatom=1,natom
                ...
                IF (p_cart) THEN
                   ...
                   ELSE IF (ASSOCIATED(dftb_parameter)) THEN
                      ...
                      END DO
                   ELSE
                      CALL cp__b("qs_mo_io.F",1292,"Unknown basis set type. ")
                   END IF
                ...
                ELSE
                   ...
                   ELSE IF (ASSOCIATED(dftb_parameter)) THEN
                      ...
                      END DO
                   ELSE
                      CALL cp__b("qs_mo_io.F",1332,"Unknown basis type. ")
                   END IF
