# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_lyp.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_lyp.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_lyp.F" 2
...
SUBROUTINE lyp_lda_eval(rho_set,deriv_set,grad_deriv,lyp_params)
  ...
  IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_lyp.F",148)
  IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_lyp.F",149)
  IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_lyp.F",150)
  IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_lyp.F",151)
...
!FM        call xc_derivative_get(deriv,deriv_data=e_ndrho_ndrho_ndrho)
  END IF
  IF (grad_deriv>3.OR.grad_deriv<-3) THEN
     CALL cp__b("xc/xc_lyp.F",209,"derivatives bigger than 3 not implemented")
  END IF
...
SUBROUTINE lyp_lsd_eval(rho_set,deriv_set,grad_deriv,lyp_params)
  ...
  IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_lyp.F",762)
  IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_lyp.F",763)
  IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_lyp.F",764)
  IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_lyp.F",765)
