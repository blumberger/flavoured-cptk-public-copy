# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph_methods.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph_methods.F" 2
    ...
    DO i = 1,ngrid(idim)
       ...
       ELSE
          IF (PRESENT(array)) THEN
             ...
             END DO
             IF (ind >np) CALL cp__b("metadyn_tools/graph_methods.F",166,"something wrong in indexing ..")
          END IF
...
! *****************************************************************************
  SUBROUTINE fes_min(fes, ndim, iperd, ngrid, dp_grid, x0, ndw)
    ...
    IF (ndw/=ndim) CALL cp__b("metadyn_tools/graph_methods.F",280,"Not implemented for projected FES!")
...
! *****************************************************************************
  SUBROUTINE fes_path(fes, ndim, ngrid, dp_grid, iperd, x0, ndw, mep_input_data, l_int)
    ...
    IF (ndw/=ndim) CALL cp__b("metadyn_tools/graph_methods.F",408,"Not implemented for projected FES!")
...
! *****************************************************************************
  SUBROUTINE fes_cube_write(idim, fes,  pos,  ndim, ngrid, dp_grid, x0, ndw, l_fes_int, file)
    ...
    INTEGER                                  :: i, id(3), ii, iix, iiy, iiz, &
                                                ix, iy, iz, natoms, np
    INTEGER, DIMENSION(:), POINTER           :: izat
    REAL(KIND=dp)                            :: cell(3,3), delta(3), dr(3), &
                                                residual, rt(3)
    ...
    IF (ndw .GT. 3) THEN
       ...
       CALL cp__b("metadyn_tools/graph_methods.F",611,"")
    END IF
    ...
    IF (residual .GT. 1.0d-6) THEN
       WRITE(*,*)
       WRITE(*,*) 'ERROR: this progam can only handle orthogonal cells'
       WRITE(*,*) ' with vectors pointing in the X, Y and Z directions'
       CALL cp__b("metadyn_tools/graph_methods.F",638,"")
    END IF
    ...
    DO iz=1,ngrid(3)
       DO iy=1,ngrid(2)
          DO ix=1,ngrid(1)
             ...
             IF (iix .LT. 1) CALL cp__b("metadyn_tools/graph_methods.F",675,"ix < 0")
             IF (iiy .LT. 1) CALL cp__b("metadyn_tools/graph_methods.F",676,"iy < 0")
             IF (iiz .LT. 1) CALL cp__b("metadyn_tools/graph_methods.F",677,"iz < 0")
             IF (iix .GT. ngrid(1)) CALL cp__b("metadyn_tools/graph_methods.F",678,"ix > cell")
             IF (iiy .GT. ngrid(2)) CALL cp__b("metadyn_tools/graph_methods.F",679,"iy > cell")
             IF (iiz .GT. ngrid(3)) CALL cp__b("metadyn_tools/graph_methods.F",680,"iz > cell")
    ...
    DO WHILE (.TRUE.)
       ...
       IF (i == nelem+1) THEN
          ...
          CALL cp__b("metadyn_tools/graph_methods.F",710,"Element not recognized!")
       END IF
       izat(natoms)=i
    END DO
