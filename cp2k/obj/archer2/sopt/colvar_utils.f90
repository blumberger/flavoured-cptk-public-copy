# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_utils.F"
!-----------------------------------------------------------------------------!
...
MODULE colvar_utils
  ...
  USE colvar_types,                    ONLY: &
       colvar_counters, colvar_setup, colvar_type, coord_colvar_id, &
       distance_from_path_colvar_id, gyration_colvar_id, mindist_colvar_id, &
       population_colvar_id, reaction_path_colvar_id, rmsd_colvar_id
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE eval_colvar ( force_env, coords, cvalues, Bmatrix, MassI, Amatrix)
    ...
    IF (PRESENT(Bmatrix)) THEN
       ...
       IF(.NOT.(check))CALL cp__a("colvar_utils.F",235)
       Bmatrix = 0.0_dp
    END IF
    CALL timeset(routineN,handle)
    ...
    IF (ASSOCIATED(gci)) THEN
       ...
       ENDIF
    END IF
    IF(.NOT.(n_tot==SIZE(cvalues)))CALL cp__a("colvar_utils.F",285)
    ...
    IF (PRESENT(Bmatrix)) THEN
       ...
       IF(.NOT.(check))CALL cp__a("colvar_utils.F",298)
    ...
    IF (PRESENT(Bmatrix).AND.PRESENT(Amatrix)) THEN
       IF(.NOT.(ASSOCIATED(Amatrix)))CALL cp__a("colvar_utils.F",311)
       check = SIZE(Bmatrix,1)==SIZE(Amatrix,2)
       IF(.NOT.(check))CALL cp__a("colvar_utils.F",313)
       check = SIZE(Bmatrix,2)==SIZE(Amatrix,1)
       IF(.NOT.(check))CALL cp__a("colvar_utils.F",315)
       ...
       IF(ABS(inv_error)>1.0E-8_dp)&
          CALL cp__w("colvar_utils.F",321,"Error in inverting the Gmatrix larger than 1.0E-8!")
    ...
    IF (PRESENT(MassI)) THEN
       ...
       IF(.NOT.(ASSOCIATED(MassI)))CALL cp__a("colvar_utils.F",328)
       IF(.NOT.(SIZE(MassI)==natom*3))CALL cp__a("colvar_utils.F",329)
       DO i = 1, natom
    ...
    IF (colvar%type_id == coord_colvar_id) THEN
       IF (colvar%coord_param%use_kinds_from.OR.colvar%coord_param%use_kinds_to) THEN
          ...
          IF (colvar%coord_param%use_kinds_from) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",632)
          END IF
          ! Atoms to
          IF (colvar%coord_param%use_kinds_to) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",650)
          END IF
          ! Atoms to b
          IF (colvar%coord_param%use_kinds_to_b) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",668)
          END IF
    ...
    IF (colvar%type_id == mindist_colvar_id) THEN
       IF (colvar%mindist_param%use_kinds_from.OR.colvar%mindist_param%use_kinds_to) THEN
          ...
          IF (colvar%mindist_param%use_kinds_from) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",694)
          END IF
          ! Atoms to
          IF (colvar%mindist_param%use_kinds_to) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",712)
          END IF
          ! Setup the colvar
          CALL colvar_setup(colvar)
       END IF
    ...
    IF (colvar%type_id == population_colvar_id) THEN
       ...
       IF (colvar%population_param%use_kinds_from.OR.colvar%population_param%use_kinds_to) THEN
          ...
          IF (colvar%population_param%use_kinds_from) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",738)
          END IF
          ! Atoms to
          IF (colvar%population_param%use_kinds_to) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",756)
          END IF
          ! Setup the colvar
          CALL colvar_setup(colvar)
       END IF
    ...
    IF (colvar%type_id == gyration_colvar_id) THEN
       ...
       IF (colvar%gyration_param%use_kinds) THEN
          ...
          IF (colvar%gyration_param%use_kinds) THEN
             ...
             IF(.NOT.(stat/=0))CALL cp__a("colvar_utils.F",783)
          END IF
          ! Setup the colvar
          CALL colvar_setup(colvar)
       END IF
