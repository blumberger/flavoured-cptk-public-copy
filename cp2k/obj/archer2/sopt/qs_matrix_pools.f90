# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_matrix_pools.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_matrix_pools.F"
!-----------------------------------------------------------------------------!
...
# 31 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_matrix_pools.F" 2
...
SUBROUTINE mpools_retain(mpools)
  ...
  IF(.NOT.(ASSOCIATED(mpools)))CALL cp__a("qs_matrix_pools.F",91)
  IF(.NOT.(mpools%ref_count>0))CALL cp__a("qs_matrix_pools.F",92)
...
SUBROUTINE mpools_release(mpools)
  ...
  IF (ASSOCIATED(mpools)) THEN
     IF(.NOT.(mpools%ref_count>0))CALL cp__a("qs_matrix_pools.F",110)
     mpools%ref_count=mpools%ref_count-1
...
SUBROUTINE mpools_rebuild_fm_pools(mpools, mos, blacs_env, para_env,&
  ...
  nrow_block=cp_fm_struct_get_nrow_block()
  ncol_block=cp_fm_struct_get_ncol_block()
  ...
  IF(.NOT.(ASSOCIATED(mos)))CALL cp__a("qs_matrix_pools.F",254)
  ...
  DO ispin=2,SIZE(mos)
     ...
     IF (max_nmo<nmo) THEN
        CALL cp__b("qs_matrix_pools.F",329,"the mo with the most orbitals must be the first ")
     END IF
