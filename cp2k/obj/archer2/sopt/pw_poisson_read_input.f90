# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_poisson_read_input.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_poisson_read_input.F"
!-----------------------------------------------------------------------------!
...
# 38 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_poisson_read_input.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_poisson_read_parameters(poisson_section, params)
    ...
    SELECT CASE (params%solver)
      ...
      CASE default
         CALL cp__b("pw_poisson_read_input.F",84,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE decode_periodic_green(periodic, params)
    ...
    SELECT CASE(periodic)
      ...
      CASE DEFAULT
         CALL cp__b("pw_poisson_read_input.F",142,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE decode_periodic_wavelet(periodic, params)
    ...
    CASE(use_perd_x,use_perd_y,use_perd_z,use_perd_xy,use_perd_yz)
       CALL cp__b("pw_poisson_read_input.F",178,"Poisson solver for this periodicity not yet implemented")
    CASE DEFAULT
       CALL cp__b("pw_poisson_read_input.F",180,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dirichlet_bc_read_parameters(dbc_section, params)
    ...
    REAL(dp)                                 :: delta_alpha, intercept, &
                                                sigma, thickness, v_D, &
                                                xaa_cylindrical_brad
    REAL(dp), DIMENSION(:), POINTER :: aa_cuboidal_xxtnt, aa_cuboidal_yxtnt, &
