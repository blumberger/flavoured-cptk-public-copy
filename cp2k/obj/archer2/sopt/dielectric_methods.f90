# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dielectric_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dielectric_methods.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/dielectric_methods.F" 2
...
!> \author Mohammad Hossein Bani-Hashemian
! *****************************************************************************
  SUBROUTINE dielectric_compute_periodic(dielectric, diel_rs_grid, pw_pool, rho, rho_core)
    ...
    IF (.NOT. dielec_functiontype .EQ. rho_dependent .AND. &
       ...
       CALL cp_abort(cp__l("pw/dielectric_methods.F",154),&
    ...
    ELSE 
      CALL cp__b("pw/dielectric_methods.F",178,"For dielectric constant larger than 1, rho_core has to be present.")
    END IF
...
! derivatives
  ...
  END SUBROUTINE dielectric_compute_periodic
...
!> \author Mohammad Hossein Bani-Hashemian
! *****************************************************************************
  SUBROUTINE dielectric_compute_neumann(dielectric, diel_rs_grid, pw_pool_orig, &
                        dct_pw_grid, recv_msgs_bnds, dests_expand, srcs_expand, &
                        flipg_stat, bounds_shftd, rho, rho_core)
    ...
    IF (.NOT. dielec_functiontype .EQ. rho_dependent .AND. &
        ((derivative_method .EQ. derivative_fft_use_deps) .OR. &
         (derivative_method .EQ. derivative_fft_use_deps))) THEN
       CALL cp_abort(cp__l("pw/dielectric_methods.F",345),&
            "The specified derivative method is not compatible with the type of "//&
            "the dielectric constant function.")
    END IF
    ...
    ELSE 
      CALL cp__b("pw/dielectric_methods.F",381,"For dielectric constant larger than 1, rho_core has to be present.")
    END IF
...
! derivatives
  ...
  END SUBROUTINE dielectric_compute_neumann
...
! *****************************************************************************
  SUBROUTINE dielectric_constant_sccs(rho, eps, deps_drho, eps0, rho_max, rho_min)
    ...
    IF (eps0 .LT. 1.0_dp) THEN
      CALL cp__b("pw/dielectric_methods.F",516,"The dielectric constant has to be greater than or equal to 1.")
    END IF
...
! *****************************************************************************
  SUBROUTINE dielectric_constant_aa_cuboidal(eps, dielec_const, pw_pool, zeta, &
    ...
    IF (dielec_const .LT. 1.0_dp) THEN
       CALL cp__b("pw/dielectric_methods.F",599,"The dielectric constant has to be greater than or equal to 1.")
    END IF
    ...
    IF (n_forb_xtnts .GT. 0) THEN
       CALL cp_abort(cp__l("pw/dielectric_methods.F",615),&
...
! *****************************************************************************
  SUBROUTINE dielectric_constant_xaa_annular(eps, dielec_const, pw_pool, zeta, &
    ...
    IF (dielec_const .LT. 1.0_dp) THEN
       CALL cp__b("pw/dielectric_methods.F",701,"The dielectric constant has to be greater than or equal to 1.")
    END IF
    ...
    n_forb_xtnts = COUNT((/forb_xtnt1, forb_xtnt2, forb_xtnt3, &
                           forb_xtnt4, forb_xtnt5, forb_xtnt6/) .EQV. test_forb_xtnts)
    IF (n_forb_xtnts .GT. 0) THEN
       CALL cp_abort(cp__l("pw/dielectric_methods.F",718),&
            "The given extents for the dielectric region are outside the range of "//&
            "the simulation cell.")
    END IF
...
! *****************************************************************************
  SUBROUTINE dielectric_constant_spatially_dependent(eps, pw_pool, dielectric_params)
    ...
    IF (n_dielectric_region .EQ. 0) THEN
       CALL cp__b("pw/dielectric_methods.F",790,"No density independent dielectric region is defined.")
    END IF
...
! *****************************************************************************
  SUBROUTINE dielectric_constant_spatially_rho_dependent(rho, eps, deps_drho, &
    ...
    IF (dielectric_params%eps0 .LT. 1.0_dp) THEN
      CALL cp__b("pw/dielectric_methods.F",853,"The dielectric constant has to be greater than or equal to 1.")
    END IF
