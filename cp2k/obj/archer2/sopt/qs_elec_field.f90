# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_elec_field.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_elec_field.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_elec_field.F" 2
...
! *****************************************************************************
  ...
  SUBROUTINE build_efg_matrix(qs_env,matrix_efg,rc)
    ...
    INTEGER :: after, handle, i, iatom, icol, ikind, inode, irow, iset, &
      istat, iw, jatom, jkind, jset, last_jatom, ldai, maxco, maxlgto, &
      maxsgf, natom, ncoa, ncob, neighbor_list_id, nkind, nseta, nsetb, sgfa, &
      sgfb
       ...
       DO iset=1,nseta
          ...
          DO jset=1,nsetb
             ...
             CALL efg(la_max(iset),la_min(iset),npgfa(iset),&
                      ...
                      rac,rbc,rab,efgab,SIZE(rr_work,1),SIZE(rr_work,2),rr_work)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,&
              qs_env%input,"DFT%PRINT%AO_MATRICES/EFG"),cp_p_file)) THEN
       ...
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(1)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(2)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(3)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(4)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(5)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
       CALL cp_dbcsr_write_sparse_matrix(matrix_efg(6)%matrix,4,after,qs_env,&
           para_env,output_unit=iw,omit_headers=omit_headers)
