# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_csr_conversions.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_csr_conversions.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_csr_conversions
  ...
  USE dbcsr_methods,                   ONLY: &
       ...
       dbcsr_nblkrows_total, dbcsr_nfullrows_local, dbcsr_nfullrows_total, &
       dbcsr_release, dbcsr_row_block_sizes, dbcsr_valid_index
...
# 75 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_csr_conversions.F" 2
    ...
    SELECT CASE (csr_mat%nzval_local%data_type)
      ...
      CASE DEFAULT
        CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,&
                          routineN, "Invalid matrix type",265)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE csr_create_template(matrix_b, matrix_a)
    ...
    CALL dbcsr_assert(matrix_a%valid,dbcsr_fatal_level,dbcsr_wrong_args_error,routineN,&
      "Source CSR matrix must be created.",300)
...
! *****************************************************************************
  SUBROUTINE csr_copy(matrix_b, matrix_a)
    ...
    CALL dbcsr_assert(matrix_b%nzval_local%data_type, "EQ",&
      ...
      routineN,"Matrices don't have the same datatype.",377)

    CALL dbcsr_assert(matrix_b%nze_local, "EQ",&
      matrix_a%nze_local,dbcsr_fatal_level,dbcsr_wrong_args_error,routineN,&
      "Matrices don't have the same number of local non-zero elements.",381)

    CALL dbcsr_assert(matrix_b%nrows_local, "EQ", matrix_a%nrows_local,&
      dbcsr_fatal_level,dbcsr_wrong_args_error,routineN,&
      "Matrices don't have the same number of local rows.",385)
...
! *****************************************************************************
  SUBROUTINE csr_create_nzerow(csr_mat, nzerow)
    ...
    CALL dbcsr_assert(csr_mat%valid,dbcsr_fatal_level,dbcsr_wrong_args_error,routineN,&
      "CSR matrix must be created.",430)
...
! *****************************************************************************
  SUBROUTINE csr_destroy(csr_mat)
    ...
    CALL dbcsr_assert(csr_mat%valid,dbcsr_fatal_level,dbcsr_wrong_args_error,routineN,& 
      "CSR matrix must be created before destroying it.",458)
...
! *****************************************************************************
  SUBROUTINE csr_create_from_brd( brd_mat, csr_mat, csr_sparsity_brd) 
    ...
    TYPE(csr_type), INTENT(OUT)              :: csr_mat
    TYPE(dbcsr_obj), INTENT(IN)              :: csr_sparsity_brd
    ...
    CALL dbcsr_assert(ALL(cdist .EQ. 0), dbcsr_fatal_level,dbcsr_wrong_args_error,&
      routineN, "DBCSR matrix not block-row distributed.",535)
...
! *****************************************************************************
  SUBROUTINE csr_get_dbcsr_mapping (brd_mat, dbcsr_index, csr_index, csr_nze_local, &
    ...
    INTEGER :: blk, blkcol, blkrow, col_blk_size, csr_ind, data_type, &
      dbcsr_ind, el_sum, fullcol_sum_blkrow, handle, l, m, n, nblkrows_total, &
      nze, prev_blk, prev_blkrow, prev_row_blk_size, row_blk_offset, &
      row_blk_size
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
      ...
      CALL dbcsr_assert(.NOT. tr,dbcsr_fatal_level, dbcsr_wrong_args_error,routineN,&
        "DBCSR block data must not be transposed",624)
    ENDDO
    ...
    SELECT CASE (data_type)
      CASE (dbcsr_type_real_4)
        csr_nze(:) = INT(csr_sparsity_brd%m%data_area%d%r_sp(1:nze))
      CASE (dbcsr_type_real_8)
        csr_nze(:) = INT(csr_sparsity_brd%m%data_area%d%r_dp(1:nze))
      CASE DEFAULT
        CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,routineN, &
                           "CSR sparsity matrix must have a real datatype",&
                           676)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE convert_csr_to_brd (brd_mat, csr_mat)
    ...
    SELECT CASE (data_type)
      CASE (dbcsr_type_real_4)
        brd_mat%m%data_area%d%r_sp(1:nze) = 0.0_sp
      CASE (dbcsr_type_real_8)
        brd_mat%m%data_area%d%r_dp(1:nze) = 0.0_dp
      CASE (dbcsr_type_complex_4)
        brd_mat%m%data_area%d%c_sp(1:nze) = 0.0_sp
      CASE (dbcsr_type_complex_8)
        brd_mat%m%data_area%d%c_dp(1:nze) = 0.0_dp
    END SELECT
...
! *****************************************************************************
  SUBROUTINE convert_brd_to_csr (brd_mat, csr_mat)
    ...
    INTEGER :: blk, blkcol, blkrow, col_blk_offset, col_blk_size, csr_ind, &
      data_type, dbcsr_ind, el_sum, handle, ind_blk_data, k, local_row_ind, &
      m, n, nblkrows_total, node_row_offset, prev_blkrow, prev_row_blk_size, &
    ...
    CALL  dbcsr_assert(csr_mat%has_mapping,dbcsr_fatal_level, dbcsr_wrong_args_error,routineN,&
        "DBCSR mapping of CSR matrix must be defined",795)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))  
      ...
      CALL dbcsr_assert(.NOT. tr,dbcsr_fatal_level, dbcsr_wrong_args_error,routineN,&
        "DBCSR block data must not be transposed",842)
...
!>                            DBCSR matrix)
!> \param[in] start_cut_row ...
!> \param[in] end_cut_row ...
!> \par History
!>      10.2014 created [Patrick Seewald]
!> \author Patrick Seewald
! *****************************************************************************
  SUBROUTINE csr_create_from_dbcsr(dbcsr_mat, csr_mat, dist_format, csr_sparsity, &
                                   numnodes, start_cut_row, end_cut_row)
    TYPE(dbcsr_obj), INTENT(IN)              :: dbcsr_mat
    TYPE(csr_type), INTENT(OUT)              :: csr_mat
    INTEGER, INTENT(IN)                      :: dist_format
    TYPE(dbcsr_obj), INTENT(IN), OPTIONAL    :: csr_sparsity
    ...
    INTEGER :: dbcsr_numnodes, end_cutout, handle, nblkcols_total, &
      nblkrows_total, nblks_local, num_p, start_cutout
    LOGICAL                                  :: equal_dist, floor_dist
    TYPE(dbcsr_obj)                          :: brd_mat, csr_sparsity_brd, &
                                                csr_sparsity_nosym, &
                                                dbcsr_mat_nosym
    ...
    dbcsr_wrong_args_error, routineN, "Invalid DBCSR matrix", 954)
    ...
    CALL dbcsr_init(csr_sparsity_nosym)
    ...
    ELSE
      ...
      CALL dbcsr_set(csr_sparsity_nosym, 1.0_sp)
    ENDIF

    CALL dbcsr_assert(dbcsr_has_same_block_structure(dbcsr_mat_nosym, csr_sparsity_nosym), &
                      dbcsr_fatal_level, dbcsr_wrong_args_error, routineN, &
                      "csr_sparsity and dbcsr_mat have different sparsity pattern", &
                      996)
    ...
    IF (PRESENT(start_cut_row)) THEN
       start_cutout = start_cut_row
    ELSE
       start_cutout = 0 
    END IF

    IF (PRESENT(end_cut_row)) THEN
       end_cutout = dbcsr_nfullrows_total(dbcsr_mat_nosym) - end_cut_row
    ELSE
       end_cutout = dbcsr_nfullrows_total(dbcsr_mat_nosym) 
    END IF

    CALL dbcsr_create_brd (dbcsr_mat_nosym, brd_mat, equal_dist, floor_dist,&
                           num_p, start_cutout, end_cutout)
...
! *****************************************************************************
  SUBROUTINE convert_csr_to_dbcsr(dbcsr_mat, csr_mat)
    ...
    CALL dbcsr_assert(dbcsr_valid_index(dbcsr_mat), dbcsr_fatal_level,&
    dbcsr_wrong_args_error, routineN, "Invalid DBCSR matrix", 1221)
...
! *****************************************************************************
  SUBROUTINE dbcsr_to_csr_filter( dbcsr_mat, csr_sparsity, eps)
    TYPE(dbcsr_obj), INTENT(IN)              :: dbcsr_mat
    TYPE(dbcsr_obj), INTENT(OUT)             :: csr_sparsity
    REAL(kind=real_8), INTENT(IN)            :: eps

    INTEGER                                  :: blkcol, blkrow, col_blk_size, &
                                                data_type, row_blk_size
    LOGICAL                                  :: tr
    REAL(kind=real_8), ALLOCATABLE, &
      DIMENSION(:)                           :: block_abs, csr_sparsity_blk
    TYPE(dbcsr_data_obj)                     :: block
    TYPE(dbcsr_iterator)                     :: iter

!REAL(kind=real_8), DIMENSION(:), POINTER :: block

    CALL dbcsr_init(csr_sparsity)
...
! *****************************************************************************
  SUBROUTINE csr_conversion_test( dbcsr_mat, csr_mat, norm, eps)
    ...
    INTEGER                                  :: handle
    TYPE(dbcsr_obj)                          :: csr_sparsity, dbcsr_mat_conv

    CALL timeset(routineN, handle)

    CALL dbcsr_to_csr_filter(dbcsr_mat, csr_sparsity, eps)

    CALL csr_create_from_dbcsr(dbcsr_mat, csr_mat, csr_eqrow_ceil_dist, csr_sparsity)
    CALL convert_dbcsr_to_csr(dbcsr_mat, csr_mat)

    CALL dbcsr_init(dbcsr_mat_conv)
    CALL dbcsr_copy(dbcsr_mat_conv, dbcsr_mat)

    CALL convert_csr_to_dbcsr(dbcsr_mat_conv, csr_mat)
...
!> \author Patrick Seewald
! *****************************************************************************
  SUBROUTINE csr_write(csr_mat, unit_nr, upper_triangle)
    TYPE(csr_type), INTENT(IN)               :: csr_mat
...
! *****************************************************************************
  SUBROUTINE csr_print_sparsity(csr_mat, unit_nr)
    ...
    REAL(KIND=real_8)                        :: dbcsr_nze_percentage, &
                                                nze_percentage

    CALL timeset(routineN, handle)

    CALL dbcsr_assert(csr_mat%valid,&
      dbcsr_fatal_level, dbcsr_wrong_args_error,routineN,&
      "CSR matrix must be created first",&
      1564)
...
! *****************************************************************************
  SUBROUTINE dbcsr_create_brd (dbcsr_mat, brd_mat, equal_dist, floor_dist, numnodes, &
    ...
    INTEGER, INTENT(IN)                      :: numnodes, start_cut_row, &
                                                end_cut_row
    ...
    INTEGER :: chunk_size, data_type, handle, i, k, l, m, mp_group, mynode, &
      nblkcols_total, nblkrows_total, nfullrows_local, nfullrows_total, &
      node_size, numnodes_total, remainder, row_index, split_row
    ...
    IF (equal_dist) THEN ! Equally distribute rows over processors -> cut blocks
      ...
      IF (floor_dist) THEN
        nfullrows_local = FLOOR(REAL(end_cut_row-start_cut_row)/numnodes)
      ELSE 
        nfullrows_local = CEILING(REAL(end_cut_row-start_cut_row)/numnodes)
      ENDIF
      ...
      m=0 ! node counter
      node_size = nfullrows_local + start_cut_row ! space available on current node in number of rows
      IF (numnodes .EQ. 1) node_size = nfullrows_total
        ...
        DO l=1,nblkrows_total
          ...
          DO WHILE(split_row .GE. node_size) ! cut block row and send it to two nodes
            ...
            IF(floor_dist .AND. (m .EQ. numnodes - 1)) THEN ! send all remaining rows to last node
              node_size = nfullrows_total-(numnodes - 1) * node_size - start_cut_row
            ENDIF
            IF (numnodes .EQ. 1) node_size = nfullrows_total
          ENDDO
