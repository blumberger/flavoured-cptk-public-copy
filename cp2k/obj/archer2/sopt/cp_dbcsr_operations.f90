# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_dbcsr_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_dbcsr_operations.F"
!-----------------------------------------------------------------------------!
...
MODULE cp_dbcsr_operations
  ...
  USE cp_dbcsr_interface,              ONLY: &
       ...
       dbcsr_distribution_row_dist, dbcsr_mp_hold, dbcsr_mp_npcols, &
       dbcsr_mp_nprows, dbcsr_mp_obj, dbcsr_norm_frobenius, &
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_dbcsr_operations.F" 2
...
! *****************************************************************************
  SUBROUTINE match_col_sizes (sizes1, sizes2, full_num)
    ...
    n2 = SIZE(sizes2)
    IF(n1 .NE. n2)&
       CALL cp__b("cp_dbcsr_operations.F",476,"distributions must be equal!")
  ...
  SUBROUTINE cp_dbcsr_plus_fm_fm_t_native(sparse_matrix,matrix_v,matrix_g,ncol,&
    ...
    IF (ncol .GT. 0) THEN
       IF(.NOT.cp_dbcsr_valid_index (sparse_matrix))&
          CALL cp__b("cp_dbcsr_operations.F",544,"sparse_matrix must pre-exist")
       !
       ! Setup matrix_v
...
!   ---------------------------------------------------------------------------
    ...
    IF (PRESENT(beta)) THEN
       IF(beta .NE. my_beta)&
          CALL cp__b("cp_dbcsr_operations.F",712,"beta not supported, use matrix addition instead")
       my_beta=beta
    ENDIF
    ...
    IF(nrow_global.NE.nfullrows_total)&
       CALL cp__b("cp_dbcsr_operations.F",727,"FM and DBCSR matrix sizes do not match in rows")
    IF(ncol_global.NE.nfullcols_total)&
       CALL cp__b("cp_dbcsr_operations.F",729,"FM and DBCSR matrix sizes do not match in columns")
    ...
    bc_rows: DO row_l = 1, nblkrows_local
       ...
       bc_cols: DO col_l = 1, nblkcols_local
          ...
          CALL cp_add_work_coordinate(bc_mat, 1, row, col, blk_p)
          IF (fm%use_sp) THEN
          ...
          ENDIF
          IF(nze .NE. (last_row(row)-first_row(row)+1)*(last_col(col)-first_col(col)+1))&
             CALL cp__b("cp_dbcsr_operations.F",811,"Block size does not match block row/col sizes")
          IF (fm%use_sp) THEN
    ...
    IF (PRESENT (alpha)) THEN
       CALL cp__b("cp_dbcsr_operations.F",831,'no more alpha... clean me')
    ELSE
...
! *****************************************************************************
  SUBROUTINE copy_fm_to_dbcsr_bc(fm,bc_mat,beta)
    ...
    INTEGER :: blk_p, col, col_l, col_size, data_type, group, handle, my_blk, &
      mypcol, myproc, myprow, nblkcols_local, nblkcols_total, nblkrows_local, &
      nblkrows_total, ncol_block, ncol_global, nfullcols_local, &
      nfullcols_total, nfullrows_local, nfullrows_total, nrow_block, &
      nrow_global, numproc, nze, row, row_l, row_size
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: first_col, first_row, &
                                                last_col, last_row, &
                                                local_col_sizes, &
                                                local_row_sizes
...
!   ---------------------------------------------------------------------------
    ...
    IF (PRESENT(beta)) THEN
       IF(beta.NE.my_beta)&
          CALL cp__b("cp_dbcsr_operations.F",886,"beta not supported, use matrix addition instead")
       my_beta=beta
    ENDIF
    group = fm%matrix_struct%para_env%group
    context => fm%matrix_struct%context
    ...
    bc_rows: DO row_l = 1, nblkrows_local
       ...
       bc_cols: DO col_l = 1, nblkcols_local
          ...
          nze = row_size*col_size
          CALL cp_add_work_coordinate(bc_mat, 1, row, col, blk_map(my_blk))
...
!$OMP        last_row, first_row, last_col, first_col, fm_block, fm_block_sp)
    bc_rows1: DO row_l = 1, nblkrows_local
       ...
       bc_cols1: DO col_l = 1, nblkcols_local
          ...
          ENDIF
          IF(nze .NE. (last_row(row)-first_row(row)+1)*(last_col(col)-first_col(col)+1))&
             CALL cp__b("cp_dbcsr_operations.F",1005,"Block size does not match block row/col sizes")
          IF (fm%use_sp) THEN
             blk_1d_sp(1:nze) = RESHAPE(&
                  fm_block_sp(&
                     first_row(row):last_row(row),first_col(col):last_col(col)&
...
!   ---------------------------------------------------------------------------
    ...
    IF(nrow_global.NE.nfullrows_total)&
       CALL cp__b("cp_dbcsr_operations.F",1084,"FM and DBCSR matrix sizes do not match in rows")
    IF(ncol_global.NE.nfullcols_total)&
       CALL cp__b("cp_dbcsr_operations.F",1086,"FM and DBCSR matrix sizes do not match in columns")
    ...
    bc_rows: DO row_l = 1, nblkrows_local
       ...
       bc_cols: DO col_l = 1, nblkcols_local
          ...
          IF(nze .NE. (last_row(row)-first_row(row)+1)*(last_col(col)-first_col(col)+1))&
             CALL cp__b("cp_dbcsr_operations.F",1159,"Block size does not match block row/col sizes")
