# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_control.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_control.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_control.F" 2
...
! *****************************************************************************
  SUBROUTINE read_mc_restart ( mc_par, force_env, iw, mc_nunits_tot,rng_stream)
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,216,&
    ...
    IF(ABS(temperature - mc_temp) .GT. 0.01E0_dp) THEN
       ...
       ENDIF
       CALL cp__b("motion/mc/mc_control.F",245,"Temperature difference between restart and input")
    ENDIF
    ...
    IF(ensemble .NE. mc_ensemble) THEN
       ...
       ENDIF
       CALL cp__b("motion/mc/mc_control.F",263,"Ensembles different between restart and input")
    ENDIF
    ...
    IF(SUM(nchains(:)) == 0) THEN
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,293,&
            "r",3*nunits(1)*dp_size)
       ALLOCATE (atom_symbols(nunits(1)),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,296,&
            "ipart",nunits_tot*5)
    ...
    ELSE
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,311,&
            "r",3*nunits_tot*dp_size)
       ALLOCATE (atom_symbols(nunits_tot),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,314,&
            "ipart",nunits_tot*5)
...
! *****************************************************************************
  SUBROUTINE mc_create_force_env ( force_env, input_declaration, para_env, input_file_name,&
    ...
    IF(ierr/=0) CALL cp__b("motion/mc/mc_control.F",389,"mc_create_force_env: destroy_force_env failed")
