# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_localization_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_localization_methods.F"
!-----------------------------------------------------------------------------!
...
# 64 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_localization_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE jacobi_rotations_serial ( weights, zij, vectors, max_iter, eps_localization, sweeps, out_each)
    ...
    INTEGER                                  :: dim2, handle, idim, istate, &
                                                jstate, nstate, unit_nr
...
! *****************************************************************************
  SUBROUTINE crazy_rotations( weights, zij, vectors,max_iter, max_crazy_angle, crazy_scale, crazy_use_diag, &
    ...
    COMPLEX(KIND=dp), DIMENSION(:, :), &
      POINTER                                :: diag_z
    COMPLEX(KIND=dp), POINTER                :: mii( : ), mij( : ), mjj( : )
    INTEGER :: dim2, handle, i, icol, idim, irow, method, ncol_global, &
...
! *****************************************************************************
  SUBROUTINE direct_mini( weights, zij, vectors, max_iter ,eps_localization, iterations)
    ...
    INTEGER :: handle, i, icol, idim, irow, line_search_count, line_searches, &
      lsl, lsm, lsr, n, ncol_local, ndim, nrow_local, output_unit
    INTEGER, DIMENSION(:), POINTER           :: col_indices, row_indices
...
!    CALL cp_fm_init_random ( matrix_A )
    ...
    DO
         ...
         CASE(3) ! golden section hunt
            ...
            ELSE
              IF (line_search_count .EQ. 150) CALL cp__b("qs_localization_methods.F",1268,"Too many")
              IF (lsr.eq.0) THEN
