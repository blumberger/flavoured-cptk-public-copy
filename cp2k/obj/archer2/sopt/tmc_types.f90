# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_types.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_types.F" 2
...
! *****************************************************************************
  SUBROUTINE tmc_env_create(tmc_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",186)
...
! *****************************************************************************
  SUBROUTINE tmc_env_release(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",253)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_types.F",254)
...
! *****************************************************************************
  SUBROUTINE tmc_master_env_create(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",297)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_types.F",298)
    IF(.NOT.(tmc_env%params%nr_temp.GT.0))CALL cp__a("tmc/tmc_types.F",299)
    
    IF(.NOT.(.NOT.ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_types.F",301)
...
! *****************************************************************************
  SUBROUTINE tmc_master_env_release(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",357)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_types.F",358)
...
! *****************************************************************************
  SUBROUTINE tmc_worker_env_create(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",391)
    IF(.NOT.(.NOT.ASSOCIATED(tmc_env%w_env)))CALL cp__a("tmc/tmc_types.F",392)
...
! *****************************************************************************
  SUBROUTINE tmc_worker_env_release(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_types.F",420)
    IF(.NOT.(ASSOCIATED(tmc_env%w_env)))CALL cp__a("tmc/tmc_types.F",421)
...
! *****************************************************************************
  SUBROUTINE allocate_tmc_atom_type(atoms, nr_atoms)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'allocate_tmc_atom_type', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(.NOT.ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_types.F",443)
    IF(.NOT.(nr_atoms.GT.0))CALL cp__a("tmc/tmc_types.F",444)

    ALLOCATE(atoms(nr_atoms))
    atoms%name = ""
    atoms%mass = 0.0_dp

    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_types.F",450)
...
! *****************************************************************************
  SUBROUTINE deallocate_tmc_atom_type(atoms)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'deallocate_tmc_atom_type', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_types.F",466)

    DEALLOCATE(atoms)

    IF(.NOT.(.NOT.ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_types.F",470)
