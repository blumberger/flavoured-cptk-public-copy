# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_methods.F"
!-----------------------------------------------------------------------------!
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_poisson_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_poisson_cleanup (poisson_env)
    ...
    IF(.NOT.(ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_methods.F",97)
    IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_methods.F",98)
...
! *****************************************************************************
  SUBROUTINE pw_poisson_check(poisson_env)
    ...
    IF(.NOT.(ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_methods.F",124)
    IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_methods.F",125)
    IF(.NOT.(ASSOCIATED(poisson_env%pw_pools)))CALL cp__a("pw/pw_poisson_methods.F",126)
    IF(.NOT.(poisson_env%pw_level>=LBOUND(poisson_env%pw_pools,1)))CALL cp__a("pw/pw_poisson_methods.F",127)
    IF(.NOT.(poisson_env%pw_level<=UBOUND(poisson_env%pw_pools,1)))CALL cp__a("pw/pw_poisson_methods.F",128)
    ...
    IF (.NOT.rebuild) THEN
       ...
       CASE default
          CALL cp__b("pw/pw_poisson_methods.F",163,"")
       END SELECT
    END IF
    IF (rebuild) THEN
       poisson_env%rebuild=.TRUE.
...
! *****************************************************************************
  SUBROUTINE pw_poisson_rebuild(poisson_env,density)
    ...
    IF(.NOT.(ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_methods.F",191)
    IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_methods.F",192)
    IF(.NOT.(ASSOCIATED(poisson_env%pw_pools)))CALL cp__a("pw/pw_poisson_methods.F",193)
    ...
    IF (poisson_env%rebuild) THEN
        ...
        CASE (pw_poisson_wavelet)
           IF(.NOT.(ASSOCIATED(density%pw_grid)))CALL cp__a("pw/pw_poisson_methods.F",205)
        ...
        CASE default
           CALL cp__b("pw/pw_poisson_methods.F",220,"")
        END SELECT
...
! *****************************************************************************
  SUBROUTINE pw_poisson_solve ( poisson_env, density, ehartree, vhartree,&
    ...
    IF(.NOT.(ASSOCIATED(poisson_env)))CALL cp__a("pw/pw_poisson_methods.F",271)
    IF(.NOT.(poisson_env%ref_count>0))CALL cp__a("pw/pw_poisson_methods.F",272)
    ...
    IF ( PRESENT ( vhartree ) ) THEN
       IF(.NOT.(ASSOCIATED(vhartree)))CALL cp__a("pw/pw_poisson_methods.F",284)
       IF(.NOT.pw_grid_compare(pw_pool%pw_grid,vhartree%pw_grid))&
          CALL cp__b("pw/pw_poisson_methods.F",286,"vhartree has a different grid than the poisson solver")
    END IF
    ! density in G space
    ...
    CASE (use_gs_grid)
       ...
       CASE (PS_IMPLICIT)
          ...
          IF (PRESENT(h_stress)) THEN
             CALL cp__b("pw/pw_poisson_methods.F",363,"No stress tensor is implemented for the implicit Poisson solver.")
          END IF
       ...
       CASE DEFAULT
          CALL cp_abort(cp__l("pw/pw_poisson_methods.F",370),&
       ...
       IF ( PRESENT ( h_stress ) ) THEN
          ...
          CASE(use_gs_grid)
             ...
             CASE(ANALYTIC2D, MT2D)
                ...
                h_stress(alpha,:) = 0.0_dp
                CALL cp__b("pw/pw_poisson_methods.F",441,"Stress Tensor not tested for 2D systems.")
             CASE(ANALYTIC1D, MT1D)
                ...
                DO alpha = 1, 3
                   ...
                   END DO
                END DO
                CALL cp__b("pw/pw_poisson_methods.F",453,"Stress Tensor not tested for 1D systems.")
             CASE(ANALYTIC0D, MT0D, MULTIPOLE0D)
                ! Zero the full stress tensor
                h_stress = 0.0_dp
             CASE DEFAULT
                CALL cp_abort(cp__l("pw/pw_poisson_methods.F",458),&
                     "unknown poisson method"//&
                     cp_to_string(poisson_env%green_fft%method))
             END SELECT
          ...
          CASE(use_rs_grid)
             ...
             CASE(WAVELET2D)
                ...
                h_stress(:,alpha) = 0.0_dp
                h_stress(alpha,:) = 0.0_dp
                CALL cp__b("pw/pw_poisson_methods.F",474,"Stress Tensor not tested for 2D systems.")
             CASE(WAVELET1D)
                ! Zero the 2 non-periodic components
                CALL cp__b("pw/pw_poisson_methods.F",477,"WAVELET 1D not implemented!")
             CASE(WAVELET0D)
                ! Zero the full stress tensor
                h_stress = 0.0_dp
             END SELECT
...
! *****************************************************************************
  SUBROUTINE pw_poisson_set ( poisson_env, cell_hmat, parameters, pw_pools, use_level, &
    ...
    IF (PRESENT(pw_pools)) THEN
       IF(.NOT.(ASSOCIATED(pw_pools)))CALL cp__a("pw/pw_poisson_methods.F",553)
       same=.FALSE.
       IF (ASSOCIATED(poisson_env%pw_pools)) THEN
