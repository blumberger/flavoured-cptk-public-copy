# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_force.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_force.F"
!-----------------------------------------------------------------------------!
...
# 83 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_force.F" 2
...
! *****************************************************************************
  SUBROUTINE fist_calc_energy_force(fist_env, debug)
    ...
    INTEGER :: do_ipol, ewald_type, fg_coulomb_size, handle, i, ii, ikind, &
      ...
      node, nparticle_kind, nparticle_local, nshell, shell_index
    ...
    TYPE(section_vals_type), POINTER         :: force_env_section, &
                                                ...
                                                ener_decomp_section !CHANGE S/M
...
!TEST
    ...
    INTEGER ::  imol, nkind,  nmol_per_kind, first_atom, last_atom,& 
                atoms_active_region, aoi, count_idx, act_atoms
    REAL(KIND=dp), ALLOCATABLE, DIMENSION(:, :) :: forces_intra_aoi
!    REAL(KIND=dp), ALLOCATABLE                 :: intra_forces(:,:)
    ...
    ener_decomp_section => section_vals_get_subs_vals(force_env_section, &
                                                      "ENERGY_DECOMP")
...
atoms_active_region = 0
IF (ALLOCATED(thermo%pot_list)) THEN
   CALL section_vals_val_get(ener_decomp_section, "INDEX_MOL_DECOMP",& !CHANGE SG
                             i_vals=my_mol_index)
   !TEST LOOP
       ...
       MOL: DO ikind = 1, nkind
          ...
          DO imol = 1, nmol_per_kind
              ...
              IF (ANY(my_mol_index==i)) THEN
                 CALL get_molecule ( molecule, first_atom=first_atom, last_atom=last_atom )
                 atoms_active_region =  atoms_active_region + (last_atom - first_atom + 1)
              END IF
          END DO
       END DO MOL
   CALL section_vals_val_get(ener_decomp_section, "NUM_ACTIVE_ATOMS", &
                             i_val=act_atoms)
   IF (act_atoms /= atoms_active_region) THEN
      CALL cp__b("fist_force.F",254,"num_act_atoms incorrect")
   END IF
ALLOCATE(forces_intra_aoi(3, atoms_active_region))
END IF
    ...
    IF(shell_present)THEN
       ...
       IF(.NOT.(ASSOCIATED(shell_particle_set)))CALL cp__a("fist_force.F",274)
       IF(.NOT.(ASSOCIATED(core_particle_set)))CALL cp__a("fist_force.F",275)
    ...
    IF(ewald_type /= do_ewald_none)THEN
       ...
       IF(shell_present.AND.do_multipoles)THEN
          CALL cp__b("fist_force.F",408,"Multipoles and Core-Shell model not implemented.")
       END IF
       ...
       IF(do_ipol /= do_fist_pol_none)THEN
          ...
          IF (ASSOCIATED(fist_nonbond_env%charges)) THEN
             CALL cp__b("fist_force.F",426,"Polarizable force field and array charges not implemented!")
          END IF
       ...
       ELSE
          ...
          CASE(do_ewald_ewald)
             ...
             IF(shell_present)THEN
                ! Check if an array of chagres was provided and in case abort due to lack of implementation
                IF (ASSOCIATED(fist_nonbond_env%charges)) THEN
                   CALL cp__b("fist_force.F",441,"Core-Shell and array charges not implemented!")
                END IF
                IF(do_multipoles)THEN
                   CALL cp__b("fist_force.F",444,"Multipole Ewald and CORE-SHELL not yet implemented within Ewald sum!")
                ELSE
                   CALL cp__b("fist_force.F",446,"Core-Shell model not yet implemented within Ewald sums.")
                END IF
             ELSE
                IF(do_multipoles)THEN
                   ! Check if an array of chagres was provided and in case abort due to lack of implementation
                   IF (ASSOCIATED(fist_nonbond_env%charges)) THEN
                      CALL cp__b("fist_force.F",452,"Multipole Ewald and array charges not implemented!")
                   END IF
          ...
          CASE(do_ewald_pme)
             ...
             IF(shell_present)THEN
                ! Check if an array of chagres was provided and in case abort due to lack of implementation
                IF (ASSOCIATED(fist_nonbond_env%charges)) THEN
                   CALL cp__b("fist_force.F",480,"Core-Shell and array charges not implemented!")
                END IF
                IF(do_multipoles)THEN
                   CALL cp__b("fist_force.F",483,"Multipole Ewald and CORE-SHELL not yet implemented within a PME scheme!")
                ELSE
             ...
             ELSE
                IF(do_multipoles)THEN
                   CALL cp__b("fist_force.F",494,"Multipole Ewald not yet implemented within a PME scheme!")
                ELSE
                   CALL pme_evaluate(ewald_env, ewald_pw, cell, particle_set, vg_coulomb, fg_coulomb,&
          ...
          CASE(do_ewald_spme)
             ...
             IF(shell_present)THEN
                ! Check if an array of chagres was provided and in case abort due to lack of implementation
                IF (ASSOCIATED(fist_nonbond_env%charges)) THEN
                   CALL cp__b("fist_force.F",507,"Core-Shell and array charges not implemented!")
                END IF
                IF(do_multipoles)THEN
                   CALL cp__b("fist_force.F",510,"Multipole Ewald and CORE-SHELL not yet implemented within a SPME scheme!")
                ELSE
             ...
             ELSE
                IF(do_multipoles)THEN
                   CALL cp__b("fist_force.F",521,"Multipole Ewald not yet implemented within a SPME scheme!")
                ELSE
                   CALL spme_evaluate(ewald_env, ewald_pw, cell, particle_set, fg_coulomb, vg_coulomb,&
                        pv_g, use_virial=use_virial, charges=fist_nonbond_env%charges, &
                        atprop=atprop_env)
    ...
    IF(PRESENT(debug))THEN
       IF (ALLOCATED(thermo%pot_list)) THEN
          CALL force_intra_control(molecule_set, molecule_kind_set, &
               ...
               pot_1_mol_list=thermo%pot_list, ener_decomp_section=ener_decomp_section)
       ELSE
          CALL force_intra_control(molecule_set, molecule_kind_set, &
               local_molecules, particle_set, shell_particle_set, &
               core_particle_set, pot_bond, pot_bend, pot_urey_bradley, &
               ...
               debug%f_bond, debug%f_bend, debug%f_torsion, debug%f_ub, &
               debug%f_imptors, debug%f_opbend, cell, use_virial, atprop_env, &
               ener_decomp_section=ener_decomp_section)
       END IF
...
IF (ALLOCATED(thermo%pot_list)) THEN
!TEST: store the forces that we need
    count_idx = 0
    nkind = SIZE ( molecule_kind_set )
    DO ikind = 1, nkind
       nmol_per_kind = local_molecules % n_el ( ikind )
       DO imol = 1, nmol_per_kind
           ...
           IF (ANY(my_mol_index==i)) THEN
              CALL get_molecule ( molecule, first_atom=first_atom, last_atom=last_atom )
              DO aoi=first_atom, last_atom
                  count_idx = count_idx + 1
                  forces_intra_aoi(1, count_idx) = particle_set(aoi)%f(1)
                  forces_intra_aoi(2, count_idx) = particle_set(aoi)%f(2)
                  forces_intra_aoi(3, count_idx) = particle_set(aoi)%f(3)
              END DO
           END IF
       END DO
    END DO
    ...
    CALL fist_env_set(fist_env=fist_env, intra_forces=forces_intra_aoi)

END IF

IF(iw>0)THEN
WRITE(iw,'(A)')" FIST:: TOSTORE"
WRITE(iw,'(3f15.9)')((particle_set(i)%f(j),j=1,3), i = 1, SIZE(particle_set))
END IF
    ...
    CASE(do_ewald_spme, do_ewald_pme)
       ...
       ELSEIF(shell_present .AND. .NOT. shell_model_ad)THEN
          CALL cp__b("fist_force.F",841,"Non adiabatic shell-model not implemented.")
       ELSE
          DO i = 1, natoms
             particle_set(i)%f(1)= f_total(1, i)+ fg_coulomb(1, i)
             particle_set(i)%f(2)= f_total(2, i)+ fg_coulomb(2, i)
    ...
    CASE(do_ewald_ewald, do_ewald_none)
       IF(shell_present .AND. shell_model_ad)THEN
          ...
          END DO
       ELSEIF(shell_present .AND. .NOT. shell_model_ad)THEN
          CALL cp__b("fist_force.F",880,"Non adiabatic shell-model not implemented.")
       ELSE
          DO i = 1, natoms
             particle_set(i)%f(1)= f_total(1, i)
             particle_set(i)%f(2)= f_total(2, i)
    ...
    IF(ALLOCATED(forces_intra_aoi))THEN 
       DEALLOCATE(forces_intra_aoi)
    END IF
    IF(ALLOCATED(fg_coulomb))THEN
