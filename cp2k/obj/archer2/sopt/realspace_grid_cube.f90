# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_cube.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_cube.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_cube.F" 2
  ...
  SUBROUTINE pw_to_cube ( pw, unit_nr, title, particles_r, particles_z, stride, zero_tails)
    ...
    IF (PRESENT(stride)) THEN
       IF(SIZE(stride)/=1.AND.SIZE(stride)/=3)&
          CALL cp_abort(cp__l("pw/realspace_grid_cube.F",69), "STRIDE keyword can accept only 1 "//&
       ...
       END IF
       IF(.NOT.(my_stride(1)>0))CALL cp__a("pw/realspace_grid_cube.F",78)
       IF(.NOT.(my_stride(2)>0))CALL cp__a("pw/realspace_grid_cube.F",79)
       IF(.NOT.(my_stride(3)>0))CALL cp__a("pw/realspace_grid_cube.F",80)
    END IF
    ...
    IF (unit_nr>0) THEN
      ...
      IF(.NOT.(PRESENT(particles_z) .EQV. PRESENT(particles_r)))CALL cp__a("pw/realspace_grid_cube.F",93)
      np=0
      IF(PRESENT(particles_z)) THEN
         IF(.NOT.(SIZE(particles_z) == SIZE(particles_r, dim=2)))CALL cp__a("pw/realspace_grid_cube.F",96)
    ...
    IF(.NOT.(checksum==1))CALL cp__a("pw/realspace_grid_cube.F",142)

    CALL mp_maxloc(rank,gid)
    IF(.NOT.(rank(1)>0))CALL cp__a("pw/realspace_grid_cube.F",145)
