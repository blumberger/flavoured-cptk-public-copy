# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/gopt_f_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/gopt_f_methods.F"
!-----------------------------------------------------------------------------!
...
# 64 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/gopt_f_methods.F" 2
...
END INTERFACE
# 68 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/gopt_f_methods.F" 2
  ...
  SUBROUTINE gopt_f_create_x0(gopt_env,x0)
    ...
    CASE (default_cell_method_id)
      ...
      CASE (default_cell_geo_opt_id,default_cell_md_id)
         ...
         END DO
      CASE DEFAULT
         CALL cp__b("motion/gopt_f_methods.F",145,"")
      END SELECT
    CASE DEFAULT
      CALL cp__b("motion/gopt_f_methods.F",148,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE gopt_f_io(gopt_env, force_env, root_section, its, opt_energy,&
    ...
    CASE (default_ts_method_id,default_minimization_method_id)
       ...
       IF (.NOT.gopt_env%dimer_rotation) THEN
          ...
          CALL write_cycle_infos(output_unit,its,etot=opt_energy,ediff=opt_energy-eold,&
               pred=pred,rat=rat,step=step,rad=rad,emin=emin,wildcard=wildcard,used_time=used_time)
          ! Possibly check convergence
          IF (PRESENT(conv)) THEN
             IF(.NOT.(PRESENT(ndf)))CALL cp__a("motion/gopt_f_methods.F",276)
             IF(.NOT.(PRESENT(dx)))CALL cp__a("motion/gopt_f_methods.F",277)
             IF(.NOT.(PRESENT(xi)))CALL cp__a("motion/gopt_f_methods.F",278)
       ...
       ELSE
          ...
          ! Possibly check convergence
          IF (PRESENT(conv)) THEN
             IF(.NOT.(ASSOCIATED(gopt_env%dimer_env)))CALL cp__a("motion/gopt_f_methods.F",288)
    ...
    CASE (default_cell_method_id)
       ...
       ! Possibly check convergence
       IF (PRESENT(conv)) THEN
          IF(.NOT.(PRESENT(ndf)))CALL cp__a("motion/gopt_f_methods.F",304)
          IF(.NOT.(PRESENT(dx)))CALL cp__a("motion/gopt_f_methods.F",305)
          IF(.NOT.(PRESENT(xi)))CALL cp__a("motion/gopt_f_methods.F",306)
    ...
    CASE (default_shellcore_method_id)
          ...
          ! Possibly check convergence
          IF (PRESENT(conv)) THEN
             IF(.NOT.(PRESENT(ndf)))CALL cp__a("motion/gopt_f_methods.F",314)
             IF(.NOT.(PRESENT(dx)))CALL cp__a("motion/gopt_f_methods.F",315)
             IF(.NOT.(PRESENT(xi)))CALL cp__a("motion/gopt_f_methods.F",316)
             CALL check_converg(ndf,dx,xi,output_unit,conv,gopt_param)
          END IF
    END SELECT
...
! *****************************************************************************
  SUBROUTINE apply_cell_change(gopt_env,cell,x,update_forces)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'apply_cell_change', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: i, iatom, idg, j, natom, &
                                                nparticle, shell_index
    ...
    TYPE(particle_list_type), POINTER        :: core_particles, particles, &
     ...
     CASE (default_cell_geo_opt_id,default_cell_md_id)
        idg = 0
     END SELECT
     IF(.NOT.((SIZE(x) == idg + 6)))CALL cp__a("motion/gopt_f_methods.F",995)
