# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_xc.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_xc.F"
!-----------------------------------------------------------------------------!
...
MODULE input_cp2k_xc
  USE bibliography,                    ONLY: &
       ...
       Grimme2010, Grimme2011, Heyd2004, Lee1988, Marques2012, Ortiz1994, &
       Perdew1981, Perdew1996, Perdew2008, Proynov2007, Tao2003, Tran2013, &
       Vosko1980, Wellendorff2012, Zhang1998
  USE cp_output_handling,              ONLY: add_last_numeric,&
                                             cp_print_key_section_create,&
                                             high_print_level
  USE input_constants,                 ONLY: &
       do_adiabatic_hybrid_mcy3, do_adiabatic_model_pade, gaussian, slater, &
       vdw_nl_drsll, vdw_nl_lmkll, vdw_nl_rvv10, vdw_pairpot_dftd2, &
       vdw_pairpot_dftd3, vdw_pairpot_dftd3bj, xc_funct_b3lyp, &
       xc_funct_beefvdw, xc_funct_blyp, xc_funct_bp, xc_funct_hcth120, &
       xc_funct_no_shortcut, xc_funct_olyp, xc_funct_pade, xc_funct_pbe, &
       xc_funct_pbe0, xc_funct_tpss, xc_none, xc_pot_energy_none, &
       xc_pot_energy_sum_eigenvalues, xc_pot_energy_xc_functional, &
  ...
  USE xc_input_constants,              ONLY: &
       c_pw92, c_pw92dmc, c_pw92vmc, c_pz, c_pzdmc, c_pzvmc, do_vwn3, &
       do_vwn5, ke_lc, ke_llp, ke_ol1, ke_ol2, ke_pbe, ke_pw86, ke_pw91, &
       ke_t92, pz_orig, xalpha, xc_b97_grimme, xc_b97_mardirossian, &
       xc_b97_orig, xc_debug_new_routine, xc_deriv_collocate, &
       ...
       xc_pbe_orig, xc_pbe_rev, xc_pbe_sol, xc_rho_nn10, xc_rho_nn50, &
       xc_rho_no_smooth, xc_rho_spline2_smooth, xc_rho_spline3_smooth, &
       xc_test_lsd_f_routine, xgga_b88x, xgga_ev93, xgga_opt, xgga_pbex, &
       xgga_pw86, xgga_pw91, xgga_revpbe
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_xc.F" 2
...
! *****************************************************************************
  SUBROUTINE create_xc_fun_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",82)
...
! *****************************************************************************
  SUBROUTINE create_libxc_section(section,name,description,usage)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",733)
    IF(.NOT.(name == "LIBXC" .OR. name == "KE_LIBXC"))CALL cp__a("input_cp2k_xc.F",734)
    NULLIFY(keyword)
...
! *****************************************************************************
  SUBROUTINE create_xc_potential_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",779)
...
! *****************************************************************************
  SUBROUTINE create_vdw_potential_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",838)
...
! *****************************************************************************
  SUBROUTINE create_xc_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",1053)
...
! *****************************************************************************
  SUBROUTINE create_adiabatic_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_xc.F",1174)
