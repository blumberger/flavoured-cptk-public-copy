# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_val_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_val_types.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_val_types.F" 2
...
SUBROUTINE val_create(val,l_val,l_vals,l_vals_ptr,i_val,i_vals,i_vals_ptr,&
  ...
  IF(.NOT.(.NOT.ASSOCIATED(val)))CALL cp__a("input/input_val_types.F",136)
  ALLOCATE(val)
...
!FM        CPPrecondition(.NOT.PRESENT(i_vals_ptr),cp_failure_level,routineP,failure)
  ...
  IF (PRESENT(c_val)) THEN
     IF(.NOT.(LEN_TRIM(c_val)<=default_string_length))CALL cp__a("input/input_val_types.F",210)
...
!FM        CPPrecondition(.NOT.PRESENT(c_vals_ptr),cp_failure_level,routineP,failure)
     IF(.NOT.(ALL(LEN_TRIM(c_vals)<=default_string_length)))CALL cp__a("input/input_val_types.F",220)
     narg=narg+1
...
!FM        CPPrecondition(.NOT.PRESENT(lc_vals_ptr),cp_failure_level,routineP,failure)
  ...
  IF (PRESENT(lc_vals)) THEN
     IF(.NOT.(ALL(LEN_TRIM(lc_vals)<=default_string_length)))CALL cp__a("input/input_val_types.F",250)
     narg=narg+1
  ...
  IF (PRESENT(lc_vals_ptr)) THEN
     ...
     val%type_of_var=lchar_t
  END IF
  IF(.NOT.(narg<=1))CALL cp__a("input/input_val_types.F",261)
  IF (PRESENT(enum)) THEN
     IF (ASSOCIATED(enum)) THEN
        IF (val%type_of_var/=no_t.AND.val%type_of_var/=integer_t.AND.&
           ...
           CALL cp__b("input/input_val_types.F",266,"")
        END IF
     ...
     END IF
  END IF
  IF(.NOT.(ASSOCIATED(val%enum).eqv.val%type_of_var==enum_t))CALL cp__a("input/input_val_types.F",275)
...
SUBROUTINE val_release(val)
  ...
  IF (ASSOCIATED(val)) THEN
     IF(.NOT.(val%ref_count>0))CALL cp__a("input/input_val_types.F",290)
...
SUBROUTINE val_retain(val)
  ...
  IF(.NOT.(ASSOCIATED(val)))CALL cp__a("input/input_val_types.F",324)
  IF(.NOT.(val%ref_count>0))CALL cp__a("input/input_val_types.F",325)
  val%ref_count=val%ref_count+1
  ...
  IF (PRESENT(l_val)) THEN
     IF (ASSOCIATED(val%l_val)) THEN
        IF (SIZE(val%l_val)>0) THEN
           l_val=val%l_val(1)
        ELSE
           CALL cp__b("input/input_val_types.F",389,"")
        END IF
     ELSE
        CALL cp__b("input/input_val_types.F",392,"")
     END IF
  END IF
  ...
  IF (PRESENT(i_val)) THEN
     IF (ASSOCIATED(val%i_val)) THEN
        IF (SIZE(val%i_val)>0) THEN
           i_val=val%i_val(1)
        ELSE
           CALL cp__b("input/input_val_types.F",402,"")
        END IF
     ELSE
        CALL cp__b("input/input_val_types.F",405,"")
     END IF
  END IF
  ...
  IF (PRESENT(r_val)) THEN
     IF (ASSOCIATED(val%r_val)) THEN
        IF (SIZE(val%r_val)>0) THEN
           r_val=val%r_val(1)
        ELSE
           CALL cp__b("input/input_val_types.F",415,"")
        END IF
     ELSE
        CALL cp__b("input/input_val_types.F",418,"")
     END IF
  END IF
     ...
     IF (ASSOCIATED(val%c_val)) THEN
        IF (SIZE(val%c_val)>0) THEN
           IF (val%type_of_var==lchar_t) THEN
              ...
              IF(l_out<l_in)&
                 CALL cp_warn(cp__l("input/input_val_types.F",431),&
           ...
           ELSE
              ...
              IF(l_out<l_in)&
                 CALL cp_warn(cp__l("input/input_val_types.F",444),&
                      "val_get will truncate value, value '"//&
                      TRIM(val%c_val(1))//"' is too long for variable")
              c_val=val%c_val(1)
           END IF
        ELSE
           CALL cp__b("input/input_val_types.F",450,"")
        END IF
     ELSE IF (ASSOCIATED(val%i_val).AND.ASSOCIATED(val%enum)) THEN
        IF (SIZE(val%i_val)>0) THEN
           c_val=enum_i2c(val%enum,val%i_val(1))
        ELSE
           CALL cp__b("input/input_val_types.F",456,"")
        END IF
     ELSE
        CALL cp__b("input/input_val_types.F",459,"")
     END IF
  END IF
  ...
  IF (ASSOCIATED(val)) THEN
     ...
     CASE (logical_t)
        IF (ASSOCIATED(val%l_val)) THEN
           ...
           END DO
        ELSE
           CALL cp__b("input/input_val_types.F",539,"")
        END IF
     CASE (integer_t)
        IF (ASSOCIATED(val%i_val)) THEN
           item = 0
           ...
           END DO loop_i
        ELSE
           CALL cp__b("input/input_val_types.F",570,"")
        END IF
     CASE (real_t)
        IF (ASSOCIATED(val%r_val)) THEN
           ...
           END DO
        ELSE
           CALL cp__b("input/input_val_types.F",587,"")
        END IF
     CASE (char_t)
        IF (ASSOCIATED(val%c_val)) THEN
           l=0
           ...
           END DO
        ELSE
           CALL cp__b("input/input_val_types.F",610,"")
        END IF
     CASE (lchar_t)
        IF (ASSOCIATED(val%c_val)) THEN
           l=0
           ...
           END IF
        ELSE
           CALL cp__b("input/input_val_types.F",622,"")
        END IF
     CASE (enum_t)
        IF (ASSOCIATED(val%i_val)) THEN
           l=0
           ...
           END DO
        ELSE
           CALL cp__b("input/input_val_types.F",639,"")
        END IF

     CASE(no_t)
        WRITE(unit=unit_nr,fmt="(' *empty*')",advance="NO")
     CASE default
        CALL cp__b("input/input_val_types.F",645,"unexpected type_of_var for val ")
     END SELECT
  ELSE
    ...
    IF (ASSOCIATED(val)) THEN
      ...
      CASE (logical_t)
        IF (ASSOCIATED(val%l_val)) THEN
          ...
          END DO
        ELSE
          CALL cp__b("input/input_val_types.F",691,"")
        END IF
      CASE (integer_t)
        IF (ASSOCIATED(val%i_val)) THEN
          DO i=1,SIZE(val%i_val)
          ...
          END DO
        ELSE
          CALL cp__b("input/input_val_types.F",699,"")
        END IF
      CASE (real_t)
        IF (ASSOCIATED(val%r_val)) THEN
          ...
          END IF
        ELSE
          CALL cp__b("input/input_val_types.F",715,"")
        END IF
      CASE (char_t)
        IF (ASSOCIATED(val%c_val)) THEN
          ipos = 1
          ...
          END DO
        ELSE
          CALL cp__b("input/input_val_types.F",725,"")
        END IF
      CASE (lchar_t)
        IF (ASSOCIATED(val%c_val)) THEN
          CALL val_get(val,c_val=string)
        ELSE
          CALL cp__b("input/input_val_types.F",731,"")
        END IF
      CASE (enum_t)
        IF (ASSOCIATED(val%i_val)) THEN
          DO i=1,SIZE(val%i_val)
          ...
          END DO
        ELSE
          CALL cp__b("input/input_val_types.F",740,"")
        END IF
      CASE default
        CALL cp__b("input/input_val_types.F",743,"unexpected type_of_var for val ")
      END SELECT
...
SUBROUTINE val_duplicate(val_in,val_out)
  ...
  IF(.NOT.(ASSOCIATED(val_in)))CALL cp__a("input/input_val_types.F",762)
  IF(.NOT.(.NOT.ASSOCIATED(val_out)))CALL cp__a("input/input_val_types.F",763)
  ALLOCATE(val_out)
  last_val_id=last_val_id+1
