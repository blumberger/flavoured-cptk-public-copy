# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_external_potential.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_external_potential.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_external_potential.F" 2
...
! *****************************************************************************
  SUBROUTINE get_external_potential(r, ext_pot_section, func, dfunc, calc_derivatives)
    ...
    IF(.NOT.(check))CALL cp__a("qs_external_potential.F",252)
    ...
    IF (my_force) THEN
       DO j = 1, 3
          ...
          IF (ABS(err)>lerr) THEN
             ...
             CALL cp_warn(cp__l("qs_external_potential.F",273),&
                  'ASSERTION (cond) failed at line '//cp_to_string(274)//&
...
! *****************************************************************************
  SUBROUTINE interpolate_external_potential(r, grid, func, dfunc, calc_derivatives)
    ...
    IF (PRESENT(calc_derivatives)) my_force=calc_derivatives
    check=PRESENT(dfunc).EQV.PRESENT(calc_derivatives)
    IF(.NOT.(check))CALL cp__a("qs_external_potential.F",323)
