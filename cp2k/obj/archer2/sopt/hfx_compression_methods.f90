# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_compression_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_compression_methods.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_compression_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE hfx_compress_cache(full_array, container, nbits, memory_usage, use_disk_storage, &
    ...
    increment_counter = (nbits*1024 + 63)/64
    end_idx = start_idx + increment_counter - 1
    IF( end_idx<1024) THEN
      CALL ints2bits_specific(nbits,1024,container%current%data(start_idx),full_array(1))
    ...
    ELSE
      ...
      CALL ints2bits_specific(nbits, 1024-tmp_nints, container%current%data(1), full_array(tmp_nints+1))
      container%element_counter = 1 + (nbits*(1024-tmp_nints) + 63)/64
    END IF
...
! *****************************************************************************
  SUBROUTINE hfx_decompress_cache(full_array, container, nbits, memory_usage, use_disk_storage)
    ...
    end_idx = start_idx + increment_counter - 1
    IF( end_idx<1024) THEN
      CALL bits2ints_specific(nbits,1024,container%current%data(start_idx),full_array(1))
      container%element_counter = container%element_counter + increment_counter
    ELSE
      !! We have to fill the container first with the remaining number of bits
      tmp_elements = 1024 - start_idx + 1
      ...
      CALL bits2ints_specific(nbits, 1024-tmp_nints, container%current%data(1), full_array(tmp_nints+1))
      container%element_counter = 1 + (nbits*(1024-tmp_nints) + 63)/64
    END IF
