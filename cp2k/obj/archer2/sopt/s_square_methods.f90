# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/s_square_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/s_square_methods.F"
!-----------------------------------------------------------------------------!
...
# 35 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/s_square_methods.F" 2
...
! *****************************************************************************
   SUBROUTINE compute_s_square(mos, matrix_s, s_square, s_square_ideal,&
    ...
    CASE (1)
          ...
          ! let's not do this
          IF(.NOT.(PRESENT(mo_derivs)))CALL cp__a("s_square_methods.F",100)
    CASE (2)
          CALL get_mo_set(mo_set=mos(1)%mo_set,mo_coeff=c_alpha,homo=nalpha,uniform_occupation=uniform_occupation)
          IF(.NOT.uniform_occupation) CALL cp__w("s_square_methods.F",103,"Found non-uniform occupation")
          CALL get_mo_set(mo_set=mos(2)%mo_set,mo_coeff=c_beta,homo=nbeta,uniform_occupation=uniform_occupation)
          IF(.NOT.uniform_occupation) CALL cp__w("s_square_methods.F",105,"Found non-uniform occupation")
          ...
          IF (PRESENT(mo_derivs)) THEN
            ...
            IF(.NOT.(SIZE(mo_derivs,1)==2))CALL cp__a("s_square_methods.F",135)
            CALL get_mo_set(mo_set=mos(1)%mo_set,uniform_occupation=uniform_occupation)
            IF(.NOT.(uniform_occupation))CALL cp__a("s_square_methods.F",137)
            CALL get_mo_set(mo_set=mos(2)%mo_set,uniform_occupation=uniform_occupation)
            IF(.NOT.(uniform_occupation))CALL cp__a("s_square_methods.F",139)
          ...
          ENDIF
    CASE DEFAULT
          CALL cp__b("s_square_methods.F",153,"alpha, beta, what else ?")
    END SELECT
...
! *****************************************************************************
   SUBROUTINE s2_restraint(mos, matrix_s, mo_derivs, energy, &
    ...
    CASE(do_s2_restraint) ! not yet implemented
       CALL cp__b("s_square_methods.F",203,"")
    CASE DEFAULT
       CALL cp__b("s_square_methods.F",205,"")
    END SELECT
