# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho0_ggrid.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho0_ggrid.F"
!-----------------------------------------------------------------------------!
...
# 72 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho0_ggrid.F" 2
...
! *****************************************************************************
  SUBROUTINE put_rho0_on_grid(qs_env, rho0, tot_rs_int)
    ...
    INTEGER :: auxbas_grid, handle, iat, iatom, igrid, ikind, istat, ithread, &
      j, l0_ikind, lmax0, lmax_global, nat, nch_ik, nch_max, npme
    ...
    IF(.NOT.(ASSOCIATED(desc)))CALL cp__a("qs_rho0_ggrid.F",165)
    IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("qs_rho0_ggrid.F",166)
    ...
    IF (igrid /= auxbas_grid) THEN
      ...
      IF(istat /= 0) CALL stop_memory(routineN,moduleN,173,&
                                      "coeff_gspace",0)
    ...
    !Compute global lmax for use in collocate_pgf_product_rspace
    lmax_global=0
    DO ikind = 1,SIZE(atomic_kind_set)
      CALL get_rho0_mpole(rho0_mpole=rho0, ikind=ikind, l0_ikind=l0_ikind)
      lmax_global=MAX(lmax_global,l0_ikind)
    END DO 
    ...
    DO ikind = 1,SIZE(atomic_kind_set)
      ...
      DO j=1,npme
        ...
        CALL collocate_pgf_product_rspace(&
           ...
           use_subpatch=.TRUE.,subpatch_pattern=0_int_8,lmax_global=lmax_global)
...
! *****************************************************************************
  ...
  IF(.NOT.(ASSOCIATED(new_pw_env)))CALL cp__a("qs_rho0_ggrid.F",318)

  NULLIFY(auxbas_pw_pool)
  CALL pw_env_get(new_pw_env, auxbas_pw_pool=auxbas_pw_pool)
  IF(.NOT.(ASSOCIATED(auxbas_pw_pool)))CALL cp__a("qs_rho0_ggrid.F",322)

  ! reallocate rho0 on the global grid in real and reciprocal space
  NULLIFY(rho_rs,rho_gs)
  IF(.NOT.(ASSOCIATED(rho0_mpole)))CALL cp__a("qs_rho0_ggrid.F",326)
...
! *****************************************************************************
  SUBROUTINE integrate_vhg0_rspace(qs_env, v_rspace, calculate_forces, &
    ...
    INTEGER :: auxbas_grid, bo(2), handle, i, iat, iatom, ic, icg, ico, ig1, &
      ...
      ispin, j, l0_ikind, llmax, lmax0, lshell, lx, ly, lz, m1, m2, &
    ...
    LOGICAL                                  :: grid_distributed, my_tddft, &
                                                paw_atom, use_virial
    ...
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_rho0_ggrid.F",475)
      ...
      DO j=0,num_pe-1
        ...
        DO iat = bo(1), bo(2)
          ...
          CALL integrate_pgf_product_rspace(&
             ...
             a_hdab=a_hdab,use_subpatch=.TRUE.,subpatch_pattern=0_int_8)
    ...
    DEALLOCATE (hab,hdab,hadb,hab_sph,hdab_sph,pab,a_hdab,a_hdab_sph)
