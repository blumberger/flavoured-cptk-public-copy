# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis.F"
!-----------------------------------------------------------------------------!
...
# 61 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis.F" 2
...
! *****************************************************************************
  ...
  SUBROUTINE compute_residuum_vectors(opt_bas,f_env_id,matrix_S_inv,tot_time,&
    ...
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
      POINTER                                :: matrix_ks, matrix_s_aux_fit, &
    ...
    DO icalc=1,SIZE(opt_bas%comp_group(mp_id)%member_list)
        ...
        CALL calculate_overlap(ks_env,"S_AB_AUX",qs_env%input)
...
! *****************************************************************************
  ...
  SUBROUTINE init_training_force_envs(opt_bas,f_env_id,input_declaration,matrix_s_inv,para_env,mpi_comm_opt)
    ...
    DO icalc=1,SIZE(opt_bas%comp_group(mp_id)%member_list)
       ...
       IF(.NOT.(ierr==0))CALL cp__a("optimize_basis.F",374)
       ...
       CALL calculate_overlap(ks_env,"S_AB",qs_env%input)
