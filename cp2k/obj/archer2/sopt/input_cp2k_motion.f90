# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/input_cp2k_motion.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/input_cp2k_motion.F"
!-----------------------------------------------------------------------------!
...
# 64 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/start/input_cp2k_motion.F" 2
...
! *****************************************************************************
  SUBROUTINE create_motion_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",88)
...
! *****************************************************************************
  SUBROUTINE create_mc_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",165)
...
! *****************************************************************************
  SUBROUTINE create_avbmc_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",381)
...
! *****************************************************************************
  SUBROUTINE create_move_prob_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",436)
...
! *****************************************************************************
  SUBROUTINE create_mol_prob_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",510)
...
! *****************************************************************************
  SUBROUTINE create_box_prob_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",570)
...
! *****************************************************************************
  SUBROUTINE create_update_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",610)
...
! *****************************************************************************
  SUBROUTINE create_max_disp_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",649)
...
! *****************************************************************************
  SUBROUTINE create_mol_disp_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",681)
...
! *****************************************************************************
  SUBROUTINE create_box_disp_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",741)
...
!> \author teo
! *****************************************************************************
  RECURSIVE SUBROUTINE create_geoopt_section(section,label,description,just_optimizers)
    TYPE(section_type), POINTER              :: section
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",779)
    ...
    CALL create_bfgs_section(subsection)
    CALL section_add_subsection(section,subsection)
    CALL section_release(subsection)
...
! *****************************************************************************
  SUBROUTINE create_cell_opt_section(section)
    ...
    CALL create_geoopt_section(section, label="CELL_OPT",&
         ...
         " (2) Finite temperature optimization. ",just_optimizers=.TRUE.)
...
!> \author Teodoro Laino [tlaino] - University of Zurich - 01.2008
! *****************************************************************************
  SUBROUTINE create_bfgs_section(section)
    TYPE(section_type), POINTER              :: section
...
! create the LBFGS section
    ...
    CALL keyword_create(keyword, name="TRUST_RADIUS",&
         description="Trust radius used in LBFGS. Not completly in dept tested. Negativ values means no trust radius is used.",&
         usage="TRUST_RADIUS {real}",unit_str='angstrom',&
...
! *****************************************************************************
  SUBROUTINE create_fp_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",1411)
...
! *****************************************************************************
  SUBROUTINE create_driver_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",1509)
...
! *****************************************************************************
  SUBROUTINE create_pint_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",1555)
    CALL section_create(section,name="PINT",&
         description="The section that controls a path integral run",&
         n_keywords=13, n_subsections=8, repeats=.FALSE.)
    NULLIFY(keyword)
    ...
    CALL section_add_keyword(section,keyword)
    CALL keyword_release(keyword)
    CALL keyword_create(keyword, name="HARM_INT",&
    ...
    CALL section_add_subsection(subsection,subsubsection)
    CALL section_release(subsubsection)
    CALL keyword_create(keyword, name="TAU", &
         ...
         n_var=1, default_r_val=10000.0_dp)
    CALL section_add_keyword(subsection,keyword)
    CALL keyword_release(keyword)
    CALL keyword_create(keyword, name="LAMBDA", &
         ...
         unit_str="fs", n_var=1, default_r_val=0.01_dp)
    CALL section_add_keyword(subsection,keyword)
    CALL keyword_release(keyword)
    CALL section_add_subsection(section,subsection)
    CALL section_release(subsection)

    CALL section_create(subsection,name="PIGLET",&
    ...
    CALL section_add_subsection(subsection,subsubsection)
    CALL section_release(subsubsection)
    CALL section_create(subsubsection,name="EXTRA_DOF",&
    ...
    CALL section_add_subsection(subsection,subsubsection)
    CALL section_release(subsubsection)
    CALL keyword_create(keyword, name="NEXTRA_DOF", &
         description="Number of extra degrees of freedom to ensure markovian dynamics", &
         repeats=.FALSE., default_i_val=8)
    CALL section_add_keyword(subsection,keyword)
    CALL keyword_release(keyword)
    CALL keyword_create(keyword, name="MATRICES_FILE_NAME", &
         description="Filename containig the raw matrices from "//&
         "https://epfl-cosmo.github.io/gle4md/index.html?page=matrix", &
         repeats=.FALSE., default_lc_val="PIGLET.MAT")
    CALL section_add_keyword(subsection,keyword)
    CALL keyword_release(keyword)
    CALL keyword_create(keyword, name="SMATRIX_INIT",&
    ...
    CALL section_add_keyword(subsection,keyword)
    CALL keyword_release(keyword)
    CALL keyword_create(keyword,name="THERMOSTAT_SEED",&
         description="Initial seed for the (pseudo)random number "//&
    ...
    CALL keyword_create(keyword, name="IMAGINARY_TIME_STRIDE", &
...
! *****************************************************************************
  SUBROUTINE create_helium_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("start/input_cp2k_motion.F",1933)
