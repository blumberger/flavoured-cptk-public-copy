# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_types.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_env_types.F" 2
...
! *****************************************************************************
  SUBROUTINE force_env_retain(force_env)
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",171)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_types.F",172)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE force_env_release(force_env)
    ...
    IF (ASSOCIATED(force_env)) THEN
       IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_types.F",195)
       force_env%ref_count=force_env%ref_count-1
       IF (force_env%ref_count==0) THEN
          ...
          CALL cp_para_env_release(force_env%para_env)
          ! Not deallocated
          IF(.NOT.(.NOT.ASSOCIATED(force_env%fist_env)))CALL cp__a("force_env_types.F",228)
          IF(.NOT.(.NOT.ASSOCIATED(force_env%qs_env)))CALL cp__a("force_env_types.F",229)
          IF(.NOT.(.NOT.ASSOCIATED(force_env%eip_env)))CALL cp__a("force_env_types.F",230)
          IF(.NOT.(.NOT.ASSOCIATED(force_env%mixed_env)))CALL cp__a("force_env_types.F",231)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE force_env_get(force_env, in_use, fist_env, qs_env, &
       ...
       qmmm_env, qmmmx_env, eip_env, globenv, input, force_env_section, &
       method_name_id, root_section, mixed_env)
    TYPE(force_env_type), POINTER            :: force_env
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",322)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_types.F",323)

    SELECT CASE(force_env%in_use)
    CASE (use_qs_force)
       IF(.NOT.(ASSOCIATED(force_env%qs_env)))CALL cp__a("force_env_types.F",327)
       IF(.NOT.(.NOT.PRESENT(fist_env)))CALL cp__a("force_env_types.F",328)
       IF(.NOT.(.NOT.PRESENT(eip_env)))CALL cp__a("force_env_types.F",329)
       CALL get_qs_env(force_env%qs_env,&
                       energy=qs_energy,&
                       input=input,&
                       cp_subsys=subsys)
       IF (PRESENT(potential_energy)) potential_energy = qs_energy%total
       IF(.NOT.(.NOT.PRESENT(kinetic_energy)))CALL cp__a("force_env_types.F",335)
    CASE (use_fist_force)
       IF(.NOT.(ASSOCIATED(force_env%fist_env)))CALL cp__a("force_env_types.F",337)
!          CPPreconditionNoFail(.NOT.PRESENT(qs_env),cp_warning_level,routineP)
!          CPPreconditionNoFail(.NOT.PRESENT(eip_env),cp_warning_level,routineP)
       IF(.NOT.(.NOT.PRESENT(input)))CALL cp__a("force_env_types.F",340)
    ...
    CASE (use_eip_force)
       IF(.NOT.(ASSOCIATED(force_env%eip_env)))CALL cp__a("force_env_types.F",349)
       IF(.NOT.(.NOT. PRESENT(qs_env)))CALL cp__a("force_env_types.F",350)
       IF(.NOT.(.NOT. PRESENT(fist_env)))CALL cp__a("force_env_types.F",351)
       ...
       IF(.NOT.(.NOT.PRESENT(kinetic_energy)))CALL cp__a("force_env_types.F",360)
    CASE (use_qmmm)
    ...
    CASE (use_qmmmx)
       CALL qmmmx_env_get(force_env%qmmmx_env,&
                          ...
                          kinetic_energy=kinetic_energy)
    CASE (use_mixed_force)
       IF(.NOT.(ASSOCIATED(force_env%mixed_env)))CALL cp__a("force_env_types.F",372)
!          CPPreconditionNoFail(.NOT.PRESENT(qs_env),cp_warning_level,routineP)
!          CPPreconditionNoFail(.NOT.PRESENT(eip_env),cp_warning_level,routineP)
       IF(.NOT.(.NOT.PRESENT(input)))CALL cp__a("force_env_types.F",375)
    ...
    CASE DEFAULT
       CALL cp__b("force_env_types.F",382,"unknown in_use flag value ")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE force_env_get_frc(force_env,frc,n)
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",506)
    IF(.NOT.(force_env%ref_count > 0))CALL cp__a("force_env_types.F",507)
    CALL force_env_get(force_env,subsys=subsys)
...
! *****************************************************************************
  SUBROUTINE force_env_get_pos(force_env,pos,n)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",535)
    IF(.NOT.(force_env%ref_count > 0))CALL cp__a("force_env_types.F",536)
    CALL force_env_get(force_env,subsys=subsys)
...
! *****************************************************************************
  SUBROUTINE force_env_get_vel(force_env,vel,n)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",564)
    IF(.NOT.(force_env%ref_count > 0))CALL cp__a("force_env_types.F",565)
    CALL force_env_get(force_env,subsys=subsys)
...
! *****************************************************************************
  SUBROUTINE force_env_set(force_env, meta_env,fp_env, force_env_section,&
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_types.F",598)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_types.F",599)
...
! Let's treat the case of Multiple force_eval
    ...
    IF (nforce_eval>0) THEN
       ...
       ELSE
          ...
          IF(.NOT.(nforce_eval-SIZE(my_i_force_eval)==1))CALL cp__a("force_env_types.F",661)
