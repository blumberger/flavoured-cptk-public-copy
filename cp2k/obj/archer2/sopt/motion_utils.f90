# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion_utils.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE rot_ana(particles, mat, dof, print_section, keep_rotations, mass_weighted,&
    ...
    IF(.NOT.(ASSOCIATED(particles)))CALL cp__a("motion_utils.F",106)
    IF (present_mat) THEN
       IF(.NOT.(.NOT.ASSOCIATED(mat)))CALL cp__a("motion_utils.F",108)
    END IF
    IF (.NOT.keep_rotations) THEN
       ...
       DO iparticle = 1, natoms
          ...
          IF(.NOT.(mass>=0.0_dp))CALL cp__a("motion_utils.F",117)
          masst = masst + mass
          rcom =  particles(iparticle)%r * mass + rcom
       END DO
       IF(.NOT.(masst>0.0_dp))CALL cp__a("motion_utils.F",121)
       rcom = rcom / masst
       ! Intertia Tensor
       Ip   = 0.0_dp
       DO iparticle = 1, natoms
...
! *****************************************************************************
  SUBROUTINE write_trajectory (force_env, root_section, it, time, dtime, etot, pk_name,&
    ...
    CHARACTER(LEN=default_string_length) :: id_label, id_wpc, my_act, my_ext, &
      ...
      title, unit_str
    INTEGER                                  :: handle, i, ii, iskip, nat, &
                                                outformat, traj_unit
    INTEGER, POINTER                         :: force_mixing_indices(:), &
                                                force_mixing_labels(:)
    LOGICAL :: charge_beta, charge_extended, charge_occup, explicit, &
    ...
    IF (PRESENT(particles)) THEN
       IF(.NOT.(ASSOCIATED(particles)))CALL cp__a("motion_utils.F",343)
    ...
    IF (traj_unit > 0) THEN
       ...
       CASE (dump_pdb)
          IF (id_wpc == "POS") THEN
             ...
             IF(i > 1)&
                CALL cp__b("motion_utils.F",404,"Either only CHARGE_OCCUP, CHARGE_BETA, or CHARGE_EXTENDED can be selected, ")
          END IF
          IF (new_file) THEN
          ...
          IF (ASSOCIATED(force_mixing_restart_section)) THEN
             ...
             IF (explicit) THEN
                ...
                DO i=1, SIZE(force_mixing_indices)
                  ...
                  IF(.NOT.(ii <= SIZE(particle_set)))CALL cp__a("motion_utils.F",441)
