# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_rho_set_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_rho_set_types.F"
!-----------------------------------------------------------------------------!
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_rho_set_types.F" 2
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_create(rho_set,local_bounds,rho_cutoff,drho_cutoff,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(rho_set)))CALL cp__a("xc/xc_rho_set_types.F",135)
    ALLOCATE(rho_set)
    rho_set%ref_count=1
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_retain(rho_set)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_rho_set_types.F",174)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_rho_set_types.F",175)
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_release(rho_set, pw_pool)
    ...
    IF (ASSOCIATED(rho_set)) THEN
       IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_rho_set_types.F",194)
       rho_set%ref_count=rho_set%ref_count-1
       IF (rho_set%ref_count==0) THEN
          IF (PRESENT(pw_pool)) THEN
             IF (ASSOCIATED(pw_pool)) THEN
                ...
                rho_set%ref_count=0
             ELSE
                CALL cp__b("xc/xc_rho_set_types.F",203,"")
             END IF
          END IF
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_get(rho_set, can_return_null, rho, drho, norm_drho,&
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_rho_set_types.F",353)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_rho_set_types.F",354)
    IF (PRESENT(rho)) THEN
       rho => rho_set%rho
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rho)))CALL cp__a("xc/xc_rho_set_types.F",357)
    END IF
    IF (PRESENT(drho)) THEN
       drho => rho_set%drho
       IF (.NOT.my_can_return_null) THEN
          DO i=1,3
             IF(.NOT.(ASSOCIATED(rho_set%drho(i)%array)))CALL cp__a("xc/xc_rho_set_types.F",363)
          END DO
       END IF
    END IF
    IF (PRESENT(norm_drho)) THEN
       norm_drho => rho_set%norm_drho
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(norm_drho)))CALL cp__a("xc/xc_rho_set_types.F",369)
    END IF
    IF (PRESENT(laplace_rho)) THEN
       laplace_rho => rho_set%laplace_rho
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(laplace_rho)))CALL cp__a("xc/xc_rho_set_types.F",373)
    END IF
    IF (PRESENT(rhoa)) THEN
       rhoa => rho_set%rhoa
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rhoa)))CALL cp__a("xc/xc_rho_set_types.F",377)
    END IF
    IF (PRESENT(rhob)) THEN
       rhob => rho_set%rhob
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rhob)))CALL cp__a("xc/xc_rho_set_types.F",381)
    END IF
    IF (PRESENT(drhoa)) THEN
       drhoa => rho_set%drhoa
       IF (.NOT.my_can_return_null) THEN
          DO i=1,3
             IF(.NOT.(ASSOCIATED(rho_set%drhoa(i)%array)))CALL cp__a("xc/xc_rho_set_types.F",387)
          END DO
       END IF
    END IF
    IF (PRESENT(drhob)) THEN
       ...
       IF (.NOT.my_can_return_null) THEN
          DO i=1,3
             IF(.NOT.(ASSOCIATED(rho_set%drhob(i)%array)))CALL cp__a("xc/xc_rho_set_types.F",395)
          END DO
       END IF
    END IF
    IF (PRESENT(laplace_rhoa)) THEN
       laplace_rhoa => rho_set%laplace_rhoa
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(laplace_rhoa)))CALL cp__a("xc/xc_rho_set_types.F",401)
    END IF
    IF (PRESENT(laplace_rhob)) THEN
       laplace_rhob => rho_set%laplace_rhob
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(laplace_rhob)))CALL cp__a("xc/xc_rho_set_types.F",405)
    END IF
   IF (PRESENT(norm_drhoa)) THEN
       norm_drhoa => rho_set%norm_drhoa
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(norm_drhoa)))CALL cp__a("xc/xc_rho_set_types.F",409)
    END IF
    IF (PRESENT(norm_drhob)) THEN
       norm_drhob => rho_set%norm_drhob
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(norm_drhob)))CALL cp__a("xc/xc_rho_set_types.F",413)
    END IF
    IF (PRESENT(drhoa_drhob)) THEN
       drhoa_drhob => rho_set%drhoa_drhob
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(drhoa_drhob)))CALL cp__a("xc/xc_rho_set_types.F",417)
    END IF
    IF (PRESENT(rho_1_3)) THEN
       rho_1_3 => rho_set%rho_1_3
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rho_1_3)))CALL cp__a("xc/xc_rho_set_types.F",421)
    END IF
    IF (PRESENT(rhoa_1_3)) THEN
       rhoa_1_3 => rho_set%rhoa_1_3
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rhoa_1_3)))CALL cp__a("xc/xc_rho_set_types.F",425)
    END IF
    IF (PRESENT(rhob_1_3)) THEN
       rhob_1_3 => rho_set%rhob_1_3
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(rhob_1_3)))CALL cp__a("xc/xc_rho_set_types.F",429)
    END IF
    IF (PRESENT(tau)) THEN
       tau => rho_set%tau
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(tau)))CALL cp__a("xc/xc_rho_set_types.F",433)
    END IF
    IF (PRESENT(tau_a)) THEN
       tau_a => rho_set%tau_a
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(tau_a)))CALL cp__a("xc/xc_rho_set_types.F",437)
    END IF
    IF (PRESENT(tau_b)) THEN
       tau_b => rho_set%tau_b
       IF(.NOT.(my_can_return_null.OR.ASSOCIATED(tau_b)))CALL cp__a("xc/xc_rho_set_types.F",441)
    END IF
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_clean(rho_set,pw_pool)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_rho_set_types.F",465)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_rho_set_types.F",466)
...
! *****************************************************************************
  SUBROUTINE xc_rho_set_update(rho_set, rho_r, rho_g, tau, needs,&
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_rho_set_types.F",623)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_rho_set_types.F",624)
    IF(ANY(rho_set%local_bounds/=pw_pool%pw_grid%bounds_local))&
       CALL cp__b("xc/xc_rho_set_types.F",626,"pw_pool cr3d have different size than expected")
    nspins = SIZE(rho_r)
    ...
    SELECT CASE(nspins)
    CASE(1)
       IF(.NOT.(SIZE(rho_r)==1))CALL cp__a("xc/xc_rho_set_types.F",635)
       IF(.NOT.(ASSOCIATED(rho_r(1)%pw)))CALL cp__a("xc/xc_rho_set_types.F",636)
       IF(.NOT.(rho_r(1)%pw%in_use==REALDATA3D))CALL cp__a("xc/xc_rho_set_types.F",637)
       IF(.NOT.(.NOT.needs%rho_spin))CALL cp__a("xc/xc_rho_set_types.F",638)
       IF(.NOT.(.NOT.needs%drho_spin))CALL cp__a("xc/xc_rho_set_types.F",639)
       IF(.NOT.(.NOT.needs%norm_drho_spin))CALL cp__a("xc/xc_rho_set_types.F",640)
       IF(.NOT.(.NOT.needs%drhoa_drhob))CALL cp__a("xc/xc_rho_set_types.F",641)
       IF(.NOT.(.NOT.needs%rho_spin_1_3))CALL cp__a("xc/xc_rho_set_types.F",642)
    CASE(2)
       IF(.NOT.(SIZE(rho_r)==2))CALL cp__a("xc/xc_rho_set_types.F",644)
       IF(.NOT.(ASSOCIATED(rho_r(1)%pw)))CALL cp__a("xc/xc_rho_set_types.F",645)
       IF(.NOT.(ASSOCIATED(rho_r(2)%pw)))CALL cp__a("xc/xc_rho_set_types.F",646)
       IF(.NOT.(rho_r(1)%pw%in_use==REALDATA3D))CALL cp__a("xc/xc_rho_set_types.F",647)
       IF(.NOT.(rho_r(2)%pw%in_use==REALDATA3D))CALL cp__a("xc/xc_rho_set_types.F",648)
    CASE default
       CALL cp__b("xc/xc_rho_set_types.F",650,"")
    END SELECT
       ...
       ELSE
          ...
          CASE (xc_rho_nn50)
             ...
             CALL pw_nn_smear_r(pw_in=rho_r(ispin)%pw,&
                  ...
                  coeffs=nn50_coeffs)
          CASE default
             CALL cp__b("xc/xc_rho_set_types.F",706,"")
          END SELECT
       END IF
          ...
          IF (needs_rho_g) THEN
             ...
             CASE (xc_deriv_collocate)
                DO idir=1,3
                   ...
                   CALL pw_transfer ( tmp_g, drho_r(idir,ispin)%pw)
                END DO
               CALL cp__b("xc/xc_rho_set_types.F",777,"Drho collocation not implemented")
             CASE default
                CALL cp__b("xc/xc_rho_set_types.F",779,"")
             END SELECT
          ...
          ELSE
             ...
             CASE (xc_deriv_collocate)
                DO idir=1,3
                   ...
                   CALL pw_transfer ( tmp_g, drho_r(idir,ispin)%pw)
                END DO
               CALL cp__b("xc/xc_rho_set_types.F",822,"Drho collocation not implemented")
             CASE default
                CALL cp__b("xc/xc_rho_set_types.F",824,"")
             END SELECT
          END IF
    ...
    IF (needs%tau.OR.needs%tau_spin) THEN
       IF(.NOT.(ASSOCIATED(tau)))CALL cp__a("xc/xc_rho_set_types.F",1118)
       DO ispin=1,nspins
          IF(.NOT.(ASSOCIATED(tau(ispin)%pw)))CALL cp__a("xc/xc_rho_set_types.F",1120)
          IF(.NOT.(ASSOCIATED(tau(ispin)%pw%cr3d)))CALL cp__a("xc/xc_rho_set_types.F",1121)
       END DO
    END IF
    IF (needs%tau) THEN
       IF (nspins==2) THEN
    ...
    IF (needs%tau_spin) THEN
       IF(.NOT.(nspins==2))CALL cp__a("xc/xc_rho_set_types.F",1147)
    ...
    IF(.NOT.(xc_rho_cflags_equal(rho_set%has,needs)))CALL cp__a("xc/xc_rho_set_types.F",1154)
