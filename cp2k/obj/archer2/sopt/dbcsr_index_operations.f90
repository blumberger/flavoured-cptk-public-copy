# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_index_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_index_operations.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_index_operations.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (ASSOCIATED (matrix%index), dbcsr_fatal_level,&
         dbcsr_internal_error, routineP,&
         "Index must be preallocated.",502)
    CALL dbcsr_assert (UBOUND(matrix%index,1),'GE',dbcsr_num_slots,&
         dbcsr_failure_level, dbcsr_internal_error, routineP,&
         "Actual index size less than declared size",505)
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (ASSOCIATED (m%index),&
         ...
         "Index array does not yet exist.", 624)
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (stat, "EQ", 0, dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Could not allocate memory for sorting buffer.", 702)
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (stat, "EQ", 0, dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Can not allocate memory.", 763)
    ALLOCATE (new_col_i(nblks_max), stat=stat)
    CALL dbcsr_assert (stat, "EQ", 0, dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Can not allocate memory.", 766)
    ALLOCATE (new_blk_p(nblks_max), stat=stat)
    CALL dbcsr_assert (stat, "EQ", 0, dbcsr_fatal_level, dbcsr_internal_error,&
         routineN, "Can not allocate memory.", 769)
    !
...
!> \param[out] row_p Storage for new index
!> \param[out] col_i Storage for new index
!> \param[out] blk_p Storage for new index
...
!>                                            process column
!> \param row_blk_offsets ...
!> \param col_blk_offsets ...
...
!   ---------------------------------------------------------------------------
    ...
    ELSE
       ...
       CALL dbcsr_assert (blk, "EQ", nblks, dbcsr_fatal_level,&
            dbcsr_internal_error, routineN, "Block mismatch", 969)
...
!> \param local_row_offsets ...
!> \param local_col_offsets ...
!> \param[in,out] meta         Metadata updates for new index
!> \note Used for making matrices dense/undense
! *****************************************************************************
...
!> \param[in,out] new_row_i merged result
!> \param[in,out] new_col_i merged result
!> \param[in,out] new_blk_p merged result
!> \param[in] new_size                  size of merged index
!> \param[in,out] old_row_i current index
!> \param[in,out] old_col_i current index
!> \param[in,out] old_blk_p current index
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (stat, "EQ", 0,&
         dbcsr_fatal_level, dbcsr_internal_error, routineN,&
         "Could not allocate memory.", 1270)
    ...
    CALL dbcsr_assert (SIZE(local_rows), "EQ", nlocal_rows,&
         dbcsr_fatal_level, dbcsr_internal_error, routineN,&
         "Mismatch in the number of local rows.", 1280)
    ...
    IF (compact) THEN
       ...
       IF (careful_mod) THEN
          ...
          CALL dbcsr_assert (matrix%m%index(dbcsr_slot_size), "EQ", new_size,&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Unexpected index size.", 1608)
          CALL dbcsr_assert (SIZE(matrix%m%index), "EQ", new_size,&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Unexpected index size.", 1611)
       ENDIF
       CALL dbcsr_repoint_index (matrix%m)
    ENDIF
    CALL timestop (error_handle)
