# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_coordinate_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_coordinate_util.F"
!-----------------------------------------------------------------------------!
...
# 54 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_coordinate_util.F" 2
...
! *****************************************************************************
  SUBROUTINE topology_coordinate_pack (particle_set,atomic_kind_set,&
    ...
    LOGICAL :: autogen, check, disable_exclusion_lists, do_center, explicit, &
      found, my_ignore_outside_box, my_qmmm, present_12_excl_ei_list, &
    ...
    IF (ANY(kind_of==0)) THEN
       DO i = 1, topology%natoms
          ...
          END IF
       END DO
       CALL cp__b("topology_coordinate_util.F",277,"")
    END IF
    CALL timestop(handle2)
    ...
    IF(.NOT.my_ignore_outside_box .AND. .NOT. check)&
      CALL cp_abort(cp__l("topology_coordinate_util.F",347), "A non-periodic calculation has been requested but the system size "//&
    ...
    IF ((method_name_id == do_fist).AND.(.NOT.disable_exclusion_lists)) THEN
       IF(.NOT.(PRESENT(exclusions)))CALL cp__a("topology_coordinate_util.F",388)
...
! *****************************************************************************
  SUBROUTINE setup_exclusion_list(exclude_section, keyword, ex_bond_list,&
    ...
    IF(.NOT.(ASSOCIATED(ex_bond_list)))CALL cp__a("topology_coordinate_util.F",782)
    IF(.NOT.(ASSOCIATED(ex_bond_list_w)))CALL cp__a("topology_coordinate_util.F",783)
    SELECT CASE(keyword)
    CASE ("BOND")
       CALL section_vals_val_get(exclude_section,keyword,n_rep_val=n_rep)
       DO j=1, SIZE(ex_bond_list)
          IF(.NOT.(ASSOCIATED(ex_bond_list(j)%array1)))CALL cp__a("topology_coordinate_util.F",788)
          IF(.NOT.(ASSOCIATED(ex_bond_list_w(j)%array1)))CALL cp__a("topology_coordinate_util.F",789)
       ...
       END DO
    CASE DEFAULT
       CALL cp__b("topology_coordinate_util.F",812,"")
    END SELECT
