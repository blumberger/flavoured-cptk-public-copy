# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_restart_force_eval.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_restart_force_eval.F"
!-----------------------------------------------------------------------------!
...
# 61 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_restart_force_eval.F" 2
...
! *****************************************************************************
  SUBROUTINE update_qmmm(qmmm_section,force_env)
    ...
    CASE(use_qmmm)
       ...
       IF(.NOT.(explicit))CALL cp__a("input_restart_force_eval.F",171)
...
! *****************************************************************************
  SUBROUTINE section_coord_val_set(coord_section, particles, molecules, conv_factor,&
    ...
    IF(.NOT.(ASSOCIATED(coord_section)))CALL cp__a("input_restart_force_eval.F",489)
    IF(.NOT.(coord_section%ref_count>0))CALL cp__a("input_restart_force_eval.F",490)
    section => coord_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("input_restart_force_eval.F",494),&
...
! *****************************************************************************
  SUBROUTINE update_dipoles_section(dipoles, dipoles_section)
    ...
    CALL timeset(routineN,handle)
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(dipoles_section)))CALL cp__a("input_restart_force_eval.F",622)
    IF(.NOT.(dipoles_section%ref_count>0))CALL cp__a("input_restart_force_eval.F",623)
    section => dipoles_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("input_restart_force_eval.F",627),&
            "section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
...
! *****************************************************************************
  SUBROUTINE update_quadrupoles_section(quadrupoles, quadrupoles_section)
    ...
    CALL timeset(routineN,handle)
    NULLIFY(my_val, old_val, section, vals)
    IF(.NOT.(ASSOCIATED(quadrupoles_section)))CALL cp__a("input_restart_force_eval.F",701)
    IF(.NOT.(quadrupoles_section%ref_count>0))CALL cp__a("input_restart_force_eval.F",702)
    section => quadrupoles_section%section
    ik=section_get_keyword_index(section,"_DEFAULT_KEYWORD_")
    IF(ik==-2)&
       CALL cp_abort(cp__l("input_restart_force_eval.F",706),&
            "section "//TRIM(section%name)//" does not contain keyword "//&
            "_DEFAULT_KEYWORD_")
...
! *****************************************************************************
  SUBROUTINE dump_coordinates_cp2k(particles,molecules,cell,conv_factor,&
    ...
    IF(.NOT.(ASSOCIATED(particles)))CALL cp__a("input_restart_force_eval.F",799)
    IF (.NOT.core_or_shell) THEN
       IF(.NOT.(ASSOCIATED(molecules)))CALL cp__a("input_restart_force_eval.F",801)
    END IF
    IF (scaled_coordinates) THEN
       IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("input_restart_force_eval.F",804)
    END IF
