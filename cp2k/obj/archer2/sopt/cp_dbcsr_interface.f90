# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/../base/base_uses.f90" 1
...
# 105 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_norm(matrix, which_norm, norm_scalar, norm_vector)
    ...
    ELSE
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",1037,"Must pass either scalar or vector norm.")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_deallocate_matrix(matrix)
    ...
    IF(cp_dbcsr_valid_index(matrix))&
       CALL cp_abort(cp__l("dbcsrwrap/cp_dbcsr_interface.F",2223),&
...
!$       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",2524,"Too few threads.")
    IF(.NOT.dbcsr_wm_use_mutable(matrix%matrix%m%wms(ithread+1)))&
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",2526,"Data loss due to no conversion of appendable to mutable data")
...
!
    IF (dbg) THEN
       ...
       IF(p .NE. dbcsr_mp_mynode(dbcsr_distribution_mp(cp_dbcsr_distribution(matrix))))&
          CALL cp__w("dbcsrwrap/cp_dbcsr_interface.F",2539,"Adding non-local element")
    ENDIF
    IF(existed) CALL cp__w("dbcsrwrap/cp_dbcsr_interface.F",2541,"You should not add existing blocks according to old API.")
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface_s.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!> \retval encapsulated ...
! *****************************************************************************
  FUNCTION make_conformant_scalar_s (scalar, matrix) RESULT (encapsulated)
    REAL(kind=real_4), INTENT(IN)                      :: scalar
    TYPE(cp_dbcsr_type), INTENT(IN)          :: matrix

    CHARACTER(len=*), PARAMETER :: routineN = 'make_conformant_scalar_s', &
      routineP = moduleN//':'//routineN
    ...
    IF (scalar_data_type .EQ. dbcsr_type_complex_4 .OR.&
       ...
       IF(data_type .NE. dbcsr_type_complex_4 .AND. data_type .NE. dbcsr_type_complex_8)&
          CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",30,"Can not conform a complex to a real number")
    END IF
    CALL dbcsr_scalar_set_type (encapsulated,data_type)
  END FUNCTION make_conformant_scalar_s
...
! *****************************************************************************
  SUBROUTINE cp_iterator_next_2d_block_s (iterator, row, column,&
    ...
    CALL dbcsr_iterator_next_block (iterator, row, column,&
       ...
       block_number, row_size, col_size, row_offset, col_offset)
    IF(transposed)&
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",93,"CP2K does not handle transposed blocks.")
...
! *****************************************************************************
  SUBROUTINE cp_iterator_next_1d_block_s (iterator, row, column, block,&
    ...
    IF(transposed)&
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",127,"CP2K does not handle transposed blocks.")
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_get_2d_block_p_s (matrix,row,col,block,found,&
    ...
    CALL dbcsr_get_block_p(matrix%matrix,row,col,block,tr,found,&
         row_size, col_size)
    IF(tr)&
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",210,"CP2K does not handle transposed blocks.")
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_get_block_p_s (matrix,row,col,block,found,&
    ...
    CALL dbcsr_get_block_p(matrix%matrix,row,col,block,tr,found,&
       row_size, col_size)
    IF(tr)&
       CALL cp__b("dbcsrwrap/cp_dbcsr_interface.F",240,"CP2K does not handle transposed blocks.")
...
!> \param trace ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_trace_a_s (matrix_a, trace)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
...
!> \param local_sum ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_trace_ab_s (matrix_a, matrix_b, trace, trans_a, trans_b, local_sum)
...
!> \param side ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_scale_by_vector_s (matrix_a, alpha, side)
    TYPE(cp_dbcsr_type), INTENT(INOUT)        :: matrix_a
...
!> \param last_column ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_scale_s (matrix_a, alpha_scalar, last_column)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
...
!> \param alpha ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_set_s (matrix, alpha)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix
    REAL(kind=real_4), INTENT(IN)                      :: alpha
...
!> \param beta_scalar ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_add_s (matrix_a, matrix_b, alpha_scalar, beta_scalar)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
    TYPE(cp_dbcsr_type), INTENT(IN)          :: matrix_b
...
# 2572 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface_d.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 2573 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface_c.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!> \retval encapsulated ...
! *****************************************************************************
  FUNCTION make_conformant_scalar_c (scalar, matrix) RESULT (encapsulated)
...
!> \param trace ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_trace_a_c (matrix_a, trace)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
...
!> \param local_sum ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_trace_ab_c (matrix_a, matrix_b, trace, trans_a, trans_b, local_sum)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a, matrix_b
...
!> \param side ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_scale_by_vector_c (matrix_a, alpha, side)
    TYPE(cp_dbcsr_type), INTENT(INOUT)        :: matrix_a
...
!> \param last_column ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_scale_c (matrix_a, alpha_scalar, last_column)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
...
!> \param alpha ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_set_c (matrix, alpha)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix
    COMPLEX(kind=real_4), INTENT(IN)                      :: alpha
...
!> \param beta_scalar ...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_add_c (matrix_a, matrix_b, alpha_scalar, beta_scalar)
    TYPE(cp_dbcsr_type), INTENT(INOUT)       :: matrix_a
    TYPE(cp_dbcsr_type), INTENT(IN)          :: matrix_b
...
!> \retval DATA ...
! *****************************************************************************
  FUNCTION cp_dbcsr_get_data_c_c (matrix, index_matrix, select_data_type, lb, ub) RESULT (DATA)
    TYPE(cp_dbcsr_type), INTENT(IN)  :: matrix
    INTEGER, INTENT(IN)              :: index_matrix
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'cp_dbcsr_get_data_p_c', &
      routineP = moduleN//':'//routineN

    DATA => dbcsr_get_data_p(matrix%matrix%m%wms(index_matrix)%data_area,select_data_type,lb,ub)
  END FUNCTION cp_dbcsr_get_data_c_c
# 2574 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface_z.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 2575 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsrwrap/cp_dbcsr_interface.F" 2
