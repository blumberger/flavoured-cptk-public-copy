# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mulliken.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mulliken.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mulliken.F" 2
       ...
       DO ispin=1,nspin
          ...
          DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
             ...
             IF ( .NOT. (ASSOCIATED(s_block) .AND. ASSOCIATED(ks_block) ) ) THEN
                CALL cp__b("mulliken.F",122,"Unexpected s / ks structure")
             END IF
...
! *****************************************************************************
  SUBROUTINE mulliken_charges_a(p_matrix,s_matrix,para_env,particle_set, &
    ...
    IF(.NOT.(ASSOCIATED(p_matrix)))CALL cp__a("mulliken.F",358)
    IF(.NOT.(ASSOCIATED(s_matrix)))CALL cp__a("mulliken.F",359)
    ! here we get the numbers for charges
    nspin=SIZE(p_matrix)
    CALL cp_dbcsr_get_info(s_matrix,nblkrows_total=nblock)
    ALLOCATE(charges(nblock,nspin))
...
! *****************************************************************************
  SUBROUTINE mulliken_charges_akp(p_matrix_kp,s_matrix_kp,para_env,particle_set, &
    ...
    IF(.NOT.(ASSOCIATED(p_matrix_kp)))CALL cp__a("mulliken.F",471)
    IF(.NOT.(ASSOCIATED(s_matrix_kp)))CALL cp__a("mulliken.F",472)
