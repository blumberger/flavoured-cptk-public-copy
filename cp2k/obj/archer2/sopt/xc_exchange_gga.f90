# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_exchange_gga.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_exchange_gga.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_exchange_gga.F" 2
...
! *****************************************************************************
  SUBROUTINE xgga_info(functional,lsd,reference,shortform, needs, max_deriv)
    ...
    IF ( PRESENT ( reference ) ) THEN
       ...
       CASE default
          CALL cp__b("xc/xc_exchange_gga.F",97,"Invalid functional requested ("//cp_to_string(functional)//")")
       END SELECT
       IF (.not.lsd) THEN
    ...
    IF ( PRESENT ( shortform ) ) THEN
       ...
       CASE default
          CALL cp__b("xc/xc_exchange_gga.F",122,"Invalid functional requested ("//cp_to_string(functional)//")")
       END SELECT
       IF (.not.lsd) THEN
...
! *****************************************************************************
  SUBROUTINE xgga_eval(functional,lsd,rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_exchange_gga.F",187)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_exchange_gga.F",188)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_exchange_gga.F",189)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_exchange_gga.F",190)
    IF (lsd) THEN
    ...
    DO ispin=1,nspin
       ...
       CASE (xgga_ev93)
          ...
          IF (lsd) tact = 1.0_dp
       CASE DEFAULT
          CALL cp__b("xc/xc_exchange_gga.F",257,"")
       END SELECT
       ...
       END IF
       IF ( order>3.OR.order<-3) THEN
          CALL cp__b("xc/xc_exchange_gga.F",318,"derivatives bigger than 3 not implemented")
       END IF
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_b88 ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_exchange_gga.F",654,"Illegal order")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_pw86 ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_exchange_gga.F",730,"Illegal order")
      END SELECT
    END DO
...
!
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_exchange_gga.F",820,"Illegal order")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_optx ( s, fs, m )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_exchange_gga.F",876,"Illegal order")
      END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE efactor_pbex ( s, fs, m, pset )
    ...
    DO ip=1,SIZE(s)
      ...
      CASE DEFAULT
         CALL cp__b("xc/xc_exchange_gga.F",1126,"Illegal order")
      END SELECT
    END DO
