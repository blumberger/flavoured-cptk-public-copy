# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/tamc_run.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/tamc_run.F"
!-----------------------------------------------------------------------------!
...
# 138 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/tamc_run.F" 2
...
SUBROUTINE qs_tamc(force_env, globenv,averages)
    ...
    NULLIFY(md_env, para_env,fs_section, virial)
    ...
    IF(.NOT.(ASSOCIATED(globenv)))CALL cp__a("motion/mc/tamc_run.F",225)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/mc/tamc_run.F",226)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      IF(.NOT.(isos==1))CALL cp__a("motion/mc/tamc_run.F",328)
...
!!!!! this shoudl go in a routine hmc_read
      ...
      CALL section_vals_val_get(mc_section,"ENSEMBLE",&
           c_val=ensemble)
      IF(.NOT.(str_comp(ensemble,"TRADITIONAL")))CALL cp__a("motion/mc/tamc_run.F",346)
      CALL section_vals_val_get(mc_section,"NSTEP",&
           i_val=nmccycles)
      IF(.NOT.(nmccycles>0))CALL cp__a("motion/mc/tamc_run.F",349)
      CALL section_vals_val_get(mc_section,"IPRINT",&
           i_val=iprint)
      CALL section_vals_val_get(mc_section,"RANDOMTOSKIP",i_val=rand2skip)
      IF(.NOT.(rand2skip>=0))CALL cp__a("motion/mc/tamc_run.F",353)
...
!!!!!!!end mc setup
    ...
    IF ( (simpar%ensemble==npt_i_ensemble).OR.&
       ...
       IF(.NOT.check)&
           CALL cp_abort(cp__l("motion/mc/tamc_run.F",400),&
       ...
       END IF
       IF(.NOT.check)&
          CALL cp_abort(cp__l("motion/mc/tamc_run.F",414),&
...
!                    IF(.NOT.check) CALL cp_abort(cp__l("motion/mc/tamc_run.F",470),&
...
!             particles=particles)
       ...
       CALL virial_evaluate(atomic_kinds%els, particles%els, local_particles,&
            virial, force_env%para_env%group)
...
!     ! Do some checks on coordinates and box
     ...
     IF(.NOT.(ASSOCIATED(force_env%meta_env)))CALL cp__a("motion/mc/tamc_run.F",990)
     IF(.NOT.(force_env%meta_env%langevin))CALL cp__a("motion/mc/tamc_run.F",991)
...
! begin the timing of the subroutine
      ...
      NULLIFY(scf_env)
      CALL get_qs_env(force_env%qs_env,scf_env=scf_env,input=input)
...
! now run the MD simulation
      ...
      IF (.NOT. localise) THEN
        ...
        CALL scf_post_calculation_gpw(dft_section, scf_env, force_env%qs_env)
      ENDIF
