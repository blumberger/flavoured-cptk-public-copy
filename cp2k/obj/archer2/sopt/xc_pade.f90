# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_pade.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_pade.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_pade.F" 2
...
! *****************************************************************************
  SUBROUTINE pade_info ( reference, shortform, lsd, needs, max_deriv)
    ...
    IF (PRESENT(needs)) THEN
       IF (.NOT. PRESENT(lsd)) &
            CALL cp__b("xc/xc_pade.F",120,"Arguments mismatch.")
       IF (lsd) THEN
          needs%rho_spin = .TRUE.
       ELSE
          needs%rho = .TRUE.
...
! *****************************************************************************
  SUBROUTINE pade_lsd_3 ( rhoa, rhob, rs, fx, potaaa, potaab, potabb, potbbb )
    ...
    IF ( .NOT. debug_flag ) CALL cp__b("xc/xc_pade.F",750,"Routine not tested")
