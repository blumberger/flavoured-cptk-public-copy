# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/md_energies.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/md_energies.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/md_energies.F" 2
...
! *****************************************************************************
  SUBROUTINE initialize_md_ener(md_ener,force_env,simpar)
    ...
    IF(.NOT.(ASSOCIATED(md_ener)))CALL cp__a("motion/md_energies.F",126)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/md_energies.F",127)
...
! *****************************************************************************
  SUBROUTINE md_write_output(md_env)
    ...
    LOGICAL                                  :: init, is_mixed, new_file, &
                                                qmmm, shell_adiabatic, &
                                                shell_present
    ...
    IF ((.NOT.init).OR.(itimes==0).OR.simpar%ensemble==reftraj_ensemble) THEN
       ...
       ELSE
          ...
          IF(BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file))&
             CALL cp_warn(cp__l("motion/md_energies.F",491),&
       ...
       IF(shell_present) THEN
          ...
          IF(shell_adiabatic) THEN
             ...
             ELSE
                ...
                IF(BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file))&
                   CALL cp_warn(cp__l("motion/md_energies.F",561),&
