# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_kind_orbitals.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_kind_orbitals.F"
!-----------------------------------------------------------------------------!
...
MODULE atom_kind_orbitals
  ...
  USE atom_operators,                  ONLY: atom_int_release,&
                                             ...
                                             atom_relint_setup
  USE atom_types,                      ONLY: &
       CGTO_BASIS, Clementi_geobas, GTO_BASIS, atom_basis_type, &
       atom_integrals, atom_orbitals, atom_potential_type, atom_type, &
       create_atom_orbs, create_atom_type, release_atom_basis, &
  ...
  USE input_constants,                 ONLY: &
       ...
       do_dkh4_atom, do_dkh5_atom, do_gapw_log, do_nonrel_atom, do_numeric, &
       do_rks_atom, do_sczoramp_atom, do_zoramp_atom, gth_pseudo, no_pseudo, &
       poly_conf, rel_dkh, rel_none, rel_sczora_mp, rel_zora, rel_zora_full, &
       rel_zora_mp
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_kind_orbitals.F" 2
...
!> \param xc_section ...
! *****************************************************************************
  SUBROUTINE calculate_atomic_orbitals (atomic_kind,qs_kind,iunit,pmat,fmat,&
                                        ispin,confine,xc_section)
    TYPE(atomic_kind_type), INTENT(IN)       :: atomic_kind
    TYPE(qs_kind_type), INTENT(IN)           :: qs_kind
    INTEGER, INTENT(IN), OPTIONAL            :: iunit
    REAL(KIND=dp), DIMENSION(:, :), &
      OPTIONAL, POINTER                      :: pmat, fmat
    ...
    INTEGER :: i, ii, ipgf, j, k, k1, k2, l, ll, lm, m, mb, mo, n, ne, &
      nexp_lpot, nexp_lsd, nexp_nlcc, ngp, nj, nr, ns, nset, nsgf, quadtype, z
    INTEGER, DIMENSION(0:3, 10)              :: ncalc, ncore, nelem
    INTEGER, DIMENSION(0:3, 100)             :: set_index, shell_index
    INTEGER, DIMENSION(:), POINTER           :: lmax, lmin, nct_lpot, &
                                                nct_lsd, nct_nlcc, npgf, &
                                                nppnl, nshell, ppeconf
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgf, last_sgf, ls
    LOGICAL                                  :: lpot_present, lsd_present, &
                                                nlcc_present
...
!LOGICAL                                  :: bs_occupation
    ...
    !!RZK CALL get_qs_kind(qs_kind,&
    !!RZK                  bs_occupation=bs_occupation,&
    !!RZK                  addel=addel,laddel=laddel,naddel=naddel)

    IF(.NOT.(ASSOCIATED(orb_basis_set)))CALL cp__a("atom_kind_orbitals.F",145)
    ...
    IF ( ASSOCIATED(gth_potential) ) THEN
      ...
      IF (nlcc_present) THEN
        ...
        potential%gth_pot%alpha_nlcc(1:nexp_nlcc) = alpha_nlcc(1:nexp_nlcc)
        potential%gth_pot%nct_nlcc(1:nexp_nlcc) = nct_nlcc(1:nexp_nlcc)
    ...
    DO i=1,nset
      ...
      DO j=1,nshell(i)
        ...
        IF ( l <= 3 ) THEN
          ...
          k = basis%nbas(l)
          IF(.NOT.(k<=100))CALL cp__a("atom_kind_orbitals.F",306)
          set_index(l,k) = i
          shell_index(l,k) = j
        END IF
      END DO
    ...
    IF (PRESENT(fmat)) THEN
       ...
       IF(ispin/=1) CALL cp__b("atom_kind_orbitals.F",457,"calculate_atomic_orbitals: only ispin==1 is implemented")
       IF(ASSOCIATED(fmat)) CALL cp__b("atom_kind_orbitals.F",458,"fmat already associated")
       IF(.NOT.ASSOCIATED(atom%fmat)) CALL cp__b("atom_kind_orbitals.F",459,"atom%fmat not associated")
       ALLOCATE(fmat(nsgf,nsgf))
       fmat = 0.0_dp
       DO l=0,3
          ll = 2*l
          DO k1=1,atom%basis%nbas(l)
          DO k2=1,atom%basis%nbas(l)
             ...
             j=first_sgf(shell_index(l,k2),set_index(l,k2))
             DO m=0,ll
               fmat(i+m,j+m) = atom%fmat%op(k1,k2,l)/scal
             END DO
          END DO
          END DO
       ENDDO
...
! *****************************************************************************
  SUBROUTINE calculate_atomic_density (density,atomic_kind,qs_kind,ngto,iunit,allelectron,confine)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'calculate_atomic_density', &
      routineP = moduleN//':'//routineN
    INTEGER, PARAMETER                       :: num_gto = 40

    INTEGER                                  :: i, ii, j, k, l, ll, lm, m, &
                                                mb, mo, n, ne, nexp_lpot, &
                                                nexp_lsd, nexp_nlcc, ngp, nn, &
                                                nr, quadtype, relativistic, z
    INTEGER, DIMENSION(0:3)                  :: econfx, starti
    INTEGER, DIMENSION(0:3, 10)              :: ncalc, ncore, nelem
    INTEGER, DIMENSION(:), POINTER           :: econf, nct_lpot, nct_lsd, &
                                                nct_nlcc, nppnl, ppeconf
    LOGICAL                                  :: lpot_present, lsd_present, &
                                                nlcc_present
    REAL(KIND=dp)                            :: ac, al, aval, cc, cval, ear, &
                                                rk, xx, zeff
    REAL(KIND=dp), DIMENSION(:), POINTER     :: alpha_lpot, alpha_lsd, &
                                                alpha_nlcc, ap, ce
    ...
    IF(.NOT.(ngto<=num_gto))CALL cp__a("atom_kind_orbitals.F",557)
...
! *****************************************************************************
  SUBROUTINE calculate_atomic_relkin (atomic_kind,qs_kind,rel_control,rtmat)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'calculate_atomic_relkin', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: i, ii, ipgf, j, k, k1, k2, l, &
                                                ll, m, n, ngp, nj, nn, nr, &
                                                ns, nset, nsgf, quadtype, &
                                                relativistic, z
    INTEGER, DIMENSION(0:3, 10)              :: ncalc, ncore, nelem
    INTEGER, DIMENSION(0:3, 100)             :: set_index, shell_index
    INTEGER, DIMENSION(:), POINTER           :: lmax, lmin, npgf, nshell
    INTEGER, DIMENSION(:, :), POINTER        :: first_sgf, last_sgf, ls
    REAL(KIND=dp)                            :: al, alpha, ear, prefac, rk, &
                                                zeff
    ...
    IF(.NOT.(ASSOCIATED(orb_basis_set)))CALL cp__a("atom_kind_orbitals.F",955)
    ...
    IF ( ASSOCIATED(all_potential) ) THEN
       ...
       SELECT CASE (rel_control%rel_method)
          CASE DEFAULT
             CALL cp__b("atom_kind_orbitals.F",972,"")
          CASE (rel_dkh)
             SELECT CASE (rel_control%rel_DKH_order)
                CASE DEFAULT
                   CALL cp__b("atom_kind_orbitals.F",976,"")
                CASE (0)
                ...
                CASE (4)
                   relativistic = do_dkh4_atom
                CASE (5)
                   relativistic = do_dkh5_atom
             END SELECT
          CASE (rel_zora)
             SELECT CASE (rel_control%rel_zora_type)
                CASE DEFAULT
                   CALL cp__b("atom_kind_orbitals.F",993,"")
                CASE (rel_zora_full)
                   CALL cp__b("atom_kind_orbitals.F",995,"")
                CASE (rel_zora_mp)
       ...
       DO i=1,nset
         ...
         DO j=1,nshell(i)
           ...
           IF ( l <= 3 ) THEN
             basis%nbas(l) = basis%nbas(l)+1
             k = basis%nbas(l)
             IF(.NOT.(k<=100))CALL cp__a("atom_kind_orbitals.F",1045)
             set_index(l,k) = i
             shell_index(l,k) = j
           END IF
         END DO
          ...
          DO k1=1,basis%nbas(l)
             DO k2=1,basis%nbas(l)
                ...
                SELECT CASE (atom%relativistic)
                   CASE DEFAULT
                      CALL cp__b("atom_kind_orbitals.F",1215,"")
                   ...
                   CASE (do_dkh0_atom,do_dkh1_atom,do_dkh2_atom,do_dkh3_atom,do_dkh4_atom,do_dkh5_atom)
                      DO m=0,ll
                         rtmat(i+m,j+m) = integrals%hdkh(k1,k2,l)-integrals%kin(k1,k2,l)+atom%zcore*integrals%core(k1,k2,l)
                      END DO
                END SELECT
             END DO
          END DO
