# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewalds_multipole_debug.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewalds_multipole_debug.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewalds_multipole_debug.F" 2
  IMPLICIT NONE
...
! *****************************************************************************
  SUBROUTINE create_multi_type(multipoles, idim, istart, iend, label, echarge,&
    ...
    IF (ASSOCIATED(multipoles)) THEN
       IF(.NOT.(SIZE(multipoles)==idim))CALL cp__a("ewalds_multipole_debug.F",484)
    ELSE
    ...
    DO i = istart, iend
       ...
       SELECT CASE(label)
       CASE("CHARGE")
          IF(.NOT.(PRESENT(charges)))CALL cp__a("ewalds_multipole_debug.F",501)
          IF(.NOT.(ASSOCIATED(charges)))CALL cp__a("ewalds_multipole_debug.F",502)
       ...
       CASE("DIPOLE")
          dx = 1.0E-4_dp
          IF(.NOT.(PRESENT(dipoles)))CALL cp__a("ewalds_multipole_debug.F",511)
          IF(.NOT.(ASSOCIATED(dipoles)))CALL cp__a("ewalds_multipole_debug.F",512)
       ...
       CASE("QUADRUPOLE")
          dx = 1.0E-2_dp
          IF(.NOT.(PRESENT(quadrupoles)))CALL cp__a("ewalds_multipole_debug.F",525)
          IF(.NOT.(ASSOCIATED(quadrupoles)))CALL cp__a("ewalds_multipole_debug.F",526)
...
! *****************************************************************************
  SUBROUTINE reallocate_charge_type(charge_typ, istart, iend)
    ...
    INTEGER                                  :: i, isize, jsize, jsize1, jsize2, j
        TYPE(charge_mono_type), DIMENSION(:),&
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 698 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewalds_multipole_debug.F" 2
  IMPLICIT NONE
  TYPE(ewald_environment_type), POINTER    :: ewald_env
  TYPE(ewald_pw_type), POINTER             :: ewald_pw
  TYPE(fist_nonbond_env_type), POINTER     :: nonbond_env
  ...
  REAL(KIND=dp)                            :: dq, energy_local, energy_glob, e_neut,&
  ...
  INTEGER :: nparticles,i,k,j,nparticle_local, iparticle_kind
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 903 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewalds_multipole_debug.F" 2
  IMPLICIT NONE
  TYPE(ewald_environment_type), POINTER    :: ewald_env
  TYPE(ewald_pw_type), POINTER             :: ewald_pw
  TYPE(fist_nonbond_env_type), POINTER     :: nonbond_env
  ...
  REAL(KIND=dp), POINTER, DIMENSION(:,:)   :: ldipoles
  REAL(KIND=dp), POINTER, DIMENSION(:,:)   :: lquadrupoles
  REAL(KIND=dp)                            :: energy_local, energy_glob, e_neut,&
       e_self, pv_local(3,3), pv_glob(3,3), tot_ene, o_tot_ene, prod
  REAL(KIND=dp), DIMENSION(:,:), ALLOCATABLE&
                                           :: forces_local, forces_glob, efield1, efield2
