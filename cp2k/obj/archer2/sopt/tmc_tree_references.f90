# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_references.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_references.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_references.F" 2
...
! *****************************************************************************
  SUBROUTINE add_to_references(gt_elem)
    ...
    IF(.NOT.(ASSOCIATED(gt_elem)))CALL cp__a("tmc/tmc_tree_references.F",57)
...
! *****************************************************************************
  SUBROUTINE remove_gt_references(gt_ptr, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(gt_ptr)))CALL cp__a("tmc/tmc_tree_references.F",105)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_references.F",106)
...
! *****************************************************************************
  SUBROUTINE remove_subtree_element_of_all_references(ptr)
    ...
    CHARACTER(LEN=*), PARAMETER :: &
      routineN = 'remove_subtree_element_of_all_references', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_references.F",142)
    ...
    pt_node_ref_loop: DO WHILE(ASSOCIATED(ptr%gt_nodes_references))
      ...
      IF(.NOT.(ASSOCIATED(tmp_gt_list_ptr%gt_elem)))CALL cp__a("tmc/tmc_tree_references.F",149)
      CALL cp_abort(cp__l("tmc/tmc_tree_references.F",150),&
      ...
      IF(ASSOCIATED(tmp_gt_list_ptr%gt_elem%conf(tmp_gt_list_ptr%gt_elem%mv_conf)%elem)) THEN
        ...
        ELSE
          ...
          END DO
          CALL cp_warn(cp__l("tmc/tmc_tree_references.F",168),&
               "for subtree "//&
        ...
        END IF
      ELSE
        CALL cp_warn(cp__l("tmc/tmc_tree_references.F",179),&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ptr%gt_nodes_references)))CALL cp__a("tmc/tmc_tree_references.F",192)
...
! *****************************************************************************
  SUBROUTINE search_and_remove_reference_in_list(gt_ptr, elem, tmc_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: &
      routineN = 'search_and_remove_reference_in_list', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: handle
    TYPE(gt_elem_list_type), POINTER         :: tmp_gt_list_last_ptr, &
                                                tmp_gt_list_ptr
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_references.F",222)
