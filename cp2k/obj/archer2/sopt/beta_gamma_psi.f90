# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/beta_gamma_psi.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/beta_gamma_psi.F"
!-----------------------------------------------------------------------------!
...
# 12 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/beta_gamma_psi.F" 2
...
SELECT CASE(i)
  ...
  CASE DEFAULT
    CALL cp__b("beta_gamma_psi.F",102,"unknown case")
END SELECT
...
!             LIMIT VALUE FOR LARGE NEGATIVE X
  ...
  RETURN
ENDIF
IF (ind==0.AND.(x > 100.0e0_dp.OR.x*x > -dxparg(1))) THEN
...
ELSEIF (a <= 2.25e0_dp) THEN
  t = (a - 0.5e0_dp) - 0.5e0_dp
  fn_val = gamln1(t)
ELSEIF (a < 10.0e0_dp) THEN
  n = INT(a - 1.25e0_dp)
  t = a
  w = 1.0e0_dp
  DO i = 1, n
...
IF (ierr1 > 0) CALL cp__b("beta_gamma_psi.F",1568,"Error in BGRAT")
w = 0.5e0_dp + (0.5e0_dp - w1)
GO TO 220
