# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_sccs.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_sccs.F"
!-----------------------------------------------------------------------------!
...
# 99 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_sccs.F" 2
...
! *****************************************************************************
  ...
  SUBROUTINE sccs(qs_env,rho_tot_gspace,v_hartree_gspace,v_sccs,h_stress)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_sccs.F",218)
    IF(.NOT.(ASSOCIATED(rho_tot_gspace)))CALL cp__a("qs_sccs.F",219)
    IF(.NOT.(ASSOCIATED(v_hartree_gspace)))CALL cp__a("qs_sccs.F",220)
    IF(.NOT.(ASSOCIATED(v_sccs)))CALL cp__a("qs_sccs.F",221)

    IF (PRESENT(h_stress)) THEN
      calculate_stress_tensor = .TRUE.
      h_stress(:,:) = 0.0_dp
      CALL cp__b("qs_sccs.F",226,"No stress tensor is implemented for SCCS models yet")
    ELSE
    ...
    IF(.NOT.(ASSOCIATED(rho_r_sccs)))CALL cp__a("qs_sccs.F",299)
    ...
    CASE DEFAULT
      CALL cp__b("qs_sccs.F",322,"Invalid method specified for SCCS model")
    END SELECT
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,TRIM(cube_path)),&
      ...
      CALL cp_print_key_finished_output(cube_unit,logger,input,TRIM(cube_path))
    END IF
    ...
    IF (should_output.AND.(ABS(eps0 - 1.0_dp) > tol)) THEN
      ...
      CASE (sccs_fattebert_gygi)
        CALL surface_fattebert_gygi(rho_elec,norm_drho_elec,theta,eps0,&
                                    ...
                                    sccs_control%delta_rho)
      CASE DEFAULT
        CALL cp__b("qs_sccs.F",434,"Invalid method specified for SCCS model")
      END SELECT
      ...
      IF (sccs_control%gamma_solvent > 0.0_dp) THEN
        ...
        CALL cp_warn(cp__l("qs_sccs.F",441),&
      ...
      DO j=lb(2),ub(2)
        DO i=lb(1),ub(1)
          IF (eps_elec%cr3d(i,j,k) < 1.0E-12_dp) THEN
            ...
            CALL cp__b("qs_sccs.F",517,message)
          END IF
...
! *****************************************************************************
  SUBROUTINE derive(f,df,method,pw_env,input,para_env)
    ...
    IF(.NOT.(ASSOCIATED(f)))CALL cp__a("qs_sccs.F",982)
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_sccs.F",983)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("qs_sccs.F",984)
...
!MK    CALL rs_grid_print(rs_grid,6)
    ...
    CASE (sccs_derivative_fft)
      ...
      END DO
    CASE DEFAULT
      CALL cp__b("qs_sccs.F",1040,"Invalid derivative method for SCCS specified")
    END SELECT
