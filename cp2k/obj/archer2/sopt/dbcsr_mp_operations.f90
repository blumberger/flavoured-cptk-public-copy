# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error,&
            routineN, "Invalid data type",123)
    END SELECT
...
! *****************************************************************************
!> \brief sendrecv of encapsulated data.
!> \param msgin ...
!> \param dest ...
!> \param msgout ...
!> \param source ...
...
! *****************************************************************************
  SUBROUTINE dbcsr_sendrecv_any(msgin,dest,msgout,source,comm)
    ...
    CALL dbcsr_assert (dbcsr_data_get_type (msgin).EQ.&
         dbcsr_data_get_type (msgout),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Different data type for msgin and msgout", 406)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 419)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_isend_any(msgin,dest,comm,request,tag)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 452)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_irecv_any(msgin,source,comm,request,tag)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 485)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_win_create_any(base,comm,win)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 514)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_rget_any(base,source,win,win_data,disp,request)
    ...
    CALL dbcsr_assert (dbcsr_data_get_type (base), "EQ", dbcsr_data_get_type(win_data),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Mismatch data type between buffer and window", 540)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 553)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_allgatherv(send_data, recv_data, recv_count, recv_displ, gid)
    ...
    CALL dbcsr_assert (dbcsr_data_get_type (send_data), "EQ",&
         ...
         dbcsr_wrong_args_error, routineN, "Data type mismatch", 577)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,&
            routineN, "Invalid data type", 593)
    END SELECT
  END SUBROUTINE dbcsr_allgatherv



# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations_d.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
! *****************************************************************************
  SUBROUTINE hybrid_alltoall_d1 (sb, scount, sdispl,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'hybrid_alltoall_d', &
      routineP = moduleN//':'//routineN
    ...
    LOGICAL                                  :: most_collective, &
                                                remainder_collective, no_h
    REAL(kind=real_8), DIMENSION(:), POINTER           :: send_data_p, recv_data_p
    TYPE(dbcsr_mp_obj)                       :: mpe
...
# 599 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations_z.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 600 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations_s.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!>      see mp_alltoall 
! *****************************************************************************
  SUBROUTINE hybrid_alltoall_s1 (sb, scount, sdispl,&
       rb, rcount, rdispl, mp_env, most_ptp, remainder_ptp, no_hybrid)
...
# 601 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations_c.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
! *****************************************************************************
  SUBROUTINE hybrid_alltoall_c1 (sb, scount, sdispl,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'hybrid_alltoall_c', &
      routineP = moduleN//':'//routineN
    ...
    LOGICAL                                  :: most_collective, &
                                                remainder_collective, no_h
    COMPLEX(kind=real_4), DIMENSION(:), POINTER           :: send_data_p, recv_data_p
    TYPE(dbcsr_mp_obj)                       :: mpe
...
! *****************************************************************************
    ...
    INTEGER                                  :: col, row
      ...
      ENDDO
    END SUBROUTINE remainder_point_to_point
  END SUBROUTINE hybrid_alltoall_c1
# 602 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_mp_operations.F" 2
