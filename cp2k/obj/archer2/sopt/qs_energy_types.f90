# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_energy_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_energy_types.F"
!-----------------------------------------------------------------------------!
...
# 20 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_energy_types.F" 2
...
! *****************************************************************************
  SUBROUTINE allocate_qs_energy(qs_energy)
    ...
    IF (.NOT.ASSOCIATED(qs_energy)) THEN
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,102,&
                                       "qs_energy",int_size)
...
! *****************************************************************************
  SUBROUTINE deallocate_qs_energy(qs_energy)
    ...
    ELSE
      CALL cp_abort(cp__l("qs_energy_types.F",131),&
  ...
  SUBROUTINE init_qs_energy(qs_energy)
    ...
    IF (ASSOCIATED(qs_energy)) THEN
      ...
      IF (.NOT.ASSOCIATED(qs_energy%ddapc_restraint)) THEN
        ...
        IF (istat /= 0) THEN
          CALL stop_memory(routineN,moduleN,195,&
                           "qs_energy%ddapc_restraint",dp_size)
        END IF
      END IF
      qs_energy%ddapc_restraint(:) = 0.0_dp
    ELSE
      CALL cp_abort(cp__l("qs_energy_types.F",201),&
           "The qs_energy pointer is not associated "//&
