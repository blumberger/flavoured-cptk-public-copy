# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_site_energies_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_site_energies_methods.F"
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_site_energies_methods.F" 2
...
SUBROUTINE single_phase_energies_forces(adiab_section,energies_1_mol_list, energies, &
   ...
   IF (total_states .ne. nadiab) THEN 
         CALL cp__b("sh_site_energies_methods.F",262,"NUMBER_DIABATIC_STATES different from total n states")
   END IF
...
SUBROUTINE multi_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
   ...
   do i=2,SIZE(donor_index)
      if (donor_index(i) .le. ind1) THEN 
        CALL cp__b("sh_site_energies_methods.F",475,"Donor mol must be ascending")
      end if
   end do
   ind1 = acceptor_index(1) 
   do i=2,SIZE(acceptor_index)
      if (acceptor_index(i) .le. ind1) THEN 
        CALL cp__b("sh_site_energies_methods.F",481,"Acceptor mol must be ascending")
      end if
   end do
...
!print *, "intra_forces_info(2,i)", intra_forces_info(2,:)   
    ...
    if (fe_block_keyword) then
        ...
        do i=2,SIZE(exc_index)
            if (exc_index(i) .le. ind1) THEN 
                CALL cp__b("sh_site_energies_methods.F",555,"EXC mol must be ascending")
            end if
        end do
    end if 
   ...
   IF (total_states .ne. nadiab) THEN 
         CALL cp__b("sh_site_energies_methods.F",574,"NUMBER_DIABATIC_STATES different from total n states")
   END IF
...
SUBROUTINE ct_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
   ...
   total_states = SIZE(donor_index)*SIZE(acceptor_index)
   IF (total_states .ne. nadiab) THEN 
      CALL cp__b("sh_site_energies_methods.F",798,"NUMBER_DIABATIC_STATES different from total n states")
   END IF
   ind1 = donor_index(1) 
   do i=2,SIZE(donor_index)
      if (donor_index(i) .le. ind1) THEN 
        CALL cp__b("sh_site_energies_methods.F",803,"Donor mol must be ascending")
      end if
   end do
   ind1 = acceptor_index(1) 
   do i=2,SIZE(acceptor_index)
      if (acceptor_index(i) .le. ind1) THEN 
        CALL cp__b("sh_site_energies_methods.F",809,"Acceptor mol must be ascending")
      end if
   end do
...
!          my_global_forces(i)%forces = global_forces(i)%forces
        ENDDO
    ELSE
        CALL cp__b("sh_site_energies_methods.F",1258,"Cannot do speed-up intra without speedup-lj")
    ENDIF
