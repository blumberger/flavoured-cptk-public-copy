# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/eri_mme_methods_low.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/eri_mme_methods_low.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/eri_mme_methods_low.F" 2
...
! *****************************************************************************
  SUBROUTINE eri_mme_2c_integrate_low(param, la_min, la_max, lb_min, lb_max, zeta, zetb, rab, &
    ...
    IF(.NOT.(param%is_valid))CALL cp__a("eri_mme_methods_low.F",106)
    ...
    IF (PRESENT(lattice_space)) THEN
      IF(.NOT.(lattice_space .GE. 0 .AND. lattice_space .LE. 2))CALL cp__a("eri_mme_methods_low.F",148)
...
! *****************************************************************************
  SUBROUTINE create_hermite_to_cartesian(zet, h_to_c)
    ...
    IF(.NOT.((UBOUND(h_to_c, 2) .EQ. l_max)))CALL cp__a("eri_mme_methods_low.F",618)
...
! *****************************************************************************
  SUBROUTINE eri_mme_set_params_low(param, cell, zet_err_minimax, zet_err_cutoff, l_err_cutoff, lmax, para_env)
    ...
    IF(.NOT.(ASSOCIATED(param%mme_section)))CALL cp__a("eri_mme_methods_low.F",694)
    IF(.NOT.(PRESENT(l_err_cutoff) .OR. s_only))CALL cp__a("eri_mme_methods_low.F",695)

    IF (PRESENT(l_err_cutoff)) THEN
      IF(.NOT.(SIZE(zet_err_cutoff) .EQ. SIZE(l_err_cutoff)))CALL cp__a("eri_mme_methods_low.F",698)
    ENDIF
    ...
    IF(calib_failed) THEN
      CALL cp_warn(cp__l("eri_mme_methods_low.F",766),"GR-switch calibration FAILED. "//&
...
! *****************************************************************************
  SUBROUTINE calibrate_cutoff(param, hmat, h_inv, G_min, vol, zet_mm, l_mm, zet_c, l_c, &
    ...
    unit_nr = cp_print_key_unit_nr(logger, param%mme_section, "ERI_MME_INFO", &
                                   extension = ".eri_mme")
    IF(.NOT.(SIZE(l_c) .EQ. SIZE(zet_c)))CALL cp__a("eri_mme_methods_low.F",920)
    ...
    IF((cutoff_r - cutoff_l)/(0.5_dp*(cutoff_r + cutoff_l)) .LE. tol)&
       CALL cp_abort(cp__l("eri_mme_methods_low.F",936), "difference of boundaries for cutoff "//&
            "(MAX - MIN) must be greater than cutoff precision.")

    IF((delta .GE. 1.0_dp) .OR. (delta.LE.0.0_dp))&
       CALL cp__b("eri_mme_methods_low.F",940,"relative delta to modify initial cutoff interval (DELTA) must be in (0, 1)")
    ...
    DO iter1 = 1, max_iter + 1
      IF(iter1.GT.max_iter)&
         CALL cp_abort(cp__l("eri_mme_methods_low.F",957), "Maximum number of iterations in bisection to determine initial "//&
    ...
    ELSE
      ...
      DO iter2 = 1, max_iter + 1
        IF(iter2.GT.max_iter)&
           CALL cp__b("eri_mme_methods_low.F",997,"Maximum number of iterations in bisection to determine cutoff has been exceeded")
...
! parameters for finding exponent maximizing cutoff error
    ...
    IF(.NOT.(SIZE(l_ctff) .EQ. SIZE(zet_ctff)))CALL cp__a("eri_mme_methods_low.F",1089)
    ...
    DO n = 1, SIZE(l_ctff)
      ...
      DO iter = 1, max_iter + 1
        ...
        IF(iter.GT.max_iter)&
           CALL cp_abort(cp__l("eri_mme_methods_low.F",1140), "Maximum number of iterations for finding "//&
...
! *****************************************************************************
  SUBROUTINE get_minimax_coeff_v_gspace(n_minimax, cutoff, G_min, minimax_aw, err_minimax, min_cutoff)
    ...
    G_max = SQRT(3.0_dp*2.0_dp*cutoff)
    IF(.NOT.(G_max .GT. G_min))CALL cp__a("eri_mme_methods_low.F",1359)

    CALL check_range(n_minimax, minimax_Rc, ierr)
    IF(.NOT.(ierr == 0))CALL cp__a("eri_mme_methods_low.F",1362)
    CALL get_minimax_coeff(n_minimax, minimax_Rc, minimax_aw, ierr)
    IF(.NOT.(ierr == 0))CALL cp__a("eri_mme_methods_low.F",1364)
    minimax_aw = minimax_aw/G_min**2

    IF(.NOT.(ALL(minimax_aw(:) .GT. 0)))CALL cp__a("eri_mme_methods_low.F",1367)
...
! *****************************************************************************
  SUBROUTINE calibrate_gr_switch(alpha_c, hmat, h_inv, lmax, failed, para_env)
    ...
    IF(.NOT.(SIZE(alpha_c, 2) .EQ. 3))CALL cp__a("eri_mme_methods_low.F",1436)
    alpha_c = 0.0_dp
      ...
      DO l = 0, lmax
        ...
        DO iter = 1, n_start + 1
          IF (iter .GT. n_start) THEN
            failed = .TRUE.
            CALL cp__w("eri_mme_methods_low.F",1458,"GR-switch calibration FAILED: No valid initial interval was found.")
            EXIT outerloop
          ENDIF
          ...
          IF(dt_R - dt_G .GT. 0.0_dp) THEN
            IF(iter .EQ. 1) THEN
              CALL cp__w("eri_mme_methods_low.F",1482,"GR-switch calibration FAILED: No valid initial interval was found.")
              EXIT outerloop
            ENDIF
        ...
        DO iter = 1, max_iter + 1
          IF (iter .GT. max_iter) THEN
            failed = .TRUE.
            CALL cp__w("eri_mme_methods_low.F",1495,"GR-switch calibration FAILED: Maximum number of iterations has been exceeded")
            EXIT outerloop
          ENDIF
