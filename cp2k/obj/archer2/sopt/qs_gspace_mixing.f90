# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_gspace_mixing.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_gspace_mixing.F"
!-----------------------------------------------------------------------------!
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_gspace_mixing.F" 2
...
! *****************************************************************************
  SUBROUTINE self_consistency_check(rho_ao,p_delta,para_env,p_out,delta)
    ...
    IF(.NOT.(ASSOCIATED(p_out)))CALL cp__a("qs_gspace_mixing.F",95)
    NULLIFY(matrix_q, p_in)
    p_in => rho_ao
    matrix_q => p_delta
    nspins = SIZE(p_in,1)
...
! *****************************************************************************
  SUBROUTINE gspace_mixing(qs_env, mixing_method, mixing_store, rho, para_env, iter_count)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store)))CALL cp__a("qs_gspace_mixing.F",158)
    ...
    IF(.NOT.((ng == SIZE(mixing_store%rhoin(1)%cc))))CALL cp__a("qs_gspace_mixing.F",166)
...
!          mixing_store%rhoin_buffer(1,ispin)%cc(ig) = rho_g(ispin)%pw%cc(ig)
    ...
    ELSE
       ...
       ELSEIF(mixing_method==pulay_mixing_nr) THEN
          IF(.NOT.(.NOT. gapw))CALL cp__a("qs_gspace_mixing.F",226)
       ...
       ELSEIF(mixing_method==broyden_mixing_new_nr) THEN
          IF(.NOT.(.NOT. gapw))CALL cp__a("qs_gspace_mixing.F",233)
          CALL broyden_mixing_new(mixing_store, rho, para_env)
          mixing_store%iter_method= "Broy."
       ELSEIF(mixing_method==multisecant_mixing_nr) THEN
          IF(.NOT.(.NOT. gapw))CALL cp__a("qs_gspace_mixing.F",237)
...
! *****************************************************************************
  ...
  SUBROUTINE gmix_potential_only (qs_env,mixing_store, rho)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store%rhoin)))CALL cp__a("qs_gspace_mixing.F",295)
    IF(.NOT.(ASSOCIATED(mixing_store%kerker_factor)))CALL cp__a("qs_gspace_mixing.F",296)
...
! *****************************************************************************
  ...
  SUBROUTINE pulay_mixing(mixing_store, rho, para_env)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store%res_buffer)))CALL cp__a("qs_gspace_mixing.F",379)
    IF(.NOT.(ASSOCIATED(mixing_store%rhoin_buffer)))CALL cp__a("qs_gspace_mixing.F",380)
    NULLIFY(rho_g)
    CALL timeset(routineN,handle)
...
! *****************************************************************************
  ...
  SUBROUTINE broyden_mixing(qs_env,mixing_store, rho, para_env)
    ...
    INTEGER                                  :: handle, i, iatom, ib, ibb, &
                                                ig, ispin, j, jb, kb, n1, n2, &
                                                natom, nb, nbuffer, ng, nspin
    LOGICAL                                  :: gapw
    ...
    IF(.NOT.(ASSOCIATED(mixing_store%res_buffer)))CALL cp__a("qs_gspace_mixing.F",555)
    IF(.NOT.(ASSOCIATED(mixing_store%rhoin)))CALL cp__a("qs_gspace_mixing.F",556)
    IF(.NOT.(ASSOCIATED(mixing_store%rhoin_old)))CALL cp__a("qs_gspace_mixing.F",557)
    IF(.NOT.(ASSOCIATED(mixing_store%drho_buffer)))CALL cp__a("qs_gspace_mixing.F",558)
...
!dbg
    ...
    ALLOCATE(a(ib-1,ib-1))
    a=0.0_dp
    ALLOCATE(b(ib-1,ib-1))
    b=0.0_dp
    ALLOCATE(c(ib-1))
    c=0.0_dp
    ALLOCATE(g(ib-1))
...
! *****************************************************************************
 SUBROUTINE broyden_mixing_new(mixing_store, rho, para_env)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store%rhoin_buffer)))CALL cp__a("qs_gspace_mixing.F",836)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store%delta_res)))CALL cp__a("qs_gspace_mixing.F",878)
    delta_res => mixing_store%delta_res
    IF(.NOT.(ASSOCIATED(mixing_store%u_vec)))CALL cp__a("qs_gspace_mixing.F",880)
    u_vec => mixing_store%u_vec
    IF(.NOT.(ASSOCIATED(mixing_store%z_vec)))CALL cp__a("qs_gspace_mixing.F",882)
...
! *****************************************************************************
 SUBROUTINE multisecant_mixing(mixing_store,rho,para_env)
    ...
    IF(.NOT.(ASSOCIATED(mixing_store)))CALL cp__a("qs_gspace_mixing.F",1251)
...
!   use_zgemm_rev = .FALSE.
    ...
    IF(.NOT.(rho_g(1)%pw%pw_grid%ngpts<HUGE(ng_global)))CALL cp__a("qs_gspace_mixing.F",1266)
    ...
    IF (PRESENT(p_delta)) THEN
      IF (mixing_method>=gspace_mixing_nr) THEN
         IF(.NOT.ASSOCIATED(p_delta)) THEN
            ...
            ENDDO
         END IF
         IF(.NOT.(ASSOCIATED(mixing_store)))CALL cp__a("qs_gspace_mixing.F",1670)
      END IF
    END IF
...
! *****************************************************************************
 SUBROUTINE mixing_init(mixing_method,rho,mixing_store,para_env,rho_atom)
    ...
    INTEGER                                  :: handle, iat, ib, ig, ig1, &
                                                ig_count, iproc, ispin, n1, &
                                                n2, natom, nbuffer, ng, nspin
