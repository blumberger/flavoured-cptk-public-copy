# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_utils.F"
!-----------------------------------------------------------------------------!
...
# 39 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE initialize_se_taper(se_taper, coulomb, exchange, lr_corr)
    ...
    LOGICAL                                  :: check, l_coulomb, l_exchange, &
                                                l_lrc

    check = .NOT.ASSOCIATED(se_taper%taper)
    IF(.NOT.(check))CALL cp__a("semi_empirical_utils.F",180)
    l_coulomb  = .FALSE.
    l_exchange = .FALSE.
    l_lrc      = .FALSE.
    ...
    IF (l_coulomb)  THEN
       ...
       IF(.NOT.(check))CALL cp__a("semi_empirical_utils.F",189)
       se_taper%taper => se_taper%taper_cou
    END IF
    IF (l_exchange) THEN
       check = (.NOT.l_coulomb).AND.(.NOT.l_lrc)
       IF(.NOT.(check))CALL cp__a("semi_empirical_utils.F",194)
       se_taper%taper => se_taper%taper_exc
    END IF
    IF (l_lrc)      THEN
       check = (.NOT.l_coulomb).AND.(.NOT.l_exchange)
       IF(.NOT.(check))CALL cp__a("semi_empirical_utils.F",199)
...
! *****************************************************************************
  SUBROUTINE finalize_se_taper(se_taper)
    ...
    IF(.NOT.(check))CALL cp__a("semi_empirical_utils.F",218)
...
! *****************************************************************************
  SUBROUTINE init_se_param(sep,orb_basis_set,ngauss)
    ...
    IF (ASSOCIATED(sep)) THEN
       ...
       DEALLOCATE (symbol)
    ELSE
       CALL cp__b("semi_empirical_utils.F",276,"The pointer sep is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE se_param_set_default(sep,z,method)
    ...
    IF (ASSOCIATED(sep)) THEN
       IF (z<0) THEN
          CALL cp__b("semi_empirical_utils.F",297,"Atomic number < 0")
       END IF
       SELECT CASE (method)
       CASE (do_method_am1)
       ...
       CASE DEFAULT
          CALL cp__b("semi_empirical_utils.F",319,"Semiempirical method unknown")
       END SELECT
    ELSE
       CALL cp__b("semi_empirical_utils.F",322,"The pointer sep is not associated")
    END IF

    ! Check if the element has been defined..
    IF(.NOT.sep%defined)&
       CALL cp_abort(cp__l("semi_empirical_utils.F",327),&
