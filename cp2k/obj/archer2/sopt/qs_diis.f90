# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_diis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_diis.F"
!-----------------------------------------------------------------------------!
...
# 58 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_diis.F" 2
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(diis_buffer)))CALL cp__a("qs_diis.F",152)
    IF(.NOT.(diis_buffer%ref_count>0))CALL cp__a("qs_diis.F",153)

    nbuffer = diis_buffer%nbuffer

    IF (.NOT.ASSOCIATED(diis_buffer%error)) THEN
      ALLOCATE (diis_buffer%error(nbuffer,nspin),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,159,&
                                       "diis_buffer%error",0)
    ...
    IF (.NOT.ASSOCIATED(diis_buffer%parameter)) THEN
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,178,&
    ...
    IF (.NOT.ASSOCIATED(diis_buffer%b_matrix)) THEN
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,197,&
...
! -------------------------------------------------------------------------
    ...
    DO ispin=1,nspin
      ...
      IF (my_roks) THEN
        ...
        IF (istat /= 0) CALL stop_memory(routineN,moduleN,336,&
                                         "occ",nmo*dp_size)
    ...
    IF (diis_step) THEN
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,451,&
                                       "a",nb1*nb1*dp_size)
      ALLOCATE (b(nb1,nb1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,454,&
                                       "b",nb1*nb1*dp_size)
      ALLOCATE (ev(nb1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,457,&
                                       "ev",nb1*dp_size)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(diis_buffer)))CALL cp__a("qs_diis.F",554)
    IF(.NOT.(diis_buffer%ref_count>0))CALL cp__a("qs_diis.F",555)
...
! Create scratch arrays
    ...
    CALL cp_dbcsr_create(matrix_tmp, &
                         ...
                         matrix_type='N')
    CALL cp_dbcsr_set(matrix_tmp,0.0_dp)     ! reset matrix
    CALL cp_dbcsr_init(matrix_KSerr_t)
    CALL cp_dbcsr_create(matrix_KSerr_t,&
                         template=ls_scf_env%matrix_ks(1),&
                         matrix_type='N')
    CALL cp_dbcsr_set(matrix_KSerr_t,0.0_dp) ! reset matrix
...
! new_errors=K*P-transpose(K*P)
    ...
    IF (diis_step) THEN
      nb1 = nb + 1
      ALLOCATE (a(nb1,nb1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,742,&
                                       "a",nb1*nb1*dp_size)
      ALLOCATE (b(nb1,nb1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,745,&
                                       "b",nb1*nb1*dp_size)
      ALLOCATE (ev(nb1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,748,&
                                       "ev",nb1*dp_size)
      ! Set up the linear DIIS equation system
      b(1:nb,1:nb) = diis_buffer%b_matrix(1:nb,1:nb)
      b(1:nb, nb1) = -1.0_dp
    ...
    END IF
      ...
      CALL cp_dbcsr_release(matrix_KSerr_t)
      CALL timestop(handle)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(diis_buffer)))CALL cp__a("qs_diis.F",839)
    IF(.NOT.(diis_buffer%ref_count>0))CALL cp__a("qs_diis.F",840)

    nbuffer = diis_buffer%nbuffer

    IF (.NOT.ASSOCIATED(diis_buffer%error)) THEN
      ALLOCATE (diis_buffer%error(nbuffer,nspin),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,846,&
                                       "diis_buffer%error",0)
    ...
    IF (.NOT.ASSOCIATED(diis_buffer%parameter)) THEN
      ALLOCATE (diis_buffer%parameter(nbuffer,nspin),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,863,&
                                       "diis_buffer%parameter",0)
    ...
    IF (.NOT.ASSOCIATED(diis_buffer%b_matrix)) THEN
      ALLOCATE (diis_buffer%b_matrix(nbuffer+1,nbuffer+1),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,879,&
                                       "diis_buffer%b_matrix",&
                                       (nbuffer + 1)**2*dp_size)
...
! -------------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(diis_buffer)))CALL cp__a("qs_diis.F",913)
    IF(.NOT.(diis_buffer%ref_count>0))CALL cp__a("qs_diis.F",914)
