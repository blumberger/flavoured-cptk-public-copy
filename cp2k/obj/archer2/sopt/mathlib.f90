# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/mathlib.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/mathlib.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/mathlib.F" 2
...
!> \param a input value
!> \retval abnormal_value TRUE for NaN and Inf
! *****************************************************************************
...
! *****************************************************************************
  SUBROUTINE diamat_all(a,eigval,dac)
    ...
    IF (SIZE(a,2) /= n) THEN
       CALL cp__b("common/mathlib.F",282,"Check the size of matrix a (parameter #1)")
    END IF

    ! Check the size of vector eigval
    IF (SIZE(eigval) /= n) THEN
       CALL cp__b("common/mathlib.F",287,"The dimension of vector eigval is too small")
    END IF
    ...
    IF (info /= 0) THEN
       IF (divide_and_conquer) THEN
          CALL cp__b("common/mathlib.F",325,"The matrix diagonalization with dsyevd failed")
       ELSE
          CALL cp__b("common/mathlib.F",327,"The matrix diagonalization with dsyev failed")
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE invmat_symm(a,cholesky_triangle)
    ...
    IF(.NOT.PRESENT(cholesky_triangle)) THEN
      ...
      IF (info /= 0) THEN
         CALL cp__b("common/mathlib.F",492,"DPOTRF failed")
      END IF
    ENDIF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",499,"Matrix inversion failed")
    END IF
...
! *****************************************************************************
  SUBROUTINE invert_matrix_d(a,a_inverse,eval_error,option,improve)
    ...
    IF (n == 0) THEN
      CALL cp__b("common/mathlib.F",582,"Matrix to be inverted of zero size")
    END IF

    IF (n /= SIZE(a,2)) THEN
      CALL cp__b("common/mathlib.F",586,"Check the array bounds of parameter #1")
    END IF

    IF ((n /= SIZE(a_inverse,1)).OR.&
        (n /= SIZE(a_inverse,2))) THEN
      CALL cp__b("common/mathlib.F",591,"Check the array bounds of parameter #2")
    END IF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",615,"The LU factorization in dgetrf failed")
    END IF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",633,"The computation of the condition number in dgecon failed")
    END IF

    IF (r_cond < EPSILON(0.0_dp)) THEN
      WRITE (message,"(A,ES10.3)") "R_COND =",r_cond
      CALL cp_abort(cp__l("common/mathlib.F",638),&
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",650,"Solving the system of linear equations in dgetrs failed")
    END IF
    ...
    IF (do_improve) THEN
      DO iter=1,10
        ...
        IF (info /= 0) THEN
          CALL cp__b("common/mathlib.F",665,"Improving the computed solution in dgerfs failed")
        END IF
...
! *****************************************************************************
  SUBROUTINE invert_matrix_z(a,a_inverse,eval_error,option)
    ...
    ! Check array dimensions
    IF (n == 0) THEN
      CALL cp__b("common/mathlib.F",745,"Matrix to be inverted of zero size")
    END IF

    IF (n /= SIZE(a,2)) THEN
      CALL cp__b("common/mathlib.F",749,"Check the array bounds of parameter #1")
    END IF

    IF ((n /= SIZE(a_inverse,1)).OR.&
        (n /= SIZE(a_inverse,2))) THEN
      CALL cp__b("common/mathlib.F",754,"Check the array bounds of parameter #2")
    END IF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",778,"The LU factorization in dgetrf failed")
    END IF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",796,"The computation of the condition number in dgecon failed")
    END IF

    IF (r_cond < EPSILON(0.0_dp)) THEN
      WRITE (message,"(A,ES10.3)") "R_COND =",r_cond
      CALL cp_abort(cp__l("common/mathlib.F",801),&
                        "Bad condition number "//TRIM(message)//" (smaller than the machine "//&
                        "working precision)")
    END IF
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",813,"Solving the system of linear equations in dgetrs failed")
    END IF
    ...
    DO iter=1,10
      ...
      IF (info /= 0) THEN
        CALL cp__b("common/mathlib.F",827,"Improving the computed solution in dgerfs failed")
      END IF
...
!> \author Dorothea Golze [02.2015]
! *****************************************************************************
  SUBROUTINE get_pseudo_inverse_svd(a,a_pinverse,rskip)
    
    REAL(KIND=dp), DIMENSION(:, :), POINTER  :: a, a_pinverse
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",887,"ERROR in DGESDD: Could not retrieve work array sizes")
    END IF
    lwork = INT(work(1))
    DEALLOCATE(work)
    ALLOCATE(work(lwork))
    ...
    IF (info /= 0) THEN
      CALL cp__b("common/mathlib.F",898,"SVD failed")
    END IF
...
! *****************************************************************************
  SUBROUTINE get_pseudo_inverse_diag(a,a_pinverse,rskip)
    ...
    ! work size query
    lwork = -1
    CALL dsyev('V','U',n,a(1,1),n,eig(1),work(1),lwork,info)
    IF (info /= 0) THEN
       CALL cp__b("common/mathlib.F",954,"ERROR in DSYEV: Could not retrieve work array sizes")
    END IF
    lwork = INT(work(1))
    DEALLOCATE(work)
    ALLOCATE(work(lwork))
    ...
    IF (info /= 0) THEN
       CALL cp__b("common/mathlib.F",964,"Matrix diagonalization failed")
    END IF
...
! *****************************************************************************
  SUBROUTINE symmetrize_matrix(a,option)
    ...
    ELSE IF (option == "anti_upper_to_lower") THEN
       ...
       END DO
    ELSE
       CALL cp__b("common/mathlib.F",1139,"Invalid option <"//TRIM(option)//"> was specified for parameter #2")
    END IF
...
! *****************************************************************************
  FUNCTION ei(x)
    ...
    IF(x <= 0._dp) THEN
       CALL cp__b("common/mathlib.F",1269,"Invalid argument")
    END IF
...
! *****************************************************************************
  FUNCTION expint(n,x)
    ...
    IF(n.lt.0.OR.x.lt.0.0_dp.OR.(x.eq.0.0_dp.AND.(n.EQ.0.or.n.EQ.1))) THEN
      CALL cp__b("common/mathlib.F",1404,"Invalid argument")
    ...
    ELSE IF(x.GT.1.0_dp) THEN  !Lentz's algorithm (§5.2).
      ...
      DO i = 1,MAXIT
        ...
        END IF
      END DO
      CALL cp__b("common/mathlib.F",1426,"continued fraction failed in expint")
    ELSE !Evaluate series.
      ...
      END DO
       CALL cp__b("common/mathlib.F",1448,"series failed in expint")
    END IF
    RETURN
  END FUNCTION expint
...
! *****************************************************************************
   SUBROUTINE swap_vector(a,b)
    ...
    IF (n /= SIZE(b)) THEN
      CALL cp__b("common/mathlib.F",1655,"Check the array bounds of the parameters")
    END IF
