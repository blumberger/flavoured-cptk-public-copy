# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_spline_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_spline_utils.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_spline_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_spline2_interpolate_values_g(spline_g)
    ...
    IF(.NOT.(ASSOCIATED(spline_g)))CALL cp__a("pw/pw_spline_utils.F",155)
    IF(.NOT.(spline_g%in_use==COMPLEXDATA1D))CALL cp__a("pw/pw_spline_utils.F",156)
    IF(.NOT.(spline_g%in_space==RECIPROCALSPACE))CALL cp__a("pw/pw_spline_utils.F",157)
    IF(.NOT.(.NOT.spline_g%pw_grid%spherical))CALL cp__a("pw/pw_spline_utils.F",158)
    IF(.NOT.(spline_g%pw_grid%grid_span==FULLSPACE))CALL cp__a("pw/pw_spline_utils.F",159)
...
! *****************************************************************************
  SUBROUTINE pw_spline3_interpolate_values_g(spline_g)
    ...
    IF(.NOT.(ASSOCIATED(spline_g)))CALL cp__a("pw/pw_spline_utils.F",231)
    IF(.NOT.(spline_g%in_use==COMPLEXDATA1D))CALL cp__a("pw/pw_spline_utils.F",232)
    IF(.NOT.(spline_g%in_space==RECIPROCALSPACE))CALL cp__a("pw/pw_spline_utils.F",233)
    IF(.NOT.(.NOT.spline_g%pw_grid%spherical))CALL cp__a("pw/pw_spline_utils.F",234)
    IF(.NOT.(spline_g%pw_grid%grid_span==FULLSPACE))CALL cp__a("pw/pw_spline_utils.F",235)
...
! *****************************************************************************
  SUBROUTINE pw_spline_scale_deriv(deriv_vals_r,transpose,scale)
    ...
    dh_inv = deriv_vals_r(1)%pw%pw_grid%dh_inv
...
! *****************************************************************************
  SUBROUTINE pw_spline3_deriv_g(spline_g,idir)
    ...
    IF(.NOT.(ASSOCIATED(spline_g)))CALL cp__a("pw/pw_spline_utils.F",408)
    IF(.NOT.(spline_g%in_use==COMPLEXDATA1D))CALL cp__a("pw/pw_spline_utils.F",409)
    IF(.NOT.(spline_g%in_space==RECIPROCALSPACE))CALL cp__a("pw/pw_spline_utils.F",410)
    IF(.NOT.(.not.spline_g%pw_grid%spherical))CALL cp__a("pw/pw_spline_utils.F",411)
    IF(.NOT.(spline_g%pw_grid%grid_span==FULLSPACE))CALL cp__a("pw/pw_spline_utils.F",412)
...
! *****************************************************************************
  SUBROUTINE pw_spline2_deriv_g(spline_g,idir)
    ...
    IF(.NOT.(ASSOCIATED(spline_g)))CALL cp__a("pw/pw_spline_utils.F",546)
    IF(.NOT.(spline_g%in_use==COMPLEXDATA1D))CALL cp__a("pw/pw_spline_utils.F",547)
    IF(.NOT.(spline_g%in_space==RECIPROCALSPACE))CALL cp__a("pw/pw_spline_utils.F",548)
    IF(.NOT.(.not.spline_g%pw_grid%spherical))CALL cp__a("pw/pw_spline_utils.F",549)
    IF(.NOT.(spline_g%pw_grid%grid_span==FULLSPACE))CALL cp__a("pw/pw_spline_utils.F",550)
...
SUBROUTINE pw_nn_compose_r(weights, pw_in, pw_out)
  ...
  CALL timeset(routineN,handle)
  IF(.NOT.(ASSOCIATED(pw_in)))CALL cp__a("pw/pw_spline_utils.F",874)
  IF(.NOT.(pw_in%in_space==REALSPACE))CALL cp__a("pw/pw_spline_utils.F",875)
  IF(.NOT.(pw_in%in_use==REALDATA3D))CALL cp__a("pw/pw_spline_utils.F",876)
  IF(.NOT.(ASSOCIATED(pw_out)))CALL cp__a("pw/pw_spline_utils.F",877)
  IF(.NOT.(pw_out%in_space==REALSPACE))CALL cp__a("pw/pw_spline_utils.F",878)
  IF(.NOT.(pw_out%in_use==REALDATA3D))CALL cp__a("pw/pw_spline_utils.F",879)
  IF (.NOT.ALL(pw_in%pw_grid%bounds_local(:,2:3) == pw_in%pw_grid%bounds(:,2:3))) THEN
     CALL cp__b("pw/pw_spline_utils.F",881,"wrong pw distribution")
  END IF
       ...
       DO j=-1,1
          DO i=-1,1
             ...
             CASE (3)
                idirVal=k
             CASE default
                CALL cp__b("pw/pw_spline_utils.F",974,"invalid idir ("//TRIM(cp_to_string(idir))//")")
             END SELECT
             IF (idirVal==0) THEN
...
! *****************************************************************************
  SUBROUTINE add_coarse2fine(coarse_coeffs_pw,fine_values_pw,&
    ...
    LOGICAL                                  :: has_i_lbound, has_i_ubound, &
                                                is_split, safe_calc
...
!    CALL timeset(routineN//"_pre",handle2)
    ...
    IF (ii>1) THEN
       CALL cp__b("pw/pw_spline_utils.F",1072,"")
    END IF
    ...
    IF (pbc) THEN
       IF(.NOT.(ALL(fine_gbo(1,:)==2*coarse_gbo(1,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1100)
       IF(.NOT.(ALL(fine_gbo(2,:)==2*coarse_gbo(2,:)+1+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1101)
    ELSE
       IF(.NOT.(ALL(fine_gbo(2,:)==2*coarse_gbo(2,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1103)
       IF(.NOT.(ALL(fine_gbo(1,:)==2*coarse_gbo(1,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1104)
    END IF
...
!       CALL timestop(handle2)
    ...
    IF (is_split) THEN
       ...
       IF (.NOT.rcv_tot_size==(coarse_bo(2,1)-coarse_bo(1,1)+1)*coarse_slice_size) THEN
          CALL cp__b("pw/pw_spline_utils.F",1209,"Error calculating rcv_tot_size ")
       END IF
       ...
       IF(.NOT.(ALL(sent_size(:n_procs-2)==send_offset(1:))))CALL cp__a("pw/pw_spline_utils.F",1305)
       IF(.NOT.(sent_size(n_procs-1)==send_tot_size))CALL cp__a("pw/pw_spline_utils.F",1306)
       ! test send/rcv sizes
       CALL mp_alltoall(send_size,real_rcv_size,1,coarse_coeffs_pw%pw_grid%para%group)
       IF(.NOT.(ALL(real_rcv_size==rcv_size)))CALL cp__a("pw/pw_spline_utils.F",1309)
       ! all2all
       ...
       IF(.NOT.(ALL(sent_size(0:n_procs-2)==rcv_offset(1:))))CALL cp__a("pw/pw_spline_utils.F",1379)
       IF(.NOT.(sent_size(n_procs-1)==rcv_tot_size))CALL cp__a("pw/pw_spline_utils.F",1380)
             ...
             IF (fk<=fine_bo(1,3)+1.OR.fk>=fine_bo(2,3)-1) THEN
                ...
                ELSE IF (fk==2*coarse_bo(1,3)+1+f_shift(3)) THEN
                   ...
                   CASE(-3)
                      wk=w_border1(3)
                   CASE default
                      CALL cp__b("pw/pw_spline_utils.F",1413,"")
                      CYCLE
                   END SELECT
                ELSE
                   SELECT CASE(ik)
                   ...
                   CASE(-1)
                      wk=w_border1(1)
                   CASE default
                      CALL cp__b("pw/pw_spline_utils.F",1425,"")
                      CYCLE
                   END SELECT
                END IF
             ELSE
...
!                      CALL timeset(routineN//"_core1",handle2)
                   ...
                   ELSE
                      ...
                      IF (.NOT.(fi+1==fine_bo(1,1).OR.&
                         ...
                         CALL cp_abort(cp__l("pw/pw_spline_utils.F",1557),&
                   ...
                   IF (fi>=fine_bo(1,1)) THEN
                      fine_values(fi,fj,fk)=fine_values(fi,fj,fk)+&
                           ...
                           ww0(3)*v2
                   ELSE
                      IF(.NOT.(fi+1==fine_bo(1,1)))CALL cp__a("pw/pw_spline_utils.F",1569)
                   END IF
!                      CALL timestop(handle2)
...
!                      CALL timeset(routineN//"_clean",handle2)
                   ...
                   ELSE
                      ...
                      fine_values(fi,fj,fk)=fine_values(fi,fj,fk)+&
                           w_border0*wj*v2
                   END IF
                   IF(.NOT.(fi==fine_bo(2,1)))CALL cp__a("pw/pw_spline_utils.F",1785)
                END IF
!                   CALL timestop(handle2)
             END DO
          END DO
...
! *****************************************************************************
  SUBROUTINE add_fine2coarse(fine_values_pw,coarse_coeffs_pw,&
    ...
    IF (pbc) THEN
       IF(.NOT.(ALL(fine_gbo(1,:)==2*coarse_gbo(1,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1897)
       IF(.NOT.(ALL(fine_gbo(2,:)==2*coarse_gbo(2,:)+f_shift+1)))CALL cp__a("pw/pw_spline_utils.F",1898)
    ELSE
       IF(.NOT.(ALL(fine_gbo(2,:)==2*coarse_gbo(2,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1900)
       IF(.NOT.(ALL(fine_gbo(1,:)==2*coarse_gbo(1,:)+f_shift)))CALL cp__a("pw/pw_spline_utils.F",1901)
    END IF
    IF(.NOT.(coarse_gbo(2,1)-coarse_gbo(1,2)>1))CALL cp__a("pw/pw_spline_utils.F",1903)
          ...
          IF (fk<=fine_bo(1,3)+1.OR.fk>=fine_bo(2,3)-1) THEN
             ...
             ELSE IF (fk==fine_bo(1,3)+1) THEN
                ...
                CASE(-3)
                   wk=w_border1(3)
                CASE default
                   CALL cp__b("pw/pw_spline_utils.F",1944,"")
                   CYCLE
                END SELECT
             ELSE
                SELECT CASE(ik)
                ...
                CASE(-1)
                   wk=w_border1(1)
                CASE default
                   CALL cp__b("pw/pw_spline_utils.F",1956,"")
                   CYCLE
                END SELECT
             END IF
          ELSE
             ...
             DO ij=-3,3
                ...
                IF (fj<=fine_bo(1,2)+1.OR.fj>=fine_bo(2,2)-1) THEN
                   ...
                   ELSE IF (fj==fine_bo(1,2)+1) THEN
                      ...
                      CASE(-3)
                         wj=w_border1(3)*wk
                      CASE default
                         CALL cp__b("pw/pw_spline_utils.F",1986,"")
                         CYCLE
                      END SELECT
                   ELSE
                      SELECT CASE(ij)
                      ...
                      CASE(3)
                         wj=w_border1(3)*wk
                      CASE default
                         CALL cp__b("pw/pw_spline_utils.F",1998,"")
                         CYCLE
                      END SELECT
                   END IF
                ELSE
                ...
                ELSE
                   ...
                   IF (.NOT.FLOOR((fine_bo(2,1)-f_shift(1))/2._dp)-coarse_bo(1,1)>=4) THEN
                      CALL cp__b("pw/pw_spline_utils.F",2137,"FLOOR((fine_bo(2,1)-f_shift(1))/2._dp)-coarse_bo(1,1)>=4")
                   END IF
                   rest_b=MODULO(FLOOR((fine_bo(2,1)-f_shift(1))/2._dp)-coarse_bo(1,1)-6,4)
                   i=FLOOR((fine_bo(2,1)-f_shift(1))/2._dp)-3-rest_b+4
                   IF(.NOT.(fi==(i-2)*2+f_shift(1)))CALL cp__a("pw/pw_spline_utils.F",2141)
                   IF (rest_b>0) THEN
                      fi=fi+1
                      vv0=fine_values(fi,fj,fk)
                      fi=fi+1
                   ...
                   ELSE
                      ...
                      END IF
                   END IF
                   IF(.NOT.(fi==fine_bo(2,1)))CALL cp__a("pw/pw_spline_utils.F",2343)
                END IF
             END DO
          END DO
       END DO
    ...
    IF (is_split) THEN
       ...
       IF(send_tot_size/=(coarse_bo(2,1)-coarse_bo(1,1)+1)*coarse_slice_size)&
          CALL cp__b("pw/pw_spline_utils.F",2428,"Error calculating send_tot_size")
       ALLOCATE(send_buf(0:send_tot_size-1))
       ...
       IF(ANY(sent_size(0:n_procs-2)/=send_offset(1:n_procs-1)))&
          CALL cp__b("pw/pw_spline_utils.F",2454,"error 1 filling send buffer")
       IF(sent_size(n_procs-1)/=send_tot_size)&
          CALL cp__b("pw/pw_spline_utils.F",2456,"error 2 filling send buffer")
    ...
    END IF
       ...
       IF(.NOT.(ALL(sent_size(:n_procs-2)==send_offset(1:))))CALL cp__a("pw/pw_spline_utils.F",2464)
       IF(.NOT.(sent_size(n_procs-1)==send_tot_size))CALL cp__a("pw/pw_spline_utils.F",2465)
       ! test send/rcv sizes
       CALL mp_alltoall(send_size,real_rcv_size,1,coarse_coeffs_pw%pw_grid%para%group)

       IF(.NOT.(ALL(real_rcv_size==rcv_size)))CALL cp__a("pw/pw_spline_utils.F",2469)
       ! all2all
       CALL mp_alltoall( sb=send_buf, scount=send_size, sdispl=send_offset,&
            rb=rcv_buf, rcount=rcv_size, rdispl=rcv_offset, &
            group=coarse_coeffs_pw%pw_grid%para%group )
       ...
       IF(ANY(sent_size(0:n_procs-2)/=rcv_offset(1:n_procs-1)))&
          CALL cp__b("pw/pw_spline_utils.F",2522,"error 1 handling the rcv buffer")
       IF(sent_size(n_procs-1)/=rcv_tot_size)&
          CALL cp__b("pw/pw_spline_utils.F",2524,"error 2 handling the rcv buffer")
       ...
       CALL timestop(handle2)
    ELSE
       IF(.NOT.(.NOT.local_data))CALL cp__a("pw/pw_spline_utils.F",2532)
    END IF
...
! *****************************************************************************
  SUBROUTINE pw_spline_precond_set_kind(preconditioner,precond_kind,pbc,&
    ...
    IF(.NOT.(ASSOCIATED(preconditioner)))CALL cp__a("pw/pw_spline_utils.F",2590)
    IF(.NOT.(preconditioner%ref_count>0))CALL cp__a("pw/pw_spline_utils.F",2591)
    ...
    CASE (precond_spl3_1)
       ...
       do_3d_coeff=.TRUE.
    CASE default
       CALL cp__b("pw/pw_spline_utils.F",2630,"")
    END SELECT
    IF (do_3d_coeff) THEN
       s=1._dp
...
! *****************************************************************************
  SUBROUTINE pw_spline_precond_retain(preconditioner)
    ...
    IF(.NOT.(ASSOCIATED(preconditioner)))CALL cp__a("pw/pw_spline_utils.F",2673)
    IF(.NOT.(preconditioner%ref_count>1))CALL cp__a("pw/pw_spline_utils.F",2674)
...
! *****************************************************************************
  SUBROUTINE pw_spline_precond_release(preconditioner)
    ...
    IF (ASSOCIATED(preconditioner)) THEN
       IF(.NOT.(preconditioner%ref_count>0))CALL cp__a("pw/pw_spline_utils.F",2690)
       preconditioner%ref_count=preconditioner%ref_count-1
...
! *****************************************************************************
  SUBROUTINE pw_spline_do_precond(preconditioner,in_v,out_v)
    ...
    IF(.NOT.(ASSOCIATED(preconditioner)))CALL cp__a("pw/pw_spline_utils.F",2714)
    IF(.NOT.(preconditioner%ref_count>0))CALL cp__a("pw/pw_spline_utils.F",2715)
    ...
    CASE(precond_spl3_3,precond_spl3_2,precond_spl3_aint2)
       ...
       END IF
    CASE default
       CALL cp__b("pw/pw_spline_utils.F",2742,"")
    END SELECT
    ...
    ext_do:DO iiter=1,max_iter,10
       ...
       DO iter=iiter,MIN(iiter+9,max_iter)
          ...
          ELSE
             ...
             eps_x_att=alpha*SQRT(pw_integral_ab(p,p)) ! try to spare if unneded?
...
! *****************************************************************************
  SUBROUTINE pw_nn_compose_r_no_pbc(weights_1d,pw_in,pw_out,&
    ...
    INTEGER, DIMENSION(2, 3)                 :: bo, gbo
    INTEGER, DIMENSION(3)                    :: s
    LOGICAL :: has_l_boundary, has_u_boundary, is_split, my_normalize, &
      my_sharpen, my_smooth_boundary, my_transpose
    REAL(kind=dp)                            :: in_val_f, in_val_l, &
                                                in_val_tmp, w_j, w_k
    ...
    IF(.NOT.(.not.my_normalize.OR.my_sharpen))CALL cp__a("pw/pw_spline_utils.F",2907)
    IF(.NOT.(.NOT.my_smooth_boundary.OR..NOT.my_sharpen))CALL cp__a("pw/pw_spline_utils.F",2908)
    DO i=1,3
       s(i)=bo(2,i)-bo(1,i)+1
    END DO
    IF (ANY(s<1)) RETURN
          ...
          DO j=bo(1,2),bo(2,2)
             DO jw=-1,1
                ...
                IF (has_l_boundary) THEN
                   IF (my_transpose) THEN
                      IF (s(1)==1) THEN
                         IF(.NOT.(.not.has_u_boundary))CALL cp__a("pw/pw_spline_utils.F",3030)
                ...
                IF (has_u_boundary) THEN
                   IF (my_transpose) THEN
                      ...
                      IF (s(1)==1) THEN
                         IF(.NOT.(.not.has_l_boundary))CALL cp__a("pw/pw_spline_utils.F",3088)
