# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE pint_methods
  ...
  USE bibliography,                    ONLY: Ceriotti2010,&
                                             Ceriotti2012,&
                                             cite_reference
  ...
  USE pint_transformations,            ONLY: pint_f2uf,&
                                             ...
                                             pint_x2u
  USE pint_types,                      ONLY: &
       e_conserved_id, e_kin_thermo_id, e_kin_virial_id, e_potential_id, &
       pint_env_type, thermostat_gle, thermostat_none, thermostat_nose, &
       thermostat_piglet, thermostat_pile
...
# 121 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/pint_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE pint_create(pint_env,input,input_declaration,para_env)
    ...
    INTEGER                                  :: handle, iat, ibead, idim, &
                                                idir, ierr, itmp, nrep, prep, &
                                                stat
    LOGICAL                                  :: explicit, ltmp, wrong_input
    ...
    TYPE(section_vals_type), POINTER :: gle_section, nose_section, &
      piglet_section, pile_section, pint_section, transform_section

    CALL timeset(routineN,handle)

    NULLIFY(f_env,subsys,particles, nose_section, gle_section)

    IF(.NOT.(.NOT.ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",177)
    IF(.NOT.(ASSOCIATED(input)))CALL cp__a("motion/pint_methods.F",178)
    IF(.NOT.(input%ref_count>0))CALL cp__a("motion/pint_methods.F",179)
    NULLIFY(rep_env)
    ...
    IF ( (prep < 1) .OR. (prep > para_env%num_pe) .OR. &
       ...
       IF (para_env%ionode) THEN
          ...
          CALL cp__w("motion/pint_methods.F",192,msg)
       END IF
    END IF
    ...
    IF(explicit) THEN
      IF (pint_env%propagator%prop_kind == propagator_rpmd) THEN
         CALL cp__b("motion/pint_methods.F",282,"RPMD Propagator with Nose-thermostat not implemented!")
      END IF
...
!TODO: rearrange to use standard nose hoover chain functions/data types
    ...
    ALLOCATE(&
         ...
         STAT=stat)
    IF(.NOT.(stat==0))CALL cp__a("motion/pint_methods.F",349)
    pint_env%x        = 0._dp
    pint_env%v        = 0._dp
    pint_env%f        = 0._dp
    ...
    ELSE
       ...
       IF(.NOT.(wrong_input))CALL cp__a("motion/pint_methods.F",377)
    END IF
...
!TODO length of pint_env%mass is redundant
    ...
    IF(.NOT.(ierr==0))CALL cp__a("motion/pint_methods.F",395)
    ...
    IF(explicit) THEN
      ...
      IF (pint_env%pimd_thermostat==thermostat_none .AND. pint_env%gle%ndim .GT. 0) THEN
         ...
         IF(.NOT.(stat==0))CALL cp__a("motion/pint_methods.F",425)
    ...
    IF (explicit) THEN
       ...
       ELSE
          CALL cp__b("motion/pint_methods.F",461,"PILE thermostat can't be used with another thermostat.")
       END IF
    END IF
    ...
    IF (explicit) THEN
       ...
       ELSE
          CALL cp__b("motion/pint_methods.F",484,"PILE thermostat can't be used with another thermostat.")
       END IF
    END IF
    ...
    CALL section_vals_val_get(pint_section,"HARM_INT",i_val=pint_env%harm_integrator)
       ...
       IF (pint_env%harm_integrator == integrate_exact) THEN
          IF (pint_env%nrespa /= 1) THEN
             ...
             CALL cp__w("motion/pint_methods.F",523,msg)
          END IF
...
! *****************************************************************************
  SUBROUTINE pint_retain(pint_env)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",569)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",570)
...
! *****************************************************************************
  SUBROUTINE pint_release(pint_env)
    ...
    IF (ASSOCIATED(pint_env)) THEN
      IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",589)
      pint_env%ref_count=pint_env%ref_count-1
...
! *****************************************************************************
  ...
  IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("motion/pint_methods.F",678)
  IF(.NOT.(ASSOCIATED(input)))CALL cp__a("motion/pint_methods.F",679)
  IF(.NOT.(para_env%ref_count>0))CALL cp__a("motion/pint_methods.F",680)
  IF(.NOT.(input%ref_count>0))CALL cp__a("motion/pint_methods.F",681)
  NULLIFY(pint_env)
...
! *****************************************************************************
  SUBROUTINE do_pint_run( para_env, input, input_declaration, globenv)
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("motion/pint_methods.F",754)
    IF(.NOT.(ASSOCIATED(input)))CALL cp__a("motion/pint_methods.F",755)
    IF(.NOT.(para_env%ref_count>0))CALL cp__a("motion/pint_methods.F",756)
    IF(.NOT.(input%ref_count>0))CALL cp__a("motion/pint_methods.F",757)
    ...
    IF (helium_only .AND. int_pot_scan) THEN
      ...
      CALL cp__b("motion/pint_methods.F",790,stmp)
    END IF
...
! avoided (has something to do with f_env_add_defaults) so leaving for now..
      ...
      CALL pint_run_scan(pint_env, helium)
      CALL helium_release(helium)
      CALL pint_release(pint_env)
    ...
    CASE DEFAULT
      CALL cp__b("motion/pint_methods.F",852,"Unknown mode ("//TRIM(ADJUSTL(cp_to_string(mode)))//")")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE pint_init(pint_env)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",877)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",878)
...
! *****************************************************************************
  SUBROUTINE pint_init_x( pint_env)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",924)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",925)
...
!      seed(:,:) = next_rng_seed()
    ...
    IF (explicit) THEN
      CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
           n_rep_val=n_rep_val)
      IF (n_rep_val>0) THEN
        IF(.NOT.(n_rep_val==1))CALL cp__a("motion/pint_methods.F",1009)
        CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
             r_vals=r_vals)
        IF(SIZE(r_vals)/=pint_env%p*pint_env%ndim)&
           CALL cp__b("motion/pint_methods.F",1013,"Invalid size of MOTION%PINT%BEADS%COORD")
        ic=0
        DO idim=1,pint_env%ndim
          DO ib=1,pint_env%p
            ic=ic+1
...
! *****************************************************************************
  SUBROUTINE pint_init_v( pint_env)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",1104)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",1105)
    ...
    IF ( explicit ) THEN
      ...
      IF(3*n_rep_val/=pint_env%ndim)&
         CALL cp__b("motion/pint_methods.F",1131,msg)
      DO ia = 1, pint_env%ndim/3
        CALL section_vals_val_get(input_section, "_DEFAULT_KEYWORD_",&
        ...
        msg = "Number of coordinates != 3 in FORCE_EVAL%SUBSYS%VELOCITY ("//&
              TRIM(ADJUSTL(stmp))//")."
        IF(itmp/=3)&
           CALL cp__b("motion/pint_methods.F",1141,msg)
        DO ib = 1, pint_env%p
          DO ic = 1, 3
          idim = 3*(ia-1)+ic
    ...
    IF (explicit) THEN
      CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
           n_rep_val=n_rep_val)
      IF (n_rep_val>0) THEN
        IF(.NOT.(n_rep_val==1))CALL cp__a("motion/pint_methods.F",1256)
        CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
             r_vals=r_vals)
        IF(SIZE(r_vals)/=pint_env%p*pint_env%ndim)&
           CALL cp__b("motion/pint_methods.F",1260,"Invalid size of MOTION%PINT%BEAD%VELOCITY")
        itmp=0
        DO idim=1,pint_env%ndim
          DO ib=1,pint_env%p
            itmp=itmp+1
    ...
    IF ( done_init .AND. done_quench ) THEN
      ...
      CALL cp__w("motion/pint_methods.F",1290,msg)
      msg = "WARNING: velocity restart took precedence"
      CALL cp__w("motion/pint_methods.F",1292,msg)
    END IF
    ...
    IF ( (.NOT. done_init) .AND. (.NOT. done_quench) ) THEN
      IF (vels_present .AND. done_scale) THEN
        ...
        CALL cp__w("motion/pint_methods.F",1301,msg)
      END IF
      IF ( done_sped ) THEN
        msg = "Added random component to the initial centroid velocities."
        CALL cp__w("motion/pint_methods.F",1305,msg)
      END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE pint_init_t( pint_env, kT)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",1335)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",1336)
    ...
    IF(pint_env%pimd_thermostat==thermostat_nose) THEN
      ...
      IF (explicit) THEN
        CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
                                  n_rep_val=n_rep_val)
        IF (n_rep_val>0) THEN
          IF(.NOT.(n_rep_val==1))CALL cp__a("motion/pint_methods.F",1360)
          CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
                                    r_vals=r_vals)
          IF(SIZE(r_vals)/=pint_env%p*pint_env%ndim*pint_env%nnos)&
             CALL cp__b("motion/pint_methods.F",1364,"Invalid size of MOTION%PINT%NOSE%COORD")
          ii=0
          DO idim=1,pint_env%ndim
            DO ib=1,pint_env%p
      ...
      IF (explicit) THEN
        CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
                                  n_rep_val=n_rep_val)
        IF (n_rep_val>0) THEN
          IF(.NOT.(n_rep_val==1))CALL cp__a("motion/pint_methods.F",1385)
          CALL section_vals_val_get(input_section,"_DEFAULT_KEYWORD_",&
                                    r_vals=r_vals)
          IF(SIZE(r_vals)/=pint_env%p*pint_env%ndim*pint_env%nnos)&
             CALL cp__b("motion/pint_methods.F",1389,"Invalid size of MOTION%PINT%NOSE%VELOCITY")
          ii=0
          DO idim=1,pint_env%ndim
            DO ib=1,pint_env%p
              DO inos=1,pint_env%nnos
...
! *****************************************************************************
  SUBROUTINE pint_init_f( pint_env, helium_solvent)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",1437)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",1438)
    CALL pint_x2u(pint_env)
...
! *****************************************************************************
  SUBROUTINE pint_do_run(pint_env, globenv, helium)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",1519)
...
!> \author Lukasz Walewski
! *****************************************************************************
  SUBROUTINE pint_run_scan(pint_env, helium)
    TYPE(pint_env_type), POINTER             :: pint_env
    TYPE(helium_solvent_type), POINTER       :: helium
...
! *****************************************************************************
  SUBROUTINE pint_step(pint_env,helium_solvent)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",1684)
...
!        CALL cp__b("motion/pint_methods.F",1921,"md & force_eval lost sychro")
...
! *****************************************************************************
  SUBROUTINE pint_calc_energy( pint_env)
    ...
    pint_env%energy(e_conserved_id) = &
      ...
      pint_env%e_pot_t + &
      pint_env%e_kin_t + pint_env%e_gle + pint_env%e_pile + pint_env%e_piglet
...
! *****************************************************************************
  SUBROUTINE pint_calc_f(pint_env,x,f,e)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",2042)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",2043)
    my_x => pint_env%x
    IF (PRESENT(x)) my_x => x
    my_f => pint_env%f
    IF (PRESENT(f)) my_f => f
...
! *****************************************************************************
  SUBROUTINE pint_calc_e_kin_beads_u(pint_env,uv,e_k)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",2089)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",2090)
    res=-1.0_dp
    my_uv => pint_env%uv
...
! *****************************************************************************
  SUBROUTINE pint_calc_e_vir(pint_env,e_vir)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",2124)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",2125)
    res=-1.0_dp
    res=0._dp
    DO idim=1,pint_env%ndim
...
! *****************************************************************************
  SUBROUTINE pint_calc_nh_energy(pint_env)
    ...
    IF(.NOT.(ASSOCIATED(pint_env)))CALL cp__a("motion/pint_methods.F",2161)
    IF(.NOT.(pint_env%ref_count>0))CALL cp__a("motion/pint_methods.F",2162)
    ekin=0._dp
    DO idim=1,pint_env%ndim
      DO ib=1,pint_env%p
        DO inos=1,pint_env%nnos
