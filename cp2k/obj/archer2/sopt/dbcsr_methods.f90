# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/base/dbcsr_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/base/dbcsr_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_methods
  ...
  USE dbcsr_types,                     ONLY: &
       dbcsr_1d_array_type, dbcsr_2d_array_type, dbcsr_data_obj, &
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/base/../../base/base_uses.f90" 1
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/base/dbcsr_methods.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.dbcsr_is_initialized (matrix))&
       CALL cp__w("dbcsr/base/dbcsr_methods.F",243,"Matrix not initialized")
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.dbcsr_is_initialized (matrix))&
       CALL cp__w("dbcsr/base/dbcsr_methods.F",271,"Can not destroy uninitialized matrix object.")
    force_all = .FALSE.
    IF (PRESENT (force)) force_all = force
    IF (dbcsr_is_initialized (matrix)) THEN
       IF(.NOT.force_all.AND.matrix%m%refcount.NE.0)&
          CALL cp__w("dbcsr/base/dbcsr_methods.F",276,"You should not destroy referenced matrix.")
       IF(force_all.AND.matrix%m%refcount.GT.1)&
          CALL cp__w("dbcsr/base/dbcsr_methods.F",278,"You should not destroy referenced matrix.")
       IF (force_all .OR. matrix%m%refcount .EQ. 0) THEN
          IF(ASSOCIATED (matrix%m%wms))&
             CALL cp__w("dbcsr/base/dbcsr_methods.F",281,"Destroying unfinalized matrix")
       ...
       SELECT CASE (matrix%m%data_area%d%data_type)
          ...
          CASE default
             CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error,&
                  routineN, "Incorrect data type",931)
          END SELECT
    ELSE
       CALL cp__w("dbcsr/base/dbcsr_methods.F",934,"Uninitialized data area")
       data_size = 0
    ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (ASSOCIATED (mutable%m), dbcsr_fatal_level,&
         dbcsr_caller_error, routineN, "Mutable data area not instantiated",1195)
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (.NOT.ASSOCIATED (mutable%m), dbcsr_fatal_level,&
         dbcsr_caller_error, routineN, "Mutable data area already instantiated",1219)
    CALL dbcsr_assert (data_type.EQ.dbcsr_type_real_4&
         ...
         dbcsr_wrong_args_error, routineN, "Invalid data type",1224)
    ALLOCATE (mutable%m)
