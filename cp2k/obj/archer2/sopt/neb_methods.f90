# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_methods.F"
!-----------------------------------------------------------------------------!
...
# 71 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/neb_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE neb(input, input_declaration, para_env, globenv)
    ...
    IF(nrep*prep/=para_env%num_pe.AND.output_unit>0) THEN
       CALL cp_warn(cp__l("motion/neb_methods.F",129),&
    ...
    IF (ASSOCIATED(rep_env)) THEN
       IF(.NOT.(SIZE(rep_env%local_rep_indices)==1))CALL cp__a("motion/neb_methods.F",143)
       ...
       IF(.NOT.(check))CALL cp__a("motion/neb_methods.F",157)
       ...
       IF(.NOT.(check))CALL cp__a("motion/neb_methods.F",161)
...
! *****************************************************************************
  SUBROUTINE neb_md(rep_env, neb_env, coords, vels, forces, particle_set, output_unit,&
    ...
    IF(.NOT.(ASSOCIATED(coords)))CALL cp__a("motion/neb_methods.F",253)
    IF(.NOT.(ASSOCIATED(vels)))CALL cp__a("motion/neb_methods.F",254)
    ! MD band for string methods type does not make anywa sense. Stop calculation.
    IF (neb_env%id_type==do_sm) THEN
       CALL cp__w("motion/neb_methods.F",257,"MD band optimization and String Method incompatible.")
    END IF
    ! Output unit
...
! *****************************************************************************
  SUBROUTINE neb_diis(rep_env, neb_env, coords, vels, forces, particle_set, output_unit,&
    ...
    do_ls = .TRUE.
    IF(.NOT.(ASSOCIATED(coords)))CALL cp__a("motion/neb_methods.F",401)
    IF(.NOT.(ASSOCIATED(vels)))CALL cp__a("motion/neb_methods.F",402)
    IF(.NOT.(ASSOCIATED(forces)))CALL cp__a("motion/neb_methods.F",403)
    iw=cp_print_key_unit_nr(logger,neb_env%neb_section,"REPLICA_INFO",&
         extension=".replicaLog")
