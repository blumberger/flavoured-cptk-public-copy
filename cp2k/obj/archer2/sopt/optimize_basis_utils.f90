# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis_utils.F"
!-----------------------------------------------------------------------------!
...
# 38 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/optimize_basis_utils.F" 2
   ...
   SUBROUTINE optimize_basis_init_read_input(opt_bas,root_section,para_env)
    ...
    TYPE(section_vals_type), POINTER         :: kind_section, optbas_section, &
      ...
      IF(opt_bas%ntraining_sets==0)&
         CALL cp__b("optimize_basis_utils.F",89,"No training set was specified in the Input")
...
! *****************************************************************************
   ...
   SUBROUTINE generate_initial_basis(kind_section,opt_bas,para_env)
    ...
    TYPE(section_vals_type), POINTER         :: set_section
      ...
      IF(.NOT.(variable_counter==SIZE(opt_bas%x_opt)))CALL cp__a("optimize_basis_utils.F",695)
    ...
    TYPE(section_vals_type), POINTER         :: const_section
      ...
      DO irep=1,nrep
         ...
         IF(is_bound.AND.is_varlim)&
            CALL cp_abort(cp__l("optimize_basis_utils.F",781), "Exponent has two constraints. "//&
                 "This is not possible at the moment. Please change input.")
         IF(.NOT.is_bound.AND..NOT.is_varlim)&
            CALL cp_abort(cp__l("optimize_basis_utils.F",784), "Exponent is declared to be constraint but none is given"//&
                 " Please change input.")
         IF(def_exp(1)==-1)THEN
            DO iset=1,flex_basis%nsets
               ...
               ELSE 
                  IF(def_exp(2).LE.flex_basis%subset(iset)%nexp)&
                     CALL cp_abort(cp__l("optimize_basis_utils.F",794),&
         ...
         ELSE
            IF(.NOT.def_exp(1).le.flex_basis%nsets)&
               CALL cp_abort(cp__l("optimize_basis_utils.F",802),&
            ...
            ELSE
               IF(.NOT.def_exp(2).le.flex_basis%subset(def_exp(1))%nexp)&
                  CALL cp_abort(cp__l("optimize_basis_utils.F",811),&
                       "Exponent declared in constraint is larger than number of exponents in the set"//&
                       " Please change input.")
   ...
   SUBROUTINE set_constraint(flex_basis,iset,ipgf,const_section,is_bound,is_varlim,irep)
    ...
    REAL(KIND=dp), DIMENSION(:), POINTER     :: r_vals
      ...
      IF(flex_basis%subset(iset)%exp_has_const(ipgf))&
         CALL cp_abort(cp__l("optimize_basis_utils.F",848),&
      ...
      IF(is_bound)THEN
         ...
         CALL cp_abort(cp__l("optimize_basis_utils.F",859),&
    ...
    INTEGER, DIMENSION(:), POINTER           :: switch
      ...
      DO iset=1,flex_basis%nsets
         ...
         CASE(do_opt_exps)
            flex_basis%subset(iset)%opt_exps=.TRUE.
         CASE DEFAULT
            CALL cp__b("optimize_basis_utils.F",958,"No initialization available?????")
         END SELECT
      END DO
      ...
      DO irep=1, nrep
         ...
         DO ipgf=1, flex_basis%subset(switch(2))%nexp
            ...
            CASE(2) ! switch only coeff
               ...
               END DO
            CASE DEFAULT
               CALL cp__b("optimize_basis_utils.F",1008,"Invalid option in SWITCH_SET_STATE, 1st value has to be 0, 1 or 2")
            END SELECT
         END DO
      END DO
...
! *****************************************************************************
   ...
   SUBROUTINE parse_basis(flex_basis, template_basis_file,element, basis_name, para_env)
    ...
    TYPE(cp_parser_type), POINTER            :: parser
      ...
      IF( .NOT. basis_found ) CALL cp_abort(cp__l("optimize_basis_utils.F",1124),&
