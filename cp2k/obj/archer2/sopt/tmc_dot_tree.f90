# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_dot_tree.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_dot_tree.F"
!-----------------------------------------------------------------------------!
...
MODULE tmc_dot_tree
  ...
  USE tmc_tree_types,                  ONLY: &
       global_tree_type, gt_elem_list_type, status_accepted, &
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_dot_tree.F" 2
...
! *****************************************************************************
  FUNCTION get_dot_file_name(tmc_params, ind) RESULT(filename)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'get_dot_file_name', &
      routineP = moduleN//':'//routineN

    filename = ""

    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",66)
    IF(.NOT.(ind.GE.0))CALL cp__a("tmc/tmc_dot_tree.F",67)
    IF(.NOT.(ASSOCIATED(tmc_params%Temp)))CALL cp__a("tmc/tmc_dot_tree.F",68)
    IF(.NOT.(ind.LE.SIZE(tmc_params%Temp)))CALL cp__a("tmc/tmc_dot_tree.F",69)
    ...
    IF(.NOT.(filename.NE.""))CALL cp__a("tmc/tmc_dot_tree.F",78)
...
! *****************************************************************************
  SUBROUTINE init_draw_trees(tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'init_draw_trees', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: file_ptr, i

    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",93)
...
! *****************************************************************************
  SUBROUTINE finalize_draw_tree(tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'finalize_draw_tree', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: file_ptr, i

    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",127)
...
! *****************************************************************************
  SUBROUTINE write_legend(file_ptr)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'write_legend', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(file_ptr.GT.0))CALL cp__a("tmc/tmc_dot_tree.F",155)
...
! *****************************************************************************
  SUBROUTINE write_color(node_nr, stat, filename)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'write_color', &
      routineP = moduleN//':'//routineN

    CHARACTER(len=11)                        :: label
    INTEGER                                  :: file_ptr

    IF(.NOT.(filename.NE.""))CALL cp__a("tmc/tmc_dot_tree.F",209)
    IF(.NOT.(node_nr.GE.0))CALL cp__a("tmc/tmc_dot_tree.F",210)
    ...
    CASE DEFAULT
      CALL cp__b("tmc/tmc_dot_tree.F",249,"element status"//cp_to_string(stat))
    END SELECT
    CALL close_file(unit_number=file_ptr, keep_preconnection=.TRUE.)
...
! *****************************************************************************
  SUBROUTINE create_dot_branch(parent_nr,child_nr,acc,tmc_params,tree)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'create_dot_branch', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: file_ptr

    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",274)
...
! *****************************************************************************
  SUBROUTINE create_dot(new_element, conf, tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'create_dot', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(new_element)))CALL cp__a("tmc/tmc_dot_tree.F",302)
    IF(.NOT.(conf.GT.0))CALL cp__a("tmc/tmc_dot_tree.F",303)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",304)
...
! *****************************************************************************
  SUBROUTINE create_global_tree_dot(new_element, tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'create_global_tree_dot', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(new_element)))CALL cp__a("tmc/tmc_dot_tree.F",332)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",333)
    ...
    IF(.NOT.ASSOCIATED(new_element%parent)) THEN
      IF(new_element%nr.GT.1)&
         CALL cp_warn(cp__l("tmc/tmc_dot_tree.F",367),&
...
! *****************************************************************************
  SUBROUTINE create_dot_color(tree_element, tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'create_dot_color', &
      routineP = moduleN//':'//routineN
    ...
    IF(.NOT.(ASSOCIATED(tree_element)))CALL cp__a("tmc/tmc_dot_tree.F",410)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",411)
...
! *****************************************************************************
  SUBROUTINE create_global_tree_dot_color(gt_tree_element, tmc_params)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'create_global_tree_dot_color', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(gt_tree_element)))CALL cp__a("tmc/tmc_dot_tree.F",450)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_dot_tree.F",451)
