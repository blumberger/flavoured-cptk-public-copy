# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dynamic))==-1) EXIT ;  END DO ; ENDIF ! only used with OMP
...
!$     CALL cp__w("dbcsr/block/dbcsr_iterator_operations.F",198,"Thread distribution should be defined for OpenMP.")
...
!$          "Number of threads has changed!", 208)
...
!   ---------------------------------------------------------------------------
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tdist))==-1) EXIT ;  END DO ; ENDIF ! only used with OMP
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tid))==-1) EXIT ;  END DO ; ENDIF   ! only used with OMP
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations_d.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 806 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations_z.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
! *****************************************************************************
  SUBROUTINE iterator_next_1d_block_z (iterator, row, column, block,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'iterator_next_1d_block_z', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE iterator_next_2d_block_z (iterator, row, column,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'iterator_next_2d_block_z', &
      routineP = moduleN//':'//routineN
...
# 807 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations_s.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 808 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations_c.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!> \param col_offset ...
! *****************************************************************************
  SUBROUTINE iterator_next_2d_block_c (iterator, row, column,&
       block, transposed,&
       block_number, row_size, col_size, row_offset, col_offset)
    TYPE(dbcsr_iterator), INTENT(INOUT)      :: iterator
    INTEGER, INTENT(OUT)                     :: row, column
...
! If we're pointing to a valid block, return that block.
    ...
    ENDIF
    IF (careful_mod) CALL timestop (error_handle)
  END SUBROUTINE iterator_next_2d_block_c
# 809 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_iterator_operations.F" 2
