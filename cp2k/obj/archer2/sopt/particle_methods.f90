# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/particle_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/particle_methods.F"
!-----------------------------------------------------------------------------!
...
# 61 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/particle_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE get_particle_set(particle_set,qs_kind_set,first_sgf,last_sgf,nsgf,&
    ...
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("particle_methods.F",113)

    nparticle = SIZE(particle_set)
    IF (PRESENT(first_sgf)) THEN
      IF(.NOT.(SIZE(first_sgf) >= nparticle))CALL cp__a("particle_methods.F",117)
    END IF
    IF (PRESENT(last_sgf)) THEN
      IF(.NOT.(SIZE(last_sgf) >= nparticle))CALL cp__a("particle_methods.F",120)
    END IF
    IF (PRESENT(nsgf)) THEN
      IF(.NOT.(SIZE(nsgf) >= nparticle))CALL cp__a("particle_methods.F",123)
    END IF
  ...
  SUBROUTINE write_particle_coordinates(particle_set,iunit,output_format,&
    ...
    IF (PRESENT(array)) THEN
       ...
       CASE ("POS_VEL","POS_VEL_FORCE")
          CALL cp__b("particle_methods.F",213,"Illegal usage")
       END SELECT
    END IF
    factor = 1.0_dp
    ...
    CASE (dump_dcd,dump_dcd_aligned_cell)
       IF (.NOT.(PRESENT(cell))) THEN
          CALL cp__b("particle_methods.F",303,"Cell is not present! Report this bug!")
       END IF
       ...
       IF (stat /= 0) CALL stop_memory(routineN,moduleN,324,&
                                       "arr",3*natom*dp_size)
       IF (PRESENT(array)) THEN
       ...
       ELSE
          ...
          CASE ("FORCE")
             ...
             END DO
          CASE DEFAULT
             CALL cp__b("particle_methods.F",343,"Illegal DCD dump type")
          END SELECT
       END IF
       ALLOCATE (x4(natom),STAT=stat)
       IF (stat /= 0) CALL stop_memory(routineN,moduleN,347,&
                                       "x4",natom*sp_size)
       ALLOCATE (y4(natom),STAT=stat)
       IF (stat /= 0) CALL stop_memory(routineN,moduleN,350,&
                                       "y4",natom*sp_size)
       ALLOCATE (z4(natom),STAT=stat)
       IF (stat /= 0) CALL stop_memory(routineN,moduleN,353,&
                                       "z4",natom*sp_size)
       IF (.NOT.cell%orthorhombic.AND.(output_format == dump_dcd_aligned_cell)) THEN
    ...
    CASE (dump_pdb)
       ...
       DO iatom=1,natom
          ...
          CASE DEFAULT
             CALL cp__b("particle_methods.F",446,"PDB dump only for trajectory available")
          END SELECT
    ...
    CASE DEFAULT
       CALL cp__b("particle_methods.F",467,"Illegal dump type")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE write_fist_particle_coordinates(particle_set,subsys_section,&
    ...
    REAL(KIND=dp)                            :: conv, mass, qcore, qeff, &
                                                qshell
...
!> \param Ilist ...
! *****************************************************************************
  SUBROUTINE write_particle_matrix(matrix,particle_set,iw,el_per_part,Ilist)
