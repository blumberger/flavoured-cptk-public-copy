# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_per_elpot.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_per_elpot.F"
!-----------------------------------------------------------------------------!
...
# 49 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_per_elpot.F" 2
...
! *****************************************************************************
  SUBROUTINE qmmm_per_potential_init(qmmm_coupl_type, per_potentials, potentials,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(per_potentials)))CALL cp__a("qmmm_per_elpot.F",129)
    ALLOCATE(per_potentials(SIZE(pgfs)))
    IF(.NOT.(SIZE(pgfs)==SIZE(potentials)))CALL cp__a("qmmm_per_elpot.F",131)
    Potential_Type: DO K = 1, SIZE(pgfs)
       ...
       CASE(do_qmmm_coulomb,do_qmmm_pcharge)
          ...
          CALL cp__b("qmmm_per_elpot.F",139,"")
       ...
       CASE(do_qmmm_coulomb,do_qmmm_pcharge)
          ! Not yet implemented for this case
          CALL cp__b("qmmm_per_elpot.F",155,"")
       CASE(do_qmmm_gauss,do_qmmm_swave)
          pgf => pgfs(K)%pgf
          idim = 0
          DO ix = 0, kmax(1)
          ...
          IF (ALL(n_rep_real == -1)) THEN
             ...
             IF (ANY(box > 0.0_dp)) THEN
                CALL cp__b("qmmm_per_elpot.F",209,"")
             END IF
...
! *****************************************************************************
  SUBROUTINE qmmm_ewald_potential_init(ewald_env,ewald_pw,qmmm_coupl_type, mm_cell, para_env,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ewald_env)))CALL cp__a("qmmm_per_elpot.F",362)
    IF(.NOT.(.NOT.ASSOCIATED(ewald_pw)))CALL cp__a("qmmm_per_elpot.F",363)
    ...
    IF(do_multipoles)&
       CALL cp__b("qmmm_per_elpot.F",377,"No multipole force fields allowed in QM-QM Ewald long range correction")

    SELECT CASE(qmmm_coupl_type)
       CASE(do_qmmm_coulomb)
          CALL cp__b("qmmm_per_elpot.F",381,"QM-QM long range correction not possible with COULOMB coupling")
       CASE(do_qmmm_pcharge)
          ! OK
       CASE(do_qmmm_gauss,do_qmmm_swave)
          CALL cp__b("qmmm_per_elpot.F",385,"QM-QM long range correction not possible with GAUSS/SWAVE coupling")
       CASE DEFAULT
          ! We should never get to this point
          CALL cp__b("qmmm_per_elpot.F",388,"")
    END SELECT
    ...
    IF ( iw > 0) THEN
       ...
       SELECT CASE(ewald_type)
          CASE(do_ewald_none)
             CALL cp__b("qmmm_per_elpot.F",399,"QM-QM long range correction not compatible with Ewald=NONE")
          CASE(do_ewald_pme)
             CALL cp__b("qmmm_per_elpot.F",401,"QM-QM long range correction not possible with Ewald=PME")
          CASE(do_ewald_ewald)
             CALL cp__b("qmmm_per_elpot.F",403,"QM-QM long range correction not possible with Ewald method")
          CASE(do_ewald_spme)
