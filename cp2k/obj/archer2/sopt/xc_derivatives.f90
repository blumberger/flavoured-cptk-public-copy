# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivatives.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivatives.F"
!-----------------------------------------------------------------------------!
...
# 120 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_derivatives.F" 2
...
SUBROUTINE xc_functional_get_info(functional,lsd,reference,shortform,&
  ...
  IF(.NOT.(ASSOCIATED(functional)))CALL cp__a("xc/xc_derivatives.F",161)
  ...
  CASE("HCTH")
     ...
     IF(.NOT.(.NOT.lsd))CALL cp__a("xc/xc_derivatives.F",223)
  ...
  CASE("P86C")
     IF(lsd) THEN
       CALL cp__b("xc/xc_derivatives.F",253,"BP functional not implemented with LSD")
     END IF
  ...
  CASE("GV09")
    IF (lsd) THEN
     ...
     END IF
  CASE default
     CALL cp__b("xc/xc_derivatives.F",325,"unknown functional '"//TRIM(functional%section%name)//"'")
  END SELECT
...
SUBROUTINE xc_functional_eval(functional, lsd, rho_set, deriv_set, &
  ...
  CASE("PADE")
     ...
     END IF
  CASE("HCTH")
     IF(.NOT.(.NOT.lsd))CALL cp__a("xc/xc_derivatives.F",429)
     CALL section_vals_val_get(functional,"PARAMETER_SET",i_val=i_param)
  ...
  CASE("KE_GGA")
     ...
     END IF
  CASE("P86C")
     IF(.NOT.(.NOT.lsd))CALL cp__a("xc/xc_derivatives.F",461)
  ...
  CASE("GV09")
    IF (lsd) THEN
     ...
     END IF
  CASE default
     CALL cp__b("xc/xc_derivatives.F",552,"unknown functional '"//TRIM(functional%section%name)//"'")
  END SELECT
...
SUBROUTINE xc_functionals_eval(functionals, lsd, rho_set, deriv_set, &
  ...
  IF(.NOT.(ASSOCIATED(functionals)))CALL cp__a("xc/xc_derivatives.F",592)
  ifun=0
  DO
     ifun=ifun+1
...
FUNCTION xc_functionals_get_needs(functionals, lsd, add_basic_components)&
  ...
  IF(.NOT.(ASSOCIATED(functionals)))CALL cp__a("xc/xc_derivatives.F",647)
