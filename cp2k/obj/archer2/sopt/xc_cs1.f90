# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_cs1.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_cs1.F"
!-----------------------------------------------------------------------------!
...
# 26 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_cs1.F" 2
...
! *****************************************************************************
  SUBROUTINE cs1_lda_eval (rho_set,deriv_set,order)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_cs1.F",173)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_cs1.F",174)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_cs1.F",175)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_cs1.F",176)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_cs1.F",233,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE cs1_lsd_eval (rho_set,deriv_set,order)
    ...
    REAL(KIND=dp), DIMENSION(:, :, :), &
      POINTER                                :: e_0, e_ndrhoa, e_ndrhob, &
                                                e_rhoa, e_rhob, norm_drhoa, &
                                                norm_drhob, rhoa, rhoa_1_3, &
                                                rhob, rhob_1_3
    TYPE(xc_derivative_type), POINTER        :: deriv
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_cs1.F",266)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_cs1.F",267)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_cs1.F",268)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_cs1.F",269)
    ...
    END IF
    IF ( order>1 .OR. order<1 ) THEN
       CALL cp__b("xc/xc_cs1.F",304,"derivatives bigger than 3 not implemented")
    END IF
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE cs1_u_0 ( rho, grho, r13, e_0, npoints)
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp)                            :: c2p, c3p, c4p, cp, dpv, F1, &
...
! *****************************************************************************
  SUBROUTINE cs1_u_1 ( rho, grho, r13, e_rho, e_ndrho, npoints)
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp)                            :: c2p, c3p, c4p, cp, dF1, dF3, &
                                                dgF2, dgF4, dpv, drF2, drF4, &
...
! *****************************************************************************
  SUBROUTINE cs1_u_2 ( rho, grho, r13, e_rho_rho, e_rho_ndrho, e_ndrho_ndrho,&
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp)                            :: c2p, c3p, c4p, cp, d2F1, &
                                                d2F3, d2gF2, d2gF4, d2rF2, &
                                                d2rF4, dpv, drgF2, drgF4, g, &
                                                oc, ocp, od, odp, r, r3
...
! *****************************************************************************
  SUBROUTINE cs1_u_3 ( rho, grho, r13, e_rho_rho_rho, e_rho_rho_ndrho,&
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp) :: c2p, c3p, c4p, cp, d2rgF2, d2rgF4, d3F1, d3F3, d3gF2, &
      d3gF4, d3rF2, d3rF4, dpv, dr2gF2, dr2gF4, g, oc, ocp, od, odp, r, r3, &
      t1, t10, t11, t13, t15, t16, t17, t19, t2, t22, t23, t26, t29, t3, t32, &
      t33, t37, t4, t44, t45, t50, t51, t52, t58, t6, t61, t7, t74, t76, t77, &
      t8, t80, t81, t82, t9
...
! *****************************************************************************
  SUBROUTINE cs1_ss_0 ( rhoa, rhob, grhoa, grhob, r13a, r13b, e_0,&
    ...
    REAL(KIND=dp), DIMENSION(*), INTENT(IN)  :: rhoa, rhob, grhoa, grhob, &
                                                r13a, r13b
    REAL(KIND=dp), DIMENSION(*), &
      INTENT(INOUT)                          :: e_0
    INTEGER, INTENT(in)                      :: npoints
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp)                            :: F1a, F1b, F2a, F2b, ga, gb, &
                                                oca, ocb, oda, odb, r3a, r3b, &
                                                ra, rb, xa, xb

!FM IF ( .NOT. debug_flag ) CALL cp__b("xc/xc_cs1.F",636,"Routine not tested")

    CALL cp__w("xc/xc_cs1.F",638,"not tested!")
...
! *****************************************************************************
  SUBROUTINE cs1_ss_1 ( rhoa, rhob, grhoa, grhob, r13a, r13b, e_rhoa,&
    ...
    REAL(KIND=dp), DIMENSION(*), INTENT(IN)  :: rhoa, rhob, grhoa, grhob, &
                                                r13a, r13b
    REAL(KIND=dp), DIMENSION(*), &
      INTENT(INOUT)                          :: e_rhoa, e_rhob, e_ndrhoa, &
                                                e_ndrhob
    INTEGER, INTENT(in)                      :: npoints
    ...
    INTEGER                                  :: ip
    REAL(KIND=dp)                            :: dF1a, dF1b, dgF2a, dgF2b, &
                                                drF2a, drF2b, ga, gb, oca, &
                                                ocb, oda, odb, r3a, r3b, ra, &
                                                rb

    CALL cp__w("xc/xc_cs1.F",716,"not tested!")
