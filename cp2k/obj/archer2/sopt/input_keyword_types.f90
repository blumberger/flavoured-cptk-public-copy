# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_keyword_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_keyword_types.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/input_keyword_types.F" 2
...
! *****************************************************************************
  SUBROUTINE keyword_create(keyword, name, description, usage, type_of_var,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",201)
    ALLOCATE(keyword)
    keyword%ref_count=1
    ...
    IF (PRESENT(usage)) THEN
       IF(.NOT.(LEN_TRIM(usage)<=LEN(keyword%usage)))CALL cp__a("input/input_keyword_types.F",223)
       keyword%usage=usage
    ELSE
       keyword%usage=""
    END IF
    ...
    IF (PRESENT(enum_i_vals)) THEN
       IF(.NOT.(PRESENT(enum_c_vals)))CALL cp__a("input/input_keyword_types.F",251)
       IF(.NOT.(.NOT.ASSOCIATED(keyword%enum)))CALL cp__a("input/input_keyword_types.F",252)
       CALL enum_create(keyword%enum,c_vals=enum_c_vals,i_vals=enum_i_vals,&
            desc=enum_desc,strict=enum_strict)
    ELSE
       IF(.NOT.(.NOT.PRESENT(enum_c_vals)))CALL cp__a("input/input_keyword_types.F",256)
    END IF
    ...
    IF (PRESENT(default_val)) THEN
       IF(PRESENT(default_l_val).OR.PRESENT(default_l_vals).OR.&
          ...
          CALL cp__b("input/input_keyword_types.F",265,"you should pass either default_val or a default value, not both")
       keyword%default_value => default_val
       IF (ASSOCIATED(default_val%enum)) THEN
          IF (ASSOCIATED(keyword%enum)) THEN
             IF(.NOT.(keyword%enum%id_nr==default_val%enum%id_nr))CALL cp__a("input/input_keyword_types.F",269)
          ELSE
             keyword%enum => default_val%enum
             CALL enum_retain(keyword%enum)
          END IF
       ELSE
          IF(.NOT.(.NOT.ASSOCIATED(keyword%enum)))CALL cp__a("input/input_keyword_types.F",275)
       END IF
    ...
    IF (keyword%type_of_var==no_t) THEN
       ...
       ELSE
          CALL cp_warn(cp__l("input/input_keyword_types.F",295),&
    ...
    ELSE IF (PRESENT(type_of_var)) THEN
       IF(keyword%type_of_var/=type_of_var)&
          CALL cp_abort(cp__l("input/input_keyword_types.F",301),&
               "keyword "//TRIM(keyword%names(1))//&
    ...
    IF (PRESENT(lone_keyword_val)) THEN
       IF(PRESENT(lone_keyword_l_val).OR.PRESENT(lone_keyword_l_vals).OR.&
          ...
          CALL cp__b("input/input_keyword_types.F",316,"you should pass either lone_keyword_val or a lone_keyword value, not both")
       ...
       IF (ASSOCIATED(lone_keyword_val%enum)) THEN
          IF (ASSOCIATED(keyword%enum)) THEN
             IF(keyword%enum%id_nr/=lone_keyword_val%enum%id_nr)&
                CALL cp__b("input/input_keyword_types.F",322,"keyword%enum%id_nr==lone_keyword_val%enum%id_nr")
          ELSE
             IF (ASSOCIATED(keyword%lone_keyword_value)) THEN
                CALL cp__b("input/input_keyword_types.F",325,".NOT. ASSOCIATED(keyword%lone_keyword_value)")
             END IF
             keyword%enum => lone_keyword_val%enum
             CALL enum_retain(keyword%enum)
          END IF
       ELSE
          IF(.NOT.(.NOT.ASSOCIATED(keyword%enum)))CALL cp__a("input/input_keyword_types.F",331)
       END IF
    END IF
    ...
    IF (ASSOCIATED(keyword%lone_keyword_value)) THEN
       ...
       ELSE
          IF(keyword%lone_keyword_value%type_of_var/=keyword%type_of_var)&
             CALL cp__b("input/input_keyword_types.F",345,"lone_keyword_value type incompatible with keyword type")
          ...
          IF (keyword%type_of_var==enum_t) THEN
             IF (keyword%enum%strict) THEN
                ...
                END DO
                IF(.NOT.check)&
                   CALL cp__b("input/input_keyword_types.F",354,"default value not in enumeration : "//keyword%names(1))
             ENDIF
          ENDIF
       END IF
    END IF
    ...
    IF (ASSOCIATED(keyword%default_value)) THEN
       ...
       CASE(enum_t)
          IF (keyword%enum%strict) THEN
             ...
             END DO
             IF(.NOT.check)&
                CALL cp__b("input/input_keyword_types.F",374,"default value not in enumeration : "//keyword%names(1))
          ENDIF
          keyword%n_var=SIZE(keyword%default_value%i_val)
       CASE(real_t)
       ...
       CASE(no_t)
          keyword%n_var=0
       CASE default
          CALL cp__b("input/input_keyword_types.F",386,"")
       END SELECT
    END IF
    IF (PRESENT(n_var)) keyword%n_var=n_var
    IF(keyword%type_of_var==lchar_t.AND.keyword%n_var/=1)&
       CALL cp__b("input/input_keyword_types.F",391,"arrays of lchar_t not supported : "//keyword%names(1))
...
! *****************************************************************************
  SUBROUTINE keyword_retain(keyword)
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",409)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",410)
...
! *****************************************************************************
  SUBROUTINE keyword_release(keyword)
    ...
    IF (ASSOCIATED(keyword)) THEN
       IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",426)
       keyword%ref_count=keyword%ref_count-1
...
! *****************************************************************************
  SUBROUTINE keyword_get(keyword,names,usage,description,type_of_var,n_var,&
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",476)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",477)
...
! *****************************************************************************
  SUBROUTINE keyword_describe(keyword, unit_nr, level)
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",509)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",510)
    IF (level>0.AND.(unit_nr>0)) THEN
       ...
       IF (level>2) THEN
          ...
          IF (level>3) THEN
             ...
             CASE default
                CALL cp__b("input/input_keyword_types.F",573,"")
             END SELECT
          END IF
          IF (keyword%type_of_var==enum_t) THEN
             IF (level>3) THEN
...
! *****************************************************************************
  SUBROUTINE keyword_describe_html(keyword, unit_nr)
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",646)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",647)
    ...
    CASE DEFAULT
       CALL cp__b("input/input_keyword_types.F",701,"")
    END SELECT
    IF (keyword%repeats) THEN
...
! *****************************************************************************
  SUBROUTINE write_keyword_xml(keyword,level,unit_number)
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",800)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",801)
    ...
    CASE DEFAULT
       CALL cp__b("input/input_keyword_types.F",881,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE keyword_typo_match(keyword,unknown_string,location_string,matching_rank,matching_string,bonus)
    ...
    IF(.NOT.(ASSOCIATED(keyword)))CALL cp__a("input/input_keyword_types.F",983)
    IF(.NOT.(keyword%ref_count>0))CALL cp__a("input/input_keyword_types.F",984)
