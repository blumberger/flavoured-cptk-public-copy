# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cg_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cg_utils.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cg_utils.F" 2
...
END INTERFACE
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/cg_utils.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE cg_linmin(gopt_env, xvec, xi, g, opt_energy, output_unit, gopt_param,&
    ...
    CASE(default_minimization_method_id)
       ...
       CASE DEFAULT
          CALL cp__b("motion/cg_utils.F",86,"Line Search type not yet implemented in CG.")
       END SELECT
    CASE(default_ts_method_id)
       ...
       CASE(ls_2pnt)
          ...
          END IF
       CASE DEFAULT
          CALL cp__b("motion/cg_utils.F",98,"Line Search type not yet implemented in CG for TS search.")
       END SELECT
    CASE(default_cell_method_id)
       ...
       CASE(ls_gold)
          CALL linmin_gold(gopt_env, xvec, xi, opt_energy, gopt_param%cg_ls%brent_tol,&
               ...
               gopt_param%cg_ls%initial_step, output_unit, globenv)
       CASE DEFAULT
          CALL cp__b("motion/cg_utils.F",110,"Line Search type not yet implemented in CG for cell optimization.")
       END SELECT
    CASE(default_shellcore_method_id)
       ...
       CASE(ls_2pnt)
          CALL linmin_2pnt(gopt_env, xvec, xi, g, opt_energy, gopt_param, use_only_grad=.TRUE.,&
               output_unit=output_unit)
       CASE DEFAULT
          CALL cp__b("motion/cg_utils.F",118,"Line Search type not yet implemented in CG for shellcore optimization.")
       END SELECT
...
! *****************************************************************************
  SUBROUTINE cg_mnbrak(gopt_env, ax, bx, cx, pcom, xicom, brack_limit, output_unit,&
    ...
    IF (hist) THEN
       IF(.NOT.(.NOT.ASSOCIATED(histpoint)))CALL cp__a("motion/cg_utils.F",583)
...
! *****************************************************************************
  FUNCTION cg_dbrent(gopt_env,ax,bx,cx,tol,itmax,xmin,pcom,xicom,output_unit,&
    ...
    LOGICAL                                  :: ok1, ok2, should_stop, skip0, &
                                                skip1
    ...
    IF (output_unit>0) THEN
       ...
       WRITE(UNIT=output_unit,FMT="(T2,A)") REPEAT("*",79)
    END IF
    IF(.NOT.(iter/=itmax+1))CALL cp__a("motion/cg_utils.F",880)
    xmin=x
    dbrent=fx
    CALL timestop(handle)
...
! *****************************************************************************
  FUNCTION FindMin(x,y,dy) RESULT(res)
    ...
    np=SIZE(x)
    IF(.NOT.(np>1))CALL cp__a("motion/cg_utils.F",992)
    sum_x=0._dp
    sum_xx=0._dp
    min_pos=1
    DO i=1,np
...
! *****************************************************************************
  SUBROUTINE get_conjugate_direction(gopt_env, Fletcher_Reeves, g, xi, h)
    ...
    ELSE
       ...
       IF(.NOT.(check))CALL cp__a("motion/cg_utils.F",1075)

       check = ABS(DOT_PRODUCT(xi,xi)-1.0_dp)<MAX(1.0E-9_dp,dimer_thrs)
       IF(.NOT.(check))CALL cp__a("motion/cg_utils.F",1078)

       check = ABS(DOT_PRODUCT(h,dimer_env%cg_rot%nvec_old))<MAX(1.0E-9_dp,dimer_thrs)
       IF(.NOT.(check))CALL cp__a("motion/cg_utils.F",1081)
