# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/cp_linked_list_pw.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/cp_linked_list_pw.F"
!-----------------------------------------------------------------------------!
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/cp_linked_list_pw.F" 2
       ...
       IF (PRESENT(iter)) THEN
          ...
          DO i=1,iter
             IF (ASSOCIATED(res%rest)) THEN
                res => res%rest
             ELSE
                CALL cp__b("pw/cp_linked_list_pw.F",297,"tried to go past end")
             END IF
          END DO
          IF (iter==-1) THEN
             DO
...
! *****************************************************************************
  FUNCTION cp_sll_pw_get_el_at(sll,index) RESULT(res)
    ...
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("pw/cp_linked_list_pw.F",369)
...
! *****************************************************************************
  SUBROUTINE cp_sll_pw_set_el_at(sll,index,value)
    ...
    ELSE
       pos => cp_sll_pw_get_rest(sll, iter=index-1)
    END IF
    IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("pw/cp_linked_list_pw.F",401)
...
! *****************************************************************************
  SUBROUTINE cp_sll_pw_rm_first_el(sll)
    ...
    ELSE
       CALL cp__b("pw/cp_linked_list_pw.F",487,"tried to remove first el of an empty list")
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_sll_pw_insert_el_at(sll,el,index)
    ...
    ELSE
       ...
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("pw/cp_linked_list_pw.F",521)
  ...
  SUBROUTINE cp_sll_pw_rm_el_at(sll,index)
    ...
    ELSE
       ...
       ELSE
          pos => cp_sll_pw_get_rest(sll, iter=index-2)
       END IF
       IF(.NOT.(ASSOCIATED(pos)))CALL cp__a("pw/cp_linked_list_pw.F",553)
...
FUNCTION cp_sll_pw_to_array(sll) RESULT(res)
  ...
  DO i=1,len
     ...
     IF(.NOT.(cp_sll_pw_next(iter).OR.i==len))CALL cp__a("pw/cp_linked_list_pw.F",609)
  END DO
