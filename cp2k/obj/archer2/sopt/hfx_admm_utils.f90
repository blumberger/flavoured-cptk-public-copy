# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_admm_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_admm_utils.F"
!-----------------------------------------------------------------------------!
...
# 85 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_admm_utils.F" 2
...
! *****************************************************************************
 SUBROUTINE  hfx_admm_init(qs_env,do_mp2_hfx)
    ...
    IF ( .NOT. do_hfx )&
       CALL cp__b("hfx_admm_utils.F",166,"Wavefunction fitting requested without Hartree-Fock.")

    ! ** Method only implemented for GPW
    IF( dft_control%qs_control%gapw ) THEN
      CALL cp__b("hfx_admm_utils.F",170,"ADMM only implemented for GPW")
    END IF


    CALL section_vals_get(hfx_sections,n_repetition=n_rep_hf)
    IF ( n_rep_hf > 1 )&
       CALL cp__b("hfx_admm_utils.F",176,"ADMM can handle only one HF section.")
...
! *****************************************************************************
  ...
  SUBROUTINE hfx_ks_matrix(qs_env,matrix_ks,rho,energy,calculate_forces,&
    ...
    INTEGER                                  :: handle, ikind, img, irep, &
                                                ...
                                                nimages, ns, nspins
    LOGICAL :: distribute_fock_matrix, do_adiabatic_rescaling, &
    ...
    DO ispin = 1,nspins
       DO img=1,nimages
          CALL cp_dbcsr_set(matrix_ks(ispin,img)%matrix,0.0_dp)
       END DO
    END DO
    ...
    DO irep = 1,n_rep_hf
       ...
       IF( do_adiabatic_rescaling .AND. hfx_treat_lsd_in_core)&
          CALL cp__b("hfx_admm_utils.F",315,"HFX_TREAT_LSD_IN_CORE not implemented for adiabatically rescaled hybrids")
       ...
       IF( qs_env%run_rtp)THEN
          ...
          DO ispin = 1,nspins
             CALL cp_dbcsr_set(matrix_ks_im(ispin)%matrix,0.0_dp)
          END DO
          IF( dft_control%do_admm ) THEN
             ...
             DO ispin = 1,nspins
                CALL cp_dbcsr_set(matrix_ks_aux_fit_im(ispin)%matrix,0.0_dp)
             END DO
          ELSE
      ...
      SELECT CASE (hfx_potential_type)
        ...
        CASE (do_hfx_potential_truncated)
          ...
          CALL section_vals_val_set(xc_fun_section,"XWPBE%SCALE_X0",&
                                    r_val=-hfx_fraction)
        CASE DEFAULT
          CALL cp__b("hfx_admm_utils.F",720,"")
      END SELECT
