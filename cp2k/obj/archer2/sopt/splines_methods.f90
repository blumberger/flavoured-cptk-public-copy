# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/splines_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/splines_methods.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/splines_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE init_splinexy ( spl, nn )
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'init_splinexy', &
    ...
    IF (err /= 0) CALL stop_memory(routineN,moduleN,71,&
                                   "spl%y",dp_size*nn)

    ALLOCATE ( spl % y2 ( 1:nn ), STAT = err )
    IF (err /= 0) CALL stop_memory(routineN,moduleN,75,&
                                   "spl%y2",dp_size*nn)
...
! *****************************************************************************
  SUBROUTINE init_spline(spl, dx, y1a, y1b)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'init_spline', &
      routineP = moduleN//':'//routineN
    ...
    IF (err /= 0) CALL stop_memory(routineN,moduleN,129,&
                                   "ww",dp_size*n)
    IF (PRESENT(y1a)) THEN
       spl%y2(1) = -0.5_dp
...
! *****************************************************************************
  FUNCTION potential_s (spl_p, xxi, y1, spl_f, logger )
    ...
    REAL(KIND=dp)                            :: a, b, h26, invh, x4, xx, xx0, &
                                                y2hi, y2lo, yhi, ylo, yy
