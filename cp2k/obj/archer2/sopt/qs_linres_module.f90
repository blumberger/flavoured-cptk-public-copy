# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_module.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_module.F"
!-----------------------------------------------------------------------------!
...
# 93 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_module.F" 2
...
! *****************************************************************************
  SUBROUTINE linres_calculation(force_env)
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("qs_linres_module.F",124)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("qs_linres_module.F",125)
    ...
    CASE DEFAULT
       CALL cp__b("qs_linres_module.F",133,"Doesnt recognize this force_env.")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE linres_init(lr_section,p_env,qs_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(p_env)))CALL cp__a("qs_linres_module.F",290)
...
! ** update qs_env%rho
    ...
    IF(output_unit>0) THEN
      ...
      CASE DEFAULT
         CALL cp__b("qs_linres_module.F",365,"Preconditioner NYI")
      END SELECT
...
! *****************************************************************************
  SUBROUTINE nmr_epr_linres(linres_control,qs_env,p_env,dft_control,nmr_present,epr_present,output_unit)
    ...
    IF(.NOT.linres_control%localized_psi0) THEN
       CALL cp_abort(cp__l("qs_linres_module.F",423),&
    ...
    IF(dft_control%nspins/=2.AND.epr_present) THEN
       CALL cp__b("qs_linres_module.F",430,"LSD is needed to perform a g tensor calculation!")
    ENDIF
    !
...
! *****************************************************************************
  SUBROUTINE issc_linres(linres_control,qs_env, p_env, dft_control)
    ...
    linres_control%property = lr_current
    IF(.NOT.linres_control%localized_psi0) THEN
       CALL cp_abort(cp__l("qs_linres_module.F",530),&
            "Are you sure that you want to calculate the chemical "//&
            "shift without localized psi0?")
       CALL linres_localize(qs_env,linres_control,&
            dft_control%nspins,centers_only=.TRUE.)
