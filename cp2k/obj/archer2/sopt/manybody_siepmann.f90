# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_siepmann.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_siepmann.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_siepmann.F" 2
...
! *****************************************************************************
  SUBROUTINE siep_a_ij_d(siepmann,r_last_update_pbc,iparticle,jparticle,f_nonbond,&
    ...
    DO ilist = 1, n_loc_size
       ...
       IF (use_virial) THEN
        CALL cp_abort(cp__l("manybody_siepmann.F",315),&
...
! *****************************************************************************
  FUNCTION siep_Phi_ij(siepmann,r_last_update_pbc,iparticle,jparticle,&
       cell_v, cell, rcutsq, particle_set,nr_oh)
    TYPE(siepmann_pot_type), POINTER         :: siepmann
    TYPE(pos_type), DIMENSION(:), POINTER    :: r_last_update_pbc
    INTEGER, INTENT(IN)                      :: iparticle, jparticle
    REAL(KIND=dp), DIMENSION(3)              :: cell_v
    ...
    TYPE(particle_type), DIMENSION(:), &
      POINTER                                :: particle_set
    INTEGER, INTENT(INOUT), OPTIONAL         :: nr_oh
    ...
    DO iatom=1,natom
       ...
       ELSEIF(count_h==2) THEN
        index_h2=iatom
       ELSE
        CALL cp__b("manybody_siepmann.F",386,"Error in Siepmann-Sprik part: too many H atoms")
       ENDIF 
    ENDDO

    IF (count_h==0) THEN
        CALL cp__b("manybody_siepmann.F",391,"No H atoms for O found")
    ELSEIF (count_h==1) THEN
        IF(siepmann%allow_oh_formation) THEN
           IF(PRESENT(nr_oh)) THEN
              nr_oh = nr_oh + 1
           ENDIF
           siep_Phi_ij = 0.0_dp
        ELSE
          CALL cp__b("manybody_siepmann.F",399,"Only one H atom of O atom found")
        ENDIF
    ENDIF
  ...
  SUBROUTINE siep_Phi_ij_d(siepmann,r_last_update_pbc,iparticle,jparticle,f_nonbond,&
    ...
    DO iatom=1,natom
       ...
       ELSEIF(count_h==2) THEN
        index_h2=iatom
       ELSE
        CALL cp__b("manybody_siepmann.F",487,"Error in Siepmann-Sprik part: too many H atoms")
       ENDIF 
    ENDDO

    IF (count_h==0) THEN
        CALL cp__b("manybody_siepmann.F",492,"No H atoms for O found")
    ELSEIF (count_h==1.AND..NOT.siepmann%allow_oh_formation) THEN
        CALL cp__b("manybody_siepmann.F",494,"Only one H atom of O atom found")
    ENDIF 
    ...
    IF(count_h==2) THEN
     ...
     IF (use_virial) THEN
      CALL cp_abort(cp__l("manybody_siepmann.F",536),&
           "using virial with Siepmann-Sprik"//&
           " not implemented")
     END IF
...
! *****************************************************************************
  SUBROUTINE siepmann_forces_v3(siepmann,r_last_update_pbc, cell_v, n_loc_size,&
    ...
    IF (use_virial) THEN
     CALL cp_abort(cp__l("manybody_siepmann.F",613),&
          "using virial with Siepmann-Sprik"//&
          " not implemented")
    END IF
    ...
    IF (use_virial) THEN
     CALL cp_abort(cp__l("manybody_siepmann.F",628),&
          "using virial with Siepmann-Sprik"//&
          " not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE siepmann_forces_v2(siepmann,r_last_update_pbc, cell_v, cell,&
    ...
    IF (use_virial) THEN
     CALL cp_abort(cp__l("manybody_siepmann.F",703),&
          "using virial with Siepmann-Sprik"//&
          " not implemented")
    END IF
...
!   ! Lets do the f_A2 piece derivative of rji**(-3)
    ...
    IF (use_virial) THEN
     CALL cp_abort(cp__l("manybody_siepmann.F",718),&
          "using virial with Siepmann-Sprik"//&
          " not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE setup_siepmann_arrays(nonbonded, potparm, glob_loc_list, glob_cell_v,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(glob_loc_list)))CALL cp__a("manybody_siepmann.F",764)
    IF(.NOT.(.NOT.ASSOCIATED(glob_loc_list_a)))CALL cp__a("manybody_siepmann.F",765)
    IF(.NOT.(.NOT.ASSOCIATED(glob_cell_v)))CALL cp__a("manybody_siepmann.F",766)
...
! *****************************************************************************
!> \brief prints the number of OH ions
!> \param nr_oh ...
!> \param mm_section ...
!> \param para_env ...
! *****************************************************************************
  SUBROUTINE print_nr_oh_siepmann(nr_oh,mm_section,para_env)
    INTEGER, INTENT(INOUT)                   :: nr_oh
    TYPE(section_vals_type), POINTER         :: mm_section
    TYPE(cp_para_env_type), OPTIONAL, &
      POINTER                                :: para_env

    CHARACTER(LEN=*), PARAMETER :: routineN = 'print_nr_oh_siepmann', &
      routineP = moduleN//':'//routineN
    ...
    CALL mp_sum(nr_oh,para_env%group)
    logger => cp_get_default_logger()

    iw= cp_print_key_unit_nr(logger,mm_section,"PRINT%PROGRAM_RUN_INFO",&
        extension=".mmLog")

    IF(iw > 0 .AND. nr_oh > 0) THEN
      WRITE(iw,'(/,A,T71,I10,/)') " SIEPMANN: number of OH ions at surface", nr_oh
    ENDIF

    CALL cp_print_key_finished_output(iw,logger,mm_section,"PRINT%PROGRAM_RUN_INFO")

  END SUBROUTINE print_nr_oh_siepmann
