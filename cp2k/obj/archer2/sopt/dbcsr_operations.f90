# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/../../base/base_uses.f90" 1
...
# 108 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_conjg(matrix)
    ...
    INTEGER                                  :: blk, col, data_type, handle, &
                                                row
    LOGICAL                                  :: tr
...
! *****************************************************************************
  SUBROUTINE dbcsr_scale_anytype(matrix_a, alpha_scalar, limits)
    ...
    INTEGER :: a_col, a_col_size, a_row, a_row_size, col_offset, handle, &
      row_offset, scale_col_offset, scale_col_size, scale_row_offset, &
      scale_row_size
       ...
       IF (limits(last_col_i) .NE. 0) THEN
          IF (debug_mod) THEN
             CALL dbcsr_assert (limits(last_col_i) .GE. 0, "AND",&
               ...
               "Specified last column is out of bounds.",359)
          ENDIF
       ...
       IF (limits(first_col_i) .NE. 0) THEN
          IF (debug_mod) THEN
             CALL dbcsr_assert (limits(first_col_i) .GE. 0, "AND",&
                  ...
                  dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
               "Specified first column is out of bounds.",368)
          ENDIF
       ...
       IF (limits(last_row_i) .NE. 0) THEN
          IF (debug_mod) THEN
             CALL dbcsr_assert (limits(last_row_i) .GE. 0, "AND",&
                  ...
                  dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
               "Specified last row is out of bounds.",377)
          ENDIF
       ...
       IF (limits(first_row_i) .NE. 0) THEN
          IF (debug_mod) THEN
             CALL dbcsr_assert (limits(first_row_i) .GE. 0, "AND",&
                  ...
                  dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
               "Specified first row is out of bounds.",386)
          ENDIF
...
! *****************************************************************************
  SUBROUTINE dbcsr_scale_by_vector_anytype(matrix_a, alpha, side)
    ...
    INTEGER :: a_blk, a_col, a_col_size, a_nze, a_row, a_row_size, &
      col_offset, data_type, handle, i, icol, irow, row_offset
    LOGICAL                                  :: right, tr
    TYPE(dbcsr_data_obj)                     :: data_any
    TYPE(dbcsr_iterator)                     :: iter
...
! check that alpha and matrix have the same data type
    CALL dbcsr_assert (dbcsr_get_data_type (matrix_a).EQ.alpha%d%data_type, dbcsr_fatal_level,&
         dbcsr_wrong_args_error, routineN, "wrong data type matrix_a",493)
    !
    ! set vars
    right = .TRUE.
    SELECT CASE(side)
...
!>
! *****************************************************************************
  SUBROUTINE dbcsr_add_anytype(matrix_a, matrix_b, alpha_scalar, beta_scalar)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix_a
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_valid_index (matrix_a), dbcsr_fatal_level,&
         dbcsr_caller_error, routineN, "Invalid matrix", 651)
    ...
    CALL dbcsr_assert (dbcsr_nblkrows_total(matrix_a).EQ.dbcsr_nblkrows_total(matrix_b), &
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN, "matrices not consistent",672)

    do_scale = dbcsr_scalar_are_equal (&
         my_beta_scalar, dbcsr_scalar_one (data_type_b))

    CALL dbcsr_scale(matrix_a, alpha_scalar=my_alpha_scalar)
  ...
  SUBROUTINE dbcsr_add_d(matrix_a, matrix_b, alpha_scalar, beta_scalar)
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Invalid combination of data type, NYI",777)
    ENDIF
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE dbcsr_add_s(matrix_a, matrix_b, alpha_scalar, beta_scalar)
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Invalid combination of data type, NYI",807)
    ENDIF
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE dbcsr_add_z(matrix_a, matrix_b, alpha_scalar, beta_scalar)
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Invalid combination of data type, NYI",842)
    ENDIF
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE dbcsr_add_c(matrix_a, matrix_b, alpha_scalar, beta_scalar)
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Invalid combination of data type, NYI",872)
    ENDIF
    CALL timestop(handle)
       ...
       CASE (dbcsr_type_real_8)
          ...
          CASE (dbcsr_func_artanh)
             ...
             CALL dbcsr_assert(MAXVAL(ABS(a_data%d%r_dp)).LT.1.0_real_8,&
                     dbcsr_fatal_level,dbcsr_internal_error, routineN,&
                     "ARTANH is undefined for |x|>=1", 1004)
          ...
          CASE (dbcsr_func_asin)
             ...
             CALL dbcsr_assert(MAXVAL(ABS(a_data%d%r_dp)).LE.1.0_real_8,&
                     dbcsr_fatal_level,dbcsr_internal_error, routineN,&
                     "ASIN is undefined for |x|>1", 1020)
             a_data%d%r_dp(1:nze) = ASIN(a_data%d%r_dp(1:nze))
          CASE DEFAULT
             CALL dbcsr_assert(.FALSE.,&
                     dbcsr_fatal_level,dbcsr_internal_error, routineN,&
                     "Unknown function of matrix elements", 1025)
          END SELECT
       !CASE (dbcsr_type_complex_4)
       !CASE (dbcsr_type_complex_8)
       CASE DEFAULT
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_get_data_type(matrix_a).EQ.dbcsr_get_data_type(matrix_b).AND.&
         ...
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN, &
         "data types not consistent, need to fix that",1084)
   ...
   DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       CASE (dbcsr_type_real_8)
          ...
          CALL dbcsr_assert (tr_a.EQV.tr_b, dbcsr_fatal_level, dbcsr_wrong_args_error, routineN, &
               "tr not consistent, need to fix that",1110)
          IF(found) THEN
             SELECT CASE (data_type)
             CASE (dbcsr_type_real_4)
       ...
       CASE DEFAULT
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
               routineN,"Only real double precision",1144)
       END SELECT
    ENDDO
    CALL dbcsr_iterator_stop(iter)
...
! *****************************************************************************
  SUBROUTINE dbcsr_add_on_diag_anytype(matrix, alpha_scalar, first_row, last_row)
    ...
    INTEGER :: handle, hold, imax, imin, my_first_row, my_last_row, mynode, &
      offset_beg, offset_end, row, row_size, stored_row
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_nblkrows_total(matrix).EQ.dbcsr_nblkcols_total(matrix).AND.&
         ...
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN, "matrices not consistent",1183)
...
! *****************************************************************************
  SUBROUTINE dbcsr_init_random(matrix, keep_sparsity)
    ...
    INTEGER                                  :: col, col_size, handle, hold, &
                                                iseed(4), mynode, ncol, nrow, &
                                                row, row_size, stored_col, &
                                                stored_row
...
! *****************************************************************************
  LOGICAL FUNCTION symmetry_consistent(matrix_type,data_type)
    ...
    CASE DEFAULT
      CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,&
           routineN, "Invalid data type.",1495)
    END SELECT 
  ...
  LOGICAL FUNCTION symmetry_compatible(matrix_type_a,matrix_type_b)
    ...
    CASE DEFAULT
      CALL dbcsr_assert(.FALSE., dbcsr_failure_level,&
           dbcsr_wrong_args_error, routineP, "Invalid matrix type.",1533)
    END SELECT
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (symmetry_consistent(dbcsr_get_matrix_type(matrix_a),&
                                           dbcsr_get_data_type(matrix_a)),&
         dbcsr_fatal_level, dbcsr_internal_error, routineN,&
         "Source matrix symmetry not consistent with its data type.",1575)
    ...
    CALL dbcsr_assert (dbcsr_is_initialized (matrix_b),&
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
         "Target matrix must be initialized",1581)
    CALL dbcsr_assert (.not.keep_sparse.or.dbcsr_valid_index(matrix_b),&
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
         "Target matrix must be valid to keep its sparsity",1584)
    IF(keep_sparse.AND.shallow)&
       CALL cp__w("dbcsr/ops/dbcsr_operations.F",1586,"Shallow copy not compatibly with sparsity retainment")
    IF (keep_sparse) THEN
    ...
    ELSE
       ...
       END IF
       CALL dbcsr_assert (symmetry_consistent(new_matrix_type,new_type),&
            dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
       ...
       IF (.NOT. shallow) THEN
          CALL dbcsr_assert (matrix_a%m%nze, "LE", dbcsr_get_data_size(matrix_a),&
               dbcsr_fatal_level, dbcsr_internal_error, routineN,&
               "Source matrix sizes not consistent!",1632)
       ...
       ELSE
          CALL dbcsr_assert (dbcsr_get_data_type(matrix_a) &
               ...
               "Shallow copy only possible when retaining data type.", 1647)
...
! *****************************************************************************
  SUBROUTINE dbcsr_copy_into_existing(matrix_b, matrix_a)
    ...
    INTEGER                                  :: col_size, data_type, dst_col, &
                                                dst_row, handle, rel, &
                                                row_size, src_col, src_cs, &
                                                src_row, src_rs
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_get_data_type(matrix_b)&
         ...
         dbcsr_wrong_args_error, routineN, "Matrices have different data types.",1688)
    ...
    DO WHILE (dbcsr_iterator_blocks_left (dst_iter))
       ...
       CASE (0)
          ...
          CALL dbcsr_assert (dbcsr_data_get_size (src_data)&
               ...
               dbcsr_internal_error, routineN, "Block sizes not equal!",1734)
       ...
       CASE default
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error,&
               routineN, "Trouble syncing iterators",1754)
       END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE dbcsr_copy_into_existing_sym(matrix_b, matrix_a)
    ...
    INTEGER                                  :: col_size, data_type, dst_col, &
                                                dst_row, handle, row_size, &
                                                src_col, src_cs, src_row, &
                                                src_rs
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_get_data_type(matrix_b)&
         .EQ. dbcsr_get_data_type(matrix_a), dbcsr_fatal_level,&
         dbcsr_wrong_args_error, routineN, "Matrices have different data types.",1793)
    data_type = dbcsr_get_data_type (matrix_b)
...
! *****************************************************************************
  SUBROUTINE dbcsr_copy_submatrix(matrix_b, matrix_a, name,&
    ...
    INTEGER                                  :: blk_p, col, handle, nblocks, &
                                                new_blk, old_blk, row
...
! *****************************************************************************
  SUBROUTINE dbcsr_crop_matrix(matrix_b, matrix_a, &
    ...
    INTEGER                                  :: col, f_col_f, f_row_f, &
                                                handle, l_col_l, l_row_l, row
...
! *****************************************************************************
  SUBROUTINE dbcsr_filter_anytype(matrix, eps, method,&
    ...
    COMPLEX(KIND=real_8), DIMENSION(:), &
      POINTER                                :: data_z
    INTEGER                                  :: blk, blk_nze, col, col_size, &
                                                handle, my_method, row, &
                                                row_size
    LOGICAL                                  :: gt0, my_filter_diag, &
                                                reshuffle, tr
    ...
    IF(gt0) THEN
       ...
       DO WHILE (dbcsr_iterator_blocks_left(iter))
          ...
          CASE (dbcsr_type_real_4)
             ...
             CASE DEFAULT
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                     routineN,"Only Frobenius based filtering",2342)
             END SELECT
          ...
          CASE (dbcsr_type_real_8)
             ...
             CASE DEFAULT
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                     routineN,"Only Frobenius based filtering",2359)
             END SELECT
          ...
          CASE (dbcsr_type_complex_4)
             ...
             CASE DEFAULT
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                     routineN,"Only Frobenius based filtering",2376)
             END SELECT
          ...
          CASE (dbcsr_type_complex_8)
             ...
             CASE DEFAULT
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                     routineN,"Only Frobenius based filtering",2393)
             END SELECT

          CASE DEFAULT
             CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
                  routineN,"Wrong data type",2398)
          END SELECT
       ENDDO
       CALL dbcsr_iterator_stop(iter)
...
! *****************************************************************************
  SUBROUTINE dbcsr_norm_anytype(matrix, which_norm, norm_scalar, norm_vector)
    ...
    INTEGER                                  :: blk, col, col_offset, handle, &
                                                i, j, row, row_offset
    LOGICAL                                  :: tr
    TYPE(dbcsr_data_obj)                     :: data_a
    TYPE(dbcsr_iterator)                     :: iter
    ...
    CASE(dbcsr_norm_column)
       ...
       IF(PRESENT(norm_vector)) THEN
          CALL dbcsr_assert (dbcsr_data_get_type(norm_vector), "EQ",&
               ...
               dbcsr_wrong_args_error, routineN,&
               "Mismatched vector/matrix data types", 2460)
          ...
          DO WHILE (dbcsr_iterator_blocks_left(iter))
             ...
             CASE DEFAULT
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                     routineN,"Only real values",2525)
             END SELECT
          ENDDO
          CALL dbcsr_iterator_stop(iter)
          CALL dbcsr_data_clear_pointer (data_a)
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (nr.EQ.nc, dbcsr_fatal_level, dbcsr_wrong_args_error, &
         routineN, "not a square matrix",2653)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       CASE (dbcsr_type_complex_4)
          ...
          DO i=1,SIZE(data_c,1)
             ...
             IF(any_sym.AND.row.NE.col) &
                  CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                  routineN,"Only nonsymmetric matrix so far",2689)
       ...
       CASE (dbcsr_type_complex_8)
          ...
          DO i=1,SIZE(data_z,1)
             ...
             IF(any_sym.AND.row.NE.col) &
                  CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                  routineN,"Only nonsymmetric matrix so far",2701)
             !     buff_d(col_offset+j-1) = buff_d(col_offset+j-1) + ABS(data_z(i,j))
          ENDDO
          ENDDO
       CASE DEFAULT
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
               routineN,"Wrong data type",2707)
       END SELECT
    ENDDO
    CALL dbcsr_iterator_stop(iter)
...
!   ---------------------------------------------------------------------------
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       CASE (dbcsr_type_complex_4)
          ...
          IF(any_sym.AND.row.NE.col) &
               CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
               routineN,"Only nonsymmetric matrix so far",2827)
       ...
       CASE (dbcsr_type_complex_8)
          ...
          IF(any_sym.AND.row.NE.col) &
               CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
               routineN,"Only nonsymmetric matrix so far",2834)
          norm = norm + fac * REAL(SUM(CONJG(data_z)*data_z),KIND=real_8)
       CASE DEFAULT
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
               routineN,"Wrong data type",2838)
       END SELECT
    ENDDO
    CALL dbcsr_iterator_stop(iter)
...
! *****************************************************************************
  SUBROUTINE dbcsr_sum_replicated (matrix)
    ...
    INTEGER                                  :: comm, handle, index_checksum, &
                                                mynode, numnodes
...
!   ---------------------------------------------------------------------------
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect data type", 2893)
    END SELECT
    !
    CALL timestop(handle)
...
! *****************************************************************************
  SUBROUTINE dbcsr_trace_a_b_d(matrix_a, matrix_b, trace, trans_a, trans_b, local_sum)
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
            "Invalid combination of data type, NYI",2936)
    ENDIF
    CALL timestop(handle)
...
!   ---------------------------------------------------------------------------
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error,&
            routineN, "Invalid data type.",2971)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_get_info(matrix, nblkrows_total, nblkcols_total,&
    ...
    INTEGER, DIMENSION(:), OPTIONAL, POINTER :: local_rows, local_cols, &
      proc_row_dist, proc_col_dist, row_blk_size, col_blk_size, &
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (matrix%m%initialized,&
         ...
         "Matrix not initialized",3063)
    !vw avoid massive priting of warnings
    !CALL cp__w("dbcsr/ops/dbcsr_operations.F",3065,"Invalid matrix")
    ...
    IF (PRESENT (matrix_type)) THEN
       ...
       CALL dbcsr_assert (matrix_type.NE.dbcsr_type_invalid, dbcsr_failure_level, dbcsr_caller_error,&
            routineN, "Incorrect symmetry",3084)
    ENDIF
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations_d.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
    ...
    DO a_row = 1, matrix_a%m%nblkrows_total
       ...
       DO a_blk = matrix_a%m%row_p(a_row)+1,matrix_a%m%row_p(a_row+1)
          ...
          CALL dbcsr_assert (a_row_size.EQ.a_col_size, dbcsr_fatal_level,&
               dbcsr_internal_error, routineN, "is that a square matrix?",58)
          a_nze = a_row_size**2
...
!> \param local_sum ...
! *****************************************************************************
  SUBROUTINE dbcsr_trace_ab_d(matrix_a, matrix_b, trace, trans_a, trans_b, local_sum)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix_a, matrix_b
    REAL(kind=real_8), INTENT(INOUT)                   :: trace
    CHARACTER(LEN=*), INTENT(IN), OPTIONAL   :: trans_a, trans_b
    LOGICAL, INTENT(IN), OPTIONAL            :: local_sum
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.my_local_sum) THEN!fixme
       CALL dbcsr_assert (matrix_a%m%replication_type .EQ. dbcsr_repl_none&
            ...
            "Trace of product of replicated matrices not yet possible.",120)
    ENDIF
    ...
    IF((dbcsr_get_matrix_type(matrix_a).EQ.dbcsr_type_symmetric.OR.&
       ...
       (dbcsr_get_matrix_type(matrix_b).EQ.dbcsr_type_symmetric.OR.&
        dbcsr_get_matrix_type(matrix_b).EQ.dbcsr_type_antisymmetric)) THEN
       CALL dbcsr_assert (.FALSE.,dbcsr_fatal_level,&
            dbcsr_unimplemented_error_nr, routineN, "Tracing general with symmetric matrix NYI",135)
    ENDIF
    ...
    CALL dbcsr_assert (my_trans_a.EQ.'T'.AND.my_trans_b.EQ.'N', dbcsr_fatal_level,&
         dbcsr_unimplemented_error_nr, routineN, "this combination of transpose is NYI",156)
    !
    ! let's go
    trace = REAL(0.0,real_8)
    CALL dbcsr_assert (matrix_a%m%nblkrows_total.EQ.matrix_b%m%nblkrows_total,&
         dbcsr_fatal_level, dbcsr_internal_error, routineN, "this combination of transpose is NYI",161)
    DO row = 1, matrix_a%m%nblkrows_total
       ...
       IF(a_row_size.NE.b_row_size) THEN
          ...
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
               routineN, "matrices not consistent",169)
       ENDIF
       ...
       DO a_blk = matrix_a%m%row_p(row)+1,matrix_a%m%row_p(row+1)
          ...
          IF(found) THEN
             ...
             IF(a_col_size.NE.b_col_size)  THEN
                ...
                CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
                     routineN, "matrices not consistent",188)
             ENDIF
             !
             ...
             IF(nze.GT.0) THEN
                ...
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_4.AND.&
                   ...
                   trace = trace + &
                        REAL(fac * SUM ( a_data_r(a_beg:a_end) * b_data_d(b_beg:b_end) ),kind=real_8)
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_8.AND.&
                       matrix_b%m%data_type.EQ.dbcsr_type_real_4) THEN
                   trace = trace + &
                        REAL(fac * SUM ( a_data_d(a_beg:a_end) * b_data_r(b_beg:b_end) ),kind=real_8)
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_8.AND.&
                       matrix_b%m%data_type.EQ.dbcsr_type_real_8) THEN
                ...
                ELSE
                   CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
                        routineN, "combination of types NYI",233)
                ENDIF
             ENDIF
          ENDIF
       ENDDO ! a_col
...
!> \param last_row ...
! *****************************************************************************
  SUBROUTINE dbcsr_add_on_diag_d(matrix, alpha_scalar, first_row, last_row)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix
    REAL(kind=real_8), INTENT(IN)                      :: alpha_scalar
    INTEGER, INTENT(in), OPTIONAL            :: first_row, last_row
...
# 3170 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations_z.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!> \param local_sum ...
! *****************************************************************************
  SUBROUTINE dbcsr_trace_ab_z(matrix_a, matrix_b, trace, trans_a, trans_b, local_sum)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix_a, matrix_b
    COMPLEX(kind=real_8), INTENT(INOUT)                   :: trace
    CHARACTER(LEN=*), INTENT(IN), OPTIONAL   :: trans_a, trans_b
    LOGICAL, INTENT(IN), OPTIONAL            :: local_sum
...
# 3171 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations_s.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
# 3172 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations_c.f90" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
!   ---------------------------------------------------------------------------
    ...
    row_blk_size => array_data (matrix_a%m%row_blk_size)
    col_blk_size => array_data (matrix_a%m%col_blk_size)
    CALL dbcsr_assert (dbcsr_get_data_type (matrix_a), "EQ", dbcsr_type_complex_4,&
         dbcsr_fatal_level, dbcsr_wrong_args_error, routineN,&
         "Incompatible data types", 35)
    CALL dbcsr_get_data (matrix_a%m%data_area, data_p)
    dist = dbcsr_distribution (matrix_a)
          ...
          IF(found) THEN
             ...
             IF(nze.GT.0) THEN
                ...
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_4.AND.&
                     matrix_b%m%data_type.EQ.dbcsr_type_real_8) THEN
                   trace = trace + &
                        CMPLX(fac * SUM ( a_data_r(a_beg:a_end) * b_data_d(b_beg:b_end) ),kind=real_4)
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_8.AND.&
                       matrix_b%m%data_type.EQ.dbcsr_type_real_4) THEN
                   trace = trace + &
                        CMPLX(fac * SUM ( a_data_d(a_beg:a_end) * b_data_r(b_beg:b_end) ),kind=real_4)
                ELSEIF(matrix_a%m%data_type.EQ.dbcsr_type_real_8.AND.&
                       matrix_b%m%data_type.EQ.dbcsr_type_real_8) THEN
                   trace = trace + CMPLX(fac * DDOT (nze,&
                        a_data_d(ABS(matrix_a%m%blk_p(a_blk))),1,&
...
! *****************************************************************************
  SUBROUTINE dbcsr_scale_by_vector_c(matrix_a, alpha, side)
    ...
    CHARACTER(LEN=*), INTENT(IN)              :: side
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_scale_by_vector_c', &
      routineP = moduleN//':'//routineN
    COMPLEX(kind=real_4), DIMENSION(:), POINTER            :: tmp_p
    TYPE(dbcsr_data_obj)                      :: enc_alpha_vec
...
! *****************************************************************************
  SUBROUTINE dbcsr_set_diag_c(matrix, diag)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_set_diag_c', &
      routineP = moduleN//':'//routineN

    COMPLEX(kind=real_4), DIMENSION(:), POINTER           :: diag_p
    TYPE(dbcsr_data_obj)                     :: diag_a
...
! *****************************************************************************
  SUBROUTINE dbcsr_get_diag_c(matrix, diag)
    ...
    TYPE(dbcsr_obj), INTENT(IN)                    :: matrix
    COMPLEX(kind=real_4), DIMENSION(:), INTENT(INOUT), TARGET   :: diag

    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_get_diag_c', &
      routineP = moduleN//':'//routineN
...
# 3173 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_operations.F" 2
