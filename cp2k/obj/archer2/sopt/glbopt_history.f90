# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/glbopt_history.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/glbopt_history.F"
!-----------------------------------------------------------------------------!
...
! *****************************************************************************
!> \brief History of minima, calulates, stores and compares fingerprints of minima.
...
# 16 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/swarm/glbopt_history.F" 2
...
!>        Calulates a fingerprint based on inter-atomic distances.
!> \param pos ...
!> \param res ...
...
! *****************************************************************************
 SUBROUTINE goedecker_fingerprint(pos, res)
    ...
    IF(ASSOCIATED(res)) CALL cp__b("swarm/glbopt_history.F",131,"goedecker_fingerprint: res already allocated")
    ...
    IF(info /= 0) CALL cp__b("swarm/glbopt_history.F",149,"goedecker_fingerprint: DSYEV failed")
...
!>        Calulates the distance between two given fingerprints.
!> \param fp1 ...
!> \param fp2 ...
!> \retval res ...
!> \author Stefan Goedecker
       ...
       DO k=1, history%length
          ...
          IF(k>1)THEN
             IF(history%entries(k-1)%p%Epot > history%entries(k)%p%Epot) &
               CALL cp__b("swarm/glbopt_history.F",241,"history_add: history in wrong order")
           END IF
       END DO
    ENDIF
...
! *****************************************************************************
  SUBROUTINE history_lookup(history, fingerprint, found, id)
    ...
    INTEGER                                  :: found_i, handle, i, k, k_max, &
                                                k_min
...
! *****************************************************************************
  SUBROUTINE verify_history_lookup(history, fingerprint, found_i_ref)
    ...
    IF(found_i /= found_i_ref) THEN
      ...
      CALL cp__b("swarm/glbopt_history.F",384,"verify_history_lookup failed")
    ENDIF
