# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_grid_atom.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_grid_atom.F"
!-----------------------------------------------------------------------------!
...
# 17 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_grid_atom.F" 2
...
! *****************************************************************************
  SUBROUTINE deallocate_grid_atom(grid_atom)
    ...
    IF (ASSOCIATED(grid_atom)) THEN
       ...
       IF (ASSOCIATED(grid_atom%cotan_pol)) THEN
          DEALLOCATE (grid_atom%cotan_pol)
       END IF
    ...
    ELSE
       CALL cp_abort(cp__l("qs_grid_atom.F",158),&
...
! *****************************************************************************
  SUBROUTINE create_grid_atom(grid_atom,nr,na,llmax,ll,quadrature)
    ...
    IF (ASSOCIATED(grid_atom)) THEN
       ...
       CALL reallocate(grid_atom%cotan_pol,1,na)
       ...
       IF (ll > 0) THEN
          ...
          DO ia = 1,na
             ...
             IF(ABS(grid_atom%sin_pol(ia)) > EPSILON(1.0_dp)) THEN
                grid_atom%cotan_pol(ia) = &
                     grid_atom%cos_pol(ia)/grid_atom%sin_pol(ia)
             ELSE
                grid_atom%cotan_pol(ia) = 0.0_dp
             END IF
    ...
    ELSE
       CALL cp__b("qs_grid_atom.F",265,"The pointer grid_atom is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE radial_grid(n,r,r2,wr,radial_quadrature)
    ...
    CASE DEFAULT
       ...
       CALL cp__b("qs_grid_atom.F",357,"Invalid radial quadrature type specified")
