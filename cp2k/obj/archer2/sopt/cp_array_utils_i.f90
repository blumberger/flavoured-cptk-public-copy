# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_i.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_i.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_i.F" 2

  IMPLICIT NONE
  PRIVATE

  LOGICAL, PRIVATE, PARAMETER :: debug_this_module=.TRUE.
  CHARACTER(len=*), PARAMETER, PRIVATE :: moduleN = 'cp_array_utils_i'

  ! types
  PUBLIC :: cp_1d_i_p_type, cp_2d_i_p_type, cp_3d_i_p_type

  ! generic interfaces
  PUBLIC :: cp_guarantee_size

  ! the underlying procedures
  PUBLIC :: cp_1d_i_guarantee_size, &
...
! *****************************************************************************
SUBROUTINE cp_1d_i_write(array, unit_nr, el_format)
    INTEGER(kind=int_4), INTENT(in)          :: array(:)
    INTEGER, INTENT(in)                      :: unit_nr
    CHARACTER(len=*), INTENT(in), OPTIONAL   :: el_format

    CHARACTER(len=*), PARAMETER :: defaultFormat = "(i6)", &
      routineN = 'cp_1d_i_write', routineP = moduleN//':'//routineN

    INTEGER                                  :: i, iostat

  WRITE(unit=unit_nr,fmt="('( ')",advance="no", iostat=iostat)
  IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_i.F",127)
...
! *****************************************************************************
SUBROUTINE cp_2d_i_write(array, unit_nr, el_format)
    INTEGER(kind=int_4), INTENT(in)          :: array(:,:)
    INTEGER, INTENT(in)                      :: unit_nr
    CHARACTER(len=*), INTENT(in), OPTIONAL   :: el_format

    CHARACTER(len=*), PARAMETER :: defaultFormat = "(i6)", &
      routineN = 'cp_2d_i_write', routineP = moduleN//':'//routineN
  ...
  DO i=1,SIZE(array,1)
     ...
     ELSE
        ...
        WRITE(unit=unit_nr,fmt=fmtstr,iostat=iostat) array(i,:)
     END IF
     IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_i.F",188)
  END DO
  CALL m_flush(unit_nr)
...
! *****************************************************************************
  SUBROUTINE cp_1d_i_guarantee_size(array, n)
    INTEGER(kind=int_4), POINTER             :: array(:)
    INTEGER, INTENT(in)                      :: n

    CHARACTER(len=*), PARAMETER :: routineN = 'cp_1d_i_guarantee_size', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(n>=0))CALL cp__a("common/cp_array_utils_i.F",217)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array) /= n) THEN
          CALL cp__w("common/cp_array_utils_i.F",220,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_2d_i_guarantee_size(array, n_rows, n_cols)
    ...
    IF(.NOT.(n_cols>=0))CALL cp__a("common/cp_array_utils_i.F",253)
    IF(.NOT.(n_rows>=0))CALL cp__a("common/cp_array_utils_i.F",254)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array,1) /= n_rows .OR. SIZE(array,2) /= n_cols) THEN
          CALL cp__w("common/cp_array_utils_i.F",257,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
    IF (.NOT.ASSOCIATED(array)) THEN
...
! *****************************************************************************
FUNCTION cp_1d_i_bsearch(array, el, l_index, u_index)&
     RESULT(res)
    INTEGER(kind=int_4), INTENT(in)          :: array(:), el
    INTEGER, INTENT(in), OPTIONAL            :: l_index, u_index
    INTEGER                                  :: res

    CHARACTER(len=*), PARAMETER :: routineN = 'cp_1d_i_bsearch', &
      routineP = moduleN//':'//routineN
...
!  lessQ = "CP_I_LESS_Q"
!  nametype1 = "i"
!  type1 = "integer(kind=int_4)"


END MODULE cp_array_utils_i
