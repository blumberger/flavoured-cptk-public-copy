# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_load_balance_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_load_balance_methods.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_load_balance_methods.F" 2
...
!$OMP MASTER
      ...
      IF(para_env%mepos == 0 ) THEN
        IF( shm_local_quartet_counter /= shm_global_quartet_counter ) THEN
          ...
          CALL cp__b("hfx_load_balance_methods.F",697,error_msg)
        END IF
      END IF
!$OMP END MASTER
...
! *****************************************************************************
  SUBROUTINE hfx_update_load_balance(x_data, para_env, &
    ...
    INTEGER :: data_from, dest, end_idx, handle, i, ibin, icpu, iprocess, j, &
      ...
      nbins, ncpu, req(2), source, start_idx
    INTEGER(int_8), DIMENSION(:), POINTER    :: local_cost_matrix, recbuffer, &
                                                sendbuffer, swapbuffer
    INTEGER(int_8), DIMENSION(:), POINTER, &
      SAVE                                   :: cost_matrix
...
! *****************************************************************************
  SUBROUTINE optimize_distribution(total_number_of_bins, number_of_processes, bin_costs, &
    ...
    DO i=total_number_of_bins,1,-nstep
       ...
       IF( do_randomize ) THEN
         CALL reshuffle(MIN(i,nstep), tmp_cpu_index(1:MIN(i,nstep)),rng_stream)
       END IF
       DO j=1,MIN(i,nstep)
...
# 2621 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_load_balance_methods.F" 2
