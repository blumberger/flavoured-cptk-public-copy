# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xray_diffraction.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xray_diffraction.F"
!-----------------------------------------------------------------------------!
...
# 67 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xray_diffraction.F" 2
...
! *****************************************************************************
  SUBROUTINE xray_diffraction_spectrum(qs_env,unit_number,q_max)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("xray_diffraction.F",126)
    ...
    DO ig=2,ng
      IF(.NOT.(gsq(ig)>=gsq(jg)))CALL cp__a("xray_diffraction.F",221)
...
! *****************************************************************************
  SUBROUTINE calculate_rhotot_elec_gspace(qs_env,auxbas_pw_pool,&
    ...
    INTEGER :: atom, handle, iatom, ico, ico1_pgf, ico1_set, ikind, ipgf, &
      iset, iso, iso1_pgf, iso1_set, ison, ispin, jco, jco1_pgf, jco1_set, &
      jpgf, jset, jso, jso1_pgf, jso1_set, json, la, lb, maxco, maxso, na, &
      natom, nb, ncoa, ncob, ncotot, nkind, nsatbas, nset, nsoa, nsob, &
      nsotot, nspin
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("xray_diffraction.F",547)
    IF(.NOT.(ASSOCIATED(auxbas_pw_pool)))CALL cp__a("xray_diffraction.F",548)
    ...
    IF(.NOT.(orthorhombic))CALL cp__a("xray_diffraction.F",617)
