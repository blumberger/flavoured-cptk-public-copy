# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_topology_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_topology_util.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_topology_util.F" 2
...
! *****************************************************************************
  SUBROUTINE qmmm_coordinate_control(topology, qmmm_env, subsys_section)
    ...
    !
    IF(.NOT.(SIZE(qmmm_env%qm_atom_index) /= 0))CALL cp__a("qmmm_topology_util.F",70)
    qmmm_index_in_range=(MAXVAL(qmmm_env%qm_atom_index)<=SIZE(topology%atom_info%id_atmname)) &
                        .AND.(MINVAL(qmmm_env%qm_atom_index)>0)
    IF(.NOT.(qmmm_index_in_range))CALL cp__a("qmmm_topology_util.F",73)
       ...
       IF (ANY(qm_atom_index >= first_atom .AND. qm_atom_index <= last_atom)) THEN
          ...
          IF (detected_link) THEN
             ...
             IF (.NOT.qmmm_env%qmmm_link) THEN
                ...
                CALL cp__b("qmmm_topology_util.F",176,"")
             END IF
          END IF
