# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hirshfeld_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hirshfeld_methods.F"
!-----------------------------------------------------------------------------!
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hirshfeld_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE create_shape_function(hirshfeld_env,qs_kind_set,atomic_kind_set)
    ...
    IF(.NOT.(ASSOCIATED(hirshfeld_env)))CALL cp__a("hirshfeld_methods.F",156)
    ...
    SELECT CASE(hirshfeld_env%shape_function_type)
       ...
       CASE DEFAULT
          CALL cp__b("hirshfeld_methods.F",191,"Unknown shape function")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE comp_hirshfeld_charges(qs_env,hirshfeld_env,charges)
    ...
    DO is=1,SIZE(rho_r)
      ...
      ELSE
        CALL cp__b("hirshfeld_methods.F",239,"We need rho in real space")
      END IF
...
! *****************************************************************************
  SUBROUTINE hfun_scale(fout,fun1,fun2)
    ...
    n2 = SIZE(fout,2)
    n3 = SIZE(fout,3)
    IF(.NOT.(n1==SIZE(fun1,1)))CALL cp__a("hirshfeld_methods.F",270)
    IF(.NOT.(n2==SIZE(fun1,2)))CALL cp__a("hirshfeld_methods.F",271)
    IF(.NOT.(n3==SIZE(fun1,3)))CALL cp__a("hirshfeld_methods.F",272)
    IF(.NOT.(n1==SIZE(fun2,1)))CALL cp__a("hirshfeld_methods.F",273)
    IF(.NOT.(n2==SIZE(fun2,2)))CALL cp__a("hirshfeld_methods.F",274)
    IF(.NOT.(n3==SIZE(fun2,3)))CALL cp__a("hirshfeld_methods.F",275)
       ...
       DO is=1,SIZE(rho_r)
         IF(rho_r_valid) THEN
           CALL hfun_scale(rhonorm%pw%cr3d,rho_r(is)%pw%cr3d,&
                           hirshfeld_env%fnorm%pw%cr3d)
         ELSE
           CALL cp__b("hirshfeld_methods.F",346,"We need rho in real space")
         END IF
         CALL hirshfeld_integration(qs_env,hirshfeld_env,rhonorm,charges(:,is))
         charges(:,is) = charges(:,is)*hirshfeld_env%charges(:)
       END DO
