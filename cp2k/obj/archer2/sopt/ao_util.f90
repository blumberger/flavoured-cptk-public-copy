# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ao_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ao_util.F"
!-----------------------------------------------------------------------------!
...
# 29 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ao_util.F" 2
...
!> \retval radius Calculated radius of the Gaussian functio
!> \date    26.02.1999
!> \par History
...
! *****************************************************************************
  FUNCTION exp_radius(l,alpha,threshold,prefactor,epsin) RESULT(radius)
    ...
    IF (l < 0) THEN
       CALL cp__b("aobasis/ao_util.F",188,"The angular momentum quantum number is negative")
    END IF

    IF (alpha == 0.0_dp) THEN
       CALL cp__b("aobasis/ao_util.F",192,"The Gaussian function exponent is zero")
    ELSE
       a = ABS(alpha)
    END IF

    IF (threshold == 0.0_dp) THEN
       CALL cp__b("aobasis/ao_util.F",198,"The requested threshold is zero")
    ELSE
       t = ABS(threshold)
    END IF
    ...
    DO
       ...
       IF (iter.gt.maxiter) THEN
          CALL cp__b("aobasis/ao_util.F",229,"Maximum number of iterations exceeded")
       END IF
    ...
    DO iter=1,maxiter
       ...
       ENDIF
    ENDDO
    CALL cp__b("aobasis/ao_util.F",251,"Maximum number of iterations exceeded")
...
! *****************************************************************************
  SUBROUTINE transform_c2s(CPC_co,CPC_so,maxl,lm1,lm2)
    ...
    IF (istat /= 0) THEN
       CALL stop_memory(routineN,moduleN,496,&
...
! *****************************************************************************
  SUBROUTINE transform_c2s_new(CPC_co,CPC_so,maxl)
    ...
    IF (istat /= 0) THEN
       CALL stop_memory(routineN,moduleN,599,&
                        "work",dp_size*ncoset(maxl)*nsoset(maxl))
    END IF
    ldc = ncoset(maxl)
    lds = nsoset(maxl)
...
! *****************************************************************************
  SUBROUTINE transform_s2c(matso,matco,maxl,lm1,lm2)
    ...
    IF (istat /= 0) THEN
       CALL stop_memory(routineN,moduleN,643,&
...
! *****************************************************************************
  SUBROUTINE sph2cart_mat(mat,ld_mat,sd_mat,n,lmax1,lmax2)
    ...
    IF (lmax1>3.or.lmax2>3) THEN
       CALL cp__b("aobasis/ao_util.F",721,"l>3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE cart2sph_mat(mat,ld_mat,sd_mat,n,lmax1,lmax2)
    ...
    IF ((lmax1 > 3).OR.(lmax2 > 3)) THEN
       CALL cp__b("aobasis/ao_util.F",904,"l > 3 not implemented")
    END IF
