# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_forces.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_forces.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_forces.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE ewald_ddapc_force(qs_env, coeff, apply_qmmm_periodic,&
    ...
    IF(.NOT.(PRESENT(charges)))CALL cp__a("cp_ddapc_forces.F",107)
    IF(.NOT.(ASSOCIATED(radii)))CALL cp__a("cp_ddapc_forces.F",108)
    IF(.NOT.(cell%orthorhombic))CALL cp__a("cp_ddapc_forces.F",109)
    ...
    END DO ! iparticle1
    !NB parallelization
    CALL mp_sum(d_el, para_env%group)
...
! *****************************************************************************
  SUBROUTINE cp_decpl_ddapc_forces(qs_env, M, charges, dq, d_el, particle_set)
    ...
    CALL mp_sum(chf, para_env%group)
    DO iatom=1,natom
       ikind = kind_of(iatom)
...
! order parameter (i.e. the sum of the charges of the selected atoms)
    ...
    CASE DEFAULT
       CALL cp__b("cp_ddapc_forces.F",389,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE solvation_ddapc_force(qs_env, solvation_section, particle_set,&
    ...
    REAL(KIND=dp) :: center(3), dcos1(3), dcos2(3), dpos1(3), dpos2(3), &
      ...
      pos2, pos2i, ptcos, q1t, q2t, r1(3), r1s, r2(3), r2s, Rs, rvec(3)
    ...
    ELSE
       ...
       END IF
    END IF
    IF(.NOT.(n_rep1/=0.OR.n_rep2/=0))CALL cp__a("cp_ddapc_forces.F",551)
