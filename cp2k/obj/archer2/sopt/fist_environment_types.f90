# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment_types.F"
!-----------------------------------------------------------------------------!
...
# 67 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment_types.F" 2
...
! *****************************************************************************
  SUBROUTINE fist_env_get( fist_env, atomic_kind_set, particle_set, ewald_pw,  &
       ...
       core_particle_set, multipoles, results, exclusions, intra_forces)
    ...
    REAL(KIND=dp), ALLOCATABLE, OPTIONAL     :: intra_forces(:,:)
    ...
    IF ( PRESENT (intra_forces)) THEN 
!print *, "BOB"
                intra_forces = fist_env%intra_forces
        END IF
...
! *****************************************************************************
  SUBROUTINE fist_env_set( fist_env, atomic_kind_set, particle_set, ewald_pw, &
                           ...
                           input, shell_model, shell_model_ad, exclusions, &
                           intra_forces)
    ...
    IF ( PRESENT ( fist_nonbond_env ) ) THEN
       CALL fist_nonbond_env_retain ( fist_nonbond_env)
    ...
    END IF
    IF (PRESENT(intra_forces)) THEN
       fist_env%intra_forces = intra_forces
    END IF
...
! *****************************************************************************
  SUBROUTINE fist_env_create(fist_env,para_env, energy_decomp_section)
    TYPE(fist_environment_type), POINTER      :: fist_env
    TYPE(cp_para_env_type), POINTER           :: para_env
    TYPE(section_vals_type), POINTER,OPTIONAL :: energy_decomp_section
    INTEGER                                   :: num_atoms
    ...
    ALLOCATE(fist_env)
    IF (PRESENT(energy_decomp_section)) THEN
        CALL section_vals_val_get(energy_decomp_section, "NUM_ACTIVE_ATOMS", &
                                  i_val=num_atoms)
        ALLOCATE(fist_env%intra_forces(3,num_atoms))
    END IF
...
!> \author Fawzi Mohamed
! *****************************************************************************
  SUBROUTINE fist_env_retain(fist_env)
    TYPE(fist_environment_type), POINTER     :: fist_env

    CHARACTER(len=*), PARAMETER :: routineN = 'fist_env_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(fist_env)))CALL cp__a("fist_environment_types.F",479)
    IF(.NOT.(fist_env%ref_count>0))CALL cp__a("fist_environment_types.F",480)
...
! *****************************************************************************
  SUBROUTINE fist_env_release(fist_env)
    ...
    IF (ASSOCIATED(fist_env)) THEN
       IF(.NOT.(fist_env%ref_count>0))CALL cp__a("fist_environment_types.F",498)
       fist_env%ref_count=fist_env%ref_count-1
       IF (fist_env%ref_count<1) THEN
          ...
          CALL ewald_pw_release(fist_env%ewald_pw)
          CALL ewald_env_release(fist_env%ewald_env)
          CALL cp_para_env_release(fist_env%para_env)
          ...
          IF (ALLOCATED(fist_env%intra_forces)) THEN
              DEALLOCATE(fist_env%intra_forces)
          END IF
          DEALLOCATE(fist_env)
       END IF
    END IF
