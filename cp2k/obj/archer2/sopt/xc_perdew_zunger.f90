# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_zunger.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_zunger.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew_zunger.F" 2
...
! *****************************************************************************
  SUBROUTINE pz_info ( method, lsd, reference, shortform, needs, max_deriv)
    ...
    SELECT CASE (method)
    CASE DEFAULT
       CALL cp__b("xc/xc_perdew_zunger.F",79,"")
    CASE (pz_orig)
       p_string='ORIG'
    CASE (pz_dmc)
       p_string='DMC'
...
! *****************************************************************************
  SUBROUTINE pz_lda_eval ( method, rho_set, deriv_set, order, pz_params)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_perdew_zunger.F",159)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_perdew_zunger.F",160)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_perdew_zunger.F",161)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_perdew_zunger.F",162)
    ...
    END IF
    IF (order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_perdew_zunger.F",197,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE pz_lsd_eval ( method, rho_set, deriv_set, order,pz_params)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_perdew_zunger.F",298)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_perdew_zunger.F",299)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_perdew_zunger.F",300)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_perdew_zunger.F",301)
    ...
    END IF
    IF (order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_perdew_zunger.F",351,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE pz_init ( method, cutoff )
    ...
    CASE DEFAULT
       CALL cp__b("xc/xc_perdew_zunger.F",458,"Unknown method")
