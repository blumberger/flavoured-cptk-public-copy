# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_run.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_run.F"
!-----------------------------------------------------------------------------!
...
MODULE mc_run
  ...
  USE mc_environment_types,            ONLY: get_mc_env,&
                                             ...
                                             set_mc_env
  USE mc_types,                        ONLY: &
       find_mc_rcut, get_mc_molecule_info, get_mc_par, &
       mc_determine_molecule_info, mc_input_file_create, &
       mc_input_file_destroy, mc_input_file_type, mc_input_parameters_check, &
       mc_molecule_info_destroy, mc_molecule_info_type, mc_sim_par_create, &
       mc_sim_par_destroy, mc_simulation_parameters_p_type, read_mc_section, &
       set_mc_par
...
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/mc/mc_run.F" 2
...
! *****************************************************************************
  SUBROUTINE do_mon_car ( force_env_1, globenv, input_declaration, input_file_name)
    ...
    INTEGER                                  :: box_number, ibox, isos, &
                                                istat, iw, nboxes, nmol_types
    INTEGER, DIMENSION(:), POINTER           :: nunits_tot
    LOGICAL                                  :: lbias, lhmc, lrestart, &
                                                lskip_box, lterminate
    REAL(dp)                                 :: rcut
    ...
    TYPE(section_vals_type), POINTER         :: force_env_section, &
      ...
      IF(.NOT.(isos==1))CALL cp__a("motion/mc/mc_run.F",149)
...
! now we create the various pointers that contain information for all boxes
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,179,&
                                       "force_env",int_size*nboxes)
      SELECT CASE(ensemble)
      CASE("TRADITIONAL","VIRIAL")
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,189,&
                                       "mc_par",int_size*nboxes)
      ALLOCATE (mc_env(1:nboxes),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,192,&
                                       "mc_env",int_size*nboxes)
...
! figure out if we have an empty box
      ...
      DO ibox=1,nboxes
         IF(nunits_tot(ibox) == 0) THEN
            IF(lskip_box) THEN
               CALL cp__b("motion/mc/mc_run.F",274,'More than one box has no atoms in it!')
            END IF
            box_number=ibox
            lskip_box=.TRUE.
         ENDIF
...
! potential cutoff and write it to the screen
      IF(force_env(1)%force_env%in_use .EQ. use_fist_force) THEN
         ...
         IF(lterminate) THEN
            CALL cp__b("motion/mc/mc_run.F",297,'Cutoff larger than twice the boxlength')
         ENDIF
      ENDIF
