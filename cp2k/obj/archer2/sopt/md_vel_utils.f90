# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_vel_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_vel_utils.F"
!-----------------------------------------------------------------------------!
...
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_vel_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE initialize_velocities(simpar, part, force_env, globenv, md_env, &
       ...
       shell_part, core_part, force_rescaling, para_env)
    ...
    IF (iw>0) THEN
       ...
       IF (simpar%nfree == 0) THEN
          IF(.NOT.(ekin==0.0_dp))CALL cp__a("motion/md_vel_utils.F",657)
          temp = 0.0_dp
       ELSE
...
! *****************************************************************************
  SUBROUTINE read_input_velocities(simpar, part, force_env, md_env, subsys_section,&
    ...
    LOGICAL :: atomvel_explicit, atomvel_read, corevel_explicit, &
      corevel_read, is_ok, rescale_regions, shellvel_explicit, shellvel_read
...
! Initializing parameters
    ...
    IF (shell_present) THEN
       IF(.NOT.(ASSOCIATED(core_part)))CALL cp__a("motion/md_vel_utils.F",751)
       IF(.NOT.(ASSOCIATED(shell_part)))CALL cp__a("motion/md_vel_utils.F",752)
    ...
    IF(.NOT.(shellvel_explicit.EQV.corevel_explicit))CALL cp__a("motion/md_vel_utils.F",764)
...
! *****************************************************************************
  SUBROUTINE soften_velocities(simpar, part, force_env, md_env, is_fixed, iw)
    ...
    IF(ANY(is_fixed/=use_perd_none))&
       CALL cp__b("motion/md_vel_utils.F",979,"Velocitiy softening with constraints is not supported.")
...
! *****************************************************************************
  SUBROUTINE temperature_control(simpar, md_env, md_ener,force_env,logger)
    ...
    IF(.NOT.(ASSOCIATED(simpar)))CALL cp__a("motion/md_vel_utils.F",1580)
    IF(.NOT.(ASSOCIATED(md_ener)))CALL cp__a("motion/md_vel_utils.F",1581)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/md_vel_utils.F",1582)
...
! *****************************************************************************
  SUBROUTINE comvel_control(md_ener,force_env, md_section, logger)
    ...
    CALL timeset(routineN,handle)
    NULLIFY(subsys)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/md_vel_utils.F",1641)
...
! *****************************************************************************
  SUBROUTINE angvel_control(md_ener, force_env, md_section, logger)
    ...
    IF ( explicit ) THEN
       NULLIFY(subsys, cell)
       IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/md_vel_utils.F",1719)
  ...
  SUBROUTINE setup_velocities(force_env, simpar, globenv, md_env, md_section, &
    ...
    IF (shell_present.AND.shell_adiabatic) THEN
       ...
       IF(nconstraint - nconstraint_fixd /= 0)&
          CALL cp__b("motion/md_vel_utils.F",1855,"Only the fixed atom constraint is implemented for core-shell models")
!MK    CPPostcondition(.NOT.simpar%constraint,cp_failure_level,routineP,failure)
       IF(.NOT.(ASSOCIATED(shell_particles)))CALL cp__a("motion/md_vel_utils.F",1857)
       IF(.NOT.(ASSOCIATED(core_particles)))CALL cp__a("motion/md_vel_utils.F",1858)
    ...
    IF (simpar%ensemble /= reftraj_ensemble) THEN
       IF (apply_cns0) THEN
          ...
          CALL force_env_shake(force_env,shake_tol=simpar%shake_tol,&
               ...
               dump_lm=simpar%dump_lm,compold=.TRUE.)
...
! *****************************************************************************
  SUBROUTINE initialize_cascade(simpar,particle_set,molecule_kinds,md_section)
    ...
    IF (init_cascade) THEN
       ...
       IF(energy < 0.0_dp)&
          CALL cp__b("motion/md_vel_utils.F",1982,"Error occurred reading &CASCADE section: Negative energy found")
       ...
       IF(natom <= 0)&
          CALL cp__b("motion/md_vel_utils.F",1997,"Error occurred reading &CASCADE section: No atom list found")
...
999       IF(.NOT.no_read_error)&
             CALL cp__b("motion/md_vel_utils.F",2017,"Error occurred reading &CASCADE section. Last line read <"//TRIM(line)//">")
          IF((atom_index(iatom) <= 0).OR.((atom_index(iatom) > nparticle)))&
             CALL cp__b("motion/md_vel_utils.F",2019,"Error occurred reading &CASCADE section: Invalid atom index found")
          IF(weight(iatom) < 0.0_dp)&
             CALL cp__b("motion/md_vel_utils.F",2021,"Error occurred reading &CASCADE section: Negative weight found")
          IF (iw > 0) THEN
       ...
       DO iatom=1,natom
          ...
          IF(particle_set(iparticle)%shell_index /= 0)&
             CALL cp__w("motion/md_vel_utils.F",2033,"Warning: The primary knock-on atom is a core-shell atom")
       ...
       IF (iw > 0) THEN
          ...
          IF (simpar%nfree == 0) THEN
             IF(.NOT.(ekin == 0.0_dp))CALL cp__a("motion/md_vel_utils.F",2088)
             temp = 0.0_dp
          ELSE
             temp = 2.0_dp*ekin/REAL(simpar%nfree,KIND=dp)
          END IF
