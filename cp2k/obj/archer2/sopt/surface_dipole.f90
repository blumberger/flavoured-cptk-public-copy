# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/surface_dipole.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/surface_dipole.F"
!-----------------------------------------------------------------------------!
...
# 36 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/surface_dipole.F" 2
       ...
       IF(i /= idir_surfdip) THEN
           IF(ABS(wf_r%pw%pw_grid%dh(idir_surfdip,i)) > 1.E-7_dp) THEN
             ! stop surface dipole defined only for slab perpendigular to one of the Cartesian axis
              CALL cp_abort(cp__l("surface_dipole.F",132),&
...
!  is that the cell angles are 90 degrees.
      ...
      IF(rhoav_min>=1.E-5_dp)&
         CALL cp__b("surface_dipole.F",192," Dipole correction needs more vacuum space above the surface ")
