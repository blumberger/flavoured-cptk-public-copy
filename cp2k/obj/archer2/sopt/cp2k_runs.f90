# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/start/cp2k_runs.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/start/cp2k_runs.F"
!-----------------------------------------------------------------------------!
...
MODULE cp2k_runs
  ...
  USE input_constants,                 ONLY: &
       bsse_run, cell_opt_run, debug_run, do_atom, do_band, do_cp2k, &
       do_farming, do_fist, do_mixed, do_opt_basis, do_optimize_input, &
       do_qmmm, do_qs, do_swarm, do_tamc, do_test, do_tree_mc, &
       do_tree_mc_ana, driver_run, ehrenfest, electronic_spectra_run, &
  ...
  USE input_section_types,             ONLY: &
       ...
       section_type, section_vals_create, section_vals_get_subs_vals, &
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/start/../base/base_uses.f90" 1
...
# 138 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/start/cp2k_runs.F" 2
...
! *****************************************************************************
  RECURSIVE FUNCTION cp2k_run(input_declaration,input_file_name,output_unit,mpi_comm) RESULT(cp2k_run_OK)
    ...
    INTEGER                                  :: f_env_handle, ierr, &
                                                iter_level, method_name_id, &
                                                new_env_id, prog_name_id, &
                                                run_type_id
    INTEGER(KIND=int_8)                      :: m_memory_max_mpi
    LOGICAL                                  :: echo_input, I_was_ionode, &
                                                was_present
    ...
    IF (output_unit > 0 .AND.&
       ...
       CALL cp_dbcsr_print_config (unit_nr=output_unit)
    ...
    CASE (do_farming)
       ...
       CALL cp_dbcsr_finalize_lib(group=mpi_comm, output_unit=0)
    ...
    CASE(do_cp2k)
       ...
       IF(.NOT.(ierr==0))CALL cp__a("start/cp2k_runs.F",283)
    ...
    CASE default
       CALL cp__b("start/cp2k_runs.F",297,"")
    END SELECT
    ...
    CASE (energy_run, energy_force_run)
       IF(  method_name_id /= do_qs .AND.&
            ...
            CALL cp__b("start/cp2k_runs.F",313,"Energy/Force run not available for all methods ")
       ...
       CASE default
          CALL cp__b("start/cp2k_runs.F",331,"")
       END SELECT
    ...
    CASE (electronic_spectra_run)
       IF(method_name_id /= do_qs) &
            CALL cp__b("start/cp2k_runs.F",346,"Electron spectra available only with Quickstep. ")
       CALL force_env_calc_energy_force(force_env,calc_force=.FALSE.)
       CALL tddfpt_calculation(force_env%qs_env)
    CASE (real_time_propagation)
       IF(method_name_id /= do_qs) &
            CALL cp__b("start/cp2k_runs.F",351,"Real time propagation needs METHOD QS. ")
    ...
    CASE (ehrenfest)
       IF(method_name_id /= do_qs) &
            CALL cp__b("start/cp2k_runs.F",357,"Ehrenfest dynamics needs METHOD QS ")
       CALL get_qs_env(force_env%qs_env, dft_control=dft_control)
    ...
    CASE (linear_response_run)
       IF(method_name_id /= do_qs .AND. &
          ...
          CALL cp__b("start/cp2k_runs.F",366,"Property calculations by Linear Response only within the QS or QMMM program ")
    ...
    CASE (debug_run)
       ...
       CASE DEFAULT
          CALL cp__b("start/cp2k_runs.F",375,"Debug run available only with QS, FIST, and QMMM program ")
       END SELECT
    CASE (vib_anal)
    ...
    CASE default
       CALL cp__b("start/cp2k_runs.F",382,"")
    END SELECT
    ...
    CALL pw_cuda_finalize()
    CALL cp_dbcsr_finalize_lib(group=mpi_comm, output_unit=output_unit)
    ...
    IF (prog_name_id==do_cp2k) THEN
       ...
       CALL f_env_rm_defaults(f_env,ierr=ierr,&
            handle=f_env_handle)
       IF(.NOT.(ierr==0))CALL cp__a("start/cp2k_runs.F",406)
       CALL destroy_force_env(new_env_id,ierr=ierr)
       IF(.NOT.(ierr==0))CALL cp__a("start/cp2k_runs.F",408)
       CALL globenv_release(globenv)
    ELSE
       I_was_ionode=para_env%ionode
       CALL cp2k_finalize(root_section,para_env,globenv)
       IF(.NOT.(globenv%ref_count==1))CALL cp__a("start/cp2k_runs.F",413)
  ...
  RECURSIVE SUBROUTINE farming_run(input_declaration,root_section,para_env)
    ...
    IF (farming_env%master_slave) THEN
       ...
       IF (para_env%mepos==0) THEN
           ...
           IF(.NOT.(num_slaves==1))CALL cp__a("start/cp2k_runs.F",503)
           num_slaves=para_env%num_pe-1
           slave_rank=-1
       ENDIF
       IF(.NOT.(num_slaves==para_env%num_pe-1))CALL cp__a("start/cp2k_runs.F",507)
    ELSE
    ...
    IF (slave) THEN
        ...
        ELSE
           CALL cp__b("start/cp2k_runs.F",541,"must set either group_size_wish or ngroup_wish")
        ENDIF
             ...
             ELSE
               ...
               IF (todo==do_deadlock) THEN
                   IF (output_unit>0) THEN
                      ...
                      CALL m_flush(output_unit)
                   ENDIF
                   IF(.NOT.(todo.NE.do_deadlock))CALL cp__a("start/cp2k_runs.F",692)
               ENDIF
             ENDIF
...
! change to the new working directory
       ...
       IF (ierr.NE.0) &
          CALL cp__b("start/cp2k_runs.F",783,"Failed to change dir to: "//TRIM(farming_env%Job(i)%cwd))
       ...
       IF(.NOT.(ierr==0))CALL cp__a("start/cp2k_runs.F",832)
...
! *****************************************************************************
  SUBROUTINE write_xml_file()
    ...
    DO i=1,SIZE(html_entity_table),2
       ...
       IF(.NOT.(is > 0))CALL cp__a("start/cp2k_runs.F",1015)
       ie = INDEX(html_entity_table(i),";") - 1
       IF(.NOT.(ie >= is))CALL cp__a("start/cp2k_runs.F",1017)
       WRITE (UNIT=unit_number,FMT="(A)")&
