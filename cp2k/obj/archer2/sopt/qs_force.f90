# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_force.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_force.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_force.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_forces(qs_env)
    ...
    IF (dft_control%do_admm_dm) THEN
       CALL cp__b("qs_force.F",360,"Forces with ADMM DM methods not implemented")
    END IF
  ...
  SUBROUTINE write_forces(qs_force,atomic_kind_set,ftype,output_unit,&
    ...
    IF (output_unit>0) THEN
       ...
       IF (.NOT.ASSOCIATED(qs_force)) THEN
          CALL cp_abort(cp__l("qs_force.F",541),&
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,549,&
                                        "atom_of_kind",natom*int_size)
       ALLOCATE (kind_of(natom),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,552,&
