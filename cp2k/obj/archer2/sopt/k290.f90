# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/k290.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/k290.F"
!-----------------------------------------------------------------------------!
...
# 16 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/k290.F" 2
...
! *****************************************************************************
  SUBROUTINE release_csym_type(csym)
    ...
    IF(.NOT.(ASSOCIATED(csym)))CALL cp__a("k290.F",123)

    IF ( ASSOCIATED(csym%xkappa) ) THEN
       DEALLOCATE(csym%xkappa)
    END IF
...
!> \param iounit ...
! *****************************************************************************
  SUBROUTINE kp_sym_gen(csym,coor,types,hmat,nk,shift,stress,symm,delta,&
                        wkpoint,xkpoint,iounit)
    TYPE(csym_type), POINTER                 :: csym
    ...
    REAL(KIND=dp), DIMENSION(:), OPTIONAL, &
      POINTER                                :: wkpoint
    REAL(KIND=dp), DIMENSION(:, :), &
      OPTIONAL, POINTER                      :: xkpoint
    INTEGER, INTENT(IN), OPTIONAL            :: iounit

    CHARACTER(LEN=*), PARAMETER :: routineN = 'kp_sym_gen', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: handle, ik, isum, n, nat, &
                                                nhash, nkk, nkpts, nsp
    ...
    IF(.NOT.(.NOT.ASSOCIATED(csym)))CALL cp__a("k290.F",181)
    ALLOCATE(csym)

    !..total number of atoms
    nat = SIZE(coor,2)
    !..allocate arrays for coordinates and atom types
    ALLOCATE (csym%xkappa(3,nat))
    ALLOCATE (csym%ty(nat))
    ...
    END IF
    csym%nkpoint = csym%iq1*csym%iq2*csym%iq3 * 8
    nkk = csym%nkpoint
    ...
    ALLOCATE (wvkl(3,nkk))
    ALLOCATE (lwght(nkk))
    ALLOCATE (lrot(48,nkk))
    ALLOCATE (includ(nkk))
    nhash = MAX(2000,nkk/10)
    ALLOCATE (list(nkk+nhash))
    ALLOCATE (rlist(3,nkk))
    ...
    CALL k290prg(csym%punit,csym%nat,csym%nkpoint,csym%iq1,csym%iq2,csym%iq3,&
         ...
         wvkl,lwght,lrot,nhash,includ,list,rlist,csym%delta)
    IF ( PRESENT(xkpoint) ) THEN
       !..return kpoints and weights
       IF(.NOT.(PRESENT(wkpoint)))CALL cp__a("k290.F",265)
       isum=0
       nkpts=0
       DO ik=1,nkk
          IF(lwght(ik) == 0) EXIT
          nkpts=ik
          isum = isum + lwght(nkpts)
       END DO
       wsum=REAL(isum,KIND=dp)
       ALLOCATE (xkpoint(3,nkpts),wkpoint(nkpts))
       DO ik=1,nkpts
          wkpoint(ik) = REAL(lwght(ik),KIND=dp)/wsum
          xkpoint(1:3,ik) = wvkl(1:3,ik)
       END DO
    END IF
    !..deallocate intermediate arrays
    DEALLOCATE (rx,tvec,isc,f0,wvkl,lwght,lrot,includ,list,rlist)
...
!>       WORLTON-WARREN).
! *****************************************************************************
  SUBROUTINE k290prg(iout,nat,nkpoint,iq1,iq2,iq3,istriz,a1,a2,a3, &
       alat,strain,xkapa,rx,tvec,ty,isc,f0,ntvec,wvk0,ihg,ihc,isy,li,nc,indpg,ib,v,r,&
       xb,origin,wvkl,lwght,lrot,nhash,includ,list,rlist,delta)
    INTEGER                                  :: iout, nat, nkpoint, iq1, iq2, &
                                                iq3, istriz
    REAL(KIND=dp)                            :: a1(3), a2(3), a3(3), alat, &
    ...
    !----------------------------------------------------------------------------!
    CALL group1(a1,a2,a3,nat,ty,xkapa,b1,b2,b3,ihg,ihc,isy,li,nc, &
         indpg,ib,ntvec,v,f0,xb,r,tvec,origin,rx,isc,delta)
    !----------------------------------------------------------------------------!
    DO n = nc + 1, 48
       ib(n) = 0
    END DO
    ...
    IF (ABS(iq1)+ABS(iq2)+ABS(iq3)/=0) THEN
       ...
       !----------------------------------------------------------------------------!
       IF(iout>0) WRITE (iout,'(/,1X,20("-"),A,20("-"),/,A)') &
                      ' The (unstrained) bravais lattice ', &
                      ' (used for generating the largest possible mesh in the B.Z.)'
       !----------------------------------------------------------------------------!
       CALL group1(a01,a02,a03,1,ty,x0,b01,b02,b03,ihg0,ihc0,isy0,li0, &
            nc0,indpg0,ib0,ntvec0,v0,f00,xb0,r0,tvec0,origin0,rx,isc,delta)
       !----------------------------------------------------------------------------!
       ...
       !----------------------------------------------------------------------------!
       IF(iout>0) WRITE (iout,'(/,1X,I5," Special points generated")') ntot
       IF (ntot/=0) THEN
          IF (ntot<0) THEN
             IF(iout>0) WRITE (iout,'(A,I5,/,A,/,A)') ' Dimension nkpoint =', nkpoint, &
          ...
          END DO
          IF(iout>0) WRITE (iout,'(8X,A,T33,A,4X,A)') 'WAVEVECTOR K', 'WEIGHT', &
               'UNFOLDING ROTATIONS'
          ...
          DO l = 1, ntot
             ...
             END IF
          END DO
          IF(iout>0) WRITE (iout,'(24X,"TOTAL:",I8)') iswght
       END IF
    END IF
...
!>       CONTAINS INVERSION.
! *****************************************************************************
  SUBROUTINE group1(a1,a2,a3,nat,ty,x,b1,b2,b3,ihg,ihc,isy,li,nc, &
       indpg,ib,ntvec,v,f0,xb,r,tvec,origin,rx,isc,delta)
    ...
    CALL atftm1(r,v,x,f0,xb,origin,ib,ty,nat,ihg,rx,nc,indpg,ntvec,a,ai, &
         li,isy,isc,delta)
    !------------------------------------------------------------------------------!
...
!>                            group of the crystal
! *****************************************************************************
  SUBROUTINE atftm1(r,v,x,f0,xb,origin,ib,ty,nat,ihg,rx,nc,indpg,ntvec, &
       a,ai,li,isy,isc,delta)
    ...
    DO n = 1, nc
       ...
       DO k2 = 1, nat
          IF (f0(49,k2)<k2.OR.ty(1)/=ty(k2)) CYCLE
          DO i = 1, 3
             xb(i) = rx(i,1) - x(i,k2)
          END DO
...
! *****************************************************************************
  SUBROUTINE checkrlv3(n,nat,ty,rx,x,vr,f0,ai,isc,nodupli,oksym,delta)
...
!------------------------------------------------------------------------------!
    ...
    DO i1 = 1, iq1
       DO i2 = 1, iq2
          DO i3 = 1, iq3
             ...
             CALL bzrduc(wvk,a1,a2,a3,b1,b2,b3,rsdir,nrsdir,nplane,delta)
             IF (istriz==1) THEN
    ...
    IF (iout>0) THEN
       ...
       DO i = 1, imesh
          CALL mesh(iout,wva,i,igarb0,igarbg,nkpoint,nhash,list,rlist,delta)
          IF (MOD(i,2)==1) THEN
    ...
    IF (istriz==1) THEN
       ...
       DO i = 1, (imesh-1)
          ...
          jloop: DO j = (i+1), imesh
             ...
             DO k = 1, 3
                ...
                IF (ABS(REAL(NINT(diff),KIND=dp)-diff)>delta) CYCLE jloop
             END DO
       ...
       IF (iremov>0 .AND. iout>0) WRITE (iout,'(A,A,/,A,I6,A,/)') &
            ...
            'translation vectors', ' K290| ', iremov, &
    ...
    DO iwvk = 1, imesh
       ...
       CALL garbag(wvk,igarbage,igarb0,nkpoint,nhash,list,rlist,delta)
       IF (igarbage>0) CYCLE
       ntot = ntot + 1
...
363       CONTINUE
          ...
          IF (iplace==0) THEN
             ...
             ELSE
               ...
               CALL garbag(wva,igarbage,igarb0,nkpoint,nhash,list,rlist,delta)
               IF (igarbage==0) THEN
...
450 CONTINUE
    ...
    CALL cp__b("k290.F",2572,"")
    !------------------------------------------------------------------------------!
470 CONTINUE
    WRITE (*,'(A,/)') ' SUBROUTINE SPPT2 *** FATAL ERROR ***'
    WRITE (*,*) 'MESH SIZE EXCEEDS NKPOINT=', nkpoint
    CALL cp__b("k290.F",2577,"")
    !------------------------------------------------------------------------------!
490 CONTINUE
    WRITE (*,'(A,/)') ' SUBROUTINE SPPT2 *** FATAL ERROR ***'
    WRITE (*,'(A,3F10.4,/,A,3F10.4,A,/,A,I3,A)') ' THE VECTOR     ', &
...
! *****************************************************************************
  SUBROUTINE remove(wvk,iplace,igarb0,igarbg,nmesh,nhash,list,rlist,delta)
    ...
    DO i = 1, 100
       ...
       lskip = .FALSE.
       DO j = 1, 3
          IF (ABS(wvk(j)-rlist(j,ipoint))>delta1) lskip=.TRUE.
       END DO
       IF (lskip) THEN
         !       Next element of list
         ihash = ipoint
...
! *****************************************************************************
  SUBROUTINE garbag(wvk,iplace,igarb0,nmesh,nhash,list,rlist,delta)
    REAL(KIND=dp)                            :: wvk(3)
    ...
    nloop: DO i = 1, nmesh
       ...
       !       Compare WVK with this element
       DO j = 1, 3
          IF (ABS(wvk(j)-rlist(j,ipoint))>delta1) THEN
             !       Next element of list
             ihash = ipoint
             ipoint = list(ihash)
             CYCLE nloop
    ...
    DO n1 = 1, nnn
       ...
       DO n2 = 1, nnn
          ...
          DO n3 = 1, nnn
             ...
             END DO
             IF (inbz(wva,rsdir,nrsdir,nplane,delta)==yes) THEN
             !------------------------------------------------------------------------------!
             !     The reduced vector
                DO i = 1, 3
                   wvk(i) = wva(i)
...
!------------------------------------------------------------------------------!
    ...
    DO n1 = 1, nnb1
       ...
       DO n2 = 1, nnb2
          ...
          n3loop: DO n3 = 1, nnb3
             i3 = nb3 + 1 - n3
             IF (i1==0 .AND. i2==0 .AND. i3==0) CYCLE n3loop
             DO i = 1, 3
             ...
             DO n = 1, nplane
                ...
                !             and of the -(plane)
                IF (ABS(projct)>0.5E0_dp-delta) CYCLE n3loop
             END DO
