# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_initialization.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_initialization.F"
!-----------------------------------------------------------------------------!
...
# 65 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/sh_initialization.F" 2
...
! *****************************************************************************
  SUBROUTINE create_sh_env(nadiab, norbitals, natom, &
    ...
    !conditional section added for coulomb barrier keyword, if
    !keyword=T, the site number is called from the input file, and the
    !coulomb barrer values are read from the FRZ_DIAGONALS.include file
    !-- FI 05/01/24
    ...
    !Added the coditional coulomb barrier section in the ct_block
    ...
    CASE("AOM")
         ...
         if (sh_env%multistate) then
            CALL cp__b("sh_initialization.F",299,"Multi-state forces not implemented")
         end if   
    ...
    CASE("TRESP_COUPLINGS")
         if (sh_env%multistate) then
            CALL cp__b("sh_initialization.F",349,"Multi-state forces not implemented")
         end if   
...
!TEST
    ...
    CASE DEFAULT
        CALL cp__b("sh_initialization.F",388,"There is no method to calculate the coupling")
    END SELECT
...
!     CHARACTER(LEN=default_string_length)           :: file_name_s_psigma_S_S,file_name_s_ppi_S_S
     ...
     IF (atoms_per_mol_a .ne. atoms_per_acceptor) THEN 
        CALL cp__b("sh_initialization.F",544,"Error in number of atoms acceptor")
     ENDIF 
     IF (atoms_per_mol_d .ne. atoms_per_donor) THEN 
        CALL cp__b("sh_initialization.F",547,"Error in number of atoms donor")
     ENDIF 

      IF (norbitals .ne. 1) THEN
        CALL cp__b("sh_initialization.F",551,"DEGENERACY IS NOT IMPLEMENTED FOR MULTI-PHASE")
      ENDIF
...
!> WTP_4.2: and 5 6 for CR!!
     ...
     ! sanity checks
     IF (atoms_per_mol_a .ne. atoms_per_acceptor) THEN
        CALL cp__b("sh_initialization.F",884,"Error in number of atoms acceptor")
     ENDIF
     IF (atoms_per_mol_d .ne. atoms_per_donor) THEN
        CALL cp__b("sh_initialization.F",887,"Error in number of atoms donor")
     ENDIF

      IF (norbitals .ne. 1) THEN
        CALL cp__b("sh_initialization.F",891,"DEGENERACY IS NOT IMPLEMENTED FOR MULTI-PHASE")
      ENDIF
