# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_pool_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_pool_types.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_pool_types.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_pool_retain(pool)
    ...
    IF(.NOT.(ASSOCIATED(pool)))CALL cp__a("pw/pw_pool_types.F",164)
    IF(.NOT.(pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",165)
...
! *****************************************************************************
  SUBROUTINE pw_pool_flush_cache(pool)
    ...
    IF(.NOT.(ASSOCIATED(pool)))CALL cp__a("pw/pw_pool_types.F",196)
    IF(.NOT.(pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",197)
    ...
    DO
       ...
       IF(.NOT.(pw_el%ref_count==0))CALL cp__a("pw/pw_pool_types.F",209)
       pw_el%ref_count=1
    ...
    DO
       IF (.NOT.cp_sll_pw_next(iterator,el_att=pw_el)) EXIT
       IF(.NOT.(pw_el%ref_count==0))CALL cp__a("pw/pw_pool_types.F",218)
       pw_el%ref_count=1
       CALL pw_release(pw_el)
    END DO
    ...
    DO
       IF (.NOT.cp_sll_pw_next(iterator,el_att=pw_el)) EXIT
       IF(.NOT.(pw_el%ref_count==0))CALL cp__a("pw/pw_pool_types.F",227)
       pw_el%ref_count=1
       CALL pw_release(pw_el)
    END DO
    ...
    DO
       IF (.NOT.cp_sll_pw_next(iterator,el_att=pw_el)) EXIT
       IF(.NOT.(pw_el%ref_count==0))CALL cp__a("pw/pw_pool_types.F",236)
       pw_el%ref_count=1
       CALL pw_release(pw_el)
    END DO
...
! *****************************************************************************
  SUBROUTINE pw_pool_release(pool)
    ...
    IF (ASSOCIATED(pool)) THEN
       IF(.NOT.(pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",270)
       pool%ref_count=pool%ref_count-1
       IF (debug_this_module) THEN
          WRITE (unit=cp_logger_get_default_unit_nr(logger,local=.TRUE.),&
       ...
       IF (pool%ref_count==0) THEN
          ...
          pool%ref_count=0
          IF(.NOT.(ASSOCIATED(pool%pw_grid)))CALL cp__a("pw/pw_pool_types.F",282)
...
! *****************************************************************************
  SUBROUTINE pw_pool_create_pw(pool, pw, use_data, in_space)
    ...
    NULLIFY(cr3d_ptr)
    logger => cp_get_default_logger()
    IF(.NOT.(ASSOCIATED(pool)))CALL cp__a("pw/pw_pool_types.F",342)
    IF(.NOT.(pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",343)
    ...
    CASE default
! unknown use_data
       CALL cp__b("pw/pw_pool_types.F",362,"")
    END SELECT
    ...
    IF (.NOT.ASSOCIATED(pw)) THEN
       ...
       END IF
    ELSE
       IF(.NOT.(pw%ref_count==0))CALL cp__a("pw/pw_pool_types.F",384)
       pw%ref_count=1
       IF (debug_this_module) THEN
          WRITE(unit=cp_logger_get_default_unit_nr(logger,local=.TRUE.),&
...
! *****************************************************************************
  SUBROUTINE pw_pool_give_back_pw(pool, pw, accept_non_compatible)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(pool)))CALL cp__a("pw/pw_pool_types.F",435)
    IF(.NOT.(pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",436)
    IF (.NOT.ASSOCIATED(pw)) THEN
       IF(.NOT.(my_accept_non_compatible))CALL cp__a("pw/pw_pool_types.F",438)
       failure = .TRUE.
    END IF
    IF (.NOT. failure) THEN
       IF(.NOT.(pw%ref_count==1))CALL cp__a("pw/pw_pool_types.F",442)
       IF (.NOT.pw_grid_compare(pw%pw_grid,pool%pw_grid)) THEN
          ...
          IF(.NOT.my_accept_non_compatible)&
             CALL cp__b("pw/pw_pool_types.F",454,"pool cannot reuse pw of another grid")
          CALL pw_release(pw)
          failure = .TRUE.
       END IF
    END IF
    IF (.NOT. failure) THEN
       ...
       CASE(REALDATA1D)
          ...
          ELSE
             IF(max_max_cache>=0)&
                CALL cp__w("pw/pw_pool_types.F",475,"hit max_cache")
             CALL pw_release(pw)
          END IF
       CASE(REALDATA3D)
          IF (ASSOCIATED(pw%cr3d)) THEN
             ...
             ELSE
                IF(max_max_cache>=0)&
                   CALL cp__w("pw/pw_pool_types.F",484,"hit max_cache")
                CALL pw_release(pw)
             END IF
          ELSE
             IF (debug_this_module) THEN
                ...
                CALL print_stack(cp_logger_get_default_unit_nr(logger,local=.TRUE.))
             END IF
             IF(.NOT.(my_accept_non_compatible))CALL cp__a("pw/pw_pool_types.F",494)
             CALL pw_release(pw)
          END IF
       CASE(COMPLEXDATA1D)
          ...
          ELSE
             IF(max_max_cache>=0)&
                CALL cp__w("pw/pw_pool_types.F",502,"hit max_cache")
             CALL pw_release(pw)
          END IF
       CASE(COMPLEXDATA3D)
          ...
          ELSE
             IF(max_max_cache>=0)&
                CALL cp__w("pw/pw_pool_types.F",510,"hit max_cache")
             CALL pw_release(pw)
          END IF
       CASE default
          ! unknown in_use
          CALL cp__b("pw/pw_pool_types.F",515,"")
       END SELECT
...
SUBROUTINE pw_pool_create_cr3d(pw_pool,cr3d)
  ...
  IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("pw/pw_pool_types.F",549)
  IF(.NOT.(pw_pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",550)
  IF(.NOT.(.NOT.ASSOCIATED(cr3d)))CALL cp__a("pw/pw_pool_types.F",551)
  ...
  ELSE
     ...
     IF (ASSOCIATED(pw)) THEN
        IF(.NOT.(pw%ref_count==0))CALL cp__a("pw/pw_pool_types.F",558)
        pw%ref_count=1
        cr3d => pw%cr3d
        NULLIFY(pw%cr3d)
        CALL pw_release(pw)
  ...
  IF (.NOT.ASSOCIATED(cr3d)) THEN
     ALLOCATE(cr3d(pw_pool%pw_grid%bounds_local(1,1):pw_pool%pw_grid%bounds_local(2,1),&
          ...
          stat=stat)
     IF(.NOT.(stat==0))CALL cp__a("pw/pw_pool_types.F",570)
     IF (debug_this_module) THEN
        WRITE (unit=cp_logger_get_default_unit_nr(logger,local=.TRUE.),&
...
SUBROUTINE pw_pool_give_back_cr3d(pw_pool,cr3d,accept_non_compatible)
  ...
  IF(.NOT.(ASSOCIATED(pw_pool)))CALL cp__a("pw/pw_pool_types.F",611)
  IF(.NOT.(pw_pool%ref_count>0))CALL cp__a("pw/pw_pool_types.F",612)
  IF (ASSOCIATED(cr3d)) THEN
     ...
     IF(.NOT.(compatible.OR.my_accept_non_compatible))CALL cp__a("pw/pw_pool_types.F",623)
     IF (compatible) THEN
        ...
        ELSE
           IF(max_max_cache>=0)&
              CALL cp__w("pw/pw_pool_types.F",629,"hit max_cache")
           DEALLOCATE(cr3d)
        END IF
     ELSE
        IF (debug_this_module) THEN
  ...
  ELSE
     IF (debug_this_module) THEN
        ...
        CALL print_stack(cp_logger_get_default_unit_nr(logger,local=.TRUE.))
     END IF
     IF(.NOT.(my_accept_non_compatible))CALL cp__a("pw/pw_pool_types.F",648)
  END IF
  NULLIFY(cr3d)
...
SUBROUTINE pw_pools_create_pws(pools, pws, use_data, in_space)
  ...
  IF(.NOT.(ASSOCIATED(pools)))CALL cp__a("pw/pw_pool_types.F",676)
...
SUBROUTINE pw_pools_give_back_pws(pools, pws)
  ...
  IF(.NOT.(ASSOCIATED(pools)))CALL cp__a("pw/pw_pool_types.F",703)
  IF(.NOT.(ASSOCIATED(pws)))CALL cp__a("pw/pw_pool_types.F",704)
  IF(.NOT.(SIZE(pws)==SIZE(pools)))CALL cp__a("pw/pw_pool_types.F",705)
  DO i=1,SIZE(pools)
...
SUBROUTINE pw_pools_copy(source_pools, target_pools)
  ...
  IF(.NOT.(ASSOCIATED(source_pools)))CALL cp__a("pw/pw_pool_types.F",729)
