# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kind_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kind_types.F"
!-----------------------------------------------------------------------------!
...
# 91 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kind_types.F" 2
...
! *****************************************************************************
  TYPE qs_kind_type
     ...
     INTEGER                                :: pao_potential_maxl = -1
     REAL(KIND=dp)                          :: pao_potential_beta = 0.0_dp
     INTEGER                                :: pao_potential_neighbors = -1
...
! *****************************************************************************
  SUBROUTINE deallocate_qs_kind_set(qs_kind_set)
    ...
    ELSE
       CALL cp_abort(cp__l("qs_kind_types.F",279),&
...
!> \param pao_potential_maxl ...
!> \param pao_potential_beta ...
!> \param pao_potential_neighbors ...
! *****************************************************************************
  SUBROUTINE get_qs_kind(qs_kind, &
       ...
       pao_basis_size, pao_potential_maxl, pao_potential_beta, pao_potential_neighbors)
    ...
    INTEGER, INTENT(OUT), OPTIONAL           :: pao_basis_size, &
                                                pao_potential_maxl
    REAL(KIND=dp), INTENT(OUT), OPTIONAL     :: pao_potential_beta
    INTEGER, INTENT(OUT), OPTIONAL           :: pao_potential_neighbors
...
! Retrieve basis set from the kind container
    ...
    IF (PRESENT(pao_potential_maxl)) pao_potential_maxl = qs_kind%pao_potential_maxl
    IF (PRESENT(pao_potential_neighbors)) pao_potential_neighbors = qs_kind%pao_potential_neighbors
    IF (PRESENT(pao_potential_beta))  pao_potential_beta = qs_kind%pao_potential_beta
...
! *****************************************************************************
  SUBROUTINE get_qs_kind_set(qs_kind_set,&
    ...
    IF (ASSOCIATED(qs_kind_set)) THEN
       ...
       END DO
    ELSE
       CALL cp__b("qs_kind_types.F",999,"The pointer qs_kind_set is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE init_qs_kind(qs_kind)
    ...
    IF(.NOT.(ASSOCIATED(qs_kind)))CALL cp__a("qs_kind_types.F",1022)
...
! *****************************************************************************
  SUBROUTINE init_qs_kind_set(qs_kind_set)
    ...
    IF (.NOT. ASSOCIATED(qs_kind_set)) THEN
       CALL cp__b("qs_kind_types.F",1066,"init_qs_kind_set: The pointer qs_kind_set is not associated")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE init_gapw_basis_set(qs_kind_set,qs_control,force_env_section)
    ...
    IF (ASSOCIATED(qs_kind_set)) THEN
       ...
       END IF
    ELSE
       CALL cp__b("qs_kind_types.F",1147,"The pointer qs_kind_set is not associated")
    END IF
...
! *****************************************************************************
  SUBROUTINE read_qs_kind(qs_kind,kind_section,para_env,force_env_section,&
    ...
    IF (k_rep<0.AND.(.NOT.no_fail)) THEN
       CALL cp_abort(cp__l("qs_kind_types.F",1279),&
    ...
    IF (k_rep>0) THEN
       ...
       IF(.NOT.(nb_rep<=maxbas))CALL cp__a("qs_kind_types.F",1296)
       DO i=1,nb_rep
          CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
          ...
          ELSE
             CALL cp__b("qs_kind_types.F",1308,"")
          END IF
       END DO
       ...
       IF(aux_fit_basis_set_name /= "") THEN
          nb_rep = nb_rep + 1
          IF(.NOT.(nb_rep<=maxbas))CALL cp__a("qs_kind_types.F",1323)
       ...
       IF(aux_basis_set_name /= "") THEN
          nb_rep = nb_rep + 1
          IF(.NOT.(nb_rep<=maxbas))CALL cp__a("qs_kind_types.F",1331)
       ...
       IF(lri_basis_set_name /= "") THEN
          nb_rep = nb_rep + 1
          IF(.NOT.(nb_rep<=maxbas))CALL cp__a("qs_kind_types.F",1339)
       ...
       IF(ri_aux_basis_set_name /= "") THEN
          nb_rep = nb_rep + 1
          IF(.NOT.(nb_rep<=maxbas))CALL cp__a("qs_kind_types.F",1347)
       ...
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
            keyword_name="PAO_BASIS_SIZE", i_val=qs_kind%pao_basis_size)
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
            keyword_name="PAO_POTENTIAL_MAXL", i_val=qs_kind%pao_potential_maxl)
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
            keyword_name="PAO_POTENTIAL_NEIGHBORS", i_val=qs_kind%pao_potential_neighbors)
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
            keyword_name="PAO_POTENTIAL_BETA", r_val=qs_kind%pao_potential_beta)
       ...
       IF(qs_kind%hard_radius<qs_kind%hard0_radius)&
          CALL cp__b("qs_kind_types.F",1399,"rc0 should be <= rc")
       ...
       IF(qs_kind%ngrid_ang <= 0)&
          CALL cp__b("qs_kind_types.F",1406,"# point lebedev grid < 0")
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
            keyword_name="RADIAL_GRID",i_val=qs_kind%ngrid_rad)
       IF(qs_kind%ngrid_rad <= 0)&
          CALL cp__b("qs_kind_types.F",1410,"# point radial grid < 0")
       CALL section_vals_val_get(kind_section,i_rep_section=k_rep,&
       ...
       IF (section_enabled) THEN
          ...
          IF(explicit) THEN
              ...
              IF(.NOT.(ASSOCIATED(add_el)))CALL cp__a("qs_kind_types.F",1445)
              ...
              IF(.NOT.(ASSOCIATED(add_el)))CALL cp__a("qs_kind_types.F",1452)
              IF(.NOT.(SIZE(add_el)==SIZE( qs_kind%addel,1)))CALL cp__a("qs_kind_types.F",1453)
          ...
          IF(explicit) THEN
            ...
            CALL section_vals_val_get(spin_section,&
                 keyword_name="NEL",i_vals=add_el)
            IF(.NOT.(SIZE(add_el)==SIZE( qs_kind%addel,1)))CALL cp__a("qs_kind_types.F",1479)
            qs_kind%addel(1:SIZE(add_el),2) = add_el(1:SIZE(add_el))
            qs_kind%addel(:,:) = qs_kind%addel(:,:)
            NULLIFY(add_el)
            CALL section_vals_val_get(spin_section,&
                 keyword_name="L",i_vals=add_el)
            IF(.NOT.(SIZE(add_el)==SIZE( qs_kind%addel,1)))CALL cp__a("qs_kind_types.F",1485)
       ...
       IF (section_enabled) THEN
          ...
          IF(l < 0)&
             CALL cp__b("qs_kind_types.F",1519,"DFT+U| Invalid orbital angular momentum quantum number specified: l < 0")
          CALL section_vals_val_get(dft_plus_u_section,&
          ...
          IF (subsection_enabled) THEN
             ...
             IF(norbitals<=0.OR.norbitals>2*l+1)&
                CALL cp_abort(cp__l("qs_kind_types.F",1552),"DFT+U| Invalid number of ORBITALS specified: "//&
             ...
             DO m=1,norbitals
                IF(qs_kind%dft_plus_u%orbitals(m) > l)&
                   CALL cp__b("qs_kind_types.F",1559,"DFT+U| Invalid orbital magnetic quantum number specified: m > l")
                IF(qs_kind%dft_plus_u%orbitals(m) < -l)&
                   CALL cp__b("qs_kind_types.F",1561,"DFT+U| Invalid orbital magnetic quantum number specified: m < -l")
                DO j=1,norbitals
                   IF (j /= m) THEN
                      IF(qs_kind%dft_plus_u%orbitals(j) == qs_kind%dft_plus_u%orbitals(m))&
                         CALL cp__b("qs_kind_types.F",1565,"DFT+U| An orbital magnetic quantum number was specified twice")
                   END IF
                END DO
             END DO
             CALL section_vals_val_get(enforce_occupation_section,&
    ...
    CASE(do_method_dftb, do_method_scptb, do_method_rm1, do_method_am1, do_method_mndo, &
       ...
       CASE (do_method_rm1, do_method_am1, do_method_mndo, do_method_pdg,&
          ...
          IF(.NOT.(check))CALL cp__a("qs_kind_types.F",1639)
       ...
       CASE (do_method_dftb,do_method_scptb)
          ...
          ! later
       CASE DEFAULT
          CALL cp__b("qs_kind_types.F",1664,"")
       END SELECT

       check = ((potential_name /= '').OR.explicit_potential)
       IF(check)&
          CALL cp_warn(cp__l("qs_kind_types.F",1669),&
       ...
       IF(check)&
          CALL cp_warn(cp__l("qs_kind_types.F",1675),&
    ...
    CASE DEFAULT
       ...
       END DO
       IF (nobasis) THEN
          CALL cp_abort(cp__l("qs_kind_types.F",1734),&
       ...
       ELSE
          ...
          IF ((potential_name /= '').OR.explicit_potential) THEN
             ...
             CASE ("GTH")
                ...
                END IF
             CASE DEFAULT
                CALL cp_abort(cp__l("qs_kind_types.F",1777),&
             ...
             END SELECT
          ELSE
             CALL cp_abort(cp__l("qs_kind_types.F",1784),&
          ...
          IF ((kgpot_name /= '').OR.explicit_kgpot) THEN
             ...
             CASE DEFAULT
                CALL cp_abort(cp__l("qs_kind_types.F",1807),&
...
! *****************************************************************************
  SUBROUTINE create_qs_kind_set(qs_kind_set, atomic_kind_set,kind_section,para_env,force_env_section)
    ...
    IF(ASSOCIATED(qs_kind_set)) CALL cp__b("qs_kind_types.F",1848,"create_qs_kind_set: qs_kind_set already associated")
    IF(.NOT.ASSOCIATED(atomic_kind_set)) CALL cp__b("qs_kind_types.F",1849,"create_qs_kind_set: atomic_kind_set not associated")
...
! *****************************************************************************
  SUBROUTINE check_qs_kind(qs_kind,dft_control,subsys_section)
    ...
    IF (dft_control%qs_control%semi_empirical) THEN
       ...
       IF(.NOT.(ASSOCIATED(se_parameter)))CALL cp__a("qs_kind_types.F",1905)
       CALL get_se_param(se_parameter,defined=defined)
       IF(.NOT.(defined))CALL cp__a("qs_kind_types.F",1907)
    ...
    ELSE IF (dft_control%qs_control%dftb) THEN
       ...
       IF(.NOT.(ASSOCIATED(dftb_parameter)))CALL cp__a("qs_kind_types.F",1911)
       CALL get_dftb_atom_param(dftb_parameter,defined=defined)
       IF(.NOT.(defined))CALL cp__a("qs_kind_types.F",1913)
    ...
    ELSE IF (dft_control%qs_control%scptb) THEN
       ...
       IF(.NOT.(ASSOCIATED(scptb_parameter)))CALL cp__a("qs_kind_types.F",1917)
       CALL get_scptb_parameter(scptb_parameter,defined=defined)
       IF(.NOT.(defined))CALL cp__a("qs_kind_types.F",1919)
...
! *****************************************************************************
  SUBROUTINE check_qs_kind_set(qs_kind_set,dft_control,subsys_section)
    ...
    IF (ASSOCIATED(qs_kind_set)) THEN
       ...
       END DO
    ELSE
       CALL cp__b("qs_kind_types.F",1952,"The pointer qs_kind_set is not associated")
    END IF
    CALL timestop(handle)
  ...
  SUBROUTINE write_qs_kind(qs_kind,kind_number,output_unit)
    ...
    IF (output_unit > 0) THEN
       ...
       IF (ASSOCIATED(qs_kind)) THEN
          ...
          END IF
       ELSE
          CALL cp__b("qs_kind_types.F",2148,"")
       END IF
  ...
  SUBROUTINE write_qs_kind_set(qs_kind_set,subsys_section)
    ...
    IF (output_unit > 0) THEN
       IF (ASSOCIATED(qs_kind_set)) THEN
          ...
          END DO
       ELSE
          CALL cp__b("qs_kind_types.F",2190,"")
       END IF
    END IF
  ...
  SUBROUTINE write_gto_basis_sets(qs_kind_set,subsys_section)
    ...
    IF (output_unit > 0) THEN
       IF (ASSOCIATED(qs_kind_set)) THEN
          ...
          END DO
       ELSE
          CALL cp__b("qs_kind_types.F",2272,"")
       END IF
    END IF
