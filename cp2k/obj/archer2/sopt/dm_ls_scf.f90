# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dm_ls_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dm_ls_scf.F"
!-----------------------------------------------------------------------------!
...
MODULE dm_ls_scf
  ...
  USE input_constants,                 ONLY: &
       ...
       ls_scf_pexsi, ls_scf_tc2, ls_scf_trs4, transport_transmission
...
# 108 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dm_ls_scf.F" 2
  ...
  SUBROUTINE ls_scf_create(qs_env)
    ...
    IF (ls_scf_env%do_pexsi) THEN
      IF(dft_control%qs_control%eps_filter_matrix .NE. 0.0_dp)&
         CALL cp__b("dm_ls_scf.F",235,"EPS_FILTER_MATRIX must be set to 0 for PEXSI.")
...
! *****************************************************************************
  SUBROUTINE ls_scf_init_read_write_input(input,ls_scf_env,unit_nr)
    ...
    IF(ls_scf_env%ls_diis.AND.ls_scf_env%do_rho_mixing)&
       CALL cp__b("dm_ls_scf.F",640,"LS_DIIS and RHO_MIXING are not compatible.")
    ...
    IF(ls_scf_env%curvy_steps .AND. ls_scf_env%do_pexsi)&
       CALL cp__b("dm_ls_scf.F",690,"CURVY_STEPS cannot be used together with PEXSI.")
    IF(ls_scf_env%curvy_steps .AND. ls_scf_env%do_transport)&
       CALL cp__b("dm_ls_scf.F",692,"CURVY_STEPS cannot be used together with TRANSPORT.")
    IF(ls_scf_env%curvy_steps.AND.ls_scf_env%has_s_preconditioner)&
       CALL cp__b("dm_ls_scf.F",694,"S Preconditioning not implemented in combination with CURVY_STEPS.")
    IF(ls_scf_env%curvy_steps.AND..NOT.ls_scf_env%use_s_sqrt)&
       CALL cp__b("dm_ls_scf.F",696,"CURVY_STEPS requires the use of the sqrt inversion.")
    ...
    IF (unit_nr>0) THEN
       ...
       CASE DEFAULT
           CALL cp__b("dm_ls_scf.F",785,"Unkown cluster type")
       END SELECT
...
! *****************************************************************************
  SUBROUTINE ls_scf_main(qs_env,ls_scf_env)
    ...
    INTEGER                                  :: handle, iscf, ispin, &
                                                ...
                                                nspin, unit_nr
    LOGICAL :: check_convergence, diis_step, do_transport, extra_scf, &
      maxscf_reached, scf_converged, should_stop, transm_maxscf_reached, &
      transm_scf_converged
         ...
         DO ispin=1,nspin
            ...
            IF (do_transport) THEN
               IF(ls_scf_env%has_s_preconditioner)&
                  CALL cp__b("dm_ls_scf.F",1004,"NOT YET IMPLEMENTED with S preconditioner. ")
               IF(ls_scf_env%ls_mstruct%cluster_type .NE. ls_cluster_atomic)&
                  CALL cp__b("dm_ls_scf.F",1006,"NOT YET IMPLEMENTED with molecular clustering. ")
            ...
            ELSE 
               ...
               CASE(ls_scf_pexsi)
                 IF(ls_scf_env%has_s_preconditioner)&
                    CALL cp__b("dm_ls_scf.F",1034,"S preconditioning not implemented in combination with the PEXSI library. ")
                 IF(ls_scf_env%ls_mstruct%cluster_type .NE. ls_cluster_atomic)&
                    CALL cp__b("dm_ls_scf.F",1036,"Molecular clustering not implemented in combination with the PEXSI library. ")
                 IF(ls_scf_env%ls_mstruct%single_precision)&
                    CALL cp__b("dm_ls_scf.F",1038,"PEXSI library requires double precision datatype. ")
...
! *****************************************************************************
  SUBROUTINE post_scf_homo_lumo(ls_scf_env)
    ...
    IF(.NOT.(.NOT.ls_scf_env%has_s_preconditioner))CALL cp__a("dm_ls_scf.F",1255)
    IF(.NOT.(ls_scf_env%use_s_sqrt))CALL cp__a("dm_ls_scf.F",1256)
