# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/dimer_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/dimer_methods.F"
!-----------------------------------------------------------------------------!
...
# 35 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/dimer_methods.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE  cp_eval_at_ts (gopt_env, x, f, gradient, calc_force)
    ...
    IF(.NOT.(ASSOCIATED(gopt_env)))CALL cp__a("motion/dimer_methods.F",79)
    dimer_env => gopt_env%dimer_env
    IF(.NOT.(ASSOCIATED(dimer_env)))CALL cp__a("motion/dimer_methods.F",81)
       ...
       IF (natoms>1) THEN
          ...
          DO i = 1, dof
             ...
             DO j = i+1, dof
                ...
                IF(.NOT.(ABS(norm)<thrs_motion))CALL cp__a("motion/dimer_methods.F",229)
             END DO
          END DO
          DO i = 1, dof
  ...
  SUBROUTINE  cp_eval_at_ts_low (gopt_env, x,  dimer_index, dimer_env, calc_force,&
    ...
    IF (PRESENT(gradient)) THEN
       ...
       DO ip=1,particles%n_els
          DO idir=1,3
             idg=idg+1
             IF(.NOT.(SIZE(gradient)>=idg))CALL cp__a("motion/dimer_methods.F",299)
