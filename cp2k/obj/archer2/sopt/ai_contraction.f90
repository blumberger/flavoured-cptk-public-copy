# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_contraction.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_contraction.F"
!-----------------------------------------------------------------------------!
...
# 20 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_contraction.F" 2
...
! *****************************************************************************
  SUBROUTINE contraction_ab(sab,qab,ca,na,ma,cb,nb,mb,fscale,trans)
    ...
    REAL(KIND=dp), DIMENSION(:, :), &
      INTENT(INOUT)                          :: qab
    REAL(KIND=dp), DIMENSION(:, :), &
      INTENT(IN), OPTIONAL                   :: ca
    INTEGER, INTENT(IN), OPTIONAL            :: na, ma
    REAL(KIND=dp), DIMENSION(:, :), &
      INTENT(IN), OPTIONAL                   :: cb
    ...
    INTEGER                                  :: lda, ldb, ldq, lds, ldw, mal, &
                                                mbl, nal, nbl
    LOGICAL                                  :: my_trans
    REAL(KIND=dp)                            :: fs
...
! Should output matrix be transposed?
    ...
    ELSE
       ...
       CALL cp__b("aobasis/ai_contraction.F",175,"Copy of arrays is not covered here")
    END IF
...
! Active matrix size
    ...
    ELSE IF(PRESENT(ca) .AND. PRESENT(cb)) THEN
       CALL cp__b("aobasis/ai_contraction.F",283,"Not implemented")
    ELSE IF(PRESENT(ca) .AND. PRESENT(cc)) THEN
       CALL cp__b("aobasis/ai_contraction.F",285,"Not implemented")
    ELSE IF(PRESENT(cb) .AND. PRESENT(cc)) THEN
       CALL cp__b("aobasis/ai_contraction.F",287,"Not implemented")
    ELSE IF(PRESENT(ca)) THEN
       CALL cp__b("aobasis/ai_contraction.F",289,"Not implemented")
    ELSE IF(PRESENT(cb)) THEN
       CALL cp__b("aobasis/ai_contraction.F",291,"Not implemented")
    ELSE IF(PRESENT(cc)) THEN
       CALL cp__b("aobasis/ai_contraction.F",293,"Not implemented")
    ELSE
       ! Copy of arrays is not covered here
       CALL cp__b("aobasis/ai_contraction.F",296,"Copy of arrays is not covered here")
    END IF
...
! *****************************************************************************
  SUBROUTINE force_trace_ab(force,sab,pab,na,nb,m,trans)
    ...
    IF(.NOT.(m<=SIZE(SAB,3)))CALL cp__a("aobasis/ai_contraction.F",393)
    IF(.NOT.(m<=SIZE(force,1)))CALL cp__a("aobasis/ai_contraction.F",394)
...
! *****************************************************************************
  SUBROUTINE block_add_ab(dir,sab,na,nb,qab,ia,ib,trans)
    ...
    ELSEIF(dir == "OUT" .OR. dir == "out" ) THEN
       ...
       END IF
    ELSE
       CALL cp__b("aobasis/ai_contraction.F",469,"")
    END IF
