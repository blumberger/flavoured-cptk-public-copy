# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_electronic_structure.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_electronic_structure.F"
!-----------------------------------------------------------------------------!
...
MODULE atom_electronic_structure
  ...
  USE atom_utils,                      ONLY: &
       ...
       ceri_contract, coulomb_potential_analytic, coulomb_potential_numeric, &
       eeri_contract, err_matrix, exchange_numeric, exchange_semi_analytic, &
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_electronic_structure.F" 2
...
!> \param noguess ...
! *****************************************************************************
  SUBROUTINE calculate_atom(atom,iw,noguess)
    ...
    SELECT CASE (method)
      CASE (do_rks_atom, do_rhf_atom)
        CALL calculate_atom_restricted(atom,iw,noguess)
      CASE (do_uks_atom, do_uhf_atom)
        CALL calculate_atom_unrestricted(atom,iw,noguess)
      CASE (do_rohf_atom)
        CALL cp__b("atom_electronic_structure.F",79,"")
      CASE DEFAULT
        CALL cp__b("atom_electronic_structure.F",81,"")
    END SELECT
...
!> \param noguess ...
! *****************************************************************************
  SUBROUTINE calculate_atom_restricted(atom,iw,noguess)
    TYPE(atom_type), POINTER                 :: atom
    INTEGER, INTENT(IN)                      :: iw
    LOGICAL, INTENT(IN), OPTIONAL            :: noguess
    ...
    INTEGER                                  :: handle, i, iter, l, max_iter, &
                                                method, reltyp
    LOGICAL                                  :: do_hfx, doguess, need_vxc, &
    ...
    SELECT CASE (method)
      CASE DEFAULT
        CALL cp__b("atom_electronic_structure.F",140,"")
      CASE (do_rks_atom)
        need_x = do_hfx
        need_xc = .TRUE.
      CASE (do_uks_atom)
        CALL cp__b("atom_electronic_structure.F",145,"")
      CASE (do_rhf_atom)
        need_x = .TRUE.
        need_xc = .FALSE.
        hf_frac = 1._dp
      CASE (do_uhf_atom)
        CALL cp__b("atom_electronic_structure.F",151,"")
      CASE (do_rohf_atom)
        need_x = .TRUE.
        need_xc = .FALSE.
        hf_frac = 1._dp
        CALL cp__b("atom_electronic_structure.F",156,"")
    END SELECT
...
! ZMP starting to read external vxc for the zmp calculation
    ...
    SELECT CASE (atom%potential%ppot_type)
      CASE DEFAULT
        CALL cp__b("atom_electronic_structure.F",191,"")
      CASE (NO_PSEUDO)
        SELECT CASE (reltyp)
          CASE DEFAULT
            CALL cp__b("atom_electronic_structure.F",195,"")
          CASE (do_nonrel_atom)
    ...
    DO            !SCF Loop
      ...
      SELECT CASE (atom%potential%ppot_type)
        CASE DEFAULT
          CALL cp__b("atom_electronic_structure.F",263,"")
        CASE (NO_PSEUDO)
      ...
      SELECT CASE (atom%coulomb_integral_type)
        CASE DEFAULT
          CALL cp__b("atom_electronic_structure.F",287,"")
        CASE (do_analytic)
      ...
      IF (need_x) THEN
        ...
        SELECT CASE (atom%exchange_integral_type)
          CASE DEFAULT
            CALL cp__b("atom_electronic_structure.F",306,"")
          CASE (do_analytic)
      ...
      CALL atom_history_update (history,fmat%op,jmat%op)
      CALL atom_opt (fmat%op,history,deps)
...
!> \param noguess ...
! *****************************************************************************
  SUBROUTINE calculate_atom_unrestricted(atom,iw,noguess)
    TYPE(atom_type), POINTER                 :: atom
    INTEGER, INTENT(IN)                      :: iw
    LOGICAL, INTENT(IN), OPTIONAL            :: noguess
    ...
    SELECT CASE (method)
      CASE DEFAULT
        CALL cp__b("atom_electronic_structure.F",461,"")
      CASE (do_rks_atom)
        CALL cp__b("atom_electronic_structure.F",463,"")
      CASE (do_uks_atom)
        need_x = do_hfx
        need_xc = .TRUE.
      CASE (do_rhf_atom)
        CALL cp__b("atom_electronic_structure.F",468,"")
      CASE (do_uhf_atom)
        need_x = .TRUE.
        need_xc = .FALSE.
        hf_frac = 1._dp
      CASE (do_rohf_atom)
        ...
        need_xc = .FALSE.
        hf_frac = 1._dp
        CALL cp__b("atom_electronic_structure.F",477,"")
    END SELECT
    ...
    SELECT CASE (atom%potential%ppot_type)
      CASE DEFAULT
        CALL cp__b("atom_electronic_structure.F",514,"")
      CASE (NO_PSEUDO)
        SELECT CASE (reltyp)
          CASE DEFAULT
            CALL cp__b("atom_electronic_structure.F",518,"")
          CASE (do_nonrel_atom)
            hcore%op = atom%integrals%kin - atom%zcore*atom%integrals%core
          CASE (do_zoramp_atom, do_sczoramp_atom)
            hcore%op = atom%integrals%kin + atom%integrals%tzora - atom%zcore*atom%integrals%core
    ...
    DO            !SCF Loop
      ...
      SELECT CASE (atom%potential%ppot_type)
        CASE DEFAULT
          CALL cp__b("atom_electronic_structure.F",604,"")
        CASE (NO_PSEUDO)
          atom%energy%eploc = 0._dp
          atom%energy%epnl = 0._dp
        CASE (GTH_PSEUDO)
      ...
      SELECT CASE (atom%coulomb_integral_type)
        CASE DEFAULT
          CALL cp__b("atom_electronic_structure.F",628,"")
        CASE (do_analytic)
          CALL ceri_contract(jmat%op,atom%integrals%ceri,atom%orbitals%pmat,atom%integrals%n)
        CASE (do_semi_analytic)
          CALL coulomb_potential_analytic(cpot%op,atom%orbitals%pmat,atom%basis,atom%basis%grid,&
      ...
      IF (need_x) THEN
        ...
        SELECT CASE (atom%exchange_integral_type)
          CASE DEFAULT
            CALL cp__b("atom_electronic_structure.F",648,"")
          CASE (do_analytic)
      ...
      ! update history container and extrapolate KS matrix
      CALL atom_history_update (historya,fmata%op,xcmata%op)
      CALL atom_history_update (historyb,fmatb%op,xcmatb%op)
      CALL atom_opt (fmata%op,historya,depsa)
      CALL atom_opt (fmatb%op,historyb,depsb)
