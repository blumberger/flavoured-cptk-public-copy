# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_coulomb.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_coulomb.F"
!-----------------------------------------------------------------------------!
...
# 97 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dftb_coulomb.F" 2
...
! *****************************************************************************
  SUBROUTINE build_dftb_coulomb(qs_env,ks_matrix,rho,mcharge,energy,&
    ...
    INTEGER :: atom_i, atom_j, blk, ewald_type, handle, i, ia, iatom, ic, &
      icol, ikind, img, irow, is, jatom, jkind, natom, nimg, nmat
    ...
    DO i=1,nmat
      mcout = 0._dp
      CALL cp_dbcsr_multiply_local(gamma_matrix(i)%matrix,mcin,mcout,ncol=1)
      gmcharge(:,i) = mcout(:,1)
    END DO
    ...
    IF (do_ewald) THEN
      ...
      CASE DEFAULT
        CALL cp__b("qs_dftb_coulomb.F",254,"Invalid Ewald type")
      CASE(do_ewald_none)
        CALL cp__b("qs_dftb_coulomb.F",256,"Not allowed with DFTB")
      CASE(do_ewald_ewald)
        CALL cp__b("qs_dftb_coulomb.F",258,"Standard Ewald not implemented in DFTB")
      CASE(do_ewald_pme)
        CALL cp__b("qs_dftb_coulomb.F",260,"PME not implemented in DFTB")
      CASE(do_ewald_spme)
    ...
    ELSE
      ...
      DO ikind=1,SIZE(local_particles%n_el)
        ...
        END DO
      END DO
      IF(.NOT.(.NOT.use_virial))CALL cp__a("qs_dftb_coulomb.F",285)
    END IF
    ...
    IF ( .NOT. just_energy ) THEN
      ...
      DO WHILE (neighbor_list_iterate(nl_iterator)==0)
         ...
         ELSE
            ...
            IF(.NOT.(ic > 0))CALL cp__a("qs_dftb_coulomb.F",369)
         END IF
         ...
         IF(.NOT.(found))CALL cp__a("qs_dftb_coulomb.F",377)
         DO is=1,SIZE(ks_matrix,1)
            ...
            IF(.NOT.(found))CALL cp__a("qs_dftb_coulomb.F",382)
...
! *****************************************************************************
  SUBROUTINE build_dftb_coulomb_qmqm(qs_env,ks_matrix,rho,mcharge,energy,&
    ...
    REAL(KIND=dp)                            :: alpha, deth, dfr, dr, fi, fr, &
                                                gmij
    REAL(KIND=dp), DIMENSION(3)              :: fij, rij
    REAL(KIND=dp), DIMENSION(:, :), POINTER  :: dsblock, gmcharge, ksblock, &
                                                ksblock_2, pblock, sblock
    TYPE(atomic_kind_type), DIMENSION(:), &
      POINTER                                :: atomic_kind_set
    TYPE(atprop_type), POINTER               :: atprop
    ...
    IF(.NOT.(.NOT.atprop%energy))CALL cp__a("qs_dftb_coulomb.F",533)
    ! no stress tensor possible for QMMM
    IF(.NOT.(.NOT.use_virial))CALL cp__a("qs_dftb_coulomb.F",535)
    ...
    SELECT CASE(ewald_type)
    CASE DEFAULT
       CALL cp__b("qs_dftb_coulomb.F",581,"Invalid Ewald type")
    CASE(do_ewald_none)
       CALL cp__b("qs_dftb_coulomb.F",583,"Not allowed with DFTB")
    CASE(do_ewald_ewald)
       CALL cp__b("qs_dftb_coulomb.F",585,"Standard Ewald not implemented in DFTB")
    CASE(do_ewald_pme)
       CALL cp__b("qs_dftb_coulomb.F",587,"PME not implemented in DFTB")
    CASE(do_ewald_spme)
    ...
    IF ( .NOT. just_energy ) THEN
       ...
       CALL cp_dbcsr_iterator_start(iter, ks_matrix(1,1)%matrix)
       DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          CALL cp_dbcsr_iterator_next_block(iter, iatom, jatom, ksblock, blk)
          NULLIFY(sblock,ksblock_2)
          IF (SIZE(ks_matrix,1)>1) CALL cp_dbcsr_get_block_p(matrix=ks_matrix(2,1)%matrix,&
...
! *****************************************************************************
  SUBROUTINE dftb_spme_evaluate (ewald_env,ewald_pw,particle_set,box,&
    ...
    INTEGER                                  :: group, handle, i, ig, ipart, &
                                                j, n, npart, o_spline, p1
