# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_local.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_local.F"
!-----------------------------------------------------------------------------!
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_efield_local.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_efield_integrals (qs_env,rpoint)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_efield_local.F",127)
    ...
    DO i=1,3
       ...
       CALL cp_dbcsr_init(dipmat(i)%matrix)
       CALL cp_dbcsr_copy(dipmat(i)%matrix,matrix_s(1)%matrix,'DIP MAT')
       CALL cp_dbcsr_set(dipmat(i)%matrix,0.0_dp)
    END DO
...
! *****************************************************************************
  SUBROUTINE qs_efield_mo_derivatives(qs_env,rpoint,just_energy,calculate_forces)
    ...
    INTEGER :: atom_a, atom_b, handle, i, ia, iatom, icol, idir, ikind, &
      inode, irow, iset, ispin, jatom, jkind, jset, ldab, natom, ncoa, ncob, &
    ...
    DO ispin = 1,SIZE(matrix_p)
       DO idir=1,3
          CALL cp_dbcsr_trace(matrix_p(ispin)%matrix,dipmat(idir)%matrix,tmp)
       ...
       IF(calculate_forces)THEN
          ...
          DO WHILE (neighbor_list_iterate(nl_iterator)==0)
             ...
             IF(SIZE(matrix_p) > 1) THEN
                ...
                IF(.NOT.(found))CALL cp__a("qs_efield_local.F",343)
             END IF
             forcea = 0.0_dp
             forceb = 0.0_dp
