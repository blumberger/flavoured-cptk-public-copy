# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_utils.F"
!-----------------------------------------------------------------------------!
...
# 109 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE low_spin_roks(energy,qs_env,dft_control,just_energy,&
    ...
    ! we have two spins
    IF(.NOT.(SIZE(mo_array,1)==2))CALL cp__a("qs_ks_utils.F",196)
    Nspin=2
    ! we want uniform occupations
    CALL get_mo_set(mo_set=mo_array(1)%mo_set, uniform_occupation=uniform_occupation)
    IF(.NOT.(uniform_occupation))CALL cp__a("qs_ks_utils.F",200)
    CALL get_mo_set(mo_set=mo_array(2)%mo_set,mo_coeff_b=mo_coeff, uniform_occupation=uniform_occupation)
    IF(.NOT.(uniform_occupation))CALL cp__a("qs_ks_utils.F",202)

    NULLIFY(dbcsr_deriv)
    CALL cp_dbcsr_init_p(dbcsr_deriv)
    CALL cp_dbcsr_copy(dbcsr_deriv,mo_derivs(1)%matrix)
    CALL cp_dbcsr_set(dbcsr_deriv,0.0_dp)
    ...
    IF(.NOT.(n_rep==Nterms))CALL cp__a("qs_ks_utils.F",224)
    CALL section_vals_val_get(low_spin_roks_section,"SPIN_CONFIGURATION",i_rep_val=1,i_vals=ivec)
    Nelectron=SIZE(ivec)
    IF(.NOT.(Nelectron==k_alpha-k_beta))CALL cp__a("qs_ks_utils.F",227)
    ...
    DO iterm=1,Nterms
       ...
       IF(.NOT.(Nelectron==SIZE(ivec)))CALL cp__a("qs_ks_utils.F",232)
       in_range=ALL(ivec>=1) .AND. ALL(ivec<=2)
       IF(.NOT.(in_range))CALL cp__a("qs_ks_utils.F",234)
       DO k=1,Nelectron
    ...
    NULLIFY(matrix_p)
    CALL cp_dbcsr_allocate_matrix_set(matrix_p,Nspin)
    DO ispin=1,Nspin
       ALLOCATE(matrix_p(ispin)%matrix)
       CALL cp_dbcsr_init(matrix_p(ispin)%matrix)
       CALL cp_dbcsr_copy(matrix_p(ispin)%matrix,rho_ao(1)%matrix,&
            name="density matrix low spin roks")
       CALL cp_dbcsr_set(matrix_p(ispin)%matrix,0.0_dp)
    ENDDO

    NULLIFY(matrix_h)
    CALL cp_dbcsr_allocate_matrix_set(matrix_h,Nspin)
    DO ispin=1,Nspin
       ALLOCATE(matrix_h(ispin)%matrix)
       CALL cp_dbcsr_init(matrix_h(ispin)%matrix)
       CALL cp_dbcsr_copy(matrix_h(ispin)%matrix,rho_ao(1)%matrix,&
            name="KS matrix low spin roks")
       CALL cp_dbcsr_set(matrix_h(ispin)%matrix,0.0_dp)
    ENDDO
    ...
    CALL cp_dbcsr_init_p(fm_scaled)
    CALL cp_dbcsr_init_p(fm_deriv)
    CALL cp_dbcsr_copy(fm_scaled,mo_coeff)
    CALL cp_dbcsr_copy(fm_deriv,mo_coeff)
    ...
    DO iterm=1,Nterms
       ...
       DO ispin=1,Nspin
          ...
          CALL cp_dbcsr_copy(fm_scaled,mo_coeff)
          CALL cp_dbcsr_scale_by_vector(fm_scaled,scaling,side='right')
       ...
       IF (just_energy) THEN
           exc=xc_exc_calc(rho_r=rho_r, rho_g=rho_g, tau=tau, xc_section=xc_section,&
                   pw_pool=xc_pw_pool)
       ELSE
           IF(.NOT.(.NOT.compute_virial))CALL cp__a("qs_ks_utils.F",317)
       ...
       IF (.NOT. just_energy) THEN
           ...
           DO ispin=1,Nspin
              ...
              scaling=1.0_dp
              scaling(k_alpha-Nelectron+1:k_alpha)=occupations(ispin,:,iterm)
              CALL cp_dbcsr_scale_by_vector(dbcsr_deriv,scaling,side='right')
              CALL cp_dbcsr_add(mo_derivs(1)%matrix, dbcsr_deriv,1.0_dp,1.0_dp)
           ENDDO
    ...
    CALL cp_dbcsr_deallocate_matrix_set(matrix_p)
    CALL cp_dbcsr_deallocate_matrix_set(matrix_h)

    CALL pw_pool_give_back_pw(auxbas_pw_pool,work_v_rspace%pw)

    CALL cp_dbcsr_release_p(fm_deriv)
    CALL cp_dbcsr_release_p(fm_scaled)

    DEALLOCATE(occupations)
    DEALLOCATE(energy_scaling)
    DEALLOCATE(scaling)

    CALL cp_dbcsr_release_p(dbcsr_deriv)
...
! *****************************************************************************
  SUBROUTINE sic_explicit_orbitals(energy,qs_env,dft_control,poisson_env,just_energy,&
    ...
    ! we have two spins
    IF(.NOT.(SIZE(mo_array,1)==2))CALL cp__a("qs_ks_utils.F",464)
    ! we want uniform occupations
    CALL get_mo_set(mo_set=mo_array(1)%mo_set, uniform_occupation=uniform_occupation)
    IF(.NOT.(uniform_occupation))CALL cp__a("qs_ks_utils.F",467)
    CALL get_mo_set(mo_set=mo_array(2)%mo_set,mo_coeff=mo_coeff, uniform_occupation=uniform_occupation)
    IF(.NOT.(uniform_occupation))CALL cp__a("qs_ks_utils.F",469)

    NULLIFY (tmp_dbcsr)
    CALL cp_dbcsr_allocate_matrix_set(tmp_dbcsr,SIZE(mo_derivs,1))
    DO i=1,SIZE(mo_derivs,1)!fm->dbcsr
       !
       NULLIFY(tmp_dbcsr(i)%matrix)
       CALL cp_dbcsr_init_p(tmp_dbcsr(i)%matrix)
       CALL cp_dbcsr_copy(tmp_dbcsr(i)%matrix,mo_derivs(i)%matrix)
       CALL cp_dbcsr_set(tmp_dbcsr(i)%matrix,0.0_dp)
    ENDDO!fm->dbcsr
    ...
    CASE(sic_list_unpaired)
      ! we have two spins
      IF(.NOT.(SIZE(mo_array,1)==2))CALL cp__a("qs_ks_utils.F",517)
      ! we have them restricted
      IF(.NOT.(SIZE(mo_derivs,1)==1))CALL cp__a("qs_ks_utils.F",519)
      IF(.NOT.(dft_control%restricted))CALL cp__a("qs_ks_utils.F",520)
    ...
    DO iorb=1,Norb
       ...
       IF (just_energy) THEN
           exc=xc_exc_calc(rho_r=rho_r, rho_g=rho_g, tau=tau, xc_section=xc_section,&
                   pw_pool=xc_pw_pool)
       ELSE
           IF(.NOT.(.NOT.compute_virial))CALL cp__a("qs_ks_utils.F",644)
           CALL xc_vxc_pw_create1(vxc_rho=vxc, rho_r=rho_r,&
                   rho_g=rho_g, tau=tau, vxc_tau=vxc_tau, exc=exc, xc_section=xc_section, &
    ...
    CALL cp_dbcsr_deallocate_matrix(orb_density_matrix)
    CALL cp_dbcsr_deallocate_matrix(orb_h)
    ...
    CALL cp_dbcsr_deallocate_matrix_set(tmp_dbcsr)!fm->dbcsr
...
! *****************************************************************************
  SUBROUTINE calc_v_sic_rspace(v_sic_rspace,energy,&
    ...
    IF(dft_control%qs_control%gapw)&
       CALL cp__b("qs_ks_utils.F",748,"sic and GAPW not yet compatible")

    ! OK, right now we like two spins to do sic, could be relaxed for AD
    IF(.NOT.(dft_control%nspins == 2))CALL cp__a("qs_ks_utils.F",751)
    ...
    CASE ( sic_ad )
       ...
       CALL pw_poisson_solve(poisson_env,work_rho%pw, ener, work_v%pw)
    CASE DEFAULT
       CALL cp__b("qs_ks_utils.F",780,"Unknown sic method id")
    END SELECT
    ...
    CASE DEFAULT
       CALL cp__b("qs_ks_utils.F",813,"Unknown sic method id")
    END SELECT
    ...
    IF(BTEST(cp_print_key_should_output(logger%iter_info,scf_section,"PRINT%TOTAL_DENSITIES"),cp_p_file)) THEN
      DO ispin=1,dft_control%nspins
         DO img=1,dft_control%nimages
            CALL cp_dbcsr_trace(rho_ao(ispin,img)%matrix,matrix_s(1,img)%matrix,trace_tmp)
...
! *****************************************************************************
  SUBROUTINE compute_matrix_vxc(qs_env,v_rspace, matrix_vxc)
    ...
    DO ispin=1,SIZE(matrix_ks)
       ...
       CALL cp_dbcsr_scale(matrix_vxc(ispin)%matrix,v_rspace(ispin)%pw%pw_grid%dvol)
    ENDDO
...
! *****************************************************************************
  SUBROUTINE sum_up_and_integrate(qs_env,ks_matrix,rho,my_rho,&
    ...
    REAL(dp)                                 :: dvol
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
      POINTER                                :: ksmat, matrix_ks_aux_fit, &
    ...
    IF (ASSOCIATED(v_rspace_new)) THEN
       DO ispin=1,nspins
          IF (gapw_xc) THEN
             ...
             IF(.NOT.(dft_control%sic_method_id==sic_none))CALL cp__a("qs_ks_utils.F",1267)
          ...
          END IF
          IF (do_ppl) THEN
             IF(.NOT.(.NOT.gapw))CALL cp__a("qs_ks_utils.F",1332)
             v_rspace_new(ispin)%pw%cr3d = v_rspace_new(ispin)%pw%cr3d + &
    ...
    ELSE
       ...
       IF(.NOT.(dft_control%sic_method_id==sic_none))CALL cp__a("qs_ks_utils.F",1372)
       IF(.NOT.(.NOT.dft_control%qs_control%ddapc_restraint_is_spin))CALL cp__a("qs_ks_utils.F",1373)
       DO ispin=1,nspins
          ! the efield contribution
          IF (dft_control%apply_efield_field) THEN
...
!, v_h_gspace, &
    ...
    IF(.NOT.(stat==0))CALL cp__a("qs_ks_utils.F",1525)
