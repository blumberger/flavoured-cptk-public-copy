# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_methods.F"
!-----------------------------------------------------------------------------!
...
# 81 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE kpoint_initialize(kpoint, particle_set, cell)
    ...
    CHARACTER(LEN=2), ALLOCATABLE, &
      DIMENSION(:)                           :: element
    INTEGER                                  :: handle, i, ik, iounit, natom
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_methods.F",125)
    ...
    SELECT CASE (kpoint%kp_scheme)
       ...
       CASE ("MONKHORST-PACK","MACDONALD")
          ...
          IF(kpoint%full_grid) THEN
             ...
             natom = 10
             ALLOCATE(coord(3,natom),zeff(natom),weight(natom),atype(natom),element(natom))
             DO i=1,natom
                atype(i) = i
          ...
          DEALLOCATE(coord,zeff,weight,atype,element)

       CASE ("GENERAL")
          ! do nothing
       CASE DEFAULT
          IF(.NOT.(.FALSE.))CALL cp__a("kpoint_methods.F",183)
    END SELECT
    ...
    SELECT CASE (kpoint%kp_scheme)
       ...
       CASE ("GAMMA")
          IF(.NOT.(kpoint%nkp==1))CALL cp__a("kpoint_methods.F",191)
          IF(.NOT.(SUM(ABS(kpoint%xkp))<=1.e-12_dp))CALL cp__a("kpoint_methods.F",192)
          IF(.NOT.(kpoint%wkp(1)==1.0_dp))CALL cp__a("kpoint_methods.F",193)
          IF(.NOT.(.NOT.kpoint%symmetry))CALL cp__a("kpoint_methods.F",194)
       CASE ("GENERAL")
          IF(.NOT.(.NOT.kpoint%symmetry))CALL cp__a("kpoint_methods.F",196)
          IF(.NOT.(kpoint%nkp>=1))CALL cp__a("kpoint_methods.F",197)
       CASE ("MONKHORST-PACK","MACDONALD")
          IF(.NOT.(kpoint%nkp>=1))CALL cp__a("kpoint_methods.F",199)
    END SELECT
    IF(kpoint%use_real_wfn) THEN
       ...
       IF (.NOT.spez .AND. .NOT.(inv .AND. symmo)) THEN
          ...
          CALL cp_warn(cp__l("kpoint_methods.F",211),&
...
! *****************************************************************************
  SUBROUTINE kpoint_env_initialize(kpoint)
    ...
    INTEGER :: comm_cart, comm_inter_kp, comm_kp, handle, igr, ik, ikk, ngr, &
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_methods.F",246)
    para_env => kpoint%para_env
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("kpoint_methods.F",248)
    IF(.NOT.(.NOT.ASSOCIATED(kpoint%kp_env)))CALL cp__a("kpoint_methods.F",249)
    ...
    ELSE
       ...
       ELSE
          IF(.NOT.(.FALSE.))CALL cp__a("kpoint_methods.F",298)
       END IF
       nkp_grp = npe/ngr
       ! processor dimensions
       dims(1)=ngr
       dims(2)=nkp_grp
       IF(.NOT.(MOD(nkp,nkp_grp)==0))CALL cp__a("kpoint_methods.F",304)
       nkp_loc=nkp/nkp_grp

       IF ((dims(1)*dims(2)/=npe)) THEN
          CALL cp__b("kpoint_methods.F",308,"Number of processors is not divisible by the kpoint group size.")
       END IF
...
! *****************************************************************************
  SUBROUTINE kpoint_initialize_mos(kpoint, mos)
    ...
    INTEGER                                  :: handle, ic, ik, is, nao, nc, &
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_methods.F",402)
    ...
    IF (ASSOCIATED(mos)) THEN
       ...
       IF (nkp_loc > 0) THEN
          IF(.NOT.(SIZE(kpoint%kp_env)==nkp_loc))CALL cp__a("kpoint_methods.F",414)
...
! *****************************************************************************
  SUBROUTINE kpoint_init_cell_index(kpoint, sab_nl, para_env, dft_control)
    ...
    IF(.NOT.(ASSOCIATED(kpoint)))CALL cp__a("kpoint_methods.F",524)
...
! *****************************************************************************
  SUBROUTINE rskp_transform(rmatrix,cmatrix,rsmat,ispin,&
    ...
    IF(wfn_real_only) THEN
       CALL cp_dbcsr_set(rmatrix,0.0_dp)
    ELSE
       CALL cp_dbcsr_set(rmatrix,0.0_dp)
       CALL cp_dbcsr_set(cmatrix,0.0_dp)
    END IF
...
! *****************************************************************************
  SUBROUTINE kpoint_set_mo_occupation(kpoint,smear)
    ...
    INTEGER                                  :: handle, ik, ikpgr, ispin, &
                                                kplocal, nb, ne_a, ne_b, &
                                                nelectron, nkp, nmo, nspin
...
! first collect all the eigenvalues
    ...
    IF(nspin==2) THEN
       ...
       IF(.NOT.(nmo==nb))CALL cp__a("kpoint_methods.F",749)
    END IF
       ...
       SELECT CASE (smear%method)
          CASE (smear_fermi_dirac)
             ...
             ELSE IF (smear%fixed_mag_mom > 0.0_dp) THEN
                CALL cp__b("kpoint_methods.F",778,"kpoints: Not supoorted")
             ...
             END IF
          CASE DEFAULT
             CALL cp__b("kpoint_methods.F",793,"kpoints: Not supported")
       END SELECT
    ELSE
...
! *****************************************************************************
  SUBROUTINE kpoint_density_matrices(kpoint,energy_weighted)
    ...
    INTEGER                                  :: handle, ikpgr, ispin, &
                                                kplocal, nao, nmo, nspin
    INTEGER, DIMENSION(2)                    :: kp_range
    LOGICAL                                  :: wtype
...
! *****************************************************************************
  SUBROUTINE kpoint_density_transform(kpoint,denmat,wtype,tempmat,sab_nl,fmwork)
    ...
    INTEGER                                  :: bcntxt, handle, iatom, ic, &
                                                icell, icol, ik, ikk, irow, &
                                                ispin, jatom, nc, nimg, nkp, &
                                                nspin
    INTEGER, DIMENSION(3)                    :: cell
    INTEGER, DIMENSION(:, :, :), POINTER     :: cell_to_index
    LOGICAL                                  :: do_symmetric, found, &
    ...
    IF(.NOT.kpoint%full_grid)&
       CALL cp_abort(cp__l("kpoint_methods.F",975),"k-points need FULL_GRID currently. "//&
                  "Reduced grids not yet working correctly")

    ! work storage
    ALLOCATE(rpmat,cpmat)
    ...
    DO ispin=1,nspin
       ...
       DO ik=1,nkp
          ...
          IF(.NOT.(SIZE(fmwork)>=nc))CALL cp__a("kpoint_methods.F",1012)
          DO ic=1,nc
             IF(my_kpgrp) THEN
          ...
          DO WHILE (neighbor_list_iterate(nl_iterator)==0)
             ...
             IF(.NOT.(found))CALL cp__a("kpoint_methods.F",1057)

             IF(real_only) THEN
                CALL cp_dbcsr_get_block_p(matrix=rpmat,row=irow,col=icol,&
                                          block=rblock,found=found)
                IF(.NOT.(found))CALL cp__a("kpoint_methods.F",1062)
                dblock = dblock + coskl*rblock
             ELSE
                CALL cp_dbcsr_get_block_p(matrix=rpmat,row=irow,col=icol,&
                                          block=rblock,found=found)
                IF(.NOT.(found))CALL cp__a("kpoint_methods.F",1067)
                CALL cp_dbcsr_get_block_p(matrix=cpmat,row=irow,col=icol,&
                                          block=cblock,found=found)
                IF(.NOT.(found))CALL cp__a("kpoint_methods.F",1070)
                dblock = dblock + coskl*rblock
