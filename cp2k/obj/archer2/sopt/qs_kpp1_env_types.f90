# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_types.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_kpp1_env_types.F" 2
...
! *****************************************************************************
  SUBROUTINE kpp1_release(kpp1_env)
    ...
    IF (ASSOCIATED(kpp1_env)) THEN
       IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_kpp1_env_types.F",97)
          ...
          IF (ASSOCIATED(kpp1_env%v_ao)) THEN
             DO ispin=1,SIZE(kpp1_env%v_ao)
                IF (ASSOCIATED(kpp1_env%v_ao(ispin)%matrix)) THEN
                   CALL cp_dbcsr_deallocate_matrix(kpp1_env%v_ao(ispin)%matrix)
                END IF
             END DO
...
! *****************************************************************************
  SUBROUTINE kpp1_retain(kpp1_env)
    ...
    IF(.NOT.(ASSOCIATED(kpp1_env)))CALL cp__a("qs_kpp1_env_types.F",153)
    IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_kpp1_env_types.F",154)
    kpp1_env%ref_count=kpp1_env%ref_count+1
