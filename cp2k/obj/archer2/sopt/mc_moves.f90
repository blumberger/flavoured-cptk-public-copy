# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_moves.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_moves.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/../../base/base_uses.f90" 1
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_moves.F" 2
...
! do some allocation
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,192,&
...
! figure out which molecule number we are
      ...
      ENDDO
      IF(molecule_number == 0) CALL cp__b("motion/mc/mc_moves.F",216,'Cannot find the molecule number')
...
! save the coordinates
      ...
      IF(.NOT. ASSOCIATED(molecule_kind)) CALL cp__b("motion/mc/mc_moves.F",249,'Cannot find the molecule type')
! do the move
...
! do some allocation
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,498,&
...
! find the index of the last atom of this molecule, and the molecule number
      ...
      DO imolecule=1,SUM(nchains(:,box_number))
         ...
         atom_number=atom_number+nunits(mol_type(imolecule+start_mol-1))
      ENDDO
      IF(molecule_number == 0) CALL cp__b("motion/mc/mc_moves.F",513,'Cannot find the molecule number')
...
! do some allocation
      ALLOCATE (r_old(1:3,1:nunits_tot(box_number)),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,732,&
                       "r",3*nunits_tot(box_number)*dp_size)
...
! molecule number this is
      ...
      DO imolecule=1,SUM(nchains(:,box_number))
         ...
         atom_number=atom_number+nunits(mol_type(imolecule+start_mol-1))
      ENDDO
      IF(molecule_number == 0) CALL cp__b("motion/mc/mc_moves.F",751,'Cannot find the molecule number')
...
! do some allocation
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1051,&
         "r",3*nunits_tot(box_number)*dp_size)
...
! stop if we're trying to change a box to a boxlength smaller than rcut
      ...
      IF(ltoo_small) &
         CALL cp__b("motion/mc/mc_moves.F",1239,"Attempted a volume move where box size got too small.")
...
! pick which bond in the molecule at random
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1390,&
                       "connection",2*natom*dp_size)
! assume at most six bonds per atom
      ALLOCATE(connectivity(1:6,1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1394,&
                       "connectivity",6*natom*dp_size)
      ALLOCATE(counter(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1397,&
                       "counter",natom)
      ALLOCATE(atom_a(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1400,&
                       "atom_a",natom)
      ALLOCATE(atom_b(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1403,&
                       "atom_b",natom)
      connection(:,:)=0
      connectivity(:,:)=0
      counter(:)=0
...
! pick which bond in the molecule at random
      ...
      ALLOCATE(connection(1:natom,1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1575,&
                       "connection",2*natom*dp_size)
! assume at most six bonds per atom
      ALLOCATE(connectivity(1:6,1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1579,&
                       "connectivity",6*natom*dp_size)
      ALLOCATE(counter(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1582,&
                       "counter",natom)
      ALLOCATE(atom_a(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1585,&
                       "atom_a",natom)
      ALLOCATE(atom_c(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1588,&
                       "atom_c",natom)
      connection(:,:)=0
      connectivity(:,:)=0
      counter(:)=0
...
!      CALL RANDOM_NUMBER(rand)
      ...
      ALLOCATE(connection(1:natom,1:2),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1853,&
                       "connection",2*natom*dp_size)
! assume at most six bonds per atom
      ALLOCATE(connectivity(1:6,1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1857,&
                       "connectivity",6*natom*dp_size)
      ALLOCATE(counter(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1860,&
                       "counter",natom)
      ALLOCATE(atom_a(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1863,&
                       "atom_a",natom)
      ALLOCATE(atom_d(1:natom),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,1866,&
                       "atom_d",natom)
      connection(:,:)=0
      connectivity(:,:)=0
      counter(:)=0
...
! do some allocation
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,2091,&
...
! do some allocation
      ALLOCATE (r(1:3,1:nunits_tot(box_number)),STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,2472,&
         "r",3*nunits_tot(box_number)*dp_size)
      ALLOCATE (hmc_ekin)
