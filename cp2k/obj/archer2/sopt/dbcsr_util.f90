# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_util.F"
!-----------------------------------------------------------------------------!
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dist/dbcsr_util.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    IF (local) THEN
       CALL dbcsr_assert (nlocal, "EQ", nlo,&
            ...
            "Invalid size for local offsets",278)
    ENDIF
    IF (local) THEN
       acc = 1
       DO lel = 1, nlocal
...
! *****************************************************************************
  SUBROUTINE dbcsr_verify_matrix(m, verbosity, local)
    ...
    INTEGER :: bc, blk, blk_p, br, data_size_referenced, dbg, handle, i, mb, &
      mn, n, n_have_blocks_local, n_have_blocks_total, prev_br
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_is_initialized(m),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Can not verify non-existing matrix object.",708)
    dbg = 2
    nocomm = .FALSE.
    IF (PRESENT (local)) nocomm = local
    ...
    IF(.NOT.dbcsr_has_symmetry (m)) THEN
       n_full_blocks_total = INT(m%m%nblkrows_total,KIND=int_8) * INT(m%m%nblkcols_total,KIND=int_8)
    ELSE
       CALL dbcsr_assert(m%m%nblkrows_total, "EQ", m%m%nblkcols_total,&
            dbcsr_fatal_level, dbcsr_internal_error, r,&
            'Symmetric matrix is not square',729)
       n_full_blocks_total = INT(m%m%nblkrows_total,KIND=int_8) * (m%m%nblkrows_total+1)/2
    ENDIF
...
2047 FORMAT (I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7)
    ...
    CALL dbcsr_assert(m%m%index(dbcsr_slot_row_p+1)-m%m%index(dbcsr_slot_row_p)+1,&
         ...
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of row_p index inconsistent with number of rows',758)
    CALL dbcsr_assert(SIZE(m%m%row_p), "EQ", m%m%nblkrows_total+1,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of row_p inconsistent with number of rows',761)
    !
    ...
    CALL dbcsr_assert(m%m%index(dbcsr_slot_col_i+1)-m%m%index(dbcsr_slot_col_i)+1,&
         "EQ", m%m%nblks,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of col_i index inconsistent with number of blocks',768)
    CALL dbcsr_assert(SIZE(m%m%col_i), "EQ", m%m%nblks,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of col inconsistent with number of blocks',771)
    !
    ...
    CALL dbcsr_assert(m%m%index(dbcsr_slot_blk_p+1)-m%m%index(dbcsr_slot_blk_p)+1,&
         "EQ", m%m%nblks,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of blk_p index inconsistent with number of blocks',778)
    CALL dbcsr_assert(SIZE(m%m%col_i), "EQ", m%m%nblks,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Size of blk_p inconsistent with number of blocks',781)
    !
    CALL dbcsr_assert(SIZE(row_blk_size), "EQ", m%m%nblkrows_total,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Row block size array inconsistent with number of blocked rows',785)
    CALL dbcsr_assert(SIZE(col_blk_size), "EQ", m%m%nblkcols_total,&
         dbcsr_fatal_level, dbcsr_internal_error, r,&
         'Column block size array inconsistent with number of blocked columns',788)
    !
    IF(dbg.GE.4) THEN
       WRITE(*,'(1X,A,I7,A,I7)')r//' nze=',m%m%nze,'data size',&
            dbcsr_data_get_size(m%m%data_area)
    ENDIF
    data_size_referenced = dbcsr_data_get_size_referenced(m%m%data_area)
    ...
    !     dbcsr_fatal_level, dbcsr_internal_error, r,&
    !     'Data storage may be too small.',799)
    IF (dbg.GE.5) THEN
