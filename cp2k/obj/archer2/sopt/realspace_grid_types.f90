# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_types.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/realspace_grid_types.F" 2
...
! *****************************************************************************
  SUBROUTINE rs_grid_create_descriptor ( desc, pw_grid, input_settings, border_points)
    ...
    INTEGER                                  :: border_size, dir, handle, i, &
                                                j, k, l, lb( 2 ), &
                                                n_slices(3), n_slices_tmp(3), &
                                                neighbours(3), nmin
    LOGICAL                                  :: overlap
    ...
    IF(.NOT.(ASSOCIATED(pw_grid)))CALL cp__a("pw/realspace_grid_types.F",225)
    ...
    ELSE
       ...
       IF (input_settings%distribution_type == rsgrid_replicated) THEN
          ...
          n_slices = 1
          IF (border_size > 0) THEN
             CALL cp_abort(cp__l("pw/realspace_grid_types.F",291),&
...
! *****************************************************************************
  SUBROUTINE rs_pw_transfer ( rs, pw, dir)
    ...
    IF ( rs % desc % grid_id /= pw % pw_grid % id_nr ) &
       CALL cp__b("pw/realspace_grid_types.F",714,"Different rs and pw indentitfiers")
    IF ( (pw%in_use .NE. REALDATA3D) .AND. (pw%in_use .NE. COMPLEXDATA3D)) &
       CALL cp__b("pw/realspace_grid_types.F",716,"Need REALDATA3D or COMPLEXDATA3D")
    IF ( (dir.NE.rs2pw) .AND. (dir.NE.pw2rs) ) &
       CALL cp__b("pw/realspace_grid_types.F",718,"Direction must be rs2pw or pw2rs")
    ...
    ELSE ! treat simple serial case locally
       IF (dir == rs2pw) THEN
          IF (pw%in_use == REALDATA3D) THEN
             ...
             ELSE
                IF(.NOT.(LBOUND(pw%cr3d,3) .EQ. rs%lb_real(3)))CALL cp__a("pw/realspace_grid_types.F",730)
          ...
          ELSEIF (pw%in_use == COMPLEXDATA3D) THEN
             ...
             ELSE
                IF(.NOT.(LBOUND(pw%cr3d,3) .EQ. rs%lb_real(3)))CALL cp__a("pw/realspace_grid_types.F",742)
                !$omp parallel do default(none) shared(pw,rs)
                DO i = rs%lb_real(3),rs%ub_real(3)
             ...
             END IF
          ELSE
             CALL cp__b("pw/realspace_grid_types.F",752,"PW type not compatible")
          END IF
       ELSE
          IF (pw%in_use == REALDATA3D) THEN
             IF (rs%desc%border == 0) THEN
          ...
          ELSEIF (pw % in_use == COMPLEXDATA3D) THEN
             ...
             END IF
          ELSE
             CALL cp__b("pw/realspace_grid_types.F",830,"PW type not compatible")
          END IF
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE rs_pw_transfer_replicated(rs,pw,dir)
    ...
    INTEGER                                  :: dest, group, i, ii, im, ip, &
                                                ix, iy, iz, j, jm, k, km, &
                                                mepos, nma, nn, np, req(2), &
                                                s(3), source
    ...
    IF ( dir == rs2pw ) THEN
       ...
       ELSE
          CALL cp__b("pw/realspace_grid_types.F",924,"PW type not compatible")
       END IF
    ELSE
       nn = rcount ( mepos )
       IF ( pw % in_use == REALDATA3D ) THEN
       ...
       ELSE
          CALL cp__b("pw/realspace_grid_types.F",933,"PW type not compatible")
       END IF
...
! *****************************************************************************
  SUBROUTINE rs_pw_transfer_distributed(rs,pw,dir)
    ...
    INTEGER :: completed, dest_down, dest_up, i, idir, j, k, lb, my_id, &
      ...
      source_down, source_up, ub, x, y, z
...
!$omp end parallel do
       ...
       IF(.NOT.(SUM(send_sizes)==PRODUCT(ub_recv - lb_recv + 1)))CALL cp__a("pw/realspace_grid_types.F",1406)
...
!$omp end parallel do
       ...
       IF (debug_this_module) THEN
          ...
          IF (ABS(pw_sum-rs_sum)/MAX(1.0_dp,ABS(pw_sum),ABS(rs_sum))>EPSILON(rs_sum)*1000) THEN
             ...
             CALL cp_abort(cp__l("pw/realspace_grid_types.F",1505),&
...
!$omp end parallel do
       ...
       IF(.NOT.(SUM(recv_sizes)==PRODUCT(ub_recv - lb_recv + 1)))CALL cp__a("pw/realspace_grid_types.F",1635)
...
! *****************************************************************************
  SUBROUTINE rs_grid_set_box ( pw_grid, rs)
    ...
    IF(.NOT.(ASSOCIATED(pw_grid)))CALL cp__a("pw/realspace_grid_types.F",2045)
    IF(.NOT.(ASSOCIATED(rs)))CALL cp__a("pw/realspace_grid_types.F",2046)
    IF(.NOT.(rs%desc % grid_id==pw_grid%id_nr))CALL cp__a("pw/realspace_grid_types.F",2047)
...
! *****************************************************************************
  SUBROUTINE rs_grid_retain(rs_grid)
    ...
    IF(.NOT.(ASSOCIATED(rs_grid)))CALL cp__a("pw/realspace_grid_types.F",2066)
    IF(.NOT.(rs_grid%ref_count>0))CALL cp__a("pw/realspace_grid_types.F",2067)
...
! *****************************************************************************
  SUBROUTINE rs_grid_retain_descriptor(rs_desc)
    ...
    IF(.NOT.(ASSOCIATED(rs_desc)))CALL cp__a("pw/realspace_grid_types.F",2084)
    IF(.NOT.(rs_desc%ref_count>0))CALL cp__a("pw/realspace_grid_types.F",2085)
...
! *****************************************************************************
  SUBROUTINE rs_grid_release(rs_grid)
    ...
    IF (ASSOCIATED(rs_grid)) THEN
       IF(.NOT.(rs_grid%ref_count>0))CALL cp__a("pw/realspace_grid_types.F",2103)
       rs_grid%ref_count=rs_grid%ref_count-1
...
! *****************************************************************************
  SUBROUTINE rs_grid_release_descriptor(rs_desc)
    ...
    IF (ASSOCIATED(rs_desc)) THEN
       IF(.NOT.(rs_desc%ref_count>0))CALL cp__a("pw/realspace_grid_types.F",2133)
       rs_desc%ref_count=rs_desc%ref_count-1
