# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_util.F"
!-----------------------------------------------------------------------------!
...
# 36 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_util.F" 2
    ...
    IF (explicit) THEN
       ...
       CASE(do_qmmm_wall_quadratic)
          IF(do_qmmm_force_mixing) THEN
            CALL cp_warn(cp__l("qmmm_util.F",78),&
...
! *****************************************************************************
  SUBROUTINE apply_qmmm_walls_reflective(force_env)
    ...
    LOGICAL                                  :: explicit, is_x(2), is_y(2), &
                                                is_z(2)
    ...
    IF (force_env%in_use==use_qmmmx) THEN
       IF(iwall_type /= do_qmmm_wall_none)&
          CALL cp_warn(cp__l("qmmm_util.F",137),&
    ...
    IF(.NOT.(ASSOCIATED(force_env%qmmm_env)))CALL cp__a("qmmm_util.F",145)
    IF(.NOT.(force_env%qmmm_env%ref_count>0))CALL cp__a("qmmm_util.F",146)
    ...
    IF(.NOT.(ASSOCIATED(qm_atom_index)))CALL cp__a("qmmm_util.F",151)
    ...
    DO ip=1,SIZE(qm_atom_index)
       ...
       IF (ANY(coord<skin).OR.ANY(coord>(qm_cell_diag-skin))) THEN
          ...
          ELSE
             ...
             CALL cp_warn(cp__l("qmmm_util.F",197),&
...
! *****************************************************************************
  SUBROUTINE apply_qmmm_walls_quadratic(qmmm_env, walls_section)
    ...
    IF(.NOT.(ASSOCIATED(qmmm_env)))CALL cp__a("qmmm_util.F",239)
    IF(.NOT.(qmmm_env%ref_count>0))CALL cp__a("qmmm_util.F",240)
    ...
    IF(.NOT.(ASSOCIATED(qm_atom_index)))CALL cp__a("qmmm_util.F",246)
...
! *****************************************************************************
  SUBROUTINE apply_qmmm_translate(qmmm_env)
    ...
    IF(.NOT.(ASSOCIATED(qmmm_env)))CALL cp__a("qmmm_util.F",419)
    IF(.NOT.(qmmm_env%ref_count>0))CALL cp__a("qmmm_util.F",420)

    CALL fist_env_get(qmmm_env%fist_env,cell=mm_cell,subsys=subsys_mm)
    CALL get_qs_env(qmmm_env%qs_env,cell=qm_cell,cp_subsys=subsys_qm)
    qm_atom_index   => qmmm_env%qm%qm_atom_index
    IF(.NOT.(ASSOCIATED(qm_atom_index)))CALL cp__a("qmmm_util.F",425)
