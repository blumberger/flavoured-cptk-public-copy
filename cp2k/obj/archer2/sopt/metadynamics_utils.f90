# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_utils.F"
!-----------------------------------------------------------------------------!
...
# 56 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadynamics_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE metadyn_read(meta_env,force_env,root_section,para_env,fe_section)
    ...
    IF (explicit) THEN
       ...
       IF(.NOT.(.NOT.ASSOCIATED(meta_env)))CALL cp__a("metadynamics_utils.F",121)
       ...
       IF (meta_env%TAMCSteps<0) THEN
          CALL cp__b("metadynamics_utils.F",145,"TAMCSteps must be positive!")
       ENDIF
       CALL section_vals_val_get(metadyn_section,"Timestep",r_val=meta_env%zdt)
       IF (meta_env%zdt<=0.0_dp) THEN
          CALL cp__b("metadynamics_utils.F",149,"Timestep must be positive!")
       ENDIF
       ...
       IF (meta_env%hills_env%nt_hills<=0) THEN
          ...
          CALL cp_warn(cp__l("metadynamics_utils.F",156),&
       ...
       IF(.NOT.check)&
          CALL cp_abort(cp__l("metadynamics_utils.F",163),"MIN_NT_HILLS must have a value smaller or equal to NT_HILLS! "//&
       ...
       IF(.NOT.(meta_env%n_steps>=0))CALL cp__a("metadynamics_utils.F",179)
       ...
       IF (meta_env%well_tempered) THEN
         ...
         IF(.NOT.check)&
            CALL cp_abort(cp__l("metadynamics_utils.F",195),"When using Well-Tempered metadynamics, "//&
                          "DELTA_T (or WTGAMMA) should be explicitly specified.")
         IF(meta_env%extended_lagrange)&
            CALL cp__b("metadynamics_utils.F",198,"Well-Tempered metadynamics not possible with extended-lagrangian formulation.")
         IF(meta_env%hills_env%min_disp > 0.0_dp)&
            CALL cp__b("metadynamics_utils.F",200,"Well-Tempered metadynamics not possible with Adaptive hills.")
       ENDIF
       ...
       DO i= 1, n_colvar
          ...
          IF(.NOT.check)&
             CALL cp_abort(cp__l("metadynamics_utils.F",211),&
       ...
       IF (meta_env%do_multiple_walkers) THEN
          ...
          IF (explicit) THEN
             ...
             IF(.NOT.check)&
                CALL cp_abort(cp__l("metadynamics_utils.F",239),&
          ...
          IF(.NOT.check)&
             CALL cp_abort(cp__l("metadynamics_utils.F",255),&
                  "Number of Walkers specified in the input does not match with the "//&
...
! *****************************************************************************
  SUBROUTINE restart_hills(ss_history, delta_s_history, ww_history, ww,&
    ...
    IF (explicit) THEN
       ...
       ENDIF
       !
       IF(.NOT.(n_hills==ndum))CALL cp__a("metadynamics_utils.F",779)
       ...
       DO i = 1, n_hills
          ...
          IF(.NOT.(SIZE(rvals)==n_colvar))CALL cp__a("metadynamics_utils.F",790)
       ...
       IF (explicit) THEN
          ! delta_s_history
          CALL section_vals_val_get( hills_history,"_DEFAULT_KEYWORD_",n_rep_val=ndum)
          IF(.NOT.(n_hills==ndum))CALL cp__a("metadynamics_utils.F",799)
          DO i = 1, n_hills
             CALL section_vals_val_get(hills_history,"_DEFAULT_KEYWORD_",&
                  i_rep_val=i, r_vals=rvals)
             IF(.NOT.(SIZE(rvals)==n_colvar))CALL cp__a("metadynamics_utils.F",803)
             delta_s_history(1:n_colvar,i)=rvals
          END DO
       ELSE
          CALL cp_warn(cp__l("metadynamics_utils.F",807),&
       ...
       IF (explicit) THEN
          ...
          CALL section_vals_val_get( hills_history,"_DEFAULT_KEYWORD_",&
               n_rep_val=ndum)
          IF(.NOT.(n_hills==ndum))CALL cp__a("metadynamics_utils.F",823)
          DO i = 1, n_hills
             CALL section_vals_val_get(hills_history,"_DEFAULT_KEYWORD_",&
                  i_rep_val=i, r_val=rval)
             IF(.NOT.(SIZE(rvals)==n_colvar))CALL cp__a("metadynamics_utils.F",827)
             ww_history(i)=rval
          END DO
       ELSE
          CALL cp_warn(cp__l("metadynamics_utils.F",831),&
       ...
       IF (wtcontrol) THEN
          IF (explicit) THEN
             ...
             CALL section_vals_val_get( hills_history,"_DEFAULT_KEYWORD_",&
                  n_rep_val=ndum)
             IF(.NOT.(n_hills==ndum))CALL cp__a("metadynamics_utils.F",844)
             DO i = 1, n_hills
                CALL section_vals_val_get(hills_history,"_DEFAULT_KEYWORD_",&
                     i_rep_val=i, r_val=rval)
                IF(.NOT.(SIZE(rvals)==n_colvar))CALL cp__a("metadynamics_utils.F",848)
                invdt_history(i)=rval
             END DO
          ELSE
             CALL cp_warn(cp__l("metadynamics_utils.F",852),&
       ...
       ELSE
          IF (explicit) THEN
             CALL cp_abort(cp__l("metadynamics_utils.F",859),&
