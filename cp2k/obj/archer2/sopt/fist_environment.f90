# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment.F"
!-----------------------------------------------------------------------------!
...
# 74 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_environment.F" 2
...
! *****************************************************************************
  SUBROUTINE fist_init ( fist_env, root_section,para_env, force_env_section,&
    ...
    TYPE(section_vals_type), POINTER         :: cell_section, ewald_section, &
    ...
    CALL force_field_control(subsys%atomic_kinds%els, subsys%particles%els, &
                             subsys%molecule_kinds_new%els, subsys%molecules_new%els,&
    ...
    CALL fist_nonbond_env_release ( fist_nonbond_env)
...
!> \author  MK
!> \version 1.0
!> \note Changed by Matt (17/07/2017). Added a method for doing energy
!>         decomposition.
! *****************************************************************************
  SUBROUTINE fist_init_subsys ( fist_env, subsys, cell, cell_ref, fist_nonbond_env,&
    ...
    LOGICAL                                  :: do_multipoles, do_ener_decomp
    ...
    IF (.NOT.ASSOCIATED(energy_decomp_section)) THEN
       energy_decomp_section => section_vals_get_subs_vals(force_env_section,"ENERGY_DECOMP", can_return_null=.TRUE.)
    END IF

!CHANGE MATT
    CALL section_vals_val_get(force_env_section, "DO_DECOMP", &
                                         l_val=do_ener_decomp)
    PRINT *, "DECOMP = ", do_ener_decomp
    ! Create the fist_energy_type
    IF (do_ener_decomp) THEN
       CALL allocate_fist_energy( thermo, energy_decomp_section )
    ELSE
...
!END CHANGE
    ...
    CALL release_multipole_type(multipoles)
    CALL timestop(handle)
