# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_atomic_matrix_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_atomic_matrix_methods.F"
!-----------------------------------------------------------------------------!
...
# 39 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_atomic_matrix_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE fb_atmatrix_calc_size(dbcsr_mat, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_atomic_matrix_methods.F",99)
    check_ok = SIZE(blk_col_start) .GE. (natoms_in_halo + 1)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_atomic_matrix_methods.F",101)
    blk_row_start = 0
    blk_col_start = 0
    nrows = 0
    ncols = 0
    ...
    DO ipe = 1, numprocs
       ...
       DO ipair = 1, send_pair_count(ipe)
          ...
          CALL cp_dbcsr_get_block_p(matrix=dbcsr_mat, &
                                    ...
                                    found=found)
          IF (.NOT. found) THEN
             CALL cp__b("qs_fb_atomic_matrix_methods.F",256,"Matrix block not found")
          ELSE
    ...
    DO ipe = 1, numprocs
       ...
       DO ipair = 1, recv_pair_count(ipe)
          ...
          CALL fb_atomic_halo_atom_global2halo(atomic_halo, &
                                               iatom, iatom_in_halo, &
                                               found)
          IF(.NOT.(found))CALL cp__a("qs_fb_atomic_matrix_methods.F",300)
          CALL fb_atomic_halo_atom_global2halo(atomic_halo, &
                                               jatom, jatom_in_halo, &
                                               found)
          IF(.NOT.(found))CALL cp__a("qs_fb_atomic_matrix_methods.F",304)
...
! *****************************************************************************
  SUBROUTINE fb_atmatrix_construct_2(matrix_storage, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_atomic_matrix_methods.F",377)
    check_ok = fb_atomic_halo_has_data(atomic_halo)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_atomic_matrix_methods.F",379)
