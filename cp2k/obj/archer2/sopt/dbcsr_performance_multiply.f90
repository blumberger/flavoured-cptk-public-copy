# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_performance_multiply.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_performance_multiply.F"
!-----------------------------------------------------------------------------!
...
# 60 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_performance_multiply.F" 2
...
! parsing
    ...
    CALL dbcsr_assert( narg, "GE", 28 , dbcsr_fatal_level, dbcsr_wrong_args_error, &
     ...
     CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
          routineN, "allocattion problem",156)
...
! *****************************************************************************
  SUBROUTINE dbcsr_perf_multiply_low (mp_group, mp_env, npdims, io_unit, &
    ...
    INTEGER, DIMENSION(:), POINTER :: col_dist_a, col_dist_b, col_dist_c, &
      my_sizes_k, my_sizes_m, my_sizes_n, row_dist_a, row_dist_b, row_dist_c, &
...
!   ---------------------------------------------------------------------------
    ...
    IF( a_symm.NE.dbcsr_type_no_symmetry .AND. matrix_sizes(1).NE.matrix_sizes(3) ) CALL dbcsr_abort(routineP,285,"")

    IF( b_symm.NE.dbcsr_type_no_symmetry .AND. matrix_sizes(2).NE.matrix_sizes(3) ) CALL dbcsr_abort(routineP,287,"")

    IF( c_symm.NE.dbcsr_type_no_symmetry .AND. matrix_sizes(1).NE.matrix_sizes(2) ) CALL dbcsr_abort(routineP,289,"")
    ...
    IF( c_symm.NE.dbcsr_type_no_symmetry ) THEN
       IF(.NOT.(transa.EQ.dbcsr_no_transpose.AND.transb.EQ.dbcsr_transpose.OR.&
            ...
            .NOT.do_complex)) CALL dbcsr_abort(routineP,320,"")
    ...
    IF( c_symm.NE.dbcsr_type_no_symmetry ) THEN
       IF(limits(1).NE.1.OR.limits(2).NE.matrix_sizes(1).OR.&
            limits(3).NE.1.OR.limits(4).NE.matrix_sizes(2)) CALL dbcsr_abort(routineP,327,"")
    ...
    ELSE
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, " something wrong here... ",363)
    ENDIF
...
! *****************************************************************************
  SUBROUTINE perf_multiply (mp_group, mp_env, io_unit,&
    ...
    INTEGER                                  :: c_a, c_b, c_c, handle, irep, &
                                                istat, mynode, nthreads, &
                                                numnodes, r_a, r_b, r_c
    INTEGER(int_8)                           :: flop
    INTEGER(int_8), ALLOCATABLE, &
      DIMENSION(:)                           :: flop_sum
    INTEGER(int_8), ALLOCATABLE, &
      DIMENSION(:, :)                        :: flops
    INTEGER, DIMENSION(:), POINTER           :: blk_offsets, col_dist_a, &
...
!$  nthreads = OMP_GET_MAX_THREADS()
    ...
    CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "allocattion problem",524)
    times(:,:) = 0.0_real_8
    t_max(:) = 0.0_real_8
    t_min(:) = 0.0_real_8
    t(:) = 0.0_real_8
