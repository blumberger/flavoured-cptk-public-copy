# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_force.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_force.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmmx_force.F" 2
  ...
  SUBROUTINE qmmmx_calc_energy_force(qmmmx_env,calc_force,consistent_energies,linres,&
    ...
    IF (PRESENT(require_consistent_energy_force)) THEN
       IF(require_consistent_energy_force)&
          CALL cp__b("qmmmx_force.F",90,"qmmmx_energy_and_forces got require_consistent_energy_force but force mixing is active. ")
    ENDIF
    ...
    IF (mom_conserv_type /= do_fm_mom_conserv_none) THEN
       ...
       ELSE
          CALL cp__b("qmmmx_force.F",131,"Got unknown MOMENTUM_CONSERVATION_REGION (not CORE, QM, or BUFFER) !")
       ENDIF
...
! *****************************************************************************
 SUBROUTINE qmmmx_calc_energy_force_low(qmmm_env,calc_force,consistent_energies,linres,label)
    ...
    CHARACTER(default_string_length)         :: new_restart_fn, &
                                                new_restart_hist_fn, &
                                                old_restart_fn, &
                                                old_restart_hist_fn
