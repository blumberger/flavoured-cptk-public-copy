# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/rt_propagation.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/rt_propagation.F"
!-----------------------------------------------------------------------------!
...
# 75 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/rt_propagation.F" 2
    ...
    ELSE
       IF(qs_env%rtp%linear_scaling.AND.rtp_control%initial_wfn==use_scf_wfn) THEN
          ...
          IF(ASSOCIATED(mos)) THEN
             ...
             CALL rt_initialize_rho_from_mos(rtp,mos,matrix_s,rtp_control%orthonormal)
          ELSE
             !The wavefunction was minimized using a linear scaling method. Because the rho in the qs_env has the sparsity of S
             !the matrix P needs to be recalculated from H
             CALL rt_initialize_rho_from_ks(rtp,matrix_ks,matrix_s,nelectron_spin,rtp_control%orthonormal)
          ENDIF
       ENDIF
...
! *****************************************************************************
  ...
  SUBROUTINE run_propagation(qs_env,force_env,globenv)
    ...
    IF(.NOT.rtp%converged)&
       CALL cp_abort(cp__l("motion/rt_propagation.F",331),"propagation did not converge, "//&
    ...
    CASE(use_restart_wfn,use_rt_restart)
       ...
       IF(.NOT.rtp_control%linear_scaling.OR.rtp_control%initial_wfn==use_restart_wfn) THEN
          ...
          IF(dft_control%do_admm) THEN
             ...
             IF(.NOT.(ASSOCIATED(qs_env%mos_aux_fit)))CALL cp__a("motion/rt_propagation.F",428)
