# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_all.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_all.F"
!-----------------------------------------------------------------------------!
...
# 91 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields_all.F" 2
...
! *****************************************************************************
  SUBROUTINE force_field_pack_charges(charges, charges_section, particle_set, &
    ...
    IF(ASSOCIATED(inp_info%shell_list)) THEN
       CALL cp__b("force_fields_all.F",1938,"Array of charges not implemented for CORE-SHELL model!!")
    END IF

    ! Allocate array to particle_set size
    IF(.NOT.(.NOT.(ASSOCIATED(charges))))CALL cp__a("force_fields_all.F",1942)
    ...
    IF(.NOT.(nval==SIZE(charges)))CALL cp__a("force_fields_all.F",1947)
    ...
    DO iatom=1,nval
       ...
       IF(charge/=-HUGE(0.0_dp))&
          CALL cp_warn(cp__l("force_fields_all.F",1966),&
       ...
       IF (only_qm.AND.my_qmmm) THEN
          IF (qmmm_env%qmmm_coupl_type /= do_qmmm_none) THEN
             ...
             IF (is_link_atom) THEN
                ...
                END DO
                IF(.NOT.(ilink <= SIZE(qmmm_env%mm_link_atoms)))CALL cp__a("force_fields_all.F",2010)
...
! *****************************************************************************
  SUBROUTINE force_field_pack_charge(atomic_kind_set, qmmm_env, fatal, iw, iw4,&
    ...
    LOGICAL                                  :: found, found_p, is_link_atom, &
                                                is_shell, only_manybody, &
                                                only_qm
       ...
       IF(ASSOCIATED(inp_info%shell_list)) THEN
          DO j=1,SIZE(inp_info%shell_list)
             IF((inp_info%shell_list(j)%atm_name)==atmname) THEN
                ...
                IF (found) THEN
                   IF(found)&
                      CALL cp_warn(cp__l("force_fields_all.F",2104),&
       ...
       IF (only_qm.AND.my_qmmm) THEN
          IF (qmmm_env%qmmm_coupl_type /= do_qmmm_none) THEN
             ...
             IF (is_link_atom) THEN
                ...
                END DO
                IF(.NOT.(ilink <= SIZE(qmmm_env%mm_link_atoms)))CALL cp__a("force_fields_all.F",2161)
                scale_factor = qmmm_env%fist_scale_charge_link(ilink)
             END IF
           ...
           IF (my_atm_name1==atm_name1) THEN
              ...
              DO k=1,SIZE(atomic_kind_set)
                 ...
                 IF (my_atm_name2==atm_name2) THEN
                    ...
                    CASE DEFAULT
                        CALL cp__b("force_fields_all.F",2371,"Unknown damping type.")
                    END SELECT
              ...
              END DO
              IF(.NOT.found)&
                 CALL cp_warn(cp__l("force_fields_all.F",2380),&
...
! *****************************************************************************
  SUBROUTINE force_field_pack_shell(particle_set, atomic_kind_set,&
    ...
    LOGICAL :: core_coord_read, found_shell, is_a_shell, is_link_atom, &
      null_massfrac, only_qm, save_mem, shell_adiabatic, shell_coord_read
    ...
    IF (nshell_tot > 0) THEN
       IF (shell_adiabatic .AND. null_massfrac) THEN
          CALL cp__b("force_fields_all.F",2509,"Shell-model adiabatic: at least one shell_kind has mass zero")
       END IF
       ...
       DO i=1,SIZE(particle_set)
          ...
          END IF
       END DO
       IF(.NOT.(counter==nshell_tot))CALL cp__a("force_fields_all.F",2536)
    END IF
    ...
    END IF
    IF(.NOT.(first_shell-1==nshell_tot))CALL cp__a("force_fields_all.F",2608)
    IF(.NOT.(n==nshell_tot))CALL cp__a("force_fields_all.F",2609)
    CALL timestop(handle2)
             ...
             DO k=1,SIZE(inp_info%nonbonded14%pot)
                ...
                IF((((name_atm_a)==(inp_info%nonbonded14%pot(k)%pot%at1))  .AND. &
                   ...
                   IF (ff_type%multiple_potential) THEN
                      ...
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",2824),&
                   ...
                   ELSE
                      ...
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",2832),&
                   ...
                   DO k=1,SIZE(qmmm_env%inp_info%nonbonded14%pot)
                      ...
                      IF(( ((name_atm_a) ==(qmmm_env%inp_info%nonbonded14%pot(k)%pot%at1))  .AND. &
                         ...
                         IF (qmmm_env%multiple_potential) THEN
                            ...
                            IF(found)&
                               CALL cp_warn(cp__l("force_fields_all.F",2860),&
                                    "Multiple ONFO declaration: "//TRIM(name_atm_a)//&
                         ...
                         ELSE
                            ...
                            IF(found)&
                               CALL cp_warn(cp__l("force_fields_all.F",2868),&
                                    "Multiple ONFO declaration: "//TRIM(name_atm_a)//&
             ...
             DO k=1,SIZE(inp_info%nonbonded%pot)
                ...
                IF(( ((name_atm_a)==(inp_info%nonbonded%pot(k)%pot%at1))  .AND. &
                   ...
                   IF (ff_type%multiple_potential) THEN
                      ...
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",3086),&
                   ...
                   ELSE
                      ...
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",3094),&
                              "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                              " and "//TRIM(name_atm_b)//" OVERWRITING! ")
                   END IF
                   IF(iw>0) WRITE(iw,*) "    FOUND ",TRIM(name_atm_a)," ",TRIM(name_atm_b)
                ...
                DO k=1,SIZE(inp_info%nonbonded%pot)
                   ...
                   IF( (name_atm_a==inp_info%nonbonded%pot(k)%pot%at1) .OR.&
                      ...
                      IF (ff_type%multiple_potential) THEN
                         CALL pair_potential_single_add(inp_info%nonbonded%pot(k)%pot,pot)
                         IF(found)&
                            CALL cp_warn(cp__l("force_fields_all.F",3119),&
                                 "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                                 " and "//TRIM(name_atm_b)//" ADDING! ")
                         potparm_nonbond%pot(i,j)%pot => pot
                         potparm_nonbond%pot(j,i)%pot => pot
                      ELSE
                         CALL pair_potential_single_copy(inp_info%nonbonded%pot(k)%pot,pot)
                         IF(found)&
                            CALL cp_warn(cp__l("force_fields_all.F",3127),&
                                 "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                                 " and "//TRIM(name_atm_b)//" OVERWRITING! ")
                      END IF
             ...
             IF (.not.found) THEN
                DO k=1,SIZE(inp_info%nonbonded%pot)
                   ...
                   IF (ff_type%multiple_potential) THEN
                      CALL pair_potential_single_add(inp_info%nonbonded%pot(k)%pot,pot)
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",3149),&
                              "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                              " and "//TRIM(name_atm_b)//" ADDING! ")
                      potparm_nonbond%pot(i,j)%pot => pot
                      potparm_nonbond%pot(j,i)%pot => pot
                   ELSE
                      CALL pair_potential_single_copy(inp_info%nonbonded%pot(k)%pot,pot)
                      IF(found)&
                         CALL cp_warn(cp__l("force_fields_all.F",3157),&
                              "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                              " and "//TRIM(name_atm_b)//" OVERWRITING! ")
                   END IF
                   ...
                   DO k=1,SIZE(qmmm_env%inp_info%nonbonded%pot)
                      ...
                      IF(( ((name_atm_a) ==(qmmm_env%inp_info%nonbonded%pot(k)%pot%at1))  .AND. &
                         ...
                         IF (qmmm_env%multiple_potential) THEN
                            ...
                            IF(found)&
                               CALL cp_warn(cp__l("force_fields_all.F",3186),&
                                    "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                                    " and "//TRIM(name_atm_b)//" ADDING QM/MM forcefield specifications! ")
                            potparm_nonbond%pot(i,j)%pot => pot
                            potparm_nonbond%pot(j,i)%pot => pot
                         ELSE
                            ...
                            IF(found)&
                               CALL cp_warn(cp__l("force_fields_all.F",3194),&
                                    "Multiple NONBONDED declaration: "//TRIM(name_atm_a)//&
                                    " and "//TRIM(name_atm_b)//" OVERWRITING QM/MM forcefield specifications! ")
                         END IF
                         IF(iw>0) WRITE(iw,*) "    FOUND ",TRIM(name_atm_a)," ",TRIM(name_atm_b)
...
! *****************************************************************************
  SUBROUTINE issue_duplications(found, tag_label, name_atm_a, name_atm_b,&
    ...
    item = TRIM(item)//" )"
    IF(found)&
       CALL cp__w("force_fields_all.F",3412,"Multiple "//TRIM(tag_label)//" declarations: "//TRIM(item)//" overwriting! ")
 ...
 IF  (((atm1==my_atm1).AND.(atm2==my_atm2)).OR.&
      ((atm1==my_atm2).AND.(atm2==my_atm1))) found = .TRUE.
          CASE DEFAULT
             ...
             CALL cp__b("force_fields_all.F",3553,"")
          END SELECT
          IF (found) EXIT
       END DO
    ENDIF
