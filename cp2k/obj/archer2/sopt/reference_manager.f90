# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/reference_manager.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/reference_manager.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/reference_manager.F" 2
...
! *****************************************************************************
  SUBROUTINE cite_reference(key)
    ...
    IF (key<1 .OR. key>max_reference) CALL cp__b("common/reference_manager.F",88,"citation key out of range")
...
! *****************************************************************************
  SUBROUTINE add_reference(key,ISI_record,DOI)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'add_reference', &
    ...
    IF (nbib+1>max_reference) CALL cp__b("common/reference_manager.F",142,"increase max_reference")
    nbib=nbib+1
    key =nbib
...
! the most recent first, publications without a year get last
    ...
    DO I=1,nref
       ...
       IF (.NOT. cited_only .OR. thebib(irecord)%ref%is_cited) THEN
          ...
          CASE DEFAULT
            CALL cp__b("common/reference_manager.F",285,"print_reference: wrong format")
          END SELECT
          ...
          CASE(print_format_html)
            WRITE(unit,'(A)') '</TD></TR>'
          CASE DEFAULT
            CALL cp__b("common/reference_manager.F",297,"print_reference: wrong format")
          END SELECT
       ENDIF
    ENDDO
    IF (FORMAT.EQ.print_format_html) THEN
...
! *****************************************************************************
  SUBROUTINE print_reference(key,FORMAT,unit)
    ...
    IF (key<1 .OR. key>max_reference) CALL cp__b("common/reference_manager.F",324,"citation key out of range")
    ...
    CASE DEFAULT
      CALL cp__b("common/reference_manager.F",336,"print_reference: wrong format")
    END SELECT
