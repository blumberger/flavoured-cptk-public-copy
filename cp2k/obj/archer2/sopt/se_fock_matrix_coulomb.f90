# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_coulomb.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_coulomb.F"
!-----------------------------------------------------------------------------!
...
# 93 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/se_fock_matrix_coulomb.F" 2
...
! *****************************************************************************
  SUBROUTINE build_fock_matrix_coulomb (qs_env,ks_matrix,matrix_p,energy,calculate_forces,&
    ...
    IF(.NOT.(ASSOCIATED(matrix_p)))CALL cp__a("se_fock_matrix_coulomb.F",200)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix_coulomb.F",201)
    ...
    CALL cp_dbcsr_allocate_matrix_set(diagmat_ks,nspins)
    CALL cp_dbcsr_allocate_matrix_set(diagmat_p,nspins)

    DO ispin=1,nspins
       ! Allocate diagonal block matrices
       ALLOCATE(diagmat_p(ispin)%matrix,diagmat_ks(ispin)%matrix)!sm->dbcsr
       CALL cp_dbcsr_init(diagmat_p(ispin)%matrix)
       CALL cp_dbcsr_init(diagmat_ks(ispin)%matrix)
       CALL cp_dbcsr_get_block_diag(matrix_p(ispin)%matrix, diagmat_p(ispin)%matrix)
       CALL cp_dbcsr_get_block_diag(ks_matrix(ispin)%matrix, diagmat_ks(ispin)%matrix)
       CALL cp_dbcsr_set(diagmat_ks(ispin)%matrix, 0.0_dp)
       CALL cp_dbcsr_replicate_all(diagmat_p(ispin)%matrix)
       CALL cp_dbcsr_replicate_all(diagmat_ks(ispin)%matrix)
    END DO
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (inode==1) THEN
          ...
          IF(.NOT.(ASSOCIATED(pa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",254)
          check = (SIZE(pa_block_a,1)==natorb_a).AND.(SIZE(pa_block_a,2)==natorb_a)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",256)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=iatom,col=iatom,BLOCK=ksa_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",259)
          ...
          IF ( nspins == 2 ) THEN
             ...
             IF(.NOT.(ASSOCIATED(pa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",265)
             check = (SIZE(pa_block_b,1)==natorb_a).AND.(SIZE(pa_block_b,2)==natorb_a)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",267)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=iatom,col=iatom,BLOCK=ksa_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",270)
       ...
       IF ( dr1 > rij_threshold ) THEN
          ...
          IF(.NOT.(ASSOCIATED(pb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",288)
          check = (SIZE(pb_block_a,1)==natorb_b).AND.(SIZE(pb_block_a,2)==natorb_b)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",290)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=jatom,col=jatom,BLOCK=ksb_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",293)
          ...
          IF ( nspins == 2 ) THEN
             ...
             IF(.NOT.(ASSOCIATED(pb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",300)
             check = (SIZE(pb_block_b,1)==natorb_b).AND.(SIZE(pb_block_b,2)==natorb_b)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",302)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=jatom,col=jatom,BLOCK=ksb_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",305)
             check = (SIZE(pb_block_a,1)==SIZE(pb_block_b,1)).AND.(SIZE(pb_block_a,2)==SIZE(pb_block_b,2))
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",307)
          ...
          CASE (do_method_mndo, do_method_am1, do_method_pm3, do_method_pm6, do_method_pdg,&
             ...
             END IF
          CASE DEFAULT
             CALL cp__b("se_fock_matrix_coulomb.F",413,"")
          END SELECT
       ELSE
          IF ( se_int_control%do_ewald_gks ) THEN
             IF(.NOT.(iatom==jatom))CALL cp__a("se_fock_matrix_coulomb.F",417)
             ! Two-centers One-electron terms
             ecores=0._dp
             IF      ( nspins == 1 ) THEN
    ...
    DO ispin = 1, nspins
       CALL cp_dbcsr_sum_replicated( diagmat_ks(ispin)%matrix)
       CALL cp_dbcsr_distribute(diagmat_ks(ispin)%matrix)
       CALL cp_dbcsr_add(ks_matrix(ispin)%matrix, diagmat_ks(ispin)%matrix,&
            1.0_dp,1.0_dp)
    END DO
    CALL cp_dbcsr_deallocate_matrix_set ( diagmat_p)
    CALL cp_dbcsr_deallocate_matrix_set ( diagmat_ks)
...
! *****************************************************************************
  SUBROUTINE build_fock_matrix_coulomb_lr (qs_env, ks_matrix, matrix_p, energy,&
    ...
    INTEGER :: atom_a, ewald_type, forces_g_size, handle, iatom, ikind, &
      ...
      nlocal_particles, node, nparticle_local, nspins, size_1c_int
    INTEGER, DIMENSION(:), POINTER           :: atom_of_kind
    LOGICAL                                  :: anag, atener, defined, found, &
                                                use_virial
    ...
    CASE DEFAULT
       CALL cp__b("se_fock_matrix_coulomb.F",565,"Periodic SE implemented only for standard EWALD sums.")
    END SELECT
    ...
    nspins=dft_control%nspins
    IF(.NOT.(ASSOCIATED(matrix_p)))CALL cp__a("se_fock_matrix_coulomb.F",576)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix_coulomb.F",577)
    ...
    CASE(do_method_mndo, do_method_am1, do_method_pm3, do_method_pm6, do_method_pdg,&
       ...
       itype    = get_se_type(dft_control%qs_control%method_id)
    CASE DEFAULT
       CALL cp__b("se_fock_matrix_coulomb.F",601,"")
    END SELECT
    ...
    CASE(do_multipole_quadrupole)
       task    = .TRUE.
    CASE DEFAULT
       CALL cp__b("se_fock_matrix_coulomb.F",621,"")
    END SELECT
       ...
       DO ikind=1,nkind
          ...
          DO ilist=1, nparticle_local
             ...
             CALL cp_dbcsr_get_block_p(matrix=diagmat_p(ispin)%matrix,&
                  row=iatom,col=iatom,BLOCK=pa_block_a,found=found)
             IF(.NOT.(ASSOCIATED(pa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",647)
             ! Nuclei
       ...
       DO ilist=1, nparticle_local
          ...
          DO ispin = 1, nspins
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(ispin)%matrix,&
                  row=iatom,col=iatom,BLOCK=ksa_block_a,found=found)
             IF(.NOT.(ASSOCIATED(ksa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",768)
...
! *****************************************************************************
  SUBROUTINE build_fock_matrix_coul_lrc (qs_env,ks_matrix,matrix_p,energy,&
    ...
    nspins=dft_control%nspins
    IF(.NOT.(ASSOCIATED(matrix_p)))CALL cp__a("se_fock_matrix_coulomb.F",943)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix_coulomb.F",944)
    IF(.NOT.(ASSOCIATED(diagmat_p)))CALL cp__a("se_fock_matrix_coulomb.F",945)
    IF(.NOT.(ASSOCIATED(diagmat_ks)))CALL cp__a("se_fock_matrix_coulomb.F",946)
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (inode==1) THEN
          CALL cp_dbcsr_get_block_p(matrix=diagmat_p(1)%matrix,&
               row=iatom,col=iatom,BLOCK=pa_block_a,found=found)
          IF(.NOT.(ASSOCIATED(pa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1007)
          check = (SIZE(pa_block_a,1)==natorb_a).AND.(SIZE(pa_block_a,2)==natorb_a)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1009)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=iatom,col=iatom,BLOCK=ksa_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1012)
          p_block_tot_a(1:natorb_a,1:natorb_a) = 2.0_dp * pa_block_a
          pa_a(1:natorb_a2)                    = RESHAPE(pa_block_a,(/natorb_a2/))
          IF ( nspins == 2 ) THEN
             CALL cp_dbcsr_get_block_p(matrix=diagmat_p(2)%matrix,&
                  row=iatom,col=iatom,BLOCK=pa_block_b,found=found)
             IF(.NOT.(ASSOCIATED(pa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1018)
             check = (SIZE(pa_block_b,1)==natorb_a).AND.(SIZE(pa_block_b,2)==natorb_a)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1020)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=iatom,col=iatom,BLOCK=ksa_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1023)
             p_block_tot_a(1:natorb_a,1:natorb_a) = pa_block_a + pa_block_b
             pa_b(1:natorb_a2)                    = RESHAPE(pa_block_b,(/natorb_a2/))
          END IF
       ...
       IF ( dr1 > rij_threshold ) THEN
          ...
          CALL cp_dbcsr_get_block_p(matrix=diagmat_p(1)%matrix,&
               row=jatom,col=jatom,BLOCK=pb_block_a,found=found)
          IF(.NOT.(ASSOCIATED(pb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1051)
          check = (SIZE(pb_block_a,1)==natorb_b).AND.(SIZE(pb_block_a,2)==natorb_b)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1053)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=jatom,col=jatom,BLOCK=ksb_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1056)
          p_block_tot_b(1:natorb_b,1:natorb_b) = 2.0_dp * pb_block_a
          pb_a(1:natorb_b2)                    = RESHAPE(pb_block_a,(/natorb_b2/))
          ! Handle more than one configuration
          IF ( nspins == 2 ) THEN
             CALL cp_dbcsr_get_block_p(matrix=diagmat_p(2)%matrix,&
                  row=jatom,col=jatom,BLOCK=pb_block_b,found=found)
             IF(.NOT.(ASSOCIATED(pb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1063)
             check = (SIZE(pb_block_b,1)==natorb_b).AND.(SIZE(pb_block_b,2)==natorb_b)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1065)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=jatom,col=jatom,BLOCK=ksb_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1068)
             check = (SIZE(pb_block_a,1)==SIZE(pb_block_b,1)).AND.(SIZE(pb_block_a,2)==SIZE(pb_block_b,2))
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1070)
             p_block_tot_b(1:natorb_b,1:natorb_b) = pb_block_a + pb_block_b
             pb_b(1:natorb_b2)                    = RESHAPE(pb_block_b,(/natorb_b2/))
          END IF
          ...
          CASE (do_method_mndo, do_method_am1, do_method_pm3, do_method_pm6, do_method_pdg,&
             ...
             END IF
          CASE DEFAULT
             CALL cp__b("se_fock_matrix_coulomb.F",1185,"")
          END SELECT
       END IF
    END DO
    CALL neighbor_list_iterator_release(nl_iterator)
...
! *****************************************************************************
  SUBROUTINE build_fock_matrix_coul_lr_r3(qs_env,ks_matrix,matrix_p,energy,calculate_forces)
    ...
    INTEGER :: atom_a, atom_b, ewald_type, handle, iatom, ikind, inode, &
      ...
      natorb_b2, nkind, nlocal_particles, nspins
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: se_natorb
    INTEGER, DIMENSION(:), POINTER           :: atom_of_kind
    LOGICAL                                  :: anag, atener, check, defined, &
                                                found, switch
    ...
    CASE (do_ewald_ewald)
       ! Do Nothing
    CASE DEFAULT
       CALL cp__b("se_fock_matrix_coulomb.F",1322,"Periodic SE implemented only for standard EWALD sums.")
    END SELECT
    ...
    nspins=dft_control%nspins
    IF(.NOT.(ASSOCIATED(matrix_p)))CALL cp__a("se_fock_matrix_coulomb.F",1332)
    IF(.NOT.(SIZE(ks_matrix)>0))CALL cp__a("se_fock_matrix_coulomb.F",1333)
    ...
    DO WHILE (neighbor_list_iterate(nl_iterator)==0)
       ...
       IF (inode==1) THEN
          CALL cp_dbcsr_get_block_p(matrix=diagmat_p(1)%matrix,&
               row=iatom,col=iatom,BLOCK=pa_block_a,found=found)
          IF(.NOT.(ASSOCIATED(pa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1385)
          check = (SIZE(pa_block_a,1)==natorb_a).AND.(SIZE(pa_block_a,2)==natorb_a)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1387)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=iatom,col=iatom,BLOCK=ksa_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksa_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1390)
          p_block_tot_a(1:natorb_a,1:natorb_a) = 2.0_dp * pa_block_a
          pa_a(1:natorb_a2)                    = RESHAPE(pa_block_a,(/natorb_a2/))
          IF ( nspins == 2 ) THEN
             CALL cp_dbcsr_get_block_p(matrix=diagmat_p(2)%matrix,&
                  row=iatom,col=iatom,BLOCK=pa_block_b,found=found)
             IF(.NOT.(ASSOCIATED(pa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1396)
             check = (SIZE(pa_block_b,1)==natorb_a).AND.(SIZE(pa_block_b,2)==natorb_a)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1398)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=iatom,col=iatom,BLOCK=ksa_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksa_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1401)
             p_block_tot_a(1:natorb_a,1:natorb_a) = pa_block_a + pa_block_b
             pa_b(1:natorb_a2)                    = RESHAPE(pa_block_b,(/natorb_a2/))
          END IF
       END IF
       ...
       IF ( dr1 > rij_threshold ) THEN
          ...
          CALL cp_dbcsr_get_block_p(matrix=diagmat_p(1)%matrix,&
               row=jatom,col=jatom,BLOCK=pb_block_a,found=found)
          IF(.NOT.(ASSOCIATED(pb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1418)
          check = (SIZE(pb_block_a,1)==natorb_b).AND.(SIZE(pb_block_a,2)==natorb_b)
          IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1420)
          CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(1)%matrix,&
               row=jatom,col=jatom,BLOCK=ksb_block_a,found=found)
          IF(.NOT.(ASSOCIATED(ksb_block_a)))CALL cp__a("se_fock_matrix_coulomb.F",1423)
          p_block_tot_b(1:natorb_b,1:natorb_b) = 2.0_dp * pb_block_a
          pb_a(1:natorb_b2)                    = RESHAPE(pb_block_a,(/natorb_b2/))
          ! Handle more than one configuration
          IF ( nspins == 2 ) THEN
             CALL cp_dbcsr_get_block_p(matrix=diagmat_p(2)%matrix,&
                  row=jatom,col=jatom,BLOCK=pb_block_b,found=found)
             IF(.NOT.(ASSOCIATED(pb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1430)
             check = (SIZE(pb_block_b,1)==natorb_b).AND.(SIZE(pb_block_b,2)==natorb_b)
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1432)
             CALL cp_dbcsr_get_block_p(matrix=diagmat_ks(2)%matrix,&
                  row=jatom,col=jatom,BLOCK=ksb_block_b,found=found)
             IF(.NOT.(ASSOCIATED(ksb_block_b)))CALL cp__a("se_fock_matrix_coulomb.F",1435)
             check = (SIZE(pb_block_a,1)==SIZE(pb_block_b,1)).AND.(SIZE(pb_block_a,2)==SIZE(pb_block_b,2))
             IF(.NOT.(check))CALL cp__a("se_fock_matrix_coulomb.F",1437)
             p_block_tot_b(1:natorb_b,1:natorb_b) = pb_block_a + pb_block_b
             pb_b(1:natorb_b2)                    = RESHAPE(pb_block_b,(/natorb_b2/))
          END IF
          ...
          CASE (do_method_mndo, do_method_am1, do_method_pm3, do_method_pm6, do_method_pdg,&
             ...
             END IF
          CASE DEFAULT
             CALL cp__b("se_fock_matrix_coulomb.F",1538,"")
          END SELECT
       END IF
    END DO
    CALL neighbor_list_iterator_release(nl_iterator)
