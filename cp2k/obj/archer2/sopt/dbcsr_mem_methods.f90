# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_mem_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_mem_methods.F"
!-----------------------------------------------------------------------------!
...
# 29 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/data/dbcsr_mem_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_mempool_create(pool)
    ...
    IF(ASSOCIATED(pool)) CALL dbcsr_abort(routineP,56,"pool already allocated")
    ALLOCATE(pool)
...
! *****************************************************************************
  SUBROUTINE dbcsr_mempool_ensure_capacity(pool, capacity)
    ...
    CHARACTER(LEN=*), PARAMETER :: &
      routineN = 'dbcsr_mempool_ensure_capacity', &
      routineP = moduleN//':'//routineN

    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,77,"pool not allocated")
...
! *****************************************************************************
  FUNCTION dbcsr_mempool_get(memtype, datatype, datasize) RESULT(res)
    ...
    pool => memtype%pool
    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,105,"pool not allocated")
    ...
    IF(ASSOCIATED(best_cur)) THEN
       IF(best_cur%area%d%refcount /= 0) CALL dbcsr_abort(routineP,130,"refcount /= 0")
...
! *****************************************************************************
 SUBROUTINE dbcsr_mempool_add(area)
    ...
    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,158,"pool not allocated")
    IF(.NOT.dbcsr_data_exists(area)) CALL dbcsr_abort(routineP,159,"area not allocated")
    IF(area%d%refcount /= 0) CALL dbcsr_abort(routineP,160,"refcount /= 0")
...
! *****************************************************************************
 SUBROUTINE mempool_collect_garbage(pool)
    ...
    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,186,"pool not allocated")
...
! *****************************************************************************
 SUBROUTINE dbcsr_mempool_destruct(pool)
    ...
    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,220,"pool not allocated")
...
! *****************************************************************************
 SUBROUTINE dbcsr_mempool_clear(pool)
    ...
    IF(.NOT.ASSOCIATED(pool)) CALL dbcsr_abort(routineP,244,"pool not allocated")
...
! variable aim is initialized with default values from type definition
    ...
    CALL dbcsr_abort(routineP,300,"acc_stream missing")
