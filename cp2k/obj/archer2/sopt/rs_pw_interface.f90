# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/rs_pw_interface.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/rs_pw_interface.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/rs_pw_interface.F" 2
...
! *****************************************************************************
  SUBROUTINE density_rs2pw(pw_env,rs_rho,rho,rho_gspace)
    ...
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("rs_pw_interface.F",89)
   ...
   IF (gridlevel_info%ngrid_levels==1) THEN
    ...
    ELSE
       ...
       CASE(spline3_pbc_interp)
          ...
          CALL pw_transfer(rho%pw,rho_gspace%pw)
       CASE default
          CALL cp_abort(cp__l("rs_pw_interface.F",137),&
               "interpolator "//&
...
! *****************************************************************************
  SUBROUTINE density_rs2pw_basic(pw_env,rs_rho,rho,rho_gspace)
    ...
    CALL timeset(routineN,handle)
    NULLIFY(gridlevel_info, mgrid_gspace, mgrid_rspace, rs_descs, pw_pools)
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("rs_pw_interface.F",182)
    CALL pw_env_get(pw_env, rs_descs=rs_descs, pw_pools=pw_pools)
    ...
    ELSE
       ...
       CASE(spline3_pbc_interp)
          ...
          CALL pw_transfer(rho%pw,rho_gspace%pw)
       CASE default
          CALL cp_abort(cp__l("rs_pw_interface.F",228),&
               "interpolator "//&
               cp_to_string(interp_kind))
       END SELECT
    END IF
...
! *****************************************************************************
  SUBROUTINE potential_pw2rs(rs_v,v_rspace,pw_env)
    ...
    CASE(spline3_pbc_interp)
       ...
       END DO
    CASE default
       CALL cp_abort(cp__l("rs_pw_interface.F",323),&
