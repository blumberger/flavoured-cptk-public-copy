# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_worker.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_worker.F"
!-----------------------------------------------------------------------------!
...
# 96 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_worker.F" 2
...
! *****************************************************************************
  SUBROUTINE do_tmc_worker(tmc_env, ana_list)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_worker.F",137)
    ...
    IF(tmc_env%tmc_comp_set%group_nr.GT.0) THEN
      IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set%para_env_sub_group)))CALL cp__a("tmc/tmc_worker.F",144)
    ...
    ELSE
      ! analysis group
      IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set%para_env_m_ana)))CALL cp__a("tmc/tmc_worker.F",157)
      ...
      IF(master) THEN
        ...
        master_work_time: DO
          ...
          CASE(TMC_STAT_APPROX_ENERGY_REQUEST)
            IF(.NOT.(tmc_env%w_env%env_id_approx.GT.0))CALL cp__a("tmc/tmc_worker.F",243)
            itmp = bcast_group
          ...
          CASE(TMC_STAT_MD_REQUEST, TMC_STAT_NMC_REQUEST)
            ...
            ELSEIF(work_stat.EQ.TMC_STAT_MD_REQUEST) THEN
              ...
              CALL cp__b("tmc/tmc_worker.F",276,"there is no Hybrid MC implemented yet.")

            ELSE
              CALL cp__b("tmc/tmc_worker.F",279,"unknown task type for workers.")
            END IF
            ...
            CASE(TMC_STATUS_CALCULATING)
              ...
              CASE DEFAULT
                CALL cp_abort(cp__l("tmc/tmc_worker.F",296),&
            ...
            CASE(TMC_CANCELING_MESSAGE)
              work_stat = TMC_CANCELING_RECEIPT
            CASE DEFAULT
              CALL cp_abort(cp__l("tmc/tmc_worker.F",303),&
          ...
          CASE(TMC_STAT_ENERGY_REQUEST)
            IF(.NOT.(tmc_env%w_env%env_id_ener.GT.0))CALL cp__a("tmc/tmc_worker.F",314)
            !-- DISTRIBUTING WORK (group master) to all other group members
            itmp = bcast_group
            CALL tmc_message(msg_type=work_stat, send_recv=send_msg, &
                             dest=itmp, &
            ...
            CASE(TMC_STATUS_CALCULATING)
              ...
              CASE(TMC_STAT_ENERGY_REQUEST)
                ...
                IF(tmc_env%params%print_dipole) THEN
                  ...
                  IF(.NOT.flag)&
                     CALL cp_abort(cp__l("tmc/tmc_worker.F",347),&
                          "TMC: The requested dipoles are not porvided by the "//&
                          "force environment.")
                END IF
              CASE DEFAULT
                CALL cp_abort(cp__l("tmc/tmc_worker.F",352),&
            ...
            CASE(TMC_CANCELING_MESSAGE)
               work_stat = TMC_CANCELING_RECEIPT
            CASE DEFAULT
               CALL cp_abort(cp__l("tmc/tmc_worker.F",359),&
          ...
          CASE(TMC_STAT_INIT_ANALYSIS)
            IF(.NOT.(ASSOCIATED(ana_restart_conf)))CALL cp__a("tmc/tmc_worker.F",375)
            IF(.NOT.(SIZE(ana_restart_conf).EQ.tmc_env%params%nr_temp))CALL cp__a("tmc/tmc_worker.F",376)
            IF(.NOT.(PRESENT(ana_list)))CALL cp__a("tmc/tmc_worker.F",377)
            IF(.NOT.(ASSOCIATED(ana_list)))CALL cp__a("tmc/tmc_worker.F",378)
            itmp = MASTER_COMM_ID
            CALL communicate_atom_types(atoms=tmc_env%params%atoms, &
...
!              ana_list(itmp)%temp%io_unit     = tmc_env%w_env%io_unit
              ...
              IF((.NOT.ASSOCIATED(ana_list(itmp)%temp%last_elem)) .AND. &
                ...
                CALL cp_warn(cp__l("tmc/tmc_worker.F",405),&
              ...
              ELSE IF(ASSOCIATED(ana_list(itmp)%temp%last_elem))THEN
                IF(.NOT.(ana_list(itmp)%temp%last_elem%nr.EQ.ana_restart_conf(itmp))) THEN
                  ...
                  i2 = ana_restart_conf(itmp)
                  CALL cp_warn(cp__l("tmc/tmc_worker.F",414),&
          ...
          CASE(TMC_STAT_ANALYSIS_REQUEST)
            IF(.NOT.(PRESENT(ana_list)))CALL cp__a("tmc/tmc_worker.F",429)
            IF(.NOT.(ASSOCIATED(ana_list(conf%sub_tree_nr)%temp)))CALL cp__a("tmc/tmc_worker.F",430)
            ...
            CALL tmc_message(msg_type=work_stat, send_recv=send_msg,&
                             ...
                             tmc_params=tmc_env%params, elem=conf)
          CASE DEFAULT
            CALL cp_abort(cp__l("tmc/tmc_worker.F",440),&
      ...
      ELSE
        worker_work_time: DO
          ...
          CASE(TMC_STAT_START_CONF_RESULT)
            IF(.NOT.(tmc_env%w_env%env_id_approx.GT.0))CALL cp__a("tmc/tmc_worker.F",472)
            ...
            CASE(task_type_MC, task_type_ideal_gas)
              ...
              END IF
            CASE DEFAULT
              CALL cp_abort(cp__l("tmc/tmc_worker.F",484),&
          ...
          CASE(TMC_STAT_NMC_REQUEST, TMC_STAT_MD_REQUEST)
            ...
            ELSEIF(work_stat.EQ.TMC_STAT_MD_REQUEST) THEN
              !TODO Hybrid MC routine
              CALL cp__b("tmc/tmc_worker.F",498,"there is no Hybrid MC implemented yet.")

            ELSE
              CALL cp__b("tmc/tmc_worker.F",501,"unknown task type for workers.")
            END IF
          !-- energy calculations
          CASE(TMC_STAT_APPROX_ENERGY_REQUEST)
            !--- do calculate energy
            IF(.NOT.(tmc_env%w_env%env_id_approx.GT.0))CALL cp__a("tmc/tmc_worker.F",506)
            CALL calc_potential_energy(conf=conf, &
                                       env_id=tmc_env%w_env%env_id_approx, &
                                       exact_approx_pot=.FALSE., &
                                       tmc_env=tmc_env)
          CASE(TMC_STAT_ENERGY_REQUEST)
            !--- do calculate energy
            IF(.NOT.(tmc_env%w_env%env_id_ener.GT.0))CALL cp__a("tmc/tmc_worker.F",513)
            CALL calc_potential_energy(conf=conf, &
                                       env_id=tmc_env%w_env%env_id_ener, &
                                       exact_approx_pot=.TRUE., &
                                       tmc_env=tmc_env)
          CASE DEFAULT
            CALL cp_abort(cp__l("tmc/tmc_worker.F",519),&
...
! *****************************************************************************
  SUBROUTINE nested_markov_chain_MC(conf, env_id, tmc_env, calc_status)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_worker.F",617)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_worker.F",618)
    IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set)))CALL cp__a("tmc/tmc_worker.F",619)
    IF(.NOT.(ASSOCIATED(tmc_env%rng_stream)))CALL cp__a("tmc/tmc_worker.F",620)
    IF(.NOT.(ASSOCIATED(conf)))CALL cp__a("tmc/tmc_worker.F",621)
    IF(.NOT.(conf%temp_created.GT.0))CALL cp__a("tmc/tmc_worker.F",622)
    IF(.NOT.(conf%temp_created.LE.tmc_env%params%nr_temp))CALL cp__a("tmc/tmc_worker.F",623)
    IF(.NOT.(env_id.GT.0))CALL cp__a("tmc/tmc_worker.F",624)
    ...
    NMC_steps: DO substeps=1, INT(tmc_env%params%move_types%mv_size(mv_type_NMC_moves,1))
      ...
      CASE(TMC_CANCELING_MESSAGE)
        ...
        EXIT NMC_steps
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_worker.F",663),&
      ...
      IF(conf%move_type .EQ. mv_type_MD)THEN
        ...
        CALL cp_abort(cp__l("tmc/tmc_worker.F",691),&
...
! *****************************************************************************
  SUBROUTINE get_initial_conf(tmc_params, init_conf, env_id)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(init_conf)))CALL cp__a("tmc/tmc_worker.F",784)
    ...
    IF(.NOT.(ierr.EQ.0))CALL cp__a("tmc/tmc_worker.F",791)
    ndim  = 3*nr_atoms
...
! *****************************************************************************
  SUBROUTINE get_atom_kinds_and_cell(env_id, atoms, cell)
    ...
    IF(.NOT.(env_id.GT.0))CALL cp__a("tmc/tmc_worker.F",839)
    IF(.NOT.(.NOT.ASSOCIATED(atoms)))CALL cp__a("tmc/tmc_worker.F",840)
    IF(.NOT.(.NOT.ASSOCIATED(cell)))CALL cp__a("tmc/tmc_worker.F",841)
    ...
    IF(nunits_tot .GT. 0) THEN
       ...
       END DO
       IF(.NOT.(iparticle-1.EQ.nr_atoms))CALL cp__a("tmc/tmc_worker.F",858)
    ENDIF
...
! *****************************************************************************
  SUBROUTINE set_intermediate_info_comm(comm, env_id)
    ...
    IF(.NOT.(env_id.GT.0))CALL cp__a("tmc/tmc_worker.F",883)

    CALL f_env_get_from_id(env_id,f_env)

    IF(.NOT.(ASSOCIATED(f_env)))CALL cp__a("tmc/tmc_worker.F",887)
    IF(.NOT.(ASSOCIATED(f_env%force_env)))CALL cp__a("tmc/tmc_worker.F",888)
    IF(.NOT.ASSOCIATED(f_env%force_env%qs_env))&
       CALL cp_abort(cp__l("tmc/tmc_worker.F",890),&
...
! *****************************************************************************
  SUBROUTINE remove_intermediate_info_comm(env_id)
    ...
    IF(.NOT.(env_id.GT.0))CALL cp__a("tmc/tmc_worker.F",925)

    CALL f_env_get_from_id(env_id,f_env)

    IF(.NOT.(ASSOCIATED(f_env)))CALL cp__a("tmc/tmc_worker.F",929)
    IF(.NOT.(ASSOCIATED(f_env%force_env)))CALL cp__a("tmc/tmc_worker.F",930)
    IF(.NOT.ASSOCIATED(f_env%force_env%qs_env))&
       CALL cp_abort(cp__l("tmc/tmc_worker.F",932),&
