# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_binary_restarts.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_binary_restarts.F"
!-----------------------------------------------------------------------------!
...
# 43 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_binary_restarts.F" 2
...
! *****************************************************************************
  SUBROUTINE read_binary_coordinates(topology,root_section,para_env,&
    ...
    NULLIFY (logger)
    IF(.NOT.(ASSOCIATED(root_section)))CALL cp__a("input_cp2k_binary_restarts.F",93)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("input_cp2k_binary_restarts.F",94)
    IF(.NOT.(ASSOCIATED(subsys_section)))CALL cp__a("input_cp2k_binary_restarts.F",95)
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,153,&
                                     "id_name",int_size*natomkind)
    ...
    IF (nmolecule > 0) THEN
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,250,&
...
! *****************************************************************************
  SUBROUTINE read_binary_cs_coordinates(prefix,particle_set,root_section,&
    ...
    NULLIFY (logger)
    IF(.NOT.(ASSOCIATED(root_section)))CALL cp__a("input_cp2k_binary_restarts.F",361)
    IF(.NOT.(ASSOCIATED(subsys_section)))CALL cp__a("input_cp2k_binary_restarts.F",362)
    logger => cp_get_default_logger()
       ...
       IF (TRIM(section_label) == TRIM(section_name)) THEN
          IF (nbuf /= nparticle) THEN
             ...
             IF (nbuf > 0) THEN
                ...
                IF (istat /= 0) CALL stop_memory(routineN,moduleN,418,&
                                                 "rbuf",dp_size*3*nbuf)
                ...
                IF (istat /= 0) CALL stop_memory(routineN,moduleN,427,&
                                                 "ibuf",int_size*nbuf)
          ...
          END IF
       ELSE
          CALL cp_abort(cp__l("input_cp2k_binary_restarts.F",445),&
    ...
    IF(.NOT.(nparticle > 0))CALL cp__a("input_cp2k_binary_restarts.F",461)

    ALLOCATE (rbuf(3,nparticle),STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,464,&
    ...
    IF (istat /= 0) CALL stop_memory(routineN,moduleN,483,&
...
! *****************************************************************************
  SUBROUTINE read_binary_velocities(prefix,particle_set,root_section,para_env,&
    ...
    NULLIFY (logger)
    IF(.NOT.(ASSOCIATED(root_section)))CALL cp__a("input_cp2k_binary_restarts.F",570)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("input_cp2k_binary_restarts.F",571)
    IF(.NOT.(ASSOCIATED(subsys_section)))CALL cp__a("input_cp2k_binary_restarts.F",572)
    logger => cp_get_default_logger()
       ...
       DO
          ...
          IF (INDEX(section_label,"THERMOSTAT") > 0) THEN
             IF (nbuf > 0) THEN
                ...
                IF (istat /= 0) CALL stop_memory(routineN,moduleN,623,&
                                                 "rbuf",dp_size*nbuf)
                ! Perform dummy read
                DO i=1,4
          ...
          ELSE IF (INDEX(section_label,"VELOCIT") == 0) THEN
             CALL cp_abort(cp__l("input_cp2k_binary_restarts.F",642),&
                  "Section label <"//TRIM(section_label)//"> read from the "//&
                  "binary restart file <"//TRIM(binary_restart_file_name)//&
                  "> does not match the requested section name <"//&
                  TRIM(section_name)//">.")
    ...
    IF (have_velocities) THEN
       ...
       ALLOCATE (rbuf(3,nbuf),STAT=istat)
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,660,&
                                        "rbuf",dp_size*3*nbuf)
...
! *****************************************************************************
  SUBROUTINE read_binary_thermostats_nose(prefix,nhc,binary_restart_file_name,&
    ...
    IF(.NOT.(ASSOCIATED(nhc)))CALL cp__a("input_cp2k_binary_restarts.F",757)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("input_cp2k_binary_restarts.F",758)
    ...
    IF (para_env%ionode) THEN
       ...
       IF (istat /= 0) CALL stop_read("nhc_size (IOSTAT = "//&
                                      ...
                                      input_unit)
       IF (INDEX(section_label,"THERMOSTAT") == 0) THEN
          CALL cp_abort(cp__l("input_cp2k_binary_restarts.F",788),&
               "Section label <"//TRIM(section_label)//"> read from the "//&
               "binary restart file <"//TRIM(binary_restart_file_name)//&
               "> does not match the requested section name <"//&
               TRIM(section_name)//">.")
    ...
    IF (nhc_size > 0) THEN
       ...
       IF (istat /= 0) CALL stop_memory(routineN,moduleN,808,&
                                     "rbuf",dp_size*nhc_size)
       rbuf(:) = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE stop_read(object,unit_number)
    ...
    CALL cp__b("input_cp2k_binary_restarts.F",949,message)
