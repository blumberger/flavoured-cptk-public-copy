# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/f77_interface.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/f77_interface.F"
!-----------------------------------------------------------------------------!
...
MODULE f77_interface
  ...
  USE cp_log_handling,                 ONLY: &
       ...
       cp_get_default_logger, cp_logger_create, &
       cp_logger_get_default_unit_nr, cp_logger_release, cp_logger_retain, &
  ...
  USE input_section_types,             ONLY: &
       ...
       section_vals_duplicate, section_vals_get, section_vals_get_subs_vals, &
...
# 134 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/f77_interface.F" 2
...
!sample peak memory
    ...
    ELSE
       ...
       CALL cp_dbcsr_finalize_lib (group=default_para_env%group, output_unit=0)
       CALL pw_cuda_finalize()
       DO ienv=n_f_envs,1,-1
          CALL destroy_force_env(f_envs(ienv)%f_env%id_nr,ierr=ierr)
          IF(.NOT.(ierr==0))CALL cp__a("f77_interface.F",307)
       END DO
       DEALLOCATE(f_envs)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE f_env_dealloc(f_env)
    ...
    IF(.NOT.(ASSOCIATED(f_env)))CALL cp__a("f77_interface.F",338)
    ...
    IF (f_env%old_path/=f_env%my_path) THEN
       ...
       IF(.NOT.(ierr==0))CALL cp__a("f77_interface.F",345)
    END IF
...
! *****************************************************************************
  SUBROUTINE f_env_get_from_id(f_env_id,f_env)
    ...
    IF (f_env_pos<1) THEN
       CALL cp__b("f77_interface.F",404,"invalid env_id "//cp_to_string(f_env_id))
    ELSE
...
! *****************************************************************************
  SUBROUTINE f_env_add_defaults(f_env_id,f_env, handle)
    ...
    f_env_pos = get_pos_of_env(f_env_id)
    IF (f_env_pos<1) THEN
       CALL cp__b("f77_interface.F",440,"invalid env_id "//cp_to_string(f_env_id))
    ELSE
       f_env => f_envs(f_env_pos)%f_env
       logger => f_env%logger
       IF(.NOT.(ASSOCIATED(logger)))CALL cp__a("f77_interface.F",444)
       CALL m_getcwd(f_env%old_path)
       IF (f_env%old_path/=f_env%my_path) THEN
          CALL m_chdir(TRIM(f_env%my_path),ierr)
          IF(.NOT.(ierr==0))CALL cp__a("f77_interface.F",448)
       END IF
  ...
  SUBROUTINE f_env_rm_defaults(f_env,ierr,handle)
    ...
    IF (ASSOCIATED(f_env)) THEN
       IF (PRESENT(handle)) THEN
          IF(.NOT.(handle==cp_default_logger_stack_size()))CALL cp__a("f77_interface.F",488)
       END IF
       ...
       IF(.NOT.(ASSOCIATED(logger)))CALL cp__a("f77_interface.F",495)
       IF(.NOT.(ASSOCIATED(d_logger)))CALL cp__a("f77_interface.F",496)
       IF(.NOT.(ASSOCIATED(d_timer_env)))CALL cp__a("f77_interface.F",497)
       IF(.NOT.(ASSOCIATED(d_mp_perf_env)))CALL cp__a("f77_interface.F",498)
       IF(.NOT.(logger%id_nr==d_logger%id_nr))CALL cp__a("f77_interface.F",499)
       ! IF(.NOT.(d_timer_env%id_nr==f_env%timer_env%id_nr))CALL cp__a("f77_interface.F",500)
       IF(.NOT.(d_mp_perf_env%id_nr==f_env%mp_perf_env%id_nr))CALL cp__a("f77_interface.F",501)
       IF (f_env%old_path/=f_env%my_path) THEN
          CALL m_chdir(TRIM(f_env%old_path),ierr2)
          IF(.NOT.(ierr2==0))CALL cp__a("f77_interface.F",504)
       END IF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE create_force_env(new_env_id,input_declaration,input_path,&
    ...
    INTEGER :: handle, i, ierr2, iforce_eval, isubforce_eval, k, &
      method_name_id, my_group, nforce_eval, ngroups, nsubforce_size, unit_nr
    INTEGER, DIMENSION(:), POINTER           :: group_distribution, &
                                                i_force_eval, &
                                                lgroup_distribution
    LOGICAL :: check, do_qmmm_force_mixing, multiple_subsys, my_echo, &
      my_owns_out_unit, use_motion_section, use_multiple_para_env
    ...
    TYPE(section_vals_type), POINTER :: fe_section, force_env_section, &
      force_env_sections, fp_section, input_file, qmmm_section, &
      qmmmx_section, root_section, subsys_section, wrk_section
    TYPE(timer_env_type), POINTER            :: timer_env

    IF(.NOT.(ASSOCIATED(input_declaration)))CALL cp__a("f77_interface.F",594)
    NULLIFY(para_env, force_env,timer_env,mp_perf_env, globenv,meta_env,&
            fp_env,eip_env,mixed_env,qs_env,qmmm_env)
    new_env_id=-1
    ...
    print *, "TOTAL NUMBER OF FORCE EVALS", nforce_eval
    ...
    DO iforce_eval = 1, nforce_eval
       ...
       IF (use_multiple_para_env) THEN
          ...
          CALL section_vals_val_get(force_env_sections, "METHOD", i_val=method_name_id, &
               i_rep_section=i_force_eval(1))
          IF(method_name_id/=do_mixed)&
             CALL cp_abort(cp__l("f77_interface.F",723),&
          ...
          IF(.NOT.(check))CALL cp__a("f77_interface.F",728)
       ...
       CASE (do_qs)
          ...
          CALL qs_init(qs_env, my_para_env, globenv, root_section, force_env_section=force_env_section,&
       ...
       CASE (do_qmmm)
          ...
          CALL qmmm_env_create(qmmm_env, root_section, para_env, globenv,&
               force_env_section, qmmm_section, subsys_section, use_motion_section)
          CALL force_env_create(my_force_env,root_section, qmmm_env=qmmm_env, para_env=para_env,&
               globenv=globenv, force_env_section=force_env_section)
          CALL qmmm_env_release(qmmm_env)

       CASE (do_qmmmx)
          CALL qmmmx_env_create(qmmmx_env, root_section, para_env, globenv,&
               force_env_section, subsys_section, use_motion_section)
          CALL force_env_create(my_force_env,root_section, qmmmx_env=qmmmx_env, para_env=para_env,&
               globenv=globenv, force_env_section=force_env_section)
          CALL qmmmx_env_release(qmmmx_env)
       ...
       CASE (do_mixed)
          ...
          CALL force_env_create(my_force_env, root_section, mixed_env=mixed_env, para_env=my_para_env,&
               globenv=globenv, force_env_section=force_env_section)
          CALL mixed_env_release(mixed_env)
       ...
       CASE default
          ...
          CALL cp_abort(cp__l("f77_interface.F",815),&
       ...
       IF (use_motion_section) THEN
          ...
          CALL meta_env_release(meta_env)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE destroy_force_env(env_id,ierr)
    ...
    IF(.NOT.(ASSOCIATED(globenv)))CALL cp__a("f77_interface.F",916)
...
! *****************************************************************************
  SUBROUTINE get_cell(env_id, cell, per, ierr)
    ...
    IF(.NOT.(ASSOCIATED(cell_full)))CALL cp__a("f77_interface.F",1003)
    cell = cell_full%hmat
...
! *****************************************************************************
  SUBROUTINE set_cell(env_id, new_cell, ierr)
    ...
    IF(.NOT.(ASSOCIATED(cell)))CALL cp__a("f77_interface.F",1197)
    cell%hmat = new_cell
    CALL init_cell(cell)
    CALL force_env_get(f_env%force_env, subsys=subsys)
