# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_para_env.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_para_env.F"
!-----------------------------------------------------------------------------!
...
# 19 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_para_env.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_para_env_create(para_env, group, source,mepos, num_pe,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(para_env)))CALL cp__a("common/cp_para_env.F",53)
    ALLOCATE(para_env)
...
! *****************************************************************************
  SUBROUTINE cp_para_env_retain(para_env)
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("common/cp_para_env.F",84)
    IF(.NOT.(para_env%ref_count>0))CALL cp__a("common/cp_para_env.F",85)
...
! *****************************************************************************
  SUBROUTINE cp_para_env_release(para_env)
    ...
    IF (ASSOCIATED(para_env)) THEN
      IF(.NOT.(para_env%ref_count>0))CALL cp__a("common/cp_para_env.F",107)
      para_env%ref_count=para_env%ref_count-1
...
! *****************************************************************************
  SUBROUTINE cp_para_env_update(para_env)
    ...
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("common/cp_para_env.F",132)
    IF(.NOT.(para_env%ref_count>0))CALL cp__a("common/cp_para_env.F",133)
...
SUBROUTINE cp_cart_create(cart, group, ndims,owns_group)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(cart)))CALL cp__a("common/cp_para_env.F",156)
    ALLOCATE(cart)
...
SUBROUTINE cp_cart_update(cart)
  ...
  IF(.NOT.(ASSOCIATED(cart)))CALL cp__a("common/cp_para_env.F",185)
  IF(.NOT.(cart%ref_count>0))CALL cp__a("common/cp_para_env.F",186)
...
SUBROUTINE cp_cart_release(cart)
  ...
  IF (ASSOCIATED(cart)) THEN
     IF(.NOT.(cart%ref_count>0))CALL cp__a("common/cp_para_env.F",204)
