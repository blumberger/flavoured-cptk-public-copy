# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_sched.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_sched.F"
!-----------------------------------------------------------------------------!
...
# 59 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/mm/dbcsr_mm_sched.F" 2
...
!> \author Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_mm_sched_init(this, product_wm)
    TYPE(dbcsr_mm_sched_type), INTENT(INOUT) :: this
    ...
    IF (has_acc) &
       CALL dbcsr_mm_accdrv_init(this%accdrv, product_wm)
...
! *****************************************************************************
  SUBROUTINE dbcsr_mm_sched_process(this, left, right, stack_data,&
    ...
    IF(stack_fillcount <= 0)&
       CALL cp__b("dbcsr/mm/dbcsr_mm_sched.F",337,"dbcsr_mm_sched_process: got empty stack")
    ...
    CALL dbcsr_data_ensure_size(this%product_wm%data_area, stacked_datasize, &
     ...
     IF(.FALSE.) THEN
        DO sp = 1, stack_fillcount
           IF(stack_data(p_a_first,sp) > SIZE(left%data_area%d%r_dp)) &
              CALL dbcsr_abort(routineP,354,"left data out of range")
           IF(stack_data(p_b_first,sp) > SIZE(right%data_area%d%r_dp)) &
              CALL dbcsr_abort(routineP,356,"right data out of range")
           IF(stack_data(p_c_first,sp) > SIZE(this%product_wm%data_area%d%r_dp)) THEN
              ...
              CALL dbcsr_abort(routineP,360,"product data out of range")
           END IF
        END DO
     ENDIF
       ...
       IF(stack_descr%defined_mnk) THEN
         DO sp = 1, stack_fillcount
           IF(stack_data(p_m,sp) /= stack_descr%m)&
              CALL cp__b("dbcsr/mm/dbcsr_mm_sched.F",371,"homogenous stacks check failed")
           IF(stack_data(p_n,sp) /= stack_descr%n)&
              CALL cp__b("dbcsr/mm/dbcsr_mm_sched.F",373,"homogenous stacks check failed")
           IF(stack_data(p_k,sp) /= stack_descr%k)&
              CALL cp__b("dbcsr/mm/dbcsr_mm_sched.F",375,"homogenous stacks check failed")
         END DO
       ENDIF
     ENDIF
     ...
     IF(.NOT. success) CALL cp__b("dbcsr/mm/dbcsr_mm_sched.F",421,"dbcsr_mm_sched_process_stack failed")
...
! *****************************************************************************
 SUBROUTINE stats_add(stats, m,n,k, stacksize_cpu, stacksize_smm, stacksize_acc,&
    ...
    INTEGER                                  :: i, s
    INTEGER(kind=int_8) :: my_nstacks_acc, my_nstacks_cpu, my_nstacks_smm, &
      my_stacksize_acc, my_stacksize_cpu, my_stacksize_smm
...
! *****************************************************************************
  SUBROUTINE stats_print_report(report, output_unit)
    ...
    DO i=1, SIZE(sort_idx)
       ...
       WRITE (output_unit,"(A,I4,' x ',I4,' x ',I4,T30,I20,5X,F5.1,'%',4X,F5.1,'%',4X,F5.1,'%')")&
          " flops ",report%num_mnk_stacks(j,1:3),&
          2*total*PRODUCT(report%num_mnk_stacks(j,1:3)),&
