# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_utils.F"
!-----------------------------------------------------------------------------!
...
# 93 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE retain_history(mo_loc_history, mo_loc)
    ...
    DO i=1,SIZE(mo_loc_history)
       ...
       IF(.NOT.(ncol_hist==ncol_loc))CALL cp__a("qs_loc_utils.F",136)
...
! *****************************************************************************
  SUBROUTINE rotate_state_to_ref(mo_new, mo_ref, matrix_S)
    ...
    IF(.NOT.(ncol==ncol_ref))CALL cp__a("qs_loc_utils.F",168)
...
! *****************************************************************************
  SUBROUTINE rotate_zij_real(istate,jstate,st,ct,zij)
    ...
    natom = SIZE(zij, 1)
    DO iatom = 1, natom
# 372 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_loc_utils.F"
...
! *****************************************************************************
  SUBROUTINE qs_loc_env_init(qs_loc_env,localized_wfn_control,qs_env,myspin,do_localize,&
    ...
    INTEGER :: dim_op, handle, i, iatom, imo, imoloc, ispin, j, l_spin, lb, &
    ...
    IF (qs_loc_env%do_localize) THEN
       ...
       DO ispin = s_spin,l_spin
          ...
          CALL cp_fm_get_info(moloc_coeff(ispin)%matrix, nrow_global=naosub, &
               ncol_global=nmosub)
          IF(.NOT.(nao==naosub))CALL cp__a("qs_loc_utils.F",521)
          IF(localized_wfn_control%do_homo)THEN
             IF(.NOT.(nmo>=nmosub))CALL cp__a("qs_loc_utils.F",523)
          ELSE
             IF(.NOT.(nao-nmo>=nmosub))CALL cp__a("qs_loc_utils.F",525)
          END IF
       ...
       DO ispin =  s_spin,l_spin
          ...
          IF(localized_wfn_control%set_of_states == state_loc_list) THEN
             ...
             IF(.NOT.(nmosub>0))CALL cp__a("qs_loc_utils.F",549)
             imoloc = 0
             DO i = lb,ub
                ...
                IF(localized_wfn_control%do_homo)THEN
                   ...
                   IF(ABS(occ_imo-my_occ) > localized_wfn_control% eps_occ) THEN
                      IF(localized_wfn_control%localization_method /= do_loc_none) &
                           CALL cp_abort(cp__l("qs_loc_utils.F",560),&
          ...
          ELSE
             ...
             IF(ABS(occ_imo-my_occ) >  localized_wfn_control% eps_occ) THEN
                IF(localized_wfn_control%localization_method /= do_loc_none) &
                     CALL cp_abort(cp__l("qs_loc_utils.F",577),&
                          "States with different occupations "//&
                          "cannot be rotated together")
             END IF
             nmosub = localized_wfn_control%nloc_states(ispin)
       ...
       IF ( localized_wfn_control%operator_type==op_loc_berry )  THEN
          ...
          DO i=1, dim_op
             DO j=1, SIZE ( qs_loc_env % op_sm_set, 1 )
                ...
                CALL cp_dbcsr_init(qs_loc_env%op_sm_set(j,i)%matrix)
                ...
                CALL cp_dbcsr_set(qs_loc_env%op_sm_set(j,i)%matrix,0.0_dp)
             ENDDO
          END DO
       ...
       ELSEIF ( localized_wfn_control%operator_type== op_loc_pipek) THEN
          ...
          DO ispin = 1, SIZE(qs_loc_env % op_fm_set ,2)
             ...
             DO iatom = 1,natoms
                ...
                IF(.NOT.(nmo>=nmosub))CALL cp__a("qs_loc_utils.F",638)
                CALL cp_fm_set_all(qs_loc_env %op_fm_set(iatom,ispin)%matrix,0.0_dp)
             END DO  ! iatom
          END DO  ! ispin
       ELSE
          CALL cp__b("qs_loc_utils.F",643,"Type of operator not implemented")
       ENDIF
       CALL cp_fm_struct_release ( tmp_fm_struct)
...
! *****************************************************************************
  SUBROUTINE get_berry_operator ( qs_loc_env, qs_env)
    ...
    INTEGER :: dim_op, handle, i, iatom, icol, ikind, inode, irow, iset, &
      ...
      ncob, nkind, nrow, nseta, nsetb, sgfa, sgfb
...
! *****************************************************************************
  SUBROUTINE  loc_read_restart(qs_loc_env,mos,mos_localized,section, section2,para_env,&
    ...
    CHARACTER(LEN=default_path_length)       :: filename
    CHARACTER(LEN=default_string_length)     :: my_middle
    INTEGER :: group, handle, homo_read, i, ispin, lfomo_read, max_nloc, &
      n_rep_val, nao, nelectron_read, nloc, nmo, nmo_read, nspin, &
    ...
    ELSE
      ...
      IF(.NOT.PRESENT(evals))&
         CALL cp__b("qs_loc_utils.F",1074,"Missing argument to localize unoccupied states.")
    END IF
    ...
    DO ispin=1,nspin
       ...
       IF (para_env%ionode.AND.(nmo > 0)) THEN
         ...
         END IF
         IF(nmo_read < nmo)&
            CALL cp_warn(cp__l("qs_loc_utils.F",1153),&
                 "The number of MOs on the restart unit is smaller than the number of "//&
                 "the allocated MOs. ")
         IF(nmo_read>nmo)&
            CALL cp_warn(cp__l("qs_loc_utils.F",1157),&
...
!                  .OR. section_get_ival(dft_section,"PRINT%MO_CUBES%NHOMO")==0)&
            ...
            ELSE IF(localized_wfn_control%set_of_states == energy_loc_range.AND.my_do_homo) THEN
               ...
               IF(occupation(ilow)-occupation(iup) > localized_wfn_control%eps_occ) THEN
                  CALL cp_abort(cp__l("qs_loc_utils.F",1375),&
            ...
            ELSE
               ...
               IF(n_mo(ispin)>homo.AND.my_do_homo) THEN
                  ...
                  ! write warning
                      CALL cp_warn(cp__l("qs_loc_utils.F",1393),&
    ...
    ELSE
       ...
       CALL cp_warn(cp__l("qs_loc_utils.F",1414),&
...
! *****************************************************************************
  SUBROUTINE read_loc_section(localized_wfn_control,loc_section,&
    ...
    IF(PRESENT(do_xas)) THEN
       my_do_xas=do_xas
       IF(.NOT.(PRESENT(nloc_xas)))CALL cp__a("qs_loc_utils.F",1458)
    END IF
    IF(.NOT.(ASSOCIATED(loc_section)))CALL cp__a("qs_loc_utils.F",1460)
    NULLIFY(logger)
    logger => cp_get_default_logger()
...
! *****************************************************************************
  SUBROUTINE set_loc_wfn_lists(localized_wfn_control,nmoloc,nmo,nspins)
    ...
    CASE(state_loc_list)
       ! List
       IF(.NOT.(ASSOCIATED(localized_wfn_control%loc_states)))CALL cp__a("qs_loc_utils.F",1745)
       DO ispin = 1,nspins
          localized_wfn_control%lu_bound_states(1,ispin) = 1
    ...
    CASE(state_loc_range)
       ...
       DO ispin = 1,nspins
          ...
          END DO
          IF(.NOT.(max_iloc<=nmo(ispin)))CALL cp__a("qs_loc_utils.F",1767)
       END DO
    CASE(energy_loc_range)
       ! Energy
       ALLOCATE(localized_wfn_control%loc_states(max_nmoloc,2))
