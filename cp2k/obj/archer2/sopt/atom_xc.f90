# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_xc.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_xc.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_xc.F" 2
    ...
    IF ( ASSOCIATED(xc_section) ) THEN
      ...
      ELSE
        ...
        IF ( needs%laplace_rho ) THEN
          ...
          IF ( nlcc ) THEN
            CALL atom_core_density(lap(:,1),atom%potential%gth_pot,typ="LAP",rr=atom%basis%grid%rad)
          END IF
        END IF
    ...
    IF ( ASSOCIATED(xc_section) ) THEN
       ...
       ELSE
         ...
         IF ( needs%rho_spin ) THEN
           ...
           IF ( nlcc ) THEN
             xfun = 0.0_dp
             CALL atom_core_density(xfun(:),atom%potential%gth_pot,typ="RHO",rr=atom%basis%grid%rad)
         ...
         IF ( needs%norm_drho_spin ) THEN
           ...
           IF ( nlcc ) THEN
             xfun = 0.0_dp
             CALL atom_core_density(xfun(:),atom%potential%gth_pot,typ="DER",rr=atom%basis%grid%rad)
         ...
         IF ( needs%laplace_rho_spin ) THEN
           ...
           IF ( nlcc ) THEN
             xfun = 0.0_dp
             CALL atom_core_density(xfun(:),atom%potential%gth_pot,typ="LAP",rr=atom%basis%grid%rad)
...
! *****************************************************************************
  SUBROUTINE fill_rho_set(rho_set,nspins,needs,rho,drho,tau,lap,na)
    ...
    SELECT CASE(nspins)
    CASE(1)
       IF(.NOT.(.NOT.needs%rho_spin))CALL cp__a("atom_xc.F",637)
       IF(.NOT.(.NOT.needs%drho_spin))CALL cp__a("atom_xc.F",638)
       IF(.NOT.(.NOT.needs%norm_drho_spin))CALL cp__a("atom_xc.F",639)
       IF(.NOT.(.NOT.needs%drhoa_drhob))CALL cp__a("atom_xc.F",640)
       IF(.NOT.(.NOT.needs%rho_spin_1_3))CALL cp__a("atom_xc.F",641)
       IF(.NOT.(.NOT.needs%tau_spin))CALL cp__a("atom_xc.F",642)
       IF(.NOT.(.NOT.needs%drho))CALL cp__a("atom_xc.F",643)
       ! Give rho to 1/3
       ...
       END IF
    CASE(2)
       IF(.NOT.(.NOT.needs%drho))CALL cp__a("atom_xc.F",669)
       IF(.NOT.(.NOT.needs%drho_spin))CALL cp__a("atom_xc.F",670)
       IF(.NOT.(.NOT.needs%drhoa_drhob))CALL cp__a("atom_xc.F",671)
    ...
    END IF
    IF (needs%tau_spin) THEN
       IF(.NOT.(nspins==2))CALL cp__a("atom_xc.F",735)
       DO ia = 1,na
    ...
    END IF
    IF (needs%laplace_rho_spin) THEN
       IF(.NOT.(nspins==2))CALL cp__a("atom_xc.F",761)
       DO ia = 1,na
