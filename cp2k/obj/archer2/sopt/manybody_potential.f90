# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_potential.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_potential.F"
!-----------------------------------------------------------------------------!
...
# 50 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_potential.F" 2
  ...
  SUBROUTINE energy_manybody ( fist_nonbond_env, atomic_kind_set, local_particles,&
    ...
    DO ikind = 1, SIZE ( atomic_kind_set )
       ...
       DO i = 1, SIZE(pot%type)
         ...
         fembed(:) = 0._dp
         IF(.NOT.(ASSOCIATED(eam_data)))CALL cp__a("manybody_potential.F",130)
    ...
    IF (any_siepmann) THEN 
       ...
       IF(siepmann%allow_oh_formation) THEN
         CALL print_nr_oh_siepmann(nr_oh,mm_section,para_env)
       ENDIF
    END IF
...
! *****************************************************************************
  SUBROUTINE force_nonbond_manybody ( fist_nonbond_env, particle_set, cell,  &
    ...
    INTEGER :: atom_a, atom_b, handle, i, i_a, i_b, iend, ifirst, igrp, &
      ...
      kind_b, mpair, nkinds, nloc_size, npairs, nunique
       ...
       DO jkind = ikind, nkinds
          DO i = 1, SIZE(potparm%pot(ikind,jkind)%pot%type)
             IF (potparm%pot(ikind,jkind)%pot%type(i)==ea_type) THEN
                ...
                IF(.NOT.(eam_kinds_index(ikind,jkind)==-1))CALL cp__a("manybody_potential.F",411)
                IF(.NOT.(eam_kinds_index(jkind,ikind)==-1))CALL cp__a("manybody_potential.F",412)
    ...
    DO ilist=1,nonbonded%nlists
       ...
       Kind_Group_Loop1: DO igrp = 1, neighbor_kind_pair%ngrp_kind
          ...
          IF (i==-1) CYCLE
          ! EAM
          IF(.NOT.(ASSOCIATED(eam_data)))CALL cp__a("manybody_potential.F",445)
