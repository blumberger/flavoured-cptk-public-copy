# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew86.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew86.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_perdew86.F" 2
...
! *****************************************************************************
  SUBROUTINE p86_lda_eval ( rho_set, deriv_set, order, p86_params)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_perdew86.F",141)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_perdew86.F",142)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_perdew86.F",143)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_perdew86.F",144)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_perdew86.F",210,"derivatives bigger than 3 not implemented")
    END IF
    DEALLOCATE ( rs)
