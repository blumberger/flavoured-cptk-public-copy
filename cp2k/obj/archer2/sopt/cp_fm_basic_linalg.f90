# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_fm_scale_and_add(alpha,matrix_a,beta,matrix_b)
    ...
    IF(.NOT.(ASSOCIATED(matrix_a)))CALL cp__a("fm/cp_fm_basic_linalg.F",88)
    IF(.NOT.(matrix_a%ref_count>0))CALL cp__a("fm/cp_fm_basic_linalg.F",89)

    IF (PRESENT(beta)) THEN
       IF(.NOT.(PRESENT(matrix_b)))CALL cp__a("fm/cp_fm_basic_linalg.F",92)
       IF(.NOT.(ASSOCIATED(matrix_b)))CALL cp__a("fm/cp_fm_basic_linalg.F",93)
       IF(.NOT.(matrix_b%ref_count>0))CALL cp__a("fm/cp_fm_basic_linalg.F",94)
       IF (matrix_a%id_nr==matrix_b%id_nr) THEN
          IF(matrix_a%id_nr==matrix_b%id_nr)&
             CALL cp__w("fm/cp_fm_basic_linalg.F",97,"Bad use of routine. Call cp_fm_scale instead")
    ...
    IF (my_beta.NE.0.0_dp) THEN
       IF(matrix_a%matrix_struct%context%group/=matrix_b%matrix_struct%context%group)&
          CALL cp__b("fm/cp_fm_basic_linalg.F",122,"matrixes must be in the same blacs context")
       ...
       ELSE
          ...
          CALL cp__b("fm/cp_fm_basic_linalg.F",144,"")
...
# 211 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    lda=SIZE(a,1)
...
! *****************************************************************************
  SUBROUTINE cp_fm_gemm(transa,transb,m,n,k,alpha,matrix_a,matrix_b,beta,&
       matrix_c,b_first_col,a_first_row,b_first_row,c_first_col,c_first_row)
...
# 343 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    ...
    ELSE
       CALL cp__b("fm/cp_fm_basic_linalg.F",362,"Mixed precision gemm NYI")
    ENDIF
...
# 426 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
...
# 526 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
...
! *****************************************************************************
  SUBROUTINE cp_fm_trace(matrix_a,matrix_b,trace)
...
! *****************************************************************************
  SUBROUTINE cp_fm_scale(alpha, matrix_a)
    ...
    IF(.NOT.(ASSOCIATED(matrix_a)))CALL cp__a("fm/cp_fm_basic_linalg.F",791)
    IF(.NOT.(matrix_a%ref_count>0))CALL cp__a("fm/cp_fm_basic_linalg.F",792)
...
! *****************************************************************************
  SUBROUTINE cp_fm_transpose(matrix,matrixt)
    ...
    IF(.NOT.(ASSOCIATED(matrix)))CALL cp__a("fm/cp_fm_basic_linalg.F",827)
    IF(.NOT.(ASSOCIATED(matrixt)))CALL cp__a("fm/cp_fm_basic_linalg.F",828)
    nrow_global = matrix%matrix_struct%nrow_global
    ncol_global = matrix%matrix_struct%ncol_global
    IF(.NOT.(nrow_global==ncol_global))CALL cp__a("fm/cp_fm_basic_linalg.F",831)
...
# 884 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"

    IF(.NOT.(ASSOCIATED(matrix)))CALL cp__a("fm/cp_fm_basic_linalg.F",885)
    IF(.NOT.(ASSOCIATED(work)))CALL cp__a("fm/cp_fm_basic_linalg.F",886)
    nrow_global = matrix%matrix_struct%nrow_global
    ncol_global = matrix%matrix_struct%ncol_global
    IF(.NOT.(nrow_global==ncol_global))CALL cp__a("fm/cp_fm_basic_linalg.F",889)
    nrow_global = work%matrix_struct%nrow_global
    ncol_global = work%matrix_struct%ncol_global
    IF(.NOT.(nrow_global==ncol_global))CALL cp__a("fm/cp_fm_basic_linalg.F",892)
    IF(.NOT.(matrix%use_sp.EQV.work%use_sp))CALL cp__a("fm/cp_fm_basic_linalg.F",893)
...
# 957 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
...
# 1031 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    DO i=1,k
       IF(matrixa%use_sp) THEN
...
# 1072 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    LOGICAL                                  :: sign
    REAL(KIND=dp)                            :: eps1
    ...
    ALLOCATE(ipivot(n+matrix_a%matrix_struct%nrow_block))
    ipivot(:)=0
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    sign=.TRUE.
...
! *****************************************************************************
  SUBROUTINE cp_fm_qr_factorization(matrix_a, matrix_r, nrow_fact, ncol_fact, first_row, first_col)
    ...
    IF(.NOT.(nrow>=ncol))CALL cp__a("fm/cp_fm_basic_linalg.F",1251)
    ndim = SIZE(a,2)
!    ALLOCATE(ipiv(ndim))
    ALLOCATE(tau(ndim))

# 1269 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    lwork = -1
...
# 1332 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_fm_basic_linalg.F"
    lda=SIZE(a,1)
...
SUBROUTINE cp_complex_fm_gemm(transa,transb,m,n,k,alpha,A_re,A_im,B_re,B_im,beta,&
       C_re,C_im,b_first_col,a_first_row,b_first_row,c_first_col,c_first_row)
    CHARACTER(LEN=1), INTENT(IN)             :: transa, transb
    INTEGER, INTENT(IN)                      :: m, n, k
    REAL(KIND=dp), INTENT(IN)                :: alpha
    ...
    CALL cp_fm_gemm(transa,transb,m,n,k,alpha,A_re,B_re,beta,&
       C_re,b_first_col,a_first_row,b_first_row,c_first_col,c_first_row)
    CALL cp_fm_gemm(transa,transb,m,n,k,-alpha,A_im,B_im,1.0_dp,&
       C_re,b_first_col,a_first_row,b_first_row,c_first_col,c_first_row)
    CALL cp_fm_gemm(transa,transb,m,n,k,alpha,A_re,B_im,beta,&
       C_im,b_first_col,a_first_row,b_first_row,c_first_col,c_first_row)
