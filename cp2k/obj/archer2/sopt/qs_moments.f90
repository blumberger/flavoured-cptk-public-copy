# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_moments.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_moments.F"
!-----------------------------------------------------------------------------!
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_moments.F" 2
...
! *****************************************************************************
  SUBROUTINE build_local_moment_matrix(qs_env,moments,nmoments,ref_point,ref_points)
    ...
    INTEGER :: handle, i, iatom, icol, ikind, inode, irow, iset, jatom, &
      jkind, jset, last_jatom, maxco, maxsgf, natom, ncoa, ncob, nkind, nm, &
    ...
    IF(.NOT.(SIZE(moments)>=nm))CALL cp__a("qs_moments.F",159)
...
! *****************************************************************************
  SUBROUTINE build_local_magmom_matrix(qs_env,magmom,nmoments,ref_point,ref_points)
    ...
    nm = 3
    CALL cp_dbcsr_allocate_matrix_set ( magmom, nm)

    DO i=1,nm
       ALLOCATE(magmom(i)%matrix)
       CALL cp_dbcsr_init (magmom(i)%matrix)
       CALL cp_dbcsr_copy(magmom(i)%matrix, matrix_s(1)%matrix, "Moments")
       CALL cp_dbcsr_set(magmom(i)%matrix,0.0_dp)
    END DO
...
!> \param kvec ...
! *****************************************************************************
  SUBROUTINE build_berry_moment_matrix(qs_env,cosmat,sinmat,kvec)
    ...
    INTEGER :: handle, iatom, icol, ikind, inode, irow, iset, jatom, jkind, &
      jset, ldab, ldsa, ldsb, ldwork, natom, ncoa, ncob, nkind, nseta, nsetb, &
      sgfa, sgfb
    INTEGER, DIMENSION(:), POINTER           :: la_max, la_min, lb_max, &
    ...
    CALL cp_dbcsr_set(sinmat,0.0_dp)
    CALL cp_dbcsr_set(cosmat,0.0_dp)
...
! *****************************************************************************
  SUBROUTINE qs_moment_berry_phase (qs_env,magnetic,nmoments,reference,ref_point,unit_number)
    ...
    INTEGER                                  :: handle, i, ia, idim, ikind, &
                                                ispin, ix, iy, iz, j, k, l, &
                                                nao, nm, nmo, nmom, nmotot, &
                                                tmp_dim
    LOGICAL                                  :: ghost, uniform
    ...
    TYPE(rt_prop_type), POINTER              :: rtp
     ...
     IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_moments.F",825)
    ...
    ENDIF
     ...
     CALL cp_dbcsr_init(cosmat)
     CALL cp_dbcsr_init(sinmat)
     CALL cp_dbcsr_copy(cosmat,matrix_s(1)%matrix,'COS MOM')
     CALL cp_dbcsr_copy(sinmat,matrix_s(1)%matrix,'SIN MOM')
     CALL cp_dbcsr_set(cosmat,0.0_dp)
     CALL cp_dbcsr_set(sinmat,0.0_dp)
     ...
     DO ispin = 1, dft_control%nspins
       ...
       IF (.NOT.uniform) THEN
          CALL cp__b("qs_moments.F",901,"Berry phase moments for non uniform MOs' occupation numbers not implemented")
       END IF
     END DO
     ...
     DO l = 1, nmom
       SELECT CASE (l)
         ...
         CASE (2)
           ! Quadrupole
           CALL cp__b("qs_moments.F",960,"Berry phase moments bigger than 1 not implemented")
         ...
         CASE (3)
           ! Octapole
           CALL cp__b("qs_moments.F",1001,"Berry phase moments bigger than 2 not implemented")
         CASE (4)
           ! Hexadecapole
           CALL cp__b("qs_moments.F",1004,"Berry phase moments bigger than 3 not implemented")
         CASE DEFAULT
           CALL cp__b("qs_moments.F",1006,"Berry phase moments bigger than 4 not implemented")
       END SELECT
     END DO
     ...
     trace = 0.0_dp
     DO ispin=1,dft_control%nspins
       CALL cp_dbcsr_trace(rho_ao(ispin)%matrix,matrix_s(1)%matrix,trace)
     ...
     DO l = 1, nmom
       SELECT CASE (l)
         ...
         CASE (2)
           ! Quadrupole
           CALL cp__b("qs_moments.F",1060,"Berry phase moments bigger than 1 not implemented")
           DO i = 1, 3
             DO j = i, 3
                kvec(:) = twopi*(cell%h_inv(i,:)+cell%h_inv(j,:))
                CALL build_berry_moment_matrix(qs_env,cosmat,sinmat,kvec)
         ...
         CASE (3)
           ! Octapole
           CALL cp__b("qs_moments.F",1092,"Berry phase moments bigger than 2 not implemented")
         CASE (4)
           ! Hexadecapole
           CALL cp__b("qs_moments.F",1095,"Berry phase moments bigger than 3 not implemented")
         CASE DEFAULT
           CALL cp__b("qs_moments.F",1097,"Berry phase moments bigger than 4 not implemented")
       END SELECT
     END DO
     DO l = 1, nmom
       SELECT CASE (l)
         ...
         CASE (2)
           ! Quadrupole
           CALL cp__b("qs_moments.F",1112,"Berry phase moments bigger than 1 not implemented")
           DO i=1,3
             DO j=1,3
               zz = zij(i,j)/zi(i)/zi(j)
               cij(i,j) =AIMAG(LOG(zz))/twopi
         ...
         CASE (3)
           ! Octapole
           CALL cp__b("qs_moments.F",1135,"Berry phase moments bigger than 2 not implemented")
         CASE (4)
           ! Hexadecapole
           CALL cp__b("qs_moments.F",1138,"Berry phase moments bigger than 3 not implemented")
         CASE DEFAULT
           CALL cp__b("qs_moments.F",1140,"Berry phase moments bigger than 4 not implemented")
       END SELECT
     END DO
     ...
     CALL cp_dbcsr_deallocate_matrix ( cosmat)
     CALL cp_dbcsr_deallocate_matrix ( sinmat)
...
! *****************************************************************************
  SUBROUTINE qs_moment_locop (qs_env,magnetic,nmoments,reference,ref_point,unit_number)
    ...
    CHARACTER(LEN=8), ALLOCATABLE, &
      DIMENSION(:)                           :: rlab
    CHARACTER(LEN=default_string_length)     :: description
    INTEGER                                  :: akind, handle, i, ia, iatom, &
                                                ikind, ispin, ix, iy, iz, l, &
                                                nm, nmom
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_moments.F",1351)
    ...
    CALL get_qs_env(qs_env=qs_env,matrix_s=matrix_s)
    nm = (6 + 11*nmom + 6*nmom**2 + nmom**3)/6 - 1
    CALL cp_dbcsr_allocate_matrix_set(moments,nm)
    DO i=1,nm
       ALLOCATE(moments(i)%matrix)
       CALL cp_dbcsr_init(moments(i)%matrix)
       CALL cp_dbcsr_copy(moments(i)%matrix,matrix_s(1)%matrix,"Moments")
       CALL cp_dbcsr_set(moments(i)%matrix,0.0_dp)
    END DO
    ...
    DO i = 1, SIZE(moments)
       strace = 0._dp
       DO ispin=1,dft_control%nspins
         CALL cp_dbcsr_trace(rho_ao(ispin)%matrix,moments(i)%matrix,trace)
         strace = strace + trace
       END DO
       rmom(i+1,1) = strace
    END DO

    CALL cp_dbcsr_deallocate_matrix_set ( moments)
    ...
    IF (magnetic) THEN
       ...
       DO i = 1, SIZE(magmom)
          strace = 0._dp
          DO ispin=1,dft_control%nspins
            CALL cp_dbcsr_trace(rho_ao(ispin)%matrix,magmom(i)%matrix,trace)
            strace = strace + trace
          END DO
          mmom(i) = strace
       END DO
       CALL cp_dbcsr_deallocate_matrix_set(magmom)
    END IF
