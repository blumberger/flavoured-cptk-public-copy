# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment.F"
!-----------------------------------------------------------------------------!
...
MODULE qs_environment
  ...
  USE input_constants,                 ONLY: &
       ...
       general_roks, rel_none, rel_trans_atom, vdw_pairpot_dftd3, &
  ...
  USE kpoint_types,                    ONLY: kpoint_type,&
                                             read_kpoint_section,&
                                             write_kpoint_info
  USE lri_environment_init,            ONLY: lri_env_init
  ...
  USE qs_interactions,                 ONLY: init_interaction_radii,&
                                             ...
                                             write_ppl_radii,&
                                             write_ppnl_radii
  USE qs_kind_types,                   ONLY: check_qs_kind_set,&
                                             get_qs_kind_set,&
                                             init_gapw_basis_set,&
                                             init_qs_kind_set,&
                                             qs_kind_type,&
...
# 185 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_init(qs_env,para_env,globenv,root_section,cp_subsys,cell,cell_ref,qmmm,&
       qmmm_env_qm,force_env_section,subsys_section,use_motion_section)
    ...
    LOGICAL                                  :: do_et, do_exx, do_hfx, &
                                                do_kpoints, mp2_present, &
                                                my_qmmm, qmmm_decoupl, &
                                                use_ref_cell
    ...
    TYPE(section_vals_type), POINTER :: dft_section, et_coupling_section, &
      hfx_section, kpoint_section, mp2_section, transport_section

    NULLIFY(my_cell, my_cell_ref, atomic_kind_set, particle_set, &
         qs_kind_set, kpoint_section, dft_section, &
    ...
    CALL read_kpoint_section(kpoints,kpoint_section)
    ...
    IF (dft_control%qs_control%do_kg) THEN
      ...
      CALL kg_env_create(qs_env%kg_env, qs_env%input)
    END IF
    ...
    IF (method_id == do_method_lrigpw.OR.dft_control%qs_control%lri_optbas) THEN
       CALL lri_env_init(qs_env, force_env_section, qs_kind_set)
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_init_subsys(qs_env,para_env,subsys,cell,cell_ref,use_ref_cell,&
    ...
    INTEGER :: handle, ispin, iw, lmax_sphere, maxl, maxlgto, maxlgto_lri, &
      maxlppl, maxlppnl, method_id, multiplicity, my_ival, n_ao, &
    ...
    IF (my_ival==do_et_becke)THEN
       ...
       dft_control%qs_control%et_coupling_calc=.TRUE.
       et_becke_section =>  section_vals_get_subs_vals(et_coupling_section,"BECKE_RESTRAINT_A")
       CALL  read_becke_section(dft_control%qs_control,et_becke_section)
    END IF
    CALL read_mgrid_section(dft_control%qs_control,dft_section,para_env=para_env)
    ...
    CALL rel_c_release(rel_control)
    ...
    ELSE IF ( dft_control%qs_control%method == "SCPTB" ) THEN
       ...
       print_section => section_vals_get_subs_vals(subsys_section,"PRINT")
       CALL scptb_parameter_init(atomic_kind_set,qs_kind_set,scptb_control,print_section,para_env)
       ! check for Ewald
    ...
    IF ( (dft_control%qs_control%method == "GPW") .OR. &
       ...
       IF( all_potential_present ) THEN
          CALL cp__b("qs_environment.F",622,"all-el calculations with GPW, GAPW_XC, and OFGPW are not implemented ")
       END IF
    END IF
    ...
    IF(dft_control%qs_control%method == "LRIGPW".OR.dft_control%qs_control%lri_optbas) THEN
       CALL get_qs_kind_set(qs_kind_set,maxlgto=maxlgto_lri,basis_type="LRI")
    ...
    output_unit = cp_print_key_unit_nr(logger,root_section,"GLOBAL%PRINT/SPHERICAL_HARMONICS",&
           extension=".Log")
    CALL init_spherical_harmonics(maxl,output_unit)
    CALL cp_print_key_finished_output(output_unit,logger,root_section,&
            "GLOBAL%PRINT/SPHERICAL_HARMONICS")
    ...
    CALL dft_control_release(dft_control)
    ...
    IF ((dft_control%nspins < 1).OR.(dft_control%nspins > 2)) THEN
       CALL cp__b("qs_environment.F",959,"nspins should be 1 or 2 for the time being ...")
    END IF

    IF ((MODULO(nelectron,2) /= 0).AND.(dft_control%nspins == 1)) THEN
       IF ( .NOT. dft_control%qs_control%ofgpw .AND.  .NOT. dft_control%smear) THEN
          CALL cp__b("qs_environment.F",964,"Use the LSD option for an odd number of electrons")
       END IF
    END IF
    ...
    IF (dft_control%do_xas_calculation) THEN
       IF (dft_control%nspins == 1) THEN
          CALL cp__b("qs_environment.F",972,"Use the LSD option for XAS with transition potential")
       END IF
    END IF
    ...
    IF ( dft_control%qs_control%ofgpw ) THEN
       ...
       ELSE
          IF (MODULO(nelectron + multiplicity - 1,2) /= 0) THEN
             CALL cp__b("qs_environment.F",993,"LSD: try to use a different multiplicity")
          END IF
          nelectron_spin(1) = (nelectron + multiplicity - 1)/2
          nelectron_spin(2) = (nelectron - multiplicity + 1)/2
          IF (nelectron_spin(1) < 0) THEN
             CALL cp__b("qs_environment.F",998,"LSD: too few electrons for this multiplicity")
          END IF
    ...
    ELSE
       ...
       ELSE
          ...
          IF (MODULO(nelectron + multiplicity - 1,2) /= 0) THEN
             CALL cp__b("qs_environment.F",1023,"LSD: try to use a different multiplicity")
          END IF
          ...
          IF (nelectron_spin(2) < 0) THEN
             CALL cp__b("qs_environment.F",1030,"LSD: too few electrons for this multiplicity")
          END IF
    ...
    IF(scf_control%added_mos(1) > n_ao - n_mo(1))&
       CALL cp_warn(cp__l("qs_environment.F",1047),&
    ...
    IF (dft_control%nspins == 2) THEN
      ...
      IF(n_mo_add > n_ao - n_mo(2))&
         CALL cp__w("qs_environment.F",1060,"More added MOs requested for beta spin than available.")
      scf_control%added_mos(2) = MIN(n_mo_add,n_ao - n_mo(2))
      n_mo(2) = n_mo(2) + scf_control%added_mos(2)
      IF(n_mo(2) > n_mo(1))&
         CALL cp_warn(cp__l("qs_environment.F",1064),&
              "More beta than alpha MOs requested. "//&
              "The number of beta MOs will be reduced to the number alpha MOs.")
      n_mo(2) = MIN(n_mo(1),n_mo(2))
      IF(.NOT.(n_mo(1) >= nelectron_spin(1)))CALL cp__a("qs_environment.F",1068)
      IF(.NOT.(n_mo(2) >= nelectron_spin(2)))CALL cp__a("qs_environment.F",1069)
    END IF
    ...
    IF(do_kpoints .AND. dft_control%nspins == 2) THEN
       ...
       IF(n_mo(2) /= n_mo(1))&
         CALL cp_warn(cp__l("qs_environment.F",1077),&
              "Kpoints: Different number of MOs requested. "//&
              "The number of beta MOs will be set to the number alpha MOs.")
       n_mo(2) = n_mo(1)
       IF(.NOT.(n_mo(1) >= nelectron_spin(1)))CALL cp__a("qs_environment.F",1081)
       IF(.NOT.(n_mo(2) >= nelectron_spin(2)))CALL cp__a("qs_environment.F",1082)
    END IF

    ! Compatibility checks for smearing
    IF (scf_control%smear%do_smear) THEN
      IF (scf_control%added_mos(1) == 0) THEN
        CALL cp__b("qs_environment.F",1088,"Extra MOs (ADDED_MOS) are required for smearing")
      END IF
    END IF
    ...
    IF (dft_control%roks.AND.(.NOT.scf_control%use_ot)) THEN
       IF(scf_control%roks_scheme == general_roks)&
          CALL cp__w("qs_environment.F",1105,"General ROKS scheme is not yet tested!")

       IF (scf_control%smear%do_smear) THEN
         CALL cp_abort(cp__l("qs_environment.F",1108),&
...
!       CALL write_xas_control(dft_control%xas_control,dft_section)
    ...
    CALL qs_env_setup(qs_env,cp_logger_get_default_io_unit(logger))
