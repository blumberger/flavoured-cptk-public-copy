# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/atomic_kind_list_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/atomic_kind_list_types.F"
!-----------------------------------------------------------------------------!
...
# 39 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/atomic_kind_list_types.F" 2
  ...
  PUBLIC :: atomic_kind_list_type, atomic_kind_list_p_type,&
            atomic_kind_list_create, atomic_kind_list_retain,&
...
SUBROUTINE atomic_kind_list_create(list, els_ptr, &
  ...
  IF(.NOT.(PRESENT(els_ptr).OR.PRESENT(n_els)))CALL cp__a("subsys/atomic_kind_list_types.F",111)
  ...
  IF (.NOT.ASSOCIATED(list%els)) THEN
     ...
     IF(.NOT.(list%owns_els))CALL cp__a("subsys/atomic_kind_list_types.F",130)
  END IF
...
!> \author Fawzi Mohamed
! *****************************************************************************
SUBROUTINE atomic_kind_list_retain(list)
    TYPE(atomic_kind_list_type), POINTER     :: list

    CHARACTER(len=*), PARAMETER :: routineN = 'atomic_kind_list_retain', &
      routineP = moduleN//':'//routineN

  IF(.NOT.(ASSOCIATED(list)))CALL cp__a("subsys/atomic_kind_list_types.F",147)
  IF(.NOT.(list%ref_count>0))CALL cp__a("subsys/atomic_kind_list_types.F",148)
...
!> \author Fawzi Mohamed
! *****************************************************************************
SUBROUTINE atomic_kind_list_release(list)
    TYPE(atomic_kind_list_type), POINTER     :: list
  ...
  IF (ASSOCIATED(list)) THEN
     IF(.NOT.(list%ref_count>0))CALL cp__a("subsys/atomic_kind_list_types.F",166)
     list%ref_count=list%ref_count-1
