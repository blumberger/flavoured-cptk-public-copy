# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/dimer_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/dimer_types.F"
!-----------------------------------------------------------------------------!
...
# 24 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/dimer_types.F" 2
...
! *****************************************************************************
  SUBROUTINE dimer_env_create(dimer_env, natom, globenv, dimer_section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(dimer_env)))CALL cp__a("motion/dimer_types.F",117)
    ALLOCATE(dimer_env)
    ...
    IF (explicit) THEN
       ...
       DO i = 1, n_rep_val
          ...
          END DO
       END DO
       IF(.NOT.(isize==SIZE(dimer_env%nvec)))CALL cp__a("motion/dimer_types.F",145)
    ELSE
    ...
    IF(norm<=EPSILON(0.0_dp))&
       CALL cp__b("motion/dimer_types.F",169,"The norm of the dimer vector is 0! Calculation cannot proceed further.")
...
! *****************************************************************************
  SUBROUTINE dimer_env_retain(dimer_env)
    ...
    IF(.NOT.(ASSOCIATED(dimer_env)))CALL cp__a("motion/dimer_types.F",196)
    IF(.NOT.(dimer_env%ref_count>0))CALL cp__a("motion/dimer_types.F",197)
...
! *****************************************************************************
  SUBROUTINE dimer_env_release(dimer_env)
    ...
    IF (ASSOCIATED(dimer_env)) THEN
       IF(.NOT.(dimer_env%ref_count>0))CALL cp__a("motion/dimer_types.F",215)
       dimer_env%ref_count=dimer_env%ref_count-1
