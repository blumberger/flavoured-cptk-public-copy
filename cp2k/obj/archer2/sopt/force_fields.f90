# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields.F"
!-----------------------------------------------------------------------------!
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/force_fields.F" 2
...
! *****************************************************************************
  SUBROUTINE force_field_control(atomic_kind_set, particle_set, &
    ...
    INTEGER                                  :: exclude_ei, exclude_vdw, &
                                                handle, iw
    LOGICAL                                  :: found
    ...
    CASE (do_ff_charmm,do_ff_amber,do_ff_g96,do_ff_g87)
       ...
       IF (.NOT.found) THEN
          CALL cp__b("force_fields.F",153,"Force field file missing")
       END IF
    CASE (do_ff_undef)
       ! Do Nothing
    CASE DEFAULT
       CALL cp__b("force_fields.F",158,"Force field type not implemented")
    END SELECT
    ...
    CASE (do_ff_undef)
       ! Do Nothing
    CASE DEFAULT
       CALL cp__b("force_fields.F",174,"Force field type not implemented")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE print_pot_parameter_file(ff_type, mm_section)
    ...
    REAL(KIND=dp)                            :: eps, k, phi0, r0, sigma, &
                                                theta0
    TYPE(cp_logger_type), POINTER            :: logger
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,mm_section,"PRINT%FF_PARAMETER_FILE")&
       ...
       SELECT CASE(ff_type%ff_type)
       CASE (do_ff_charmm)
          CALL cp__w("force_fields.F",243,"Dumping FF parameter file for CHARMM FF  not implemented!")
       CASE (do_ff_amber)
          IF (iw>0) THEN
             ! Bonds
             WRITE(iw,1001)
          ...
          END IF
       CASE (do_ff_g87,do_ff_g96)
          CALL cp__w("force_fields.F",287,"Dumping FF parameter file for GROMOS FF not implemented!")
       CASE (do_ff_undef)
          CALL cp__w("force_fields.F",289,"Dumping FF parameter file for INPUT FF  not implemented!")
       END SELECT
       IF (iw>0) THEN
          WRITE(iw,'(/,A)')"END"
       END IF
