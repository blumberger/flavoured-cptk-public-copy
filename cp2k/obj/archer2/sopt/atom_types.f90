# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_types.F"
!-----------------------------------------------------------------------------!
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_types.F" 2
...
! *****************************************************************************
  SUBROUTINE init_atom_basis(basis,basis_section,zval,btyp)
    ...
    INTEGER                                  :: basistype, i, j, k, l, ll, m, &
...
!   btyp = AP : high accuracy pseudopotential basis
    ...
    IF(ngp <= 0)&
       CALL cp__b("atom_types.F",346,"# point radial grid < 0")
    ...
    SELECT CASE (basistype)
      ...
      CASE (gaussian)
         ...
         ELSE
           ...
           IF ( basis%nbas(0) > 0 ) THEN
             ...
             IF(.NOT.(SIZE(expo)>=basis%nbas(0)))CALL cp__a("atom_types.F",391)
             DO i=1,basis%nbas(0)
               basis%am(i,0) = expo(i)
             END DO
           END IF
           IF ( basis%nbas(1) > 0 ) THEN
             ...
             IF(.NOT.(SIZE(expo)>=basis%nbas(1)))CALL cp__a("atom_types.F",399)
             DO i=1,basis%nbas(1)
               basis%am(i,1) = expo(i)
             END DO
           END IF
           IF ( basis%nbas(2) > 0 ) THEN
             ...
             IF(.NOT.(SIZE(expo)>=basis%nbas(2)))CALL cp__a("atom_types.F",407)
             DO i=1,basis%nbas(2)
               basis%am(i,2) = expo(i)
             END DO
           END IF
           IF ( basis%nbas(3) > 0 ) THEN
             ...
             IF(.NOT.(SIZE(expo)>=basis%nbas(3)))CALL cp__a("atom_types.F",415)
             DO i=1,basis%nbas(3)
               basis%am(i,3) = expo(i)
             END DO
           END IF
      ...
      CASE (geometrical_gto)
         ...
         ELSE
           ...
           DO i=1,SIZE(sindex)
             starti(i-1) = sindex(i)
             IF(.NOT.(sindex(i)>=0))CALL cp__a("atom_types.F",485)
           END DO
      ...
      CASE (slater)
         ...
         ELSE
           ...
           IF ( basis%nbas(0) > 0 ) THEN
             NULLIFY(expo)
             CALL section_vals_val_get(basis_section,"S_EXPONENTS",r_vals=expo)
             IF(.NOT.(SIZE(expo)>=basis%nbas(0)))CALL cp__a("atom_types.F",579)
             DO i=1,basis%nbas(0)
               basis%as(i,0) = expo(i)
             END DO
             NULLIFY(nqm)
             CALL section_vals_val_get(basis_section,"S_QUANTUM_NUMBERS",i_vals=nqm)
             IF(.NOT.(SIZE(nqm)>=basis%nbas(0)))CALL cp__a("atom_types.F",585)
             DO i=1,basis%nbas(0)
               basis%ns(i,0) = nqm(i)
             END DO
           END IF
           IF ( basis%nbas(1) > 0 ) THEN
             NULLIFY(expo)
             CALL section_vals_val_get(basis_section,"P_EXPONENTS",r_vals=expo)
             IF(.NOT.(SIZE(expo)>=basis%nbas(1)))CALL cp__a("atom_types.F",593)
             DO i=1,basis%nbas(1)
               basis%as(i,1) = expo(i)
             END DO
             NULLIFY(nqm)
             CALL section_vals_val_get(basis_section,"P_QUANTUM_NUMBERS",i_vals=nqm)
             IF(.NOT.(SIZE(nqm)>=basis%nbas(1)))CALL cp__a("atom_types.F",599)
             DO i=1,basis%nbas(1)
               basis%ns(i,1) = nqm(i)
             END DO
           END IF
           IF ( basis%nbas(2) > 0 ) THEN
             NULLIFY(expo)
             CALL section_vals_val_get(basis_section,"D_EXPONENTS",r_vals=expo)
             IF(.NOT.(SIZE(expo)>=basis%nbas(2)))CALL cp__a("atom_types.F",607)
             DO i=1,basis%nbas(2)
               basis%as(i,2) = expo(i)
             END DO
             NULLIFY(nqm)
             CALL section_vals_val_get(basis_section,"D_QUANTUM_NUMBERS",i_vals=nqm)
             IF(.NOT.(SIZE(nqm)>=basis%nbas(2)))CALL cp__a("atom_types.F",613)
             DO i=1,basis%nbas(2)
               basis%ns(i,2) = nqm(i)
             END DO
           END IF
           IF ( basis%nbas(3) > 0 ) THEN
             NULLIFY(expo)
             CALL section_vals_val_get(basis_section,"F_EXPONENTS",r_vals=expo)
             IF(.NOT.(SIZE(expo)>=basis%nbas(3)))CALL cp__a("atom_types.F",621)
             DO i=1,basis%nbas(3)
               basis%as(i,3) = expo(i)
             END DO
             NULLIFY(nqm)
             CALL section_vals_val_get(basis_section,"F_QUANTUM_NUMBERS",i_vals=nqm)
             IF(.NOT.(SIZE(nqm)>=basis%nbas(3)))CALL cp__a("atom_types.F",627)
             DO i=1,basis%nbas(3)
               basis%ns(i,3) = nqm(i)
             END DO
           END IF
...
! *****************************************************************************
  SUBROUTINE create_atom_type(atom)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(atom)))CALL cp__a("atom_types.F",711)
...
! *****************************************************************************
  SUBROUTINE release_atom_type(atom)
    ...
    IF(.NOT.(ASSOCIATED(atom)))CALL cp__a("atom_types.F",735)
...
! *****************************************************************************
  SUBROUTINE set_atom(atom,basis,state,integrals,orbitals,potential,zcore,pp_calc,do_zmp,doread,&
    ...
    LOGICAL, INTENT(IN), OPTIONAL            :: pp_calc, do_zmp, doread, &
                                                read_vxc
    ...
    IF(.NOT.(ASSOCIATED(atom)))CALL cp__a("atom_types.F",792)
...
! *****************************************************************************
  SUBROUTINE create_atom_orbs(orbs,mbas,mo)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(orbs)))CALL cp__a("atom_types.F",831)
...
! *****************************************************************************
  SUBROUTINE release_atom_orbs(orbs)
    ...
    IF(.NOT.(ASSOCIATED(orbs)))CALL cp__a("atom_types.F",879)
...
! *****************************************************************************
  SUBROUTINE create_opmat(opmat,n)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(opmat)))CALL cp__a("atom_types.F",944)
...
! *****************************************************************************
  SUBROUTINE release_opmat(opmat)
    ...
    IF(.NOT.(ASSOCIATED(opmat)))CALL cp__a("atom_types.F",964)
...
! *****************************************************************************
  SUBROUTINE create_opgrid(opgrid,grid)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(opgrid)))CALL cp__a("atom_types.F",987)
...
! *****************************************************************************
  SUBROUTINE release_opgrid(opgrid)
    ...
    IF(.NOT.(ASSOCIATED(opgrid)))CALL cp__a("atom_types.F",1010)
...
! *****************************************************************************
  SUBROUTINE read_basis_set(element_symbol,basis,basis_set_name,basis_set_file,&
    ...
    INTEGER                                  :: i, ii, ipgf, irep, iset, &
                                                ishell, j, k, lshell, nj, &
                                                nmin, ns, nset, strlen1, &
                                                strlen2
    ...
    IF (read_from_input) THEN
      ...
      IF(.NOT.(is_ok))CALL cp__a("atom_types.F",1843)
      CALL val_get(val,c_val=line_att)
      READ(line_att,*) nset
      IF(.NOT.(nset <= maxset))CALL cp__a("atom_types.F",1846)
      DO iset=1,nset
        is_ok=cp_sll_val_next(list,val)
        IF(.NOT.(is_ok))CALL cp__a("atom_types.F",1849)
        CALL val_get(val,c_val=line_att)
        ...
        CALL remove_word(line_att)
        IF(.NOT.(npgf(iset) <= maxpri))CALL cp__a("atom_types.F",1859)
        nshell(iset) = 0
        DO lshell=lmin(iset),lmax(iset)
          ...
          END DO
        END DO
        IF(.NOT.(LEN_TRIM(line_att)==0))CALL cp__a("atom_types.F",1870)
        DO ipgf=1,npgf(iset)
          is_ok=cp_sll_val_next(list,val)
          IF(.NOT.(is_ok))CALL cp__a("atom_types.F",1873)
          CALL val_get(val,c_val=line_att)
      ...
      search_loop: DO
        ...
        IF (found) THEN
          ...
          IF (match) THEN
            ...
            IF(.NOT.(nset <= maxset))CALL cp__a("atom_types.F",1903)
            DO iset=1,nset
              ...
              IF(.NOT.(npgf(iset) <= maxpri))CALL cp__a("atom_types.F",1909)
              nshell(iset) = 0
              DO lshell=lmin(iset),lmax(iset)
                nmin = n(iset) + lshell - lmin(iset)
...
! *****************************************************************************
  SUBROUTINE read_gth_potential(element_symbol,potential,pseudo_name,pseudo_file,&
    ...
    IF (read_from_input) THEN
      ...
      l = 0
      is_ok=cp_sll_val_next(list,val)
      IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2195)
      CALL val_get(val,c_val=line_att)
      ...
      is_ok=cp_sll_val_next(list,val)
      IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2208)
      CALL val_get(val,c_val=line_att)
      ...
      DO
        is_ok=cp_sll_val_next(list,val)
        IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2222)
        CALL val_get(val,c_val=line_att)
        IF(INDEX(line_att,"LPOT") /= 0) THEN
          ...
          DO ipot=1,potential%nexp_lpot
             is_ok=cp_sll_val_next(list,val)
             IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2230)
             CALL val_get(val,c_val=line_att)
        ...
        ELSEIF(INDEX(line_att,"NLCC") /= 0) THEN
          ...
          DO ipot=1,potential%nexp_nlcc
             is_ok=cp_sll_val_next(list,val)
             IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2247)
             CALL val_get(val,c_val=line_att)
        ...
        ELSEIF(INDEX(line_att,"LSD") /= 0) THEN
          ...
          DO ipot=1,potential%nexp_lsd
             is_ok=cp_sll_val_next(list,val)
             IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2266)
             CALL val_get(val,c_val=line_att)
      ...
      IF (nlmax > 0) THEN
        ...
        DO l=0,nlmax-1
          is_ok=cp_sll_val_next(list,val)
          IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2288)
          CALL val_get(val,c_val=line_att)
          ...
          DO i=1,potential%nl(l)
            IF (i == 1) THEN
              ...
              CALL remove_word(line_att)
            ELSE
              IF(.NOT.(LEN_TRIM(line_att)==0))CALL cp__a("atom_types.F",2299)
              is_ok=cp_sll_val_next(list,val)
              IF(.NOT.(is_ok))CALL cp__a("atom_types.F",2301)
              CALL val_get(val,c_val=line_att)
            ...
            END DO
          END DO
          IF(.NOT.(LEN_TRIM(line_att)==0))CALL cp__a("atom_types.F",2312)
        END DO
      END IF
    ELSE
      NULLIFY(parser)
             ...
             DO
               ...
               ELSEIF(parser_test_next_token(parser) == "STR") THEN
                 ...
                 ELSEIF(INDEX(LINE,"LSD") /= 0) THEN
                   ...
                   END DO
                 ELSE
                   CALL cp__b("atom_types.F",2396,"")
                 END IF
               ELSE
                 CALL cp__b("atom_types.F",2399,"")
               END IF
             END DO
             ! Read the parameters for the non-local part of the GTH pseudopotential (ppnl)
