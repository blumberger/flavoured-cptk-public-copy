# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_gpw.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_gpw.F"
!-----------------------------------------------------------------------------!
...
# 182 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_post_gpw.F" 2
...
! *****************************************************************************
  SUBROUTINE scf_post_calculation_gpw(dft_section, scf_env,qs_env,wf_type)
    ...
    INTEGER :: handle, homo, ispin, min_lumos, n_rep, nhomo, nlumo, &
      nlumo_stm, nlumo_tddft, nlumos, nmo, output_unit, unit_nr
    INTEGER, DIMENSION(:, :, :), POINTER     :: marked_states
    LOGICAL :: check_write, compute_lumos, do_homo, do_kpoints, do_mo_cubes, &
      do_stm, do_wannier_cubes, has_homo, has_lumo, loc_explicit, &
    ...
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
      POINTER                                :: ks_rmpv, matrix_s, mo_derivs
    TYPE(cp_dbcsr_p_type), DIMENSION(:, :), &
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_post_gpw.F",304)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_post_gpw.F",305)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_gpw.F",306)
    ...
    IF (cp_print_key_should_output(logger%iter_info,input,&
       ...
       IF(.NOT.(ASSOCIATED(kinetic_m)))CALL cp__a("qs_scf_post_gpw.F",328)
       IF(.NOT.(ASSOCIATED(kinetic_m(1,1)%matrix)))CALL cp__a("qs_scf_post_gpw.F",329)
    ...
    IF(n_rep==0.AND.p_loc_lumo)THEN
        CALL cp_abort(cp__l("qs_scf_post_gpw.F",382),"No LIST_UNOCCUPIED was specified, "//&
    ...
    IF (do_kpoints) THEN
       IF (do_mo_cubes) THEN
          CALL cp__w("qs_scf_post_gpw.F",427,"Print MO Cubes not implemented for k-point calculations!!")
       END IF
    ELSE
       CALL get_qs_env(qs_env,&
                       mos=mos,&
    ...
    IF (p_loc_homo) THEN
       IF (do_kpoints) THEN
          CALL cp__w("qs_scf_post_gpw.F",469,"Localization not implemented for k-point calculations!!")
    ...
    IF (do_kpoints) THEN
       ...
       CALL cp__w("qs_scf_post_gpw.F",518,"Localization and MO related output not implemented for k-point calculations!")
    ELSE
       IF(nlumo .GT. -1) THEN
    ...
    IF(do_stm)THEN
      IF(do_kpoints) THEN
         CALL cp__w("qs_scf_post_gpw.F",668,"STM not implemented for k-point calculations!")
      ELSE
...
! *****************************************************************************
  SUBROUTINE make_lumo(qs_env,scf_env,unoccupied_orbs,unoccupied_evals,nlumo,nlumos)
    ...
    INTEGER                                  :: handle, homo, ispin, n, nao, &
                                                nmo, output_unit
    TYPE(admm_type), POINTER                 :: admm_env
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_charges(input, logger, qs_env)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       ...
       IF(do_kpoints) THEN
          CALL cp__w("qs_scf_post_gpw.F",995,"Lowdin charges not implemented for k-point calculations!")
       ELSE
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_loc_dip(input, dft_control, qs_loc_env, logger, qs_env)
    ...
    INTEGER                                  :: handle, i, ikind, ispins, j, &
                                                n_rep, reference, unit_nr
    LOGICAL                                  :: do_berry, first_time, ghost
  ...
  SUBROUTINE qs_scf_post_occ_cubes(input, dft_section, dft_control, logger, qs_env,&
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,dft_section,"PRINT%MO_CUBES")&
       ...
       DO ivector=MAX(1,homo-nhomo+1),homo
          ...
          CALL cp_print_key_finished_output(unit_nr,logger,input,"DFT%PRINT%MO_CUBES")
       ENDDO
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_moments(input, logger, qs_env, output_unit)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       ...
       IF (do_kpoints) THEN
          CALL cp__w("qs_scf_post_gpw.F",1396,"Moments not implemented for k-point calculations!")
       ELSE
          IF (periodic) THEN
...
! *****************************************************************************
  SUBROUTINE qs_scf_post_elf(input, logger, qs_env)
    ...
    INTEGER                                  :: handle, ispin, output_unit, &
                                                unit_nr
    LOGICAL                                  :: append_cube, do_elf, gapw
    REAL(dp)                                 :: rho_cutoff
    TYPE(dft_control_type), POINTER          :: dft_control
    TYPE(particle_list_type), POINTER        :: particles
    TYPE(pw_env_type), POINTER               :: pw_env
    ...
    IF(do_elf)THEN
      ...
      IF(.NOT. gapw) THEN
        ...
        DO ispin = 1,dft_control%nspins
          ...
          CALL cp_pw_to_cube(elf_r(ispin)%pw,unit_nr,title,particles=particles,&
                 stride=section_get_ivals(elf_section,"STRIDE"))
          CALL cp_print_key_finished_output(unit_nr,logger,input,"DFT%PRINT%ELF_CUBE")
      ...
      ELSE
         ! not implemented
         CALL cp__w("qs_scf_post_gpw.F",1644,"ELF not implemented for GAPW calculations!!")
...
! *****************************************************************************
  SUBROUTINE qs_elf_calc(qs_env, elf_r, rho_cutoff)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'qs_elf_calc', &
      routineP = moduleN//':'//routineN
    REAL(KIND=dp), PARAMETER :: ELFCUT = 0.0001_dp, f18 = (1.0_dp/8.0_dp), &
      f23 = (2.0_dp/3.0_dp), f53 = (5.0_dp/3.0_dp)

    INTEGER                                  :: handle, i, idir, ispin, j, k, &
                                                nspin
    INTEGER, DIMENSION(2, 3)                 :: bo
    INTEGER, DIMENSION(3, 3)                 :: nd
    LOGICAL                                  :: deriv_pw, drho_r_valid, &
                                                tau_r_valid
...
! *****************************************************************************
  SUBROUTINE write_mo_dependent_results(qs_env,scf_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_gpw.F",2027)
    CALL get_qs_env(qs_env,&
                    dft_control=dft_control,&
                    molecule_set=molecule_set, &
                    atomic_kind_set=atomic_kind_set,&
    ...
    IF(.NOT.qs_env%run_rtp)THEN
       ...
       IF (BTEST(cp_print_key_should_output(logger%iter_info,dft_section,"PRINT%PDOS")&
            ,cp_p_file) ) THEN
          IF(do_kpoints) THEN
             CALL cp__w("qs_scf_post_gpw.F",2068,"Projected density of states not implemented for k-points.")
          ELSE
             CALL get_qs_env(qs_env,&
                             mos=mos,&
                             matrix_ks=ks_rmpv)
    ...
    IF (dft_control%nspins.eq.2) THEN
       ...
       !
       IF(do_kpoints) THEN
          CALL cp__w("qs_scf_post_gpw.F",2135,"Spin contamination estimate not implemented for k-points.")
       ELSE
          all_equal = .TRUE.
          DO ispin=1,dft_control%nspins
...
! *****************************************************************************
  SUBROUTINE write_mo_free_results(qs_env)
    ...
    INTEGER :: after, boundary_condition, handle, i, iat, id, ikind, img, &
      iso, ispin, iw, j, l, n_cstr, n_tiles, nd(3), ngto, niso, nkind, np, &
      nr, output_unit, print_level, unit_nr
    LOGICAL :: append_cube, do_cstr_charge_cube, do_dielectric_cube, &
      do_dirichlet_bc_cube, do_kpoints, has_dirichlet_bc, has_implicit_ps, &
      omit_headers, print_it, print_total_density, rho_r_valid, tile_cubes, &
      write_ks, write_xc, xrd_interface
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_post_gpw.F",2246)
    CALL get_qs_env(qs_env,&
                    atomic_kind_set=atomic_kind_set,&
                    qs_kind_set=qs_kind_set,&
                    particle_set=particle_set,&
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       CALL cp_pw_to_cube(wf_r%pw,unit_nr,"TOTAL DENSITY",&
            particles=particles,&
            stride=section_get_ivals(dft_section,"PRINT%TOT_DENSITY_CUBE%STRIDE"))
       CALL cp_print_key_finished_output(unit_nr,logger,input,&
            "DFT%PRINT%TOT_DENSITY_CUBE")
       CALL pw_pool_give_back_pw(auxbas_pw_pool,wf_r%pw)
    END IF
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       IF (dft_control%qs_control%gapw.AND.print_total_density) THEN
          ...
          IF(.NOT.(.NOT.do_kpoints))CALL cp__a("qs_scf_post_gpw.F",2432)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%V_HARTREE_CUBE",&
            extension=".cube",middle_name="v_hartree",file_position=my_pos_cube)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       IF(dft_control%apply_external_potential)THEN
          ...
          unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%EXTERNAL_POTENTIAL_CUBE",&
               extension=".cube",middle_name="ext_pot",file_position=my_pos_cube)
          ...
          CALL cp_print_key_finished_output(unit_nr,logger,input,&
               "DFT%PRINT%EXTERNAL_POTENTIAL_CUBE")
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,input,&
       ...
       DO id=1,3
         ...
         CALL cp_pw_to_cube(aux_r%pw,&
            ...
            stride=section_get_ivals(dft_section,&
            "PRINT%EFIELD_CUBE%STRIDE"))
         CALL cp_print_key_finished_output(unit_nr,logger,input,&
              "DFT%PRINT%EFIELD_CUBE")
       END DO
    ...
    do_dielectric_cube = BTEST(cp_print_key_should_output(logger%iter_info,input,&
                               "DFT%PRINT%DIELECTRIC_CUBE"),cp_p_file)
    ...
    IF (has_implicit_ps .AND. do_dielectric_cube) THEN
       append_cube = section_get_lval(input,"DFT%PRINT%DIELECTRIC_CUBE%APPEND")
       my_pos_cube="REWIND"
       IF(append_cube) THEN
          my_pos_cube="APPEND"
       END IF
       unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%DIELECTRIC_CUBE",&
            extension=".cube", middle_name="DIELCTRIC_CONSTANT", file_position=my_pos_cube)
       ...
       CALL cp_pw_to_cube(aux_r%pw, unit_nr, "DIELECTRIC CONSTANT", particles=particles,&
            stride=section_get_ivals(dft_section, "PRINT%DIELECTRIC_CUBE%STRIDE"))
       CALL cp_print_key_finished_output(unit_nr, logger, input, "DFT%PRINT%DIELECTRIC_CUBE")
    ...
    do_cstr_charge_cube = BTEST(cp_print_key_should_output(logger%iter_info,input,&
                               "DFT%PRINT%DIRICHLET_CSTR_CHARGE_CUBE"),cp_p_file)
    has_implicit_ps = .FALSE.
    CALL get_qs_env(qs_env=qs_env, pw_env=pw_env)
    IF (pw_env%poisson_env%parameters%solver .EQ. pw_poisson_implicit) has_implicit_ps = .TRUE.
    ...
    IF (has_implicit_ps .AND. do_cstr_charge_cube .AND. has_dirichlet_bc) THEN
       append_cube = section_get_lval(input,"DFT%PRINT%DIRICHLET_CSTR_CHARGE_CUBE%APPEND")
       my_pos_cube="REWIND"
       IF(append_cube) THEN
          my_pos_cube="APPEND"
       END IF
       unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%DIRICHLET_CSTR_CHARGE_CUBE",&
       ...
       CASE (MIXED_BC)
          CALL pw_shrink(pw_env%poisson_env%implicit_env%dct_env%dests_shrink, &
       ...
       CALL cp_pw_to_cube(aux_r%pw, unit_nr, "DIRICHLET CONSTRAINT CHARGE", particles=particles,&
            stride=section_get_ivals(dft_section, "PRINT%DIRICHLET_CSTR_CHARGE_CUBE%STRIDE"))
       CALL cp_print_key_finished_output(unit_nr, logger, input, "DFT%PRINT%DIRICHLET_CSTR_CHARGE_CUBE")
    ...
    do_dirichlet_bc_cube = BTEST(cp_print_key_should_output(logger%iter_info,input,&
                               "DFT%PRINT%DIRICHLET_BC_CUBE"),cp_p_file)
    has_dirichlet_bc = .FALSE.
    IF (has_implicit_ps) THEN
       boundary_condition = pw_env%poisson_env%parameters%ps_implicit_params%boundary_condition
       IF (boundary_condition .EQ. MIXED_PERIODIC_BC .OR. boundary_condition .EQ. MIXED_BC) THEN
    ...
    IF (has_implicit_ps .AND. has_dirichlet_bc .AND. do_dirichlet_bc_cube) THEN
       append_cube = section_get_lval(input,"DFT%PRINT%DIRICHLET_BC_CUBE%APPEND")
       my_pos_cube="REWIND"
       IF(append_cube) THEN
          my_pos_cube="APPEND"
       END IF
       tile_cubes = section_get_lval(input,"DFT%PRINT%DIRICHLET_BC_CUBE%TILE_CUBES")
       ...
       IF (tile_cubes) THEN
          ...
          DO j = 1 , n_cstr
             ...
             DO i = 1, n_tiles
                ...
                unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%DIRICHLET_BC_CUBE", &
                ...
                CALL cp_pw_to_cube(aux_r%pw, unit_nr, "DIRICHLET TYPE CONSTRAINT", particles=particles, &
                                   stride=section_get_ivals(dft_section, "PRINT%DIRICHLET_BC_CUBE%STRIDE"))
                CALL cp_print_key_finished_output(unit_nr, logger, input, "DFT%PRINT%DIRICHLET_BC_CUBE")
             END DO
          END DO
       ELSE 
          ! a single cube file 
    ...
    IF (write_xc) THEN
       ...
       IF(.NOT.(ASSOCIATED(matrix_vxc)))CALL cp__a("qs_scf_post_gpw.F",2894)
    ...
    IF (BTEST(cp_print_key_should_output(logger%iter_info,print_key),cp_p_file)) THEN
       ...
       IF(rho_r_valid) THEN
          unit_nr=cp_print_key_unit_nr(logger,input,"DFT%PRINT%HIRSHFELD",extension=".hirshfeld",&
                                       middle_name="",log_filename=.FALSE.)
  ...
  SUBROUTINE hirshfeld_charges(qs_env,input_section,unit_nr)
    ...
    SELECT CASE (refc)
       ...
       CASE (ref_charge_mulliken)
          ...
          END DO
       CASE DEFAULT
         CALL cp__b("qs_scf_post_gpw.F",3016,"Unknown type of reference charge for Hirshfeld partitioning.")
    END SELECT
    !
    charges = 0.0_dp
...
! *****************************************************************************
  SUBROUTINE project_function_a(ca,a,cb,b,l)
    ...
    IF(.NOT.(info==0))CALL cp__a("qs_scf_post_gpw.F",3081)
    ca(:) = v(:,1)
...
! *****************************************************************************
  SUBROUTINE project_function_b(ca,a,bfun,grid_atom,l)
    ...
    END DO
    CALL lapack_sgesv ( n, 1, smat, n, ipiv, v, n, info )
    IF(.NOT.(info==0))CALL cp__a("qs_scf_post_gpw.F",3122)
    ca(:) = v(:,1)
