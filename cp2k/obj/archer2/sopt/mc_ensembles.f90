# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ensembles.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ensembles.F"
!-----------------------------------------------------------------------------!
...
# 85 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_ensembles.F" 2
...
! *****************************************************************************
  SUBROUTINE mc_run_ensemble ( mc_env, para_env, globenv, input_declaration, nboxes, rng_stream)
    ...
    IF(pmvol_box(nboxes) .LT. 1.0E0_dp) THEN
       CALL cp__b("motion/mc/mc_ensembles.F",257,'The last value of PMVOL_BOX needs to be 1.0')
    ENDIF
    IF(pmhmc_box(nboxes) .LT. 1.0E0_dp) THEN
       CALL cp__b("motion/mc/mc_ensembles.F",260,'The last value of PMHMC_BOX needs to be 1.0')
    ENDIF
...
! don't care about overlaps if we're only doing HMC
       ...
       IF(.NOT. lhmc)THEN
          ...
          IF(loverlap) CALL cp__b("motion/mc/mc_ensembles.F",313,"overlap in an initial configuration")
       ENDIF
    ...
    IF(ldiscrete) THEN
       IF(nboxes .NE. 1) &
            CALL cp__b("motion/mc/mc_ensembles.F",433,'ldiscrete=.true. ONLY for systems with 1 box')
...
! pick a box at random
       ...
       ELSEIF ( rand .LT. pmavbmc) THEN
          ...
          IF(molecule_type_swap == 0)&
             CALL cp__b("motion/mc/mc_ensembles.F",633,'Did not choose a molecule type to swap...check AVBMC input')
          ...
          DO imove=1,nmoves
             ...
             IF(rand .LT. pmtraion) THEN
                ...
                DO imol_type=1,nmol_types
                   ...
                   ENDIF
                ENDDO
                IF(molecule_type == 0) CALL cp_abort(cp__l("motion/mc/mc_ensembles.F",726),&
             ...
             ELSEIF(rand .LT. pmtrans) THEN
                ...
                DO imol_type=1,nmol_types
                   ...
                   ENDIF
                ENDDO
                IF(molecule_type == 0) CALL cp_abort(cp__l("motion/mc/mc_ensembles.F",783),&
             ...
             ELSE
                ...
                DO imol_type=1,nmol_types
                   ...
                   ENDIF
                ENDDO
                IF(molecule_type == 0) CALL cp_abort(cp__l("motion/mc/mc_ensembles.F",814),&
          ...
          DO ibox=1,nboxes
             IF(SUM(nchains(:,ibox)) .NE. 0) THEN
                ...
                IF(loverlap) THEN
                   ...
                   CALL cp__b("motion/mc/mc_ensembles.F",969,'coordinate overlap at the end of the above step')
       ...
       IF(debug_this_module) THEN
          DO ibox=1,nboxes
             ...
             IF(ABS(initial_energy(ibox)+energy_check(ibox)-&
                ...
                END IF
                CALL cp__b("motion/mc/mc_ensembles.F",1032,'!!!!!!! We have an energy problem. !!!!!!!!')
             ENDIF
          ENDDO
       ENDIF
    ENDDO
...
! better statistics
    ...
    IF(abc(1) .NE. abc(2) .OR. abc(2) .NE. abc(3)) THEN
       CALL cp__b("motion/mc/mc_ensembles.F",1258,'The box needs to be cubic for a virial calculation (it is easiest).')
    END IF
    IF(virial_cutoffs(nintegral_divisions) .GT. abc(1)/2.0E0_dp) THEN
       ...
       CALL cp__b("motion/mc/mc_ensembles.F",1264,'You need a bigger box to deal with this virial cutoff (see above).')
    END IF
