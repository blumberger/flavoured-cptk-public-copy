# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_setup.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_setup.F"
!-----------------------------------------------------------------------------!
...
MODULE tmc_setup
  ...
  USE cp_log_handling,                 ONLY: &
       ...
       cp_logger_get_default_io_unit, cp_logger_release, cp_logger_set, &
...
# 82 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_setup.F" 2
...
! start the timing
    ...
    IF(success) THEN
      ...
      ELSE IF(tmc_env%tmc_comp_set%group_nr.GT.0) THEN
        ...
        IF(tmc_env%tmc_comp_set%group_nr.LE.tmc_env%tmc_comp_set%group_ener_nr) THEN
          CALL create_force_env(new_env_id=tmc_env%w_env%env_id_ener, &
                  ...
                  ierr=ierr)
          IF(ierr.NE.0)&
             CALL cp__b("tmc/tmc_setup.F",245,"creating force env result in error "//cp_to_string(ierr))
        END IF
        ...
        IF(tmc_env%params%NMC_inp_file.NE."" .AND. &
          ...
          CALL create_force_env(new_env_id=tmc_env%w_env%env_id_approx, &
                  ...
                  ierr=ierr)
          IF(ierr.NE.0)&
             CALL cp__b("tmc/tmc_setup.F",259,"creating approx force env result in error "//cp_to_string(ierr))
        END IF
      ...
      IF(.NOT.(ASSOCIATED(tmc_env%rng_stream)))CALL cp__a("tmc/tmc_setup.F",315)
...
! *****************************************************************************
  SUBROUTINE do_analyze_files(input_declaration, root_section, para_env)
    ...
    IF(para_env%num_pe.NE.1)&
       CALL cp__w("tmc/tmc_setup.F",381,"just one out of "//cp_to_string(para_env%num_pe)//"cores is used ")
    ! distribute work to availuble cores
    IF(para_env%mepos.EQ.0) THEN
      !TODO get the correct usage of creating and handling the logger...
      logger=>cp_get_default_logger()
      output_unit= cp_logger_get_default_io_unit(logger)
      IF(.NOT.(output_unit.GT.0))CALL cp__a("tmc/tmc_setup.F",387)
      ! write the header
      CALL tmc_ana_header(output_unit)
      ...
      IF(.NOT.(SIZE(ana_list).GT.0))CALL cp__a("tmc/tmc_setup.F",398)
      ...
      DO temp=1, SIZE(ana_list)
        ...
        IF(.NOT.ASSOCIATED(elem).AND..NOT.ASSOCIATED(ana_list(temp)%temp%last_elem))&
           CALL cp__b("tmc/tmc_setup.F",421,"uncorrect initialization of the initial configuration")
...
! *****************************************************************************
  SUBROUTINE tmc_read_ana_files_input(input_declaration, input, ana_list, elem, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(input)))CALL cp__a("tmc/tmc_setup.F",496)
    IF(.NOT.(.NOT.ASSOCIATED(ana_list)))CALL cp__a("tmc/tmc_setup.F",497)
    IF(.NOT.(.NOT.ASSOCIATED(elem)))CALL cp__a("tmc/tmc_setup.F",498)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_setup.F",499)
    ...
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_setup.F",507)
    ...
    IF((nr_temp .GT. 1).AND.(SIZE(inp_Temp).NE.2))&
       CALL cp__b("tmc/tmc_setup.F",516,"specify each temperature, skip keyword NR_TEMPERATURE")
    ...
    ELSE
      ...
      IF(ANY(Temps.LT.0.0_dp))&
         CALL cp_abort(cp__l("tmc/tmc_setup.F",530),"The temperatures are negative. Should be specified using "//&
...
! *****************************************************************************
  SUBROUTINE tmc_preread_input(input,tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(input)))CALL cp__a("tmc/tmc_setup.F",594)
    ...
    tmc_section => section_vals_get_subs_vals(input,"MOTION%TMC")
    CALL section_vals_get(tmc_section,explicit=flag)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_setup.F",605)

    CALL section_vals_val_get(tmc_section,"PRINT_TEST_OUTPUT",l_val=tmc_env%params%print_test_output)

    IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set)))CALL cp__a("tmc/tmc_setup.F",609)
    ...
    IF(tmc_env%tmc_comp_set%ana_on_the_fly.GT.1)&
       CALL cp_abort(cp__l("tmc/tmc_setup.F",618),&
    ...
    ! file name keyword without file name
      CALL cp__b("tmc/tmc_setup.F",631,"no or a valid NMC input file has to be specified ")
    ELSE IF(tmc_env%params%NMC_inp_file.EQ."") THEN
    ! no keyword
      IF(tmc_env%tmc_comp_set%group_cc_size.GT.0)&
         CALL cp_warn(cp__l("tmc/tmc_setup.F",635),&
    ...
    ! check file existance
      ...
      IF(.NOT.flag .OR. itmp.NE.0)&
         CALL cp__b("tmc/tmc_setup.F",643,"a valid NMC input file has to be specified")
    END IF

    CALL section_vals_val_get(tmc_section,"TEMPERATURE",r_vals=inp_Temp)
    IF(tmc_env%params%nr_temp.GT.1 .AND. SIZE(inp_Temp).NE.2) &
       CALL cp__b("tmc/tmc_setup.F",648,"specify each temperature, skip keyword NR_TEMPERATURE")
    ...
    ELSE    
      ...
      IF(ANY(tmc_env%params%Temp.LT.0.0_dp))&
         CALL cp_abort(cp__l("tmc/tmc_setup.F",662), "The temperatures are negative. Should be specified using "//&
              "TEMPERATURE {T_min} {T_max} and NR_TEMPERATURE {#temperatures}")
    END IF
    ...
    IF(explicit_key)THEN
      ...
      CASE DEFAULT
        CALL cp_warn(cp__l("tmc/tmc_setup.F",675),&
...
! *****************************************************************************
  SUBROUTINE tmc_read_input(input,tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(input)))CALL cp__a("tmc/tmc_setup.F",707)

    tmc_section => section_vals_get_subs_vals(input,"MOTION%TMC")
    CALL section_vals_get(tmc_section,explicit=flag)
    IF(.NOT.(flag))CALL cp__a("tmc/tmc_setup.F",711)

    ! only for the master
    IF(tmc_env%tmc_comp_set%group_nr == 0) THEN 
      IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_setup.F",715)
      ...
      IF(tmc_env%m_env%restart_out_step.LT.0)&
         CALL cp_abort(cp__l("tmc/tmc_setup.F",736),&
    ...
    ELSE IF(tmc_env%tmc_comp_set%group_nr .NE. 0) THEN 
      IF(.NOT.(ASSOCIATED(tmc_env%w_env)))CALL cp__a("tmc/tmc_setup.F",750)
    END IF
    ...
    IF(tmc_env%params%energy_inp_file.EQ."")&
       CALL cp__b("tmc/tmc_setup.F",759,"a valid exact energy input file has to be specified ")
    ! check file existance
    INQUIRE (FILE=TRIM(tmc_env%params%energy_inp_file),EXIST=flag,IOSTAT=itmp)
    IF(.NOT.flag.OR.itmp.NE.0)&
       CALL cp_abort(cp__l("tmc/tmc_setup.F",763), "a valid exact energy input file has to be specified, "//&
    ...
    IF(SIZE(r_arr_tmp).GT.1) THEN
      IF(SIZE(r_arr_tmp).NE.tmc_env%params%dim_per_elem)&
         CALL cp__b("tmc/tmc_setup.F",776,"The entered sub box sizes does not fit in number of dimensions.")
      IF(ANY(r_arr_tmp.LE.0.0_dp))&
         CALL cp__b("tmc/tmc_setup.F",778,"The entered sub box lenghts should be greater than 0.")
...
! *****************************************************************************
  SUBROUTINE tmc_redistributing_cores(tmc_comp_set,para_env, ana_on_the_fly, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_comp_set)))CALL cp__a("tmc/tmc_setup.F",834)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_setup.F",835)
    ...
    IF(para_env%num_pe.LE.1) THEN
      CALL cp__w("tmc/tmc_setup.F",855,"TMC need at least 2 cores (one for master, one for worker)")
      success = .FALSE.
    ELSE
      ...
      IF(tmc_comp_set%group_ener_size*tmc_comp_set%group_ener_nr.GT.(para_env%num_pe-1))&
         CALL cp__w("tmc/tmc_setup.F",860,"The selected energy group size is too huge. ")
      IF(flag)THEN
        ...
        IF(tmc_comp_set%group_ener_nr.LT.1)&
           CALL cp__w("tmc/tmc_setup.F",865,"The selected energy group size is too huge. ")
      ...
      IF(tmc_comp_set%group_cc_size.GT.0)THEN
        ...
        IF(tmc_comp_set%group_cc_nr.LT.1)&
           CALL cp_warn(cp__l("tmc/tmc_setup.F",877),&
      ...
      IF(para_env%num_pe-1.GT.total_used)&
         CALL cp__w("tmc/tmc_setup.F",886," mpi ranks are unused, but can be used for analysis.")
      ...
      ELSE
        ...
        ELSE
          ...
          IF(para_env%mepos .EQ. para_env%num_pe-2) THEN
            ...
            IF(.NOT.(tmc_comp_set%group_nr.LT.0))CALL cp__a("tmc/tmc_setup.F",936)
...
! *****************************************************************************
  SUBROUTINE tmc_print_params(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_setup.F",1025)
    IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set)))CALL cp__a("tmc/tmc_setup.F",1026)
    ! only the master prints out
    IF(tmc_env%tmc_comp_set%group_nr == 0) THEN
      file_nr = tmc_env%m_env%io_unit 
      IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set%para_env_m_w)))CALL cp__a("tmc/tmc_setup.F",1030)
      IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_setup.F",1031)
