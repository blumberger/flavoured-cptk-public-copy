# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fp_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fp_types.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fp_types.F" 2
...
! *****************************************************************************
  SUBROUTINE fp_env_create(fp_env)
    ...
    CALL timeset(routineN,handle)
      ...
      IF(.NOT.(.NOT.ASSOCIATED(fp_env)))CALL cp__a("fp_types.F",74)
      ALLOCATE(fp_env)
      fp_env%ref_count=1
  ...
  SUBROUTINE fp_env_release(fp_env)
    ...
    CALL timeset(routineN,handle)
     ...
     IF (ASSOCIATED(fp_env)) THEN
        IF(.NOT.(fp_env%ref_count>0))CALL cp__a("fp_types.F",101)
...
!> \param fp_env ...
! *****************************************************************************
  SUBROUTINE fp_env_retain(fp_env)
    TYPE(fp_type), POINTER                   :: fp_env

    CHARACTER(len=*), PARAMETER :: routineN = 'fp_env_retain', &
      routineP = moduleN//':'//routineN

      IF(.NOT.(ASSOCIATED(fp_env)))CALL cp__a("fp_types.F",125)
      fp_env%ref_count=fp_env%ref_count+1
...
! *****************************************************************************
  SUBROUTINE fp_env_read(fp_env,fp_section)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(fp_env)))CALL cp__a("fp_types.F",148)
...
! *****************************************************************************
  SUBROUTINE fp_env_write(fp_env,fp_section)
    ...
    IF(.NOT.(ASSOCIATED(fp_env)))CALL cp__a("fp_types.F",195)
