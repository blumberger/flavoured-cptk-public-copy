# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential_types.F"
!-----------------------------------------------------------------------------!
...
# 26 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pair_potential_types.F" 2
...
! *****************************************************************************
  SUBROUTINE compare_pot(pot1, pot2, compare)
    ...
    IF(.NOT.(ASSOCIATED(pot1%type)))CALL cp__a("pair_potential_types.F",310)
    IF(.NOT.(ASSOCIATED(pot2%type)))CALL cp__a("pair_potential_types.F",311)
    IF (SIZE(pot1%type)/=SIZE(pot2%type)) RETURN
    IF (ANY(pot1%type/=pot2%type))        RETURN

    ! Checking the real values of parameters
    IF(.NOT.(ASSOCIATED(pot1%set)))CALL cp__a("pair_potential_types.F",316)
    IF(.NOT.(ASSOCIATED(pot2%set)))CALL cp__a("pair_potential_types.F",317)
       ...
       CASE (siepmann_type)
          IF  ((pot1%set(i)%siepmann%B       == pot2%set(i)%siepmann%B       ).AND.&
               ...
               (pot1%set(i)%siepmann%allow_oh_formation .EQV.&
                                   pot2%set(i)%siepmann%allow_oh_formation )) mycompare=.TRUE.
       END SELECT
...
! *****************************************************************************
  SUBROUTINE pair_potential_single_create(potparm, nset)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(potparm)))CALL cp__a("pair_potential_types.F",419)
    ALLOCATE(potparm)
    lnset = 1
...
! *****************************************************************************
  SUBROUTINE pair_potential_single_copy(potparm_source, potparm_dest)
    ...
    IF(.NOT.(ASSOCIATED(potparm_source)))CALL cp__a("pair_potential_types.F",509)
...
! *****************************************************************************
  SUBROUTINE pair_potential_single_add(potparm_source, potparm_dest)
    ...
    IF(.NOT.(ASSOCIATED(potparm_source)))CALL cp__a("pair_potential_types.F",569)
    ...
    IF(.NOT.(check))CALL cp__a("pair_potential_types.F",574)
    check = (.NOT.ASSOCIATED( potparm_dest % pair_spline_data )).AND.&
            (.NOT.ASSOCIATED( potparm_dest % spl_f ))
    IF(.NOT.(check))CALL cp__a("pair_potential_types.F",577)
    ...
    check= (potparm_dest % shell_type == potparm_source % shell_type).AND.&
           ...
           (potparm_dest % rcutsq     == potparm_source % rcutsq    )
    IF(.NOT.(check))CALL cp__a("pair_potential_types.F",655)
...
! *****************************************************************************
  SUBROUTINE pair_potential_single_release ( potparm)
    ...
    IF(.NOT.(ASSOCIATED(potparm)))CALL cp__a("pair_potential_types.F",693)
    CALL spline_data_p_release( potparm% pair_spline_data)
    CALL spline_factor_release( potparm% spl_f)
    DO i = 1, SIZE(potparm%type)
...
! *****************************************************************************
  SUBROUTINE pair_potential_pp_create ( potparm, nkinds)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(potparm)))CALL cp__a("pair_potential_types.F",731)
    ALLOCATE ( potparm)
...
! *****************************************************************************
  SUBROUTINE pair_potential_p_create ( potparm, ndim, ub, lb)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(potparm)))CALL cp__a("pair_potential_types.F",796)
    ALLOCATE ( potparm)
    IF (PRESENT(ndim)) THEN
       ...
       IF (PRESENT(lb).OR.PRESENT(ub)) THEN
          CALL cp__b("pair_potential_types.F",803,"")
       END IF
    ELSE IF (PRESENT(lb).AND.PRESENT(ub)) THEN
       ...
       ALLOCATE ( potparm%pot( loc_lb : loc_ub ))
       IF (PRESENT(ndim)) THEN
          CALL cp__b("pair_potential_types.F",810,"")
       END IF
    ELSE
       CALL cp__b("pair_potential_types.F",813,"")
    END IF
    DO i = loc_lb, loc_ub
...
! *****************************************************************************
  SUBROUTINE pair_potential_p_copy(source, dest, istart, iend)
    ...
    IF(.NOT.(ASSOCIATED(source)))CALL cp__a("pair_potential_types.F",866)
    IF(.NOT.(ASSOCIATED(dest)))CALL cp__a("pair_potential_types.F",867)
...
! *****************************************************************************
  SUBROUTINE pair_potential_reallocate(p,lb1_new,ub1_new,lj,lj_charmm,williams,&
    ...
    INTEGER                                  :: i, ipot, lb1_old, std_dim, &
                                                ub1_old
    LOGICAL :: check, lbmhft, lbmhftd, lbuck4r, lbuckmo, leam, lgoodwin, lgp, &
    ...
    IF (llj) THEN
       ...
       check = .NOT.(llj_charmm.OR.lwilliams.OR.lgoodwin.OR.leam.OR.lquip.OR.lbmhft&
                     ...
                     .OR.lsiepmann)
       IF(.NOT.(check))CALL cp__a("pair_potential_types.F",939)
    END IF
    IF (llj_charmm) THEN
       ipot  = lj_charmm_type
    ...
    IF (lgp) THEN
       ...
       IF(.NOT.(check))CALL cp__a("pair_potential_types.F",1017)
    END IF
    ...
    DO i = ub1_old+1, ub1_new
       ...
       IF(.NOT.(check))CALL cp__a("pair_potential_types.F",1037)
...
! *****************************************************************************
  SUBROUTINE pair_potential_gp_create(gp)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(gp)))CALL cp__a("pair_potential_types.F",1093)
    ALLOCATE(gp)
...
! *****************************************************************************
  SUBROUTINE pair_potential_lj_create(lj)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(lj)))CALL cp__a("pair_potential_types.F",1189)
    ALLOCATE(lj)
...
! *****************************************************************************
  SUBROUTINE pair_potential_williams_create(willis)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(willis)))CALL cp__a("pair_potential_types.F",1260)
    ALLOCATE(willis)
...
! *****************************************************************************
  SUBROUTINE pair_potential_goodwin_create(goodwin)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(goodwin)))CALL cp__a("pair_potential_types.F",1333)
    ALLOCATE(goodwin)
...
! *****************************************************************************
  SUBROUTINE pair_potential_eam_create(eam)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(eam)))CALL cp__a("pair_potential_types.F",1408)
    ALLOCATE(eam)
...
! *****************************************************************************
  SUBROUTINE pair_potential_quip_create(quip)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(quip)))CALL cp__a("pair_potential_types.F",1532)
    ALLOCATE(quip)
    quip%quip_file_name=""
    quip%init_args=""
    quip%calc_args=""
...
! *****************************************************************************
  SUBROUTINE pair_potential_bmhft_create(ft)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ft)))CALL cp__a("pair_potential_types.F",1604)
    ALLOCATE(ft)
...
! *****************************************************************************
  SUBROUTINE pair_potential_bmhftd_create(ftd)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ftd)))CALL cp__a("pair_potential_types.F",1676)
    ALLOCATE(ftd)
...
! *****************************************************************************
  SUBROUTINE pair_potential_ipbv_create(ipbv)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(ipbv)))CALL cp__a("pair_potential_types.F",1751)
    ALLOCATE(ipbv)
...
! *****************************************************************************
  SUBROUTINE pair_potential_buck4r_create(buck4r)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(buck4r)))CALL cp__a("pair_potential_types.F",1823)
    ALLOCATE(buck4r)
...
! *****************************************************************************
  SUBROUTINE pair_potential_buckmo_create(buckmo)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(buckmo)))CALL cp__a("pair_potential_types.F",1908)
    ALLOCATE(buckmo)
...
! *****************************************************************************
  SUBROUTINE pair_potential_tersoff_create(tersoff)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(tersoff)))CALL cp__a("pair_potential_types.F",1992)
    ALLOCATE(tersoff)
...
! *****************************************************************************
  SUBROUTINE pair_potential_siepmann_create(siepmann)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(siepmann)))CALL cp__a("pair_potential_types.F",2086)
    ALLOCATE(siepmann)
