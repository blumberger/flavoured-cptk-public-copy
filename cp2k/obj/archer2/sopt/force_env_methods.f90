# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/force_env_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/force_env_methods.F"
!-----------------------------------------------------------------------------!
...
!> End CHANGE_AC
  ...
  USE molecule_types_new,              ONLY: molecule_type,&
                                                get_molecule
  USE sh_site_energies_methods,        ONLY: add_energies_forces_mem_saver_V2, &
                                             add_energies_forces, &
                                             create_ext_press_list, &
                                             ct_block_energies_forces,&
                                             multi_block_energies_forces,&
                                             add_energies_forces_mem_saver, &
                                             single_phase_energies_forces

  USE aom_utils,                       ONLY: test_diagonalisation
...
# 147 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/force_env_methods.F" 2
!> CHANGE_AC
!> 15-12-11
!> To create a new way to combine the force
!> To get the adiabatic force
  ...
  USE non_adiabatic_methods,           ONLY: adiab_fssh_routines
...
!> CHANGE_AC
!> 1-01-25
!> Get the itimes information
!> Require for the Non Adiabatic Molecular Dynamics
      require_consistent_energy_force, linres, calc_stress_tensor,itimes)
!> END CHANGE_AC
    ...
    INTEGER, POINTER, OPTIONAL               :: itimes ! CHANGE_AC
...
!> END CHANGE_AC
    ...
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_methods.F",259)
    IF(.NOT.(force_env%ref_count>0))CALL cp__a("force_env_methods.F",260)
    ...
    IF (eval_ef) THEN
       ...
       CASE ( use_mixed_force )
          CALL mixed_energy_forces(force_env,calculate_forces, my_itimes) !CHANGE_AC
       CASE default
          CALL cp__b("force_env_methods.F",290,"")
       END SELECT
    END IF
       ...
       ELSE
          ...
          ELSE
             IF (calculate_stress_tensor) THEN
                CALL cp_warn(cp__l("force_env_methods.F",304),"The calculation of the stress tensor "//&
    ...
    ELSE
       output_unit = cp_print_key_unit_nr(logger,force_env%force_env_section,"PRINT%STRESS_TENSOR",&
                                          extension=".stress_tensor")
       IF (output_unit > 0) THEN
          CALL cp_warn(cp__l("force_env_methods.F",393),"To print the stress tensor switch on the "//&
    ...
    IF (atprop_env%energy) THEN
       ...
       IF (output_unit > 0) THEN
          ...
          WRITE (UNIT=output_unit,FMT="(/,(T2,A,T56,F25.13))")&
           ...
           "Difference               :",checksum
          IF(.NOT.((checksum < ateps*ABS(e_pot))))CALL cp__a("force_env_methods.F",418)
       END IF
!       CALL mp_sum(atprop_env%atener, force_env%para_env%group)
       CALL cp_print_key_finished_output(output_unit,logger,force_env%force_env_section,&
                                         "PRINT%PROGRAM_RUN_INFO")
       ...
       IF (output_unit > 0) THEN
          ...
          IF (virial%pv_availability.AND.calculate_forces) THEN
             ...
             WRITE (UNIT=output_unit,FMT="(/,(T2,A,11X,F25.13))")&
              ...
              "Difference               :",checksum
             IF(.NOT.(checksum < ateps))CALL cp__a("force_env_methods.F",483)
          END IF
       END IF
       CALL cp_print_key_finished_output(output_unit,logger,force_env%force_env_section,&
                                         "PRINT%PROGRAM_RUN_INFO")
...
! *****************************************************************************
  SUBROUTINE force_env_calc_num_pressure(force_env,dx)
    ...
    INTEGER                                  :: i, ip, iq, j, k, natom, &
                                                ncore, ndigits, nshell, &
                                                output_unit
    REAL(KIND=dp)                            :: dx_w
...
! *****************************************************************************
  SUBROUTINE force_env_create(force_env,root_section,para_env,globenv,fist_env,&
    ...
    IF (PRESENT(fist_env)) THEN
       IF(.NOT.(ASSOCIATED(fist_env)))CALL cp__a("force_env_methods.F",751)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",752)
       force_env%in_use=use_fist_force
       force_env%fist_env => fist_env
       CALL fist_env_retain(fist_env)
    END IF
    IF (PRESENT(eip_env)) THEN
       IF(.NOT.(ASSOCIATED(eip_env)))CALL cp__a("force_env_methods.F",758)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",759)
       force_env%in_use = use_eip_force
       force_env%eip_env => eip_env
       CALL eip_env_retain(eip_env)
    END IF
    IF (PRESENT(qs_env)) THEN
       IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("force_env_methods.F",765)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",766)
       force_env%in_use=use_qs_force
       force_env%qs_env => qs_env
       CALL qs_env_retain(qs_env)
    END IF
    IF (PRESENT(qmmm_env)) THEN
       IF(.NOT.(ASSOCIATED(qmmm_env)))CALL cp__a("force_env_methods.F",772)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",773)
       force_env%in_use=use_qmmm
       force_env%qmmm_env => qmmm_env
       CALL qmmm_env_retain(qmmm_env)
    END IF
    IF (PRESENT(qmmmx_env)) THEN
       IF(.NOT.(ASSOCIATED(qmmmx_env)))CALL cp__a("force_env_methods.F",779)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",780)
       force_env%in_use=use_qmmmx
       force_env%qmmmx_env => qmmmx_env
       CALL qmmmx_env_retain(qmmmx_env)
    END IF
    IF (PRESENT(mixed_env)) THEN
       IF(.NOT.(ASSOCIATED(mixed_env)))CALL cp__a("force_env_methods.F",786)
       IF(.NOT.(force_env%in_use==0))CALL cp__a("force_env_methods.F",787)
       force_env%in_use=use_mixed_force
       force_env%mixed_env => mixed_env
       CALL mixed_env_retain ( mixed_env)
    END IF
    IF(.NOT.(force_env%in_use/=0))CALL cp__a("force_env_methods.F",792)
...
! *****************************************************************************
  SUBROUTINE mixed_energy_forces(force_env, calculate_forces, my_itimes) !CHANGE_AC
...
!> To get the adiabatic force 
    ...
    CHARACTER(LEN=default_string_length)     :: adiab_keyword
    TYPE(mixed_environment_type), POINTER    :: mixed_env
    TYPE(atomic_kind_list_type), POINTER     :: atomic_kinds
    TYPE(distribution_1d_type), POINTER      :: local_particles
    LOGICAL                                  :: do_speedup_lj, do_speedup_intra!,&
!                                                do_mem_save
!    TYPE(mixed_force_type), DIMENSION(:), &
!      POINTER                                :: my_global_forces
    REAL(KIND=dp), DIMENSION(:), POINTER     :: my_energies

    TYPE(fist_energy_type), POINTER              :: thermo !CHANGE SG
    TYPE(fist_environment_type), POINTER         :: fist_env  !CHANGE SG
!    TYPE(molecule_kind_type), POINTER        :: molecule_kind
    ...
    REAL(KIND=dp), ALLOCATABLE                 :: intra_forces(:,:)
    REAL(KIND=dp), ALLOCATABLE                 :: intra_forces_global(:,:,:)
    TYPE(section_vals_type), POINTER         :: energy_decomp_section
    INTEGER                                  :: act_atoms, output_unit
    INTEGER, DIMENSION(:), POINTER           :: my_mol_index !CHANGE SG
!additional keywords
    ...
    LOGICAL                                  :: multistate
    CHARACTER(LEN=default_string_length)     :: block_type_keyword
    LOGICAL                                  :: fe_block_keyword, cr_block_keyword
    REAL(kind=dp), dimension(4,4) :: C
    REAL(kind=dp), dimension(4) :: E

    NULLIFY(atomic_kinds, local_particles, molecule_set, molecule_kind_set, local_molecules)
!> End CHANGE_AC and SG

    logger => cp_get_default_logger()
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("force_env_methods.F",907)
    ...
    CALL cp_subsys_get(subsys=subsys_mix,&
                       atomic_kinds=atomic_kinds,& !CHANGE_AC
                       local_particles=local_particles,& !CHANGE_AC
                       molecule_set=molecule_set, & !CHANGE SG
                       molecule_kind_set=molecule_kind_set,& !CHANGE SG
                       local_molecules_new=local_molecules,& !CHANGE SG
                       particles=particles_mix,&
                       virial=virial_mix,&
                       results=results_mix)


    CALL force_env_get(force_env%sub_force_env(1)%force_env, fist_env=fist_env)
    ...
    IF (ALLOCATED(thermo%pot_list)) THEN
       ALLOCATE(energies_1_mol_list(nforce_eval, SIZE(thermo%pot_list) )) !CHANGE SG
       energy_decomp_section => section_vals_get_subs_vals(force_env_section, &
                                                      "ENERGY_DECOMP")
       CALL section_vals_val_get(energy_decomp_section, "INDEX_MOL_DECOMP",& !CHANGE SG
                                      i_vals=my_mol_index) 
       CALL section_vals_val_get(energy_decomp_section, "NUM_ACTIVE_ATOMS", &
                                     i_val=act_atoms)
       ALLOCATE(intra_forces_global(nforce_eval, 3, act_atoms))
    END IF
    ...
    energies    = 0.0_dp
    IF (ALLOCATED(thermo%pot_list)) THEN
        energies_1_mol_list = 0.0_dp !CHANGE SG
        intra_forces_global = 0.0_dp
    END IF

    glob_natoms = 0
!> COMMENT_SG: start looping over force_eval section
    DO iforce_eval = 1, nforce_eval
       ...
       IF ( force_env%sub_force_env(iforce_eval)%force_env%para_env%mepos==&
          ...
          CALL force_env_get(force_env%sub_force_env(iforce_eval)%force_env, fist_env=fist_env)
          CALL fist_env_get(fist_env, thermo=thermo, intra_forces=intra_forces)
            IF (ALLOCATED(intra_forces)) THEN 
              intra_forces_global(iforce_eval, :, :) = intra_forces(:,:)
            END IF 

             energies_1_mol_list(iforce_eval, :) = thermo%pot_list
          END IF
    ...
    !CALL mp_sum(energies_1_mol_list, force_env%para_env%group)
...
!NB: here is where the forces from each particles in els are passed to the global
!environment that store all of them and then they will be used in
!add_energies_forces
             END DO
          END IF
       END IF
...
!    IF (ALLOCATED(thermo%pot_list)) THEN
       !force_env%mixed_env%energies_1_mol_list = energies_1_mol_list
!    END IF
    ...
    CASE(mix_linear_combination)
       ...
       IF(.NOT.(nforce_eval==2))CALL cp__a("force_env_methods.F",1093)
       ...
       !CHANGE SAM 
       output_unit = cp_print_key_unit_nr(logger,force_env%force_env_section,& 
                 "PRINT%PROGRAM_RUN_INFO", extension=".Log")
       IF (output_unit > 0) THEN
          WRITE(*,'(A,F16.13)') "delta_energy in Ha = ", energies(2) - energies(1)
       END IF
    ...
    CASE(mix_minimum)
       ! Support offered only 2 force_eval
       IF(.NOT.(nforce_eval==2))CALL cp__a("force_env_methods.F",1110)
    ...
    CASE(mix_coupled)
       ! Support offered only 2 force_eval
       IF(.NOT.(nforce_eval==2))CALL cp__a("force_env_methods.F",1122)
    ...
    CASE(mix_restrained)
       ! Support offered only 2 force_eval
       IF(.NOT.(nforce_eval==2))CALL cp__a("force_env_methods.F",1136)
    ...
    CASE(mix_generic)
       ...
       IF(.NOT.(EvalErrType<=0))CALL cp__a("force_env_methods.F",1159)
       ...
       DO iforce_eval = 1, nforce_eval
          ...
          IF (ABS(err)>lerr) THEN
             ...
             CALL cp_warn(cp__l("force_env_methods.F",1171),&
                  'ASSERTION (cond) failed at line '//cp_to_string(1172)//&
       ...
       CALL finalizef()
!> CHANGE_AC [Extended SG 2019-2020]
!> 15-12-11
!> To create a new way to combine the force
!> To get the adiabatic force 
    CASE(mix_adiab)
        adiab_section => section_vals_get_subs_vals(mixed_section, "ADIABATIC")
        CALL section_vals_val_get(adiab_section,"METHOD_PROPAGATION",c_val=adiab_keyword)
        ...
        mixed_env => force_env%mixed_env
           DO iforce_eval = 2, nforce_eval
              IF (glob_natoms(iforce_eval).NE.glob_natoms(1)) THEN
                 CALL cp__b("force_env_methods.F",1198,"Illegal use of CLASSICAL_PATH methods")
              END IF
           ENDDO 
           !print*, "AC_energies", energies 

         !if thermo%pot is associated then the my_mol_index is taken from the
         !decomp file, otherwise it must be specified as input 
         IF (.NOT. ALLOCATED(thermo%pot_list)) THEN
            CALL section_vals_val_get(adiab_section,"INDEX_ACTIVE_MOLS",&
                i_vals=my_mol_index) 
         ELSE IF (ALLOCATED(thermo%pot_list)) THEN 
             my_mol_index = my_mol_index
         ELSE
             CALL cp__b("force_env_methods.F",1211,"The program must know the index of ative mol")
         END IF          

           !> create the list of active molecules if not present in DECOMP
           !and the array of active atoms: ext_pres (logical array with T for active atoms),
           ! which is use for kinetic energy calculation of the active subsystem.
           !> ext_pres will be overwritten in AOM case becuase active molecules
           ...
           CALL create_ext_press_list(molecule_kind_set, local_molecules, molecule_set, natom, &
                                       my_mol_index,  ext_pres, & 
                                       active_system_info, full_system_info=full_system_info)

!> some prints to help debugging
!print *, "FOB_SH| EXT PRESS:", ext_pres
!print *, "active sys info", active_system_info(1, :)
!print *, "active sys info", active_system_info(2, :)
!print *, "full sys info", full_system_info(1, :)
!print *, "full sys info", full_system_info(2, :)

           IF (ALLOCATED(thermo%pot_list)) THEN
           !Enter this part only if the active atoms DECOMP has been constructed,
           !otherwise you MUST provide N FORCE_EVALS one for each site energy

               site_energy_section => section_vals_get_subs_vals(adiab_section,"METHOD_SITE_ENERGIES")
               ! this keyword determines if the number of states is larger then
               ! the number of molecules in decomp (e.g. in case of CT states)
               CALL section_vals_val_get(site_energy_section,"MULTI_STATES",l_val=multistate)
               
               IF (multistate) THEN 
                   fe_block_keyword  = .False.
                   cr_block_keyword  = .False.

                   CALL section_vals_val_get(site_energy_section,"BLOCK_TYPE",c_val=block_type_keyword) 
                   SELECT CASE(block_type_keyword)
                   CASE("CT")   
                      fe_block_keyword  = .False.
                      cr_block_keyword  = .False.
                      !OLD subroutine that does the same as below 
                      !CALL ct_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
                      !              intra_forces_global, global_forces, my_energies, &
                      !              active_system_info, mixed_env, my_itimes, my_mol_index)

                      CALL multi_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
                                    intra_forces_global, global_forces, my_energies, &
                                    active_system_info, mixed_env, my_itimes, my_mol_index, &
                                    fe_block_keyword, cr_block_keyword) 


                   CASE("FE")  ! this could be used also for just hole or electron like done so far 
                      CALL add_energies_forces_mem_saver_V2( energies_1_mol_list, energies, &
                                                      intra_forces_global, &
                                                      global_forces, my_energies, &
                                                      active_system_info, &
                                                       mixed_env, my_itimes)
                   CASE("CT-FE")   
                      fe_block_keyword  = .True.
                      IF (nforce_eval .ne. 3) THEN
                         CALL cp__b("force_env_methods.F",1268,"TRY FORCE_EVAL_ORDER 1..4 FOR CT-FE BLOCK_TYPE")
                      ENDIF
                      cr_block_keyword  = .False.
                      CALL multi_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
                                    intra_forces_global, global_forces, my_energies, &
                   ...
                   CASE("FECT-1-PHASE")
                           fe_block_keyword = .True.
                      IF (nforce_eval .ne. 4) THEN
                         CALL cp__b("force_env_methods.F",1278,"MUST HAVE 4 FORCE EVAL SECTIONS FOR SINGLE PHASE FECT")
                      ENDIF
                      cr_block_keyword  = .False.

!                      call test_diagonalisation(C,E)
                      !open(454,file="single-phase-ff.txt",position="append")
                      !write(454,*) energies_1_mol_list(1,:)
                      !write(454,*) energies_1_mol_List(3,:)
                      !close(454)

                      CALL single_phase_energies_forces(adiab_section,energies_1_mol_list, energies, &
                                    intra_forces_global, global_forces, my_energies, &
                                    active_system_info, mixed_env, my_itimes, my_mol_index, &
                                    fe_block_keyword, cr_block_keyword) 

                      open(555,file="single-phase-se.txt",position="append")
                      write(555,*) my_energies
                      close(555)
                    CASE("CT-FE-CR")
                      fe_block_keyword  = .True.
                      cr_block_keyword  = .True.
                      CALL multi_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
                                    intra_forces_global, global_forces, my_energies, &
                                    active_system_info, mixed_env, my_itimes, my_mol_index, &
                                    fe_block_keyword, cr_block_keyword) 
                   CASE("CT-CR")
                      ...
                      CALL multi_block_energies_forces(adiab_section,energies_1_mol_list, energies, &
                                    ...
                                    fe_block_keyword, cr_block_keyword) 
                   CASE DEFAULT
                      CALL cp__b("force_env_methods.F",1311,"Site energy construction not specified")
                   END SELECT

               ELSE
                !> This does add-sub scheme for Hamiltoninans where num of
                !states = num of molecules (e.g. AOM and TRESP)
                      !open(455,file="fobsh-ff.txt",position="append")
                      !write(455,*) energies_1_mol_list(1,:)
                      !write(455,*) energies_1_mol_list(2,:)
                      !close(455)

                   CALL add_energies_forces_mem_saver_V2( energies_1_mol_list, energies, intra_forces_global, &
                                                      global_forces, my_energies, &
                                                      active_system_info, &
                                                       mixed_env, my_itimes)
                 ...
                 !OLD subroutine
                !CALL add_energies_forces_mem_saver( energies_1_mol_list, energies, intra_forces_global, &
                !                                      my_mol_index, &
                !                                      global_forces, my_energies, &
                !                            molecule_kind_set, local_molecules, molecule_set, mixed_env, my_itimes)
               END IF 

                CALL adiab_fssh_routines(particles_mix, virial_mix, results_mix,   &
                        my_energies, virials, results, nforce_eval,&
                        map_index, mapping_section,adiab_section,mixed_env,my_itimes, &
                        adiab_keyword, mixed_energy%pot,atomic_kinds, &
                        local_particles, my_mol_index, ext_pres, active_system_info, full_system_info)
           ELSE
                !COMMENT_SG: these two additional ways of combining force_evals
                !should be DEPRECATED and removed but they are kept for testing
                !purposes.
                CALL section_vals_val_get(optimization_section, "DO_SPEEDUP_LJ", l_val=do_speedup_lj)
                CALL section_vals_val_get(optimization_section, "DO_SPEEDUP_INTRA", l_val=do_speedup_intra)

                CALL add_energies_forces(energies, global_forces, my_energies, do_speedup_lj, &
                                                              do_speedup_intra, mixed_env, my_itimes)
                CALL adiab_fssh_routines(particles_mix, virial_mix, results_mix,   &
                        my_energies, virials, results, nforce_eval,&
                        map_index, mapping_section,adiab_section,mixed_env,my_itimes, &
                        adiab_keyword, mixed_energy%pot,atomic_kinds, &
                        local_particles, my_mol_index, ext_pres, active_system_info, full_system_info)
           END IF
           !print*, "AC_energies", my_energies 

           DEALLOCATE(my_energies)
    CASE DEFAULT
       CALL cp__b("force_env_methods.F",1362,"")
    END SELECT
    ...
    DEALLOCATE(energies)
    IF (ASSOCIATED(ext_pres)) DEALLOCATE(ext_pres)
    IF (ALLOCATED(energies_1_mol_list)) DEALLOCATE(energies_1_mol_list)
    IF (ALLOCATED(intra_forces_global)) DEALLOCATE(intra_forces_global)
