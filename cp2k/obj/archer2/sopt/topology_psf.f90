# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_psf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_psf.F"
!-----------------------------------------------------------------------------!
...
# 51 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_psf.F" 2
  ...
  SUBROUTINE read_topology_psf (filename,topology,para_env,subsys_section, psf_type)
    ...
    DO WHILE (parser_test_next_token(parser)/="EOL")
       ...
       CASE ("EXT")
          IF (psf_type==do_conn_psf) THEN
            ...
            c_int      = 'I10'
          ELSE
            CALL cp__b("topology_psf.F",136,"PSF_INFO| "//field(1:3)//" :: not available for UPSF format!")
          ENDIF
       CASE DEFAULT
          CALL cp__b("topology_psf.F",139,"PSF_INFO| "//field(1:3)//" :: Unimplemented keyword in CP2K PSF/UPSF format!")
       END SELECT
    END DO
    ...
    ELSE
       ...
       IF(natom_prev+natom>topology%natoms)&
          CALL cp_abort(cp__l("topology_psf.F",154),&
    ...
    ELSE
       ...
       IF ( ANY(conn_info%bond_a(nbond_prev+1:)<=0) .OR. &
          ...
          CALL cp__b("topology_psf.F",246,"topology_read, invalid bond")
       END IF
...
9   CONTINUE
    ...
    CALL cp__b("topology_psf.F",408,"Error while reading PSF data!")
...
! *****************************************************************************
  SUBROUTINE  idm_psf(topology,section,subsys_section)
    ...
    INTEGER                                  :: handle, i, iend, iend1, &
                                                istart, istart1, item, iw, j, &
                                                mol_id, n_rep, natom, nbond, &
                                                nimpr, noe, nphi, ntheta
    ...
    IF (explicit) THEN
       ...
       IF (explicit) THEN
          ...
          DO i = 2, natom
             ...
             IF(.NOT.(iend1-istart1+1==noe))CALL cp__a("topology_psf.F",646)
          ...
          istart1 = MINVAL(wrk(istart:iend))
          iend1   = MAXVAL(wrk(istart:iend))
          IF(.NOT.(iend1-istart1+1==noe))CALL cp__a("topology_psf.F",655)
          atom_info%id_molname(istart1:iend1)=str2id(s2s("MOL"//cp_to_string(item)))
          ! Deallocate bonds
          DO i=1,natom
             DEALLOCATE(ex_bond_list(i)%array1)
