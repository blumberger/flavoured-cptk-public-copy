# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_data_operations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_data_operations.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/block/dbcsr_data_operations.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (ASSOCIATED (source_area%d), dbcsr_fatal_level,&
         dbcsr_caller_error, routineN, "Attempt to copy unassigned data",111)
    IF(source_area%d%refcount.LE.0)&
       CALL cp__w("dbcsr/block/dbcsr_data_operations.F",113,"Attempt to copy unheld data")
    shallow_copy = .FALSE.
    ...
    ELSE
       ...
       CASE default
          CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,&
               routineN, "Invalid data type",149)
       END SELECT
    ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    IF(.NOT.ASSOCIATED(source_area%d))&
       CALL cp__w("dbcsr/block/dbcsr_data_operations.F",186,"Attempt to copy unassigned data")
    IF(source_area%d%refcount.LE.0)&
       CALL cp__w("dbcsr/block/dbcsr_data_operations.F",188,"Attempt to copy unheld data")
    ...
    IF (n .GT. 0) THEN
       ...
       CASE (dbcsr_type_real_8)
          ...
          CASE default
             CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error, &
                  routineN, "Invalid data type",226)
          END SELECT
       CASE (dbcsr_type_real_4)
          ...
          CASE default
             CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error, &
                  routineN, "Invalid data type",253)
          END SELECT
       CASE (dbcsr_type_complex_8)
       ...
       CASE (dbcsr_type_complex_4)
          ...
          CASE default
             CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error, &
                  routineN, "Invalid data type",307)
          END SELECT
       CASE default
          CALL dbcsr_assert (.FALSE., dbcsr_failure_level, dbcsr_caller_error, &
               routineN, "Invalid data type",311)
       END SELECT
    ENDIF
...
! Analyze parameters
    ...
    CALL dbcsr_assert(SIZE(blk_p),'GE',nblks, dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN, 'Destination blk_p too small.',364)
    IF (nblks .GE. 1) &
       ...
       DO blk = row_p(row)+1, row_p(row+1)
          IF (old_blk_p(blk) .NE. 0) THEN
             ...
             CALL dbcsr_assert(&
                  ...
                  'Destination data space is too small.',381)
             IF (.NOT. trb) THEN
