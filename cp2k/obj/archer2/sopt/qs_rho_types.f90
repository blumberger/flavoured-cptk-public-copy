# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_types.F"
!-----------------------------------------------------------------------------!
...
# 29 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_rho_types.F" 2
...
! *****************************************************************************
  TYPE qs_rho_type
     ...
     TYPE(cp_dbcsr_p_type), DIMENSION(:), POINTER   :: rho_ao_im => Null()
...
! *****************************************************************************
 SUBROUTINE qs_rho_create(rho)
   ...
   IF(ASSOCIATED(rho)) CALL cp__b("qs_rho_types.F",113,"rho already associated")
...
! *****************************************************************************
  SUBROUTINE qs_rho_retain(rho_struct)
    ...
    IF(.NOT.(ASSOCIATED(rho_struct)))CALL cp__a("qs_rho_types.F",136)
    IF(.NOT.(rho_struct%ref_count>0))CALL cp__a("qs_rho_types.F",137)
...
! *****************************************************************************
  SUBROUTINE qs_rho_release(rho_struct)
    ...
    IF (ASSOCIATED(rho_struct)) THEN
       IF(.NOT.(rho_struct%ref_count>0))CALL cp__a("qs_rho_types.F",157)
       rho_struct%ref_count=rho_struct%ref_count-1
...
! *****************************************************************************
!> \brief Deallocates all components, whithout deallocating rho_struct itself.
!> \param rho_struct ...
!> \author Ole Schuett
! *****************************************************************************
  SUBROUTINE qs_rho_clear(rho_struct)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'qs_rho_clear', &
      routineP = moduleN//':'//routineN
    ...
    IF (ASSOCIATED(rho_struct%rho_ao_im))&
       CALL cp_dbcsr_deallocate_matrix_set(rho_struct%rho_ao_im)
...
! *****************************************************************************
  SUBROUTINE qs_rho_get(rho_struct, rho_ao, rho_ao_im, rho_ao_kp, rho_r, drho_r, &
    ...
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
      OPTIONAL, POINTER                      :: rho_ao, rho_ao_im
    TYPE(cp_dbcsr_p_type), DIMENSION(:, :), &
      OPTIONAL, POINTER                      :: rho_ao_kp
    ...
    IF(.NOT.(ASSOCIATED(rho_struct)))CALL cp__a("qs_rho_types.F",287)
    IF(.NOT.(rho_struct%ref_count>0))CALL cp__a("qs_rho_types.F",288)
...
! *****************************************************************************
  SUBROUTINE qs_rho_set(rho_struct, rho_ao, rho_ao_im, rho_ao_kp, rho_r, drho_r, &
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'qs_rho_set', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(rho_struct)))CALL cp__a("qs_rho_types.F",363)
    IF(.NOT.(rho_struct%ref_count>0))CALL cp__a("qs_rho_types.F",364)
