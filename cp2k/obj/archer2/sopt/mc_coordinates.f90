# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_coordinates.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_coordinates.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/mc/mc_coordinates.F" 2
...
! get the particle coordinates and the cell length
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,108,&
...
! figure out which molecule number we're on
      ...
      ENDDO
      IF(molecule_number == 0) CALL cp_abort(cp__l("motion/mc/mc_coordinates.F",453),&
      ...
      IF(lremove) THEN
         ...
         IF(loverlap_array(1)) THEN
            IF(ionode) THEN
               ...
               ENDDO
            ENDIF
            CALL cp__b("motion/mc/mc_coordinates.F",468,'CBMC swap move found an overlap in the old config')
         ENDIF
      ENDIF
...
! in the acceptance rule
         IF(ionode) THEN
            ...
            ENDDO
         ENDIF
         CALL cp__b("motion/mc/mc_coordinates.F",569,'CBMC swap move found a bad old weight')
      ENDIF
      all_overlaps=.TRUE.
...
! this should never be the case, but I'm testing something
         IF(lremove) THEN
            IF(ionode) THEN
               ...
               ENDDO
            ENDIF
            CALL cp__b("motion/mc/mc_coordinates.F",603,'CBMC swap move found all overlaps for the remove config')
         ENDIF
...
! make sure a configuration was choosen
      IF(choosen == 0)&
         CALL cp__b("motion/mc/mc_coordinates.F",617,'CBMC swap move failed to select config')
...
! make sure things are good
      ...
      IF(.NOT.(start_atom/=0))CALL cp__a("motion/mc/mc_coordinates.F",880)
      IF(.NOT.(box_number/=0))CALL cp__a("motion/mc/mc_coordinates.F",881)
      IF(.NOT.(molecule_type/=0))CALL cp__a("motion/mc/mc_coordinates.F",882)
