# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/library_tests.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/library_tests.F"
!-----------------------------------------------------------------------------!
...
MODULE library_tests
  ...
  USE eri_mme_test,                    ONLY: eri_mme_perf_acc_test
...
# 110 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/library_tests.F" 2
...
! *****************************************************************************
  SUBROUTINE lib_test ( root_section, para_env, globenv)
    ...
    TYPE(section_vals_type), POINTER :: cp_dbcsr_test_section, &
      cp_fm_gemm_test_section, eigensolver_section, eri_mme_test_section, &
      pw_transfer_section, rs_pw_transfer_section
    ...
    DO iall = 1, 100
       ...
       IF ( stat == 0 ) THEN
          DO it = 1, 3
             ...
             IF (ierr /= 0) CALL stop_memory(routineN,moduleN,620,&
                                             "ra",dp_size*len*len*len)
             ALLOCATE ( ca ( len, len, len ), STAT = ierr )
             IF (ierr /= 0) THEN
                CALL stop_memory(routineN,moduleN,624,&
          ...
          n = len
          ALLOCATE ( ra ( len, len, len ), STAT = ierr )
          IF (ierr /= 0) CALL stop_memory(routineN,moduleN,662,&
                                          "ra",dp_size*len*len*len)
          ALLOCATE ( ca ( len, len, len ), STAT = ierr )
          IF (ierr /= 0) CALL stop_memory(routineN,moduleN,665,&
                                          "ca",2*dp_size*len*len*len)
          ALLOCATE ( cb ( len, len, len ), STAT = ierr )
          IF (ierr /= 0) CALL stop_memory(routineN,moduleN,668,&
                                          "cb",2*dp_size*len*len*len)
          ALLOCATE ( cc ( len, len, len ), STAT = ierr )
          IF (ierr /= 0) CALL stop_memory(routineN,moduleN,671,&
    ...
    DO i_rep=1,n_rep
       ...
       DO i_layout=1,SIZE(layouts,2)
          ...
          IF ( em > toler .OR. et > toler ) THEN
             CALL cp__w("library_tests.F",1030,"The FFT results are not accurate ... starting debug pw_transfer")
