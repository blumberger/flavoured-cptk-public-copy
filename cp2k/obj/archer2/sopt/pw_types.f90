# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_types.F"
!-----------------------------------------------------------------------------!
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/pw_types.F" 2
...
! *****************************************************************************
  SUBROUTINE pw_retain(pw)
    ...
    IF(.NOT.(ASSOCIATED(pw)))CALL cp__a("pw/pw_types.F",93)
    IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_types.F",94)
...
SUBROUTINE pw_release(pw)
  ...
  IF (ASSOCIATED(pw)) THEN
     IF(.NOT.(pw%ref_count>0))CALL cp__a("pw/pw_types.F",114)
     pw%ref_count=pw%ref_count-1
     IF (pw%ref_count==0) THEN
        ...
        CASE(NODATA)
        CASE default
           CALL cp__b("pw/pw_types.F",134,"unknown data type "//cp_to_string(pw%in_use))
        END SELECT
...
SUBROUTINE pw_create(pw, pw_grid, use_data, in_space, cr3d_ptr)
  ...
  IF(.NOT.(.NOT.ASSOCIATED(pw)))CALL cp__a("pw/pw_types.F",172)
  ALLOCATE(pw)
  ...
  IF (PRESENT(cr3d_ptr)) THEN
     IF (ASSOCIATED(cr3d_ptr)) THEN
        IF(.NOT.(use_data==REALDATA3D))CALL cp__a("pw/pw_types.F",187)
     END IF
  END IF
     ...
     IF (PRESENT(cr3d_ptr)) THEN
        IF (ASSOCIATED(cr3d_ptr)) THEN
           IF (ALL(bounds(1,:) <= bounds(2,:))) THEN
             IF(.NOT.(ALL(LBOUND(cr3d_ptr)==bounds(1,:))))CALL cp__a("pw/pw_types.F",216)
             IF(.NOT.(ALL(UBOUND(cr3d_ptr)==bounds(2,:))))CALL cp__a("pw/pw_types.F",217)
           END IF
           pw%cr3d => cr3d_ptr
        END IF
     END IF
  ...
  CASE(NODATA)
  CASE default
     CALL cp__b("pw/pw_types.F",236,"unknown data type")
  END SELECT
