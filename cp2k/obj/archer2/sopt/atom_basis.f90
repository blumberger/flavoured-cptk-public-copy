# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_basis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_basis.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_basis.F" 2
    ...
    DO in = 1, n_rep
      DO im = 1, n_meth
        ...
        IF ( pp_calc ) THEN
          ...
          IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_basis.F",200)
        ELSE
          state%core=0._dp
          ...
          IF(.NOT.(ALL(state%maxn_calc(:) >= state%maxn_occ)))CALL cp__a("atom_basis.F",207)
        END IF
        ...
        ELSE
          CALL cp__b("atom_basis.F",229,"METHOD_TYPE and MULTIPLICITY are incompatible")
        END IF
