# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_utils.F"
!-----------------------------------------------------------------------------!
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/thermostat_utils.F" 2
...
!    map_loc_thermo_gen=>thermostat_info%map_loc_thermo_gen
    ...
    CASE DEFAULT
        CALL cp__b("motion/thermostat/thermostat_utils.F",290,'Unknown ensemble')
...
! use thermolist ( ipart ) to get global indexing correct
    ...
    IF(.NOT.(stat==0))CALL cp__a("motion/thermostat/thermostat_utils.F",361)
    natom_local = 0
    DO ikind = 1, SIZE(molecule_kind_set)
       nmol_per_kind = local_molecules%n_el(ikind)
       DO imol = 1, nmol_per_kind
    ...
    END DO
        ...
        IF ( nointer ) THEN
           ...
           DO ikind = 1, nkind
              ...
              ELSE
                 CALL cp__b("motion/thermostat/thermostat_utils.F",400,'Invalid region setup')
              END IF
           END DO
        ELSE
...
! *****************************************************************************
  SUBROUTINE get_adiabatic_region_info(region_sections, sum_of_thermostats,&
    ...
    INTEGER :: first_atom, i, ig, ikind, ilist, imol, ipart, itherm, jg, &
      last_atom, mregions, n_rep, nregions, output_unit
    ...
    ! IF(.NOT.(.NOT.(ASSOCIATED(map_loc_thermo_gen))))CALL cp__a("motion/thermostat/thermostat_utils.F",464)
    ...
    DO ig = 1, mregions
       ...
       DO jg = 1, n_rep
          ...
          DO i = 1, SIZE(tmplist)
             ipart = tmplist(i)
             IF(.NOT.(((ipart>0).AND.(ipart<=particles%n_els))))CALL cp__a("motion/thermostat/thermostat_utils.F",477)
             IF (thermolist(ipart)==HUGE(0)) THEN
                itherm = itherm + 1
                thermolist(ipart) = itherm
             ELSE
                CALL cp__b("motion/thermostat/thermostat_utils.F",482,"")
             END IF
          END DO
       END DO
                   ...
                   DO imol=1,SIZE(molecule_kind%molecule_list)
                      ...
                      DO ipart = first_atom, last_atom
                         IF (thermolist(ipart)==HUGE(0)) THEN
                            ...
                            thermolist(ipart) = itherm
                         ELSE
                            CALL cp__b("motion/thermostat/thermostat_utils.F",501,"")
                         END IF
                      END DO
                   END DO
                END IF
    ...
    IF(.NOT.(.NOT.ALL(thermolist==HUGE(0))))CALL cp__a("motion/thermostat/thermostat_utils.F",515)
...
! *****************************************************************************
  SUBROUTINE setup_thermostat_info(thermostat_info, molecule_kind_set, local_molecules,&
    ...
    SELECT CASE(ensemble)
    CASE DEFAULT
       CALL cp__b("motion/thermostat/thermostat_utils.F",605,'Unknown ensemble')
       ...
       IF (check) THEN
          ...
          CASE(do_region_defined)
             ...
             IF(sum_of_thermostats<1)&
                CALL cp_abort(cp__l("motion/thermostat/thermostat_utils.F",644),&
          ...
          IF ( nointer ) THEN
             ...
             DO ikind = 1, nkind
                ...
                ELSE IF (region==do_region_massive) THEN
                   number = number + 3*nmol_local*natom
                ELSE
                   CALL cp__b("motion/thermostat/thermostat_utils.F",671,'Invalid region setup')
                END IF
             END DO
          ELSE
             ! REPlicated thermostats, INTERacting via communication
...
! *****************************************************************************
  SUBROUTINE get_defined_region_info(region_sections, number, sum_of_thermostats,&
    ...
    INTEGER :: first_atom, i, ig, ikind, ilist, imol, ipart, jg, last_atom, &
      mregions, n_rep, natom_local, nmol_per_kind, nregions, output_unit
    ...
    logger      => cp_get_default_logger()
    output_unit= cp_logger_get_default_io_unit(logger)
    IF(.NOT.(.NOT.(ASSOCIATED(map_loc_thermo_gen))))CALL cp__a("motion/thermostat/thermostat_utils.F",746)
    CALL section_vals_get(region_sections, n_repetition=nregions)
    ALLOCATE(thermolist(particles%n_els))
    thermolist = HUGE(0)
    molecule_set => molecules%els
    ...
    DO ig = 1, mregions
       ...
       DO jg = 1, n_rep
          ...
          DO i = 1, SIZE(tmplist)
             ipart = tmplist(i)
             IF(.NOT.(((ipart>0).AND.(ipart<=particles%n_els))))CALL cp__a("motion/thermostat/thermostat_utils.F",758)
             IF (thermolist(ipart)==HUGE(0)) THEN
                thermolist(ipart) = ig
             ELSE
                CALL cp__b("motion/thermostat/thermostat_utils.F",762,"")
             END IF
          END DO
       END DO
       CALL section_vals_val_get(region_sections,"MOLNAME", i_rep_section=ig,n_rep_val=n_rep)
                   ...
                   DO imol=1,SIZE(molecule_kind%molecule_list)
                      ...
                      DO ipart = first_atom, last_atom
                         IF (thermolist(ipart)==HUGE(0)) THEN
                            thermolist(ipart) = ig
                         ELSE
                            CALL cp__b("motion/thermostat/thermostat_utils.F",780,"")
                         END IF
                      END DO
                   END DO
                END IF
    ...
    IF (ANY(thermolist==HUGE(0))) THEN
       ...
       DEALLOCATE(tmp)
    END IF
    IF(.NOT.(ALL(thermolist/=HUGE(0))))CALL cp__a("motion/thermostat/thermostat_utils.F",814)
          ...
          CASE(do_constr_atomic)
             DO i = 1, SIZE(atom_index1)
                ...
                IF (thermolist(ipart)==HUGE(0)) THEN
                   thermolist(ipart) = ig
                ELSE
                   CALL cp_abort(cp__l("motion/thermostat/thermostat_utils.F",913),&
                ...
                IF (ANY(atom_index1 >= first_atom .AND. atom_index1 <= last_atom)) THEN
                   DO ipart = first_atom, last_atom
                      IF (thermolist(ipart)==HUGE(0)) THEN
                         thermolist(ipart) = ig
                      ELSE
                         CALL cp_abort(cp__l("motion/thermostat/thermostat_utils.F",929),&
                              'One atom ('//cp_to_string(ipart)//') of the '//&
                              TRIM(label1)//' was already assigned to'//&
                              ' the thermostatting region Nr.'//cp_to_string(thermolist(ipart))//&
                              '. Please check the input for inconsistencies!')
  ...
  SUBROUTINE vel_rescale_particles(map_info, molecule_kind_set, molecule_set,&
    ...
    IF (present_vel) THEN
       IF (shell_adiabatic) THEN
          IF(.NOT.(PRESENT(shell_vel)))CALL cp__a("motion/thermostat/thermostat_utils.F",1189)
          IF(.NOT.(PRESENT(core_vel)))CALL cp__a("motion/thermostat/thermostat_utils.F",1190)
       END IF
    ELSE
       IF (shell_adiabatic) THEN
          IF(.NOT.(PRESENT(shell_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1194)
          IF(.NOT.(PRESENT(core_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1195)
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE ke_region_shells(map_info, particle_set, atomic_kind_set,&
    ...
    IF (present_vel) THEN
       IF(.NOT.(PRESENT(core_vel)))CALL cp__a("motion/thermostat/thermostat_utils.F",1290)
    ELSE
       IF(.NOT.(PRESENT(shell_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1292)
       IF(.NOT.(PRESENT(core_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1293)
    END IF
...
! *****************************************************************************
  SUBROUTINE vel_rescale_shells(map_info, atomic_kind_set, particle_set, local_particles, &
    ...
    ! Preliminary checks for consistency usage
    IF (present_vel) THEN
       IF(.NOT.(PRESENT(shell_vel)))CALL cp__a("motion/thermostat/thermostat_utils.F",1372)
       IF(.NOT.(PRESENT(core_vel)))CALL cp__a("motion/thermostat/thermostat_utils.F",1373)
    ELSE
       IF(.NOT.(PRESENT(shell_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1375)
       IF(.NOT.(PRESENT(core_particle_set)))CALL cp__a("motion/thermostat/thermostat_utils.F",1376)
    END IF
    ii = 0
    nparticle_kind = SIZE(atomic_kind_set)
  ...
  SUBROUTINE get_nhc_energies ( nhc, nhc_pot, nhc_kin, para_env, array_kin, array_pot)
    ...
    IF (PRESENT(array_pot)) THEN
       IF (ASSOCIATED(array_pot)) THEN
          IF(.NOT.(SIZE(array_pot)==number))CALL cp__a("motion/thermostat/thermostat_utils.F",1473)
       ELSE
    ...
    IF (PRESENT(array_kin)) THEN
       IF (ASSOCIATED(array_kin)) THEN
          IF(.NOT.(SIZE(array_kin)==number))CALL cp__a("motion/thermostat/thermostat_utils.F",1481)
       ELSE
...
! *****************************************************************************
  SUBROUTINE get_kin_energies(map_info,loc_num,glob_num,thermo_energy,thermostat_kin, &
    ...
    IF (PRESENT(array_pot)) THEN
       IF (ASSOCIATED(array_pot)) THEN
          IF(.NOT.(SIZE(array_pot)==number))CALL cp__a("motion/thermostat/thermostat_utils.F",1541)
       ELSE
          ALLOCATE(array_pot(number))
       ENDIF
       array_pot=0.0_dp
    ...
    IF (PRESENT(array_kin)) THEN
       IF (ASSOCIATED(array_kin)) THEN
          IF(.NOT.(SIZE(array_kin)==number))CALL cp__a("motion/thermostat/thermostat_utils.F",1549)
       ELSE
          ALLOCATE(array_kin(number))
       ENDIF
       array_kin=akin
  ...
  SUBROUTINE get_temperatures ( map_info, loc_num, glob_num, nkt, dof, para_env,&
    ...
    IF (PRESENT(array_temp)) THEN
       IF (ASSOCIATED(array_temp)) THEN
          IF(.NOT.(SIZE(array_temp)==number))CALL cp__a("motion/thermostat/thermostat_utils.F",1618)
       ELSE
  ...
  SUBROUTINE get_thermostat_energies ( thermostat, thermostat_pot, thermostat_kin, para_env,&
    ...
    IF (ASSOCIATED(thermostat)) THEN
       IF      (thermostat%type_of_thermostat==do_thermo_nose) THEN
          ...
          IF(.NOT.(ASSOCIATED(thermostat%nhc)))CALL cp__a("motion/thermostat/thermostat_utils.F",1660)
       ...
       ELSE IF (thermostat%type_of_thermostat==do_thermo_csvr) THEN
          ...
          IF(.NOT.(ASSOCIATED(thermostat%csvr)))CALL cp__a("motion/thermostat/thermostat_utils.F",1665)
       ...
       ELSE IF (thermostat%type_of_thermostat==do_thermo_gle) THEN
          ...
          IF(.NOT.(ASSOCIATED(thermostat%gle)))CALL cp__a("motion/thermostat/thermostat_utils.F",1677)
...
! *****************************************************************************
  SUBROUTINE get_region_temperatures ( thermostat, tot_temperature, para_env, array_temp)
    ...
    IF (ASSOCIATED(thermostat)) THEN
       IF      (thermostat%type_of_thermostat==do_thermo_nose) THEN
          ! Energy associated with the Nose-Hoover thermostat
          IF(.NOT.(ASSOCIATED(thermostat%nhc)))CALL cp__a("motion/thermostat/thermostat_utils.F",1718)
       ...
       ELSE IF (thermostat%type_of_thermostat==do_thermo_csvr) THEN
          ! Energy associated with the CSVR thermostat
          IF(.NOT.(ASSOCIATED(thermostat%csvr)))CALL cp__a("motion/thermostat/thermostat_utils.F",1731)
       ...
       ELSE IF (thermostat%type_of_thermostat==do_thermo_al) THEN
          ...
          IF(.NOT.(ASSOCIATED(thermostat%al)))CALL cp__a("motion/thermostat/thermostat_utils.F",1745)
       ...
       ELSE IF (thermostat%type_of_thermostat==do_thermo_gle) THEN
          ! Energy associated with the GLE thermostat
          IF(.NOT.(ASSOCIATED(thermostat%gle)))CALL cp__a("motion/thermostat/thermostat_utils.F",1759)
    ...
    DO i = 1, number
       ...
       IF (ncheck/=0) THEN
          ...
          DO icheck = 1, para_env%num_pe
             ...
             END IF
          END DO
          IF(.NOT.(ncheck==SIZE(work2)))CALL cp__a("motion/thermostat/thermostat_utils.F",1932)
          IF(.NOT.(ALL(work2==work2(1))))CALL cp__a("motion/thermostat/thermostat_utils.F",1933)
    ...
    DO i = 1, number2
       ...
       IF (ncheck/=0) THEN
          ...
          DO icheck = 1, para_env%num_pe
             ...
             END IF
          END DO
          IF(.NOT.(ncheck==SIZE(work2,2)))CALL cp__a("motion/thermostat/thermostat_utils.F",1982)
          DO j = 1, ncheck
             IF(.NOT.(ALL(work2(:,j)==work2(:,1))))CALL cp__a("motion/thermostat/thermostat_utils.F",1984)
          END DO
          array(:,i) = work2(:,1)
          DEALLOCATE(work2)
       END IF
