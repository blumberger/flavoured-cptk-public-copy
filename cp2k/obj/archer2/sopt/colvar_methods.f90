# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE colvar_methods
  ...
  USE colvar_types,                    ONLY: &
       HBP_colvar_id, Wc_colvar_id, angle_colvar_id, colvar_create, &
       colvar_setup, colvar_type, combine_colvar_id, coord_colvar_id, &
       dfunct_colvar_id, dist_colvar_id, distance_from_path_colvar_id, &
       do_clv_fix_point, do_clv_geo_center, do_clv_x, do_clv_xy, do_clv_xz, &
       do_clv_y, do_clv_yz, do_clv_z, eval_point_der, eval_point_mass, &
       eval_point_pos, gyration_colvar_id, hydronium_colvar_id, &
       mindist_colvar_id, plane_def_atoms, plane_def_vec, &
       plane_distance_colvar_id, plane_plane_angle_colvar_id, &
       population_colvar_id, qparm_colvar_id, reaction_path_colvar_id, &
       ring_puckering_colvar_id, rmsd_colvar_id, rotation_colvar_id, &
       torsion_colvar_id, u_colvar_id, xyz_diag_colvar_id, &
...
# 101 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/colvar_methods.F" 2
...
! *****************************************************************************
  RECURSIVE SUBROUTINE colvar_read(colvar, icol, colvar_section, para_env)
    ...
    INTEGER :: handle, i, iatm, icomponent, iend, ifunc, ii, isize, istart, &
      iw, iw1, j, k, kk, n_var, n_var_k, ncol, ndim, nr_frame, v_count
    ...
    CALL section_vals_get(hydronium_section,    explicit=my_subsection( 9))
    ...
    IF(.NOT.(COUNT(my_subsection) == 1))CALL cp__a("colvar_methods.F",236)
    IF(.NOT.(.NOT.ASSOCIATED(colvar)))CALL cp__a("colvar_methods.F",237)
    ...
    ELSE IF (my_subsection(4)) THEN
       ...
       ELSE
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",287)
          DO k = 1, n_var
       ...
       ELSE
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",317)
          DO k = 1, n_var
       ...
       IF (n_var /= 0 .OR. n_var_k /= 0) THEN
          ...
          ELSE
             ! KINDS
             CALL section_vals_val_get(coordination_section,"KINDS_TO_B",n_rep_val=n_var_k)
             IF(.NOT.(n_var_k>0))CALL cp__a("colvar_methods.F",354)
             DO k = 1, n_var_k
    ...
    ELSE IF (my_subsection(5)) THEN
       ...
       IF(.NOT.(SIZE(iatms) == 3))CALL cp__a("colvar_methods.F",389)
    ...
    ELSE IF (my_subsection(9)) THEN
       ! Hydronium
       CALL colvar_create(colvar,hydronium_colvar_id)
       NULLIFY(colvar%hydronium_param%i_oxygens)
       NULLIFY(colvar%hydronium_param%i_hydrogens)

       CALL section_vals_val_get(hydronium_section,"OXYGENS",n_rep_val=n_var)
       ndim = 0
       DO k = 1, n_var
         CALL section_vals_val_get(hydronium_section,"OXYGENS",i_vals=iatms)
       ...
       CALL section_vals_val_get(hydronium_section,"HYDROGENS",n_rep_val=n_var)
       ndim = 0
       DO k = 1, n_var
         CALL section_vals_val_get(hydronium_section,"HYDROGENS",i_vals=iatms)
       ...
       CALL section_vals_val_get(hydronium_section,"ROO",r_val=colvar%hydronium_param%r_OO)
       CALL section_vals_val_get(hydronium_section,"ROH",r_val=colvar%hydronium_param%r_OH)
       CALL section_vals_val_get(hydronium_section,"pNH",i_val=colvar%hydronium_param%pnh)
       CALL section_vals_val_get(hydronium_section,"qNH",i_val=colvar%hydronium_param%qnh)
       CALL section_vals_val_get(hydronium_section,"pNO",i_val=colvar%hydronium_param%pno)
       CALL section_vals_val_get(hydronium_section,"qNO",i_val=colvar%hydronium_param%qno)
       CALL section_vals_val_get(hydronium_section,"p",i_val=colvar%hydronium_param%p)
       CALL section_vals_val_get(hydronium_section,"q",i_val=colvar%hydronium_param%q)
       CALL section_vals_val_get(hydronium_section,"NH",r_val=colvar%hydronium_param%nh)
       CALL section_vals_val_get(hydronium_section,"LAMBDA",r_val=colvar%hydronium_param%lambda)
    ELSE IF(my_subsection(10) .OR. my_subsection(16))THEN
       ...
       IF(colvar%reaction_path_param%dist_rmsd .AND. colvar%reaction_path_param%rmsd)THEN
          CALL cp__b("colvar_methods.F",495,"CV REACTION PATH: only one between DISTANCES_RMSD and RMSD can be used ")
       END IF
       IF(colvar%reaction_path_param%dist_rmsd .OR. colvar%reaction_path_param%rmsd) THEN
          ...
          ELSE IF (colvar%reaction_path_param%subset==rmsd_list) THEN
             ...
             ELSE
                CALL cp__b("colvar_methods.F",525,"CV REACTION PATH: if SUBSET_TYPE=LIST a list of atoms needs to be provided ")
             END IF
          END IF
       ...
       ELSE
          ...
          IF (ncol>0) THEN
             ...
             ENDDO
          ELSE
             CALL cp__b("colvar_methods.F",540,"CV REACTION PATH: the number of CV to define the path must be >0 ")
          ENDIF
          ...
          IF(.NOT.(check))CALL cp__a("colvar_methods.F",555)
          CALL initf(ncol)
          DO i=1,ncol
          ...
          IF(iw1>0)THEN
             ...
             IF(.NOT.(ncol ==SIZE(grid_sp)))CALL cp__a("colvar_methods.F",577)
    ...
    ELSE IF(my_subsection(11))THEN
       ...
       IF(SIZE(my_par)/=ncol)&
          CALL cp_abort(cp__l("colvar_methods.F",637),&
    ...
    ELSE IF (my_subsection(12)) THEN
       ...
       ELSE
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",686)
          DO k = 1, n_var
       ...
       ELSE
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",716)
          DO k = 1, n_var
    ...
    ELSE IF (my_subsection(13)) THEN
       ...
       IF(n_var/=2)&
          CALL cp__b("colvar_methods.F",745,"PLANE_PLANE_ANGLE Colvar section: Two PLANE sections must be provided!")
       ! Plane 1
    ...
    ELSE IF (my_subsection(14)) THEN
       ...
       ELSE
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",795)
          DO k = 1, n_var
    ...
    ELSE IF (my_subsection(15)) THEN
       ...
       IF(.NOT.(nr_frame>=1.AND.nr_frame<=2))CALL cp__a("colvar_methods.F",821)
       ...
       ELSE IF (colvar%rmsd_param%subset==rmsd_list) THEN
          ...
          ELSE
             CALL cp__b("colvar_methods.F",848,"CV RMSD: if SUBSET_TYPE=LIST a list of atoms needs to be provided ")
          END IF
       ELSE IF (colvar%rmsd_param%subset==rmsd_weightlist) THEN
          ...
          IF (n_var /= 0) THEN
             ...
             colvar%rmsd_param%n_atoms = ndim
          ELSE
             CALL cp__b("colvar_methods.F",863,"CV RMSD: if SUBSET_TYPE=WEIGHT_LIST a list of atoms needs to be provided ")
          END IF
          ...
          IF (n_var /= 0) THEN
             ...
             IF(ndim/=colvar%rmsd_param%n_atoms)&
                CALL cp_abort(cp__l("colvar_methods.F",876),"CV RMSD: list of atoms and list of "//&
             ...
             DEALLOCATE (weights)
          ELSE
             CALL cp__b("colvar_methods.F",884,"CV RMSD: if SUBSET_TYPE=WEIGHT_LIST a list of weights need to be provided. ")
          END IF

       ELSE
          CALL cp__b("colvar_methods.F",888,"CV RMSD: unknown SUBSET_TYPE.")
       END IF
    ...
    ELSE IF (my_subsection(22)) THEN
       ...
       IF(ndim <= 3)&
          CALL cp__b("colvar_methods.F",959,"CV Ring Puckering: Ring size has to be 4 or larger. ")
       ii = colvar%ring_puckering_param%iq
       IF(ABS(ii)==1 .OR. ii<-(ndim-1)/2 .OR. ii>ndim/2)&
          CALL cp__b("colvar_methods.F",962,"CV Ring Puckering: Invalid coordinate number.")
    ELSE IF (my_subsection(23)) THEN
       ...
       ELSE
         !KINDS
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",990)
          DO k = 1, n_var
       ...
       ELSE
         !KINDS
          ...
          IF(.NOT.(n_var>0))CALL cp__a("colvar_methods.F",1021)
          DO k = 1, n_var
...
!       CALL section_vals_val_get(mindist_section,"NC",r_val=colvar%mindist_param%n_cut)
    ...
    IF (iw>0) THEN
       ...
       CASE(reaction_path_colvar_id)
          CALL cp__w("colvar_methods.F",1201,"Description header for REACTION_PATH COLVAR missing!!")
       CASE(distance_from_path_colvar_id)
          CALL cp__w("colvar_methods.F",1203,"Description header for REACTION_PATH COLVAR missing!!")
       CASE(hydronium_colvar_id)
          CALL cp__w("colvar_methods.F",1205,"Description header for HYDRONIUM COLVAR missing!!")
       CASE(rmsd_colvar_id)
          CALL cp__w("colvar_methods.F",1207,"Description header for RMSD COLVAR missing!!")
...
! *****************************************************************************
  SUBROUTINE  colvar_check_points(colvar, section)
    ...
    NULLIFY(atoms)
    NULLIFY(weights)
    IF(.NOT.(ASSOCIATED(colvar)))CALL cp__a("colvar_methods.F",1345)
       ...
       DO i = 1, npoints
          ...
          CASE(do_clv_geo_center)
             ...
             IF (nrep/=0) THEN
                ...
                END DO
                IF(.NOT.(natoms==nweights))CALL cp__a("colvar_methods.F",1385)
             END IF
...
! *****************************************************************************
  SUBROUTINE colvar_eval_mol_f(colvar, cell, particles, pos, fixd_list)
    ...
    IF(.NOT.(colvar_ok))CALL cp__a("colvar_methods.F",1423)
    ...
    CASE (hydronium_colvar_id)
       CALL hydronium_colvar(colvar,cell,particles=particles)
    CASE(rmsd_colvar_id)
    ...
    CASE (u_colvar_id)
       CALL cp__b("colvar_methods.F",1475,"need force_env!")
    CASE (Wc_colvar_id)
    ...
    CASE DEFAULT
       CALL cp__b("colvar_methods.F",1483,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE colvar_eval_glob_f(icolvar,force_env)
    ...
    IF(.NOT.(colvar_ok))CALL cp__a("colvar_methods.F",1515)
    ...
    CASE (hydronium_colvar_id)
       CALL hydronium_colvar(colvar,cell,subsys=subsys)
    CASE(rmsd_colvar_id)
    ...
    CASE DEFAULT
       CALL cp__b("colvar_methods.F",1568,"")
    END SELECT
    ! Check for fixed atom constraints
...
! Initialize the content of the derivative
    ...
    CASE (mindist_colvar_id)
       CALL mindist_colvar(colvar,cell,particles=particles)
    CASE (u_colvar_id)
       CALL cp__b("colvar_methods.F",1632,"need force_env!")
    CASE (Wc_colvar_id)
       CALL Wc_colvar(colvar, cell, particles=particles)
    CASE (HBP_colvar_id)
       CALL HBP_colvar(colvar, cell, particles=particles)
    CASE DEFAULT
       CALL cp__b("colvar_methods.F",1638,"")
    END SELECT
...
! *****************************************************************************
  SUBROUTINE xyz_diag_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==xyz_diag_colvar_id))CALL cp__a("colvar_methods.F",1736)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",1740)
    ...
    ELSE
       ...
       CASE DEFAULT
          ...
          CALL cp__b("colvar_methods.F",1808,"")
       END SELECT
       fi(:)= xi
    ENDIF
...
! *****************************************************************************
  SUBROUTINE xyz_outerdiag_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==xyz_outerdiag_colvar_id))CALL cp__a("colvar_methods.F",1845)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",1849)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    DO k = 1, 2
    ...
    IF (PRESENT(force_env)) THEN
       ...
       ELSE
          IF(force_env%in_use/=use_mixed_force)&
             CALL cp_abort(cp__l("colvar_methods.F",1966),&
                  'ASSERTION (cond) failed at line '//cp_to_string(1967)//&
          ...
          IF(.NOT.(EvalErrType<=0))CALL cp__a("colvar_methods.F",2030)
          ...
          DO iforce_eval = 1, nforce_eval
             ...
             IF (ABS(err)>lerr) THEN
                ...
                CALL cp_warn(cp__l("colvar_methods.F",2041),&
                     'ASSERTION (cond) failed at line '//cp_to_string(2042)//&
       ...
       END IF
    ELSE
        CALL cp__b("colvar_methods.F",2074,"need force_env!")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE plane_distance_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==plane_distance_colvar_id))CALL cp__a("colvar_methods.F",2107)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2111)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE plane_plane_angle_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(check))CALL cp__a("colvar_methods.F",2230)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2234)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    ...
    norm1 = SQRT(a1)
    IF(.NOT.(norm1/=0.0_dp))CALL cp__a("colvar_methods.F",2269)
    ...
    norm2 = SQRT(a2)
    IF(.NOT.(norm2/=0.0_dp))CALL cp__a("colvar_methods.F",2301)
...
! *****************************************************************************
  SUBROUTINE rotation_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==rotation_colvar_id))CALL cp__a("colvar_methods.F",2451)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2455)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE dfunct_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==dfunct_colvar_id))CALL cp__a("colvar_methods.F",2538)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2542)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE angle_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==angle_colvar_id))CALL cp__a("colvar_methods.F",2614)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2618)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE dist_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==dist_colvar_id))CALL cp__a("colvar_methods.F",2688)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2692)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE torsion_colvar(colvar,cell,subsys,particles,no_riemann_sheet_op)
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==torsion_colvar_id))CALL cp__a("colvar_methods.F",2767)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2771)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE qparm_colvar(colvar,cell,subsys,particles)
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==qparm_colvar_id))CALL cp__a("colvar_methods.F",2932)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",2936)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    ...
    DO ii=1,n_atoms_from
       ...
       DO m = 0, l
          ...
          DO jj=1,n_atoms_to
             ...
             IF ((l+m) > maxfac) THEN
                CALL cp__b("colvar_methods.F",2980,"(l-m) > maxfac")
             END IF
             bond  = 1.0_dp/(1.0_dp + EXP(alpha*(r_tmp - rcut)))
             IF (bond > 1.0_dp) THEN
                CALL cp__b("colvar_methods.F",2984,"bond > 1.0_dp")
             END IF
             pre_fac = bond*SQRT(((2*l+1)*fac(l-m))/(4*pi*fac(l+m)))
             IF (plm < 0.0_dp) THEN
                dylm = -pre_fac*dplm
             ELSE
                dylm = pre_fac*dplm
             ...
             ymag = (SQRT(REAL(ylm_calc2*CONJG(ylm_calc2),KIND=dp)))
          ...
          IF (nbond < 0.0001_dp) THEN
             CALL cp__b("colvar_methods.F",3011,"NBOND = 0.0")
          END IF
          ylm = ylm/nbond
          grad(:) = grad(:)/nbond
          grad_nb(:) = grad_nb(:)/nbond
          ymag1 = REAL(ylm*CONJG(ylm),KIND=dp)
          qlm = qlm + fact*ymag1
...
! *****************************************************************************
  SUBROUTINE hydronium_colvar(colvar,cell,subsys,particles)
    TYPE(colvar_type), POINTER               :: colvar
    TYPE(cell_type), POINTER                 :: cell
    TYPE(cp_subsys_type), OPTIONAL, POINTER  :: subsys
    TYPE(particle_type), DIMENSION(:), &
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==hydronium_colvar_id))CALL cp__a("colvar_methods.F",3084)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",3088)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF

    ALLOCATE ( dfunc_nh ( 3, n_oxygens, n_hydrogens ))
    ALLOCATE ( nhcoord  ( n_oxygens))
    ALLOCATE ( M (  n_oxygens ))
    ALLOCATE ( dM ( 3, n_oxygens,n_hydrogens ))

    ALLOCATE ( dfunc_no ( 3, n_oxygens, n_oxygens ))
    ALLOCATE ( no       (  n_oxygens))
...
! to fill few missing informations...
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==coord_colvar_id))CALL cp__a("colvar_methods.F",3259)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",3263)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! to fill few missing informations...
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==mindist_colvar_id))CALL cp__a("colvar_methods.F",3392)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",3396)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE combine_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==combine_colvar_id))CALL cp__a("colvar_methods.F",3562)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",3566)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    ...
    DO i = 1, ncolv
       ...
       IF ((ABS(err)>colvar%combine_cvs_param%lerr)) THEN
          ...
          CALL compress(this_error,.TRUE.)
          CALL compress(def_error,.TRUE.)
          CALL cp_warn(cp__l("colvar_methods.F",3600),&
               'ASSERTION (cond) failed at line '//cp_to_string(3601)//&
               ' Error '//TRIM(this_error)//' in computing numerical derivatives larger then'//&
               TRIM(def_error)//' . ')
       END IF
    END DO
...
! *****************************************************************************
  SUBROUTINE reaction_path_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==reaction_path_colvar_id))CALL cp__a("colvar_methods.F",3657)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",3661)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE  distance_from_path_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==distance_from_path_colvar_id))CALL cp__a("colvar_methods.F",4038)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",4042)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! to fill few missing informations...
    ...
    NULLIFY(particles_i)
    IF(.NOT.(colvar%type_id==population_colvar_id))CALL cp__a("colvar_methods.F",4393)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",4397)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE gyration_radius_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==gyration_colvar_id))CALL cp__a("colvar_methods.F",4495)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",4499)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE rmsd_colvar_low(colvar,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==rmsd_colvar_id))CALL cp__a("colvar_methods.F",4608)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",4612)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    ...
    ELSE IF (nframes==1) THEN
       ...
       END DO
    ELSE
        CALL cp__b("colvar_methods.F",4675,"RMSD implemented only for 1 and 2 reference frames!")
    END IF
    colvar%ss=cv_val
...
! *****************************************************************************
  SUBROUTINE  ring_puckering_colvar(colvar,cell,subsys,particles)
    ...
    IF(.NOT.(colvar%type_id==ring_puckering_colvar_id))CALL cp__a("colvar_methods.F",4721)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",4725)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    ...
    ELSE
       ...
       IF(.NOT.(m/=1))CALL cp__a("colvar_methods.F",4814)
       IF(MOD(nring,2)==0 .AND. colvar%ring_puckering_param%iq == nring/2) THEN
          ...
          END DO
       ELSE
          IF(.NOT.(m<=(nring-1)/2))CALL cp__a("colvar_methods.F",4838)
          a = 0._dp
          b = 0._dp
          DO ii = 1,nring
     ...
     DO i = 1,nr_frames
       ...
       IF (explicit) THEN
          ...
          IF(i==1) THEN
            ...
            n_atoms = natom
          ELSE
           IF(.NOT.(3*natom==SIZE(r_ref,1)))CALL cp__a("colvar_methods.F",4987)
          END IF
          DO j = 1, natom
              CALL section_vals_val_get(coord_section,"_DEFAULT_KEYWORD_",&
       ...
       ELSE
          ...
          IF(.NOT.(TRIM(filename)/=""))CALL cp__a("colvar_methods.F",4996)
          NULLIFY(parser)
          ALLOCATE(rptr(3))
          ...
          IF(i==1) THEN
             ...
             n_atoms = natom
          ELSE
           IF(.NOT.(3*natom==SIZE(r_ref,1)))CALL cp__a("colvar_methods.F",5008)
          END IF
          DO j = 1, natom
            ! Atom coordinates
            CALL parser_get_next_line(parser,1,at_end=my_end)
            IF(my_end)&
               CALL cp_abort(cp__l("colvar_methods.F",5014),&
...
! *****************************************************************************
  SUBROUTINE Wc_colvar(colvar,cell,subsys,particles,qs_env)
    ...
    IF(.NOT.(colvar%type_id==Wc_colvar_id))CALL cp__a("colvar_methods.F",5065)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",5069)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
...
! *****************************************************************************
  SUBROUTINE HBP_colvar(colvar,cell,subsys,particles,qs_env)
    ...
    IF(.NOT.(colvar%type_id==HBP_colvar_id))CALL cp__a("colvar_methods.F",5183)
    IF (PRESENT(particles)) THEN
       my_particles => particles
    ELSE
       IF(.NOT.(PRESENT(subsys)))CALL cp__a("colvar_methods.F",5187)
       CALL cp_subsys_get(subsys,particles=particles_i)
       my_particles => particles_i%els
    END IF
    CALL get_qs_env(qs_env,WannierCentres=wc)
