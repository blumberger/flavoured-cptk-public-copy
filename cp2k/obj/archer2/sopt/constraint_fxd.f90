# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_fxd.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_fxd.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/constraint_fxd.F" 2
...
! *****************************************************************************
  SUBROUTINE fix_atom_control( force_env,w)
    ...
    IF(.NOT.((SIZE(particle_set) == natom)))CALL cp__a("constraint_fxd.F",119)
    IF (shell_present) THEN
       core_particle_set => core_particles%els
       IF(.NOT.((SIZE(core_particle_set) == ncore)))CALL cp__a("constraint_fxd.F",122)
       shell_particle_set => shell_particles%els
       IF(.NOT.((SIZE(shell_particle_set) == nshell)))CALL cp__a("constraint_fxd.F",124)
    END IF
...
! *****************************************************************************
  SUBROUTINE create_local_fixd_list(lfixd_list, nkind, molecule_kind_set, &
    ...
    INTEGER :: handle, i, ikind, iparticle, iparticle_local, isize, jsize, &
      ncnst, nparticle_local, nparticle_local_all, nsize
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(.NOT.ASSOCIATED(lfixd_list)))CALL cp__a("constraint_fxd.F",454)
    nsize = 0
    DO ikind = 1, nkind
       molecule_kind => molecule_kind_set(ikind)
       CALL get_molecule_kind ( molecule_kind, fixd_list=fixd_list )
...
! *****************************************************************************
  SUBROUTINE release_local_fixd_list(lfixd_list)
    ...
    IF(.NOT.(ASSOCIATED(lfixd_list)))CALL cp__a("constraint_fxd.F",562)
