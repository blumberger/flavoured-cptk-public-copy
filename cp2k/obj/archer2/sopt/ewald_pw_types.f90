# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_pw_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_pw_types.F"
!-----------------------------------------------------------------------------!
...
# 53 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_pw_types.F" 2
...
! *****************************************************************************
  SUBROUTINE ewald_pw_retain(ewald_pw)
    ...
    IF(.NOT.(ASSOCIATED(ewald_pw)))CALL cp__a("ewald_pw_types.F",86)
    IF(.NOT.(ewald_pw%ref_count>0))CALL cp__a("ewald_pw_types.F",87)
...
! *****************************************************************************
  SUBROUTINE ewald_pw_release ( ewald_pw)
    ...
    IF (ASSOCIATED(ewald_pw)) THEN
       IF(.NOT.(ewald_pw%ref_count>0))CALL cp__a("ewald_pw_types.F",140)
       ewald_pw%ref_count=ewald_pw%ref_count-1
...
! *****************************************************************************
  SUBROUTINE ewald_pw_init ( ewald_pw, ewald_env, cell, cell_ref, print_section)
    ...
    IF(.NOT.(ASSOCIATED ( ewald_pw )))CALL cp__a("ewald_pw_types.F",202)
    IF(.NOT.(ASSOCIATED ( ewald_env )))CALL cp__a("ewald_pw_types.F",203)
    IF(.NOT.(ASSOCIATED ( cell )))CALL cp__a("ewald_pw_types.F",204)
    IF(.NOT.(ewald_pw%ref_count>0))CALL cp__a("ewald_pw_types.F",205)
    ...
    CASE ( do_ewald_ewald )
       ...
       IF ( ANY(gmax == 2 * ( gmax / 2 )) ) THEN
          CALL cp__b("ewald_pw_types.F",225,"gmax has to be odd.")
       END IF
       bo ( 1, : ) = -gmax / 2
       bo ( 2, : ) = +gmax / 2
    ...
    CASE ( do_ewald_pme )
       ...
       IF ( ns_max == 2 * ( ns_max / 2 ) ) THEN
          CALL cp__b("ewald_pw_types.F",249,"ns_max has to be odd.")
       END IF
       npts_s ( : ) = ns_max
    ...
    CASE ( do_ewald_none )
       ! No EWALD sums..
    CASE default
       CALL cp__b("ewald_pw_types.F",342,"")
    END SELECT
    ! Poisson Environment
