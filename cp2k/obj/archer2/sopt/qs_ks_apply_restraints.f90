# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_apply_restraints.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_apply_restraints.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_ks_apply_restraints.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_ks_becke_restraint(qs_env,auxbas_pw_pool,calculate_forces,matrix_s,becke)
    ...
    IF(dft_control%qs_control%becke_restraint)THEN
       ! Test no k-points
       IF(.NOT.(SIZE(matrix_s,2)==1))CALL cp__a("qs_ks_apply_restraints.F",79)
       ...
       IF(dft_control%qs_control%et_coupling_calc)THEN
          IF(qs_env%et_coupling%keep_matrix)THEN
             IF(qs_env%et_coupling%first_run)THEN
                ...
                CALL cp_dbcsr_copy(qs_env%et_coupling%rest_mat(1)%matrix, matrix_s(1,1)%matrix, &
             ...
             ELSE
                ...
                CALL cp_dbcsr_copy(qs_env%et_coupling%rest_mat(2)%matrix, matrix_s(1,1)%matrix, &
...
! *****************************************************************************
  SUBROUTINE qs_ks_mulliken_restraint(energy,dft_control,just_energy,para_env,&
    ...
    IF (dft_control%qs_control%mulliken_restraint) THEN
       ...
       ! Test no k-points
       IF(.NOT.(SIZE(matrix_s,2)==1))CALL cp__a("qs_ks_apply_restraints.F",164)
...
! *****************************************************************************
  SUBROUTINE qs_ks_s2_restraint(dft_control,qs_env,matrix_s,&
    ...
    IF (dft_control%qs_control%s2_restraint) THEN
       ! Test no k-points
       IF(.NOT.(SIZE(matrix_s,2)==1))CALL cp__a("qs_ks_apply_restraints.F",218)
       ! adds s2_restraint energy and orbital derivatives
       IF(.NOT.(dft_control%nspins == 2))CALL cp__a("qs_ks_apply_restraints.F",220)
       IF(.NOT.(qs_env%requires_mo_derivs))CALL cp__a("qs_ks_apply_restraints.F",221)
       ! forces are not implemented (not difficult, but ... )
       IF(.NOT.(.NOT. calculate_forces))CALL cp__a("qs_ks_apply_restraints.F",223)
