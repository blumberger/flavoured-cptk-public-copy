# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_transformations.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_transformations.F"
!-----------------------------------------------------------------------------!
...
# 113 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/ops/dbcsr_transformations.F" 2
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_valid_index(normal), dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Matrix does not exist.",183)
    ! Internalize options
    shallow = .FALSE.
    ...
    CALL dbcsr_create (transposed, name="transposed "//TRIM(normal%m%name),&
         ...
         row_blk_size = array_data(normal%m%col_blk_size),&
         col_blk_size = array_data(normal%m%row_blk_size),&
         max_rbs=normal%m%max_cbs,&
         max_cbs=normal%m%max_rbs,&
         row_blk_offset=normal%m%col_blk_offset,&
         col_blk_offset=normal%m%row_blk_offset)
    ...
    ! Copy the data
    IF (shallow) THEN
       CALL dbcsr_switch_data_area (transposed, normal%m%data_area)
...
! *****************************************************************************
  SUBROUTINE dbcsr_desymmetrize_deep(sm, desm, untransposed_data)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_desymmetrize_deep', &
      routineP = moduleN//':'//routineN
    INTEGER, PARAMETER                       :: idata = 2, imeta = 1, &
                                                metalen = 3

    INTEGER :: blk, blk_l, blk_p, blk_ps, blks, col, col_size, dst_p, handle, &
      mp_group, nsymmetries, numproc, nze, pcol, prow, row, row_size, src_p, &
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_valid_index(matrix%m),&
         ...
         routineN, "Matrix not initialized.",599)
    IF(matrix%m%replication_type.EQ.dbcsr_repl_none)&
       CALL cp__w("dbcsr/ops/dbcsr_transformations.F",601,"Distributing a non-replicated matrix makes no sense.")
    ...
    CASE (dbcsr_type_real_4)
       ...
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
            routineN,"Only real double precision",613)
    CASE (dbcsr_type_complex_8)
       CALL dbcsr_get_data (matrix%m%data_area, c_dp)
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
            routineN,"Only real double precision",617)
    CASE (dbcsr_type_complex_4)
       CALL dbcsr_get_data (matrix%m%data_area, c_sp)
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_unimplemented_error_nr, &
            routineN,"Only real double precision",621)
    END SELECT
    ...
    IF (mini) THEN
       ...
       DO WHILE (dbcsr_iterator_blocks_left (iter))
          ...
          tr = .FALSE.
          CALL dbcsr_get_stored_coordinates (matrix%m, row, col, p)
          IF (mynode .EQ. p) THEN
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_get_local_rows (dense_dist, dense_local_rows,&
         dense_matrix%m%index(dbcsr_slot_home_prow))
    CALL dbcsr_get_local_cols (dense_dist, dense_local_cols,&
         dense_matrix%m%index(dbcsr_slot_home_pcol))
    !
...
! *****************************************************************************
  SUBROUTINE dbcsr_make_undense (matrix, undense_matrix, distribution,&
    ...
    INTEGER :: handle, nblkcols_local, nblkcols_total, nblkrows_local, &
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_data_get_size (dense_data), 'GE',&
         nfullrows*nfullcols,&
         dbcsr_fatal_level, dbcsr_internal_error, routineN,&
         "Dense data too small",1190)
    IF(.NOT.join_cols.AND..NOT.join_rows)&
       CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1192,"Joining neither rows nor columns is untested")
    !
...
! *****************************************************************************
  SUBROUTINE make_undense_data (matrix, dense_data, nfullrows, nfullcols,&
    ...
    INTEGER, DIMENSION(:), INTENT(IN) :: dense_row_blk_offsets, &
      dense_col_blk_offsets, row_map, col_map, row_internal_offsets, &
      col_internal_offsets
...
! *****************************************************************************
  SUBROUTINE dbcsr_replicate (matrix, replicate_rows, replicate_columns,&
    ...
    INTEGER :: blk, blk_l, blk_p, blk_ps, blks, col, col_size, data_type, &
      dst_p, handle, mp_group, mynode, mypcol, myprow, nblks, numnodes, nze, &
...
!   ---------------------------------------------------------------------------
    ...
    ELSE
       ...
       CALL dbcsr_assert(replicate_rows, "OR", replicate_columns, &
            ...
            "Some replication must be specified", 1431)
    ENDIF
    ...
    CALL dbcsr_assert(MAXVAL(row_dist).LE.UBOUND(blacs2mpi,1), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Row distribution references unexistent processor rows',1447)
    IF (dbg) THEN
       IF(MAXVAL(row_dist).NE.UBOUND(blacs2mpi,1))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1450,'Range of row distribution not equal to processor rows')
    ENDIF
    CALL dbcsr_assert(MAXVAL(col_dist).LE.UBOUND(blacs2mpi,2), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Col distribution references unexistent processor cols',1454)
    IF (dbg) THEN
       IF(MAXVAL(col_dist).NE.UBOUND(blacs2mpi,2))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1457,'Range of col distribution not equal to processor cols')
    ENDIF
...
! *****************************************************************************
  SUBROUTINE dbcsr_complete_redistribute(matrix, redist, keep_sparsity)
    ...
    INTEGER :: blk, blk_col_new, blk_ps, blk_row_new, blks, cnt_fnd, cnt_new, &
      ...
      col_size, col_size_new, data_offset_l, data_type, dst_p, handle, i, &
      meta_l, mp_group, numnodes, nze_rle, row, row_int, row_offset_new, &
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_valid_index(matrix%m),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Input not valid.",1699)
    CALL dbcsr_assert (dbcsr_is_initialized(redist%m),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Output matrix not initialized.",1702)
    IF(matrix%m%replication_type.NE.dbcsr_repl_none)&
       CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1704,"Can not redistribute replicated matrix.")
    !
    ...
    CALL dbcsr_assert(MAXVAL(row_dist_new),"LE",UBOUND(pgrid,1), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Row distribution references unexistent processor rows',1756)
    IF (dbg) THEN
       IF(MAXVAL(row_dist_new).NE.UBOUND(pgrid,1))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1759,'Range of row distribution not equal to processor rows')
    ENDIF
    CALL dbcsr_assert(MAXVAL(col_dist_new),"LE",UBOUND(pgrid,2), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Col distribution references unexistent processor cols',1763)
    IF (dbg) THEN
       IF(MAXVAL(col_dist_new).NE.UBOUND(pgrid,2))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",1766,'Range of col distribution not equal to processor cols')
    ENDIF
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_wrong_args_error,&
            routineN, "Invalid matrix type",1933)
    END SELECT
...
! *****************************************************************************
  SUBROUTINE dbcsr_redistribute(matrix, redist)
    ...
    INTEGER :: blk, blk_ps, blks, col, col_size, data_type, dst_p, handle, &
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_assert (dbcsr_valid_index(matrix%m),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Input not valid.",2100)
    CALL dbcsr_assert (dbcsr_is_initialized(redist%m),&
         dbcsr_fatal_level, dbcsr_caller_error,&
         routineN, "Output matrix not initialized.",2103)
    IF(matrix%m%replication_type.NE.dbcsr_repl_none)&
       CALL cp__w("dbcsr/ops/dbcsr_transformations.F",2105,"Can not redistribute replicated matrix.")
    data_type = matrix%m%data_type
    ! Get row and column start and end positions
    ! Old matrix
    ! New matrix
    ...
    CALL dbcsr_assert(MAXVAL(row_dist_new).LE.UBOUND(pgrid,1), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Row distribution references unexistent processor rows',2123)
    IF (dbg) THEN
       IF(MAXVAL(row_dist_new).NE.UBOUND(pgrid,1))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",2126,'Range of row distribution not equal to processor rows')
    ENDIF
    CALL dbcsr_assert(MAXVAL(col_dist_new).LE.UBOUND(pgrid,2), dbcsr_failure_level,&
         dbcsr_wrong_args_error, routineN,&
         'Col distribution references unexistent processor cols',2130)
    IF (dbg) THEN
       IF(MAXVAL(col_dist_new).NE.UBOUND(pgrid,2))&
          CALL cp__w("dbcsr/ops/dbcsr_transformations.F",2133,'Range of col distribution not equal to processor cols')
    ENDIF
    ALLOCATE (send_count(2, 0:numnodes-1))
    ALLOCATE (recv_count(2, 0:numnodes-1))
    ALLOCATE (total_send_count(2, 0:numnodes-1))
    ...
    CALL timeset(routineN, handle)
     ...
     DO ind = 1, job_count*nblkrows_total
        ...
        DO blk = row_p(ind+i-1)+1, row_p(ind+i)
           ...
           sym_tr = .FALSE.
           CALL dbcsr_get_stored_coordinates (new_matrix%m, brow, bcol, dst_p)
           send_count(1, dst_p) = send_count(1, dst_p) + 1
