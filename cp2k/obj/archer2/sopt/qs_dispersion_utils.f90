# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_utils.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_dispersion_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_dispersion_env_set(dispersion_env,xc_section)
    ...
    TYPE(section_vals_type), POINTER         :: nl_section, pp_section, &
                                                vdw_section, xc_fun_section

    IF(.NOT.(ASSOCIATED(dispersion_env)))CALL cp__a("qs_dispersion_utils.F",70)
    ...
    IF ( dispersion_env%type == xc_vdw_fun_pairpot ) THEN
       ...
       IF ( dispersion_env%pp_type == vdw_pairpot_dftd2 ) THEN
          ...
          IF ( .NOT. explicit ) THEN
            ...
            IF(.NOT.(exfun))CALL cp__a("qs_dispersion_utils.F",98)
       ...
       IF ( dispersion_env%pp_type == vdw_pairpot_dftd3  .OR. &
          ...
          IF ( .NOT. explicit ) THEN
             CALL section_vals_val_get(pp_section, "REFERENCE_FUNCTIONAL" ,explicit=exfun)
             IF(.NOT.(exfun))CALL cp__a("qs_dispersion_utils.F",121)
             IF ( dispersion_env%pp_type == vdw_pairpot_dftd3 ) THEN
    ...
    IF (output_unit>0) THEN
       ...
       ELSE IF ( dispersion_env%type == xc_vdw_fun_nonloc ) THEN
          ...
          SELECT CASE (dispersion_env%nl_type)
             CASE DEFAULT
                ! unknown functional
                CALL cp__b("qs_dispersion_utils.F",268,"")
             CASE (vdw_nl_DRSLL)
                WRITE (output_unit, &
