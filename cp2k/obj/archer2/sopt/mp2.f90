# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2.F"
!-----------------------------------------------------------------------------!
...
# 107 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mp2.F" 2
...
! *****************************************************************************
  SUBROUTINE mp2_main(qs_env,calc_forces)
    ...
    LOGICAL                                  :: calc_ex, do_admm, &
                                                do_dynamic_load_balancing, &
                                                do_exx, do_gw, free_hfx_buffer
...
!JW 2del    IF (qs_env%mp2_env%hf_fail) THEN
!       CALL cp_abort(cp__l("mp2.F",184),"SCF not converged: "//&
...
!    ENDIF
    ...
    IF (calc_forces) THEN
       ...
       IF(mp2_env%method/=ri_mp2_method_gpw) THEN
         CALL cp__b("mp2.F",234,"No forces/gradients for the selected method.")
       END IF
    ENDIF
    ...
    CALL hfx_create_basis_types(basis_parameter,basis_info, qs_kind_set, do_admm=.FALSE.)
    ...
    IF(.NOT.(dimen==nao))CALL cp__a("mp2.F",282)
...
!$  n_threads = omp_get_max_threads()
    ...
    SELECT CASE(mp2_env%method)
     CASE(mp2_method_laplace)
       CALL cp__b("mp2.F",434,"laplace not implemented")
...
! *****************************************************************************
  SUBROUTINE mp2_direct_energy(dimen,occ_i,occ_j,mp2_biel,mp2_env,C_i,Auto_i,Emp2,Emp2_Cou,Emp2_ex,&
    ...
    INTEGER :: batch_number, color_sub, comm_sub, counter, elements_ij_proc, &
      ...
      number_j_subset, one, sqrt_number_groups, total_I_size_batch_group, &
    ...
    IF(number_groups*mp2_env%mp2_num_proc/=para_env%num_pe) THEN
      CALL cp__b("mp2.F",817," The number of processors needs to be a multiple of the processors per group. ")
    END IF
    ...
    IF(.NOT.(SUM(batch_sizes)<=my_batch_size))CALL cp__a("mp2.F",970)
...
! *****************************************************************************
  SUBROUTINE calculate_exx(qs_env,unit_nr,do_gw,do_admm,E_ex_from_GW,t3)
    ...
    DO i=1, SIZE(matrix_ks)
      CALL cp_dbcsr_set(matrix_ks(i)%matrix,0.0_dp)
      IF(do_admm) THEN
        CALL cp_dbcsr_set(matrix_ks_aux_fit(i)%matrix,0.0_dp)
      END IF
    END DO
    ...
    ELSE
      ...
      DO irep = 1,n_rep_hf
        ...
        END IF
        CALL integrate_four_center(qs_env, matrix_ks_2d, energy, rho_ao_2d, hfx_sections,&
...
! *****************************************************************************
  SUBROUTINE compute_vec_Sigma_x_minus_vxc_gw(qs_env,mp2_env,mos_mp2,energy_ex,t3)
    ...
    INTEGER :: dimen, gw_corr_lev_occ, gw_corr_lev_virt, homo, irep, ispin, &
      myfun, myfun_aux, myfun_prim, n_rep_hf, nmo, ns
    LOGICAL                                  :: charge_constrain_tmp, &
                                                do_admm_rpa, do_hfx, &
                                                do_mix_exchange, do_ri_Sigma_x
    REAL(KIND=dp)                            :: frac_exx, hfx_fraction, t1, t2
    REAL(KIND=dp), ALLOCATABLE, &
    ...
    NULLIFY(admm_env,matrix_ks,matrix_ks_aux_fit,rho_ao,matrix_sigma_x_minus_vxc,input,&
            ...
            energy_dummy,dft_control,para_env,ks_env,mo_coeff)
    ...
    DO ispin=1,SIZE(matrix_ks)
       ...
       CALL cp_dbcsr_init_p(matrix_sigma_x_minus_vxc(ispin)%matrix)
...
!JW    PRINT *, 'matrix_ks_aux_fit after qs_ks_build_kohn_sham_matrix: '
!JW    CALL cp_dbcsr_print(matrix_ks_aux_fit(1)%matrix,error=error)

!JW    PRINT *, 'matrix_ks after build ks matrix: '
!JW    CALL cp_dbcsr_print(matrix_ks(1)%matrix,error=error)
    ...
    DO ispin=1,dft_control%nspins
      CALL cp_dbcsr_add(matrix_sigma_x_minus_vxc(ispin)%matrix, matrix_ks(ispin)%matrix, -1.0_dp, 1.0_dp)
    END DO
...
!JW    PRINT *, 'matrix_ks before int 4 c:' 
!JW    CALL cp_dbcsr_print(matrix_ks(1)%matrix,error=error)
    ...
    IF(.NOT. do_ri_Sigma_x .OR. do_mix_exchange) THEN
      ...
      DO irep = 1,n_rep_hf
        ...
        CALL integrate_four_center(qs_env,matrix_ks_2d,energy_dummy,rho_ao_2d,hfx_sections,&
                                   para_env,.TRUE.,irep,.TRUE.,&
                                   ispin=1,do_exx=.TRUE.)
      END DO
...
!JW    PRINT *, 'matrix_ks after int 4 c:'        
!JW    CALL cp_dbcsr_print(matrix_ks(1)%matrix,error=error)

!JW    PRINT *, 'rho_ao for auxiliary basis hopefully'
!JW    CALL cp_dbcsr_print(rho_ao(1)%matrix,error=error)

!JW    PRINT *, 'matrix_ks_aux_fit after int 4c: '
!JW    CALL cp_dbcsr_print(matrix_ks_aux_fit(1)%matrix,error=error)
...
!JW    PRINT *, 'matrix_ks after merging: '
!JW    CALL cp_dbcsr_print(matrix_ks(1)%matrix,error=error)
    ...
    DO ispin=1,dft_control%nspins
      CALL cp_dbcsr_add(matrix_sigma_x_minus_vxc(ispin)%matrix, matrix_ks(ispin)%matrix, 1.0_dp, frac_exx)
    END DO
    ...
    CALL cp_dbcsr_init (mo_coeff_b)
    CALL cp_dbcsr_desymmetrize(matrix_ks(1)%matrix,mo_coeff_b)
    CALL cp_dbcsr_set(mo_coeff_b, 0.0_dp)
    ...
    DO ispin=1,dft_control%nspins
      ...
      IF(ispin==1) THEN
        ...
        CALL cp_dbcsr_create(matrix_tmp,template=mo_coeff_b)
        CALL cp_dbcsr_copy(matrix_tmp,mo_coeff_b)
        CALL cp_dbcsr_set(matrix_tmp, 0.0_dp)
    
        CALL cp_dbcsr_init(matrix_tmp_2)
        CALL cp_dbcsr_create(matrix_tmp_2,template=mo_coeff_b)
        CALL cp_dbcsr_copy(matrix_tmp_2,mo_coeff_b)
        CALL cp_dbcsr_set(matrix_tmp_2, 0.0_dp)
      END IF
      ...
      CALL cp_dbcsr_multiply('N','N',1.0_dp,matrix_sigma_x_minus_vxc(ispin)%matrix,&
      ...
      CALL cp_dbcsr_get_diag(matrix_tmp_2,vec_Sigma_x_minus_vxc_gw(:,ispin))
    ...
    CALL cp_dbcsr_release(mo_coeff_b)
    CALL cp_dbcsr_release(matrix_tmp)
    CALL cp_dbcsr_release(matrix_tmp_2)

    DO ispin=1,SIZE(matrix_ks)
      CALL cp_dbcsr_release_p(matrix_sigma_x_minus_vxc(ispin)%matrix)
    END DO
