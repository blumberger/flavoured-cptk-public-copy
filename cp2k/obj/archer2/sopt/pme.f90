# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pme.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pme.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pme.F" 2
...
! *****************************************************************************
  SUBROUTINE pme_evaluate ( ewald_env, ewald_pw, box, particle_set, vg_coulomb, &
    ...
    IF(PRESENT(shell_particle_set)) THEN
       IF(.NOT.(ASSOCIATED(shell_particle_set)))CALL cp__a("pme.F",158)
       IF(.NOT.(ASSOCIATED(core_particle_set)))CALL cp__a("pme.F",159)
...
! *****************************************************************************
  SUBROUTINE get_patch ( dg, particle_set, exp_igr, box, p1, p2, &
    ...
    IF(my_is1_core .OR. my_is2_core) THEN
       IF(.NOT.(PRESENT(core_particle_set)))CALL cp__a("pme.F",548)
    END IF
    my_is1_shell = .FALSE.
    my_is2_shell = .FALSE.
    ...
    IF(my_is1_core .AND. my_is1_shell) THEN
       CALL cp__b("pme.F",555,"Shell-model: cannot be core and shell simultaneously")
    END IF
