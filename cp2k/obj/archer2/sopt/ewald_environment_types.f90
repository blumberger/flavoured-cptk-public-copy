# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_environment_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_environment_types.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/ewald_environment_types.F" 2
...
! *****************************************************************************
  SUBROUTINE ewald_env_get(ewald_env, ewald_type, alpha, eps_pol, epsilon, &
    ...
    IF(.NOT.(ASSOCIATED(ewald_env)))CALL cp__a("ewald_environment_types.F",154)
...
! *****************************************************************************
  SUBROUTINE ewald_env_set(ewald_env, ewald_type, alpha, epsilon, eps_pol, &
    ...
    IF(.NOT.(ASSOCIATED(ewald_env)))CALL cp__a("ewald_environment_types.F",220)
...
!> \author Fawzi Mohamed
! *****************************************************************************
  SUBROUTINE ewald_env_retain(ewald_env)
    TYPE(ewald_environment_type), POINTER    :: ewald_env

    CHARACTER(len=*), PARAMETER :: routineN = 'ewald_env_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(ewald_env)))CALL cp__a("ewald_environment_types.F",280)
    IF(.NOT.(ewald_env%ref_count>0))CALL cp__a("ewald_environment_types.F",281)
...
! *****************************************************************************
  SUBROUTINE ewald_env_release(ewald_env)
    ...
    IF (ASSOCIATED(ewald_env)) THEN
       IF(.NOT.(ewald_env%ref_count>0))CALL cp__a("ewald_environment_types.F",299)
       ewald_env%ref_count=ewald_env%ref_count-1
...
! *****************************************************************************
  SUBROUTINE read_ewald_section ( ewald_env, ewald_section)
    ...
    CASE (do_ewald_ewald, do_ewald_spme)
       ...
       CASE DEFAULT
          CALL cp__b("ewald_environment_types.F",362,"")
       END SELECT
    ...
    IF (ewald_env%do_multipoles) THEN
       SELECT CASE(ewald_env%ewald_type)
       CASE(do_ewald_ewald)
          CALL section_vals_val_get(multipole_section,"MAX_MULTIPOLE_EXPANSION",i_val=ewald_env%max_multipole)
          CALL section_vals_val_get(multipole_section,"MAX_IPOL_ITER",i_val=ewald_env%max_ipol_iter)
       CASE DEFAULT
          CALL cp__b("ewald_environment_types.F",387,"Multipole code works at the moment only with standard EWALD sums.")
       END SELECT
    END IF
    ...
    IF ( iw > 0 ) THEN
       ...
       IF ( ewald_env%ewald_type /= do_ewald_none ) THEN
          ...
          CASE DEFAULT
             CALL cp__b("ewald_environment_types.F",433,"")
          END SELECT
       ELSE
...
! *****************************************************************************
  FUNCTION find_ewald_optimal_value(precs) RESULT(value)
    ...
    IF(.NOT.(func>0.0_dp))CALL cp__a("ewald_environment_types.F",462)
    ...
    DO WHILE (.TRUE.)
       ...
       IF(.NOT.(func1>=0))CALL cp__a("ewald_environment_types.F",473)
       IF(.NOT.(func2<=0))CALL cp__a("ewald_environment_types.F",474)
       s    = 0.5_dp * (s1 + s2)
       func = EXP(-s**2)/s**2 - precs
       IF      (func > 0.0_dp) THEN
          s1 = s
