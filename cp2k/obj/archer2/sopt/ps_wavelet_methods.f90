# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_methods.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw/ps_wavelet_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE ps_wavelet_create(poisson_params,wavelet,pw_grid)
    ...
    IF (poisson_params%wavelet_method==WAVELET0D) THEN
       IF (hx.NE.hy) &
          CALL cp__b("pw/ps_wavelet_methods.F",99,"Poisson solver for non cubic cells not yet implemented")
        IF(hz.NE.hy) &
           CALL cp__b("pw/ps_wavelet_methods.F",101,"Poisson solver for non cubic cells not yet implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE cp2k_distribution_to_z_slices (density , wavelet, pw_grid)
    ...
    INTEGER                                  :: dest, handle, i, ii, iproc, &
                                                j, k, l, local_z_dim, loz, m, &
                                                m2, md2, nproc, should_warn
    ...
    IF(.NOT.(ASSOCIATED(wavelet)))CALL cp__a("pw/ps_wavelet_methods.F",191)
    ...
    CALL mp_max(should_warn, pw_grid % para % group)
    IF (should_warn>0 .AND. iproc == 0) &
       CALL cp__w("pw/ps_wavelet_methods.F",242,"Density non-zero on the edges of the unit cell: wrong results in WAVELET solver")
...
! *****************************************************************************
  SUBROUTINE z_slices_to_cp2k_distribution(density , wavelet, pw_grid)
    ...
    IF(.NOT.(ASSOCIATED(wavelet)))CALL cp__a("pw/ps_wavelet_methods.F",352)
