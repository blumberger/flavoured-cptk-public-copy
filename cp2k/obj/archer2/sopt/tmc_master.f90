# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_master.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_master.F"
!-----------------------------------------------------------------------------!
...
# 78 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_master.F" 2
...
! *****************************************************************************
  SUBROUTINE cancel_calculations(cancel_list, work_list, cancel_count,&
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_master.F",118)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_master.F",119)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_master.F",120)
    IF(.NOT.(ASSOCIATED(work_list)))CALL cp__a("tmc/tmc_master.F",121)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_master.F",122)
    ...
    cancel_elem_loop: DO
      ...
      IF(.NOT.(wg.GE.0))CALL cp__a("tmc/tmc_master.F",141)
      IF(.NOT.(stat.NE.TMC_STATUS_FAILED))CALL cp__a("tmc/tmc_master.F",142)
      IF(.NOT.(work_list(wg)%elem%stat.NE.status_calc_approx_ener))CALL cp__a("tmc/tmc_master.F",143)
...
! *****************************************************************************
  SUBROUTINE send_analysis_tasks(ana_list, ana_worker_info, para_env, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(ana_worker_info)))CALL cp__a("tmc/tmc_master.F",192)
    IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("tmc/tmc_master.F",193)
...
! *****************************************************************************
  SUBROUTINE do_tmc_master(tmc_env, globenv)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_master.F",250)

    IF(.NOT.(tmc_env%tmc_comp_set%group_nr==0))CALL cp__a("tmc/tmc_master.F",252)
    IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set)))CALL cp__a("tmc/tmc_master.F",253)
    IF(.NOT.(ASSOCIATED(tmc_env%tmc_comp_set%para_env_m_w)))CALL cp__a("tmc/tmc_master.F",254)

    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_master.F",256)
    ...
    IF(stat.NE.TMC_STAT_START_CONF_RESULT)&
       CALL cp_abort(cp__l("tmc/tmc_master.F",304),&
    ...
    task_loop: DO
      ...
      worker_request_loop: DO 
        ...
        IF(.NOT.ASSOCIATED(worker_info(wg)%elem))&
           CALL cp_abort(cp__l("tmc/tmc_master.F",420),&
        ...
        CASE(TMC_CANCELING_RECEIPT)
          ...
          IF(.NOT.(worker_info(wg)%canceled))CALL cp__a("tmc/tmc_master.F",438)
        ...
        CASE(TMC_STAT_START_CONF_RESULT)
           ...
           CALL cp__b("tmc/tmc_master.F",476,"")
        ! -- ENERGY RESULT -----------------
        ...
        CASE DEFAULT
          CALL cp__b("tmc/tmc_master.F",623,"received message with unknown info/stat type")
        END SELECT
...
!                           temperature=tmc_env%params%Temp(gt_elem_tmp%mv_conf), &
        ...
        CASE(status_cancel_nmc, status_cancel_ener)
          ...
          ! no status update
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_master.F",789),&
...
! *****************************************************************************
  SUBROUTINE set_walltime_delay(time, walltime_delay)
    ...
    IF(.NOT.(time.GE.0.0_dp))CALL cp__a("tmc/tmc_master.F",952)
