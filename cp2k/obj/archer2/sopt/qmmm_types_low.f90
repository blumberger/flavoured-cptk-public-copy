# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_types_low.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_types_low.F"
!-----------------------------------------------------------------------------!
...
# 36 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qmmm_types_low.F" 2
  ...
  PUBLIC :: add_shell_type, add_shell_release, create_add_shell_type
...
! *****************************************************************************
  TYPE image_charge_type
     ...
     TYPE(eri_mme_param)                        :: eri_mme_param
...
! *****************************************************************************
  SUBROUTINE qmmm_env_mm_create(qmmm_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(qmmm_env)))CALL cp__a("qmmm_types_low.F",265)
    ALLOCATE(qmmm_env)
...
! *****************************************************************************
  SUBROUTINE qmmm_env_mm_retain(qmmm_env)
    ...
    IF(.NOT.(ASSOCIATED(qmmm_env)))CALL cp__a("qmmm_types_low.F",294)
    IF(.NOT.(qmmm_env%ref_count>0))CALL cp__a("qmmm_types_low.F",295)
...
! *****************************************************************************
  SUBROUTINE qmmm_env_mm_release(qmmm_env)
    ...
    IF (ASSOCIATED(qmmm_env)) THEN
       IF(.NOT.(qmmm_env%ref_count>0))CALL cp__a("qmmm_types_low.F",312)
       qmmm_env%ref_count=qmmm_env%ref_count-1
...
! *****************************************************************************
  SUBROUTINE qmmm_env_qm_create(qmmm_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(qmmm_env)))CALL cp__a("qmmm_types_low.F",352)
    ALLOCATE(qmmm_env)
    qmmm_env%ref_count=1
    last_qmmm_env_id_nr=last_qmmm_env_id_nr+1
    qmmm_env%id_nr=last_qmmm_env_id_nr
...
! *****************************************************************************
  SUBROUTINE qmmm_env_qm_release(qmmm_env)
    ...
    IF (ASSOCIATED(qmmm_env)) THEN
       IF(.NOT.(qmmm_env%ref_count>0))CALL cp__a("qmmm_types_low.F",401)
       qmmm_env%ref_count=qmmm_env%ref_count-1
       IF (qmmm_env%ref_count==0) THEN
          IF (ASSOCIATED(qmmm_env%qm_atom_index)) THEN
             DEALLOCATE(qmmm_env%qm_atom_index)
...
! *****************************************************************************
  SUBROUTINE qmmm_image_charge_dealloc(image_charge_pot)
    ...
    IF (ASSOCIATED(image_charge_pot)) THEN
         ...
         END IF
         CALL eri_mme_release(image_charge_pot%eri_mme_param)
    ...
    DO I = 1, SIZE(Per_Potentials)
       IF (ASSOCIATED(Per_Potentials(I)%Pot)) THEN
          ...
          IF (ASSOCIATED(Per_Potentials(I)%Pot%pw_pool)) THEN
             ...
             IF(.NOT.(.NOT.ASSOCIATED(Per_Potentials(I)%Pot%pw_pool)))CALL cp__a("qmmm_types_low.F",613)
          END IF
          IF (ASSOCIATED(Per_Potentials(I)%Pot%pw_grid)) THEN
             CALL pw_grid_release ( Per_Potentials(I)%Pot%pw_grid)
             IF(.NOT.(.NOT.ASSOCIATED(Per_Potentials(I)%Pot%pw_grid)))CALL cp__a("qmmm_types_low.F",617)
          END IF
