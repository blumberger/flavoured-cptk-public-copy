# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_types.F"
!-----------------------------------------------------------------------------!
...
# 39 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_types.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_ddapc_retain(cp_ddapc_env)
    TYPE(cp_ddapc_type), POINTER             :: cp_ddapc_env

    CHARACTER(len=*), PARAMETER :: routineN = 'cp_ddapc_retain', &
      routineP = moduleN//':'//routineN

    IF(.NOT.(ASSOCIATED(cp_ddapc_env)))CALL cp__a("cp_ddapc_types.F",193)
    IF(.NOT.(cp_ddapc_env%ref_count>0))CALL cp__a("cp_ddapc_types.F",194)
...
! *****************************************************************************
  SUBROUTINE cp_ddapc_release(cp_ddapc_env)
    ...
    IF (ASSOCIATED(cp_ddapc_env)) THEN
       IF(.NOT.(cp_ddapc_env%ref_count>0))CALL cp__a("cp_ddapc_types.F",212)
       cp_ddapc_env%ref_count=cp_ddapc_env%ref_count-1
...
! *****************************************************************************
  SUBROUTINE cp_ddapc_ewald_create(cp_ddapc_ewald, qmmm_decoupl, qm_cell,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(cp_ddapc_ewald)))CALL cp__a("cp_ddapc_types.F",281)
  ...
  SUBROUTINE cp_ddapc_ewald_release(cp_ddapc_ewald)
    ...
    IF (ASSOCIATED(cp_ddapc_ewald)) THEN
       ...
       IF (ASSOCIATED(cp_ddapc_ewald%pw_pool_qm)) THEN
          ...
          IF(.NOT.(.NOT.ASSOCIATED(cp_ddapc_ewald%pw_pool_qm)))CALL cp__a("cp_ddapc_types.F",462)
       END IF
       IF (ASSOCIATED(cp_ddapc_ewald%pw_pool_mm)) THEN
          CALL pw_pool_release ( cp_ddapc_ewald%pw_pool_mm)
          IF(.NOT.(.NOT.ASSOCIATED(cp_ddapc_ewald%pw_pool_mm)))CALL cp__a("cp_ddapc_types.F",466)
       END IF
       IF (ASSOCIATED(cp_ddapc_ewald%pw_grid_qm)) THEN
          CALL pw_grid_release ( cp_ddapc_ewald%pw_grid_qm)
          IF(.NOT.(.NOT.ASSOCIATED(cp_ddapc_ewald%pw_grid_qm)))CALL cp__a("cp_ddapc_types.F",470)
       END IF
       IF (ASSOCIATED(cp_ddapc_ewald%pw_grid_mm)) THEN
          CALL pw_grid_release ( cp_ddapc_ewald%pw_grid_mm)
          IF(.NOT.(.NOT.ASSOCIATED(cp_ddapc_ewald%pw_grid_mm)))CALL cp__a("cp_ddapc_types.F",474)
       END IF
