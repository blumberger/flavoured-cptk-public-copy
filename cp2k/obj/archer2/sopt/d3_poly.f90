# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/d3_poly.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/d3_poly.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/d3_poly.F" 2
...
SUBROUTINE init_d3_poly_module()
    INTEGER                                  :: grad, i, ii, ij, j, nthreads, &
                                                subG
    ...
    IF(nthreads /= 1) CALL cp__b("d3_poly.F",80,"init_d3_poly_module must not be called within OMP PARALLEL")
...
SUBROUTINE poly_mult1(p1,p2,pRes,np1,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",393,"module d3_poly not initialized")
    mySumUp=.FALSE.
    myNp1=1
    ...
    IF(.NOT.(newSize>=poly_size1(newGrad)))CALL cp__a("d3_poly.F",402)
...
SUBROUTINE poly_mult2(p1,p2,pRes,np1,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",441,"module d3_poly not initialized")
    mySumUp=.FALSE.
    IF (PRESENT(sumUp)) mySumUp=sumUp
    myNp1=1
    IF (PRESENT(np1)) myNp1=np1
    ...
    newGrad=grad1+grad2
    newSize=SIZE(pRes)/myNp1
    IF(.NOT.(newSize>=poly_size2(newGrad)))CALL cp__a("d3_poly.F",452)
    IF (.not.mySumUp) pRes=0
    iShift=0
    shiftRes=0
    DO ipoly=1,myNp1
...
SUBROUTINE poly_mult3(p1,p2,pRes,np1,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",522,"module d3_poly not initialized")
    mySumUp=.FALSE.
    IF (PRESENT(sumUp)) mySumUp=sumUp
    myNp1=1
    IF (PRESENT(np1)) myNp1=np1
    ...
    newGrad=grad1+grad2
    newSize=SIZE(pRes)/myNp1
    IF(.NOT.(newSize>=poly_size3(newGrad)))CALL cp__a("d3_poly.F",533)
...
SUBROUTINE poly_mult3b(p1,size_p1,grad1,p2,size_p2,grad2,pRes,size_pRes,np1,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",574,"module d3_poly not initialized")
...
SUBROUTINE poly_mult3ab(p1,size_p1,grad1,p2,pRes,size_pRes,np1,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",684,"module d3_poly not initialized")
    my_size_p1=size_p1/np1
    newSize=size_pRes/np1
...
SUBROUTINE poly_write2(p,out_f)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",792,"module d3_poly not initialized")
    did_write=.FALSE.
    DO i=1,SIZE(p)
...
SUBROUTINE poly_write3(p,out_f)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",820,"module d3_poly not initialized")
    did_write=.FALSE.
    DO i=1,SIZE(p)
...
FUNCTION poly_random(p,maxSize,minSize) RESULT(res)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",856,"module d3_poly not initialized")
    myMinSize=1
    IF (PRESENT(minSize)) myMinSize=minSize
    CALL RANDOM_NUMBER(g)
    pSize=MIN(maxSize,myMinSize+INT((maxSize-myMinSize+1)*g))
    IF(.NOT.(SIZE(p)>=pSize))CALL cp__a("d3_poly.F",861)
...
SUBROUTINE poly_affine_t3t(p,m,b,pRes,npoly)
    ...
    INTEGER :: grad, i, igrad, ii, ii1, ipoly, j, k, minResSize, monoDim1, &
      ...
      resShift, rest_size_p, size_p, size_res, start_idx1
    REAL(dp), ALLOCATABLE, DIMENSION(:)      :: monoG1, monoG2
    REAL(dp), DIMENSION(4, 3)                :: basepoly

    IF (.not.module_initialized) CALL cp__b("d3_poly.F",898,"module d3_poly not initialized")
    my_npoly=1
    ...
    IF(.NOT.(size_res==minResSize))CALL cp__a("d3_poly.F",911)
    IF(.NOT.(size_p>=minResSize))CALL cp__a("d3_poly.F",912)
    pRes=0
    IF (size_p==0) RETURN
    ii1=1
    ii=1
    ...
    DO
        ...
        CALL poly_mult3ab(monoG1(1),monoFullDim1,igrad,&
            basepoly(1,1),&
            monoG2(1),monoDimAtt,monoSize1,.FALSE.)
        monoDimAtt2=monoFullDim2-monoDim2
        start_idx1=(monoSize1-igrad-1)*monoDim1
        CALL poly_mult3ab(monoG1(start_idx1+1),&
            monoFullDim1-start_idx1,igrad,basepoly(1,2),&
            monoG2(monoDimAtt+1),&
            monoDimAtt2-monoDimAtt,igrad+1,.FALSE.)
        CALL poly_mult3ab(monoG1(monoFullDim1-monoDim1+1),&
            monoDim1,igrad,basepoly(1,3),&
            monoG2(monoDimAtt2+1),&
        ...
        CALL poly_mult3ab(monoG2(1),monoFullDim2,igrad,&
            basepoly(1,1),monoG1(1),&
            monoDimAtt,monoSize2,.FALSE.)
        monoDimAtt2=monoFullDim1-monoDim1
        start_idx1=(monoSize2-igrad-1)*monoDim2
        CALL poly_mult3ab(monoG2(start_idx1+1),&
            monoFullDim2-start_idx1,igrad,basepoly(1,2),&
            monoG1(monoDimAtt+1),monoDimAtt2-monoDimAtt,&
            igrad+1,.FALSE.)
        CALL poly_mult3ab(monoG2(monoFullDim2-monoDim2+1),&
            monoDim2,igrad,basepoly(1,3),&
            monoG1(monoDimAtt2+1),&
...
SUBROUTINE poly_affine_t3(p,m,b,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1062,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    basepoly(1,:)=b
    DO j=1,3
    ...
    minResSize=poly_size3(grad)
    IF(.NOT.(size_res>=minResSize))CALL cp__a("d3_poly.F",1075)
    pRes=0
    IF (size_p==0) RETURN
    ii1=1
    ii=1
    ...
    DO
        ...
        CALL poly_mult3ab(monoG2(1),monoFullDim2,igrad,&
            basepoly(1,1),&
            monoG1(1),monoDimAtt,monoSize2,.FALSE.)
        monoDimAtt2=monoFullDim1-monoDim1
        start_idx1=(monoSize2-igrad-1)*monoDim2
        CALL poly_mult3ab(monoG2(start_idx1+1),&
...
SUBROUTINE poly_p_eval3(p,x,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1221,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    grad=grad_size3(size_p)
    newSize=SIZE(pRes)/my_npoly
    IF(.NOT.(newSize>=poly_size2(grad)))CALL cp__a("d3_poly.F",1227)
    pRes=0.0
    ALLOCATE(xi(grad+1))
...
SUBROUTINE poly_p_eval3b(p,size_p,x,pRes,size_pRes,npoly,grad,xi)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1268,"module d3_poly not initialized")
    inSize=size_p/npoly
...
SUBROUTINE poly_padd_uneval3(p,x,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1336,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    newSize=SIZE(pRes)/my_npoly
    grad=grad_size2(newSize)
    IF(.NOT.(size_p>=poly_size3(grad)))CALL cp__a("d3_poly.F",1342)
    IF(.NOT.(newSize==poly_size2(grad)))CALL cp__a("d3_poly.F",1343)
    ALLOCATE(xi(grad+1))
...
SUBROUTINE poly_padd_uneval3b(p,size_p,x,pRes,size_pRes,npoly,grad,xi)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1384,"module d3_poly not initialized")
    inSize=size_p/npoly
    newSize=size_pRes/npoly
...
SUBROUTINE poly_p_eval2(p,x,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1450,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    ...
    newSize=SIZE(pRes)/my_npoly
    pRes=0.0_dp
    IF(.NOT.(newSize>=poly_size1(grad)))CALL cp__a("d3_poly.F",1457)
    ALLOCATE(xi(grad+1))
...
SUBROUTINE poly_p_eval2b(p,size_p,x,pRes,size_pRes,npoly,grad,xi)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1495,"module d3_poly not initialized")
    inSize=size_p/npoly
    newSize=size_pRes/npoly
...
SUBROUTINE poly_padd_uneval2(p,x,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1559,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    newSize=SIZE(pRes)/my_npoly
    grad=grad_size1(newSize)
    IF(.NOT.(size_p>=poly_size2(grad)))CALL cp__a("d3_poly.F",1565)
    IF(.NOT.(newSize==poly_size1(grad)))CALL cp__a("d3_poly.F",1566)
    ALLOCATE(xi(grad+1))
...
SUBROUTINE poly_padd_uneval2b(p,size_p,x,pRes,size_pRes,npoly,grad,xi)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1605,"module d3_poly not initialized")
    inSize=size_p/npoly
...
SUBROUTINE poly_eval1(p,x,pRes,npoly)
    ...
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    IF(.NOT.(SIZE(pRes)>=my_npoly))CALL cp__a("d3_poly.F",1670)
    pShift=0
    DO ipoly=1,my_npoly
        xx=1.0_dp
        vv=0.0_dp
...
SUBROUTINE poly_eval2(p,x,y,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1707,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    grad=grad_size2(size_p)
    IF(.NOT.(SIZE(pRes)>=my_npoly))CALL cp__a("d3_poly.F",1712)
...
SUBROUTINE poly_eval3(p,x,y,z,pRes,npoly)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1778,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    size_p=SIZE(p)/my_npoly
    grad=grad_size3(size_p)
    IF(.NOT.(SIZE(pRes)>=my_npoly))CALL cp__a("d3_poly.F",1783)
...
SUBROUTINE poly_derive3(p,pRes,npoly,sumUp)
    ...
    IF (.not.module_initialized) CALL cp__b("d3_poly.F",1854,"module d3_poly not initialized")
    my_npoly=1
    IF (PRESENT(npoly)) my_npoly=npoly
    my_sumUp=.FALSE.
    ...
    grad=grad_size3(size_p)
    IF(.NOT.(newSize>=poly_size3(grad)))CALL cp__a("d3_poly.F",1862)
...
SUBROUTINE poly_cp2k2d3(poly_cp2k,grad,poly_d3)
    ...
    IF(.NOT.(SIZE(poly_cp2k)>=size_p))CALL cp__a("d3_poly.F",1940)
    IF(.NOT.(SIZE(poly_d3)>=size_p))CALL cp__a("d3_poly.F",1941)
    cp_ii=0
    sgrad2k=0
    sgrad3k=0
    DO k=0,grad
...
SUBROUTINE poly_d32cp2k(poly_cp2k,grad,poly_d3)
    ...
    size_p=(grad+1)*(grad+2)*(grad+3)/6
    IF(.NOT.(SIZE(poly_cp2k)>=size_p))CALL cp__a("d3_poly.F",1993)
    IF(.NOT.(SIZE(poly_d3)>=size_p))CALL cp__a("d3_poly.F",1994)
    cp_ii=0
    sgrad2k=0
    sgrad3k=0
    DO k=0,grad
