# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_types.F"
!-----------------------------------------------------------------------------!
...
# 154 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_environment_types.F" 2
...
! *****************************************************************************
  SUBROUTINE get_qs_env(qs_env,atomic_kind_set,qs_kind_set,cell,super_cell,cell_ref,use_ref_cell,kpoints,&
    ...
    TYPE(neighbor_list_set_p_type), DIMENSION(:), OPTIONAL, POINTER :: &
      ...
      sab_tbe, sab_core, sab_almo, sab_kp
    ...
    NULLIFY(rho0_m)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_environment_types.F",642)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_environment_types.F",643)
    IF(.NOT.(ASSOCIATED(qs_env%ks_env)))CALL cp__a("qs_environment_types.F",644)
...
! *****************************************************************************
  SUBROUTINE set_qs_env(qs_env,super_cell,&
       ...
       ks_qmmm_env,wf_history,scf_env,&
...
!    CPPrecondition(ASSOCIATED(qs_env),cp_failure_level,routineP,failure)
    ...
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_environment_types.F",1082)
    ...
    IF (PRESENT(linres_control)) THEN ! accept also null pointers?
       CALL linres_control_retain(linres_control)
    ...
    IF (PRESENT(rho_external)) THEN ! accepts also null pointers !
       IF (ASSOCIATED(rho_external)) CALL qs_rho_retain(rho_external)
    ...
    IF (PRESENT(cp_ddapc_env)) THEN
       CALL cp_ddapc_retain(cp_ddapc_env)
...
! *****************************************************************************
  SUBROUTINE qs_env_retain(qs_env)
    ...
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_environment_types.F",1273)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_environment_types.F",1274)
...
! *****************************************************************************
  SUBROUTINE qs_env_release(qs_env)
    ...
    IF (ASSOCIATED(qs_env)) THEN
       IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_environment_types.F",1294)
       qs_env%ref_count=qs_env%ref_count-1
       IF (qs_env%ref_count<1) THEN
          ...
          IF (ASSOCIATED(qs_env%mo_derivs)) THEN
             DO I=1,SIZE(qs_env%mo_derivs)
                CALL cp_dbcsr_release_p(qs_env%mo_derivs(I)%matrix)
             ENDDO
