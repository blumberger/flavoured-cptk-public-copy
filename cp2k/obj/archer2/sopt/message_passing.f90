# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/../base/base_uses.f90" 1
...
# 31 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
...
# 70 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"

# 87 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
...
# 616 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    mp_comm = 0
...
! *****************************************************************************
  SUBROUTINE mp_reordering(mp_comm,mp_new_comm,ranks_order)
    ...
    ierr = 0
# 670 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(ranks_order))==-1) EXIT ;  END DO ; ENDIF
    mp_new_comm = mp_comm
...
# 697 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    CALL rm_mp_perf_env()
...
! *****************************************************************************
  SUBROUTINE add_mp_perf_env(perf_env)
    ...
    IF (stack_pointer > max_stack_size) THEN
       CALL cp__b("mpiwrap/message_passing.F",721,"stack_pointer too large : message_passing @ add_mp_perf_env")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_perf_env_create(perf_env)
    ...
    IF (stat/=0) THEN
       CALL cp__b("mpiwrap/message_passing.F",746,"allocation failed in mp_perf_env_create")
    ENDIF
  ...
  SUBROUTINE mp_perf_env_release(perf_env)
    ...
    IF (ASSOCIATED(perf_env)) THEN
       IF (perf_env%ref_count<1) THEN
          CALL cp__b("mpiwrap/message_passing.F",769,"invalid ref_count: message_passing @ mp_perf_env_release")
       END IF
...
! *****************************************************************************
  SUBROUTINE mp_perf_env_retain(perf_env)
    ...
    IF (.NOT.ASSOCIATED(perf_env)) THEN
       CALL cp__b("mpiwrap/message_passing.F",787,"unassociated perf_env: message_passing @ mp_perf_env_retain")
    END IF
    IF (perf_env%ref_count<1) THEN
       CALL cp__b("mpiwrap/message_passing.F",790,"invalid ref_count: message_passing @ mp_perf_env_retain")
    END IF
    perf_env%ref_count=perf_env%ref_count+1
...
! *****************************************************************************
  SUBROUTINE mp_perf_env_describe ( perf_env, iw )
    ...
    IF (.NOT.ASSOCIATED(perf_env)) THEN
       CALL cp__b("mpiwrap/message_passing.F",811,"unassociated perf_env : message_passing @ mp_perf_env_describe")
    ENDIF
    IF (perf_env%ref_count<1) THEN
       CALL cp__b("mpiwrap/message_passing.F",814,"invalid perf_env%ref_count : message_passing @ mp_perf_env_describe")
    ENDIF
# 849 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(iw))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE rm_mp_perf_env ()
    IF (stack_pointer<1) THEN
       CALL cp__b("mpiwrap/message_passing.F",858,"no perf_env in the stack : message_passing @ rm_mp_perf_env")
    ENDIF
...
! *****************************************************************************
  FUNCTION get_mp_perf_env () RESULT(res)
    ...
    IF (stack_pointer<1) THEN
       CALL cp__b("mpiwrap/message_passing.F",872,"no perf_env in the stack : message_passing @ get_mp_perf_env")
    ENDIF
...
# 918 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(perf_id))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(time))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg_size))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_stop ( ierr, prg_code )
    ...
    CALL cp__b("mpiwrap/message_passing.F",972,full_error)
...
! *****************************************************************************
  SUBROUTINE mp_sync ( group )
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE mp_environ_l ( numtask, taskid, groupid )
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(groupid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_cart_create ( comm_old, ndims, dims, pos, comm_cart )
    ...
    pos ( 1:ndims ) = -1
    comm_cart = comm_old
# 1173 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    pos ( 1:ndims ) = 0
    dims = 1
    comm_cart = 0
...
! *****************************************************************************
  SUBROUTINE mp_cart_coords ( comm, rank, coords)
    ...
    coords = 0
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rank))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_comm_compare ( comm1, comm2, res)
    ...
    iout = 0
    res = 0
# 1252 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm1))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm2))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_cart_sub ( comm, rdim, sub_comm )
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rdim))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_rank_compare ( comm1, comm2, rank )
    ...
    rank = 0
# 1403 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm1))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm2))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_dims_create ( nodes, dims )
    ...
    dims = 1
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(nodes))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_cart_rank ( group, pos, rank )
    ...
    rank = 0
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(pos))==-1) EXIT ;  END DO ; ENDIF
...
# 1510 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 1552 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(requests))==-1) EXIT ;  END DO ; ENDIF
...
# 1593 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(requests))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
# 1619 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
...
# 1655 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(requests))==-1) EXIT ;  END DO ; ENDIF
    completed = 1
...
# 1698 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(requests))==-1) EXIT ;  END DO ; ENDIF
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(requests))==-1) EXIT ;  END DO ; ENDIF
    IF(PRESENT(completed)) completed = 1
    IF(PRESENT(flag)) flag = .TRUE.
...
!> \author Joost VandeVondele 
! *****************************************************************************
# 1771 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
...
! *****************************************************************************
  SUBROUTINE mp_comm_split_direct(comm,sub_comm,color,key)
    ...
    my_key = 0
# 1805 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    CALL mp_comm_dup(comm,sub_comm)
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(color))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(key))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_comm_split(comm,sub_comm,ngroups, group_distribution, &
    ...
    IF (.NOT. PRESENT(subgroup_min_size) .AND. .NOT. PRESENT(n_subgroups)) THEN
       CALL cp__b("mpiwrap/message_passing.F",1861,routineP//" missing arguments")
    ENDIF
    IF (PRESENT(subgroup_min_size) .AND. PRESENT(n_subgroups)) THEN
       CALL cp__b("mpiwrap/message_passing.F",1864,routineP//" too many arguments")
    ENDIF

    CALL mp_environ(nnodes,mepos,comm)

    IF (.NOT. ASSOCIATED(group_distribution)) THEN
       CALL cp__b("mpiwrap/message_passing.F",1870,routineP//" group_distribution not associated")
    ENDIF
    IF (LBOUND(group_distribution,1) .NE. 0 .OR. &
         UBOUND(group_distribution,1).NE.nnodes-1) THEN
       CALL cp__b("mpiwrap/message_passing.F",1874,routineP//" group_distribution wrong bounds")
    ENDIF

# 1938 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    CALL mp_comm_dup(comm,sub_comm)
    group_distribution(0)=0
    ngroups=1
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(stride))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group_partition))==-1) EXIT ;  END DO ; ENDIF
...
!   ---------------------------------------------------------------------------
    ...
    ierr = 0
# 1996 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    tag = -1
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_bcast_b(msg,source,gid)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 2120 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 2189 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_minloc_dv(msg,gid)
    ...
    IF ("d" .EQ. "l" .AND. real_8 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2221,"Minimal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2238 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_minloc_iv(msg,gid)
    ...
    IF ("i" .EQ. "l" .AND. int_4 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2269,"Minimal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2284 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_minloc_lv(msg,gid)
    ...
    IF ("l" .EQ. "l" .AND. int_8 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2315,"Minimal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2330 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_minloc_rv(msg,gid)
    ...
    IF ("r" .EQ. "l" .AND. real_4 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2361,"Minimal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2376 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_maxloc_dv(msg,gid)
    ...
    ierr = 0
    IF ("d" .EQ. "l" .AND. real_8 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2407,"Maximal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2422 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_maxloc_iv(msg,gid)
    ...
    ierr = 0
    IF ("i" .EQ. "l" .AND. int_4 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2453,"Maximal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2468 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_maxloc_lv(msg,gid)
    ...
    ierr = 0
    IF ("l" .EQ. "l" .AND. int_8 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2499,"Maximal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2514 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_maxloc_rv(msg,gid)
    ...
    ierr = 0
    IF ("r" .EQ. "l" .AND. real_4 .EQ. int_8) THEN
       CALL cp__b("mpiwrap/message_passing.F",2545,"Maximal location not available with long integers @ "//routineN)
    ENDIF
    CALL mp_timeset(routineN,handle)

# 2560 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_file_open(groupid, fh, filepath, amode_status, info)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(groupid))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(info))==-1) EXIT ;  END DO ; ENDIF
...
# 2738 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msglen))==-1) EXIT ;  END DO ; ENDIF
...
# 2767 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    WRITE(UNIT=fh, POS=offset+1) msg
...
# 2802 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msglen))==-1) EXIT ;  END DO ; ENDIF
    WRITE(UNIT=fh, POS=offset+1) msg
...
# 2831 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    WRITE(UNIT=fh, POS=offset+1) msg
...
# 2867 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msglen))==-1) EXIT ;  END DO ; ENDIF
...
# 2896 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    READ(UNIT=fh, POS=offset+1) msg
...
# 2931 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msglen))==-1) EXIT ;  END DO ; ENDIF
    READ(UNIT=fh, POS=offset+1) msg
...
# 2960 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    READ(UNIT=fh, POS=offset+1) msg
...
! *****************************************************************************
  FUNCTION mp_type_make_struct (subtypes,&
    ...
    ENDDO
# 3061 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF (PRESENT (vector_descriptor) .OR. PRESENT (index_descriptor)) THEN
       CALL cp__b("mpiwrap/message_passing.F",3062,routineN//" Vectors and indices NYI")
    ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_isend_custom(msgin,dest,comm,request,tag)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    ierr=1
...
! *****************************************************************************
  SUBROUTINE mp_irecv_custom(msgout,source,comm,request,tag)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    ierr=1
    CALL cp__b("mpiwrap/message_passing.F",3172,"mp_irecv called in non parallel case")
...
# 3200 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
# 3234 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
# 3267 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90" 1
! *****************************************************************************
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_i11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rdispl))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_send_i(msg,dest,tag,gid)
    ...
    CALL cp__b("mpiwrap/message_passing.F",600,"not in parallel mode")
...
! *****************************************************************************
  SUBROUTINE mp_send_iv(msg,dest,tag,gid)
    ...
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    ! only defined in parallel
    CALL cp__b("mpiwrap/message_passing.F",638,"not in parallel mode")
...
! *****************************************************************************
  SUBROUTINE mp_recv_i(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    ! only defined in parallel
    CALL cp__b("mpiwrap/message_passing.F",685,"not in parallel mode") 
...
! *****************************************************************************
  SUBROUTINE mp_recv_iv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    ! only defined in parallel
    CALL cp__b("mpiwrap/message_passing.F",731,"not in parallel mode") 
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_im3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_im4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_iv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_im(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_iv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_i(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_iv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_im(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    CALL cp__b("mpiwrap/message_passing.F",2415,"mp_irecv called in non parallel case")
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    CALL cp__b("mpiwrap/message_passing.F",2484,"mp_irecv called in non parallel case")
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
!>      temporaries. They must point to contiguous memory.
! *****************************************************************************
  SUBROUTINE mp_rget_iv(base,source,win,win_data,disp,request)
    INTEGER(KIND=int_4), DIMENSION(:), POINTER                      :: base
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_i(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_i(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
    DEALLOCATE(DATA)
...
! *****************************************************************************
  FUNCTION mp_type_make_i (ptr,&
    ...
    IF (PRESENT (vector_descriptor) .OR. PRESENT (index_descriptor)) THEN
       CALL cp__b("mpiwrap/message_passing.F",3002,routineN//": Vectors and indices NYI")
    ENDIF
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_i.f90"
...
# 3273 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90" 1
! *****************************************************************************
!> \brief Shift around the data in msg
!> \param[in,out] msg         Rank-2 data to shift
!> \param[in] group           message passing environment identifier
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_l11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i) SHARED(rcount,rdispl,sdispl,rb,sb)
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_recv_l(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_recv_lv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_lm3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_lm4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_lv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_lm(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_lv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_l(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_lv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_lm(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    recvbuf(1+displs(1):) = sendbuf
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_l(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    ALLOCATE(DATA(len), stat=ierr)
    IF (ierr/=0 .AND. .NOT. PRESENT(stat)) &
       CALL mp_stop(ierr, "ALLOCATE @ "//routineN)
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_l(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
    DEALLOCATE(DATA)
    IF(PRESENT(stat)) stat = 0
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_l.f90"
     INTEGER                                 :: length, mystat
     length = MAX(len,1)
     IF (PRESENT (stat)) THEN
        ALLOCATE(DATA(length), stat=mystat)
...
# 3274 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90" 1
! *****************************************************************************
!> \brief Shift around the data in msg
!> \param[in,out] msg         Rank-2 data to shift
!> \param[in] group           message passing environment identifier
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_r11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i) SHARED(rcount,rdispl,sdispl,rb,sb)
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_recv_r(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_recv_rv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_rm3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_rm4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_rv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_rm(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_rv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_r(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_rv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_rm(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    recvbuf(1+displs(1):) = sendbuf
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
!>      temporaries. They must point to contiguous memory.
! *****************************************************************************
  SUBROUTINE mp_rget_rv(base,source,win,win_data,disp,request)
    REAL(kind=real_4), DIMENSION(:), POINTER                      :: base
    INTEGER, INTENT(IN)                                 :: source, win
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_r(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    ALLOCATE(DATA(len), stat=ierr)
    IF (ierr/=0 .AND. .NOT. PRESENT(stat)) &
       CALL mp_stop(ierr, "ALLOCATE @ "//routineN)
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_r(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
    DEALLOCATE(DATA)
    IF(PRESENT(stat)) stat = 0
...
! *****************************************************************************
  FUNCTION mp_type_make_r (ptr,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'mp_make_type_r', &
         routineP = moduleN//':'//routineN
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_r.f90"
     INTEGER                                 :: length, mystat
     length = MAX(len,1)
     IF (PRESENT (stat)) THEN
        ALLOCATE(DATA(length), stat=mystat)
...
# 3275 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90" 1
! *****************************************************************************
!> \brief Shift around the data in msg
!> \param[in,out] msg         Rank-2 data to shift
!> \param[in] group           message passing environment identifier
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_d11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i) SHARED(rcount,rdispl,sdispl,rb,sb)
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_recv_d(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_recv_dv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_dm3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_dm4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_dv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_dm(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_dv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_d(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_dv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_dm(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    recvbuf(1+displs(1):) = sendbuf
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
!>      temporaries. They must point to contiguous memory.
! *****************************************************************************
  SUBROUTINE mp_rget_dv(base,source,win,win_data,disp,request)
    REAL(kind=real_8), DIMENSION(:), POINTER                      :: base
    INTEGER, INTENT(IN)                                 :: source, win
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_d(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    ALLOCATE(DATA(len), stat=ierr)
    IF (ierr/=0 .AND. .NOT. PRESENT(stat)) &
       CALL mp_stop(ierr, "ALLOCATE @ "//routineN)
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_d(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
    DEALLOCATE(DATA)
    IF(PRESENT(stat)) stat = 0
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_d.f90"
     INTEGER                                 :: length, mystat
     length = MAX(len,1)
     IF (PRESENT (stat)) THEN
        ALLOCATE(DATA(length), stat=mystat)
...
# 3276 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90" 1
! *****************************************************************************
!> \brief Shift around the data in msg
!> \param[in,out] msg         Rank-2 data to shift
!> \param[in] group           message passing environment identifier
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_c11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i) SHARED(rcount,rdispl,sdispl,rb,sb)
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_recv_c(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_recv_cv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_cm3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_cm4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_cv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_cm(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_cv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_c(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_cv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_cm(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    recvbuf(1+displs(1):) = sendbuf
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_c(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    ALLOCATE(DATA(len), stat=ierr)
    IF (ierr/=0 .AND. .NOT. PRESENT(stat)) &
       CALL mp_stop(ierr, "ALLOCATE @ "//routineN)
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_c(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
    DEALLOCATE(DATA)
    IF(PRESENT(stat)) stat = 0
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_c.f90"
     INTEGER                                 :: length, mystat
     length = MAX(len,1)
     IF (PRESENT (stat)) THEN
        ALLOCATE(DATA(length), stat=mystat)
...
# 3277 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90" 1
! *****************************************************************************
!> \brief Shift around the data in msg
!> \param[in,out] msg         Rank-2 data to shift
!> \param[in] group           message passing environment identifier
...
# 55 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
# 117 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(displ_in))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_alltoall_z11v ( sb, scount, sdispl, rb, rcount, rdispl, group )
    ...
    ierr = 0
# 174 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(i) SHARED(rcount,rdispl,sdispl,rb,sb)
...
# 229 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(scount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(sdispl))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
...
# 283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 327 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 371 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 417 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb=sb
...
# 464 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 511 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
# 558 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(count))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(group))==-1) EXIT ;  END DO ; ENDIF
    rb = RESHAPE(sb, SHAPE(rb))
...
! *****************************************************************************
  SUBROUTINE mp_recv_z(msg,source,tag,gid)
    ...
    msglen = 1
# 680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_recv_zv(msg,source,tag,gid)
    ...
    msglen = SIZE(msg)
# 726 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 934 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
# 978 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msg))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_zm3(msg,gid)
    ...
    msglen = SIZE(msg)
# 1012 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_zm4(msg,gid)
    ...
    msglen = SIZE(msg)
# 1046 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_zv(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1096 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_sum_root_zm(msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1146 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_scatter_zv(msg_scatter,msg,root,gid)
    ...
    msglen = SIZE(msg)
# 1320 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg = msg_scatter
...
! *****************************************************************************
  SUBROUTINE mp_gather_z(msg,msg_gather,root,gid)
    ...
    msglen = 1
# 1358 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_zv(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1399 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
! *****************************************************************************
  SUBROUTINE mp_gather_zm(msg,msg_gather,root,gid)
    ...
    msglen = SIZE(msg)
# 1440 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msg_gather = msg
...
# 1493 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(recvcounts))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(root))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    recvbuf(1+displs(1):) = sendbuf
...
# 1537 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1580 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,1) = msgout(:)
...
# 1618 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,1) = msgout(:,:)
...
# 1656 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin(:,:,:,1) = msgout(:,:,:)
...
# 1740 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(rcount))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(gid))==-1) EXIT ;  END DO ; ENDIF
    msgin = msgout
...
# 1790 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1842 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1894 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 1946 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    msgout = msgin
...
# 2005 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2086 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(tag))==-1) EXIT ;  END DO ; ENDIF
...
# 2148 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2213 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2283 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgin))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(dest))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2346 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    CALL cp__b("mpiwrap/message_passing.F",2346,"mp_irecv called in non parallel case")
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
...
# 2410 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2479 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(msgout))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(request))==-1) EXIT ;  END DO ; ENDIF
...
# 2533 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(base))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    win = mp_win_null
...
# 2598 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(source))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(win))==-1) EXIT ;  END DO ; ENDIF
...
! *****************************************************************************
  SUBROUTINE mp_allocate_z(DATA, len, stat)
    ...
    ierr = 0
# 2642 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    ALLOCATE(DATA(len), stat=ierr)
    IF (ierr/=0 .AND. .NOT. PRESENT(stat)) &
       CALL mp_stop(ierr, "ALLOCATE @ "//routineN)
...
! *****************************************************************************
  SUBROUTINE mp_deallocate_z(DATA, stat)
    ...
    ierr = 0
# 2680 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
    DEALLOCATE(DATA)
    IF(PRESENT(stat)) stat = 0
...
# 3032 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing_z.f90"
     INTEGER                                 :: length, mystat
     length = MAX(len,1)
     IF (PRESENT (stat)) THEN
        ALLOCATE(DATA(length), stat=mystat)
...
# 3278 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F" 2
...
# 3672 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mpiwrap/message_passing.F"
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(comm))==-1) EXIT ;  END DO ; ENDIF
    IF(.FALSE.)THEN; DO ; IF(SIZE(SHAPE(npow))==-1) EXIT ;  END DO ; ENDIF
