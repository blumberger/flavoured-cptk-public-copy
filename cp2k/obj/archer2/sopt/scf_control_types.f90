# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scf_control_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scf_control_types.F"
!-----------------------------------------------------------------------------!
...
MODULE scf_control_types
  ...
  USE input_constants,                 ONLY: &
       ...
       outer_scf_s2_constraint, smear_energy_window, smear_fermi_dirac, &
       smear_list
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/scf_control_types.F" 2
...
! *****************************************************************************
  ...
  TYPE diagonalization_type
    ...
    LOGICAL       :: mom, mom_didguess
    INTEGER       :: mom_proj_formula
    ! indices of de-occupied and newly occupied alpha / beta molecular orbitals
    INTEGER, DIMENSION(:), POINTER :: mom_deoccA, mom_deoccB, mom_occA, mom_occB
...
!> \author Fawzi Mohamed
! *****************************************************************************
  SUBROUTINE scf_c_retain(scf_control)
    ...
    IF(.NOT.(ASSOCIATED(scf_control)))CALL cp__a("scf_control_types.F",293)

    IF(.NOT.(scf_control%ref_count > 0))CALL cp__a("scf_control_types.F",295)
...
! *****************************************************************************
  SUBROUTINE scf_c_release(scf_control)
    ...
    IF (ASSOCIATED(scf_control)) THEN
      IF(.NOT.(scf_control%ref_count>0))CALL cp__a("scf_control_types.F",317)
      scf_control%ref_count = scf_control%ref_count - 1
...
! *****************************************************************************
  SUBROUTINE scf_c_read_parameters(scf_control,inp_section)
    ...
    IF(.NOT.(ASSOCIATED(scf_control)))CALL cp__a("scf_control_types.F",365)
    IF(.NOT.(scf_control%ref_count > 0))CALL cp__a("scf_control_types.F",366)
    ...
    IF ( scf_control%use_diag .AND. scf_control%use_ot ) THEN
       ...
       CALL cp__b("scf_control_types.F",377,"Don't activate OT and Diagonaliztion together")
    ...
    IF(.NOT.(ASSOCIATED(added_mos)))CALL cp__a("scf_control_types.F",470)
    ...
    IF (scf_control%use_ot.AND.(scf_control%density_guess == core_guess)) THEN
       CALL cp__b("scf_control_types.F",483,"Use GUESS {ATOMIC,RESTART,RANDOM} with OT")
    END IF
    ...
    CALL section_vals_val_get(outer_scf_section,"_SECTION_PARAMETERS_",&
         l_val=scf_control%outer_scf%have_scf)
...
! *****************************************************************************
  SUBROUTINE init_smear(smear)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(smear)))CALL cp__a("scf_control_types.F",536)
    ALLOCATE(smear)
    ...
    IF (smear%do_smear) THEN
       ...
       IF (smear%method==smear_list) THEN
          CALL section_vals_val_get(smear_section,"LIST",&
                                    r_vals=r_vals)
          IF(.NOT.(ASSOCIATED(r_vals)))CALL cp__a("scf_control_types.F",577)
...
! *****************************************************************************
  SUBROUTINE scf_c_write_parameters(scf_control,dft_section)
    ...
    IF(.NOT.(ASSOCIATED(scf_control)))CALL cp__a("scf_control_types.F",622)
    IF(.NOT.(scf_control%ref_count>0))CALL cp__a("scf_control_types.F",623)
    ...
    IF (output_unit > 0) THEN
       ...
       IF (scf_control%max_scf > 0) THEN
          ...
          IF (scf_control%outer_scf%have_scf) THEN
             ...
             CASE DEFAULT
                CALL cp__b("scf_control_types.F",730,"")
             END SELECT
             ...
             CASE DEFAULT
                CALL cp__b("scf_control_types.F",747,"")
             END SELECT
