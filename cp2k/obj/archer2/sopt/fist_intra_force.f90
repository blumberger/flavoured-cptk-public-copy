# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_intra_force.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_intra_force.F"
!-----------------------------------------------------------------------------!
...
# 39 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_intra_force.F" 2
...
! *****************************************************************************
  SUBROUTINE force_intra_control(molecule_set, molecule_kind_set, &
       ...
       f_imptor, f_opbend, cell, use_virial, atprop_env, pot_1_mol_list, &
       ener_decomp_section)
    ...
    REAL(kind=dp), ALLOCATABLE, OPTIONAL         :: pot_1_mol_list(:) !CHANGE S/M
    TYPE(section_vals_type), POINTER             :: ener_decomp_section
    ...
    INTEGER                                  :: count_mol, idx
  !  LOGICAL                                  :: store
    REAL(KIND=dp)                            :: pot_bond_1_mol, &  
                                                pot_shell_1_mol, pot_urey_bradley_1_mol, &
                                                pot_bend_1_mol, pot_torsion_1_mol, & 
                                                pot_imp_torsion_1_mol, pot_opbend_1_mol, &
                                                unit_conv
...
IF (PRESENT(pot_1_mol_list)) THEN
    CALL section_vals_val_get(ener_decomp_section, "INDEX_MOL_DECOMP",& !CHANGE SAM
                                  i_vals=my_mol_index)
END IF
    ...
    pot_shell =  0.0_dp
!CHANGE S/M
    IF (PRESENT(pot_1_mol_list))  pot_1_mol_list(:) = 0.0_dp
    ...
    count_mol = 0 !not needed we could just use i
    ...
    MOL: DO ikind = 1, nkind
       ...
       DO imol = 1, nmol_per_kind
          ...
          pot_bond_1_mol = 0.0_dp !CHANGE SAM
          ...
          BOND: DO ibond = 1, nbonds
             ...
             pot_bond_1_mol = pot_bond_1_mol + energy !CHANGE SAM
          ...
          SHELL: DO ishell=1,nshell
             ...
             pot_shell_1_mol = pot_shell_1_mol + energy
          ...
          UREY_BRADLEY: DO ibend = 1, nub
             ...
             pot_urey_bradley_1_mol = pot_urey_bradley_1_mol + energy
          ...
          BEND: DO ibend = 1, nbends
             ...
             pot_bend_1_mol = pot_bend_1_mol + energy
          ...
          TORSION: DO itorsion = 1, ntorsions
             ...
             DO imul = 1, torsion_list(itorsion)%torsion_kind%nmul
                ...
                pot_torsion_1_mol = pot_torsion_1_mol + energy
          ...
          IMP_TORSION: DO itorsion = 1, nimptors
             ...
             pot_imp_torsion_1_mol = pot_imp_torsion_1_mol + energy
          ...
          OPBEND: DO iopbend = 1, nopbends
             ...
             pot_opbend_1_mol = pot_opbend_1_mol + energy
          ...
          IF (PRESENT(pot_1_mol_list)) THEN
             DO idx=1, SIZE(my_mol_index)
                 IF (my_mol_index(idx) .eq. count_mol) THEN
                    pot_1_mol_list(idx) = pot_bond_1_mol + pot_shell_1_mol + &
                       pot_urey_bradley_1_mol + pot_bend_1_mol + pot_torsion_1_mol + &
                       pot_imp_torsion_1_mol + pot_opbend_1_mol
                 END IF
             END DO
          END IF
