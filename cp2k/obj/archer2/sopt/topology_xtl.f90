# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_xtl.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_xtl.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_xtl.F" 2
...
! *****************************************************************************
  SUBROUTINE read_coordinate_xtl (topology,para_env,subsys_section)
    ...
    INTEGER                                  :: dimensions, handle, icol, ii, &
                                                isym, iw, jj, natom, &
                                                natom_orig, newsize
    ...
    IF (found) THEN
       ...
       IF (dimensions/=3) THEN
          CALL cp__b("topology_xtl.F",136,"XTL file with working DIMENSION different from 3 cannot be parsed!")
       END IF
    ELSE
    ...
    CALL parser_search_string(parser,"CELL",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_xtl.F",149,"The field CELL was not found in XTL file! ")
    ...
    CALL parser_search_string(parser,"ATOMS",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__b("topology_xtl.F",183,"The field ATOMS was not found in XTL file! ")
    CALL parser_get_next_line(parser,1)
    ...
    IF(.NOT.found)&
       CALL cp__b("topology_xtl.F",188,"The field ATOMS in XTL file, is not followed by name and coordinates tags! ")
    CALL parser_get_next_line(parser,1)
    ! Parse real info
    natom = 0
    ...
    DO ii = 1, natom
       ...
       DO jj = ii+1,natom
          ...
          IF(.NOT.(check))CALL cp__a("topology_xtl.F",234)
       END DO
    END DO
    ...
    CALL parser_search_string(parser,"SYM MAT",ignore_case=.FALSE.,found=found,&
         begin_line=.FALSE.,search_from_begin_of_file=.TRUE.)
    IF(.NOT.found)&
       CALL cp__w("topology_xtl.F",249,"The field SYM MAT was not found in XTL file! ")
    ...
    IF (found) THEN
       ...
       DO WHILE (found)
          ...
          Loop_over_unique_atoms: DO ii = 1, natom_orig
             ...
             CALL matvec_3x3(r1, rot_mat, atom_info%r(1:3,ii))
             r1 = r1 + transl_vec
