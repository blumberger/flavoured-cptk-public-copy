# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_energy_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_energy_types.F"
!-----------------------------------------------------------------------------!
...
# 22 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_energy_types.F" 2
...
! *****************************************************************************
  SUBROUTINE allocate_fist_energy(fist_energy, energy_decomp_section)
    ...
    IF (.NOT.ASSOCIATED(fist_energy)) THEN
      ...
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,66,&
...
!      IF (istat /= 0) CALL stop_memory(routineN,moduleN,74,&
!                             "fist_energy",8*dp_size)
!    END IF
!!END MATT

    IF (PRESENT(energy_decomp_section)) THEN
      CALL section_vals_val_get(energy_decomp_section, "INDEX_MOL_DECOMP",&
                                         i_vals=my_mol_index)
      !PRINT *, "my_mol_index", my_mol_index
      ALLOCATE(fist_energy%pot_list(SIZE(my_mol_index)), STAT=istat)
      IF (istat /= 0) CALL stop_memory(routineN,moduleN,84,&
                            "fist_energy",8*dp_size)
    END IF
!END SAM
...
! *****************************************************************************
  SUBROUTINE deallocate_fist_energy(fist_energy)
    ...
    ELSE
      CALL cp_abort(cp__l("fist_energy_types.F",113),&
...
! *****************************************************************************
  SUBROUTINE init_fist_energy(fist_energy)
    ...
    IF (ASSOCIATED(fist_energy)) THEN
      ...
      END IF
    ELSE
      CALL cp_abort(cp__l("fist_energy_types.F",148),&
           "The fist_energy pointer is not associated "//&
