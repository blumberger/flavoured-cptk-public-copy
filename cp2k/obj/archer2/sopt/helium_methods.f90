# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/helium_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/helium_methods.F"
!-----------------------------------------------------------------------------!
...
# 77 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/helium_methods.F" 2
  ...
  SUBROUTINE helium_create( helium, input, solute)
    ...
    IF ( helium%num_env_restart .GT. helium%num_env ) THEN
      ...
      IF (ltmp) THEN
        ...
        CALL cp__w("motion/helium_methods.F",218,msg_str)
      ELSE
        CALL cp__b("motion/helium_methods.F",220,msg_str)
      END IF
    END IF
    ...
    ELSE
        ...
        IF ( ABS(helium%cell_size-rtmp) .GT. 100.0_dp*EPSILON(0.0_dp)* &
           ...
           IF ( expl_dens .AND. expl_nats ) THEN
              ...
              CALL cp__w("motion/helium_methods.F",289,msg_str)
           END IF
           ...
           IF ( .NOT. expl_dens ) THEN
              ...
              IF ( .NOT. expl_nats ) THEN
                 ...
                 CALL cp__w("motion/helium_methods.F",297,msg_str)
              END IF
           ELSE ! ( expl_dens .AND. .NOT. expl_nats )
              ...
              IF ( ABS(helium%cell_size-rtmp) .GT. 100.0_dp*EPSILON(0.0_dp)&
                 ...
                 CALL cp__w("motion/helium_methods.F",311,msg_str)
                 helium%cell_size = rtmp
              END IF
           END IF
        END IF
    ...
    IF (tcheck>100.0_dp*EPSILON(0.0_dp)) THEN
       ...
       CALL cp__b("motion/helium_methods.F",401,msg_str)
    END IF
...
!TODO Adjust helium bead number if not comm. and if coords not given expl.
      ...
      IF (tcheck>1.0e-14_dp) THEN
         ...
         CALL cp__b("motion/helium_methods.F",552,msg_str)
      END IF
      ...
      IF (helium%solute_interaction .EQ. helium_solute_intpot_none) THEN
        ...
        CALL cp__b("motion/helium_methods.F",565,msg_str)
      END IF
...
! the forces are stored
    ...
    IF (actlen/=msglen) THEN
       err_str = TRIM(ADJUSTL(err_str)) // &
                 TRIM(ADJUSTL(stmp)) // "'."
       CALL cp__b("motion/helium_methods.F",1111,err_str)
    END IF
...
! is num_env_restart == helium%num_env_restart ?
    ...
    IF (ic/=msglen) THEN 
       err_str = TRIM(ADJUSTL(err_str)) // &
                 TRIM(ADJUSTL(stmp)) // ")."
       CALL cp__b("motion/helium_methods.F",1243,err_str)
    END IF
      ...
      ELSE
        ...
        SELECT CASE (helium%cell_shape)
          ...
          CASE DEFAULT
            ...
            CALL cp__b("motion/helium_methods.F",1764,err_str)
        END SELECT
      END IF
    END IF
    ...
    IF (explicit) THEN
      ...
      IF ( .NOT. ltmp ) THEN
        ...
        err_str = "Size of the RDF array in the input (" // &
                  ...
                  TRIM(ADJUSTL(stmp2)) // ")."
        CALL cp__b("motion/helium_methods.F",1839,err_str)
      END IF
    ELSE
      RETURN
    END IF
    ...
    IF (explicit) THEN
      ...
      IF ( .NOT. ltmp ) THEN
        ...
        err_str = "Size of the S density array in the input (" // &
                  ...
                  ") .NE. that in the runtime (" // &
                  TRIM(ADJUSTL(stmp2)) // ")."
        CALL cp__b("motion/helium_methods.F",2278,err_str)
      END IF
    ELSE
      RETURN
    END IF
...
!     safer solution: change int_arr_ptr%iap from POINTER to ALLOCATABLE
    ...
    DO i=1, natoms
      ...
      SELECT CASE (mnum)
        ...
        CASE DEFAULT
          ...
          CALL cp__b("motion/helium_methods.F",2377,msg_str)
      END SELECT
    ...
    DO i=1, natoms
      ...
      CASE DEFAULT
          ...
          CALL cp__b("motion/helium_methods.F",2420,msg_str)
      END SELECT
    END DO
...
! get the cell structure from pint_env
    ...
    IF (.NOT. my_orthorhombic) THEN
      CALL cp__b("motion/helium_methods.F",2461,"Helium solvent not implemented for non-orthorhombic cells.")
    ELSE
