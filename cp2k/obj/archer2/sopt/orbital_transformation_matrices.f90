# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_transformation_matrices.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_transformation_matrices.F"
!-----------------------------------------------------------------------------!
...
# 34 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_transformation_matrices.F" 2
...
! *****************************************************************************
  SUBROUTINE create_spherical_harmonics(maxl)
    ...
    IF (current_maxl > -1) THEN
      CALL cp_abort(cp__l("aobasis/orbital_transformation_matrices.F",88),&
           "Spherical harmonics are already allocated. "//&
           "Use the init routine for an update")
    END IF

    IF (maxl < 0) THEN
      CALL cp_abort(cp__l("aobasis/orbital_transformation_matrices.F",94),&
    ...
    nc = ncoset(maxl)
    ns = nsoset(maxl)
    ALLOCATE (c2s_tramat(ns,nc),s2c_tramat(ns,nc),STAT=istat)
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,108,&
                       "c2s_tramat, s2c_tramat",(2*ns*nc)*int_size)
    END IF
    c2s_tramat = 0.0_dp
    s2c_tramat = 0.0_dp
    ...
    DO l=0,maxl
      ...
      IF (istat /= 0) THEN
        CALL stop_memory(routineN,moduleN,121,&
      ...
      IF (istat /= 0) THEN
        CALL stop_memory(routineN,moduleN,128,&
      ...
      IF (istat /= 0) THEN
        CALL stop_memory(routineN,moduleN,135,&
...
! *****************************************************************************
  SUBROUTINE init_spherical_harmonics(maxl,output_unit)
    ...
    IF (maxl < 0) THEN
      CALL cp_abort(cp__l("aobasis/orbital_transformation_matrices.F",334),&
           "A negative maximum angular momentum quantum "//&
           "number is invalid")
    END IF
