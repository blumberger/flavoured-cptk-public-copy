# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph.F"
!-----------------------------------------------------------------------------!
...
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/../base/base_uses.f90" 1
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/metadyn_tools/graph.F" 2
  ...
  IF(argcount==0)THEN
     ...
     WRITE(iw,*)''
     CALL cp__b("metadyn_tools/graph.F",125,"Please provide arguments to run FES!")
  ENDIF
  ...
  IF (COUNT((/l_orac,l_cp2k,l_cpmd/))/=1) &
       CALL cp__b("metadyn_tools/graph.F",200,"Error! You've to specify either ORAC, CP2K or CPMD!!")
  ...
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",214,"Allocation Error")
  ALLOCATE(dp_grid(ndim),stat=stat)
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",216,"Allocation Error")
  ALLOCATE(idw(ndw),stat=stat)
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",218,"Allocation Error")
  ALLOCATE(iperd(ndim),stat=stat)
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",220,"Allocation Error")
  ALLOCATE(iprd(nprd),stat=stat)
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",222,"Allocation Error")
  DO i = 1, ndim
     idw(i)   = i
     iperd(i) = 0
  END DO
  ...
  DO i=1,argcount
     ...
     IF (INDEX(wq_char,'-ndw').NE.0)THEN
        ...
        ALLOCATE(idw(ndw),stat=stat)
        IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",245,"Allocation Error")
     ...
     IF (INDEX(wq_char,'-periodic').NE.0)THEN
        ...
        DEALLOCATE(iprd)
        ALLOCATE(iprd(nprd),stat=stat)
        IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",266,"Allocation Error")
  ...
  IF (l_pmin) THEN
     ...
     IF (ANY(mep_input_data%minima==HUGE(0.0_dp))) &
          CALL cp__b("metadyn_tools/graph.F",343,"-find-path requires the specification of -point-a and -point-b !")
  ELSE
...
! Read parameters for Path_optimization 
  ...
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",391,"Allocation Error")
  i_map = 0
  ...
  IF(l_grid) THEN
     ...
     IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",417,"Allocation Error")
     tmp=ngrid
     DO i=1,ndim
        ngrid(i_map(i))=tmp(i)
     END DO
...
100  REWIND(unit_nr)
  ...
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",448,"Allocation Error")
  ALLOCATE( xf(ndim)         , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",450,"Allocation Error")
  ALLOCATE( ss0(ndim,nt)     , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",452,"Allocation Error")
  ALLOCATE( delta_s(ndim,nt) , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",454,"Allocation Error")
  ALLOCATE( ww(nt)           , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",456,"Allocation Error")
  ALLOCATE( ind(ndim)        , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",458,"Allocation Error")
  ALLOCATE( inds(ndim)       , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",460,"Allocation Error")
  ALLOCATE( nn(ndim,nt)      , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",462,"Allocation Error")
  ALLOCATE( nn_max(ndim)     , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",464,"Allocation Error")
  ALLOCATE( dp_cut(ndim)     , stat=stat )
  IF (stat/=0) CALL cp__b("metadyn_tools/graph.F",466,"Allocation Error")

  IF      (l_cp2k) THEN
     CALL get_val_res(unit=unit_nr,section="&METADYN",subsection="&SPAWNED_HILLS_POS")
     DO i = 1, nt
        READ(unit_nr,*)(ss0(i_map(id),i),id=1,ndim)
     END DO
     CALL get_val_res(unit=unit_nr,section="&METADYN",subsection="&SPAWNED_HILLS_SCALE")
     DO i = 1, nt
        READ(unit_nr,*)(delta_s(i_map(id),i),id=1,ndim)
     END DO
  ...
  IF ((l_pmin) .AND. (l_popt)) CALL cp__b("metadyn_tools/graph.F",674,"USE EITHER -find-path OR -opt-path")
