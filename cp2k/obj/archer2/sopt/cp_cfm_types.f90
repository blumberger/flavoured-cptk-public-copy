# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_cfm_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_cfm_types.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_cfm_types.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_cfm_create(matrix,matrix_struct,name)
    ...
    IF(.NOT.(ASSOCIATED(matrix_struct)))CALL cp__a("fm/cp_cfm_types.F",99)
    ALLOCATE(matrix)
...
! *****************************************************************************
  SUBROUTINE cp_cfm_retain(matrix)
    ...
    IF(.NOT.(ASSOCIATED(matrix)))CALL cp__a("fm/cp_cfm_types.F",137)
    IF(.NOT.(matrix%ref_count>0))CALL cp__a("fm/cp_cfm_types.F",138)
...
! *****************************************************************************
  SUBROUTINE cp_cfm_release(matrix)
    ...
    IF (ASSOCIATED(matrix)) THEN
       IF(.NOT.(matrix%ref_count>0))CALL cp__a("fm/cp_cfm_types.F",153)
       matrix%ref_count=matrix%ref_count-1
...
# 238 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_cfm_types.F"
...
# 289 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fm/cp_cfm_types.F"
  ...
  SUBROUTINE cp_cfm_to_cfm_matrix(source,destination)
    ...
    IF(.NOT. cp2k_is_parallel .OR.&
      ...
      IF(SIZE(source%local_data,1)/=SIZE(destination%local_data,1).OR.&
         ...
         CALL cp__b("fm/cp_cfm_types.F",431,"internal local_data has different sizes")
   ...
   ELSE
      IF(source%matrix_struct%nrow_global/=destination%matrix_struct%nrow_global)&
         CALL cp__b("fm/cp_cfm_types.F",436,"cannot copy between full matrixes of differen sizes")
      IF(source%matrix_struct%ncol_global/=destination%matrix_struct%ncol_global)&
         CALL cp__b("fm/cp_cfm_types.F",438,"cannot copy between full matrixes of differen sizes")
      ...
      CALL cp__b("fm/cp_cfm_types.F",445,"")
...
! *****************************************************************************
  SUBROUTINE cp_cfm_to_fm(msource,mtargetr,mtargeti)
    ...
    IF(.NOT.cp_fm_struct_equivalent(msource%matrix_struct,mtargetr%matrix_struct))&
       CALL cp__b("fm/cp_cfm_types.F",531,"internal local_data has different sizes")
    IF(.NOT.cp_fm_struct_equivalent(msource%matrix_struct,mtargeti%matrix_struct))&
       CALL cp__b("fm/cp_cfm_types.F",533,"internal local_data has different sizes")
    IF(SIZE(a,1)/=SIZE(br,1).OR.SIZE(a,2)/=SIZE(br,2)&
       .OR.SIZE(a,1)/=SIZE(bi,1).OR.SIZE(a,2)/=SIZE(bi,2))&
       CALL cp__b("fm/cp_cfm_types.F",536,"internal local_data has different sizes")
