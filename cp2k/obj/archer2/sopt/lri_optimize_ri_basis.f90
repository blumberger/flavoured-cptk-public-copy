# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_optimize_ri_basis.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_optimize_ri_basis.F"
!-----------------------------------------------------------------------------!
...
# 70 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/lri_optimize_ri_basis.F" 2
...
! *****************************************************************************
  SUBROUTINE get_optimization_parameter(lri_opt,lri_optbas_section,&
    ...
    CASE(do_lri_opt_exps)
       lri_opt%opt_exps=.TRUE.
    CASE DEFAULT
       CALL cp__b("lri_optimize_ri_basis.F",338,"No initialization available?????")
    END SELECT
      ...
      DO ispin = 1, nspin
        ...
        DO WHILE (neighbor_list_iterate(nl_iterator)==0)
          ...
          ELSE
            ...
            trans = .TRUE.
          END IF
          IF(.NOT.(found))CALL cp__a("lri_optimize_ri_basis.F",654)
...
! *****************************************************************************
  SUBROUTINE get_condition_number_of_overlap(lri_env)
    ...
    INTEGER :: handle, iac, iatom, ikind, ilist, info, jatom, jkind, &
      jneighbor, lwork, nfa, nfb, nkind, nlist, nn, nneighbor
