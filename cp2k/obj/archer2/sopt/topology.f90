# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology.F"
!-----------------------------------------------------------------------------!
...
# 90 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology.F" 2
...
! *****************************************************************************
  SUBROUTINE topology_control(atomic_kind_set, particle_set, molecule_kind_set,&
    ...
    CALL read_cell (topology%cell, topology%cell_ref, cell_section=cell_section,&
                    para_env=para_env)
    CALL write_cell(topology%cell, subsys_section, label="CELL_TOP|")
...
! *****************************************************************************
  SUBROUTINE connectivity_control(topology,para_env,qmmm,qmmm_env,subsys_section,&
    ...
    IF (topology%aa_element) THEN
       ...
       IF(.NOT.(check))CALL cp__a("topology.F",330)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE read_topology_conn(topology, conn_type, conn_file_name, para_env,&
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'read_topology_conn', &
      routineP = moduleN//':'//routineN

    CHARACTER(len=default_path_length)       :: filename
    INTEGER                                  :: i_rep, imol, loc_conn_type, &
                                                n_rep, nmol
  ...
  SUBROUTINE coordinate_control(topology,root_section,para_env,subsys_section)
    ...
    IF (topology%coordinate) THEN
       ...
       IF (istat /= 0) THEN
          ...
          CALL cp__b("topology.F",554,TRIM(message))
       END IF
       IF (.NOT.found) THEN
          CALL cp_abort(cp__l("topology.F",557),&
    ...
    IF(topology%natoms<=0)&
       CALL cp__b("topology.F",599,"No atomic coordinates have been found! ")
    CALL timestop(handle)
    CALL cp_print_key_finished_output(iw,logger,subsys_section,&
         "PRINT%TOPOLOGY_INFO")
...
! *****************************************************************************
  SUBROUTINE topology_post_proc_colvar(colvar_p, particle_set)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'topology_post_proc_colvar', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE setup_cell_muc(cell_muc, cell, subsys_section)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'setup_cell_muc', &
      routineP = moduleN//':'//routineN

    INTEGER, DIMENSION(:), POINTER           :: multiple_unit_cell
    REAL(KIND=dp), DIMENSION(3, 3)           :: hmat_ref

    IF(.NOT.(.NOT.ASSOCIATED(cell_muc)))CALL cp__a("topology.F",650)
    ...
    IF (ANY(multiple_unit_cell/=1)) THEN
       ...
       ! Create the MUC cell
       CALL cell_create(cell_muc,hmat=hmat_ref,periodic=cell%perd)
       CALL write_cell(cell_muc,subsys_section,label="CELL_UC|")
    ELSE
