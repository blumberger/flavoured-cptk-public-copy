# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param.F"
!-----------------------------------------------------------------------------!
...
# 33 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pao_param.F" 2
...
! *****************************************************************************
  SUBROUTINE pao_calc_U(pao, penalty)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(arow /= acol) CALL cp__b("pao_param.F",159,"encountered off-diagonal block")
       iatom = arow
       n1 = SIZE(block_X,1)
       n2 = SIZE(block_X,2)

       CALL cp_dbcsr_get_block_p(matrix=pao%matrix_U, row=iatom, col=iatom, block=block_U, found=found)
       IF(.NOT.(ASSOCIATED(block_U)))CALL cp__a("pao_param.F",165)
...
! *****************************************************************************
  SUBROUTINE pao_assert_unitary(pao, matrix_test, matrix_Y)
    ...
    DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
       ...
       IF(.NOT.(ASSOCIATED(block_Y)))CALL cp__a("pao_param.F",274)
       N = SIZE(block_Y, 1)
       M = SIZE(block_Y, 2)
    ...
    IF(delta_max > pao%check_unitary_tol)&
       CALL cp__b("pao_param.F",295,"Found bad unitaryness:"//cp_to_string(delta_max))
