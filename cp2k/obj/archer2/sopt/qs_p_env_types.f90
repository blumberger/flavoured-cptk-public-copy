# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_types.F"
!-----------------------------------------------------------------------------!
...
# 29 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_p_env_types.F" 2
...
SUBROUTINE p_env_retain(p_env)
  ...
  IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_p_env_types.F",118)
  IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_types.F",119)
...
! *****************************************************************************
  SUBROUTINE p_env_release(p_env)
    ...
    IF (ASSOCIATED(p_env)) THEN
       IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_p_env_types.F",140)
       p_env%ref_count=p_env%ref_count-1
       IF (p_env%ref_count<1) THEN
          ...
          IF (ASSOCIATED(p_env%kpp1)) CALL cp_dbcsr_deallocate_matrix_set(p_env%kpp1)
          IF (ASSOCIATED(p_env%p1)) CALL cp_dbcsr_deallocate_matrix_set(p_env%p1)
