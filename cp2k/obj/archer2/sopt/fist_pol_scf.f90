# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_pol_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_pol_scf.F"
!-----------------------------------------------------------------------------!
...
# 40 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/fist_pol_scf.F" 2
...
! *****************************************************************************
  SUBROUTINE fist_pol_evaluate_sc (atomic_kind_set, multipoles, ewald_env, ewald_pw,&
    ...
    INTEGER :: ewald_type, handle, i, iatom, ii, ikind, iter, iw, iw2, j, &
      max_ipol_iter, natom_of_kind, natoms, nkind, ntot
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: iwarn
    REAL(KIND=dp)                            :: apol, cpol, eps_pol, &
...
! compute quadrupole
       ...
       IF(iwarn)&
          CALL cp__w("fist_pol_scf.F",285,"Self-consistent Polarization not converged! ")
    END DO pol_scf
...
! *****************************************************************************
  SUBROUTINE fist_pol_evaluate_cg (atomic_kind_set, multipoles, ewald_env, ewald_pw,&
    ...
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: iwarn
    REAL(KIND=dp)                            :: alpha, apol, beta, denom, &
                                                eps_pol, pot_nonbond_local, &
                                                rmsd
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :)                        :: conjugate, conjugate_applied, &
                                                efield1, efield1_ext, &
                                                residual, tmp_dipoles
    TYPE(atomic_kind_type), POINTER          :: atomic_kind
    TYPE(cp_logger_type), POINTER            :: logger
    ...
    pol_scf: DO iter=1, max_ipol_iter
       ...
       IF (iwarn.AND.iw>0) WRITE(iw,FMT='(T5,"POL_SCF|",1X,"Self-consistent Polarization not converged!")')
       IF(iwarn)&
          CALL cp__w("fist_pol_scf.F",582,"Self-consistent Polarization not converged! ")
    END DO pol_scf
    ...
    IF (debug_this_module) THEN
       ...
       IF ( rmsd > eps_pol ) THEN
         CALL cp__w("fist_pol_scf.F",614,"Error in the conjugate gradient method for self-consistent polarization! ")
       END IF
    ELSE
       ! Now evaluate after convergence to obtain forces and converged energies
       CALL eval_pol_ewald(ewald_type, ewald_env, ewald_pw, fist_nonbond_env, cell,&
...
! *****************************************************************************
  SUBROUTINE eval_pol_ewald(ewald_type, ewald_env, ewald_pw, fist_nonbond_env,&
    ...
    CASE(do_ewald_pme)
       CALL cp__b("fist_pol_scf.F",732,"Multipole Ewald not yet implemented within a PME scheme!")
    CASE(do_ewald_spme)
       CALL cp__b("fist_pol_scf.F",734,"Multipole Ewald not yet implemented within a SPME scheme!")
    END SELECT
    CALL timestop(handle)
