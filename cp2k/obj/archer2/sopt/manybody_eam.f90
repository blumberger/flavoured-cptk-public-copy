# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_eam.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_eam.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/manybody_eam.F" 2
...
! *****************************************************************************
  SUBROUTINE density_nonbond (fist_nonbond_env, particle_set, cell, para_env)
    ...
    INTEGER :: atom_a, atom_b, handle, i, i_a, i_b, iend, igrp, ikind, ilist, &
      ipair, iparticle, istart, jkind, kind_a, kind_b, nkinds, nparticle
       ...
       DO jkind = ikind, nkinds
          DO i = 1, SIZE(potparm%pot(ikind,jkind)%pot%type)
             IF (potparm%pot(ikind,jkind)%pot%type(i)==ea_type) THEN
                ...
                IF(.NOT.(eam_kinds_index(ikind,jkind)==-1))CALL cp__a("manybody_eam.F",87)
                IF(.NOT.(eam_kinds_index(jkind,ikind)==-1))CALL cp__a("manybody_eam.F",88)
