# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_force_eval.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_force_eval.F"
!-----------------------------------------------------------------------------!
...
MODULE input_cp2k_force_eval
  ...
  USE input_constants,                 ONLY: &
       do_eip, do_fist, do_mixed, do_qmmm, do_qs, do_stress_analytical, &
       do_stress_diagonal_anal, do_stress_diagonal_numer, do_stress_none, &
...
# 46 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input_cp2k_force_eval.F" 2
...
!> \author fawzi
!> \note Added a keyword letting CP2K know that we want to decompose the
!>        intra-energy per molecule.
! *****************************************************************************
  SUBROUTINE create_force_eval_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_force_eval.F",73)
 ...
 !CHANGE MATT
   CALL keyword_create(keyword, name="DO_DECOMP",&
                       description="Whether to apply addition-subtraction scheme."//&
                       "This keyword is part of FIST method. When True, each"//&
                       "energy term in the given FORCE_EVAL is decomposed in molecular contributions." ,&
                       usage="NUM_DECOMP {logical}",&
 ...
 !END CHANGE 
    ...
    CALL create_ener_decomp_section(subsection)
    CALL section_add_subsection(section, subsection)
    CALL section_release(subsection)
...
! *****************************************************************************
!> \brief Creates the section for decomposing the energies in fist
!> \param section ... 
!> \date 07.2018
!> \author Matt
! *****************************************************************************
  SUBROUTINE create_ener_decomp_section(section)
    TYPE(section_type), POINTER              :: section

    CHARACTER(len=*), PARAMETER :: routineN = 'create_ener_decomp_section', &
      routineP = moduleN//':'//routineN

    TYPE(keyword_type), POINTER              :: keyword

    NULLIFY(section, keyword)
    CALL section_create(section,name="ENERGY_DECOMP",&
         description="Section controlling the decomposition of energies for"//&
         " individual molecules in FIST and addition-subtraction scheme",&
         n_keywords=2, n_subsections=0, repeats=.FALSE.)

    CALL keyword_create(keyword, name="NUM_ACTIVE_ATOMS", &
                        description="Number of atoms in all active molecules", &
                        type_of_var=integer_t, &
                        usage="NUM_ACTIVE_ATOMS {INT}")
    CALL section_add_keyword(section, keyword)
    CALL keyword_release(keyword)

    CALL keyword_create(keyword, name="INDEX_MOL_DECOMP",&
         description='Specify the index -ordering referred to xyz (or topology) file-'//&
                     ' of the molecules part of the active region', &
  ...
  END SUBROUTINE create_ener_decomp_section
! *****************************************************************************
...
! *****************************************************************************
  SUBROUTINE create_ext_pot_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_force_eval.F",230)
...
! *****************************************************************************
  SUBROUTINE create_rescale_force_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_force_eval.F",302)
...
! *****************************************************************************
  SUBROUTINE create_f_env_print_section(section)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(section)))CALL cp__a("input_cp2k_force_eval.F",338)
