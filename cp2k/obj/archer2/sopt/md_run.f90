# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_run.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_run.F"
!-----------------------------------------------------------------------------!
...
# 103 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-single-phase/cp2k/src/motion/md_run.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_mol_dyn_low(md_env, md_section, motion_section, force_env, globenv, hmc_e_initial,hmc_e_final, mdctrl)
    ...
    IF(.NOT.(ASSOCIATED(globenv)))CALL cp__a("motion/md_run.F",221)
    IF(.NOT.(ASSOCIATED(force_env)))CALL cp__a("motion/md_run.F",222)
    ...
    IF(simpar%ensemble == reftraj_ensemble .AND. ehrenfest_md)&
          CALL cp__b("motion/md_run.F",284,"Ehrenfest MD does not support reftraj ensemble ")
    ...
    IF ( (simpar%ensemble==npt_i_ensemble).OR.&
       ...
       IF(.NOT.check)&
          CALL cp_abort(cp__l("motion/md_run.F",339),&
       ...
       END IF
       IF(.NOT.check)&
          CALL cp_abort(cp__l("motion/md_run.F",353),&
...
print *, "FIRST CALL: force_env_calc_energy_force"
                ...
                IF (force_env%meta_env%well_tempered) THEN
                 ...
                 IF (force_env%meta_env%wtgamma>EPSILON(1._dp)) THEN
                    ...
                    IF (force_env%meta_env%delta_t>EPSILON(1._dp)) THEN
                       ...
                       IF(.NOT.check)&
                          CALL cp_abort(cp__l("motion/md_run.F",405),&
...
! if we need the initial kinetic energy for Hybrid Monte Carlo
    ...
    DO istep=1, simpar%nsteps, md_stride
       ...
       IF (should_stop) THEN
          ...
          IF(ehrenfest_md)THEN
             CALL rt_prop_output(force_env%qs_env,ehrenfest)
             CALL rt_write_input_restart(md_env,force_env)
          END IF
          EXIT
       END IF
