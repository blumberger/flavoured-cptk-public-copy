# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xalpha.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xalpha.F"
!-----------------------------------------------------------------------------!
...
# 31 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_xalpha.F" 2
...
! *****************************************************************************
  SUBROUTINE xalpha_lda_eval(rho_set,deriv_set,order,xa_params,xa_parameter)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_xalpha.F",170)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_xalpha.F",171)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_xalpha.F",172)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_xalpha.F",173)
    ...
    END IF
    IF ( order>3.OR.order<-3) THEN
       CALL cp__b("xc/xc_xalpha.F",209,"derivatives bigger than 3 not implemented")
    END IF
...
! *****************************************************************************
  SUBROUTINE xalpha_lsd_eval (rho_set,deriv_set,order,xa_params,xa_parameter)
    ...
    IF(.NOT.(ASSOCIATED(rho_set)))CALL cp__a("xc/xc_xalpha.F",252)
    IF(.NOT.(rho_set%ref_count>0))CALL cp__a("xc/xc_xalpha.F",253)
    IF(.NOT.(ASSOCIATED(deriv_set)))CALL cp__a("xc/xc_xalpha.F",254)
    IF(.NOT.(deriv_set%ref_count>0))CALL cp__a("xc/xc_xalpha.F",255)
    ...
    DO ispin=1,2
       ...
       END IF
       IF ( order>3.OR.order<-3) THEN
          CALL cp__b("xc/xc_xalpha.F",298,"derivatives bigger than 3 not implemented")
       END IF
    END DO
    CALL timestop(handle)
