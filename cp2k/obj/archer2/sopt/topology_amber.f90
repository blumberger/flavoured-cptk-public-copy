# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_amber.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_amber.F"
!-----------------------------------------------------------------------------!
...
# 45 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_amber.F" 2
...
! *****************************************************************************
  SUBROUTINE read_coordinate_crd (topology,para_env,subsys_section)
    ...
    IF ((my_end).AND.(j/=natom-MOD(natom,2)+1)) THEN
       IF(j/=natom)&
          CALL cp__b("topology_amber.F",188,"Error while reading CRD file. Unexpected end of file.")
    ...
    IF (my_end) THEN
       IF(j/=natom)&
          CALL cp__w("topology_amber.F",211,"No VELOCITY or BOX information found in CRD file. ")
    ELSE
       ! Velocities
       ...
       IF ((my_end).AND.(j/=natom-MOD(natom,2)+1)) THEN
          IF(j/=natom)&
             CALL cp_warn(cp__l("topology_amber.F",235),&
    ...
    IF (my_end) THEN
       IF(j/=natom)&
          CALL cp__w("topology_amber.F",260,"BOX information missing in CRD file. ")
    ELSE
       IF(j/=natom)&
          CALL cp_warn(cp__l("topology_amber.F",263),&
...
! *****************************************************************************
  SUBROUTINE rdparm_amber_8(filename, output_unit, para_env, do_connectivity,&
    ...
    INTEGER :: handle, handle2, i, ifbox, ifcap, ifpert, index_now, info(31), &
      ...
      ntheta_prev, ntheth, ntypes, numang, numbnd, numextra, unique_torsions
       ...
       DO WHILE (get_section_parmtop(parser, section, input_format))
          ...
          CASE("POINTERS")
             ...
             IF (do_connectivity) THEN
                ...
                IF(.NOT.(check))CALL cp__a("topology_amber.F",463)
                natom_prev = 0
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_i1(parser, section, array1, dim)
    ...
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1044),&
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_i3(parser, section, array1, array2, array3, dim)
    ...
    ! Trigger end of file aborting
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1094),&
            "End of file while reading section "//TRIM(section)//" in amber topology file!")
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_i4(parser, section, array1, array2, array3, array4, dim)
    ...
    ! Trigger end of file aborting
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1150),&
            "End of file while reading section "//TRIM(section)//" in amber topology file!")
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_i5(parser, section, array1, array2, array3, array4, &
    ...
    ! Trigger end of file aborting
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1214),&
            "End of file while reading section "//TRIM(section)//" in amber topology file!")
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_c1(parser, section, array1, dim)
    ...
    ! Trigger end of file aborting
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1252),&
            "End of file while reading section "//TRIM(section)//" in amber topology file!")
...
! *****************************************************************************
  SUBROUTINE rd_amber_section_r1(parser, section, array1, dim)
    ...
    ! Trigger end of file aborting
    IF(my_end.AND.(i<=dim))&
       CALL cp_abort(cp__l("topology_amber.F",1289),&
            "End of file while reading section "//TRIM(section)//" in amber topology file!")
  ...
  FUNCTION  get_section_parmtop(parser, section, input_format) RESULT(another_section)
    ...
    IF (found) THEN
       ...
       IF(INDEX(parser%input_line,"%FORMAT")==0 .OR. my_end)&
          CALL cp__b("topology_amber.F",1324,"Expecting %FORMAT. Not found! Abort reading of AMBER topology file!")
...
! *****************************************************************************
  FUNCTION check_amber_8_std(parser, output_unit) RESULT(found_AMBER_V8)
    ...
    IF(.NOT.found_AMBER_V8)&
       CALL cp_abort(cp__l("topology_amber.F",1352),&
  ...
  SUBROUTINE post_process_torsions_info(label_a, label_b, label_c, label_d, k,&
    ...
    IF (nphi/=0) THEN
       ...
       IF (m(iphi)-pn   (icp(iwork(1))).GT.EPSILON(1.0_dp)) THEN
         ...
         CALL cp__b("topology_amber.F",1638,"")
       ENDIF
       ...
       DO i = 2, nphi
          ...
          IF  ((work_label(1,i)/=label_a(iphi)).OR.&
             ...
             IF (m(iphi)-pn   (icp(iwork(i))).GT.EPSILON(1.0_dp)) THEN
               ! non integer torsions not supported
               CALL cp__b("topology_amber.F",1672,"")
             ENDIF
...
! *****************************************************************************
  SUBROUTINE post_process_LJ_info(atom_label, eps, sigma, particle_set,&
    ...
    IF(.NOT.(check))CALL cp__a("topology_amber.F",1743)
    my_sigma = 0.0_dp
    my_eps   = 0.0_dp
    IF (F6/=0.0_dp) THEN
    ...
    DO i = 2, natom
       IF  (work_label(i)/=atom_label(nsize)) THEN
          ...
          check = (F6==0.0_dp).EQV.(F12==0.0_dp)
          IF(.NOT.(check))CALL cp__a("topology_amber.F",1767)
          my_sigma = 0.0_dp
          my_eps   = 0.0_dp
          IF (F6/=0.0_dp) THEN
             sigma6   = (2.0_dp*F12/F6)
