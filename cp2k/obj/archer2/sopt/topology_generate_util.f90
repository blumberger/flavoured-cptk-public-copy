# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_generate_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_generate_util.F"
!-----------------------------------------------------------------------------!
...
# 63 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/topology_generate_util.F" 2
...
! (each bond is present in the forward and backward direction
    ...
    IF(.NOT.(check))CALL cp__a("topology_generate_util.F",123)
    DO i = 1, natom
  ...
  RECURSIVE SUBROUTINE generate_molname_low(i, atom_bond_list, molname, id_molname)
    ...
    IF (debug_this_module) THEN
       ...
       IF ((TRIM(id2str(id_molname(i)))/="__UNDEF__").AND.&
          ...
          CALL cp__b("topology_generate_util.F",167,"")
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE topology_generate_molecule(topology,qmmm,qmmm_env,subsys_section)
    ...
    DO i = 2, natom
       IF (mol_typ /= wrk1(i)) THEN
          ...
          IF (iend-istart+1 /= nlocl) THEN
             ...
             CALL cp_abort(cp__l("topology_generate_util.F",338),&
    ...
    IF (iend-istart+1 /= nlocl) THEN
       IF (debug_this_module) WRITE(*,*)iend, istart, iend-istart+1, first, last, nlocl
       CALL cp_abort(cp__l("topology_generate_util.F",356),&
            "CP2K requires molecules to be contiguous and we have detected a non contiguous one!! "//&
            "In particular a molecule defined from index ("//cp_to_string(first)//") to ("//&
            cp_to_string(last)//") contains other molecules, not connected! "//&
            "Too late at this stage everything should be already ordered! "//&
    ...
    ELSE
       ...
       DO i=2,natom
          IF (atom_info%map_mol_typ(i) /= mol_typ) THEN
             ...
             IF(.NOT.(myind/=atom_info%map_mol_num(i-1)))CALL cp__a("topology_generate_util.F",386)
    ...
    IF (my_qmmm) THEN
       ...
       IF(.NOT.(ALL(qm_atom_index /= 0)))CALL cp__a("topology_generate_util.F",456)
       DO myind = 1, SIZE(qm_atom_index)
          ...
          IF(.NOT.(((ifirst/=0).OR.(ilast/=natom))))CALL cp__a("topology_generate_util.F",464)
          ...
          IF (ifirst /= 1) THEN
             ...
             IF(.NOT.(jump1<=1.AND.jump1>=0))CALL cp__a("topology_generate_util.F",480)
             jump1 = ABS(jump1-1)
          ELSE
             jump1 = 0
          END IF
          IF (ilast /= natom) THEN
             ...
             IF(.NOT.(jump2<=1.AND.jump2>=0))CALL cp__a("topology_generate_util.F",487)
             jump2 = ABS(jump2-1)
          ELSE
             jump2 = 0
          END IF
    ...
    IF (iw>0) THEN
       ...
       DO i = 1, ntype
          ...
          IF(.NOT.(last-first+1==atom_in_kind))CALL cp__a("topology_generate_util.F",542)
          ...
          DO j = 1, max_mol_num
             IF (COUNT(atom_info%map_mol_num(first:last)==j) /= atom_in_mol) THEN
                ...
                CALL cp_abort(cp__l("topology_generate_util.F",556),&
...
! *****************************************************************************
  SUBROUTINE topology_generate_bond(topology,para_env,subsys_section)
    ...
    DO iatom=1,natom
       ...
       ELSE
          CALL cp__b("topology_generate_util.F",655,"Illegal bondparm_type")
       END IF
    ...
    IF (my_maxrad*bondparm_factor>r_max(1,1).AND.(.NOT.topology%molname_generated)) THEN
       ...
       END IF
       CALL cp__b("topology_generate_util.F",688,"")
    END IF
    DO i=1,natom
    ...
    DO WHILE (.NOT.connectivity_ok)
       ...
       DO iatm1 = 1, natom
          ...
          DO j = 1,SIZE(bond_list(iatm1)%array1)
             ...
             IF(r2 <= r_minsq(1,1)) THEN
                CALL cp_abort(cp__l("topology_generate_util.F",770),&
       ...
       DO iatm1 = 1, natom
          ...
          DO j = 1,SIZE(bond_list(iatm1)%array1)
             ...
             r2 = DOT_PRODUCT(dr,dr)
             IF(r2 <= r_minsq(1,1)) THEN
                CALL cp_abort(cp__l("topology_generate_util.F",818),&
                     "bond distance between atoms less then the smallest distance provided "//&
                     "in input "//cp_to_string(tmp)//" [bohr]")
             END IF
             IF (r2 <= r2_min) THEN
       ...
       IF (my_maxrad*bondparm_factor>r_max(1,1).AND.(.NOT.topology%molname_generated)) THEN
          IF (output_unit>0) THEN
             ...
             WRITE(output_unit,'(T2,"GENERATE|",2(A,F11.6),A)')&
                  ...
                  " Present BONDLENGTH_MAX (",r_max(1,1)," )"
          END IF
          CALL cp__b("topology_generate_util.F",878,"")
       END IF
    END DO
       ...
       DO ibond=1,SIZE(conn_info%bond_a)
          ...
          ELSE
             IF(atom_info%id_molname(iatom)/=atom_info%id_molname(jatom)) THEN
                CALL cp__b("topology_generate_util.F",925,"Bonds between different molecule types?")
             END IF
          END IF
       END DO
    END IF
  ...
  SUBROUTINE connectivity_external_control(section, Iarray1, Iarray2, Iarray3, Iarray4, nvar,&
    ...
    IF (explicit) THEN
       ...
       CASE DEFAULT
          ...
          CALL cp__b("topology_generate_util.F",1145,"")
       END SELECT
       ...
       DO i = 1, n_rep
          ...
          DO j = 1, n_rep_val
             CALL section_vals_val_get(section,"ATOMS",i_rep_section=i,i_rep_val=j,&
                  i_vals=atlist)
 IF(.NOT.(SIZE(atlist)==nsize))CALL cp__a("topology_generate_util.F",1157)
       ...
       CASE DEFAULT
          ! Should never reach this point
          CALL cp__b("topology_generate_util.F",1249,"")
       END SELECT
    END IF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE match_iterative_path(Iarray1, Iarray2, Iarray3,&
    ...
    IF(.NOT.(check))CALL cp__a("topology_generate_util.F",1644)
    IF (.NOT.PRESENT(Ilist)) THEN
       SELECT CASE(max_levl)
       CASE(2)
    IF(.NOT.(.NOT.PRESENT(Iarray2)))CALL cp__a("topology_generate_util.F",1648)
    IF(.NOT.(.NOT.PRESENT(Iarray3)))CALL cp__a("topology_generate_util.F",1649)
    IF(.NOT.(.NOT.PRESENT(Oarray3)))CALL cp__a("topology_generate_util.F",1650)
    IF(.NOT.(.NOT.PRESENT(Oarray4)))CALL cp__a("topology_generate_util.F",1651)
       CASE(3)
    IF(.NOT.(PRESENT(Iarray2)))CALL cp__a("topology_generate_util.F",1653)
    IF(.NOT.(.NOT.PRESENT(Iarray3)))CALL cp__a("topology_generate_util.F",1654)
    IF(.NOT.(PRESENT(Oarray3)))CALL cp__a("topology_generate_util.F",1655)
    IF(.NOT.(.NOT.PRESENT(Oarray4)))CALL cp__a("topology_generate_util.F",1656)
       CASE(4)
    IF(.NOT.(PRESENT(Iarray2)))CALL cp__a("topology_generate_util.F",1658)
    IF(.NOT.(PRESENT(Iarray3)))CALL cp__a("topology_generate_util.F",1659)
    IF(.NOT.(PRESENT(Oarray3)))CALL cp__a("topology_generate_util.F",1660)
    IF(.NOT.(PRESENT(Oarray4)))CALL cp__a("topology_generate_util.F",1661)
       END SELECT
    END IF
    natom   = SIZE(Iarray1)
    IF (.NOT.PRESENT(Ilist)) THEN
    ...
    ELSE
       ...
       DO j = 1, SIZE(Iarray1(i)%array1)
          ...
          ELSEIF (it_levl==max_levl) THEN
             ...
             CASE DEFAULT
                !should never reach this point
 CALL cp__b("topology_generate_util.F",1746,"")
             END SELECT
             Ilist(it_levl) = -1
          ELSE
             !should never reach this point
 CALL cp__b("topology_generate_util.F",1751,"")
          END IF
       END DO
    END IF
