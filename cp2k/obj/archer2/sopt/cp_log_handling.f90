# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_log_handling.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_log_handling.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_log_handling.F" 2
...
! *****************************************************************************
  FUNCTION cp_default_logger_stack_size() RESULT(res)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'cp_default_logger_stack_size', &
...
! *****************************************************************************
  SUBROUTINE cp_add_default_logger(logger)
    ...
    IF (stack_pointer+1>max_stack_pointer) THEN
       CALL cp_abort(cp__l("common/cp_log_handling.F",198),routineP//&
...
! *****************************************************************************
  SUBROUTINE cp_rm_default_logger()
    IF (stack_pointer-1<0) THEN
       CALL cp_abort(cp__l("common/cp_log_handling.F",218),moduleN//":cp_rm_default_logger"//&
...
! *****************************************************************************
  FUNCTION cp_get_default_logger() RESULT(res)
    ...
    IF ( .NOT. stack_pointer>0 ) THEN
       CALL cp_abort(cp__l("common/cp_log_handling.F",241),"cp_log_handling:cp_get_default_logger"//&
    ...
    IF (.NOT.ASSOCIATED(res)) THEN
       CALL cp_abort(cp__l("common/cp_log_handling.F",246),"cp_log_handling:cp_get_default_logger"//&
...
! *****************************************************************************
  SUBROUTINE cp_logger_create(logger, para_env,print_level,&
    ...
    IF (stat/=0)&
       CALL cp__b("common/cp_log_handling.F",311,routineP//" could not ALLOCATE a logger")
    ...
    IF (PRESENT(template_logger)) THEN
       IF (template_logger%ref_count<1) &
            CALL cp__b("common/cp_log_handling.F",321,routineP//" template_logger%ref_count<1")
    ...
    IF (.NOT.ASSOCIATED(logger%para_env))&
       CALL cp__b("common/cp_log_handling.F",357,routineP//" para env not associated")
    IF (logger%para_env%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",359,routineP//" para_env%ref_count<1")
...
! *****************************************************************************
  SUBROUTINE cp_logger_retain(logger)
    ...
    IF (.NOT.ASSOCIATED(logger))&
       CALL cp__b("common/cp_log_handling.F",425,routineP//" logger not associated")
    IF (logger%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",427,routineP//" logger%ref_count<1")
...
! *****************************************************************************
  SUBROUTINE cp_logger_release(logger)
    ...
    IF (ASSOCIATED(logger)) THEN
       IF (logger%ref_count<1)&
          CALL cp__b("common/cp_log_handling.F",446,routineP//" logger%ref_count<1")
       logger%ref_count=logger%ref_count-1
...
! *****************************************************************************
  FUNCTION cp_logger_would_log(logger, level) RESULT(res)
    ...
    IF (lggr%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",496,routineP//" logger%ref_count<1")
...
! *****************************************************************************
  SUBROUTINE cp_logger_set_log_level(logger, level)
    ...
    IF (.NOT.ASSOCIATED(logger))&
       CALL cp__b("common/cp_log_handling.F",566,routineP//" logger not associated")
    IF (logger%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",568,routineP//" logger%ref_count<1")
...
! *****************************************************************************
  RECURSIVE FUNCTION cp_logger_get_default_unit_nr(logger, local, skip_not_ionode) RESULT(res)
    ...
    IF (.NOT.ASSOCIATED(lggr)) lggr => cp_get_default_logger()
    IF (lggr%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",606,routineP//" logger%ref_count<1")
  ...
  SUBROUTINE my_cp_para_env_release(para_env)
    ...
    IF (ASSOCIATED(para_env)) THEN
       IF (para_env%ref_count<1) THEN
          CALL cp__b("common/cp_log_handling.F",697,routineP//" para_env%ref_count<1")
       END IF
...
! *****************************************************************************
  SUBROUTINE cp_logger_generate_filename(logger,res, root, postfix,&
    ...
    IF (.NOT.ASSOCIATED(lggr)) lggr => cp_get_default_logger()
    IF (lggr%ref_count<1)&
       CALL cp__b("common/cp_log_handling.F",746,routineP//" logger%ref_count<1")
    IF (PRESENT(local)) loc=local
    IF (loc) THEN
...
! *****************************************************************************
  SUBROUTINE cp_logger_set(logger,local_filename,global_filename)
    ...
    IF (.NOT.ASSOCIATED(logger))&
       CALL cp__b("common/cp_log_handling.F",775,routineP//" unassociated logger")
