# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/multipole_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/multipole_types.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/subsys/multipole_types.F" 2
...
! *****************************************************************************
  SUBROUTINE create_multipole_type(multipoles, particle_set, subsys_section, max_multipole)
    ...
    CASE DEFAULT
       CALL cp__b("subsys/multipole_types.F",104,"")
    END SELECT
    ...
    IF (multipoles%task(2)) THEN
       ...
       IF (explicit) THEN
          ...
          IF(.NOT.(n_rep==nparticles))CALL cp__a("subsys/multipole_types.F",126)
    ...
    IF (multipoles%task(3)) THEN
       ...
       IF (explicit) THEN
          CALL section_vals_val_get(work_section,"_DEFAULT_KEYWORD_",n_rep_val=n_rep)
          IF(.NOT.(n_rep==nparticles))CALL cp__a("subsys/multipole_types.F",142)
          DO iparticle = 1, n_rep
             CALL section_vals_val_get(work_section,"_DEFAULT_KEYWORD_",i_rep_val=iparticle, r_vals=work)
             DO i = 1, 3
                DO j = 1, 3
...
! *****************************************************************************
  SUBROUTINE release_multipole_type(multipoles)
    ...
    IF (ASSOCIATED(multipoles)) THEN
       IF(.NOT.(multipoles%ref_count>0))CALL cp__a("subsys/multipole_types.F",172)
...
!> \author Teodoro Laino
! *****************************************************************************
  SUBROUTINE retain_multipole_type(multipoles)
    TYPE(multipole_type), POINTER            :: multipoles
    ...
    IF (ASSOCIATED(multipoles)) THEN
       IF(.NOT.(multipoles%ref_count>0))CALL cp__a("subsys/multipole_types.F",206)
       multipoles%ref_count=multipoles%ref_count+1
    END IF
