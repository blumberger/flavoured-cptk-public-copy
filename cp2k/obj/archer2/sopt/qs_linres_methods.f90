# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_methods.F"
!-----------------------------------------------------------------------------!
...
# 126 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE linres_solver(p_env, qs_env, psi1, h1_psi0, psi0_order, should_stop)
    ...
    INTEGER                                  :: handle, ispin, iter, maxnmo, &
                                                maxnmo_o, nao, ncol, nmo, &
                                                nspins, output_unit
    LOGICAL                                  :: restart
...
!
    ...
    CALL preortho(psi1,mo_coeff_array,Sc,buf)
    !
    ...
    !
    ! A * x0
    CALL apply_op(qs_env,p_env,psi0_order,psi1,Ap,chc,buf)
    !
    !
    ! r_0 = b - Ax0
    DO ispin = 1,nspins
    ...
    !
    ! proj r
    CALL postortho(r,mo_coeff_array,Sc,buf)
    !
    ! preconditioner
    linres_control%flag=""
    ...
    DO ispin = 1,nspins
       ...
       IF(tr_rz0(ispin).LT.0.0_dp) CALL cp__b("qs_linres_methods.F",453,"tr(r_j*z_j) < 0")
...
!       t1 = m_walltime()
       ...
       CALL apply_op(qs_env,p_env,psi0_order,p,Ap,chc,buf)
       !
       !
!       ! proj Ap onto the virtual subspace
       CALL postortho(Ap,mo_coeff_array,Sc,buf)
       !
       !
       DO ispin = 1,nspins
          !
          ...
          IF (tr_pAp(ispin).LT.0.0_dp) THEN
             ...
             CALL postortho(Ap,mo_coeff_array,Sc,buf)
             CALL cp_fm_trace(Ap(ispin)%matrix,p(ispin)%matrix,tr_pAp(ispin))
             CALL cp__b("qs_linres_methods.F",534,"tr(Ap_j*p_j) < 0")
          END IF
          !
...
!       ! proj Ap onto the virtual subspace
       ...
       DO ispin = 1,nspins
          ...
          IF(tr_rz1(ispin).LT.0.0_dp) CALL cp__b("qs_linres_methods.F",600,"tr(r_j+1*z_j+1) < 0")
...
!> \param buf ...
! *****************************************************************************
  SUBROUTINE apply_op(qs_env,p_env,c0,v,Av,chc,buf)
    !
    TYPE(qs_environment_type), POINTER       :: qs_env
    TYPE(qs_p_env_type), POINTER             :: p_env
    TYPE(cp_fm_p_type), DIMENSION(:), &
...
!
    ...
    CALL timeset(routineN,handle)
    !
    IF(.NOT.(ASSOCIATED(v)))CALL cp__a("qs_linres_methods.F",682)
    IF(.NOT.(ASSOCIATED(Av)))CALL cp__a("qs_linres_methods.F",683)
    IF(.NOT.(ASSOCIATED(chc)))CALL cp__a("qs_linres_methods.F",684)
    IF(.NOT.(ASSOCIATED(buf)))CALL cp__a("qs_linres_methods.F",685)
    !
    CALL get_qs_env(qs_env=qs_env,&
                    matrix_ks=matrix_ks,&
                    matrix_s=matrix_s,&
    ...
    DO ispin = 1,nspins
       CALL apply_op_1(v(ispin)%matrix,Av(ispin)%matrix,matrix_ks(ispin)%matrix,&
                       matrix_s(1)%matrix,chc(ispin)%matrix,buf)
    ENDDO
    ...
    IF(linres_control%do_kernel) THEN!.AND.chksum.GT.1.0E-10_dp) THEN
       ...
       chksum = 0.0_dp
       DO ispin = 1,nspins
          chksum = chksum + cp_dbcsr_checksum(p_env%p1(ispin)%matrix)
       ENDDO
       ...
       IF(chksum.GT.1.0E-14_dp) THEN
          ...
          DO ispin = 1,nspins
             CALL cp_dbcsr_copy(rho1_ao(ispin)%matrix,p_env%p1(ispin)%matrix)
          ENDDO
          ...
          CALL apply_op_2(qs_env,p_env,c0,v,Av,chc,buf)
...
!
    ...
    IF(.NOT.(ASSOCIATED(c0)))CALL cp__a("qs_linres_methods.F",777)
    IF(.NOT.(ASSOCIATED(c1)))CALL cp__a("qs_linres_methods.F",778)
    IF(.NOT.(ASSOCIATED(dm)))CALL cp__a("qs_linres_methods.F",779)

    nspins = SIZE(dm,1)

    DO ispin = 1,nspins
       CALL cp_dbcsr_set(dm(ispin)%matrix,0.0_dp)
       CALL cp_fm_get_info(c0(ispin)%matrix,ncol_global=ncol)
...
!> \param buf ...
! *****************************************************************************
  SUBROUTINE apply_op_1(v,Av,matrix_ks,matrix_s,chc,buf)
    !
...
!
    ...
    CALL timeset(routineN,handle)
    !
    IF(.NOT.(ASSOCIATED(v)))CALL cp__a("qs_linres_methods.F",824)
    IF(.NOT.(ASSOCIATED(Av)))CALL cp__a("qs_linres_methods.F",825)
    IF(.NOT.(ASSOCIATED(matrix_ks)))CALL cp__a("qs_linres_methods.F",826)
    IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("qs_linres_methods.F",827)
    IF(.NOT.(ASSOCIATED(chc)))CALL cp__a("qs_linres_methods.F",828)
    IF(.NOT.(ASSOCIATED(buf)))CALL cp__a("qs_linres_methods.F",829)
    !
...
!> \param buf ...
! *****************************************************************************
  SUBROUTINE apply_op_2(qs_env,p_env,c0,v,Av,chc,buf)
    !
    TYPE(qs_environment_type), POINTER       :: qs_env
    TYPE(qs_p_env_type), POINTER             :: p_env
    TYPE(cp_fm_p_type), DIMENSION(:), &
    ...
    IF(.NOT.(ASSOCIATED(c0)))CALL cp__a("qs_linres_methods.F",906)
    IF(.NOT.(ASSOCIATED(v)))CALL cp__a("qs_linres_methods.F",907)
    IF(.NOT.(ASSOCIATED(Av)))CALL cp__a("qs_linres_methods.F",908)
    IF(.NOT.(ASSOCIATED(chc)))CALL cp__a("qs_linres_methods.F",909)
    IF(.NOT.(ASSOCIATED(buf)))CALL cp__a("qs_linres_methods.F",910)


    IF(.NOT.(ASSOCIATED(p_env%kpp1_env)))CALL cp__a("qs_linres_methods.F",913)
    IF(.NOT.(ASSOCIATED(p_env%kpp1)))CALL cp__a("qs_linres_methods.F",914)
    rho1    => p_env%rho1
    rho1_xc => p_env%rho1_xc
    IF(.NOT.(ASSOCIATED(rho1)))CALL cp__a("qs_linres_methods.F",917)

    IF(.NOT.(p_env%kpp1_env%ref_count>0))CALL cp__a("qs_linres_methods.F",919)
    ...
    IF(gapw_xc) THEN
       IF(.NOT.(ASSOCIATED(rho1_xc)))CALL cp__a("qs_linres_methods.F",937)
    END IF
...
! gets the tmp grids
    IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("qs_linres_methods.F",949)
    ...
    DO ispin=1,nspins
       ...
       ! XC uses the sofft basis set only
          ...
          IF ( nspins==1) THEN
             ...
             CALL cp_dbcsr_set(p_env%kpp1_env%v_ao(ispin)%matrix,0.0_dp)
...
! *****************************************************************************
  SUBROUTINE p_env_check_i_alloc(p_env, qs_env)
    ...
    IF(.NOT.(ASSOCIATED(p_env)))CALL cp__a("qs_linres_methods.F",1245)
    IF(.NOT.(p_env%ref_count>0))CALL cp__a("qs_linres_methods.F",1246)
...
! ------------------------------------------------------------------
    ...
    IF(.NOT.(ASSOCIATED(kpp1_env)))CALL cp__a("qs_linres_methods.F",1308)
    IF(.NOT.(kpp1_env%ref_count>0))CALL cp__a("qs_linres_methods.F",1309)
    ...
    IF (.NOT.ASSOCIATED(kpp1_env%v_ao)) THEN
       CALL cp_dbcsr_allocate_matrix_set(kpp1_env%v_ao,nspins)
          DO ispin=1,nspins
             ALLOCATE(kpp1_env%v_ao(ispin)%matrix)
             CALL cp_dbcsr_init(kpp1_env%v_ao(ispin)%matrix)
...
!> \param buf ...
! *****************************************************************************
  SUBROUTINE preortho(v,psi0,S_psi0,buf)
    !v = (I-PS)v
    !
    TYPE(cp_fm_p_type), DIMENSION(:), &
...
!
    ...
    CALL timeset(routineN,handle)
    !
    IF(.NOT.(ASSOCIATED(v)))CALL cp__a("qs_linres_methods.F",1401)
    IF(.NOT.(ASSOCIATED(S_psi0)))CALL cp__a("qs_linres_methods.F",1402)
    IF(.NOT.(ASSOCIATED(psi0)))CALL cp__a("qs_linres_methods.F",1403)
    IF(.NOT.(ASSOCIATED(buf)))CALL cp__a("qs_linres_methods.F",1404)
    !
    nspins = SIZE(v,1)
    !
    DO ispin = 1,nspins
       ...
       IF(.NOT.(nv==np))CALL cp__a("qs_linres_methods.F",1413)
       IF(.NOT.(mt>=mv))CALL cp__a("qs_linres_methods.F",1414)
       IF(.NOT.(mt>=mp))CALL cp__a("qs_linres_methods.F",1415)
       IF(.NOT.(nt==nv))CALL cp__a("qs_linres_methods.F",1416)
       !
       ! buf = v' * S_psi0
...
!> \param buf ...
! *****************************************************************************
  SUBROUTINE postortho(v,psi0,S_psi0,buf)
    !v = (I-SP)v
    !
    TYPE(cp_fm_p_type), DIMENSION(:), &
      POINTER                                :: v, psi0, S_psi0
...
!
    ...
    CALL timeset(routineN,handle)
    !
    IF(.NOT.(ASSOCIATED(v)))CALL cp__a("qs_linres_methods.F",1454)
    IF(.NOT.(ASSOCIATED(S_psi0)))CALL cp__a("qs_linres_methods.F",1455)
    IF(.NOT.(ASSOCIATED(psi0)))CALL cp__a("qs_linres_methods.F",1456)
    IF(.NOT.(ASSOCIATED(buf)))CALL cp__a("qs_linres_methods.F",1457)
    !
    nspins = SIZE(v,1)
    !
    DO ispin = 1,nspins
       ...
       CALL cp_fm_get_info(psi0(ispin)%matrix,ncol_global=mp,nrow_global=np)
       CALL cp_fm_get_info(buf,ncol_global=mt,nrow_global=nt)
       IF(.NOT.(nv==np))CALL cp__a("qs_linres_methods.F",1466)
       IF(.NOT.(mt>=mv))CALL cp__a("qs_linres_methods.F",1467)
       IF(.NOT.(mt>=mp))CALL cp__a("qs_linres_methods.F",1468)
       IF(.NOT.(nt==nv))CALL cp__a("qs_linres_methods.F",1469)
       !
       ! buf = v' * psi0
...
! *****************************************************************************
  SUBROUTINE linres_read_restart(qs_env,linres_section,vec,ivec,tag,ind)
    ...
    INTEGER :: group, handle, i, i_block, ia, ie, iostat, ispin, iv, iv1, &
      ivec_tmp, j, max_block, n_rep_val, nao, nao_tmp, nmo, nmo_tmp, nspins, &
    ...
    IF(file_exists) THEN
       ...
       DO iv = iv1,ivec
          ...
          IF(nspins_tmp.NE.nspins) CALL cp__b("qs_linres_methods.F",1729,"nspins not consistent")
          IF(nao_tmp   .NE.nao   ) CALL cp__b("qs_linres_methods.F",1730,"nao not consistent")
          !
          DO ispin = 1,nspins
             CALL get_mo_set(mos(ispin)%mo_set,mo_coeff=mo_coeff)
             ...
             IF(nmo_tmp.NE.nmo) CALL cp__b("qs_linres_methods.F",1738,"nmo not consistent")
             !
             ! read the response
             DO i=1,nmo,MAX(max_block,1)
                i_block=MIN(max_block,nmo-i+1)
...
! *****************************************************************************
  SUBROUTINE check_p_env_init(p_env,linres_control, nspins)
    ...
    IF(linres_control%preconditioner_type /= ot_precond_none) THEN
       IF(.NOT.(ASSOCIATED(p_env%preconditioner)))CALL cp__a("qs_linres_methods.F",1801)
       DO ispin = 1,nspins
          CALL cp_fm_get_info(p_env%PS_psi0(ispin)%matrix,nrow_global=nrow,ncol_global=ncol)
          IF(.NOT.(nrow==p_env%n_ao(ispin)))CALL cp__a("qs_linres_methods.F",1804)
          IF(.NOT.(ncol==p_env%n_mo(ispin)))CALL cp__a("qs_linres_methods.F",1805)
       ENDDO
    ENDIF
