# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_pointers.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_pointers.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/orbital_pointers.F" 2
...
! *****************************************************************************
  SUBROUTINE create_orbital_pointers(maxl)
    ...
    IF (current_maxl > -1) THEN
      CALL cp_abort(cp__l("aobasis/orbital_pointers.F",86),&
           "Orbital pointers are already allocated. "//&
           "Use the init routine for an update")
    END IF

    IF (maxl < 0) THEN
      CALL cp_abort(cp__l("aobasis/orbital_pointers.F",92),&
...
!   *** Number of Cartesian orbitals for each l ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,101,&
                       "nco",(maxl+2)*int_size)
    END IF
...
!   *** Number of Cartesian orbitals up to l ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,115,&
...
!   *** Build the Cartesian orbital pointer and the shell orbital pointer ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,129,&
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,137,&
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,160,&
...
!   *** Number of spherical orbitals for each l ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,179,&
                       "nso",(maxl+2)*int_size)
    END IF
...
!   *** Number of spherical orbitals up to l ***
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,193,&
                       "nsoset",(maxl+2)*int_size)
    END IF
    ALLOCATE (nsoset_pm(-1:maxl),STAT=istat)
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,198,&
    ...
    IF (istat /= 0) THEN
      CALL stop_memory(routineN,moduleN,236,&
...
! *****************************************************************************
  SUBROUTINE init_orbital_pointers(maxl)
    ...
    IF (maxl < 0) THEN
      CALL cp_abort(cp__l("aobasis/orbital_pointers.F",311),&
           "A negative maximum angular momentum quantum "//&
           "number is invalid")
    END IF
