# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_util.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_util.F"
!-----------------------------------------------------------------------------!
...
# 62 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/cp_ddapc_util.F" 2
...
! *****************************************************************************
  SUBROUTINE cp_ddapc_init(qs_env)
    ...
    IF (allocate_ddapc_env.AND.unimplemented) THEN
       CALL cp__b("cp_ddapc_util.F",121,"DDAP charges work only with GPW/GAPW code.")
    END IF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE get_ddapc(qs_env, calc_force, density_fit_section,&
    ...
    INTEGER :: handle, handle2, handle3, i, ii, iparticle, iparticle0, ispin, &
      iw, j, myid, n_rep_val, ndim, nparticles, num_gauss, pmax, pmin
    LOGICAL                                  :: need_f
...
!!NB refactor math in inner loop - new temporaries
    ...
    IF (PRESENT(qout1)) THEN
       IF (ASSOCIATED(qout1)) THEN
          IF(.NOT.(SIZE(qout1)==SIZE(qv)))CALL cp__a("cp_ddapc_util.F",383)
       ELSE
    ...
    IF (PRESENT(qout2)) THEN
       IF (ASSOCIATED(qout2)) THEN
          IF(.NOT.(SIZE(qout2)==SIZE(qtot)))CALL cp__a("cp_ddapc_util.F",391)
       ELSE
    ...
    IF (need_f) THEN
       ...
       DO iparticle0 = 1, SIZE(particle_set), 100
          nparticles = MIN(100, SIZE(particle_set)-iparticle0+1)
       ...
       IF(.NOT.(PRESENT(dq_out)))CALL cp__a("cp_ddapc_util.F",493)
       IF (.NOT.ASSOCIATED(dq_out)) THEN
          ALLOCATE(dq_out(SIZE(dqv,1),SIZE(dqv,2),SIZE(dqv,3)))
       ELSE
          IF(.NOT.(SIZE(dqv,1)==SIZE(dq_out,1)))CALL cp__a("cp_ddapc_util.F",497)
          IF(.NOT.(SIZE(dqv,2)==SIZE(dq_out,2)))CALL cp__a("cp_ddapc_util.F",498)
          IF(.NOT.(SIZE(dqv,3)==SIZE(dq_out,3)))CALL cp__a("cp_ddapc_util.F",499)
       END IF
       dq_out = dqv
       IF (debug_this_module) THEN
          ...
          IF (ddbv(kk).GT.1.0E-8_dp) THEN
             ...
             IF (v0.GT.0.1_dp) THEN
                WRITE(*,'(A,2I5,2F15.9)')"ERROR IN DERIVATIVE OF B VECTOR, IPARTICLE, ICOORD:",iparticle,i,&
                     dbv(kk,i),ddbv(kk)
                CALL cp__b("cp_ddapc_util.F",759,"")
             END IF
          END IF
       END DO
          ...
          DO ll = 1, SIZE(ddAm,2)
             IF (ddAm(kk,ll).GT.1.0E-8_dp) THEN
                ...
                IF (v0.GT.0.1_dp) THEN
                   ...
                   CALL cp__b("cp_ddapc_util.F",843,"")
                END IF
             END IF
          END DO
       END DO
