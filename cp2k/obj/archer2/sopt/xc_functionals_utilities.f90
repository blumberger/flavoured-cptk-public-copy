# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_functionals_utilities.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_functionals_utilities.F"
!-----------------------------------------------------------------------------!
...
# 19 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_functionals_utilities.F" 2
...
! *****************************************************************************
    ...
    IF ( ABS(order) > 3 ) &
       CALL cp__b("xc/xc_functionals_utilities.F",78,"Order of derivative too high")
    ...
    CASE ( 211 )          ! GGA spin-polarized, with crossterms
          ...
          m(3,2) = 57
    CASE DEFAULT
       CALL cp__b("xc/xc_functionals_utilities.F",136,"Invalid tag")
    END SELECT
...
!   rs parameter : f*rho**(-1/3)
    ...
    IF (SIZE(rs) < SIZE(rho)) THEN
       CALL cp__b("xc/xc_functionals_utilities.F",188,"Size of array rs too small.")
    END IF
...
!         R: spin density, total density wave vector
    ...
    !FM IF ( n > SIZE(s) ) &
    !FM   CALL cp__b("xc/xc_functionals_utilities.F",285,"Incompatible array sizes.")
    !FM IF ( n > SIZE(grho) ) &
    !FM   CALL cp__b("xc/xc_functionals_utilities.F",287,"Incompatible array sizes.")
...
!   *** Parameters ***
    ...
    IF ( m > 3 ) CALL cp__b("xc/xc_functionals_utilities.F",334,"Order too high.")
!!    IF (.NOT.ASSOCIATED(fx)) THEN
!!       ALLOCATE(fx(n,m+1), STAT=ierr)
!!       IF (ierr /= 0) CALL stop_memory(routineP, "fx", n*m)
!!    ELSE
       IF (SIZE(fx,1)<n) CALL cp__b("xc/xc_functionals_utilities.F",339,"SIZE(fx,1) too small")
       IF (SIZE(fx,2)<m) CALL cp__b("xc/xc_functionals_utilities.F",340,"SIZE(fx,2) too small")
!!    END IF
