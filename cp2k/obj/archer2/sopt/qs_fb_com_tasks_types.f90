# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_com_tasks_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_com_tasks_types.F"
!-----------------------------------------------------------------------------!
...
# 25 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_com_tasks_types.F" 2
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_retain(com_tasks)
    ...
    IF(.NOT.(ASSOCIATED(com_tasks%obj)))CALL cp__a("qs_fb_com_tasks_types.F",169)
    IF(.NOT.(com_tasks%obj%ref_count>0))CALL cp__a("qs_fb_com_tasks_types.F",170)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_retain(atom_pairs)
    ...
    IF(.NOT.(ASSOCIATED(atom_pairs%obj)))CALL cp__a("qs_fb_com_tasks_types.F",187)
    IF(.NOT.(atom_pairs%obj%ref_count>0))CALL cp__a("qs_fb_com_tasks_types.F",188)
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_release(com_tasks)
    ...
    IF (ASSOCIATED(com_tasks%obj)) THEN
       IF(.NOT.(com_tasks%obj%ref_count>0))CALL cp__a("qs_fb_com_tasks_types.F",207)
       com_tasks%obj%ref_count = com_tasks%obj%ref_count - 1
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_release(atom_pairs)
    ...
    IF (ASSOCIATED(atom_pairs%obj)) THEN
       IF(.NOT.(atom_pairs%obj%ref_count>0))CALL cp__a("qs_fb_com_tasks_types.F",238)
       atom_pairs%obj%ref_count = atom_pairs%obj%ref_count - 1
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_create(com_tasks)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(com_tasks%obj)))CALL cp__a("qs_fb_com_tasks_types.F",373)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_create(atom_pairs)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(atom_pairs%obj)))CALL cp__a("qs_fb_com_tasks_types.F",398)
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_init(com_tasks)
    ...
    IF(.NOT.(ASSOCIATED(com_tasks%obj)))CALL cp__a("qs_fb_com_tasks_types.F",421)
    IF (ASSOCIATED(com_tasks%obj%tasks)) THEN
       DEALLOCATE(com_tasks%obj%tasks)
    END IF
    com_tasks%obj%task_dim = TASK_N_RECORDS
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_init(atom_pairs)
    ...
    IF(.NOT.(ASSOCIATED(atom_pairs%obj)))CALL cp__a("qs_fb_com_tasks_types.F",444)
    IF (ASSOCIATED(atom_pairs%obj%pairs)) THEN
       DEALLOCATE(atom_pairs%obj%pairs)
    END IF
    atom_pairs%obj%npairs = 0
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_get(com_tasks, &
    ...
    IF(.NOT.(ASSOCIATED(com_tasks%obj)))CALL cp__a("qs_fb_com_tasks_types.F",478)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_get(atom_pairs, &
    ...
    IF(.NOT.(ASSOCIATED(atom_pairs%obj)))CALL cp__a("qs_fb_com_tasks_types.F",509)
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_set(com_tasks, &
    ...
    IF(.NOT.(ASSOCIATED(com_tasks%obj)))CALL cp__a("qs_fb_com_tasks_types.F",541)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_set(atom_pairs, &
    ...
    IF(.NOT.(ASSOCIATED(atom_pairs%obj)))CALL cp__a("qs_fb_com_tasks_types.F",578)
...
! *****************************************************************************
  SUBROUTINE fb_com_tasks_build_atom_pairs(com_tasks, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",816)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_calc_buffer_sizes(atom_pairs, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1077)

    check_ok = fb_com_atom_pairs_has_data(atom_pairs)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1080)
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_gather_blks(dbcsr_mat, &
    ...
    INTEGER :: handle, iatom, ii, ind, ipair, ipe, jatom, jj, ncols_blk, &
      ...
      numprocs, pe, recv_encode, send_encode
    INTEGER(KIND=int_8), DIMENSION(:), &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1164)
    check_ok = fb_com_atom_pairs_has_data(atom_pairs_send)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1166)
    check_ok = fb_matrix_data_has_data(matrix_storage)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1168)
    ...
    DO ipe = 1, numprocs
       ...
       DO ipair = 1, send_pair_count(ipe)
          ...
          CALL cp_dbcsr_get_block_p(matrix=dbcsr_mat, &
                                    ...
                                    found=found)
          IF (.NOT. found) THEN
             CALL cp__b("qs_fb_com_tasks_types.F",1237,"Matrix block not found")
          ELSE
...
! *****************************************************************************
  SUBROUTINE fb_com_atom_pairs_distribute_blks(matrix_storage, &
    ...
    check_ok = fb_com_atom_pairs_has_data(atom_pairs_send)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1360)
    check_ok = fb_com_atom_pairs_has_data(atom_pairs_send)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1362)
    check_ok = fb_matrix_data_has_data(matrix_storage)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_com_tasks_types.F",1364)
    ...
    DO ipe = 1, numprocs
       ...
       DO ipair = 1, send_pair_count(ipe)
          ...
          CALL fb_matrix_data_get(matrix_storage, &
                                  ...
                                  mat_block, found)
          IF (.NOT. found) THEN
             CALL cp__b("qs_fb_com_tasks_types.F",1431,"Matrix block not found")
          ELSE
             nrows_blk = row_block_size_data(iatom)
             ncols_blk = col_block_size_data(jatom)
             DO jj = 1, ncols_blk
