# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_overlap.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_overlap.F"
!-----------------------------------------------------------------------------!
...
# 23 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/aobasis/ai_overlap.F" 2
...
! *****************************************************************************
  SUBROUTINE overlap_ab(la_max,la_min,npgfa,rpgfa,zeta,&
    ...
    INTEGER                                  :: ax, ay, az, bx, by, bz, coa, &
                                                cob, ia, ib, ipgf, jpgf, la, &
                                                lb, ldrr, lma, lmb, ma, mb, &
                                                na, nb, ofa, ofb
...
! Distance of the centers a and b
    ...
    IF (PRESENT(sab)) THEN
       IF(.NOT.((SIZE(sab,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",732)
       IF(.NOT.((SIZE(sab,2) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",733)
    END IF
    IF (PRESENT(dab)) THEN
       IF(.NOT.((SIZE(dab,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",736)
       IF(.NOT.((SIZE(dab,2) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",737)
       IF(.NOT.((SIZE(dab,3) >= 3)))CALL cp__a("aobasis/ai_overlap.F",738)
    END IF
    IF (PRESENT(ddab)) THEN
       IF(.NOT.((SIZE(ddab,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",741)
       IF(.NOT.((SIZE(ddab,2) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",742)
       IF(.NOT.((SIZE(ddab,3) >= 6)))CALL cp__a("aobasis/ai_overlap.F",743)
    END IF
...
! *****************************************************************************
  SUBROUTINE overlap_aab(la1_max,la1_min,npgfa1,rpgfa1,zeta1,&
    ...
    INTEGER :: ax, ax1, ax2, ay, ay1, ay2, az, az1, az2, bx, by, bz, coa1, &
      coa2, cob, i1pgf, i2pgf, ia1, ia2, ib, jpgf, la1, la2, lb, ldrr, lma, &
      lmb, ma1, ma2, mb, na1, na2, nb, ofa1, ofa2, ofb
...
! Distance of the centers a and b
    ...
    nb = ncoset(lb_max) - ofb
    IF (PRESENT(saab)) THEN
       IF(.NOT.((SIZE(saab,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1021)
       IF(.NOT.((SIZE(saab,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1022)
       IF(.NOT.((SIZE(saab,3) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",1023)
    END IF
    IF (PRESENT(daab)) THEN
       IF(.NOT.((SIZE(daab,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1026)
       IF(.NOT.((SIZE(daab,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1027)
       IF(.NOT.((SIZE(daab,3) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",1028)
       IF(.NOT.((SIZE(daab,4) >= 3)))CALL cp__a("aobasis/ai_overlap.F",1029)
    END IF
...
! *****************************************************************************
  SUBROUTINE overlap_abb(la_max,la_min,npgfa,rpgfa,zeta,&
    ...
    INTEGER :: ax, ay, az, bx, bx1, bx2, by, by1, by2, bz, bz1, bz2, coa, &
      cob1, cob2, ia, ib1, ib2, ipgf, j1pgf, j2pgf, la, lb1, lb2, ldrr, lma, &
      lmb, ma, mb1, mb2, na, nb1, nb2, ofa, ofb1, ofb2
    REAL(KIND=dp)                            :: a, b, dumx, dumy, dumz, f0, &
...
! Distance of the centers a and b
    ...
    IF (PRESENT(sabb)) THEN
       IF(.NOT.((SIZE(sabb,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",1204)
       IF(.NOT.((SIZE(sabb,2) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1205)
       IF(.NOT.((SIZE(sabb,3) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1206)
    END IF
    IF (PRESENT(dabb)) THEN
       IF(.NOT.((SIZE(dabb,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",1209)
       IF(.NOT.((SIZE(dabb,2) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1210)
       IF(.NOT.((SIZE(dabb,3) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1211)
       IF(.NOT.((SIZE(dabb,4) >= 3)))CALL cp__a("aobasis/ai_overlap.F",1212)
    END IF
...
! *****************************************************************************
  SUBROUTINE overlap_aaab(la1_max,la1_min,npgfa1,rpgfa1,zeta1,&
    ...
    INTEGER :: ax, ax1, ax2, ax3, ay, ay1, ay2, ay3, az, az1, az2, az3, bx, &
      ...
      jpgf, la1, la2, la3, lb, ldrr, lma, lmb, ma1, ma2, ma3, mb, na1, na2, &
      na3, nb, ofa1, ofa2, ofa3, ofb
    REAL(KIND=dp)                            :: a, b, dumx, dumy, dumz, f0, &
                                                rab2, rpgfa, tab, xhi, zet
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :, :)                     :: rr
...
! Distance of the centers a and b
    ...
    nb = ncoset(lb_max) - ofb
    IF (PRESENT(saaab)) THEN
       IF(.NOT.((SIZE(saaab,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1399)
       IF(.NOT.((SIZE(saaab,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1400)
       IF(.NOT.((SIZE(saaab,3) >= na3*npgfa3)))CALL cp__a("aobasis/ai_overlap.F",1401)
       IF(.NOT.((SIZE(saaab,4) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",1402)
    END IF
    IF (PRESENT(daaab)) THEN
       IF(.NOT.((SIZE(daaab,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1405)
       IF(.NOT.((SIZE(daaab,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1406)
       IF(.NOT.((SIZE(daaab,3) >= na3*npgfa3)))CALL cp__a("aobasis/ai_overlap.F",1407)
       IF(.NOT.((SIZE(daaab,4) >= nb*npgfb)))CALL cp__a("aobasis/ai_overlap.F",1408)
       IF(.NOT.((SIZE(daaab,5) >= 3)))CALL cp__a("aobasis/ai_overlap.F",1409)
    END IF
...
! *****************************************************************************
  SUBROUTINE overlap_aabb(la1_max,la1_min,npgfa1,rpgfa1,zeta1,&
    ...
    INTEGER :: ax, ax1, ax2, ay, ay1, ay2, az, az1, az2, bx, bx1, bx2, by, &
      ...
      ib1, ib2, j1pgf, j2pgf, la1, la2, lb1, lb2, ldrr, lma, lmb, ma1, ma2, &
      mb1, mb2, na1, na2, nb1, nb2, ofa1, ofa2, ofb1, ofb2
    REAL(KIND=dp)                            :: a, b, dumx, dumy, dumz, f0, &
...
! Distance of the centers a and b
    ...
    nb2 = ncoset(lb2_max) - ofb2
    IF (PRESENT(saabb)) THEN
       IF(.NOT.((SIZE(saabb,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1610)
       IF(.NOT.((SIZE(saabb,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1611)
       IF(.NOT.((SIZE(saabb,3) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1612)
       IF(.NOT.((SIZE(saabb,4) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1613)
    END IF
    IF (PRESENT(daabb)) THEN
       IF(.NOT.((SIZE(daabb,1) >= na1*npgfa1)))CALL cp__a("aobasis/ai_overlap.F",1616)
       IF(.NOT.((SIZE(daabb,2) >= na2*npgfa2)))CALL cp__a("aobasis/ai_overlap.F",1617)
       IF(.NOT.((SIZE(daabb,3) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1618)
       IF(.NOT.((SIZE(daabb,4) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1619)
       IF(.NOT.((SIZE(daabb,5) >= 3)))CALL cp__a("aobasis/ai_overlap.F",1620)
    END IF
...
! *****************************************************************************
  SUBROUTINE overlap_abbb(la_max,la_min,npgfa,rpgfa,zeta,&
    ...
    INTEGER :: ax, ay, az, bx, bx1, bx2, bx3, by, by1, by2, by3, bz, bz1, &
      bz2, bz3, coa, cob1, cob2, cob3, ia, ib1, ib2, ib3, ipgf, j1pgf, j2pgf, &
      j3pgf, la, lb1, lb2, lb3, ldrr, lma, lmb, ma, mb1, mb2, mb3, na, nb1, &
      nb2, nb3, ofa, ofb1, ofb2, ofb3
    REAL(KIND=dp)                            :: a, b, dumx, dumy, dumz, f0, &
                                                rab2, rpgfb, tab, xhi, zet
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :, :)                     :: rr
...
! Distance of the centers a and b
    ...
    IF (PRESENT(sabbb)) THEN
       IF(.NOT.((SIZE(sabbb,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",1824)
       IF(.NOT.((SIZE(sabbb,2) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1825)
       IF(.NOT.((SIZE(sabbb,3) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1826)
       IF(.NOT.((SIZE(sabbb,4) >= nb3*npgfb3)))CALL cp__a("aobasis/ai_overlap.F",1827)
    END IF
    IF (PRESENT(dabbb)) THEN
       IF(.NOT.((SIZE(dabbb,1) >= na*npgfa)))CALL cp__a("aobasis/ai_overlap.F",1830)
       IF(.NOT.((SIZE(dabbb,2) >= nb1*npgfb1)))CALL cp__a("aobasis/ai_overlap.F",1831)
       IF(.NOT.((SIZE(dabbb,3) >= nb2*npgfb2)))CALL cp__a("aobasis/ai_overlap.F",1832)
       IF(.NOT.((SIZE(dabbb,4) >= nb3*npgfb3)))CALL cp__a("aobasis/ai_overlap.F",1833)
       IF(.NOT.((SIZE(dabbb,5) >= 3)))CALL cp__a("aobasis/ai_overlap.F",1834)
    END IF
