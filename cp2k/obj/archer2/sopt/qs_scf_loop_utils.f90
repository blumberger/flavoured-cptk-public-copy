# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_loop_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_loop_utils.F"
!-----------------------------------------------------------------------------!
...
# 66 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_loop_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_scf_new_mos(qs_env,scf_env,scf_control,scf_section,diis_step,&
    ...
    CASE DEFAULT
       CALL cp_abort(cp__l("qs_scf_loop_utils.F",161),&
            "unknown scf method: "//&
...
! *****************************************************************************
  SUBROUTINE qs_scf_new_mos_kp(qs_env,scf_env,scf_control,diis_step)
    ...
    IF(dft_control%roks)&
       CALL cp__b("qs_scf_loop_utils.F",294,"KP code: ROKS method not available: ")

    SELECT CASE (scf_env%method)
       CASE DEFAULT
          CALL cp_abort(cp__l("qs_scf_loop_utils.F",298),&
       ...
       CASE(special_diag_method_nr)
          ...
          IF(.NOT.(has_unit_metric))CALL cp__a("qs_scf_loop_utils.F",308)
          ! Diagonlization in orthonormal case
          CALL cp_abort(cp__l("qs_scf_loop_utils.F",310),&
       ...
       CASE(ot_diag_method_nr,&
            ...
            ot_method_nr)
          CALL cp_abort(cp__l("qs_scf_loop_utils.F",317),&
               "KP code: Scf method not available: "//&
               cp_to_string(scf_env%method))
    END SELECT
    ...
    DO ispin=1,SIZE(mos,2)
       energy%kTS = energy%kTS + mos(1,ispin)%mo_set%kTS
       energy%efermi = energy%efermi + mos(1,ispin)%mo_set%mu
    ENDDO
...
! *****************************************************************************
 SUBROUTINE qs_scf_density_mixing(scf_env,rho,para_env,diis_step)
    ...
    CASE(no_mixing_nr)
    CASE DEFAULT
       CALL cp_abort(cp__l("qs_scf_loop_utils.F",452),&
...
! *****************************************************************************
  SUBROUTINE cleanup_scf_loop(scf_env)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_loop_utils.F",696)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_loop_utils.F",697)
! *** method dependent cleanup
   ...
   !
       CASE DEFAULT
          CALL cp_abort(cp__l("qs_scf_loop_utils.F",716),&
