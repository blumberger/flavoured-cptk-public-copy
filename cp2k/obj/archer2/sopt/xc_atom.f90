# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_atom.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_atom.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xc/xc_atom.F" 2
...
! *****************************************************************************
  SUBROUTINE vxc_of_r_new(xc_fun_section,rho_set,deriv_set,deriv_order,needs,w,&
                          lsd,na,nr,exc,vxc,vxg,vtau,energy_only,epr_xc,adiabatic_rescale_factor)
...
! *****************************************************************************
  SUBROUTINE fill_rho_set(rho_set,lsd,nspins,needs,rho,drho,tau,na,ir)
    ...
    IF (lsd) THEN
    ELSE
       IF(.NOT.(SIZE(rho,2)==1))CALL cp__a("xc/xc_atom.F",600)
    END IF
    SELECT CASE(my_nspins)
    CASE(1)
       IF(.NOT.(.NOT.needs%rho_spin))CALL cp__a("xc/xc_atom.F",604)
       IF(.NOT.(.NOT.needs%drho_spin))CALL cp__a("xc/xc_atom.F",605)
       IF(.NOT.(.NOT.needs%norm_drho_spin))CALL cp__a("xc/xc_atom.F",606)
       IF(.NOT.(.NOT.needs%drhoa_drhob))CALL cp__a("xc/xc_atom.F",607)
       IF(.NOT.(.NOT.needs%rho_spin_1_3))CALL cp__a("xc/xc_atom.F",608)
    CASE(2)
    CASE default
       CALL cp__b("xc/xc_atom.F",611,"")
    END SELECT
...
!!                     (0.25_dp*drho(3,ia,ir,1)**2)
    ...
    IF (needs%tau.OR.needs%tau_spin) THEN
       IF(.NOT.(ASSOCIATED(tau)))CALL cp__a("xc/xc_atom.F",816)
       IF(.NOT.(SIZE(tau,2)==my_nspins))CALL cp__a("xc/xc_atom.F",817)
    END IF
    IF (needs%tau) THEN
       IF (my_nspins==2) THEN
          DO ia = 1,na
