# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_types.F"
!-----------------------------------------------------------------------------!
...
# 73 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/hfx_types.F" 2
...
! *****************************************************************************
  TYPE hfx_cache_type
    INTEGER(int_8), DIMENSION(1024)    :: DATA
      ...
      DO irep=1,n_rep_hf
        ...
        IF (actual_x_data%potential_parameter%potential_type == do_hfx_potential_truncated .OR. &
          ...
          IF ( .NOT. file_exists(char_val) ) THEN
            ...
            CALL cp__b("hfx_types.F",571,error_msg)
          ELSE
...
! *****************************************************************************
  SUBROUTINE erfc_cutoff(eps,omg,r_cutoff)
    ...
    END DO
    IF(.NOT.(iter<=itermax))CALL cp__a("hfx_types.F",887)
    r_cutoff = r0
    ...
    DO ikind = 1,nkind
        ...
        ELSE
          ...
          CALL get_qs_kind_set(qs_kind_set,&
                               ...
                               basis_type="AUX_FIT")
        END IF
        IF (basis_info%max_set<max_set) CALL cp__b("hfx_types.F",960,"UNEXPECTED MAX_SET")
...
! *****************************************************************************
  SUBROUTINE parse_memory_section(memory_parameter, hf_sub_section, storage_id,&
    ...
    IF(.NOT.(check))CALL cp__a("hfx_types.F",1114)
    ...
    END IF
    memory_parameter%cache_size = 1024
    memory_parameter%bits_max_val = 6
    ...
    IF (.NOT.skip_disk) THEN
       ...
       IF(stat/=0) THEN
          ...
          CALL cp__b("hfx_types.F",1166,error_msg)
       END IF
...
! *****************************************************************************
  SUBROUTINE hfx_create_neighbor_cells(x_data, pbc_shells, cell, i_thread)
    ...
    IF( i_thread == 1 ) THEN
      ...
      CALL cp_warn(cp__l("hfx_types.F",1350),&
      ...
      ELSE
        ...
        IF( x_data%potential_parameter%cutoff_radius >= l_min ) THEN
          CALL cp_warn(cp__l("hfx_types.F",1360),&
      ...
      ELSE
        ...
        IF( total_number_of_cells < SIZE(x_data%neighbor_cells)) THEN
          IF( i_thread == 1) THEN
            ...
            CALL cp__w("hfx_types.F",1639,error_msg)
          END IF
        END IF
        total_number_of_cells = 0
        DO i = 0,x_data%periodic_parameter%number_of_shells
