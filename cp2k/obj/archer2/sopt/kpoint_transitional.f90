# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_transitional.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_transitional.F"
!-----------------------------------------------------------------------------!
...
# 21 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/kpoint_transitional.F" 2
    ...
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
     ...
     IF(ASSOCIATED(this%ptr_1d)) THEN
        IF(SIZE(this%ptr_2d, 2) /= 1) &
           CALL cp__b("kpoint_transitional.F",50,"Method not implemented for k-points")
     ENDIF
  ...
  SUBROUTINE kpoint_transitional_release(this)
    ...
    IF (ASSOCIATED(this%ptr_1d)) THEN
      IF (this%set_as_1d) THEN
        CALL cp_dbcsr_deallocate_matrix_set(this%ptr_1d)
      ELSE
        CALL cp_dbcsr_deallocate_matrix_set(this%ptr_2d)
      ENDIF
    ENDIF
