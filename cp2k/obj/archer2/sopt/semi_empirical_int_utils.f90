# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_utils.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_utils.F" 2

  IMPLICIT NONE

# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.h" 1
!-----------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations         !
!   Copyright (C) 2000 - 2016  CP2K developers group                          !
!-----------------------------------------------------------------------------!
...
END INTERFACE 
# 31 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_utils.F" 2
  ...
  FUNCTION ijkl_sp(sepi, sepj, ij, kl, li, lj, lk, ll, ic, r, se_int_control,&
    ...
    IF ((.NOT.se_int_control%pc_coulomb_int).AND.(itype/=do_method_pchg)) THEN
       ...
       IF (se_int_control%shortrange.AND.se_int_control%do_ewald_r3) THEN
          res = res - ijkl_low_3(sepi, sepj, ij, kl, li, lj, lk, ll, ic,&
                                 itype, charg_int_3)
       END IF
    END IF
...
! Computing only Integral Values
    ...
    CALL cp__b("semi_empirical_int_utils.F",527,"")
...
! Computing only Integral Derivatives
    ...
    END IF
    ! We should NEVER reach this point
    CALL cp__b("semi_empirical_int_utils.F",733,"")
...
! Computing only Integral Derivatives
    ...
    END IF
    ! We should NEVER reach this point
    CALL cp__b("semi_empirical_int_utils.F",942,"")
...
! *****************************************************************************
  FUNCTION charg_int_ri(r, l1_i, l2_i, m, da_i, db_i, add0, fact_screen) RESULT(charg)
    ...
    END IF
    ! We should NEVER reach this point
    CALL cp__b("semi_empirical_int_utils.F",1380,"")
...
! *****************************************************************************
  FUNCTION dcharg_int_ri(r, l1_i, l2_i, m, da_i, db_i, add0, fact_screen) RESULT(charg)
    ...
    END IF
    ! We should NEVER reach this point
    CALL cp__b("semi_empirical_int_utils.F",1518,"")
...
! *****************************************************************************
  FUNCTION dcharg_int_ri_fs(r, l1_i, l2_i, m, da_i, db_i, add0, fact_screen) RESULT(charg)
    ...
    END IF
    ! We should NEVER reach this point
    CALL cp__b("semi_empirical_int_utils.F",1660,"")
...
! *****************************************************************************
  RECURSIVE SUBROUTINE rotmat (sepi, sepj, rjiv, r, ij_matrix, do_derivatives,&
    ...
    IF(.NOT.(ASSOCIATED(ij_matrix)))CALL cp__a("semi_empirical_int_utils.F",1704)
    ...
    IF ((sepi%natorb>1).OR.(sepj%natorb>1)) THEN
       ...
       IF (dbg_inv) THEN
          ...
          IF(.NOT.(.NOT.do_derivatives))CALL cp__a("semi_empirical_int_utils.F",1747)
          imap(1)   = 3
          imap(2)   = 2
          imap(3)   = 1
          eval      = .TRUE.
       ...
       IF (do_derivatives) THEN
          ...
          IF(.NOT.(eval))CALL cp__a("semi_empirical_int_utils.F",1902)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE rot_2el_2c_first (sepi, sepj, rijv, se_int_control, se_taper,&
    ...
    IF (lgrad) THEN
       IF(.NOT.(PRESENT( rep_d)))CALL cp__a("semi_empirical_int_utils.F",2125)
       IF(.NOT.(PRESENT(   v_d)))CALL cp__a("semi_empirical_int_utils.F",2126)
       IF(.NOT.(PRESENT(logv_d)))CALL cp__a("semi_empirical_int_utils.F",2127)
       IF(.NOT.(PRESENT(  drij)))CALL cp__a("semi_empirical_int_utils.F",2128)
    END IF
    limkl = indexb(kk, kk)
    DO k = 1, limkl
       DO i = 1, 45
...
! *****************************************************************************
  SUBROUTINE store_2el_2c_diag (limij, limkl, ww, w, ww_dx, ww_dy, ww_dz, dw)
    ...
    ELSE IF (PRESENT(ww_dx).AND.PRESENT(ww_dy).AND.PRESENT(ww_dz).AND.PRESENT(dw)) THEN
       ...
       END DO
    ELSE
       CALL cp__b("semi_empirical_int_utils.F",2470,"")
    END IF
