# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_tp_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_tp_scf.F"
!-----------------------------------------------------------------------------!
...
# 103 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_tp_scf.F" 2
...
! *****************************************************************************
  SUBROUTINE xas_do_tp_scf (dft_control,xas_env,iatom,scf_env,qs_env,&
    ...
    IF(.NOT.(ASSOCIATED(xas_env)))CALL cp__a("xas_tp_scf.F",177)
    IF(.NOT.(xas_env%ref_count>0))CALL cp__a("xas_tp_scf.F",178)
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("xas_tp_scf.F",179)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("xas_tp_scf.F",180)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("xas_tp_scf.F",181)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("xas_tp_scf.F",182)
    ...
    IF(.NOT.(gapw))CALL cp__a("xas_tp_scf.F",202)
    ...
    scf_loop: DO
       ...
       CASE DEFAULT
          CALL cp_abort(cp__l("xas_tp_scf.F",239),&
       ...
       CASE(no_mixing_nr)
       CASE DEFAULT
          CALL cp_abort(cp__l("xas_tp_scf.F",281),&
       ...
       IF (scf_env%mixing_method > 0) THEN
          DO ispin=1,nspin
             ! does not yet handle k-points
             CALL cp_dbcsr_copy(rho_ao_kp(ispin,1)%matrix,scf_env%p_mix_new(ispin,1)%matrix)
          END DO
       ENDIF
...
! *****************************************************************************
  SUBROUTINE cls_prepare_states(xas_control,xas_env,qs_env,iatom,xas_section,output_unit)
    ...
    INTEGER :: handle, i, ikind, isgf, ispin, istate, j, my_kind, my_state, &
      nao, natom, nexc_search, nmo, nvirtual2, uno_iter, xas_estate
    ...
    IF(.NOT.(ASSOCIATED(xas_env)))CALL cp__a("xas_tp_scf.F",444)
    ...
    IF(.NOT.(ASSOCIATED(excvec_overlap)))CALL cp__a("xas_tp_scf.F",464)
...
! *****************************************************************************
  SUBROUTINE xes_scf_once(qs_env,xas_env,converged,should_stop)
    ...
    INTEGER                                  :: handle, ispin, istate, nmo, &
                                                nvirtual, nvirtual2, &
                                                output_unit
    REAL(KIND=dp), DIMENSION(:), POINTER     :: all_evals, eigenvalues
    TYPE(cp_dbcsr_p_type), DIMENSION(:), &
    ...
    IF(.NOT.(ASSOCIATED(xas_env)))CALL cp__a("xas_tp_scf.F",675)
