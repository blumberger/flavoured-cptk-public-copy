# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_epr_hyp.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_epr_hyp.F"
!-----------------------------------------------------------------------------!
...
# 64 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_epr_hyp.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_epr_hyp_calc(qs_env)
    ...
    CHARACTER(LEN=*), PARAMETER :: routineN = 'qs_epr_hyp_calc', &
      routineP = moduleN//':'//routineN

    CHARACTER(LEN=2)                         :: element_symbol
    INTEGER :: bo(2), group, ia, iat, iatom, idir1, idir2, ig, ikind, ir, &
      iso, jatom, mepos, natom, natomkind, nkind, num_pe, output_unit, z
    INTEGER, DIMENSION(:), POINTER           :: atom_list
    LOGICAL                                  :: lsd, paw_atom
    REAL(dp)                                 :: arg, esum, hard_radius, &
                                                hypanisotemp, hypfactor, &
                                                int_radius, rab2, rtemp
    ...
    TYPE(pw_p_type), POINTER                 :: hypaniso_gspace, &
    ...
    IF(.NOT.(SIZE(rho_g)>1))CALL cp__a("qs_epr_hyp.F",306)
    ...
    ! Global sum
    CALL mp_sync(group)
    CALL mp_sum(hypaniso,group)
    CALL mp_sum(hypiso,group)
    CALL mp_sum(hypiso_one,group)
