# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/population_analyses.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/population_analyses.F"
!-----------------------------------------------------------------------------!
...
MODULE population_analyses
  ...
  USE cp_dbcsr_interface,              ONLY: &
       ...
       cp_dbcsr_iterator_stop, cp_dbcsr_p_type, cp_dbcsr_set, &
       cp_dbcsr_setname, cp_dbcsr_type
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/population_analyses.F" 2
...
! *****************************************************************************
  SUBROUTINE lowdin_population_analysis(qs_env,output_unit,print_level)
    ...
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("population_analyses.F",143)
    IF(.NOT.(ASSOCIATED(qs_kind_set)))CALL cp__a("population_analyses.F",144)
    IF(.NOT.(ASSOCIATED(matrixkp_s)))CALL cp__a("population_analyses.F",145)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("population_analyses.F",146)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("population_analyses.F",147)
    IF(.NOT.(ASSOCIATED(scf_control)))CALL cp__a("population_analyses.F",148)

    IF(SIZE(matrixkp_s,2) > 1) THEN

      CALL cp__w("population_analyses.F",152,"Lowdin population analysis not implemented for k-points.")
    ...
    ELSE
       ...
       IF(ndep /= 0)&
          CALL cp_warn(cp__l("population_analyses.F",196),&
...
! *****************************************************************************
  SUBROUTINE mulliken_population_analysis(qs_env,output_unit,print_level)
    ...
    CHARACTER(LEN=default_string_length)     :: headline
    INTEGER                                  :: blk, handle, iatom, ic, isgf, &
                                                ispin, jatom, jsgf, natom, &
                                                nsgf, nspin, sgfa, sgfb
    ...
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("population_analyses.F",316)
    IF(.NOT.(ASSOCIATED(qs_kind_set)))CALL cp__a("population_analyses.F",317)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("population_analyses.F",318)
    IF(.NOT.(ASSOCIATED(rho)))CALL cp__a("population_analyses.F",319)
    IF(.NOT.(ASSOCIATED(matrix_s)))CALL cp__a("population_analyses.F",320)
    ...
    DO ispin=1,nspin
      DO ic=1,SIZE(matrix_s,2)
        IF (print_level > 2) THEN
           CALL cp_dbcsr_set(sm_ps,0.0_dp)
        END IF
        ...
        ! CALL cp_dbcsr_hadamard_product(sm_p,sm_s,sm_ps)
        ...
        DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          ...
          IF (print_level > 2) THEN
            CALL cp_dbcsr_get_block_p(matrix=sm_ps,&
                                   ...
                                   block=ps_block,&
                                   found=found)
            IF(.NOT.(ASSOCIATED(ps_block)))CALL cp__a("population_analyses.F",380)
          END IF
      ...
      IF (print_level > 2) THEN
        ...
        IF (nspin > 1) THEN
          IF (ispin == 1) THEN
             CALL cp_dbcsr_setname(sm_ps,TRIM(headline)//" For Alpha Spin")
          ELSE
             CALL cp_dbcsr_setname(sm_ps,TRIM(headline)//" For Beta Spin")
          END IF
        END IF
    ...
    CALL mp_sum(orbpop,para_env%group)
    ...
    ! Release local working storage
    IF (ASSOCIATED(sm_ps)) CALL cp_dbcsr_deallocate_matrix(sm_ps)
    IF (ASSOCIATED(orbpop)) THEN
      DEALLOCATE (orbpop)
    END IF
...
! *****************************************************************************
  SUBROUTINE write_orbpop(orbpop,atomic_kind_set,qs_kind_set,particle_set,output_unit,&
    ...
    IF(.NOT.(ASSOCIATED(orbpop)))CALL cp__a("population_analyses.F",494)
    IF(.NOT.(ASSOCIATED(atomic_kind_set)))CALL cp__a("population_analyses.F",495)
    IF(.NOT.(ASSOCIATED(particle_set)))CALL cp__a("population_analyses.F",496)
