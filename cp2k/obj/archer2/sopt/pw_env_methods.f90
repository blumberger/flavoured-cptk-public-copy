# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_env_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_env_methods.F"
!-----------------------------------------------------------------------------!
...
MODULE pw_env_methods
  ...
  USE basis_set_container_types,       ONLY: translate_basis_type
...
# 98 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/pw_env_methods.F" 2
...
SUBROUTINE pw_env_create(pw_env)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(pw_env)))CALL cp__a("pw_env_methods.F",127)
    ALLOCATE(pw_env)
...
SUBROUTINE pw_env_rebuild(pw_env, qs_env, external_para_env)
    ...
    INTEGER :: blocked_id, blocked_id_input, grid_span, handle, i, &
      igrid_level, iounit, ncommensurate, ngrid_level, xc_deriv_method_id, &
      xc_smooth_method_id
    INTEGER, DIMENSION(2)                    :: distribution_layout
    INTEGER, DIMENSION(3)                    :: higher_grid_layout
    LOGICAL :: efg_present, linres_present, odd, set_vdw_pool, should_output, &
    ...
    TYPE(pw_grid_type), POINTER :: dct_pw_grid, mt_super_ref_grid, &
      old_pw_grid, pw_grid, super_ref_grid, vdw_grid, xc_super_ref_grid
...
!
  ...
  IF(.NOT.(ASSOCIATED(pw_env)))CALL cp__a("pw_env_methods.F",218)
  IF(.NOT.(pw_env%ref_count>0))CALL cp__a("pw_env_methods.F",219)
  ...
  IF(dft_control%qs_control%gapw) THEN
     ...
     IF(.NOT.(ASSOCIATED(rho0_mpole)))CALL cp__a("pw_env_methods.F",243)
...
!
  ...
  IF (PRESENT(external_para_env)) THEN
     ...
     IF(.NOT.(ASSOCIATED(para_env)))CALL cp__a("pw_env_methods.F",264)
  ENDIF
  ...
  CASE DEFAULT
      CALL cp__b("pw_env_methods.F",355,"")
  END SELECT
  ...
  CASE(xc_rho_spline2_smooth,xc_rho_spline3_smooth,xc_rho_nn10,xc_rho_nn50)
      smooth_required=smooth_required.OR..TRUE.
  CASE DEFAULT
      CALL cp__b("pw_env_methods.F",363,"")
  END SELECT
  ...
  IF (set_vdw_pool) THEN
     IF(.NOT.(ASSOCIATED(old_pw_grid)))CALL cp__a("pw_env_methods.F",471)
     CALL pw_grid_create(vdw_grid,para_env%group)
...
!
  ...
  IF ((poisson_params%ps_implicit_params%boundary_condition .EQ. MIXED_PERIODIC_BC) .OR. &
     ...
     pw_env%poisson_env%parameters%dbc_params%do_dbc_cube = &
        BTEST(cp_print_key_should_output(logger%iter_info,input,"DFT%PRINT%DIRICHLET_BC_CUBE"),cp_p_file)
  END IF
  ...
  IF ((poisson_params%ps_implicit_params%boundary_condition .EQ. NEUMANN_BC) .OR. &
     ...
     CALL setup_dct_pw_grids(pw_env%poisson_env%pw_pools(pw_env%poisson_env%pw_level)%pool%pw_grid,&
                          my_cell%hmat, pw_env%poisson_env%dct_pw_grid)
  END IF
...
!
  ...
  DO igrid_level=1,ngrid_level
     ...
     CALL rs_grid_create(rs_grids(igrid_level)%rs_grid, rs_descs(igrid_level)%rs_desc)
  END DO
  ...
  IF (iounit > 0) THEN
    ...
    CASE(pw_poisson_wavelet)
      ...
      CASE DEFAULT
        CALL cp__b("pw_env_methods.F",637,"Invalid periodicity for wavelet solver selected")
      END SELECT
    CASE(pw_poisson_implicit)
      ...
      CASE DEFAULT
        CALL cp__b("pw_env_methods.F",656,"Invalid boundary conditions for the implicit (iterative) poisson solver.")
      END SELECT
    ...
    CASE(pw_poisson_none)
      WRITE (UNIT=iounit,FMT="(/,T2,A,T51,A30)")&
       "POISSON| Solver","NONE"
    CASE default
      CALL cp__b("pw_env_methods.F",678,"Invalid Poisson solver selected")
    END SELECT
...
! parallel cost of rs_pw_transfer goes as safety_factor**3 so it is worthwhile keeping it tight
    ...
    IF(dft_control%qs_control%gapw) THEN
       CALL get_qs_env(qs_env=qs_env,rho0_mpole=rho0_mpole)
       IF(.NOT.(ASSOCIATED(rho0_mpole)))CALL cp__a("pw_env_methods.F",756)
    ...
    DO igrid_level=1,ngrid_level
       ...
       ! the orb_basis_set and the aux_fit_basis_set (and others!)
       ! Seemingly others (e.g. gapw soft basis set) are not needed?
       ! This is really ugly code and prone to fail at some point
       ! Urgent rethinking and refactoring needed (24.07.2015)
       DO ibasis_set_type=1,5
          basis_type = translate_basis_type(ibasis_set_type)
          DO ikind=1,nkind
            ...
            IF (dft_control%qs_control%dftb) CYCLE
             ...
             !
             IF(basis_type=="LRI" .AND. dft_control%qs_control%lri_optbas) CYCLE
...
! *****************************************************************************
  SUBROUTINE setup_super_ref_grid(super_ref_pw_grid,mt_super_ref_pw_grid,&
    ...
    IF(.NOT.(.NOT.ASSOCIATED(mt_super_ref_pw_grid)))CALL cp__a("pw_env_methods.F",930)
    IF(.NOT.(.NOT.ASSOCIATED(xc_super_ref_pw_grid)))CALL cp__a("pw_env_methods.F",931)
    IF(.NOT.(.NOT.ASSOCIATED(super_ref_pw_grid)))CALL cp__a("pw_env_methods.F",932)
    poisson_section => section_vals_get_subs_vals(input,"DFT%POISSON")
    ...
    IF (uf_grid) THEN
       CALL pw_grid_create(xc_super_ref_pw_grid,para_env%group)
    ...
    IF (mt_s_grid) THEN
       ...
       IF (ASSOCIATED(xc_super_ref_pw_grid)) THEN
          CALL cp__b("pw_env_methods.F",962,"special grid for mt and fine xc grid not compatible")
       ELSE
          ...
          IF(.NOT.(ALL(nn>no)))CALL cp__a("pw_env_methods.F",972)
