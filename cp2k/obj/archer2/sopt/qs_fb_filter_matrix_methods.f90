# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_filter_matrix_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_filter_matrix_methods.F"
!-----------------------------------------------------------------------------!
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_fb_filter_matrix_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE fb_fltrmat_build(H_mat, &
    ...
    IF(.NOT.(.NOT.ASSOCIATED(filter_mat)))CALL cp__a("qs_fb_filter_matrix_methods.F",142)
...
! *****************************************************************************
  SUBROUTINE fb_fltrmat_build_2(H_mat, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_filter_matrix_methods.F",311)
...
! *****************************************************************************
  SUBROUTINE fb_fltrmat_add_blkcol(H_mat, &
    ...
    ALLOCATE(atomic_H_blk_row_start(natoms_in_halo + 1), &
             ...
             STAT=stat)
    IF(.NOT.(stat==0))CALL cp__a("qs_fb_filter_matrix_methods.F",613)
    ...
    ALLOCATE(atomic_S_blk_row_start(natoms_in_halo + 1), &
             ...
             STAT=stat)
    IF(.NOT.(stat==0))CALL cp__a("qs_fb_filter_matrix_methods.F",633)
    ...
    DO ipe = 1, numprocs
       ...
       DO ipair = 1, send_pair_count(ipe)
          ...
          IF(.NOT.(iatom_in_halo>0))CALL cp__a("qs_fb_filter_matrix_methods.F",683)
          jatom_in_halo = ind_in_halo(jatom_global)
          IF(.NOT.(jatom_in_halo>0))CALL cp__a("qs_fb_filter_matrix_methods.F",685)
          atomic_kind => particle_set(jatom_global)%atomic_kind
          CALL get_atomic_kind(atomic_kind=atomic_kind, &
                               kind_number=jkind)
...
! *****************************************************************************
  SUBROUTINE fb_fltrmat_add_blkcol_2(H_mat, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_filter_matrix_methods.F",846)
    check_ok = fb_matrix_data_has_data(S_mat_data)
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_filter_matrix_methods.F",848)
    ...
    ALLOCATE(atomic_H_blk_row_start(natoms_in_halo + 1), &
             atomic_H_blk_col_start(natoms_in_halo + 1), &
             STAT=stat)
    IF(.NOT.(stat==0))CALL cp__a("qs_fb_filter_matrix_methods.F",877)
    CALL fb_atmatrix_calc_size(H_mat, &
                               atomic_halo, &
                               nrows_atmatrix, &
                               ncols_atmatrix, &
    ...
    ALLOCATE(atomic_S_blk_row_start(natoms_in_halo + 1), &
             atomic_S_blk_col_start(natoms_in_halo + 1), &
             STAT=stat)
    IF(.NOT.(stat==0))CALL cp__a("qs_fb_filter_matrix_methods.F",895)
    CALL fb_atmatrix_calc_size(S_mat, &
                               atomic_halo, &
                               nrows_atmatrix, &
                               ncols_atmatrix, &
...
! *****************************************************************************
  SUBROUTINE fb_fltrmat_build_atomic_fltrmat(atomic_H, &
    ...
    IF(.NOT.(check_ok))CALL cp__a("qs_fb_filter_matrix_methods.F",1343)
    ...
    IF (info .NE. 0) THEN
       ...
       CALL cp__b("qs_fb_filter_matrix_methods.F",1398,"DSYGV failed")
    END IF
