# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_build.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_build.F"
!-----------------------------------------------------------------------------!
...
# 79 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/tmc/tmc_tree_build.F" 2
...
! *****************************************************************************
  SUBROUTINE allocate_new_global_tree_node(next_el, nr_temp)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(next_el)))CALL cp__a("tmc/tmc_tree_build.F",114)
...
! *****************************************************************************
  SUBROUTINE deallocate_global_tree_node(gt_elem)
    ...
    IF(.NOT.(ASSOCIATED(gt_elem)))CALL cp__a("tmc/tmc_tree_build.F",148)
...
! *****************************************************************************
  SUBROUTINE allocate_new_sub_tree_node(tmc_params, next_el, nr_dim)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(next_el)))CALL cp__a("tmc/tmc_tree_build.F",178)
...
! *****************************************************************************
  SUBROUTINE deallocate_sub_tree_node(tree_elem)
    ...
    IF(.NOT.(ASSOCIATED(tree_elem)))CALL cp__a("tmc/tmc_tree_build.F",237)
...
! *****************************************************************************
  SUBROUTINE init_tree_mod(start_elem, tmc_env, job_counts, worker_timings)
    ...
    IF(.NOT.(ASSOCIATED(start_elem)))CALL cp__a("tmc/tmc_tree_build.F",287)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",288)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_build.F",289)
    ...
    IF(tmc_env%params%nr_temp.NE.1 .AND. tmc_env%m_env%temp_decrease.NE.1.0_dp)&
       CALL cp_abort(cp__l("tmc/tmc_tree_build.F",379),&
...
! *****************************************************************************
  SUBROUTINE finalize_init(gt_tree_ptr, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(gt_tree_ptr)))CALL cp__a("tmc/tmc_tree_build.F",442)
    IF(.NOT.(.NOT.ASSOCIATED(gt_tree_ptr%parent)))CALL cp__a("tmc/tmc_tree_build.F",443)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",444)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_build.F",445)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_tree_build.F",446)
...
! *****************************************************************************
  SUBROUTINE create_new_gt_tree_node(tmc_env, stat, new_elem, &
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",517)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_tree_build.F",518)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_build.F",519)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_act)))CALL cp__a("tmc/tmc_tree_build.F",520)
    ...
    IF(keep_on) THEN
      ...
      IF((n_acc.AND.ASSOCIATED(tmp_elem%acc)).OR.&
        ...
        IF(.NOT.(ASSOCIATED(new_elem%conf(new_elem%mv_conf)%elem)))CALL cp__a("tmc/tmc_tree_build.F",561)
        ...
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_tree_build.F",570),&
        ...
        CASE(mv_type_NMC_moves)
          IF(new_elem%conf(new_elem%mv_conf)%elem%stat.NE.status_canceled_nmc)&
             CALL cp_warn(cp__l("tmc/tmc_tree_build.F",588),&
        ...
        CASE(mv_type_atom_trans, mv_type_mol_trans, mv_type_mol_rot, &
          ...
          CALL cp_abort(cp__l("tmc/tmc_tree_build.F",598),&
        ...
        CASE DEFAULT
          CALL cp__b("tmc/tmc_tree_build.F",603,"Unknown move type while reactivating subtree element.")
        END SELECT
      ELSE
        ...
        IF(n_acc)THEN
          IF(ASSOCIATED(tmp_elem%acc))&
             CALL cp__b("tmc/tmc_tree_build.F",617,"creating new subtree element on an occupied acc branch")
          tmp_elem%acc => new_elem
        ELSE
          IF(ASSOCIATED(tmp_elem%nacc))&
             CALL cp__b("tmc/tmc_tree_build.F",621,"creating new subtree element on an occupied nacc branch")
    ...
    IF(keep_on) THEN ! status changes
      IF(new_elem%stat.EQ.status_accepted_result .OR. &
         ...
         new_elem%stat.EQ.status_rejected_result)&
           CALL cp__b("tmc/tmc_tree_build.F",769,"selected existing RESULT gt node")
      ...
      CASE(status_cancel_nmc, status_canceled_nmc)
         ...
         IF(tmc_env%params%DRAW_TREE) &
           CALL create_dot_color(tree_element=new_elem%conf(new_elem%mv_conf)%elem,&
                                 tmc_params=tmc_env%params)
      CASE DEFAULT
        CALL cp_abort(cp__l("tmc/tmc_tree_build.F",820),&
             "unknown stat "//&
             cp_to_string(new_elem%conf(new_elem%mv_conf)%elem%stat)//&
             "of subtree element "//&
...
! *****************************************************************************
  SUBROUTINE create_new_subtree_node(act_gt_el, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(act_gt_el)))CALL cp__a("tmc/tmc_tree_build.F",857)
    IF(.NOT.(ASSOCIATED(act_gt_el%conf(act_gt_el%mv_conf)%elem)))CALL cp__a("tmc/tmc_tree_build.F",858)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",859)
    IF(.NOT.(ASSOCIATED(tmc_env%params)))CALL cp__a("tmc/tmc_tree_build.F",860)
    ...
    CASE DEFAULT
      CALL cp_abort(cp__l("tmc/tmc_tree_build.F",979),&
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(act_gt_el%conf(act_gt_el%mv_conf)%elem)))CALL cp__a("tmc/tmc_tree_build.F",987)
...
! *****************************************************************************
  SUBROUTINE remove_gt_elem(gt_ptr, draw, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(gt_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1011)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1012)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(gt_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1051)
...
! *****************************************************************************
  SUBROUTINE remove_st_elem(ptr, draw, tmc_env)
    ...
    ready = .TRUE.
    IF(.NOT.(ASSOCIATED(ptr)))CALL cp__a("tmc/tmc_tree_build.F",1074)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1075)
    ...
    IF(ASSOCIATED(ptr%gt_nodes_references)) THEN
      IF(ASSOCIATED(ptr%parent))&
         CALL cp_warn(cp__l("tmc/tmc_tree_build.F",1083),&
      ...
      IF(.NOT.(ASSOCIATED(ptr%gt_nodes_references%gt_elem)))CALL cp__a("tmc/tmc_tree_build.F",1088)
    ELSE
      ...
      CASE(status_calculate_energy, &
        ...
        IF(.NOT.(tmc_env%params%SPECULATIVE_CANCELING))CALL cp__a("tmc/tmc_tree_build.F",1096)
      ...
      IF(ASSOCIATED(ptr%parent).AND.&
        ...
        CALL cp__b("tmc/tmc_tree_build.F",1106,"")
      END IF
...
! *****************************************************************************
  RECURSIVE SUBROUTINE remove_unused_g_tree(begin_ptr, end_ptr, removed, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(begin_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1161)
    IF(.NOT.(ASSOCIATED(end_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1162)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1163)
    ...
    IF(.NOT.ASSOCIATED(begin_ptr, end_ptr)) THEN
      ...
      IF(acc_removed.AND.nacc_removed)THEN
        ...
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_tree_build.F",1201),&
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(end_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1209)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE remove_unused_s_tree(begin_ptr, end_ptr,working_elem_list,&
    ...
    IF(.NOT.(ASSOCIATED(begin_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1248)
    IF(.NOT.(ASSOCIATED(end_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1249)
    IF(.NOT.(ASSOCIATED(working_elem_list)))CALL cp__a("tmc/tmc_tree_build.F",1250)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1251)
    ...
    IF(.NOT.ASSOCIATED(begin_ptr, end_ptr)) THEN
      ...
      IF(acc_removed.AND.nacc_removed)THEN
        ...
        CASE DEFAULT
          CALL cp_abort(cp__l("tmc/tmc_tree_build.F",1292),&
        ...
        IF(remove_this)THEN
          ...
          IF(.NOT.ASSOCIATED(begin_ptr%parent))&
             CALL cp_abort(cp__l("tmc/tmc_tree_build.F",1303),&
...
! *****************************************************************************
  RECURSIVE SUBROUTINE remove_result_g_tree(end_of_clean_tree, actual_ptr, &
    ...
    IF(.NOT.(ASSOCIATED(end_of_clean_tree)))CALL cp__a("tmc/tmc_tree_build.F",1353)
    IF(.NOT.(ASSOCIATED(actual_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1354)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE remove_result_s_tree(end_of_clean_tree, actual_ptr,&
    ...
    IF(.NOT.(ASSOCIATED(end_of_clean_tree)))CALL cp__a("tmc/tmc_tree_build.F",1396)
    IF(.NOT.(ASSOCIATED(actual_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1397)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1398)
    ...
    IF(.NOT.ASSOCIATED(actual_ptr, end_of_clean_tree)) THEN
      ...
      CALL cp__b("tmc/tmc_tree_build.F",1412,"")
      END IF
...
! *****************************************************************************
  SUBROUTINE remove_all_trees(working_elem_list, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(working_elem_list)))CALL cp__a("tmc/tmc_tree_build.F",1450)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1451)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_build.F",1452)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_act)))CALL cp__a("tmc/tmc_tree_build.F",1453)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_clean_end)))CALL cp__a("tmc/tmc_tree_build.F",1454)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%result_list)))CALL cp__a("tmc/tmc_tree_build.F",1455)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%st_clean_ends)))CALL cp__a("tmc/tmc_tree_build.F",1456)
    ...
    IF(tmc_env%params%USE_REDUCED_TREE)THEN
      ...
      IF(.NOT.(.NOT.ASSOCIATED(tmc_env%m_env%gt_head%parent)))CALL cp__a("tmc/tmc_tree_build.F",1481)
    ...
    DO tree=1, tmc_env%params%nr_temp
      ...
      conf_loop: DO i=1, SIZE(tmc_env%m_env%result_list)
        ...
        IF(last_acc_st_elem%sub_tree_nr .EQ. tree) &
          EXIT conf_loop
      END DO conf_loop
      IF(.NOT.(last_acc_st_elem%sub_tree_nr.EQ.tree))CALL cp__a("tmc/tmc_tree_build.F",1494)
    ...
    IF(tmc_env%params%USE_REDUCED_TREE .AND. change_trajec)THEN
      DO tree=1, tmc_env%params%nr_temp
        ...
        IF(.NOT.(tmp_ptr%sub_tree_nr.EQ.tree))CALL cp__a("tmc/tmc_tree_build.F",1505)
    ...
    ! end the timing
    CALL timestop(handle)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_act)))CALL cp__a("tmc/tmc_tree_build.F",1517)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env%gt_clean_end)))CALL cp__a("tmc/tmc_tree_build.F",1518)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE dealloc_whole_g_tree(begin_ptr, removed, tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(begin_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1539)
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1540)
...
! *****************************************************************************
  RECURSIVE SUBROUTINE dealloc_whole_subtree(begin_ptr, removed, tmc_params)
    ...
    IF(.NOT.(ASSOCIATED(begin_ptr)))CALL cp__a("tmc/tmc_tree_build.F",1583)
    IF(.NOT.(ASSOCIATED(tmc_params)))CALL cp__a("tmc/tmc_tree_build.F",1584)
...
! *****************************************************************************
  SUBROUTINE finalize_trees(tmc_env)
    ...
    IF(.NOT.(ASSOCIATED(tmc_env)))CALL cp__a("tmc/tmc_tree_build.F",1625)
    IF(.NOT.(ASSOCIATED(tmc_env%m_env)))CALL cp__a("tmc/tmc_tree_build.F",1626)
