# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
     ...
     DO i = 1, 3
        DO k = 1, 3
           IF (.NOT.check_value(matrix_n%sp_d(j,k,i), ij_matrix%sp_d(j,k,i), dx, 0.1_dp)) THEN
              ...
              CALL cp__b("semi_empirical_int_debug.F",73,"")
           END IF
        END DO
     END DO
     ! PP
        ...
        DO k = 1, 3
           DO l = 1, 6
              IF (.NOT.check_value(matrix_n%pp_d(j,l,k,i), ij_matrix%pp_d(j,l,k,i), dx, 0.1_dp)) THEN
                 ...
                 CALL cp__b("semi_empirical_int_debug.F",84,"")
              END IF
           END DO
        END DO
     END DO
        ...
        DO i = 1, 5
           DO k = 1, 5
              IF (.NOT.check_value(matrix_n%sd_d(j,k,i), ij_matrix%sd_d(j,k,i), dx, 0.1_dp)) THEN
                 ...
                 CALL cp__b("semi_empirical_int_debug.F",97,"")
              END IF
           END DO
        END DO
        ! DP
           ...
           DO k = 1, 5
              DO l = 1, 15
                 IF (.NOT.check_value(matrix_n%pd_d(j,l,k,i), ij_matrix%pd_d(j,l,k,i), dx, 0.1_dp)) THEN
                    ...
                    CALL cp__b("semi_empirical_int_debug.F",108,"")
                 END IF
              END DO
           END DO
        END DO
           ...
           DO k = 1, 5
              DO l = 1, 15
                 IF (.NOT.check_value(matrix_n%dd_d(j,l,k,i), ij_matrix%dd_d(j,l,k,i), dx, 0.1_dp)) THEN
                    ...
                    CALL cp__b("semi_empirical_int_debug.F",120,"")
                 END IF
              END DO
           END DO
        END DO
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 163 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
     ...
     DO i = 1, 45
        DO k = 1, limkl
           ...
           IF (.NOT.check_value(v_d(j,i,k), v_n(i,k), dx, 0.1_dp)) THEN
              ...
              CALL cp__b("semi_empirical_int_debug.F",228,"")
           END IF
        END DO
     END DO
  END DO
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 258 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  REAL(dp), INTENT(IN)                     :: r
  ...
  IF (.NOT.check_value(nssss, dssss, delta, 0.1_dp)) THEN
     ...
     CALL cp__b("semi_empirical_int_debug.F",283,"")
  END IF
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 310 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  REAL(dp), INTENT(IN)                     :: r
  ...
  DO i = 1, 2
     DO j = 1, 10
        IF (.NOT.check_value(ncore(j,i), dcore(j,i), delta, 0.1_dp)) THEN
           ...
           CALL cp__b("semi_empirical_int_debug.F",339,"")
        END IF
     END DO
  END DO
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 411 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  ...
  IF (l_e1b) THEN
     DO j = 1, 45
        IF (.NOT.check_value(e1b2(j), e1b(j), delta, 0.1_dp)) THEN
           ...
           CALL cp__b("semi_empirical_int_debug.F",445,"")
        END IF
     END DO
  END IF
  IF (l_e2a) THEN
     DO J = 1, 45
        IF (.NOT.check_value(e2a2(j), e2a(j), delta, 0.1_dp)) THEN
           ...
           CALL cp__b("semi_empirical_int_debug.F",453,"")
        END IF
     END DO
  END IF
           ...
           DO j = 1, 45
              ...
              IF (ABS(e1b2(j))>delta) THEN
                 IF (.NOT.check_value(de1b2(i,j), de1b(i,j), delta, 0.1_dp)) THEN
                    ...
                    CALL cp__b("semi_empirical_int_debug.F",469,"")
                 END IF
              END IF
           END DO
        END DO
           ...
           DO j = 1, 45
              ...
              IF (ABS(e2a2(j))>delta) THEN
                 IF (.NOT.check_value(de2a2(i,j), de2a(i,j), delta, 0.1_dp)) THEN
                    ...
                    CALL cp__b("semi_empirical_int_debug.F",482,"")
                 END IF
              END IF
           END DO
        END DO
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 516 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  REAL(dp), DIMENSION(3), INTENT(IN)       :: rijv
  INTEGER, INTENT(IN)                      :: itype
  ...
  IF (PRESENT(enuc)) THEN
     ...
     IF (.NOT.check_value(enuc, enuc_num, delta, 0.001_dp)) THEN
        ...
        CALL cp__b("semi_empirical_int_debug.F",541,"")
     END IF
  END IF
  IF (PRESENT(denuc)) THEN
     ...
     DO j = 1, 3
        IF (.NOT.check_value(denuc(j), denuc_num(j), delta, 0.001_dp)) THEN
           ...
           CALL cp__b("semi_empirical_int_debug.F",549,"")
        END IF
     END DO
  END IF
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 579 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  REAL(dp), INTENT(IN)                     :: r
  ...
  DO j = 1, 491
     IF  (ABS(ri(j)-ri0(j))>EPSILON(0.0_dp)) THEN
        ...
        CALL cp__b("semi_empirical_int_debug.F",611,"")
     END IF
     IF (lgrad) THEN
        IF (.NOT.check_value(dri(j), nri(j), delta*100.0_dp, 0.1_dp)) THEN
           ...
           END DO
           CALL cp__b("semi_empirical_int_debug.F",620,"")
        END IF
     END IF
  END DO
...
! This code should be valid for any Fortran variable, is always standard conforming,
! and will be optimized away completely by the compiler
# 650 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/semi_empirical_int_debug.F" 2
  IMPLICIT NONE
  TYPE(semi_empirical_type), POINTER       :: sepi, sepj
  REAL(dp), DIMENSION(3), INTENT(IN)       :: rijv
  ...
  IF (PRESENT(w)) THEN
     ...
     DO j = 1, 2025
        IF (.NOT.check_value(w(j), w2(j), delta, 0.1_dp)) THEN
           ...
           CALL cp__b("semi_empirical_int_debug.F",677,"")
        END IF
     END DO
  END IF
  IF (PRESENT(dw)) THEN
        ...
        DO j = 1, 2025
           IF ((ABS(w2(j))>delta).AND.(ABS(dw2(i,j))>delta*10)) THEN
              IF (.NOT.check_value(dw(i,j), dw2(i,j), delta, 0.1_dp)) THEN
                 ...
                 CALL cp__b("semi_empirical_int_debug.F",693,"")
              END IF
           END IF
        END DO
     END DO
