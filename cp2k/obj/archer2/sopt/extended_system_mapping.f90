# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_mapping.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_mapping.F"
!-----------------------------------------------------------------------------!
...
MODULE extended_system_mapping
  ...
  USE input_constants,                 ONLY: &
       ...
       nvt_adiabatic_ensemble, nvt_ensemble, reftraj_ensemble
  USE kinds,                           ONLY: dp
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/motion/thermostat/extended_system_mapping.F" 2
...
! *****************************************************************************
  SUBROUTINE nhc_to_barostat_mapping ( simpar, nhc)
    ...
    CASE  DEFAULT
       CALL cp__b("motion/thermostat/extended_system_mapping.F",75,'Never reach this point!')
...
! *****************************************************************************
  SUBROUTINE nhc_to_particle_mapping ( thermostat_info, simpar, local_molecules,&
    ...
    SELECT CASE ( simpar % ensemble )
    CASE DEFAULT
       CALL cp__b("motion/thermostat/extended_system_mapping.F",163,'Unknown ensemble!')
    CASE ( nve_ensemble, isokin_ensemble,npe_f_ensemble, npe_i_ensemble, nph_uniaxial_ensemble,&
       ...
       CALL cp__b("motion/thermostat/extended_system_mapping.F",166,'Never reach this point!')
    CASE ( nvt_ensemble, npt_i_ensemble, npt_f_ensemble )
       ...
       IF ( ( sum_of_thermostats == 1 ) .AND. ( map_info%dis_type /= do_thermo_no_communication ) ) THEN
          ...
          IF ( fac == 0.0_dp ) THEN
             CALL cp__b("motion/thermostat/extended_system_mapping.F",191,'Zero degrees of freedom. Nothing to thermalize!')
          END IF
...
! *****************************************************************************
  SUBROUTINE nhc_to_particle_mapping_slow ( thermostat_info, simpar, local_molecules,&
    ...
    SELECT CASE ( simpar % ensemble )
    CASE DEFAULT
       CALL cp__b("motion/thermostat/extended_system_mapping.F",334,'Unknown ensemble!')
    CASE ( nvt_adiabatic_ensemble )
       ...
       IF ( ( sum_of_thermostats == 1 ) .AND. ( map_info%dis_type /= do_thermo_no_communication ) ) THEN
          fac = map_info%s_kin(1)-deg_of_freedom(1)-simpar%nfree_rot_transl
          IF ( fac == 0.0_dp ) THEN
             CALL cp__b("motion/thermostat/extended_system_mapping.F",359,'Zero degrees of freedom. Nothing to thermalize!')
          END IF
...
! *****************************************************************************
  SUBROUTINE nhc_to_particle_mapping_fast ( thermostat_info, simpar, local_molecules,&
    ...
    SELECT CASE ( simpar % ensemble )
    CASE DEFAULT
       CALL cp__b("motion/thermostat/extended_system_mapping.F",432,'Unknown ensemble!')
    CASE ( nvt_adiabatic_ensemble )
       CALL setup_adiabatic_thermostat(nhc, thermostat_info, deg_of_freedom, massive_atom_list,&
            molecule_kind_set, local_molecules, molecule_set, para_env, natoms_local, &
            simpar, sum_of_thermostats, gci)
       ...
       IF ( ( sum_of_thermostats == 1 ) .AND. ( map_info%dis_type /= do_thermo_no_communication ) ) THEN
          fac = map_info%s_kin(1)-deg_of_freedom(1)-simpar%nfree_rot_transl
          IF ( fac == 0.0_dp ) THEN
             CALL cp__b("motion/thermostat/extended_system_mapping.F",457,'Zero degrees of freedom. Nothing to thermalize!')
          END IF
...
! *****************************************************************************
  SUBROUTINE nhc_to_shell_mapping( thermostat_info, simpar, local_molecules,&
    ...
    SELECT CASE ( simpar % ensemble )
    CASE DEFAULT
       CALL cp__b("motion/thermostat/extended_system_mapping.F",598,'Unknown ensemble!')
    CASE ( isokin_ensemble, nph_uniaxial_ensemble,&
           nph_uniaxial_damped_ensemble, reftraj_ensemble, langevin_ensemble )
       CALL cp__b("motion/thermostat/extended_system_mapping.F",601,'Never reach this point!')
