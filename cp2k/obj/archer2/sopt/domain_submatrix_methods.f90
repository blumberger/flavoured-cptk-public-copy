# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/domain_submatrix_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/domain_submatrix_methods.F"
!-----------------------------------------------------------------------------!
...
# 37 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/domain_submatrix_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE copy_submatrix_array(original,copy,copy_data)
    ...
    ndomainsB=SIZE(copy)
    IF(.NOT.(ndomains.eq.ndomainsB))CALL cp__a("domain_submatrix_methods.F",165)
...
! *****************************************************************************
  SUBROUTINE copy_submatrix_data(array,copy)
    ...
    IF(.NOT.(copy%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",281)
...
! *****************************************************************************
  SUBROUTINE set_submatrix(submatrix,scalar)
    ...
    IF(.NOT.(submatrix%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",350)
    IF(.NOT.(submatrix%nrows.gt.0))CALL cp__a("domain_submatrix_methods.F",351)
    IF(.NOT.(submatrix%ncols.gt.0))CALL cp__a("domain_submatrix_methods.F",352)
...
! *****************************************************************************
  SUBROUTINE multiply_submatrices_once(transA,transB,alpha,A,B,beta,C)
    ...
    IF(.NOT.(A%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",473)
    IF(.NOT.(B%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",474)
    IF(.NOT.(C%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",475)
    ...
    IF(.NOT.(K.eq.K1))CALL cp__a("domain_submatrix_methods.F",504)
    ...
    IF (.NOT.ALLOCATED(C%mdata)) THEN
       ...
       IF(.NOT.(beta.eq.0.0_dp))CALL cp__a("domain_submatrix_methods.F",550)
    ...
    ELSE
       ...
       IF ((C%nrows.ne.cs1).OR.(C%ncols.ne.cs2)) THEN
          ...
          IF(.NOT.(beta.eq.0.0_dp))CALL cp__a("domain_submatrix_methods.F",557)
          DEALLOCATE(C%mdata)
          ALLOCATE(C%mdata(C%nrows,C%ncols))
       ENDIF
    ENDIF
...
! *****************************************************************************
  SUBROUTINE multiply_submatrices_array(transA,transB,alpha,A,B,beta,C)
    ...
    IF(.NOT.(ndomains.eq.ndomainsB))CALL cp__a("domain_submatrix_methods.F",607)
    IF(.NOT.(ndomainsB.eq.ndomainsC))CALL cp__a("domain_submatrix_methods.F",608)
    ...
    DO idomain = 1, ndomains
       ...
       IF(.NOT.(idomainA.eq.idomainB))CALL cp__a("domain_submatrix_methods.F",615)
...
! *****************************************************************************
  SUBROUTINE add_submatrices_once(alpha,A,beta,B,transB)
    ...
    IF(.NOT.(A%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",656)
    IF(.NOT.(B%domain.gt.0))CALL cp__a("domain_submatrix_methods.F",657)
    ...
    ! these checks are for debugging only
    IF(.NOT.(C1.eq.C2))CALL cp__a("domain_submatrix_methods.F",673)
    IF(.NOT.(R1.eq.R2))CALL cp__a("domain_submatrix_methods.F",674)
...
! *****************************************************************************
  SUBROUTINE add_submatrices_array(alpha,A,beta,B,transB)
    ...
    IF(.NOT.(ndomains.eq.ndomainsB))CALL cp__a("domain_submatrix_methods.F",719)
    ...
    DO idomain = 1, ndomains
       ...
       IF(.NOT.(idomainA.eq.idomainB))CALL cp__a("domain_submatrix_methods.F",726)
...
!TYPE(cp_dbcsr_type), INTENT(IN)                        :: distr_pattern
    ...
    IF(.NOT.(ndomainsA.eq.ndomainsB))CALL cp__a("domain_submatrix_methods.F",822)
    ...
    DO idomain = 1, ndomainsA
       ...
       IF(.NOT.(domainA.eq.domainB))CALL cp__a("domain_submatrix_methods.F",829)

       ! check if the submatrix is local
       IF (domainA.gt.0) THEN

          IF(.NOT.(A(idomain)%nrows.eq.B(idomain)%nrows))CALL cp__a("domain_submatrix_methods.F",834)
          IF(.NOT.(A(idomain)%ncols.eq.B(idomain)%ncols))CALL cp__a("domain_submatrix_methods.F",835)
...
!    subm_col_size(:)=0
    ...
    IF (ndomains2.ne.ndomains) THEN
       CALL cp__b("domain_submatrix_methods.F",1192,"wrong submatrix size")
    ENDIF
             ...
             DO iBlock=1, recv_size2_cpu(iNode)/2
                ...
                IF ((first_col(col).NE.-1).AND.(first_row(row).NE.-1)) THEN
                   ...
                   IF (job_type==select_row_col) THEN
                      ...
                      ELSE
                         CALL cp__b("domain_submatrix_methods.F",1343,"matrix type is NYI")
                      ENDIF
                   ENDIF
                ENDIF
...
! *****************************************************************************
  SUBROUTINE construct_dbcsr_from_submatrices(matrix,submatrix,distr_pattern)
    ...
    INTEGER :: block_offset, col, col_offset, colsize, dest_node, GroupID, &
      ...
      send_size_total, smroff, start_data, unit_nr, idomain_tmp
    ...
    IF (ndomains.ne.ndomains2) THEN
       CALL cp__b("domain_submatrix_methods.F",1420,"domain mismatch")
    ENDIF
    ...
    IF (matrix_type.ne.dbcsr_type_no_symmetry) THEN
       CALL cp__b("domain_submatrix_methods.F",1432,"only non-symmetric matrices so far")
    ENDIF
       ...
       IF (submatrix(idomain)%domain.gt.0) THEN
          ...
          DO irow_subm = 1, submatrix(idomain)%nbrows
             ...
             IF (submatrix(idomain)%nbcols.ne.1) THEN
                CALL cp__b("domain_submatrix_methods.F",1459,"corrupt submatrix structure")
             ENDIF
             ...
             IF (col.ne.idomain) THEN
                CALL cp__b("domain_submatrix_methods.F",1466,"corrupt submatrix structure")
             ENDIF
             ...
             idomain_tmp = idomain
             CALL cp_dbcsr_get_stored_coordinates(distr_pattern,&
                     row, idomain_tmp, dest_node)
             
             send_descriptor(1,dest_node+1)=send_descriptor(1,dest_node+1)+1
             send_descriptor(2,dest_node+1)=send_descriptor(2,dest_node+1)+&
                submatrix(idomain_tmp)%size_brow(irow_subm)*&
                submatrix(idomain_tmp)%size_bcol(1)
          ...
          DO irow_subm = 1, submatrix(idomain)%nbrows
             ...
             DO icol=1,colsize
                ...
                send_data(start_data+1:start_data+rowsize)=&
                   submatrix(idomain_tmp)%mdata(smroff+1:smroff+rowsize,icol)
    ...
    DO iNode=1, nNodes
       ...
       DO iBlock=1, recv_size2_cpu(iNode)/2
          ...
          IF(.NOT.(ASSOCIATED(block_p)))CALL cp__a("domain_submatrix_methods.F",1614)
          !ENDIF
