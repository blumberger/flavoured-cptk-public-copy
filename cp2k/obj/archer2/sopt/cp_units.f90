# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_units.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_units.F"
!-----------------------------------------------------------------------------!
...
# 42 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_units.F" 2
  ...
  SUBROUTINE cp_unit_create(unit, string)
    ...
    DO
       ...
       IF (i_unit>cp_unit_max_kinds) THEN
          CALL cp__b("common/cp_units.F",199,"Maximum number of combined units exceeded")
          EXIT
       END IF
       ! read unit
       ...
       CASE("force")
          ...
          kind_id(i_unit)=cp_ukind_force
       CASE("au")
          CALL cp_abort(cp__l("common/cp_units.F",361),&
               "au unit without specifing its kind not accepted, use "//&
               "(au_e, au_f, au_t, au_temp, au_l, au_m, au_p, au_pot)")
       CASE default
          CALL cp__b("common/cp_units.F",365,"Unknown unit: "//string(i_low:i_high-1))
       END SELECT
       ...
       IF (i_high<=len_string) THEN
          IF (string(i_low:i_high)=='^') THEN
             ...
             IF (i_high<=i_low.OR.i_low>len_string) THEN
                CALL cp__b("common/cp_units.F",399,"an integer number is expected after a '^'")
                EXIT
             END IF
...
! *****************************************************************************
  SUBROUTINE cp_unit_create2(unit, kind_id, unit_id, power)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(unit)))CALL cp__a("common/cp_units.F",466)
    IF(.NOT.(SIZE(kind_id)<=cp_unit_max_kinds))CALL cp__a("common/cp_units.F",467)
    IF(.NOT.(SIZE(unit_id)<=cp_unit_max_kinds))CALL cp__a("common/cp_units.F",468)
    ALLOCATE(unit)
    unit%ref_count=1
...
! *****************************************************************************
  SUBROUTINE cp_unit_retain(unit)
    ...
    IF(.NOT.(ASSOCIATED(unit)))CALL cp__a("common/cp_units.F",561)
    IF(.NOT.(unit%ref_count>0))CALL cp__a("common/cp_units.F",562)
...
! *****************************************************************************
  SUBROUTINE cp_unit_release(unit)
    ...
    IF (ASSOCIATED(unit)) THEN
       IF(.NOT.(unit%ref_count>0))CALL cp__a("common/cp_units.F",580)
       unit%ref_count=unit%ref_count-1
...
! *****************************************************************************
  SUBROUTINE cp_basic_unit_check(basic_kind,basic_unit)
    ...
    CASE(cp_ukind_undef)
       ...
       CASE(cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",605,"unknown undef unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_energy)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",613,"unknown energy unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_length)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",620,"unknown length unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_temperature)
       SELECT CASE (basic_unit)
       CASE(cp_units_k,cp_units_au,cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",626,"unknown temperature unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_pressure)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",632,"unknown pressure unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_angle)
       SELECT CASE (basic_unit)
       CASE(cp_units_rad, cp_units_deg,cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",638,"unknown angle unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_time)
       SELECT CASE (basic_unit)
       CASE(cp_units_s, cp_units_fs, cp_units_ps, cp_units_au, cp_units_wn, cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",644,"unknown time unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_mass)
       SELECT CASE (basic_unit)
       CASE(cp_units_kg, cp_units_amu, cp_units_m_e, cp_units_au, cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",650,"unknown mass unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_potential)
       SELECT CASE (basic_unit)
       CASE(cp_units_volt, cp_units_au, cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",656,"unknown potential unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_force)
       SELECT CASE (basic_unit)
       CASE(cp_units_Newton, cp_units_mNewton, cp_units_au, cp_units_none)
       CASE default
          CALL cp__b("common/cp_units.F",662,"unknown force unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_none)
       IF(basic_unit/=cp_units_none)&
            CALL cp_abort(cp__l("common/cp_units.F",666),&
                 "if the kind of the unit is none also unit must be undefined,not:"&
                 //TRIM(cp_to_string(basic_unit)))
    CASE default
       CALL cp__b("common/cp_units.F",670,"unknown kind of unit:"//TRIM(cp_to_string(basic_kind)))
    END SELECT
  ...
  FUNCTION cp_basic_unit_to_cp2k(value,basic_kind,basic_unit,power)  RESULT(res)
    ...
    IF (basic_unit==cp_units_none.AND.basic_kind/=cp_ukind_undef) THEN
       IF(basic_kind/=cp_units_none)&
          CALL cp_abort(cp__l("common/cp_units.F",698),&
    ...
    CASE(cp_ukind_undef)
       ...
       CASE(cp_units_none)
          res = value
       CASE default
          CALL cp__b("common/cp_units.F",708,"unknown energy unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_energy)
       SELECT CASE (basic_unit)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",733,"unknown energy unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_length)
       SELECT CASE (basic_unit)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",748,"unknown length unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_temperature)
       SELECT CASE (basic_unit)
       CASE(cp_units_k)
       ...
       CASE(cp_units_au)
          res=value
       CASE default
          CALL cp__b("common/cp_units.F",757,"unknown temperature unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_pressure)
       SELECT CASE (basic_unit)
       CASE(cp_units_bar)
       ...
       CASE(cp_units_au)
          res = value
       CASE default
          CALL cp__b("common/cp_units.F",776,"unknown pressure unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_angle)
       SELECT CASE (basic_unit)
       CASE(cp_units_rad)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",785,"unknown angle unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_time)
       SELECT CASE (basic_unit)
       CASE(cp_units_s)
       ...
       CASE default
          CALL cp__b("common/cp_units.F",800,"unknown time unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_mass)
       SELECT CASE (basic_unit)
       CASE(cp_units_kg)
       ...
       CASE(cp_units_m_e,cp_units_au)
          res=value
       CASE default
          CALL cp__b("common/cp_units.F",811,"unknown mass unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_potential)
       SELECT CASE (basic_unit)
       CASE(cp_units_volt)
       ...
       CASE(cp_units_au)
          res=value
       CASE default
          CALL cp__b("common/cp_units.F",820,"unknown potential unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_force)
       SELECT CASE (basic_unit)
       ...
       CASE(cp_units_au)
          res=value
       CASE default
          CALL cp__b("common/cp_units.F",831,"unknown force unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_none)
       CALL cp_abort(cp__l("common/cp_units.F",834),&
            "if the kind of the unit is none also unit must be undefined,not:"&
            //TRIM(cp_to_string(basic_unit)))
    CASE default
       CALL cp__b("common/cp_units.F",838,"unknown kind of unit:"//TRIM(cp_to_string(basic_kind)))
    END SELECT
  ...
  FUNCTION cp_basic_unit_desc(basic_kind,basic_unit,power,accept_undefined)&
    ...
    IF (basic_unit==cp_units_none) THEN
       IF(.NOT.my_accept_undefined.AND.basic_kind==cp_units_none)&
          CALL cp_abort(cp__l("common/cp_units.F",871),"unit not yet fully specified, unit of kind "//&
    ...
    CASE(cp_ukind_undef)
       ...
       CASE(cp_units_none)
          res="internal_cp2k"
       CASE DEFAULT
          CALL cp_abort(cp__l("common/cp_units.F",880),&
               "unit not yet fully specified, unit of kind "//&
               TRIM(res))
       END SELECT
    CASE(cp_ukind_energy)
       ...
       CASE(cp_units_none)
          ...
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",909),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",913,"unknown energy unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_length)
       SELECT CASE (basic_unit)
       CASE(cp_units_bohr,cp_units_au)
       ...
       CASE default
          res="length"
          CALL cp__b("common/cp_units.F",929,"unknown length unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_temperature)
       SELECT CASE (basic_unit)
       CASE(cp_units_k)
       ...
       CASE(cp_units_none)
          res="temperature"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",940),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",944,"unknown temperature unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_pressure)
       SELECT CASE (basic_unit)
       CASE(cp_units_bar)
       ...
       CASE(cp_units_none)
          res="pressure"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",965),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",969,"unknown pressure unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_angle)
       SELECT CASE (basic_unit)
       CASE(cp_units_rad)
       ...
       CASE(cp_units_none)
          res="angle"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",980),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",984,"unknown angle unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_time)
       SELECT CASE (basic_unit)
       CASE(cp_units_s)
       ...
       CASE(cp_units_none)
          res="time"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",1001),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",1005,"unknown time unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_mass)
       SELECT CASE (basic_unit)
       CASE(cp_units_kg)
       ...
       CASE(cp_units_none)
          res="mass"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",1018),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",1022,"unknown mass unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_potential)
       SELECT CASE (basic_unit)
       CASE(cp_units_volt)
       ...
       CASE(cp_units_none)
          res="potential"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",1033),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",1037,"unknown potential unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_force)
       SELECT CASE (basic_unit)
       CASE(cp_units_Newton)
       ...
       CASE(cp_units_none)
          res="force"
          IF(.NOT.my_accept_undefined)&
             CALL cp_abort(cp__l("common/cp_units.F",1050),&
                  "unit not yet fully specified, unit of kind "//&
                  TRIM(res))
       CASE default
          CALL cp__b("common/cp_units.F",1054,"unknown potential unit:"//TRIM(cp_to_string(basic_unit)))
       END SELECT
    CASE(cp_ukind_none)
       CALL cp_abort(cp__l("common/cp_units.F",1057),&
            "if the kind of the unit is none also unit must be undefined,not:"&
            //TRIM(cp_to_string(basic_unit)))
    CASE default
       CALL cp__b("common/cp_units.F",1061,"unknown kind of unit:"//TRIM(cp_to_string(basic_kind)))
    END SELECT
    IF (my_power/=1) THEN
       a=LEN_TRIM(res)
       IF(.NOT.(LEN(res)-a>=3))CALL cp__a("common/cp_units.F",1065)
...
! *****************************************************************************
  FUNCTION cp_unit_desc(unit,defaults,accept_undefined)&
    ...
    LOGICAL                                  :: check, has_defaults, &
                                                my_accept_undefined

    IF(.NOT.(ASSOCIATED(unit)))CALL cp__a("common/cp_units.F",1095)
    res=""
    pos=1
    my_accept_undefined=.FALSE.
    IF (PRESENT(accept_undefined)) my_accept_undefined=accept_undefined
    DO i=1,unit%n_kinds
       IF(.NOT.(unit%kind_id(i)/=0))CALL cp__a("common/cp_units.F",1101)
       IF(.NOT.(pos<LEN(res)))CALL cp__a("common/cp_units.F",1102)
       ...
       IF (my_unit==0) THEN
          ...
          ELSE
             ...
             IF(.NOT.(check))CALL cp__a("common/cp_units.F",1111)
          END IF
       END IF
       IF (i > 1) THEN
          res(pos:pos) = "*"
...
! *****************************************************************************
  FUNCTION cp_unit_to_cp2k1(value,unit,defaults,power) RESULT(res)
    ...
    IF (PRESENT(power)) my_power=power
    res=value
    IF(.NOT.(ASSOCIATED(unit)))CALL cp__a("common/cp_units.F",1153)
    DO i_unit=1,unit%n_kinds
       IF(.NOT.(unit%kind_id(i_unit)>0))CALL cp__a("common/cp_units.F",1155)
       my_basic_unit=unit%unit_id(i_unit)
       IF (my_basic_unit==0.AND.unit%kind_id(i_unit)/=cp_ukind_undef) THEN
          IF(.NOT.(PRESENT(defaults)))CALL cp__a("common/cp_units.F",1158)
          IF(.NOT.(ASSOCIATED(defaults)))CALL cp__a("common/cp_units.F",1159)
          IF(.NOT.(ASSOCIATED(defaults%units(unit%kind_id(i_unit))%unit)))CALL cp__a("common/cp_units.F",1160)
...
! *****************************************************************************
  SUBROUTINE cp_unit_set_create(unit_set,name)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(unit_set)))CALL cp__a("common/cp_units.F",1313)
    ALLOCATE(unit_set)
       ...
       CASE('OUTPUT')
          ...
          CASE (cp_ukind_force)
             CALL cp_unit_create2(unit_set%units(i)%unit, kind_id=(/i/), unit_id=(/cp_units_newton/),&
                  power=(/1/))
          CASE default
             CALL cp__b("common/cp_units.F",1367,"unhandled unit type "//TRIM(cp_to_string(i)))
             EXIT
          END SELECT
       CASE default
          CALL cp__b("common/cp_units.F",1371,'unknown parameter set name '//TRIM(name))
       END SELECT
    END DO
...
! *****************************************************************************
  SUBROUTINE cp_unit_set_retain(unit_set)
    ...
    IF(.NOT.(ASSOCIATED(unit_set)))CALL cp__a("common/cp_units.F",1387)
    IF(.NOT.(unit_set%ref_count>0))CALL cp__a("common/cp_units.F",1388)
...
! *****************************************************************************
  SUBROUTINE cp_unit_set_release(unit_set)
    ...
    IF (ASSOCIATED(unit_set)) THEN
       IF(.NOT.(unit_set%ref_count>0))CALL cp__a("common/cp_units.F",1406)
       unit_set%ref_count=unit_set%ref_count-1
