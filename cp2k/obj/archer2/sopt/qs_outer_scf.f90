# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_outer_scf.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_outer_scf.F"
!-----------------------------------------------------------------------------!
...
# 32 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_outer_scf.F" 2
...
! *****************************************************************************
  SUBROUTINE outer_loop_gradient(qs_env, scf_env)
    ...
    IF(.NOT.(scf_control%outer_scf%have_scf))CALL cp__a("qs_outer_scf.F",101)

    ihistory=scf_env%outer_scf%iter_count
    IF(.NOT.(ihistory<=SIZE(scf_env%outer_scf%energy,1)))CALL cp__a("qs_outer_scf.F",104)
    ...
    CASE (outer_scf_ddapc_constraint)
       IF(.NOT.(dft_control%qs_control%ddapc_restraint))CALL cp__a("qs_outer_scf.F",114)
       ...
       END DO
       IF(.NOT.(is_constraint))CALL cp__a("qs_outer_scf.F",121)
    ...
    CASE (outer_scf_s2_constraint)
       IF(.NOT.(dft_control%qs_control%s2_restraint))CALL cp__a("qs_outer_scf.F",127)
       ...
       IF(.NOT.(is_constraint))CALL cp__a("qs_outer_scf.F",130)
    ...
    CASE (outer_scf_becke_constraint)
       IF(.NOT.(dft_control%qs_control%becke_restraint))CALL cp__a("qs_outer_scf.F",136)
...
! *****************************************************************************
  SUBROUTINE outer_loop_optimize(scf_env, scf_control)
    ...
    CASE(outer_scf_optimizer_bisect) ! bisection on the gradient, needs to be 1D
       IF(.NOT.(SIZE(scf_env%outer_scf%gradient(:,1))==1))CALL cp__a("qs_outer_scf.F",183)
    ...
    CASE(outer_scf_optimizer_diis)
       IF(.NOT.(scf_control%outer_scf%diis_buffer_length>0))CALL cp__a("qs_outer_scf.F",223)
       ! set up DIIS matrix
...
! *****************************************************************************
  SUBROUTINE outer_loop_extrapolate(qs_env)
    ...
    IF(.NOT.(nhistory>0))CALL cp__a("qs_outer_scf.F",363)
