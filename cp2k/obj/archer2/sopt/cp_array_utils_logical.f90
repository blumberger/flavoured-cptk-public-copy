# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_logical.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_logical.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_logical.F" 2
...
SUBROUTINE cp_1d_logical_write(array, unit_nr, el_format)
  ...
  IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_logical.F",127)
...
SUBROUTINE cp_2d_logical_write(array, unit_nr, el_format)
  ...
  DO i=1,SIZE(array,1)
     ...
     ELSE
        ...
        WRITE(unit=unit_nr,fmt=fmtstr,iostat=iostat) array(i,:)
     END IF
     IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_logical.F",188)
  END DO
  CALL m_flush(unit_nr)
...
! *****************************************************************************
  SUBROUTINE cp_1d_logical_guarantee_size(array, n)
    ...
    IF(.NOT.(n>=0))CALL cp__a("common/cp_array_utils_logical.F",217)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array) /= n) THEN
          CALL cp__w("common/cp_array_utils_logical.F",220,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_2d_logical_guarantee_size(array, n_rows, n_cols)
    ...
    IF(.NOT.(n_cols>=0))CALL cp__a("common/cp_array_utils_logical.F",253)
    IF(.NOT.(n_rows>=0))CALL cp__a("common/cp_array_utils_logical.F",254)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array,1) /= n_rows .OR. SIZE(array,2) /= n_cols) THEN
          CALL cp__w("common/cp_array_utils_logical.F",257,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
    IF (.NOT.ASSOCIATED(array)) THEN
...
FUNCTION cp_1d_logical_bsearch(array, el, l_index, u_index)&
  ...
  DO WHILE (lindex <= uindex)
     ...
     IF (( (.not.array(aindex)).and.el )) THEN
        lindex=aindex+1
     ELSE
        uindex=aindex-1
     END IF
