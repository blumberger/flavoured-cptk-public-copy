# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_result_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_result_methods.F"
!-----------------------------------------------------------------------------!
...
# 27 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_result_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE put_result_r1(results,description,values)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",70)
    IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",71)
    check  = SIZE(results%result_label)==SIZE(results%result_value)
    IF(.NOT.(check))CALL cp__a("common/cp_result_methods.F",73)
...
! *****************************************************************************
  SUBROUTINE put_result_r2(results,description,values)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",107)
    IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",108)
    check  = SIZE(results%result_label)==SIZE(results%result_value)
    IF(.NOT.(check))CALL cp__a("common/cp_result_methods.F",110)
    isize=SIZE(results%result_label)
...
! *****************************************************************************
  FUNCTION test_for_result(results,description) RESULT(res_exist)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",142)
...
! *****************************************************************************
  SUBROUTINE get_result_r1(results,description,values,nval,n_rep,n_entries)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",180)
    nlist=SIZE(results%result_value)
    IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",182)
    IF(.NOT.(SIZE(results%result_label)==nlist))CALL cp__a("common/cp_result_methods.F",183)
    nrep = 0
    DO i = 1, nlist
    ...
    IF(nrep.LE.0)&
       CALL cp_abort(cp__l("common/cp_result_methods.F",194),&
    ...
    DO i=1,nlist
       IF(TRIM(results%result_label(i))==TRIM(description))THEN
          IF(results%result_value(i)%value%type_in_use/=result_type_real)&
             CALL cp__b("common/cp_result_methods.F",200,"Attempt to retrieve a RESULT which is not a REAL!")
    ...
    IF(PRESENT(nval))THEN
       IF(.NOT.(size_res==size_values))CALL cp__a("common/cp_result_methods.F",209)
    ELSE
       IF(.NOT.(nrep*size_res==size_values))CALL cp__a("common/cp_result_methods.F",211)
    END IF
    k=0
    DO i = 1,nlist
       IF(TRIM(results%result_label(i))==TRIM(description))THEN
...
! *****************************************************************************
  SUBROUTINE get_result_r2(results,description,values,nval,n_rep,n_entries)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",256)
    nlist=SIZE(results%result_value)
    IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",258)
    IF(.NOT.(SIZE(results%result_label)==nlist))CALL cp__a("common/cp_result_methods.F",259)
    nrep = 0
    DO i = 1, nlist
       IF(TRIM(results%result_label(i))==TRIM(description)) nrep = nrep + 1
    END DO
    ...
    IF(nrep.LE.0)&
       CALL cp_abort(cp__l("common/cp_result_methods.F",270),&
            " Trying to access result ("//TRIM(description)//") which was never stored!")

    DO i=1,nlist
       IF(TRIM(results%result_label(i))==TRIM(description))THEN
          IF(results%result_value(i)%value%type_in_use/=result_type_real)&
             CALL cp__b("common/cp_result_methods.F",276,"Attempt to retrieve a RESULT which is not a REAL!")
    ...
    IF(PRESENT(nval))THEN
       IF(.NOT.(size_res==size_values))CALL cp__a("common/cp_result_methods.F",285)
    ELSE
       IF(.NOT.(nrep*size_res==size_values))CALL cp__a("common/cp_result_methods.F",287)
    END IF
    k=0
    DO i = 1,nlist
       IF(TRIM(results%result_label(i))==TRIM(description))THEN
...
! *****************************************************************************
  SUBROUTINE get_nreps(results,description,n_rep,n_entries,type_in_use)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",329)
    nlist=SIZE(results%result_value)
    IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",331)
    IF(.NOT.(SIZE(results%result_label)==nlist))CALL cp__a("common/cp_result_methods.F",332)
    IF(PRESENT(n_rep))THEN
       n_rep=0
       DO i=1,nlist
       ...
       DO i=1,nlist
          IF(TRIM(results%result_label(i))==TRIM(description))THEN
             ...
             CASE DEFAULT
                ...
                CALL cp__b("common/cp_result_methods.F",352,"")
             END SELECT
             EXIT
          END IF
       END DO
...
! *****************************************************************************
  SUBROUTINE cp_results_erase(results,description,nval)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",392)
    new_size = 0
    IF(PRESENT(description))THEN
       IF(.NOT.(description(1:1)=='['))CALL cp__a("common/cp_result_methods.F",395)
       nlist=SIZE(results%result_value)
       nrep=0
       DO i=1,nlist
          IF(TRIM(results%result_label(i))==TRIM(description)) nrep = nrep + 1
       ...
       IF(nrep.NE.0)THEN
          ...
          DO i=1,nlist
             ...
             END IF
          END DO
          IF(.NOT.(nlist-entry_deleted>=0))CALL cp__a("common/cp_result_methods.F",417)
...
! *****************************************************************************
  SUBROUTINE cp_results_mp_bcast(results, source, para_env)
    ...
    IF(.NOT.(ASSOCIATED(results)))CALL cp__a("common/cp_result_methods.F",465)
    nlist = 0
    ...
    DO i = 1, nlist
       ...
       CASE DEFAULT
          CALL cp__b("common/cp_result_methods.F",503,"Type not implemented in cp_result_type")
       END SELECT
    END DO
