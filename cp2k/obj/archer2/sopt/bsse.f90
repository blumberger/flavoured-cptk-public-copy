# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/bsse.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/bsse.F"
!-----------------------------------------------------------------------------!
...
# 52 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/bsse.F" 2
...
! *****************************************************************************
  SUBROUTINE eval_bsse_energy_low(force_env, conf, conf_loc, n_frags, &
    ...
    INTEGER :: i, ir, isize, j, k, method_name_id, my_targ, n_rep, &
      num_of_frag, old_size, present_charge, present_multpl
    ...
    CALL section_vals_get(n_frags,n_repetition=num_of_frag)
    IF(.NOT.(SIZE(conf)==num_of_frag))CALL cp__a("bsse.F",260)
    ...
    DO i = 1, num_of_frag
       IF (conf(i) == 1) THEN
          ...
          IF(.NOT.(conf(i)/=0))CALL cp__a("bsse.F",292)
      END IF
    END DO
    ...
    IF (method_name_id == do_qs) THEN
       ...
       CALL qs_init(qs_env, para_env, globenv, root_section, cp_subsys=subsys_loc,&
            force_env_section=force_env_section, subsys_section=subsys_section,&
    ...
    IF (explicit) THEN
       DO i=1, nconf
          ...
          IF(SIZE(glb_conf)/=SIZE(conf))&
             CALL cp_abort(cp__l("bsse.F",447),&
          ...
          IF(SIZE(sub_conf)/=SIZE(conf))&
             CALL cp_abort(cp__l("bsse.F",452),&
