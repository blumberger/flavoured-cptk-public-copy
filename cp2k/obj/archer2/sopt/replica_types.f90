# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_types.F"
!-----------------------------------------------------------------------------!
...
# 41 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/replica_types.F" 2
...
! *****************************************************************************
  SUBROUTINE rep_env_release(rep_env)
    ...
    IF (ASSOCIATED(rep_env)) THEN
       IF(.NOT.(rep_env%ref_count>0))CALL cp__a("replica_types.F",146)
       ...
       IF (rep_env%ref_count==0) THEN
          ...
          IF (rep_env%f_env_id>0) THEN
             ...
             IF(.NOT.(ierr==0))CALL cp__a("replica_types.F",152)
          END IF
...
! *****************************************************************************
  SUBROUTINE rep_env_destroy_low(rep_env_id, ierr)
    ...
    IF(.NOT.ASSOCIATED(rep_env))&
       CALL cp__b("replica_types.F",213,"could not find rep_env with id_nr"//cp_to_string(rep_env_id))
    ...
    IF(.NOT.(ierr==0))CALL cp__a("replica_types.F",219)
...
! *****************************************************************************
  SUBROUTINE rep_env_sync(rep_env,vals)
    ...
    CALL timeset(routineN,handle)
    IF(.NOT.(ASSOCIATED(rep_env)))CALL cp__a("replica_types.F",242)
    IF(.NOT.(rep_env%ref_count>0))CALL cp__a("replica_types.F",243)
    IF(.NOT.(SIZE(vals,2)==rep_env%nrep))CALL cp__a("replica_types.F",244)
...
! *****************************************************************************
  SUBROUTINE rep_env_sync_results(rep_env,results)
    ...
    CALL timeset(routineN,handle)
    nrep=rep_env%nrep
    IF(.NOT.(ASSOCIATED(rep_env)))CALL cp__a("replica_types.F",273)
    IF(.NOT.(rep_env%ref_count>0))CALL cp__a("replica_types.F",274)
    IF(.NOT.(SIZE(results)==rep_env%nrep))CALL cp__a("replica_types.F",275)
    DO irep=1, nrep
!         source = 0
...
! *****************************************************************************
  SUBROUTINE rep_envs_rm_rep_env(rep_env)
    ...
    IF (ASSOCIATED(rep_env)) THEN
       IF(.NOT.(module_initialized))CALL cp__a("replica_types.F",375)
       ALLOCATE(new_rep_envs(SIZE(rep_envs)-1))
       ii=0
       DO i=1,SIZE(rep_envs)
          ...
          END IF
       END DO
       IF(.NOT.(ii==SIZE(new_rep_envs)))CALL cp__a("replica_types.F",384)
       DEALLOCATE(rep_envs)
       rep_envs => new_rep_envs
