# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_optimization.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_optimization.F"
!-----------------------------------------------------------------------------!
...
# 14 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_optimization.F" 2
...
!> \param emat ...
! *****************************************************************************
  SUBROUTINE atom_history_update (history,fmat,emat)
    TYPE(atom_history_type), INTENT(INOUT)   :: history
...
!> \param err ...
! *****************************************************************************
  SUBROUTINE atom_opt (fmat,history,err)
    REAL(dp), DIMENSION(:, :, :), &
      INTENT(INOUT)                          :: fmat
    TYPE(atom_history_type), INTENT(INOUT)   :: history
    ...
    INTEGER                                  :: i, info, j, lwork, na, nb, &
                                                nlen, nm, nmax, nnow, rank
    REAL(dp)                                 :: a, rcond, t
    ...
    IF ( history%hlen > 1 ) THEN
      IF ( err < history%eps_diis ) THEN
        ...
        IF(.NOT.(info==0))CALL cp__a("atom_optimization.F",192)
        fmat = 0._dp
        DO i=1,nlen
          na = nnow + 1 - i
          IF ( na < 1 ) na = nmax + na
    ...
    ELSE
      CALL cp__b("atom_optimization.F",210,"")
    END IF
