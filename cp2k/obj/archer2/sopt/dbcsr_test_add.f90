# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_add.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_add.F"
!-----------------------------------------------------------------------------!
...
MODULE dbcsr_test_add
  ...
  USE dbcsr_methods,                   ONLY: &
       dbcsr_col_block_sizes, dbcsr_get_data_type, dbcsr_get_matrix_type, &
  ...
  USE dbcsr_types,                     ONLY: &
       dbcsr_data_obj, dbcsr_distribution_obj, dbcsr_mp_obj, dbcsr_obj, &
       dbcsr_scalar_type, dbcsr_type_antihermitian, dbcsr_type_antisymmetric, &
...
# 57 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/dbcsr/dbcsr_test_add.F" 2
...
! *****************************************************************************
  SUBROUTINE dbcsr_test_adds(test_name, mp_group, mp_env, npdims, io_unit, &
    ...
    INTEGER                                  :: a_c, a_r, b_c, b_r, handle, &
                                                isymm, itype, mynode, &
                                                numnodes, TYPE
    INTEGER, DIMENSION(:), POINTER           :: my_sizes_m, my_sizes_n, &
                                                sizes_m, sizes_n
    LOGICAL                                  :: do_complex
...
!   ---------------------------------------------------------------------------
    ...
    IF (ALL (limits==0)) THEN
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, " limits shouldnt be 0 ",331)
    ELSE
...
! *****************************************************************************
  SUBROUTINE dbcsr_check_add(test_name, matrix_a, dense_a_dbcsr, dense_a, dense_b,&
    ...
    CALL dbcsr_assert (valid, dbcsr_fatal_level, dbcsr_internal_error, &
         routineN, "dense matrix not valid",433)
    !
    !
    ...
    CASE (dbcsr_type_real_8_2d)
       ...
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocattion problem",450)
       eps          = dlamch('eps')
    ...
    CASE (dbcsr_type_real_4_2d)
       ...
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocattion problem",473)
       eps          = slamch('eps')
    ...
    CASE (dbcsr_type_complex_8_2d)
       ALLOCATE(work(lwork), STAT=istat)
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocattion problem",496)
       eps          = dlamch('eps')
    ...
    CASE (dbcsr_type_complex_4_2d)
       ALLOCATE(work_sp(lwork), STAT=istat)
       CALL dbcsr_assert (istat==0, dbcsr_fatal_level, dbcsr_internal_error, &
            routineN, "allocattion problem",519)
    ...
    CASE default
       CALL dbcsr_assert (.FALSE., dbcsr_fatal_level, dbcsr_caller_error,&
            routineN, "Incorrect or 1-D data type", 574)
    END SELECT
