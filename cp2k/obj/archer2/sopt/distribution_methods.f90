# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/distribution_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/distribution_methods.F"
!-----------------------------------------------------------------------------!
...
# 74 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/distribution_methods.F" 2
...
! integer, allocatable :: all_local_molecules(:)
    ...
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 171, &
                                     "workload", npe*int_size)
    workload_count(:) = 0

    ALLOCATE (workload_fill(npe), STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 176, &
    ...
    IF(heap_error)&
       CALL cp__b("distribution_methods.F",204,"Error initially filling the heap.")

    CALL heap_new (bin_heap_fill, nbins)
    CALL heap_fill (bin_heap_fill, &
         (/(bin, bin=1, nbins)/), workload_fill , heap_error)
    IF(heap_error)&
       CALL cp__b("distribution_methods.F",210,"Error initially filling the heap.")
...
!     *** Get the number of local molecules of the current molecule kind ***
      ...
      DO imolecule=1, nmolecule
         ...
         ELSE
            ...
            IF(heap_error)&
               CALL cp__b("distribution_methods.F",246,"Error getting topmost heap element.")
            IF(.NOT.found)&
               CALL cp__b("distribution_methods.F",248,"No topmost heap element found.")

            ipe = bin
            IF(bin_price/=workload_count(ipe))&
               CALL cp__b("distribution_methods.F",252,"inconsistent heap")
            ...
            IF(heap_error)&
               CALL cp__w("distribution_methods.F",262,"Error setting price of top heap element.")
         END IF
      END DO
...
!     *** Distribute the molecules ***
      ...
      IF (n > 0) THEN
        ...
        IF (istat /= 0) THEN
           CALL stop_memory(routineN, moduleN, 273, &
      ...
      DO imolecule=1, nmolecule
         ...
         ELSE
            ...
            IF(heap_error)&
               CALL cp__b("distribution_methods.F",294,"Error getting topmost heap element.")
            IF(.NOT.found)&
               CALL cp__b("distribution_methods.F",296,"No topmost heap element found.")

            ipe = bin
            IF(bin_price/=workload_fill(ipe))&
               CALL cp__b("distribution_methods.F",300,"inconsistent heap")
         ...
         IF (.NOT. has_prev_subsys_info) THEN
            ...
            IF(heap_error)&
               CALL cp__w("distribution_methods.F",321,"Error setting price of top heap element.")
         END IF
      END DO
    ...
    IF(ANY(workload_fill.NE.workload_count))&
       CALL cp__b("distribution_methods.F",328,"Inconsistent heaps encountered")
...
!     *** Print molecule distribution ***
      ...
      IF (istat /= 0) CALL stop_memory(routineN, moduleN, 388, &
                                       "work", npe*int_size)
      work(:) = 0
...
! *****************************************************************************
  SUBROUTINE distribute_molecules_2d(cell, atomic_kind_set, particle_set, &
    ...
    LOGICAL :: basic_cluster_optimization, basic_optimization, &
      basic_spatial_optimization, molecular_distribution, skip_optimization
...
!...
    ...
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 601, &
                                     "row_distribution", natom*int_size)

    ALLOCATE (col_distribution(natom,2), STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 605, &
    ...
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 612, &
                                     "local_particle_col", &
                                     nparticle_kind*int_size)

    ALLOCATE (local_particle_row(nparticle_kind), STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 617, &
                                     "local_particle_col", &
                                     nparticle_kind*int_size)

    ALLOCATE (nparticle_local_row(nparticle_kind), STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 622, &
                                     "nparticle_local_row", &
                                     nparticle_kind*int_size)

    ALLOCATE (nparticle_local_col(nparticle_kind), STAT=istat)
    IF (istat /= 0) CALL stop_memory(routineN, moduleN, 627, &
                                     "nparticle_local_col", &
                                     nparticle_kind*int_size)
       ...
       IF (.NOT.molecular_distribution) THEN
          DO iatom = 1, natom
             IF(iatom .GT. nclusters)&
                CALL cp__b("distribution_methods.F",659,"Bounds error")
       ...
       ELSE
          IF(basic_cluster_optimization)THEN
             IF(molecular_distribution)&
               CALL cp__b("distribution_methods.F",715,"clustering and molecular blocking NYI")
    ...
    ELSE
       ...
       CALL cp__b("distribution_methods.F",786,"")
    ENDIF
    ...
    DO iparticle_kind=1, nparticle_kind
      ...
      IF (istat /= 0) CALL stop_memory(routineN, moduleN, 805, &
      ...
      IF (istat /= 0) CALL stop_memory(routineN, moduleN, 810, &
...
!     *** Print row distribution ***
      ...
      IF (istat /= 0) THEN
        CALL stop_memory(routineN, moduleN, 855, &
                         "work", nprow*int_size)
      END IF
      work(:) = 0
...
!     *** Print column distribution ***
      ...
      IF (istat /= 0) THEN
        CALL stop_memory(routineN, moduleN, 881, &
                         "work", npcol*int_size)
      END IF
      work(:) = 0
...
!   ---------------------------------------------------------------------------
    ...
    IF(heap_error)&
       CALL cp__b("distribution_methods.F",965,"Error initially filling the heap.")
    !
    ...
    DO cluster_index = nclusters, 1, -1
       ...
       IF(heap_error)&
          CALL cp__b("distribution_methods.F",974,"Error getting topmost heap element.")
       IF(.NOT.found)&
          CALL cp__b("distribution_methods.F",976,"No topmost heap element found.")
       !
       prow = INT (bin*pgrid_gcd / npcols)
       IF(prow .GE. nprows)&
          CALL cp__b("distribution_methods.F",980,"Invalid process row.")
       pcol = INT (bin*pgrid_gcd / nprows)
       IF(pcol .GE. npcols)&
          CALL cp__b("distribution_methods.F",983,"Invalid process column.")
       ...
       IF(heap_error)&
          CALL cp__w("distribution_methods.F",991,"Error setting price of top heap element.")
    ENDDO
...
! *****************************************************************************
  SUBROUTINE make_cluster_distribution(coords, scaled_coords, cell, costs, &
    ...
    INTEGER, ALLOCATABLE, DIMENSION(:) :: atom_to_cluster, cluster_cost, &
      cluster_count, cluster_to_col, cluster_to_row, piv_cost, proc_cost, &
      sorted_cost
    REAL(KIND=dp)                            :: fold(3)
    REAL(KIND=dp), ALLOCATABLE, &
      DIMENSION(:, :)                        :: cluster_center, cluster_high, &
                                                cluster_low
    ...
    ENDDO
    cluster_low=HUGE(0.0_dp)
    cluster_high=-HUGE(0.0_dp)
    DO i=1,natom
    ...
    IF (output_unit>0) THEN
       ...
       WRITE(output_unit,'(T2,A,T48,F8.3,I8)') "Largest cartesian extend [a.u.]/cluster x=", &
                   MAXVAL(cluster_high(1,:)-cluster_low(1,:)),MAXLOC(cluster_high(1,:)-cluster_low(1,:))
       WRITE(output_unit,'(T2,A,T48,F8.3,I8)') "Largest cartesian extend [a.u.]/cluster y=", &
                   MAXVAL(cluster_high(2,:)-cluster_low(2,:)),MAXLOC(cluster_high(2,:)-cluster_low(2,:))
       WRITE(output_unit,'(T2,A,T48,F8.3,I8)') "Largest cartesian extend [a.u.]/cluster z=", &
                   MAXVAL(cluster_high(3,:)-cluster_low(3,:)),MAXLOC(cluster_high(3,:)-cluster_low(3,:))
    ENDIF
