# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_current_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_current_utils.F"
!-----------------------------------------------------------------------------!
...
# 113 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_linres_current_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE current_response(current_env,p_env,qs_env)
    ...
    INTEGER :: first_center, handle, i, icenter, idir, ii, iii, ispin, &
      ist_true, istate, j, jcenter, jstate, max_nbr_center, max_states, nao, &
      natom, nbr_center(2), ncubes, nmo, nspins, nstates(2), output_unit
    INTEGER, ALLOCATABLE, DIMENSION(:)       :: first_sgf, last_sgf
    INTEGER, DIMENSION(:), POINTER           :: list_cubes, row_blk_sizes
    INTEGER, DIMENSION(:, :, :), POINTER     :: statetrueindex
    LOGICAL                                  :: append_cube, ionode, &
                                                should_stop
    ...
    IF(current_env%full) THEN
       ...
       !
       !
       CALL cp_dbcsr_set(op_p_ao(1)%matrix,0.0_dp)
       DO idir = 2,3
          ...
          CALL cp_dbcsr_set(op_p_ao(idir)%matrix,0.0_dp)
       ENDDO
       !
    ...
    DO idir = 1,3
       ...
       CALL linres_solver(p_env,qs_env,psi1_ptr,hpsi0_ptr,psi0_order,should_stop)
       !
       !
...
! *****************************************************************************
  SUBROUTINE current_env_init(current_env,qs_env)
    ...
    INTEGER :: handle, homo, i, iao, iatom, ibox, icenter, icount, idir, ii, &
      ini, ir, is, ispin, istate, istate2, istate_next, ix, iy, iz, j, &
    ...
    LOGICAL                                  :: gapw, is0, uniform_occupation
    ...
    DO ispin = 1,nspins
       ...
       !
       ! check that homo=nmo
       IF (nmo.NE.homo) CALL cp__b("qs_linres_current_utils.F",684,"nmo != homo")
       !
...
!       IF (nmoloc.NE.nmo) CALL cp_abort(cp__l("qs_linres_current_utils.F",688),&
!                                            "The nbr of localized functions "//&
!            &                               "is not equal to the nbr of states.")
       !
       ! check that the occupation is uniform
       IF (.NOT.uniform_occupation) CALL cp__b("qs_linres_current_utils.F",693,"nmo != homo")
    ENDDO
    !
    !
    logger => cp_get_default_logger()
    ...
    IF(current_env%ref_count /= 0) CALL current_env_cleanup(current_env,qs_env)
    ...
    CASE DEFAULT
       CALL cp__b("qs_linres_current_utils.F",731,"Unknown gauge, try again...")
    END SELECT
    !
    ...
    CASE DEFAULT
       CALL cp__b("qs_linres_current_utils.F",773,"Unknown orbital center, try again...")
    END SELECT
    !
    ...
    !   CALL cp__b("qs_linres_current_utils.F",778,"restart_nmr requires density_guess=restart")
    !ENDIF
    !
    ! check that the psi0 are localized and you have all the centers
    IF(.NOT.(linres_control%localized_psi0))CALL cp__a("qs_linres_current_utils.F",782)
    IF(output_unit>0) THEN
       WRITE(output_unit,'(A)') &
    ...
    IF(linres_control%preconditioner_type.EQ.ot_precond_full_all.AND.current_env%do_selected_states) &
         CALL cp__b("qs_linres_current_utils.F",832,"Selected states doesnt work with the preconditioner FULL_ALL")
    !
    !
    ...
    DO ispin = 1,nspins
       ...
       CASE DEFAULT
          CALL cp__b("qs_linres_current_utils.F",1034,"Unknown orbital center, try again...")
       END SELECT
    ENDDO
    !
    !
    ...
    DO idir = 1,3
       ...
       CALL qs_rho_retain(current_env%jrho1_set(idir)%rho)
    ENDDO
    !
    ...
    ALLOCATE(current_env%rs_gauge(3))
...
!> \param qs_env ...
! *****************************************************************************
  SUBROUTINE current_env_cleanup(current_env,qs_env)
    ...
    IF(current_env%ref_count == 0 ) THEN
       ...
       DO idir=1,3
          IF(ASSOCIATED(current_env%rs_gauge(idir)%rs)) THEN
             DO i=1,SIZE(current_env%rs_gauge(idir)%rs)
               CALL rs_grid_release(current_env%rs_gauge(idir)%rs(i)%rs_grid)
             END DO
             DEALLOCATE(current_env%rs_gauge(idir)%rs)
          ENDIF
       ENDDO
       !
       ...
       IF(ASSOCIATED(current_env%rs_buf)) THEN
          DO i=1,SIZE(current_env%rs_buf)
            CALL rs_grid_release(current_env%rs_buf(i)%rs_grid)
          END DO
