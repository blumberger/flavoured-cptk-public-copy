# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mixed_environment_types.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mixed_environment_types.F"
!-----------------------------------------------------------------------------!
...
!> CHANGE_AC
!> 15-12-11
!> Add a new object into the mixed_environment_type
!> Require for Non Adiabatic Molecular Dynamics
...
# 58 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/mixed_environment_types.F" 2
...
!     REAL(KIND=dp), DIMENSION(:,:), POINTER             :: energies_1_mol_list
!CHANGE SAM
...
! *****************************************************************************
  SUBROUTINE get_mixed_env( mixed_env, atomic_kind_set, particle_set,  &
    ...
    IF(.NOT.(ASSOCIATED(mixed_env%subsys)))CALL cp__a("mixed_environment_types.F",182)
...
! *****************************************************************************
  SUBROUTINE init_mixed_env ( mixed_env, para_env)
    ...
    NULLIFY (mixed_env%F)
!    NULLIFY (mixed_env%energies_1_mol_list)
...
! *****************************************************************************
  SUBROUTINE set_mixed_env( mixed_env, atomic_kind_set, particle_set, &
    ...
    IF(.NOT.(mixed_env%ref_count>0))CALL cp__a("mixed_environment_types.F",284)
...
! *****************************************************************************
  SUBROUTINE mixed_env_retain(mixed_env)
    ...
    IF(.NOT.(ASSOCIATED(mixed_env)))CALL cp__a("mixed_environment_types.F",372)
    IF(.NOT.(mixed_env%ref_count>0))CALL cp__a("mixed_environment_types.F",373)
...
! *****************************************************************************
  SUBROUTINE mixed_env_release(mixed_env)
    ...
    IF (ASSOCIATED(mixed_env)) THEN
       IF(.NOT.(mixed_env%ref_count>0))CALL cp__a("mixed_environment_types.F",391)
       mixed_env%ref_count=mixed_env%ref_count-1
       IF (mixed_env%ref_count<1) THEN
          ...
          !IF(ASSOCIATED(mixed_env%energies_1_mol_list)) THEN
          !     DEALLOCATE(mixed_env%energies_1_mol_list)
          !END IF
