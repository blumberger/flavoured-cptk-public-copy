# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_control.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_control.F"
!-----------------------------------------------------------------------------!
...
# 30 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/xas_control.F" 2
  ...
  SUBROUTINE write_xas_control(xas_control,dft_section)
    ...
    IF (output_unit>0) THEN
      SELECT CASE(xas_control%xas_method)
        ...
        CASE default
          CALL cp__b("xas_control.F",228,"unknown xas method "//TRIM(ADJUSTL(cp_to_string(xas_control%xas_method))))
      END SELECT
...
! *****************************************************************************
  SUBROUTINE xas_control_create(xas_control)
    ...
    IF(.NOT.(.NOT.ASSOCIATED(xas_control)))CALL cp__a("xas_control.F",254)
    ALLOCATE(xas_control)
...
! *****************************************************************************
  SUBROUTINE xas_control_release(xas_control)
    ...
    IF(ASSOCIATED(xas_control)) THEN
      IF(.NOT.(xas_control%ref_count>0))CALL cp__a("xas_control.F",285)
...
!> \param xas_control ...
! *****************************************************************************
  SUBROUTINE xas_control_retain(xas_control)
    ...
    IF(.NOT.(ASSOCIATED(xas_control)))CALL cp__a("xas_control.F",311)
    xas_control%ref_count=xas_control%ref_count+1
