# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_fit.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_fit.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_fit.F" 2
...
! *****************************************************************************
  SUBROUTINE atom_fit_density (atom,num_gto,norder,iunit,powell_section,results)
    ...
    IF(PRESENT(results)) THEN
      IF(.NOT.(SIZE(results)>=num_gto+2))CALL cp__a("atom_fit.F",150)
      results(1) = x(1)
      results(2) = x(2)
...
! *****************************************************************************
  SUBROUTINE atom_fit_basis (atom_info,basis,pptype,iunit,powell_section)
    ...
    INTEGER                                  :: i, j, k, l, ll, m, n, n10, &
                                                nl, nr
    INTEGER, ALLOCATABLE, DIMENSION(:, :)    :: xtob
    LOGICAL                                  :: explicit, mult
      ...
      IF ( ostate%state == 2 ) THEN
         SELECT CASE (basis%basis_type)
            ...
            CASE (STO_BASIS)
              ...
              END DO
         END SELECT
         CALL basis_fit (atom_info,basis,pptype,ostate%f,0)
...
! weights for the optimization
    ...
    CALL pseudo_fit(atom_info,ppot,ostate%f,pval)
    DO i=1,SIZE(atom_info,1)
      DO j=1,SIZE(atom_info,2)
        atom => atom_info(i,j)%atom
        IF(atom_consistent_method(atom%method_type,atom%state%multiplicity)) THEN
...
! *****************************************************************************
  SUBROUTINE density_fit (density,atom,n,aval,cval,co,aerr)
    ...
    INTEGER                                  :: i, info, ip, ipiv(1000), iq, &
                                                k, nr
    REAL(dp)                                 :: a, rk, zval
    ...
    IF(.NOT.(info==0))CALL cp__a("atom_fit.F",896)
    co(1:n) = tval(1:n,1)
...
!> \param iw ...
! *****************************************************************************
  SUBROUTINE basis_fit (atom_info,basis,pptype,afun,iw)
    TYPE(atom_p_type), DIMENSION(:, :), &
      POINTER                                :: atom_info
    TYPE(atom_basis_type), POINTER           :: basis
    LOGICAL, INTENT(IN)                      :: pptype
    ...
    INTEGER                                  :: do_eric, do_erie, im, in, nm, &
                                                nn, reltyp, zval
    LOGICAL                                  :: eri_c, eri_e
...
!> \param pval ...
! *****************************************************************************
  SUBROUTINE pseudo_fit (atom_info,ppot,afun,pval)
    TYPE(atom_p_type), DIMENSION(:, :), &
      POINTER                                :: atom_info
    TYPE(atom_potential_type), POINTER       :: ppot
    REAL(dp), INTENT(OUT)                    :: afun
    ...
    DO i=1,SIZE(atom_info,1)
      DO j=1,SIZE(atom_info,2)
        ...
        IF(atom_consistent_method(atom%method_type,atom%state%multiplicity)) THEN
          ...
          CALL calculate_atom(atom,0,noguess=.TRUE.)
...
! *****************************************************************************
  FUNCTION rcpro(id,xval) RESULT(yval)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'rcpro', &
      routineP = moduleN//':'//routineN
    ...
    ELSE IF ( id==-1 ) THEN
      x1 = SQRT(xval)
      IF(.NOT.(x1<=1._dp))CALL cp__a("atom_fit.F",1290)
      x2 = 0.5_dp * LOG((1._dp+x1)/(1._dp-x1))
      yval = x2/0.1_dp
    ELSE
      CALL cp__b("atom_fit.F",1294,"wrong id")
    END IF
  END FUNCTION rcpro
...
! at least one parameter to be optimized
    ...
    IF(.NOT.(num_pol*num_gau > 0))CALL cp__a("atom_fit.F",1327)
   ...
   CALL putvar(x,co,num_pol,num_gau)
    ...
    IF(PRESENT(results)) THEN
      IF(.NOT.(SIZE(results)>=SIZE(x)))CALL cp__a("atom_fit.F",1415)
      results = x
    END IF
