# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_inpp_methods.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_inpp_methods.F"
!-----------------------------------------------------------------------------!
...
# 28 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/input/cp_parser_inpp_methods.F" 2
...
! *****************************************************************************
  SUBROUTINE inpp_process_directive(inpp, input_line, input_file_name, input_line_number,&
    ...
    INTEGER                                  :: i, indf, indi, istat, &
                                                output_unit, pos1, pos2, unit
    LOGICAL                                  :: check

    output_unit = cp_logger_get_default_io_unit()

    IF(.NOT.(ASSOCIATED(inpp)))CALL cp__a("input/cp_parser_inpp_methods.F",73)
    ...
    CASE ("@INCLUDE")
       ...
       IF(LEN_TRIM(filename)==0) THEN
          ...
          CALL cp__b("input/cp_parser_inpp_methods.F",101,TRIM(message))
       ENDIF
       indi=1
          ...
          ELSE
             ...
             IF((pos1/=0).OR.(pos2/=0)) THEN
                WRITE(UNIT=message,FMT="(3A,I6)") &
                     ...
                     TRIM(input_file_name), "  Line:",input_line_number
                CALL cp__b("input/cp_parser_inpp_methods.F",126,TRIM(message))
             ENDIF
       ...
       DO i = 1, inpp%io_stack_level
          ...
          IF(.NOT.(check))CALL cp__a("input/cp_parser_inpp_methods.F",135)
       END DO
    ...
    CASE ("@SET")
       ...
       IF(LEN_TRIM(varname)==0) THEN
          WRITE(UNIT=message,FMT="(3A,I6)") &
               ...
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",173,TRIM(message))
       ENDIF
       ...
       IF(LEN_TRIM(value)==0) THEN
          WRITE(UNIT=message,FMT="(3A,I6)") &
               "INPP_@SET: Incorrect @SET directive in file: ",&
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",196,TRIM(message))
       ENDIF
    ...
    CASE ("@IF")
       ...
       IF(pos1>0) THEN
          ...
          IF((pos2>0).OR.(INDEX(cond2,"==")>0)) THEN
             WRITE(UNIT=message,FMT="(3A,I6)") &
                  ...
                  TRIM(input_file_name), "  Line:",input_line_number
             CALL cp__b("input/cp_parser_inpp_methods.F",250,TRIM(message))
          ENDIF
       ELSEIF(pos2>0) THEN
          ...
          IF((pos1>0).OR.(INDEX(cond2,"/=")>0)) THEN
             WRITE(UNIT=message,FMT="(3A,I6)") &
                  "INPP_@IF: Incorrect @IF directive in file: ",&
                  TRIM(input_file_name), "  Line:",input_line_number
             CALL cp__b("input/cp_parser_inpp_methods.F",260,TRIM(message))
          ENDIF
       ELSE
       ...
       IF(LEN_TRIM(cond2)==0) THEN
          WRITE(UNIT=message,FMT="(3A,I6)") &
               "INPP_@IF: Incorrect @IF directive in file: ",&
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",302,TRIM(message))
       ENDIF
       ...
       ELSE
          ...
          DO WHILE(istat==0)
             ...
             DO WHILE (input_line(indf:indf)/=" ")
                indf = indf + 1
             END DO
             IF(.NOT.((indf-indi)<=default_string_length))CALL cp__a("input/cp_parser_inpp_methods.F",347)
             mytag =input_line(indi:indf-1)
             CALL uppercase(mytag)
          ...
          IF(istat/=0) THEN
             WRITE(UNIT=message,FMT="(3A,I6)") &
                  ...
                  TRIM(input_file_name), "  Line:",input_line_number
             CALL cp__b("input/cp_parser_inpp_methods.F",362,TRIM(message))
          ENDIF
       ENDIF
...
! *****************************************************************************
  SUBROUTINE inpp_end_include(inpp, input_file_name, input_line_number, input_unit)
    ...
    IF(.NOT.(ASSOCIATED(inpp)))CALL cp__a("input/cp_parser_inpp_methods.F",407)
...
! *****************************************************************************
  SUBROUTINE inpp_expand_variables(inpp, input_line, input_file_name, input_line_number)
    ...
    IF(.NOT.(ASSOCIATED(inpp)))CALL cp__a("input/cp_parser_inpp_methods.F",444)
    ...
    DO WHILE (INDEX(input_line,'${') > 0)
       ...
       IF(.NOT.(pos2>0))CALL cp__a("input/cp_parser_inpp_methods.F",451)
       IF(pos2==0) THEN
          WRITE(UNIT=message,FMT="(3A,I6)") &
               "Missing '}' in file: ",&
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",456,TRIM(message))
       ENDIF

       pos2=pos1+pos2-2
       varname=input_line(pos1:pos2)
       idx=inpp_find_variable(inpp,varname)
       IF(.NOT.(idx>=0))CALL cp__a("input/cp_parser_inpp_methods.F",462)
       IF(idx==0) THEN
          WRITE(UNIT=message,FMT="(5A,I6)") &
               "Variable ${",TRIM(varname),"} not defined in file: ",&
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",467,TRIM(message))
       ENDIF
    ...
    DO WHILE (INDEX(input_line,'$') > 0)
       ...
       pos1=pos1+1
       pos2=INDEX(input_line(pos1:),' ')
       IF(.NOT.(pos2>=0))CALL cp__a("input/cp_parser_inpp_methods.F",485)
       IF(pos2==0) pos2 = LEN_TRIM(input_line(pos1:))+1

       pos2=pos1+pos2-1
       varname=input_line(pos1:pos2)
       idx=inpp_find_variable(inpp,varname)
       IF(.NOT.(idx>=0))CALL cp__a("input/cp_parser_inpp_methods.F",491)
       IF(idx==0) THEN
          WRITE(UNIT=message,FMT="(5A,I6)") &
               "INPP_@SET: Variable $",TRIM(varname)," not defined in file: ",&
               TRIM(input_file_name), "  Line:",input_line_number
          CALL cp__b("input/cp_parser_inpp_methods.F",496,TRIM(message))
       ENDIF
