# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_initialization.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_initialization.F"
!-----------------------------------------------------------------------------!
...
MODULE qs_scf_initialization
  ...
  USE cp_dbcsr_interface,              ONLY: &
       ...
       cp_dbcsr_init_p, cp_dbcsr_p_type, cp_dbcsr_row_block_sizes, &
       cp_dbcsr_type, dbcsr_type_no_symmetry, dbcsr_type_real_default
  ...
  USE input_constants,                 ONLY: &
       ...
       cholesky_off, diag_block_davidson, diag_block_krylov, &
       diag_filter_matrix, diag_ot, diag_standard, direct_p_mix, kerker_mix, &
       multisec_mix, no_mix, plus_u_lowdin, pulay_mix, wfi_frozen_method_nr, &
  ...
  USE qs_scf_types,                    ONLY: &
       ...
       diag_subspace_env_create, filter_matrix_diag_method_nr, &
       general_diag_method_nr, krylov_space_create, ot_diag_method_nr, &
       ot_method_nr, qs_scf_env_type, scf_env_create, special_diag_method_nr
...
# 114 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/qs_scf_initialization.F" 2
...
! *****************************************************************************
  SUBROUTINE qs_scf_ensure_mos(qs_env,scf_env)
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_initialization.F",461)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_initialization.F",462)
...
!   *** finish initialization of the MOs ***
    IF(.NOT.(ASSOCIATED(mos)))CALL cp__a("qs_scf_initialization.F",476)
    DO ispin=1,SIZE(mos)
...
!   *** finish initialization of the MOs for ADMM and derivs if needed ***
    IF(dft_control%do_admm) THEN
      IF(.NOT.(ASSOCIATED(mos_aux_fit)))CALL cp__a("qs_scf_initialization.F",516)
    ...
    IF(kpoints%nkp /= 0) THEN
       ...
       IF(qs_env%requires_mo_derivs)&
          CALL cp__b("qs_scf_initialization.F",547,"No MO derivative methods allowed with kpoints")
       IF(dft_control%do_admm)&
          CALL cp__b("qs_scf_initialization.F",549,"No ADMM implemented with kpoints")
       IF(dft_control%do_xas_calculation)&
          CALL cp__b("qs_scf_initialization.F",551,"No XAS implemented with kpoints")
       DO ik=1,SIZE(kpoints%kp_env)
          ...
          IF(.NOT.(ASSOCIATED(mos_k)))CALL cp__a("qs_scf_initialization.F",556)
          DO ispin=1,SIZE(mos_k,2)
             DO ic=1,SIZE(mos_k,1)
                ...
                IF(.NOT.(.NOT.ASSOCIATED(mo_coeff_b)))CALL cp__a("qs_scf_initialization.F",566)
             END DO
          END DO
       END DO
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_scf_ensure_mixing(scf_control,scf_section,scf_env,dft_control)
    ...
    CASE DEFAULT
      CALL cp__b("qs_scf_initialization.F",606,"Unknown mixing method")
    END SELECT
    ...
    IF (scf_control%use_diag .AND. scf_env%mixing_method==no_mixing_nr) THEN
       CALL cp__b("qs_scf_initialization.F",618,"Diagonalization procedures without mixing are not recommendable")
    END IF
    ...
    IF(scf_env%mixing_method>direct_mixing_nr) THEN
      ...
      IF(scf_env%mixing_store%beta==0.0_dp) THEN
          CALL cp__b("qs_scf_initialization.F",625,"Mixing employing the Kerker damping factor needs BETA /= 0.0")
      END IF
    END IF
    ...
    IF (scf_env%mixing_method==direct_mixing_nr) THEN
      ...
      IF(scf_control%eps_diis<scf_control%eps_scf) THEN
        scf_env%skip_diis = .TRUE.
        CALL cp__w("qs_scf_initialization.F",633,"the DIIS scheme is disabled, since EPS_DIIS < EPS_SCF")
      END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE qs_scf_ensure_diagonalization(scf_env,scf_section,qs_env,&
    ...
    LOGICAL                                  :: do_kpoints, need_coeff_b, &
                                                not_se_or_dftb
...
! sanity check whether combinations are allowed
      IF(dft_control%restricted)&
         CALL cp__b("qs_scf_initialization.F",677,"OT only for restricted (ROKS)")
      SELECT CASE (scf_control%diagonalization%method)
      CASE(diag_ot,diag_block_krylov,diag_block_davidson)
         IF(.NOT.not_se_or_dftb)&
            CALL cp__b("qs_scf_initialization.F",681,"DFTB and SE not possible with OT diagonalization")
      END SELECT
      SELECT CASE (scf_control%diagonalization%method)
      ...
      CASE (diag_ot)
        IF(dft_control%roks)&
           CALL cp__b("qs_scf_initialization.F",694,"ROKS with OT diagonalization not possible")
         IF(do_kpoints)&
            CALL cp__b("qs_scf_initialization.F",696,"OT diagonalization not possible with kpoint calculations")
      ...
      CASE (diag_block_krylov)
         IF(dft_control%roks)&
            CALL cp__b("qs_scf_initialization.F",703,"ROKS with block PF diagonalization not possible")
         IF(do_kpoints)&
            CALL cp__b("qs_scf_initialization.F",705,"Block Krylov diagonalization not possible with kpoint calculations")
      ...
      CASE (diag_block_davidson)
         IF(do_kpoints)&
            CALL cp__b("qs_scf_initialization.F",714,"Block Davidson diagonalization not possible with kpoint calculations")
      ...
      CASE DEFAULT
         CALL cp__b("qs_scf_initialization.F",736,"Unknown diagonalization method")
      END SELECT
      ...
      IF(scf_control%do_diag_sub) THEN
        ...
        IF(do_kpoints)&
           CALL cp__b("qs_scf_initialization.F",746,"No subspace diagonlization with kpoint calculation")
      END IF
    ...
    ELSEIF (scf_control%use_ot) THEN
      ...
      IF(scf_control%added_mos(1)+scf_control%added_mos(2) > 0) &
         CALL cp__b("qs_scf_initialization.F",753,"OT with ADDED_MOS/=0 not implemented")
      IF(dft_control%restricted .AND. dft_control%nspins.NE.2)&
         CALL cp__b("qs_scf_initialization.F",755,"nspin must be 2 for restricted (ROKS)")
      IF(do_kpoints)&
         CALL cp__b("qs_scf_initialization.F",757,"OT not possible with kpoint calculations")
    ELSE
      CALL cp__b("qs_scf_initialization.F",759,"OT or DIAGONALIZATION have to be set")
    END IF
    DO ispin=1,dft_control%nspins
...
! *****************************************************************************
 SUBROUTINE qs_scf_compute_properties(qs_env,dft_section,wf_type)
    ...
    ELSEIF (dft_control%qs_control%do_kg) THEN
       CALL cp__w("qs_scf_initialization.F",794,"No properties from PRINT section available for KG methods")
    ELSEIF (dft_control%qs_control%ofgpw) THEN
       CALL cp__w("qs_scf_initialization.F",796,"No properties from PRINT section available for OFGPW methods")
...
! *****************************************************************************
  SUBROUTINE init_scf_run(scf_env, qs_env, scf_section,scf_control)
    ...
    INTEGER                                  :: after, handle, ikind, iw, &
                                                nao, ndep, output_unit
    LOGICAL :: dft_plus_u_atom, do_kpoints, init_u_ramping_each_scf, &
      omit_headers, s_minus_half_available
...
!
    ...
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_initialization.F",855)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_initialization.F",856)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_initialization.F",857)
    IF(.NOT.(qs_env%ref_count>0))CALL cp__a("qs_scf_initialization.F",858)
    NULLIFY(para_env)
...
! *****************************************************************************
  SUBROUTINE scf_env_initial_rho_setup(scf_env, qs_env, scf_section, scf_control)
    ...
    logger => cp_get_default_logger()
    IF(.NOT.(ASSOCIATED(scf_env)))CALL cp__a("qs_scf_initialization.F",1031)
    IF(.NOT.(scf_env%ref_count>0))CALL cp__a("qs_scf_initialization.F",1032)
    IF(.NOT.(ASSOCIATED(qs_env)))CALL cp__a("qs_scf_initialization.F",1033)
