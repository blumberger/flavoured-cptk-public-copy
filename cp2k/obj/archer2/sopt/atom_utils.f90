# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_utils.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_utils.F"
!-----------------------------------------------------------------------------!
...
# 44 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/atom_utils.F" 2
...
! *****************************************************************************
  SUBROUTINE atom_set_occupation(ostring,occupation,wfnocc,multiplicity)
    ...
    IF(.NOT.(ASSOCIATED(ostring)))CALL cp__a("atom_utils.F",103)
    IF(.NOT.(SIZE(ostring) > 0))CALL cp__a("atom_utils.F",104)
    ...
    IF ( is <= no ) THEN
      IF ( INDEX(ostring(is),"(") /= 0 ) THEN
        i1 = INDEX(ostring(is),"(")
        i2 = INDEX(ostring(is),")")
        IF(.NOT.((i2-i1-1 > 0).AND.(i2-i1-1 < 3)))CALL cp__a("atom_utils.F",115)
    ...
    IF ( is <= no ) THEN
      IF ( INDEX(ostring(is),"[") /= 0 ) THEN
        ...
        i1 = INDEX(ostring(is),"[")
        i2 = INDEX(ostring(is),"]")
        IF(.NOT.((i2-i1-1 > 0).AND.(i2-i1-1 < 3)))CALL cp__a("atom_utils.F",139)
        elem = ostring(is)(i1+1:i2-1)
        ielem=0
        DO k=1,nelem
          ...
          END IF
        END DO
        IF(.NOT.(ielem /= 0))CALL cp__a("atom_utils.F",148)
          DO l=0,3
    ...
    DO i=is,no
      ...
      jd = INDEX(pstring,"D")
      jf = INDEX(pstring,"F")
      IF(.NOT.(js+jp+jd+jf > 0))CALL cp__a("atom_utils.F",170)
      IF ( js > 0 ) THEN
        IF(.NOT.(jp+jd+jf == 0))CALL cp__a("atom_utils.F",172)
        READ(pstring(1:js-1),*) n
        READ(pstring(js+1:),*) oo
        IF(.NOT.(n > 0))CALL cp__a("atom_utils.F",175)
        IF(.NOT.(oo >= 0._dp))CALL cp__a("atom_utils.F",176)
        IF(.NOT.(occupation(0,n)==0))CALL cp__a("atom_utils.F",177)
        occupation(0,n) = oo
      END IF
      IF ( jp > 0 ) THEN
        IF(.NOT.(js+jd+jf == 0))CALL cp__a("atom_utils.F",181)
        READ(pstring(1:jp-1),*) n
        READ(pstring(jp+1:),*) oo
        IF(.NOT.(n > 1))CALL cp__a("atom_utils.F",184)
        IF(.NOT.(oo >= 0._dp))CALL cp__a("atom_utils.F",185)
        IF(.NOT.(occupation(1,n-1)==0))CALL cp__a("atom_utils.F",186)
        occupation(1,n-1) = oo
      END IF
      IF ( jd > 0 ) THEN
        IF(.NOT.(js+jp+jf == 0))CALL cp__a("atom_utils.F",190)
        READ(pstring(1:jd-1),*) n
        READ(pstring(jd+1:),*) oo
        IF(.NOT.(n > 2))CALL cp__a("atom_utils.F",193)
        IF(.NOT.(oo >= 0._dp))CALL cp__a("atom_utils.F",194)
        IF(.NOT.(occupation(2,n-2)==0))CALL cp__a("atom_utils.F",195)
        occupation(2,n-2) = oo
      END IF
      IF ( jf > 0 ) THEN
        IF(.NOT.(js+jp+jd == 0))CALL cp__a("atom_utils.F",199)
        READ(pstring(1:jf-1),*) n
        READ(pstring(jf+1:),*) oo
        IF(.NOT.(n > 3))CALL cp__a("atom_utils.F",202)
        IF(.NOT.(oo >= 0._dp))CALL cp__a("atom_utils.F",203)
        IF(.NOT.(occupation(3,n-3)==0))CALL cp__a("atom_utils.F",204)
        occupation(3,n-3) = oo
      END IF
    ...
    IF (mult /= -1) THEN
      ...
      IF (js==0) THEN
        IF(.NOT.(mult==1))CALL cp__a("atom_utils.F",239)
      END IF
      IF (js==1) THEN
        l=i1
        i=i2
        ...
        IF(.NOT.(MOD(k+1-mult,2)==0))CALL cp__a("atom_utils.F",248)
      END IF
      IF (js>1 .AND. mult/=-2) THEN
        IF(.NOT.(mult==-2))CALL cp__a("atom_utils.F",251)
      END IF
...
! *****************************************************************************
  SUBROUTINE atom_density(density,pmat,basis,maxl,typ,rr)
    ...
    IF(my_typ == "KIN") THEN
      IF(.NOT.(PRESENT(rr)))CALL cp__a("atom_utils.F",365)
    END IF
      ...
      DO i=1,n
        ...
        DO j=1,gthpot%nct_nlcc(i)
          ...
          ELSE IF(my_typ == "DER") THEN
            ...
            END IF
          ELSE IF(my_typ == "LAP") THEN
            fb = 2._dp*gthpot%cval_nlcc(j,i)/a
            corden(:) = corden(:) - fb*fe(:)/rr(:)*rc**(2*j-1)
            corden(:) = corden(:) + fe(:)*rc**(2*j)*gthpot%cval_nlcc(j,i)/a2
            IF(j>1) THEN
              corden(:) = corden(:) + fb*REAL(2*j-2,dp)*fe(:)/rr(:)*rc**(2*j-3)
              corden(:) = corden(:) + REAL((2*j-2)*(2*j-3),dp)*fe(:)*rc**(2*j-4)*gthpot%cval_nlcc(j,i)/a2
              corden(:) = corden(:) - REAL(2*j-2,dp)*fe(:)*rc**(2*j-2)*gthpot%cval_nlcc(j,i)/a2
            END IF
          ELSE
            CALL cp__b("atom_utils.F",735,"")
          END IF
        END DO
      END DO
      DEALLOCATE(fe,rc)
...
! *****************************************************************************
  SUBROUTINE atom_solve(hmat,umat,orb,ener,nb,nv,maxl)
    ...
    IF(.NOT.(ALL(nb >= nv)))CALL cp__a("atom_utils.F",918)
...
! *****************************************************************************
  SUBROUTINE coulomb_potential_numeric ( cpot, density, grid)
    ...
    IF(.NOT.(r(1)>r(nc)))CALL cp__a("atom_utils.F",1051)
    DO i = 1, nc
...
! *****************************************************************************
  SUBROUTINE potential_numeric ( cpot, density, nu, grid)
    ...
    ! test that grid is decreasing
    IF(.NOT.(r(1)>r(nc)))CALL cp__a("atom_utils.F",1364)
    DO i = 1, nc
      ...
      CASE ( GTO_BASIS )
        DO i = 1, basis%nbas(la)
          DO j = 1, basis%nbas(lb)
            ...
            SELECT CASE (ll)
              CASE DEFAULT
                CALL cp__b("atom_utils.F",1418,"")
              CASE (0)
                IF(.NOT.(nu==0))CALL cp__a("atom_utils.F",1420)
              CASE (1)
                IF(.NOT.(nu==1))CALL cp__a("atom_utils.F",1422)
                cpot(:,i,j) = cpot(:,i,j)-6._dp*expa(:)/z(:)
              CASE (2)
                SELECT CASE (nu)
                  CASE DEFAULT
                    CALL cp__b("atom_utils.F",1427,"")
                  CASE (0)
      ...
      CASE ( CGTO_BASIS )
        DO i = 1, basis%nprim(la)
          DO j = 1, basis%nprim(lb)
            ...
            SELECT CASE (ll)
              CASE DEFAULT
                CALL cp__b("atom_utils.F",1497,"")
              CASE (0)
                IF(.NOT.(nu==0))CALL cp__a("atom_utils.F",1499)
              CASE (1)
                IF(.NOT.(nu==1))CALL cp__a("atom_utils.F",1501)
                erfa(:) = erfa(:)-6._dp*expa(:)/z(:)
              CASE (2)
                SELECT CASE (nu)
                  CASE DEFAULT
                    CALL cp__b("atom_utils.F",1506,"")
                  CASE (0)
