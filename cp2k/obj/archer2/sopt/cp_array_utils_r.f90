# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_r.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_r.F"
!-----------------------------------------------------------------------------!
...
# 47 "/work/e05/e05/fivanovic/flavoured-cptk-X-SH-coulomb-barrier/cp2k/src/common/cp_array_utils_r.F" 2

  IMPLICIT NONE
  PRIVATE

  LOGICAL, PRIVATE, PARAMETER :: debug_this_module=.TRUE.
  CHARACTER(len=*), PARAMETER, PRIVATE :: moduleN = 'cp_array_utils_r'
...
! *****************************************************************************
SUBROUTINE cp_1d_r_write(array, unit_nr, el_format)
  ...
  IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_r.F",127)
...
! *****************************************************************************
SUBROUTINE cp_2d_r_write(array, unit_nr, el_format)
    REAL(kind=dp), INTENT(in)                :: array(:,:)
    INTEGER, INTENT(in)                      :: unit_nr
    CHARACTER(len=*), INTENT(in), OPTIONAL   :: el_format
  ...
  DO i=1,SIZE(array,1)
     ...
     ELSE
        ...
        WRITE(unit=unit_nr,fmt=fmtstr,iostat=iostat) array(i,:)
     END IF
     IF(.NOT.(iostat==0))CALL cp__a("common/cp_array_utils_r.F",188)
  END DO
  CALL m_flush(unit_nr)
...
! *****************************************************************************
  SUBROUTINE cp_1d_r_guarantee_size(array, n)
    ...
    IF(.NOT.(n>=0))CALL cp__a("common/cp_array_utils_r.F",217)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array) /= n) THEN
          CALL cp__w("common/cp_array_utils_r.F",220,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_2d_r_guarantee_size(array, n_rows, n_cols)
    ...
    IF(.NOT.(n_cols>=0))CALL cp__a("common/cp_array_utils_r.F",253)
    IF(.NOT.(n_rows>=0))CALL cp__a("common/cp_array_utils_r.F",254)
    IF (ASSOCIATED(array)) THEN
       IF (SIZE(array,1) /= n_rows .OR. SIZE(array,2) /= n_cols) THEN
          CALL cp__w("common/cp_array_utils_r.F",257,'size has changed')
          DEALLOCATE(array)
       END IF
    END IF
    IF (.NOT.ASSOCIATED(array)) THEN
...
! *****************************************************************************
FUNCTION cp_1d_r_bsearch(array, el, l_index, u_index)&
     RESULT(res)
    REAL(kind=dp), INTENT(in)                :: array(:), el
