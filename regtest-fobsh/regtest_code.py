...
2 modes are possible:   
   ...
   **** Second called: NtoN ****************** 
   Can run at command line from an application directory, e.g.:
     $ python regtest_code.py NtoN OUTDIR1_GLOBAL OUTDIR2_GLOBAL
   to compare the data within the subdirectories of in OUTDIR1_GLOBAL
     to the regression data in subdirectories of OUTDIR2_GLOBAL.
...
def cmp_dir(outdir, regdir, files):
    ...
    if (len(mismatch)>0):
        ...
        for file in mismatch:
            ...
            #tol = 1.e-8  # What I consider numberical precision
...
if __name__=="__main__":
    ...
    if sys.argv[1] == "1to1":    
        ...
        # take all files from mother directory
        #files = os.listdir(regression_dir)
    
        # take just selected files
        #files = ["run-coeff-1.xyz", "run-pseudo-hamilt-1.xyz", "run-sh-1.log", "run.inp", "run-pos-1.xyz", "run-pop-1.xyz" ]
        files = ["run-coeff-1.xyz", "run-pseudo-hamilt-1.xyz", "run-sh-1.log", "run.inp", "run-pos-1.xyz", ]
        # start the comparison
        cmp_dir(outdir, regression_dir, files)
    ...
    # This allows the comparison of many subdirectories inside the mother directory
    elif sys.argv[1] == "NtoN":
        try:
           outdir_global = sys.argv[2]
           regression_dir_global  = sys.argv[3] 
        except:
            raise Exception("*** Must specify output directory, and optionally regression directory")
    
        # take all the sub dir in regression_dir_global
        res = [i for i in os.listdir(regression_dir_global) if "run-fssh-" in i] 
        for sub_dir in res:
           regression_dir = os.path.join(regression_dir_global, sub_dir)
           outdir = os.path.join(outdir_global, sub_dir)
        ...
        raise Exception("*** Must specify 1to1 or NtoN mode and Must specify output directory, and optionally regression directory")
